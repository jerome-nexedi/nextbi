Jedox.wss.dialog.format.font = function(callback, _pre, toDisable, _noPadding) {
	var format = {
		fontFamily : "",
		fontStyle : "",
		fontSize : "",
	...
	var fontTypeDataChart = [ [ "Arial", "Arial" ],
			...
			[ "Arial Narrow", "Arial Narrow" ], [ "Calibri", "Calibri" ],
			[ "Comic Sans MS", "Comic Sans MS" ], [ "Georgia", "Georgia" ],
	...
	var fontStyleList = [ [ "regular", "Regular" ], [ "bold", "Bold" ],
	...
	var fontSizeStore = new Ext.data.SimpleStore( {
		fields : [ "font_size" ],
		data : fontSizeList
	});
	...
	var fontSizeDV = new Ext.DataView(
			{
				...
				listeners : {
					click : {
						fn : function(dView, index, node, ev) {
							fontSize = "font-size:".concat(fontSizeList[index],
									"pt; ");
							refreshPreview();
							fontSizeField.setRawValue(fontSizeList[index]);
							format.fontSize = "".concat(fontSizeField
									.getRawValue(), "pt ")
						},
						scope : this
					}
	...
	var btnColorFont = new Ext.Toolbar.SplitButton(
			{
				minWidth : 100,
				menu : new Ext.menu.ColorMenu(
						{
							...
							listeners : {
								select : function(colorP, colorStr) {
									...
									_setupColorButton(btnColorFont,
											format.color, false);
