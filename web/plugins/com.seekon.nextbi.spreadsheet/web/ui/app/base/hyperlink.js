Jedox.wss.hl = new function() {
	var that = this;
	this.hlTag = {
		begin : '<span class="hl" onmouseover="Jedox.wss.hl.toolTip(event, true);" onmouseout="Jedox.wss.hl.toolTip(event, false);">',
		end : "</span>",
		...
		if (data.link.type == "ref") {
			...
			case "local":
				...
				case "workbook":
					...
					loc = '"'.concat(sheetExists || rangeExists ? "[" : "",
							data.link.value.target.path, sheetExists
									|| rangeExists ? "]" : "",
							sheetExists ? (data.link.value.target.sheet
									.indexOf(" ") >= 0 ? "'".concat(
									data.link.value.target.sheet, "'")
									: data.link.value.target.sheet).concat("!")
									: "",
	...
	function _convertSDataLink(link) {
		var re_inSquareBracket = /^\[[\w\W]*\]/, wbType = link
				.match(re_inSquareBracket), path = wbType == null ? link
				: wbType[0].substring(1, wbType[0].length - 1), isSelf = !path
				.search(/^[\w\W]*.wss$/)
				|| path.toUpperCase() == "SELF", isLocal = wbType != null
				|| !path.search(/^\/[\w]*/) || isSelf, locTarget = !isLocal ? path
				: {
					path : path,
					ghnt : isSelf ? null : Jedox.wss.backend.ha
		...
		if (isLocal && (isSelf || loc.value.target.ghnt.t == "workbook")) {
			...
			if (selStr.length) {
				...
				if (selArr.length > 1) {
					loc.value.target.sheet = !selArr[0].search(/^'[^']*'$/) ? selArr[0]
							.substring(1, selArr[0].length - 1)
							: selArr[0];
	...
	function _dDataToSData(attr, incTrans) {
		...
		function resolveRef(refStr) {
			if (!refStr.indexOf("@")) {
				...
				ref = ref[0];
				return activeBook.getCellValue(ref.rng[0], ref.rng[1])
			}
		}
		...
		var re_inSquareBracket = /^\[[\w\W]*\]/, loc = attr.link[0] == attr.link[1] ? _convertSDataLink(getPairVal(attr.link))
				: {
					type : "ref",
					value : attr.link[0]
				};
		...
		var trans = [];
		if (incTrans === true) {
			for ( var i = 0, trns = attr.trans, trnsLen = trns.length, inSqareBrack, trnsPair = {}, formula = Jedox.wss.formula; i < trnsLen; i++, trnsPair = {}) {
	...
	function _resolveFormula(frm) {
		var activeBook = Jedox.wss.app.activeBook, conn = activeBook._conn, actSheet = activeBook
		...
		if (res[0][0]) {
			conn.cmd(0, [ "ndel" ], [ res[0][1][0].uuid ])
		}
		return res[0][0] ? [ res[0][0], res[0][1][0].uuid, res[0][1][0].value ]
				: res[0]
	}
	function _resolveSData(data, incTrans) {
		...
		if (data.tip.type == "ref") {
			resTarget.push( [ data.tip, "rvalue" ]), resData
					.push(data.tip.value)
		}
		if (incTrans === true) {
			for ( var i = 0, trns = data.trans, trnsLen = trns.length, resTypes = "range,nrange,var", parseDst = data.link.type == "obj"
				...
				if (parseDst && resTypes.search(trns[i].dst.type) >= 0) {
		...
		if (resData.length) {
			var rslVal = _resolveFormula("=RESOLVE(".concat(resData
					.join(Jedox.wss.i18n.separators[2]), ")"));
			if (!rslVal[0]) {
				throw "resolveError"
			}
			for ( var j = 0, rT = resTarget, rVals = Ext.util.JSON
					.decode(rslVal[2]), rTLen = resT.length; j < rTLen; j++) {
				rT[j][0][rT[j][1]] = rVals[j]
			}
		}
		if (data.link.type == "ref") {
			data.link.rvalue = _convertSDataLink(data.link.rvalue)["value"]
		}
		return rslVal ? rslVal[1] : null
	}
	this.set = function(range, hdata) {
		var activeBook = Jedox.wss.app.activeBook, general = Jedox.wss.general, conn = activeBook._conn, cell = activeBook._aPane
		...
		conn.createBatch();
		if (rslVal[0]) {
			conn.cmd(null, [ "ndel" ], [ rslVal[1] ])
		}
		conn.cmd(null, [ "cdrn", {
			cm : true
		} ], [ range ]);
		conn.sendBatch( [ activeBook, activeBook.exec ], activeBook
				.getRealGridRange(), false, false, conn.Q_VALUE | conn.Q_STYLE
				| conn.Q_ATTRS);
		Jedox.wss.app.environment.selectedCellValue = this.hlTag.begin.concat(
				txt, this.hlTag.end)
	};
	this.get = function(range) {
		var cell = Jedox.wss.app.activePane.getCell(range[0], range[1]);
		if (typeof cell == "object" && ("a" in cell) && ("mousedown" in cell.a)
				&& cell.a.mousedown[0] == "hl") {
	...
	this.remove = function(range) {
		var activeBook = Jedox.wss.app.activeBook, general = Jedox.wss.general, conn = activeBook._conn, cell = activeBook._aPane
				.getCell(range[0], range[1]);
		if (!cell) {
			return
		}
		var txt = Jedox.wss.general.filterHLTags(range[0], range[1], activeBook
				.getCellValue(range[0], range[1]), false), rngUpd = {
			s : Jedox.wss.style.delHyperlinkStyle
		};
		if (cell.t == "h") {
			rngUpd.v = txt
		}
		...
		conn.sendBatch( [ activeBook, activeBook.exec ], activeBook
				.getRealGridRange(), false, false, conn.Q_VALUE
				| conn.Q_FORMULA | conn.Q_STYLE | conn.Q_ATTRS)
	};
	this.updateText = function(range, txt) {
		if (Jedox.wss.app.activeBook.getCellType(range[0], range[1]) == "h") {
			...
			Jedox.wss.backend.conn.cmd(0, [ "cdrn", {
				cm : true
			} ], [ range ])
		}
	};
	this.followURL = function(data, target) {
		if (!data.link.value.target.search(/^mailto:*/i)) {
			window.location = data.link.value.target;
			return false
		}
		function getSDVals(val) {
			var resTypes = "range,nrange,var,clist", res = val[resTypes
					.search(val.type) >= 0 ? "rvalue" : "value"];
			return Ext.isArray(res) ? res : [ res ]
		}
		var transData = {};
		for ( var i = 0, trns = data.trans, trnsLen = trns.length, dest; i < trnsLen; i++) {
			if (!trns[i].src || !trns[i].dst
					|| !(dest = getSDVals(trns[i].dst)).length) {
				continue
			}
			for ( var j = 0, k = 0, tData = transData, src = getSDVals(trns[i].src), dst = dest, srcLen = src.length; j < srcLen; j++, k++) {
				tData[src[j]] = !dst[k] ? dst[k = 0] : dst[k]
			}
		}
		transData = Ext.urlEncode(transData);
		var url = (data.link.value.target.search(/:\/\//) < 0 ? "http://"
				.concat(data.link.value.target) : data.link.value.target)
				.replace(/\/$/, "").concat(
						transData.length ? "?".concat(transData) : "");
			...
			try {
				for ( var triggers = Jedox.wss.events.triggers.openURL, i = triggers.length - 1, hlURL = url; i >= 0; i--) {
					triggers[i][0]["openURL"].call(parent, triggers[i][1],
							hlURL, target,
							data.text[data.text.type == "string" ? "value"
									: "rvalue"], true)
				}
			} catch (e) {
				Jedox.wss.general.showMsg("Application Error".localize(), e
						.localize(), Ext.MessageBox.WARNING)
			}
		}
		return false
	};
	this.followOther = function(link, target) {
		if (Jedox.wss.app.standalone) {
			Jedox.wss.general.showMsg("follHLInvalidRef".localize(),
					"follHLNotSuppInStandalone".localize(),
	...
	this.followWb = function(data) {
		var activeBook = Jedox.wss.app.activeBook, showMsg = Jedox.wss.general.showMsg, linkVal = data.link.type == "ref" ? data.link.rvalue
		...
		var transExec = function(params, cb) {
			...
			if (!params[linkVal.target.range]) {
				...
				var paramsNR = !params[linkVal.target.range].search(/^=/) ? params[linkVal.target.range]
						.substr(1)
						: params[linkVal.target.range], rngP = Jedox.wss.formula
						.parse(paramsNR);
			...
			var sheetPool = {}, namedRangePool = [], sheets = {}, sheetList = Jedox.wss.backend.ha
					.getSheets(), actSheetId = sheetList[1], actSheetName;
			...
			var resolveRange = function(range) {
				...
				var srcVal = src.type == "cval" || src.type == "clist" ? src.value
						: src.rvalue;
				srcVal = srcVal == null ? "" : srcVal;
				srcVal = Ext.isArray(srcVal) ? srcVal : [ srcVal ];
				var srcObjs = [], res = [];
				for ( var i = 0, sArr = srcVal, srcLen = sArr.length; i < srcLen; i++) {
					srcObjs.push(plVal ? (sArr[i] == null ? "" : sArr[i]) : {
						v : sArr[i] == null ? "" : sArr[i]
					})
				}
			...
			for ( var i = 0, trns = data.trans, trnsLen = trns.length, shts = sheets, splitRng = splitRange, sPool = sheetPool, nPool = namedRangePool, genSData = genSrcData, actSId = actSheetId, parsedDstRng, dataArr, rngElems; i < trnsLen; i++) {
				...
				case "var":
					dataArr = [
							!trns[i].dst.value.search(/^@/) ? trns[i].dst.value
									.substr(1) : trns[i].dst.value,
			...
			if (namedRangePool.length) {
				...
				for ( var i = 0, nrPool = namedRangePool, nrPoolLen = nrPool.length; i < nrPoolLen; i++) {
					conn.cmd(0, [ "nget" ], [ [ 1, 1, nrPool[i].name,
							nrPool[i].uuid == undefined ? "" : nrPool[i].uuid,
							true ] ]);
					conn.cmd(0, [ "nget" ],
							[ [ 1, 1, nrPool[i].name, "", true ] ])
				}
				for ( var i = 0, nrRes = conn.sendBatch(), nrResLen = nrRes.length, nrPool = namedRangePool, refTo, refToElems; i < nrResLen; i += 2) {
			...
			if (execTrans) {
				...
				if (srcWsId == actSheetId) {
					conn.sendBatch( [ activeBook, activeBook.exec, cb ],
							activeBook.getRealGridRange(), true, true,
							conn.Q_VALUE | conn.Q_FORMULA | conn.Q_STYLE
									| conn.Q_ATTRS)
				} else {
					conn.sendBatch(cb)
				}
		...
		try {
			...
			if (ghnt == null) {
				...
				if (!path.search(/^[\w\W]*.wss$/)
						&& path != Jedox.wss.workspace
								.getMetaByWinId(Jedox.wss.app.activeBook
										.getWinId()).name) {
					throw "follHLInvalidWB"
				}
				var cbTransExec = function() {
					if (ext.nrange) {
						var conn = Jedox.wss.backend.conn, targSheetId = ext.sheet_name ? sheetSelector
								.getIdByName(ext.sheet_name)
								: sheetSelector.getActiveSheetId();
						if (!targSheetId) {
							throw "follHLInvalidSheet"
						}
						conn.createBatch();
						conn.cmd(0, [ "nget" ], [ [ 1, 1, ext.nrange,
								targSheetId, true ] ]);
						conn.cmd(0, [ "nget" ],
								[ [ 1, 1, ext.nrange, "", true ] ]);
						var ngetRes = conn.sendBatch();
						if (!ngetRes[0][0] && !ngetRes[1][0]) {
							throw "follHLInvTrgNRange"
						}
						var nrRef = ngetRes[ngetRes[0][0] ? 0 : 1][1][0].refers_to;
						if (!nrRef.search(/^=/)) {
							nrRef = nrRef.substr(1)
						}
						var nrRefSplit = (!nrRef.search(/^=/) ? nrRef.substr(1)
								: nrRef).split("!", 2);
						...
						if (nrRefSplit.length > 1
							...
							Jedox.wss.sheet.select( [ this, makeSelection ],
									sheetId, activeBook._wbId, true)
						} else {
							makeSelection()
						}
					} else {
						if (ext.sheet_name) {
							...
							if (sheetSelector.getActiveSheetId() != sheetId) {
								Jedox.wss.sheet.select(
										[ this, makeSelection ], sheetId,
										activeBook._wbId, true)
							} else {
								makeSelection()
							}
						} else {
				...
				transExec( {}, [ this, cbTransExec ])
			} else {
				if (Jedox.wss.app.UPRestrictMode) {
					var actBookMetaData = Jedox.wss.workspace
							.getMetaByWinId(activeBook._winId).ghn;
					if (actBookMetaData.n != ghnt.n
				...
				if (!Jedox.wss.book.load( [ this, makeSelection,
						[ this, afterWbOpen ] ], ghnt.n, ghnt.g, ghnt.h, ext, {
					tag : "hl",
					func : transExec,
					scope : this,
					params : []
	...
	this.follow = function(ev, data) {
		...
		var targEl = (Ext.isGecko2) ? ev.explicitOriginalTarget.parentNode
		...
		try {
			if (data.dyn) {
				data = _dDataToSData(data, true)
			} else {
				var tmpNRId = _resolveSData(data, true)
			}
		} catch (e) {
			Jedox.wss.general.showMsg("follHLInvalidRef".localize(), e
					.localize(), Ext.MessageBox.WARNING);
			return false
		}
		var linkVal = data.link.type == "ref" ? data.link.rvalue
				: data.link.value;
		if (linkVal.type == "url") {
			return Jedox.wss.hl.followURL(data, "tab")
		}
		if (linkVal.type == "local") {
			return linkVal.target.ghnt == null
					|| linkVal.target.ghnt.t == "workbook" ? Jedox.wss.hl
					.followWb(data) : Jedox.wss.hl.followOther(linkVal, "tab")
		}
		return false
	};
	this.rangeContextMenu = function(ev) {
		var env = Jedox.wss.app.environment, selStartCoord = env.lastRangeStartCoord, data = this
				.get( [ selStartCoord[0], selStartCoord[1] ]), dynData = data != undefined
				&& data.dyn, insEditCapt = data ? "Edit Hyperlink"
				: "Hyperlink", targEl = (Ext.isGecko2) ? ev.explicitOriginalTarget.parentNode
				: document.elementFromPoint(ev.clientX, ev.clientY), isTargHL = !(targEl == undefined
				|| targEl.tagName.toUpperCase() != "SPAN" || targEl.className != "hl"), cntx = [ "-" ];
		if (data && isTargHL) {
			if (dynData) {
				data = _dDataToSData(data)
			} else {
				var tmpNRId = _resolveSData(data)
			}
			var linkVal = data.link.type == "ref" ? data.link.rvalue
					: data.link.value;
			if (linkVal.type == "url") {
				if (!Jedox.wss.app.standalone) {
					cntx.push( {
						...
						handler : function() {
							Jedox.wss.hl.followURL(data, "tab")
						}
					})
				}
				cntx.push( {
					...
					handler : function() {
						Jedox.wss.hl.followURL(data, "win")
					}
				})
			}
			if (linkVal.type == "local") {
				if (linkVal.target.ghnt == null
					...
					cntx.push( {
						...
						iconCls : "ico_hl_open",
						handler : function() {
							Jedox.wss.hl.followWb(data)
						}
					})
				} else {
					if (!Jedox.wss.app.standalone) {
						if (Jedox.wss.app.fileTypesReg[linkVal.target.ghnt.t]
								|| linkVal.target.ghnt.t == "hyperlink") {
							cntx.push( {
								text : "HLCntxNewTab".localize(),
								iconCls : "ico_hl_target_tab",
								handler : function() {
									Jedox.wss.hl.followOther(linkVal, "tab")
								}
							});
							cntx.push( {
								text : "HLCntxNewWin".localize(),
								iconCls : "ico_hl_target_win",
								handler : function() {
									Jedox.wss.hl.followOther(linkVal, "win")
								}
							})
						}
						if ("hyperlink,ahview".search(linkVal.target.ghnt.t) < 0) {
							cntx.push( {
								text : "Export".localize().concat("..."),
								handler : function() {
									Jedox.wss.hl.followOther(linkVal, "export")
								}
							})
						}
					}
		...
		if (env.viewMode != Jedox.wss.grid.viewMode.USER) {
			if (!dynData) {
				cntx.push( {
					...
					handler : function() {
						Jedox.wss.app.load(Jedox.wss.app.dynJSRegistry.openHL)
					}
				})
			}
			if (data) {
				cntx.push( {
					...
					handler : function() {
						Jedox.wss.hl.remove( [ selStartCoord[0],
								selStartCoord[1], selStartCoord[0],
								selStartCoord[1] ])
					}
				})
			}
	...
	this.toolTip = function(ev, show) {
		...
		if (show) {
			var activeBook = Jedox.wss.app.activeBook, elParent = el.parentNode, rng = elParent.id
					.search(/_cursorField$/) >= 0 ? Jedox.wss.app.environment.selectedCellCoords
					: activeBook.getCoordsByCell(elParent), data = this
					.get(rng), tipTitle, tipText;
			if (!data) {
				return
			}
			try {
				...
				if (data.link[linkSrc].type == "local") {
					tipText = !data.link[linkSrc].target.path ? "Place in This Document"
							.localize()
							.concat(
									" - ",
									!data.link[linkSrc].target.sheet ? activeBook
											.getSheetSelector()
											.getActiveSheetName()
			...
			el.tooltip.showAt( [ ev.clientX, ev.clientY ])
		} else {
			if (el.tooltip) {
				el.tooltip.destroy()
			}
