 ...
 *
 * $Id: SparklineClass.php 1979 2009-08-03 11:36:40Z predragm $
 *
 */
...
class SparklineClass extends Object {
  ...
  // constructor
  //
  function SparklineClass($catch_errors = true) {
  ...
  function OutputToFile($file) {
	...
	{
		$tmpImg = imagecreatetruecolor(imagesy($this->imageHandle), imagesx($this->imageHandle));
		imagecolortransparent($tmpImg, imagecolorallocate($tmpImg, 0, 0, 0));
		//imagealphablending($tmpImg, false);
		imagesavealpha($tmpImg);

		for ($i = 0; $i <= imagesx($this->imageHandle); $i++)
		{
			for ($j = 0; $j <= imagesy($this->imageHandle); $j++)
			{
				$alphaColor = imagecolorat($this->imageHandle, $i, $j);
