  ...
  protected String getPropertyListXML(String indent) {
    ...
    if (getDataSourceInfo() != null && getDataSourceInfo().trim().length() > 0)
      sb.append("\n" + indent + elementIndent + "<DataSourceInfo>"
        + getDataSourceInfo().trim() + "</DataSourceInfo>");

    if (getCatalog() != null && getCatalog().trim().length() > 0)
      sb.append("\n" + indent + elementIndent + "<Catalog>" + getCatalog().trim()
        + "</Catalog>");

    if (getFormat() != null && getFormat().trim().length() > 0)
      sb.append("\n" + indent + elementIndent + "<Format>" + getFormat().trim()
        + "</Format>");

    if (getContent() != null && getContent().trim().length() > 0)
      sb.append("\n" + indent + elementIndent + "<Content>" + getContent().trim()
        + "</Content>");

    if (axisFormat != null && axisFormat.trim().length() > 0)
      sb.append("\n" + indent + elementIndent + "<AxisFormat>" + axisFormat.trim()
        + "</AxisFormat>");
