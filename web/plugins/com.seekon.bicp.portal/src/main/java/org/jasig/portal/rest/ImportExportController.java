...
/**
 * ImportExportController provides AJAX/REST targets for import/export
 * operations.
 * 
...
public class ImportExportController {
  ...
  public void importEntity(@RequestParam("file") MultipartFile entityFile,
    ...
    if (!ap.hasPermission("UP_SYSTEM", "IMPORT_ENTITY", portalDataKey.getName()
      .getLocalPart())) {
  ...
  protected PortalDataKey getPortalDataKey(
   ...
   * 
   * The path for this method is /entity/type/identifier. The identifier
   * generally a string that may be used as a unique identifier, but is
   * dependent on the entity type. For example, to delete the "demo" user one
   * might use the path /entity/user/demo.
   */
  ...
  public void exportEntity(@PathVariable("entityId") String entityId,
    ...
    final String fileName = portalDataHandlerService.exportData(entityType,
      entityId, new StreamResult(exportBuffer));
