...
/**
 * Runs the Hibernate Schema Export tool using the specified DataSource for the
 * target DB.
 * 
...
public class DataSourceSchemaExport implements ISchemaExport {
   ...
   * (non-Javadoc)
   * 
   * @see org.jasig.portal.tools.dbl.ISchemaExport#hbm2ddl(boolean, boolean,
   * boolean, java.lang.String, boolean)
   */
  @SuppressWarnings("deprecation")
  @Override
  public void hbm2ddl(boolean export, boolean create, boolean drop,
    String outputFile, boolean haltOnError) {
   ...
   * (non-Javadoc)
   * 
   * @see org.jasig.portal.tools.dbloader.ISchemaExport#create(boolean, boolean,
   * boolean, java.lang.String, boolean)
   */
  @Override
  public void create(final boolean export, final boolean create, final boolean drop,
    final String outputFile, final boolean haltOnError) {
    ...
    if (create) {
      ...
      if (haltOnError) {
        ...
        if (!exceptions.isEmpty()) {
          ...
          logger.error("Schema Export threw " + exceptions.size()
            + " exceptions and was halted");
  ...
  protected SchemaExport createExporter(String outputFile, boolean haltOnError) {
    final Configuration configuration = this.hibernateToolConfigurationSource
      .getConfiguration();
   ...
   * (non-Javadoc)
   * 
   * @see org.jasig.portal.tools.dbloader.ISchemaExport#update(boolean, boolean,
   * boolean, java.lang.String, boolean)
   */
  @Override
  public void update(boolean export, String outputFile, boolean haltOnError) {
    ...
    if (haltOnError) {
      ...
      if (!exceptions.isEmpty()) {
        ...
        logger.error("Schema Update threw " + exceptions.size()
          + " exceptions and was halted");
        throw new RuntimeException(e);
      }
    }
