...
public interface EventSessionDao {
   ...
   * 
   * @param loginEvent
   *          the event to base the session on
   * @return
   */
  EventSession createEventSession(LoginEvent loginEvent);

  /**
   * Get the current event session, null if no session has been created by
   * {@link #createEventSession(LoginEvent)}
   * 
   * @param eventSessionId
   *          The current session id, see
  ...
  /**
   * @param eventSessionId
   *          The id of the session to delete, see
   *          {@link PortalEvent#getEventSessionId()}
   */
