 ...
 * @version $Revision$
 */
public class PortletDefinitionImporterExporter extends
  AbstractJaxbDataHandler<ExternalPortletDefinition> implements
  IPortletPublishingService {
  ...
  public void setPortletDefinitionRegistry(
    IPortletDefinitionDao portletDefinitionRegistry) {
  ...
  public void importData(ExternalPortletDefinition portletRep) {
    ...
    if (isFramework != null
      && isFramework
      && "UPGRADED_CHANNEL_IS_NOT_A_PORTLET".equals(portletDescriptor
        .getPortletName())) {
      if (errorOnChannel) {
      ...
      logger
        .warn(portletRep.getFname()
          + " is not a portlet. It was likely an IChannel from a previous version of uPortal and will not be imported.");
      return;
    }

    // get the portlet type
    final IPortletType portletType = portletTypeRegistry.getPortletType(portletRep
      .getType());
    if (portletType == null) {
      throw new IllegalArgumentException("No portlet type registered for: "
        + portletRep.getType());
    }

    final List<PortletCategory> categories = new ArrayList<PortletCategory>();
    for (String categoryName : portletRep.getCategories()) {
      EntityIdentifier[] cats = GroupService.searchForGroups(categoryName,
        IGroupConstants.IS, IPortletDefinition.class);
      ...
      if (category == null) {
        throw new IllegalArgumentException("No category '" + categoryName
          + "' found when importing porltet: " + portletRep.getFname());
      }
    ...
    for (String groupName : portletRep.getGroups()) {
      EntityIdentifier[] gs = GroupService.searchForGroups(groupName,
        IGroupConstants.IS, IPerson.class);
      IGroupMember group;
      if (gs != null && gs.length > 0) {
        group = GroupService.findGroup(gs[0].getKey());
      } else {
        // An actual group key might be specified, so try looking up group
        // directly
        group = GroupService.findGroup(groupName);
      }

      if (group == null) {
        throw new IllegalArgumentException("No group '" + groupName
          + "' found when importing porltet: " + portletRep.getFname());
      }
    ...
    if (def == null) {
      def = portletDefinitionDao.createPortletDefinition(portletType, fname,
        portletRep.getName(), portletRep.getTitle(), portletDescriptor
          .getWebAppName(), portletDescriptor.getPortletName(),
        isFramework != null ? isFramework : false);
    } else {
      final IPortletDescriptorKey portletDescriptorKey = def
    ...
    for (ExternalPortletParameter param : portletRep.getParameters()) {
      parameters.add(new PortletDefinitionParameterImpl(param.getName(), param
        .getValue()));
    }
    ...
    for (ExternalPortletPreference pref : portletRep.getPortletPreferences()) {
      ...
      preferenceList.add(new PortletPreferenceImpl(pref.getName(),
        readOnly != null ? readOnly : false, values));
    }
    def.setPortletPreferences(preferenceList);

    savePortletDefinition(def, PersonFactory.createSystemPerson(), categories,
      groups);
  }
  ...
  public ExternalPortletDefinition deleteData(String fname) {
    final IPortletDefinition def = this.portletDefinitionDao
   ...
   * 
   * @see
   * org.jasig.portal.channel.IChannelPublishingService#saveChannelDefinition
   * (org.jasig.portal.portlet.om.IPortletDefinition,
   * org.jasig.portal.security.IPerson,
   * org.jasig.portal.channel.ChannelLifecycleState, java.util.Date,
   * java.util.Date, org.jasig.portal.ChannelCategory[],
   * org.jasig.portal.groups.IGroupMember[])
   */
  @Override
  public IPortletDefinition savePortletDefinition(IPortletDefinition definition,
    IPerson publisher, List<PortletCategory> categories,
    ...
    definition = portletDefinitionDao.getPortletDefinitionByFname(definition
      .getFName());

    final String defId = definition.getPortletDefinitionId().getStringId();
    final IEntity portletDefEntity = GroupService.getEntity(defId,
      IPortletDefinition.class);

    // Sync on groups during update. This really should be a portal wide
    // thread-safety check or
    ...
    synchronized (this.groupUpdateLock) {
      ...
      if (!newChannel) {
        ...
        final Iterator<IEntityGroup> iter = portletDefEntity
      ...
      final IUpdatingPermissionManager upm = authService
        .newUpdatingPermissionManager(FRAMEWORK_OWNER);
      final List<IPermission> permissions = new ArrayList<IPermission>(groupMembers
        .size());
      for (final IGroupMember member : groupMembers) {
        final IAuthorizationPrincipal authPrincipal = authService
          .newPrincipal(member);
      ...
      // If modifying the channel, remove the existing permissions before adding
      // the new ones
      if (!newChannel) {
        IPermission[] oldPermissions = upm.getPermissions(SUBSCRIBER_ACTIVITY,
          target);
  ...
  @Transactional
  @Override
  public void removePortletDefinition(IPortletDefinition portletDefinition,
    IPerson person) {
    IPortletDefinition portletDef = portletDefinitionDao
      .getPortletDefinition(portletDefinition.getPortletDefinitionId());

    // Delete existing category memberships for this channel
    String portletDefinitionId = portletDefinition.getPortletDefinitionId()
      .getStringId();
    IEntity channelDefEntity = GroupService.getEntity(portletDefinitionId,
      IPortletDefinition.class);
    @SuppressWarnings("unchecked")
    ...
    IUpdatingPermissionManager upm = authService
      .newUpdatingPermissionManager(FRAMEWORK_OWNER);
  ...
  protected ExternalPortletDefinition convert(IPortletDefinition def) {
    ...
    final List<ExternalPortletPreference> portletPreferenceList = rep
    ...
    final IGroupMember gm = GroupService.getGroupMember(def.getPortletDefinitionId()
      .getStringId(), IPortletDefinition.class);
    @SuppressWarnings("unchecked")
    final Iterator<IEntityGroup> categories = GroupService
      .getCompositeGroupService().findContainingGroups(gm);
    ...
    final AuthorizationService authService = org.jasig.portal.services.AuthorizationService
      .instance();
    final IPermissionManager pm = authService
      .newPermissionManager("UP_PORTLET_SUBSCRIBE");
    final IAuthorizationPrincipal[] principals = pm.getAuthorizedPrincipals(
      "SUBSCRIBE", IPermission.PORTLET_PREFIX
        + def.getPortletDefinitionId().getStringId());

    for (IAuthorizationPrincipal principal : principals) {
      IGroupMember member = authService.getGroupMember(principal);
      if (member.isGroup()) {
        final EntityNameFinderService entityNameFinderService = EntityNameFinderService
          .instance();
        final IEntityNameFinder nameFinder = entityNameFinderService
          .getNameFinder(member.getType());
        try {
          groupList.add(nameFinder.getName(member.getKey()));
        } catch (Exception e) {
          throw new RuntimeException("Could not find group name for entity: "
            + member.getKey(), e);
        }
      } else {
