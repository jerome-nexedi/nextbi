...
public class HibernateStyleCounterStore implements ICounterStore {
  ...
  public void setTransactionOperations(
    @Qualifier("PortalDb") TransactionOperations transactionOperations) {
  ...
  private Optimizer getCounterOptimizer(String counterName) {
    ...
    if (optimizer == null) {
      ...
      optimizer = ConcurrentMapUtils.putIfAbsent(counterOptimizers, counterName,
        optimizer);
    }
    ...
    return (Integer) optimizer.generate(new AccessCallback() {
      ...
      public IntegralDataTypeHolder getNextValue() {
        return transactionOperations
          .execute(new TransactionCallback<IntegralDataTypeHolder>() {
            @Override
            public IntegralDataTypeHolder doInTransaction(TransactionStatus status) {
              ...
              int rows;
              do {
                // Try and load the current value, returns true if the
                // exepected row exists, null otherwise
                ...
                // Update the counter row, if rows returns 0 the update failed
                // due to a race condition, it will be retried
                rows = jdbcOperations.update(UPDATE_QUERY,
