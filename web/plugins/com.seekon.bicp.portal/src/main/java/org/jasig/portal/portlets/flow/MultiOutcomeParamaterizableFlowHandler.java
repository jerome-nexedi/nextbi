...
public class MultiOutcomeParamaterizableFlowHandler extends AbstractFlowHandler {
  ...
  /**
   * @param redirectOnEnd
   *          The location to redirect to at the end of the flow
   */
   ...
   * 
   * @see
   * org.springframework.webflow.mvc.portlet.AbstractFlowHandler#getFlowId()
   */
  @Override
   ...
   * (non-Javadoc)
   * 
   * @seeorg.springframework.webflow.mvc.portlet.AbstractFlowHandler#
  ...
  public boolean handleExecutionOutcome(FlowExecutionOutcome outcome,
    ...
    if (this.redirectOnEnds != null
      && this.redirectOnEnds.containsKey(outcome.getId())) {
      final String redirectPath = request.getContextPath()
        + this.redirectOnEnds.get(outcome.getId());
      try {
