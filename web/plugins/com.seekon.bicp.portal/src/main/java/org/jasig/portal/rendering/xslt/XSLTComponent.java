 ...
 * @version $Revision$
 */
public class XSLTComponent extends StAXPipelineComponentWrapper implements
  ...
  public XSLTComponent() {
    this.errorListener = new SimpleTransformErrorListener(LogFactory.getLog(this
      .getClass()));
  }

  public void setXsltParameterSource(
    TransformerConfigurationSource xsltParameterSource) {
   ...
   * 
   * @see
   * org.jasig.portal.rendering.StAXPipelineComponent#getXmlStreamReader(java
  ...
  public PipelineEventReader<XMLEventReader, XMLEvent> getEventReader(
    ...
    final Transformer transformer = this.transformerSource.getTransformer(request,
      response);
    ...
    if (this.xsltParameterSource != null) {
      final Map<String, Object> transformerParameters = this.xsltParameterSource
        .getParameters(request, response);
      if (transformerParameters != null) {
        this.logger.debug("{} - Setting Transformer Parameters: ", this.beanName,
      ...
      final Properties outputProperties = this.xsltParameterSource
        .getOutputProperties(request, response);
      if (outputProperties != null) {
        this.logger.debug("{} - Setting Transformer Output Properties: ",
          this.beanName, outputProperties);
    ...
    try {
      ...
      throw new RuntimeException(
        "Failed to create XMLStreamReader from XMLEventReader", e);
    }
    ...
    final ContentHandler contentHandler = StaxUtils
    ...
    final XMLEventReader outputEventReader = new XMLEventBufferReader(eventBuffer
      .listIterator());
  ...
  public CacheKey getCacheKey(HttpServletRequest request,
    ...
    final CacheKey parentCacheKey = this.wrappedComponent.getCacheKey(request,
      response);
    ...
    if (this.xsltParameterSource != null) {
      transformerConfigurationKey = this.xsltParameterSource.getCacheKey(request,
        response);
    } else {
      transformerConfigurationKey = null;
    }

    return CacheKey.build(this.beanName, parentCacheKey, transformerKey,
      transformerConfigurationKey);
  }
}
