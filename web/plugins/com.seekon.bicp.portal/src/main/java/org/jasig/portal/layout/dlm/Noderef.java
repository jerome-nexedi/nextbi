 ...
 * model of a layout. A Noderef is the opposite (counterpart) of a
 * {@link Pathref}. In other words, a Noderef identifies internally what a
 * {@link Pathref} identifies externally. The need for two formats stems from
 * the fact that Noderefs use database identifiers in their format, which are
 * not (always) stable across migrations.
 * 
 ...
 * <li>structureId of the node within the fragment layout (e.g. 's2' but the
 * preceeding character is not always an 's')</li>
 * </ul>
 * 
...
public final class Noderef {
  ...
  public Noderef(int userId, int layoutId, String structureId) {
    ...
    Validate.isTrue(userAndLayoutIdsAreValid, "Arguments 'userId' and "
      + "'layoutId' must either both be zero or both " + "greater than zero.");
  ...
  public String toString() {
    ...
    if (userId != 0 && layoutId != 0) {
      ...
      sb.append(USER_ID_PREFIX).append(userId).append(LAYOUT_ID_PREFIX).append(
        layoutId).append(structureId);
      rslt = sb.toString();
    } else {
