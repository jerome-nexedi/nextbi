...
public class TabColumnPrefsHandler {
  ...
  /**
   * This method is called from the TabColumnPrefsState class after a node has
   * already been moved from its old parent to its new in the ILF. We can get at
   * the new parent via the compViewNode moved but need a separate handle of the
   * parent from whence it came. The goal of this method is to make the
   * appropriate change in the PLF to persist this action take by the user. For
   * ILF nodes this generally means adding an entry to the position set for the
   * new parent and removing any entry if it existed in the position set in the
   * old parent. For nodes that are owned by the user (PLF owned nodes) the
   * nodes are moved outright and now position set is needed unless the new
   * parent contains ILF nodes as well requiring preservation of the user's
   * ordering of the nodes for when the ILF and PLF are merged again later on.
   */
  public static void moveElement(Element compViewNode, Element oldCompViewParent,
    ...
    if (oldCompViewParent != compViewParent) {
      if (LOG.isInfoEnabled())
        LOG.info("reparenting from "
          + oldCompViewParent.getAttribute(Constants.ATT_ID) + " to "
          + compViewParent.getAttribute(Constants.ATT_ID));
      // update previous parent if found in PLF
      Element plfParent = HandlerUtils.getPLFNode(oldCompViewParent, person, // only
        // needed
        // if
        // creating
    ...
    Element plfParent = HandlerUtils.getPLFNode(compViewParent, person, true, // create
      // parent
      // if
      // not
      // found
   ...
   * delegates to the DeleteManager to add a delete directive. For PLF owned
   * nodes it deletes the node outright.
   */
  public static void deleteNode(Element compViewNode, Element compViewParent,
    IPerson person) throws PortalException {
