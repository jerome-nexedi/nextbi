...
/**
 * Base for XMLEventReader that implements the {@link #getElementText()} and
 * {@link #nextTag()} APIs in a way that is agnostic from the rest of the
 * XMLEventReader implementation. Both will use the subclasses
...
public abstract class BaseXMLEventReader extends EventReaderDelegate {
  ...
  /**
   * @return The XMLEvent returned by the last call to
   *         {@link #internalNextEvent()}
   */
  ...
  public final String getElementText() throws XMLStreamException {
    ...
    if (!event.isStartElement()) {
      throw new XMLStreamException("Must be on START_ELEMENT to read next text",
        event.getLocation());
    }
    ...
    while (!event.isEndDocument()) {
      ...
      default: {
        throw new XMLStreamException("Unexpected event type '"
          ...
          + "' encountered. Found event: " + event, event.getLocation());
      }
      }
