...
public class ReferenceIndividualGroupService extends ReferenceCompositeGroupService
  ...
  private static final Log log = LogFactory
    .getLog(ReferenceIndividualGroupService.class);
   ...
   * locked, which can occur if an attempt to remove the group from one of its
   * containing groups fails and throws a GroupsException. In this event, we do
   * not try to roll back any successful removes, since that would probably fail
   * anyway.
   * 
   * @param group
  ...
  private void removeDeletedGroupFromContainingGroups(ILockableEntityGroup group)
    ...
    try {
      ...
      for (itr = group.getContainingGroups(); itr.hasNext();) {
        ...
        lockableGroup = GroupService.findLockableGroup(containingGroup.getKey(),
          lockOwner);
  ...
  public IEntityGroup findGroup(CompositeEntityIdentifier ent)
    throws GroupsException {
    return (cacheInUse()) ? findGroupWithCache(ent) : primFindGroup(ent
      .getLocalKey());
  }
  ...
  public ILockableEntityGroup findGroupWithLock(String key, String owner, int secs)
    ...
    try {
      IEntityLock lock = (secs == 0) ? EntityLockService.instance().newWriteLock(
        groupType, key, owner) : EntityLockService.instance().newWriteLock(
        groupType, key, owner, secs);
   ...
   * member groups that are local to this service. Then retrieve the keys of all
   * of the member groups and ask the GroupService to find the groups we do not
   * yet have.
   * 
   * @param eg
   *          IEntityGroup
  ...
  /**
   * Returns an <code>IEntity</code> representing a portal entity. This does not
   * guarantee that the underlying entity actually exists.
   */
  public IEntity getEntity(String key, Class type) throws GroupsException {
    ...
    if (cacheInUse()) {
      try {
        ...
        throw new GroupsException("Problem retrieving group member " + type + "("
          + key + ")", ce);
      }
    }
    return ent;
  ...
  /**
   * Returns an <code>IEntity</code> representing a portal entity. This does not
   * guarantee that the entity actually exists.
   */
  ...
  /**
   * Returns a cached <code>IEntityGroup</code> or null if it has not been
   * cached.
   */
  protected IEntityGroup getGroupFromCache(String key) throws CachingException {
    return (IEntityGroup) EntityCachingService.instance().get(
      org.jasig.portal.EntityTypes.GROUP_ENTITY_TYPE, key);
  }
   ...
   * <code>IGroupMember</code> is an <code>IEntityGroup</code> else it is an
   * <code>IEntity</code>.
   */
  ...
  /**
   * Returns an <code>IGroupMember</code> representing either a group or a
   * portal entity, based on the <code>EntityIdentifier</code>, which refers to
   * the UNDERLYING entity for the <code>IGroupMember</code>.
   */
  ...
  /**
   * Returns the implementation of <code>IEntityGroupStore</code> whose class
   * name was retrieved by the PropertiesManager (see initialize()).
   */
  ...
  */
  ...
  private void initialize() throws GroupsException {
    ...
    if (groupStoreFactoryName == null) {
      ...
      try {
        ...
        eMsg = "ReferenceIndividualGroupService.initialize(): Failed to instantiate group store ("
          + svcName + "): " + e;
        log.error(eMsg);
    ...
    if (entityStoreFactoryName == null) {
      ...
      try {
        ...
        eMsg = "ReferenceIndividualGroupService.initialize(): Failed to instantiate entity store "
          + e;
        log.error(eMsg);
        throw new GroupsException(eMsg, e);
      }
    ...
    if (entitySearcherFactoryName == null) {
      ...
      try {
        ...
        eMsg = "ReferenceIndividualGroupService.initialize(): Failed to instantiate entity searcher "
          + e;
        log.error(eMsg);
        throw new GroupsException(eMsg, e);
      }
  ...
  /**
   * Answers if this service is a leaf in the composite; a service that actually
   * operates on groups.
   */
  ...
  */
  ...
  /**
   * Updates the <code>ILockableEntityGroup</code> in the store and removes it
   * from the cache.
   * 
   * @param group
   *          ILockableEntityGroup
  ...
  /**
   * Adjust the back pointers of the updated group members to either add or
   * remove the parent group. Then update the cache to invalidate copies on peer
   * servers.
   * 
   * @param group
   *          ILockableEntityGroup
  ...
  /**
   * Answers if <code>group</code> contains <code>member</code>. If the group
   * belongs to another service and the present service is not editable, simply
   * return false.
   * 
   * @return boolean
   * @param group
  ...
  public boolean contains(IEntityGroup group, IGroupMember member)
    throws GroupsException {
    return (isForeign(member) && !isEditable()) ? false : getGroupStore().contains(
      group, member);
  }
