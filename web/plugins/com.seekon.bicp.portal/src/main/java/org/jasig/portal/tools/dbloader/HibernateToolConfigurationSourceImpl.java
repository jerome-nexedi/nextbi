 ...
 * @version $Revision$
 */
public class HibernateToolConfigurationSourceImpl implements InitializingBean,
  HibernateToolConfigurationSource {
   ...
   * The persistence provider will throw an exception if ambiguous EntityManager
  ...
  private final void loadConfiguration() {
    // Use DefaultPersistenceUnitManager and Ejb3Configuration to build
    // hibernate configuration object
    final DefaultPersistenceUnitManager defaultPersistenceUnitManager = new DefaultPersistenceUnitManager();
    defaultPersistenceUnitManager.setDataSourceLookup(new SingleDataSourceLookup(
      dataSource));
    defaultPersistenceUnitManager.setDefaultDataSource(dataSource);
    defaultPersistenceUnitManager.afterPropertiesSet();
    final PersistenceUnitInfo persistenceUnitInfo = defaultPersistenceUnitManager
      .obtainPersistenceUnitInfo(this.persistenceUnitName);
    ...
    final Ejb3Configuration jpaConfiguration = configBuilder.configure(
      persistenceUnitInfo, ImmutableMap.of("hibernate.cache.use_query_cache",
        "false", "hibernate.cache.use_second_level_cache", "false"));
  ...
  private void loadServiceRegistry() {
    ...
    this.serviceRegistry = new ServiceRegistryBuilder().applySettings(properties)
