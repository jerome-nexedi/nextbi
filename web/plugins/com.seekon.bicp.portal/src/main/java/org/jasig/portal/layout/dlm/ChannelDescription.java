...
public class ChannelDescription extends UserLayoutChannelDescription {
  ...
  public void resetParameter(String parameterName) throws PortalException {
     ...
     * resetting to potentially drop back to the publish time parameter value if
     * not ad-hoc. If from a fragment then we drop back to the value held by the
     * fragment or if ad-hoc remove it completely.
     */
  ...
  /**
   * For DLM parameters starting with the dlm namespace are reserved for use by
   * DLM and hence can't be overridden. The channel published definition may
   * restrict updating. And finally, a fragment may restrict updating.
   */
  public boolean getParameterOverrideValue(String parameterName) {
    if (parameterName.startsWith(Constants.NS) || (fragmentChannelInfo != null))
      return false;
    return true;
  }
  ...
  public static UserLayoutNodeDescription createUserLayoutNodeDescription(
    ...
    if (nodeName.equals("channel"))
      ...
      return UserLayoutNodeDescription.createUserLayoutNodeDescription(xmlNode);
  }
