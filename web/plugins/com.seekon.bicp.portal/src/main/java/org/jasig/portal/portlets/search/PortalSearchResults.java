...
public class PortalSearchResults implements Serializable {
  ...
  public PortalSearchResults(String defaultTab,
    Map<String, Set<String>> resultTypeMappings) {
      ...
      .<String, List<Tuple<SearchResult, String>>> build(
        new CacheLoader<String, List<Tuple<SearchResult, String>>>() {
          ...
          public List<Tuple<SearchResult, String>> load(String key) throws Exception {
            return Collections
              .synchronizedList(new LinkedList<Tuple<SearchResult, String>>());
          }
        });
  }
  ...
  public void addPortletSearchResults(String url, SearchResult result) {
    ...
    for (final String tab : tabs) {
      final List<Tuple<SearchResult, String>> typeResults = this.results
        .getUnchecked(tab);
  ...
  protected Set<String> getTabs(SearchResult result) {
    ...
    // For each type the search result declares lookup the tab(s) mapped to that
    // type
