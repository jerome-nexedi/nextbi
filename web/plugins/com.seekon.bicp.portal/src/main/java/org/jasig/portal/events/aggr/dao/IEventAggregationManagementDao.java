...
public interface IEventAggregationManagementDao {
  ...
  /**
   * Default group includes/excludes, applied to all aggregators that do not
  ...
  /**
   * Default interval includes/excludes, applied to all aggregators that do not
   * have specific configurations.
   */
   ...
   * Store changes made to the specified config
   */
  void updateAggregatedIntervalConfig(
    AggregatedIntervalConfig aggregatedIntervalConfig);

  /**
   * Delete the specified configuration
   */
  void deleteAggregatedIntervalConfig(
    AggregatedIntervalConfig aggregatedIntervalConfig);

  /**
   * An immutable list of the configured quarter details sorted by its natural
   * ordering
   */
   ...
   * 
   * @param quarterDetail
   *          Must contain four sequential quarters with no gaps between dates
   */
  void setQuarterDetails(List<QuarterDetail> quarterDetail);

  /**
   * An immutable list of the currently configured academic terms sorted by its
   * natural ordering
   */
  List<AcademicTermDetail> getAcademicTermDetails();

  /**
   * Adds a new academic term, if the term overlaps with an existing term an
