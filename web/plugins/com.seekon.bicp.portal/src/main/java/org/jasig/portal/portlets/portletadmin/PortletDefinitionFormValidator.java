...
public class PortletDefinitionFormValidator {
  ...
  public void validateChooseType(PortletDefinitionForm def, MessageContext context) {
    if (def.getTypeId() == 0) {
      context.addMessage(new MessageBuilder().error().source("typeId").code(
        "please.choose.portlet.type").build());
    }
  }

  public void validateBasicInfo(PortletDefinitionForm def, MessageContext context) {
    if (StringUtils.isEmpty(def.getFname())) {
      context.addMessage(new MessageBuilder().error().source("fName").code(
        "please.enter.fname").build());
    ...
    if (StringUtils.isEmpty(def.getTitle())) {
      context.addMessage(new MessageBuilder().error().source("title").code(
        "please.enter.title").build());
    }

    if (StringUtils.isEmpty(def.getName())) {
      context.addMessage(new MessageBuilder().error().source("name").code(
        "please.enter.name").build());
    }
  ...
  public void validateSetParameters(PortletDefinitionForm def, MessageContext context) {
    PortletPublishingDefinition cpd = channelPublishingDefinitionDao
      .getChannelPublishingDefinition(def.getTypeId());
      ...
      if (step.getParameters() != null) {
        for (Parameter param : step.getParameters()) {
          ...
          if (def.getParameters().containsKey(param.getName())
            && !StringUtils.isEmpty(def.getParameters().get(param.getName())
              .getValue())) {
            ...
            // // if this restriction is an enumeration of allowed values, check
            // to
  ...
  public void validateChooseCategory(PortletDefinitionForm def,
    MessageContext context) {
    // make sure the user has picked at least one category
    if (def.getCategories().size() == 0) {
      context.addMessage(new MessageBuilder().error().source("categories").code(
        "please.choose.at.least.one.category").build());
    }
  }
  ...
  public void validateChooseGroup(PortletDefinitionForm def, MessageContext context) {
    ...
    if (def.getGroups().size() == 0) {
      context.addMessage(new MessageBuilder().error().source("groups").code(
        "please.choose.at.least.one.group").build());
    }
  }
    ...
    if (def.getExpirationDate() != null) {
      if (def.getExpirationDateTime().before(now)) {
        messageContext.addMessage(new MessageBuilder().error().source(
          "expirationDate").code("auto.expire.date.must.be.future").build());
      }
    }
    if (def.getPublishDate() != null && def.getExpirationDate() != null) {
      if (def.getExpirationDateTime().before(def.getPublishDateTime())) {
        messageContext.addMessage(new MessageBuilder().error().source(
          "expirationDate").code("auto.expire.date.must.be.after.publish").build());
      }
    }
  }
