...
/**
 * The SmartCache class is used to store objects in memory for a specified
 * amount of time. The time should be specified in seconds. If the time is
 * specified as a negative value, it will be cahced indefinitely.
 * 
...
public class SmartCache extends HashMap {
  ...
  /**
   * Instantiate a new SmartCache. Usually instances of SmartCache are declared
   * as static. When retrieving a value from SmartCache, it will be null if the
   * value has expired. It is up to the client to then retrieve the value and
   * put it in the cache again. Example: <code>
   ...
   *          an expiration timeout value, in seconds, which will override the
   *          default cache value just for this item. If a negative timeout
   ...
   * @param key
   *          the key, typically a String
   * @return the value to which the key is mapped in this cache; null if the key
   *         is not mapped to any value in this cache.
   */
