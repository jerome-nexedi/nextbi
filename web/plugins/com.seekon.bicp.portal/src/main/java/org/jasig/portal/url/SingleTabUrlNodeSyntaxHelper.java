...
/**
 * Maps tabs and portlets to folder names and back. Handles a single set of tabs
 * and uses tab IDs for folder names.
 * 
...
public class SingleTabUrlNodeSyntaxHelper implements IUrlNodeSyntaxHelper {
  ...
  // private String tabIdExpression =
  // "/layout/folder/folder[@ID=$nodeId or descendant::node()[@ID=$nodeId]]/@ID";
   ...
   * 
   * @see
   * org.jasig.portal.url.IUrlNodeSyntaxHelper#getDefaultLayoutNodeId(javax.
  ...
  public String getDefaultLayoutNodeId(HttpServletRequest httpServletRequest) {
    final IUserInstance userInstance = this.userInstanceManager
      .getUserInstance(httpServletRequest);
    ...
    this.logger.warn("Failed to find default tab id for "
      + userInstance.getPerson().getUserName() + " with default tab index "
      + defaultTabIndex + ". Index 1 will be tried as a fall-back.");
  ...
  protected String getTabId(final IUserLayout userLayout, final String tabIndex) {
    return this.xpathOperations.doWithExpression(defaultLayoutNodeIdExpression,
  ...
  protected String getDefaultTabIndex(HttpServletRequest httpServletRequest) {
    final String stylesheetParameter = this.stylesheetUserPreferencesService
      .getStylesheetParameter(httpServletRequest, PreferencesScope.STRUCTURE,
        this.defaultTabParameter);
    if (stylesheetParameter != null) {
      return stylesheetParameter;
    }

    final IStylesheetDescriptor stylesheetDescriptor = this.stylesheetUserPreferencesService
      .getStylesheetDescriptor(httpServletRequest, PreferencesScope.STRUCTURE);
    final IStylesheetParameterDescriptor stylesheetParameterDescriptor = stylesheetDescriptor
      .getStylesheetParameterDescriptor(this.defaultTabParameter);
   ...
   * 
   * @see
   * org.jasig.portal.url.IUrlNodeSyntaxHelper#getFolderNamesForLayoutNode(javax
   * .servlet.http.HttpServletRequest, java.lang.String)
   */
  @RequestCache(keyMask = { false, true })
  @Override
  public List<String> getFolderNamesForLayoutNode(HttpServletRequest request,
    String layoutNodeId) {
    final IUserInstance userInstance = this.userInstanceManager
    ...
    final String tabId = userLayout
      .findNodeId(new PortletTabIdResolver(layoutNodeId));
   ...
   * 
   * @see
   * org.jasig.portal.url.IUrlNodeSyntaxHelper#getLayoutNodeForFolderNames(javax
   * .servlet.http.HttpServletRequest, java.util.List)
   */
  @RequestCache(keyMask = { false, true })
  @Override
  public String getLayoutNodeForFolderNames(HttpServletRequest request,
   ...
   * 
   * @see
   * org.jasig.portal.url.IUrlNodeSyntaxHelper#getFolderNameForPortlet(javax
   * .servlet.http.HttpServletRequest,
   * org.jasig.portal.portlet.om.IPortletWindowId)
   */
  @RequestCache(keyMask = { false, true })
  @Override
  public String getFolderNameForPortlet(HttpServletRequest request,
   ...
   * 
   * @see
   * org.jasig.portal.url.IUrlNodeSyntaxHelper#getPortletForFolderName(javax
   * .servlet.http.HttpServletRequest, java.lang.String, java.lang.String)
   */
  @RequestCache(keyMask = { false, true, true })
  @Override
  public IPortletWindowId getPortletForFolderName(HttpServletRequest request,
    String targetedLayoutNodeId, String folderName) {
    ...
    if (subscribeId != null) {
      ...
      if (portletEntity == null
        ...
        // If no entity found or the fname doesn't match ignore the provided
        // subscribeId by setting it to null
        subscribeId = null;
      } else {
        // subscribeId matches fname, lookup the window for the entity and
        // return the windowId
    ...
    // If a layout node is targeted then look for a matching subscribeId under
    // that targeted node
    ...
    if (subscribeId == null) {
      portletWindow = this.portletWindowRegistry
        .getOrCreateDefaultPortletWindowByFname(request, fname);
    } else {
      portletWindow = this.portletWindowRegistry
