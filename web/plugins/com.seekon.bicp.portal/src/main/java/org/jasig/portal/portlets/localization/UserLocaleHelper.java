...
public class UserLocaleHelper {
  ...
  /**
   * Return a list of LocaleBeans matching the currently available locales for
   * the portal.
   * 
   * @param currentLocale
   * @return
  ...
  public Locale getCurrentUserLocale(PortletRequest request) {
    final HttpServletRequest originalPortalRequest = this.portalRequestUtils
   ...
   * implementation will update the session locale, and if the user is not a
   * guest, will also update the locale in the user's persisted preferences.
   * 
   * @param request
   * @param localeString
   */
    ...
    if (localeString != null) {
      ...
      if (!person.isGuest()) {
        try {
          ...
          // remove person layout framgent from session since it contains some
          // of the data in previous
          // translation and won't be cleared until next logout-login (applies
          // when using
