...
/**
 * Initiates portlet rendering based each encountered
 * {@link IUserLayoutManager#CHANNEL_HEADER} and
 * {@link IUserLayoutManager#CHANNEL} element in the event stream
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public class PortletRenderingInitiationStAXComponent extends
  StAXPipelineComponentWrapper {
  ...
  public PipelineEventReader<XMLEventReader, XMLEvent> getEventReader(
    ...
    final PortletRenderingXMLEventReader filteredEventReader = new PortletRenderingXMLEventReader(
      request, response, eventReader);
  ...
  private class PortletRenderingXMLEventReader extends FilteringXMLEventReader {
    ...
    public PortletRenderingXMLEventReader(HttpServletRequest request,
    ...
    protected XMLEvent filterEvent(XMLEvent event, boolean peek) {
      if (event.isStartElement()) {
        ...
        if (IUserLayoutManager.CHANNEL.equals(localName)) {
          ...
          if (!portletExecutionManager.isPortletRenderRequested(portletWindowId,
            this.request, this.response)) {
            portletExecutionManager.startPortletRender(portletWindowId,
              this.request, this.response);
            logger
              .debug("Initiated portlet markup rendering for: {}", portletWindow);
          } else {
        ...
        if (IUserLayoutManager.CHANNEL_HEADER.equals(localName)) {
          ...
          if (!portletExecutionManager.isPortletRenderHeaderRequested(
            ...
            portletExecutionManager.startPortletHeaderRender(portletWindowId,
              this.request, this.response);
            logger.debug("Initiated portlet head rendering for: {}", portletWindow);
          } else {
            logger.debug("Portlet header render already requested for: {}",
              portletWindow);
          }
