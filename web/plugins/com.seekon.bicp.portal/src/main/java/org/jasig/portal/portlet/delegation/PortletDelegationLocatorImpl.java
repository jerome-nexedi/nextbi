...
public class PortletDelegationLocatorImpl implements PortletDelegationLocator {
   ...
   * 
   * @see
   * org.jasig.portal.api.portlet.PortletDelegationLocator#createRequestDispatcher
   * (java.lang.String)
   */
  @Override
  ...
  public PortletDelegationDispatcher createRequestDispatcher(
    ...
    final IPortletWindowId parentPortletWindowId = this.portletWindowRegistry
      .getPortletWindowId(request, windowID);

    final IPortletEntity delegatePortletEntity = this.portletEntityRegistry
      .getOrCreateDelegatePortletEntity(request, parentPortletWindowId,
        delegatePortletDefinitionId);
    final IPortletEntityId delegatePortletEntityId = delegatePortletEntity
      .getPortletEntityId();

    final IPortletWindow delegatePortletWindow = this.portletWindowRegistry
      .createDelegatePortletWindow(request, delegatePortletEntityId,
        parentPortletWindowId);

    final IPerson person = this.personManager.getPerson(request);
    final int userId = person.getID();

    return new PortletDelegationDispatcherImpl(delegatePortletWindow, userId,
      portalRequestUtils, personManager, portletRenderer, portalUrlProvider,
   ...
   * 
   * @see
   * org.jasig.portal.api.portlet.PortletDelegationLocator#getRequestDispatcher
  ...
  public PortletDelegationDispatcher getRequestDispatcher(
    ...
    if (delegationParentId == null) {
      throw new IllegalArgumentException("Portlet window '" + portletWindow
        + "' is not a delegate window and cannot be delgated to.");
    }

    return new PortletDelegationDispatcherImpl(portletWindow, person.getID(),
      this.portalRequestUtils, this.personManager, this.portletRenderer,
      this.portalUrlProvider, this.portletDelegationManager);
  }
}
