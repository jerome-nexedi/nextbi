...
/**
 * Provides access to IPortletEntity objects and convenience methods for
 * creating and converting them and related objects.
 * 
 * The portlet adaptor channel will be responsible for listenting to unsubscribe
 * events and cleaning up entity objects
 * 
...
public class PortletEntityRegistryImpl implements IPortletEntityRegistry {
  ...
  static final String DELEGATE_LAYOUT_NODE_ID_PREFIX = "dlg"
    + DELEGATE_LAYOUT_NODE_ID_SEPERATOR;

  static final String PORTLET_ENTITY_DATA_ATTRIBUTE = PortletEntityRegistryImpl.class
    .getName()
    + ".PORTLET_ENTITY_DATA";

  static final String PORTLET_ENTITY_ATTRIBUTE = PortletEntityRegistryImpl.class
    .getName()
    + ".PORTLET_ENTITY.thread-";

  static final String PORTLET_ENTITY_LOCK_MAP_ATTRIBUTE = PortletEntityRegistryImpl.class
    .getName()
    + ".PORTLET_ENTITY_LOCK_MAP_ATTRIBUTE";

  static final String PORTLET_DEFINITION_LOOKUP_MAP_ATTRIBUTE = PortletEntityRegistryImpl.class
    .getName()
  ...
  public void setEntityIdParseCache(
    @Qualifier("org.jasig.portal.portlet.dao.jpa.PortletEntityImpl.idParseCache") Ehcache entityIdParseCache) {
   ...
   * 
   * @see
   * org.jasig.portal.portlet.registry.IPortletEntityRegistry#getPortletEntity
   * (org.jasig.portal.portlet.om.IPortletEntityId)
   */
  @Override
  public IPortletEntity getPortletEntity(HttpServletRequest request,
    IPortletEntityId portletEntityId) {
    Validate.notNull(portletEntityId, "portletEntityId can not be null");

    // Sync on the request map to make sure duplicate IPortletEntitys aren't
    // ever created
    final PortletEntityCache<IPortletEntity> portletEntityCache = this
      .getPortletEntityMap(request);

    return this.getPortletEntity(request, portletEntityCache, portletEntityId, null,
      -1);
  }
  ...
  @Override
  public IPortletEntity getPortletEntity(HttpServletRequest request,
    String portletEntityIdString) {
    final IPortletEntityId portletEntityId = this.parseConsistentPortletEntityId(
      request, portletEntityIdString);
  ...
  public IPortletEntity getOrCreateDefaultPortletEntity(HttpServletRequest request,
    ...
    final IPortletDefinition portletDefinition = this.getPortletDefinition(request,
      portletDefinitionId);
    if (portletDefinition == null) {
      throw new IllegalArgumentException("No portlet definition found for id '"
        + portletDefinitionId + "'.");
    }
    ...
    if (layoutNodeId == null) {
      throw new IllegalArgumentException("No layout node ID found for fname '"
        + portletFName + "'.");
    }

    this.logger.trace("Found layout node {} for portlet definition {}",
      layoutNodeId, portletFName);

    final IPerson person = userInstance.getPerson();
    final int personId = person.getID();
    return this.getOrCreatePortletEntity(request, portletDefinitionId, layoutNodeId,
      personId);
  }

  @Override
  public IPortletEntity getOrCreatePortletEntity(HttpServletRequest request,
    IUserInstance userInstance, String layoutNodeId) {
    final IUserPreferencesManager preferencesManager = userInstance
      .getPreferencesManager();
    final IUserLayoutManager userLayoutManager = preferencesManager
      .getUserLayoutManager();
    ...
    if (channelNode == null) {
      this.logger.warn("No layout node exists for id " + layoutNodeId
    ...
    final IPortletDefinition portletDefinition = this.getPortletDefinition(request,
      userInstance, channelPublishId);

    if (portletDefinition != null) {
      final IPerson person = userInstance.getPerson();
      return this.getOrCreatePortletEntity(request, portletDefinition
        .getPortletDefinitionId(), layoutNodeId, person.getID());
    }
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.portlet.registry.IPortletEntityRegistry#
   * getOrCreatePortletEntity(org.jasig.portal.portlet.om.IPortletDefinitionId,
  ...
  public IPortletEntity getOrCreatePortletEntity(HttpServletRequest request,
    ...
    IPortletEntity portletEntity = this.getPortletEntity(request,
      portletEntityCache, null, layoutNodeId, userId);

    // Found an existing entity!
    if (portletEntity != null) {
      // Verify the definition IDs match, this is a MUST in the case where the
      // subscribed portlet changes
    ...
    // Create the entity data object and store it in the session map (if not
    // already there)
    final PortletEntityCache<PortletEntityData> portletEntityDataMap = this
      .getPortletEntityDataMap(request);

    final IPortletEntityId portletEntityId = this.createConsistentPortletEntityId(
      portletDefinitionId, layoutNodeId, userId);
    PortletEntityData portletEntityData = new PortletEntityData(portletEntityId,
      portletDefinitionId, layoutNodeId, userId);
  ...
  @Override
  public IPortletEntity getOrCreatePortletEntityByFname(HttpServletRequest request,
    IUserInstance userInstance, String fname) {
    final IUserPreferencesManager preferencesManager = userInstance
      .getPreferencesManager();
    final IUserLayoutManager userLayoutManager = preferencesManager
      .getUserLayoutManager();
  ...
  @Override
  public IPortletEntity getOrCreatePortletEntityByFname(HttpServletRequest request,
    IUserInstance userInstance, String fname, String preferredChannelSubscribeId) {
    try {
      final IPortletEntity portletEntity = this.getOrCreatePortletEntity(request,
        userInstance, preferredChannelSubscribeId);
      ...
      // Ignored, can be the case if no layout node exists for the specified
      // subscribe ID
    }

    // Either the layout node didn't exist or the entity for the node doesn't
    // match the requested fname
  ...
  public IPortletEntity getOrCreateDelegatePortletEntity(HttpServletRequest request,
    ...
    final String layoutNodeId = DELEGATE_LAYOUT_NODE_ID_PREFIX
      + parentPortletWindowId.getStringId().replace(ID_PART_SEPERATOR,
        DELEGATE_LAYOUT_NODE_ID_SEPERATOR);
    ...
    return getOrCreatePortletEntity(request, delegatePortletDefinitionId,
      layoutNodeId, userId);
  }
   ...
   * 
   * @see
   * org.jasig.portal.portlet.registry.IPortletEntityRegistry#storePortletEntity
   * (org.jasig.portal.portlet.om.IPortletEntity)
   */
  @Override
  public void storePortletEntity(HttpServletRequest request,
    ...
    final IPortletEntityId wrapperPortletEntityId = portletEntity
      .getPortletEntityId();
    final Lock portletEntityLock = this.getPortletEntityLock(request,
      ...
      if (portletEntity instanceof PersistentPortletEntityWrapper) {
        ...
        if (shouldBePersisted) {
          try {
            ...
            // Check if this exception is from the entity being deleted from
            // under us.
            final boolean exists = this.portletEntityDao
              .portletEntityExists(persistentEntity.getPortletEntityId());
            if (!exists) {
              this.logger
                .warn("The persistent portlet has already been deleted: "
                  + persistentEntity
        ...
        // Already persistent entity that should not be, DELETE!
          ...
          // Capture identifiers needed to recreate the entity as session
          // persistent
          final IPortletDefinitionId portletDefinitionId = portletEntity
          ...
          this.getOrCreatePortletEntity(request, portletDefinitionId, layoutNodeId,
            userId);
        }
      } else if (portletEntity instanceof SessionPortletEntityImpl) {
        // There are preferences on the interim entity, create an store it
        if (shouldBePersisted) {
          // Remove the session scoped entity from the request and session
          // caches
          this.deletePortletEntity(request, portletEntity, false);

          final IPortletEntity persistentEntity = createPersistentEntity(
            portletEntity, wrapperPortletEntityId);

          if (this.logger.isTraceEnabled()) {
            this.logger
              .trace("Session scoped entity "
                + wrapperPortletEntityId
        ...
        // Session scoped entity that is still session scoped,
          ...
          if (existingPersistentEntity != null) {
            ...
            existingPersistentEntity = new PersistentPortletEntityWrapper(
              existingPersistentEntity, consistentPortletEntityId);

            this.logger
              .warn("A persistent portlet entity already exists: "
            ...
            final PortletEntityCache<PortletEntityData> portletEntityDataMap = this
              .getPortletEntityDataMap(request);
            portletEntityDataMap
              .storeIfAbsentEntity(((SessionPortletEntityImpl) portletEntity)
                .getPortletEntityData());
          }
        }
      } else {
  ...
  @Override
  public Lock getPortletEntityLock(HttpServletRequest request,
    IPortletEntityId portletEntityId) {
    ...
    // Create a new lock and do a putIfAbsent to avoid synchronizing but still
    // get a single lock instance for the session.
  ...
  protected IPortletDefinition getPortletDefinition(HttpServletRequest request,
    IUserInstance userInstance, String portletDefinitionIdStr) {
    ...
    IPortletDefinition portletDefinition = portletDefinitions
      .get(portletDefinitionIdStr);
    ...
    portletDefinition = this.portletDefinitionRegistry
      .getPortletDefinition(portletDefinitionIdStr);
    portletDefinition = checkPortletDefinitionRenderPermissions(userInstance,
      portletDefinition);
    if (portletDefinition == null) {
      portletDefinitions.put(portletDefinitionIdStr,
        NO_PERMISSION_PORTLET_DEFINITION);
    } else {
  ...
  private IPortletDefinition checkPortletDefinitionRenderPermissions(
    ...
    final IAuthorizationPrincipal ap = AuthorizationService.instance().newPrincipal(
  ...
  protected IPortletEntity createPersistentEntity(
    ...
    IPortletEntity persistentEntity = this.portletEntityDao.getPortletEntity(
      layoutNodeId, userId);
    if (persistentEntity != null) {
      ...
      persistentEntity = this.portletEntityDao.createPortletEntity(
        portletDefinitionId, layoutNodeId, userId);
    }

    // Copy over preferences to avoid modifying any part of the interim entity
    // by reference
    final List<IPortletPreference> existingPreferences = portletEntity
      .getPortletPreferences();
    final List<IPortletPreference> persistentPreferences = persistentEntity
      .getPortletPreferences();
  ...
  /**
   * Delete a portlet entity, removes it from the request, session and
   * persistent stores
   */
  protected void deletePortletEntity(HttpServletRequest request,
    IPortletEntity portletEntity, boolean cacheOnly) {
    ...
    if (!cacheOnly && portletEntity instanceof PersistentPortletEntityWrapper) {
      ...
      try {
        ...
        this.logger.warn("This persistent portlet has already been deleted: "
          + persistentEntity
          + ", trying to find and delete by layout node and user.");
        final IPortletEntity existingPersistentEntity = this.portletEntityDao
          .getPortletEntity(persistentEntity.getLayoutNodeId(), persistentEntity
            .getUserId());
        if (existingPersistentEntity != null) {
  ...
  protected IPortletEntity getPortletEntity(HttpServletRequest request,
    ...
    if (portletEntity != null) {
      logger.trace("Found IPortletEntity {} in request cache", portletEntity
        .getPortletEntityId());
      return portletEntity;
    }
    ...
    if (portletEntityData != null) {
      ...
      // Stick the entity wrapper in the request map (if it wasn't already added
      // by another thread)
      portletEntity = portletEntityCache.storeIfAbsentEntity(portletEntityData
        .getPortletEntityId(), new Function<IPortletEntityId, IPortletEntity>() {
        @Override
    ...
    if (portletEntityId != null) {
      if (portletEntityId instanceof PortletEntityIdImpl) {
        ...
        portletEntity = this.portletEntityDao.getPortletEntity(localLayoutNodeId,
          localUserId);
      } else {
    ...
    // Found a persistent entity, wrap it to make the id consistent between the
    // persistent and session stored entities
    if (portletEntity != null) {
      final IPortletEntityId consistentPortletEntityId = this
        .createConsistentPortletEntityId(portletEntity);

      final IPortletEntity anonPortletEntity = portletEntity;

      // Stick the entity wrapper in the request map (if it wasn't already added
      // by another thread)
      portletEntity = portletEntityCache.storeIfAbsentEntity(
        consistentPortletEntityId, new Function<IPortletEntityId, IPortletEntity>() {
          @Override
          public IPortletEntity apply(IPortletEntityId input) {
            logger
              .trace(
            ...
            return new PersistentPortletEntityWrapper(anonPortletEntity,
  ...
  protected IPortletEntityId createConsistentPortletEntityId(
    ...
    final String layoutNodeId = portletEntity.getLayoutNodeId();
    final int userId = portletEntity.getUserId();
    return this.createConsistentPortletEntityId(portletDefinitionId, layoutNodeId,
      userId);
  }

  protected IPortletEntityId createConsistentPortletEntityId(
    PortletEntityData portletEntityData) {
    final IPortletDefinitionId portletDefinitionId = portletEntityData
      .getPortletDefinitionId();
  ...
  protected IPortletEntityId createConsistentPortletEntityId(
    IPortletDefinitionId portletDefinitionId, String layoutNodeId, int userId) {
    final String compositeId = portletDefinitionId.getStringId() + ID_PART_SEPERATOR
      + layoutNodeId + ID_PART_SEPERATOR + userId;
  ...
  protected IPortletEntityId parseConsistentPortletEntityId(
    ...
    Validate.notNull(consistentEntityIdString,
      "consistentEntityIdString can not be null");
    ...
    final String[] idParts = ID_PART_SEPERATOR_PATTERN
      .split(consistentEntityIdString);
    if (idParts.length != 3) {
      throw new IllegalArgumentException(
    ...
    if (portletDefinition == null) {
      throw new IllegalArgumentException("No parent IPortletDefinition found for "
        + portletDefinitionIdString + " from entity id string: "
        + consistentEntityIdString);
    }
    final IPortletDefinitionId portletDefinitionId = portletDefinition
      .getPortletDefinitionId();
    ...
    if (!layoutNodeId.startsWith(DELEGATE_LAYOUT_NODE_ID_PREFIX)) {
      final IUserLayoutNodeDescription node = userLayoutManager
        .getNode(layoutNodeId);
      if (node == null || node.getType() != LayoutNodeType.PORTLET) {
        throw new IllegalArgumentException("No portlet layout node found for "
          + layoutNodeId + " from entity id string: " + consistentEntityIdString);
      }
      ...
      if (!portletDefinitionId.getStringId().equals(channelPublishId)) {
        throw new IllegalArgumentException("The portlet layout node found for "
          + layoutNodeId + " does not match the IPortletDefinitionId "
          + portletDefinitionId + " specified in entity id string: "
          + consistentEntityIdString);
      }
    }
    ...
    try {
      ...
      throw new IllegalArgumentException("The user id " + idParts[2]
        + " is not a valid integer from entity id string: "
        + consistentEntityIdString, e);
    }

    final IPortletEntityId portletEntityId = createConsistentPortletEntityId(
      portletDefinitionId, layoutNodeId, userId);

    // Cache the resolution
    this.entityIdParseCache.put(new Element(consistentEntityIdString,
      portletEntityId));
  ...
  protected ConcurrentMap<IPortletEntityId, Lock> getPortletEntityLockMap(
    ...
    return PortalWebUtils.getMapSessionAttribute(session,
      PORTLET_ENTITY_LOCK_MAP_ATTRIBUTE);
  }

  protected PortletEntityCache<IPortletEntity> getPortletEntityMap(
    HttpServletRequest request) {
    request = portalRequestUtils.getOriginalPortletOrPortalRequest(request);

    // create the thread specific cache name
    final String entityMapAttribute = PORTLET_ENTITY_ATTRIBUTE
  ...
  protected IPortletEntity wrapPortletEntityData(
    final PortletEntityData portletEntityData) {
    final IPortletDefinitionId portletDefinitionId = portletEntityData
      .getPortletDefinitionId();
    final IPortletDefinition portletDefinition = this.portletDefinitionRegistry
  ...
  public boolean shouldBePersisted(IPortletEntity portletEntity) {
    ...
    // Only non delegate entities with preferences or a non-null window state
    // should be persisted
    final List<IPortletPreference> preferences = portletEntity
      .getPortletPreferences();
