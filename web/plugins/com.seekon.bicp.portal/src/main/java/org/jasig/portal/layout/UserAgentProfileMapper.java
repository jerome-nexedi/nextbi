...
/**
 * Maps User-Agents to profile names using regular expressions. A list of
 * {@link Mapping}s is evaluated in order and the first match is returned. If
 * the user agent header is null or no match is found the
...
public class UserAgentProfileMapper implements IProfileMapper {
  ...
  /**
   * Default profile name to return if no match is found, defaults to
   * <code>null</code>.
   */
  ...
  /**
   * {@link HttpServletRequest} header name to get the user agent string.
   ...
   * 
   * @see
   * org.jasig.portal.layout.IProfileMapper#getProfileFname(org.jasig.portal
   * .security.IPerson, javax.servlet.http.HttpServletRequest)
   */
  @Override
  public String getProfileFname(IPerson person, HttpServletRequest request) {
    ...
    if (userAgent == null) {
      this.logger.debug("No {} header for {} returning default profile",
        this.userAgentHeader, person.getUserName());
    ...
    for (final Mapping mapping : this.mappings) {
      ...
      if (matcher.matches()) {
        this.logger.debug("Matched {} header {} for {} returning profile {}",
          new Object[] { this.userAgentHeader, person.getUserName(),
            mapping.profileName });
  ...
  public static class Mapping {
    ...
    public int hashCode() {
      ...
      result = prime * result
        + ((this.profileName == null) ? 0 : this.profileName.hashCode());
      return result;
    }
    ...
    public String toString() {
      return "Mapping [pattern=" + this.pattern + ", profileName="
        + this.profileName + "]";
    }
  }
}
