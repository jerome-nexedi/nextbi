...
/**
 * A portlet entity represents what a user subscribes to (adds to their layout)
...
public interface IPortletEntity extends IPortletEntityDescriptor {
  ...
  /**
   * @return A read-only Map of the WindowStates that have been set. The key is
   *         the {@link IStylesheetDescriptor#getId()} value.
   */
  ...
  /**
   * @param state
   *          The persisted window state for this portlet and stylesheet
   */
  ...
  /**
   * @return A hash code created based on the userId, layoutId and parent
  ...
  /**
   * Must compare correctly with any other {@link IPortletEntity} on
   * {@link #getUserId()}, {@link #getLayoutNodeId()}, and
