...
public class PortletRendererImpl implements IPortletRenderer {
  ...
  @Autowired
  public void setPortletDelegationLocator(
    PortletDelegationLocator portletDelegationLocator) {
    this.portletDelegationLocator = portletDelegationLocator;
  }

  /**
   * @param portletCacheControlService
   *          the portletCacheControlService to set
   */
  @Autowired
  ...
  /**
   * PLT 22.1 If the content of a portlet is cached and the portlet is target of
   * request with an action-type semantic (e.g. an action or event call), the
   * portlet container should discard the cache and invoke the corresponding
   * request handling methods of the portlet like processAction,or processEvent.
   * 
   * (non-Javadoc)
   * 
   * @see org.jasig.portal.channels.portlet.IPortletRenderer#doAction(org.jasig.portal.portlet.om.IPortletWindowId,
   *      javax.servlet.http.HttpServletRequest,
   *      javax.servlet.http.HttpServletResponse)
   */
  @Override
  public long doAction(IPortletWindowId portletWindowId,
    HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
    this.portletCacheControlService.purgeCachedPortletData(portletWindowId,
      httpServletRequest);

    final IPortletWindow portletWindow = this.portletWindowRegistry
      .getPortletWindow(httpServletRequest, portletWindowId);
    ...
    try {
      this.portletContainer.doAction(portletWindow.getPlutoPortletWindow(),
        httpServletRequest, httpServletResponse);
    } catch (PortletException pe) {
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing action.", portletWindow, pe);
      ...
      throw new PortletDispatchException(
        "The portlet container threw an exception while executing action on portlet window '"
          + portletWindow + "'.", portletWindow, pce);
    } catch (IOException ioe) {
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing action.", portletWindow, ioe);
    }
    ...
    final Map<String, List<String>> parameters = this.getParameters(
      httpServletRequest, portletWindowId, false);
    this.portalEventFactory.publishPortletActionExecutionEvent(httpServletRequest,
      this, fname, executionTime, parameters);
   ...
   * (non-Javadoc)
   * 
   * @see org.jasig.portal.portlet.rendering.IPortletRenderer#doEvent(org.jasig.portal.portlet.om.IPortletWindowId,
   *      javax.servlet.http.HttpServletRequest,
   *      javax.servlet.http.HttpServletResponse, javax.portlet.Event)
   */
  @Override
  public long doEvent(IPortletWindowId portletWindowId,
    HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
    Event event) {
    this.portletCacheControlService.purgeCachedPortletData(portletWindowId,
      httpServletRequest);
    ...
    if (this.logger.isDebugEnabled()) {
      this.logger
        .debug("Executing portlet event for window '" + portletWindow + "'");
    }

    final long start = System.nanoTime();
    try {
      this.portletContainer.doEvent(portletWindow.getPlutoPortletWindow(),
        httpServletRequest, httpServletResponse, event);
    } catch (PortletException pe) {
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing event.", portletWindow, pe);
    } catch (PortletContainerException pce) {
      throw new PortletDispatchException(
      ...
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing event.", portletWindow, ioe);
    }
    ...
    final Map<String, List<String>> parameters = this.getParameters(
      httpServletRequest, portletWindowId, false);
    this.portalEventFactory.publishPortletEventExecutionEvent(httpServletRequest,
      this, fname, executionTime, parameters, event.getQName());
  ...
  public PortletRenderResult doRenderHeader(IPortletWindowId portletWindowId,
    ...
    return doRender(portletWindowId, httpServletRequest, httpServletResponse,
      portletOutputHandler, RenderPart.HEADERS);
  }

  /**
   * Interacts with the {@link IPortletCacheControlService} to determine if the
   * markup should come from cache or not. If cached data doesn't exist or is
   * expired, this delegates to
   * {@link #doRenderMarkupInternal(IPortletWindowId, HttpServletRequest, HttpServletResponse, Writer)}
   * .
   * 
   * @throws IOException
   */
  ...
  public PortletRenderResult doRenderMarkup(IPortletWindowId portletWindowId,
    ...
    return doRender(portletWindowId, httpServletRequest, httpServletResponse,
      portletOutputHandler, RenderPart.MARKUP);
  }

  /**
   * Describes the part of the render request and defines the part specific
   * behaviors
   */
  protected enum RenderPart {
    HEADERS(PortletRequest.RENDER_HEADERS) {
      ...
      public CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> getCacheState(
        ...
        return portletCacheControlService.getPortletRenderHeaderState(request,
          portletWindowId);
      }
      ...
      public void publishRenderExecutionEvent(
        IPortalEventFactory portalEventFactory, PortletRendererImpl source,
        HttpServletRequest request, String fname, long executionTime,
        Map<String, List<String>> parameters, boolean targeted, boolean cached) {
        portalEventFactory.publishPortletRenderHeaderExecutionEvent(request, source,
          fname, executionTime, parameters, targeted, cached);
      }
    },
    MARKUP(PortletRequest.RENDER_MARKUP) {
      ...
      public CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> getCacheState(
        IPortletCacheControlService portletCacheControlService,
        HttpServletRequest request, IPortletWindowId portletWindowId) {
        return portletCacheControlService.getPortletRenderState(request,
          portletWindowId);
      }
      ...
      public void publishRenderExecutionEvent(
        ...
        HttpServletRequest request, String fname, long executionTime,
        Map<String, List<String>> parameters, boolean targeted, boolean cached) {
        portalEventFactory.publishPortletRenderExecutionEvent(request, source,
          fname, executionTime, parameters, targeted, cached);
      }
    };
    ...
    /**
     * Determine the {@link RenderPart} from the
     * {@link PortletRequest#RENDER_PART}
     */
    public static RenderPart getRenderPart(String renderPart) {
      ...
      throw new IllegalArgumentException("Unknown " + PortletRequest.RENDER_PART
        + " specified: " + renderPart);
    }
  }

  protected PortletRenderResult doRender(IPortletWindowId portletWindowId,
    HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
    ...
    final CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> cacheState = renderPart
      .getCacheState(this.portletCacheControlService, httpServletRequest,
        portletWindowId);
    ...
    /*
     * If the portlet is rendering in EXCLUSIVE WindowState ignore the provided
     * PortletOutputHandler and write directly to the response.
     * 
     * THIS IS VERY BAD AND SHOULD BE DEPRECATED ALONG WITH EXCLUSIVE WINDOW
     * STATE
     */
    ...
    if (cacheState.isUseCachedData()) {
      return doRenderReplayCachedContent(portletWindow, httpServletRequest,
        cacheState, portletOutputHandler, renderPart, 0);
    }

    final int cacheSizeThreshold = this.portletCacheControlService
      .getCacheSizeThreshold();
    ...
    httpServletRequest.setAttribute(ATTRIBUTE__PORTLET_OUTPUT_HANDLER,
    ...
    try {
      httpServletRequest.setAttribute(PortletRequest.RENDER_PART, renderPart
        .getRenderPart());
      this.portletContainer.doRender(portletWindow.getPlutoPortletWindow(),
        httpServletRequest, httpServletResponse);
    } catch (PortletException pe) {
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing renderMarkup.", portletWindow, pe);
    } catch (PortletContainerException pce) {
      throw new PortletDispatchException(
      ...
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing renderMarkup.", portletWindow, ioe);
    }
    ...
    if (useCachedContent) {
      final CachedPortletData<PortletRenderResult> cachedPortletData = cacheState
      ...
      renderPart.cachePortletOutput(portletCacheControlService, portletWindowId,
        httpServletRequest, cacheState, cachedPortletData);

      return doRenderReplayCachedContent(portletWindow, httpServletRequest,
        cacheState, portletOutputHandler, renderPart, executionTime);
    }

    publishRenderEvent(portletWindow, httpServletRequest, renderPart, executionTime,
      false);

    // Build the render result
    final PortletRenderResult portletRenderResult = constructPortletRenderResult(
      httpServletRequest, executionTime);

    // Check if the portlet's output should be cached
    if (cacheState != null) {
      boolean shouldCache = this.portletCacheControlService
        .shouldOutputBeCached(cacheControl);
  ...
  /**
   * Replay the cached content inside the {@link CachedPortletData} as the
   * response to a doRender.
   */
  protected PortletRenderResult doRenderReplayCachedContent(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Replaying cached content for Render " + renderPart
        + " request to " + portletWindow);
    }
    ...
    final long executionTime = baseExecutionTime
      + (System.nanoTime() - renderStartTime);
   ...
   */
  protected void publishRenderEvent(IPortletWindow portletWindow,
    HttpServletRequest httpServletRequest, RenderPart renderPart,
    long executionTime, boolean cached) {
    ...
    final boolean targeted = portletWindowId.equals(portalRequestInfo
    ...
    final Map<String, List<String>> parameters = this.getParameters(
      httpServletRequest, portletWindowId, true);

    renderPart.publishRenderExecutionEvent(this.portalEventFactory, this,
      httpServletRequest, fname, executionTime, parameters, targeted, cached);
  }

  /**
   * Construct a {@link PortletRenderResult} from information in the
   * {@link HttpServletRequest}. The second argument is how long the render
   * action took.
   * 
  ...
  protected PortletRenderResult constructPortletRenderResult(
    ...
    final String title = (String) httpServletRequest
      .getAttribute(IPortletRenderer.ATTRIBUTE__PORTLET_TITLE);
    ...
    final String link = (String) httpServletRequest
      .getAttribute(IPortletRenderer.ATTRIBUTE__PORTLET_LINK);

    return new PortletRenderResult(title, link, newItemCount, renderTime);
  }

  @Override
  public long doServeResource(IPortletWindowId portletWindowId,
    HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
    PortletResourceOutputHandler portletOutputHandler) throws IOException {
    ...
    if (cacheState.isUseBrowserData()) {
      return doResourceReplayBrowserContent(portletWindow, httpServletRequest,
        cacheState, portletOutputHandler);
    }

    if (cacheState.isUseCachedData()) {
      return doResourceReplayCachedContent(portletWindow, httpServletRequest,
        cacheState, portletOutputHandler, 0);
    }

    final int cacheSizeThreshold = this.portletCacheControlService
      .getCacheSizeThreshold();
    final CachingPortletResourceOutputHandler cachingPortletOutputHandler = new CachingPortletResourceOutputHandler(
      portletOutputHandler, cacheSizeThreshold);
    ...
    // TODO? CachingPortletHttpServletResponseWrapper responseWrapper =
    // this.setupCachingPortletResponse(httpServletResponse,
    // this.portletCacheControlService.getCacheSizeThreshold());

    CacheControl cacheControl = cacheState.getCacheControl();
    // Wrap the cache control so it immediately sets the caching related
    // response headers
    cacheControl = new HeaderSettingCacheControl(cacheControl,
      cachingPortletOutputHandler);
    httpServletRequest.setAttribute(ATTRIBUTE__PORTLET_CACHE_CONTROL, cacheControl);
    httpServletRequest.setAttribute(ATTRIBUTE__PORTLET_OUTPUT_HANDLER,
      cachingPortletOutputHandler);
    ...
    httpServletResponse = new ResourceHttpServletResponseWrapper(
      httpServletResponse, cacheControl);
    ...
    final long start = System.nanoTime();
    try {
      this.portletContainer.doServeResource(portletWindow.getPlutoPortletWindow(),
        httpServletRequest, httpServletResponse);
    } catch (PortletException pe) {
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing serveResource.", portletWindow, pe);
    } catch (PortletContainerException pce) {
      throw new PortletDispatchException(
      ...
      throw new PortletDispatchException("The portlet window '" + portletWindow
        + "' threw an exception while executing serveResource.", portletWindow, ioe);
    }
    final long executionTime = System.nanoTime() - start;
    ...
    final boolean useCachedContent = cacheControl.useCachedContent();
    if (useCachedContent) {
      final CachedPortletResourceData<Long> cachedPortletResourceData = cacheState
        .getCachedPortletData();

      if (cachedPortletResourceData != null) {
        // Update the expiration time and re-store in the cache
        final CachedPortletData<Long> cachedPortletData = cachedPortletResourceData
          .getCachedPortletData();
        cachedPortletData.updateExpirationTime(cacheControl.getExpirationTime());
        this.portletCacheControlService.cachePortletResourceOutput(portletWindowId,
          httpServletRequest, cacheState, cachedPortletResourceData);
      }
      ...
      return doResourceReplayCachedContent(portletWindow, httpServletRequest,
        cacheState, cachingPortletOutputHandler, executionTime);
    }

    publishResourceEvent(portletWindow, httpServletRequest, executionTime, false,
      false);
      ...
      if (shouldCache) {
        ...
        if (cachedPortletResourceData != null) {
          this.portletCacheControlService.cachePortletResourceOutput(
            portletWindowId, httpServletRequest, cacheState,
  ...
  protected long doResourceReplayBrowserContent(IPortletWindow portletWindow,
    HttpServletRequest httpServletRequest,
    ...
    if (cachedPortletResourceData != null) {
      ...
      final CachedPortletData<Long> cachedPortletData = cachedPortletResourceData
        .getCachedPortletData();
      PortletCachingHeaderUtils.setCachingHeaders(cachedPortletData,
        portletOutputHandler);
    }

    final long executionTime = System.nanoTime() - start;

    publishResourceEvent(portletWindow, httpServletRequest, executionTime, true,
      false);
  ...
  /**
   * Replay the cached content inside the {@link CachedPortletData} as the
   * response to a doResource.
   */
  protected Long doResourceReplayCachedContent(IPortletWindow portletWindow,
    HttpServletRequest httpServletRequest,
    CacheState<CachedPortletResourceData<Long>, Long> cacheState,
    PortletResourceOutputHandler portletOutputHandler, long baseExecutionTime)
    throws IOException {

    if (logger.isDebugEnabled()) {
      logger.debug("Replaying cached content for resource request to "
        + portletWindow);
    }
   ...
   */
  protected String getResourceId(IPortletWindowId portletWindowId,
    final IPortalRequestInfo portalRequestInfo) {
    final IPortletRequestInfo portletRequestInfo = portalRequestInfo
   ...
   */
  protected void publishResourceEvent(IPortletWindow portletWindow,
    HttpServletRequest httpServletRequest, long executionTime,
    boolean usedBrowserCache, boolean usedPortalCache) {
    ...
    this.portalEventFactory.publishPortletResourceExecutionEvent(httpServletRequest,
      this, fname, executionTime, parameters, resourceId, usedBrowserCache,
      usedPortalCache);
  }
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.IPortletRenderer#doReset(org.jasig.portal
   * .portlet.om.IPortletWindowId, javax.servlet.http.HttpServletRequest,
   * javax.servlet.http.HttpServletResponse)
   */
  @Override
  public void doReset(IPortletWindowId portletWindowId,
    ...
    if (portletWindow != null) {
      ...
      httpServletRequest = this.setupPortletRequest(httpServletRequest);
      httpServletResponse = new OutputCapturingHttpServletResponseWrapper(
        httpServletResponse, new PrintWriter(responseOutput));
      ...
      try {
        this.portletContainer.doAdmin(portletWindow.getPlutoPortletWindow(),
          httpServletRequest, httpServletResponse);
      } catch (PortletException pe) {
        throw new PortletDispatchException("The portlet window '" + portletWindow
      ...
      if (initResults.length() > 0) {
        throw new PortletDispatchException(
          "Content was written to response during reset of portlet window '"
            + portletWindow + "'. Response Content: " + initResults, portletWindow);
      }
    } else {
      logger
        .debug("ignoring doReset as portletWindowRegistry#getPortletWindow returned a null result for portletWindowId "
          + portletWindowId);
    }
  }
  ...
  protected Map<String, List<String>> getParameters(
    HttpServletRequest httpServletRequest, IPortletWindowId portletWindowId,
  ...
  protected HttpServletRequest setupPortletRequest(
    HttpServletRequest httpServletRequest) {
    final PortletHttpServletRequestWrapper portletHttpServletRequestWrapper = new PortletHttpServletRequestWrapper(
      httpServletRequest);
  ...
  /**
   * Wrap the {@link HttpServletResponse} in a
   * {@link PortletHttpServletResponseWrapper}.
   * 
   * @param httpServletResponse
   * @return the wrapped response
   */
  protected HttpServletResponse setupPortletResponse(
    HttpServletResponse httpServletResponse) {
    final PortletHttpServletResponseWrapper portletHttpServletResponseWrapper = new PortletHttpServletResponseWrapper(
      httpServletResponse);
    return portletHttpServletResponseWrapper;
  }

  /**
   * Wrap the {@link HttpServletResponse} like
   * {@link #setupPortletResponse(HttpServletResponse)}, additionally override
   * the response's outputstream with a {@link TeeServletOutputStream}.
   * 
   * @param httpServletResponse
   * @param toTee
   * @param captureHeaders
   *          set to true if you expect to capture headers
   * @return the wrapepd response.
   * @throws IOException
   */
  protected CachingPortletHttpServletResponseWrapper setupCachingPortletResponse(
    HttpServletResponse httpServletResponse, int cacheThresholdSize)
    throws IOException {
    final CachingPortletHttpServletResponseWrapper portletHttpServletResponseWrapper = new CachingPortletHttpServletResponseWrapper(
      httpServletResponse, cacheThresholdSize);
    return portletHttpServletResponseWrapper;
  }

  protected void setupPortletWindow(HttpServletRequest httpServletRequest,
    IPortletWindow portletWindow, IPortletUrlBuilder portletUrl) {
    ...
    if (portletMode != null) {
      if (IPortletRenderer.CONFIG.equals(portletMode)) {
        ...
        final AuthorizationService authorizationService = AuthorizationService
          .instance();
        final IAuthorizationPrincipal ap = authorizationService.newPrincipal(ei
          .getKey(), ei.getType());
        ...
        if (!ap.canConfigure(portletDefinition.getPortletDefinitionId()
          .getStringId())) {
          throw new AuthorizationException(person.getUserName()
            + " does not have permission to render '" + portletDefinition.getFName()
            + "' in " + portletMode + " PortletMode");
        }
      }
    }
  }
