...
/**
 * Provides concurrent scanning of a directory tree. Clients provide a callback
 * {@link Function} to call for each found {@link File}. Optionally the function
 * results can be collated and returned in a Map with the file the result is
 * for.
 * 
 * Note that the provided ExecutorService can never block on
...
public final class ConcurrentDirectoryScanner implements DirectoryScanner {
   ...
   *          Used to submit the file processing and directory recursion tasks
   */
   ...
   *          If directories should be passed to the fileProcessor
   *          {@link Function}, defaults to false.
   */
  ...
  /**
   * @param recursive
   *          If processing should recurse on directories, defaults to true.
   */
  ...
  /**
   * @param maxWait
   *          Maximum wait for any one {@link Future}, defaults to -1 (forever)
   ...
   *          {@link TimeUnit} for the {@link #setMaxWait(long)} value.
   * @see Future#get(long, TimeUnit)
   */
   ...
   * 
   * @see
   * org.jasig.portal.utils.DirectoryScanner#scanDirectoryWithResults(java.io
   * .File, java.io.FileFilter, com.google.common.base.Function)
   */
  @Override
  public <T> Map<File, T> scanDirectoryWithResults(File directory,
    FileFilter fileFilter, Function<Resource, T> fileProcessor) {
   ...
   * 
   * @see
   * org.jasig.portal.utils.DirectoryScanner#scanDirectoryNoResults(java.io.
   * File, java.io.FileFilter, com.google.common.base.Function)
   */
  @Override
  public void scanDirectoryNoResults(File directory, FileFilter fileFilter,
    Function<Resource, ?> fileProcessor) {
    this.scanDirectory(directory, null, fileFilter, fileProcessor);
  }

  protected <T> void scanDirectory(File directory, ConcurrentMap<File, T> results,
    FileFilter fileFilter, Function<Resource, T> fileProcessor) {
  ...
  protected <T> void recurseOnDirectory(final Queue<Tuple<File, Future<T>>> futures,
    final ConcurrentMap<File, T> results, final File directory,
    final FileFilter fileFilter, final Function<Resource, T> fileProcessor) {
  ...
  protected <T> void submitProcessFile(final Queue<Tuple<File, Future<T>>> futures,
    final File child, final Function<Resource, T> fileProcessor) {
  ...
  protected <T> void cleanFutures(final Queue<Tuple<File, Future<T>>> futures,
    final ConcurrentMap<File, T> results) {
    for (final Iterator<Tuple<File, Future<T>>> futureItr = futures.iterator(); futureItr
      .hasNext();) {
  ...
  protected <T> void waitForFutures(final Queue<Tuple<File, Future<T>>> futures,
    final ConcurrentMap<File, T> results) {
  ...
  protected <T> void processResult(final ConcurrentMap<File, T> results,
    final Tuple<File, Future<T>> future) throws InterruptedException {
    final T result;
    try {
      if (maxWait < 0) {
