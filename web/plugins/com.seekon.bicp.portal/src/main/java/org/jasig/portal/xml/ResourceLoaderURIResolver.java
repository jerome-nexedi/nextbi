...
/**
 * A URIResolver that uses a provided Spring {@link ResourceLoader} to resolve
 * references in a XSL document. Assumes that the provided 'base + href'
 * argument or if base is null 'href' argument can be correctly resolved by the
 * {@link ResourceLoader}
 * 
...
public class ResourceLoaderURIResolver implements URIResolver {
  ...
  protected Source createSource(final Resource resolvedResource, String base)
    ...
    try {
      ...
      throw new TransformerException("Failed to get InputStream for "
        + resolvedResource, e);
    }
  ...
  protected Resource resolveResource(String href, String base)
    ...
    if (base != null) {
      ...
      try {
        ...
        throw new TransformerException("Failed to find '" + href + "' relative to: "
          + baseResource, e);
      }

      this.logger.debug("Created resource {} for href: {} and base: {}",
