...
public class FragmentAdministrationHelper {
  ...
  /**
   * @param legacyConfigurationLoader
   *          the legacyConfigurationLoader to set
   */
  @Autowired
   ...
   */
  public String swapToFragmentOwner(final String remoteUser,
    final String targetFragmentOwner, RequestContext requestContext) {
    IAuthorizationPrincipal principal = authorizationService.newPrincipal(
      remoteUser, IPerson.class);
    if (principal.hasPermission(UP_USERS, IMPERSONATE, targetFragmentOwner)) {
      PortletRequest portletRequest = (PortletRequest) requestContext
        .getExternalContext().getNativeRequest();
      PortletSession session = portletRequest.getPortletSession();
      session.setAttribute(LoginController.SWAP_TARGET_UID, targetFragmentOwner,
