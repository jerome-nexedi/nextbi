...
public abstract class BaseJpaDao implements InitializingBean {
  ...
  /**
   * Subclasses can implement this method to generate {@link CriteriaQuery}
   * objects. Called by {@link #afterPropertiesSet()}
   */
  ...
  protected <T> ParameterExpression<T> createParameterExpression(Class<T> paramClass) {
    ...
    final CriteriaBuilder criteriaBuilder = entityManagerFactory
      .getCriteriaBuilder();

    return criteriaBuilder.parameter(paramClass);
  }

  protected <T> ParameterExpression<T> createParameterExpression(
  ...
  protected <T> CriteriaQuery<T> createCriteriaQuery(
    ...
    entityManager.createQuery(criteriaQuery); // pre-compile critera query to
   ...
   * 
   * @param criteriaQuery
   *          The criteria to create the query from
   */
   ...
   *          The criteria to create the query from
   * @param queryName
   *          A name for the query, used with the query root class name to
   *          generate the cache region name. For example a query with root
   *          class "org.jasig.portal.events.aggr.session.EventSessionImpl" and
   *          queryName "FIND_BY_EVENT_SESSION_ID" will result in a region named"org.jasig.portal.events.aggr.session.EventSessionImpl.query.FIND_BY_EVENT_SESSION_ID"
   */
   ...
   * 
   * @param criteriaQuery
   *          The criteria to create the cache name for
   */
  protected <T> String getCacheRegionName(CriteriaQuery<T> criteriaQuery) {
    ...
    if (roots.size() > 1) {
      logger.warn("Query " + criteriaQuery + " in " + this.getClass() + " has "
        + roots.size()
