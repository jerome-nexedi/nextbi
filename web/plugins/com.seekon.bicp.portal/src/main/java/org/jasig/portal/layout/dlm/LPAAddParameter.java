...
public class LPAAddParameter implements ILayoutProcessingAction {
  ...
  /**
   * Add a parameter for a channel in both the ILF and PLF using the appropriate
   * mechanisms for incorporated nodes versus owned nodes.
   */
  public void perform() throws PortalException {
    ...
    if (nodeId.startsWith(Constants.FRAGMENT_ID_USER_PREFIX)) {
      ...
      ParameterEditManager
        .addParmEditDirective(ilfNode, nodeId, name, value, person);
    } else {
   ...
   * 
   * @param node
   *          the parent node in which to create the parameter.
   * @param name
   *          the name of the parameter to be created.
   * @param value
   *          the value of the parameter to be created.
   */
  static void addParameterChild(Element node, String name, String value) {
    if (node != null) {
      ...
      /*
       * Set the override attribute to 'yes'. This isn't persisted since this is
       * determined upon loading the layout based upon the channel definition.
       * But for code that references this in-memory value like
       * ChannelStaticData it needs to be set.
       */
