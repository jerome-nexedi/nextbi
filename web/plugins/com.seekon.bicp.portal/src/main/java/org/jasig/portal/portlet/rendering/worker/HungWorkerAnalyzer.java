...
public final class HungWorkerAnalyzer {
   ...
   * process. If a WARN message is logged, an INOFO message will not be logged.
   */
   ...
   */
  private volatile Map<String, PortletHungWorkerAnalysisEntry> hungWorkerAnalysis = Collections
    .emptyMap();

  /**
   * The ratio, relative to a portlet's configured timeout value, of how long it
   * can hold onto a rendering thread before being considered errant. Portlets
   * with too many erant threads may be denied further worker threads, thus
   * preventing starvation.
   */
  private double errantThreshold = 2.0D; // Spring-configurable, though the
   ...
   * threads will render an error, but this outcome is better than starving the
   * portal for threads and causing ALL portlets to render errors for all users.
   */
  ...
  /**
   * @param errantThreshold
   *          The ratio, relative to a portlet's configured timeout value, of
   *          how long it can hold onto a rendering thread before being
   *          considered errant. Default is 2.0D (which is probably reasonable
  ...
  public void analyze(final Queue<IPortletExecutionWorker<?>> hungWorkers) {
    ...
    for (final IPortletExecutionWorker<?> worker : hungWorkers) {
      ...
      if (!worker.isComplete()) {
        ...
        if (entry == null) {
          entry = new PortletHungWorkerAnalysisEntry(fname, worker
            .getApplicableTimeout());
    ...
    if (log.isTraceEnabled()) {
      ...
      log
        .trace("Hung worker analysis performed;  analysis completed in (milliseconds):  "
          + runTime);
    }
  ...
  public boolean allowWorkerThreadAllocationForPortlet(final String fname) {
    ...
    /*
     * Setting numberPermittedErrantByFname=0 shuts this feature off and allows
     * portlets to take workers no matter how many errant threads they have.
     */
   ...
   * and how many of them are "errant" or rogue. A worker is considered errant
   * if its render time exceeds its configured timeout multiplied by
  ...
  private/* not-static */final class PortletHungWorkerAnalysisEntry {
    ...
    public PortletHungWorkerAnalysisEntry(final String fname,
    ...
    public String toString() {
      ...
      rslt.append("Portlet '").append(fname).append("' has ").append(
        numberInHungWorkersQueue).append(
        " workers not completing properly, of which ").append(numberErrant).append(
        " are considered errant.  The longest runtime is ").append(longestRunTime)
        .append(" milliseconds.");
