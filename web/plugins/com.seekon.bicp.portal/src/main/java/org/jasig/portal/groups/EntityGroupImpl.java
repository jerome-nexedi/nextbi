 ...
 * The members are cached externally. The rules for controlling access to the
 * key caches are a bit obscure, but you should understand them before writing
 * code that updates groups. Access to the caches themselves is synchronized via
 * the cache getters and setters. All requests to get group members and to add
 * or remove group members ultimately go through these methods. The mutating
 * methods, <code>addMember()</code> and <code>removeMember()</code> however, do
 * a copy-on-write. That is, they first make a copy of the cache, add or remove
 * the member key, and then replace the original cache with the copy. This
 * permits multiple read and write threads to run concurrently without throwing
 * <code>ConcurrentModificationExceptions</code>. But it still leaves open the
 * danger of data races because nothing in this class guarantees serialized
 * write access. You must impose this from without, either via explicit locking
 * (<code>GroupService.getLockableGroup()</code>) or by synchronizing access
 * from the caller.
 * 
...
public class EntityGroupImpl extends GroupMemberImpl implements IEntityGroup {
  ...
  /*
   * The Sets of keys to the members of this group. The
   * <code>IGroupMembers</code> themselves are cached by the service.
   */
  ...
  /**
   * Adds <code>IGroupMember</code> gm to our member <code>Map</code> and
   * conversely, adds <code>this</code> to gm's group <code>Map</code>, after
   * checking that the addition does not violate group rules. Remember that we
   * have added it so we can update the database if necessary.
   * 
   * @param gm
   ...
   * <code>IEntityGroup</code>. If it is a group, it must have a unique name
   * within each of its containing groups and the resulting group must not
   * contain a circular reference. Removed the requirement for unique group
   * names. (03-04-2004, de)
   * 
   * @param gm
   *          org.jasig.portal.groups.IGroupMember
  ...
  /**
   * Returns an <code>Iterator</code> over the <code>Set</code> of this <code>IEntityGroup's</code>
   * recursively-retrieved members that are <code>IEntities</code>.
   * 
  ...
  /**
   * Returns an <code>Iterator</code> over the <code>Set</code> of
   * recursively-retrieved <code>IGroupMembers</code> that are members of this
  ...
  /**
   * Returns an <code>Iterator</code> over this <code>IEntityGroup's</code> members that
   * are <code>IEntities</code>.
   * 
   * @return java.util.Iterator
   */
  ...
  /**
   * Returns this object's type for purposes of caching and locking, as opposed
   * to the underlying entity type.
   * 
   * @return java.lang.Class
   */
  ...
  /**
   * Answers if there are any added memberships not yet committed to the
   * database.
   * 
   * @return boolean
   */
  ...
  /**
   * Answers if there are any deleted memberships not yet committed to the
   * database.
   * 
   * @return boolean
   */
  ...
  /**
   * Answers if there are any added or deleted memberships not yet committed to
   * the database.
   * 
   * @return boolean
   */
  ...
  /**
   * Adds the <code>IGroupMember</code> key to the appropriate member key cache
   * by copying the cache, adding to the copy, and then replacing the original
   * with the copy. At this point, <code>gm</code> does not yet have
   * <code>this</code> in its containing group cache. That cache entry is not
   * added until update(), when changes are committed to the store.
   * 
   * @param gm
   *          org.jasig.portal.groups.IGroupMember
   */
  ...
  /**
   * Returns the <code>Set</code> of <code>IEntities</code> in our member
  ...
  /**
   * Removes the <code>IGroupMember</code> key from the appropriate key cache,
   * by copying the cache, removing the key from the copy and replacing the
   * original with the copy. At this point, <code>gm</code> still has
   * <code>this</code> in its containing groups cache. That cache entry is not
  ...
  /**
   * @param newIndividualGroupService
  ...
  /**
   * @param newMemberKeysInitialized
   *          boolean
   */
