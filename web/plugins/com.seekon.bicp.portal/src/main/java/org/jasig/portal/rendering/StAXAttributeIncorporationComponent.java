 ...
 * @version $Revision$
 */
public class StAXAttributeIncorporationComponent extends
  StAXPipelineComponentWrapper implements BeanNameAware {
  ...
  public CacheKey getCacheKey(HttpServletRequest request,
    ...
    final CacheKey attributeKey = this.attributeSource
  ...
  public PipelineEventReader<XMLEventReader, XMLEvent> getEventReader(
    ...
    // Get the reader from the parent and add the attribute incorporating
    // wrapper
    final PipelineEventReader<XMLEventReader, XMLEvent> cachingEventReader = this.wrappedComponent
      .getEventReader(request, response);
    final XMLEventReader eventReader = new AttributeIncorporatingXMLEventReader(
      request, response, cachingEventReader.getEventReader());

    final Map<String, String> outputProperties = cachingEventReader
      .getOutputProperties();
    return new PipelineEventReaderImpl<XMLEventReader, XMLEvent>(eventReader,
      outputProperties);
  }

  private final class AttributeIncorporatingXMLEventReader extends
    FilteringXMLEventReader {
    private final HttpServletRequest request;

    private final HttpServletResponse response;

    public AttributeIncorporatingXMLEventReader(HttpServletRequest request,
      HttpServletResponse response, XMLEventReader reader) {
      super(reader);
    ...
    protected XMLEvent filterEvent(XMLEvent event, boolean peek) {
      ...
      // Get the additional attributes, return if there are no attributes for
      // this element
      final StartElement startElement = event.asStartElement();
      final Iterator<Attribute> additionalAttributes = attributeSource
        .getAdditionalAttributes(request, response, startElement);
      ...
      for (final Iterator<Attribute> attributes = startElement.getAttributes(); attributes
        .hasNext();) {
      ...
      while (additionalAttributes.hasNext()) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("{} - Adding Attribute {} to event {}", new Object[] {
            beanName, XmlUtilitiesImpl.toString(attribute),
      ...
      final StartElement modifiedStartElement = eventFactory.createStartElement(
        startElement.getName(), mergedAttributes.values().iterator(), startElement
          .getNamespaces());
