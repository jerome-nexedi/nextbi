...
public class ClusterLockServiceImpl implements IClusterLockService {
  ...
  private final ConcurrentMap<String, ReentrantLock> localLocks = new MapMaker()
    .weakValues().makeComputingMap(new Function<String, ReentrantLock>() {
      @Override
  ...
  /**
   * Rate at which {@link IClusterLockDao#updateLock(String)} is called while a
   * mutex is locked, defaults to 500ms
   */
  ...
  /**
   * Maximum duration that a lock can be held, functionally longest duration
   * that the lockFunction can take to execute. Defaults to 15 minutes
   */
   ...
   * 
   * @see
   * org.jasig.portal.concurrency.locking.IClusterLockService#doInTryLock(java
   * .lang.String, com.google.common.base.Function)
   */
  @Override
  public <T> TryLockFunctionResult<T> doInTryLock(final String mutexName,
    Function<String, T> lockFunction) throws InterruptedException {
    /*
     * locking strategy requires 2 threads the caller thread is the 'work
     * thread', it executes the lockFunction an additional 'lock thread' is used
     * to acquire and maintain the database lock
     */
    ...
    try {
      ...
      final DatabaseLockWorker databaseLockWorker = new DatabaseLockWorker(dbLocked,
        mutexName, dbLockLatch, workCompleteLatch);
   ...
   * 
   * @see
   * org.jasig.portal.concurrency.locking.IClusterLockService#isLockOwner(java
   * .lang.String)
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.concurrency.locking.IClusterLockService#isLocked(java.
   * lang.String)
   */
  @Override
  ...
  private final class DatabaseLockWorker implements Callable<Boolean> {
    ...
    private DatabaseLockWorker(AtomicBoolean dbLocked, String mutexName,
      CountDownLatch dbLockLatch, CountDownLatch workCompleteLatch) {
    ...
    public Boolean call() throws Exception {
      try {
        final long lockTimeout = System.currentTimeMillis()
          + maximumLockDuration.getMillis();
        try {
          // Try to acquire the lock, set the success to the dbLocked holder
          this.dbLocked.set(clusterLockDao.getLock(this.mutexName));
        } finally {
          // Signal the work thread that we've attempted to get the DB lock,
          // done in finally so
          // the work thread won't hang if something goes wrong during
          // acquisition
        ...
        // wait for the work to complete using the updateLockRate as the wait
        // duration, if the wait time
        // passes without the work thread signaling completion update the mutex
        // (signal we still have the lock)
        // and wait again
        while (!this.workCompleteLatch.await(updateLockRate.getMillis(),
          TimeUnit.MILLISECONDS)) {
          clusterLockDao.updateLock(this.mutexName);

          if (lockTimeout < System.currentTimeMillis()) {
            // TODO is there some way to force the work thread to stop now that
            // the lock thread is giving up?
            // TODO better exception type?
            throw new RuntimeException(
              "The database lock has been held for more than " + maximumLockDuration
                + ", giving up and releasing the DB lock.");
          }
        }
      } catch (Exception e) {
        logger.warn("DB Lock Worker failed for " + mutexName
  ...
  public static class TryLockFunctionResultImpl<T> implements
    TryLockFunctionResult<T> {
