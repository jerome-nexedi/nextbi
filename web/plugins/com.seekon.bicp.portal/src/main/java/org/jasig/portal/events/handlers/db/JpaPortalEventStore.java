...
/**
 * Stores portal events using JPA/Hibenate no internal batch segmentation is
 * done to the passed list of {@link PortalEvent}s. If a {@link PortalEvent} is
 * not mapped as a persistent entity a message is logged at the WARN level and
...
public class JpaPortalEventStore extends BaseJpaDao implements IPortalEventDao {
   ...
   * {@link #aggregatePortalEvents(DateTime, DateTime, int, FunctionWithoutResult)}
   * , defaults to 1000.
   */
  ...
  public void afterPropertiesSet() throws Exception {
    this.startTimeParameter = this.createParameterExpression(DateTime.class,
      "startTime");
    this.endTimeParameter = this
      .createParameterExpression(DateTime.class, "endTime");

    this.selectQuery = "SELECT e " + "FROM " + PersistentPortalEvent.class.getName()
      + " e " + "WHERE e." + PersistentPortalEvent_.timestamp.getName() + " >= :"
      + this.startTimeParameter.getName() + " " + "AND e."
      + PersistentPortalEvent_.timestamp.getName() + " < :"
      + this.endTimeParameter.getName() + " " + "ORDER BY e."
      + PersistentPortalEvent_.timestamp.getName() + " ASC";

    this.selectUnaggregatedQuery = "SELECT e " + "FROM "
      + PersistentPortalEvent.class.getName() + " e " + "WHERE e."
      + PersistentPortalEvent_.timestamp.getName() + " >= :"
      + this.startTimeParameter.getName() + " " + "AND e."
      + PersistentPortalEvent_.timestamp.getName() + " < :"
      + this.endTimeParameter.getName() + " " + "AND (e."
      + PersistentPortalEvent_.aggregated.getName() + " is null OR e."
      + PersistentPortalEvent_.aggregated.getName() + " = false) " + "ORDER BY e."
      + PersistentPortalEvent_.timestamp.getName() + " ASC";

    this.deleteQuery = "DELETE FROM " + PersistentPortalEvent.class.getName()
      + " e " + "WHERE e." + PersistentPortalEvent_.timestamp.getName() + " < :"
      + this.endTimeParameter.getName();

    this.findNewestPersistentPortalEventTimestampQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<DateTime>>() {
        @Override
        public CriteriaQuery<DateTime> apply(CriteriaBuilder cb) {
          final CriteriaQuery<DateTime> criteriaQuery = cb
            .createQuery(DateTime.class);
          final Root<PersistentPortalEvent> eventRoot = criteriaQuery
            .from(PersistentPortalEvent.class);

          // Get the largest event timestamp
          criteriaQuery.select(cb.greatest(eventRoot
            .get(PersistentPortalEvent_.timestamp)));

          return criteriaQuery;
        }
      });

    this.findOldestPersistentPortalEventTimestampQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<DateTime>>() {
        @Override
        public CriteriaQuery<DateTime> apply(CriteriaBuilder cb) {
          final CriteriaQuery<DateTime> criteriaQuery = cb
          ...
          criteriaQuery.select(cb.least(eventRoot
            .get(PersistentPortalEvent_.timestamp)));
   ...
   * 
   * @see
   * org.jasig.portal.events.handlers.db.IPortalEventDao#storePortalEvent(org
   * .jasig.portal.events.PortalEvent)
   */
  @Override
  @Transactional(value = "rawEvents")
  public void storePortalEvent(PortalEvent portalEvent) {
    final PersistentPortalEvent persistentPortalEvent = this
      .wrapPortalEvent(portalEvent);
   ...
   * 
   * @see
   * org.jasig.portal.events.handlers.db.IPortalEventDao#storePortalEvents(org
   * .jasig.portal.events.PortalEvent[])
   */
  @Override
  @Transactional(value = "rawEvents")
  public void storePortalEvents(PortalEvent... portalEvents) {
    for (final PortalEvent portalEvent : portalEvents) {
      try {
        ...
        this.logger.warn(portalEvent.getClass().getName()
          + " is not mapped as a persistent entity and will not be stored. "
          + portalEvent + " Exception=" + iae.getMessage());
      }
    }
  }
   ...
   * 
   * @see
   * org.jasig.portal.events.handlers.db.IPortalEventDao#storePortalEvents(java
   * .lang.Iterable)
   */
  @Override
  @Transactional(value = "rawEvents")
  ...
  public DateTime getOldestPortalEventTimestamp() {
    final TypedQuery<DateTime> query = this
      .createQuery(this.findOldestPersistentPortalEventTimestampQuery);
  ...
  public DateTime getNewestPortalEventTimestamp() {
    final TypedQuery<DateTime> query = this
      .createQuery(this.findNewestPersistentPortalEventTimestampQuery);
    final List<DateTime> results = query.getResultList();
    return DataAccessUtils.uniqueResult(results);
  }
  ...
  @Override
  @Transactional(value = "rawEvents")
  public void aggregatePortalEvents(DateTime startTime, DateTime endTime,
    int maxEvents, FunctionWithoutResult<PortalEvent> handler) {
    final Session session = this.getEntityManager().unwrap(Session.class);
    final org.hibernate.Query query = session
      .createQuery(this.selectUnaggregatedQuery);
    ...
    int resultCount = 0;
    for (final ScrollableResults results = query.scroll(ScrollMode.FORWARD_ONLY); results
      .next();) {
      final PersistentPortalEvent persistentPortalEvent = (PersistentPortalEvent) results
        .get(0);
      final PortalEvent portalEvent = this.toPortalEvent(persistentPortalEvent
        .getEventData(), persistentPortalEvent.getEventType());
      ...
      if (++resultCount % this.flushPeriod == 0) {
        this.logger.debug("Aggregated {} events, flush and clear session.",
          resultCount);
        session.flush();
        session.clear();
      }
  ...
  @Override
  public void getPortalEvents(DateTime startTime, DateTime endTime,
   ...
   * 
   * @see
   * org.jasig.portal.events.handlers.db.IPortalEventDao#deletePortalEventsBefore
   * (java.util.Date)
   */
  @Override
  @Transactional(value = "rawEvents")
  ...
  protected <E extends PortalEvent> E toPortalEvent(final String eventData,
    Class<E> eventType) {
    try {
