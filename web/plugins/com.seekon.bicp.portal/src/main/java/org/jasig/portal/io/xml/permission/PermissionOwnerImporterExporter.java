...
public class PermissionOwnerImporterExporter extends
  ...
  public Iterable<? extends IPortalData> getPortalData() {
    final List<IPermissionOwner> permissionOwners = this.permissionOwnerDao
      .getAllPermissionOwners();

    return Lists.transform(permissionOwners,
      new Function<IPermissionOwner, IPortalData>() {
        ...
        public IPortalData apply(IPermissionOwner permissionOwner) {
          return new SimpleStringPortalData(permissionOwner.getFname(),
            permissionOwner.getName(), permissionOwner.getDescription());
        }
      });
  }
  ...
  public void importData(ExternalPermissionOwner externalPermissionOwner) {
    ...
    final IPermissionOwner permissionOwner = this.permissionOwnerDao
      .getOrCreatePermissionOwner(name, fname);

    final String desc = externalPermissionOwner.getDesc();
    permissionOwner.setDescription(desc);

    for (final ExternalActivity externalActivity : externalPermissionOwner
      .getActivities()) {
      ...
      final IPermissionActivity permissionActivity = this.permissionOwnerDao
        .getOrCreatePermissionActivity(permissionOwner, activityName, activityFname,
          targetProvider);
  ...
  protected ExternalPermissionOwner convert(final IPermissionOwner permissionOwner) {
    ...
    final List<ExternalActivity> externalActivities = externalPermissionOwner
      .getActivities();
