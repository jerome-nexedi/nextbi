...
/**
 * Used to coordinate cluster wide locking via the database. Tracks the server
 * that currently owns the lock, when the lock started, last updated and
 * released.
 * 
...
class ClusterMutex implements Serializable {
  ...
  void unlock() {
    if (!this.locked) {
      throw new IllegalStateException("Cannot unlock already unlocked mutex: "
        + this);
    }
    this.locked = false;
  ...
  public String toString() {
    return "ClusterMutex [id=" + this.id + ", name=" + this.name + ", locked="
      + this.locked + ", serverId=" + this.serverId + ", lockStart="
      + this.lockStart + ", lastUpdate=" + this.lastUpdate + ", lockEnd="
      + this.lockEnd + "]";
  }
}
