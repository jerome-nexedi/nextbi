...
public abstract class AbstractPermissionsController {
  ...
  protected final boolean isAuthorized(HttpServletRequest req) throws Exception {
    ...
    /*
     * This is sensitive data; we must verify that the user has the appropriate
     * level of access to see it...
     */
    ...
    if (person != null) {
      ...
      if (securityContext != null && securityContext.isAuthenticated()) {
        ...
        // STEP (3): Does this user have SUBSCRIBE permission for
        // permissionsAdminChannel?
        ...
        final IPortletDefinition permissionsAdminPortlet = portletDefinitionRegistry
          .getPortletDefinitionByFname(PERMISSIONS_ADMIN_PORTLET_FNAME);
        if (permissionsAdminPortlet == null) {
          return false;
        }

        final String portletId = permissionsAdminPortlet.getPortletDefinitionId()
          .getStringId();
