...
public interface IUserIdentityStore {
  ...
  /**
   * Returns a unique uPortal key for a user. A boolean flag determines whether
   * or not to auto-create data for a new user.
   * 
   * @param person
   *          person whose portalUID will be returned
   * @param createPortalData
   *          indicates whether to try to create all uPortal data for a new
   *          user.
   ...
   * 
   * @param uPortalUID
   *          The portal ID to find a user name for.
   * @return The user name associated with the specified portal id, null if one
   *         isn't found.
   * @throws Exception
   *           If there are any problems retrieving the user name.
   */
  public String getPortalUserName(int uPortalUID);

  /**
   * Gets a portal user id that is associated with the specified portal user
   * name
   */
  public Integer getPortalUserId(String userName);

  /**
   * @return true if the specified username is used as a default user for any
   *         other user in the portal
   */
