...
/**
 * Provides the default implementation of the window registry, the backing for
 * the storage of IPortletWindow objects is a Map stored in the HttpSession for
 * the user.
 * 
...
public class PortletWindowRegistryImpl implements IPortletWindowRegistry {
  public static final QName PORTLET_WINDOW_ID_ATTR_NAME = new QName(
    "portletWindowId");

  static final char ID_PART_SEPERATOR = '.';

  static final Pattern ID_PART_SEPERATOR_PATTERN = Pattern.compile(Pattern
    .quote(String.valueOf(ID_PART_SEPERATOR)));

  static final String STATELESS_PORTLET_WINDOW_ID = "tw";

  static final String PORTLET_WINDOW_DATA_ATTRIBUTE = PortletWindowRegistryImpl.class
    .getName()
    + ".PORTLET_WINDOW_DATA";

  static final String PORTLET_WINDOW_ATTRIBUTE = PortletWindowRegistryImpl.class
    .getName()
  ...
  private Set<WindowState> persistentWindowStates = Sets
    .newHashSet(WindowState.MINIMIZED);
  ...
  /**
   * The set of WindowStates that should be copied to the {@link IPortletEntity}
   * when {@link #storePortletWindow(HttpServletRequest, IPortletWindow)} is
   * called
   */
  ...
  @Autowired
  public void setStylesheetDescriptorDao(
   ...
   * 
   * @see
   * org.jasig.portal.portlet.registry.IPortletWindowRegistry#convertPortletWindow
   * (javax.servlet.http.HttpServletRequest, org.apache.pluto.PortletWindow)
   */
  @Override
  public IPortletWindow convertPortletWindow(HttpServletRequest request,
    ...
    if (plutoWindowId instanceof IPortletWindowId) {
      ...
      portletWindowId = this
        .getPortletWindowId(request, plutoWindowId.getStringId());
    }
  ...
  public IPortletWindow getOrCreateDefaultPortletWindowByFname(
    ...
    final IPortletEntity portletEntity = this.portletEntityRegistry
      .getOrCreatePortletEntityByFname(request, userInstance, fname);
    if (portletEntity == null) {
      return null;
    }

    final IPortletWindow portletWindow = this.getOrCreateDefaultPortletWindow(
      request, portletEntity.getPortletEntityId());
    logger.trace("Found portlet window {} for portlet definition fname {}",
      portletWindow, fname);
  ...
  public IPortletWindow getOrCreateDefaultPortletWindowByLayoutNodeId(
    ...
    final IUserInstance userInstance = this.userInstanceManager
      .getUserInstance(request);
    final IPortletEntity portletEntity = this.portletEntityRegistry
      .getOrCreatePortletEntity(request, userInstance, subscribeId);
    if (portletEntity == null) {
      logger.debug(
    ...
    final IPortletWindow portletWindow = this.getOrCreateDefaultPortletWindow(
      request, portletEntity.getPortletEntityId());
    logger.trace("Found portlet window {} for layout node {}", portletWindow,
      subscribeId);
  ...
  public IPortletWindow getOrCreateDefaultPortletWindow(HttpServletRequest request,
    ...
    final IPortletWindowId portletWindowId = this.getDefaultPortletWindowId(request,
      portletEntityId);
    ...
    if (portletWindow != null) {
      logger.trace("Found IPortletWindow {} in request cache", portletWindow
        .getPortletWindowId());
      return portletWindow;
    }

    final PortletWindowData portletWindowData = this
      .getOrCreateDefaultPortletWindowData(request, portletEntityId, portletWindowId);
   ...
   * 
   * @see
   * org.jasig.portal.portlet.registry.IPortletWindowRegistry#getPortletWindow
  ...
  public IPortletWindow getPortletWindow(HttpServletRequest request,
    ...
    final PortletWindowIdImpl localPortletWindowId = this.convertPortletWindowId(
      request, portletWindowId);

    // Find the window data from the correct window data map
    final PortletWindowData portletWindowData;
    if (STATELESS_PORTLET_WINDOW_ID.equals(localPortletWindowId
      .getWindowInstanceId())) {
   ...
   * 
   * @see
   * org.jasig.portal.portlet.registry.IPortletWindowRegistry#getPortletWindowId
   * (java.lang.String)
   */
  @Override
  public PortletWindowIdImpl getPortletWindowId(HttpServletRequest request,
    String portletWindowId) {
    Validate.notNull(portletWindowId, "portletWindowId can not be null");

    final String[] portletWindowIdParts = ID_PART_SEPERATOR_PATTERN
      .split(portletWindowId);
    ...
    if (portletWindowIdParts.length == 1) {
      ...
      throw new IllegalArgumentException("Provided portlet window ID '"
        + portletWindowId + "' is not valid");
    }

    final IPortletEntity portletEntity = this.portletEntityRegistry
      .getPortletEntity(request, entityIdStr);
    if (portletEntity == null) {
      throw new IllegalArgumentException("No parent IPortletEntity found for id '"
        + entityIdStr + "' from portlet window id: " + portletWindowId);
    }
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.portlet.registry.IPortletWindowRegistry#
   * getDefaultPortletWindowId(org.jasig.portal.portlet.om.IPortletEntityId)
   */
  @Override
  public IPortletWindowId getDefaultPortletWindowId(HttpServletRequest request,
    IPortletEntityId portletEntityId) {
    final IPortletWindowId portletWindowId = this.createPortletWindowId(null,
      portletEntityId);
    logger.trace("Determined default portlet window id {} for portlet entity {}",
  ...
  public IPortletWindow createDelegatePortletWindow(HttpServletRequest request,
    IPortletEntityId portletEntityId, IPortletWindowId delegationParentId) {
    Validate.notNull(request, "request can not be null");
    Validate.notNull(portletEntityId, "portletEntityId can not be null");

    // TODO does a delegate portlet entity need some sort of special treatment
    // or do we just assume that the calling code is using one?
    ...
    final PortletWindowData portletWindowData = this
      .getOrCreateDefaultPortletWindowData(request, portletEntityId,
        portletWindowId, delegationParentId);
    portletWindow = wrapPortletWindowData(request, portletWindowData);
    if (portletWindow == null) {
      return null;
    }
  ...
  public Set<IPortletWindow> getAllPortletWindowsForEntity(
    ...
    // Check for session cached windows that haven't been accessed in this
    // request
    final PortletWindowCache<PortletWindowData> portletWindowDataMap = this
      .getPortletWindowDataMap(request);
    this.addPortletWindowData(request, portletEntityId, portletWindows,
      portletWindowMap, portletWindowDataMap);
    ...
    if (statelessPortletWindowDataMap != null) {
      this.addPortletWindowData(request, portletEntityId, portletWindows,
        portletWindowMap, statelessPortletWindowDataMap);
    }
  ...
  public Tuple<IPortletWindow, StartElement> getPortletWindow(
    ...
    final Attribute windowIdAttribute = element
      .getAttributeByName(PORTLET_WINDOW_ID_ATTR_NAME);
    if (windowIdAttribute != null) {
      final String windowIdStr = windowIdAttribute.getValue();
      final IPortletWindowId portletWindowId = this.getPortletWindowId(request,
        windowIdStr);
      final IPortletWindow portletWindow = this.getPortletWindow(request,
        portletWindowId);
    ...
    final Attribute nodeIdAttribute = element
      .getAttributeByName(IUserLayoutManager.ID_ATTR_NAME);
    final String layoutNodeId = nodeIdAttribute.getValue();

    IPortletWindow portletWindow = this
      .getOrCreateDefaultPortletWindowByLayoutNodeId(request, layoutNodeId);
    if (portletWindow == null) {
    ...
    final IPortalRequestInfo portalRequestInfo = this.urlSyntaxProvider
      .getPortalRequestInfo(request);
    if (portalRequestInfo.getUrlState() == UrlState.DETACHED) {
      // Handle detached portlets explicitly
      // TODO Can we ever have non-targeted portlets render in a detached
      // request? If so should they all be stateless windows anyways?
      final IPortletWindowId portletWindowId = portletWindow.getPortletWindowId();
      portletWindow = this.getOrCreateStatelessPortletWindow(request,
        portletWindowId);
    }
  ...
  protected StartElement addPortletWindowId(StartElement element,
    IPortletWindowId portletWindowId) {
    final Attribute windowIdAttribute = xmlEventFactory.createAttribute(
    ...
    // Create a new iterator of the existing attributes + the new window id
    // attribute
    final Iterator<Attribute> newAttributes = Iterators.concat(attributes, Iterators
      .forArray(windowIdAttribute));

    return xmlEventFactory.createStartElement(prefix, namespaceURI, localPart,
      newAttributes, namespaces, namespaceContext);
  }
   ...
   */
  protected void addPortletWindowData(HttpServletRequest request,
    IPortletEntityId portletEntityId, final Set<IPortletWindow> portletWindows,
    final PortletWindowCache<IPortletWindow> portletWindowMap,
    final PortletWindowCache<PortletWindowData> portletWindowDataMap) {

    final Set<PortletWindowData> windows = portletWindowDataMap
      .getWindows(portletEntityId);
    if (windows == null) {
      return;
    }

    for (final PortletWindowData portletWindowData : windows) {
      final IPortletWindowId portletWindowId = portletWindowData
        .getPortletWindowId();

      // Skip data windows that aren't for this entity and for windows that are
      // already in the request cache
      ...
      IPortletWindow portletWindow = this.wrapPortletWindowData(request,
        portletWindowData);
  ...
  public IPortletWindow getOrCreateStatelessPortletWindow(
    ...
    // Need the basePortletWindowId to be an instance of PortletWindowIdImpl so
    // that we can extract the entity ID
    ...
    final IPortletEntityId portletEntityId = ((PortletWindowIdImpl) basePortletWindowId)
      .getPortletEntityId();

    // Create the stateless ID
    final PortletWindowIdImpl statelessPortletWindowId = this.createPortletWindowId(
      STATELESS_PORTLET_WINDOW_ID, portletEntityId);

    // See if there is already a request cached stateless window
    IPortletWindow statelessPortletWindow = this.getPortletWindow(request,
      statelessPortletWindowId);
    if (statelessPortletWindow != null) {
      return statelessPortletWindow;
    }

    // Lookup the base portlet window to clone the stateless from
    final IPortletWindow basePortletWindow = this.getPortletWindow(request,
      basePortletWindowId);
    ...
    if (basePortletWindow == null) {
      final IPortletEntity portletEntity = this.portletEntityRegistry
        .getPortletEntity(request, portletEntityId);
      if (portletEntity == null) {
        throw new IllegalArgumentException("No IPortletEntity could be found for "
          + portletEntity + " while creating stateless portlet window for "
          + basePortletWindowId);
      }
      ...
      final PortletWindowData portletWindowData = new PortletWindowData(
        statelessPortletWindowId, portletEntityId);
      statelessPortletWindowDataMap.storeWindow(portletWindowData);

      statelessPortletWindow = new StatelessPortletWindowImpl(portletWindowData,
        portletEntity, portletDescriptor);
    }
    // Clone the existing base window
      ...
      portletWindowData.setPublicRenderParameters(basePortletWindow
      ...
      final PortletDefinition portletDescriptor = basePortletWindow
        .getPlutoPortletWindow().getPortletDefinition();
      statelessPortletWindow = new StatelessPortletWindowImpl(portletWindowData,
        portletEntity, portletDescriptor);
    }

    // Cache the stateless window in the request
    final PortletWindowCache<IPortletWindow> portletWindowMap = this
      .getPortletWindowMap(request);
  ...
  @Override
  public void storePortletWindow(HttpServletRequest request,
    ...
    final IStylesheetDescriptor themeStylesheetDescriptor = this
      .getThemeStylesheetDescriptor(request);

    final WindowState windowState = portletWindow.getWindowState();

    final IPortletEntity portletEntity = portletWindow.getPortletEntity();
    final WindowState entityWindowState = portletEntity
      .getWindowState(themeStylesheetDescriptor);

    // If the window and entity states are different
    if (windowState != entityWindowState && !windowState.equals(entityWindowState)) {
      final WindowState defaultWindowState = this
        .getDefaultWindowState(themeStylesheetDescriptor);

      // If a window state is set and is one of the persistent states set it on
      // the entity
  ...
  public Set<IPortletWindow> getAllLayoutPortletWindows(HttpServletRequest request) {
    ...
    final Set<String> allSubscribedChannels = userLayoutManager
    ...
    for (final String channelSubscribeId : allSubscribedChannels) {
      final IPortletEntity portletEntity = this.portletEntityRegistry
        .getOrCreatePortletEntity(request, userInstance, channelSubscribeId);
      if (portletEntity == null) {
        this.logger.debug("No portlet entity found for layout node {} for user {}",
          channelSubscribeId, userInstance.getPerson().getUserName());
        continue;
      }
  ...
  public Set<IPortletWindow> getAllPortletWindows(HttpServletRequest request) {
    ...
    for (final String channelSubscribeId : allSubscribedChannels) {
      ...
      final IPortletEntityId portletEntityId = portletEntity.getPortletEntityId();
      final Set<IPortletWindow> portletWindows = this.getAllPortletWindowsForEntity(
        request, portletEntityId);
  ...
  protected IPortletWindow wrapPortletWindowData(HttpServletRequest request,
    ...
    final IPortletWindow portletWindow = new PortletWindowImpl(portletDescriptor,
      portletEntity, portletWindowData);

    logger.trace("Wrapping PortletWindowData {} as IPortletWindow", portletWindow
      .getPortletWindowId());

    return portletWindow;
  }

  protected PortletWindowData getPortletWindowData(HttpServletRequest request,
    IPortletWindowId portletWindowId) {
    final PortletWindowCache<PortletWindowData> portletWindowDataMap = getPortletWindowDataMap(
      request, false);
    if (portletWindowDataMap == null) {
      return null;
    }

    final PortletWindowData portletWindowData = portletWindowDataMap
      .getWindow(portletWindowId);
    if (portletWindowData == null) {
      logger.trace("No PortletWindowData {} in session cache", portletWindowId);
      return null;
    }

    logger.trace("Found PortletWindowData {} in session cache", portletWindowData
      .getPortletWindowId());
    return portletWindowData;
  }

  protected PortletWindowCache<IPortletWindow> getPortletWindowMap(
    HttpServletRequest request) {
    request = portalRequestUtils.getOriginalPortletOrPortalRequest(request);

    final String mapAttributeName = PORTLET_WINDOW_ATTRIBUTE
      + Thread.currentThread().getId();

    // No need to do this in a request attribute mutex since the map is scoped
  ...
  protected PortletWindowData getOrCreateDefaultPortletWindowData(
    HttpServletRequest request, IPortletEntityId portletEntityId,
    IPortletWindowId portletWindowId) {
    return getOrCreateDefaultPortletWindowData(request, portletEntityId,
      portletWindowId, null);
  }

  protected PortletWindowData getOrCreateDefaultPortletWindowData(
    HttpServletRequest request, IPortletEntityId portletEntityId,
    IPortletWindowId portletWindowId, IPortletWindowId delegationParentId) {
    // Sync on session map to make sure duplicate PortletWindowData is never
    // created
    ...
    PortletWindowData portletWindowData = portletWindowDataMap
      .getWindow(portletWindowId);
    if (portletWindowData != null) {
      logger.trace("Found PortletWindowData {} in session cache", portletWindowData
        .getPortletWindowId());
  ...
  /**
   * Initializes a newly created {@link PortletWindow}, the default
   * implementation sets up the appropriate {@link WindowState} and
   * {@link javax.portlet.PortletMode}
   */
  protected void initializePortletWindowData(HttpServletRequest request,
    PortletWindowData portletWindowData) {
    ...
    final IPortletEntity portletEntity = this.portletEntityRegistry
      .getPortletEntity(request, portletEntityId);
    final WindowState entityWindowState = portletEntity
      .getWindowState(stylesheetDescriptor);
    if (persistentWindowStates.contains(entityWindowState)) {
      ...
      this.logger
        .warn("PortletEntity.windowState="
          + entityWindowState
  ...
  protected WindowState getDefaultWindowState(
  ...
  protected IStylesheetDescriptor getThemeStylesheetDescriptor(
    HttpServletRequest request) {
    final IUserInstance userInstance = this.userInstanceManager
      .getUserInstance(request);
    final IUserPreferencesManager preferencesManager = userInstance
  ...
  /**
   * Generates a new, unique, portlet window ID for the window instance ID &
   * entity id.
   * 
   ...
   */
  protected PortletWindowIdImpl createPortletWindowId(String windowInstanceId,
    IPortletEntityId portletEntityId) {
    final StringBuilder compositeIdString = new StringBuilder(portletEntityId
      .getStringId());
