...
/**
 * Base class for {@link XMLEventReader}s for classes that want to inject
 * additional events into the stream.
 * 
...
public abstract class InjectingXMLEventReader extends BaseXMLEventReader {
  ...
  protected final XMLEvent internalNextEvent() throws XMLStreamException {
    ...
    if (this.additionalEvents != null) {
      // Stick the current event at the bottom of the deque so it isn't
      // forgotten
   ...
   * @param event
   *          The current event
   * @return Any additional events that should be injected before the current
   *         event. If null the current event is returned
   */
  ...
  /**
   * Called by {@link #getAdditionalEvents(XMLEvent)} and then wrapped with a
   * {@link Deque}. If there is a need to inject more than a single event
   * override {@link #getAdditionalEvents(XMLEvent)}
   */
   ...
   * @param event
   *          The peeked event
   * @return An event to return in place of the peeked event, if null the peeked
   *         event is returned.
   */
