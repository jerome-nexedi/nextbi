...
/**
 * A composite key and type that uniquely identify a portal entity. The
 * composite key contains a service name, which may be compound, and a native
 * key, which is the key that identifies the entity in the local service.
 * 
...
public class CompositeEntityIdentifier extends org.jasig.portal.EntityIdentifier
  ...
  private static final LoadingCache<String, Name> NAME_PARSE_CACHE = CacheBuilder
    .newBuilder().maximumSize(1000).build(new CacheLoader<String, Name>() {
      @Override
  ...
  /**
   * If the composite key is either empty or has a single node, there is no
   * service name.
   * 
   * @return javax.naming.Name
   */
