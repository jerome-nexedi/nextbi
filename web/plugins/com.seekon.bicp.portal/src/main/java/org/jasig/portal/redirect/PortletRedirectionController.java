 ...
 * URL to the configured target URL. This controller can be used to map links
 * from one portlet to another without requiring the linking portlet know the
 * entire URL structure of the target.
 * 
...
public class PortletRedirectionController {
  ...
  public void redirect(HttpServletRequest request, HttpServletResponse response,
    @PathVariable String serviceKey) throws IOException {
  ...
  protected String getUrlString(IRedirectionUrl url, HttpServletRequest request) {
    ...
    if (url instanceof ExternalRedirectionUrl) {
      ...
      try {
        ...
        String separator = "?";
        for (Map.Entry<String, String[]> param : externalUrl
        ...
        for (Map.Entry<String, String> param : externalUrl.getDynamicParameters()
          .entrySet()) {
        ...
        log
          .error("Unable to encode URL parameter for external service redirect", ex);
        return null;
      }
      ...
      final IPortalUrlBuilder portalUrlBuilder = this.portalUrlProvider
        .getPortalUrlBuilderByPortletWindow(request, portletWindow
          .getPortletWindowId(), portletUrl.getType());
      ...
      for (Map.Entry<String, String[]> param : portletUrl.getAdditionalParameters()
        .entrySet()) {
      ...
      for (Map.Entry<String, String> param : portletUrl.getDynamicParameters()
        .entrySet()) {
        String[] values = request.getParameterValues(param.getKey());
        if (values != null) {
