 ...
 * {@link CacheControl}s are stored in a {@link Map} stored as a
 * {@link HttpServletRequest} attribute.
 * 
...
public class PortletCacheControlServiceImpl implements IPortletCacheControlService,
  ...
  private static final String SESSION_ATTRIBUTE__PORTLET_RENDER_HEADER_CACHE_KEYS_MAP = PortletCacheControlServiceImpl.class
    .getName()
    + ".PORTLET_RENDER_HEADER_CACHE_KEYS_MAP";

  private static final String SESSION_ATTRIBUTE__PORTLET_RENDER_CACHE_KEYS_MAP = PortletCacheControlServiceImpl.class
    .getName()
    + ".PORTLET_RENDER_CACHE_KEYS_MAP";

  private static final String SESSION_ATTRIBUTE__PORTLET_RESOURCE_CACHE_KEYS_MAP = PortletCacheControlServiceImpl.class
    .getName()
    + ".PORTLET_RESOURCE_CACHE_KEYS_MAP";

  protected final Logger logger = LoggerFactory.getLogger(this.getClass());

  // Used to coordinate mass-purge of cached data for portlets at action/event
  // requests
  ...
  private final PrivatePortletCacheKeyTracker privatePortletRenderHeaderCacheKeyTracker = new PrivatePortletCacheKeyTracker(
    SESSION_ATTRIBUTE__PORTLET_RENDER_HEADER_CACHE_KEYS_MAP);

  private final PrivatePortletCacheKeyTracker privatePortletRenderCacheKeyTracker = new PrivatePortletCacheKeyTracker(
    SESSION_ATTRIBUTE__PORTLET_RENDER_CACHE_KEYS_MAP);

  private final PrivatePortletCacheKeyTracker privatePortletResourceCacheKeyTracker = new PrivatePortletCacheKeyTracker(
    SESSION_ATTRIBUTE__PORTLET_RESOURCE_CACHE_KEYS_MAP);
  ...
  public void setPrivateScopePortletRenderHeaderOutputCache(
    ...
    this.privateScopePortletRenderHeaderOutputCache
      .getCacheEventNotificationService().registerListener(
        privatePortletRenderHeaderCacheKeyTracker);
  }
  ...
  public void setPublicScopePortletRenderHeaderOutputCache(
    ...
    this.publicScopePortletRenderHeaderOutputCache
      .getCacheEventNotificationService().registerListener(
        publicPortletRenderHeaderCacheKeyTracker);
  }
  ...
  /**
   * @param cacheSizeThreshold
   *          the cacheSizeThreshold to set in bytes
   */
  ...
  public CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> getPortletRenderHeaderState(
    ...
    final PublicPortletCacheKey publicCacheKey = PublicPortletCacheKey
      .createPublicPortletRenderHeaderCacheKey(definitionId, portletWindow, locale);

    return this
      .<CachedPortletData<PortletRenderResult>, PortletRenderResult> getPortletState(
        request, portletWindow, publicCacheKey,
        this.publicScopePortletRenderHeaderOutputCache,
        this.privateScopePortletRenderHeaderOutputCache, false);
  }
  ...
  public CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> getPortletRenderState(
    ...
    final Locale locale = RequestContextUtils.getLocale(request);
    final PublicPortletCacheKey publicCacheKey = PublicPortletCacheKey
      .createPublicPortletRenderCacheKey(definitionId, portletWindow, locale);

    return this
      .<CachedPortletData<PortletRenderResult>, PortletRenderResult> getPortletState(
        request, portletWindow, publicCacheKey,
        this.publicScopePortletRenderOutputCache,
        this.privateScopePortletRenderOutputCache, false);
  }
  ...
  public CacheState<CachedPortletResourceData<Long>, Long> getPortletResourceState(
    ...
    final Locale locale = RequestContextUtils.getLocale(request);
    final PublicPortletCacheKey publicCacheKey = PublicPortletCacheKey
      .createPublicPortletResourceCacheKey(definitionId, portletWindow, resourceId,
        locale);

    return this.<CachedPortletResourceData<Long>, Long> getPortletState(request,
      portletWindow, publicCacheKey, this.publicScopePortletResourceOutputCache,
      this.privateScopePortletResourceOutputCache, true);
  }

  private <D extends CachedPortletResultHolder<T>, T extends Serializable> CacheState<D, T> getPortletState(
    HttpServletRequest request, IPortletWindow portletWindow,
    PublicPortletCacheKey publicCacheKey, Ehcache publicOutputCache,
    Ehcache privateOutputCache, boolean useHttpHeaders) {
    ...
    final CacheState<D, T> cacheState = this.<D, T> getPortletCacheState(request,
      portletWindow, publicCacheKey, publicOutputCache, privateOutputCache);
      ...
      if (useHttpHeaders) {
        ...
        if (etagHeader != null && etagHeader.equals(cachedPortletData.getEtag())) {
          ...
          if (ifModifiedSince >= 0
            ...
            // Cached content hasn't been modified since header date, mark the
            // browser data as matching
            cacheState.setBrowserDataMatches(true);
          }
        }
      }
  ...
  /**
   * Get the cached portlet data looking in both the public and then private
   * caches returning the first found
   * 
   * @param request
   *          The current request
   * @param portletWindow
   *          The window to get data for
   * @param publicCacheKey
   *          The public cache key
   * @param publicOutputCache
   *          The public cache
   * @param privateOutputCache
   *          The private cache
   */
  @SuppressWarnings("unchecked")
  protected <D extends CachedPortletResultHolder<T>, T extends Serializable> CacheState<D, T> getPortletCacheState(
    HttpServletRequest request, IPortletWindow portletWindow,
    PublicPortletCacheKey publicCacheKey, Ehcache publicOutputCache,
    ...
    D cachedPortletData = (D) this.getCachedPortletData(publicCacheKey,
      publicOutputCache, portletWindow);
    if (cachedPortletData != null) {
    ...
    cachedPortletData = (D) this.getCachedPortletData(privateCacheKey,
      privateOutputCache, portletWindow);
    if (cachedPortletData != null) {
      cacheState.setCachedPortletData(cachedPortletData);
      return cacheState;
    }
  ...
  /**
   * Get the cached portlet data for the key, cache and window. If there is
   * {@link CachedPortletData} in the cache it will only be returned if
   * {@link CachedPortletData#isExpired()} is false or
   ...
   *          The cache
   * @param portletWindow
   *          The portlet window the lookup is for
  ...
  protected <T extends Serializable> CachedPortletResultHolder<T> getCachedPortletData(
    ...
    final CachedPortletResultHolder<T> cachedPortletData = (CachedPortletResultHolder<T>) publicCacheElement
      .getValue();
    ...
    logger.debug("Returning cached output with key {} for {}", cacheKey,
      portletWindow);
   ...
   */
  private String getResourceId(IPortletWindowId portletWindowId,
    HttpServletRequest httpRequest) {
    final IPortalRequestInfo portalRequestInfo = this.urlSyntaxProvider
      .getPortalRequestInfo(httpRequest);
    final Map<IPortletWindowId, ? extends IPortletRequestInfo> portletRequestInfoMap = portalRequestInfo
      .getPortletRequestInfoMap();
    final IPortletRequestInfo portletRequestInfo = portletRequestInfoMap
      .get(portletWindowId);
  ...
  public void cachePortletResourceOutput(IPortletWindowId portletWindowId,
    ...
    cachePortletOutput(portletWindowId, httpRequest, cacheState,
      cachedPortletResourceData, this.publicScopePortletResourceOutputCache,
  ...
  private <D extends CachedPortletResultHolder<T>, T extends Serializable> void cachePortletOutput(
    ...
    CacheState<D, T> cacheState, D cachedPortletData, Ehcache publicOutputCache,
    Ehcache privateOutputCache) {

    final IPortletWindow portletWindow = this.portletWindowRegistry
      .getPortletWindow(httpRequest, portletWindowId);
    final CacheControl cacheControl = cacheState.getCacheControl();

    if (cacheControl.isPublicScope()) {
      final PublicPortletCacheKey publicCacheKey = cacheState
        .getPublicPortletCacheKey();
      this.cacheElement(publicOutputCache, publicCacheKey, cachedPortletData,
        cacheControl);
      logger.debug("Cached public data under key {} for {}", publicCacheKey,
        portletWindow);
    } else {
      PrivatePortletCacheKey privateCacheKey = cacheState
        .getPrivatePortletCacheKey();

      // Private key can be null if getPortletState found publicly cached data
      // but the portlet's response is now privately scoped
      if (privateCacheKey == null) {
        ...
        final PublicPortletCacheKey publicCacheKey = cacheState
          .getPublicPortletCacheKey();
        privateCacheKey = new PrivatePortletCacheKey(sessionId, portletWindowId,
          entityId, publicCacheKey);
      }

      this.cacheElement(privateOutputCache, privateCacheKey, cachedPortletData,
        cacheControl);
      logger.debug("Cached private data under key {} for {}", privateCacheKey,
        portletWindow);
    }
  }
   ...
   * The element's ttl will be set depending on whether expiration or validation
   * method is indicated from the CacheControl and the cache's configuration.
   */
  protected void cacheElement(Ehcache cache, Serializable cacheKey,
    CachedPortletResultHolder<?> data, CacheControl cacheControl) {
    // using validation method, ignore expirationTime and defer to cache
    // configuration
    ...
    // using expiration method, -1 for CacheControl#expirationTime means
    // "forever" (e.g. ignore and defer to cache configuration)
    ...
    // using expiration method with a positive expiration, set that value as the
    // element's TTL if it is lower than the configured cache TTL
  ...
  public boolean purgeCachedPortletData(IPortletWindowId portletWindowId,
    ...
    removed = this.purgePublicCache(this.publicPortletRenderHeaderCacheKeyTracker,
      this.publicScopePortletRenderHeaderOutputCache, definitionId)
      || removed;
    ...
    removed = this.purgePublicCache(this.publicPortletRenderCacheKeyTracker,
      this.publicScopePortletRenderOutputCache, definitionId)
      || removed;
    ...
    removed = this.purgePublicCache(this.publicPortletResourceCacheKeyTracker,
      this.publicScopePortletResourceOutputCache, definitionId)
      || removed;
    ...
    removed = this.purgePrivateCache(this.privatePortletRenderHeaderCacheKeyTracker,
      this.privateScopePortletRenderHeaderOutputCache, session, portletWindowId)
      || removed;

    // Remove all privately cached render data
    removed = this.purgePrivateCache(this.privatePortletRenderCacheKeyTracker,
      this.privateScopePortletRenderOutputCache, session, portletWindowId)
      || removed;

    // Remove all privately cached render data
    removed = removed
      || this.purgePrivateCache(this.privatePortletResourceCacheKeyTracker,
        this.privateScopePortletResourceOutputCache, session, portletWindowId);

    // If any keys were found remove them
    return removed;
  }

  protected boolean purgePublicCache(PublicPortletCacheKeyTracker tracker,
    Ehcache cache, IPortletDefinitionId definitionId) {
  ...
  protected boolean purgePrivateCache(PrivatePortletCacheKeyTracker tracker,
    Ehcache cache, HttpSession session, IPortletWindowId portletWindowId) {
    final Set<PrivatePortletCacheKey> keys = tracker.getCacheKeys(session,
      portletWindowId);
    final boolean removed = !keys.isEmpty();
    cache.removeAll(keys);
