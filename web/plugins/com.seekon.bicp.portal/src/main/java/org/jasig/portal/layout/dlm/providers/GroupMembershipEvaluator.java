 ...
 * 
 * Added support for a 'deepMemberOf' mode, from 2.5 patches (UP-1284) and cache
 * group key rather than the group itself (UP-1532). d.e 2006/12/19.
 * 
...
public class GroupMembershipEvaluator extends Evaluator {
  ...
  private String getGroupKey() {
    ...
    try {
      groups = GroupService.searchForGroups(groupName, IGroupConstants.IS,
  ...
  private IEntityGroup getGroup(String key) {
    try {
      ...
      throw new RuntimeException("An exception occurred retrieving " + "the group "
        + groupName + ".", e);
    }
  }
  ...
  public boolean isApplicable(IPerson p) {
    ...
    try {
      ...
      throw new RuntimeException("Unable to determine if user " + p.getFullName()
  ...
  public void toElement(Element parent) {
    ...
    case DEEP_MEMBER_OF_MODE:
      ...
      break;
    default:
      throw new IllegalStateException("Unrecognized evaluatorMode constant:  "
        + this.evaluatorMode);
    }
