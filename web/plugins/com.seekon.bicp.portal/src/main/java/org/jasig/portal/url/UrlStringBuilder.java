...
public final class UrlStringBuilder implements Serializable, Cloneable {
   ...
   *          The encoding to use for parameters
   * @param protocol
   *          The protocol for the URL, ex: http
   * @param host
  ...
  public UrlStringBuilder(UrlStringBuilder urlBuilder) {
    ...
    for (final Map.Entry<String, List<String>> paramEntry : urlBuilder.parameters
      .entrySet()) {
  ...
  protected String encode(String s) {
    try {
      ...
      throw new IllegalStateException("Encoding '" + this.encoding
        + "' is not supported. This should have been caught in the consructor.", e);
    }
  }
   ...
   * @param values
   *          Values for the parameter, null is valid
   * @return this
   */
  ...
  /**
   * Adds to a URL parameter, if a parameter with the same name already exists
   * its values are added to
   * 
   * @param name
   *          Parameter name, can not be null
   * @param values
   ...
   * 
   * @param namespace
   *          String to prepend to each parameter name in the Map
   * @param parameters
   *          Map of parameters to set
   * @return this
   */
  public UrlStringBuilder setParameters(String namespace,
  ...
  /**
   * Adds the contents of the specified Map as the parameters, the values of the
   * Map are List<String>
   * 
   * @param namespace
   *          String to prepend to each parameter name in the Map
   ...
   * @return this
   */
  public UrlStringBuilder addParameters(String namespace,
    Map<String, List<String>> parameters) {
    for (final String name : parameters.keySet()) {
      Validate.notNull(name, "parameter map cannot contain any null keys");
    }
   ...
   * @return this
   */
  public UrlStringBuilder addParametersArray(String namespace,
  ...
  /**
   * Removes any existing path elements and sets the provided elements as the
   * path
   * 
   * @param elements
   *          Path elements to set
   * @return this
   */
  ...
  public String toString() {
    ...
    if (this.parameters.size() > 0) {
      ...
      for (final Iterator<Map.Entry<String, List<String>>> paramEntryItr = this.parameters
        .entrySet().iterator(); paramEntryItr.hasNext();) {
        ...
        if (values == null || values.size() == 0) {
          ...
          for (final Iterator<String> valueItr = values.iterator(); valueItr
            .hasNext();) {
