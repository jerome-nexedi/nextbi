...
/**
 * Worker used to execute render requests on the error portlet. Does not use any
 * thread-pool code to make sure the error portlet still renders in the event of
 * the thread pool being broken.
 * 
...
final class PortletFailureExecutionWorker implements IPortletFailureExecutionWorker {
  ...
  public PortletFailureExecutionWorker(IPortletRenderer portletRenderer,
    List<IPortletExecutionInterceptor> interceptors, HttpServletRequest request,
    HttpServletResponse response, IPortletWindowId errorPortletWindowId,
    IPortletWindowId failedPortletWindowId, String failedPortletFname,
    Exception cause) {
  ...
  protected synchronized void renderError(long timeout) {
    ...
    final PortletHttpServletRequestWrapper wrappedRequest = new PortletHttpServletRequestWrapper(
      request);
    ...
    // Aggressive exception handling to make sure at least something is written
    // out when an error happens.
    try {
      final String characterEncoding = response.getCharacterEncoding();
      final RenderPortletOutputHandler renderPortletOutputHandler = new RenderPortletOutputHandler(
        characterEncoding);

      this.portletRenderResult = this.portletRenderer.doRenderMarkup(
