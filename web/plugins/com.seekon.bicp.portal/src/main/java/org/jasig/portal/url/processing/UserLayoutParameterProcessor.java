...
public class UserLayoutParameterProcessor implements IRequestParameterProcessor {
  ...
  public boolean processParameters(HttpServletRequest request,
    ...
    if (tabId != null) {
      this.stylesheetUserPreferencesService.setStylesheetParameter(request,
        PreferencesScope.STRUCTURE, "focusedTabID", tabId);
    }
    ...
    case DETACHED:
      this.stylesheetUserPreferencesService.setStylesheetParameter(request,
        PreferencesScope.STRUCTURE, "detached", Boolean.TRUE.toString());
    case MAX: {
      ...
      if (portletRequestInfo != null) {
        ...
        final IPortletWindow portletWindow = this.portletWindowRegistry
          .getPortletWindow(request, targetWindowId);
        final IPortletEntity portletEntity = portletWindow.getPortletEntity();

        final String channelSubscribeId = portletEntity.getLayoutNodeId();

        this.stylesheetUserPreferencesService.setStylesheetParameter(request,
          PreferencesScope.STRUCTURE, "userLayoutRoot", channelSubscribeId);
    ...
    default: {
      this.stylesheetUserPreferencesService.setStylesheetParameter(request,
        PreferencesScope.STRUCTURE, "userLayoutRoot", IUserLayout.ROOT_NODE_NAME);
      break;
    }
    }
  ...
  protected String findTabIndex(IUserLayoutManager userLayoutManager, String tabId) {
    ...
    final Enumeration<String> rootsChildren = userLayoutManager
      .getChildIds(rootFolderId);

    int tabIndex = 0;
    for (String topNodeId = rootsChildren.nextElement(); rootsChildren
      .hasMoreElements(); topNodeId = rootsChildren.nextElement()) {
      tabIndex++;
