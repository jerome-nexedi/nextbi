 ...
 * XML definition file. Once parsing is complete the generated objects are
 * availabe via {@link #getTables()}
 * 
...
public class TableXmlHandler extends BaseDbXmlHandler implements ITableDataProvider {
  ...
  public void endElement(String uri, String localName, String name)
    ...
    if ("table".equals(name)) {
      ...
      this.tableColumnTypes
        .put(this.currentTable.getName(), this.currentColumnTypes);
      ...
      this.logger
        .warn("the 'key' element is ignored, use the table level 'primary-key' element instead");
    }
  ...
  protected int getSqlType(final String sqlTypeName) {
    try {
      ...
      throw new RuntimeException("Cannot access field '" + sqlTypeName + "' on "
        + Types.class + " for column '" + this.currentColumn.getName() + "'", e);
    } catch (NoSuchFieldException e) {
      throw new IllegalArgumentException("No SQL Type field '" + sqlTypeName
        + "' on " + Types.class + " for column '" + this.currentColumn.getName()
        + "'", e);
  ...
  protected String getHibernateType(final int sqlType) {
    ...
    try {
      ...
      throw new IllegalArgumentException("No mapped hibernate type found for '"
        + sqlType + "' Types value=" + sqlType + " for column '"
        + this.currentColumn.getName() + "'", e);
    }
