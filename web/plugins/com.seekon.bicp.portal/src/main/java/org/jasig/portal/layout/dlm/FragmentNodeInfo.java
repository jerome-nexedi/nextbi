 ...
 * persisted. Attributes on elements in the user's layout do not indicate if
 * they came from a user change or from a fragment's defined value. This class
 * enables the layout manager to make such determinations.
 * 
...
public class FragmentNodeInfo {
  ...
  /**
   * Returns true if an attribute can be modified for a node. This is based on a
   * single dlm:editAllowed attribute for nodes in fragments. If not included
   * then edits to node attributes are allowed. If included with a value other
   * than true then edits are prevented.
   * 
   * @param name
   * @return
   */
