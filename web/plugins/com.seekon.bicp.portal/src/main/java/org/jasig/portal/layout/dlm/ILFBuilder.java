...
/**
 * Performs merging of layout fragments into a single document containing all
 * incorporated layout fragment elements from the set of fragments passed in.
 * This merge is trivial, appending all children of each fragment into the
 * composite document and recording their identifiers in the document identifier
 * cache. No changes are made to the source fragments passed in.
 * 
...
public class ILFBuilder {
  ...
  public static Document constructILF(Document PLF, List<Document> sequence,
    IPerson person) {
  ...
  /**
   * Passes the layout root of each of these documents to mergeChildren causing
   * all children of newLayout to be merged into compositeLayout following
   * merging protocal for distributed layout management.
   * 
   * @throws AuthorizationException
   **/
  public static void mergeFragment(Document fragment, Document composite,
    IAuthorizationPrincipal ap) throws AuthorizationException {
   ...
   *          parent of children
   * @param dest
   *          receiver of children
   * @param ap
   *          User's authorization principal for determining if they can view a
   *          channel
   * @param visitedNodes
   *          A Set of nodes from the source tree that have been visited to get
   *          to this node, used to ensure a loop doesn't exist in the source
   *          tree.
   * @throws AuthorizationException
   */
  private static void mergeChildren(Element source, Element dest,
    IAuthorizationPrincipal ap, Set visitedNodes) throws AuthorizationException {
    // Record this node in the visited nodes set. If add returns false a loop
    // has been detected
    if (!visitedNodes.add(source)) {
      final String msg = "mergeChildren has encountered a loop in the source DOM. currentNode='"
        + source
        + "', currentDepth='"
        + visitedNodes.size()
        + "', visitedNodes='"
  ...
  /**
   * Tests to see if channels to be merged from ILF can be rendered by the end
   * user. If not then they are discarded from the merge.
   * 
   * @param child
   * @param person
   * @return
