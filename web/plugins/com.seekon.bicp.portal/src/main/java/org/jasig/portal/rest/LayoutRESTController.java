...
public class LayoutRESTController {
  ...
  public ModelAndView getRESTController(HttpServletRequest request,
    ...
    try {
      ...
      final DistributedUserLayout userLayout = userLayoutStore.getUserLayout(person,
        profile);
      ...
      for (int i = 0; i < portletNodes.getLength(); i++) {
        try {
          ...
          portlet.setDescription(attributes.getNamedItem("description")
            .getNodeValue());
          portlet.setNodeId(attributes.getNamedItem("ID").getNodeValue());

          IPortletDefinition def = portletDao.getPortletDefinitionByFname(attributes
            .getNamedItem("fname").getNodeValue());
          ...
          final IPortalUrlBuilder portalUrlBuilder = urlProvider
            .getPortalUrlBuilderByLayoutNode(request, attributes.getNamedItem("ID")
              .getNodeValue(), UrlType.RENDER);
          ...
          log
            .warn("Exception construction JSON representation of mobile portlet", e);
        }
      }
