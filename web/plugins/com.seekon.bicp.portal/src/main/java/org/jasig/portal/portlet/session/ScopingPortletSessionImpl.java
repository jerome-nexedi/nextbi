...
/**
 * Custom portlet session impl, uses the entityId instead of the windowId for
...
public class ScopingPortletSessionImpl extends PortletSessionImpl {
  ...
  public ScopingPortletSessionImpl(IPortletEntityId portletEntityId,
    PortletContext portletContext, PortletWindow portletWindow,
  ...
  protected String createPortletScopedId(String name) {
    return PORTLET_SCOPE_NAMESPACE + portletEntityId.getStringId()
      + ID_NAME_SEPARATOR + name;
  }
  ...
  protected boolean isInCurrentPortletScope(String name) {
    ...
    if (name.startsWith(PORTLET_SCOPE_NAMESPACE)
      ...
      final String id = name.substring(PORTLET_SCOPE_NAMESPACE.length(), name
        .indexOf(ID_NAME_SEPARATOR));
  ...
  private void checkForExceptionAttribute(String name, Object value) throws Error {
    ...
    if ("org.springframework.web.portlet.DispatcherPortlet.ACTION_EXCEPTION"
      .equals(name)) {
