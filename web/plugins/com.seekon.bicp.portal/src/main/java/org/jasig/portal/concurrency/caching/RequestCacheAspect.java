...
/**
 * Aspect that caches the results of a method invocation in the current
...
public class RequestCacheAspect {
  private static final String CACHE_MAP = RequestCacheAspect.class.getName()
    + ".CACHE_MAP";
  ...
  public Object cacheRequest(ProceedingJoinPoint pjp, RequestCache requestCache)
    ...
    if (this.logger.isTraceEnabled()) {
      ...
      this.logger.trace("Hits " + hits + ", Miss " + misses + ", TPH " + hitTimeAvg
        + ", TPM " + missTimeAvg);
    ...
    try {
      ...
      logger.debug("No current portal request, will not cache result of: {}",
        cacheKey);
    ...
    final ConcurrentMap<CacheKey, Object> cache = PortalWebUtils
      .getMapRequestAttribute(currentPortalRequest, CACHE_MAP);
  ...
  private void logMiss(final long start) {
    ...
    if (this.isLogTimes()) {
      this.missTime.addAndGet(TimeUnit.NANOSECONDS.toMicros(System.nanoTime()
        - start));
    }
  }
  ...
  private void logHit(final long start) {
    ...
    if (this.isLogTimes()) {
      this.hitTime.addAndGet(TimeUnit.NANOSECONDS
        .toMicros(System.nanoTime() - start));
    }
  }

  protected CacheKey createCacheKey(ProceedingJoinPoint pjp,
  ...
  private static class CacheKey implements Serializable {
     ...
     */
    public CacheKey(Class<?> declaringType, String signatureLongString,
      Object[] parameters) {
    ...
    public int hashCode() {
      ...
      if (h == -1) {
        ...
        h = prime * h
          + ((signatureLongString == null) ? 0 : signatureLongString.hashCode());
