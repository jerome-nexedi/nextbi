...
/**
 * When retrieving a new person, the value of the <code>REMOTEUSER</code>
 * environment variable is passed to the security context. If it is set then the
 * server has authenticated the user and the username may be used for login.
 * 
...
public class RemoteUserPersonManager extends AbstractPersonManager {
  ...
  public IPerson getPerson(HttpServletRequest request)
    ...
    try {
      ...
      // We don't want to ignore the security contexts which are already
      // configured in security.properties, so we
      // retrieve the existing security contexts. If one of the existing
      // security contexts is a RemoteUserSecurityContext,
      // we set the REMOTE_USER field of the existing RemoteUserSecurityContext
      // context.
      //
      // If a RemoteUserSecurityContext does not already exist, we create one
      // and populate the REMOTE_USER field.
      ...
      if (subContexts != null) {
        while (subContexts.hasMoreElements()) {
          ...
          // Check to see if a RemoteUserSecurityContext already exists, and set
          // the REMOTE_USER
      ...
      if (!remoteUserSecurityContextExists) {
        RemoteUserSecurityContext remoteuserctx = new RemoteUserSecurityContext(
          remoteUser);
