...
/**
 * Wraps {@link IUserLayoutManager} interface to provide ability to incorporate
 * channels into a user layout that are not part of their layout structure
 * (persistent).
 * 
 * The channels are incorporated upon request (functional name) and remain part
 * of the layout structure only as long as they are the target channel.
 * 
...
public class TransientUserLayoutManagerWrapper implements IUserLayoutManager {
  ...
  private static final Log log = LogFactory
    .getLog(TransientUserLayoutManagerWrapper.class);
  ...
  private Map<String, String> mFnameMap = Collections
    .synchronizedMap(new HashMap<String, String>());

  // contains subscribe id --> fname mappings (for transient channels only)
  private Map<String, String> mSubIdMap = Collections
    .synchronizedMap(new HashMap<String, String>());

  // stores channel defs by subscribe id (transient channels only)
  private Map<String, IPortletDefinition> mChanMap = Collections
    .synchronizedMap(new HashMap<String, IPortletDefinition>());
  ...
  public Set<String> getAllSubscribedChannels() {
    final Set<String> allSubscribedChannels = new LinkedHashSet<String>(man
  ...
  public boolean moveNode(String nodeId, String parentId, String nextSiblingId)
    throws PortalException {
    // allow all moves, except for those related to the transient channels and
    // folders
    if (nodeId != null && (!mSubIdMap.containsKey(nodeId))
  ...
  public boolean deleteNode(String nodeId) throws PortalException {
    // allow all deletions, except for those related to the transient channels
    // and folders
    if (nodeId != null && (!mSubIdMap.containsKey(nodeId))
      && (!nodeId.equals(TRANSIENT_FOLDER_ID))) {
  ...
  public boolean updateNode(IUserLayoutNodeDescription node) throws PortalException {
    // allow all updates, except for those related to the transient channels and
    // folders
   ...
   * 
   * @param subId
   *          the subscribe id for the ChannelDefinition.
  ...
  protected IPortletDefinition getChannelDefinition(String subId)
    ...
    if (null == chanDef) {
      ...
      if (log.isDebugEnabled())
        log
          .debug("TransientUserLayoutManagerWrapper>>getChannelDefinition, "
            + "attempting to get a channel definition using functional name: "
            + fname);
      try {
        chanDef = PortletDefinitionRegistryLocator.getPortletDefinitionRegistry()
  ...
  public String getSubscribeId(String fname) throws PortalException {
    ...
    if (subId == null) {
      try {
        IPortletDefinition chanDef = PortletDefinitionRegistryLocator
          .getPortletDefinitionRegistry().getPortletDefinitionByFname(fname);
        if (chanDef != null) {
          // assign a new id
   ...
   * 
   * @see
   * org.jasig.portal.layout.IUserLayoutManager#getSubscribeId(java.lang.String,
   * java.lang.String)
   */
  @Override
   ...
   * 
   * @param nodeId
   *          the node (subscribe) id to get the channel for.
  ...
  private IUserLayoutChannelDescription getTransientNode(String nodeId)
    ...
    try {
      ...
      throw new PortalException("Failed to obtain channel definition using fname: "
        + fname);
    }
  }
