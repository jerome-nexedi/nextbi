...
public class ParameterEditManager {
   ...
   * @throws Exception
   */
  static void applyAndUpdateParmEditSet(Document plf, Document ilf,
  ...
  /**
   * Attempt to apply a single channel parameter edit command and return true if
   * it succeeds or false otherwise. If the edit is disallowed or the target
   * element no longer exists in the document the edit command fails and returns
   * false.
   * 
   * @throws Exception
   */
  private static boolean applyEdit(Element edit, Document ilf) {
     ...
     * 
     * In the commented code below, the check for editAllowed will never be seen
     * on a parameter element in the current database schema approach used by
     * DLM. This is because parameters are second class citizens of the layout
     * structure. They are not found in the up_layout_struct table but only in
     * the up_layout_param table. DLM functionality like dlm:editAllowed,
     ...
     * 'dlm:' is placed as an attribute on the containing structural element. So
     * any channel parameter entry with dlm:editAllowed has that value placed as
     * an attribute on the containing channel not on the parameter that was
     * meant to have it.
     * 
     * The only solution would be to add special dlm:parm children below
     * channels that would get the editAllowed value and then when creating the
     * DOM don't create those as child elements but use them to set the
     ...
     * related.
     * 
     * The result of this lack of functionality is that fragments can't mark any
     * channel parameters as dlm:editAllowed='false' thereby further restricting
     * which channel parameters can be edited beyond what the channel definition
     * specifies during publishing.
     */
    // Attr editAllowed = targetParm.getAttributeNode(
    // Constants.ATT_EDIT_ALLOWED );
  ...
  private static Element getParmEditSet(Document plf, IPerson person, boolean create)
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new parameter edit set node " + "Id for userId="
        + person.getID(), e);
    }
   ...
   * channel represented by the passed-in target id. If one already exists for
   * that node and that name then the value of the existing edit is changed to
   * the passed-in value.
   */
  ...
  /**
   * This method does the actual work of adding a newly created parameter edit
   * and adding it to the parameter edits set.
   */
  private static void addParmEditDirective(String targetID, String name,
    String value, IPerson person, Document plf, Element parmSet)
    throws PortalException {
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new parameter edit node " + "Id for userId=" + person.getID(),
        e);
    }
  ...
  /**
   * Remove a parameter edit directive from the parameter edits set for applying
   * user specified values to a named parameter of an incorporated channel
   * represented by the passed-in target id. If one doesn't exists for that node
   * and that name then this call returns without any effects.
   */
  public static void removeParmEditDirective(String targetId, String name,
