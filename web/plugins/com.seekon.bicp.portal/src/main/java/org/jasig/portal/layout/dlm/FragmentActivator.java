...
public class FragmentActivator {
  ...
  private final LoadingCache<String, List<Locale>> fragmentOwnerLocales = CacheBuilder
    .newBuilder().<String, List<Locale>> build(
  ...
  private static final Pattern STANDARD_PATTERN = Pattern
    .compile("\\A[Rr][Ee][Gg][Uu][Ll][Aa][Rr]\\z");

  private static final Pattern EXPANDED_PATTERN = Pattern.compile(".*");

  @Autowired
  public void setUserViewErrors(
    @Qualifier("org.jasig.portal.layout.dlm.FragmentActivator.userViewErrors") Ehcache userViewErrors) {
    this.userViewErrors = userViewErrors;
  }

  @Autowired
  public void setUserViews(
    @Qualifier("org.jasig.portal.layout.dlm.FragmentActivator.userViews") Ehcache userViews) {
    this.userViews = new SelfPopulatingCache(userViews, new CacheEntryFactory() {
      ...
      public Object createEntry(Object key) throws Exception {
        ...
        // made and re-throw
        final net.sf.ehcache.Element exceptionElement = userViewErrors
          .get(userViewKey);
  ...
  private UserView activateFragment(final UserViewKey userViewKey) {
    ...
    if (fd.isNoAudienceIncluded()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Skipping activation of FragmentDefinition " + fd.getName()
          + ", no evaluators found. " + fd);
      }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Activating FragmentDefinition " + fd.getName() + " with locale "
        + locale);
    }
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info("Activated FragmentDefinition " + fd.getName() + " with locale "
        + locale);
    }
    return view;
  }
  ...
  private int createOwner(IPerson owner, FragmentDefinition fragment) {
    ...
    if (fragment.defaultLayoutOwnerID != null) {
      ...
      final String defaultLayoutOwner = PropertiesManager
        .getProperty("org.jasig.portal.layout.dlm.defaultLayoutOwner");
      if (defaultLayoutOwner != null) {
        defaultUser = defaultLayoutOwner;
      } else {
        try {
          defaultUser = PropertiesManager
            .getProperty(RDBMDistributedLayoutStore.TEMPLATE_USER_NAME);
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("\n\nOwner '" + fragment.getOwnerId() + "' of fragment '"
        + fragment.getName() + "' not found. Creating as copy of '" + defaultUser
        + "'\n");
    }
  ...
  private void loadLayout(UserView view, FragmentDefinition fragment, IPerson owner,
    ...
    try {
      ...
      root.setAttribute(Constants.ATT_ID, Constants.FRAGMENT_ID_USER_PREFIX
        + view.getUserId() + Constants.FRAGMENT_ID_LAYOUT_PREFIX + view.layoutId);
      view.layout = layout;
    } catch (Exception e) {
      throw new RuntimeException(
        "Anomaly occurred while loading layout for fragment '" + fragment.getName()
          + "'. The fragment will not be "
  ...
  private void fragmentizeLayout(UserView view, FragmentDefinition fragment) {
    ...
    boolean allowExpandedContent = Boolean.parseBoolean(PropertiesManager
      .getProperty(PROPERTY_ALLOW_EXPANDED_CONTENT));
    ...
    for (int i = children.getLength() - 1; i >= 0; i--) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE
        ...
        boolean isApplicable = contentPattern.matcher(folder.getAttribute("type"))
          .matches();
    ...
    setIdsAndAttribs(layout, layout.getAttribute(Constants.ATT_ID), ""
      + fragment.getIndex(), "" + fragment.getPrecedence());
  }

  /**
   * Recursive method that passes through a layout tree and changes all ids from
   * the regular format of sXX or nXX to the globally safe incorporated id of
   * form uXlXsXX or uXlXnXX indicating the user id and layout id from which
   * this node came.
   */
  private void setIdsAndAttribs(Element parent, String labelBase, String index,
    String precedence) {
    ...
    for (int i = 0; i < children.getLength(); i++) {
      if (children.item(i).getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (!id.equals("")) {
          ...
          child.setAttributeNS(Constants.NS_URI, Constants.ATT_PRECEDENCE,
            precedence);
