 ...
 */
public class LayoutCachingService implements ApplicationListener<PortalEvent>,
  ...
  public void onApplicationEvent(PortalEvent event) {
    if (event instanceof LoginEvent || event instanceof LogoutEvent) {
      ...
      // Try invalidating just the layout associated with the current user and
      // profile
      final IUserProfile currentUserProfile = (IUserProfile) person
        .getAttribute(IUserProfile.USER_PROFILE);
      ...
      final IUserLayoutStore userLayoutStore = UserLayoutStoreLocator
        .getUserLayoutStore();
      ...
      try {
        ...
        this.logger
          .warn(
            "Failed to load all UserProfiles for '"
              + person.getUserName()
              + "'. The user's layouts will not be explicitly removed from the layout cache.",
            e);
        return;
      }
  ...
  @Override
  public void cacheLayout(IPerson owner, IUserProfile profile,
    DistributedUserLayout layout) {
