...
public class EventAggregationConfigurationImporterExporter extends
  ...
  @Autowired
  public void setAggregatedGroupLookupDao(
  ...
  public Set<PortalDataKey> getImportDataKeys() {
    return Collections
      .singleton(EventAggregationConfigurationPortalDataType.IMPORT_40_DATA_KEY);
  }
  ...
  public void importData(ExternalEventAggregationConfiguration data) {
    ...
    for (final ExternalAggregatedIntervalConfig extAggregatedIntervalConfig : data
      ...
      final String aggregatorTypeName = extAggregatedIntervalConfig
        .getAggregatorType();
      ...
      // Copy over excludes
      final Set<AggregationInterval> excluded = aggregatedIntervalConfig
        .getExcluded();
      excluded.clear();
      for (final ExternalAggregationInterval extInterval : extAggregatedIntervalConfig
        .getExcludes()) {
        excluded.add(convert(extInterval));
      }

      // Copy over includes
      final Set<AggregationInterval> included = aggregatedIntervalConfig
        .getIncluded();
      included.clear();
      for (final ExternalAggregationInterval extInterval : extAggregatedIntervalConfig
        .getIncludes()) {
      ...
      this.aggregationManagementDao
        .updateAggregatedIntervalConfig(aggregatedIntervalConfig);
    }
    ...
    for (final AggregatedIntervalConfig aggregatedIntervalConfig : oldAggregatedIntervalConfigs) {
      this.aggregationManagementDao
        .deleteAggregatedIntervalConfig(aggregatedIntervalConfig);
    }
    ...
    for (final ExternalAggregatedGroupConfig extAggregatedGroupConfig : data
      .getAggregatedGroupConfigs()) {
      ...
      if (aggregatedGroupConfig == null) {
        aggregatedGroupConfig = this.aggregationManagementDao
          .createAggregatedGroupConfig(aggregatorType);
      }
      ...
      // Copy over excludes
      final Set<AggregatedGroupMapping> excluded = aggregatedGroupConfig
        .getExcluded();
      excluded.clear();
      for (final ExternalAggregatedGroupMapping extGroup : extAggregatedGroupConfig
        .getExcludes()) {
        excluded.add(convert(extGroup));
      }

      // Copy over includes
      final Set<AggregatedGroupMapping> included = aggregatedGroupConfig
        .getIncluded();
      included.clear();
      for (final ExternalAggregatedGroupMapping extGroup : extAggregatedGroupConfig
        .getIncludes()) {
      ...
      this.aggregationManagementDao
        .updateAggregatedGroupConfig(aggregatedGroupConfig);
    }

    // Delete interval configs that were not updated
    for (final AggregatedGroupConfig aggregatedGroupConfig : oldAggregatedGroupConfigs) {
      this.aggregationManagementDao
        .deleteAggregatedGroupConfig(aggregatedGroupConfig);
    }
    ...
    for (final AcademicTermDetail academicTermDetail : this.aggregationManagementDao
    ...
    for (final ExternalTermDetail externalTermDetail : data.getTermDetails()) {
      this.aggregationManagementDao.addAcademicTermDetails(new DateMidnight(
        externalTermDetail.getStart()),
  ...
  protected List<QuarterDetail> convertQuarterDetail(
    ...
    for (final ExternalQuarterDetail externalQuarterDetail : externalQuarterDetails) {
      quarterDetails.add(new QuarterDetailImpl(MonthDay.parse(externalQuarterDetail
        .getStart()), MonthDay.parse(externalQuarterDetail.getEnd()),
  ...
  protected AggregationInterval convert(
    ExternalAggregationInterval externalAggregationInterval) {
    return AggregationInterval.valueOf(externalAggregationInterval.name());
  }

  protected Class<? extends IPortalEventAggregator> getAggregatorType(
    final String aggregatorTypeName) {
    final Class<?> aggregatorType;
    try {
      aggregatorType = Class.forName(aggregatorTypeName);
    } catch (ClassNotFoundException e) {
      throw new RuntimeException("Specified aggregator type name "
        + aggregatorTypeName + " could not be resolved to a Class", e);
    }

    if (!IPortalEventAggregator.class.isAssignableFrom(aggregatorType)) {
      throw new IllegalArgumentException("Specified aggregator type "
        + aggregatorType.getName() + " is not an instance of "
   ...
   * 
   * @see
   * org.jasig.portal.io.xml.IDataImporterExporter#exportData(java.lang.String)
   */
  @Override
  public ExternalEventAggregationConfiguration exportData(String id) {
    ...
    final List<ExternalAggregatedIntervalConfig> aggregatedIntervalConfigs = externalData
      .getAggregatedIntervalConfigs();
    for (final AggregatedIntervalConfig aggregatedIntervalConfig : this.aggregationManagementDao
      .getAggregatedIntervalConfigs()) {
      final ExternalAggregatedIntervalConfig externalIntervalConfig = new ExternalAggregatedIntervalConfig();
      externalIntervalConfig.setAggregatorType(aggregatedIntervalConfig
        .getAggregatorType().getName());

      final List<ExternalAggregationInterval> extIncludes = externalIntervalConfig
        .getIncludes();
      for (final AggregationInterval interval : aggregatedIntervalConfig
        .getIncluded()) {
        extIncludes.add(convert(interval));
      }

      final List<ExternalAggregationInterval> extExcludes = externalIntervalConfig
        .getExcludes();
      for (final AggregationInterval interval : aggregatedIntervalConfig
        .getExcluded()) {
    ...
    // Copy group configs
    final List<ExternalAggregatedGroupConfig> aggregatedGroupConfigs = externalData
      .getAggregatedGroupConfigs();
    for (final AggregatedGroupConfig aggregatedGroupConfig : this.aggregationManagementDao
      .getAggregatedGroupConfigs()) {
      final ExternalAggregatedGroupConfig externalGroupConfig = new ExternalAggregatedGroupConfig();
      externalGroupConfig.setAggregatorType(aggregatedGroupConfig
        .getAggregatorType().getName());

      final List<ExternalAggregatedGroupMapping> extIncludes = externalGroupConfig
        .getIncludes();
      for (final AggregatedGroupMapping Group : aggregatedGroupConfig.getIncluded()) {
        extIncludes.add(convert(Group));
      }

      final List<ExternalAggregatedGroupMapping> extExcludes = externalGroupConfig
        .getExcludes();
    ...
    // Copy term details
    final List<ExternalTermDetail> externalTermDetails = externalData
      .getTermDetails();
    for (final AcademicTermDetail academicTermDetail : this.aggregationManagementDao
      .getAcademicTermDetails()) {
      ...
      externalTermDetail.setStart(academicTermDetail.getStart()
        .toGregorianCalendar());
    ...
    final List<ExternalQuarterDetail> quarterDetails = externalData
      .getQuarterDetails();
    for (final QuarterDetail quarterDetail : this.aggregationManagementDao
      .getQuartersDetails()) {
  ...
  protected ExternalAggregationInterval convert(
  ...
  protected ExternalAggregatedGroupMapping convert(
    ...
    externalAggregatedGroupMapping.setGroupService(aggregatedGroupMapping
      .getGroupService());
    externalAggregatedGroupMapping.setGroupName(aggregatedGroupMapping
      .getGroupName());
   ...
   * 
   * @see
   * org.jasig.portal.io.xml.IDataImporterExporter#deleteData(java.lang.String)
   */
  @Transactional("aggrEventsTransactionManager")
  @Override
  public ExternalEventAggregationConfiguration deleteData(String id) {
    ...
    this.aggregationManagementDao.setQuarterDetails(EventDateTimeUtils
