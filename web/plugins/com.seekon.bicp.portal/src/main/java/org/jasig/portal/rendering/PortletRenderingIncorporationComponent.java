 ...
 * @version $Revision$
 */
public class PortletRenderingIncorporationComponent extends
  CharacterPipelineComponentWrapper {
  private IPortletExecutionManager portletExecutionManager;

  @Autowired
  public void setPortletExecutionManager(
    IPortletExecutionManager portletExecutionManager) {
  ...
  public CacheKey getCacheKey(HttpServletRequest request,
    ...
    /*
     * TODO do all the portlet cache keys need to be included here? Probably for
     * this to be useful
     */
  ...
  public PipelineEventReader<CharacterEventReader, CharacterEvent> getEventReader(
    ...
    final PortletIncorporatingEventReader portletIncorporatingEventReader = new PortletIncorporatingEventReader(
      eventReader, request, response);
  ...
  private class PortletIncorporatingEventReader extends
    FilteringCharacterEventReader {
    private final HttpServletRequest request;

    private final HttpServletResponse response;

    public PortletIncorporatingEventReader(CharacterEventReader delegate,
      HttpServletRequest request, HttpServletResponse response) {
      super(delegate);
    ...
    protected CharacterEvent filterEvent(CharacterEvent event, boolean peek) {
      ...
      case PORTLET_HEADER: {
        ...
        final IPortletWindowId portletWindowId = headerPlaceholderEvent
          .getPortletWindowId();

        final String output = portletExecutionManager.getPortletHeadOutput(
      ...
      case PORTLET_CONTENT: {
        ...
        final IPortletWindowId portletWindowId = contentPlaceholderEvent
          .getPortletWindowId();

        final String output = portletExecutionManager.getPortletOutput(
          portletWindowId, this.request, this.response);
      ...
      case PORTLET_TITLE: {
        ...
        final IPortletWindowId portletWindowId = titlePlaceholderEvent
          .getPortletWindowId();

        final String title = portletExecutionManager.getPortletTitle(
          portletWindowId, this.request, this.response);
      ...
      case PORTLET_NEW_ITEM_COUNT: {
        ...
        final IPortletWindowId portletWindowId = newItemCountPlaceholderEvent
          .getPortletWindowId();

        final int newItemCount = portletExecutionManager.getPortletNewItemCount(
          portletWindowId, this.request, this.response);
      ...
      case PORTLET_LINK: {
        ...
        final IPortletWindowId portletWindowId = linkPlaceholderEvent
          .getPortletWindowId();
        final String defaultPortletUrl = linkPlaceholderEvent.getDefaultPortletUrl();

        final String link = portletExecutionManager.getPortletLink(portletWindowId,
          defaultPortletUrl, this.request, this.response);
