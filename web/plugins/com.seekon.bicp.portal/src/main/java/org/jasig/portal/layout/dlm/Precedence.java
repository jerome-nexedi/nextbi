...
public final class Precedence {
  ...
  static Precedence newInstance(String fragmentIdx) {
    if (fragmentIdx == null || fragmentIdx.equals(""))
   ...
   * precedence of the passed in Precedence object. The complete precedence
   * takes into account the location in the configuration file of the fragment
   * definition. If the "precedence" value is equal then the precedence object
   * with the lowest index has the higher complete precedence. And index of -1
   * indicates the highest index in the file.
   */
  public boolean isLessThan(Precedence p) {
    if (this.precedence < p.precedence
      || (this.precedence == p.precedence && (this.index == -1 && p.index > -1 || this.index > p.index)))
      return true;
    return false;
  }
