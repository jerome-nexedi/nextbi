...
public class XsltPortalUrlProvider {
  ...
  /**
   * Create a portal URL builder for the specified fname or layoutId (fname
   * takes precedence)
   */
  public IPortalUrlBuilder getPortalUrlBuilder(HttpServletRequest request,
    String fname, String layoutId, String type) {
    try {
      ...
      if (StringUtils.isNotEmpty(fname)) {
        final IPortalUrlBuilder portalUrlBuilderByPortletFName = this.portalUrlProvider
          .getPortalUrlBuilderByPortletFName(request, fname, urlType);
        return portalUrlBuilderByPortletFName;
      }

      if (StringUtils.isNotEmpty(layoutId)) {
        final IPortalUrlBuilder portalUrlBuilderByLayoutNode = this.portalUrlProvider
          .getPortalUrlBuilderByLayoutNode(request, layoutId, urlType);
        return portalUrlBuilderByLayoutNode;
      }

      return this.portalUrlProvider.getDefaultUrl(request);
    } catch (Exception e) {
      this.logger.error("Faild to create IPortalUrlBuilder for fname='" + fname
        + "', layoutId='" + layoutId + "', type='" + type
        + "'. # will be returned instead.", e);
      return new FailSafePortalUrlBuilder();
    }
  }

  /**
   * Get the portlet URL builder for the specified fname or layoutId (fname
   * takes precedence)
   */
  public IPortletUrlBuilder getPortletUrlBuilder(HttpServletRequest request,
    IPortalUrlBuilder portalUrlBuilder, String fname, String layoutId, String state,
    String mode, String copyCurrentRenderParameters) {
    ...
    if (StringUtils.isNotEmpty(fname)) {
      ...
      final IPortletWindow portletWindow = this.portletWindowRegistry
        .getOrCreateDefaultPortletWindowByLayoutNodeId(request, layoutId);
      final IPortletWindowId portletWindowId = portletWindow.getPortletWindowId();
      portletUrlBuilder = portalUrlBuilder.getPortletUrlBuilder(portletWindowId);
    } else {
      final IPortletWindowId targetPortletWindowId = portalUrlBuilder
      ...
      if (targetPortletWindowId == null) {
        if (this.logger.isDebugEnabled()) {
          ...
          this.logger
            .warn("Can only target the default portlet if the root portal-url targets a portlet. Enable debug for stack trace.");
        }
    ...
    portletUrlBuilder.setCopyCurrentRenderParameters(Boolean
      .parseBoolean(copyCurrentRenderParameters));
