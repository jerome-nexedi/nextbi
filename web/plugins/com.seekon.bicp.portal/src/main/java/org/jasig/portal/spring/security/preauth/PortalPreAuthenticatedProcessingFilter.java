 ...
 * pre-authentication in uPortal by returning the current IPerson object as the
 * user details.
 * 
 * @author Jen Bourey, jennifer.bourey@gmail.com
 * @version $Revision$
 */
public class PortalPreAuthenticatedProcessingFilter extends
  AbstractPreAuthenticatedProcessingFilter {
  protected final Log swapperLog = LogFactory
    .getLog("org.jasig.portal.portlets.swapper");
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
     ...
     * we're currently at the login servlet. Since that servlet sets up the user
     * session and authentication, we need it to run before this filter is
     * useful.
     */
  ...
  private void doPortalAuthentication(HttpServletRequest request) {
    ...
    boolean swap = false;
    Assertion assertion = null;// //TODO 2012-06-16
    if (request.isRequestedSessionIdValid()) {
      try {
        ...
        assertion = (Assertion) s.getAttribute("_const_cas_assertion_");

        s.invalidate();
      } catch (IllegalStateException ise) {
        // ISE indicates session was already invalidated.
        // This is fine. This servlet trying to guarantee that the session has
        // been invalidated;
    ...
    if (assertion != null) {
      s.setAttribute("_const_cas_assertion_", assertion);// //TODO: 2012-06-16
    }

    final String requestedProfile = request
      .getParameter(LoginController.REQUESTED_PROFILE_KEY);
    ...
    try {
      ...
      if (targetUid != null && originalUid != null) {
        if (swap) {
          swapperLog.warn("Swapping identity for '" + originalUid + "' to '"
            + targetUid + "'");
          ...
          swapperLog.warn("Reverting swapped identity from '" + targetUid + "' to '"
            + originalUid + "'");
        ...
        final IdentitySwapperPrincipal identitySwapperPrincipal = new IdentitySwapperPrincipal(
          person);
      ...
      // Norm authN path
        ...
        // WE grab all of the principals and credentials from the request and
        // load
      ...
      request.getSession(true).setAttribute(LoginController.AUTH_ERROR_KEY,
        Boolean.TRUE);
    }
  }

  /**
   * Get the values represented by each token from the request and load them
   * into a HashMap that is returned.
   * 
   * @param tokens
   * @param request
  ...
  private HashMap<String, String> getPropertyFromRequest(
    ...
    // Iterate through all of the other property keys looking for the first
    // property
    ...
    for (Map.Entry<String, String> entry : tokens.entrySet()) {
      ...
      // the subcontexts are named has to be closely looked at to make this
      // work.
      // The keys are either "root" or the subcontext name that follows "root.".
      // As
      // as example, the contexts ["root", "root.simple", "root.cas"] are
      // represented
      // as ["root", "simple", "cas"].
      String key = (contextName.startsWith("root.") ? contextName.substring(5)
        : contextName);
