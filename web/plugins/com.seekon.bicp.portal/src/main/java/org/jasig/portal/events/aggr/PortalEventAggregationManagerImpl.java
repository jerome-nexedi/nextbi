...
public class PortalEventAggregationManagerImpl implements
  IPortalEventAggregationManager {
  private static final String DIMENSION_LOCK_NAME = PortalEventAggregationManagerImpl.class
    .getName()
    + ".DIMENSION_LOCK";

  private static final String AGGREGATION_LOCK_NAME = PortalEventAggregationManagerImpl.class
    .getName()
    + ".AGGREGATION_LOCK";

  private static final String PURGE_RAW_EVENTS_LOCK_NAME = PortalEventAggregationManagerImpl.class
    .getName()
    + ".PURGE_RAW_EVENTS_LOCK";

  private static final String PURGE_EVENT_SESSION_LOCK_NAME = PortalEventAggregationManagerImpl.class
    .getName()
  ...
  public boolean populateDimensions() {
    try {
      final TryLockFunctionResult<Object> result = this.clusterLockService
        .doInTryLock(DIMENSION_LOCK_NAME, new FunctionWithoutResult<String>() {
          @Override
  ...
  public boolean aggregateRawEvents() {
    ...
    do {
      if (result != null) {
        logger
          .debug("doAggregateRawEvents signaled that not all events were aggregated in a single transaction, running again.");
      }

      result = aggrEventsTransactionOperations
        .execute(new TransactionCallback<TryLockFunctionResult<Boolean>>() {
          @Override
          public TryLockFunctionResult<Boolean> doInTransaction(
      ...
      // Loop if doAggregateRawEvents returns false, this means that there is
      // more to aggregate
  ...
  public boolean purgeRawEvents() {
    try {
      final TryLockFunctionResult<Object> result = this.clusterLockService
        .doInTryLock(PURGE_RAW_EVENTS_LOCK_NAME,
  ...
  public boolean purgeEventSessions() {
    try {
      final TryLockFunctionResult<Object> result = this.clusterLockService
        .doInTryLock(PURGE_EVENT_SESSION_LOCK_NAME,
          new FunctionWithoutResult<String>() {
            @Override
            protected void applyWithoutResult(String input) {
  ...
  void doPopulateTimeDimensions() {
    final List<TimeDimension> timeDimensions = this.timeDimensionDao
      .getTimeDimensions();
    if (timeDimensions.isEmpty()) {
      logger.info("No TimeDimensions exist, creating them");
    } else if (timeDimensions.size() != (24 * 60)) {
      this.logger.info("There are only " + timeDimensions.size()
        + " time dimensions in the database, there should be " + (24 * 60)
        + " creating missing dimensions");
    } else {
      this.logger.debug("Found expected " + timeDimensions.size()
        + " time dimensions");
      return;
    }
  ...
  void doPopulateDateDimensions() {
    ...
    final DateTime oldestPortalEventTimestamp = this.portalEventDao
      .getOldestPortalEventTimestamp();
    if (oldestPortalEventTimestamp == null
      || now.isBefore(oldestPortalEventTimestamp)) {
      startIntervalInfo = this.intervalHelper.getIntervalInfo(
        AggregationInterval.YEAR, now.minus(this.dimensionBuffer));
    } else {
      startIntervalInfo = this.intervalHelper.getIntervalInfo(
        AggregationInterval.YEAR, oldestPortalEventTimestamp
          .minus(this.dimensionBuffer));
    }

    final AggregationIntervalInfo endIntervalInfo;
    final DateTime newestPortalEventTimestamp = this.portalEventDao
      .getNewestPortalEventTimestamp();
    if (newestPortalEventTimestamp == null
      || now.isAfter(newestPortalEventTimestamp)) {
      endIntervalInfo = this.intervalHelper.getIntervalInfo(
        AggregationInterval.YEAR, now.plus(this.dimensionBuffer));
    } else {
      endIntervalInfo = this.intervalHelper.getIntervalInfo(
        AggregationInterval.YEAR, newestPortalEventTimestamp
          .plus(this.dimensionBuffer));
    }
  ...
  void doPopulateDateDimensions(final DateMidnight start, final DateMidnight end) {
    ...
    final List<QuarterDetail> quartersDetails = this.eventAggregationManagementDao
      .getQuartersDetails();
    final List<AcademicTermDetail> academicTermDetails = this.eventAggregationManagementDao
      .getAcademicTermDetails();

    final List<DateDimension> dateDimensions = this.dateDimensionDao
      .getDateDimensionsBetween(start, end);
  ...
  protected void createDateDimension(List<QuarterDetail> quartersDetails,
    ...
    final QuarterDetail quarterDetail = EventDateTimeUtils.findDateRangeSorted(date,
      quartersDetails);
    final AcademicTermDetail termDetail = EventDateTimeUtils.findDateRangeSorted(
      date, academicTermDetails);
    this.dateDimensionDao.createDateDimension(date, quarterDetail.getQuarterId(),
      termDetail != null ? termDetail.getTermName() : null);
  }

  /**
   * @return true if all events for the time period were aggregated, false if
   *         not
   */
  boolean doAggregateRawEvents() {
    if (!this.checkedDimensions.get()
      && this.checkedDimensions.compareAndSet(false, true)) {
      // First time aggregation has happened, run populateDimensions to ensure
      // enough dimension data exists
      final boolean populatedDimensions = this.populateDimensions();
      if (!populatedDimensions) {
        this.logger
          .warn("First time doAggregateRawEvents has run and populateDimensions returned false, assuming current dimension data is available");
      }
    }
    ...
    DateTime newestEventTime = DateTime.now().minus(this.aggregationDelay)
      .secondOfMinute().roundFloorCopy();

    logger.debug("Starting aggregation of events between {} (inc) and {} (exc)",
      lastAggregated, newestEventTime);
    ...
    portalEventDao.aggregatePortalEvents(lastAggregated, newestEventTime,
      this.eventAggregationBatchSize, new AggregateEventsHandler(events,
        eventAggregatorStatus));
    eventAggregatorStatus.setLastEnd(new DateTime());

    logger.debug("Aggregated {} events between {} and {} in {}ms", new Object[] {
      events, lastAggregated, newestEventTime,
      TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start) });

    // Store the results of the aggregation
    eventAggregationManagementDao.updateEventAggregatorStatus(eventAggregatorStatus);

    return this.eventAggregationBatchSize <= 0
  ...
  void doPurgeRawEvents() {
    ...
    if (eventAggregatorStatus == null
      || eventAggregatorStatus.getLastEventDate() == null) {
    ...
    // Calculate purge end date from most recent aggregation minus the purge
    // delay
    ...
    logger.debug("Purged {} events before {} in {}ms", new Object[] { events,
      purgeEnd, TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start) });

    // Update the status object and store it
    eventPurgerStatus.setLastEventDate(purgeEnd.minusMillis(1)); // decrement by
    // 1ms since
  ...
  private final class AggregateEventsHandler extends
    FunctionWithoutResult<PortalEvent> {
    ...
    // Local caches of aggregator config data, shouldn't ever change for the
    // duration of an aggregation run
    ...
    private AggregateEventsHandler(MutableInt eventCounter,
      IEventAggregatorStatus eventAggregatorStatus) {
      this.eventCounter = eventCounter;
      this.eventAggregatorStatus = eventAggregatorStatus;
      this.defaultAggregatedGroupConfig = eventAggregationManagementDao
    ...
    protected void applyWithoutResult(PortalEvent event) {
      ...
      if (this.currentIntervalInfo.isEmpty()) {
        ...
        if (lastEventDate != null) {
          // If there was a previously aggregated event use that date to make
          // sure an interval is not missed
        ...
        for (final AggregationInterval interval : AggregationInterval.values()) {
          final AggregationIntervalInfo intervalInfo = intervalHelper
            .getIntervalInfo(interval, intervalDate);
      ...
      for (final AggregationInterval interval : AggregationInterval.values()) {
        AggregationIntervalInfo intervalInfo = this.currentIntervalInfo
          .get(interval);
        if (intervalInfo != null && !intervalInfo.getEnd().isAfter(eventDate)) { // if
          // there
          // is
          // no
          // IntervalInfo
    ...
    private void doAggregateEvent(PortalEvent item) {
      ...
      for (final IPortalEventAggregator<PortalEvent> portalEventAggregator : portalEventAggregators) {
        if (portalEventAggregator.supports(item.getClass())) {
          final Class<? extends IPortalEventAggregator> aggregatorType = portalEventAggregator
            .getClass();
          ...
          // If there is an event session get the aggregator specific version of
          // it
          if (eventSession != null) {
            final AggregatedGroupConfig aggregatorGroupConfig = getAggregatorGroupConfig(aggregatorType);
            eventSession = eventSession
              .getFilteredEventSession(aggregatorGroupConfig);
          }
    ...
    private void doHandleIntervalBoundary(AggregationInterval interval,
      ...
      for (final IPortalEventAggregator<PortalEvent> portalEventAggregator : portalEventAggregators) {
        ...
        // If the aggreagator is configured to use the interval notify it of the
        // interval boundary
        if (aggregatorIntervalConfig.isIncluded(interval)) {
          final Map<AggregationInterval, AggregationIntervalInfo> aggregatorIntervalInfo = this
            .getAggregatorIntervalInfo(aggregatorType);
          portalEventAggregator.handleIntervalBoundary(interval,
            aggregatorIntervalInfo);
        }
      }
    }
    ...
    protected Map<AggregationInterval, AggregationIntervalInfo> getAggregatorIntervalInfo(
      ...
      Map<AggregationInterval, AggregationIntervalInfo> intervalInfo = this.aggregatorReadOnlyIntervalInfo
        .get(aggregatorType);
      if (intervalInfo == null) {
        intervalInfo = Maps.filterKeys(this.readOnlyIntervalInfo,
          new Predicate<AggregationInterval>() {
            @Override
    ...
    /**
     * @return The group config for the aggregator, returns the default config
     *         if no aggregator specific config is set
     */
    protected AggregatedGroupConfig getAggregatorGroupConfig(
      ...
      if (config == null) {
        config = eventAggregationManagementDao
    ...
    /**
     * @return The interval config for the aggregator, returns the default
     *         config if no aggregator specific config is set
     */
    protected AggregatedIntervalConfig getAggregatorIntervalConfig(
      final Class<? extends IPortalEventAggregator> aggregatorType) {
      AggregatedIntervalConfig config = this.aggregatorIntervalConfigs
        .get(aggregatorType);
      if (config == null) {
        config = eventAggregationManagementDao
