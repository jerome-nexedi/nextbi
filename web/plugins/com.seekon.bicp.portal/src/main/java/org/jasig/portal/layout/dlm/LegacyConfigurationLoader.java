...
public class LegacyConfigurationLoader implements ConfigurationLoader {
  ...
  private final SingletonDoubleCheckedCreator<Boolean> loadedFlag = new SingletonDoubleCheckedCreator<Boolean>() {
     ...
     * (non-Javadoc)
     * 
     * @seeorg.jasig.portal.utils.threading.SingletonDoubleCheckedCreator#
     * createSingleton(java.lang.Object[])
     */
    @Override
   ...
   *          The dlm.xml file to load configuration from
   */
  ...
  protected void loadFragmentInfo() {
    ...
    try {
      ...
      throw new IllegalArgumentException(
        "Could not convert dlm configuration resource to URL "
          + this.configurationFile, e);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException("Could load dlm configuration resource "
        + this.configurationFile, e);
    } catch (SAXException e) {
      throw new IllegalArgumentException("Could parse dlm configuration resource "
        + this.configurationFile, e);
    }
    ...
    for (int i = 0; i < fragmentCount; i++) {
      ...
      try {
        ...
        if (logger.isInfoEnabled()) {
          logger
            .info("DLM loaded fragment definition '"
              + fragment.getName()
              + "' owned by '"
              + fragment.getOwnerId()
              + "' with precedence "
              ...
              + (fragment.isNoAudienceIncluded() ? " and no specified audience"
                + ". It will be editable by '" + fragment.getOwnerId() + "' but "
                + "not included in any user's layout."
                : (fragment.isNoAudienceIncluded() ? " with no audience. It will be editable by '"
                  + fragment.getOwnerId()
                  + "' but "
                  + "not included in any user's layout."
        ...
        if (this.logger.isDebugEnabled()) {
          ...
          logger.warn(msg + " Enable DEBUG logging for stack trace.\n\tCaused By: "
            + e.getMessage() + " " + XmlUtilitiesImpl.toString(fragmentElement));
        }
      }
    }
