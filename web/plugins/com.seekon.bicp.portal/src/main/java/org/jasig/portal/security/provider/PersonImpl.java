...
public class PersonImpl implements IPerson {
  ...
  /**
   * Returns an attribute for a key. For objects represented as strings, a
   * <code>java.lang.String</code> will be returned. Binary values will be
  ...
  /**
   * Returns multiple attributes for a key. If only one value exists, it will be
   * returned in an array of size one.
   * 
   * @param key
   *          the attribute name
  ...
  /**
   * Returns an enumeration of all of the attribute names associated with the
   * user
   * 
  ...
  public void setAttribute(String key, List<Object> value) {
    ...
    if (!this.entityIdentifierSet && key.equals(IPerson.USERNAME)) {
      final Object userName = value != null && value.size() > 0 ? value.get(0)
        : null;
  ...
  /**
   * Returns the user's ID that was used for authentication. Does not correlate
   * to any eduPerson attribute but is the key for user data in uPortal
   * reference rdbms.
   * 
   * @return User's ID.
   */
   ...
   * 
   * @param sID
   *          User's ID as supplied for authentication Does not correlate to any
   *          eduPerson attribute but is the key for user data in uPortal
   *          reference rdbms.
   */
   ...
   * 
   * @param sFullName
   *          User's name as established during authentication Correlates to cn
   *          (common name) in the eduPerson 1.0 specification.
   */
   ...
   * <code>org.jasig.portal.security.PersonImpl.guest_user_name</code> in
   * <code>portal.properties</code>.</li>
   * </ol>
   * 
   * @return <code>true</code> if person is a guest, otherwise
   *         <code>false</code>
   */
  public boolean isGuest() {
    boolean isGuest = false;
    String userName = (String) getAttribute(IPerson.USERNAME);
    if (PersonFactory.GUEST_USERNAME.equals(userName) && m_securityContext != null
  ...
  /**
   * Returns an EntityIdentifier for this person. The key contains the value of
   * the eudPerson username attribute, or null
   * 
