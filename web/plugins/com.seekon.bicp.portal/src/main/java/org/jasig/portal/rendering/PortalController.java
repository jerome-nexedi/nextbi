...
public class PortalController {
  ...
  @RequestMapping(headers = { "org.jasig.portal.url.UrlType=RENDER",
    "org.jasig.portal.url.UrlState.EXCLUSIVE=true" })
  public void renderExclusive(HttpServletRequest request,
    ...
    this.portletExecutionManager
      .getPortletOutput(portletWindowId, request, response);
  }
  ...
  public void actionRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    if (portletRequestInfo != null) {
      final IPortletWindowId targetWindowId = portletRequestInfo
        .getPortletWindowId();
      actionRedirectUrl = this.portalUrlProvider.getPortalUrlBuilderByPortletWindow(
      ...
      if (targetedLayoutNodeId != null) {
        actionRedirectUrl = this.portalUrlProvider.getPortalUrlBuilderByLayoutNode(
    ...
    // Stuff the action-redirect URL builder into the request so other code can
    // use it during request processing
    this.portalUrlProvider.convertToPortalActionUrlBuilder(request,
      actionRedirectUrl);
    ...
    if (portletRequestInfo != null) {
      ...
      try {
        this.portletExecutionManager.doPortletAction(targetWindowId, request,
          response);
      } catch (RuntimeException e) {
        this.logger.error("Exception thrown while executing portlet action for: "
          + portletRequestInfo, e);

        // TODO this should be a constant right?
        actionRedirectUrl.setParameter("portletActionError", targetWindowId
          .toString());
      }
    }
  ...
  public void resourceRequest(HttpServletRequest request,
    HttpServletResponse response) {
    final IPortalRequestInfo portalRequestInfo = this.urlSyntaxProvider
      .getPortalRequestInfo(request);
    final IPortletRequestInfo portletRequestInfo = portalRequestInfo
    ...
    if (portletRequestInfo != null) {
      final IPortletWindowId targetWindowId = portletRequestInfo
        .getPortletWindowId();
      this.portletExecutionManager.doPortletServeResource(targetWindowId, request,
        response);
    } else {
  ...
  private void sendRedirect(final IPortalUrlBuilder portalUurl,
