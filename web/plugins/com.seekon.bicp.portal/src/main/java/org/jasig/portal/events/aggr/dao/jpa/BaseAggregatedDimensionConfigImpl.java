...
/**
 * Base impl for aggregated dimension configs, if the subclass does not override
 * {@link #isIncluded(Object)} it should call {@link #clearIncludedCache()}
 * after the includes or excludes are modified.
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
@MappedSuperclass
public abstract class BaseAggregatedDimensionConfigImpl<D> implements
  BaseAggregatedDimensionConfig<D> {
  ...
  public boolean isIncluded(D dimension) {
    ...
    final boolean included = IncludeExcludeUtils.included(dimension, this
      .getIncluded(), this.getExcluded());
