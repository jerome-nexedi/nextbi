...
/**
 * Base for portlet execution dispatching. Tracks the target, request, response
 * objects as well as submitted, started and completed timestamps.
 */
abstract class PortletExecutionWorker<V> implements IPortletExecutionWorker<V> {
  ...
  public PortletExecutionWorker(ExecutorService executorService,
    List<IPortletExecutionInterceptor> interceptors,
    IPortletRenderer portletRenderer, HttpServletRequest request,
    HttpServletResponse response, IPortletWindow portletWindow, long timeout /*
                                                                              * IPortletWindowId
                                                                              * portletWindowId
                                                                              * ,
                                                                              * String
    ...
    this.portletFname = portletWindow.getPortletEntity().getPortletDefinition()
      .getFName();
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.portlet.rendering.worker.IPortletExecutionContext#
   * getPortletFname()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#submit()
   */
  @Override
  public final void submit() {
    if (this.submitted > 0) {
      throw new IllegalStateException(this.getClass().getSimpleName() + " for "
        + this.getPortletWindowId() + " has already been submitted.");
    }
    ...
    /*
     * Time to prepare a Callable for the executorService; choose whether to
     * create a normal Callable (that invokes the portlet code), or a special
     * (dummy) Callable that throws an Exception indicating the portlet has too
     * many errant worker threads in the hungWorker queue.
     */
    Callable<V> callable = null;
    if (this.portletRenderer.getHungWorkerAnalyzer()
      .allowWorkerThreadAllocationForPortlet(portletFname)) {
        ...
        public V call() throws Exception {
          ...
          try {
            ...
            for (final IPortletExecutionInterceptor interceptor : PortletExecutionWorker.this.interceptors) {
              interceptor.preExecution(request, response,
                PortletExecutionWorker.this);
            }
            ...
            logger.warn(
              "Portlet '" + portletWindowId + "' failed with an exception", e);
            doPostExecution(e);
            throw e;
          } finally {
            complete = System.currentTimeMillis();
            if (logger.isDebugEnabled()) {
              logger.debug("Execution complete on portlet " + portletWindowId
                + " in " + getDuration() + "ms");
            }
      ...
      // All is NOT well -- replace the Callable with one that throws a
      ...
      callable = new PortletExecutionCallable<V>(new Callable<V>() {
        ...
        public V call() throws Exception {
          Exception e = new RuntimeException(
            "Portlet '"
              + portletFname
              + "' was not allocated a worker thread because it already has too many workers in a hung state.");
          logger.warn("Portlet '" + portletWindowId + "' failed with an exception",
            e);
          doPostExecution(e);
          throw e;
        }
  ...
  private void doPostExecution(Exception e) {
    ...
    final ListIterator<IPortletExecutionInterceptor> listIterator = this.interceptors
      .listIterator(this.interceptors.size());
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#isStarted
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#waitForStart
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#get(long)
   */
  @Override
  public V get(long timeout) throws Exception {
    ...
    try {
      ...
      final long waitTime = Math.max(0, timeout
        - (System.currentTimeMillis() - startTime));
      return this.future.get(waitTime, TimeUnit.MILLISECONDS);
    } catch (InterruptedException e) {
      this.logger.warn("Execution interrupted on portlet window "
        + this.portletWindowId, e);
      throw e;
    } catch (ExecutionException e) {
      this.logger.warn("Execution failed on portlet window " + this.portletWindowId,
        e);
      throw e;
    } catch (TimeoutException e) {
      final StringBuilder errorBuilder = new StringBuilder(
        "Execution timed out on portlet window ");
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#cancel()
   */
  @Override
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#
   * getCancelCount()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#getSubmitted
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#getStarted
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#getComplete
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#getWait()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#getDuration
   * ()
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.rendering.worker.IPortletExecutionWorker#toString
   * ()
   */
  @Override
  public String toString() {
    return "PortletExecutionWorker [" + "portletWindowId=" + this.portletWindowId
      + ", " + "started=" + this.started + ", " + "submitted=" + this.submitted
      + ", " + "complete=" + this.complete + ", " + "retrieved=" + this.retrieved
      + ", " + "canceled=" + this.canceled + ", " + "cancelCount="
      + this.cancelCount + ", " + "wait=" + this.getWait() + ", " + "duration="
      + this.getDuration() + "]";
  }
}