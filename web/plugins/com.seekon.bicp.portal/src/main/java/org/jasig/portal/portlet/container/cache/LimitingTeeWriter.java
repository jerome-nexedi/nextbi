...
/**
 * Subclass of {@link TeeWriter} that stops writing to the branch once the limit
 * is hit by calling {@link #setBranch(Writer)} with {@link NullWriter}.
 * 
...
public class LimitingTeeWriter extends TeeWriter {
  ...
  /**
   * Sets the character count back to 0. If {@link #isLimitReached()} is true it
   * is set back to false and the original branch {@link Writer} is set as the
  ...
  protected void beforeWrite(int n) throws IOException {
    ...
    if (this.maximumCharacters > 0 && !this.limitReached
