...
public class GroupService implements IGroupConstants {
   ...
   * HttpSession. In response, we remove the corresponding group member from the
   * cache.
   * 
   * @param person
  ...
  /**
   * Refers to the PropertiesManager to get the key for the group associated
   * with 'name' and asks the group store implementation for the corresponding
   ...
   * <code> IEntity </code> .
   */
  public static IGroupMember getGroupMember(String key, Class type)
    throws GroupsException {
   ...
   */
  public static IGroupMember getGroupMember(
   ...
   * associated with 'type' and asks the group store implementation for the
   * corresponding <code>IEntityGroup</code>.
   */
   ...
   * Receives notice that the UserInstance has been unbound from the
   * HttpSession. In response, we remove the corresponding group member from the
   * cache. We use the roundabout route of creating a group member and then
   * getting its EntityIdentifier because we need the EntityIdentifier for the
   * group member, which is cached, not the EntityIdentifier for the IPerson,
   * which is not.
   * 
   * @param person
   *          org.jasig.portal.security.IPerson
  ...
  protected void ifinishedSession(IPerson person) throws GroupsException {
    ...
    try {
      ...
      EntityCachingService.getEntityCachingService().remove(
        entityIdentifier.getType(), entityIdentifier.getKey());
    } catch (CachingException ce) {
      throw new GroupsException("Problem removing group member " + gm.getKey()
        + " from cache", ce);
    }
  }
   ...
   * @return org.jasig.portal.groups.IEntity
   */
  protected IEntity igetEntity(String key, Class type, String service)
    throws GroupsException {
   ...
   * <code>IEntity</code>.
   */
  protected IGroupMember igetGroupMember(String key, Class type)
    throws GroupsException {
   ...
   * @return org.jasig.portal.groups.IEntityGroup
   */
  protected IEntityGroup inewGroup(Class type, String serviceName)
    throws GroupsException {
    try {
      return compositeGroupService.newGroup(type, parseServiceName(serviceName));
    } catch (InvalidNameException ine) {
      throw new GroupsException("GroupService.inewGroup(): invalid service name",
        ine);
    }
  }
  ...
  private void initialize() throws GroupsException {
    composite = false;
    String eMsg = null;
    String factoryName = PropertiesManager
      .getProperty("org.jasig.portal.groups.GroupServiceFactory");
    ...
    try {
      IGroupServiceFactory groupServiceFactory = (IGroupServiceFactory) Class
  ...
  private void initializeCompositeService() throws GroupsException {
    ...
    try {
      ...
      ICompositeGroupServiceFactory serviceFactory = (ICompositeGroupServiceFactory) Class
        .forName(factoryName).newInstance();
  ...
  /**
   * Answer if the underlying group service is a composite service, implementing
   ...
   * @return org.jasig.portal.groups.IEntityGroup
   */
  public static IEntityGroup newGroup(Class type, String serviceName)
    throws GroupsException {
  ...
  public static String parseLocalKey(String compositeKey)
    ...
    return new CompositeEntityIdentifier(compositeKey, new Object().getClass())
      .getLocalKey();
  }
  ...
  public static EntityIdentifier[] searchForEntities(String query, int method,
  ...
  public static EntityIdentifier[] searchForGroups(String query, int method,
