...
public class EditManager {
  ...
  /**
   * Get the edit set if any stored in the passed in node. If not found and if
   * the create flag is true then create a new edit set and add it as a child to
   * the passed in node. Then return it.
   */
  private static Element getEditSet(Element node, Document plf, IPerson person,
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new edit set node " + "Id for userId=" + person.getID(), e);
    }
  ...
  /**
   * Create and append an edit directive to the edit set if not there. This only
   * records that the attribute was changed and the value in the plf copy node
   * should be used, if allowed, during the merge at login time.
   */
  ...
  /**
   * Create and append a user preferences edit directive to the edit set if not
   * there. This only records that the attribute was changed. The value will be
   * in the user preferences object for the user.
   */
  public static void addPrefsDirective(Element plfNode, String attributeName,
   ...
   */
  private static void addDirective(Element plfNode, String attributeName,
    String type, IPerson person) throws PortalException {
    ...
    while (child != null && edit == null) {
      if (child.getNodeName().equals(type)
        && child.getAttribute(Constants.ATT_NAME).equals(attributeName))
        edit = child;
    ...
    if (edit == null) // if not found then newly mark as edited
    {
      ...
      try {
        ...
        throw new PortalException("Exception encountered while "
          + "generating new edit node " + "Id for userId=" + person.getID(), e);
      }
  ...
  /**
   * Evaluate whether attribute changes exist in the ilfChild and if so apply
   * them. Returns true if some changes existed. If changes existed but matched
   * those in the original node then they are not applicable, are removed from
   * the editSet, and false is returned.
   */
  ...
  /**
   * Searches for a dlm:pref command which indicates that a user preference was
   * change and if found removes it from the user's PLF.
   */
  public static void removePreferenceDirective(IPerson person, String elementId,
   ...
   * reset to the value in the fragment and if found removes it from the user's
   * PLF.
   */
  public static void removeEditDirective(String elementId, String attributeName,
    IPerson person) {
   ...
   * the user's PLF.
   */
  private static void removeDirective(String elementId, String attributeName,
    String type, IPerson person) {
    Document plf = (Document) person.getAttribute(Constants.PLF);
    ...
    try {
       ...
       * create=false meaning that the only portion of that method that tosses
       * an exception will not be reached with this call. But if a runtime
       * exception somehow occurs we will log it so that we don't lose the
       * information.
       */
      LOG.error(e, e);
      return;
