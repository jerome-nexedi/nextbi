...
public class DatabaseMetaDataImpl implements IDatabaseMetadata, InitializingBean {
  ...
  private static final JoinQueryString jdbcDb = new DatabaseMetaDataImpl.JdbcDb(
    "{oj UP_USER LEFT OUTER JOIN UP_USER_LAYOUT ON UP_USER.USER_ID = UP_USER_LAYOUT.USER_ID} WHERE");

  private static final JoinQueryString postgreSQLDb = new DatabaseMetaDataImpl.PostgreSQLDb(
    "UP_USER LEFT OUTER JOIN UP_USER_LAYOUT ON UP_USER.USER_ID = UP_USER_LAYOUT.USER_ID WHERE");

  private static final JoinQueryString oracleDb = new DatabaseMetaDataImpl.OracleDb(
    "UP_USER, UP_USER_LAYOUT WHERE UP_USER.USER_ID = UP_USER_LAYOUT.USER_ID(+) AND");

  /** Array of join tests to perform. */
  private static final JoinQueryString[] joinTests = { oracleDb, postgreSQLDb,
    jdbcDb };
   ...
   * 
   * @param ds
   *          The {@link DataSource} to use as the base for this server
   *          interface.
   */
  ...
  public String sqlTimeStamp(final long date) {
    ...
    if (useToDate) {
      final SimpleDateFormat oracleToDate = new SimpleDateFormat(
        "yyyy MM dd HH:mm:ss");
  ...
  public String toString() {
    ...
    if (this.portalTablesExist) {
      ...
      dbInfo
        .append("    WARNING: uPortal tables do no exist, not all meta-data tests were executed.");
    }
  ...
  private void runDatabaseTests() {
    ...
    try {
      ...
      /*
       * We must throw a RuntimeException here to avoid starting the portal with
       * incorrect assumptions about what the database supports.
       */
  ...
  private void testDatabaseInitialized(final SimpleJdbcTemplate jdbcTemplate) {
    try {
      ...
      LOG
        .warn("The uPortal database is not initialized, the database tests will not be performed.");
    }
  }
    ...
    if (this.dbmdSupportsOuterJoins) {
      for (final JoinQueryString joinQueryString : joinTests) {
        ...
        try {
          ...
          final String logMessage = "Join test failed: "
            + joinQueryString.getClass().getName() + " on statement: '"
            + joinTestQuery + "':";
  ...
  private void testTimeStamp(final SimpleJdbcTemplate jdbcTemplate) {
    try {
      ...
      // Try using TO_DATE()
      try {
        final String toDateTestQuery = "SELECT USER_ID "
          + "FROM UP_USER "
