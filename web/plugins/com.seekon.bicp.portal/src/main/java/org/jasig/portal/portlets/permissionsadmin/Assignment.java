...
public class Assignment implements Comparable<Assignment>, Serializable {
   ...
   * 
   * @param principal
   *          User or group to which this permissions record applies
   */
   ...
   *          User or group to which this permissions record applies
   * @param type
   *          Either {@see Type.INHERIT}, {@see Type.GRANT}, or {@see Type.DENY}
   */
  ...
  public Assignment findDecendentOrSelfIfExists(JsonEntityBean principal) {
    ...
    if (principal.getId().equals(this.principal.getId())
      && principal.getEntityTypeAsString().equals(
        this.principal.getEntityTypeAsString())) {
      rslt = this;
    } else {
