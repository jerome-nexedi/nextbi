 ...
 * {@link CharacterEventTypes#PORTLET_HEADER} and
 * {@link CharacterEventTypes#PORTLET_CONTENT} element in the event stream
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public class PortletRenderingInitiationCharacterComponent extends
  ...
  public PipelineEventReader<CharacterEventReader, CharacterEvent> getEventReader(
    ...
    while (eventReader.hasNext()) {
      ...
      case PORTLET_HEADER: {
        ...
        if (!this.portletExecutionManager.isPortletRenderHeaderRequested(
          ...
          this.portletExecutionManager.startPortletHeaderRender(portletWindowId,
            request, response);
        }
      ...
      case PORTLET_CONTENT: {
        ...
        if (!this.portletExecutionManager.isPortletRenderRequested(portletWindowId,
          request, response)) {
          this.portletExecutionManager.startPortletRender(portletWindowId, request,
            response);
        }
    ...
    final Map<String, String> outputProperties = pipelineEventReader
      .getOutputProperties();
    final CharacterEventBufferReader bufferEventReader = new CharacterEventBufferReader(
