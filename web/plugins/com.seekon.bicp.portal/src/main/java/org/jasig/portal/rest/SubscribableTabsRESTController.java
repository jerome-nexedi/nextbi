...
public class SubscribableTabsRESTController {
  ...
  public ModelAndView getSubscriptionList(HttpServletRequest request) {
    ...
    IAuthorizationPrincipal principal = authService.newPrincipal(person
     ...
     * determine if the current user has permissions to subscribe to each. Any
     * subscribable fragments will be transformed into a JSON-friendly bean and
     * added to the model.
     */
    ...
    final List<FragmentDefinition> fragmentDefinitions = configurationLoader
      .getFragments();
    ...
    for (FragmentDefinition fragmentDefinition : fragmentDefinitions) {
      ...
      if (isSubscribable(fragmentDefinition, principal)) {
        ...
        if (principal.hasPermission("UP_FRAGMENT", "FRAGMENT_SUBSCRIBE", owner)) {
          ...
          final String name = getMessage("fragment." + owner + ".name",
            fragmentDefinition.getName(), locale);
          final String description = getMessage(
            "fragment." + owner + ".description", fragmentDefinition
              .getDescription(), locale);
          SubscribableFragment jsonFragment = new SubscribableFragment(name,
            description, owner, subscribed);
  ...
  protected boolean isSubscribable(FragmentDefinition definition,
  ...
  public class SubscribableFragment {
    ...
    public SubscribableFragment(String name, String description, String ownerId,
      boolean subscribed) {
      this.name = name;
