...
/**
 * Utility for caching portlet entities and entity data in memory. Ensures a
 * consistent view for accessing the data by different sets of keys
 * 
...
class PortletEntityCache<T extends IPortletEntityDescriptor> {
  ...
  /**
   * @param threadSafe
   *          If set to false no locking is done around read or write operations
  ...
  public T storeIfAbsentEntity(IPortletEntityId portletEntityId,
    Function<IPortletEntityId, T> entityCreator) {
  ...
  public void storeEntity(T entity) {
    writeLock.lock();
    try {
      final SubscribeKey subscribeKey = new SubscribeKey(entity.getUserId(), entity
        .getLayoutNodeId());
