...
public class PersonDirectoryCacheKeyGenerator implements CacheKeyGenerator {
  private static final Serializable POSSIBLE_USER_ATTRIBUTE_NAMES_VALUE = new Serializable() {
    private static final long serialVersionUID = 1L;
  };

  private static final Serializable AVAILABLE_QUERY_ATTRIBUTES_VALUE = new Serializable() {
    private static final long serialVersionUID = 1L;
  };
  ...
  /**
   * If seed attributes with empty values (null, empty string or empty list
   * values) should be ignored when generating the cache key. Defaults to false.
   */
  ...
  public Serializable generateKey(MethodInvocation methodInvocation) {
    ...
    final CachableMethod cachableMethod = this
      .resolveCacheableMethod(methodInvocation);

    // Use the resolved cachableMethod to determine the seed Map and then get
    // the hash of the key elements
    final Object[] methodArguments = methodInvocation.getArguments();

    final CacheKeyBuilder<String, Serializable> cacheKeyBuilder = CacheKey
      .builder(cachableMethod.getName());

    switch (cachableMethod) {
    // Both methods that take a Map argument can just have the first argument
    // returned
    case PEOPLE_MAP:
    case PEOPLE_MULTIVALUED_MAP:
    case MULTIVALUED_USER_ATTRIBUTES__MAP:
    case USER_ATTRIBUTES__MAP: {
      for (final Map.Entry<String, Object> e : ((Map<String, Object>) methodArguments[0])
        .entrySet()) {
        ...
        if (ignoreEmptyAttributes
          && (value == null
            ...
            || (value instanceof Map && ((Map) value).isEmpty()) || (value
            .getClass().isArray() && Array.getLength(value) == 0))) {
          continue;
        }
      ...
      // The multivalued attributes with a string needs to be converted to
    ...
    case MULTIVALUED_USER_ATTRIBUTES__STR: {
      ...
      cacheKeyBuilder.put(this.defaultAttributeName, (Serializable) Collections
        .singletonList(uid));
      break;
    }

      // The single valued attributes with a string needs to be converted to
      // Map<String, Object>
    case PERSON_STR:
    case USER_ATTRIBUTES__STR: {
      ...
      // The getPossibleUserAttributeNames has a special Map seed that we return
      // to represent calls to it
    case POSSIBLE_USER_ATTRIBUTE_NAMES: {
      cacheKeyBuilder.put("getPossibleUserAttributeNames_seedMap",
        POSSIBLE_USER_ATTRIBUTE_NAMES_VALUE);
      break;
    }

      // The getAvailableQueryAttributes has a special Map seed that we return
      // to represent calls to it
    case AVAILABLE_QUERY_ATTRIBUTES: {
      cacheKeyBuilder.put("getAvailableQueryAttributes_seedMap",
        AVAILABLE_QUERY_ATTRIBUTES_VALUE);
      break;
    }

    default: {
      throw new IllegalArgumentException("Unsupported CachableMethod resolved: '"
        + cachableMethod + "'");
    }
    }
    ...
    if (cacheKeyBuilder.size() == 0) {
      if (this.logger.isDebugEnabled()) {
        this.logger.debug("No cache key generated for MethodInvocation='"
          + methodInvocation + "'");
      }
   ...
   * instance the passed {@link MethodInvocation} applies to.
   */
  protected CachableMethod resolveCacheableMethod(MethodInvocation methodInvocation) {
    ...
    for (final CachableMethod cachableMethod : CachableMethod.values()) {
      ...
      try {
        cacheableMethod = targetClass.getMethod(cachableMethod.getName(),
          cachableMethod.getArgs());
      } catch (SecurityException e) {
        this.logger.warn(
          "Security exception while attempting to if the target class '"
            + targetClass + "' implements the cachable method '" + cachableMethod
            + "'", e);
    ...
    throw new IllegalArgumentException(
      "Do not know how to generate a cache for for '" + targetMethod
        + "' on class '" + targetClass
        + "'. Is the advice applied to the correct bean and methods?");
  }

  /**
   * Methods on {@link org.jasig.services.persondir.IPersonAttributeDao} that
   * are cachable
   */
  public enum CachableMethod {
    ...
    PERSON_STR("getPerson", String.class), PEOPLE_MAP("getPeople", Map.class), PEOPLE_MULTIVALUED_MAP(
      ...
      "getPossibleUserAttributeNames"), AVAILABLE_QUERY_ATTRIBUTES(
