...
public class EventProviderImpl implements EventProvider {
  ...
  public EventProviderImpl(IPortletWindow portletWindow,
    ...
    try {
      this.portletClassLoader = portletContextService
        .getClassLoader(portletApplicationName);
    } catch (PortletContainerException e) {
      throw new IllegalStateException(
        "Failed to find ClassLoader for portlet applicaiton: "
    ...
    if (this.isDeclaredAsPublishingEvent(qname)) {
      ...
      try {
        ...
        try {
          ...
          final JAXBElement<Serializable> element = new JAXBElement<Serializable>(
            qname, clazz, value);
  ...
  private boolean isDeclaredAsPublishingEvent(QName qname) {
    final PortletDefinition portletDescriptor = this.portletWindow
      .getPlutoPortletWindow().getPortletDefinition();
    final List<? extends EventDefinitionReference> events = portletDescriptor
    ...
    final PortletApplicationDefinition application = portletDescriptor
      .getApplication();
  ...
  private boolean isValueInstanceOfDefinedClass(QName qname, Serializable value) {
    final PortletDefinition portletDefinition = this.portletWindow
      .getPlutoPortletWindow().getPortletDefinition();
