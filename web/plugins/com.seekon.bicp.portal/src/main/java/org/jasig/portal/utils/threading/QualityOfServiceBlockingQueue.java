...
/**
 * A thread-safe blocking queue that places elements into sub-queues based on
 * the key returned for each element by {@link #getElementKey(Object)}.
 * Implementations are responsible for providing the logic to determine the key
 * for an element and to determine the order in which queued elements are
 * returned via {@link #take()}, {@link #poll()}, {@link #poll(long, TimeUnit)},
 * {@link #remove()}, {@link #element()}, {@link #peek()},
 * {@link #drainTo(Collection)}, and {@link #drainTo(Collection, int)}
 * <p/>
 * The class appropriately handles {@link #peek()} such that the peeked element
 * will be the element operated on by {@link #take()}, {@link #poll()},
 * {@link #poll(long, TimeUnit)}, {@link #remove()}, {@link #element()},
 * {@link #drainTo(Collection)}, and {@link #drainTo(Collection, int)} no matter
 * how much time has elapsed
 * 
 * @author Eric Dalquist
 * @param <K>
 *          The type of key used for grouping elements in the queue
 * @param <T>
 *          The type of elements in the queue
 */
public abstract class QualityOfServiceBlockingQueue<K, T> implements
  BlockingQueue<T> {
  private final ConcurrentMap<K, Queue<T>> keyedQueues = new ConcurrentHashMap<K, Queue<T>>();

  private final Set<K> queueKeySet = Collections.unmodifiableSet(this.keyedQueues
    .keySet());
  ...
  // Track the total size of the queue and peek data, these fields MUST be
  // accessed within a read or write lock and
  ...
  /**
   * Get the next key to use for a call to {@link #take()}, {@link #poll()},
   * {@link #poll(long, TimeUnit)}, {@link #remove()}, {@link #element()},
   * {@link #peek()}, {@link #drainTo(Collection)}, or
   * {@link #drainTo(Collection, int)}
   * 
   * This method will only be called if {@link #isEmpty()} is false and will
   * never be called concurrently. It must only return a key for which
   * {@link #isKeyEmpty(Object)} returns false;
   */
   ...
   */
  @Override
  public final boolean offer(T e, long timeout, TimeUnit unit)
   ...
   */
  @Override
  public final T poll(final long timeout, final TimeUnit unit)
    throws InterruptedException {
    final long start = getWriteLockWithOptionalWait(timeout, unit);
    if (start == Long.MIN_VALUE) {
      // Min value signals a timeout while waiting
  ...
  public final <AT> AT[] toArray(AT[] a) {
    ...
    try {
      ...
      if (a.length < this.size) {
        a = (AT[]) java.lang.reflect.Array.newInstance(a.getClass()
   ...
   * 
   * @param failWhenFull
   *          If true and the queue is at capacity then this method throws a
   ...
   * @param create
   *          If true a Queue will be created for the element if one does not
   *          already exist
   */
  ...
  /**
   * Combination peek/poll method that uses a boolean parameter to switch
   * between the two behaviors
   * 
   * @param peek
   *          If true this method returns the peeked element, if false it
  ...
  private T pollInternal(boolean peek) {
    ...
    try {
      ...
      // Get the associated Queue and sanitity check the value from
      // getNextElementKey()
      final Queue<T> queue = this.keyedQueues.get(key);
      if (queue == null || queue.isEmpty()) {
        throw new IllegalStateException(
  ...
  /**
   * This MUST be called while {@link #writeLock} is locked by the current
   * thread
   */
  private boolean waitForRemove(long waitStart, long maxWait)
    throws InterruptedException {
    if (this.size == this.capacity) {
  ...
  /**
   * Acquires a write lock either using {@link Lock#tryLock(long, TimeUnit)} if
   * timeout >= 0 or using {@link Lock#lock()} if timeout < 0.
   * 
   * @param timeout
   *          Duration to wait for lock, if less than 0 will wait forever via
   *          {@link Lock#lock()}
   * @param unit
   *          Time units for timeout
   * @return If waiting with timeout the {@link System#currentTimeMillis()} that
   *         the waiting started, if waiting with timeout timed out will return
   *         {@value Long#MIN_VALUE}
   * @throws InterruptedException
   */
  private long getWriteLockWithOptionalWait(final long timeout, final TimeUnit unit)
    throws InterruptedException {
    // Get the write lock and capture start time and max wait time if waiting a
    // specified timeout
    final long start;
    if (timeout >= 0) {
   ...
   * thread
   */
  private boolean waitForAdd(long waitStart, long maxWait)
    throws InterruptedException {
    while (this.size == 0) {
   ...
   * @param maxWait
   *          The maximum time in milliseconds to wait, waits forever if less
   *          than 0
   * @param waitStart
   *          The original start time of the waiting, only used if maxWait is >=
   *          0
   */
  private boolean waitOnCondition(final Condition condition, long maxWait,
    long waitStart) throws InterruptedException {
    if (maxWait >= 0) {
  ...
  public String toString() {
    ...
    try {
      ...
      for (final Iterator<Entry<K, Queue<T>>> entryItr = this.keyedQueues.entrySet()
        .iterator(); entryItr.hasNext();) {
