 ...
 * @version $Revision$
 */
public abstract class CachingPipelineComponent<R, E> extends
  PipelineComponentWrapper<R, E> implements BeanNameAware {
  ...
  @Override
  public final PipelineEventReader<R, E> getEventReader(HttpServletRequest request,
    HttpServletResponse response) {
    ...
    // Get the key for this request from the target component and see if there
    // is a cache entry
    ...
    if (cachedEventReader == null) {
      // No cached data for key, call target component to get events and an
      // updated cache key
      logger.debug("{} - No cached events found for key {}, calling parent",
        this.beanName, cacheKey);
      final PipelineEventReader<R, E> pipelineEventReader = this.wrappedComponent
      ...
      final Map<String, String> outputProperties = pipelineEventReader
        .getOutputProperties();
      cachedEventReader = new CachedEventReader<E>(eventCache,
        new LinkedHashMap<String, String>(outputProperties));

      // Cache the buffer
      element = new Element(cacheKey, cachedEventReader);
      this.cache.put(element);
      logger.debug("{} - Cached {} events for key {}", new Object[] { this.beanName,
        eventCache.size(), cacheKey });
    } else {
      logger.debug("{} - Founed  cached events for key {}", new Object[] {
        this.beanName, cacheKey });
    }

    final List<E> eventCache = cachedEventReader.getEventCache();
    final Map<String, String> outputProperties = cachedEventReader
      .getOutputProperties();
  ...
  // Ugly!!! Needed because XMLEventReader implements Iterator but does not
  // parameterize it
