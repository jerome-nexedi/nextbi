...
/**
 * Receives the username and password and tries to authenticate the user. The
 * form presented by org.jasig.portal.channels.CLogin is typically used to
 * generate the post to this servlet.
 * 
 ...
 *         and credential for each security context.
 */
@Controller
...
public class LoginController {
  ...
  public static final String SWAP_TARGET_UID = LoginController.class.getName()
    + ".SWAP_TARGET_UID";

  public static final String SWAP_ORIGINAL_UID = LoginController.class.getName()
    + ".SWAP_ORIGINAL_UID";
  ...
  public void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (refUrl != null) {
      if (refUrl.startsWith("/")) {
        ...
        log
          .warn("Refernce URL passed in does not start with a / and will be ignored: "
            + refUrl);
      }
    }
    ...
    if (redirectTarget == null) {
      /*
       * Grab the target functional name, if any, off the login request. Also
       * any arguments for the target We will pass them along after
       * authentication.
       */
      String targetFname = request.getParameter("uP_fname");

      if (targetFname == null) {
        final IPortalUrlBuilder defaultUrl = this.portalUrlProvider
          .getDefaultUrl(request);
        ...
        try {
          ...
          while (e.hasMoreElements()) {
            ...
            if (!paramName.equals("uP_fname")) {
              urlBuilder.addParameter(paramName, request
    ...
    final Object authError = request.getSession(false).getAttribute(
      LoginController.AUTH_ERROR_KEY);
    ...
    if (person == null || !person.getSecurityContext().isAuthenticated()) {
      ...
      // Preserve the attempted username so it can be redisplayed to the user by
      // CLogin
      String attemptedUserName = request.getParameter("userName");
      if (attemptedUserName != null)
        request.getSession(false).setAttribute(ATTEMPTED_USERNAME_KEY,
