...
/**
 * CacheFactory impl that provides Map instances that wrap a configured
 * {@link CacheProviderFacade}. This uses the {@link MapCacheProvider} to
 * perform the wrapping. Refer to that class for which operations on the
 * {@link Map} interface are supported.
 * 
...
public class CacheProviderFactory implements CacheFactory {
  ...
  // Stores caches that are created with a soft reference to the cache Map to
  // avoid re-creating wrapper objects without need
  ...
  private class MapCacheCreator extends
    MapCachingDoubleCheckedCreator<String, Map<?, ?>> {
    @SuppressWarnings("unchecked")
     ...
     * 
     * @see
     * org.jasig.portal.utils.threading.MapCachingDoubleCheckedCreator#getKey
     * (java.lang.Object[])
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @seeorg.jasig.portal.utils.threading.MapCachingDoubleCheckedCreator#
     * createInternal(java.lang.Object, java.lang.Object[])
     */
    @SuppressWarnings("unchecked")
    @Override
    protected Map<?, ?> createInternal(String cacheName, Object... args) {
      ...
      if (cacheManager.cacheExists(cacheName)) {
        ...
        try {
          ...
          // Ignore, some other thread created the cache while we were trying to
          // do the same thing
        }
        cache = cacheManager.getCache(cacheName);
