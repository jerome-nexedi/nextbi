...
/**
 * Interface for a cache factory that returns a cache that externally acts like
 * a map. Underlying caches don't have to be a map, but they must be wrapped in
 * the map interface if they do not.
 * 
 * @author Scott Battaglia
 * @version $Revision$ $Date$
 * @deprecated Inject the {@link Ehcache} instances directly into spring managed
 *             beans using @Qualifier to specify the cache name
 */
public interface CacheFactory {
   ...
   *           if a cache by that name cannot be retrieved.
   */
