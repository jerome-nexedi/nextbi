...
/**
 * @author George Lindholm <a
 *         href="mailto:George.Lindholm@ubc.ca">George.Lindholm@ubc.ca</a>
...
public class MovingAverage {
  ...
  public synchronized MovingAverageSample add(final long sample) {
    ...
    if (totalSamples >= samples.length) {
      sum -= samples[first]; // We've wrapped, so we can remove the 'first'
      // entry
    }
    sum += sample;
    ...
    return new MovingAverageSample(sum / arraySize, highMax, lastSample, max, min,
      totalSamples);
  }
}
