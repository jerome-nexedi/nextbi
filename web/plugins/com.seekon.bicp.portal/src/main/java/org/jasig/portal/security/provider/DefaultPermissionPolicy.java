...
/**
 * Implements a strategy for answering the basic authorization question: does
 * the <code>principal</code> have permission to perform the
 * <code>activity</code> on the <code>target</code>.
 * 
...
public class DefaultPermissionPolicy implements IPermissionPolicy {
   ...
   * <code>service</code>, <code>owner</code> and <code>activity</code> must be
   * non-null.
   * 
   * @return boolean
   * @param service
  ...
  public boolean doesPrincipalHavePermission(IAuthorizationService service,
    ...
    try {
      ...
      while (i.hasNext() && !hasPermission) {
        ...
        hasPermission = primDoesPrincipalHavePermission(prn, owner, activity,
          target, service);
      }
  ...
  private boolean permissionIsGranted(IPermission p) {
    Date now = new Date();
    return (p.getType().equals(IPermission.PERMISSION_TYPE_GRANT))
      && (p.getEffective() == null || !p.getEffective().after(now))
      && (p.getExpires() == null || p.getExpires().after(now));
  }

  /**
   * Answers if this specific principal (as opposed to its parents) has the
   * permission.
   * 
   * @return boolean
   * @param principal
   ...
   *              indicates authorization information could not be retrieved or
   *              was invalid.
   */
  private boolean primDoesPrincipalHavePermission(IAuthorizationPrincipal principal,
    String owner, String activity, String target, IAuthorizationService service)
