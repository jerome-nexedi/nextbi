...
public class PermissionAssignmentMapController extends AbstractPermissionsController {
  ...
  public ModelAndView updatePermission(@RequestParam("principal") String principal,
    ...
    final IPerson currentUser = personManager
      .getPerson((HttpServletRequest) request);
    ...
    if (bean != null) {
      ...
      IPermission[] directPermissions = permissionStore.select(owner, p
        .getPrincipalString(), activity, target, null);
      ...
      log.warn("Unable to resolve the following principal (will "
        + "be omitted from the list of assignments):  " + principal);
    }
  ...
  public ModelAndView getOwners(@RequestParam("principals[]") String[] principals,
    ...
    Map<String, Object> model = Collections.<String, Object> singletonMap(
      "assignments", assignments);
    return new ModelAndView("jsonView", model);
  }

  private void placeInHierarchy(Assignment a, List<Assignment> hierarchy,
    String owner, String activity, String target) {
    ...
    if (entityEnum.isGroup()) {
      ...
      member = GroupService.getGroupMember(a.getPrincipal().getId(), entityEnum
        .getClazz());
    }

    AuthorizationService authService = AuthorizationService.instance();
    Iterator<?> it = GroupService.getCompositeGroupService().findContainingGroups(
      member);
    if (it.hasNext()) {
      ...
      while (it.hasNext()) {
        ...
        if (parent == null) {
          ...
          Assignment.Type assignmentType = getAssignmentType(principal, owner,
            activity, target);
          parent = new Assignment(principal.getPrincipalString(), bean,
            assignmentType);
          parent.addChild(a);
  ...
  private Assignment.Type getAssignmentType(IAuthorizationPrincipal principal,
    String owner, String activity, String target) {
    IPermission[] directPermissions = permissionStore.select(owner, principal
      .getPrincipalString(), activity, target, null);
    Assignment.Type type;
    if (directPermissions.length > 0) {
      type = directPermissions[0].getType()
        .equals(IPermission.PERMISSION_TYPE_GRANT) ? Assignment.Type.GRANT
