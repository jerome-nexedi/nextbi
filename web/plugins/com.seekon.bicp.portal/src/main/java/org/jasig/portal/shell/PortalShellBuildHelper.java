...
/**
 * Utility used by the groovy scripts generated in build.xml and passed into
 * PortalShell
 * 
...
public class PortalShellBuildHelper {
  ...
  public void db(String target, String tablesFile, String dataFile,
    String scriptFile, boolean dropTables, boolean createTables,
    ...
    try {
      ...
      throw new RuntimeException(target + " for " + tablesFile + " and " + dataFile
        + " failed", e);
    }
  }

  public void hibernateCreate(String target, String schemaExportBeanName,
    boolean export, boolean create, boolean drop, String outputFile,
    boolean haltOnError) {
    final ISchemaExport schemaExportBean = this.schemaExportBeans
      .get(schemaExportBeanName);
    if (schemaExportBean == null) {
      throw new RuntimeException(target + " could not find schemaExportBean "
        + schemaExportBeanName);
    }
    ...
    try {
      ...
      throw new RuntimeException(
        target + " for " + schemaExportBeanName + " failed", e);
    }
  }

  public void hibernateUpdate(String target, String schemaExportBeanName,
    boolean export, String outputFile, boolean haltOnError) {
    final ISchemaExport schemaExportBean = this.schemaExportBeans
      .get(schemaExportBeanName);
    if (schemaExportBean == null) {
      throw new RuntimeException(target + " could not find schemaExportBean "
  ...
  public void dataList(String target, String type) {
    try {
      if (StringUtils.isBlank(type)) {
        ...
        PortalDataHandlerServiceUtils.format(portalDataHandlerService,
          PortalShell.LOGGER);
        PortalShell.LOGGER
          .info("Add -Dtype=dataType To get a list of data keys for the type");
      } else {
        // List Data Script
        final Iterable<? extends IPortalData> data = portalDataHandlerService
          .getPortalData(type);
        PortalShell.LOGGER.info("");
        PortalShell.LOGGER.info("");
  ...
  public void dataExport(String target, String dataDir, String type, String sysid,
    String logDir) {
    PortalShell.LOGGER.info("");
    PortalShell.LOGGER.info("");
    ...
    if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(sysid)) {
      try {
        for (final String id : COMMA_DELIM.split(sysid)) {
          PortalShell.LOGGER.info("Exporting Data " + id + " of type " + type
            + " to: " + dataDir);
          portalDataHandlerService.exportData(type, id, dataDirFile);
        }
      } catch (Exception e) {
        throw new RuntimeException(target + " to " + dataDir + " of " + type + " "
          + sysid + " failed", e);
      }
      ...
      try {
        ...
        if (types.size() == 1) {
          PortalShell.LOGGER.info("Exporting All Data of type " + type + " to: "
            + dataDir);
        } else {
          PortalShell.LOGGER.info("Exporting All Data of types " + types + " to: "
            + dataDir);
        }

        portalDataHandlerService.exportAllDataOfType(types, dataDirFile,
  ...
  public void dataImport(String target, String dataDir, String pattern, String file,
    String logDir) {
    PortalShell.LOGGER.info("");
    PortalShell.LOGGER.info("");
    ...
    if (StringUtils.isBlank(dataDir)) {
      ...
      PortalShell.LOGGER.info("Importing Data from: " + dataDir + " that matches "
        + pattern);
      pattern = StringUtils.trimToNull(pattern);

      try {
        portalDataHandlerService.importData(new File(dataDir), pattern,
          new IPortalDataHandlerService.BatchImportOptions()
            .setLogDirectoryParent(logDir));
      } catch (Exception e) {
        if (pattern != null) {
          throw new RuntimeException(target + " from " + dataDir + " matching "
            + pattern + " failed", e);
        } else {
  ...
  public void dataDelete(String target, String type, String sysid) {
    ...
    for (final String id : COMMA_DELIM.split(sysid)) {
      try {
        ...
        throw new RuntimeException(target + " for " + type + " and " + id
          + " failed", e);
      }
    }
  }
