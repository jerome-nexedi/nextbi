...
public class FragmentDefinition extends EvaluatorGroup {
  ...
  public static final Namespace NAMESPACE = DocumentHelper.createNamespace("dlm",
    NAMESPACE_URI);
  ...
  /*
   * These variables are bound to a uP userId later in the life cycle, not
   ...
   * 
   * @param e
   *          An Element representing a single <dlm:fragment> in dlm.xml.
   * @throws Exception
   */
  ...
  public void loadFromEelement(Element e) {
    ...
    this.defaultLayoutOwnerID = loadAttribute("defaultLayoutOwnerID", atts,
      NOT_REQUIRED, e);
  ...
  private void loadAudienceEvaluators(NodeList nodes) {
    ...
    for (int i = 0; i < nodes.getLength(); i++) {
      ...
      if (att == null || att.getNodeValue().equals(""))
        throw new RuntimeException("Required attibute '" + evaluatorFactoryAtt
          + "' " + "is missing or empty on 'audience' " + " element in\n'"
  ...
  private void addEvaluator(EvaluatorFactory factory, Node audience) {
    ...
    if (evaluator == null)
      throw new RuntimeException("Evaluator factory '"
        + factory.getClass().getName() + "' failed to "
        + "return an evaluator for 'audience' element" + " in\n'"
        + XmlUtilitiesImpl.toString(audience) + "'");
    if (evaluators == null) {
  ...
  private EvaluatorFactory loadEvaluatorFactory(String factoryClassName,
    ...
    try {
      ...
      throw new RuntimeException("java.lang.ClassNotFoundException occurred"
        + " while loading evaluator factory class '" + factoryClassName
        + "' (or one of its " + "dependent classes) for 'audience' element "
        + "in\n'" + XmlUtilitiesImpl.toString(audience) + "'");
    } catch (ExceptionInInitializerError eiie) {
      throw new RuntimeException("java.lang.ExceptionInInitializerError "
        + "occurred while " + "loading evaluator factory Class '" + factoryClassName
        + "' (or one of its " + "dependent classes) for 'audience' element "
        + "in\n'" + XmlUtilitiesImpl.toString(audience)
        + "'. \nThis indicates that an exception "
        + "occurred during evaluation of a static"
        + " initializer or the initializer for a " + "static variable.", eiie);
    } catch (LinkageError le) {
      throw new RuntimeException("java.lang.LinkageError occurred while "
        + "loading evaluator factory Class '" + factoryClassName + "' for "
        + "'audience' element in\n'" + XmlUtilitiesImpl.toString(audience)
        + "'. \nThis typically means that a " + "dependent class has changed "
        + "incompatibly after compiling the " + "factory class.", le);
    }
    ...
    try {
      ...
      throw new RuntimeException("java.lang.IllegalAccessException occurred "
        ...
        + "and that it contains a public, zero " + "argument constructor.", iae);
      ...
      throw new RuntimeException("java.lang.InstantiationException occurred "
        ...
        + "class and not an interface or abstract " + "class.", ie);
    }
    try {
  ...
  public boolean isApplicable(IPerson p) {
    ...
    if (LOG.isInfoEnabled())
      LOG.info("---- " + name + ".isApplicable( " + p.getAttribute("username")
        + " )=" + isApplicable);
    return isApplicable;
  }

  private String loadAttribute(String name, NamedNodeMap atts, boolean required,
    Element e) {
  ...
  public Class<? extends EvaluatorFactory> getFactoryClass() {
    String msg = "This method is not necessary for serializing "
      + "FragmentDefinition instances and should " + "not be invoked.";
