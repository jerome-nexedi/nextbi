...
/**
 * Used with code that serializes StAX events into a string. Watches for
 * specific XML tags in a StAX stream and chunks the string data at each XML tag
 * occurrence.
 * 
...
public class ChunkingEventReader extends BaseXMLEventReader {
  ...
  public ChunkingEventReader(HttpServletRequest request,
    ...
    Pattern[] chunkingPatterns, XMLEventReader xmlEventReader,
    XMLEventWriter xmlEventWriter, StringWriter writer) {
  ...
  private XMLEvent tryChunking(StartElement startElement) throws XMLStreamException {
    ...
    CharacterEventSource characterEventSource = this.chunkingElements
      .get(elementName.getLocalPart());
    while (characterEventSource != null) {
      final XMLEvent previousEvent = this.getPreviousEvent();
      if (previousEvent != null && previousEvent.isStartElement()) {
        this.captureEvent = startElement;

        // Write an empty Character event to force the serializer to finish
        // writing the previous StartElement
        // It is left open since ATTRIBUTE events can follow a START_ELEMENT
        // event.
      ...
      characterEventSource.generateCharacterEvents(this.request, parent,
        startElement, this.characterEvents);
  ...
  /**
   * Breaks up the String into a List of CharacterEvents based on the configured
   * Map of Patterns to CharacterEventSources
   */
  protected void chunkString(final List<CharacterEvent> characterEvents,
    final CharSequence buffer, int patternIndex) {
    ...
    for (; patternIndex < this.chunkingPatterns.length; patternIndex++) {
      ...
      if (matcher.find()) {
        final CharacterEventSource eventSource = this.chunkingPatternEventSources
          .get(pattern);
        int prevMatchEnd = 0;

        do {
          // Add all of the text up to the match as a new chunk, use subSequence
          // to avoid extra string alloc
          this.chunkString(characterEvents, buffer.subSequence(prevMatchEnd, matcher
            .start()), patternIndex + 1);

          // Get the generated CharacterEvents for the match
          final MatchResult matchResult = matcher.toMatchResult();
          eventSource.generateCharacterEvents(this.request, matchResult,
            characterEvents);
        ...
        if (prevMatchEnd < buffer.length()) {
          this.chunkString(characterEvents, buffer.subSequence(prevMatchEnd, buffer
            .length()), patternIndex + 1);
        }
