...
/**
 * Provides standard access to the portal's {@link ApplicationContext}. If
 * running in a web application a {@link WebApplicationContext} is available.
 * 
 * {@link #getApplicationContext()} should be used by most uPortal code that
 * needs access to the portal's {@link ApplicationContext}. It ensures that a
 * single {@link ApplicationContext} is used portal-wide both when the portal is
 * running as a web-application and when tools are run from the command line.
 * 
 * For legacy portal code that is not yet Spring managed and does not have
 * access to the {@link ServletContext} this class provides similar
 * functionality to {@link WebApplicationContextUtils} via the
 * {@link #getWebApplicationContext()} and
 * {@link #getRequiredWebApplicationContext()}. These methods are deprecated as
 * any code that requires a {@link WebApplicationContext} should either be
 * refactored as a Spring managed bean or have access to the
 * {@link ServletContext}
 * 
...
public class PortalApplicationContextLocator implements ServletContextListener {
  private static Log LOGGER = LogFactory
    .getLog(PortalApplicationContextLocator.class);
  ...
  /**
   * @return <code>true</code> if a WebApplicationContext is available,
   *         <code>false</code> if only an ApplicationContext is available
   * @deprecated Only needed for using
   *             {@link #getRequiredWebApplicationContext()} or
   ...
   *           if no ServletContext is available to retrieve a
   *           WebApplicationContext for or if the root WebApplicationContext
   *           could not be found
   * @deprecated This method is a work-around for areas in uPortal that do not
  ...
  /**
   * @return The WebApplicationContext for the portal, null if no ServletContext
   *         is available
   * @deprecated This method is a work-around for areas in uPortal that do not
   *             have the ability to use the
  ...
  /**
   * If running in a web application the existing {@link WebApplicationContext}
   * will be returned. if not a singleton {@link ApplicationContext} is created
   * if needed and returned. Unless a {@link WebApplicationContext} is
   * specifically needed this method should be used as it will work both when
   * running in and out of a web application
   * 
  ...
  public static ApplicationContext getApplicationContext() {
    ...
    if (context != null) {
      ...
      if (applicationContextCreator.isCreated()) {
        ...
        LOGGER.error("Stack trace of original ApplicationContext creator",
  ...
  /**
   * If the ApplicationContext returned by {@link #getApplicationContext()} is
   * 'portal managed' the shutdown hook for the context is called, closing and
   * cleaning up all spring managed resources.
   * 
   * If the ApplicationContext returned by {@link #getApplicationContext()} is
   * actually a WebApplicationContext this method does nothing but log an error
   * message.
   */
  public static void shutdown() {
    if (applicationContextCreator.isCreated()) {
      final ConfigurableApplicationContext applicationContext = applicationContextCreator
        .get();
  ...
  /**
   * Creator class that knows how to instantiate the lazily initialized portal
  ...
  private static class PortalApplicationContextCreator extends
    ...
    protected ConfigurableApplicationContext createSingleton(Object... args) {
      if (Boolean.getBoolean("org.jasig.portal.test")) {
        throw new IllegalStateException(PortalApplicationContextLocator.class
          .getName()
          + " MUST NOT be used in unit tests");
      }

      LOGGER
        .info("Creating new lazily initialized GenericApplicationContext for the portal");

      final long startTime = System.currentTimeMillis();
      // //2012, TODO
      // final GenericApplicationContext genericApplicationContext = new
      ...
      // //add 2012-05-15
      final OsgiBundleXmlWebApplicationContext genericApplicationContext = new OsgiBundleXmlWebApplicationContext();
      genericApplicationContext.setConfigLocation("/properties/contexts/*.xml");
      genericApplicationContext.setBundleContext((BundleContext) servletContext
        .getAttribute(BundleContext.class.getName()));
      genericApplicationContext.setServletContext(servletContext);
      ...
      LOGGER
        .info("Created new lazily initialized GenericApplicationContext for the portal in "
