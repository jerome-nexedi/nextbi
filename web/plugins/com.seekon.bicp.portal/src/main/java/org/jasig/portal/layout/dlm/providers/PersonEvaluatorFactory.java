...
/**
 * Implementation of the Evaluator Factory interface that creates evaluators of
 * string attributes in implementations of IPerson to determine if a user gets a
 * layout fragment.
 * 
...
public class PersonEvaluatorFactory implements EvaluatorFactory {
  ...
  private Evaluator getGroupEvaluator(int type, Node node) {
    ...
    if (nodes == null || nodes.getLength() == 0
      ...
      throw new RuntimeException("Invalid content. Expected one to many "
        + "<paren>, <NOT>, or <attribute> in '" + XmlUtilitiesImpl.toString(node)
        + "'");
    }
    return container;
  }
  ...
  private Evaluator createParen(Node n) {
    ...
    if (opNode == null)
      throw new RuntimeException("Invalid mode. Expected 'AND','OR', or 'NOT'"
        + " in '" + XmlUtilitiesImpl.toString(n) + "'");
   ...
   * subclasses can override this method to return the Evaluator that's
   * appropriate to their implementation.
   * 
   * @param name
   *          the attribute's name.
   * @param mode
   *          the attribute's mode. (i.e. 'equals')
   * @param value
