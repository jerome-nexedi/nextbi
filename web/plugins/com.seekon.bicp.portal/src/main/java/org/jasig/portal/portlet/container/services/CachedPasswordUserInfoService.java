...
public class CachedPasswordUserInfoService implements UserInfoService {
  ...
  /**
   * The default name of the preferences attribute used to pass the PT to the
   * portlet.
   */
  ...
  /**
   * Set whether the password should be decrypted before adding it to the user
   * info map.
   * 
   ...
   * (non-Javadoc)
   * 
   * @seeorg.apache.pluto.container.UserInfoService#getUserInfo(javax.portlet.
   * PortletRequest, org.apache.pluto.container.PortletWindow)
   */
  @Override
  public Map<String, String> getUserInfo(PortletRequest request,
    PortletWindow portletWindow) throws PortletContainerException {
    ...
    if (isPasswordRequested(request, portletWindow)) {
      ...
      final IUserInstance userInstance = userInstanceManager
   ...
   *          portlet window
   * @return <code>true</code> if a CAS proxy ticket is expected,
  ...
  public boolean isPasswordRequested(PortletRequest request,
    ...
    // check to see if the proxy ticket key is one of the requested user
    // attributes
    List<? extends UserAttribute> requestedUserAttributes = portletApplicationDescriptor
      .getUserAttributes();
  ...
  /**
   * Retrieves the users password by iterating over the user's security contexts
   * and returning the first available cached password.
   * 
   * @param baseContext
