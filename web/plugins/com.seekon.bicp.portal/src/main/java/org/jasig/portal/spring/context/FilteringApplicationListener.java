...
/**
 * Uses list of {@link ApplicationEventFilter} to decide if an event should be
...
public abstract class FilteringApplicationListener<E extends ApplicationEvent>
  ...
  private List<ApplicationEventFilter<E>> applicationEventFilters = Collections
    .emptyList();

  /**
   * @param applicationEventFilters
   *          The list of filters to test each event with
   */
  public final void setApplicationEventFilters(
    List<ApplicationEventFilter<E>> applicationEventFilters) {
    applicationEventFilters = new ArrayList<ApplicationEventFilter<E>>(
  ...
  public final void onApplicationEvent(E event) {
    for (final ApplicationEventFilter<E> applicationEventFilter : this.applicationEventFilters) {
      if (!applicationEventFilter.supports(event)) {
        logger.debug("Filter {} does not support event {} it will be ignored",
          applicationEventFilter, event);
        return;
      }
    }
  ...
  /**
   * @param event
   *          Event that has passed all configured
   *          {@link ApplicationEventFilter}s
   */
