 ...
 * @version $Revision$
 */
public class CachedPortletResourceData<T extends Serializable> implements
  CachedPortletResultHolder<T>, Serializable {
  ...
  public CachedPortletResourceData(CachedPortletData<T> cachedPortletData,
    Map<String, List<Serializable>> headers, Integer status,
    String characterEncoding, Integer contentLength, Locale locale) {

    this.cachedPortletData = cachedPortletData;

    final Builder<String, List<Serializable>> headersBuilder = ImmutableMap
      .builder();
  ...
  public final void replay(PortletResourceOutputHandler portletOutputHandler)
    throws IOException {
    // Write status
    if (status != null) {
