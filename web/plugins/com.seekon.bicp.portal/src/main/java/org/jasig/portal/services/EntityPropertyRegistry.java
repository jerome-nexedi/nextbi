...
/**
 * A Service to allow the querying and storing of properties relating to portal
 * entities. Configured using /properties/EntityPropertyRegistry.xml
 * 
 ...
 * @version $Revision$
 * 
 * @author Don Fracapane df7@columbia.edu Removed caching from this class and
 *         delegated it to the finder classes. Each finder can choose the method
 *         of caching if caching is appropriate.
 */
public class EntityPropertyRegistry {
  ...
  protected void init() throws Exception {
    ...
    if (ss.getLength() == 1) {
      ...
      this.store = (IEntityPropertyStore) Class.forName(s.getAttribute("impl"))
        .newInstance();
    ...
    for (int i = 0; i < ff.getLength(); i++) {
      ...
      finders[p] = (IEntityPropertyFinder) Class.forName(f.getAttribute("impl"))
        .newInstance();
    ...
    propsType = Class
      .forName("org.jasig.portal.services.entityproperties.EntityProperties");
  }
  ...
  public static void storeProperty(EntityIdentifier entityID, String name,
    String value) {
  ...
  protected String getPropKey(EntityIdentifier entityID) {
    String key = org.jasig.portal.EntityTypes.getEntityTypeID(entityID.getType())
      .toString()
      + "." + entityID.getKey();
    return key;
  }
  ...
  protected EntityProperties getProperties(EntityIdentifier entityID) {
    ...
    for (int i = 0; i < finders.length; i++) {
      ...
      if (i == storePrecedence) {
        ...
        if ((finderTypes[i] != null)
          && (finderTypes[i].equals("*") || entityID.getType()
            .equals(finderTypes[i]))) {
          finder = finders[i];
        } else {
          finder = null;
        }
  ...
  public void clearCache(EntityIdentifier entityID) {
    try {
      EntityCachingService.getEntityCachingService().remove(propsType,
        getPropKey(entityID));
  ...
  public EntityProperties getCachedProperties(EntityIdentifier entityID) {
    EntityProperties ep = null;
    try {
      ep = (EntityProperties) EntityCachingService.getEntityCachingService().get(
        propsType, entityID.getKey());
    } catch (CachingException e) {
