...
public class XmlUtilitiesImpl implements XmlUtilities {
  ...
  public Templates getTemplates(Resource stylesheet)
    ...
    final CachedResource<Templates> templates = this
      .getStylesheetCachedResource(stylesheet);
  ...
  public String serializeXMLEvents(List<XMLEvent> xmlEvents, boolean isHtml) {
    ...
    try {
      xmlEventWriter = new IndentingXMLEventWriter(outputFactory
  ...
  public Node convertToDom(XMLEventReader xmlEventReader) throws XMLStreamException {
    // Remove when woodstox 4.1.3 is released:
    // http://jira.codehaus.org/browse/WSTX-271
    ...
    final XMLEventWriter sourceWriter = xmlOutputFactory
      .createXMLEventWriter(sourceDom);
  ...
  public String getUniqueXPath(Node node) {
    ...
    for (; node != null && node.getNodeType() == Node.ELEMENT_NODE; node = node
      .getParentNode()) {
  ...
  /**
   * Gets the index of this element relative to other siblings with the same
   * node name
   */
  private int getElementIndex(Node node) {
    ...
    int count = 1;
    for (Node previousSibling = node.getPreviousSibling(); previousSibling != null; previousSibling = previousSibling
      .getPreviousSibling()) {
  ...
  public static String toString(Node node) {
    ...
    try {
      ...
      throw new RuntimeException(
        "Failed to convert Node to String using Transformer", e);
    }
