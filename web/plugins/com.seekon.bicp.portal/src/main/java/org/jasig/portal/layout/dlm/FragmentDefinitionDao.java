...
@Repository
public class FragmentDefinitionDao extends BaseJpaDao implements
    ...
    this.findFragmentByNameQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<FragmentDefinition>>() {
        ...
        public CriteriaQuery<FragmentDefinition> apply(CriteriaBuilder cb) {
          final CriteriaQuery<FragmentDefinition> criteriaQuery = cb
            .createQuery(FragmentDefinition.class);
          final Root<FragmentDefinition> fragDefRoot = criteriaQuery
            .from(FragmentDefinition.class);
          criteriaQuery.select(fragDefRoot);
          criteriaQuery.where(cb.equal(fragDefRoot.get(FragmentDefinition_.name),
            nameParameter));
      ...
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<FragmentDefinition>>() {
        ...
        public CriteriaQuery<FragmentDefinition> apply(CriteriaBuilder cb) {
          ...
          final Root<FragmentDefinition> fragDefRoot = criteriaQuery
            .from(FragmentDefinition.class);
          criteriaQuery.select(fragDefRoot);
          criteriaQuery.where(cb.equal(fragDefRoot.get(FragmentDefinition_.ownerID),
            ownerParameter));
  ...
  public List<FragmentDefinition> getAllFragments() {
    final TypedQuery<FragmentDefinition> query = this
      .createCachedQuery(this.findAllFragmentsQuery);
  ...
  public FragmentDefinition getFragmentDefinition(String name) {
    final TypedQuery<FragmentDefinition> query = this
      .createCachedQuery(this.findFragmentByNameQuery);
  ...
  public FragmentDefinition getFragmentDefinitionByOwner(String ownerId) {
    final TypedQuery<FragmentDefinition> query = this
      .createCachedQuery(this.findFragmentByOwnerQuery);
