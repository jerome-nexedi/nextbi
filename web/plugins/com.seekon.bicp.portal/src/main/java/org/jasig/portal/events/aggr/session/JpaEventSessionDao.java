...
public class JpaEventSessionDao extends BaseJpaDao implements EventSessionDao {
  ...
  public void afterPropertiesSet() throws Exception {
    this.eventSessionIdParameter = this.createParameterExpression(String.class,
      "eventSessionId");
    this.dateTimeParameter = this.createParameterExpression(DateTime.class,
      "dateTime");

    this.findByEventSessionIdQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<EventSessionImpl>>() {
        @Override
        public CriteriaQuery<EventSessionImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<EventSessionImpl> criteriaQuery = cb
            .createQuery(EventSessionImpl.class);
          final Root<EventSessionImpl> root = criteriaQuery
            .from(EventSessionImpl.class);
          criteriaQuery.select(root);
          criteriaQuery.where(cb.equal(root.get(EventSessionImpl_.eventSessionId),
            eventSessionIdParameter));

          return criteriaQuery;
        }
      });

    this.findExpiredEventSessionsQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<EventSessionImpl>>() {
        @Override
        public CriteriaQuery<EventSessionImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<EventSessionImpl> criteriaQuery = cb
          ...
          final Root<EventSessionImpl> root = criteriaQuery
            .from(EventSessionImpl.class);
          criteriaQuery.select(root);
          criteriaQuery.where(cb.lessThanOrEqualTo(root
            .get(EventSessionImpl_.lastAccessed), dateTimeParameter));

          return criteriaQuery;
        }
      });

    this.deleteByEventSessionIdQuery = "DELETE FROM "
      + EventSessionImpl.class.getName() + " e " + "WHERE e."
  ...
  public EventSession createEventSession(LoginEvent loginEvent) {
    ...
    for (final String groupKey : loginEvent.getGroups()) {
      final AggregatedGroupMapping groupMapping = this.aggregatedGroupLookupDao
        .getGroupMapping(groupKey);
      groupMappings.add(groupMapping);
    }

    final EventSessionImpl eventSession = new EventSessionImpl(loginEvent
      .getEventSessionId(), groupMappings);
  ...
  public EventSession getEventSession(String eventSessionId) {
    final TypedQuery<EventSessionImpl> query = this
      .createCachedQuery(this.findByEventSessionIdQuery);
  ...
  public void deleteEventSession(String eventSessionId) {
    final Query query = this.entityManager
      .createQuery(this.deleteByEventSessionIdQuery);
  ...
  public void purgeExpiredEventSessions() {
    final TypedQuery<EventSessionImpl> query = this
      .createQuery(this.findExpiredEventSessionsQuery);
    query.setParameter(this.dateTimeParameter, DateTime.now().minus(
