 ...
 * externally. The need for two formats stems from the fact that {@link Noderef}
 * s use database identifiers in their format, which are not (always) stable
 * across migrations.<br/>
 * 
 * Pathrefs contain either 2 or 3 elements:
 * <ul>
 * <li>username of the fragment owner (e.g. 'admin-lo')</li>
 * <li>XPath that uniquely identifies a layout node (e.g.
 * '/layout/folder/folder[3]')</li>
 * <li><strong>SOMETIMES</strong> a portlet fname (layout nodes referring to
 * portlets only)</li>
 * </ul>
 * 
 * @author awills
...
public final class Pathref {
  ...
  public Pathref(String layoutOwnerUsername, String uniquePath, String portletFname) {
    ...
    // NB: portletFname can and SHOULD be null for Pathrefs that refer to
    // non-portlet nodes
