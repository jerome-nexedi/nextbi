...
/**
 * A wrapper implementation that records output content to a buffer without
 * actually writing it to the underlying response.
 * 
 * @version $Revision: 11911 $
 */
public class OutputCapturingHttpServletResponseWrapper extends
   ...
   *          response
   */
  public OutputCapturingHttpServletResponseWrapper(HttpServletResponse res,
    Writer writer) {
    super(res);
  ...
  public ServletOutputStream getOutputStream() {
    throw new UnsupportedOperationException(
      "As of Pluto 1.1.4 Portlet rendering should always only use getWriter() on the underlying HttpServletResponse");
  }
  ...
  public boolean equals(Object obj) {
    ...
    return new EqualsBuilder().append(this.getWrappedResponse(),
      rhs.getWrappedResponse()).append(this.committed, rhs.committed).append(
      this.wrappedWriter, rhs.wrappedWriter).isEquals();
  }
  ...
  @Override
  public int hashCode() {
    return new HashCodeBuilder(977252351, 179084321).append(
      this.getWrappedResponse()).append(this.committed).append(this.wrappedWriter)
      .toHashCode();
  }

  /**
   * Wrapper to watch for {@link #flush()} calls and to mark the response
