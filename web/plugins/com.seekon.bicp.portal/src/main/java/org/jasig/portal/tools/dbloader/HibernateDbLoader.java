...
/**
 * Can drop and/or create tables based on an XML table definition file and
 * populate tables based on a XML data definition file. Table creation is done
 * using the Hibernate mapping APIs to allow for a full range of database
 * support.
 * 
...
public class HibernateDbLoader implements IDbLoader, ResourceLoaderAware {
  ...
  public void setTransactionManager(
    @Qualifier("PortalDb") PlatformTransactionManager transactionManager) {
  ...
  public void setHibernateToolConfigurationSource(
    HibernateToolConfigurationSource hibernateToolConfigurationSource) {
   ...
   * 
   * @see
   * org.jasig.portal.tools.dbloader.IDbLoader#process(org.jasig.portal.tools
   * .dbloader.DbLoaderConfiguration)
   */
  @Override
  public void process(DbLoaderConfig configuration)
    ...
    if (this.preferedDialect != null) {
      ...
      final ServiceRegistry serviceRegistry = this.hibernateToolConfigurationSource
        .getServiceRegistry();
      final DialectResolver resolver = serviceRegistry
        .getService(DialectResolver.class);

      dialect = this.jdbcTemplate.execute(new ConnectionCallback<Dialect>() {
        @Override
        public Dialect doInConnection(Connection con) throws SQLException,
          DataAccessException {
    ...
    if (configuration.isDropTables() || configuration.isCreateTables()) {
      ...
      if (configuration.isDropTables()) {
        final List<String> dropScript = this.dropScript(tables.values(), dialect,
      ...
      for (final Map.Entry<String, DataAccessException> failedSqlEntry : failedSql
        .entrySet()) {
        this.logger.warn("'" + failedSqlEntry.getKey()
          + "' failed to execute due to " + failedSqlEntry.getValue());
      }

      // Generate and execute create table scripts
      if (configuration.isCreateTables()) {
        final List<String> createScript = this.createScript(tables.values(),
    ...
    if (script == null && configuration.isPopulateTables()) {
      ...
      final Map<String, Map<String, Integer>> tableColumnTypes = tableData
        .getTableColumnTypes();
    ...
    if (script != null) {
      for (final ListIterator<String> iterator = script.listIterator(); iterator
        .hasNext();) {
  ...
  protected ITableDataProvider loadTables(DbLoaderConfig configuration,
    Dialect dialect) throws ParserConfigurationException, SAXException, IOException {
    // Locate tables.xml
  ...
  protected List<String> dropScript(Collection<Table> tables, Dialect dialect,
    ...
    if (dialect.dropConstraints()) {
      for (final Table table : tables) {
        if (table.isPhysicalTable()) {
          for (final Iterator<ForeignKey> subIter = table.getForeignKeyIterator(); subIter
            .hasNext();) {
   ...
   */
  @SuppressWarnings("unchecked")
  protected List<String> createScript(Collection<Table> tables, Dialect dialect,
    Mapping mapping, String defaultCatalog, String defaultSchema) {
    final List<String> script = new ArrayList<String>(tables.size() * 2);

    for (final Table table : tables) {
      if (table.isPhysicalTable()) {
        script.add(table.sqlCreateString(dialect, mapping, defaultCatalog,
          defaultSchema));
      }
    }
    ...
    for (final Table table : tables) {
      if (table.isPhysicalTable()) {
        if (!dialect.supportsUniqueConstraintInCreateAlterTable()) {
          for (final Iterator<UniqueKey> subIter = table.getUniqueKeyIterator(); subIter
            .hasNext();) {
            final UniqueKey uk = subIter.next();
            final String constraintString = uk.sqlCreateString(dialect, mapping,
              defaultCatalog, defaultSchema);
        ...
        for (final Iterator<Index> subIter = table.getIndexIterator(); subIter
          .hasNext();) {
          final Index index = subIter.next();
          script.add(index.sqlCreateString(dialect, mapping, defaultCatalog,
            defaultSchema));
        }
        ...
        if (dialect.hasAlterTable()) {
          for (final Iterator<ForeignKey> subIter = table.getForeignKeyIterator(); subIter
            ...
            final ForeignKey fk = subIter.next();
            if (fk.isPhysicalConstraint()) {
              script.add(fk.sqlCreateString(dialect, mapping, defaultCatalog,
                defaultSchema));
            }
          }
        }
  ...
  protected void populateTables(DbLoaderConfig configuration,
