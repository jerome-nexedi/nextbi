 ...
 * We endeavor here to provide considerable encapsulation of the data we are
 * trying to present.
 * 
 * @author Andrew Newman, newman@yale.edu
 * @version $Revision$
 * @author Don Fracapane (df7@columbia.edu) Added getSubContextNames() to
 *         support principal and credential tokens
 */
public interface ISecurityContext extends Serializable {
   ...
   * 
   * @return The unique authentication value identifier. Values with the high
   *         order 16 bits clear are local (0x0000 - 0x00FF) where values with
   *         the high order 16 bits set (0xFF00 - 0xFFFF are foundation types
   *         distributed by JASIG. All other should be registered and globally
   *         unique.
   */
   ...
   * interface. By operating on this returned object the implementation class
   * for the credentials type will be able to access any values set in the
   * instance without exposing an interface method that would allow others
   * (inappropriate) acces to the fields.
   * 
   ...
   * IOpaqueCredentials interface. By operating on this returned object the
   * implementation class for the credentials type will be able to access any
   * values set in the Opaque credentials without exposing an interface method
   ...
   * values set in the Principal object (whose reference is returned by
   * <code>getPrincipalInstance()</code>) and the OpaqueCredentials object
   * (whose reference is returned by <code>getOpaqueCredentialsInstance()</code>
   * ).
   * 
   ...
   * authenticated. Note that merely testing this for a non-null pointer is not
   * sufficient to verify authenticated status. The isAuthenticated() call
   * should be used. In some authentication schemes, an asyncronous event could
   * potentially change one's authentication status.
   * 
  ...
  /**
   * Returns any credentials that an authenticated principal currently has. Note
   * that opaque credentials don't have any methods for examination of the
   * credentials contents. This call would primarily be useful to chain
   * authentication manually within the same authentication schem.
   * 
   ...
   * methods and the object returned will have to be cast to some concrete type
   * or alternate interface to be useful.
   * 
   ...
   * context.
   * 
   * @return The security context object reference associated with the name
   ...
   * 
   * @param name
   *          The non-compound name of the subserviant context. Note that under
   *          normal circumstances the establishment of the
   *          InitialSecurityContext will automatically register all subcontext.
   * 
   * @param ctx
