...
public class PortalInfoProviderImpl implements IPortalInfoProvider {
  ...
  /**
   * @param serverName
   *          A specific server name for {@link #getServerName()} to return
   */
   ...
   *          The name to use to lookup a NetworkInterface via
   *          {@link NetworkInterface#getByName(String)}
   */
  ...
  public final String getServerName() {
    ...
    this.logger
      .warn("Failed to get serverName for NetworkInterface ("
  ...
  protected String getDefaultNetworkInterfaceName() {
    ...
    // Use a local variable here to try and return the first hostName found that
    // doesn't start with localhost
    String name = null;
    while (networkInterfaceEnum.hasMoreElements()) {
      final NetworkInterface networkInterface = networkInterfaceEnum.nextElement();

      for (Enumeration<InetAddress> inetAddressEnum = networkInterface
  ...
  protected String getLocalHostName() {
    this.logger
      .info("Attempting to resolve serverName using InetAddress.getLocalHost()");
  ...
  protected String getNetworkInterfaceName(String networkInterfaceName) {
    ...
    try {
      ...
      logger.warn("Failed to get NetworkInterface for name (" + networkInterfaceName
        + ").", e);
      return null;
    }

    if (networkInterface == null) {
      logger.warn("No NetworkInterface could be found for name ("
        + networkInterfaceName + "). Available interface names: "
        + getNetworkInterfaceNames());
      return null;
    }

    final Enumeration<InetAddress> inetAddressesEnum = networkInterface
      .getInetAddresses();
