...
/**
 * Wraps a {@link Scope} to provide functionality when the wrapped scope can't
 * function due to an illegal state (no current session, request, ...)
 * 
...
public class FailsafeSingletonScope implements Scope, DisposableBean {
    ...
    for (final InstanceHolder instanceHolder : this.instances.values()) {
      if (instanceHolder.destructionCallback != null) {
        try {
          ...
          this.logger.warn("Destruction callback for bean named '"
            + instanceHolder.name + "' failed.", e);
        }
      }
    }
    ...
    try {
      ...
      synchronized (this.instances) {
        ...
        if (instanceHolder == null) {
          ...
          // Add to instances map before creating to ensure if a destruction
          // callback is added it is caught
  ...
  /**
   * @see org.springframework.web.context.request.AbstractRequestAttributesScope#registerDestructionCallback(java.lang.String,
   *      java.lang.Runnable)
   */
  public void registerDestructionCallback(String name, Runnable callback) {
    try {
      ...
      if (instanceHolder != null) {
        ...
        this.logger.info("Ignoring destruction callback for singleton bean '" + name
  ...
  private static class InstanceHolder {
    ...
    @Override
    public int hashCode() {
      return new HashCodeBuilder(217891979, 1307635269).append(this.name)
        .toHashCode();
    }
    ...
    public String toString() {
      return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).append(
        "name", this.name).append("instance", this.instance).append(
        "destructionCallback", this.destructionCallback).toString();
    }
  }
