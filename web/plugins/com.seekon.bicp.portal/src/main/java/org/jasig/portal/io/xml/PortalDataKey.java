...
public class PortalDataKey {
  ...
  /**
   * {@link #hashCode()} is called A LOT but never changes since this object and
   * all field types are immutable. A local variable is used to cache the
  ...
  public PortalDataKey(Node rootElement) {
    ...
    if (attributes != null) {
      final Node scriptAttr = attributes.getNamedItem(SCRIPT_ATTRIBUTE_NAME
        .getLocalPart());
      if (scriptAttr != null) {
      ...
      final Node versionAttr = attributes.getNamedItem(VERSION_ATTRIBUTE_NAME
        .getLocalPart());
      if (versionAttr != null) {
