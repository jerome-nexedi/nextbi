...
public class CachingResourceLoaderImpl implements CachingResourceLoader {
  ...
  @Autowired
  public void setResourcesElementsProvider(
    ResourcesElementsProvider resourcesElementsProvider) {
    this.resourcesElementsProvider = resourcesElementsProvider;
  }

  /**
   * How frequently the resource should be checked for updates (in ms). Defaults
   * to 1 minute.
   */
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.utils.cache.CachingResourceLoader#getResource(org.
   * springframework.core.io.Resource,
   * org.jasig.portal.utils.cache.ResourceBuilder)
   */
  @Override
   ...
   * springframework.core.io.Resource,
   * org.jasig.portal.utils.cache.ResourceBuilder,
   * org.jasig.portal.utils.cache.ResourceLoaderOptions)
   */
  @Override
  public <T> CachedResource<T> getResource(Resource resource, Loader<T> builder,
    ...
    if (Included.PLAIN == this.resourcesElementsProvider.getDefaultIncludedType()) {
      this.logger
        .trace("Resoure Aggregation Disabled, ignoring resource cache and loading '"
          + resource + "' directly");
      return this.loadResource(resource, builder);
    }

    // Look for the resource in the cache, since it has been wrapped with a
    // SelfPopulatingCache it should never return null.
    final GetResourceArguments<T> arguments = new GetResourceArguments<T>(resource,
      builder);
    final Element element = this.entryFactory.getWithData(this.resourceCache,
      resource, arguments);
    ...
    if (lastCheckTime + checkInterval >= System.currentTimeMillis()) {
      if (this.logger.isTraceEnabled()) {
        this.logger.trace(cachedResource + " is within checkInterval "
          + checkInterval + ", returning");
      }
      return cachedResource;
    }
    if (this.logger.isTraceEnabled()) {
      this.logger.trace(cachedResource + " is older than checkInterval "
        + checkInterval + ", checking for modification");
    }
    ...
    if (!resourceModified) {
      ...
      this.resourceCache.put(element); // do a cache put to notify the cache the
  ...
  protected <T> boolean checkIfModified(CachedResource<T> cachedResource) {
    ...
    if (resourceModified) {
      this.logger.trace("Resource {} was modified at {}, reloading", new Object[] {
        resource, mainLastModified });
      return true;
    }

    // If the main resource hasn't changed check additional resources for
    // modifications
    for (final Map.Entry<Resource, Long> additionalResourceEntry : cachedResource
      .getAdditionalResources().entrySet()) {
    ...
    this.logger.trace("{} has not been modified since last loaded {}, returning",
      cachedResource, lastLoadTime);
    return false;
  }
  ...
  protected long getLastModified(Resource resource) {
    try {
      ...
      this.logger.warn("Could not determine lastModified for " + resource
        + ". This resource will never be reloaded due to lastModified changing.", e);
    }
  ...
  private <T> CachedResource<T> loadResource(Resource resource, Loader<T> builder)
    ...
    final Serializable cacheKey = (Serializable) Arrays.asList(lastModified,
      loadedResource.getAdditionalResources());

    // Create the CachedResource based on if digesting was enabled
    return new CachedResourceImpl<T>(resource, loadedResource, lastLoadTime,
      cacheKey);
  }
  ...
  private class CachedResourceEntryFactory extends
    ThreadLocalCacheEntryFactory<GetResourceArguments<?>> {
