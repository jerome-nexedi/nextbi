...
/**
 * <p>
 * An interface that defines the required methods for assigning and retrieving
 * information about the authenticated principal (user). Providers requiring
 * additional principal information should extend this interface rather than
 * replacing it.
 * </p>
 * 
...
public interface IPrincipal extends Serializable {
   ...
   * returned by the CommonName (CN) attribute for those security contexts using
   * X.509 style naming.
   */
   ...
   * that post-authentication, an attempt to set a UID may either fail or reset
   * the authentication status of the security context container.
   * 
   * @param UID
