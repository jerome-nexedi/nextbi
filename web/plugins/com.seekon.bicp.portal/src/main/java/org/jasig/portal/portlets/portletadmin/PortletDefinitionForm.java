...
public class PortletDefinitionForm implements Serializable {
  ...
  public PortletDefinitionForm(IPortletDefinition def) {
    ...
    if (def.getParameter(IPortletDefinition.EDITABLE_PARAM) != null) {
      this.setEditable(Boolean.parseBoolean(def.getParameter(
        IPortletDefinition.EDITABLE_PARAM).getValue()));
    }
    if (def.getParameter(IPortletDefinition.HAS_HELP_PARAM) != null) {
      this.setHasHelp(Boolean.parseBoolean(def.getParameter(
        IPortletDefinition.HAS_HELP_PARAM).getValue()));
    }
    if (def.getParameter(IPortletDefinition.HAS_ABOUT_PARAM) != null) {
      this.setHasAbout(Boolean.parseBoolean(def.getParameter(
        IPortletDefinition.HAS_ABOUT_PARAM).getValue()));
    }
    ...
    for (IPortletDefinitionParameter param : def.getParameters()) {
      if (param.getName().startsWith("PORTLET.")) {
        this.portletPreferences.put(param.getName(), new StringListAttribute(
          new String[] { param.getValue() }));
      } else {
    ...
    for (IPortletPreference pref : def.getPortletPreferences()) {
      ...
      this.portletPreferences.put(pref.getName(), new StringListAttribute(pref
        .getValues()));
      this.portletPreferenceReadOnly.put(pref.getName(), new BooleanAttribute(!pref
        .isReadOnly()));
    }
      ...
      if (step.getParameters() != null) {
        for (Parameter param : step.getParameters()) {
          ...
          if (attribute == null || attribute.getValue() == null
            ...
            if (input != null) {
              this.parameters
                .put(param.getName(), new Attribute(input.getDefault()));
            }
          }
        }
      }
      if (step.getPreferences() != null) {
        for (Preference pref : step.getPreferences()) {
          ...
          if (!this.portletPreferences.containsKey(pref.getName())
            ...
            || (this.portletPreferences.get(pref.getName()).getValue().size() == 1 && this.portletPreferences
              .get(pref.getName()).getValue().get(0).trim().equals(""))) {
            ...
            if (input instanceof SingleValuedPreferenceInputType) {
              ...
              if (singleValued.getDefault() != null) {
                this.portletPreferences.get(pref.getName()).getValue().add(
                  singleValued.getDefault());
              }
              ...
              if (multiValued.getDefaults() != null) {
                this.portletPreferences.get(pref.getName()).getValue().addAll(
                  multiValued.getDefaults());
              }
            }
  ...
  public void setPortletPreferences(
    Map<String, StringListAttribute> portletParameters) {
