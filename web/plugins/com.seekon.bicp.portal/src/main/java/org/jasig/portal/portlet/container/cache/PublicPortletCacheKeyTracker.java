...
/**
 * Used to track keys by portlet definition id for a cache, at any point clients
 * can call
 * {@link PublicPortletCacheKeyTracker#getCacheKeys(IPortletDefinitionId)} to
 * get all of the keys in the cache for that portlet definition id
 */
final class PublicPortletCacheKeyTracker extends CacheEventListenerAdapter {
  ...
  private final LoadingCache<IPortletDefinitionId, Set<PublicPortletCacheKey>> publicPortletCacheKeys = CacheBuilder
    .newBuilder().build(
      new CacheLoader<IPortletDefinitionId, Set<PublicPortletCacheKey>>() {
        @Override
        public Set<PublicPortletCacheKey> load(IPortletDefinitionId key)
          throws Exception {
  ...
  public Set<PublicPortletCacheKey> getCacheKeys(
    IPortletDefinitionId portletDefinitionId) {
    final Set<PublicPortletCacheKey> keySet = publicPortletCacheKeys
      .getIfPresent(portletDefinitionId);
    if (keySet == null) {
  ...
  protected void removeEntry(Ehcache cache, Element element) {
    final PublicPortletCacheKey key = (PublicPortletCacheKey) element.getKey();
    final Set<PublicPortletCacheKey> keySet = publicPortletCacheKeys
      .getIfPresent(key.getPortletDefinitionId());
    if (keySet != null) {
