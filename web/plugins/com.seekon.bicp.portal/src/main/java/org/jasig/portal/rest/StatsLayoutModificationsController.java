 ...
 * <li>fromDate: Date (inclusive) from which to count backwards; default is
 * today</li>
 * </ul>
 * 
...
public class StatsLayoutModificationsController implements InitializingBean {
  ...
  public ModelAndView getEventCounts(HttpServletRequest req, HttpServletResponse res)
    ...
    if (req.getParameter("days") != null) {
      ...
      try {
        ...
        String msg = "Unrecognizable days parameter (must be a valid integer): "
          + daysParam;
        log.warn(msg, nfe);
    ...
    dateOnOrBefore.roll(Calendar.DATE, true); // default is tomorrow with time
    // fields cleared
      ...
      if (datePattern.matcher(fromDateParam).matches()) {
        try {
          ...
          if (log.isInfoEnabled()) {
            String msg = "Unrecognizable fromDate parameter (format 'mm/dd/yyyy'): "
              + fromDateParam;
            log.info(msg, pe);
          }
        }
    ...
    List<CountingTuple> completeList = factory.getEventCounts(dateOnOrBefore
      .getTime(), days);
  ...
  private List<CountingTuple> filterByPermissions(IPerson user,
    List<CountingTuple> completeList) {
  ...
  public static final class CountingTuple implements Comparable<CountingTuple> {
    ...
    public CountingTuple(int id, String portletFName, String portletTitle,
      String portletDescription, int count) {
  ...
  private static final class EventCountFactory {
    ...
    private static final String ADDED_PORTLETS_SQL = "SELECT uppd.portlet_def_id, uppd.portlet_fname, uppd.portlet_title, uppd.portlet_desc, (SELECT COUNT(*) "
      ...
      + "AND ste.act_date >= ? AND ste.act_date <= ?) " + "FROM up_portlet_def uppd";

    private static final String PORTLETS_ADDED_CACHE_KEY = "PortalStats.org.jasig.portal.rest.StatsLayoutModificationsController.portletsAdded";
    ...
    private List<CountingTuple> buildEventCounts(Date dateOnOrBefore, int days) {
      ...
      List<CountingTuple> rslt = simpleJdbcTemplate.query(ADDED_PORTLETS_SQL,
        rowMapper, dateOnOrAfter, dateOnOrBefore);
  ...
  private static final class RowMapperImpl implements
    ParameterizedRowMapper<CountingTuple> {

    @Override
    public CountingTuple mapRow(ResultSet rs, int index) throws SQLException {
      return new CountingTuple(rs.getInt("portlet_def_id"), rs
        .getString("portlet_fname"), rs.getString("portlet_title"), rs
        .getString("portlet_desc"), rs.getInt(5));
    }
