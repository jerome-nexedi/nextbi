...
/**
 * Spring controller that returns a JSON or XML representation of DLM fragments.
 * For non-admins, this will only display the channels the user is allowed to
...
public class FragmentListController {
  ...
  public ModelAndView listFragments(HttpServletRequest req) throws ServletException {
    ...
    for (FragmentDefinition frag : dlmConfig.getFragments()) {
      ...
      Document layout = fragmentLayoutMap != null ? fragmentLayoutMap.get(frag
        .getOwnerId()) : null;

      List<String> portlets = null;
      if (layout != null) {
        portlets = new ArrayList<String>();
        NodeList channelFNames = this.xpathOperations.evaluate(CHANNEL_FNAME_XPATH,
          layout, XPathConstants.NODESET);
        for (int i = 0; i < channelFNames.getLength(); i++) {
          String fname = channelFNames.item(i).getTextContent();
          IPortletDefinition pDef = portletRegistry
  ...
  public static final class FragmentBean {
    ...
    public static FragmentBean create(FragmentDefinition frag, List<String> portlets) {
      ...
      return new FragmentBean(frag.getName(), frag.getOwnerId(), frag
        .getPrecedence(), frag.getEvaluators(), portlets);
    ...
    private FragmentBean(String name, String ownerId, Double precedence,
      List<Evaluator> audience, List<String> portlets) {
