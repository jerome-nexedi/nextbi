...
public class PortletRegistrySearchService implements IPortalSearchService {
  ...
  public SearchResults getSearchResults(PortletRequest request, SearchRequest query) {
    ...
    final List<IPortletDefinition> portlets = portletDefinitionRegistry
    ...
    for (IPortletDefinition portlet : portlets) {
      if (matches(queryString, portlet)) {
        ...
        final IPortletWindow portletWindow = this.portletWindowRegistry
          .getOrCreateDefaultPortletWindowByFname(httpServletRequest, portlet
            .getFName());
        if (portletWindow != null) {
          final IPortletWindowId portletWindowId = portletWindow
            .getPortletWindowId();
          final IPortalUrlBuilder portalUrlBuilder = this.portalUrlProvider
            .getPortalUrlBuilderByPortletFName(httpServletRequest, portlet
              .getFName(), UrlType.RENDER);
  ...
  protected boolean matches(String query, IPortletDefinition portlet) {
    return portlet.getTitle().contains(query)
      ...
      || (portlet.getDescription() != null && portlet.getDescription().contains(
        query)) || portlet.getFName().contains(query);
  }
