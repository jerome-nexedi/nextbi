 ...
 * <br>
 * The only required property is {@link #setPortOne(int)}, if portTwo is not set
 * is will be calculated as portOne + 1. faileOnException is set to false buy
 * default. <br>
 * <br>
 ...
 * <td>Having this property set (value doesn't matter) enables starting of the
 * JMX Server</td>
 * </tr>
 * <tr>
 ...
 * <td>The password file that contains usernames and passwords for connecting to
 * the server</td>
 * </tr>
 * <tr>
 ...
 * <br>
 * <p>
 * <b>IMPORTNANT NOTE</b> Using this bean starts an RMI server. If this bean is
 * used in a JVM that is long running and has any sort of generational garbage
 * collector configured the system properties sun.rmi.dgc.client.gcInterval and
 * sun.rmi.dgc.server.gcInterval need to be set to some high value. They
 * configure how often the RMI server requests a full GC. To disable the RMI
 * induced GCs set the properties to '0x7ffffffffffffffe' Only do this if you
 * can rely on the generational GC to reguarly clean up RMI objects.
 * </p>
 * 
 * @author Eric Dalquist <a
 *         href="mailto:eric.dalquist@doit.wisc.edu">eric.dalquist
 *         @doit.wisc.edu</a>
 * @version $Revision$
 */
public class JavaManagementServerBean {
  ...
  public void startServer() {
    if (!System.getProperties().containsKey(JMX_ENABLED_PROPERTY)) {
      this.logger.info("System Property '" + JMX_ENABLED_PROPERTY
    ...
    try {
      ...
      try {
        ...
        throw new IllegalStateException("Could not create RMI Registry on port "
          + portOne, re);
      }
      ...
      try {
        this.jmxConnectorServer = JMXConnectorServerFactory.newJMXConnectorServer(
          jmxServiceUrl, jmxEnv, mbeanServer);

        if (this.logger.isDebugEnabled()) {
          this.logger.debug("Created JMXConnectorServer for JMXServiceURL='"
            + jmxServiceUrl + "', jmxEnv='" + jmxEnv + "' MBeanServer='"
            + mbeanServer + "'");
        }
      ...
      try {
        ...
        this.logger.info("Started JMXConnectorServer. Listening on '"
          + jmxServiceUrl + "'");
      } catch (IOException ioe) {
        throw new IllegalStateException("Failed to start the JMXConnectorServer",
          ioe);
      }
   ...
   * 
   * @param portOne
   *          Base port to calculate the second port from if needed.
   ...
   *           If localhost cannot be resolved or if the JMXServiceURL is
   *           malformed.
   */
  protected JMXServiceURL getServiceUrl(final int portOne, int portTwo) {
    ...
    final String jmxUrl = "service:jmx:rmi://" + jmxHost + ":" + portTwo
      + "/jndi/rmi://" + jmxHost + ":" + portOne + "/server";
    ...
    if (this.logger.isDebugEnabled()) {
      this.logger.debug("Generated JMXServiceURL='" + jmxServiceUrl
        + "' from String " + jmxUrl + "'.");
    }
