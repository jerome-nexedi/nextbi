 ...
 * @version $Id$
 */
public class CachedPortletData<T extends Serializable> implements
   ...
   *          Time in seconds the content is valid for (from now)
   */
  public CachedPortletData(T portletResult, String cachedWriterOutput,
    byte[] cachedStreamOutput, String contentType, boolean publicScope, String etag,
    int expirationTime) {
  ...
  /**
   * The time since the epoch in milliseconds that this content expires. -1 if
   * it never expires
   */
  ...
  public void updateExpirationTime(int expirationTime) {
    if (expirationTime == -1) {
      ...
      this.expirationTime = System.currentTimeMillis()
        + TimeUnit.SECONDS.toMillis(expirationTime);
    }
  }
