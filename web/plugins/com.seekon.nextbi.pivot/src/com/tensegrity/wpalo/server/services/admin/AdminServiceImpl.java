...
/**
 * <code>AdminServiceImpl</code> TODO DOCUMENT ME
 * 
 * @version $Id: AdminServiceImpl.java,v 1.15 2010/04/12 11:13:36
 *          PhilippBouillon Exp $
 **/
public class AdminServiceImpl extends AdminService implements WPaloAdminService {
  ...
  public XAccount[] getAccounts(String sessionId, XUser xUser)
  ...
  public synchronized XAccount[] listAccounts(String sessionId, XUser xUser)
    throws SessionExpiredException {
    List<XAccount> xAccounts = new ArrayList<XAccount>();
  ...
  private final User getNativeUser(String sessionId, XUser xUser)
    throws SessionExpiredException {
    AuthUser loggedInUser = getLoggedInUser(sessionId);
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(loggedInUser);
  ...
  public XGroup[] getGroups(String sessionId, XUser xUser)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  public XRole[] getRoles(String sessionId, XUser user)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  public Boolean[] hasRoles(String sessionId, XUser xuser, String[] roles)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  public Boolean hasAccount(String sessionId, XConnection con)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
    for (Account acc : adminService.getAccounts()) {
      if (acc.getConnection() != null
        && acc.getConnection().getId().equals(con.getId())) {
        return true;
      }
    }
    return false;
  ...
  public XObject saveXObject(String sessionId, XObject obj)
    ...
    try {
      ...
      throw new DbOperationFailedException(usrSession.translate("couldNotSave", obj
  ...
  private final Account findReplaceAccount(String sessionId, User user, Account acc)
    throws SessionExpiredException {
    if (acc != null) {
      if (acc.getUser() != null) {
        if (acc.getUser().getId().equals(user.getId())) {
          ...
          IAccountManagement accountManagement = MapperRegistry.getInstance()
            .getAccountManagement();
          try {
            List<Account> accounts = accountManagement.getAccountsBy(acc
              .getConnection().getId());
  ...
  public String[] mayDelete(String sessionId, XObject xObj)
    throws SessionExpiredException {
    String type = xObj.getType();
    if (type.equals(XUser.TYPE)) {
      User user = getNative(sessionId, (XUser) xObj);
      IViewManagement viewManagement = MapperRegistry.getInstance()
        .getViewManagement();
      try {
  ...
  public void delete(String sessionId, XObject xObj)
    throws DbOperationFailedException, SessionExpiredException {
    try {
      String type = xObj.getType();
      if (type.equals(XAccount.TYPE)) {
        ...
        try {
          ...
          try {
            ...
            for (View v : allViews) {
              Account acc = v.getAccount();
              Account replaceAccount = findReplaceAccount(sessionId, a.getUser(),
                acc);
              if (replaceAccount != null
                && !v.getAccount().getId().equals(replaceAccount.getId())) {
        ...
        try {
          try {
            IFolderManagement folders = MapperRegistry.getInstance()
              .getFolderManagement();
            AuthUser aUser = ServiceProvider.getAuthenticationService()
              .authenticateHash(user.getLoginName(), user.getPassword());
      ...
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
        xObj.getName(), message), e);
    }
  }

  private final Account getNative(String sessionId, XAccount xAccount)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  private final PaloConnection getNative(String sessionId, XConnection xConnection)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  private final Group getNative(String sessionId, XGroup xGroup)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  private final Role getNative(String sessionId, XRole xRole)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
  ...
  private final User getNative(String sessionId, XUser xUser)
    throws SessionExpiredException {
    AdministrationService adminService = ServiceProvider
      .getAdministrationService(getLoggedInUser(sessionId));
    return adminService.getUser(xUser.getId());
    ...
    if (v != null) {
      try {
        ...
        if (acc != null) {
          ...
          List<Account> allAccounts = MapperRegistry.getInstance()
            .getAccountManagement().getAccountsBy(conId);
  ...
  public XConnection getConnection(String sessionId, XView view)
    throws SessionExpiredException {
    View v = null;
    try {
    ...
    if (v != null && v.getAccount() != null
      && v.getAccount().getConnection() != null) {
  ...
  public synchronized XCube[] getCubes(String sessionId, XConnection xCon,
    XDatabase xDb) throws SessionExpiredException {
    AuthUser loggedInUser = getLoggedInUser(sessionId);
    try {
      List<Account> accounts = MapperRegistry.getInstance().getAccountManagement()
        .getAccounts(loggedInUser);
            ...
            if (acc instanceof PaloAccountImpl) {
              ...
              for (Database db : con.getDatabases()) {
                if (db.getId().equals(xDb.getId())) {
                  ...
                  ConnectionPoolManager.getInstance().disconnect(acc, sessionId,
                    "getCubes");
