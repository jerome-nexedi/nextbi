 ...
 * 
 * @version $Id: ViewImporterTreeLoader.java,v 1.23 2010/02/16 13:53:42
 *          PhilippBouillon Exp $
 **/
public class ViewImporterTreeLoader implements ChildLoader {
  ...
  private final XObject[] load(XDatabase xDatabase, AuthUser user, String sessionId) {
    try {
      ...
      if (database != null) {
        ...
        for (Cube cube : cubes) {
          if (isValid(cube)) {
            XObject xCube = new XCube(cube.getId(), cube.getName(), xDatabase);
  ...
  private final XObject[] load(XCube xCube, AuthUser user, String sessionId,
    ...
    try {
      ...
      if (cube != null) {
        ...
        for (String viewId : viewIds) {
          ...
          XView view = createXView(viewId, viewName, cube, xCube.getAccountId(),
            user);
  ...
  private final Database getDatabase(XDatabase xDatabase, AuthUser user,
    String sessionId) {
    Connection paloConnection = getConnectionForAccount(xDatabase.getAccountId(),
      user, sessionId);
    if (paloConnection != null)
  ...
  private final synchronized Connection getConnectionForAccount(String accountId,
    AuthUser user, String sessionId) {
    ...
    if (account instanceof PaloAccount) {
      ...
      ServerConnectionPool pool = ConnectionPoolManager.getInstance().getPool(
        account, sessionId);
      Connection con = pool
        .getConnection("ViewImporterTreeLoader.getConnectionForAccount");
