...
/**
 * <code>ViewBrowserTreeLoader</code> TODO DOCUMENT ME
 * 
 * @version $Id: ViewBrowserTreeLoader.java,v 1.16 2009/12/17 16:14:20
 *          PhilippBouillon Exp $
 **/
public class ViewBrowserTreeLoader implements ChildLoader {
  ...
  private final List<View> loadAllViews(AuthUser user) {
    ...
    for (Account account : user.getAccounts()) {
      if (account instanceof PaloAccount) {
        List<View> viewsPerAccount = ServiceProvider.getViewService(user).getViews(
          account);
  ...
  private final void removeViewsWhichAreInFolders(final List<View> allViews,
    AuthUser user) {
    ...
    FolderVisitor visitor = new FolderVisitor() {
      public boolean visit(ExplorerTreeNode folder) {
        if (folder instanceof FolderElement) {
          ParameterReceiver srcObj = ((FolderElement) folder).getSourceObject();
  ...
  private final void addRemainingViewsToRootFolder(List<View> allViews, AuthUser user)
  ...
  private XObject[] loadFolder(XStaticFolder xFolder, AuthUser user) {
    ...
    for (int i = 0; i < children.length; ++i) {
      ...
      if (child instanceof StaticFolder) {
        ...
        xStaticFolder.setHasChildren(staticFolder.getChildren().length > 0);
    ...
    // }
    // });
    // System.arraycopy(folders.toArray(new XObjectWrapper[0]), 0, xChildren, 0,
    // folders.size());
    // System.arraycopy(elements.toArray(new XObjectWrapper[0]), 0, xChildren,
    // folders.size(), elements.size());
    return xChildren;
  }
  ...
  private final void addAllViewsFor(AuthUser user, Account account,
    List<XView> toAllViews) {
