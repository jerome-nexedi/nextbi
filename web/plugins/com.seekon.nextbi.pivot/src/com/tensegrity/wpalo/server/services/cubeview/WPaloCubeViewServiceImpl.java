...
import com.tensegrity.wpalo.server.ServletContextWrapper;
import com.tensegrity.wpalo.server.WPaloPropertyServiceImpl;

/**
 * <code>CubeViewServiceImpl</code> TODO DOCUMENT ME
 * 
 * @version $Id: WPaloCubeViewServiceImpl.java,v 1.71 2010/04/15 09:55:22
 *          PhilippBouillon Exp $
 **/
public class WPaloCubeViewServiceImpl extends CubeViewService implements
  ...
  public XLoadInfo willChangeSelectedElement(String sessionId,
    XViewModel xViewModel, XAxisHierarchy xAxishHierarchy)
    throws SessionExpiredException {
    return super.willChangeSelectedElement(sessionId, xViewModel, xAxishHierarchy);
  }

  public synchronized XLoadInfo willOpen(String sessionId, XView xView)
  ...
  public synchronized XViewModel proceedOpen(String sessionId, XView xView)
    throws SessionExpiredException, PaloGwtCoreException {
    try {
      ViewOpenWarnings.getInstance().clearWarnings();
      XViewModel model = proceedOpenView(sessionId, getLoggedInUser(sessionId),
        xView.getId());
  ...
  public XViewModel saveView(String sessionId, XViewModel xViewModel)
    ...
    try {
      ...
      if (e.getCause() != null
        ...
        && e.getCause().getMessage().toLowerCase().indexOf("not enough rights") != -1) {
        return null;
      }
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotSave",
        xViewModel.getName(), e.getLocalizedMessage()), e);
    } catch (Throwable t) {
      ...
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotSave",
        xViewModel.getName(), t.getLocalizedMessage()), t);
    }
  }
  ...
  public void deleteView(String sessionId, XView view)
    throws DbOperationFailedException, SessionExpiredException {
    if (view == null)
      return;
    try {
      ...
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
        view.getName(), e.getLocalizedMessage()), e);
    }
  }

  public void closeView(String sessionId, XViewModel xViewModel)
    throws SessionExpiredException {
    remove(sessionId, xViewModel);
  }

  public XView[] importViews(String sessionId, XView[] views)
    throws SessionExpiredException {
  ...
  public void renameView(String sessionId, XView xView, String newName)
    ...
    try {
      ...
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotRename",
        xView.getName(), e.getLocalizedMessage()), e);
    }
  }
  ...
  private final boolean checkPermission(AuthUser forUser, View view, int right) {
    switch (right) {
    case 0:
      return forUser.hasPermissionIgnoreOwner(Right.NONE, view);
    case 1:
      return forUser.hasPermissionIgnoreOwner(Right.READ, view);
    case 2:
      return forUser.hasPermissionIgnoreOwner(Right.WRITE, view);
    case 4:
      return forUser.hasPermissionIgnoreOwner(Right.DELETE, view);
    case 8:
      return forUser.hasPermissionIgnoreOwner(Right.CREATE, view);
    case 16:
      return forUser.hasPermissionIgnoreOwner(Right.GRANT, view);
    }
    return false;
  }

  public boolean checkPermission(String sessionId, String viewId, int right)
    throws SessionExpiredException {
    UserSession userSession = getUserSession(sessionId);
  ...
  public boolean checkPermission(String sessionId, int right)
    throws SessionExpiredException {
  ...
  private final synchronized AxisHierarchy toNative(String sessionId,
    String axisHierarchyId, String viewId, String axisId)
    throws SessionExpiredException, PaloGwtCoreException {
    // UserSession userSession = getUserSession(sessionId);
    View view = CubeViewController.getViewById(sessionId, viewId);
    if (view == null) {
    ...
    if (cv == null) {
      try {
        ...
        ConnectionPoolManager.getInstance().disconnect(view.getAccount(), sessionId,
          "WPaloCubeViewServiceImpl.toNative");
      }
    }
  ...
  private final synchronized Axis toNative(String sessionId, XAxis xAxis)
    throws SessionExpiredException, PaloGwtCoreException {
    UserSession userSession = getUserSession(sessionId);
    ...
    if (cv == null) {
      try {
        ...
        ConnectionPoolManager.getInstance().disconnect(view.getAccount(), sessionId,
          "WPaloCubeViewServiceImpl.toNative2");
      }
    }
  ...
  private final boolean tryToMatch(ElementNode[] rootNodes, ElementNode[] filter,
    ElementNode[] structure, Hierarchy hier, String[] allPaths, boolean showLeft) {
    if (showLeft) {
      return true;
    }
    ...
    for (ElementNode node : filter) {
      ...
      if (node.hasChildren()) {
        result = tryToMatch(rootNodes, node.getChildren(), structure, hier,
          allPaths, showLeft);
      }
      if (!result) {
        return false;
  ...
  private static final void updateLocalFilter(AxisHierarchy axisHierarchy,
    XElementNode[] visibleElements) {
  ...
  private static final void addChildren(ElementNode parent, XElementNode xParent,
    Hierarchy hierarchy) {
  ...
  public boolean checkLocalFilter(String sessionId, String axisHierarchyId,
    String viewId, String axisId, String filterPaths,
    XElementNode[] visibleElements, boolean displayLeft)
    throws SessionExpiredException, PaloGwtCoreException {
    AxisHierarchy axisHierarchy = toNative(sessionId, axisHierarchyId, viewId,
      axisId);
    if (axisHierarchy == null) {
      return false;
    }
    ...
    if (axisHierarchy.getProperty("filterPaths") != null) {
      String filterPath = (String) axisHierarchy.getProperty("filterPaths")
        .getValue();
      if (filterPath != null) {
    ...
    if (axisHierarchy.getSubset() == null) {
      result = tryToMatch(nodes, nodes, axisHierarchy.getHierarchy()
        .getElementsTree(), axisHierarchy.getHierarchy(), allPaths, displayLeft);
    } else {
      result = tryToMatch(nodes, nodes, axisHierarchy.getSubset().getRootNodes(),
        axisHierarchy.getHierarchy(), allPaths, displayLeft);
    }
    updateLocalFilter(axisHierarchy, initialLocalFilter);
    return result;
  }
  ...
  private final String getAliasForElement(String name, String elementName,
    String format) {
  ...
  public XElementNode[] applyAlias(String sessionId, String axisHierarchyId,
    String viewId, String axisId, XAlias alias, XElementNode[] allNodes)
    throws SessionExpiredException, PaloGwtCoreException {
    AxisHierarchy axisHierarchy = toNative(sessionId, axisHierarchyId, viewId,
      axisId);
    if (axisHierarchy == null) {
    ...
    for (XElementNode node : allNodes) {
      ...
      if (newName != null) {
        if (aliasFormat == null) {
          ...
          String name = getAliasForElement(newName, node.getElement().getName(),
            aliasFormat);
          node.setName(name);
  ...
  public String[] getWarningThresholds(String sessionId, String[] browserPrefixes)
    ...
    for (String prefix : browserPrefixes) {
      newLoadedCellsWarningThreshold = properties.getIntProperty(prefix
        + "newLoadedCellsWarningThreshold", newLoadedCellsWarningThreshold);
      displayCellsWarningThreshold = properties.getIntProperty(prefix
        + "displayCellsWarningThreshold", displayCellsWarningThreshold);
      totalLoadedCellsWarningThreshold = properties.getIntProperty(prefix
        + "totalLoadedCellsWarningThreshold", totalLoadedCellsWarningThreshold);
      showWarnDialog = properties.getBooleanProperty(prefix
        + "cellsWarningThresholdActive", showWarnDialog);
      showTreeWarnDialog = properties.getBooleanProperty(prefix
        + "treeWarningThresholdActive", showWarnDialog);
      totalTreeChildrenWarningThreshold = properties.getIntProperty(prefix
        + "totalTreeChildrenWarningThreshold", totalTreeChildrenWarningThreshold);

    }
    return new String[] { "" + showWarnDialog, "" + showTreeWarnDialog,
      "" + displayCellsWarningThreshold, "" + newLoadedCellsWarningThreshold,
      "" + totalLoadedCellsWarningThreshold, "" + totalTreeChildrenWarningThreshold };
  }
  ...
  public synchronized XViewModel proceedReload(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
  ...
  public synchronized XViewModel proceedUpdateView(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
    try {
      XViewModel model = proceedUpdateView(sessionId, getLoggedInUser(sessionId),
        xViewModel);
      removeLocalFilter(model);
      model.setNeedsRestore(true);
      return model;
  ...
  public XViewModel proceedUpdateViewWithoutTable(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
    try {
      return proceedUpdateViewWithoutTable(sessionId, getLoggedInUser(sessionId),
        xViewModel);
    } catch (PaloGwtCoreException e) {
      UserSession userSession = getUserSession(sessionId);
      throw new SessionExpiredException(userSession.translate("noAccount"), e);
  ...
  public boolean containsElement(String sessionId, String axisHierarchyId,
  ...
  public XPrintResult generatePDF(String sessionId, XViewModel xViewModel,
    XPrintConfiguration config) throws SessionExpiredException {
    try {
  ...
  public Boolean[] getRoles(String sessionId, String viewId)
    throws SessionExpiredException {
    AuthUser user = getLoggedInUser(sessionId);
  ...
  private final void assignViewerAndEditorRole(String sessionId, FolderElement fe,
    View view, boolean isPublic, boolean isEditable) throws SQLException {
    IRoleManagement roleMgmt = MapperRegistry.getInstance().getRoleManagement();
    Role viewerRole = (Role) roleMgmt.findByName("VIEWER");
    if (viewerRole == null) {
      viewerRole = new RoleImpl.Builder(null).name("VIEWER").permission(Right.READ)
        .build();
  ...
  private final void ensureRoles(FolderService folderService, ExplorerTreeNode etn,
    Role role) {
    if (etn == null) {
      return;
    }
  ...
  private final void saveRoot(String sessionId, ExplorerTreeNode root)
    throws SessionExpiredException {
    try {
      AuthUser user = getLoggedInUser(sessionId);
  ...
  public XView setVisibility(String sessionId, XFolderElement element,
    ...
    String dbId, String cubeId) throws SessionExpiredException {
    ...
    if (!accountId.equals(xView.getAccountId())) {
      try {
        Account newAccount = (Account) MapperRegistry.getInstance()
          .getAccountManagement().find(accountId);
        if (newAccount != null) {
    ...
    if (!cubeId.equals(xView.getCubeId())) {
      try {
        ...
        if (index != -1) {
          ...
          if (rIndex != -1) {
            def = def.substring(0, index + 6) + cubeId + def.substring(rIndex);
    ...
    if (!ownerId.equals(xView.getOwnerId())) {
      try {
        User newOwner = (User) MapperRegistry.getInstance().getUserManagement()
          .find(ownerId);
        if (newOwner != null) {
          try {
            CubeViewReader.CHECK_RIGHTS = false;
    ...
    try {
      ExplorerTreeNode root = FolderModel.getInstance().load(
        getLoggedInUser(sessionId));
  ...
  public String getElementPath(String sessionId, String axisHierarchyId,
  ...
  public int getNumberOfChildren(String sessionId, String axisHierarchyId,
    String viewId, String axisId, String selectedElementId)
    throws SessionExpiredException, PaloGwtCoreException {
    AxisHierarchy hier = toNative(sessionId, axisHierarchyId, viewId, axisId);
    if (hier != null) {
