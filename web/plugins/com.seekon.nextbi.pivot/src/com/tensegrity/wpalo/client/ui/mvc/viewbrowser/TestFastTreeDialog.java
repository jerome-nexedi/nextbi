 ...
 * 
 * @version $Id: TestFastTreeDialog.java,v 1.2 2010/02/12 13:49:50
 *          PhilippBouillon Exp $
 **/
public class TestFastTreeDialog extends Window {
  ...
  private Widget createLazyTree() {
    ...
    root = new SimpleTreeNode(null, hierarchy.getType(), hierarchy.getId(),
      hierarchy.getName(), 1, hierarchy);
    loadChildren(t, root);
    return t;
  }

  private void loadChildren(final HasFastMSTreeItems parent,
    final SimpleTreeNode node) {
    XUser user = ((Workbench) Registry.get(Workbench.ID)).getUser();
    final String sessionId = user.getSessionId();

    // WPaloServiceProvider.getInstance().loadChildren(sessionId,
    // hierarchy.getId(), node, xViewModel, new AsyncCallback <String []>() {
    ...
    // loadChildren(this, new SimpleTreeNode(node, dataType, dataId, dataName,
    // dataCC,
    // new XElementNode(new XElement(dataId, dataName, dataCC > 0 ?
    // XElementType.CONSOLIDATED : XElementType.NUMERIC), hierarchy)));
    // }
    // };
    // if (dataCC > 0) {
