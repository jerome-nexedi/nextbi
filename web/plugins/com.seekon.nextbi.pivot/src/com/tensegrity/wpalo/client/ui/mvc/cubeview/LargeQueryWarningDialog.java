...
public class LargeQueryWarningDialog extends Dialog {
  ...
  private LargeQueryWarningDialog(XLoadInfo loadInfo,
    final Listener<WindowEvent> callback) {
    final String msg = messages.largeCellQueryWarning(loadInfo.loadCells,
      loadInfo.totalCells, loadInfo.visibleCells);

    WPaloPropertyServiceProvider.getInstance().getStringProperty(
      constants.warningMessageAllBrowsers(), new AsyncCallback<String>() {
  ...
  private LargeQueryWarningDialog(int treeChildren,
    final Listener<WindowEvent> callback) {
    final String msg = messages.largeTreeQueryWarning(treeChildren);

    WPaloPropertyServiceProvider.getInstance().getStringProperty(
      constants.warningTreeMessageAllBrowsers(), new AsyncCallback<String>() {
        public void onFailure(Throwable arg0) {
  ...
  private final void showDialog(final boolean isCells, String msg,
    final Listener<WindowEvent> callback) {
    ...
    if (UserAgent.getInstance().isIE && !UserAgent.getInstance().isIE8) {
      ...
      final String mmsg = msg;
      WPaloPropertyServiceProvider.getInstance().getStringProperty(
        constants.warningMessageFF(), new AsyncCallback<String>() {
          public void onFailure(Throwable arg0) {
            doShowDialog(isCells, mmsg, callback);
          }
      ...
      final String mmsg = msg;
      WPaloPropertyServiceProvider.getInstance().getStringProperty(
        constants.warningMessageIE8(), new AsyncCallback<String>() {
          public void onFailure(Throwable arg0) {
            doShowDialog(isCells, mmsg, callback);
          }
  ...
  private final void doShowDialog(final boolean isCells, String msg,
    ...
    setHeading(isCells ? constants.confirmCellLoading() : constants
      .confirmTreeLoading());
    setResizable(false);
    setConstrain(true);
  ...
  public static final void readThresholds(AsyncCallback<String[]> cb) {
    ...
    WPaloCubeViewServiceProvider.getInstance()
      .getWarningThresholds("", prefixes, cb);
  }
  ...
  private static final String[] getBrowserPrefixes() {
    ...
    if (ua.isGecko)
      buf.append("firefox.,");
    if (ua.isGecko36)
      buf.append("firefox36.,");
    if (ua.isGecko35)
      buf.append("firefox35.,");
    if (ua.isGecko3)
      buf.append("firefox3.,");
    if (ua.isGecko2)
      buf.append("firefox2.,");
  ...
  public static final void confirm(XLoadInfo loadInfo,
    final ConfirmLoadDialogListener dlgListener) {
    if (hideWarnDialog) {
  ...
  public static final void confirm(int treeChildren,
    final ConfirmLoadDialogListener dlgListener) {
    if (hideWarnDialog) {
      dlgListener.proceed(true);
      return;
