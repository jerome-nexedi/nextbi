...
class VariableTab extends EditorTab {
  ...
  private final String instructionText = "Here, you can assign variables in a report to variables resulting "
  ...
  private final String chooseHint = "First, please select a report.";
  ...
  public boolean save(XObject input) {
    if (input instanceof XReport) {
      ...
      WPaloServiceProvider.getInstance().applyMapping((XReport) input,
        keys.toArray(new XObject[0]), values.toArray(new String[0]), user,
  ...
  private final void setLabelText(final Callback<XVariableDescriptor> globalCallback) {
    if (report == null) {
      ...
      store.removeAll();
      combo.removeAll();
      WPaloServiceProvider.getInstance().getVariables(report, user,
        new Callback<XVariableDescriptor>() {
          public void onSuccess(XVariableDescriptor desc) {
            ...
            // store.add(new VariableMatch("Test", report.getName(),
            // "Test 3"));
            panel.add(grid);
            panel.layout();
  ...
  private final ContentPanel createPanel() {
    ...
    column.setRenderer(new GridCellRenderer<VariableMatch>() {
      public String render(VariableMatch model, String property, ColumnData config,
        int rowIndex, int colIndex, ListStore<VariableMatch> store) {
