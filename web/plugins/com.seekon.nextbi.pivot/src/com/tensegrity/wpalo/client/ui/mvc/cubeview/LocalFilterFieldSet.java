...
class LocalFilterFieldSet extends FieldSet implements Listener<ComponentEvent>,
  ...
  public final Object[] getVisibleElements() {
    ...
    if (targetHierarchyTree.isEmpty()) {
      // final Map <FastMSTreeItem, XElementNode> parents = new
      // HashMap<FastMSTreeItem, XElementNode>();
      final LinkedHashSet<FastMSTreeItem> currentSelection = sourceHierarchyTree
        .getSelection();
      ...
      sourceHierarchyTree.traverse(new FastMSTreeItemVisitor() {
        public boolean visit(FastMSTreeItem item, FastMSTreeItem parent) {
          ...
          XElementNode xParent = getParent(parent); // , parents);
          // //parents.get(parent);
          if (xParent == null)
            roots.add(elNode);
          else {
  ...
  private final XElementNode getElementNodeCopyFrom(TreeItem item) {
    ...
    XElementNode newXElementNode = new XElementNode(oldXElementNode.getElement(),
      oldXElementNode.getAxisHierarchyId(), oldXElementNode.getViewId());
  ...
  private final XElementNode getParent(TreeItem item,
    Map<TreeItem, XElementNode> parents) {
  ...
  public boolean isLeft() {
    return targetHierarchyTree.isEmpty()
      && sourceHierarchyTree.getTree().getChildCount() > 0;
  }

  private final void setInputInternal() {
    ((Workbench) Registry.get(Workbench.ID)).showWaitCursor(constants
      .initializingLocalFilter());
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    boolean showLeft = (targetHierarchyTree.isEmpty() && sourceHierarchyTree
      .getTree().getChildCount() > 0)
      || initOnLeft;
    inputSet = true;
    // final XAxis oldAxis = xAxisHierarchy.getAxis();
    // xAxisHierarchy.setAxis(null);
    WPaloCubeViewServiceProvider.getInstance().checkLocalFilter(sessionId,
      xAxisHierarchy.getId(), xAxisHierarchy.getViewId(),
      xAxisHierarchy.getAxisId(), xAxisHierarchy.getProperty("filterPaths"),
            ...
            sourceHierarchyTree.addLoadListener(new LoadListener() {
              public void loaderLoad(LoadEvent le) {
                ((Workbench) Registry.get(Workbench.ID)).showWaitCursor(constants
                  .initializingLocalFilter());
                sourceHierarchyTree
                  .apply(initialSelectedElements, targetHierarchyTree, /* initOnRight */
                    true, paths, xAxisHierarchy);
                sourceHierarchyTree.removeLoadListener(this);
              }
            });
            sourceHierarchyTree.setInput(xAxisHierarchy, xViewModel);
  ...
  public final void setInput(final XAxisHierarchy hierarchy,
    final XElementNode[] initialSelectedElements, final boolean propagate,
    final XViewModel xViewModel) {
    ...
    // if (initialSelectedElements != null && initialSelectedElements.length >
    // 0) {
  ...
  private final LayoutContainer createSourcePanel() {
    ...
    // all together
    LayoutContainer result = arrangeCaptionTreeAndButtons(headerButtons, constants
      .selectElementsIncludingOriginalHierarchy(), treePanel, extendedButtonRow,
      levelButtonRow, regexPanel);
  ...
  private final LayoutContainer createSelectionButtons() {
    ...
    RowData layoutData = new RowData();
    layoutData.setMargins(new Margins(0, 5, 0, 0));
    addButton(BTN_ONE, BTN_ONE, panel, layoutData, false, constants
      .showSelectLevel1());
    addButton(BTN_TWO, BTN_TWO, panel, layoutData, false, constants
      .showSelectLevel2());
    addButton(BTN_THREE, BTN_THREE, panel, layoutData, false, constants
      .showSelectLevel3());
    addButton(BTN_FOUR, BTN_FOUR, panel, layoutData, false, constants
      .showSelectLevel4());
    addButton(BTN_FIVE, BTN_FIVE, panel, layoutData, false, constants
      .showSelectLevel5());
    RowData spaceData = new RowData();
    spaceData.setMargins(new Margins(0, 2, 0, 0));
    panel.add(new LabelField(), spaceData);
    addButton(BTN_LEAVES, constants.shortHandForBaseElements(), panel, layoutData,
      false, constants.showSelectBase());
    RowData spaceData2 = new RowData();
    spaceData2.setMargins(new Margins(0, 5, 0, 0));
    panel.add(new LabelField(), spaceData2);
    addButton(BTN_ALL, constants.shortHandForAllElements(), panel, layoutData,
      false, constants.showSelectAll());
    return panel;
  }
  ...
  private final LayoutContainer createExtendedButtons() {
    ...
    RowData layoutData = new RowData();
    layoutData.setMargins(new Margins(0, 5, 0, 0));
    // addButton(BTN_ALL, constants.shortHandForAllElements(), panel,
    // layoutData, false, constants.showSelectAll());
    // RowData spaceData = new RowData();
    // spaceData.setMargins(new Margins(0, 2, 0, 0));
    // panel.add(new LabelField(), spaceData);
    addButton(BTN_SELECT_BRANCH, constants.shortHandForSelectBranch(), panel,
      layoutData, false, constants.showSelectBranch());
    RowData spaceData2 = new RowData();
    spaceData2.setMargins(new Margins(0, 2, 0, 0));
    panel.add(new LabelField(), spaceData2);
    addButton(BTN_INVERT, constants.shortHandForInvertSelection(), panel,
      layoutData, false, constants.invertSelection());
    return panel;
  }
  ...
  private final LayoutContainer createRegexPanel() {
    ...
    addButton(BTN_SEARCH_SELECT, constants.shortHandForSearchSelect(), panel, lData,
      false, constants.showSearchSelect());
    return panel;
  }
  ...
  private final LayoutContainer createTargetPanel() {
    ...
    RowData layoutData = new RowData();
    layoutData.setMargins(new Margins(0, 5, 0, 0));
    addButton(BTN_EMPTY_LIST, constants.emptyList(), panel, layoutData, false,
      constants.emptyList());

    LayoutContainer cont = arrangeCaptionTreeAndButtons(panel, constants
      .createCustomizedHierarchy(), treePanel, null, buttonRow, null);
    return cont;
  }
  ...
  private final LayoutContainer arrangeCaptionTreeAndButtons(
    LayoutContainer headerButtons, String caption, LayoutContainer treePanel,
    LayoutContainer eButtons, LayoutContainer buttonRow, LayoutContainer regex) {
  ...
  class KeyButton extends Button {
    KeyButton(String caption) {
      super(caption);
      sinkEvents(Event.ONKEYDOWN | Event.ONKEYUP | Event.ONMOUSEDOWN
        | Event.ONMOUSEUP);
    }
  ...
  private final void addButton(String id, String caption, LayoutContainer panel,
    LayoutData layoutData, boolean toggle, String tooltip) {
  ...
  private final LayoutContainer createButtonsPanel() {
    ...
    addFilterButton = addIconButton(BTN_ADD, buttons, layoutData, constants
      .addElementToFilter());
    addIconButton(BTN_DELETE, buttons, layoutData, constants
      .removeElementFromFilter());
  ...
  private final Button addIconButton(String icon, LayoutContainer panel,
    LayoutData layoutData, String tooltip) {
  ...
  public void handleEvent(ComponentEvent be) {
    ...
    if (btnId.equals(BTN_ADD)) {
      ...
      sourceHierarchyTree.selectLevel(getLevel(btnId), true, isShiftPressed
        || isCtrlPressed);
    } else if (btnId.equals(BTN_SELECT_BRANCH)) {
      sourceHierarchyTree.selectBranch();
    } else if (btnId.equals(BTN_SEARCH_SELECT)) {
      sourceHierarchyTree.selectByRegEx(regExField.getValue(), true, isShiftPressed
        || isCtrlPressed);
        ...
        private final void doRemoveSelection() {
          try {
            ...
            if (targetHierarchyTree.getTree().getSelectedItems() != null) {
              selectionCount = targetHierarchyTree.getTree().getSelectedItems()
                .size();
            }
            if (selectionCount <= 1) {
    ...
    prompt.getTextBox().addKeyListener(new KeyListener() {
      public void componentKeyUp(ComponentEvent event) {
        ...
        if (pressedEnter(event.getKeyCode())) {
          prompt.getDialog().close(prompt.getDialog().getButtonById(Dialog.OK));
        }
      }
    });
  ...
  private final void createAndAddNode(String name) {
    ...
    // XElementNode xVirtualNode = new XElementNode(xVirtualElement,
    // xAxisHierarchy.getId(), xAxisHierarchy.getViewId());
