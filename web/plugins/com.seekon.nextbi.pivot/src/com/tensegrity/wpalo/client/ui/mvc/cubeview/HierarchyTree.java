...
/**
 * <code>HierarchyTree</code> TODO DOCUMENT ME
 * 
 * @version $Id: HierarchyTree.java,v 1.30 2010/04/12 11:13:36 PhilippBouillon
 *          Exp $
 **/
public abstract class HierarchyTree {
  ...
  protected abstract void loadChildren(final HasFastMSTreeItems parentItem,
    final TreeNode parentNode);

  // private final void loadChildren(final HasFastMSTreeItems parentItem, final
  // TreeNode parentNode) {
  ...
  //
  // WPaloServiceProvider.getInstance().loadHierarchyTree(sessionId, hierarchy,
  // -1, new AsyncCallback <List<TreeNode>>() {
  // public void onFailure(Throwable arg0) {
  // }
  //
  // public void onSuccess(final List <TreeNode> kids) {
  // final HashMap <TreeNode, FastMSTreeItem> parents = new HashMap<TreeNode,
  // FastMSTreeItem>();
  ...
  // });
  //		
  // // WPaloServiceProvider.getInstance().loadChildren(sessionId, parentNode,
  ...
  // // index++;
  // // if (item.getParentItem() != null) {
  // // if (item.getParentItem().getFinishHandler() != null && index >=
  // kids.size()) {
  ...
  /**
   * selects all currently visible elements. note: this only works if tree is a
   * checkbox tree...
   */
  ...
  /**
   * inverts the currently visible selection. note: this only works if tree is a
   * checkbox tree...
   */
  ...
  public final void apply(XElementNode[] initialSelection,
    DnDHierarchyTree otherTree, boolean showOnRight, String paths,
    ...
    if (initialSelection != null) {
      ...
      nodeSelector.applySelection(flatSelection, data, showOnRight, paths,
        xAxisHierarchy);
    } else {
      ((Workbench) Registry.get(Workbench.ID)).hideWaitCursor();
    }
  ...
  // //
  // ((Workbench)Registry.get(Workbench.ID)).showWaitCursor("Loading children of '"
  // + parent.getXObject().getName() + "'.");
  ...
  // }
  // };
  // MessageBox.info("SessionExpired",
  // cause.getLocalizedMessage()+"<br/>Please log in again.", callback);
  // }
  ...
  private final void traverse(ArrayList<FastMSTreeItem> items,
...
class TreeNodeSelector extends LoadListener {
  ...
  class SelectionTimer {
    ...
    public void run() {
      ...
      WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, timeout,
  ...
  public void loaderLoad(LoadEvent le) {
    ...
    if (activeSelection != null && activeSelection.size() == 0) {// &&
      // postponedSelection.size()
      // == 0 &&
      // !mustDisplayRight)
      // {
  ...
  final void applySelection(final ArrayList<XElementNode> selection,
    final InitialTreeData data, boolean showOnRight, final String paths,
    final XAxisHierarchy xAxisHierarchy) {
    if (showOnRight) {
      boolean hide = false;
  ...
  final void selectBranch() {
    ...
    Filter f = new DefaultFilter() {
      ...
      public boolean filter(FastMSTreeItem item) {
        ...
        if (par != null && par.isOpen()
          && (par.isSelected() || selPar.contains(par))) {
          selPar.add(item);
          select(item, true);
        }
        return true;
    ...
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
      new Callback<Void>() {
        public void onSuccess(Void arg0) {
          ...
          ensureTimer();
          if (found.size() == 0) {
            MessageBox.info(constants.information(), constants.noMatchFound(), null);
          }
        }
      });
  }
  ...
  final void collapseDeepestLevel() {
    ...
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
      new Callback<Void>() {
        public void onSuccess(Void arg0) {
          final HashSet<FastMSTreeItem> itemsToCollapse = new HashSet<FastMSTreeItem>();
          final LinkedHashSet<FastMSTreeItem> itemsToDeselect = new LinkedHashSet<FastMSTreeItem>();
          Filter f = new DefaultFilter() {
  ...
  private final void ensureTimer() {
    XUser user = ((Workbench) Registry.get(Workbench.ID)).getUser();
    final String sessionId = user.getSessionId();
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 500,
      new AsyncCallback<Void>() {
            ...
            if (doIt) {
              DeferredCommand.addCommand(new Command() {
                public void execute() {
                  ...
                  ((Workbench) Registry.get(Workbench.ID)).hideWaitCursor();
                  // ((LayoutContainer)
                  // tree.getTree().getParent()).scrollIntoView(node);
                }
              });
  ...
  private final void traverse(final FastMSTreeItem item, final Filter filter,
    boolean traverseClosed) {
    if (item == null || filter == null)
      return;
    if (filter.filter(item)) {
