...
/**
 * <code>ViewBrowserEditor</code> TODO DOCUMENT ME
 * 
 * @version $Id: ViewBrowserEditor.java,v 1.46 2010/04/12 11:13:36
 *          PhilippBouillon Exp $
 **/
public class ViewBrowserEditor implements IEditor {
        ...
        if (ce.item instanceof ViewEditorTab) {
          ...
          case Events.BeforeRemove:
            if (vEditor.isDirty()) {
              ...
              MessageBoxUtils.yesNoCancel(constants.saveView(), messages
                ...
                new Listener<WindowEvent>() {
                  public void handleEvent(WindowEvent be) {
                    ...
                    } else if (be.buttonClicked.getItemId().equalsIgnoreCase(
                      Dialog.NO)) {
                      close(editorTab);
                    }
  ...
  private final void saveAndClose(final ViewEditorTab tab,
    final AsyncCallback<Boolean> cb) {
    ...
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
      new Callback<Void>() {
        public void onSuccess(Void none) {
          tab.getEditor().save(
            new Callback<XViewModel>(constants.savingViewFailed()) {
  ...
  public void doSave(final AsyncCallback<Boolean> cb) {
    ...
    for (final TabItem item : tabFolder.getItems()) {
      if (item instanceof ViewEditorTab) {
        if (((ViewEditorTab) item).getEditor() != null) {
          if (((ViewEditorTab) item).getEditor().isDirty()) {
            ((ViewEditorTab) item).getEditor().save(new Callback<XViewModel>() {
  ...
  private final void setInputInternal(final XView xView,
    final ViewEditorTab viewTab, final XObject input) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, viewDelay,
  ...
  public void setInput(Object input) {
    if (input instanceof TreeNode || input instanceof XObject) {
      ...
      if (input instanceof XObject) {
        ...
        if (_input instanceof XObjectWrapper
          && ((XObjectWrapper) _input).getXObject() instanceof XFolderElement) {
          XObject so = ((XFolderElement) ((XObjectWrapper) _input).getXObject())
            .getSourceObject();
          if (so != null) {
      ...
      if (!isAlreadyOpen(_input) && !viewOpen) {
        ...
        DeferredCommand.addCommand(new Command() {
          public void execute() {
            final ViewEditorTab viewTab = new ViewEditorTab(_input.getName(), false,
              DisplayFlags.getDisplayFlagsFor(xView));
            ...
            if (viewDelay == -1) {
              WPaloPropertyServiceProvider.getInstance().getIntProperty(
                "delayViewOpen", 500, new AsyncCallback<Integer>() {
                  public void onFailure(Throwable arg0) {
                    viewDelay = 500;
  ...
  private final void handleUnknownView(final XObject xView, final ViewEditorTab tab) {
    ...
    MessageBox.confirm(constants.error(), constants.underlyingViewGone(), callback);
  }

  /**
   * shows the view if it is already open, otherwise calling this method has no
   * effect
   * 
   * @param input
   */
  ...
  private final void askBeforeClose(final ViewEditorTab editorTab,
  ...
  class SaveListenerCallback implements AsyncCallback<Boolean> {
    ...
    SaveListenerCallback(ArrayList<ViewEditorTab> dirtyTabs,
