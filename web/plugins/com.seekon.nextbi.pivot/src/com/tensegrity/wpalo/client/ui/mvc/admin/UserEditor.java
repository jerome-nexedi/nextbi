...
public class UserEditor extends AbstractTabEditor {
  ...
  public final EditorTab[] getEditorTabs() {
    return new EditorTab[] { new UserPropertiesTab(this), new AccountTab(this),
      new GroupTab(this), new RolesTab(this) };
  }
...
class GroupTab extends EditorTab {
  ...
  private final String translateDescription(String s) {
    ...
    if (s
      .equals("Grants the right to view & edit administration area and modify & share all existing views (System)")) {
      ...
      .equals("Grants the right to modify views shared by other users (System)")) {
      return constants.grantModify();
    } else if (s.equals("Grants the right to create and modify own views")) {
      return constants.grantCreate();
    } else if (s
      .equals("Grants the right to see views shared by other users (System)")) {
      return constants.grantView();
    } else if (s
      .equals("Grants the right to create views and edit these views (System)")) {
              ...
              public void onSuccess(XGroup[] userGroups) {
                ...
                for (XGroup group : groups) {
                  ...
                  TableItem item = new TableItem(new Object[] {
                    userGroupSet.contains(group.getId()), group.getName(),
                    roles.toString(), translateDescription(group.getDescription()) });
                  groupTable.add(item);
      ...
      new Callback<XAccount[]>(constants.loadingAllAccountsFailed()) {
        public void onSuccess(XAccount[] accounts) {
          ...
          for (XAccount account : accounts) {
            ...
            String type = connection.getConnectionType() == XConnection.TYPE_HTTP ? "Palo"
              : "XMLA";
            accountTable.insert(new TableItem(new Object[] { account.getLogin(),
              connection.getName(), connection.getHost(), connection.getService(),
              type }), index++);
          }
        }
      });
...
class RolesTab extends EditorTab {
  ...
  public void set(XObject input) {
    if (input instanceof XUser) {
      XUser user = (XUser) input;
      currentUser = user;
      boolean isAdm = currentUser != null && currentUser.getLogin() != null
        && currentUser.getLogin().equals("admin");
      ((CheckBoxTableColumn) rolesTable.getColumnModel().getColumn(0))
        .setEnabled(!isAdm);
        ...
        new Callback<XRole[]>(constants.loadingAllRolesFailed()) {
          public void onSuccess(XRole[] roles) {
            for (XRole role : roles) {
              ...
              if (p.equals("R"))
                rightName = constants.read();
              else if (p.equals("W"))
                rightName = constants.write();
              else if (p.equals("D"))
                rightName = constants.delete();
              else if (p.equals("C"))
                rightName = constants.create();
              else if (p.equals("G"))
                rightName = constants.grant();
              String desc = translateDescription(role.getDescription());
              TableItem item = new TableItem(new Object[] { check, role.getName(),
                rightName, desc });
