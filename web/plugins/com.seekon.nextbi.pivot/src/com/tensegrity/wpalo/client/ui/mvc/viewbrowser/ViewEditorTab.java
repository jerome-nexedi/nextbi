...
/**
 * <code>ViewEditorTab</code> TODO DOCUMENT ME
 * 
 * @version $Id: ViewEditorTab.java,v 1.50 2010/04/15 09:55:22 PhilippBouillon
 *          Exp $
 **/
public class ViewEditorTab extends EditorTab {
  ...
  public ViewEditorTab(String name, final boolean showSaveButtons,
    final DisplayFlags displayFlags) {
    super(modify(name));
    // setBorders(false);
  ...
  public void close() {
    if (vEditor.isDirty()) {
      ...
      MessageBox.confirm(constants.saveView(), messages
        .saveViewBeforeClosing(ViewBrowserModel.modify(view.getName())),
  ...
  public void saveAs(String name, XObject input, final boolean isPublic,
    final boolean isEditable, final Callback<Boolean> callback) {
    if (input instanceof XViewModel) {
      final XViewModel xViewModel = (XViewModel) input;
      ((Workbench) Registry.get(Workbench.ID))
        .showWaitCursor(constants.savingView());
      ...
      WPaloCubeViewServiceProvider.getInstance().saveViewAs(sessionId, name,
        xViewModel, new Callback<XView>(constants.savingViewFailed()) {
          ...
          public void onSuccess(XView xView) {
             ...
             * constants.notEnoughRightsToSaveView(), null); } else {
             */
            /*
             * ViewBrowser viewBrowser = (ViewBrowser)
             * Registry.get(ViewBrowser.ID); viewBrowser.addView(inputView,
             * xView, true, isPublic, isEditable);
             */
            if (callback != null) {
              callback.onSuccess(false);
              MessageBox.info(constants.subobjectSaved(), constants
                .subobjectSavedMsg(), null);
            }
            // }
            // ViewEditorTab.super.close();
          }
    ...
    WPaloCubeViewServiceProvider.getInstance().willOpen(sessionId, xView,
      new Callback<XLoadInfo>(constants.openingViewFailed()) {
        public void onFailure(Throwable t) {
          ...
          if (t instanceof PaloGwtCoreException && t.getMessage() != null
            && t.getMessage().equals("Session expired!")) {
            hideWaitCursor();
            ...
            if (locMessage != null
              && locMessage.toLowerCase().indexOf("session expired") != -1) {
  ...
  private final void displayWarningDialog(XViewModel model, String[] warnings) {
    StringBuffer message = new StringBuffer(messages
      .warningsWhenOpeningView(ViewBrowserModel.modify(model.getName())));
    for (String w : warnings) {
      message.append(w);
      message.append("<br/>");
    }
    MessageBox.alert(constants.warningsWhenOpeningView(), message.toString(),
      new Listener<WindowEvent>() {
        public void handleEvent(WindowEvent be) {
  ...
  private final void proceedOpenView(final XView xView) {
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().proceedOpen(
      sessionId,
      xView,
      new Callback<XViewModel>(messages.loadingViewFailed(ViewBrowserModel
        .modify(xView.getName()))) {
        ...
        public void onSuccess(final XViewModel model) {
          ...
          DeferredCommand.addCommand(new Command() {
            public void execute() {
              ...
              // BUG IN GXT??? WE HAVE TO MANUALLY RESIZE TAB FOLDER ON
              // INITIAL OPEN...
              final ViewEditorTabFolder tabFolder = (ViewEditorTabFolder) getParent();
  ...
  private final void handleFailure(String reason, final XView xView) {
    ...
    MessageBox.alert(constants.error(), messages.failedToLoadView(ViewBrowserModel
      .modify(xView.getName()), reason), callback);
  }
