 ...
 * 
 * @version $Id: ViewSelectionTree.java,v 1.13 2010/02/12 13:49:50
 *          PhilippBouillon Exp $
 **/
public class ViewSelectionTree {
  ...
  static {
    WPaloCubeViewServiceProvider.getInstance().initializeRoles(
      ((Workbench) Registry.get(Workbench.ID)).getUser().getSessionId(),
  ...
  private final TreeNode createRootNode(XAccount xAccount) {
    // we have to adjust XObject type so that it gets loaded by corresponding
    // child loader:
  ...
  private final XView createDefaultXView(XCube xCube, boolean isPublic,
    boolean isEditable) {
  ...
  private final TreeLoader<TreeNode> createTreeLoader() {
    ...
    treeLoader.addLoadListener(new LoadListener() {
      public void loaderLoadException(LoadEvent le) {
        MessageBox.alert(constants.loadingChildrenFailed(), constants
          .loadingDimensionDataError()
          + (le.exception != null ? messages.errorCause(le.exception.getMessage())
            : ""), null);
      }
...
class ViewTreeBinder extends TreeBinder<TreeNode> {
  ...
  protected TreeItem createItem(TreeNode model) {
    ...
    boolean isLeaf = doImport ? (xObj instanceof XView) : (xObj instanceof XCube);
