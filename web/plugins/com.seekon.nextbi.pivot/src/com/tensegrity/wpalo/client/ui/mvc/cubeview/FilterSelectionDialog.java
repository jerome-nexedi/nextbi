...
/**
 * <code>FilterSelectionDialog</code> TODO DOCUMENT ME
 * 
 * @version $Id: FilterSelectionDialog.java,v 1.43 2010/04/15 09:55:22
 *          PhilippBouillon Exp $
 **/
class FilterSelectionDialog extends Window {
  ...
  private final void addSubsetCombo(FormPanel form) {
    ...
    subsetCombo
      .addSelectionChangedListener(new SelectionChangedListener<XObjectModel>() {
  ...
  private final void addAliasFormatCombo(FormPanel form) {
    ...
    aliasFormats.add(new AliasFormat("elementNameDashAlias", constants
      .elementNameDashAlias()));
    aliasFormats.add(new AliasFormat("aliasDashElementName", constants
      .aliasDashElementName()));
    aliasFormats.add(new AliasFormat("elementNameParenAlias", constants
      .elementNameParenAlias()));
    aliasFormats.add(new AliasFormat("aliasParenElementName", constants
      .aliasParenElementName()));
    aliasFormats.add(new AliasFormat("elementNameAlias", constants
      .elementNameCommaAlias()));
    aliasFormats.add(new AliasFormat("aliasElementName", constants
      .aliasCommaElementName()));

    aliasFormats.setFieldLabel(constants.aliasFormatLabel());
    String prop = hierarchy.getProperty("aliasFormat");
    if (prop == null) {
      aliasFormats.setValue(aliasFormats.findModel(new AliasFormat("aliasFormat",
        constants.aliasFormat())));
    } else {
      aliasFormats
        .setValue(aliasFormats.findModel(new AliasFormat(prop, "ignored")));
    }
    aliasFormats.setEnabled(aliases.getSelection() != null);
    aliasFormats.addSelectionChangedListener(new SelectionChangedListener() {
          ...
          updateHierarchy(new UpdateListener() {
            public void updateFinished() {
              ...
              if (aliasFormats.isEnabled()) {
                hierarchy.addProperty("aliasFormat", aliasFormats.getValue()
                  .getValue().id);
              } else {
  ...
  private final void resetAndCloseFilterDialog(final Button pressedButton) {
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    ((Workbench) Registry.get(Workbench.ID)).showWaitCursor(constants.resetFilter());
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
      new Callback<Void>() {
        public void onSuccess(Void arg0) {
          disableComboBoxEvents(true);
  ...
  private final void applySubsetSelection(final XSubset subset) {
    ...
    if (localFilter.isExpanded()) {
      hierarchy
        .setVisibleElements((XElementNode[]) localFilter.getVisibleElements()[0]);
    }
    updateHierarchy(null, true, false, true);
  }
  ...
  private final void updateHierarchy(final UpdateListener listener,
    final boolean disableLocal, final boolean updateAlias, final boolean setInput) {
    ...
    WPaloCubeViewServiceProvider.getInstance().updateAxisHierarchy(sessionId,
      hierarchy, new Callback<XElement>() {
        ...
        public void onSuccess(XElement newSelectedElement) {
          if (updateAlias
            || (hierarchy.getSelectedElement() == null && newSelectedElement != null)) {
