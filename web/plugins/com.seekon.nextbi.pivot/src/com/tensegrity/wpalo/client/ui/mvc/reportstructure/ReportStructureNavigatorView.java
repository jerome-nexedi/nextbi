 ...
 * 
 * @version $Id: ReportStructureNavigatorView.java,v 1.20 2010/02/12 13:49:50
 *          PhilippBouillon Exp $
 */
public class ReportStructureNavigatorView extends View {
  ...
  protected void handleEvent(AppEvent event) {
    ...
    case WPaloEvent.INIT:
      ...
      Dispatcher
        .forwardEvent(WPaloEvent.EXPANDED_REPORT_STRUCTURE_SECTION, rootNode);
  ...
  private final void initUI() {
    ...
    addStaticFolder.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        RequestNameDialog rnd = new RequestNameDialog("Create new static folder",
          "Name", new ResultListener<String>() {
            public void requestCancelled() {
            }

            public void requestFinished(String result) {
              WPaloServiceProvider.getInstance().createStaticFolder(result, parent,
                user, new Callback<XStaticReportFolder>() {
    ...
    TextToolItem addDynamicFolder = new TextToolItem("",
      "icon-create-dynamic-folder");
    ...
    addDynamicFolder.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        final XObject parent = ((TreeNode) item.getModel()).getXObject();
        RequestNameDialog rnd = new RequestNameDialog("Create new dynamic folder",
          "Name", new ResultListener<String>() {
            public void requestCancelled() {
            }

            public void requestFinished(String result) {
              WPaloServiceProvider.getInstance().createDynamicFolder(result, parent,
                user, new Callback<XDynamicReportFolder>() {
    ...
    deleteItem.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        WPaloServiceProvider.getInstance().deleteElementTreeNodes(objs, user,
    ...
    quickView.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        // store.removeAll();
        // XNode root = new XNode(user,
        // XConstants.TYPE_ROOT_REPORT_STRUCTURE_NODE);
        // root.setName("DummyRoot");
        // root.setId("ReportStructureNavigatorView#RootNode");
        // root.setHasChildren(true);
        // rootNode = new TreeNode(null, root);
    ...
    navigator.addListener(Events.Expand, new Listener<BaseEvent>() {
      public void handleEvent(BaseEvent be) {
        fireEvent(new AppEvent<TreeNode>(WPaloEvent.EDIT_REPORT_STRUCTURE,
          new TreeNode(null, user)));
      }
    });
  ...
  private final void validateButtons(List<ModelData> selection) {
    ...
    for (ModelData node : selection) {
      ...
      if (parent.getXObject().getId()
        .equals("ReportStructureNavigatorView#RootNode")) {
        deleteItem.setEnabled(false);
        return;
      }
    }
    ...
    binder.addSelectionChangedListener(new SelectionChangedListener<ModelData>() {
      public void selectionChanged(SelectionChangedEvent<ModelData> se) {
        ...
        if (node != null && node.getXObject() != null) {
          fireEvent(new AppEvent<TreeNode>(WPaloEvent.SET_EDITOR_INPUT, node));
        }
      }
    });
    return tree;
...
class TreeNodeBinder extends TreeBinder<TreeNode> {
  ...
  TreeNodeBinder(Tree tree, TreeStore<TreeNode> store,
    ReportStructureNavigatorView view) {
    super(tree, store);
    this.view = view;
  }
