  ...
  public void set(final TreeNode input) {
    ...
    if (input.getXObject() instanceof XReport) {
      ...
      if (report.getReceiverType().equals(XConstants.TYPE_FOLDER_ELEMENT_SHEET)) {
        ViewModeWorkbench wb = (ViewModeWorkbench) Registry
          .get(ViewModeWorkbench.ID);
              ...
              WPaloServiceProvider.getInstance().getVariables(report, user,
                new Callback<XVariableDescriptor>() {
                  public void onSuccess(XVariableDescriptor desc) {
                    ...
                    // url += "&var_" + mapping.get(match) + "=" +
                    // xe.getName();
                    // break;
                    // }
                    // } else if (match instanceof XSubset) {
                    // if (xe.getHierarchy().equals(((XSubset)
                    // match).getHierarchy())) {
                    // url += "&var_" + mapping.get(match) + "=" +
                    // xe.getName();
                    // break;
                    ...
                    frame.setUrl(url);
                    ReportTab tab = new ReportTab(url + report.getName(), report
                      .getName());
