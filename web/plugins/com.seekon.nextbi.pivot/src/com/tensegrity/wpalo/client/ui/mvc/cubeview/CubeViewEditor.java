...
public class CubeViewEditor extends LayoutContainer implements ContainerListener,
  Listener<ToolBarEvent>, ItemExpandListener, ItemClickListener,
  HierarchyWidgetListener, ClickHandler {
  protected transient final ILocalConstants constants = Resources.getInstance()
    .getConstants();

  protected transient final ILocalMessages messages = Resources.getInstance()
    .getMessages();
  ...
  // drag n drop support:
  private final PickupDragController dragController = new PickupDragController(
    RootPanel.get(), false);
  ...
  private final void initMaxWidths() {
    WPaloPropertyServiceProvider.getInstance().getStringProperty(
      "tableColumnsMaxWidth", new AsyncCallback<String>() {
    ...
    WPaloPropertyServiceProvider.getInstance().getStringProperty(
      "tableRowsHeaderMaxWidth", new AsyncCallback<String>() {
        private final void determineWidth(String r) {
    ...
    WPaloPropertyServiceProvider.getInstance().getStringProperty(
      "tableDataCellToLongString", new AsyncCallback<String>() {
        public void onFailure(Throwable arg0) {
  ...
  public CubeViewEditor(EditorTab tab) {
    WPaloPropertyServiceProvider.getInstance().getBooleanProperty(
      "hideInfoButtonInView", false, new AsyncCallback<Boolean>() {
  ...
  private final void fastSetInput(XViewModel view) {
    ...
    int clientWidth = displayFlags.isHideStaticFilter() ? Limiter.setClientWidth(0)
      : Limiter.setClientWidth(repositoryContainer,
        dimRepository.getInnerWidth() - 2);
    int clientHeight = displayFlags.isHideStaticFilter() ? Limiter
      .setClientHeight(0) : Limiter.setClientHeight(repositoryContainer,
      dimRepository.getInnerHeight() - 1);
    if (clientWidth <= 0) {
      clientWidth = 22;
    }
    if (clientHeight <= 0) {
  ...
  private final void restoreRepository(XViewModel view) {
    ...
    if (!displayFlags.isHideStaticFilter()) {
      ...
      int width = getPreferredWidth(repositoryAxis
        .getProperty(XAxis.PREFERRED_WIDTH));
  ...
  private final void modifyView(XView xView, final String viewId) {
    ...
    ((Workbench) Registry.get(Workbench.ID))
      .showWaitCursor(constants.creatingView());
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloFolderServiceProvider.getInstance().importView(sessionId, xView,
  ...
  public void handleEvent(final ToolBarEvent tbe) {
    ...
    if (button.equals(SAVE_BTN))
      ...
      WPaloCubeViewServiceProvider.getInstance().updateLoadInfo(sessionId, view,
        cellsToDisplay, new Callback<XLoadInfo>(constants.updatingViewFailed()) {
      ...
      final PaloSuiteViewCreationDialog dia = new PaloSuiteViewCreationDialog();
      dia.addButtonListener(ViewImportDialog.BUTTON_OK, new Listener<BaseEvent>() {
        public void handleEvent(BaseEvent be) {
          ...
          if (xViews != null && xViews.length == 1) {
            ...
            String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
              .getSessionId();
            WPaloCubeViewServiceProvider.getInstance().saveView(sessionId, view,
  ...
  public void dropped(final ContainerWidget widget, int atIndex) {
    ...
    if (hasRowChanged() || hasColumnChanged()) {
      ...
      removeLocalFilter();
      view.setNeedsRestore(true);
      WPaloCubeViewServiceProvider.getInstance().proceedUpdateViewWithoutTable(
        sessionId,
        view,
        new Callback<XViewModel>(messages.updatingViewFailed(ViewBrowserModel
          .modify(view.getName()))) {
  ...
  private boolean hasColumnChanged() {
    return !areEqual(editorPanel.getColumnHierarchies(), view.getColumnAxis()
      .getAxisHierarchies());
  }
  ...
  private final void restoreSwappedFilterNodes(XAxis axis, XAxis newAxis) {
    for (XAxisHierarchy hier : axis.getAxisHierarchies()) {
      if (oldVisibles.containsKey(hier.getId())) {
        newAxis.getAxisHierarchy(hier.getId()).setOldVisibleElements(
          oldVisibles.get(hier.getId()));
      } else {
        newAxis.getAxisHierarchy(hier.getId()).setOldVisibleElements(null);
      }
      if (visibles.containsKey(hier.getId())) {
        newAxis.getAxisHierarchy(hier.getId()).setVisibleElements(
          visibles.get(hier.getId()));
      } else {
  ...
  private final void proceedUpdateView(XLoadInfo loadInfo) {
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().proceedUpdateView(
      sessionId,
      view,
      new Callback<XViewModel>(messages.updatingViewFailed(ViewBrowserModel
        .modify(view.getName()))) {
  ...
  private final void cancelUpdateView() {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().cancelUpdateView(
      sessionId,
      view,
      new WaitCursorCallback<XViewModel>(constants.cancelingUpdate(), messages
        .cancelViewUpdateFailed(ViewBrowserModel.modify(view.getName()))) {
        public void onSuccess(XViewModel view) {
          if (view.needsRestore()) {
            restoreLocalFilter(view);
            view.setNeedsRestore(false);
  ...
  final void checkDnDState() {
    if (writeRight == -1 && view != null) {
      ...
      WPaloCubeViewServiceProvider.getInstance().isOwner(usr.getSessionId(),
        view.getId(), new AsyncCallback<Boolean>() {
          public void onFailure(Throwable arg0) {
            XUser user = ((Workbench) Registry.get(Workbench.ID)).getUser();
            writeRight = 0;
  ...
  private final void initRepositoryContainer() {
    repositoryContainer = new Container(new DimensionRepositoryRenderer(displayFlags
      .isHideStaticFilter())) {
  ...
  private final void initViewPanel(boolean showSaveButtons) {
    ...
    WPaloCubeViewServiceProvider.getInstance().getSpagoBIUserMode(
      new Callback<String>() {
        ...
        public void onSuccess(String id) {
           ...
           * (hideSaveSubobjects[0]).equals("true")) { toolbar.add(new
           * SeparatorToolItem()); }
           */
        }
      });
    ...
    editorPanel = new ViewEditorPanel(this, displayFlags.isHideFilter(),
      displayFlags.isHideHorizontalAxis(), displayFlags.isHideVerticalAxis());
        ...
        WPaloCubeViewServiceProvider.getInstance().writeCell(sessionId,
          cell.getXCell(), view, new Callback<XCellCollection>() {
            public void onFailure(Throwable caught) {
              ...
              if (message == null) {
                MessageBox.alert(constants.error(), constants.writingCellFailed(),
                  null);
              } else if (message.indexOf("(1009) insufficient access rights") != -1) {
                MessageBox.alert(constants.notEnoughRights(), constants
                  .notEnoughRightsToModifyCell(), null);
              } else if (message.indexOf("(5012) sum of weights is") != -1) {
                MessageBox.alert(constants.splashingError(), constants
                  .splashingHint(), null);
              } else {
                MessageBox.alert(constants.error(), constants.writingCellFailed(),
                  null);
  ...
  private final void _layout() {
    ...
    int clientWidth = displayFlags.isHideStaticFilter() ? Limiter.setClientWidth(1)
      : Limiter.setClientWidth(repositoryContainer,
        dimRepository.getInnerWidth() - 2);
    int clientHeight = displayFlags.isHideStaticFilter() ? Limiter
      .setClientHeight(1) : Limiter.setClientHeight(repositoryContainer,
      dimRepository.getInnerHeight() - 1);
    if (clientWidth <= 0) {
      clientWidth = 22;
    }
    ...
    editorPanel.layout(clientWidth, clientHeight,
      displayFlags.isHideStaticFilter() ? 0 : 0);
  }

  private final boolean areEqual(XAxisHierarchy[] hierarchies,
    List<XAxisHierarchy> list) {
          ...
          updateAndSaveView(new Callback<XViewModel>(constants.savingViewFailed()) {
            ...
            public void onSuccess(XViewModel view) {
              if (view == null) {
                ...
                String sessionId = ((Workbench) Registry.get(Workbench.ID))
                ...
                MessageBox.alert(constants.notEnoughRights(), constants
                  .notEnoughRightsToSaveView(), null);
                return;
              }
              restoreLocalFilter(view);
              view.setNeedsRestore(false);
  ...
  private final void executePrint(XPrintConfiguration config) {
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    removeLocalFilter();
    view.setNeedsRestore(true);
    config.setMaxWidths(tableColumnsMaxWidth, tableRowsHeaderMaxWidth,
      tableDataCellToLongString);
      ...
      new Callback<XPrintResult>() {
        public void onSuccess(final XPrintResult result) {
          ...
          if (result == null || result.getFilename() == null) {
            ...
            int index = Math.max(fileName.lastIndexOf("/"), fileName
              .lastIndexOf("\\"));
            if (index != -1) {
            ...
            final String link = /* GWT.getModuleBaseURL() + */"downloads/"
              + fileName;

            WPaloPropertyServiceProvider.getInstance().getBooleanProperty(
              "askForPDFFileDeletion", false, new Callback<Boolean>() {
                public void onSuccess(Boolean res) {
                  if (res) {
                    MessageBox.confirm(constants.downloadPDFHeader(), messages
                      .downloadPDF(link), new Listener<WindowEvent>() {
                      public void handleEvent(WindowEvent be) {
                        if (be.buttonClicked.getItemId()
                          .equalsIgnoreCase(Dialog.YES)) {
                          WPaloCubeViewServiceProvider.getInstance().deleteFile(
                            result.getFilename(), new Callback<Void>() {
                              public void onSuccess(Void arg0) {
                              }
                            });
                        }
                    ...
                    MessageBox.info(constants.downloadPDFHeader(), messages
                      .downloadPDFOnlyClose(link), new Listener<WindowEvent>() {
                      public void handleEvent(WindowEvent be) {
                        WPaloCubeViewServiceProvider.getInstance().deleteFile(
                          result.getFilename(), new Callback<Void>() {
                            public void onSuccess(Void arg0) {
  ...
  private final void doPrint() {
    if (view.getColumnAxis().getAxisHierarchies().isEmpty()
      ...
      MessageBox.alert(constants.printError(), constants.cantPrintEmptyView(), null);
      return;
    }

    final PrintDialog pd = new PrintDialog(view);
    pd.addButtonListener(PrintDialog.BUTTON_OK, new Listener<BaseEvent>() {
      public void handleEvent(BaseEvent be) {
  ...
  private final void doSaveAsAfterCheck() {
    ...
    WPaloCubeViewServiceProvider.getInstance().checkPermission(user.getSessionId(),
      permission, new AsyncCallback<Boolean>() {
                ...
                if (be.buttonClicked.getItemId().equals(SaveAsDialog.SAVE)) {
                  ...
                  updateView(new Callback<Void>() {
                    ...
                    public void onSuccess(Void arg0) {
                      ...
                      // Window.alert("ok!");
                      ((ViewEditorTab) tab).saveAs(viewName, view, saveAsDlg
                        .isPublic(), saveAsDlg.isEditable(),
                        new Callback<Boolean>() {
                          public void onSuccess(Boolean result) {
                            WPaloCubeViewServiceProvider.getInstance()
                              .getSubobjectId(new Callback<String>() {
                                public void onFailure(Throwable t) {
                                  super.onFailure(t);
                                }
  ...
  private final void doShowAbout() {
    WPaloPropertyServiceProvider.getInstance().getBuildNumber(
      new AsyncCallback<String>() {
        ...
        public void onSuccess(final String result) {
          WPaloPropertyServiceProvider.getInstance().getCurrentBuildNumber(
            new AsyncCallback<String[]>() {
              public void onFailure(Throwable arg0) {
  ...
  private final void changeSelectedElement(
    final HierarchySelectionWidget hierWidget, final XElement selectedElement) {
      ...
      new Callback<XLoadInfo>(constants.changingSelectedElementFailed()) {
        ...
        public void onSuccess(final XLoadInfo loadInfo) {
          ConfirmLoadDialogListener dlgListener = new ConfirmLoadDialogListener() {
            ...
            public void proceed(boolean state) {
              proceedChangeSelectedElement(hierWidget, selectedElement, loadInfo);
            }
          };
          LargeQueryWarningDialog.confirm(loadInfo, dlgListener);
        }
  ...
  private final void proceedChangeSelectedElement(
    final HierarchySelectionWidget hierWidget, final XElement selectedElement,
    XLoadInfo loadInfo) {
    // if(loadInfo.loadCells > SHOW_WAIT_CURSOR_THRESHOLD)
    showWaitDialog(constants.changingSelectedElement());
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().proceedChangeSelectedElement(
      sessionId, view, xAxisHierarchy, selectedElement,
  ...
  private final void updateView(Callback<Void> updateCallback) {
    ...
    /*
     * WPaloCubeViewServiceProvider.getInstance().getSubobjectId(new
     * Callback<String>(){ public void onFailure(Throwable t) {
     * super.onFailure(t); } public void onSuccess(String id) {
     * refreshSubobjects(id); } });
     */
  ...
  private final void showMenu(final HierarchySelectionWidget widget) {
    ((Workbench) Registry.get(Workbench.ID)).showWaitCursor(constants
      .displayingElementTree(), true);
    final XAxisHierarchy xAxisHierarchy = widget.getHierarchy();

    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    String elementId = xAxisHierarchy.getSelectedElement() == null ? ""
      : xAxisHierarchy.getSelectedElement().getId();
    WPaloCubeViewServiceProvider.getInstance().getNumberOfChildren(sessionId,
      xAxisHierarchy.getId(), xAxisHierarchy.getViewId(),
      xAxisHierarchy.getAxisId(), elementId, new Callback<Integer>() {
        public void onFailure(Throwable t) {
          ((Workbench) Registry.get(Workbench.ID)).hideWaitCursor();
          super.onFailure(t);
        }
          ...
          LargeQueryWarningDialog.confirm(result, new ConfirmLoadDialogListener() {
            public void proceed(final boolean state) {
              ((Workbench) Registry.get(Workbench.ID)).showWaitCursor(constants
                .displayingElementTree(), true);
              try {
                final SelectElementDialog menu = new SelectElementDialog(view,
                  xAxisHierarchy, state);
                menu.addSelectionListener(new Listener<SelectionEvent>() {
                  public void handleEvent(SelectionEvent se) {
                    ...
                    if (selectedElement != null
                      && !selectedElement
                        .equals(xAxisHierarchy.getSelectedElement()))
  ...
  private final void checkSubset(final XAxisHierarchy hierarchy,
    final XAlias oldAlias, final XSubset oldSubset,
    final XElement oldSelectedElement, final XSubset selectedSubset,
    final String oldPaths, final String oldAliasFormat,
    final XElementNode[] initialVisibleElements, final HierarchyWidget widget) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().containsElement(sessionId,
      hierarchy.getId(), hierarchy.getViewId(), hierarchy.getAxisId(),
      oldSelectedElement, selectedSubset, new Callback<Boolean>() {
        public void onSuccess(Boolean result) {
          if (result) {
            adjustView();
            String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
            ...
            WPaloCubeViewServiceProvider.getInstance()
              .proceedUpdateViewWithoutTable(
                sessionId,
                view,
                new Callback<XViewModel>(messages
                  .updatingViewFailed(ViewBrowserModel.modify(view.getName()))) {
                  public void onSuccess(XViewModel view) {
                    markDirty(true);
                    fastSetInput(view);
                    if (widget instanceof HierarchySelectionWidget) {
                      ((HierarchySelectionWidget) widget)
  ...
  private final void showFilterSelection(final HierarchyWidget widget,
    boolean fastTreeTest) {
    final TestFastTreeDialog tftd = new TestFastTreeDialog(view, widget
      .getHierarchy());
    tftd.show();
  }

  private final void applyFilterSettings(final FilterSelectionDialog dlg,
    final XElementNode[] initialVisibleElements, final HierarchyWidget widget) {
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
      new Callback<Void>() {
        public void onSuccess(Void arg0) {
          ...
          final String oldAliasFormat = widget.getHierarchy().getProperty(
            "aliasFormat");
          ...
          final boolean containsSelected = contains(hierarchy.getVisibleElements(),
            oldSelectedElement);
          if (isInRowOrColumn(hierarchy)) {
            ...
            if (!containsSelected
              || (dlg.getOldSelectedElement() == null && hierarchy
                .getSelectedElement() != null)) {
              willUpdateHierarchy(widget, oldSubset, oldAlias,
                initialVisibleElements, oldPaths, oldAliasFormat, dlg
                  .getOldSelectedElement());
            } else {
              if (dlg.getSelectedSubset() != null) {
                checkSubset(hierarchy, oldAlias, oldSubset, oldSelectedElement, dlg
                  .getSelectedSubset(), oldPaths, oldAliasFormat,
                  initialVisibleElements, widget);
  ...
  private final void showFilterSelection(final HierarchyWidget widget) {
    final XElementNode[] initialVisibleElements = widget.getHierarchy()
      .getVisibleElements();
    final FilterSelectionDialog dlg = new FilterSelectionDialog(widget
      .getHierarchy(), view);

    dlg.addListener(Events.Close, new Listener<WindowEvent>() {
      public void handleEvent(WindowEvent be) {
        // which button was pressed:
  ...
  private final void willUpdateHierarchy(final HierarchyWidget hierWidget,
    ...
    final XElementNode[] oldVisibleElements, final String oldPaths,
    final String oldAliasFormat, final XElement oldSelectedElement) {
    ...
    WPaloCubeViewServiceProvider.getInstance().willUpdateAxisHierarchy(sessionId,
      hierarchy, new Callback<XLoadInfo>(constants.updatingHierarchyFailed()) {
        public void onFailure(Throwable caught) {
          hideWaitCursor();
          super.onFailure(caught);
        }
    ...
    WPaloCubeViewServiceProvider.getInstance().willUpdateAxisHierarchy(sessionId,
      hierarchy, new Callback<XLoadInfo>(constants.updatingHierarchyFailed()) {
        ...
        public void onSuccess(final XLoadInfo loadInfo) {
          ...
          // ConfirmLoadDialogListener dlgListener = new
  ...
  private final void proceedUpdateHierarchy(final XAxisHierarchy hierarchy,
    XLoadInfo loadInfo) {
    // if(loadInfo.loadCells > SHOW_WAIT_CURSOR_THRESHOLD)
    showWaitDialog(constants.updatingView());
  ...
  private final void proceedUpdateHierarchy(final HierarchyWidget hierWidget,
    XLoadInfo loadInfo) {
    // if(loadInfo.loadCells > SHOW_WAIT_CURSOR_THRESHOLD)
    showWaitDialog(constants.updatingView());
    // PR 727: we have to update and reload view completely...
  ...
  public void collapse(XAxisItem item, String viewId, String axisId, boolean column) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
  ...
  private final void proceedWithCollapse(XAxisItem item, String viewId,
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().proceedCollapse(sessionId, item,
      viewId, axisId,
  ...
  private final void cancelCollapse(XAxisItem item, String viewId, String axisId,
    boolean column) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().cancelCollapse(sessionId, item,
      viewId, axisId,
      new Callback<Void>(messages.expandingItemFailed(item.getName())) {
        public void onSuccess(Void v) {
          hideWaitCursor();
        }
      });
  ...
  public void willCollapse(final XAxisItem item, final String viewId,
    final String axisId, final boolean column) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    showWaitDialog(messages.collapsingItem(item.getName()));
    WPaloCubeViewServiceProvider.getInstance().willCollapse(sessionId, item, viewId,
      axisId, new Callback<XLoadInfo>(messages.collapseItemFailed(item.getName())) {
        public void onSuccess(final XLoadInfo loadInfo) {
          ConfirmLoadDialogListener dlgListener = new ConfirmLoadDialogListener() {
            public void cancel() {
  ...
  public void willExpand(final XAxisItem item, final String viewId,
    final String axisId, final boolean column) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    showWaitDialog(messages.expandingItem(item.getName()));
    WPaloCubeViewServiceProvider.getInstance().willExpand(sessionId, item, viewId,
      axisId, new Callback<XLoadInfo>(messages.expandingItemFailed(item.getName())) {
        public void onSuccess(final XLoadInfo loadInfo) {
          ConfirmLoadDialogListener dlgListener = new ConfirmLoadDialogListener() {
            public void cancel() {
  ...
  private final void proceedWithExpand(XAxisItem item, String viewId, String axisId,
    final boolean column, XLoadInfo loadInfo) {
    // if(loadInfo.loadCells > SHOW_WAIT_CURSOR_THRESHOLD)
    showWaitDialog(constants.updatingView());
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().proceedExpand(sessionId, item,
      viewId, axisId,
  ...
  private final void cancelExpand(XAxisItem item, String viewId, String axisId,
    boolean column) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
  ...
  public void setExpandState(final XAxisItem[] expanded,
    final XAxisItem[] collapsed, int expandDepth, final String viewId,
    String axisId, final boolean column) {
  ...
  private final void proceedSetExpandState(String viewId, final boolean column,
    XLoadInfo loadInfo) {
    // if (loadInfo.loadCells > SHOW_WAIT_CURSOR_THRESHOLD)
    showWaitDialog(constants.updatingView());
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
  ...
  private final void cancelSetExpandState(String viewId) {
    ...
    WPaloCubeViewServiceProvider.getInstance().cancelSetExpandState(sessionId,
      viewId, new Callback<Void>(constants.settingExpandStateFailed()) {
        public void onSuccess(Void v) {
          hideWaitCursor();
        }
      });
  ...
  final void checkWritePermission(AsyncCallback<Boolean> cb) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().checkPermission(sessionId,
      view.getId(), RIGHT_WRITE, cb);
  }
  ...
  private final void hideElement(final XAxisItem item, final List<XAxisItem> roots,
    String viewId, String axisId, boolean column, boolean hideLevel) {
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    if (hideLevel) {
    ...
    WPaloCubeViewServiceProvider.getInstance().hideItem(sessionId, item, roots,
      viewId, axisId, column, hideLevel, new AsyncCallback<String[]>() {
        public void onFailure(Throwable t) {
        ...
        public void onSuccess(final String[] result) {
          WPaloCubeViewServiceProvider.getInstance().runAsync(sessionId, 0,
            new Callback<Void>() {
              public void onSuccess(Void arg0) {
                XAxisHierarchy hierarchy = view.getAxisHierarchy(item
                  .getHierarchyId());
                ...
                if (result != null) {
                  ...
                  for (int i = 0; i < result.length; i += 5) {
                    ...
                    XElementNode xElemNode = new XElementNode(xElement, item
                      .getHierarchyId(), view.getId());
                ...
                willUpdateHierarchy(hierarchy, oldSubset, oldAlias, currentVisible,
                  oldPaths, oldSelectedElement);
              }
            });
        }
      });
  ...
  private final boolean isHideAllowed(XAxisItem item, List<XAxisItem> siblings,
    boolean isLevel) {
  ...
  public void leftClicked(final XAxisItem item, final List<XAxisItem> roots,
    final String viewId, final String axisId, final boolean column, int x, int y) {
    ...
    hideElement.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        if (item.depth != 0 || isHideAllowed(item, roots, false)) {
          ...
          MessageBox.info(constants.information(), constants
            .hideForbiddenLastElement(), null);
        }
      }
    });
      ...
      public void componentSelected(ComponentEvent ce) {
        if (item.depth == 0) {
          if (isHideAllowed(item, roots, true)) {
            ...
            MessageBox
              .info(constants.information(), constants.hideForbidden(), null);
          }
        } else {
    ...
    boolean enabled = hierarchy != null && hierarchy.getVisibleElements() != null
      && hierarchy.getVisibleElements().length > 0;
  ...
  public void rightClicked(XAxisItem item, List<XAxisItem> roots, String viewId,
    String axisId, boolean column, int x, int y) {
  ...
  public void onClick(ClickEvent ignored) {
    ...
    final String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().willSwapAxes(sessionId, view.getId(),
      new Callback<XLoadInfo>() {
        public void onSuccess(XLoadInfo loadInfo) {
          ConfirmLoadDialogListener dlgListener = new ConfirmLoadDialogListener() {
            ...
            public void proceed(boolean state) {
              removeLocalFilter();
              view.setNeedsRestore(true);
              WPaloCubeViewServiceProvider.getInstance().proceedSwapAxes(sessionId,
...
class SetupHandler implements Listener<BaseEvent> {
  ...
  void setupDone() {
    ...
    managedComponent.addListener(Events.Resize, new Listener<BoxComponentEvent>() {
  ...
  private final boolean isExpand() {
    ...
    return !state.containsKey("collapsed")
      && !editor.displayFlags.isHideStaticFilter();
  }
  ...
  private final void expandOnPermission() {
    ...
    if (initiallyExpand) // we are allow to expand if it was initially
      // expanded...
      expand(true);
      ...
      if (writePermission == -1) {
        editor.checkWritePermission(new Callback<Boolean>() {
          public void onSuccess(Boolean granted) {
            if (!granted) {
              MessageBox.alert(constants.notEnoughRights(), constants
                .notEnoughRightsToModifyStaticFilters(), null);
            }
            expand(granted);
          }
        });
  ...
  void setupWidth(int width) {
    ...
    if (initiallyExpand && !editor.displayFlags.isHideStaticFilter()) {
      BorderLayoutData layoutData = (BorderLayoutData) ComponentHelper
        .getLayoutData(managedComponent);
