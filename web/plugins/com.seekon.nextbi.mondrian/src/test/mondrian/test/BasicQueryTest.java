/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/BasicQueryTest.java#172 $
 ...
 * @since Feb 14, 2003
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/BasicQueryTest.java#172 $
 */
public class BasicQueryTest extends FoodMartTestCase {
  static final String EmptyResult = "Axis #0:\n" + "{}\n" + "Axis #1:\n"
    + "Axis #2:\n";

  private static final String timeWeekly = TestContext.hierarchyName("Time",
    "Weekly");
  ...
  public void testSample5Snowflake() {
    propSaver.set(MondrianProperties.instance().FilterChildlessSnowflakeMembers,
      false);
    final TestContext context = getTestContext().withFreshConnection();
    try {
      context.assertQueryReturns(sampleQueries[5].query, "Axis #0:\n"
        + "{[Time].[1997]}\n" + "Axis #1:\n" + "{[Measures].[Store Cost]}\n"
        + "{[Measures].[Store Sales]}\n" + "{[Measures].[Store Profit Rate]}\n"
        + "Axis #2:\n" + "{[Product].[Food].[Breakfast Foods]}\n"
        + "{[Product].[Non-Consumable].[Carousel]}\n"
  ...
  public void testGoodComments() {
    ...
    allCommentList.add("");
    final String[] allComments = allCommentList.toArray(new String[allCommentList
      .size()]);
    ...
    for (String comment : allComments) {
      assertQueryReturns(comment + "SELECT {} ON ROWS, {} ON COLUMNS FROM [Sales]",
        EmptyResult);
    }

    // Comment after SELECT.
    for (String comment : allComments) {
      assertQueryReturns("SELECT" + comment
        + "{} ON ROWS, {} ON COLUMNS FROM [Sales]", EmptyResult);
    }
    ...
    for (String comment : allComments) {
      assertQueryReturns("SELECT {} ON ROWS" + comment
        + ", {} ON COLUMNS FROM [Sales]", EmptyResult);
    }

    // Comment before slicer.
    for (String comment : allComments) {
      assertQueryReturns("SELECT {} ON ROWS, {} ON COLUMNS FROM [Sales] WHERE"
        + comment + "([Gender].[F])", "Axis #0:\n" + "{[Gender].[F]}\n"
        + "Axis #1:\n" + "Axis #2:\n");
    }

    // Comment after query.
    for (String comment : allComments) {
      assertQueryReturns("SELECT {} ON ROWS, {} ON COLUMNS FROM [Sales]" + comment,
        EmptyResult);
    }
    ...
    assertQueryReturns("-- an entire select statement commented out\n"
  ...
  public void testBadComments() {
    ...
    assertQueryThrows("/* a simple /* nested */ comment */\n"
  ...
  /**
   * Tests that a query whose axes are empty works; bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-52">MONDRIAN-52</a>.
   */
  public void testBothAxesEmpty() {
    ...
    assertQueryReturns("SELECT Filter({[Gender].MEMBERS}, 1 = 0) ON COLUMNS, \n"
      + "{} ON ROWS\n" + "FROM [Sales]", EmptyResult);

    // with slicer
    assertQueryReturns("SELECT {} ON ROWS, {} ON COLUMNS \n"
      + "FROM [Sales] WHERE ([Gender].[F])", "Axis #0:\n" + "{[Gender].[F]}\n"
      + "Axis #1:\n" + "Axis #2:\n");
  }

  /**
   * Used to test that a slicer with multiple values gives an error; bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-96">MONDRIAN-96</a>. But now
   * compound slicers are valid.
   */
  public void testCompoundSlicer() {
    ...
    assertQueryReturns("SELECT {[Measures].[Unit Sales]} ON COLUMNS,\n"
      ...
      + "WHERE ([Marital Status].[S], " + timeWeekly + ".[1997].[20])", "Axis #0:\n"
    ...
    assertQueryReturns("SELECT {[Measures].[Unit Sales]} ON COLUMNS,\n"
      + " {[Gender].MEMBERS} ON ROWS\n" + "FROM [Sales]"
      + "WHERE ([Time].[1997].[Q1], " + timeWeekly + ".[1997].[4])", "Axis #0:\n"
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-814">MONDRIAN-814,
   * "MDX with specific where clause doesn't work" </a>. This test case was as
   * close as I could get to the original test case on the foodmart data set,
   * but it did not reproduce the bug.
   */
  public void testCompoundSlicerNonEmpty() {
    ...
    final String mdx = "select non empty [Measures].[Sales Count] on 0,\n"
      + " non empty hierarchize(\n" + "  Crossjoin(\n" + "    Union(\n"
      + "     [Gender].CurrentMember,\n" + "     [Gender].Children),\n"
      + "    Union(\n" + "     [Product].CurrentMember,\n"
      + "     [Product].[Brand Name].Members))) on 1\n" + "from [Sales]\n"
      + "where { " + timeWeekly + ".[1997].[20]" + "      , " + timeWeekly
      + ".[1997].[21]" + "      , " + timeWeekly + ".[1997].[22]" + "      , "
      + timeWeekly + ".[1997].[23]" + " }";
    if (false) {
  ...
  /**
   * Requires the use of a sparse segment, because the product dimension has 6
   * atttributes, the product of whose cardinalities is ~8M. If we use a dense
   * segment, we run out of memory trying to allocate a huge array.
   */
  public void testBigQuery() {
    ...
    assertEquals(
      MondrianProperties.instance().FilterChildlessSnowflakeMembers.get() ? 2256
        : 2266, rowCount);
    assertEquals("152", result.getCell(new int[] { 0, rowCount - 1 })
      .getFormattedValue());
  }
  ...
  public void testGetContext() {
    ...
    assertEquals("[Measures].[Unit Sales]", cell.getContextMember(
      hierarchyMap.get("[Measures]")).getUniqueName());
    assertEquals("[Time].[1997]", cell.getContextMember(hierarchyMap.get("[Time]"))
      .getUniqueName());
    assertEquals("[Time].[Weekly].[1997].[6].[1]", cell.getContextMember(
      hierarchyMap.get("[Time].[Weekly]")).getUniqueName());
    assertEquals("[Gender].[All Gender]", cell.getContextMember(
      hierarchyMap.get("[Gender]")).getUniqueName());
    assertEquals("[Marital Status].[S]", cell.getContextMember(
      hierarchyMap.get("[Marital Status]")).getUniqueName());
  }
   ...
   * http://msdn.microsoft.com/library/en-us/olapdmad/agmdxadvanced_6jn7.asp if
   * solve orders are the same then the dimension specified first when defining
   * the cube wins.
   * 
   * <p>
   ...
   * 
   * <p>
   * There used to be something wrong with non-measure calculated members where
   * the ordering of the WITH MEMBER would determine whether or not the member
   * would be found in the cube. This test would fail but the previous one would
   * work ok.
   */
  ...
  /**
   * This one had the same problem. It wouldn't find the [Store].[x] member
   * because it has the same leaf name as [Product].[x]. (See MONDRIAN-77.)
   */
  ...
  /**
   * Disabled test. It used throw an 'infinite loop' error (which is what Plato
   * does). But now we revert to the context of the default member when
  ...
  public void testHalfYears() {
    Util
      .discard(executeQuery("WITH MEMBER [Measures].[ProfitPercent] AS\n"
  ...
  public void testAsSample7ButUsingVirtualCube() {
    Util
      .discard(executeQuery("with member [Measures].[Accumulated Sales] as 'Sum(YTD(),[Measures].[Store Sales])'\n"
        + "select\n"
        + "    {[Measures].[Store Sales],[Measures].[Accumulated Sales]} on columns,\n"
        + "    {Descendants([Time].[1997],[Time].[Month])} on rows\n"
        + "from [Warehouse and Sales]"));
  ...
  private static final List<QueryAndResult> taglibQueries = Arrays
    .asList(
    // 0
      new QueryAndResult(
        "select\n"
  ...
  public void testFormatOfNulls() {
    ...
    // explicit null value
    assertQueryReturns("with member [Measures].[Foo] as null,\n"
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-434">MONDRIAN-434</a>,
  ...
  /**
   * If a measure (in this case, <code>[Measures].[Sales Count]</code>) occurs
   * only within a format expression, bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-14">MONDRIAN-14</a>. causes
   * an internal error ("value not found") when the cell's formatted value is
   * retrieved.
   */
   ...
   * required expression [Measures].[Unit Sales] is not in the cache; bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-34">MONDRIAN-34</a>.
   */
  ...
  /**
   * Tuple as slicer causes {@link ClassCastException}; bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-36">MONDRIAN-36</a>.
   */
   ...
   * {@link ArrayIndexOutOfBoundsException}; <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-46">MONDRIAN-46</a>.
   */
   ...
   * 
   * <p>
   * The [Fact Count] measure, which is implicitly created because the cube
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-8">MONDRIAN-8,
   * "Problem getting children in hierarchy based on join."</a>. It happens when
   * getting the children of a member crosses a table boundary.
   */
  ...
  public void _testBug793616() {
    ...
    assertTrue("performance problem: execute took " + execMillis + " milliseconds, "
      + execNonDbMillis + " milliseconds excluding db", execNonDbMillis <= 2000
      && execMillis <= 30000);
  }
  ...
  /**
   * Turn off aggregate caching and run query with both use of aggregate tables
   * on and off - should result in the same answer. Note that if the
   * "mondrian.rolap.aggregates.Read" property is not true, then no aggregate
   * tables is be read in any event.
   */
   ...
   * 
   * [Store Size in SQFT].[All Store Size in SQFTs] [Store Size in SQFT].[null]
   * [Store Size in SQFT].[<each distinct store size>]
   * 
   * Postgres shows:
   * 
   * [Store Size in SQFT].[All Store Size in SQFTs] [Store Size in SQFT].[<each
   * distinct store size>] [Store Size in SQFT].[null]
   * 
   * The test failure is due to some inherent differences in the way Postgres
   * orders NULLs in a result set, compared with MySQL and Access.
   * 
   ...
   * 
   * So, this test has expected results that vary depending on whether the
   * database is being used sorts nulls high or low.
   * 
   */
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-977">MONDRIAN-977,
   * "NPE in Query with Crossjoin Descendants of Unknown Member"</a>.
   */
  public void testCrossjoinWithDescendantsAndUnknownMember() {
    propSaver.set(MondrianProperties.instance().IgnoreInvalidMembersDuringQuery,
      true);
    assertQueryReturns("select {[Measures].[Unit Sales]} on columns,\n"
      + "NON EMPTY CrossJoin(\n"
  ...
  /**
   * Slicer contains <code>[Promotion Media].[Daily Paper]</code>, but filter
   * expression is in terms of <code>[Promotion Media].[Radio]</code>.
   */
  ...
  public void testUnparse() {
    ...
    Query query = connection
      .parseQuery("with member [Measures].[Rendite] as \n"
        + " '(([Measures].[Store Sales] - [Measures].[Store Cost])) / [Measures].[Store Cost]',\n"
  ...
  public void testUnparse2() {
    Connection connection = getConnection();
    Query query = connection.parseQuery("with member [Measures].[Foo] as '1', "
      + "format_string='##0.00', " + "funny=IIf(1=1,\"x\"\"y\",\"foo\") "
    ...
    TestContext.assertEqualsVerbose("with member [Measures].[Foo] as '1', "
      + "format_string = \"##0.00\", "
   ...
   * against a cube other than the cube currently indicated by query context to
   * retrieve a single string or numeric result.
   * 
   * <p>
   * For example, the Budget cube in the FoodMart 2000 database contains budget
   * information that can be displayed by store. The Sales cube in the FoodMart
   * 2000 database contains sales information that can be displayed by store.
   * Since no virtual cube exists in the FoodMart 2000 database that joins the
   * Sales and Budget cubes together, comparing the two sets of figures would be
   * difficult at best.
   * 
   * <p>
   * <b>Note<b> In many situations a virtual cube can be used to integrate data
   * from multiple cubes, which will often provide a simpler and more efficient
   * solution than the LookupCube function. This example uses the LookupCube
   * function for purposes of illustration.
   * 
   * <p>
   * The following MDX query, however, uses the LookupCube function to retrieve
   * unit sales information for each store from the Sales cube, presenting it
   * side by side with the budget information from the Budget cube.
   */
  ...
  /**
   * <p>
   * Basket analysis is a topic better suited to data mining discussions, but
   ...
   * Province levels of the Customers dimension. The count and store sales total
   * of qualified customers is represented by the Qualified Count and Qualified
   * Sales columns, respectively.
   * 
   * <p>
   * To accomplish this basic form of basket analysis, the following MDX query
   * constructs two calculated members. The first calculated member uses the MDX
   * Count, Filter, and Descendants functions to create the Qualified Count
   * column, while the second calculated member uses the MDX Sum, Filter, and
   * Descendants functions to create the Qualified Sales column.
   * 
   * <p>
   * The key to this MDX query is the use of Filter and Descendants together to
   * screen out non-qualified customers. Once screened out, the Sum and Count
   * MDX functions can then be used to provide aggregation data only on
   * qualified customers.
   */
   ...
   * has been known to fail when run standalone.
   */
   ...
   * comparison functions from the Microsoft Visual Basic for Applications (VBA)
   * external function library.
   * 
   * <p>
   * For example, you want to report the unit sales of all fruit-based products
   * -- not only the sales of fruit, but canned fruit, fruit snacks, fruit
   * juices, and so on. By using the LCase and InStr VBA functions, the
   ...
   * 
   * <p>
   * The following MDX query demonstrates how to achieve the results displayed
   * in the previous table. For each member in the Product dimension, the name
   * of the member is converted to lowercase using the LCase VBA function. Then,
   * the InStr VBA function is used to discover whether or not the name contains
   * the word "fruit". This information is used to then construct a set, using
   * the Filter MDX function, from only those members from the Product dimension
   * that contain the substring "fruit" in their names.
   */
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-539">MONDRIAN-539,
   ...
   * 
   * <p>
   * Another common business question easily answered through MDX is the display
   * of percent values created as available measures.
   * 
   * <p>
   * For example, the Sales cube in the FoodMart 2000 database contains unit
   * sales for each store in a given city, state, and country, organized along
   * the Sales dimension. A report is requested to show, for California, the
   * percentage of total unit sales attained by each city with a store. The
   * results are illustrated in the following table.
   * 
   * <p>
   * Because the parent of a member is typically another, aggregated member in a
   * regular dimension, this is easily achieved by the construction of a
   * calculated member, as demonstrated in the following MDX query, using the
   ...
   * 
   * <p>
   * Another common business request, cumulative sums, is useful for business
   * reporting purposes. However, since aggregations are handled in a
   * hierarchical fashion, cumulative sums present some unique challenges in
   * Analysis Services.
   * 
   * <p>
   * The best way to create a cumulative sum is as a calculated measure in MDX,
   * using the Rank, Head, Order, and Sum MDX functions together.
   * 
   * <p>
   * For example, the following table illustrates a report that shows two views
   * of employee count in all stores and cities in California, sorted by
   * employee count. The first column shows the aggregated counts for each store
   * and city, while the second column shows aggregated counts for each store,
   * but cumulative counts for each city.
   * 
   * <p>
   * The cumulative number of employees for San Diego represents the value of
   * both Los Angeles and San Diego, the value for Beverly Hills represents the
   * cumulative total of Los Angeles, San Diego, and Beverly Hills, and so on.
   * 
   * <p>
   * Since the members within the state of California have been ordered from
   * highest to lowest number of employees, this form of cumulative sum measure
   * provides a form of pareto analysis within each state.
   * 
   * <p>
   * To support this, the Order function is first used to reorder members
   * accordingly for both the Rank and Head functions. Once reordered, the Rank
   * function is used to supply the ranking of each tuple within the reordered
   * set of members, progressing as each member in the Store dimension is
   * examined. The value is then used to determine the number of tuples to
   * retrieve from the set of reordered members using the Head function.
   * Finally, the retrieved members are then added together using the Sum
   * function to obtain a cumulative sum. The following MDX query demonstrates
   * how all of this works in concert to provide cumulative sums.
   * 
   * <p>
   * As an aside, a named set cannot be used in this situation to replace the
   * duplicate Order function calls. Named sets are evaluated once, when a query
   * is parsed -- since the set can change based on the fact that the set can be
   * different for each store member because the set is evaluated for the
   * children of multiple parents, the set does not change with respect to its
   * use in the Sum function. Since the named set is only evaluated once, it
   * would not satisfy the needs of this query.
   */
   ...
   * purpose, and understanding how the WHERE clause is used in MDX can assist
   * in constructing such business queries.
   * 
   * <p>
   ...
   * query are further sliced. As such, only expressions that resolve to a
   * single tuple are allowed. The WHERE clause implicitly supports a logical
   * AND operation involving members across different dimensions, by including
   * the members as part of a tuple. To support logical AND operations involving
   ...
   * clause is then used to restrict return of information only to the
   * calculated member, effectively implementing a logical OR to return
   * information for all time periods that contain unit sales values for either
   * food, drink, or both types of products.
   * 
   * <p>
   ...
   * WHERE clause that effectively restricts the query to retrieve unit sales
   * for drink products in the USA, shown by quarter and year for 1997.
   */
   ...
   * same dimension, you can use a calculated member or a named set to filter
   * out the unwanted members, as demonstrated in the following MDX query.
   * 
   * <p>
   ...
   * 
   * <p>
   * Member properties are a good way of adding secondary business information
   * to members in a dimension. However, getting that information out can be
   * confusing -- member properties are not readily apparent in a typical MDX
   * query.
   * 
   * <p>
   * Member properties can be retrieved in one of two ways. The easiest and most
   * used method of retrieving member properties is to use the DIMENSION
   * PROPERTIES MDX statement when constructing an axis in an MDX query.
   * 
   * <p>
   * For example, a member property in the Store dimension in the FoodMart 2000
   * database details the total square feet for each store. The following MDX
   * query can retrieve this member property as part of the returned cellset.
   */
  ...
  /**
   * <p>
   * The drawback to using the DIMENSION PROPERTIES statement is that, for most
   * client applications, the member property is not readily apparent. If the
   * previous MDX query is executed in the MDX sample application shipped with
   * SQL Server 2000 Analysis Services, for example, you must double-click the
   * name of the member in the grid to open the Member Properties dialog box,
   * which displays all of the member properties shipped as part of the cellset,
   * including the [Store].[Store Name].[Store Sqft] member property.
   * 
   * <p>
   * The other method of retrieving member properties involves the creation of a
   * calculated member based on the member property. The following MDX query
   * brings back the total square feet for each store as a measure, included in
   * the COLUMNS axis.
   * 
   * <p>
   * The [Store SqFt] measure is constructed with the Properties MDX function to
   * retrieve the [Store SQFT] member property for each member in the Store
   * dimension. The benefit to this technique is that the calculated member is
   * readily apparent and easily accessible in client applications that do not
   * support member properties.
   */
   ...
   * 
   * <p>
   * Drilling down is an essential ability for most OLAP products, and Analysis
   * Services is no exception. Several functions exist that support drilling up
   * and down the hierarchy of dimensions within a cube. Typically, drilling up
   * and down the hierarchy is done one level at a time; think of this
   * functionality as a zoom feature for OLAP data.
   * 
   * <p>
   * There are times, though, when the need to drill down more than one level at
   * the same time, or even skip levels when displaying information about
   * multiple levels, exists for a business scenario.
   * 
   * <p>
   * For example, you would like to show report results from a query of the
   * Sales cube in the FoodMart 2000 sample database showing sales totals for
   * individual cities and the subtotals for each country, as shown in the
   * following table.
   * 
   * <p>
   * The Customers dimension, however, has Country, State Province, and City
   * levels. In order to show the above report, you would have to show the
   * Country level and then drill down two levels to show the City level,
   * skipping the State Province level entirely.
   * 
   * <p>
   * However, the MDX ToggleDrillState and DrillDownMember functions provide
   * drill down functionality only one level below a specified set. To drill
   * down more than one level below a specified set, you need to use a
   * combination of MDX functions, including Descendants, Generate, and Except.
   * This technique essentially constructs a large set that includes all levels
   * between both upper and lower desired levels, then uses a smaller set
   * representing the undesired level or levels to remove the appropriate
   * members from the larger set.
   * 
   * <p>
   * The MDX Descendants function is used to construct a set consisting of the
   * descendants of each member in the Customers dimension. The descendants are
   * determined using the MDX Descendants function, with the descendants of the
   * City level and the level above, the State Province level, for each member
   * of the Customers dimension being added to the set.
   * 
   * <p>
   * The MDX Generate function now creates a set consisting of all members at
   * the Country level as well as the members of the set generated by the MDX
   * Descendants function. Then, the MDX Except function is used to exclude all
   * members at the State Province level, so the returned set contains members
   * at the Country and City levels.
   * 
   * <p>
   * Note, however, that the previous MDX query will still order the members
   * according to their hierarchy. Although the returned set contains members at
   * the Country and City levels, the Country, State Province, and City levels
   ...
   * 
   * <p>
   * This type of MDX query is common when only the facts for the lowest level
   * of a dimension within a cube are needed, but information about other levels
   * within the same dimension may also be required to satisfy a specific
   * business scenario.
   * 
   * <p>
   * For example, a report that shows the unit sales for the store with the
   * highest unit sales from each country is needed for marketing purposes. The
   * following table provides an example of this report, run against the Sales
   * cube in the FoodMart 2000 sample database.
   * 
   * <p>
   * This looks simple enough, but the Country Name column provides unexpected
   * difficulty. The values for the Store Country column are taken from the
   * Store Country level of the Store dimension, so the Store Country column is
   * constructed as a calculated member as part of the MDX query, using the MDX
   * Ancestor and Name functions to return the country names for each store.
   * 
   * <p>
   * A combination of the MDX Generate, TopCount, and Descendants functions are
   * used to create a set containing the top stores in unit sales for each
   * country.
   */
  ...
  /**
   * <p>
   * The MDX Descendants function is used to construct a set consisting of only
   * those members at the Store Name level in the Store dimension. Then, the MDX
   * TopCount function is used to return only the topmost store based on the
   * Unit Sales measure. The MDX Generate function then constructs a set based
   * on the topmost stores, following the hierarchy of the Store dimension.
   * 
   * <p>
   * Alternate techniques, such as using the MDX Crossjoin function, may not
   * provide the desired results because non-related joins can occur. Since the
   * Store Country and Store Name levels are within the same dimension, they
   * cannot be cross-joined. Another dimension that provides the same regional
   * hierarchy structure, such as the Customers dimension, can be employed with
   * the Crossjoin function. But, using this technique can cause non-related
   * joins and return unexpected results.
   * 
   * <p>
   * For example, the following MDX query uses the Crossjoin function to attempt
   * to return the same desired results.
   * 
   * <p>
   * However, some unexpected surprises occur because the topmost member in the
   * Store dimension is cross-joined with all of the children of the Customers
   * dimension, as shown in the following table.
   * 
   * <p>
   * In this instance, the use of a calculated member to provide store country
   * names is easier to understand and debug than attempting to cross-join
   ...
   * 
   * <p>
   * One of the issues commonly encountered in business scenarios is the need to
   * rank the members of a dimension according to their corresponding measure
   * values. The Order MDX function allows you to order a set based on a string
   * or numeric expression evaluated against the members of a set. Combined with
   * other MDX functions, the Order function can support several different types
   * of ranking.
   * 
   * <p>
   * For example, the Sales cube in the FoodMart 2000 database can be used to
   * show unit sales for each store. However, the business scenario requires a
   * report that ranks the stores from highest to lowest unit sales,
   * individually, of nonconsumable products.
   * 
   * <p>
   ...
   * hierarchy, based on the topmost level represented in the set to be sorted,
   * or, by breaking the hierarchy, sorting all of the members of the set as if
   * they existed on the same level, with the same parent.
   * 
   * <p>
   * The following MDX query illustrates the use of the Order function to rank
   * the members according to unit sales.
   */
   ...
   * 
   * <p>
   * This type of MDX query frequently occurs when different aggregations are
   * needed at different levels in a dimension. One easy way to support such
   * functionality is through the use of a calculated measure, created as part
   * of the query, which uses the MDX Descendants function in conjunction with
   * one of the MDX aggregation functions to provide results.
   * 
   * <p>
   * For example, the Warehouse cube in the FoodMart 2000 database supplies the
   * [Units Ordered] measure, aggregated through the Sum function. But, you
   * would also like to see the average number of units ordered per store. The
   * following table demonstrates the desired results.
   * 
   * <p>
   * By using the following MDX query, the desired results can be achieved. The
   * calculated measure, [Average Units Ordered], supplies the average number of
   * ordered units per store by using the Avg, CurrentMember, and Descendants
   * MDX functions.
   */
  ...
  /**
   * <p>
   * This calculated measure is more powerful than it seems; if, for example,
   * you then want to see the average number of units ordered for beer products
   * in all of the stores in the California area, the following MDX query can be
   ...
   * dimensions. However, there are times where a different aggregation function
   * may be needed to represent a measure for reporting purposes. Two basic
   * cases involve aggregating a single dimension using a different aggregation
   * function than the one used for other dimensions.
   * <ul>
   * 
   ...
   * measures must be taken over the set of averaging time periods, constructed
   * through the use of the Descendants MDX function. Minimum and maximum values
   * are more easily calculated through the use of the Min and Max MDX
   * functions, also combined with the Descendants function.
   * 
   * <p>
   * For example, the Warehouse cube in the FoodMart 2000 database contains
   * information on ordered and shipped inventory; from it, a report is
   * requested to show the average number of units shipped, by product, to each
   * store. Information on units shipped is added on a monthly basis, so the
   * aggregated measure [Units Shipped] is divided by the count of descendants,
   * at the Month level, of the current member in the Time dimension. This
   * calculation provides a measure representing the average number of units
   * shipped per month, as demonstrated in the following MDX query.
   */
   ...
   * For example, the Warehouse cube in the FoodMart 2000 database contains
   * information on ordered and shipped inventory; from it, a report is
   * requested to show on-hand inventory at the end of every month. Because the
   * inventory on hand should equal ordered inventory minus shipped inventory,
   * the ClosingPeriod MDX function can be used to create a calculated measure
   * to supply the value of inventory on hand, as demonstrated in the following
   * MDX query.
   */
   ...
   * 
   * <p>
   * Date ranges are a frequently encountered problem. Business questions use
   * ranges of dates, but OLAP objects provide aggregated information in date
   * levels.
   * 
   * <p>
   * Using the technique described here, you can establish date ranges in MDX
   * queries at the level of granularity provided by a time dimension. Date
   * ranges cannot be established below the granularity of the dimension without
   * additional information. For example, if the lowest level of a time
   * dimension represents months, you will not be able to establish a two-week
   * date range without other information. Member properties can be added to
   * supply specific dates for members; using such member properties, you can
   * take advantage of the date and time functions provided by VBA and Excel
   * external function libraries to establish date ranges.
   * 
   * <p>
   * The easiest way to specify a static date range is by using the colon (:)
   * operator. This operator creates a naturally ordered set, using the members
   * specified on either side of the operator as the endpoints for the ordered
   * set. For example, to specify the first six months of 1998 from the Time
   * dimension in FoodMart 2000, the MDX syntax would resemble:
   * 
   * <blockquote>
   * 
   * <pre>
   ...
   * 
   * <p>
   * There are several techniques that can be used in MDX to support rolling
   * date ranges. All of these techniques tend to fall into two groups. The
   * first group involves the use of relative hierarchical functions to
   * construct named sets or calculated members, and the second group involves
   * the use of absolute date functions from external function libraries to
   * construct named sets or calculated members. Both groups are applicable in
   * different business scenarios.
   * 
   * <p>
   * In the first group of techniques, typically a named set is constructed
   * which contains a number of periods from a time dimension. For example, the
   * following table illustrates a 12-month rolling period, in which the figures
   * for unit sales of the previous 12 months are shown.
   * 
   * <p>
   ...
   * dimensions that share data with the time dimension at the Month level. The
   * example assumes that there is at least one measure, such as [Unit Sales],
   * with a value greater than zero in the current period. The Filter function
   * creates a set of months with unit sales greater than zero, while the Tail
   * function returns the last month in this set, the current month. The
   * LastPeriods function, finally, is then used to retrieve the last 12 periods
   * at this level, including the current period.
   */
  public void _testRolling() {
    assertQueryReturns("WITH SET Rolling12 AS\n"
   ...
   * 
   * <p>
   * A few techniques can be used, depending on the structure of the cube being
   * queried, to support different calculations for members depending on the
   * time period. The following example includes the MDX IIf function, and is
   * easy to use but difficult to maintain. This example works well for ad hoc
   * queries, but is not the ideal technique for client applications in a
   ...
   * database, for drink products. The standard forecast is double the warehouse
   * sales of the previous year, while the dynamic forecast varies from month to
   * month -- the forecast for January is 120 percent of previous sales, while
   * the forecast for July is 260 percent of previous sales.
   * 
   * <p>
   * The most flexible way of handling this type of report is the use of nested
   * MDX IIf functions to return a multiplier to be used on the members of the
   * Products dimension, at the Drinks level. The following MDX query
  ...
  /**
   * <p>
   * Other techniques, such as the addition of member properties to the Time or
   * Product dimensions to support such calculations, are not as flexible but
   * are much more efficient. The primary drawback to using such techniques is
   * that the calculations are not easily altered for speculative analysis
   * purposes. For client applications, however, where the calculations are
   * static or slowly changing, using a member property is an excellent way of
   * supplying such functionality to clients while keeping maintenance of
   * calculation variables at the server level. The same MDX query, for example,
   * could be rewritten to use a member property named [Dynamic Forecast
   * Multiplier] as shown in the following MDX query.
   */
   ...
   * 
   * <p>
   * For example, the following table represents a comparison of YTD unit sales
   * between 1997 and 1998, run against the Sales cube in the FoodMart 2000
   * database.
   * 
   * <p>
   * The following MDX query uses three calculated members to illustrate how to
   * use the YTD and ParallelPeriod functions in combination to compare time
   * periods.
   */
  ...
  private void runParallelQueries(final int threadCount, final int iterationCount,
    ...
    TestCaseForker threaded = new TestCaseForker(this, timeoutMs, threadCount,
      new ChooseRunnable() {
        public void run(int i) {
          for (int j = 0; j < iterationCount; j++) {
            ...
            try {
              ...
              e.printStackTrace();
              throw Util.newInternal(e, "Thread #" + i
                + " failed while executing query #" + queryIndex);
            }
          }
        }
      });
    threaded.run();
    assertEquals("number of executions", threadCount * iterationCount,
      executeCount[0]);
  }
   ...
   * Products])
     *
     * </code> depends on the current member of the Product dimension,
   * although [Product].[All Products] is referenced from the expression.
   */
  ...
  public void testFilterWithCrossJoin() throws Exception {
    String queryWithFilter = "WITH SET [#DataSet#] AS 'Filter(Crossjoin({[Store].[All Stores]}, {[Customers].[All Customers]}), "
    ...
    String queryWithoutFilter = "WITH SET [#DataSet#] AS 'Crossjoin({[Store].[All Stores]}, {[Customers].[All Customers]})' "
    ...
    // returned
    assertQueryReturns(queryWithoutFilter, expectedResultWithoutFilter);
    assertQueryReturns(queryWithFilter, expectedResultWithFilter);
  }

  /**
   * This resulted in {@link OutOfMemoryError} when the BatchingCellReader did
   * not know the values for the tuples that were used in filters.
   */
  ...
  /**
   * Tests a query with a CrossJoin so large that we run out of memory unless we
   * can push down evaluation to SQL.
   */
  ...
  /**
   * NonEmptyCrossJoin() is not the same as NON EMPTY CrossJoin() because it's
   * evaluated independently of the other axes. (see
  ...
  /**
   * description of this testcase: A calculated member is created on the
   * time.month level. On Hierarchize this member is compared to a month. The
   * month has a numeric key, while the calculated members key type is string.
  ...
  /**
   * the following query raised a classcast exception because an empty property
   * evaluated as "NullMember" note: Store "HQ" does not have a "Store Manager"
   */
  ...
  /**
   * This test modifies the Sales cube to contain both the regular usage of the
   * [Store] shared dimension, and another usage called [Other Store] which is
   * connected to the [Unit Sales] column
   */
  public void _testCubeWhichUsesSameSharedDimTwice() {
    ...
    assertEquals("select `store`.`store_country` as `Store Country`,"
      ...
      + " `sales_fact_1997`.`unit_sales` as `Unit Sales` " + "from `store` "
      + tableQualifier + "`store`," + " `sales_fact_1997` " + tableQualifier
      + "`sales_fact_1997`," + " `time_by_day` " + tableQualifier + "`time_by_day`,"
      ...
      + " and `store_1`.`store_country` = 'USA'", sql);
  }
  ...
  public void testMemberVisibility() {
    ...
    SchemaReader scr = testContext.getConnection().getSchema().lookupCube(cubeName,
      true).getSchemaReader(null);
    Member member = scr.getMemberByUniqueName(Id.Segment.toList("Measures",
      "Unit Sales"), true);
    Object visible = member.getPropertyValue(Property.VISIBLE.name);
    assertEquals(Boolean.FALSE, visible);

    member = scr.getMemberByUniqueName(Id.Segment.toList("Measures", "Store Cost"),
      true);
    visible = member.getPropertyValue(Property.VISIBLE.name);
    assertEquals(Boolean.TRUE, visible);

    member = scr
      .getMemberByUniqueName(Id.Segment.toList("Measures", "Profit"), true);
    visible = member.getPropertyValue(Property.VISIBLE.name);
    assertEquals(Boolean.FALSE, visible);
  }
  ...
  public void testDimWithoutAll() {
    ...
    final String schema = TestContext
      .instance()
      .getSchema(
        null,
        "<Cube name=\"Sales_DimWithoutAll\">\n"
          ...
          + "      formatString=\"#,###.00\"/>\n" + "</Cube>", null, null, null,
        null);
    TestContext testContext = TestContext.instance().withSchema(schema).withCube(
      "Sales_DimWithoutAll");
    ...
    testContext.assertExprReturns("([Gender].[F], [Measures].[Unit Sales])",
      "12,202");
    // evaluated at in the context: [Product].[Drink], [Gender].[M]
    testContext.assertExprReturns("([Gender].[M], [Measures].[Unit Sales])",
      "12,395");
    // evaluated in the context:
    // [Product].[Food].[Canned Foods], [Gender].[F]
    testContext.assertExprReturns(
      "([Product].[Food].[Canned Foods], [Measures].[Unit Sales])", "9,407");
    testContext.assertExprReturns(
  ...
  /**
   * Tests whether the agg mgr behaves correctly if a cell request causes a
   * column to be constrained multiple times. This happens if two levels map to
   * the same column via the same join-path. If the constraints are
  ...
  public void testBadMeasure1() {
    ...
    TestContext.checkThrowable(throwable,
  ...
  public void testInvalidMembersInQuery() {
    ...
    assertQueryReturns(mdx3, "Axis #0:\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Row #0: \n");
    ...
    assertQueryReturns(mdx2, "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
  ...
  public void testFormatInheritance() {
    assertQueryReturns("with member measures.foo as 'measures.bar' "
   ...
   * format. In this particular case foo will use profit's format, i.e neither
   * [unit sales] nor [customer count] format is used.
   */
  public void testFormatInheritanceWorksWithFirstFormatItFinds() {
    assertQueryReturns(
      "with member measures.foo as 'measures.bar' "
        ...
        + "from sales where measures.foo", "Axis #0:\n" + "{[Measures].[foo]}\n"
  ...
  /**
   * Test format string values. Previously, a bug meant that string values were
   * printed as is, never passed through the format string.
   */
  ...
  /**
   * Test format inheritance to pickup format from second measure when the first
   * does not have one.
   */
  ...
  /**
   * Tests format inheritance with complex expression to assert that the format
   * of the first member that has a valid format is used.
   */
  ...
  public void testQueryIterationLimit() {
    ...
    String queryString = "With Set [*NATIVE_CJ_SET] as "
    ...
    TestContext.checkThrowable(throwable,
      "Number of iterations exceeded limit of 11");
  ...
  public void testDefaultMeasureInCubeForIncorrectMeasureName() {
    ...
    assertQueriesReturnSimilarResults(queryWithoutFilter, queryWithFirstMeasure,
      testContext);
  }
  ...
  /**
   * This tests for bug #1706434, the ability to convert numeric types to
   * logical (boolean) types.
   */
  public void testNumericToLogicalConversion() {
    assertQueryReturns("select " + "{[Measures].[Unit Sales]} on columns, "
      + "Filter(Descendants(" + "[Product].[Food].[Baked Goods].[Bread]), "
      + "Count([Product].currentMember.children)) on Rows " + "from [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
        + "Axis #2:\n" + "{[Product].[Food].[Baked Goods].[Bread]}\n"
   ...
   * under some circumstances. This exceeded the limit for Oracle resulting in
   * an ORA-01795 error.
   */
  public void testBug1630754() {
    ...
    Result result = testContext.executeQuery("WITH SET [#DataSet#] AS "
  ...
  /**
   * Tests a query which uses filter and crossjoin. This query caused problems
   * when the retrowoven version of mondrian was used in jdk1.5, specifically a
   * {@link ClassCastException} trying to cast a {@link List} to a
   * {@link Iterable}.
   */
  ...
  public void testDuplicateAxisFails() {
    assertQueryThrows("select [Gender].Members on columns,"
      + " [Measures].Members on columns " + "from [Sales]",
      "Duplicate axis name 'COLUMNS'.");
  }

  public void testInvalidAxisFails() {
    assertQueryThrows("select [Gender].Members on 0," + " [Measures].Members on 10 "
      + "from [Sales]",
   ...
   * inspired by forum post <a
   * href="http://forums.pentaho.com/showthread.php?p=177135">summing
   * properties</a>.
   */
  public void testSummingProperties() {
    ...
    assertQueryReturns("with member [Measures].[Sum Sqft] as '" + "sum("
  ...
  public void testIifWithTupleFirstAndMemberNextWithoutMeasure() {
    assertQueryReturns("WITH\n" + "MEMBER [Gender].agg " + "AS 'IIF(1=1, "
      ...
      + "([Gender].[All Gender]))', " + "SOLVE_ORDER = 4 "
      + "SELECT {[Measures].[unit sales]} ON 0, "
      + "{([Gender].agg)} on 1 FROM sales", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n" + "{[Gender].[agg]}\n"
      + "Row #0: 266,773\n");
  }
  ...
  /**
   * Testcase for Pentaho bug <a
   * href="http://jira.pentaho.com/browse/BISERVER-1323">BISERVER-1323</a>,
  ...
  public void testEmptySqlBug() {
    ...
    assertQueryReturns("select {[Measures].[Unit Sales]} ON COLUMNS, "
      ...
      + " * [Customers].[All Customers] ON ROWS " + "from [Sales]", expectedResult);
    // combining tuple
    assertQueryReturns("select {[Measures].[Unit Sales]} ON COLUMNS, "
      + "NON EMPTY [Store].[All Stores] " + " * {([Product].[All Products],"
      + "     [Customers].[All Customers])} ON ROWS " + "from [Sales]",
      expectedResult);
    // combining two members with tuple
    final String expectedResult4 = "Axis #0:\n"
      + "{}\n"
      + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n"
      + "Axis #2:\n"
  ...
  public void testHeterogeneousAxis() {
    ...
    // dimensionality.
    assertQueryThrows("select {[Measures].[Unit Sales], [Gender].Members} on 0,\n"
      + " [Store].[USA].Children on 1\n" + "from [Sales]",
      "All arguments to function '{}' must have same hierarchy.");
    assertQueryThrows("select {[Marital Status].Members, [Gender].Members} on 0,\n"
      + " [Store].[USA].Children on 1\n" + "from [Sales]",
      "All arguments to function '{}' must have same hierarchy.");
  }
  ...
  public void testHierarchiesOfSameDimensionOnDifferentAxes() {
    ...
    assertQueryReturns("select [Time].[Year].Members on columns,\n"
  ...
  /**
   * A simple user-defined function which adds one to its argument, but sleeps 1
   * ms before doing so.
   */
  ...
  /**
   * This unit test would cause connection leaks without a fix for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-571">MONDRIAN-571,
   * "HighCardSqlTupleReader does not close SQL Connections"</a>. It would be
   * better if there was a way to verify that no leaks occurred in the data
   * source.
   */
  public void testHighCardSqlTupleReaderLeakingConnections() {
    assertQueryReturns(
      "WITH MEMBER [Measures].[NegativeSales] AS '- [Measures].[Store Sales]' "
        ...
        + "[Store].[Store Country].members ON ROWS " + "from [Sales] "
        + "where [Time].[1997]", "Axis #0:\n" + "{[Time].[1997]}\n" + "Axis #1:\n"
  ...
  public void testZeroValuesAreNotTreatedAsNull() {
    String mdx = "select"
      ...
      + "  on 0," + "  [Measures].[units shipped] on 1" + " from warehouse";
    assertQueryReturns(mdx, "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testDirectMemberReferenceOnDimensionWithCalculationsDefined() {
    ...
    testContext.assertQueryReturns("select {[Gender].[M]} on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Gender].[M]}\n"
        + "Row #0: 135,215\n");
  }
  ...
  public void testExplainComplex() throws SQLException {
    ...
    new RectangularCellSetFormatter(true).format(cellSet, new PrintWriter(
      new StringWriter()));
    cellSet.close();
    final String actual = strings[0].replaceAll("callMillis=[0-9]+",
      "callMillis=nnn").replaceAll("[0-9]+ms", "nnnms");
    TestContext
      .assertEqualsVerbose(
        "Axis (FILTER):\n"
    ...
    assertTrue(strings[1], strings[1]
      .contains("SqlStatement-SqlTupleReader.readTuples [[Product].[Product "
  ...
  public void testExplainInvalid() throws SQLException {
    ...
    try {
      ...
      TestContext.checkThrowable(e,
        "MDX object '[Measures].[Store Margin]' not found in cube 'Sales'");
    }
  }

  /**
   * This is a test for MONDRIAN-1014. Executing a statement twice concurrently
   * would fail because the statement wasn't cleaning up properly its execution
   * context.
   */
  public void testConcurrentStatementRun() throws Exception {
    final OlapConnection olapConnection = TestContext.instance()
      .getOlap4jConnection();
    ...
    es.submit(new Callable<CellSet>() {
      public CellSet call() throws Exception {
        return stmt
          .executeOlapQuery("select {Crossjoin([Store].Members, [Customers].Members)} on columns from [Sales]");
      }
    });
