/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/CVConcurrentMdxTest.java#6 $
...
/**
 * A copy of {@link ConcurrentMdxTest} with modifications to take as input
 * ref.xml files. This does not fully use {@link DiffRepository} and does not
 * generate log files. This Class is not added to the Main test suite. Purpose
 * of this test is to simulate Concurrent access to Aggregation and data load.
 * Simulation will be more effective if we run this single test again and again
 * with a fresh connection.
 * 
 * @author Khanh Vu
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/CVConcurrentMdxTest.java
 *          #6 $
 */
public class CVConcurrentMdxTest extends FoodMartTestCase {
  ...
  public void testConcurrentQueriesInRandomOrder() {
    ...
    assertTrue(ConcurrentValidatingQueryRunner.runTest(3, 100, true, true, true,
      queryList).size() == 0);
  }
  ...
  public void testConcurrentCVQueriesInRandomOrder() {
    ...
    assertEquals(Collections.<Throwable> emptyList(),
      ConcurrentValidatingQueryRunner.runTest(3, 100, true, true, true, queryList));
  }
  ...
  /**
   * Generates an array of QueryAndResult objects from the list of test classes
   * 
   * @param testList
