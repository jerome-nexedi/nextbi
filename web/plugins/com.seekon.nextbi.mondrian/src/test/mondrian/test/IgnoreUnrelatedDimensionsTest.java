/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/IgnoreUnrelatedDimensionsTest.java#11 $
...
/**
 * Test case to push unrelatedDimensions to top level when
 * ignoreUnrelatedDimensions property is set to true on a base cube usage.
 * 
 * @author ajoglekar
 * @since Dec 03, 2007
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/IgnoreUnrelatedDimensionsTest
 *          .java#11 $
 */
public class IgnoreUnrelatedDimensionsTest extends FoodMartTestCase {
  ...
  private static final String cubeSales3 = "<Cube name=\"Sales 3\">\n"
  ...
  private static final String cubeWarehouseAndSales3 = "<VirtualCube name=\"Warehouse and Sales 3\" defaultMeasure=\"Store Invoice\">\n"
    + "  <CubeUsages>\n"
  ...
  public void testTotalingOnCrossJoinOfJoiningAndNonJoiningDimensions() {
    assertQueryReturns("WITH MEMBER [Measures].[Unit Sales VM] AS "
      + "'ValidMeasure([Measures].[Unit Sales])', SOLVE_ORDER = 3000 "
      + "MEMBER Gender.G AS 'AGGREGATE(CROSSJOIN({[Gender].[Gender].MEMBERS},"
      + "[WAREHOUSE].[STATE PROVINCE].MEMBERS))'" + "SELECT "
      + "{[MEASURES].[Unit Sales VM]} ON 0," + "{Gender.G} ON 1 "
  ...
  public void testVMShouldNotPushUpAggMemberDefinedOnNonJoiningDimension() {
    assertQueryReturns("WITH MEMBER [Measures].[Total Sales] AS "
      ...
      + "SOLVE_ORDER = 3000 " + "MEMBER Gender.G AS "
      + "'AGGREGATE(CROSSJOIN({GENDER.[M]},{[Product].[All Products].[Drink]}))',"
      + "SOLVE_ORDER = 4 " + "SELECT " + "{[MEASURES].[Total Sales]} ON 0,"
      + "{Gender.G} ON 1 FROM [WAREHOUSE AND SALES2]", "Axis #0:\n" + "{}\n"
  ...
  public void testAggMemberDefinedOnNonJoiningDimensionWithNonAllDefltMember() {
    ...
    final TestContext context = TestContext.instance().create(null, cubeSales3,
      cubeWarehouseAndSales3, null, null, null);
    context.assertQueryReturns("WITH MEMBER [Measures].[Total Sales] AS "
      + "'ValidMeasure(Measures.[Warehouse Sales]) + [Measures].[Unit Sales]',"
      + "SOLVE_ORDER = 3000 " + "MEMBER Gender.G AS "
      + "'AGGREGATE(CROSSJOIN({GENDER.[M]},{[Product].[All Products].[Drink]}))',"
      + "SOLVE_ORDER = 4 " + "SELECT " + "{[MEASURES].[Total Sales]} ON 0,"
      + "{Gender.G} ON 1 FROM [WAREHOUSE AND SALES 3]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Total Sales]}\n" + "Axis #2:\n"
      + "{[Gender].[G]}\n" + "Row #0: 30,405.602\n");
  }
  ...
  public void testTotalingForValidAndNonValidMeasuresWithJoiningDimensions() {
    assertQueryReturns("WITH MEMBER [Measures].[Unit Sales VM] AS "
      ...
      + "[STORE].[STORE NAME].MEMBERS))'" + "SELECT "
      + "{[MEASURES].[Unit Sales VM], [MEASURES].[STORE COST]} ON 0,"
      + "{PRODUCT.G} ON 1 FROM [WAREHOUSE AND SALES2]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales VM]}\n"
      + "{[Measures].[Store Cost]}\n" + "Axis #2:\n" + "{[Product].[G]}\n"
      + "Row #0: 266,773\n" + "Row #0: 225,627.23\n");
  }
  ...
  public void testTotalingWhenIgnoreUnrelatedDimensionsPropertyIsTrue() {
    assertQueryReturns(
      "WITH MEMBER [Measures].[Unit Sales VM] AS "
        ...
        + "{[Warehouse].[State Province].&[DF].[Mexico City].[Freeman And Co], "
        + "[Warehouse].[State Province].&[BC].[Vancouver].[Bellmont Distributing]})' "
        + "SELECT " + "{[Measures].[Unit Sales VM]} ON AXIS(0), "
