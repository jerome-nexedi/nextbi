 ...
 * @since Apr 19, 2004
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/RaggedHierarchyTest.java
 *          #22 $
 */
public class RaggedHierarchyTest extends FoodMartTestCase {
  private void assertRaggedReturns(String expression, String expected) {
    getTestContext().withCube("[Sales Ragged]").assertAxisReturns(expression,
      expected);
  }
  ...
  public void testChildrenOfRoot() {
    assertRaggedReturns("[Store].children", "[Store].[Canada]\n"
      + "[Store].[Israel]\n" + "[Store].[Mexico]\n" + "[Store].[USA]\n"
      + "[Store].[Vatican]");
  }

  public void testChildrenOfUSA() {
    assertRaggedReturns("[Store].[USA].children", "[Store].[USA].[CA]\n"
  ...
  public void testParentOfHaifa() {
    assertRaggedReturns("[Store].[Israel].[Haifa].Parent", "[Store].[Israel]");
  }

  public void testParentOfVatican() {
    assertRaggedReturns("[Store].[Vatican].Parent", "[Store].[All Stores]");
  }
  ...
  public void testNextMemberOfBC() {
    ...
    assertRaggedReturns("[Store].[Canada].[BC].NextMember", "[Store].[Mexico].[DF]");
  }

  public void testLead() {
    assertRaggedReturns("[Store].[Mexico].[DF].Lead(1)",
      "[Store].[Mexico].[Guerrero]");
    assertRaggedReturns("[Store].[Mexico].[DF].Lead(0)", "[Store].[Mexico].[DF]");
    // Israel is immediately before Mexico, but is hidden
    assertRaggedReturns("[Store].[Mexico].[DF].Lead(-1)", "[Store].[Canada].[BC]");
    assertRaggedReturns("[Store].[Mexico].[DF].Lag(1)", "[Store].[Canada].[BC]");
    // Fall off the edge of the world
    assertRaggedReturns("[Store].[Mexico].[DF].Lead(-2)", "");
    assertRaggedReturns("[Store].[Mexico].[DF].Lead(-543)", "");
  }

  // disabled: (1) does not work with SmartMemberReader and (2) test returns
  // [null] member
  public void dont_testDescendantsOfVatican() {
    assertRaggedReturns("Descendants([Store].[Vatican])", "[Store].[Vatican]\n"
      + "[Store].[Vatican].[Vatican].[null].[Store 17]");
  }
  ...
  public void testAncestorOfHaifa() {
    assertRaggedReturns("Ancestor([Store].[Israel].[Haifa], [Store].[Store State])",
      "");
  }
   ...
   * 
   * <p>
   * The Vatican is the tricky case, because one of the columns is null, so the
   * SQL generator might get confused.
   */
  // disabled: (1) does not work with SmartMemberReader and (2) test returns
  // [null] member
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-628">MONDRIAN-628</a>,
   * "ClassCastException in Mondrian for query using Sales Ragged cube".
   * 
   * <p>
   * Cause was that ancestor yielded a null member, which was a RolapMember but
   * Order required it to be a RolapCubeMember.
   */
  ...
  public void testHideIfBlankHidesWhitespace() {
    if (TestContext.instance().getDialect().getDatabaseProduct() != Dialect.DatabaseProduct.ORACLE) {
      return;
    }
    final TestContext testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "<Dimension name=\"Gender4\" foreignKey=\"customer_id\">\n"
          ...
          + "         <NameExpression> " + " <SQL dialect='generic'> "
          + "case \"gender\" " + "when 'F' then ' ' " + "when 'M' then 'M' "
          + " end " + "</SQL> " + "</NameExpression>  " + "      </Level>"
          + "    </Hierarchy>\n" + "  </Dimension>");
    testContext.assertQueryReturns(" select {[Gender4].[Gender].members} "
