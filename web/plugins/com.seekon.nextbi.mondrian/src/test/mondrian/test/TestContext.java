/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/TestContext.java#95 $
 ...
 * necessary to run mondrian tests (otherwise we'd have to pass this information
 * into the constructor of TestCases).
 * 
 * <p>
 * The singleton instance (retrieved via the {@link #instance()} method)
 * contains a connection to the FoodMart database, and runs expressions in the
 * context of the <code>Sales</code> cube.
 * 
 * <p>
 * Using the {@link DelegatingTestContext} subclass, you can create derived
 * classes which use a different connection or a different cube.
 * 
 * @author jhyde
 * @since 29 March, 2002
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/TestContext.java#95
 *          $
 */
public class TestContext {
  ...
  private static final String[] AllHiers = {
    ...
    "[Store Type]",
    "[Time]",
    MondrianProperties.instance().SsasCompatibleNaming.get() ? "[Time].[Weekly]"
      : "[Time.Weekly]", "[Product]", "[Promotion Media]", "[Promotions]",
  ...
  /**
   * Returns the connect string by which the unit tests can talk to the FoodMart
   * database.
   * 
   * <p>
   * In the base class, the result is the same as the static method
   * {@link #getDefaultConnectString}. If a derived class overrides
   * {@link #getConnectionProperties()}, the result of this method will change
   * also.
   */
   ...
   * <ul>
   * <li>Starts with {@link MondrianProperties#TestConnectString}, if it is set.
   * </li>
   * <li>If {@link MondrianProperties#FoodmartJdbcURL} is set, this overrides
   * the <code>Jdbc</code> property.</li>
   * <li>If the <code>catalog</code> URL is unset or invalid, it assumes that we
   * are at the root of the source tree, and references
   ...
   * 
   * <p>
   * When invoked on the default TestContext instance, returns a connection to
   * the FoodMart database.
   */
  public synchronized Connection getConnection() {
    if (foodMartConnection == null) {
      foodMartConnection = DriverManager.getConnection(getConnectionProperties(),
        null, null);
    }
  ...
  /**
   * Returns a connection to the FoodMart database with a dynamic schema
   * processor and disables use of RolapSchema Pool.
   */
  public TestContext withSchemaProcessor(
    ...
    properties.put(RolapConnectionProperties.DynamicSchemaProcessor.name(),
  ...
  public TestContext withSchemaPool(boolean usePool) {
    final Util.PropertyList properties = getConnectionProperties().clone();
    properties.put(RolapConnectionProperties.UseSchemaPool.name(), Boolean
      .toString(usePool));
    return withProperties(properties);
  }

  public Util.PropertyList getConnectionProperties() {
    final Util.PropertyList propertyList = Util
      .parseConnectString(getDefaultConnectString());
    if (MondrianProperties.instance().TestHighCardinalityDimensionList.get() != null
      && propertyList.get(RolapConnectionProperties.DynamicSchemaProcessor.name()) == null) {
      propertyList.put(RolapConnectionProperties.DynamicSchemaProcessor.name(),
   ...
   * definitions.
   */
  public String getSchema(String parameterDefs, String cubeDefs,
    String virtualCubeDefs, String namedSetDefs, String udfDefs, String roleDefs) {
  ...
  /**
   * Returns the definition of the "FoodMart" schema as stored in {@code
   * FoodMart.xml}.
   * 
  ...
  public String getRawSchema() {
    final Connection connection = withSchemaProcessor(SnoopingSchemaProcessor.class)
      .getConnection();
    connection.close();
  ...
  /**
   * Returns a the XML of the foodmart schema, adding dimension definitions to
   * the definition of a given cube.
   */
  private String substituteSchema(String rawSchema, String cubeName,
    String dimensionDefs, String measureDefs, String memberDefs, String namedSetDefs) {
    String s = rawSchema;
  ...
  private void assertResultValid(Result result) {
    for (Cell cell : cellIter(result)) {
      ...
      Assert.assertFalse(value instanceof Number
   ...
   *          Query string
   * @param pattern
   *          Pattern which exception must match
   */
  ...
  public void assertExprThrows(String expression, String pattern) {
    ...
    try {
      ...
      Result result = executeQuery("with member [Measures].[Foo] as '" + expression
  ...
  private String generateExpression(String expression) {
    ...
    return "with member [Measures].[Foo] as " + Util.singleQuoteString(expression)
      + " select {[Measures].[Foo]} on columns from " + cubeName;
  }
  ...
  /**
   * Asserts that an expression, with a given set of parameter bindings, returns
   * a given result.
   * 
   * @param expr
   *          Scalar MDX expression
   * @param expected
   *          Expected result
   * @param paramValues
   *          Array of parameter names and values
   */
  public void assertParameterizedExprReturns(String expr, String expected,
    Object... paramValues) {
    Connection connection = getConnection();
  ...
  public String upgradeActual(String actual) {
    if (!MondrianProperties.instance().SsasCompatibleNaming.get()) {
      ...
      actual = Util
        .replace(actual, "[All Store.MyHierarchys]", "[All MyHierarchys]");
      actual = Util.replace(actual, "[Store2].[All Store2s]",
        "[Store2].[Store].[All Stores]");
      actual = Util.replace(actual,
      ...
      actual = Util.replace(actual, "<Employees>80000.0000</Employees>",
  ...
  /**
   * Massages an MDX query to handle differences in unique names betweeen old
   * and new behavior.
   * 
   * <p>
   * The main difference addressed is with level naming. The problem arises when
   * dimension, hierarchy and level have the same name:
   * <ul>
   * 
   * <li>In old behavior, the [Gender].[Gender] represents the Gender level, and
   * [Gender].[Gender].[Gender] is invalid.
   * 
   * <li>In new behavior, [Gender].[Gender] represents the Gender hierarchy, and
   * [Gender].[Gender].[Gender].members represents the Gender level.
   * </ul>
   * </p>
   * 
   * <p>
   * So, {@code upgradeQuery("[Gender]")} returns "[Gender].[Gender]" for old
   * behavior, "[Gender].[Gender].[Gender]" for new behavior.
   * </p>
   * 
   * @see mondrian.olap.MondrianProperties#SsasCompatibleNaming
   * 
  ...
  public String upgradeQuery(String queryString) {
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      String[] names = { "[Gender]", "[Education Level]", "[Marital Status]",
        "[Store Type]", "[Yearly Income]", };
      for (String name : names) {
        queryString = Util.replace(queryString, name + "." + name, name + "." + name
          + "." + name);
      }
      queryString = Util.replace(queryString, "[Time.Weekly].[All Time.Weeklys]",
  ...
  /**
   * Executes a set expression which is expected to return 0 or 1 members. It is
   * an error if the expression returns tuples (as opposed to members), or if it
   * returns two or more members.
   * 
   * @param expression
   *          Expression string
  ...
  public Member executeSingletonAxis(String expression) {
    ...
    Result result = executeQuery("select {" + expression + "} on columns from "
      + cubeName);
    ...
    case 1:
      ...
      return member;
    default:
      throw Util.newInternal("expression " + expression + " yielded "
  ...
  public void assertAxisThrows(String expression, String pattern) {
    ...
    try {
      final String cubeName = getDefaultCubeName();
      final String queryString = "select {" + expression + "} on columns from "
        + cubeName;
      Query query = connection.parseQuery(queryString);
  ...
  public void assertQueryReturns(String query, String desiredResult) {
    ...
    if (desiredResult != null) {
      assertEqualsVerbose(desiredResult, upgradeActual(resultString));
    }
  }
   ...
   * 
   * <p>
   * This forces the schema to be loaded and performs a basic sanity check. If
   * this is a negative schema test, causes schema validation errors to be
   * thrown.
   */
   ...
   * 
   * <p>
   * If they do not, throws a {@link junit.framework.ComparisonFailure} and
   * prints the difference, including the actual string as an easily pasted Java
   * string literal.
   */
   ...
   *          Actual string
   * @param java
   *          Whether to generate actual string as a Java string literal if the
   *          values are not equal
   * @param message
   *          Message to display, optional
   */
  public static void assertEqualsVerbose(String expected, String actual,
    boolean java, String message) {
   ...
   * 
   * @param safeExpected
   *          Expected string, where all line endings have been converted into
   ...
   *          Message to display, optional
   */
  public static void assertEqualsVerbose(SafeString safeExpected, String actual,
    boolean java, String message) {
  ...
  /**
   * Checks that an actual string matches an expected pattern. If they do not,
   * throws a {@link ComparisonFailure} and prints the difference, including the
   * actual string as an easily pasted Java string literal.
   */
  public void assertMatchesVerbose(Pattern expected, String actual) {
    ...
    String message = "Expected pattern:" + nl + expected + nl + "Actual: " + nl
      + actual + nl + "Actual java: " + nl + s + nl;
  ...
  public static String toString(CellSet cellSet) {
    final StringWriter sw = new StringWriter();
    new TraditionalCellSetFormatter().format(cellSet, new PrintWriter(sw));
    return sw.toString();
  }
   ...
   * If the input string already has platform-dependent line endings, no
   * replacements are made.
   * 
   * @param string
   *          String whose line endings are to be made platform- dependent.
   *          Typically these are constant "expected value" string expressions
   *          where the linefeed is represented as linefeed "\n", but sometimes
   *          this method will receive strings created dynamically where the
   *          line endings are already appropriate for the platform.
  ...
  public static Dialect getFakeDialect() {
    final DatabaseMetaData metaData = (DatabaseMetaData) Proxy.newProxyInstance(
      null, new Class<?>[] { DatabaseMetaData.class },
    ...
    final java.sql.Connection connection = (java.sql.Connection) Proxy
      .newProxyInstance(null, new Class<?>[] { java.sql.Connection.class },
        new DelegatingInvocationHandler() {
  ...
  /**
   * Checks that expected SQL equals actual SQL. Performs some normalization on
   * the actual SQL to compensate for differences between dialects.
   */
   ...
   * 
   * <p>
   * This is not intended to be a general purpose method: it looks for specific
   * patterns known to occur in tests, in particular "=as=" and
   * "fname + ' ' + lname".
   * 
   * @param sql
  ...
  private String dialectize(String sql) {
    ...
    case LUCIDDB:
    case TERADATA:
      sql = sql.replaceAll(search, "`fname` || ' ' || `lname`");
      break;
    case DERBY:
      sql = sql.replaceAll(search, "`customer`.`fullname`");
      break;
    case INGRES:
      sql = sql.replaceAll(search, "fullname");
      break;
    case DB2:
    case DB2_AS400:
    case DB2_OLD_AS400:
  ...
  private void checkSqlAgainstDatasource(String actualSql, int expectedRows) {
    ...
    try {
      String jdbcDrivers = connectProperties
        .get(RolapConnectionProperties.JdbcDrivers.name());
      if (jdbcDrivers != null) {
      ...
      jdbcConn = java.sql.DriverManager.getConnection(connectProperties
        .get(RolapConnectionProperties.Jdbc.name()), connectProperties
        .get(RolapConnectionProperties.JdbcUser.name()), connectProperties
      ...
      throw new RuntimeException("ERROR in SQL - invalid for database: "
  ...
  public void assertExprDependsOn(String expr, String hierList) {
    ...
    final String queryString = "WITH MEMBER [Measures].[Foo] AS "
      + Util.singleQuoteString(expr) + " SELECT FROM [Sales]";
    final Query query = connection.parseQuery(queryString);
    query.resolve();
  ...
  private void checkDependsOn(final Query query, final Exp expression,
    String expectedHierList, final boolean scalar) {
    final Calc calc = query.compileExpression(expression, scalar, scalar ? null
      : ResultStyle.ITERABLE);
    final List<RolapHierarchy> hierarchies = ((RolapCube) query.getCube())
      .getHierarchies();
    StringBuilder buf = new StringBuilder("{");
    int dependCount = 0;
  ...
  /**
   * Creates a TestContext which is based on a variant of the FoodMart schema,
   * which parameter, cube, named set, and user-defined function definitions
   * added.
   * 
   * @param parameterDefs
   *          Parameter definitions. If not null, the string is is inserted into
   *          the schema XML in the appropriate place for parameter definitions.
   * @param cubeDefs
   *          Cube definition(s). If not null, the string is is inserted into
   *          the schema XML in the appropriate place for cube definitions.
   * @param virtualCubeDefs
   *          Definitions of virtual cubes. If not null, the string is inserted
   *          into the schema XML in the appropriate place for virtual cube
   *          definitions.
   * @param namedSetDefs
   *          Definitions of named sets. If not null, the string is inserted
   *          into the schema XML in the appropriate place for named set
   *          definitions.
   * @param udfDefs
   *          Definitions of user-defined functions. If not null, the string is
   *          inserted into the schema XML in the appropriate place for UDF
   *          definitions.
   * @param roleDefs
   *          Definitions of roles
   * @return TestContext which reads from a slightly different hymnbook
   */
  public final TestContext create(final String parameterDefs, final String cubeDefs,
    final String virtualCubeDefs, final String namedSetDefs, final String udfDefs,
    final String roleDefs) {
    final String schema = getSchema(parameterDefs, cubeDefs, virtualCubeDefs,
      namedSetDefs, udfDefs, roleDefs);
  ...
  /**
   * Creates a TestContext which is like this one but uses the given connection
   * properties.
   * 
   * @param properties
   ...
   * 
   * @param cubeName
   *          Name of a cube in the schema (cube must exist)
   * @param dimensionDefs
   *          String defining dimensions, or null
   * @return TestContext with modified cube defn
   */
  public final TestContext createSubstitutingCube(final String cubeName,
  ...
  public final TestContext createSubstitutingCube(final String cubeName,
    ...
    return createSubstitutingCube(cubeName, dimensionDefs, null, memberDefs, null);
  }
   ...
   *          String defining dimensions, or null
   * @param measureDefs
   *          String defining measures, or null
   * @param memberDefs
   *          String defining calculated members, or null
   * @param namedSetDefs
   ...
   */
  public final TestContext createSubstitutingCube(final String cubeName,
    final String dimensionDefs, final String measureDefs, final String memberDefs,
    final String namedSetDefs) {
    final String schema = substituteSchema(getRawFoodMartSchema(), cubeName,
      dimensionDefs, measureDefs, memberDefs, namedSetDefs);
    return withSchema(schema);
  }
  ...
  /**
   * Generates a string containing all dimensions except those given. Useful as
   * an argument to {@link #assertExprDependsOn(String, String)}.
   * 
  ...
  /**
   * Creates a FoodMart connection with "Ignore=true" and returns the list of
   * warnings in the schema.
   * 
  ...
  public OlapConnection getOlap4jConnection() throws SQLException {
    ...
    final java.sql.Connection connection = java.sql.DriverManager
      .getConnection("jdbc:mondrian:" + connectString);
  ...
  public static String hierarchyName(String dimension, String hierarchy) {
    return MondrianProperties.instance().SsasCompatibleNaming.get() ? "["
      + dimension + "].[" + hierarchy + "]" : (hierarchy.equals(dimension) ? "["
  ...
  public static class SnoopingSchemaProcessor extends FilterDynamicSchemaProcessor {
    public static final ThreadLocal<String> THREAD_RESULT = new ThreadLocal<String>();

    protected String filter(String schemaUrl, Util.PropertyList connectInfo,
      InputStream stream) throws Exception {
      String catalogContent = super.filter(schemaUrl, connectInfo, stream);
  ...
  /**
   * Schema processor that flags dimensions as high-cardinality if they appear
   * in the list of values in the
   * {@link MondrianProperties#TestHighCardinalityDimensionList} property. It's
   * a convenient way to run the whole suite against high-cardinality dimensions
   * without modifying FoodMart.xml.
   */
  public static class HighCardDynamicSchemaProcessor extends
    FilterDynamicSchemaProcessor {
    protected String filter(String schemaUrl, Util.PropertyList connectInfo,
      InputStream stream) throws Exception {
      String s = super.filter(schemaUrl, connectInfo, stream);
      final String highCardDimensionList = MondrianProperties.instance().TestHighCardinalityDimensionList
        .get();
      if (highCardDimensionList != null && !highCardDimensionList.equals("")) {
        for (String dimension : highCardDimensionList.split(",")) {
          final String match = "<Dimension name=\"" + dimension + "\"";
