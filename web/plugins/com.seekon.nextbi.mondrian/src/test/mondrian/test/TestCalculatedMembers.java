/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/TestCalculatedMembers.java#70 $
...
/**
 * Tests the expressions used for calculated members. Please keep in sync with
 * the actual code used by the wizard.
 * 
 * @author jhyde
 * @since 5 October, 2002
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/TestCalculatedMembers
 *          .java#70 $
 */
public class TestCalculatedMembers extends BatchTestCase {
  ...
  public void testCalculatedMemberInCubeViaApi() {
    ...
    salesCube.createCalculatedMember("<CalculatedMember name='Profit2'"
      + "  dimension='Measures'"
    ...
    try {
      ...
      if (!msg.equals("Mondrian Error:Calculated member "
        + "'[Measures].[Measures].[Profit2]' " + "already exists in cube 'Sales'")) {
        throw e;
      }
    }
  }
  ...
  /**
   * Tests a calculated member with spaces in its name against a virtual cube
  ...
  public void testCalculatedMemberInCubeWithSpace() {
    TestContext testContext = TestContext.instance().createSubstitutingCube(
      "Warehouse and Sales",
      null,
      "<CalculatedMember name='Profit With Spaces'" + "  dimension='Measures'"
        + "  formula='[Measures].[Store Sales]-[Measures].[Store Cost]'/>")
  ...
  public void testCalculatedMemberInCubeWithProps() {
    ...
    salesCube.createCalculatedMember("<CalculatedMember name='Profit3'"
      + "  dimension='Measures'"
      + "  formula='[Measures].[Store Sales]-[Measures].[Store Cost]'>"
    ...
    try {
      salesCube.createCalculatedMember("<CalculatedMember name='Profit4'"
        + "  dimension='Measures'"
        + "  formula='[Measures].[Store Sales]-[Measures].[Store Cost]'>"
      ...
      final String msg = e.getMessage();
      if (!msg.equals("Mondrian Error:Member property must have a value or an "
    ...
    try {
      salesCube
        .createCalculatedMember("<CalculatedMember name='Profit4'"
          + "  dimension='Measures'"
          + "  formula='[Measures].[Store Sales]-[Measures].[Store Cost]'>"
      ...
      final String msg = e.getMessage();
      if (!msg
        .equals("Mondrian Error:Member property must not have both a value and "
    ...
    salesCube.createCalculatedMember("<CalculatedMember name='Profit4'"
      + "  dimension='Measures'"
      + "  formula='[Measures].[Store Sales]-[Measures].[Store Cost]+"
      + "     [Measures].[FooBar]'>" + "</CalculatedMember>");
  ...
  public void testQueryCalculatedMemberOverridesCube() {
    ...
    assertQueryReturns("SELECT {[Measures].[Profit]} ON COLUMNS,\n"
      + " {[Time].[1997].[Q2]} ON ROWS\n" + "FROM [Sales]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Profit]}\n" + "Axis #2:\n"
      + "{[Time].[1997].[Q2]}\n" + "Row #0: $79,702.05\n");
  }
  ...
  public void testQueryCalcMemberOverridesShallowerStoredMember() {
    ...
    assertQueryReturns("with member [Time].[Weekly].[1998].[1997xxx] as 4\n"
      + " select [Time].[Weekly].[1997] on 0\n" + " from [Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Time].[Weekly].[1997]}\n" + "Row #0: 266,773\n");
  }

  /**
   * If there are multiple calc members with the same name, the first is chosen,
   * even if it is not the best match.
   */
  ...
  public void testCalcMemberIsSetFails() {
    ...
    // a value, is an error.
    String queryString = "with member [Measures].[Foo] as ' Filter([Product].members, 1 <> 0) '"
      + "select {[Measures].[Foo]} on columns from [Sales]";
    String pattern = "Member expression 'Filter([Product].Members, (1 <> 0))' must "
      + "not be a set";
    assertQueryThrows(queryString, pattern);

    // A tuple is OK, because it can be converted to a scalar expression.
    queryString = "with member [Measures].[Foo] as ' ([Measures].[Unit Sales], [Gender].[F]) '"
      + "select {[Measures].[Foo]} on columns from [Sales]";
  ...
  /**
   * Tests that calculated members can have brackets in their names. (Bug
   * 1251683.)
   */
  ...
  /**
   * Tests that IIf works OK even if its argument returns the NULL value. (Bug
   * 1418689.)
   */
  ...
  public void testBracketInCubeCalcMemberName() {
    ...
    final TestContext testContext = TestContext.instance().create(null, s, null,
      null, null, null);
  ...
  public void testCalcMemberWithQuote() {
    ...
    assertQueryReturns("with member [Measures].[Foo] as '1 + 2'\n"
    ...
    assertQueryThrows(
      "with member [Measures].[Foo] as ' \"quoted string with 'apostrophe' in it\" ' "
        + "select {[Measures].[Foo]} on columns " + "from [Sales]",
      "mondrian.parser.TokenMgrError: " + "Lexical error at line 2, column 0.  "
    ...
    assertQueryReturns(
      "with member [Measures].[Foo] as ' \"quoted string with ''apostrophe'' in it\" ' "
        + "select {[Measures].[Foo]} on columns " + "from [Sales]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Measures].[Foo]}\n"
    ...
    assertQueryReturns(
      "with member [Measures].[Colored Profit] as  ' [Measures].[Store Sales] - [Measures].[Store Cost] ', "
        ...
        + " {[Product].Children} on rows " + "from [Sales]", "Axis #0:\n" + "{}\n"
  ...
  public void testNonCharacterMembers() {
    assertQueryReturns("with member [Has Coffee Bar].[Maybe] as \n"
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-263"> bug
   * MONDRIAN-263, Negative Solve Orders broken</a>.
   */
  public void testNegativeSolveOrder() {
    ...
    assertQueryReturns("with member [Product].[Foo] as ' 1 ', SOLVE_ORDER = -6\n"
  ...
  public void testCalcMemberCustomFormatterInQueryNegative2() {
    String query = "with member [Measures].[Foo] as ' [Measures].[Unit Sales] * 2 ',\n"
      ...
      + "select {[Measures].[Unit Sales], [Measures].[Foo]} on 0,\n"
      + " {[Store].Children} on rows\n" + "from [Sales]";
    assertQueryThrows(query,
      "Failed to load formatter class 'java.lang.String' for member '[Measures].[Foo]'.");
    assertQueryThrows(query, Util.PreJdk15 ? "java.lang.ClassCastException"
  ...
  /**
   * Tests a calculated member which aggregates over a set which would seem to
   * include the calculated member (but does not).
   */
  ...
  /**
   * Tests that if a filter is associated with input to a cal member with lower
   * solve order; the filter computation uses the context that contains the
   * other cal members(those with higher solve order).
   */
  ...
  /**
   * Tests that if a filter is associated with input to a cal member with higher
   * solve order; the filter computation ignores the other cal members.
   */
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-335">MONDRIAN-335</a>,
   * "Issues with calculated members". Verify that the calculated member
   * [Product].[Food].[Test] definition does not throw errors and returns
   * expected results.
   */
  public void testNonTopLevelCalculatedMember() {
    assertQueryReturns("with member [Product].[Test] as '[Product].[Food]' "
      + "select {[Measures].[Unit Sales]} on columns, "
      + "{[Product].[Test]} on rows " + "from Sales", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
      + "{[Product].[Test]}\n" + "Row #0: 191,940\n");

    assertQueryReturns("with member [Product].[Food].[Test] as '[Product].[Food]' "
      + "select {[Measures].[Unit Sales]} on columns, "
      + "{[Product].[Food].[Test]} on rows " + "from Sales", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
      + "{[Product].[Food].[Test]}\n" + "Row #0: 191,940\n");
  }
   ...
   * href="http://jira.pentaho.com/browse/MONDRIAN-335">MONDRIAN-335</a>,
   * "Issues with calculated members". Verify that the calculated member
   * [Product].[Test] returns an empty children list vs. invalid behavior of
   * returning [All Products].Children
   */
  public void testCalculatedMemberChildren() {
    assertQueryReturns("with member [Product].[Test] as '[Product].[Food]' "
      + "select {[Measures].[Unit Sales]} on columns, "
      + "[Product].[Test].children on rows " + "from Sales", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n");
    assertQueryReturns("with member [Product].[Food].[Test] as '[Product].[Food]' "
      + "select {[Measures].[Unit Sales]} on columns, "
      + "[Product].[Food].[Test].children on rows " + "from Sales", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n");
  }
  ...
  /**
   * Query that simulates a compound slicer by creating a calculated member that
   * aggregates over a set and places it in the WHERE clause.
   */
   ...
   */
  private void checkForExponentialPerformance(int n, boolean print) {
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-638">MONDRIAN-638</a>,
   ...
   * 
   * <p>
   * The problem is in the implicit conversion that occurs when using a member
   * as a numeric value. (In this case the conversion occurs because we apply
   * the numeric operator '/'.) We have to assume at prepare time that the
   * current measure will be numeric, but at run time the value may be a string.
   * 
   * <p>
   * We were wrongly throwing a ClassCastException. Correct behavior is an
  ...
  /**
   * Tests referring to a calc member by a name other than its canonical unique
   * name.
   */
  ...
  public void testCalcMemberParentOfCalcMember() {
    ...
    assertQueryThrows("with member [Gender].[X] as 4\n"
  ...
  public void testCalcMemberSameNameDifferentHierarchies() {
    assertQueryReturns("with member [Gender].[X] as 4\n"
  ...
  public void testCalcMemberTooDeep() {
    ...
    assertQueryThrows("with member [Gender].[M].[X] as 4\n"
