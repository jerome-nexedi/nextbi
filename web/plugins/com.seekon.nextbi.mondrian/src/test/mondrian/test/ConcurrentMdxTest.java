...
/**
 * Runs specified set of MDX queries concurrently. This Class is not added to
 * the Main test suite. Purpose of this test is to simulate Concurrent access to
 * Aggregation and data load. Simulation will be more effective if we run this
 * single test again and again with a fresh connection.
 * 
 * @author Thiyagu,Ajit
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/ConcurrentMdxTest.java#12
 *          $
 */
public class ConcurrentMdxTest extends FoodMartTestCase {
  ...
  static final QueryAndResult[] mdxQueries = new QueryAndResult[] {
    ...
    new QueryAndResult("select {[Measures].[Sales Count], "
      + "[Measures].[Store Invoice]} on 0 from [Warehouse and Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[Sales Count]}\n"
    ...
    new QueryAndResult("select {[Measures].[Sales Count], "
      + "[Measures].[Store Invoice]} on 0, " + "{[Time].[1997],[Time].[1998]} on 1 "
    ...
    new QueryAndResult("select {[Measures].[Sales Count], "
      ...
      + "{([Gender].[M],[Time].[1997])} on 1 " + "from [Warehouse and Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Sales Count]}\n"
        + "{[Measures].[Store Invoice]}\n" + "Axis #2:\n"
        + "{[Gender].[M], [Time].[1997]}\n" + "Row #0: 44,006\n" + "Row #0: \n"),
    ...
    new QueryAndResult("select {[Measures].[Sales Count], "
      + "[Measures].[Store Invoice]} on 0, "
      + "{([Gender].[F],[Time].[1997])} on 1 " + "from [Warehouse and Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Sales Count]}\n"
        + "{[Measures].[Store Invoice]}\n" + "Axis #2:\n"
        + "{[Gender].[F], [Time].[1997]}\n" + "Row #0: 42,831\n" + "Row #0: \n"),

    new QueryAndResult("select {[Measures].[Store Cost], "
      + "[Measures].[Supply Time]} on 0 from [Warehouse and Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[Store Cost]}\n"
      + "{[Measures].[Supply Time]}\n" + "Row #0: 225,627.23\n" + "Row #0: 10,425\n"),

    new QueryAndResult("select {[Measures].[Store Sales], "
      + "[Measures].[Units Ordered]} on 0 from [Warehouse and Sales]", "Axis #0:\n"
    ...
    new QueryAndResult("select {[Measures].[Profit], "
      + "[Measures].[Units Shipped]} on 0 from [Warehouse and Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[Profit]}\n"
