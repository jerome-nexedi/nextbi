...
/**
 * <code>AccessControlTest</code> is a set of unit-tests for access-control. For
 * these tests, all of the roles are of type RoleImpl.
 * 
 * @see Role
 * 
 * @author jhyde
 * @since Feb 21, 2003
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/AccessControlTest.java#57
 *          $
 */
public class AccessControlTest extends FoodMartTestCase {
  ...
  public void testSchemaReader() {
    ...
    final SchemaReader schemaReader = cube.getSchemaReader(connection.getRole());
  ...
  public void testGrantDimensionNone() {
    ...
    Dimension genderDimension = (Dimension) schemaReader.lookupCompound(salesCube,
      Id.Segment.toList("Gender"), true, Category.Dimension);
    role.grant(genderDimension, Access.NONE);
    role.makeImmutable();
    connection.setRole(role);
    context.assertAxisThrows("[Gender].children",
  ...
  public void testRestrictMeasures() {
    ...
    role1.assertQueryReturns("SELECT {[Measures].Members} ON COLUMNS FROM [SALES]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
        ...
        + "Row #0: 225,627.23\n" + "Row #0: 565,238.13\n" + "Row #0: 86,837\n"
        + "Row #0: 5,581\n" + "Row #0: 151,211.21\n");
    role2.assertQueryReturns("SELECT {[Measures].Members} ON COLUMNS FROM [SALES]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
        + "Row #0: 266,773\n");
  }
    ...
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        "<Role name=\"Role1\">\n"
          ...
          + "      </HierarchyGrant>\n" + "    </CubeGrant>\n"
          + "  </SchemaGrant>\n" + "</Role>").withRole("Role1");
    testContext.assertQueryThrows("select {[Store].Children} on 0 from [Sales]",
  ...
  public void testRoleMemberAccess() {
    ...
    assertMemberAccess(connection, Access.NONE,
  ...
  private void assertMemberAccess(final Connection connection,
    Access expectedAccess, String memberName) {
    final Role role = connection.getRole(); // restricted
    Schema schema = connection.getSchema();
    final boolean fail = true;
    Cube salesCube = schema.lookupCube("Sales", fail);
    final SchemaReader schemaReader = salesCube.getSchemaReader(null).withLocus();
    final Member member = schemaReader.getMemberByUniqueName(Util
      .parseIdentifier(memberName), true);
    final Access actualAccess = role.getAccess(member);
    Assert.assertEquals(memberName, expectedAccess, actualAccess);
  }

  private void assertCubeAccess(final Connection connection, Access expectedAccess,
    String cubeName) {
  ...
  private void assertHierarchyAccess(final Connection connection,
    Access expectedAccess, String cubeName, String hierarchyName) {
    final Role role = connection.getRole();
    Schema schema = connection.getSchema();
    final boolean fail = true;
    Cube cube = schema.lookupCube(cubeName, fail);
    final SchemaReader schemaReader = cube.getSchemaReader(null); // unrestricted
    final Hierarchy hierarchy = (Hierarchy) schemaReader.lookupCompound(cube, Util
      .parseIdentifier(hierarchyName), fail, Category.Hierarchy);

    final Access actualAccess = role.getAccess(hierarchy);
    Assert.assertEquals(cubeName, expectedAccess, actualAccess);
  }

  private Role.HierarchyAccess getHierarchyAccess(final Connection connection,
    String cubeName, String hierarchyName) {
    final Role role = connection.getRole();
    Schema schema = connection.getSchema();
    final boolean fail = true;
    Cube cube = schema.lookupCube(cubeName, fail);
  ...
  public void testGrantHierarchy2() {
    ...
    testContext.assertAxisReturns("[Store].[USA].children", "[Store].[USA].[CA]");
    testContext.assertAxisReturns("[Store].[USA].children", "[Store].[USA].[CA]");
  ...
  public void testGrantHierarchy5() {
    ...
    testContext.assertAxisReturns("[Store].members",
    // note:
  ...
  public void testGrantHierarchy6() {
    ...
    getRestrictedTestContext()
      .assertAxisReturns("[Customers].[USA].[CA].parent", "");
  }

  public void testGrantHierarchy7() {
    // assert: members above top level do not exist
    final TestContext testContext = getRestrictedTestContext();
    testContext.assertAxisThrows("[Customers].[Canada].children",
  ...
  public void testGrantHierarchy8() {
    ...
    testContext.assertAxisThrows(
      "[Customers].[USA].[CA].[San Francisco].[Catherine Abel]", "not found");
  ...
  public void testGrantHierarchy9() {
    ...
    tc.assertQueryReturns("with member [Measures].[California Unit Sales] as "
  ...
  public void _testSharedObjectsInGrantMappingsBug() {
    final TestContext testContext = new TestContext() {
      public Connection getConnection() {
        ...
        Hierarchy measuresInSales = salesCube.lookupHierarchy(new Id.Segment(
          "Measures", Id.Quoting.UNQUOTED), false);
        Hierarchy storeInWarehouse = warehouseCube.lookupHierarchy(new Id.Segment(
          "Store", Id.Quoting.UNQUOTED), false);
    ...
    // fail.
    testContext.assertExprThrows("[Store].DefaultMember",
   ...
   * 
   * @param restrictCustomers
   *          true to restrict access to the customers dimension. This will
   *          change the defaultMember of the dimension, all cell values will be
   *          null because there are no sales data for Canada
   * 
   * @return restricted connection
   */
  private Connection getRestrictedConnection(boolean restrictCustomers) {
    Connection connection = getTestContext().withSchemaPool(false).getConnection();
    RoleImpl role = new RoleImpl();
    Schema schema = connection.getSchema();
    final boolean fail = true;
    Cube salesCube = schema.lookupCube("Sales", fail);
    final SchemaReader schemaReader = salesCube.getSchemaReader(null).withLocus();
    Hierarchy storeHierarchy = salesCube.lookupHierarchy(new Id.Segment("Store",
      Id.Quoting.UNQUOTED), false);
    ...
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[USA].[OR]"), fail), Access.ALL);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[USA]"), fail), Access.CUSTOM);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[USA].[CA].[San Francisco]"), fail),
      Access.ALL);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[USA].[CA].[Los Angeles]"), fail),
      Access.ALL);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[Mexico]"), fail), Access.ALL);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[Mexico].[DF]"), fail), Access.NONE);
    role.grant(schemaReader.getMemberByUniqueName(Util
      .parseIdentifier("[Store].[All Stores].[Canada]"), fail), Access.NONE);
    if (restrictCustomers) {
      Hierarchy customersHierarchy = salesCube.lookupHierarchy(new Id.Segment(
        "Customers", Id.Quoting.UNQUOTED), false);
      Level stateProvinceLevel = Util.lookupHierarchyLevel(customersHierarchy,
        "State Province");
      Level customersCityLevel = Util.lookupHierarchyLevel(customersHierarchy,
        "City");
      role.grant(customersHierarchy, Access.CUSTOM, stateProvinceLevel,
        customersCityLevel, rollupPolicy);
    }
  ...
  /*
   * todo: test that access to restricted measure fails (will not work -- have
   * not fixed Cube.getMeasures)
   */
  ...
  /**
   * Test context where the [Store] hierarchy has restricted access and cell
   * values are rolled up with 'partial' policy.
   */
  private final TestContext rollupTestContext = TestContext
    .instance()
    .create(
      null,
      null,
  ...
  /**
   * Basic test of partial rollup policy. [USA] = [OR] + [WA], not the usual
   * [CA] + [OR] + [WA].
   */
  ...
  /**
   * The total for [Store].[All Stores] is similarly reduced. All children of
   * [All Stores] are visible, but one grandchild is not. Normally the total is
   * 266,773.
   */
  public void testRollupPolicyAll() {
    rollupTestContext.assertExprReturns("([Store].[All Stores])", "192,025");
  }

  /**
   * Access [Store].[All Stores] implicitly as it is the default member of the
   * [Stores] hierarchy.
   */
  ...
  /**
   * Access [Store].[All Stores] via the Parent relationship (to check that this
   * doesn't circumvent access control).
   */
  public void testRollupPolicyAllAsParent() {
    rollupTestContext.assertExprReturns("([Store].[USA].Parent)", "192,025");
  }
  ...
  private void rollupPolicyBottom(Role.RollupPolicy rollupPolicy, String v1,
    String v2, String v3) {
    final TestContext testContext = TestContext
      .instance()
      .create(
    ...
    testContext.assertExprThrows("([Customers].[USA].[CA].[Los Angeles])",
    ...
    checkQuery(testContext, "select [Customers].Children on 0, "
      + "[Gender].Members on 1 from [Sales]");
  }

  /**
   * Calls various {@link SchemaReader} methods on the members returned in a
   * result set.
   * 
   * @param testContext
   *          Test context
  ...
  private void checkQuery(TestContext testContext, String mdx) {
    ...
    final SchemaReader schemaReader = testContext.getConnection().getSchemaReader()
      .withLocus();
    for (Axis axis : result.getAxes()) {
      for (Position position : axis.getPositions()) {
        for (Member member : position) {
          final Member accessControlledParent = schemaReader.getMemberParent(member);
          if (member.getParentMember() == null) {
            assertNull(accessControlledParent);
          }
          final List<Member> accessControlledChildren = schemaReader
  ...
  /**
   * Tests that a bad value for the rollupPolicy attribute gives the appropriate
   * error.
   */
    ...
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        "<Role name=\"Role1\">\n"
          ...
          + "      </HierarchyGrant>\n" + "    </CubeGrant>\n"
          + "  </SchemaGrant>\n" + "</Role>").withRole("Role1");
    testContext.assertQueryThrows("select from [Sales]",
  ...
  public void testRollupPolicyGreatGrandchildInvisible() {
    ...
    rollupPolicyGreatGrandchildInvisible(Role.RollupPolicy.PARTIAL, "266,767",
      "74,742");
    rollupPolicyGreatGrandchildInvisible(Role.RollupPolicy.HIDDEN, "", "");
  }

  private void rollupPolicyGreatGrandchildInvisible(Role.RollupPolicy policy,
    String v1, String v2) {
    final TestContext testContext = TestContext
      .instance()
      .create(
    ...
    testContext
      .assertExprReturns("([Measures].[Unit Sales], [Customers].[USA])", v1);
    testContext.assertExprReturns(
      "([Measures].[Unit Sales], [Customers].[USA].[CA])", v2);
  }
  ...
  public void testRollupPolicySimultaneous() {
    ...
    rollupPolicySimultaneous(Role.RollupPolicy.FULL, "266,773", "74,748", "25,635");
    rollupPolicySimultaneous(Role.RollupPolicy.PARTIAL, "72,631", "72,631", "25,635");
    rollupPolicySimultaneous(Role.RollupPolicy.HIDDEN, "", "", "");
  }

  private void rollupPolicySimultaneous(Role.RollupPolicy policy, String v1,
    String v2, String v3) {
    final TestContext testContext = TestContext
      .instance()
      .create(
    ...
    testContext.assertExprReturns(
      "([Measures].[Unit Sales], [Customers].[USA].[CA])", v2);
    testContext.assertExprReturns("([Measures].[Unit Sales], "
      + "[Customers].[USA].[CA], [Store].[USA].[CA])", v2);
    testContext.assertExprReturns("([Measures].[Unit Sales], "
      + "[Customers].[USA].[CA], " + "[Store].[USA].[CA].[San Diego])", v3);
  }
  ...
  public void testUnionRole() {
    ...
    final Role.HierarchyAccess hierarchyAccess = getHierarchyAccess(connection,
      "Sales", "[Store]");
    ...
    testContext.withRole("Role1").assertQueryThrows(mdx,
      "MDX object '[Customers].[USA].[OR]' not found in cube 'Sales'");

    testContext.withRole("Role2").assertQueryThrows(mdx,
  ...
  /**
   * Test to verify that non empty crossjoins enforce role access. Testcase for
   * bug <a href="http://jira.pentaho.com/browse/MONDRIAN-369"> MONDRIAN-369,
   * "Non Empty Crossjoin fails to enforce role access".
   */
  public void testNonEmptyAccess() {
    ...
    final String mdx = "select {[Measures].[Unit Sales]} ON COLUMNS, "
      + " Crossjoin({[Gender].[All Gender]}, " + "[Product].Children) ON ROWS "
      + "from [Sales]";
   ...
   * MONDRIAN-406, "Rollup policy doesn't work for members that are implicitly
   * visible"</a>.
   */
  public void testGoodman() {
    ...
    final TestContext testContext = goodmanContext(Role.RollupPolicy.PARTIAL);
   ...
   * MONDRIAN-402, "Bug in RolapCubeHierarchy.hashCode() ?"</a>. Access-control
   * elements for hierarchies with same name in different cubes could not be
   * distinguished.
   */
  public void testBugMondrian402() {
    final TestContext testContext = TestContext.instance().create(
      ...
      "<Role name=\"California manager\">\n" + "  <SchemaGrant access=\"none\">\n"
        ...
        + "    </CubeGrant>\n" + "  </SchemaGrant>\n" + "</Role>").withRole(
      "California manager");
    assertHierarchyAccess(testContext.getConnection(), Access.NONE, "Sales", "Store");
    assertHierarchyAccess(testContext.getConnection(), Access.CUSTOM,
      "Sales Ragged", "Store");
  }
  ...
  public void testPartialRollupParentChildHierarchy() {
    ...
    final String mdx2 = "select\n"
      + "  {[Measures].[Number of Employees]} on columns,\n"
      + "  {[Employees]} on rows\n" + "from HR";
    testContext.assertQueryReturns(mdx2, "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Number of Employees]}\n" + "Axis #2:\n"
  ...
  public void testParentChildUserDefinedRole() {
    ...
    try {
      ...
      connection.setRole(new PeopleRole(savedRole, connection.getSchema(),
        "Sheri Nowmer"));
      ...
      connection.setRole(new PeopleRole(savedRole, connection.getSchema(),
        "Derrick Whelply"));
      testContext.assertExprReturns("[Employees].Members.Count", "605");
      testContext
        .assertAxisReturns(
          "Head([Employees].Members, 4)," + "Tail([Employees].Members, 2)",
          "[Employees].[All Employees]\n"
      ...
      connection.setRole(new PeopleRole(savedRole, connection.getSchema(),
        "Ann Weyerhaeuser"));
      testContext.assertExprReturns("[Employees].Members.Count", "7");
      testContext
        .assertAxisReturns(
          "[Employees].Members",
          "[Employees].[All Employees]\n"
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/BISERVER-1574">BISERVER-1574,
   * "Cube role rollupPolicy='partial' failure"</a>. The problem was a
   * NullPointerException in
   * {@link SchemaReader#getMemberParent(mondrian.olap.Member)} when called on a
   * members returned in a result set. JPivot calls that method but Mondrian
   * normally does not.
   */
  public void testBugBiserver1574() {
    final TestContext testContext = TestContext.instance().create(null, null, null,
      null, null, BiServer1574Role1).withRole("role1");
    final String mdx = "select {([Measures].[Store Invoice], [Store Size in SQFT].[All Store Size in SQFTs])} ON COLUMNS,\n"
  ...
  public void testBugMondrian435() {
    ...
    testContext
      .assertQueryReturns(
        "select hierarchize("
          ...
          + ") on 0," + "[Store Type].Members on 1 from [Warehouse]",
        "Axis #0:\n"
          + "{}\n"
          + "Axis #1:\n"
    ...
    testContext
      .assertQueryReturns(
        "select hierarchize(" + "    { ([Store Size in SQFT], [Product]),\n"
  ...
  /**
   * Tests that hierarchy-level access control works on a virtual cube. See bug
   * <a href="http://jira.pentaho.com/browse/MONDRIAN-456"> MONDRIAN-456,
   * "Roles and virtual cubes"</a>.
   */
  public void testVirtualCube() {
    ...
    testContext.assertQueryReturns(
      "select [Store].Members on 0 from [Warehouse and Sales]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Store].[All Stores]}\n" + "{[Store].[USA]}\n"
        + "{[Store].[USA].[CA]}\n" + "{[Store].[USA].[CA].[Alameda]}\n"
  ...
  /**
   * this tests the fix for http://jira.pentaho.com/browse/BISERVER-2491
   * rollupPolicy=partial and queries to upper members don't work
   */
  public void testBugBiserver2491() {
    final String BiServer2491Role2 = "<Role name=\"role2\">"
      + " <SchemaGrant access=\"none\">"
      + "  <CubeGrant cube=\"Sales\" access=\"all\">"
      + "   <HierarchyGrant hierarchy=\"[Store]\" access=\"custom\" rollupPolicy=\"partial\">"
      + "    <MemberGrant member=\"[Store].[USA].[CA]\" access=\"all\"/>"
      + "    <MemberGrant member=\"[Store].[USA].[CA].[Los Angeles]\" access=\"none\"/>"
      + "   </HierarchyGrant>" + "  </CubeGrant>" + " </SchemaGrant>" + "</Role>";

    final TestContext testContext = TestContext.instance().create(null, null, null,
      null, null, BiServer2491Role2).withRole("role2");

    final String firstBrokenMdx = "select [Measures].[Unit Sales] ON COLUMNS, {[Store].[Store Country].Members} ON ROWS from [Sales]";

    checkQuery(testContext, firstBrokenMdx);
    testContext.assertQueryReturns(firstBrokenMdx, "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
      + "{[Store].[USA]}\n" + "Row #0: 49,085\n");

    final String secondBrokenMdx = "select [Measures].[Unit Sales] ON COLUMNS, "
      + "Descendants([Store],[Store].[Store Name]) ON ROWS from [Sales]";
    checkQuery(testContext, secondBrokenMdx);
    testContext.assertQueryReturns(secondBrokenMdx, "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
      + "{[Store].[USA].[CA].[Alameda].[HQ]}\n"
      + "{[Store].[USA].[CA].[Beverly Hills].[Store 6]}\n"
      + "{[Store].[USA].[CA].[San Diego].[Store 24]}\n"
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-622">MONDRIAN-622,
  ...
  public void testBugMondrian622() {
    ...
    for (Position position : result.getAxes()[0].getPositions()) {
      ...
      String name = member.getParentMember().getName() + "." + member.getName(); // e.g.
      // "BC.Burnaby"
      // e.g. "[Customers].[State Province].[BC].[Burnaby]"
      String uniqueName = Util.replace(member.getUniqueName(), ".[All Customers]",
        "");
      // e.g. "[Customers2].[State Province].[BC].[Burnaby]"
      String uniqueName2 = Util.replace(uniqueName, "Customers", "Customers2");
      // e.g. "[Customers3].[State Province].[BC].[Burnaby]"
      String uniqueName3 = Util.replace(uniqueName, "Customers", "Customers3");
      buf
        .append("  <Role name=\""
          + name
          + "\"> \n"
          + "    <SchemaGrant access=\"none\"> \n"
          + "      <CubeGrant access=\"all\" cube=\""
          + cubeName
          + "\"> \n"
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-694">MONDRIAN-694, "Incorrect
   * handling of child/parent relationship with hierarchy grants"</a>.
   */
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-722">MONDRIAN-722, "If
   * ignoreInvalidMembers=true, should ignore grants with invalid members"</a>.
   */
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-746">MONDRIAN-746, "Report
   * returns stack trace when turning on subtotals on a hierarchy with top level
   * hidden"</a>.
   */
  public void testCalcMemberLevel() {
    checkCalcMemberLevel(getTestContext());
    checkCalcMemberLevel(TestContext
      .instance()
      .create(
        null,
        null,
  ...
  /**
   * Test for bug MONDRIAN-568. Grants on OLAP elements are validated by name,
   * thus granting implicit access to all cubes which have a dimension with the
   * same name.
   */
  public void testBugMondrian568() {
    ...
    final TestContext testContextRole1 = testContext.withRole("Role1").withCube(
      "Sales");
    final TestContext testContextRole12 = testContext.withRole("Role1,Role2")
      .withCube("Sales");

    assertMemberAccess(testContextRole1.getConnection(), Access.NONE,
      "[Measures].[Store Cost]");

    assertMemberAccess(testContextRole12.getConnection(), Access.NONE,
      "[Measures].[Store Cost]");
  }

  private void checkCalcMemberLevel(TestContext testContext) {
    Result result = testContext
      .executeQuery("with member [Store].[USA].[CA].[Foo] as\n" + " 1\n"
        + "select {[Measures].[Unit Sales]} on columns,\n"
        + "{[Store].[USA].[CA],\n" + " [Store].[USA].[CA].[Los Angeles],\n"
  ...
  /**
   * Testcase for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-935">MONDRIAN-935,
  ...
  public static class PeopleRole extends DelegatingRole {
    ...
    private void defineGrantsForUser(Schema schema) {
      ...
      Hierarchy hierarchy = cube.lookupHierarchy(new Id.Segment("Employees",
        Id.Quoting.QUOTED), false);
      ...
      List<Member> members = schema.getSchemaReader().withLocus().getLevelMembers(
        topLevel, true);
