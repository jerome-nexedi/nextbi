/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/SolveOrderScopeIsolationTest.java#13 $
 ...
 * order scope isolation behavior. Scope Isolation: In SQL Server 2005 Analysis
 * Services, when a cube Multidimensional Expressions (MDX) script contains
 * calculated members, by default the calculated members are resolved before any
 * session-scoped calculations are resolved and before any query-defined
 * calculations are resolved. This is different from SQL Server 2000 Analysis
 * Services behavior, where solve order can explicitly be used to insert a
 * session-scoped or query-defined calculation in between two cube-level
 * calculations. Further details at:
 * http://msdn2.microsoft.com/en-us/library/ms144787.aspx
 * 
 ...
 * @author ajogleka
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/SolveOrderScopeIsolationTest
 *          .java#13 $
 * @since Apr 04, 2008
 */
public class SolveOrderScopeIsolationTest extends FoodMartTestCase {
  ...
  private SolveOrderMode getSolveOrderMode() {
    return Util.lookup(SolveOrderMode.class,
  ...
  public TestContext getTestContext() {
    return TestContext.instance().createSubstitutingCube("Sales", null, memberDefs);
  }
  ...
  public void testAllSolveOrderModesHandled() {
    for (SolveOrderMode mode : SolveOrderMode.values()) {
      ...
      case SCOPED:
        break;
      default:
        fail("Tests for solve order mode " + mode.toString()
  ...
  public void testCubeMemberEvalBeforeQueryMemberAbsolute() {
    final String mdx = "WITH MEMBER [Customers].USAByWA AS\n"
  ...
  public void testOverrideCubeMemberUsingStrToMemberDoesNotHappenAbsolute() {
    final String mdx = "with\n"
      ...
      + "StrToMember(\"[Gender].[maleMinusFemale]\"))', " + "SOLVE_ORDER=5\n"
      + "select {measures.[ratio],\n" + "measures.[unit sales],\n"
      + "measures.[sales count]} on 0,\n"
      + "{[Gender].[override], gender.maleMinusFemale} on 1\n" + "from sales";
    setSolveOrderMode(ABSOLUTE);
  ...
  /**
   * This test validates that behavior is consistent with Analysis Services 2000
   * when Solve order scope is ABSOLUTE. AS2K will throw an error whenever
   * attempting to aggregate over calculated members (i.e. when the solve order
   * of the Aggregate member is higher than the calculations it intersects
   * with).
   */
  ...
  public void test2LevelOfOverrideCubeMemberDoesNotHappenAbsolute() {
    final String mdx = "With member gender.override1 as 'gender.maleMinusFemale',\n"
      + "SOLVE_ORDER=20\n"
