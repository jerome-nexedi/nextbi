/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/loader/CsvDBLoader.java#15 $
 ...
 * </blockquote>
 * <p>
 * See the testsrc/main/mondrian/rolap/aggmatcher/BUG_1541077.csv file for an
 * example.
 * 
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/loader/CsvDBLoader.java
 *          #15 $
 */
public class CsvDBLoader extends DBLoader {
  ...
  /**
   * Looks for a file relative to the current directory and all of its parent
   * directories. This gives a little flexibility if you invoke a test in a
   * sub-directory.
   * 
   * @param file
   *          File
    ...
    try {
      ...
      while (csvloader.hasNextLine()) {
        ...
        if (value0.startsWith("##") && (fileName == null)) {
          ...
          if (index != -1) {
            String s = value0.substring(index + ACTION_BEFORE_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: "
                + " no action before sql" + ", linenos " + lineNos);
            }
            s = s.trim();
            if (!s.startsWith(DROP_INDEX_TAG)) {
              // only support dropping indexes currently
              throw new IOException("CSV File parse Error: "
                + " unknown before action" + s + ", linenos " + lineNos);
            }
            // get index name
            index = s.indexOf(' ');
            if (index < 0) {
              // only support dropping indexes currently
              throw new IOException("CSV File parse Error: "
                + " no index name in before action" + s + ", linenos " + lineNos);
            }
          ...
          if (index != -1) {
            String s = value0.substring(index + ACTION_AFTER_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: "
                + " no action after sql" + ", linenos " + lineNos);
            }
            s = s.trim();
            ...
            if (index < 0) {
              // only support creating indexes currently
              throw new IOException("CSV File parse Error: "
                + " no index_name/column_name in after action" + s + ", linenos "
                + lineNos);
            }
            ...
            if (index < 0) {
              // only support creating indexes currently
              throw new IOException("CSV File parse Error: "
                + " no column_name after index_name " + "in after action" + s
                + ", linenos " + lineNos);
            }
          ...
          if (index != -1) {
            String s = value0.substring(index + TABLE_NAME_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: " + " no table name"
                + ", linenos " + lineNos);
            }
            s = s.trim();
            if (tableName != null) {
              throw new IOException("CSV File parse Error: " + " new table name \""
                + s + "\" while processing table name \"" + tableName
                + "\", linenos " + lineNos);
            }
            tableName = s;
            continue;
          ...
          if (index != -1) {
            String s = value0.substring(index + COLUMN_NAMES_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: " + " no column names"
                + ", linenos " + lineNos);
            }
            if (tableName == null) {
              throw new IOException("CSV File parse Error: "
                + " no table name for columns " + ", linenos " + lineNos);
            }
            values[0] = s.trim();
            columnNames = values;
            continue;
          ...
          if (index != -1) {
            String s = value0.substring(index + COLUMN_TYPES_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: " + " no column types"
                + ", linenos " + lineNos);
            }
            if (columnNames == null) {
              throw new IOException("CSV File parse Error: "
                + " no column names for columns types" + ", linenos " + lineNos);
            }
            values[0] = s.trim();
            columnTypes = values;
            ...
            if (columnNames.length != columnTypes.length) {
              throw new IOException("CSV File parse Error: "
                + " number of column names \"" + columnNames.length
                + "\" does not equal " + " number of column types \""
                + columnTypes.length + "\", linenos " + lineNos);
            }
            continue;
          }
          ...
          if (index != -1) {
            String s = value0.substring(index + FILE_NAME_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: " + " no file name "
                + ", linenos " + lineNos);
            }
            if (columnTypes == null) {
              throw new IOException("CSV File parse Error: "
                + " no column types for file name" + ", linenos " + lineNos);
            }
            fileName = s.trim();
            continue;
          }
          ...
          if (index != -1) {
            String s = value0.substring(index + NOS_OF_ROWS_TAG.length());
            if (s.length() == 0) {
              throw new IOException("CSV File parse Error: " + " no number of rows"
                + ", linenos " + lineNos);
            }
            if (columnTypes == null) {
              throw new IOException("CSV File parse Error: "
          ...
          if (!ok) {
            if (tableName == null) {
              throw new IOException("CSV File parse Error: "
                + " no table name before rows" + ", linenos " + lineNos);
            }
            if (columnNames == null) {
              throw new IOException("CSV File parse Error: "
                + " no column names before rows" + ", linenos " + lineNos);
            }
            if (columnTypes == null) {
              throw new IOException("CSV File parse Error: "
                + " no column types before rows" + ", linenos " + lineNos);
            }
          }
          ...
          if (fileName != null) {
            ...
            RowStream rowStream = new CsvLoaderRowStream(new CsvLoader(fileName));
            controller.setRowStream(rowStream);
            csvloader.nextSet();
            csvloader.putBack(values);
            // System.out.println("CsvLoader.loadTables: fileName putback0="
            // +values[0]);
            lineNos--;
            fileName = null;
            ...
            while (nosOfRowsStr-- > 0) {
              if (!csvloader.hasNextLine()) {
                throw new Exception("CSV File parse Error: "
                  + " not enough lines in file " + lineNos);
              }
            ...
            // System.out.println("CsvLoader.loadTables: else");
            csvloader.putBack(values);
            RowStream rowStream = new CsvLoaderRowStream(csvloader);
            controller.setRowStream(rowStream);
            csvloader = null;
            break;
          }
  ...
  protected Column[] loadColumns(String[] columnNames, String[] columnTypes,
    int lineNos) throws Exception {
    ...
    for (int i = 0; i < columnNames.length; i++) {
      ...
      if (index != -1) {
        ...
        if (nullString.equalsIgnoreCase("NULL")) {
          ...
          throw new IOException("CSV File parse Error: " + " for type name \""
            + columnType + "\" expecting \"null\" not \"" + nullString
            + "\", linenos " + lineNos);
        }
      ...
      if (type == null) {
        throw new IOException("CSV File parse Error: "
          + " no type found for type name \"" + columnType + "\", linenos "
          + lineNos);
      }
