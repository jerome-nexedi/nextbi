/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/RolapConnectionTest.java#33 $
 ...
 * @since 16 April, 2004
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/RolapConnectionTest.
 *          java#33 $
 */
public class RolapConnectionTest extends TestCase {
  private static final ThreadLocal<InitialContext> THREAD_INITIAL_CONTEXT = new ThreadLocal<InitialContext>();
  ...
  public void testPooledConnectionWithProperties() throws SQLException {
    Util.PropertyList properties = TestContext.instance().getConnectionProperties()
      .clone();
    ...
    try {
      ...
      } else if (e.getClass() == SQLException.class && e.getCause() == null
  ...
  /**
   * Handle an {@link IllegalArgumentException} which occurs when have tried to
   * create a connection with an illegal charset.
   */
  private void handleIllegalArgumentException(Util.PropertyList properties,
    ...
    if (System.getProperties().getProperty("java.version").startsWith("1.6.")) {
      ...
      final StringBuilder buf = new StringBuilder();
      DataSource dataSource = RolapConnection
        .createDataSource(null, properties, buf);
      final String desc = buf.toString();
      assertTrue(desc.startsWith("Jdbc="));
  ...
  private static void checkLocale(final String localeName, String expr,
    String expected, boolean isQuery) {
  ...
  public void testJndiConnection() throws NamingException {
    ...
    Util.PropertyList properties = TestContext.instance().getConnectionProperties()
      .clone();
    final StringBuilder buf = new StringBuilder();
    final DataSource dataSource = RolapConnection.createDataSource(null, properties,
      buf);
    ...
    Util.PropertyList properties2 = TestContext.instance().getConnectionProperties()
      .clone();
  ...
  public void testDataSourceOverrideUserPass() throws SQLException, NamingException {
    ...
    final String jdbcUser = properties
      .get(RolapConnectionProperties.JdbcUser.name());
    final String jdbcPassword = properties
    ...
    assertTrue(desc, desc.startsWith("Jdbc="));
    assertTrue(desc,
      desc.indexOf("JdbcUser=bogususer; JdbcPassword=boguspassword") >= 0);
    ...
    for (String poolNeeded : Arrays.asList("false", "true")) {
      ...
      try {
        ...
        assertTrue(s, s.indexOf("Error while creating SQL connection: "
          + "DataSource=jndiDataSource") >= 0);
        switch (dialect.getDatabaseProduct()) {
        case DERBY:
          assertTrue(s, s.indexOf("Caused by: java.sql.SQLException: "
            + "Schema 'BOGUSUSER' does not exist") >= 0);
          break;
        case ORACLE:
          assertTrue(s, s.indexOf("Caused by: java.sql.SQLException: ORA-01017: "
            + "invalid username/password; logon denied") >= 0);
          break;
        case MYSQL:
          assertTrue(s, s.indexOf("Caused by: java.sql.SQLException: Access denied "
            + "for user 'bogususer'@'localhost' (using " + "password: YES)") >= 0);
          break;
        case POSTGRESQL:
          assertTrue(
            s,
            s.indexOf("Caused by: org.postgresql.util.PSQLException: "
              + "FATAL: password authentication failed for " + "user \"bogususer\"") >= 0);
          break;
        }
      } finally {
        if (connection != null) {
