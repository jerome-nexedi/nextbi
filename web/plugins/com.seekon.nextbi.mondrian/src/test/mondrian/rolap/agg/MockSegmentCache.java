 ...
 * @author LBoudreau
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/agg/MockSegmentCache
 *          .java#9 $
 */
public class MockSegmentCache implements SegmentCache {
  private static final Map<SegmentHeader, SegmentBody> cache = new ConcurrentHashMap<SegmentHeader, SegmentBody>();
   ...
   * shutdown.
   */
  private static final ExecutorService executor = Util.getExecutorService(1,
  ...
  public Future<Boolean> put(final SegmentHeader header, final SegmentBody body) {
    ...
    try {
      ...
      throw new RuntimeException("while serializing " + body, e);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
    return executor.submit(new Callable<Boolean>() {
      public Boolean call() throws Exception {
        ...
        if (cache.size() > maxElements) {
          ...
          cache.remove(Math.floor(maxElements * Math.random()));
        }
        return true;
      }
    });
