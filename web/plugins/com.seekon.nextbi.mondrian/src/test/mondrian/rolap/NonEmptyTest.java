/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/NonEmptyTest.java#158 $
 ...
 * @since Nov 21, 2005
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/NonEmptyTest.java#158 $
 */
public class NonEmptyTest extends BatchTestCase {
  ...
  private static final String STORE_TYPE_LEVEL = TestContext.levelName("Store Type",
    "Store Type", "Store Type");

  private static final String EDUCATION_LEVEL_LEVEL = TestContext.levelName(
  ...
  public TestContext getTestContext() {
    return localTestContext != null ? localTestContext : super.getTestContext();
  }
  ...
  public void testBugMondrian584EnumOrder() {
    ...
    if (Bug.BugMondrian584Fixed) {
      checkNative(4, 4, "SELECT non empty { CrossJoin( "
        + "  {Gender.M, Gender.F}, "
  ...
  /**
   * Test case for an issue where mondrian failed to use native evaluation for
   * evaluating crossjoin. With the issue, performance is poor because mondrian
   * is doing crossjoins in memory; and the test case throws because the result
   * limit is exceeded.
   */
  public void testAnalyzerPerformanceIssue() {
    ...
    assertQueryReturns(
      "with set [*NATIVE_CJ_SET] as 'NonEmptyCrossJoin([*BASE_MEMBERS_Education Level], NonEmptyCrossJoin([*BASE_MEMBERS_Product], NonEmptyCrossJoin([*BASE_MEMBERS_Customers], [*BASE_MEMBERS_Time])))' "
        ...
        + "from [Sales] " + "where [Time].[*SLICER_MEMBER] ",
      "Axis #0:\n"
        + "{[Time].[*SLICER_MEMBER]}\n"
        + "Axis #1:\n"
    ...
    ctx
      .assertQueryReturns(
        "with member [Time].[Time].[First Term] as 'Aggregate({[Time].[1997].[Q1], [Time].[1997].[Q2]})' "
          ...
          + "from [Sales] " + "where ([Time].[First Term]) ", "Axis #0:\n"
          + "{[Time].[First Term]}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
          + "Axis #2:\n"
  ...
  public void testTopCountCacheKeyMustIncludeCount() {
    /**
     * When caching topcount results, the number of elements must be part of the
     * cache key
     */
    TestContext ctx = getTestContext();
    // fill cache
  ...
  public void testStrMeasure() {
    ...
    ctx.assertQueryReturns("select {[Measures].[Media]} on columns "
      + "from [StrMeasure]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Media]}\n" + "Row #0: TV\n");
  }
  ...
  public void testBug1515302() {
    ...
    ctx.assertQueryReturns("select {[Measures].[Unit Sales]} on columns, "
      + "non empty crossjoin({[Promotions].[Big Promo]}, "
      + "Descendants([Customers].[USA], [City], " + "SELF_AND_BEFORE)) on rows "
      + "from [Bug1515302]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
   ...
   * in SqlContextConstraint/SqlConstraintUtils. Test ensures that no exception
   * is thrown.
   */
  ...
  /**
   * verifies that redundant set braces do not prevent native evaluation for
   * example, {[Store].[Store Name].members} and {{[Store Type].[Store
   * Type].members}}
   */
  public void testNativeCJWithRedundantSetBraces() {
    ...
    checkNative(0, 20,
      "select non empty {CrossJoin({[Store].[Store Name].members}, "
        ...
        + "                         on rows, "
        + "{[Measures].[Store Sqft]} on columns " + "from [Store]", null,
  ...
  public void testExpandOneNonNativeInput() {
    ...
    checkNative(
      ...
      "With "
        ...
        + "select [*NECJ_Set] on columns " + "From [Sales]",
      "Axis #0:\n"
        + "{}\n"
        + "Axis #1:\n"
  ...
  /**
   * Verifies that the presence of calculated member in all the inputs disables
   * native evaluation, even when ExpandNonNative is true.
   */
  ...
  /**
   * Check that if both inputs to NECJ are either AllMember(currentMember,
   * defaultMember are also AllMember) or Calcculated member native CJ is not
   * used.
   */
  public void testExpandCalcMemberInputNECJ() {
    propSaver.set(MondrianProperties.instance().ExpandNonNative, true);
    checkNotNative(1, "With \n"
  ...
  public void testExpandNestedNonNativeInputs() {
    ...
    // is not refreshed for this parameter.
    boolean requestFreshConnection = true;
    checkNative(0, 6, "select " + "NonEmptyCrossJoin("
      + "  NonEmptyCrossJoin([Gender].Children, [Store].Children), "
      + "  [Product].Children) on columns " + "from [Sales]", "Axis #0:\n" + "{}\n"
  ...
  /**
   * Verify that a low value for maxConstraints disables native evaluation, even
   * when ExpandNonNative is true.
   */
  public void testExpandLowMaxConstraints() {
    propSaver.set(MondrianProperties.instance().MaxConstraints, 2);
    propSaver.set(MondrianProperties.instance().ExpandNonNative, true);
    checkNotNative(12, "select NonEmptyCrossJoin("
  ...
  public void testExpandDifferentLevels() {
    propSaver.set(MondrianProperties.instance().ExpandNonNative, true);
    checkNotNative(278, "select NonEmptyCrossJoin("
      + "    Descendants([Customers].[All Customers].[USA].[WA].[Yakima]), "
      + "    [Product].Children) on columns " + "from [Sales]", null);
  }
  ...
  public void testExpandTupleInputs1() {
    ...
    checkNotNative(
      ...
      "with "
        ...
        + "select [NECJ] on columns from [Sales]", "Axis #0:\n" + "{}\n"
        + "Axis #1:\n"
  ...
  public void testEnumLowMaxConstraints() {
    propSaver.set(MondrianProperties.instance().MaxConstraints, 2);
    checkNotNative(12, "with " + "set [All Store Types] as {"
      + "[Store Type].[Deluxe Supermarket], "
      + "[Store Type].[Gourmet Supermarket], " + "[Store Type].[Mid-Size Grocery], "
      + "[Store Type].[Small Grocery], " + "[Store Type].[Supermarket]} "
      + "set [All Products] as {" + "[Product].[Drink], " + "[Product].[Food], "
      + "[Product].[Non-Consumable]} " + "select " + "NonEmptyCrossJoin("
      + "Filter([All Store Types], ([Measures].[Unit Sales] > 10000)), "
      + "[All Products]) on columns " + "from [Sales]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Store Type].[Deluxe Supermarket], [Product].[Drink]}\n"
      + "{[Store Type].[Deluxe Supermarket], [Product].[Food]}\n"
      + "{[Store Type].[Deluxe Supermarket], [Product].[Non-Consumable]}\n"
      + "{[Store Type].[Gourmet Supermarket], [Product].[Drink]}\n"
  ...
  /**
   * Verify that the native evaluation is possible if one input does not contain
   * the All member.
   */
  ...
  public void testCjDescendantsEnumAllOnly() {
    checkNative(9, 9, "select {[Measures].[Unit Sales]} ON COLUMNS, "
      + "NON EMPTY Crossjoin("
  ...
  /**
   * checks that crossjoin returns a modifiable copy from cache because its
  ...
  public void testNativeTopCount() {
    ...
    String query = "select {[Measures].[Store Sales]} on columns,"
      + "  NON EMPTY TopCount("
      + "        CrossJoin([Customers].[All Customers].[USA].children, [Promotions].[Promotion Name].Members), "
  ...
  public void testCmNativeTopCount() {
    ...
    String query = "with member [Measures].[Store Profit Rate] as '([Measures].[Store Sales]-[Measures].[Store Cost])/[Measures].[Store Cost]', format = '#.00%' "
      + "select {[Measures].[Store Sales]} on columns,"
      + "  NON EMPTY TopCount("
      + "        [Customers].[All Customers].[USA].children, "
      + "        3, [Measures].[Store Profit Rate] / 2) ON ROWS" + " from [Sales]";
  ...
  public void testCmInTopCount() {
    checkNotNative(1, "with member [Time].[Time].[Jan] as  "
  ...
  public void testCmInSlicer() {
    checkNotNative(3, "with member [Time].[Time].[Jan] as  "
      + "'Aggregate({[Time].[1998].[Q1].[1], [Time].[1997].[Q1].[1]})'  "
      + "select NON EMPTY {[Measures].[Unit Sales]} ON columns,  "
  ...
  public void testCjMembersMembersMembers() {
    checkNative(67, 67, "select {[Measures].[Store Sales]} on columns,"
      ...
      + " from [Sales] where (" + "  [Store].[USA].[CA].[San Francisco].[Store 14],"
      + "  [Time].[1997].[Q1].[1])");
  }
  ...
  public void testCjMembersWithHideIfBlankLeafAndNoAll() {
    ...
    checkNative(9999, // Don't know why resultLimit needs to be so high.
      67, "select {[Measures].[Store Sales]} on columns," + "  NON EMPTY Crossjoin("
  ...
  public void testCjMembersWithHideIfParentsNameLeaf() {
    ...
    checkNotNative(67, "select {[Measures].[Store Sales]} on columns,"
      + "  NON EMPTY Crossjoin(" + "    Crossjoin("
      + "        [Customers].[Name].Members,"
      + "        [Product Ragged].[Product Name].Members), "
      + "    [Promotions].[Promotion Name].Members) ON rows "
  ...
  public void testCjEnumWithHideIfBlankLeaf() {
    ...
    checkNative(999, 7, "select {[Measures].[Store Sales]} on columns,"
      + "  NON EMPTY Crossjoin(" + "    Crossjoin("
      + "        [Customers].[Name].Members,"
  ...
  /**
   * {} contains members from different levels, this can not be handled by the
   * current native crossjoin.
   */
   ...
   * clause: (1) Use IN list if possible (2) Group members sharing the same
   * parent (3) Only need to compare up to the first unique parent level.
   */
  public void testMultiLevelMemberConstraintNonNullParent() {
    ...
    if (MondrianProperties.instance().UseAggregates.get()
      ...
      necjSqlMySql = necjSqlMySql.replaceAll("sales_fact_1997",
        "agg_c_14_sales_fact_1997");
      necjSqlDerby = necjSqlDerby.replaceAll("sales_fact_1997",
        "agg_c_14_sales_fact_1997");
    }
  ...
  /**
   * Checks that multi-level member list generates compact form of SQL where
   * clause: (1) Use IN list if possible(not possible if there are null values
   * because NULLs in IN lists do not match) (2) Group members sharing the same
   * parent, including parents with NULLs. (3) If parent levels include NULLs,
   * comparision includes any unique level.
   */
  public void testMultiLevelMemberConstraintNullParent() {
    ...
    String necjSqlDerby = "select \"warehouse\".\"wa_address3\", \"warehouse\".\"wa_address2\", \"warehouse\".\"wa_address1\", \"warehouse\".\"warehouse_name\", \"product_class\".\"product_family\" "
    ...
    String necjSqlMySql = "select `warehouse`.`wa_address3` as `c0`, `warehouse`.`wa_address2` as `c1`, `warehouse`.`wa_address1` as `c2`, `warehouse`.`warehouse_name` as `c3`, "
    ...
    TestContext testContext = TestContext.instance().create(dimension, cube, null,
      null, null, null);
   ...
   * because NULLs in IN lists do not match) (2) Group members sharing the same
   * parent, including parents with NULLs. (3) If parent levels include NULLs,
   * comparision includes any unique level. (4) Can handle predicates correctly
   * if the member list contains both NULL and non NULL parent levels.
   */
  public void testMultiLevelMemberConstraintMixedNullNonNullParent() {
    ...
    String necjSqlDerby = "select \"warehouse\".\"warehouse_fax\", \"warehouse\".\"wa_address1\", \"warehouse\".\"warehouse_name\", \"product_class\".\"product_family\" "
      + "from \"warehouse\" as \"warehouse\", \"inventory_fact_1997\" as \"inventory_fact_1997\", \"product\" as \"product\", \"product_class\" as \"product_class\" "
      + "where \"inventory_fact_1997\".\"warehouse_id\" = \"warehouse\".\"warehouse_id\" and \"product\".\"product_class_id\" = \"product_class\".\"product_class_id\" "
      + "and \"inventory_fact_1997\".\"product_id\" = \"product\".\"product_id\" and "
    ...
    String necjSqlMySql = "select `warehouse`.`warehouse_fax` as `c0`, `warehouse`.`wa_address1` as `c1`, "
   ...
   * clause: (1) Use IN list if possible(not possible if there are null values
   * because NULLs in IN lists do not match) (2) Group members sharing the same
   * parent (3) Only need to compare up to the first unique parent level. (4)
   * Can handle predicates correctly if the member list contains both NULL and
   * non NULL child levels.
   */
  public void testMultiLevelMemberConstraintWithMixedNullNonNullChild() {
    ...
    String necjSqlDerby = "select \"warehouse\".\"wa_address3\", \"warehouse\".\"wa_address2\", \"warehouse\".\"warehouse_fax\", \"product_class\".\"product_family\" "
      + "from \"warehouse\" as \"warehouse\", \"inventory_fact_1997\" as \"inventory_fact_1997\", \"product\" as \"product\", \"product_class\" as \"product_class\" "
      + "where \"inventory_fact_1997\".\"warehouse_id\" = \"warehouse\".\"warehouse_id\" and "
    ...
    String necjSqlMySql = "select `warehouse`.`wa_address3` as `c0`, `warehouse`.`wa_address2` as `c1`, `warehouse`.`warehouse_fax` as `c2`, "
   ...
   * "[Store].[All Stores].[USA].[CA].[San Francisco]" it shall not lookup
   * additional members.
   */
  public void testLookupMemberCache() {
    ...
    MemberCacheHelper rcsmrch = ((RolapCubeHierarchy.RolapCubeHierarchyMemberReader) smr)
    ...
    assertTrue("no additional members should be read:"
      + ssmrch.mapKeyToMember.size(), ssmrch.mapKeyToMember.size() <= 5);
    ...
    List<RolapMember> list = ssmrch.mapMemberToChildren.get(ca, scf
      .getMemberChildrenConstraint(null));
   ...
   * the year level is numeric and the constant 'Month' in the WHERE condition
   * is not. Its probably a bug that Mondrian does not take into account
   * [Time].[1997] when looking up [Month].
   */
  ...
  public void testNonEmptyCrossJoinList() {
    ...
    boolean oldEnableNativeNonEmpty = MondrianProperties.instance().EnableNativeNonEmpty
      .get();
    MondrianProperties.instance().EnableNativeNonEmpty.set(false);

    executeQuery("select non empty CrossJoin([Customers].[Name].Members, "
  ...
  /**
   * Make sure that the Crossjoin in [Measures].[CustomerCount] is not evaluated
  ...
  public void testLevelMembers() {
    ...
    List<RolapMember> list = smrch.mapLevelToMembers
      .get((RolapLevel) nameLevel, lmc);
  ...
  /**
   * Testcase for bug 1379068, which causes no children of [Time].[1997].[Q2] to
   * be found, because it incorrectly constrains on the level's key column
  ...
  public void testMemberChildrenNameCol() {
    ...
    TestCase c = new TestCase(3, 1, "select " + " {[Measures].[Count]} ON columns,"
      + " {[Time].[1997].[Q2].[April]} on rows " + "from [HR]");
    c.run();
  }

  /**
   * When a member is expanded in JPivot with mulitple hierarchies visible it
   * generates a <code>CrossJoin({[member from left hierarchy]}, [member to
   * expand].Children)</code>
   * 
   * <p>
   * This should behave the same as if <code>[member from left
  ...
  /**
   * Ensures that NON EMPTY Descendants is optimized. Ensures that Descendants
   * as a side effect collects MemberChildren that may be looked up in the
   * cache.
   */
  public void testNonEmptyDescendants() {
    ...
    RolapMember peggy = (RolapMember) result.getAxes()[1].getPositions().get(1).get(
      0);
  ...
  public void testNotNativeVirtualCubeCrossJoin1() {
    ...
    propSaver.set(MondrianProperties.instance().AlertNativeEvaluationUnsupported,
      "ERROR");
    // native cross join cannot be used due to AllMembers
    checkNotNative(3, "select " + "{[Measures].AllMembers} on columns, "
      + "non empty crossjoin([Product].[All Products].children, "
      + "[Store].[All Stores].children) on rows " + "from [Warehouse and Sales]");
  }
  ...
  public void testNotNativeVirtualCubeCrossJoinUnsupported() {
    ...
    String mdx = "select " + "{[Measures].AllMembers} on columns, "
      + "NonEmptyCrossJoin([Product].[All Products].children, "
      + "[Store].[All Stores].children) on rows " + "from [Warehouse and Sales]";
    ...
    int nEvents = countFilteredEvents(events, org.apache.log4j.Level.ERROR,
      expectedMessage);
    ...
    nEvents = countFilteredEvents(events, org.apache.log4j.Level.WARN,
      expectedMessage);
  ...
  private int countFilteredEvents(List<LoggingEvent> events,
    org.apache.log4j.Level level, String pattern) {
  ...
  public void testVirtualCubeCrossJoinCalculatedMember1() {
    ...
    checkNative(18, 3, "WITH MEMBER [Measures].[Total Cost] as "
      + "'[Measures].[Store Cost] + [Measures].[Warehouse Cost]' " + "select "
  ...
  public void testNotNativeVirtualCubeCrossJoinCalculatedMember() {
    ...
    // calculated member
    checkNotNative(3, "WITH MEMBER [Measures].[CurrMember] as "
      + "'[Measures].CurrentMember' " + "select "
  ...
  public void testCjUnionEnumCalcMembers() {
    ...
    checkNotNative(46, "with "
      ...
      + "{[Measures].[Unit Sales]} on columns, " + "non empty union (Crossjoin("
      + "    [Product].[Product Department].Members, "
      + "    {[Education Level].[*SUBTOTAL_MEMBER_SEL~AVG]}), " + "crossjoin("
      + "    [Product].[Product Department].Members, "
  ...
  public void testCrossJoinSetWithDifferentParents() {
    ...
    checkNative(5, 5, "select " + "{[Measures].[Unit Sales]} on columns, "
  ...
  public void testCrossJoinSetWithCrossProdMembers() {
    ...
    checkNative(15, 15, "select " + "{[Measures].[Unit Sales]} on columns, "
      + "NonEmptyCrossJoin(" + EDUCATION_LEVEL_LEVEL + ".Members, "
  ...
  public void testCrossJoinSetWithSameParent() {
    ...
    checkNative(10, 10, "select " + "{[Measures].[Unit Sales]} on columns, "
      + "NonEmptyCrossJoin(" + EDUCATION_LEVEL_LEVEL + ".Members, "
      + "{[Store].[All Stores].[USA].[CA].[Beverly Hills], "
      + "[Store].[All Stores].[USA].[CA].[San Francisco]}) " + "on rows from Sales");
  }
  ...
  public void testCrossJoinEvaluatorContext2() {
    ...
    assertQueryReturns(
      "With Set [*NATIVE_CJ_SET] as "
        ...
        + "Select Union " + "(CrossJoin " + "(Filter "
        + "(Generate([*NATIVE_CJ_SET], {([Time].[Time].CurrentMember)}), "
        + "Not IsEmpty ([Measures].[Unit Sales])), "
        + "[*GENERATED_MEMBERS_Measures]), " + "CrossJoin " + "(Filter "
        + "({[Time].[*SUBTOTAL_MEMBER_SEL~SUM]}, "
        + "Not IsEmpty ([Measures].[Unit Sales])), "
        + "[*GENERATED_MEMBERS_Measures])) on columns, " + "Non Empty Union "
        + "(Filter " + "(Filter " + "(Generate([*NATIVE_CJ_SET], "
        + "{([Store].CurrentMember)}), " + "([Measures].[Unit Sales], "
        + "[Time].[*SUBTOTAL_MEMBER_SEL~SUM]) > 0.0), "
        + "Not IsEmpty ([Measures].[Unit Sales])), " + "Filter("
        + "{[Store].[*SUBTOTAL_MEMBER_SEL~SUM]}, "
        + "Not IsEmpty ([Measures].[Unit Sales]))) on rows " + "From [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testVCNativeCJWithIsEmptyOnMeasure() {
    ...
    assertQueryReturns(
      "with "
        ...
        + "member [Gender].FILTER2 as Aggregate(GM_GENDER) " + "select "
        + "Filter(GM_PRODUCT, Not IsEmpty([Measures].[Unit Sales])) on rows, "
        + "GM_MEASURE on columns " + "from [Warehouse and Sales] "
        + "where ([Education Level].FILTER1, [Gender].FILTER2)", "Axis #0:\n"
  ...
  public void testVCNativeCJWithTopPercent() {
    ...
    checkNative(92, 1,
      "select {topPercent(nonemptycrossjoin([Product].[Product Department].members, "
        ...
        + "{[Measures].[Store Sales]} on rows from " + "[Warehouse and Sales]");
  }
  ...
  /**
   * Test for bug #1696772 Modified which calculations are tested for non
   * native, non empty joins
   */
  ...
  private void checkIndependentSlicerMemberNative(boolean useNative) {
    ...
    context.assertQueryReturns(
      "with set [p] as '[Product].[Product Family].members' "
        ...
        + "where ([Store].[Store Country].[Mexico])", "Axis #0:\n"
        + "{[Store].[Mexico]}\n" + "Axis #1:\n" + "{[Product].[Drink]}\n"
  ...
  public void testDependentSlicerMemberNonNative() {
    ...
    context.assertQueryReturns(
      "with set [p] as '[Product].[Product Family].members' "
        ...
        + "set [ne] as 'nonemptycrossjoin([p],[s])' "
        + "set [nep] as 'Generate([ne],{[Product].CurrentMember})' "
        + "select [nep] on columns from sales " + "where ([Time].[1998])",
      "Axis #0:\n" + "{[Time].[1998]}\n" + "Axis #1:\n");
  }
   ...
   * {@link MondrianProperties#EnableNativeNonEmpty native} is enabled. Testcase
   * for bug 1722959, "NON EMPTY Level.MEMBERS fails if nonempty.enable=false"
   */
  public void testNonEmptyLevelMembers() {
    boolean currentNativeNonEmpty = MondrianProperties.instance().EnableNativeNonEmpty
      .get();
    boolean currentNonEmptyOnAllAxis = MondrianProperties.instance().EnableNonEmptyOnAllAxis
      .get();
    try {
      MondrianProperties.instance().EnableNativeNonEmpty.set(false);
      MondrianProperties.instance().EnableNonEmptyOnAllAxis.set(true);
      assertQueryReturns("WITH MEMBER [Measures].[One] AS '1' " + "SELECT "
        + "NON EMPTY {[Measures].[One], [Measures].[Store Sales]} ON rows, "
        + "NON EMPTY [Store].[Store State].MEMBERS on columns " + "FROM sales",
        "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[Canada].[BC]}\n"
   ...
   * MONDRIAN-412, "NON EMPTY and Filter() breaking aggregate calculations"</a>.
   */
  ...
  public void testLeafMembersOfParentChildDimensionAreNativelyEvaluated() {
    final String query = "SELECT"
      ...
      + "}," + "[Store].[Store Name].members" + ") on 0 from hr";
  ...
  public void testNativeWithOverriddenNullMemberRepAndNullConstraint() {
    ...
    propSaver.set(MondrianProperties.instance().NullMemberRepresentation,
      "~Missing ");
   ...
   * MONDRIAN-321, "CrossJoin has no nulls when EnableNativeNonEmpty=true"</a>.
   */
  ...
  public void testNativeCrossjoinWillConstrainUsingArgsFromAllAxes() {
    String mdx = "select "
      ...
      + " from [Warehouse and Sales]";
    SqlPattern accessPattern = new SqlPattern(Dialect.DatabaseProduct.ACCESS,
      "select `time_by_day`.`the_year` as `c0`, "
        + "`promotion`.`promotion_name` as `c1` "
        + "from `time_by_day` as `time_by_day`, "
        + "`sales_fact_1997` as `sales_fact_1997`, "
        + "`promotion` as `promotion`, " + "`customer` as `customer` " + "where "
  ...
  public void testLevelMembersWillConstrainUsingArgsFromAllAxes() {
    ...
    SqlPattern accessPattern = new SqlPattern(Dialect.DatabaseProduct.ACCESS,
      "select `promotion`.`promotion_name` as `c0` "
        ...
        + "and `sales_fact_1997`.`customer_id` = `customer`.`customer_id` "
        + "and (`customer`.`gender` = 'F') "
        + "group by `promotion`.`promotion_name` " + "order by 1 ASC", 357);
    SqlPattern oraclePattern = new SqlPattern(
      Dialect.DatabaseProduct.ORACLE,
      "select \"promotion\".\"promotion_name\" as \"c0\" "
        ...
        + "and \"sales_fact_1997\".\"customer_id\" = \"customer\".\"customer_id\" "
        + "and (\"customer\".\"gender\" = 'F') "
        + "group by \"promotion\".\"promotion_name\" " + "order by 1 ASC", 347);
    assertQuerySql(mdx, new SqlPattern[] { oraclePattern, accessPattern });
  }
  ...
  public void testNativeCrossjoinWillExpandLagInNamedSet() {
    String mdx = "with set [blah] as '{[Gender].lastChild.lag(1),[Gender].[M]}' "
      + "select "
  ...
  public void testConstrainedMeasureGetsOptimized() {
    String mdx = "with member [Measures].[unit sales Male] as '([Measures].[Unit Sales],[Gender].[Gender].[M])' "
      ...
      + "non empty [Customers].[name].members on 1 " + "from Sales";
    final String sqlOracle = MondrianProperties.instance().UseAggregates.get() ? "select \"customer\".\"country\" as \"c0\","
    ...
    assertQuerySql(mdx, new SqlPattern[] { new SqlPattern(
      Dialect.DatabaseProduct.ORACLE, sqlOracle, sqlOracle.length()) });
  }
  ...
  public void testNestedMeasureConstraintsGetOptimized() {
    ...
    SqlPattern pattern = new SqlPattern(Dialect.DatabaseProduct.ORACLE, sqlOracle,
      sqlOracle.length());
    assertQuerySql(mdx, new SqlPattern[] { pattern });
  }
  ...
  public void testNonUniformNestedMeasureConstraintsGetOptimized() {
    ...
    final SqlPattern pattern = new SqlPattern(
      ...
      "select \"customer\".\"country\" as \"c0\", "
        ...
        + " \"customer\".\"state_province\" ASC NULLS LAST,"
        + " \"customer\".\"city\" ASC NULLS LAST, "
        + "\"fname\" || ' ' || \"lname\" ASC NULLS LAST", 852);
    assertQuerySql(mdx, new SqlPattern[] { pattern });
  }
  ...
  public void testNonUniformConstraintsAreNotUsedForOptimization() {
    ...
    final SqlPattern pattern = new SqlPattern(Dialect.DatabaseProduct.ORACLE,
      sqlOracle, sqlOracle.length());
    assertQuerySqlOrNot(getTestContext(), mdx, new SqlPattern[] { pattern }, true,
      false, true);
  }
  ...
  public void testMeasureConstraintsInACrossjoinHaveCorrectResults() {
    ...
    String mdx = "with "
      ...
      + "      {[Measures].[aa], [Measures].[bb]}) on columns," + "  non empty "
      + "  [Marital Status].[Marital Status].members on rows" + " from sales";
  ...
  public void testContextAtAllWorksWithConstraint() {
    ...
    String mdx = " select " + " NON EMPTY {[Measures].[Unit Sales]} ON COLUMNS, "
      + " NON EMPTY {[Gender].[Gender].Members} ON ROWS " + " from [onlyGender] ";
  ...
  /***
   * Before the fix this test would throw an IndexOutOfBounds exception in
   * SqlConstraintUtils.removeDefaultMembers. The method assumed that the first
   * member in the list would exist and be a measure. But, when the default
   * measure is calculated, it would have already been removed from the list by
   * removeCalculatedMembers, and thus the assumption was wrong.
   */
    ...
    final TestContext context = TestContext
      ...
      .withSchema(
        "<Schema name=\"FoodMart\">"
          ...
          + "  </VirtualCube>" + "</Schema>");
    context.assertQueryReturns("select " + " [Measures].[Unit Sales] on COLUMNS, "
      + " NON EMPTY {[Store].[Store State].Members} ON ROWS " + " from [virtual] ",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
        + "Axis #2:\n" + "{[Store].[USA].[CA]}\n" + "{[Store].[USA].[OR]}\n"
        + "{[Store].[USA].[WA]}\n" + "Row #0: 74,748\n" + "Row #1: 67,659\n"
        + "Row #2: 124,366\n");
  ...
  public void testNonEmptyCJWithMultiPositionSlicer() {
    final String mdx = "select NON EMPTY NonEmptyCrossJoin([Measures].[Sales Count], [Store].[USA].Children) ON COLUMNS, "
    ...
    // Get a fresh connection; Otherwise the mondrian property setting
    // is not refreshed for this parameter.
    checkNative(0, 5, mdx, expected, true);
  }

  void clearAndHardenCache(MemberCacheHelper helper) {
    helper.mapLevelToMembers
      .setCache(new HardSmartCache<Pair<RolapLevel, Object>, List<RolapMember>>());
    helper.mapMemberToChildren
      .setCache(new HardSmartCache<Pair<RolapMember, Object>, List<RolapMember>>());
  ...
  SmartMemberReader getSmartMemberReader(Connection con, String hierName) {
    ...
    RolapHierarchy hierarchy = (RolapHierarchy) cube.lookupHierarchy(new Id.Segment(
      hierName, Id.Quoting.UNQUOTED), false);
  ...
  SmartMemberReader getSharedSmartMemberReader(Connection con, String hierName) {
    ...
    assertNotNull(hierarchy);
    return (SmartMemberReader) hierarchy.getRolapHierarchy().createMemberReader(
  ...
  public void testFilterChildlessSnowflakeMembers() {
    propSaver.set(MondrianProperties.instance().FilterChildlessSnowflakeMembers,
      false);
    SqlPattern[] patterns = { new SqlPattern(Dialect.DatabaseProduct.MYSQL,
      "select `product_class`.`product_family` as `c0` "
        ...
        + " `product_class`.`product_family` ASC", null) };
    final TestContext context = getTestContext().withFreshConnection();
    try {
