/*
// $Id: //open/mondrian/testsrc/main/mondrian/olap/CustomizedParserTest.java#19 $
 ...
 * @author Rushan Chen
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/CustomizedParserTest.
 *          java#19 $
 */
public class CustomizedParserTest extends FoodMartTestCase {
  ...
  CustomizedFunctionTable getCustomizedFunctionTable(Set<String> funNameSet) {
    ...
    CustomizedFunctionTable cftab = new CustomizedFunctionTable(funNameSet,
      specialFunctions);
    cftab.init();
    return cftab;
  }
  ...
  private Query getParsedQueryForExpr(CustomizedFunctionTable cftab, String expr,
    ...
    final Statement statement = connectionBase.createDummyStatement();
    try {
      return (Query) connectionBase.parseStatement(statement, mdx, cftab,
        strictValidation);
    } finally {
      statement.close();
    }
  ...
  public void testAddition() {
    ...
    try {
      Query q = getParsedQueryForExpr(cftab,
  ...
  private void testMissingObject(boolean strictValidation) {
    ...
    try {
      Query q = getParsedQueryForExpr(cftab,
        "'[Measures].[Store Cost] + [Measures].[Unit Salese]'", strictValidation);
      q.resolve(q.createValidator(cftab, true));
   ...
   * 
   * <p>
   * The following expression passes parsing and validation. Its computation is
   * strange: the result is as if the measure is defined as ([Measures].[Store
   * Cost] + [Measures].[Store Cost])
   */
