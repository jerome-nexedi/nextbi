/*
// $Id: //open/mondrian/testsrc/main/mondrian/util/ObjectPoolTest.java#11 $
 ...
 * 
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/util/ObjectPoolTest.java#11 $
...
public class ObjectPoolTest extends TestCase {
  ...
  /**
   * Tests ObjectPools containing large numbers of integer and string keys, and
   * makes sure they return the same results as HashSet. Optionally measures
   * performance.
   */
  ...
  private void checkLargeMulti(int entryCount, double density, int retrieveCount,
    int expectedDistinct, int expectedHits) {
    checkLarge(true, true, entryCount, density, retrieveCount, expectedDistinct,
      expectedHits);
    checkLarge(false, true, entryCount, density, retrieveCount, expectedDistinct,
      expectedHits);
    checkLarge(false, true, entryCount, density, retrieveCount, expectedDistinct,
      expectedHits);
    checkLarge(false, false, entryCount, density, retrieveCount, expectedDistinct,
      expectedHits);
  }

  private void checkLarge(boolean usePool, boolean intKey, int entryCount,
    double density, int retrieveCount, int expectedDistinct, int expectedHits) {
    ...
    int distinctCount = 0;
    final String longString = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyy";
    for (int i = 0; i < entryCount; i++) {
      final Object key = intKey ? random.nextInt(space) : longString
    ...
    if (print) {
      System.out.println("Using " + (usePool ? "ObjectPool" : "HashSet")
        + ", density=" + density + ", " + distinctCount + " distinct entries, "
        + hitCount + " hits");
      System.out.println(entryCount + " adds took " + (t2 - t1) + " milliseconds");
      System.out
        .println(retrieveCount + " gets took " + (t3 - t2) + " milliseconds");
    }
