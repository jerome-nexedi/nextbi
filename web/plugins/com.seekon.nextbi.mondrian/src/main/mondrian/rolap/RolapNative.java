...
/**
 * A factory for {@link mondrian.olap.NativeEvaluator}. If the instance returns
 * null, then the interpreter must compute the result itself. If it returns a
 * NativeEvaluator the interpreter may choose to let the NativeEvaluator compute
 * the result.
 * 
 * <p>
 ...
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapNative.java#9 $
 */
public abstract class RolapNative {
   ...
   */
  abstract NativeEvaluator createEvaluator(RolapEvaluator evaluator, FunDef fun,
    Exp[] args);
  ...
  /**
   * Sets whether to use hard caching for testing. When using soft references,
   * we can not test caching because things may be garbage collected during the
   * tests.
   */
