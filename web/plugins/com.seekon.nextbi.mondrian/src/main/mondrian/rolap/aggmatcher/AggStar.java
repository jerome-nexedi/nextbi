/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggStar.java#45 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggStar.java#45 $
 */
public class AggStar {
  ...
  public static AggStar makeAggStar(final RolapStar star,
    final JdbcSchema.Table dbTable, final MessageRecorder msgRecorder,
    ...
    // 1. load fact count
    for (Iterator<JdbcSchema.Table.Column.Usage> it = dbTable
      .getColumnUsages(JdbcSchema.UsageType.FACT_COUNT); it.hasNext();) {
    ...
    // 2. load measures
    for (Iterator<JdbcSchema.Table.Column.Usage> it = dbTable
      .getColumnUsages(JdbcSchema.UsageType.MEASURE); it.hasNext();) {
      JdbcSchema.Table.Column.Usage usage = it.next();
    ...
    // 3. load foreign keys
    for (Iterator<JdbcSchema.Table.Column.Usage> it = dbTable
      .getColumnUsages(JdbcSchema.UsageType.FOREIGN_KEY); it.hasNext();) {
      JdbcSchema.Table.Column.Usage usage = it.next();
    ...
    // 4. load levels
    for (Iterator<JdbcSchema.Table.Column.Usage> it = dbTable
      .getColumnUsages(JdbcSchema.UsageType.LEVEL); it.hasNext();) {
      JdbcSchema.Table.Column.Usage usage = it.next();
    ...
    for (FactTable.Measure measure : aggStarFactTable.measures) {
      if (measure.aggregator.isDistinct()
        ...
        setLevelBits(measure.rollableLevelBitKey, aggStarFactTable,
          (MondrianDef.Column) measure.argument, star.getFactTable());
      }
    }
  ...
  /**
   * Sets bits in the bitmap for all levels reachable from a given table. This
   * allows us to compute which levels can be safely aggregated away when
   * rolling up a distinct-count measure.
   * 
   * <p>
   * For example, when rolling up a measure based on 'COUNT(DISTINCT
   * customer_id)', all levels in the Customers table and the Regions table
   * reached via the Customers table can be rolled up. So method sets the bit
   * for all of these levels.
   * 
   * @param bitKey
   *          Bit key of levels which can be rolled up
   * @param aggTable
   *          Fact or dimension table which is the start point for the
   *          navigation
   * @param column
   *          Foreign-key column which constraints which dimension
   * @param table
   */
  private static void setLevelBits(final BitKey bitKey, Table aggTable,
    MondrianDef.Column column, RolapStar.Table table) {
  ...
  private static void collectLevels(List<Table.Level> levelList, Table table,
    MondrianDef.Column joinColumn) {
    if (joinColumn == null) {
      levelList.addAll(table.levels);
    }
    for (Table dimTable : table.children) {
      if (joinColumn != null && !dimTable.getJoinCondition().left.equals(joinColumn)) {
        continue;
      }
  ...
  /**
   * This BitKey is for all of the columns in the AggStar (levels and measures).
   */
  ...
  /**
   * Returns a measure of the IO cost of querying this table. It can be either
   * the row count or the row count times the size of a row. If the property
   * {@link MondrianProperties#ChooseAggregateByVolume} is true, then volume is
   * returned, otherwise row count.
   */
  public int getSize() {
    return MondrianProperties.instance().ChooseAggregateByVolume.get() ? getFactTable()
      .getVolume()
   ...
   * <code>levelBitKey</code> parameter and if this AggStar's measure BitKey is
   * a super set (proper or not) of the <code>measureBitKey</code> parameter.
   */
  public boolean select(final BitKey levelBitKey, final BitKey coreLevelBitKey,
  ...
  /**
   * Get the Measure at the given bit position or return null. Note that there
   * is no check that the bit position is within the range of the array of
   * columns. Nor is there a check that the column type at that position is a
   * Measure.
   * 
   * @param bitPos
  ...
  public AggStar.FactTable.Measure lookupMeasure(final int bitPos) {
    ...
    return (column instanceof AggStar.FactTable.Measure) ? (AggStar.FactTable.Measure) column
      : null;
  }

  /**
   * Get the Level at the given bit position or return null. Note that there is
   * no check that the bit position is within the range of the array of columns.
  ...
  public AggStar.Table.Level lookupLevel(final int bitPos) {
    AggStar.Table.Column column = lookupColumn(bitPos);
    return (column instanceof AggStar.FactTable.Level) ? (AggStar.FactTable.Level) column
      : null;
  }

  /**
   * Get the Column at the bit position. Note that there is no check that the
   * bit position is within the range of the array of columns.
   */
  ...
  private static final Logger JOIN_CONDITION_LOGGER = Logger
    .getLogger(AggStar.Table.JoinCondition.class);

  /**
   * Base Table class for the FactTable and DimTable classes. This class
   * parallels the RolapStar.Table class.
   * 
   */
  public abstract class Table {

    /**
     * The query join condition between a base table and this table (the table
     * that owns the join condition).
     */
    public class JoinCondition {
      ...
      public void print(final PrintWriter pw, final String prefix) {
        ...
        if (left instanceof MondrianDef.Column) {
          ...
          mondrian.rolap.RolapStar.Column col = getTable().getAggStar().getStar()
            .getFactTable().lookupColumn(c.name);
          if (col != null) {
            pw.print(" (");
    ...
    public class Column {
       ...
       * columns making the drill-through queries easier for humans to
       * understand.
       */
      ...
      protected Column(final String name, final MondrianDef.Expression expression,
        final Dialect.Datatype datatype, final int bitPosition) {
        this.name = name;
      ...
      /**
       * Get the bit possition associted with this column. This has the same
       * value as this column's RolapStar.Column.
       */
    ...
    /**
     * This class is used for holding foreign key columns. Both DimTables and
     * FactTables can have Level columns.
     */
    final class ForeignKey extends Column {

      private ForeignKey(final String name, final MondrianDef.Expression expression,
        final Dialect.Datatype datatype, final int bitPosition) {
    ...
    /**
     * This class is used for holding dimension level information. Both
     * DimTables and FactTables can have Level columns.
     */
    final class Level extends Column {
      private final RolapStar.Column starColumn;

      private Level(final String name, final MondrianDef.Expression expression,
        final int bitPosition, RolapStar.Column starColumn) {
    ...
    /**
     * Return true if this table has a parent table (FactTable instances do not
     * have parent tables, all other do).
     */
    ...
    /**
     * Return true if this table has a join condition (only DimTables have join
     * conditions, FactTable instances do not).
     */
     ...
     * {@link AggStar.DimTable} as well as converting all columns and child
     * tables. If the rightJoinConditionColumnName parameter is null, then the
     * table's namd and the rTable parameter's condition left condition's column
     * name are used to form the join condition's left expression.
     */
    protected AggStar.DimTable convertTable(final RolapStar.Table rTable,
      ...
      if (rightJoinConditionColumnName != null) {
        left = new MondrianDef.Column(getName(), rightJoinConditionColumnName);
      } else {
        if (rleft instanceof MondrianDef.Column) {
          ...
          /*
           * // RME TODO can we catch this during validation String msg =
           * mres.BadRolapStarLeftJoinCondition.str( "AggStar.Table",
      ...
      // method).
      mondrian.rolap.RolapStar.Column col = getAggStar().getStar().getFactTable()
    ...
    /**
     * Convert a RolapStar.Table table's columns into AggStar.Table.Level
     * columns.
     * 
     * @param rTable
     */
    ...
    /**
     * Convert the child tables of a RolapStar.Table into child AggStar.DimTable
     * tables.
     * 
     * @param rTable
     */
     ...
     * @param joinToParent
     */
    public void addToFrom(final SqlQuery query, final boolean failIfExists,
  ...
  public class FactTable extends Table {
    ...
    public class Measure extends Table.Column {
      ...
      /**
       * For distinct-count measures, contains a bitKey of levels which it is OK
       * to roll up. For regular measures, this is empty, since all levels can
       * be rolled up.
       */
      private final BitKey rollableLevelBitKey;

      private Measure(final String name, final MondrianDef.Expression expression,
        final Dialect.Datatype datatype, final int bitPosition,
        final RolapAggregator aggregator, final MondrianDef.Expression argument) {
        super(name, expression, datatype, bitPosition);
        this.aggregator = aggregator;
        this.argument = argument;
        assert (argument != null) == aggregator.isDistinct();
        this.rollableLevelBitKey = BitKey.Factory.makeBitKey(star.getColumnCount());
      ...
      /**
       * Returns a <code>BitKey</code> of the levels which can be safely rolled
       * up. (For distinct-count measures, most can't.)
       */
       ...
       * For example, a "COUNT" and "DISTINCT COUNT" measures rollup using
       * "SUM".
       */
      /*
       * public String generateRollupString(SqlQuery query) { String expr =
       * generateExprString(query); final Aggregator rollup =
       * getAggregator().getRollup(); return ((RolapAggregator)
       * rollup).getExpression(expr); }
       */
      /*
       * public String generateRollupString(SqlQuery query) { String expr =
       * generateExprString(query); // final Aggregator rollup =
       * getAggregator().getRollup(); Aggregator rollup =
       * (getAggregator().isDistinct()) ?
       * getAggregator().getNonDistinctAggregator() :
       * getAggregator().getRollup();
       * 
       * String s = ((RolapAggregator) rollup).getExpression(expr); return s; }
       */
      public String generateRollupString(SqlQuery query) {
        ...
        if (fkbk.get(getBitPosition())) {
          rollup = (getAggregator().isDistinct()) ? getAggregator()
            .getNonDistinctAggregator() : getAggregator().getRollup();
        } else {
    ...
    FactTable(final JdbcSchema.Table aggTable) {
      this(aggTable.getName(), aggTable.table, aggTable.getTotalColumnSize(),
        aggTable.getNumberOfRows());
    }

    FactTable(final String name, final MondrianDef.Relation relation,
      final int totalColumnSize, final int numberOfRows) {
      super(name, relation);
    ...
    private void loadForeignKey(final JdbcSchema.Table.Column.Usage usage) {
      if (usage.rTable != null) {
        ...
        MondrianDef.Expression expression = new MondrianDef.Column(getName(), name);
        Dialect.Datatype datatype = column.getDatatype();
        RolapStar.Column rColumn = usage.rColumn;
        if (rColumn == null) {
          getLogger().warn(
            "loadForeignKey: for column " + name + ", rColumn == null");
        } else {
          int bitPosition = rColumn.getBitPosition();
          ForeignKey c = new ForeignKey(symbolicName, expression, datatype,
            bitPosition);
    ...
    private void loadMeasure(final JdbcSchema.Table.Column.Usage usage) {
      ...
      Measure aggMeasure = new Measure(symbolicName, expression, datatype,
        bitPosition, aggregator, argument);
    ...
    private void loadFactCount(final JdbcSchema.Table.Column.Usage usage) {
      ...
      Column aggColumn = new Column(symbolicName, expression, datatype, bitPosition);
    ...
    private void loadLevel(final JdbcSchema.Table.Column.Usage usage) {
      ...
      MondrianDef.Expression expression = new MondrianDef.Column(getName(),
        usage.levelColumnName);
      int bitPosition = usage.rColumn.getBitPosition();
      Level level = new Level(name, expression, bitPosition, usage.rColumn);
      addLevel(level);
    }
    ...
    private void makeNumberOfRows() {
      ...
      SqlStatement stmt = RolapUtil.executeQuery(dataSource, query.toString(),
        new Locus(Execution.NONE, "AggStar.FactTable.makeNumberOfRows",
      ...
      try {
        ...
        if (resultSet.next()) {
          ...
          getLogger().warn(
            mres.SqlQueryFailed.str("AggStar.FactTable.makeNumberOfRows", query
              .toString()));
  ...
  public class DimTable extends Table {
    ...
    DimTable(final Table parent, final String name,
      final MondrianDef.Relation relation, final JoinCondition joinCondition) {
      super(name, relation);
      this.parent = parent;
    ...
    /**
     * Add all of this Table's columns to the list parameter and then add all
     * child table columns.
     * 
     * @param list
     */
  ...
  public void print(final PrintWriter pw, final String prefix) {
    ...
    for (AggStar.Table.Column column : getFactTable().getColumns()) {
      pw.print("    ");
      pw.println(column);
    }
