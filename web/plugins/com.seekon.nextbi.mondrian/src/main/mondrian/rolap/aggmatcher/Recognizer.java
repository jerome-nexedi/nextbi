/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/Recognizer.java#28 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/Recognizer.java
 *          #28 $
 */
abstract class Recognizer {
  ...
  protected Recognizer(final RolapStar star, final JdbcSchema.Table dbFactTable,
   ...
   * <p>
   * Then the measures are checked. First the specified (defined, explicit)
   * measures are all determined. There must be at least one such measure. This
   * if followed by checking for implied measures (e.g., if base fact table as
   * both sum and average of a column and the aggregate has a sum measure, the
   * there is an implied average measure in the aggregate).
   * <p>
  ...
  protected void makeIgnore(final JdbcSchema.Table.Column aggColumn) {
    JdbcSchema.Table.Column.Usage usage = aggColumn
      .newUsage(JdbcSchema.UsageType.IGNORE);
  ...
  protected void checkFactCount() {
    ...
    try {
      ...
      for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
        ...
        if (factCountMatcher.matches(aggColumn.getName())) {
          if (aggColumn.getDatatype().isNumeric()) {
            ...
            String msg = mres.NonNumericFactCountColumn.str(aggTable.getName(),
              dbFactTable.getName(), aggColumn.getName(), aggColumn.getTypeName());
      ...
      if (nosOfFactCounts == 0) {
        String msg = mres.NoFactCountColumns.str(aggTable.getName(), dbFactTable
          .getName());
        msgRecorder.reportError(msg);
        ...
        String msg = mres.TooManyFactCountColumns.str(aggTable.getName(),
          dbFactTable.getName(), nosOfFactCounts);
        msgRecorder.reportError(msg);
  ...
  protected void makeFactCount(final JdbcSchema.Table.Column aggColumn) {
    JdbcSchema.Table.Column.Usage usage = aggColumn
      .newUsage(JdbcSchema.UsageType.FACT_COUNT);
  ...
  protected void checkNosMeasures(int nosMeasures) {
    ...
    try {
      if (nosMeasures == 0) {
        String msg = mres.NoMeasureColumns.str(aggTable.getName(), dbFactTable
          .getName());
        msgRecorder.reportError(msg);
   ...
   * either a sum or average, the other measure is implied and can be generated
   * from the measure and the fact_count column.
   * <p>
   * For each column in the fact table, get its measure usages. If there is both
   * an average and sum aggregator associated with the column, then iterator
   * over all of the column usage of type measure of the aggregator table. If
   * only one aggregate column usage measure is found and this RolapStar.Measure
   * measure instance variable is the same as the the fact table's usage's
   * instance variable, then the other measure is implied and the measure is
   * created for the aggregate table.
   */
  protected void generateImpliedMeasures() {
    for (JdbcSchema.Table.Column factColumn : aggTable.getColumns()) {
      ...
      for (Iterator<JdbcSchema.Table.Column.Usage> mit = factColumn
        .getUsages(JdbcSchema.UsageType.MEASURE); mit.hasNext();) {
      ...
      if (avgFactUsage != null && sumFactUsage != null) {
        ...
        int seenCount = 0;
        for (Iterator<JdbcSchema.Table.Column.Usage> mit = aggTable
          .getColumnUsages(JdbcSchema.UsageType.MEASURE); mit.hasNext();) {
  ...
  /**
   * Here we have the fact usage of either sum or avg and an aggregate usage of
   * the opposite type. We wish to make a new aggregate usage based on the
   * existing usage's column of the same type as the fact usage.
   * 
   * @param factUsage
   *          fact usage
   * @param aggSiblingUsage
   *          existing sibling usage
   */
  protected void makeMeasure(final JdbcSchema.Table.Column.Usage factUsage,
    ...
    JdbcSchema.Table.Column.Usage aggUsage = aggColumn
      .newUsage(JdbcSchema.UsageType.MEASURE);

    aggUsage.setSymbolicName(factUsage.getSymbolicName());
    RolapAggregator ra = convertAggregator(aggUsage, factUsage.getAggregator(),
  ...
  /**
   * Creates an aggregate table column measure usage from a fact table column
   * measure usage.
   * 
   * @param factUsage
   * @param aggColumn
  ...
  /**
   * This method determine how may aggregate table column's match the fact table
   * foreign key column return in the number matched. For each matching column a
   * foreign key usage is created.
   */
   ...
   * aggregate table columns match that column usage. If there is more than one
   * match, then that is an error. If there were no matches, then the foreign
   * key usage is added to the list of fact column foreign key that were not in
   * the aggregate table. This list is returned by this method.
   * <p>
  ...
  protected List<JdbcSchema.Table.Column.Usage> checkForeignKeys() {
    ...
    try {
      List<JdbcSchema.Table.Column.Usage> notSeenForeignKeys = Collections
        .emptyList();

      for (Iterator<JdbcSchema.Table.Column.Usage> it = dbFactTable
        ...
        if (matchCount > 1) {
          String msg = mres.TooManyMatchingForeignKeyColumns.str(aggTable.getName(),
            dbFactTable.getName(), matchCount, factUsage.getColumn().getName());
          msgRecorder.reportError(msg);
   ...
   * <p>
   * Here, we do not start from the fact table, we iterator over each cube. For
   * each of the cube's dimensions, the dimension's hirarchies are iterated
   * over. In turn, each hierarchy's usage is iterated over. if the hierarchy's
   * usage's foreign key is not in the list of not seen foreign keys (the
   * notSeenForeignKeys parameter), then that hierarchy is not considered. If
   * the hierarchy's usage's foreign key is in the not seen list, then starting
   * with the hierarchy's top level, it is determined if the combination of
   * hierarchy, hierarchy usage, and level matches an aggregated table column.
   * If so, then a level usage is created for that column and the hierarchy's
   * next level is considered and so on until a for a level an aggregate table
   * column does not match. Then we continue iterating over the hierarchy
   * usages.
   * <p>
   * This check is different. The others mine the fact table usages. This looks
   * through the fact table's cubes' dimension, hierarchy, hiearchy usages,
   * levels to match up symbolic names for levels. The other checks match on
   * "physical" characteristics, the column name; this matches on "logical"
   * characteristics.
   * <p>
   * Note: Levels should not be created for foreign keys that WERE seen.
   * Currently, this is NOT checked explicitly. For the explicit rules any extra
   * columns MUST ge declared ignored or one gets an error.
   * 
        ...
        for (Hierarchy hierarchy : hierarchies) {
          ...
          for (HierarchyUsage hierarchyUsage : hierarchyUsages) {
            ...
            boolean b = foreignKey == null
              || inNotSeenForeignKeys(foreignKey, notSeenForeignKeys);
            if (!b) {
            ...
            mid_level: for (RolapLevel level : levels) {
              if (level.isAll()) {
                continue mid_level;
              }
  ...
  private void printNotSeenForeignKeys(List notSeenForeignKeys) {
    LOGGER.debug("Recognizer.printNotSeenForeignKeys: " + aggTable.getName());
    for (Iterator it = notSeenForeignKeys.iterator(); it.hasNext();) {
      JdbcSchema.Table.Column.Usage usage = (JdbcSchema.Table.Column.Usage) it
        .next();
  ...
  /**
   * Here a measure ussage is created and the right join condition is explicitly
   * supplied. This is needed is when the aggregate table's column names may not
   * match those found in the RolapStar.
   * 
   * @param factUsage
   * @param aggColumn
  ...
  protected void makeForeignKey(final JdbcSchema.Table.Column.Usage factUsage,
    ...
    JdbcSchema.Table.Column.Usage aggUsage = aggColumn
      .newUsage(JdbcSchema.UsageType.FOREIGN_KEY);
  ...
  protected abstract boolean matchLevel(final Hierarchy hierarchy,
    final HierarchyUsage hierarchyUsage, final RolapLevel level);
   ...
   * method was to create. If there is an existing level usage for the column
   * and it matches something else, then it is an error.
   */
  protected void makeLevel(final JdbcSchema.Table.Column aggColumn,
    final Hierarchy hierarchy, final HierarchyUsage hierarchyUsage,
    final String factColumnName, final String levelColumnName,
    ...
    try {
      if (aggColumn.hasUsage(JdbcSchema.UsageType.LEVEL)) {
        ...
        for (Iterator<JdbcSchema.Table.Column.Usage> uit = aggColumn
          .getUsages(JdbcSchema.UsageType.LEVEL); uit.hasNext();) {
          ...
          if (!aggUsage.relation.equals(rel) || !aggColumn.column.name.equals(cName)) {
            ...
            String msg = mres.DoubleMatchForLevel.str(aggTable.getName(),
              dbFactTable.getName(), aggColumn.getName(), aggUsage.relation
                .toString(), aggColumn.column.name, rel.toString(), cName);
            msgRecorder.reportError(msg);
        ...
        JdbcSchema.Table.Column.Usage aggUsage = aggColumn
          .newUsage(JdbcSchema.UsageType.LEVEL);
  ...
  /**
   * If everything is ok, issue warning for each aggTable column that has not
   * been identified as a FACT_COLUMN, MEASURE_COLUMN or LEVEL_COLUMN.
   */
  protected void checkUnusedColumns() {
    ...
    SortedMap<String, String> unusedColumnMsgs = new TreeMap<String, String>();
    for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
      if (!aggColumn.hasUsage()) {
        String msg = mres.AggUnknownColumn.str(aggTable.getName(), dbFactTable
          .getName(), aggColumn.getName());
   ...
   * calling the getRollup() method of the fact table column's RolapAggregator.
   * But in the case that the fact table column's RolapAggregator is the "Avg"
   * aggregator, then the special RolapAggregator.AvgFromSum is used.
   * <p>
   * Note: this code assumes that the aggregate table does not have an explicit
   * average aggregation column.
   * 
   * @param aggUsage
   * @param factAgg
   */
  protected RolapAggregator convertAggregator(
    final JdbcSchema.Table.Column.Usage aggUsage, final RolapAggregator factAgg) {
   ...
   * </pre>
   * 
   * Note that there is no SumFromSum since that is not a special case requiring
   * a special aggregator.
   * <p>
  ...
  protected RolapAggregator convertAggregator(
    ...
    if (rollupAgg == null) {
      String msg = mres.NoAggregatorFound.str(aggUsage.getSymbolicName(), factAgg
        .getName(), siblingAgg.getName());
      msgRecorder.reportError(msg);
    }
  ...
  /**
   * Given an aggregate table column usage, find the column name of the table's
   * fact count column usage.
   * 
   * @param aggUsage
   *          Aggregate table column usage
  ...
  private String getFactCountExpr(final JdbcSchema.Table.Column.Usage aggUsage) {
    ...
    Iterator<JdbcSchema.Table.Column.Usage> it = aggTable
      .getColumnUsages(JdbcSchema.UsageType.FACT_COUNT);
    it.hasNext();
  ...
  /**
   * Given a {@link mondrian.olap.MondrianDef.Expression}, returns the
   * associated column name.
   * 
   * <p>
   * Note: if the {@link mondrian.olap.MondrianDef.Expression} is not a
   * {@link mondrian.olap.MondrianDef.Column} or
   * {@link mondrian.olap.MondrianDef.KeyExpression}, returns null. This will
   * result in an error.
   */
  protected String getColumnName(MondrianDef.Expression expr) {
    ...
    try {
      ...
      String msg = mres.NoColumnNameFromExpression.str(expr.toString());
      msgRecorder.reportError(msg);
