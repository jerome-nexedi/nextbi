...
public class AggGen {
  ...
  private void init() {
    ...
    if (getLogger().isDebugEnabled()) {
      getLogger().debug(
        "Init: " + "RolapStar:" + Util.nl + getFactTable() + Util.nl + "FactTable:"
          + Util.nl + factTable);
    }
    ...
    for (RolapStar.Column rColumn : getFactTable().getColumns()) {
      ...
      if (name == null) {
        getLogger().warn(
          "Init: " + "For fact table \"" + getFactTableName()
            + "\", could not get column name for RolapStar.Column: " + rColumn);
        return;
      }
      ...
      if (c == null) {
        getLogger().warn(
          "For RolapStar: \"" + getFactTable().getAlias() + "\" measure with name, "
  ...
  private boolean addSpecialCollapsedColumn(final JdbcSchema db,
    ...
    while (prColumn.getParentColumn() != null) {
      ...
      if (rname == null) {
        getLogger().warn(
          "Adding Special Collapsed Column: " + "For fact table \""
            + getFactTableName() + "\", could not get parent column name"
  ...
  private boolean addCollapsedColumn(final JdbcSchema db,
    ...
    if (rname == null) {
      getLogger().warn(
        "Adding Collapsed Column: " + "For fact table \"" + getFactTableName()
          + "\", could not get column name for RolapStar.Column: " + rColumn);
      return false;
    }
    ...
    for (RolapStar.Column rc : rt.getColumns()) {
      ...
      if (name == null) {
        getLogger().warn(
          "Adding Collapsed Column: " + "For fact table \"" + getFactTableName()
            + "\", could not get column name" + " for RolapStar.Column \"" + rc
            + "\" for RolapStar.Table with alias \"" + rt.getAlias() + "\"");
        return false;
      }
  ...
  public String insertIntoCollapsed() {
    ...
    for (List<JdbcSchema.Table.Column.Usage> list : collapsedColumnUsages.values()) {
      for (JdbcSchema.Table.Column.Usage usage : list) {
        ...
        pw.print(" AS ");
        String n = (usage.usagePrefix == null) ? c.getName() : usage.usagePrefix
          + c.getName();
    ...
    for (JdbcSchema.Table.Column.Usage usage : measures) {
      ...
      pw.print(prefix);
      pw.print(agg.getExpression(sqlQuery.getDialect().quoteIdentifier(t.getName(),
        c.getName())));
      pw.print(" AS ");
      pw.print(sqlQuery.getDialect().quoteIdentifier(c.getName()));
      pw.println(',');
