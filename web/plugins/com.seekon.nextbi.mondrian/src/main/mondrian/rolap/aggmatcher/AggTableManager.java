      ...
      synchronized (db) {
        ...
        loop: for (RolapStar star : getStars()) {
          ...
          for (JdbcSchema.Table dbTable : db.getTables()) {
            ...
            ExplicitRules.TableDef tableDef = ExplicitRules.getIncludeByTableDef(
              name, aggGroups);
  ...
  void bindToStar(final JdbcSchema.Table dbFactTable, final RolapStar star,
    ...
    try {
      ...
      for (JdbcSchema.Table.Column factColumn : dbFactTable.getColumns()) {
        ...
        RolapStar.Table rTable = star.getFactTable().findTableWithLeftJoinCondition(
          cname);
        if (rTable != null) {
          JdbcSchema.Table.Column.Usage usage = factColumn
        ...
        if (!factColumn.hasUsage() && getLogger().isDebugEnabled()) {
          getLogger().debug(
            mres.UnknownFactTableColumn.str(msgRecorder.getContext(), dbFactTable
              .getName(), factColumn.getName()));
        }
      }
    } finally {
