...
public class CrossJoinArgFactory {
  ...
  public Set<CrossJoinArg> buildConstraintFromAllAxes(final RolapEvaluator evaluator) {
    Set<CrossJoinArg> joinArgs = new LinkedHashSet<CrossJoinArg>();
    for (QueryAxis ax : evaluator.getQuery().getAxes()) {
      List<CrossJoinArg[]> axesArgs = checkCrossJoinArg(evaluator, ax.getSet(), true);
      if (axesArgs != null) {
  ...
  private boolean isConstrainedMeasure(Exp arg) {
    ...
    if (member instanceof RolapCalculatedMember) {
      ...
      return ((calcExp instanceof ResolvedFunCall && ((ResolvedFunCall) calcExp)
        .getFunDef() instanceof TupleFunDef))
   ...
   */
  public List<CrossJoinArg[]> checkCrossJoin(RolapEvaluator evaluator, FunDef fun,
    ...
    CrossJoinArg[] combinedCJArgs = Util.appendArrays(
      cjArgsBothInputs[0] == null ? CrossJoinArg.EMPTY_ARRAY : cjArgsBothInputs[0],
      cjArgsBothInputs[1] == null ? CrossJoinArg.EMPTY_ARRAY : cjArgsBothInputs[1]);
  ...
  private CrossJoinArg[] checkEnumeration(RolapEvaluator evaluator, FunDef fun,
    ...
    final CrossJoinArg cjArg = MemberListCrossJoinArg.create(evaluator, memberList,
  ...
  private List<CrossJoinArg[]> checkDimensionFilter(RolapEvaluator evaluator,
    ...
    if (filterArgs[1] instanceof ResolvedFunCall) {
      ...
      currentPredicateArgs = checkFilterPredicate(evaluator, predicateCall, exclude);
    } else {
    ...
    if (predicateCall.getFunName().equals("AND")) {
      ...
      if (andArg0 instanceof ResolvedFunCall && andArg1 instanceof ResolvedFunCall) {
        ...
        andCJArgs0 = checkFilterPredicate(evaluator, (ResolvedFunCall) andArg0,
          exclude);
        if (andCJArgs0 != null) {
          andCJArgs1 = checkFilterPredicate(evaluator, (ResolvedFunCall) andArg1,
            exclude);
          if (andCJArgs1 != null) {
  ...
  private CrossJoinArg[] checkFilterPredicateInIs(RolapEvaluator evaluator,
    ...
    predCJArgs = checkEnumeration(evaluator, predSecondArgFun, predSecondArgList,
      exclude);
    return predCJArgs;
  }
  ...
  private CrossJoinArg[] expandNonNative(RolapEvaluator evaluator, Exp exp) {
    ...
    if (shouldExpandNonEmpty(exp) && evaluator.getActiveNativeExpansions().add(exp)) {
  ...
  private boolean isSet(Exp exp) {
    return ((exp instanceof ResolvedFunCall) && ((ResolvedFunCall) exp).getFunName()
      .equals("{}"))
