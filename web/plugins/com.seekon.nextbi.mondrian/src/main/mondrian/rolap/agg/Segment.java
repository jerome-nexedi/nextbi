/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/Segment.java#62 $
...
/**
 * A <code>Segment</code> is a collection of cell values parameterized by a
 * measure, and a set of (column, value) pairs. An example of a segment is</p>
 * 
 * <blockquote>
 * <p>
 * (Unit sales, Gender = 'F', State in {'CA','OR'}, Marital Status = <i>
 * anything</i>)
 * </p>
 * </blockquote>
 * 
 ...
 * evalutes the query a second time, knowing that all cell values are available.
 * Finally, it releases the pins.
 * </p>
 * 
 * <p>
 * A Segment may have a list of excluded {@link Region} objects. These are
 ...
 * 
 * <p>
 * For example, the segment defined by {CA, OR, WA} * {F, M} is a 2-dimensional
 * hyper-rectangle with 6 cells. After flushing {CA, OR, TX} * {F}, the result
 * is 4 cells:
 * 
 * <pre>
 *     F     M
 ...
 * @author jhyde
 * @since 21 March, 2002
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/agg/Segment.java#62 $
 */
public class Segment {
  ...
  /**
   * <p>
   * <code>data</code> holds a reference to the <code>SegmentDataset</code> that
   * contains the underlying cell values.
   * </p>
   * 
   * <p>
   * Since the <code>SegmentDataset</code> is loaded and assigned after
   * <code>Segment</code> is constructed, threadsafe access to it is only
   * guaranteed if the access is guarded.
   * <p/>
   * 
   * <p>
   * Access which does not depend on <code>data</code> already having been
   * loaded should be guarded by obtaining either a read or write lock on
   * <code>stateLock</code>, as appropriate.
   * </p>
   * 
   * <p>
   * Access that should not proceed until the <code>data</code> reference has
   * been loaded should be guarded using the <code>dataGate</code> latch. This
   * is typically accomplished by calling <code>waitUntilLoaded()</code>, which
   * will block until the latch is released and throw an error if
   * <code>data</code> failed to load.
   * </p>
   * 
   * <p>
   * Once set, the value of <code>data</code> is presumed to be invariant and
   * should never be reset, nor should the contents be modified. Thus, for a
   * given thread, any read access to data which comes after
   * <code>dataGate.await()</code> (or, by extension,
   * <code>waitUntilLoaded</code> will be threadsafe.
   * </p>
   */
  ...
  /**
   * <p>
   * <code>state</code> == state of the segment (loading, ready, etc). Since it
   * correlates to the value of the <code>data</code> reference and may be
   * accessed from multiple threads, access to it is guarded by stateLock.
   * </p>
   * 
   * <p>
   * The initial value of state is Loading. It may then be set to either Ready
   * or Failed. Ready and Failed are both terminal states; once set to either,
   * state may not be reset.
   * </p>
   */
  ...
  /**
   * List of regions to ignore when reading this segment. This list is populated
   * when a region is flushed. The cells for these regions may be physically in
   * the segment, because trimming segments can be expensive, but should still
   * be ignored.
   */
   ...
   * 
   * @param aggregation
   *          The aggregation this <code>Segment</code> belongs to
   * @param measure
   *          Measure whose values this Segment contains
   * @param axes
   *          List of axes; each is a constraint plus a list of values
   */
  public Segment(Aggregation aggregation, RolapStar.Measure measure,
    Aggregation.Axis[] axes) {
    this(aggregation, measure, axes, new ArrayList<Region>());
  }
   ...
   */
  public Segment(Aggregation aggregation, RolapStar.Measure measure,
    Aggregation.Axis[] axes, List<Region> excludedRegions) {
    this.id = nextId++;
  ...
  /**
   * Compares internal <code>state</code> variable to a passed-in value in a
   * threadsafe way using the <code>stateLock</code> read lock.
   * 
   * @param value
   *          The State value to which <code>state</code> should be compared.
  ...
  private String printSegmentHeaderInfo(String sep) {
    ...
    buf.append(sep);
    buf.append("measure=");
    buf.append(measure.getExpression() == null ? measure.getAggregator()
      .getExpression("*") : measure.getAggregator().getExpression(
  ...
  /**
   * Retrieves the value at the location identified by <code>keys</code>.
   * 
   * <p>
   * Returns
   * <ul>
   * 
   * <li>{@link Util#nullValue} if the cell value is null (because no fact table
   * rows met those criteria);</li>
   * 
  ...
  /**
   * Returns whether the given set of key values will be in this segment when it
   * finishes loading.
   */
  ...
  /**
   * Blocks until this segment has finished loading; if this segment has already
   * loaded, returns immediately.
   */
  ...
  /**
   * Prints the state of this <code>Segment</code>, including constraints and
   * values. Blocks the current thread until the segment is loaded.
   * 
   * @param pw
   *          Writer
   */
  ...
  /**
   * Returns the number of cells in this Segment, deducting cells in excluded
   * regions.
   * 
   * <p>
   * This method may return a value which is slightly too low, or occasionally
   * even negative. This occurs when a Segment has more than one excluded
   * region, and those regions overlap. Cells which are in both regions will be
   * counted twice.
   * 
   ...
   * 
   * <p>
   * If <code>bestColumn</code> is not -1, the <code>bestColumn</code>th
   * column's predicate should be replaced by <code>bestPredicate</code>.
   * 
   * @param axisKeepBitSets
   *          For each axis, a bitmap of the axis values to keep; each axis must
   *          have at least one bit set
   * @param bestColumn
   *          The column that retains most of its values
   * @param bestPredicate
   * @param excludedRegions
   *          List of regions to exclude from segment
   * @return Segment containing a subset of the values
   */
  Segment createSubSegment(BitSet[] axisKeepBitSets, int bestColumn,
    StarColumnPredicate bestPredicate, List<Segment.Region> excludedRegions) {
    ...
    for (int j = 0; j < axes.length; j++) {
      ...
      if (firstClearBit >= axisKeys.length) {
        ...
        for (int bit = keepBitSet.nextSetBit(0); bit >= 0; bit = keepBitSet
          .nextSetBit(bit + 1)) {
          map.put(bit, newAxisKeyList.size());
          newAxisKeyList.add(axisKeys[bit]);
        }
        newAxisKeys = newAxisKeyList
          .toArray(new Comparable<?>[newAxisKeyList.size()]);
        assert newAxisKeys.length > 0;
      }
      final Aggregation.Axis newAxis = new Aggregation.Axis(newPredicate,
        newAxisKeys);
      newAxes[j] = newAxis;
    ...
    final Segment newSegment = new Segment(aggregation, measure, newAxes,
      excludedRegions);
    ...
    // will be sparse.)
    SegmentDataset newData = createDataset(data instanceof SparseSegmentDataset,
      data.getType(), valueCount);
   ...
   * 
   * <p>
   * WARNING: the returned SegmentDataset reference should not be modified; it
   * is assumed to be invariant.
   * </p>
   * 
   * @return The <code>data</code> reference if it has been loaded, null
   *         otherwise.
   */
  ...
  /**
   * <code>State</code> enumerates the allowable values of a segment's state.
   */
   ...
   * 
   * <p>
   * A region is defined by a set of constraints, one for each column in the
   * segment. A constraint may be
   * {@link mondrian.rolap.agg.LiteralStarPredicate}(true), meaning that the
   ...
   * 
   * <p>
   * Note that the resulting segment is not a hypercube: it has a 'hole'. This
   * is why regions are required.
   */
  static class Region {
    ...
    Region(List<StarColumnPredicate> predicateList,
      List<StarPredicate> multiColumnPredicateList, int cellCount) {
      this.predicates = predicateList.toArray(new StarColumnPredicate[predicateList
        .size()]);
      this.multiColumnPredicates = multiColumnPredicateList
        .toArray(new StarPredicate[multiColumnPredicateList.size()]);
    ...
    public List<StarPredicate> getMultiColumnPredicates() {
      return Collections.unmodifiableList(Arrays.asList(multiColumnPredicates));
    }
    ...
    public boolean equals(Object obj) {
      if (obj instanceof Region) {
        ...
        return Arrays.equals(this.predicates, that.predicates)
          && Arrays.equals(this.multiColumnPredicates, that.multiColumnPredicates);
      } else {
        return false;
      }
    }
    ...
    public int hashCode() {
      return Arrays.hashCode(multiColumnPredicates) ^ Arrays.hashCode(predicates);
    }
