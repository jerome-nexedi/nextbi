/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/AbstractColumnPredicate.java#11 $
 ...
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/AbstractColumnPredicate
 *          .java#11 $
 */
public abstract class AbstractColumnPredicate implements StarColumnPredicate {
  ...
  public BitKey getConstrainedColumnBitKey() {
    ...
    if (constrainedColumnBitKey == null && constrainedColumn != null
      && constrainedColumn.getTable() != null) {
      constrainedColumnBitKey = BitKey.Factory.makeBitKey(constrainedColumn
        .getStar().getColumnCount());
  ...
  public StarPredicate or(StarPredicate predicate) {
    if (predicate instanceof StarColumnPredicate) {
      StarColumnPredicate starColumnPredicate = (StarColumnPredicate) predicate;
  ...
  public StarColumnPredicate orColumn(StarColumnPredicate predicate) {
    ...
    if (predicate instanceof ListColumnPredicate) {
      ...
      final List<StarColumnPredicate> list = new ArrayList<StarColumnPredicate>(2);
      list.add(this);
      list.add(predicate);
      return new ListColumnPredicate(getConstrainedColumn(), list);
    }
  ...
  protected static List<StarColumnPredicate> cloneListWithColumn(
    ...
    List<StarColumnPredicate> newList = new ArrayList<StarColumnPredicate>(list
      .size());
  ...
  public static class Factory {
    /**
     * Returns a predicate which tests whether the column's value is equal to a
     * given constant.
     * 
     * @param column
     *          Constrained column
     * @param value
     *          Value
     * @return Predicate which tests whether the column's value is equal to a
     *         column constraint's value
     */
    ...
    /**
     * Returns a predicate which tests whether the column's value is equal to
     * column predicate's value.
     * 
     * @param predicate
     *          Column predicate
     * @return Predicate which tests whether the column's value is equal to a
    ...
    public static StarColumnPredicate equal(ValueColumnPredicate predicate) {
      return equal(predicate.getConstrainedColumn(), predicate.getValue());
    }
  }
}
