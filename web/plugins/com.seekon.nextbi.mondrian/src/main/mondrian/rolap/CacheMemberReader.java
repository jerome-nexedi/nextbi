/*
// $Id: //open/mondrian/src/main/mondrian/rolap/CacheMemberReader.java#37 $
 ...
 * <p>
 * Note: CacheMemberReader can not handle ragged hierarchies. (HR Tests fail if
 * {@link SmartMemberReader} is replaced with CacheMemberReader).
 * 
 * @author jhyde
 * @since 21 December, 2001
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/CacheMemberReader.java#37 $
 */
class CacheMemberReader implements MemberReader, MemberCache {
  ...
  // don't need to implement this MemberCache method because we're never
  // used in a context where it is needed
  public void putChildren(RolapLevel level, TupleConstraint constraint,
    List<RolapMember> children) {
    throw new UnsupportedOperationException();
  }
  ...
  public RolapMember getLeadMember(RolapMember member, int n) {
    if (n >= 0) {
      for (int ordinal = member.getOrdinal(); ordinal < members.size(); ordinal++) {
        if ((members.get(ordinal).getLevel() == member.getLevel()) && (n-- == 0)) {
