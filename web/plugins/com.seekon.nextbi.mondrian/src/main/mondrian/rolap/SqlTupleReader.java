...
public class SqlTupleReader implements TupleReader {
      ...
      if (getCurrMember() != null) {
        ...
        for (int i = 0; i <= levelDepth; i++) {
          ...
          if (parentChild) {
            ...
            if (parentValue == null) {
            ...
            } else if (parentValue.toString()
              .equals(childLevel.getNullParentValue())) {
              // member is at top of hierarchy; its parent is the
              // 'all' member
            } else {
              ...
              if (parentMember == null) {
                LOGGER.warn(MondrianResource.instance().LevelTableParentNotFound
                  .str(childLevel.getUniqueName(), String.valueOf(parentValue)));
              }
            }
          }
          ...
          if (member == null) {
            ...
            if (member == null) {
              member = memberBuilder.makeMember(parentMember, childLevel, value,
                captionValue, parentChild, stmt, key, column);
            }
          }
          ...
          if (member != members.get(i)) {
            ...
            List<RolapMember> cachedChildren = cache.getChildrenFromCache(member,
              mcc);
    ...
    try {
      if (execQuery) {
        ...
        stmt = RolapUtil.executeQuery(dataSource, sql, types, maxRows, 0, new Locus(
          Locus.peek().execution, "SqlTupleReader.readTuples " + partialTargets,
          message), -1, -1);
  ...
  public TupleList readTuples(DataSource jdbcConnection, TupleList partialResult,
    List<List<RolapMember>> newPartialResult) {
    ...
    TupleList tupleList = n == 1 ? new UnaryTupleList(members) : new ListTupleList(
      n, members);
  ...
  protected void addLevelMemberSql(SqlQuery sqlQuery, RolapLevel level,
    ...
    for (int i = 0; i <= levelDepth; i++) {
      ...
      boolean levelCollapsed = (aggStar != null)
        && SqlMemberSource.isLevelCollapsed(aggStar, (RolapCubeLevel) currLevel);
      ...
      if (levelCollapsed) {
        ...
        int bitPos = starColumn.getBitPosition();
        AggStar.Table.Column aggColumn = aggStar.lookupColumn(bitPos);
        RolapStar.Condition condition = new RolapStar.Condition(keyExp, aggColumn
          .getExpression());
      ...
      case LAST:
        ...
        sqlQuery.addOrderBy(Integer.toString(sqlQuery.getCurrentSelectListSize()),
          true, false, nullable);
      ...
      for (RolapProperty property : properties) {
        ...
        if (propExp instanceof MondrianDef.Column) {
          ...
          propSql = sqlQuery.getDialect().quoteIdentifier(currLevel.getTableAlias(),
