/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapMemberBase.java#8 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapMemberBase.java#8
 *          $
 */
public class RolapMemberBase extends MemberBase implements RolapMember {
  /**
   * For members of a level with an ordinal expression defined, the value of
   * that expression for this member as retrieved via JDBC; otherwise null.
   */
  ...
  /**
   * Ordinal of the member within the hierarchy. Some member readers do not use
   * this property; in which case, they should leave it as its default, -1.
   */
  private int ordinal;
   ...
   * 
   * <p>
   * We expect there to be a lot of members, but few of them will have
   * properties. So to reduce memory usage, when empty, this is set to an
   * immutable empty set.
   */
   ...
   *          Parent member
   * @param level
   *          Level this member belongs to
   * @param key
   *          Key to this member in the underlying RDBMS
   * @param name
   *          Name of this member
   * @param memberType
   *          Type of member
   */
  protected RolapMemberBase(RolapMember parentMember, RolapLevel level, Object key,
    String name, MemberType memberType) {
  ...
  public boolean equals(OlapElement o) {
    return (o instanceof RolapMemberBase) && equals((RolapMemberBase) o);
  }
    ...
    if (parentMember == null && key != null) {
      ...
      if (n != null) {
        ...
        if (getLogger().isDebugEnabled()) {
          getLogger().debug("RolapMember.makeUniqueName: uniqueName=" + uniqueName);
        }
      }
    }
  }
  ...
  protected void setUniqueName(Object key) {
    ...
    if (parentMember == null
      || (parentMember.isAll() && (!isCalculated()
        || this instanceof VisualTotalsFunDef.VisualTotalMember || getDataMember() != null))) {
      final RolapHierarchy hierarchy = getHierarchy();
      final Dimension dimension = hierarchy.getDimension();
      final RolapLevel level = getLevel();
      if (dimension.getDimensionType() != null
        && (dimension.getDimensionType().equals(DimensionType.MeasuresDimension) && hierarchy
          .getName().equals(dimension.getName()))) {
        ...
        if (name.equals(level.getName())) {
          this.uniqueName = Util.makeFqName(Util.makeFqName(hierarchy
            .getUniqueName(), level.getName()), name);
        } else {
  ...
  public String getName() {
    final String name = (String) getPropertyValue(Property.NAME.name);
    return (name != null) ? name : keyToString(key);
  }
  ...
  public synchronized void setProperty(String name, Object value) {
    ...
    if (mapPropertyNameToValue.isEmpty()) {
      ...
      PropertyValueMapFactory factory = PropertyValueMapFactoryFactory
  ...
  public Object getPropertyValue(String propertyName, boolean matchCase) {
    ...
    if (property != null) {
      ...
      case Property.CHILDREN_CARDINALITY_ORDINAL:
        return Locus.execute(((RolapSchema) level.getDimension().getSchema())
          .getInternalConnection(), "Member.CHILDREN_CARDINALITY",
   ...
   * 
   * <p>
   * The method first compares on keys; null keys always collate last. If the
   * keys are equal, it compares using unique name.
   * 
   * <p>
   * This method does not consider {@link #ordinal} field, because ordinal is
   * only unique within a parent. If you want to compare members which may be at
   * any position in the hierarchy, use
  ...
  public boolean isParentChildLeaf() {
    if (isParentChildLeaf == null) {
      isParentChildLeaf = getLevel().isParentChild()
        && getDimension().getSchema().getSchemaReader().getMemberChildren(this)
          .size() == 0;
    }
  ...
  /**
   * Returns a list of member lists where the first member list is the root
   * members while the last member array is the leaf members.
   * 
   * <p>
   * If you know that you will need to get all or most of the members of a
   * hierarchy, then calling this which gets all of the hierarchy's members all
   * at once is much faster than getting members one at a time.
   * 
   * @param schemaReader
   *          Schema reader
   * @param hierarchy
   ...
   * 
   * <p>
   * Gets an array of members for each level and traverses array for the lowest
   * level, setting each member's parent's parent's etc. member's ordinal if not
   * set working back down to the leaf member and then going to the next leaf
   * member and traversing up again.
   * 
   * <p>
   * The above algorithm only works for a hierarchy that has all of its leaf
   * members in the same level (that is, a non-ragged hierarchy), which is the
   * norm. After all member ordinal values have been set, traverses the array of
   * members, making sure that all members' ordinals have been set. If one is
   * found that is not set, then one must to a full Top-down setting of the
   * ordinals.
   * 
   * <p>
  ...
  public static void setOrdinals(SchemaReader schemaReader, Member seedMember) {
    ...
    /*
     * The following are times for executing different set ordinals algorithms
     * for both the FoodMart Sales cube/Store dimension and a Large Data set
     * with a dimension with about 250,000 members.
     * 
     * Times: Original setOrdinals Top-down Foodmart: 63ms Large Data set:
     * 651865ms Calling getAllMembers before calling original setOrdinals
     * Top-down Foodmart: 32ms Large Data set: 73880ms Bottom-up/Top-down
     * Foodmart: 17ms Large Data set: 4241ms
     */
    long start = System.currentTimeMillis();
  ...
  /**
   * Returns whether the ordinal assignment algorithm needs to perform the more
   * expensive top-down algorithm. If the hierarchy is 'uneven', not all leaf
   * members are at the same level, then bottom-up setting of ordinals will have
   * missed some.
   * 
   * @param levelMembers
   *          Array containing the list of members in each level except the leaf
   *          level
  ...
  /**
   * Walks up the hierarchy, setting the ordinals of ancestors until it reaches
   * the root or hits an ancestor whose ordinal has already been assigned.
   * 
   * <p>
   * Assigns the given ordinal to the ancestor nearest the root which has not
   * been assigned an ordinal, and increments by one for each descendant.
   * 
   * @param ordinal
   *          Ordinal to assign to deepest ancestor
   * @param child
   *          Member whose ancestors ordinals to set
  ...
  private static int setOrdinal(Member member, int ordinal) {
    if (member instanceof RolapMemberBase) {
      ...
      // TODO
      LOGGER.warn("RolapMember.setAllChildren: NOT RolapMember " + "member.name="
        + member.getName() + ", member.class=" + member.getClass().getName()
        + ", ordinal=" + ordinal);
      ordinal++;
    }
    return ordinal;
   ...
   * 
   * <p>
   * For big hierarchies it takes a bunch of time. SQL Server is relatively fast
   * in comparison so it might be storing such information in the DB.
   * 
   * @param schemaReader
   *          Schema reader
   * @param member
  ...
  private static void setOrdinalsTopDown(SchemaReader schemaReader, Member member) {
    ...
    try {
      ...
      if (parent == null) {
        ...
        List<Member> siblings = schemaReader.getHierarchyRootMembers(member
          .getHierarchy());
  ...
  private static int setAllChildren(int ordinal, SchemaReader schemaReader,
    Member member) {
  ...
  /**
   * Converts a key to a string to be used as part of the member's name and
   * unique name.
   * 
   * <p>
   ...
   * implementation of {@link java.util.Map} to use to store property
   * string/value pairs for member properties.
   * </p>
   * 
   * <p>
   * This permits tuning for performance, memory allocation, etcetera. For
   * example, if a member belongs to a level which has 10 member properties a
   * HashMap may be preferred, while if the level has only two member properties
   * a Flat3Map may make more sense.
   * </p>
   */
  public interface PropertyValueMapFactory {
    /**
     * Creates a {@link java.util.Map} to be used for storing property
     * string/value pairs for the specified {@link mondrian.olap.Member}.
     * 
     * @param member
     *          Member
  ...
  /**
   * Default {@link RolapMemberBase.PropertyValueMapFactory} implementation,
   * used if
   * {@link mondrian.olap.MondrianProperties#PropertyValueMapFactoryClass} is
   * not set.
   */
  public static final class DefaultPropertyValueMapFactory implements
    PropertyValueMapFactory {
    /**
     * {@inheritDoc}
     * <p>
     * This factory creates an
     * {@link org.apache.commons.collections.map.Flat3Map} if it appears that
     * the provided member has less than 3 properties, and a
     * {@link java.util.HashMap} if it appears that it has more than 3.
     * </p>
     * 
     * <p>
     * Guessing the number of properties can be tricky since some subclasses of
     * {@link mondrian.olap.Member}
     * </p>
     * have additional properties that aren't explicitly declared. The most
     * common offenders are the (@link mondrian.olap.Measure} implementations,
     * which often have 4 or more undeclared properties, so if the member is a
     * measure, the factory will create a {@link java.util.HashMap}. </p>
     * 
     * @param member
     *          {@inheritDoc}
     * @return {@inheritDoc}
  ...
  /**
   * <p>
   * Creates the PropertyValueMapFactory which is in turn used to create
   * property-value maps for member properties.
   * </p>
   * 
   * <p>
   * The name of the PropertyValueMapFactory is drawn from
   * {@link mondrian.olap.MondrianProperties#PropertyValueMapFactoryClass} in
   * mondrian.properties. If unset, it defaults to
   * {@link RolapMemberBase.DefaultPropertyValueMapFactory}.
   * </p>
   */
  public static final class PropertyValueMapFactoryFactory extends
    ObjectFactory.Singleton<PropertyValueMapFactory> {
    /**
    ...
    /**
     * The constructor for the <code>PropertyValueMapFactoryFactory</code>. This
     * passes the <code>PropertyValueMapFactory</code> class to the
    ...
    protected PropertyValueMapFactory getDefault(Class[] parameterTypes,
  ...
  public boolean containsAggregateFunction() {
    ...
    if (containsAggregateFunction == null) {
      containsAggregateFunction = foundAggregateFunction(getExpression());
    }
