...
/**
 * Implementation of {@link MemberReader} which replaces given members with a
 * substitute.
 * 
 * <p>
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/SubstitutingMemberReader
 *          .java#7 $
 * @since Oct 5, 2007
 */
public abstract class SubstitutingMemberReader extends DelegatingMemberReader {
  ...
  public RolapMember getLeadMember(RolapMember member, int n) {
    return substitute(memberReader.getLeadMember(desubstitute(member), n));
  }

  @Override
  public List<RolapMember> getMembersInLevel(RolapLevel level, int startOrdinal,
    int endOrdinal) {
    return substitute(memberReader
  ...
  public void getMemberRange(RolapLevel level, RolapMember startMember,
    ...
    memberReader.getMemberRange(level, desubstitute(startMember),
      desubstitute(endMember), new SubstitutingMemberList(list));
  }

  @Override
  public int compare(RolapMember m1, RolapMember m2, boolean siblingsAreEqual) {
    return memberReader
      .compare(desubstitute(m1), desubstitute(m2), siblingsAreEqual);
  }
  ...
  public void getMemberChildren(RolapMember parentMember, List<RolapMember> children) {
    memberReader.getMemberChildren(desubstitute(parentMember),
  ...
  public void getMemberChildren(List<RolapMember> parentMembers,
    ...
    memberReader.getMemberChildren(desubstitute(parentMembers),
      new SubstitutingMemberList(children));
  }
  ...
  public RolapMember lookupMember(List<Id.Segment> uniqueNameParts,
    ...
    return substitute(memberReader.lookupMember(uniqueNameParts, failIfNotFound));
  }

  @Override
  public void getMemberChildren(RolapMember member, List<RolapMember> children,
    MemberChildrenConstraint constraint) {
    memberReader.getMemberChildren(desubstitute(member), new SubstitutingMemberList(
      children), constraint);
  }
  ...
  public void getMemberChildren(List<RolapMember> parentMembers,
    ...
    memberReader.getMemberChildren(desubstitute(parentMembers),
      new SubstitutingMemberList(children), constraint);
  }

  @Override
  public List<RolapMember> getMembersInLevel(RolapLevel level, int startOrdinal,
    int endOrdinal, TupleConstraint constraint) {
    return substitute(memberReader.getMembersInLevel(level, startOrdinal,
  ...
  /**
   * List which writes through to an underlying list, substituting members as
   * they are written and desubstituting as they are read.
   */
  ...
  private class SubstitutingMemberBuilder implements TupleReader.MemberBuilder {
    ...
    public RolapMember makeMember(RolapMember parentMember, RolapLevel childLevel,
      ...
      Object key, int column) throws SQLException {
      return substitute(memberReader.getMemberBuilder().makeMember(
        desubstitute(parentMember), childLevel, value, captionValue, parentChild,
        stmt, key, column));
    }
