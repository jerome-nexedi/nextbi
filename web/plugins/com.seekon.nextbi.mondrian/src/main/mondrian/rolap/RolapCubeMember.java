/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapCubeMember.java#20 $
 ...
 * 
 * @author Will Gorman
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapCubeMember.java#20
 *          $
 */
public class RolapCubeMember extends DelegatingRolapMember implements
  RolapMemberInCube {
   ...
   *          Parent member
   * @param member
   *          Member of underlying (non-cube) hierarchy
   * @param cubeLevel
   *          Level
   */
  ...
  public boolean equals(OlapElement o) {
    return o.getClass() == RolapCubeMember.class && equals((RolapCubeMember) o);
  }
   ...
   * 
   * <p>
   * This method is central to how RolapCubeMember works. It allows a member
   * from the cache to be used within different usages of the same shared
   * dimension. The cache member is the same, but the RolapCubeMembers wrapping
   * the cache member report that they belong to different levels, and hence
   * different hierarchies, dimensions, and cubes.
   */
  // override with stricter return type; final important for performance
  ...
  public Object getPropertyValue(String propertyName, boolean matchCase) {
    ...
    if (property != null) {
      ...
      case Property.PARENT_UNIQUE_NAME_ORDINAL:
        return parentCubeMember == null ? null : parentCubeMember.getUniqueName();
    ...
    if (exp instanceof ResolvedFunCall) {
      ...
      for (int i = 0; i < fcall.getArgCount(); i++) {
        if (fcall.getArg(i) instanceof HierarchyExpr) {
          ...
          if (expr.getHierarchy().equals(member.getHierarchy())) {
