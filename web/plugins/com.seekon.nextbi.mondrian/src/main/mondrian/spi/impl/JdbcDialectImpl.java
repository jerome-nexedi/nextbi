...
public class JdbcDialectImpl implements Dialect {
  ...
  public String quoteIdentifier(final String qual, final String name) {
    ...
    int size = name.length()
      + ((qual == null) ? SINGLE_QUOTE_SIZE : (qual.length() + DOUBLE_QUOTE_SIZE));
    StringBuilder buf = new StringBuilder(size);
   ...
   * @return SQL type
   */
  private static String guessSqlType(String basicType, List<String[]> valueList,
    int column) {
   ...
   */
  protected final String generateOrderByNullsAnsi(String expr, boolean ascending,
    boolean collateNullsLast) {
    if (collateNullsLast) {
