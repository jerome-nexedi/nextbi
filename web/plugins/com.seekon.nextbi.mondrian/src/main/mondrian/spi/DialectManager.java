...
public abstract class DialectManager {
  ...
  private static class DialectManagerImpl {
    ...
    DialectManagerImpl() {
      ...
      factory = new CachingDialectFactory(new ChainDialectFactory(Arrays.asList(
        registeredFactory, fallbackFactory)));
    }
    ...
    synchronized Dialect createDialect(DataSource dataSource, Connection connection) {
      ...
      if (dialect == null) {
        throw Util
          .newError("Cannot create dialect for JDBC connection" + connection);
      }
      return dialect;
    }
  }
  ...
  /**
   * Implementation of {@link DialectFactory} that calls a class's {@code public
   * &lt;init&gt;(Connection connection)} constructor.
   */
