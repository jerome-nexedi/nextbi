...
public class XmlaSupport {
  ...
  public static final String DATASOURCE_INFO = "Provider=Mondrian;DataSource="
    + CATALOG_NAME + ";";
  ...
  /**
   * This is a parameterized XSLT. The parameters are: "soap" with values "none"
   * or empty "content" with values "schemadata", "schema", "data" or empty With
   * these setting one can extract from an XMLA SOAP message the soap wrapper
   * plus body or simply the body; the complete body (schema and data), only the
   * schema of the body, only the data of the body or none of the body
   * 
   */
  ...
  public static DataSourcesConfig.DataSources getDataSources(String connectString,
    Map<String, String> catalogNameUrls) throws XOMException {
  ...
  public static DataSourcesConfig.DataSources parseDataSources(
    String dataSourcesConfigString, Logger logger) {
    try {
      if (dataSourcesConfigString == null) {
        logger.warn("XmlaSupport.parseDataSources: null input");
        return null;
      }
      dataSourcesConfigString = Util.replaceProperties(dataSourcesConfigString, Util
        .toMap(System.getProperties()));

      if (logger.isDebugEnabled()) {
        logger.debug("XmlaSupport.parseDataSources: dataSources="
   ...
   * by directly, same process, communicating with XMLA Mondrian, the fact that
   * the URL contains "localhost" is not important.
   * 
   * @param connectString
   *          Connect string
  ...
  public static Node[] extractNodesFromSoapXmla(byte[] bytes) throws SAXException,
    IOException {
    Document doc = XmlUtil.parse(bytes);
    return extractNodesFromSoapXmla(doc);
  }

  public static Node[] extractNodesFromSoapXmla(Document doc) throws SAXException,
    IOException {
  ...
  public static Node[] extractNodesFromXmla(byte[] bytes) throws SAXException,
    IOException {
    Document doc = XmlUtil.parse(bytes);
    return extractNodesFromXmla(doc);
  }

  public static Node[] extractNodesFromXmla(Document doc) throws SAXException,
    IOException {
    final String xmlaPrefix = "xmla";
  ...
  public static Node[] extractFaultNodesFromSoap(byte[] bytes) throws SAXException,
    IOException {
    Document doc = XmlUtil.parse(bytes);
    return extractFaultNodesFromSoap(doc);
  }

  public static Node[] extractFaultNodesFromSoap(Document doc) throws SAXException,
    IOException {
  ...
  public static Document extractBodyFromSoap(Document doc) throws SAXException,
    IOException {
    String xpath = getSoapBodyXPath();

    String[][] nsArray = new String[][] { { SOAP_PREFIX,
  ...
  /**
   * Given a Document and an xpath/namespace-array pair, extract and return the
   * Nodes resulting from applying the xpath.
   * 
   * @throws SAXException
   *           on error
   * @throws IOException
   *           on error
   */
  public static Node[] extractNodes(Node node, String xpath, String[][] nsArray)
    throws SAXException, IOException {
   ...
   * 
   */
  public static byte[] processSoapXmla(File file, String connectString,
    Map<String, String> catalogNameUrls, String cbClassName) throws IOException,
  ...
  public static byte[] processSoapXmla(Document doc, String connectString,
    Map<String, String> catalogNameUrls, String cbClassName, Role role,
    Map<List<String>, WeakReference<Servlet>> servletCache) throws IOException,
    ServletException, SAXException {
  ...
  public static byte[] processSoapXmla(String requestText, String connectString,
    Map<String, String> catalogNameUrls, String cbClassName, Role role,
    Map<List<String>, WeakReference<Servlet>> servletCache) throws IOException,
    ServletException, SAXException {
    // read soap file
    String dataSourceText = XmlaSupport.getDataSourcesText(connectString,
      catalogNameUrls);
  ...
  private static Servlet getServlet(String cbClassName, String dataSourceText,
    ...
    servletConfig.addInitParameter(XmlaServlet.PARAM_DATASOURCES_CONFIG, "inline:"
      + dataSourceText);
  ...
  public static boolean validateSchemaSoapXmla(byte[] bytes) throws SAXException,
    ...
    return validateEmbeddedSchema(bytes, XmlUtil.getSoapXmlaXds2xs("xmla"), XmlUtil
      .getSoapXmlaXds2xd("xmla"));
  }
  ...
  public static byte[] processXmla(File file, String connectString,
    Map<String, String> catalogNameUrls) throws IOException, SAXException,
    XOMException {
    return processXmla(file, connectString, catalogNameUrls, null);
  }

  public static byte[] processXmla(File file, String connectString,
    Map<String, String> catalogNameUrls, Role role) throws IOException,
    SAXException, XOMException {
    String requestText = XmlaSupport.readFile(file);
  ...
  public static byte[] processXmla(Document requestDoc, String connectString,
    Map<String, String> catalogNameUrls, Role role) throws IOException, XOMException {
  ...
  public static byte[] processXmla(Element requestElem, String connectString,
    Map<String, String> catalogNameUrls, Role role) throws IOException, XOMException {
    // make request
    final MondrianServer server = MondrianServer.createWithRepository(
      new StringRepositoryContentFinder(getDataSourcesText(connectString,
        catalogNameUrls)), getCatalogLocator());
    final XmlaHandler handler = new XmlaHandler(
      (XmlaHandler.ConnectionFactory) server, "xmla");
    ...
    XmlaRequest request = new DefaultXmlaRequest(requestElem, roleName, null, null,
      null);

    Enumeration.ResponseMimeType responseMimeType = Enumeration.ResponseMimeType.MAP
   ...
   * validation occurs if the XMLA response contains both a content and schmema
   * section. This should not be used when the byte array contains both the SOAP
   * elements and content, but only for the content.
   * 
   */
  public static boolean validateSchemaXmla(byte[] bytes) throws SAXException,
    IOException, ParserConfigurationException, TransformerException {
    return validateEmbeddedSchema(bytes, XmlUtil.getXmlaXds2xs("xmla"), XmlUtil
      .getXmlaXds2xd("xmla"));
  }
  ...
  /**
   * This validates a SOAP-XMLA response using xpaths to extract the schema and
   * data parts. In addition, it does a little surgery on the DOMs removing the
   * schema nodes from the XMLA root node.
   */
  ...
  /**
   * This validates a XMLA response using xpaths to extract the schema and data
   * parts. In addition, it does a little surgery on the DOMs removing the
   * schema nodes from the XMLA root node.
   * 
   */
  public static boolean validateXmlaUsingXpath(byte[] bytes) throws SAXException,
    IOException {
    if (!XmlUtil.supportsValidation()) {
      return false;
    }
  ...
  /**
   * Validate Nodes with throws an error if validation was attempted but failed,
   * returns true if validation was successful and false if validation was not
   * tried.
   * 
   ...
   * 
   */
  public static boolean validateEmbeddedSchema(byte[] bytes, String schemaTransform,
    String dataTransform) throws SAXException, IOException,
  ...
  /**
   * A given Document has both content and an embedded schema (where the schema
   * has a single root node and the content has a single root node - they are
   * not interwoven). A single xsl transform is provided to extract the schema
   * part of the Document and another xsl transform is provided to extract the
   * content part and then the content is validated against the schema.
   * <p>
   ...
   * <p>
   * When Mondrian moves to Java 5 or includes the JAXP 1.3 jar, then there is a
   * utility in JAXP that does something like this (but allows for multiple
   * schema/content parts).
   * 
   */
  public static boolean validateEmbeddedSchema(Document doc, String schemaTransform,
    String dataTransform) throws SAXException, IOException,
    ParserConfigurationException, TransformerException,
    TransformerConfigurationException {
    if (!XmlUtil.supportsValidation()) {
    ...
    Node dataDoc = XmlUtil.transform(doc, new BufferedReader(new StringReader(
      dataTransform)));
    if (dataDoc == null) {
    ...
    Node schemaDoc = XmlUtil.transform(doc, new BufferedReader(new StringReader(
      schemaTransform)));
    if (schemaDoc == null) {
    ...
    if (!schemaDoc.hasChildNodes()) {
      LOGGER.debug("XmlaSupport.validateEmbeddedSchema: "
  ...
  public static Document transformSoapXmla(Document doc,
    String[][] namevalueParameters, String ns) throws SAXException, IOException,
    ParserConfigurationException, TransformerException {
    Node node = XmlUtil.transform(doc, new BufferedReader(new StringReader(
      getXmlaTransform(ns))), namevalueParameters);
