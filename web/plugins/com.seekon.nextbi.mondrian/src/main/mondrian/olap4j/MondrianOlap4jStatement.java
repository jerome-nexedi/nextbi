/*
// $Id: //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jStatement.java#11 $
...
/**
 * Implementation of {@link org.olap4j.OlapStatement} for the Mondrian OLAP
 * engine.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jStatement
 *          .java#11 $
 * @since May 24, 2007
 */
class MondrianOlap4jStatement extends StatementImpl implements OlapStatement,
  ...
  ResultSet executeQuery2(String mdx, boolean advanced, String tabFields,
    ...
    QueryPart parseTree;
    try {
      parseTree = olap4jConnection.getMondrianConnection().parseStatement(mdx);
    ...
    if (parseTree instanceof DrillThrough) {
      ...
      final List<Integer> coords = Collections.nCopies(cellSet.getAxes().size(), 0);
      final MondrianOlap4jCell cell = (MondrianOlap4jCell) cellSet.getCell(coords);
      ResultSet resultSet = cell.drillThroughInternal(drillThrough.getMaxRowCount(),
        drillThrough.getFirstRowOrdinal(), null, true, null, rowCountSlot);
      if (resultSet == null) {
      ...
      return olap4jConnection.factory.newFixedResultSet(olap4jConnection,
        Collections.singletonList("PLAN"), Collections.singletonList(Collections
          .<Object> singletonList(plan)));
    } else {
      throw olap4jConnection.helper
        .createException("Query does not have relational result. Use a DRILLTHROUGH "
  ...
  public void setQueryTimeout(int seconds) throws SQLException {
    if (seconds < 0) {
      throw olap4jConnection.helper.createException("illegal timeout value "
        + seconds);
    }
  ...
  protected Pair<Query, MondrianOlap4jCellSetMetaData> parseQuery(final String mdx)
    throws OlapException {
    try {
      final RolapConnection mondrianConnection = getMondrianConnection();
      return Locus.execute(mondrianConnection, "Parsing query",
        new Locus.Action<Pair<Query, MondrianOlap4jCellSetMetaData>>() {
          public Pair<Query, MondrianOlap4jCellSetMetaData> execute() {
            final Query query = (Query) mondrianConnection.parseStatement(
              MondrianOlap4jStatement.this, mdx, null, false);
            final MondrianOlap4jCellSetMetaData cellSetMetaData = new MondrianOlap4jCellSetMetaData(
    ...
    synchronized (this) {
      if (openCellSet != null) {
        ...
        try {
          ...
          throw olap4jConnection.helper.createException(null,
