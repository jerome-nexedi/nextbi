/*
// $Id: //open/mondrian/src/main/mondrian/olap/Evaluator.java#40 $
 ...
 * @author jhyde
 * @since 27 July, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/olap/Evaluator.java#40 $
 */
public interface Evaluator {
  ...
  /**
   * Creates a savepoint encapsulating the current state of the evalutor. You
   * can restore the evaluator to this state by calling {@link #restore(int)}
   ...
   * And, it is allowable to restore to the save savepoint more than once (or
   * not at all). However, when you have rolled back to a particular savepoint
   * you may not restore to a later savepoint.
   * 
   ...
   * the current Evaluator for its hierarchy. Other hierarchies retain the same
   * context as this Evaluator.
   * 
   * <p>
   * In mondrian-3.3 and later, a more efficient way to save the state of an
   * evaluator is to call {@link #savepoint} followed by {@link #restore(int)}.
   ...
   * affecting the evaluation context of the calling code. This behavior cannot
   * be achieved using {@code savepoint}.
   * 
   * @return Evaluator with each given member overriding the state of the
   *         current Evaluator for its hierarchy
   */
  ...
  /**
   * Creates a new evaluator with the same state except nonEmpty and
   * nativeEnabled properties.
   * 
   * <p>
   * In mondrian-3.3 and later, a more efficient way to save the state of an
   * evaluator is to call {@link #savepoint} followed by {@link #restore(int)}.
   ...
   *             {@link #setNativeEnabled(boolean)}; will be removed in
   *             mondrian-4.
   */
   ...
   * 
   * @param savepoint
   *          Savepoint returned by {@link #savepoint()}
   */
   ...
   * 
   * <p>
   * If {@code safe}, checks whether this is the first time that a member of
   * this hierarchy has been changed since {@link #savepoint()} was called. If
   * so, saves the previous member. If {@code safe} is false, never saves the
   * previous member.
   * 
   * <p>
   ...
   *          New member
   * @param safe
   *          Whether to store the member of this hierarchy that was current
   *          last time that {@link #savepoint()} was called.
   */
   ...
   *          List of members
   * @param safe
   *          Whether to store the member of each hierarchy that was current
   *          last time that {@link #savepoint()} was called.
   */
  ...
  /**
   * Retrieves the value of property <code>name</code>. If more than one member
   * in the current context defines that property, the one with the highest
   * solve order has precedence.
   * 
   * <p>
  ...
  /**
   * Simple caching of the result of an <code>Exp</code>. The key for the cache
   * consists of all members of the current context that <code>exp</code>
   * depends on. Members of independent hierarchies are not part of the key.
   * 
   ...
   * 
   * <p>
   * May be used by expression evaluators to optimize their result. For example,
   * a top-level crossjoin may be optimized by removing all non-empty set
   * elements before performing the crossjoin. This is possible because of the
   * identity
   * 
   ...
   *          Named set
   * @param create
   *          Whether to create evaluator if not found
   ...
   * Notes:
   * <ul>
   * <li>The 0th element is a measure, but otherwise the order of the members is
   * unspecified.
   ...
   * @param list
   *          List of tuples
   * @return Aggregator with <code>list</code> added to its aggregation context
   */
   ...
   * should be ignored.
   * 
   * @return whether hierarchies unrelated to the measure in the current context
   *         should be ignored
   */
   ...
   * {@link #needToReturnNullForUnrelatedDimension(mondrian.olap.Member[])} will
   * always return false.
   * 
   ...
   * members are on unrelated dimension for the measure in current context then
   * returns true.
   * 
   * <p>
   ...
   * 
   * @param members
   *          Dimensions for the members need to be checked whether related or
   *          unrelated
   * 
   * @return boolean
   */
  ...
  interface NamedSetEvaluator {
    /**
     * Returns an iterator over the tuples of the named set. Applicable if the
     * named set is a set of tuples.
     * 
     * <p>
     * The iterator from this iterable maintains the current ordinal property
     * required for the methods {@link #currentOrdinal()} and
     * {@link #currentTuple()}.
     * 
