/*
// $Id: //open/mondrian/src/main/mondrian/olap/type/TupleType.java#19 $
 ...
 * @author jhyde
 * @since Feb 17, 2005
 * @version $Id: //open/mondrian/src/main/mondrian/olap/type/TupleType.java#19 $
 */
public class TupleType implements Type {
   ...
   * 
   * @param elementTypes
   *          Array of types of the members in this tuple
   */
  ...
  public List<Hierarchy> getHierarchies() {
    final List<Hierarchy> hierarchies = new ArrayList<Hierarchy>(elementTypes.length);
    for (Type elementType : elementTypes) {
  ...
  public Type computeCommonType(Type type, int[] conversionCount) {
    ...
    if (type instanceof MemberType) {
      return commonTupleType(new TupleType(new Type[] { type }), conversionCount);
    }
  ...
  private Type createCommonTupleType(TupleType that, int[] conversionCount) {
    ...
    for (int i = 0; i < this.elementTypes.length; i++) {
      Type commonType = this.elementTypes[i].computeCommonType(that.elementTypes[i],
        conversionCount);
  ...
  /**
   * Checks that there are no duplicate dimensions in a list of member types. If
   * so, the member types will form a valid tuple type. If not, throws
   * {@link mondrian.olap.MondrianException}.
   * 
   * @param memberTypes
    ...
    for (int i = 0; i < memberTypes.length; i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (hierarchy != null && hierarchy == hierarchy1) {
          throw MondrianResource.instance().DupHierarchiesInTuple.ex(hierarchy
            .getUniqueName());
        }
      }
    }
