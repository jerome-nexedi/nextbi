 ...
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/JavaFunDef.java#9 $
 * @since Jan 5, 2008
 */
public class JavaFunDef extends FunDefBase {
   ...
   *          Return type
   * @param paramCategories
   *          Parameter types
   * @param method
   *          Java method which implements this function
   */
  public JavaFunDef(String name, String desc, Syntax syntax, int returnCategory,
    int[] paramCategories, Method method) {
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    for (int i = 0; i < calcs.length; i++) {
      calcs[i] = compileTo(compiler, call.getArgs()[i], parameterTypes[i]);
    }
  ...
  private static FunDef generateFunDef(final Method method) {
    String name = getAnnotation(method, className + "$FunctionName", method
      .getName());
    String desc = getAnnotation(method, className + "$Description", "");
    Syntax syntax = getAnnotation(method, className + "$SyntaxDef", Syntax.Function);
    ...
    return new JavaFunDef(name, desc, syntax, returnCategory, paramCategories,
      method);
  }

  /**
   * Scans a java class and returns a list of function definitions, one for each
   * static method which is suitable to become an MDX function.
   * 
   * @param clazz
   *          Class
   ...
   * 
   * <p>
   * Since the result of evaluating the calc will be passed to the method using
   * reflection, it is important that the calc returns <em>precisely</em> the
   * correct type: if a method requires an <code>int</code>, you can pass an
   * {@link Integer} but not a {@link Long} or {@link Float}.
   * 
   * <p>
   * If it can be determined that the underlying calc will never return null,
   * generates an optimal form with one fewer object instantiation.
   * 
   * @param compiler
   *          Compiler
   * @param exp
    ...
    if (clazz == String.class) {
      ...
      return new AbstractCalc2(exp, stringCalc) {
        public Object evaluate(Evaluator evaluator) {
          ...
          return Character.valueOf(string == null || string.length() < 1 ? (char) 0
            : string.charAt(0));
        }
      };
  ...
  /**
   * Annotation which allows you to tag a Java method with the name of the MDX
  ...
  /**
   * Annotation which allows you to tag a Java method with the description of
  ...
  /**
   * Annotation which allows you to tag a Java method with the signature of the
  ...
  /**
   * Annotation which allows you to tag a Java method with the syntax of the MDX
   * function it implements.
   */
  @Retention(RetentionPolicy.RUNTIME)
  @Target(ElementType.METHOD)
  ...
  /**
   * Base class for adapter calcs that convert arguments into the precise type
   * needed.
   */
  ...
  private static class JavaMethodCalc extends GenericCalc {
     ...
     * 
     * @param call
     *          Function call being implemented
     * @param calcs
     *          Calcs for arguments of function call
     * @param method
     *          Method to call
     */
