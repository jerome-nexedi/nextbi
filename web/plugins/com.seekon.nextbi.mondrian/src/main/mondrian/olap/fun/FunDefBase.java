/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/FunDefBase.java#37 $
 ...
 * <tr>
 * <td>flags</td>
 * <td>Encoding of the syntactic type, return type, and parameter types of this
 * operator. The encoding is described below.</td>
 * <td>"pxd"
 * </tr>
 * </table>
 * 
 * The <code>flags</code> field is an string which encodes the syntactic type,
 ...
 * <li>The third and subsequence characters determine the types of the arguments
 * arguments, as described by {@link FunUtil#decodeParameterCategories(String)}.
 * </ul>
 * <p/>
 * 
 * For example, <code>"pxd"</code> means "an operator with
 * {@link Syntax#Property property} syntax (p) which returns a set (x) and takes
 * a dimension (d) as its argument".
 * <p/>
 * 
 * The arguments are always read from left to right, regardless of the syntactic
 * type of the operator. For example, the
 * <code>"&lt;Set&gt;.Item(&lt;Index&gt;)"</code> operator (signature
 * <code>"mmxn"</code>) has the syntax of a method-call, and takes two
 * parameters: a set (x) and a numeric (n).
 * <p/>
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/FunDefBase.java#37 $
...
public abstract class FunDefBase extends FunUtil implements FunDef {
   ...
   * @param syntax
   *          Syntactic type of the operator (for example, function, method,
   *          infix operator)
   * @param returnCategory
   *          The {@link Category} of the value returned by this operator.
   * @param parameterCategories
   *          An array of {@link Category} codes, one for each parameter.
   */
  FunDefBase(String name, String signature, String description, Syntax syntax,
    int returnCategory, int[] parameterCategories) {
    assert name != null;
    assert syntax != null;
    this.name = name;
   ...
   *          "Returns the set of all members in a dimension."
   * @param flags
   *          Encoding of the syntactic type, return type, and parameter types
   *          of this operator. The "Members" operator has a syntactic type
   *          "pxd" which means "an operator with {@link Syntax#Property
   *          property} syntax (p) which returns a set (x) and takes a dimension
   *          (d) as its argument". See
   *          {@link FunUtil#decodeSyntacticType(String)},
   *          {@link FunUtil#decodeReturnCategory(String)},
   *          {@link FunUtil#decodeParameterCategories(String)}.
   */
  protected FunDefBase(String name, String description, String flags) {
    this(name, null, description, decodeSyntacticType(flags),
      decodeReturnCategory(flags), decodeParameterCategories(flags));
  }
   ...
   * 
   * <p>
   * In most cases, the signature can be generated automatically, and you should
   * use the constructor which creates an implicit signature,
   * {@link #FunDefBase(String, String, String, String)} instead.
   * 
   * @param name
   *          Name of the function, for example "Members".
   * @param signature
   ...
   *          {@link FunUtil#decodeParameterCategories(String)}.
   */
  protected FunDefBase(String name, String signature, String description,
    String flags) {
    this(name, signature, description, decodeSyntacticType(flags),
      decodeReturnCategory(flags), decodeParameterCategories(flags));
  }
  ...
  FunDefBase(FunDef funDef) {
    this(funDef.getName(), funDef.getSignature(), funDef.getDescription(), funDef
      .getSyntax(), funDef.getReturnCategory(), funDef.getParameterCategories());
  }
   ...
   * 
   * <p>
   * The default implementation of this method adds an implicit conversion to
   * the correct type. Derived classes may override.
   * 
   * @param validator
   *          Validator
   * @param args
   ...
   *          Ordinal of argument
   * @param category
   *          Expected {@link Category category} of argument
