...
class OrderFunDef extends FunDefBase {
      ...
      if (expCalc.isWrapperFor(MemberValueCalc.class)
        ...
        if (constantList.isEmpty()) {
          ...
          calcList[1] = MemberValueCalc.create(new DummyExp(expCalc.getType()),
            variableList.toArray(new MemberCalc[variableList.size()]), compiler
              .getEvaluator().mightReturnNullForUnrelatedDimension());
          return new ContextCalc(constantList.toArray(new MemberCalc[constantList
            .size()]), new CalcImpl(call, calcList, keySpecList));
        }
      }
    }
  ...
  private void buildKeySpecList(List<SortKeySpec> keySpecList, ResolvedFunCall call,
  ...
  private static class CalcImpl extends AbstractListCalc implements CalcWithDual {
    ...
    public TupleList evaluateDual(Evaluator rootEvaluator, Evaluator subEvaluator) {
      ...
      if (arity == 1) {
        tupleList = new UnaryTupleList(sortMembers(subEvaluator, iterable.slice(0),
          list == null ? null : list.slice(0), sortKeyCalc, sortKeyDir.descending,
          sortKeyDir.brk));
      } else {
    ...
    public TupleList evaluateList(Evaluator evaluator) {
      ...
      if (originalKeySpecCount == 1) {
        ...
        if (arity == 1) {
          tupleList = new UnaryTupleList(sortMembers(evaluator, iterable.slice(0),
            list == null ? null : list.slice(0), sortKeyCalc, sortKeyDir.descending,
            sortKeyDir.brk));
        } else {
        ...
        if (arity == 1) {
          tupleList = new UnaryTupleList(sortMembers(evaluator, iterable.slice(0),
            list == null ? null : list.slice(0), keySpecList));
        } else {
    ...
    private void purgeKeySpecList(List<SortKeySpec> keySpecList, TupleList list) {
      ...
      List<Hierarchy> listHierarchies = new ArrayList<Hierarchy>(list.getArity());
