/*
// $Id: //open/mondrian/src/main/mondrian/olap/ConnectionBase.java#32 $
 ...
 * @author jhyde
 * @since 6 August, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/olap/ConnectionBase.java#32 $
 */
public abstract class ConnectionBase implements Connection {
  ...
  public QueryPart parseStatement(String query) {
    ...
    final Locus locus = new Locus(new Execution(statement, 0),
      "Parse/validate MDX statement", null);
    Locus.push(locus);
    try {
   ...
   * 
   * <p>
   * This method is only used in testing and by clients that need to support
   * customized parser behavior. That is why this method is not part of the
   ...
   *          Evaluation context
   * @param query
   *          MDX query that requires special parsing
   * @param funTable
   *          Customized function table to use in parsing
   ...
   *           if parsing fails
   */
  public QueryPart parseStatement(Statement statement, String query,
    ...
    if (getLogger().isDebugEnabled()) {
      // debug = true;
      getLogger().debug(Util.nl + query);
    }

    try {
      return parser.parseInternal(statement, query, debug, funTable,
        strictValidation);
    } catch (Exception e) {
  ...
  private MdxParserValidator createParser() {
    return true ? new JavaccParserValidatorImpl() : new MdxParserValidatorImpl();
  }
