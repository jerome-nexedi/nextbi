/*
// $Id: //open/mondrian/src/main/mondrian/olap/MemberBase.java#52 $
 ...
 * @author jhyde
 * @since 6 August, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/olap/MemberBase.java#52 $
 */
public abstract class MemberBase extends OlapElementBase implements Member {
  ...
  /**
   * Combines member type and other properties, such as whether the member is
   * the 'all' or 'null' member of its hierarchy and whether it is a measure or
   * is calculated, into an integer field.
   * 
   * <p>
   * The fields are:
   * <ul>
   ...
   * hierarchy;
   * <li>bit 6 ({@link #FLAG_CALCULATED}) is set if this is a calculated member.
   ...
   * made each subclass implement 'boolean isNull()' - it would be slower. We
   * use one flags field rather than 4 boolean fields to save space.
   */
  ...
  protected MemberBase(Member parentMember, Level level, MemberType memberType) {
  ...
  /**
   * Returns whether this <code>Member</code>'s unique name is equal to, a child
   * of, or a descendent of a member whose unique name is
  ...
  /**
   * Computes the value to be returned by {@link #isCalculated()}, so it can be
  ...
  public List<Member> getAncestorMembers() {
    final SchemaReader schemaReader = getDimension().getSchema().getSchemaReader();
  ...
  /**
   * Returns the ordinal of this member within its hierarchy. The default
  ...
  /**
   * Returns the order key of this member among its siblings. The default
