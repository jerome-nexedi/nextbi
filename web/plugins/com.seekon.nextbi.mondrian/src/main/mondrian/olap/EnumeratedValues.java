/*
// $Id: //open/mondrian/src/main/mondrian/olap/EnumeratedValues.java#23 $
 ...
 * {@link Access} is a simple example of this.
 * </p>
 */
public class EnumeratedValues<V extends EnumeratedValues.Value> implements Cloneable {
  ...
  /**
   * An array mapping ordinals to {@link Value}s. It is biased by the min value.
   * It is built by {@link #makeImmutable}.
   */
  ...
  /**
   * Creates an enumeration, initialize it with an array of strings, and freezes
   * it.
   */
  ...
  public void makeImmutable() {
    ...
    for (Value value : valuesByName.values()) {
      ...
      if (ordinalToValueMap[index] != null) {
        throw Util.newInternal("Enumeration has more than one value with ordinal "
          + value.getOrdinal());
      }
   ...
   * @throws Error
   *           if the name is not a member of the enumeration and
  ...
  /**
   * Returns an error indicating that the value is illegal. (The client needs to
   * throw the error.)
   */
  public RuntimeException badValue(int ordinal) {
    return Util.newInternal("bad value " + ordinal + "(" + getName(ordinal)
   ...
   * enumerated type is not based upon an explicit array of values, an array of
   * {@link BasicValue}s will implicitly be created.
   */
  ...
  public static class BasicValue implements Value {
     ...
     * 
     * @deprecated I bet you meant to write <code>value.name_.equals(s)</code>
     *             rather than <code>value.equals(s)</code>, didn't you?
     */
    ...
    /**
     * Returns an error indicating that we did not expect to find this value in
     * this context. Typical use is in a <code>switch</code> statement:
     * 
     * <blockquote>
     * 
     * <pre>
