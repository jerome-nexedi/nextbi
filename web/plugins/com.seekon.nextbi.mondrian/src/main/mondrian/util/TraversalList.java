/*
// $Id: //open/mondrian/src/main/mondrian/util/TraversalList.java#5 $
...
/**
 * Implementation of {@link java.util.List} for transposing an array of lists.
 * 
 * @author Luis F. Canals
 * @version $Id: //open/mondrian/src/main/mondrian/util/TraversalList.java#5 $
 * @since Dec, 2007
 */
public class TraversalList<T> extends UnsupportedList<List<T>> {
  ...
  public TraversalList(final List<T>[] lists, Class<T> clazz) {
    this.lists = lists;
    this.clazz = clazz;
  ...
  private List<T>[] materialize(List<T>[] a) {
    final List<T>[] array;
    if (a != null && a.length == size() && a.getClass().getComponentType() == clazz) {
      array = a;
    } else {
      // noinspection unchecked
