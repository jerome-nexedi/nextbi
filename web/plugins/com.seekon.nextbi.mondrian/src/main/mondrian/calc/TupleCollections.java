/*
// $Id: //open/mondrian/src/main/mondrian/calc/TupleCollections.java#4 $
 ...
 * @see TupleIterator
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/calc/TupleCollections.java#4
 *          $
 * @author jhyde
 */
public final class TupleCollections {
   ...
   * 
   * <p>
   * If arity == 1, creates a {@link UnaryTupleList}; if arity == 0, creates a
   * {@link DelegatingTupleList}; otherwise creates a {@link ArrayTupleList}.
   * 
   ...
   * 
   * @param arity
   *          Number of members per tuple
   * @return Empty tuple list
   */
  public static TupleList emptyList(int arity) {
    return arity < EMPTY_LISTS.length ? EMPTY_LISTS[arity]
      : new DelegatingTupleList(arity, Collections.<List<Member>> emptyList());
  }
  ...
  public static TupleList unmodifiableList(TupleList list) {
    return list.getArity() == 1 ? new UnaryTupleList(Collections
      .unmodifiableList(list.slice(0))) : new DelegatingTupleList(list.getArity(),
   ...
   * 
   * <p>
   * Since the latter is a more difficult API to implement, the wrapper has some
   * extra state.
   * 
   * <p>
   * This method may be used to implement
   * {@link mondrian.calc.TupleIterable#tupleIterator()} for a
   * {@link TupleIterable} or {@link TupleList} that only has a {@code
   * TupleCursor} implementation.
   * 
   * @param cursor
   *          Cursor
   ...
   *          Iterable
   * @param column
   *          Which member of each tuple of project.
   * @return Iterable that returns a given member of each tuple
   */
  public static Iterable<Member> slice(final TupleIterable tupleIterable,
    final int column) {
  ...
  /**
   * Converts a {@link mondrian.calc.TupleIterable} to an old-style iterable
   * that creates an iterator over member arrays.
   * 
   * @param tupleIterable
   *          Tuple iterable
   * @return Iterable that creates an iterator over tuples represented as member
   *         arrays
   */
   ...
   * 
   * <p>
   * Deduces the arity of the list from the first element, if the list is not
   * empty. Otherwise assumes arity 1.
   * 
   * <p>
  ...
  public static TupleList asTupleList(List list) {
    ...
    if (o instanceof Member) {
      ...
      return new DelegatingTupleList(memberArrayList.get(0).length,
   ...
   * 
   * <p>
   * If the iterable is already a list, returns the iterable. If it is not a
   * list, the behavior depends on the {@code eager} parameter. With eager =
   * true, creates a list and populates it with the contents of the iterable.
   * With eager = false, wraps in an adapter that implements the list interface
   * and materializes to a list the first time that an operation that is in
   * TupleList but not TupleIterable -- for example, {@link TupleList#get} or
   * {@link TupleList#size} -- is called.
   * 
   * @param tupleIterable
   *          Iterable
   * @param eager
   *          Whether to convert into a list now, as opposed to on first use of
   *          a random-access method such as size or get.
   * @return List
   */
   ...
   * {@link TupleIterable} and materializes into a read-only list the first time
   * that an method is called that requires a list.
   */
  private static class MaterializingTupleList implements TupleList {
