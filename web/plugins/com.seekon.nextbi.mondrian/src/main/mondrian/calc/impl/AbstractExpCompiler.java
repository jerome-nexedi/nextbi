...
public class AbstractExpCompiler implements ExpCompiler {
  ...
  public Calc compileAs(Exp exp, Type resultType,
    ...
    if (Util.Retrowoven) {
      ...
      List<ResultStyle> tmp = new ArrayList<ResultStyle>(preferredResultTypes.size());
  ...
  private MemberCalc hierarchyToMember(HierarchyCalc hierarchyCalc) {
    ...
    if (hierarchy != null) {
      return new HierarchyCurrentMemberFunDef.FixedCalcImpl(new DummyExp(TypeUtil
        .toMemberType(hierarchyCalc.getType())), hierarchy);
    } else {
      return new HierarchyCurrentMemberFunDef.CalcImpl(new DummyExp(TypeUtil
        .toMemberType(hierarchyCalc.getType())), hierarchyCalc);
    }
  }
  ...
  public LevelCalc compileLevel(Exp exp) {
    ...
    if (type instanceof MemberType) {
      ...
      return new MemberLevelFunDef.CalcImpl(new DummyExp(LevelType.forType(type)),
        memberCalc);
    }
  ...
  public DimensionCalc compileDimension(Exp exp) {
    ...
    if (type instanceof HierarchyType) {
      final HierarchyCalc hierarchyCalc = compileHierarchy(exp);
      return new HierarchyDimensionFunDef.CalcImpl(new DummyExp(new DimensionType(
        type.getDimension())), hierarchyCalc);
    }
  ...
  public HierarchyCalc compileHierarchy(Exp exp) {
    ...
    if (type instanceof DimensionType) {
      ...
      return new DimensionHierarchyCalc(new DummyExp(HierarchyType.forType(type)),
        dimensionCalc);
    }
    if (type instanceof MemberType) {
      // <Member> --> <Member>.Hierarchy
      final MemberCalc memberCalc = compileMember(exp);
      return new MemberHierarchyFunDef.CalcImpl(new DummyExp(HierarchyType
        .forType(type)), memberCalc);
    }
    if (type instanceof LevelType) {
      // <Level> --> <Level>.Hierarchy
      final LevelCalc levelCalc = compileLevel(exp);
      return new LevelHierarchyFunDef.CalcImpl(new DummyExp(HierarchyType
        .forType(type)), levelCalc);
    }
  ...
  public IntegerCalc compileInteger(Exp exp) {
    ...
    if (type instanceof DecimalType && ((DecimalType) type).getScale() == 0) {
      ...
      if (calc instanceof ConstantCalc) {
        ...
        return new ConstantCalc(new DecimalType(Integer.MAX_VALUE, 0), constantCalc
          .evaluateInteger(null));
  ...
  public Calc compileScalar(Exp exp, boolean specific) {
    ...
    if (type instanceof MemberType) {
      ...
      final TupleValueCalc scalarCalc = new TupleValueCalc(new DummyExp(tupleType
        .getValueType()), tupleCalc, getEvaluator()
