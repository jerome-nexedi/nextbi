/*
// $Id: //open/mondrian/src/main/mondrian/calc/impl/MemberValueCalc.java#12 $
...
/**
 * Expression which evaluates a few member expressions, sets the dimensional
 * context to the result of those expressions, then yields the value of the
 * current measure in the current dimensional context.
 * 
 * <p>
 ...
 * {@link mondrian.calc.impl.ValueCalc}; see also
 * {@link mondrian.calc.impl.TupleValueCalc}.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/calc/impl/MemberValueCalc.java#12
 *          $
 * @since Sep 27, 2005
 */
public class MemberValueCalc extends GenericCalc {
   ...
   *          Expression
   * @param memberCalcs
   *          Array of members to evaluate
   * @param nullCheck
   *          Whether to check for null values due to non-joining dimensions in
   *          a virtual cube
   * @return Compiled expression to evaluate each member expression, set
   *         evaluator context to each resulting member, then evaluate the
   *         current context
   */
  public static GenericCalc create(Exp exp, MemberCalc[] memberCalcs,
    boolean nullCheck) {
  ...
  public Object evaluate(Evaluator evaluator) {
    ...
    if (nullCheck
      && evaluator.needToReturnNullForUnrelatedDimension(new Member[] { member })) {
