...
public class XmlaHandler {
  ...
  private void checkFormat(XmlaRequest request) throws XmlaException {
    ...
    if (request.isDrillThrough()) {
      ...
      final String axisFormatName = properties.get(PropertyDefinition.AxisFormat
        .name());
  ...
  static void writeDatasetXmlSchema(SaxWriter writer, SetType settype) {
    ...
    writer.startElement("xsd:schema", "xmlns:xsd", NS_XSD, "targetNamespace",
      setNsXmla, "xmlns", setNsXmla, "xmlns:xsi", NS_XSI, "xmlns:sql", NS_XML_SQL,
    ...
    { // <AxesInfo>
      ...
      { // <AxisInfo>
        ...
        { // <HierarchyInfo>
          writer.startElement("xsd:element", "name", "HierarchyInfo", "minOccurs",
          ...
          writer.element("xsd:element", "name", "DisplayInfo", "type", "PropType",
          ...
          writer.endElement(); // xsd:sequence
          writer.element("xsd:attribute", "name", "name", "type", XSD_STRING, "use",
            "required");
          writer.endElement(); // xsd:complexType
  ...
  private QueryResult executeDrillThroughQuery(XmlaRequest request)
    ...
    try {
      ...
      throw new XmlaException(SERVER_FAULT_FC, HSB_DRILL_THROUGH_SQL_CODE,
        HSB_DRILL_THROUGH_SQL_FAULT_FS, Util
          .newError(sqle, "Error in drill through"));
  ...
  static class TabularRowSet implements QueryResult {
    ...
    public TabularRowSet(ResultSet rs, int totalCount) throws SQLException {
      ...
      for (int i = 0; i < columnCount; i++) {
        columns.add(new Column(md.getColumnLabel(i + 1), md.getColumnType(i + 1), md
          .getScale(i + 1)));
      }
    ...
    public void unparse(SaxWriter writer) throws SAXException {
      ...
      for (Object[] row : rows) {
        ...
        for (int i = 0; i < row.length; i++) {
          writer.startElement(columns.get(i).encodedName, new Object[] { "xsi:type",
    ...
    public void metadata(SaxWriter writer) {
      writer.startElement("xsd:schema", "xmlns:xsd", NS_XSD, "targetNamespace",
        NS_XMLA_ROWSET, "xmlns", NS_XMLA_ROWSET, "xmlns:xsi", NS_XSI, "xmlns:sql",
      ...
      { // xsd:complexType name="row"
        ...
        writer.startElement("xsd:sequence");
        for (Column column : columns) {
          writer.element("xsd:element", "minOccurs", 0, "name", column.encodedName,
            "sql:field", column.name, "type", column.xsdType);
        }
  ...
  static abstract class MDDataSet implements QueryResult {
    ...
    protected static final List<Property> cellProps = Arrays.asList(rename(
      StandardCellProperty.VALUE, "Value"), rename(
      StandardCellProperty.FORMATTED_VALUE, "FmtValue"), rename(
      StandardCellProperty.FORMAT_STRING, "FormatString"));

    protected static final List<StandardCellProperty> cellPropLongs = Arrays.asList(
      StandardCellProperty.VALUE, StandardCellProperty.FORMATTED_VALUE,
      StandardCellProperty.FORMAT_STRING);

    protected static final List<Property> defaultProps = Arrays.asList(rename(
      StandardMemberProperty.MEMBER_UNIQUE_NAME, "UName"), rename(
      StandardMemberProperty.MEMBER_CAPTION, "Caption"), rename(
      StandardMemberProperty.LEVEL_UNIQUE_NAME, "LName"), rename(
      StandardMemberProperty.LEVEL_NUMBER, "LNum"), rename(
      StandardMemberProperty.DISPLAY_INFO, "DisplayInfo"));
  ...
  static class MDDataSet_Multidimensional extends MDDataSet {
    ...
    private void olapInfo(SaxWriter writer) throws OlapException {
      ...
      if (omitDefaultSlicerInfo) {
        ...
        List<Dimension> unseenDimensionList = new ArrayList<Dimension>(cube
          .getDimensions());
    ...
    private void cellProperty(SaxWriter writer, StandardCellProperty cellProperty,
      boolean evenEmpty, String elementName) {
    ...
    private void writeHierarchyInfo(SaxWriter writer, List<Hierarchy> hierarchies,
      List<Property> props) {
        ...
        for (Hierarchy hierarchy : hierarchies) {
          ...
          if (member != null) {
            ...
            LOGGER.warn("Can not create SlicerAxis: "
              + "null default member for Hierarchy " + hierarchy.getUniqueName());
          }
        }
    ...
    private void writeMember(SaxWriter writer, Member member, Position prevPosition,
      Position nextPosition, int k, List<Property> props) throws OlapException {
    ...
    private void slicerAxis(SaxWriter writer, Member member, List<Property> props)
      ...
      if (!empty) {
        for (int i = axes.size() - 1; i > 0; i--) {
          ...
          for (Position position : positions) {
            ...
            for (Member member : position.getMembers()) {
              if (jj == 0
                || member.getLevel().getDepth() > levels[memberOrdinal].getDepth()) {
  ...
  public interface XmlaExtra {
    ...
    ResultSet executeDrillthrough(OlapStatement olapStatement, String mdx,
      boolean advanced, String tabFields, int[] rowCountSlot) throws SQLException;
    ...
    class FunctionDefinition {
      ...
      public FunctionDefinition(String functionName, String description,
        String parameterList, int returnType, int origin, String interfaceName,
        String caption) {
  ...
  private static class XmlaExtraImpl implements XmlaExtra {
    public ResultSet executeDrillthrough(OlapStatement olapStatement, String mdx,
      boolean advanced, String tabFields, int[] rowCountSlot) throws SQLException {
    ...
    public boolean shouldReturnCellProperty(CellSet cellSet, Property cellProperty,
      boolean evenEmpty) {
      return true;
    }
    ...
    public List<Map<String, Object>> getDataSources(OlapConnection connection)
      ...
      return Collections.singletonList(Olap4jUtil.mapOf("DataSourceName",
        (Object) olapDb.getName(), "DataSourceDescription", olapDb.getDescription(),
        "URL", olapDb.getURL(), "DataSourceInfo", olapDb.getDataSourceInfo(),
        "ProviderName", olapDb.getProviderName(), "ProviderType", providerTypes,
  ...
  public interface ConnectionFactory {
     ...
     *           on error
     */
    OlapConnection getConnection(String catalog, String schema, String roleName,
