...
/**
 * A <code>FormatHandler</code> defines an additional abstraction layer to the
 * persistence handling of cube views. This provides the possibility to write
 * and read different cube view versions. Right now we have to supported cube
 * view versions which are handled by <code>{@link FormatHandler1_0}</code> and
 * <code>{@link CubeViewHandler1_1}</code> respectively.
 * 
 * @author ArndHouben
 * @version $Id: FormatHandler.java,v 1.2 2009/12/17 16:14:29 PhilippBouillon
 *          Exp $
 **/
abstract class FormatHandler {
  private final HashMap<String, IPaloEndHandler> endHandlers = new HashMap<String, IPaloEndHandler>();

  private final HashMap<String, IPaloStartHandler> startHandlers = new HashMap<String, IPaloStartHandler>();
