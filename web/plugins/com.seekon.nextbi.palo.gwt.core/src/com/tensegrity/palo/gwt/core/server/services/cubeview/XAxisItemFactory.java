...
/**
 * <code>XViewItemFactory</code> TODO DOCUMENT ME
 * 
 * @version $Id: XAxisItemFactory.java,v 1.8 2010/03/11 10:42:20 PhilippBouillon
 *          Exp $
 **/
class XAxisItemFactory {
  ...
  static final XAxisItem createX(AxisItem root, int level, int depth,
    XAxisItem parent) {
  ...
  private static final XAxisItem internalCreateX(AxisItem forItem, int level,
    ...
    XAxisItem viewItem = new XAxisItem(forItem.getPath(), forItem.getName(),
      hierarchyId);
    viewItem.index = forItem.index;
    viewItem.depth = depth;
    viewItem.level = level;
    viewItem.elementType = XElementType.fromString(forItem.getElement()
      .getTypeAsString());
    ...
    if (forItem.hasRootsInNextHierarchy()) {
      for (AxisItem root : forItem.getRootsInNextHierarchy())
        viewItem.addRootInNextHierarchy(internalCreateX(root, level + 1, 0, null));
    }
