...
/**
 * <code>CubeViewController</code> TODO DOCUMENT ME
 * 
 * @version $Id: CubeViewController.java,v 1.81 2010/04/15 09:54:49
 *          PhilippBouillon Exp $
 **/
public class CubeViewController {
  ...
  public static final synchronized CubeViewController getController(AuthUser user,
    String sessionId, View forView, HttpSession session) throws PaloGwtCoreException {
    ...
    if (controller == null) {
      try {
        ...
        ConnectionPoolManager.getInstance().disconnect(forView.getAccount(),
          sessionId, "CubeViewController.getController");
      }
    }
    return controller;
  }

  // public static final CubeViewController getController(UserSession
  // userSession, View forView) {
  ...
  // private static Map<String, CubeViewController>
  // getOrCreateViewControllersMap(
  ...
  private static Map<String, CubeViewController> getOrCreateViewControllersMap(
    String userSession) {
    Map<String, CubeViewController> viewControllers = session2controller
      .get(userSession);
    if (viewControllers == null) {
      viewControllers = new HashMap<String, CubeViewController>();
  ...
  // public static final CubeViewController getController(UserSession
  // userSession, String forView) {
  // Map<String, CubeViewController> viewControllers =
  // getOrCreateViewControllersMap(userSession);
  // return viewControllers.get(forView);
  // }

  public static final CubeViewController getController(String sessionId,
    String forView) {
    Map<String, CubeViewController> viewControllers = getOrCreateViewControllersMap(sessionId);
    if (forView == null) {
      // Thread.dumpStack();
  ...
  public static final void removeAllViews(UserSession userSession) {
    Map<String, CubeViewController> viewControllers = session2controller
      .get(userSession.getSessionId());
    if (viewControllers != null)
      viewControllers.clear();
  }

  public static final void removeAllViewsAndShutdownConnectionPool(
    UserSession userSession) {
    Map<String, CubeViewController> viewControllers = session2controller
      .get(userSession.getSessionId());
    if (viewControllers != null) {
      ...
      for (Account account : accounts) {
        ConnectionPoolManager.getInstance().getPool(account,
          userSession.getSessionId()).disconnectAll();
      }
      if (accounts.isEmpty()) {
          ...
          for (Account account : userSession.getUser().getAccounts()) {
            if (account != null) {
              ServerConnectionPool pool = ConnectionPoolManager.getInstance()
                .onlyGetPool(account, userSession.getSessionId());
              if (pool != null) {
                pool.disconnectAll();
              }
            }
  ...
  private final XLoadInfo createLoadInfo(int loadCells, int visibleCells,
    boolean newModel) {
    XLoadInfo loadInfo = new XLoadInfo();
    loadInfo.loadCells = loadCells;
    loadInfo.visibleCells = visibleCells < 0 ? viewModelController
      .getVisibleCellCount() : visibleCells;
    loadInfo.totalCells = newModel ? loadCells : viewModelController.getLoadLevel()
      + loadInfo.loadCells;
  ...
  public final synchronized XViewModel proceedOpenView(UserSession userSession,
    XViewModel oldView, AuthUser user, NumberFormat format)
    throws PaloGwtCoreException, PaloAPIException {
    Expanded expanded = initWithCurrentState();
    XViewModel xViewModel = createXView(userSession, user, viewModelController
      .getModel());
  ...
  public final XViewModel createXView(UserSession userSession, AuthUser user,
    ViewModel model) throws PaloGwtCoreException {
    ...
    for (Account acc : user.getAccounts()) {
      ...
      if (acc.getLoginName().equals(view.getAccount().getLoginName())) {
        if (acc.getConnection().getHost().equals(
          view.getAccount().getConnection().getHost())
          && acc.getConnection().getService().equals(
            view.getAccount().getConnection().getService())) {
          accountToUse = acc;
        }
      } else {
        if (acc.getConnection().getHost().equals(
    ...
    if (!accountToUse.getId().equals(view.getAccount().getId())) {
      try {
        ...
        ConnectionPoolManager.getInstance().disconnect(accountToUse,
          userSession.getSessionId(), "CubeViewController.createXView");
      }
    }
    XViewModel xViewModel = XViewModelFactory.createX(model, view.getId(),
      userSession);
    ...
    if (cView != null) {
      ...
      boolean revH = "true".equalsIgnoreCase((String) view.getCubeView()
        .getPropertyValue(CubeView.PROPERTY_ID_REVERSE_HORIZONTAL_LAYOUT));
      boolean revV = "true".equalsIgnoreCase((String) view.getCubeView()
        .getPropertyValue(CubeView.PROPERTY_ID_REVERSE_VERTICAL_LAYOUT));
  ...
  public final XCellCollection loadCells(Expanded expanded, NumberFormat format)
    throws PaloAPIException {
    try {
      List<ViewCell> cells = expanded.getCellsToLoad();
      if (cells != null) {
        Element[][] coordinates = getCoordinatesFor(cells);
        XCellCollection cellCollection = getCells(coordinates, expanded, format);
  ...
  private final Element[][] getCoordinatesFor(List<ViewCell> cells) {
    ...
    AxisHierarchy[] selectionHierarchies = viewModel.getRepositoryAxis()
      .getAxisHierarchies();
  ...
  private final void fillCoordinateWithSelectedElements(Element[] coordinate,
    ...
    for (int i = 0; i < selectionHierarchies.length; ++i) {
      int index = hierarchyIndex.get(selectionHierarchies[i].getHierarchy());
      Element[] selElements = selectionHierarchies[i].getSelectedElements();
  ...
  private final XCellCollection getCells(Element[][] coordinates, Expanded expanded,
    NumberFormat format) throws PaloAPIException {
    ...
    if (expanded.inColumn()) {
      adjustNotEmptyColumnIndices(xCells.getInsertIndex(), xCells
  ...
  private final XCell createXCell(Cell cell, int row, int col, NumberFormat format) {
    ...
    xCell.type = cell.getType() == Cell.NUMERIC ? XCell.TYPE_NUMERIC
      : XCell.TYPE_STRING;
    format(xCell, cell);
  ...
  private final RangePosition getPositionInRange(FormatRangeInfo range, Cell cell) {
    Element[][] cells = range.getCells();
    Element[] coordinate = cell.getCoordinate();
    Property<Object> rangeProperty = getCubeView().getProperty(
      FormatConverter.PROPERTY_FORMAT_RANGES);
      ...
      if (fromTo != null) {
        ...
        for (int i = 0; i < cells.length; i++) {
          if (coordinatesAreEqual(cells[i], coordinate)) {
            RangePosition positionInRange = RangePosition.getPositionInRange(i,
  ...
  private final boolean coordinatesAreEqual(Element[] coord1, Element[] coord2) {
  ...
  public final XViewModel proceedSwapAxes(UserSession userSession, AuthUser user,
    NumberFormat format, XViewModel oldView) {
  ...
  public final XDelta proceedExpand(XAxisItem parent, String axisId,
    NumberFormat format) {
  ...
  public final String[] hideItem(XAxisItem item, List<XAxisItem> roots,
    String axisId, String viewId, boolean hideLevel) {
  ...
  public final XLoadInfo willSetExpandState(XAxisItem[] expanded,
    ...
    return createLoadInfo(viewModelController.willSetExpandState(expanded,
      collapsed, axisId, expandDepth), -1, false);
  }

  public final XDelta[] proceedSetExpandState(NumberFormat format) {
    List<Expanded> allExpanded = viewModelController.proceedSetExpandStateExpanded();
    ...
    List<Collapsed> allCollapsed = viewModelController
      .proceedSetExpandStateCollapsed();
    if (delta == null) {
  ...
  private final XDelta createXDelta(Expanded expanded, XDelta parentDelta,
    ...
    if (expandedXItem != null) {
      ...
      if (parentDelta != null) {
        ...
        if (expanded.inColumn()) {
          parentDelta.getCells().setLoadedColumnsCount(
            val + expanded.getLoadedColumns());
        } else {
  ...
  public final List<XElementNode> getRoots(XAxisHierarchy ofHierarchy) {
    ...
    if (axisHierarchy != null) {
      for (ElementNode rootElement : axisHierarchy.getRootNodes()) {
        XElementNode xRoot = XElementFactory.createX(rootElement, axisHierarchyId,
          viewId);
  ...
  public final List<XElementNode> getRoots(String axisId, String hierarchyId,
    String viewId) {
    List<XElementNode> roots = new ArrayList<XElementNode>();
  ...
  private final void addChildren(int currentDepth, int level,
    ElementNode[] rootNodes, List<XElementNode> nodes, XAxisHierarchy hierarchy,
    HashMap<ElementNode, XElementNode> parents) {
  ...
  public final List<XElementNode> loadChildren(XAxisHierarchy hier,
  ...
  public final XViewModel proceedChangeSelectedElement(UserSession userSession,
    AuthUser user, XViewModel xViewModel, XAxisHierarchy xAxisHierarchy,
    XElement selectedElement, NumberFormat format) throws SessionExpiredException,
    ...
    axisHierarchy.addSelectedElement(hierarchy.getElementById(selectedElement
      .getId()));
  ...
  public final XLoadInfo willUpdate(XAxisHierarchy xAxisHierarchy) {
    ...
    AxisHierarchySnapshot snapshotBefore = AxisHierarchySnapshot
      .createSnapshot(axisHierarchy);
    AxisHierarchyUpdater.update(axisHierarchy, xAxisHierarchy);

    AxisHierarchySnapshot snapshotAfter = AxisHierarchySnapshot
      .createSnapshot(axisHierarchy);
    boolean filterDeactivated = xAxisHierarchy.getVisibleElements() == null
      && xAxisHierarchy.getOldVisibleElements() != null;
    boolean createNewModel = !snapshotAfter.equals(snapshotBefore)
      || filterDeactivated;
    ...
    int visibleCells = viewModelController.getVisibleCellCount();
    int loadCells = createNewModel ? visibleCells : viewModelController
      .getNumberOfCellsToLoad();
  ...
  private final XLoadInfo willUpdateSelectionAxis(XAxisHierarchy xAxisHierarchy) {
    ...
    int loadCells = 0;
    int visibleCells = viewModelController.getVisibleCellCount();
    if (selectedElementChanged(axisHierarchy, xAxisHierarchy.getSelectedElement())) {
  ...
  private final boolean selectedElementChanged(AxisHierarchy axisHierarchy,
    ...
    if (selectedElements != null && selectedElements.length > 0
      ...
      return !selectedElements[0].getId().equals(newSelectedElement.getId());
  ...
  public final XViewModel updateAndReload(UserSession userSession, AuthUser user,
    XViewModel xViewModel, NumberFormat format) throws PaloGwtCoreException {
    update(xViewModel);
    return proceedOpenView(userSession, xViewModel, user, format);
  }
  ...
  public final XLoadInfo willUpdate(XViewModel xViewModel) {
    ...
    ViewModelController viewController = new ViewModelController(getCubeView()
      .copy());
    // update
    ViewModelUpdater.updateNative(viewController.getModel(), xViewModel, false);
    // viewController.initWithCurrentModelState(); //marks all visible cells as
    // loaded...
  ...
  public final synchronized XViewModel proceedUpdate(UserSession userSession,
    AuthUser user, XViewModel xViewModel, NumberFormat format)
  ...
  public final XViewModel proceedUpdateWithoutTable(AuthUser user,
    XViewModel xViewModel, NumberFormat format) throws PaloGwtCoreException {
    ViewModelUpdater.updateNativeWithoutTable(viewModelController.getModel(),
      xViewModel);
    return xViewModel;
  }

  public final XViewModel cancelUpdateView(UserSession userSession, AuthUser user)
    throws PaloGwtCoreException {
  ...
  public final synchronized XViewModel proceedReloadView(UserSession userSession,
    XViewModel view, AuthUser user, NumberFormat format) throws PaloGwtCoreException {
  ...
  private final void updateExpandedPathsFor(Axis axis, XAxis xAxis) {
    axis.removeAllExpandedPaths();
    XAxisItem[] expandedItems = xAxis.getExpandedItems();
    for (XAxisItem xItem : expandedItems) {
      ElementPath path = ElementPath.restore(axis.getHierarchies(), xItem.getPath());
      axis.addExpanded(path);
    }
  }

  public final XCellCollection writeCell(XCell cell, NumberFormat format,
    AuthUser user) {
  ...
  private final Cell[] loadCellsFrom(Cube cube, Element[][] coordinates)
    throws PaloAPIException {
...
class ViewModelSnapshot {
  ...
  private final boolean leafsAreEqual(List<AxisItem> myLeafs,
    List<AxisItem> otherLeafs) {
...
class AxisHierarchySnapshot {
  ...
  private final boolean visibleElementsAreEqual(ElementNode[] others) {
    ...
    if (visibleElements != null && others != null
      && visibleElements.length == others.length) {
