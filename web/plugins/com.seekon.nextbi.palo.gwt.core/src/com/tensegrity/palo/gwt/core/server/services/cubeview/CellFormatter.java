...
/**
 * <code>CellFormatter</code> TODO DOCUMENT ME
 * 
 * @version $Id: CellFormatter.java,v 1.4 2009/12/17 16:14:29 PhilippBouillon
 *          Exp $
 **/
public class CellFormatter {

  public static final void applyDefaultFormat(XCell xCell, Cell cell,
    NumberFormat format) {
  ...
  public static final void applyFormat(Format format, XCell xCell, Cell cell,
  ...
  private static final void applyBackgroundColor(ColorDescriptor cd,
    ...
    xFormat.backgroundColor = (cd == null) ? getDefaultBackgroundColor(cell)
      : toCssColor(cd);
  }

  private static final void applyForegroundColor(ColorDescriptor cd,
    XCellFormat xFormat, Cell cell) {
    xFormat.foregroundColor = (cd == null) ? getDefaultForegroundColor()
      : toCssColor(cd);
  }
  ...
  private static final void applyBorder(BorderData[] borders, XCellFormat xFormat,
    RangePosition positionInRange) {
  ...
  private static final void applyTrafficLight(TrafficLightData trafficLight,
    ...
    if (cell.getType() == Cell.NUMERIC) {
      int index = trafficLight.getIndexForValue(Double.parseDouble(cell.getValue()
        .toString()));
      if (index > -1) {
  ...
  private static final void formatValue(XCell xCell, Cell cell,
