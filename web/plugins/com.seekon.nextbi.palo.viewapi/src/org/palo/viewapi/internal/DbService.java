...
public class DbService {
  private static final ResourceBundle rb = ResourceBundle.getBundle("deploy",
    Locale.ITALIAN);

  private static DbConnection connection;

  private static final UserImpl createUser(IUserManagement usrMgmt, String login,
    String passw) throws SQLException {
  ...
  private static final Role createRole(IRoleManagement roleMgmt, String name,
    Right right) throws SQLException {
    Role role = new RoleImpl.Builder(null).name(name).permission(right).build();
  ...
  private static final boolean initUsers() throws SQLException {
    ...
    if (admin == null) {
      ...
      if (editor == null)
        editor = createUser(usrMgmt, "editor");
      if (viewer == null)
        viewer = createUser(usrMgmt, "viewer");
      if (poweruser == null)
        poweruser = createUser(usrMgmt, "poweruser");
      if (creator == null)
        creator = createUser(usrMgmt, "creator");
      if (publisher == null)
        publisher = createUser(usrMgmt, "publisher");
      if (directLink == null)
        directLink = createUser(usrMgmt, "direct-link");
      return true;
    }
    return false;
  }
  ...
  private static final void initRoles() throws SQLException {
    ...
    if (adminRole == null) {
      ...
      if (editorRole == null)
        editorRole = createRole(roleMgmt, "EDITOR", Right.CREATE);
      if (viewerRole == null)
        viewerRole = createRole(roleMgmt, "VIEWER", Right.READ);
      if (ownerRole == null)
        ownerRole = createRole(roleMgmt, "OWNER", Right.CREATE);
      if (poweruserRole == null)
        poweruserRole = createRole(roleMgmt, "POWERUSER", Right.GRANT);
      if (creatorRole == null)
        creatorRole = createRole(roleMgmt, "CREATOR", Right.CREATE);
      if (publisherRole == null)
        publisherRole = createRole(roleMgmt, "PUBLISHER", Right.GRANT);
    }

    if (adminRole != null) {
      adminRole
        .setDescription("Grants the right to view & edit administration area and modify & share all existing views (System)");
      roleMgmt.update(adminRole);
    }
    if (poweruserRole != null) {
      poweruserRole
        .setDescription("Grants the right to share views created by this user");
      roleMgmt.update(poweruserRole);
    }
    if (editorRole != null) {
      editorRole
        .setDescription("Grants the right to modify views shared by other users (System)");
    ...
    if (viewerRole != null) {
      viewerRole
        .setDescription("Grants the right to see views shared by other users (System)");
      roleMgmt.update(viewerRole);
    }
    if (ownerRole != null) {
      ownerRole
        .setDescription("Grants the right to create views and edit these views (System)");
      roleMgmt.update(ownerRole);
    }
    if (publisherRole != null) {
      publisherRole
        .setDescription("Grants the right to create, modify and publish own views");
  ...
  static final void createConnection(IConnectionManagement conMgmt, String name,
    String host, String port, int type) throws SQLException {
    PaloConnection paloCon = conMgmt.findBy(host, port);
    if (paloCon != null)
      return;
    paloCon = new PaloConnectionImpl.Builder(null).name(name).host(host).service(
      port).type(type).build();
  ...
  static final void initDefaultConnections() throws SQLException {
    ...
    String mondrianConnectionService = rb
      .getString("jpalo.mondrian.connection.service");
    ...
    createConnection(conMgmt, mondrianConnectionName, mondrianConnectionUrl,
      mondrianConnectionService, 3);
  ...
  static final void createAccount(IAccountManagement accMgmt, UserImpl user,
    PaloConnection paloCon, String login, String pass) throws SQLException {
    if (user == null) {
      return;
    }
    ...
    if (paloAccount != null)
      return;
    paloAccount = new AccountImpl.Builder(null, user.getId()).connection(
      paloCon.getId()).username(login).password(pass).build();
  ...
  static final GroupImpl createGroup(IGroupManagement grpMgmt, String name,
    Role[] roles) throws SQLException {
    ...
    IGroupRoleManagement groles = MapperRegistry.getInstance()
      .getGroupRoleAssociation();
    for (Role r : roles) {
      if (r == null) {
        continue;
      }
  ...
  static final void initGroups() throws SQLException {
    ...
    if (adminGroup == null) {
      adminGroup = createGroup(grpMgmt, "admin", new Role[] { adminRole, editorRole,
        viewerRole });
      if (editorGroup == null)
        editorGroup = createGroup(grpMgmt, "editor", new Role[] { editorRole,
          viewerRole });
      if (viewerGroup == null)
        viewerGroup = createGroup(grpMgmt, "viewer", new Role[] { viewerRole });
      if (poweruserGroup == null)
        poweruserGroup = createGroup(grpMgmt, "poweruser", new Role[] {
          poweruserRole, editorRole, viewerRole });
      if (creatorGroup == null)
        creatorGroup = createGroup(grpMgmt, "creator", new Role[] { creatorRole,
          viewerRole });
      if (publisherGroup == null)
        publisherGroup = createGroup(grpMgmt, "publisher", new Role[] {
          publisherRole, viewerRole });
    }
    if (adminGroup != null) {
      ((GroupImpl) adminGroup)
        .setDescription("Grants the right to view & edit administration area and modify & share all existing views (System)");
      grpMgmt.update(adminGroup);
    }
    if (poweruserGroup != null) {
      ((GroupImpl) poweruserGroup)
        .setDescription("Grants the right to share views created by this user");
      grpMgmt.update(poweruserGroup);
    }
    if (editorGroup != null) {
      ((GroupImpl) editorGroup)
        .setDescription("Grants the right to modify views shared by other users (System)");
      grpMgmt.update(editorGroup);
    }
    if (creatorGroup != null) {
      ((GroupImpl) creatorGroup)
        .setDescription("Grants the right to create and modify own views");
      grpMgmt.update(creatorGroup);
    }
    if (viewerGroup != null) {
      ((GroupImpl) viewerGroup)
        .setDescription("Grants the right to see views shared by other users (System)");
      grpMgmt.update(viewerGroup);
    }
    if (publisherGroup != null) {
      ((GroupImpl) publisherGroup)
        .setDescription("Grants the right to create, modify and publish own views");
