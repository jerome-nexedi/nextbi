...
* @author Stepan Rutz
*
* @version $Id: Cube.java,v 1.82 2010/06/17 06:05:51 PhilippBouillon Exp $
*
*/
...
public interface Cube extends PaloObject {
   ...
   */
  public static final int SPLASHMODE_DISABLED = CellInfo.SPLASH_MODE_DISABLED;

  /**
   * Splash-mode constant enumeration.
   */
  public static final int SPLASHMODE_DEFAULT = CellInfo.SPLASH_MODE_DEFAULT,
   ...
   * If an element does not exist (or no longer), i.e. null is passed for its value,
   * the value for that cell will be set to "invalid coordinate" or 0, depending on 
   * the value of showZero.
   * 
   * @param coordinates coordinates an array of {@link Element}-coordinates that specifies
   * the data to read.
   * @param showZero if true, 0 is returned for missing element names,
   * 	"invalid coordinates" otherwise.
   * @return the values stored at the determined data-cells.
   */
  Object[] getDataBulkIgnoreMissing(Element[][] coordinates, boolean showZero);

  /**
   * Returns the data at the specified coordinates. The method works exactly
   * as getDataBulk(Element[][] coordinates), but the coordinates are in this case
   * specified by the element names.
   * 
   * @param elements an array of string arrays that specifies the data to read.
   * @return the values stored at the determined data-cells.
   */
  Object[] getDataBulk(String[][] elements);

  /**
   * Returns the data at the specified coordinates. The method works exactly
   * as getDataBulk(Element[][] coordinates), but the coordinates are in this case
   * specified by the element names.
   * If an element name does not exist (or no longer), the value for that cell
   * will be set to "invalid coordinate" or 0, depending on the value of showZero.
   * 
   * @param elements an array of string arrays that specifies the data to read.
   * @param showZero if true, 0 is returned for missing element names,
   * 	"invalid coordinates" otherwise.
   * @return the values stored at the determined data-cells.
   */
  Object[] getDataBulkIgnoreMissing(String[][] coordinates, boolean showZero);
  ...
  * @deprecated please use {@link #getCubeViews(PersistenceObserver)} instead
   ...
   */
  void setDataArray(Element[][] coordinates, Object[] values, boolean add,
   ...
   * @return a new rule instance represented by this definition
   */
  Rule addRule(String definition, String externalIdentifier, boolean useIt,
    String comment);
  ...
  /**
   * Copies the cell content from a given coordinate to a target coordinate.
   * The difference between splashing a cell value and copying the content is that
   * the current weights and distributions of the source cell are also copied if you
   * use copyCell. Otherwise, if you splash a number, new weights are calculated based
   * on the current cell content.
   * 
   * @param source source cell coordinate.
   * @param target target cell coordinate.
   * @return true if the call was successful, false otherwise.
   */
  boolean copyCell(Element[] source, Element[] target);

  /**
   * Sets the value of a target cell to the specified value, taking the weights from a
   * different cell.
   * 
   * @param source source cell coordinate.
   * @param target target cell coordinate.
   ...
   * @return true if the call was successful, false otherwise.
   */
  boolean copyCell(Element[] source, Element[] target, double value);

  /**
   * This function allows you to easily set the content of a cell.
   * You specify a coordinate and a text that represents the new cell value along with
   * the current number syntax.
   * 
   * Examples for the input value could be:
   * 
   * 1.4    represents the number 1.4
   * !1.4   splashes the number 1.4, using SPLASHMODE_BASE_SET
   * !!1.4  splashes the number 1.4, using SPLASHMODE_BASE_ADD
   * #1.4   splashes the number 1.4, using SPLASHMODE_DEFAULT
   * #10%   adds 10% to the current cell value
   * copy 2006;Actual   copies the cell found at the same coordinates as the target
   *                    cell, but with 2006 and Actual as coordinates for years and
   *                    datatypes, respectively
   * 30.000 like 2006;Actual   splashes the value 30.000 using the weights from the
   *                           2006;Actual cell.
   *                           
   * Depending on the values for the thousand separator and the decimal point, you can
   * also specify numbers like 30,000.00 or 30.000,00.                           
   *                                               
   * @param coordinate the coordinate of the target cell
   * @param input the new input value
   * @param thousandSeparator character that is being used to separate thousand-triplets in a number
   * @param decimalPoint decimal character
   * @return true if the call was successful, false otherwise.
   */
  boolean setInput(Element[] coordinate, Object input, char thousandSeparator,
    char decimalPoint);

  /**
   * This function allows you to easily set the content of a cell.
   * You specify a coordinate and a text that represents the new cell value. 
   * The number syntax defaults to the base (English) notation. I.e. 30,000.00
   *  
   * Examples for the input value could be:
   * 
   * 1.4    represents the number 1.4
   ...
   * @return true if the call was successful, false otherwise.
   */
  boolean setInput(Element[] coordinate, Object input);
