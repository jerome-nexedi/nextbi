 ...
 * @version $Id: DataFilter.java,v 1.15 2009/04/29 10:21:58 PhilippBouillon Exp $
 **/
public class DataFilter extends AbstractSubsetFilter implements RestrictiveFilter,
  EffectiveFilter {
  ...
  private final HashMap<Element, DataCell> dataElements = new HashMap<Element, DataCell>();
  ...
  public final DataFilter copy() {
    DataFilter copy = new DataFilter(hierarchy, setting.getSourceCube().getValue());
  ...
  public final void filter(Set<Element> elements) {
    ...
    for (Element element : elements) {
      ...
      DataCell dataCell = new DataCell(element, hierarchies, slice);
    ...
    Object[] values = srcCube.getDataBulk(coordinates
      .toArray(new Element[coordinates.size()][]));
    ...
    ArrayList<DataCell> sortedValues = new ArrayList<DataCell>(Arrays
      .asList(dataCells));
    Collections.sort(sortedValues, new DataCellComparator(setting.getCellOperator()
      .getValue()));
      ...
      if (upper > 0) {
        ...
        for (int i = sortedValues.size() - 1; i >= 0; --i) {
          ...
          if (lower < 0) {
            ...
            if (!cell.isString())
              partial += (Double) cell
                .getValue(setting.getCellOperator().getValue());
          } else {
            if (partial < bound)
              elements.remove(cell.getElement());
            if (!cell.isString())
      ...
      if (lower > 0) {
        ...
        for (int i = 0, n = sortedValues.size(); i < n; ++i) {
          ...
          if (upper > 0) {
            ...
            elements.remove(cell.getElement());
          if (!cell.isString())
            partial += (Double) cell.getValue(setting.getCellOperator().getValue());
        }
      }
    }
  }
  ...
  public final void validateSettings() throws PaloIOException {
    ...
    if (srcCube == null)
      throw new PaloIOException("DataFilter: unknown source cube with id '"
        + srcCube + "'!");
    String[][] slice = setting.getSlice();
    if (slice == null)
  ...
  private final boolean fulFillsCriteria(DataCell cell) {
    return cell
      .fulfills(setting.getCellOperator().getValue(), setting.getCriteria());
  }
}
...
class DataCell {
  ...
  final boolean fulfills(int cellOperator, DataCriteria criteria) {
    switch (cellOperator) {
    case DataFilterSetting.ALL_OP:
      return areAll(criteria);
    case DataFilterSetting.ANY_OP:
  ...
  private final Element[][] cartesianProduct(Hierarchy[] cubeHiers, String[][] slices) {
    ...
    while (coordsCount-- > 0) {
      ...
      for (String[] slice : slices) {
        ...
        i *= slice.length;
        if (!isString && coordinate[index].getType() == Element.ELEMENTTYPE_STRING)
          isString = true;
  ...
  private final boolean isInCriteriaRegion(Object val, DataCriteria criteria) {
    return pass(val, criteria.getFirstOperator(), criteria.getFirstOperand()
      .getValue())
      && pass(val, criteria.getSecondOperator(), criteria.getSecondOperand()
        .getValue());
  }
