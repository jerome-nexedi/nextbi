...
public class SubsetXMLHandler extends DefaultHandler {
  ...
  public SubsetXMLHandler(TypeInfoProvider typeProvider,
    SubsetHandlerImpl subsetHandler, String defName, int type) {
    this.type = type;
    ...
    filterHandlers.put(HierarchicalFilterHandler.XPATH,
      HierarchicalFilterHandler.class);
  ...
  public static final String getFilterXMLExpression(SubsetFilter filter) {
    ...
    case SubsetFilter.TYPE_ATTRIBUTE:
      xmlStr = AttributeFilterHandler.getPersistenceString((AttributeFilter) filter);
      break;
    case SubsetFilter.TYPE_DATA:
      xmlStr = DataFilterHandler.getPersistenceString((DataFilter) filter);
      break;
    case SubsetFilter.TYPE_HIERARCHICAL:
      xmlStr = HierarchicalFilterHandler
        .getPersistenceString((HierarchicalFilter) filter);
      break;
    case SubsetFilter.TYPE_PICKLIST:
      xmlStr = PicklistHandler.getPersistenceString((PicklistFilter) filter);
      break;
    case SubsetFilter.TYPE_SORTING:
      xmlStr = SortingFilterHandler.getPersistenceString((SortingFilter) filter);
      break;
    case SubsetFilter.TYPE_TEXT:
      xmlStr = TextFilterHandler.getPersistenceString((TextFilter) filter);
      break;
    case SubsetFilter.TYPE_ALIAS:
      xmlStr = AliasFilterHandler.getPersistenceString((AliasFilter) filter);
      break;
    default:
      throw new PaloAPIException("Couldn't store subset! Unsupported filter '"
  ...
  private final SubsetFilterHandler createFilter(String xPath) {
    ...
    if (filter != null) {
      try {
        SubsetFilterHandler filterHandler = filter.getConstructor().newInstance();
  ...
  private final Subset2 createSubset(Attributes attributes) throws SAXException {
    ...
    if (!defName.equals(name))
      name = defName;
    Subset2 subset = subsetHandler.create(id, name, dimension.getDefaultHierarchy(),
      type);
