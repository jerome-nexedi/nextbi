...
public class SubsetHandlerImpl implements SubsetHandler {
  ...
  private SubsetHandlerImpl(Hierarchy hierarchy) {
    ...
    this.storageHandler = (SubsetStorageHandlerImpl) database
      .getSubsetStorageHandler();
    if (storageHandler == null)
      throw new NullPointerException("storage handler is null!!");
    newSubsetsSupported = !hierarchy.getDimension().isSystemDimension()
      && !hierarchy.isAttributeHierarchy() && !hierarchy.isSubsetHierarchy()
      && database.supportsNewSubsets();
  }
  ...
  public final Subset2 addSubset(String name, int type) {
    if (!newSubsetsSupported)
      throw new PaloAPIException("New subsets are not supported by database '"
        + hierarchy.getDimension().getDatabase().getName() + "'!");
    if (!canWrite(type)) {
      throw new InsufficientRightsException("Cannot add subset '" + name + "' to "
        + getTypeStr(type) + " subsets!\nNot enough rights!");
    }
    try {
      String id = storageHandler.newSubsetCell(name, hierarchy, type);
      if (id == null)
        throw new PaloAPIException(
          "Adding a subset to a system dimension is not allowed!");
      Subset2 subset = new Subset2Impl(id, name, hierarchy, type);
      storageHandler.save(subset);
      register(subset);
      return subset;
    } catch (PaloIOException e) {
      throw new PaloAPIException("The subset '" + name + "' exists already!", e);
    }
  }
  ...
  public final Subset2 getSubset(String id, int type) {
    ...
    if (subset == null) {
      ...
      try {
        ...
        throw new PaloAPIException("Loading subset '" + getSubsetName(id)
          + "' of hierarchy '" + hierarchy.getName() + "' failed!!", e);
      }
    }
    return subset;
  }
  ...
  public String[] getSubsetNames(int type) {
    if (!newSubsetsSupported || hierarchy.getDimension().isSystemDimension())
      return new String[0];
