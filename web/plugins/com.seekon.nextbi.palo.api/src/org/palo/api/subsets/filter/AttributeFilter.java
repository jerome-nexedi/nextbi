 ...
 * @version $Id: AttributeFilter.java,v 1.15 2009/04/29 10:21:58 PhilippBouillon Exp $
 **/
public class AttributeFilter extends AbstractSubsetFilter implements
  RestrictiveFilter {
  ...
  public final void filter(Set<Element> elements) {
    AttributeConstraintsMatrix filterMatrix = (AttributeConstraintsMatrix) setting
  ...
  private final boolean rowFulfilled(AttributeConstraint[] row, Element element) {
    for (AttributeConstraint constraint : row) {
      Attribute attribute = hierarchy.getAttribute(constraint.getAttributeId());
      if (attribute != null) {
        String attrValue = element.getAttributeValue(attribute).toString();
