...
class Subset2Impl implements Subset2 {
  ...
  Subset2Impl(String id, String name, Hierarchy hierarchy, int type) {
    ...
    this.key = new CompoundKey(new Object[] { Subset2Impl.class,
      hierarchy.getDimension().getDatabase().getId(), hierarchy.getId(), id, type });
    reset();
  }
  ...
  public final void save() throws PaloAPIException {
    try {
      ...
      SubsetStorageHandlerImpl storageHandler = (SubsetStorageHandlerImpl) database
  ...
  private final void applyFilters() {
    if (modified) {
      ...
      rootNodes.clear();
      this.elements.clear();
      HashSet<Element> elements = new HashSet<Element>(Arrays.asList(hierarchy
        .getElements()));
  ...
  public Subset2 setDefinition(String definition) {
    try {
      ...
      return storageHandler.load(id, getDimHierarchy(), getType(),
  ...
  public boolean validate() {
    try {
      ...
      throw new PaloAPIException("Validation of subset '" + getName() + "' failed!",
        pio);
    }
  }
}
