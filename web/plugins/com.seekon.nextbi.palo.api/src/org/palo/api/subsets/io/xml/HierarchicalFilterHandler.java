...
class HierarchicalFilterHandler extends AbstractSubsetFilterHandler {
  ...
  public final SubsetFilter createFilter(Dimension dimension) {
    if (hfInfo.doLevelSelection() && subsetVersion.equals("1.0rc2")) {
      ...
      Element start = dimension.getElementById(hfInfo.getStartElement().getValue());
      if (start != null) {
        hfInfo.setStartLevel(start.getLevel());
        hfInfo.setStartElement((String) null);
      }
      Element end = dimension.getElementById(hfInfo.getEndElement().getValue());
      if (end != null) {
  ...
  public static final String getPersistenceString(HierarchicalFilter filter) {
    ...
    if (hfInfo.doAboveBelowSelection()) {
      ...
      xmlStr.append(ParameterHandler.getXML(hfInfo.getRefElement()));
      xmlStr.append("</element>\r\n");
      xmlStr.append("<above>\r\n");
      xmlStr.append(ParameterHandler.getXML(hfInfo.getAbove()));
      xmlStr.append("</above>\r\n");
      xmlStr.append("<exclusive>\r\n");
      xmlStr.append(ParameterHandler.getXML(hfInfo.getExclusive()));
      xmlStr.append("</exclusive>\r\n");
    }
    if (hfInfo.doHide()) {
      xmlStr.append("<hide>\r\n");
      xmlStr.append(ParameterHandler.getXML(hfInfo.getHideMode()));
      xmlStr.append("</hide>\r\n");
    }
    ...
    if (hfInfo.doRevolve()) {
      ...
      xmlStr.append(ParameterHandler.getXML(hfInfo.getRevolveElement()));
      xmlStr.append("</revolve_element>\r\n");

      xmlStr.append("<revolve_count>\r\n");
      xmlStr.append(ParameterHandler.getXML(hfInfo.getRevolveCount()));
      xmlStr.append("</revolve_count>\r\n");

      xmlStr.append("<revolve_add>\r\n");
      xmlStr.append(ParameterHandler.getXML(hfInfo.getRevolveMode()));
      xmlStr.append("</revolve_add>\r\n");
    }
    xmlStr.append("</hierarchical_filter>\r\n");
