...
public class RightsImpl implements Rights {
  ...
  public boolean mayDelete(PaloObject object) {
    ...
    return traverseRights(object, new IRightsTraverser() {
      public boolean checkData(Object data) {
        return data.toString().equals("S") || data.toString().equals("D");
      }
    });
  }
  ...
  public boolean mayRead(PaloObject object) {
    ...
    return traverseRights(object, new IRightsTraverser() {
      public boolean checkData(Object data) {
        return data.toString().equals("S") || data.toString().equals("D")
          || data.toString().equals("W") || data.toString().equals("R");
      }
    });
  }
  ...
  private final void setRight(String role, Class<? extends PaloObject> object,
    String right) {
    if (connection.getType() == Connection.TYPE_XMLA) {
      return;
    }
  ...
  private final boolean performCheck(Element objectElement,
    ...
    for (String role : roles) {
      Element roleElement = rightsCube.getDimensionAt(0).getElementByName(role);
      Object data = rightsCube.getData(new Element[] { roleElement, objectElement });
      if (data != null) {
  ...
  private final boolean[] checkCubeOrDimensionRight(PaloObject object,
    ...
    if (cube != null) {
      Element objectElement = cube.getDimensionAt(1).getElementByName(
        object.getName());
      if (objectElement == null) {
        return new boolean[] { false, false };
      }
      ...
      for (String group : groups) {
        Element groupElement = cube.getDimensionAt(0).getElementByName(group);
        Object data = cube.getData(new Element[] { groupElement, objectElement });
        if (data != null) {
          if (data.toString().trim().length() != 0) {
            String result = data.toString().trim();
            if (!result.equals("S") && !result.equals("D") && !result.equals("W")
              && !result.equals("R") && !result.equals("N")) {
              restricted = false;
            }
          } else {
            restricted = false;
  ...
  private final void initializeGroupsAndRoles(ConnectionImpl connection,
    ...
    if (cube != null) {
      Element userElement = cube.getDimensionAt(0).getElementByName(
        connection.getUsername());
      Element[] groupElements = cube.getDimensionAt(1).getElements();
      Object[] groupResult = cube.getDataArray(new Element[][] { { userElement },
        groupElements });

      for (int i = 0, n = groupResult.length; i < n; i++) {
        if (groupResult[i] != null && groupResult[i].toString().equals("1")) {
  ...
  private final void initializeRoles(LinkedHashSet<String> groupList,
    ...
    if (cube != null) {
      ...
      for (String elId : groupList) {
        groupElements[groupElementSize++] = groupDimension.getElementByName(elId);
      }
      Element[] roleElements = cube.getDimensionAt(1).getElements();
      Object[] groupRoleResults = cube.getDataArray(new Element[][] { groupElements,
        roleElements });
