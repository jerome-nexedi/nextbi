...
* @author Arnd Houben
*
* @version $Id: CubeImpl.java,v 1.117 2010/07/13 11:52:13 PhilippBouillon Exp $
*
*/
...
class CubeImpl extends AbstractPaloObject implements Cube {
  ...
  final static CubeImpl create(ConnectionImpl connection, Database database,
    CubeInfo cubeInfo) {
    //		String[] dimensions = cubeInfo.getDimensions();
    //		Dimension[] _dimensions = new Dimension[dimensions.length];
    //		for (int i = 0; i < dimensions.length; ++i)
  ...
  private CubeImpl(ConnectionImpl connection, Database database, CubeInfo cubeInfo,
  ...
  public final Object getData(Element[] coordinates) {
    try {
      CellInfo cell = dbConnection.getData(cubeInfo, getCoordinates(coordinates));
      return cell.getValue();
    } catch (PaloException pex) {
      throw new PaloAPIException(pex);
    } catch (RuntimeException e) {
  ...
  public final Object[] getDataBulk(String[][] coordinates) {
    Element[][] elCoords = new Element[coordinates.length][];
    for (int i = 0, n = coordinates.length; i < n; i++) {
      elCoords[i] = new Element[coordinates[i].length];
      for (int j = 0; j < elCoords[i].length; j++) {
        Element e = getDimensionAt(j).getElementByName(coordinates[i][j]);
        if (e == null) {
          throw new PaloAPIException("Unknown element name: " + coordinates[i][j]
            + " in dimension " + getDimensionAt(j).getName());
        }
        elCoords[i][j] = e;
      }
    }
    return getDataBulk(elCoords);
  }

  public final Object[] getDataBulkIgnoreMissing(String[][] coordinates,
    boolean showZero) {
    Element[][] elCoords = new Element[coordinates.length][];
    for (int i = 0, n = coordinates.length; i < n; i++) {
      elCoords[i] = new Element[coordinates[i].length];
    ...
    return getDataBulkIgnoreMissing(elCoords, showZero);
  }
  ...
  public final Object[] getDataBulkIgnoreMissing(Element[][] coordinates,
    boolean showZero) {
    if (coordinates == null || coordinates.length == 0)
      return new Object[0];
    ...
    try {
      ElementInfo[][] coords = new ElementInfo[coordinates.length][];
      for (int i = 0; i < coords.length; ++i) {
        coords[i] = getCoordinatesIgnoreMissing(coordinates[i]);
      }
      CellInfo[] cells = dbConnection.getDataArray(cubeInfo, coords);
      values = new Object[cells.length];
      if (showZero) {
        for (int i = 0; i < values.length; ++i) {
          values[i] = "invalid coordinates".equals(cells[i].getValue()) ? 0
            : cells[i].getValue();
        }
      } else {
        for (int i = 0; i < values.length; ++i) {
  ...
  public final void setData(String[] coordinates, Object value) {
    setDataInternal(getCoordinates(coordinates), value, Cube.SPLASHMODE_DISABLED);
  }

  public final boolean copyCell(Element[] source, Element[] target, double value) {
    try {
      ElementInfo[] src = getCoordinates(source);
      ElementInfo[] dest = getCoordinates(target);
      return dbConnection.copyCell(cubeInfo, src, dest, value);
    } catch (PaloException pex) {
      throw new PaloAPIException(pex);
    } catch (RuntimeException e) {
      throw new PaloAPIException(e.getMessage(), e);
  ...
  public final boolean copyCell(Element[] source, Element[] target) {
    try {
      ElementInfo[] src = getCoordinates(source);
      ElementInfo[] dest = getCoordinates(target);
      return dbConnection.copyCell(cubeInfo, src, dest);
    } catch (PaloException pex) {
      throw new PaloAPIException(pex);
    } catch (RuntimeException e) {
      throw new PaloAPIException(e.getMessage(), e);
  ...
  public final void setDataArray(Element[][] coordinates, Object[] values,
    ...
    try {
      ...
      dbConnection.setDataArray(cubeInfo, coords, values, add, splashMode,
  ...
  private final double convertNumber(String str, char thsep, char dec) {
    try {
      ...
      throw new PaloAPIException("Error converting '" + str + "' to a number.", t);
    }
  }

  private final Element[] parseCopyLikeParameter(String params) {
    try {
      String[] tokens = params.split(";");
      Element[] coords = new Element[getDimensionCount()];
      ...
      for (String t : tokens) {
        ...
        if ((index = t.indexOf(":")) != -1) {
          ...
          Dimension d = getDimensionByName(dimName);
          if (d == null) {
            throw new PaloAPIException("Dimension '" + dimName
              + "' does not exist in cube '" + getName() + "'.");
          }
          boolean found = false;
          for (int i = 0; i < getDimensionCount(); i++) {
            if (getDimensionAt(i).equals(d)) {
              coords[i] = d.getElementByName(elementName);
              if (coords[i] != null) {
                found = true;
              }
              break;
          ...
          if (!found) {
            throw new PaloAPIException("Element '" + elementName
              + "' does not exist in dimension '" + dimName + "'.");
          }
        } else {
          boolean found = false;
          for (int i = 0; i < getDimensionCount(); i++) {
            Element e = getDimensionAt(i).getElementByName(t);
            if (e != null) {
              if (found) {
                throw new PaloAPIException("Element name '" + t
                  + "' is ambiguous. Add a dimension name to disambiguate.");
              }
              coords[i] = e;
              found = true;
            }
          ...
          if (!found) {
            throw new PaloAPIException("Unknown element '" + t + "'.");
          }
        }
      }
      ...
      throw new PaloAPIException("Error parsing copy/like parameters in '" + params
        + "'.", t);
    }
  }

  public final boolean setInput(Element[] coordinate, Object input) {
    return setInput(coordinate, input, ',', '.');
  }
  ...
  public final boolean setInput(Element[] coordinate, Object input,
    ...
    for (Element e : coordinate) {
      if (e.getType() == Element.ELEMENTTYPE_STRING) {
        // Yes, so just set the input.
        setData(coordinate, input);
        return true;
      }
    }
    ...
    if (textInput.startsWith("#")) {
      if (textInput.endsWith("%")) {
        double newVal = convertNumber(
          textInput.substring(1, textInput.length() - 1), thousandSeparator,
          decimalPoint);
        double oldValue = ((Double) getData(coordinate)).doubleValue();
        newVal = oldValue * percent(newVal);
        setDataSplashed(coordinate, new Double(newVal), SPLASHMODE_DEFAULT);
      } else {
        setDataSplashed(coordinate, convertNumber(textInput.substring(1),
          thousandSeparator, decimalPoint), SPLASHMODE_DEFAULT);
      }
    } else if (textInput.startsWith("!!")) {
      setDataSplashed(coordinate, convertNumber(textInput.substring(2),
        thousandSeparator, decimalPoint), SPLASHMODE_BASE_ADD);
    } else if (textInput.startsWith("!")) {
      setDataSplashed(coordinate, convertNumber(textInput.substring(1),
        thousandSeparator, decimalPoint), SPLASHMODE_BASE_SET);
    } else {
      // No splashing. Look for copy/like:
      String temp = textInput.toLowerCase();
      int index;
      if (temp.startsWith("copy") && temp.length() > 5) {
        Element[] source = parseCopyLikeParameter(textInput.substring(5));
        for (int i = 0; i < source.length; i++) {
          if (source[i] == null) {
            source[i] = coordinate[i];
          }
        }
        return copyCell(source, coordinate);
      } else if ((index = temp.indexOf("like")) != -1 && (index + 5) < temp.length()) {
        Element[] source = parseCopyLikeParameter(textInput.substring(index + 5));
        for (int i = 0; i < source.length; i++) {
          if (source[i] == null) {
            source[i] = coordinate[i];
          }
        ...
        return copyCell(source, coordinate, convertNumber(valText,
          thousandSeparator, decimalPoint));
      } else {
        // No copy/like, just set the data:
        setData(coordinate,
          convertNumber(textInput, thousandSeparator, decimalPoint));
      }
    }
  ...
  public final Rule addRule(String definition, String externalIdentifier,
    ...
    RuleInfo rule = ruleLoader.create(definition, externalIdentifier, useIt,
      comment, activate);
    Rule newRule = createRule(rule);
    fireRulesAdded(new Rule[] { newRule });
    return newRule;
  ...
  public final Cell getCell(Element[] coordinate) {
    try {
      CellInfo cell = dbConnection.getData(cubeInfo, getCoordinates(coordinate));
  ...
  public final Cell[] getCells(Element[][] coordinates) {
    ...
    try {
      ...
      if (e.getCause() != null && e.getCause() instanceof NullPointerException) {
        throw new PaloAPIException(
          "ID not found. Are you referring to the correct cube?", e);
      }
      throw new PaloAPIException(e.getMessage(), e);
    }
    return cells;
  ...
  public final boolean rollback(Lock lock, int steps) {
    try {
      if (lock != null)
        return dbConnection.rollback(cubeInfo, ((LockImpl) lock).getInfo(), steps);
    } catch (Exception e) {
      /* ignore */
    }
    return false;
  ...
  final void reloadRuleInfos(boolean doEvents) {
    HashMap<String, RuleImpl> oldRules = new HashMap<String, RuleImpl>(loadedRules);
  ...
  private final void setDataInternal(ElementInfo[] coordinates, Object value,
    ...
    try {
      if (value instanceof Number) {
        dbConnection.setDataNumericSplashed(cubeInfo, coordinates, ((Number) value)
          .doubleValue(), splashMode);
      } else {
        dbConnection.setDataString(cubeInfo, coordinates, value.toString());
      }
    } catch (PaloException pex) {
      throw new PaloAPIException(pex);
    } catch (RuntimeException e) {
  ...
  private final ElementInfo[] getCoordinates(String[] names) {
    ...
    for (int i = 0; i < coords.length; ++i) {
      ...
      ElementImpl element = (ElementImpl) dimension.getDefaultHierarchy()
        .getElementByName(names[i]);
      if (element == null)
        throw new PaloAPIException("Element not found! Dimension '"
  ...
  private final ElementInfo[] getCoordinates(int dimIndex, String[] names) {
    ...
    for (int i = 0; i < coords.length; ++i) {
      ...
      coords[i] = ((ElementImpl) dimension.getDefaultHierarchy().getElementByName(
        names[i])).getInfo();
    }
    return coords;
  }
    ...
    try {
      for (int i = 0; i < coords.length; ++i) {
        if (elements[i] instanceof VirtualElementImpl)
          ...
          coords[i] = elements[i].getInfo(); //((ElementImpl)elements[i]).getInfo();
      }
    } catch (Throwable t) {
  ...
  private final ElementInfo[] getCoordinatesIgnoreMissing(Element[] elements) {
    ElementInfo[] coords = new ElementInfo[elements.length];
    try {
      for (int i = 0; i < coords.length; ++i) {
        if (elements[i] == null) {
          coords[i] = new ElementInfoImpl(getDimensionAt(i).getInfo(), "-1");
          continue;
        }
        if (elements[i] instanceof VirtualElementImpl)
          coords[i] = ((VirtualElementImpl) elements[i]).getInfo();
  ...
  private final void notifyLoadFailed(String sourceId, PersistenceError[] errors) {
    for (int i = 0, n = viewObservers.size(); i < n; ++i) {
      PersistenceObserver observer = (PersistenceObserver) viewObservers.get(i);
      observer.loadFailed(sourceId, errors);
    }
  }

  private final void notifyLoadIncomplete(Object view, PersistenceError[] errors) {
    for (int i = 0, n = viewObservers.size(); i < n; ++i) {
      PersistenceObserver observer = (PersistenceObserver) viewObservers.get(i);
      observer.loadIncomplete(view, errors);
    }
  ...
  private final void fireCubeRenamed(Cube cube, String oldValue) {
    ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(),
      getDatabase(), ConnectionEvent.CONNECTION_EVENT_CUBES_RENAMED,
      new Cube[] { cube });
  ...
  private final void fireRulesAdded(Rule[] rules) {
    ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(),
      getDatabase(), ConnectionEvent.CONNECTION_EVENT_RULES_ADDED, rules);
    connection.fireEvent(ev);
  }

  private final void fireRulesRemoved(Rule[] rules) {
    ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(),
      getDatabase(), ConnectionEvent.CONNECTION_EVENT_RULES_REMOVED, rules);
    connection.fireEvent(ev);
  }
