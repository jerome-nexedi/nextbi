...
public class DatabaseImpl extends AbstractPaloObject implements Database {
  ...
  // FACTORY 
  //
  final static DatabaseImpl create(ConnectionImpl connection, DatabaseInfo dbInfo,
    boolean doEvents) {
  ...
  private DatabaseImpl(ConnectionImpl connection, DatabaseInfo dbInfo) {
    ...
    //SUBSETS:
    this.legacySubsetHandler = new org.palo.api.impl.SubsetStorageHandler(this); //for handling legacy subsets
    this.viewStorageHandler = new CubeViewStorageHandler(this);
    //NEW SUSBETS:
    this.subsetIOHandler = new SubsetIOHandler(this); //handles new subsets...

    this.key = new CompoundKey(new Object[] { DatabaseImpl.class, connection,
      dbInfo.getId() });
  ...
  private Database findPaloDatabase() {
    Connection[] cons = ((ConnectionFactoryImpl) ConnectionFactoryImpl.getInstance())
  ...
  public final String parseRule(Cube cube, String definition, String functions) {
    try {
      ...
      if (connection.getType() == Connection.TYPE_XMLA) {
        ...
        definition = dbConnection.parseRule(_cube.getInfo(), definition, functions);
        ...
        rule = paloDatabase.parseRule(paloDatabase.getCubeAt(0), definition,
  ...
  private final void fireDatabaseRenamed(Object[] dbs, String oldName) {
    ConnectionEvent ev = new ConnectionEvent(connection, null,
      ConnectionEvent.CONNECTION_EVENT_DATABASES_RENAMED, dbs);
    ev.oldValue = oldName;
  ...
  private final void reloadDatabase(boolean doEvents) {
    ...
    LinkedHashMap<String, Dimension> oldDimensions = new LinkedHashMap<String, Dimension>(
      loadedDimensions);
    LinkedHashMap<String, Cube> oldCubes = new LinkedHashMap<String, Cube>(
      loadedCubes);
    ...
    if (doEvents) {
      //dimensions removed
      LinkedHashSet removedDimensions = new LinkedHashSet(oldDimensions.values());
      ...
      //dimensions added
      LinkedHashSet addedDimensions = new LinkedHashSet(loadedDimensions.values());
  ...
  private final void fireDimensionsAdded(Object[] dimensions) {
    connection.fireEvent(new ConnectionEvent(getConnection(), this,
      ConnectionEvent.CONNECTION_EVENT_DIMENSIONS_ADDED, dimensions));
  }

  private final void fireDimensionsRemoved(Object[] dimensions) {
    connection.fireEvent(new ConnectionEvent(getConnection(), this,
      ConnectionEvent.CONNECTION_EVENT_DIMENSIONS_REMOVED, dimensions));
  }

  private final void fireCubesAdded(Object[] cubes) {
    connection.fireEvent(new ConnectionEvent(getConnection(), this,
      ConnectionEvent.CONNECTION_EVENT_CUBES_ADDED, cubes));
  }

  private final void fireCubesRemoved(Object[] cubes) {
    connection.fireEvent(new ConnectionEvent(getConnection(), this,
      ConnectionEvent.CONNECTION_EVENT_CUBES_REMOVED, cubes));
  }
   ...
   * @return
   */
  private final DimensionImpl createDimension(DimensionInfo dimInfo,
    boolean fireEvent) {
    DimensionImpl dimension = DimensionImpl.create(connection, this, dimInfo,
      fireEvent);
  ...
  private final Dimension addDimensionInternal(String name, int type) {
    try {
      ...
      DimensionInfo attrDimInfo = dbConnection.getAttributeDimension(dimInfo);
      if (attrDimInfo != null) {
