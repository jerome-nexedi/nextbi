...
* @author ArndHouben
*
* @version $Id: HttpConnection.java,v 1.60 2010/07/26 07:54:33 PhilippBouillon Exp $
*
*/
...
import com.tensegrity.palojava.impl.LicenseInfoImpl;
...
public class HttpConnection implements DbConnection {
  ...
  HttpConnection(String host, String port, int timeout) throws PaloException {
    ...
    try {
      ...
      throw new PaloException("Could not connect to palo server at host '" + host
        + "' on port '" + port + "'", ioex);
    }
  }
  ...
  public final void disconnect() {
    try {
      ...
      throw new PaloException("Could not correctly disconnect from palo server!!",
        ioex);
    }
  }
  ...
  public final boolean login(String user, String pass) {
    ...
    try {
      ...
      throw new PaloException("Failed to create encrypted password for " + "user '"
        + user + "'!", ex);
    }
    ...
    try {
      ...
      if (e.getErrorCode() != null && e.getErrorCode().trim().equals("1019")) {
        return loginInternal(user, oPass);
      } else {
        throw e;
      }
    }
  ...
  public final LicenseInfo getLicenseInfo() {
    ...
    String[] info = null;
    try {
      info = srvHandler.license();
      LicenseInfo licInfo = new LicenseInfoImpl(info);
      return licInfo;
  ...
  public final ElementInfo addElement(DimensionInfo dimension, String name,
  ...
  public final boolean replaceBulk(DimensionInfo dimension, ElementInfo[] elements,
    int type, ElementInfo[][] children, Double[][] weights) {
    if ((getServerInfo().getMajor() < 3)
      || (getServerInfo().getMajor() == 3 && getServerInfo().getMinor() == 0 && getServerInfo()
        .getBuildNumber() < 555)) {
  ...
  public final ElementInfo addElement(HierarchyInfo hierarchy, String name,
    int type, ElementInfo[] children, double[] weights) {
    try {
      setOwnChange(true);
      ElementHandler elHandler = handlerRegistry.getElementHandler();
      return elHandler.create(hierarchy.getDimension(), name, type, children,
        weights);
    } catch (IOException e) {
      throw new PaloException(e.getLocalizedMessage(), e);
    }
  ...
  public final boolean copyCell(CubeInfo cube, ElementInfo[] src,
    ElementInfo[] dest, double value) {
    try {
      CellHandler cellHandler = handlerRegistry.getCellHandler();
      return cellHandler.copyCell(cube, src, dest, value);
    } catch (IOException e) {
      throw new PaloException("Failed to copy cell data in cube: " + cube.getName(),
        e);
    }
  }

  public final boolean copyCell(CubeInfo cube, ElementInfo[] src, ElementInfo[] dest) {
    try {
      CellHandler cellHandler = handlerRegistry.getCellHandler();
      return cellHandler.copyCell(cube, src, dest);
    } catch (IOException e) {
      throw new PaloException("Failed to copy cell data in cube: " + cube.getName(),
        e);
    }
  ...
  public final void setDataArray(CubeInfo cube, ElementInfo[][] coordinates,
    Object[] values, boolean add, int splashMode, boolean notifyEventProcessors) {
    try {
      CellHandler cellHandler = handlerRegistry.getCellHandler();
      cellHandler.replaceValues(cube, coordinates, values, add, splashMode,
  ...
  public final void setDataNumericSplashed(CubeInfo cube, ElementInfo[] coordinate,
  ...
  public final void setDataString(CubeInfo cube, ElementInfo[] coordinate,
    String value) {
    try {
      CellHandler cellHandler = handlerRegistry.getCellHandler();
  ...
  public final void update(ElementInfo element, int type, String[] children,
  ...
  public final RuleInfo createRule(CubeInfo cube, String definition,
    ...
    try {
      setOwnChange(true);
      RuleHandler ruleHandler = handlerRegistry.getRuleHandler();
      return ruleHandler.create(cube, definition, externalIdentifier, useIt,
        comment, activate);
    } catch (IOException e) {
      throw new PaloException(e.getLocalizedMessage(), e);
    }
  ...
  public ElementLoader getElementLoader(HierarchyInfo hierarchy) {
    ...
    String key = HttpElementLoader.class.getName() + "#" + dbId + "#"
  ...
  private final synchronized boolean loginInternal(String user, String pass) {
    ...
    try {
      ...
      if (pe.getErrorCode() != null && pe.getErrorCode().trim().equals("1019")) {
        throw pe;
      }
      return false;
    }
