...
public class AjTypeImpl<T> implements AjType<T> {
	...
	private Pointcut asPointcut(Method method) {
		...
		if (pcAnn != null) {
		...
		}else{////TODO add 2012-05-13
			Object pointcut = null;
			try{
				Field field = Method.class.getDeclaredField("declaredAnnotations");
				field.setAccessible(true);
				Map declaredAnnotations = (Map)field.get(method);
				if(declaredAnnotations != null){
					Iterator iterator = declaredAnnotations.keySet().iterator();
					while(iterator.hasNext()){
						Object key = iterator.next();
						if(key.toString().equals(org.aspectj.lang.annotation.Pointcut.class.toString())){
							pointcut = declaredAnnotations.get(key);
							break;
						}
					}
				}
			...
			if(pointcut != null){
				try{
					Method localMethod = pointcut.getClass().getDeclaredMethod("value");
					localMethod.setAccessible(true);
					String value = (String)localMethod.invoke(pointcut, null);
					
					localMethod = pointcut.getClass().getDeclaredMethod("argNames");
					localMethod.setAccessible(true);
					String argNames = (String)localMethod.invoke(pointcut, null);
					...
					return new PointcutImpl(name,value,method,AjTypeSystem.getAjType(method.getDeclaringClass()),argNames);
