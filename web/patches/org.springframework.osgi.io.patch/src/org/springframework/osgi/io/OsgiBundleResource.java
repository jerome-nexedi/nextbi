...
public class OsgiBundleResource extends AbstractResource implements ContextResource {
  ...
  public OsgiBundleResource(Bundle bundle, String path) {
    ...
    case OsgiResourceUtils.PREFIX_TYPE_CLASS_SPACE:
      pathWithoutPrefix = path.substring(ResourceLoader.CLASSPATH_URL_PREFIX
        .length());
      break;
  ...
  public URL getURL() throws IOException {
    ...
    if ("bundle".equals(url.getProtocol())) {//TODO:2012-07-17,转换成本地url
      //System.out.println("get bundle url from localUrl.");
      try {
  ...
  boolean isRelativePath(String locationPath) {
    return ((locationPath.indexOf(PREFIX_SEPARATOR) == -1) && !locationPath
      .startsWith(ABSOLUTE_PATH_PREFIX));
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof OsgiBundleResource) {
      ...
      return (this.path.equals(otherRes.path) && ObjectUtils.nullSafeEquals(
        this.bundle, otherRes.bundle));
    }
    return false;
  }
  ...
  ContextResource[] getAllUrlsFromBundleSpace(String location) throws IOException {
    ...
    if (OsgiResourceUtils.FOLDER_DELIMITER.equals(location)) {
      ...
      while (candidates != null && candidates.hasMoreElements()) {
        resources.add(new UrlContextResource((URL) candidates.nextElement(),
          contextPath));
      }
    }

    return (ContextResource[]) resources.toArray(new ContextResource[resources
      .size()]);
  }
