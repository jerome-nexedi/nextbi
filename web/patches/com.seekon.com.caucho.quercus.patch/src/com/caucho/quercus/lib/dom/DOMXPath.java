...
public class DOMXPath {
  ...
  public Object evaluate(Env env, String expression,
    ...
    Object node0 = query(env, expression, node);

    if (node0 instanceof NodeList) {
      NodeList nodeList = (NodeList) node0;
      if (nodeList.getLength() == 1) {
        return _document.wrap(nodeList.item(0));
      }
    }

    return _document.wrap(node0);

  }

  public Object query(Env env, String expression, @Optional DOMNode<Node> contextNode) {
    Node node;
    ...
    Object nodeList = query(env, expression, node);

    return _document.wrap(nodeList);
  }

  private Object query(Env env, String pattern, Node node) {
  ...
  /*
   * private NodeList query(Env env, String pattern, Node node) { try { if
   * (_context == null) { Quercus quercus = env.getQuercus();
   * 
   * ExpressionCache cache = (ExpressionCache)
   * quercus.getSpecial("caucho.domxpath.cache");
   * 
   * if (cache == null) { cache = new ExpressionCache();
   * quercus.setSpecial("caucho.domxpath.cache", cache); }
   * 
   * XPathExpression expr = cache.compile(pattern);
   * 
   * NodeList nodeList = (NodeList) expr.evaluate(node,
   * XPathConstants.NODESET);
   * 
   * cache.free(pattern, expr);
   * 
   * return nodeList; } else { XPath xpath = (XPath)
   * env.getSpecialValue("caucho.domxpath.xpath");
   * 
   * if (xpath == null) { XPathFactory factory = XPathFactory.newInstance();
   * xpath = factory.newXPath(); env.setSpecialValue("caucho.domxpath.xpath",
   * xpath); }
   * 
   ...
   * XPathConstants.NODESET);
   * 
   * return nodeList; } } catch (XPathExpressionException e) { throw new
   * QuercusModuleException(e); } }
   */
  ...
  static class ExpressionCache {
    ...
    private final LruCache<String, ExpressionEntry> _xpathCache = new LruCache<String, ExpressionEntry>(
      1024);
