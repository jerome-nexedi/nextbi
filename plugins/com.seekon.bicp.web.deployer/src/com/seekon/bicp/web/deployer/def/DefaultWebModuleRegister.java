...
public class DefaultWebModuleRegister implements WebModuleRegister {
	...
	public void registerDefault() throws RegisterException {
		try {
			...
			httpService.registerServlet(contextPath.concat("/*.jsp"),
					adaptedJspServlet, dictionary, getHttpContext());
		} catch (Exception e) {
			throw new RegisterException(e);
		}
	}
	...
	public void registerFilters(Document doc) throws RegisterException {
		try {
			...
			for (Iterator it = filterNodes.iterator(); it.hasNext();) {
				Element el = (Element) it.next();
				try {
					String filterName = el.selectSingleNode("filter-name")
							.getStringValue().trim();
					String filterClass = el.selectSingleNode("filter-class")
							.getStringValue().trim();

					this.registerFilterWithUrlPattern(doc, filterName, filterClass, el, ranking);
					this.registerFilterWithServletPattern(doc, filterName, filterClass,
							el, ranking);
					ranking++;
				} catch (Throwable e) {
					log.error(e);
	...
	private void registerFilterWithServletPattern(Document doc,
			String filterName, String filterClass, Element el, int ranking) {
	...
	private Reader getWebXmlReader() {
		...
		try {
			// 去掉namespace
			br = new BufferedReader(new InputStreamReader(getWebXMLResource()
					.openStream()));
			try {
	...
	public void registerServlets(Document doc) throws RegisterException {
		try {
			...
			for (Iterator it = servlets.iterator(); it.hasNext();) {
				Element el = (Element) it.next();
				try {
					String servletName = el.selectSingleNode("servlet-name")
							.getStringValue().trim();
					...
					if (servletClassNode != null) {
						...
						try {
							try {
								...
								servlet = (Servlet) bundle.loadClass(servletClass)
										.newInstance();
								if (servlet != null)
									Debug.trace("success create servlet: " + servlet);

							} catch (Throwable e) {
								System.err.println("can't create instance class: "
										+ servletClass + " at bundle " + bundle);
								e.printStackTrace();
							}
						} finally {
							Thread.currentThread().setContextClassLoader(contextClassLoader);
