...
public class WebAppLoader implements ModuleLoader {
	...
	public void load(Bundle bundle) throws LoadException {
		...
		if (contextPath != null) {
			...
			WebModuleRegister register = new DefaultWebModuleRegister(bundle,
					contextPath, webFolder, httpService);
			try {
				for (Iterator it = this.webModuleRegisterHandlertList.iterator(); it
						.hasNext();) {
					WebModuleRegisterHandler handler = (WebModuleRegisterHandler) it
							.next();
					handler.preRegisterWebModule(bundle, httpService, contextPath,
							register);
				}
				...
				for (Iterator it = this.webModuleRegisterHandlertList.iterator(); it
					...
					WebModuleRegisterHandler handler = (WebModuleRegisterHandler) it
							.next();
					handler.didRegisterWebModule(bundle, httpService, contextPath,
							register);
				}
