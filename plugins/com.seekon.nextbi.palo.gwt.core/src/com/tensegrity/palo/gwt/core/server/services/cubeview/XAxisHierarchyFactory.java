...
/**
 * <code>XAxisHierarchyFactory</code> TODO DOCUMENT ME
 * 
 * @version $Id: XAxisHierarchyFactory.java,v 1.13 2010/03/12 12:49:14
 *          PhilippBouillon Exp $
 **/
public class XAxisHierarchyFactory {

  static XAxisHierarchy createX(AxisHierarchy axisHierarchy, XAxis xAxis,
    UserSession userSession) {
    Hierarchy hierarchy = axisHierarchy.getHierarchy();
    XAxisHierarchy xAxisHierarchy = new XAxisHierarchy(hierarchy.getId(), hierarchy
      .getName(), xAxis.getId(), xAxis.getViewId());
  ...
  static void updateX(XAxisHierarchy xAxisHierarchy, AxisHierarchy axisHierarchy,
    UserSession userSession) {
  ...
  private static final void updateXAlias(XAxisHierarchy xHierarchy,
    AxisHierarchy hierarchy, UserSession userSession) {
    ...
    if (hierarchy.getAliasMissing() != null) {
      ...
      // hierarchy.getHierarchy().getName() +
      // "' does no longer exist. No alias is selected instead.");
    }
    setActiveXAlias(xHierarchy, hierarchy.getProperty(AxisHierarchy.USE_ALIAS));
  }

  private static final void addXAliases(XAxisHierarchy xHierarchy,
    Attribute[] attributes) {
    for (Attribute attribute : attributes) {
      if (attribute.getType() == Attribute.TYPE_STRING
        && !"format".equalsIgnoreCase(attribute.getName())) {
  ...
  private static final void setActiveXAlias(XAxisHierarchy xHierarchy,
    Property<?> alias) {
  ...
  private static final void updateXSubset(XAxisHierarchy xHierarchy,
    AxisHierarchy hierarchy, UserSession userSession) {
    ...
    if (hierarchy.getSubsetMissing() != null) {
      ViewOpenWarnings.getInstance().addWarning(
        userSession.translate("subsetRemoved", hierarchy.getHierarchy().getName()));
      // "The selected subset of dimension '" +
      // hierarchy.getHierarchy().getName() +
      // "' does no longer exist. No subset is selected instead.");
    }
  ...
  private static final void setActiveXSubset(XAxisHierarchy xAxisHierarchy,
    Subset2 subset) {
  ...
  private static final void updateXSelectedElement(XAxisHierarchy xHierarchy,
    AxisHierarchy axisHierarchy, UserSession userSession) {
  ...
  private static final XElement getSelectedElement(AxisHierarchy axisHierarchy,
    UserSession userSession) {
    ...
    if (sel == null) {
      ...
      if (roots != null && roots.length > 0) {
        ...
        if (sel != null) {
          ViewOpenWarnings.getInstance().addWarning(
            userSession.translate("selectedElementRemoved", axisHierarchy
              .getHierarchy().getName(), sel.getName()));
          // "The selected element of dimension '" +
          // axisHierarchy.getHierarchy().getName() +
          // "' does no longer exist. The element '" +
          // sel.getName() + "' has been selected instead.");
        } else {
          ViewOpenWarnings.getInstance().addWarning(
            userSession.translate("selectedElementRemovedForGood", axisHierarchy
              .getHierarchy().getName()));
          // "The selected element of dimension '" +
          // axisHierarchy.getHierarchy().getName() +
          // "' does no longer exist. No alternative" +
  ...
  private static final String getAliasForElement(String name, String elementName,
    String format) {
  ...
  private static final void applyAlias(XElement selectedElement,
    ...
    if (alias != null) {
      ...
      if (prop != null && prop.getValue() != null) {
        selectedElement.setName(getAliasForElement(alias, axisHierarchy
          .getHierarchy().getElementById(selectedElement.getId()).getName(), prop
          .getValue().toString()));
      } else {
  ...
  private static final String getAlias(XElement forElement,
    AxisHierarchy axisHierarchy) {
    ...
    if (aliasProperty != null) {
      ...
      Element element = axisHierarchy.getHierarchy().getElementById(
        forElement.getId());
  ...
  private static final void updateXLocalFilter(XAxisHierarchy xHierarchy,
  ...
  private static final XElementNode createX(ElementNode elNode,
    ...
    XElementNode xElNode = XElementFactory.createX(elNode, xHierarchy.getId(),
