...
/**
 * <code>XViewItem</code> A header item model representation for GWTs RPC
 * feature.
 * 
 * @version $Id: XViewItem.java,v 1.3 2009/12/17 16:14:29 PhilippBouillon Exp $
 **/
public class XViewItem extends XObject {
  ...
  /**
   * Creates a new {@link XViewItem} for the specified path with the given name.
   * 
   * @param path
   *          the items path within its axis
   * @param name
   *          the items name
   */
  public XViewItem(String path, String name, XAxis2 axis,
    XViewItem parentInPrevHierarchy) {
    setId(path);
    setName(name);
    this.axis = axis;
   ...
   * 
   * @param item
   *          the next hierarchy root to add
   */
