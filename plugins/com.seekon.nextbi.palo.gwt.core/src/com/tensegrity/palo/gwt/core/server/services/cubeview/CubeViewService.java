...
/**
 * <code>CubeViewService</code> TODO DOCUMENT ME
 * 
 * @version $Id: CubeViewService.java,v 1.60 2010/04/12 11:14:15 PhilippBouillon
 *          Exp $
 **/
public class CubeViewService extends BasePaloServiceServlet {
  ...
  public final synchronized XLoadInfo willOpenView(String sessionId, XView xView)
    throws SessionExpiredException, PaloGwtCoreException {
    try {
      Account accountToUse = null;
      AuthUser user = getLoggedInUser(sessionId);
      Account referenceAccount = (Account) MapperRegistry.getInstance()
        .getAccountManagement().find(xView.getAccountId());
      ...
      if (referenceAccount != null) {
        for (Account acc : user.getAccounts()) {
          ...
          if (acc.getLoginName().equals(referenceAccount.getLoginName())) {
            if (acc.getConnection().getHost().equals(
              referenceAccount.getConnection().getHost())
              && acc.getConnection().getService().equals(
                referenceAccount.getConnection().getService())) {
              accountToUse = acc;
            }
          } else {
            if (acc.getConnection().getHost().equals(
        ...
        if (accountToUse instanceof PaloAccount) {
          ServerConnectionPool pool = ConnectionPoolManager.getInstance().getPool(
            accountToUse, sessionId);
          ((PaloAccountImpl) accountToUse).setConnection(pool
            .getConnection("willOpenView"));
          // ((PaloAccount) accountToUse).login();
        }
      }
      // xView.getAccountId()

      CubeViewController viewController = getControllerFor(sessionId, xView,
        accountToUse);
  ...
  public synchronized final XViewModel proceedOpenView(String sessionId,
    AuthUser user, String viewId) throws SessionExpiredException,
    PaloGwtCoreException {
    try {
      CubeViewController viewController = getControllerFor(sessionId, viewId);
      XViewModel model = viewController.proceedOpenView(getUserSession(sessionId),
        null, user, getNumberFormat(sessionId));
      return model;
    } catch (PaloAPIException e) {
  ...
  public final void cancelOpenView(String sessionId, String viewId)
    throws SessionExpiredException {
    UserSession userSession = getUserSession(sessionId);
    CubeViewController.removeViewById(userSession, viewId);
  }

  public void update(String sessionId, XViewModel xViewModel)
    throws SessionExpiredException {
    try {
  ...
  public final synchronized XLoadInfo willUpdateView(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
  ...
  public final synchronized XLoadInfo willUpdateView(String sessionId,
    String viewId, String axisHierarchyId, String axisId)
    throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
  ...
  public final synchronized XViewModel proceedUpdateView(String sessionId,
    AuthUser user, XViewModel xViewModel) throws SessionExpiredException,
    PaloGwtCoreException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
    return viewController.proceedUpdate(getUserSession(sessionId), user, xViewModel,
      getNumberFormat(sessionId));
  }

  public String[] hideItem(String sessionId, XAxisItem item, List<XAxisItem> roots,
    String viewId, String axisId, boolean hideLevel) throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
    return viewController.hideItem(item, roots, axisId, viewId, hideLevel);
  }

  public final XViewModel proceedUpdateViewWithoutTable(String sessionId,
    AuthUser user, XViewModel xViewModel) throws SessionExpiredException,
    PaloGwtCoreException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
    return viewController.proceedUpdateWithoutTable(user, xViewModel,
      getNumberFormat(sessionId));
  }

  public final XViewModel cancelUpdateView(String sessionId, AuthUser user,
    XViewModel xViewModel) throws SessionExpiredException, PaloGwtCoreException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
  ...
  public XDelta proceedExpand(String sessionId, XAxisItem item, String viewId,
    String axisId) throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
  ...
  public XViewModel proceedSwapAxes(String sessionId, XViewModel xViewModel)
    ...
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
    AuthUser user = getLoggedInUser(sessionId);
    return viewController.proceedSwapAxes(getUserSession(sessionId), user,
      getNumberFormat(sessionId), xViewModel);
  }

  public void cancelExpand(String sessionId, XAxisItem item, String viewId,
    String axisId) throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
  ...
  public void proceedCollapse(String sessionId, XAxisItem item, String viewId,
    String axisId) throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
  ...
  public XLoadInfo willSetExpandState(String sessionId, XAxisItem[] expanded,
    XAxisItem[] collapsed, int expandDepth, String viewId, String axisId)
    throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, viewId);
    return viewController.willSetExpandState(expanded, collapsed, expandDepth,
      axisId);
  }
  ...
  public XLoadInfo willChangeSelectedElement(String sessionId,
  ...
  public XViewModel proceedChangeSelectedElement(String sessionId,
    XViewModel xViewModel, XAxisHierarchy xAxisHierarchy, XElement selectedElement)
    throws SessionExpiredException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
    try {
      return viewController.proceedChangeSelectedElement(getUserSession(sessionId),
        getLoggedInUser(sessionId), xViewModel, xAxisHierarchy, selectedElement,
        getNumberFormat(sessionId));
  ...
  public final synchronized XLoadInfo willReload(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
    try {
      CubeViewController viewController = getControllerFor(sessionId, xViewModel);
  ...
  public final synchronized XViewModel proceedReload(String sessionId,
    AuthUser user, XViewModel xViewModel) throws SessionExpiredException {
    try {
      CubeViewController viewController = getControllerFor(sessionId, xViewModel);
      return viewController.proceedReloadView(getUserSession(sessionId), xViewModel,
        user, getNumberFormat(sessionId));
    } catch (Throwable t) {
      throw new SessionExpiredException("Session expired!");
    }
  }

  public final XLoadInfo willUpdateAxisHierarchy(String sessionId,
    XAxisHierarchy hierarchy) throws SessionExpiredException {
    try {
  ...
  public XElement update(String sessionId, XAxisHierarchy xAxisHierarchy)
    ...
    CubeViewController viewController = CubeViewController.getController(sessionId,
      viewId);
  ...
  // private final CubeViewController getControllerFor(String viewId, String
  // sessionId)
  // throws SessionExpiredException {
  // return CubeViewController.getController(sessionId, viewId);
  // }

  private synchronized final CubeViewController getControllerFor(String sessionId,
    XView xView, Account accountToUse) throws SessionExpiredException,
    PaloGwtCoreException, PaloIOException {
    UserSession userSession = getUserSession(sessionId);
    ...
    if (!accountToUse.getId().equals(view.getAccount().getId())) {
      try {
        ...
        ((ViewImpl) view)
          .setAccount(accountToUse.getUser(), accountToUse, sessionId);
      } finally {
        CubeViewReader.CHECK_RIGHTS = true;
        ConnectionPoolManager.getInstance().disconnect(accountToUse, sessionId,
          "CubeViewService.willOpenView");
      }
    }
    if (!accountToUse.getUser().hasPermission(Right.READ, view)) {
      throw new NoPermissionException("User has no permission to open this view.",
        view, accountToUse.getUser(), Right.READ);
    }
    ...
    // }
    // }
    return CubeViewController.getController(getLoggedInUser(sessionId), sessionId,
      view, getSession());
  }

  private final CubeViewController getControllerFor(String sessionId,
    XViewModel xViewModel) throws SessionExpiredException {
  ...
  // throws SessionExpiredException {
  // try {
  // CubeViewController viewController = getControllerFor(sessionId,
  // xViewModel);
  ...
  public XLoadInfo updateLoadInfo(String sessionId, XViewModel xViewModel,
    int cellsToDisplay) throws SessionExpiredException {
    try {
      CubeViewController viewController = getControllerFor(sessionId, xViewModel);
  ...
  public final XViewModel proceedUpdate(String sessionId, AuthUser user,
    XViewModel xViewModel) throws SessionExpiredException {
    try {
      CubeViewController viewController = getControllerFor(sessionId, xViewModel);
      return viewController.proceedUpdate(getUserSession(sessionId), user,
        xViewModel, getNumberFormat(sessionId));
    } catch (Throwable t) {
      throw new SessionExpiredException("Session expired!");
    }
  }

  public XViewModel save(String sessionId, XViewModel xViewModel)
    ...
    if (view != null) {
      ...
      // String isSpagoBIDev = (String)
      // getSession().getAttribute("isdeveloper");
      //			
      // //SpagoBI User (executing document)
      // if(isSpagoBIDev == null || isSpagoBIDev.equals("")){
      // //System.out.println("USER");
      // //logger.info("USER");
      // ////jpaloUtil.saveSubobjectForJPalo(getSession(), view.getName(),
      // view.getName()+ " View definitions/changes", xml);
      //
      // }else{
  ...
  public synchronized XView saveAs(String sessionId, String name,
    XViewModel xViewModel) throws OperationFailedException, SessionExpiredException {
    ViewService viewService = ServiceProvider
      .getViewService(getLoggedInUser(sessionId));
    Cube cube = getCube(sessionId, xViewModel);
    View newView = viewService.createViewAsSubobject(name, cube,
      getLoggedInUser(sessionId), sessionId, xViewModel.getExternalId());
    String currentXml = null;
    try {
      currentXml = getDefinition(sessionId, xViewModel);
    } catch (Throwable t) {
      if (t.getMessage() != null
        && t.getMessage().toLowerCase().indexOf("not enough rights") != -1) {
        return null;
      }
    }
    XView xview = null;
    if (currentXml != null) {
      ...
      for (Account usAc : getLoggedInUser(sessionId).getAccounts()) {
        if (acc.getConnection().getId().equals(usAc.getConnection().getId())) {
          try {
            ((ViewImpl) newView).setAccount(getLoggedInUser(sessionId), usAc,
              sessionId);
      ...
      try {
        ...
        if (isSpagoBIDev == null || isSpagoBIDev.equals("")) {
          // System.out.println("USER");
          // logger.info("USER");
          // //jpaloUtil.saveSubobjectForJPalo(getSession(), name,
          // newView.getName()+ " View definitions/changes", currentXml);
        ...
        ConnectionPoolManager.getInstance().disconnect(newView.getAccount(),
          sessionId, "CubeViewService.saveAs");
      }
    }
    return xview;
  ...
  public XCellCollection writeCell(String sessionId, XCell cell,
    XViewModel xViewModel, NumberFormat format) throws SessionExpiredException {
    try {
  ...
  public void remove(String sessionId, XViewModel xViewModel)
    throws SessionExpiredException {
    if (xViewModel == null)
      return;
  ...
  public View convert(String sessionId, org.palo.api.CubeView legacyView,
    String newViewName) throws OperationFailedException, SessionExpiredException {
    return CubeViewConverter.toView(newViewName, legacyView,
      getLoggedInUser(sessionId), sessionId);
  }

  // public View createDefaultViewFor(String sessionId, Cube cube, String
  // viewName, String accountId)
  // throws OperationFailedException, SessionExpiredException {
  ...
  private final void updateProperties(CubeView cubeView, XViewModel model) {
    setOrRemoveProperty(cubeView, CubeView.PROPERTY_ID_HIDE_EMPTY, model
      .isHideEmptyCells());
    setOrRemoveProperty(cubeView, CubeView.PROPERTY_ID_SHOW_RULES, model
      .isShowRules());
    setOrRemoveProperty(cubeView, CubeView.PROPERTY_ID_REVERSE_HORIZONTAL_LAYOUT,
      model.isColumnsReversed());
    setOrRemoveProperty(cubeView, CubeView.PROPERTY_ID_REVERSE_VERTICAL_LAYOUT,
      model.isRowsReversed());
  }

  public final XPrintResult generatePdf(String sessionId, AuthUser user,
    XViewModel xViewModel, XPrintConfiguration config)
    throws SessionExpiredException, PaloGwtCoreException {
    CubeViewController viewController = getControllerFor(sessionId, xViewModel);
    try {
      viewController.update(xViewModel);
      ...
      conf.setIndent(config.isShowExpansionStateIcons() == false ? config.isIndent()
        : true);
      conf.setShowPageNumbers(config.isPrintPageNumbers());
      conf.setMaxWidths(config.getMaxColString(), config.getMaxRowsHeaderString(),
      ...
      // System.err.println("Path to pdf: " +
      // exporter.getOutFile().getAbsolutePath());
