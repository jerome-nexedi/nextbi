...
/**
 * <code>XViewModelFactory</code> TODO DOCUMENT ME
 * 
 * @version $Id: XViewModelFactory.java,v 1.6 2010/03/02 08:58:27
 *          PhilippBouillon Exp $
 **/
class XViewModelFactory {

  static final XViewModel createX(ViewModel viewModel, String id,
  ...
  private static final void createRowAxis(ViewModel viewModel, XViewModel xView,
    UserSession userSession) {
    AxisModel rowAxis = viewModel.getRowAxis();
    XAxis xRowAxis = XAxisFactory.createX(viewModel.getRowAxis().getAxis(), xView
      .getId(), userSession);
  ...
  private static final void createColumnAxis(ViewModel viewModel, XViewModel xView,
    UserSession userSession) {
    AxisModel colAxis = viewModel.getColumnAxis();
    XAxis xColAxis = XAxisFactory.createX(viewModel.getColumnAxis().getAxis(), xView
      .getId(), userSession);
  ...
  private static final void createRepositoryAxis(ViewModel viewModel,
    XViewModel xView, UserSession userSession) {
    XAxis repository = XAxisFactory.createX(viewModel.getRepositoryAxis(), xView
      .getId(), userSession);
    xView.addRepositoryAxis(repository);
  }

  private static final void createSelectionAxis(ViewModel viewModel,
    XViewModel xView, UserSession userSession) {
    XAxis selection = XAxisFactory.createX(viewModel.getSelectionAxis(), xView
      .getId(), userSession);
