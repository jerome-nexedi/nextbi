...
var CodeMirror = (function() {
	...
	function CodeMirror(place, options) {
		...
		forEach(
				...
				function(file) {
					html
							.push('<link rel="stylesheet" type="text/css" href="' + file + '"/>')
				});
		...
		html
				.push('</head><body style="border-width: 0;" class="editbox" spellcheck="' + (options.disableSpellcheck ? "false"
						: "true") + '"></body></html>');
	...
	CodeMirror.prototype = {
		...
		focus : function() {
			this.win.focus();
			if (this.editor.selectionSnapshot) {
	...
	CodeMirror.isProbablySupported = function() {
		...
		if (window.opera) {
			...
			if (/Apple Computers, Inc/.test(navigator.vendor)
				...
				if (document.selection
					...
					if (match = navigator.userAgent.match(/gecko\/(\d{8})/i)) {
						return Number(match[1]) >= 20050901
					} else {
						if (match = navigator.userAgent
								.match(/AppleWebKit\/(\d+)/)) {
