Jedox.wss.cndfmt = new function() {
	...
	this.set = function(rules) {
		var defSel = Jedox.wss.app.environment.defaultSelection, conn = Jedox.wss.backend.conn, fParser = Jedox.wss.formula, jwstyle = Jedox.wss.style, activeBook = Jedox.wss.app.activeBook, rule, style, borders, btype, border, ranges, key, rng, rngs, sets = [], set, j, i = -1;
		while ((rule = rules[++i]) !== undefined) {
			set = {
				rule : rule.type.concat("(", rule.operator, rule.sit ? ";1;)"
						: ";0;)")
			};
			...
			if ("applies_to" in rule) {
				...
				rngs = fParser.parse(rule.applies_to);
				j = -1;
		...
		if (sets.length) {
			conn.cmd( [ activeBook, activeBook.exec ], [ "cfset" ], defSel
					.getActiveRange().getActiveCell().getCoords().concat(sets),
					activeBook.getRealGridRange(), false, false, conn.Q_STYLE
							| conn.Q_FMT_VAL | conn.Q_FMT | conn.Q_LOCK)
		}
	};
	this.get = function(scope) {
		var defSel = Jedox.wss.app.environment.defaultSelection, conn = Jedox.wss.backend.conn, jwstyle = Jedox.wss.style, params = [], i = -1, rule, rules, ruletmp, css;
		switch (scope) {
		...
		rules = conn.cmd(null, [ "cfget" ], params);
	...
	this.remove = function(scope, ids) {
		var defSel = Jedox.wss.app.environment.defaultSelection, conn = Jedox.wss.backend.conn, activeBook = Jedox.wss.app.activeBook, params = [];
		switch (scope) {
		case that.SCOPE_CURR_SEL:
		case undefined:
		...
		conn.cmd( [ activeBook, activeBook.exec ], [ "cfdel" ], params,
				activeBook.getRealGridRange(), false, false, conn.Q_STYLE
						| conn.Q_FMT_VAL | conn.Q_FMT | conn.Q_LOCK)
	}
};