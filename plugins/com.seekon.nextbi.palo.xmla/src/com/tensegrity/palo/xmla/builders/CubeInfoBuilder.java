...
public class CubeInfoBuilder
{
        ...
        for(int j = 0; j < nlRow.getLength(); j++)
            if(nlRow.item(j).getNodeType() == 1)
                if(nlRow.item(j).getNodeName().equals("CUBE_NAME"))
                {
                    ...
                    {
                        com.tensegrity.palo.xmla.XMLAVariableInfo infos[] = BuilderRegistry.getInstance().getVariableInfoBuilder().requestVariables(xmlaClient, (XMLADatabaseInfo)cubeInfo.getDatabase(), cubeInfo.getId());
                        cubeInfo.setVariables(infos);
                    }
                } else
                if(nlRow.item(j).getNodeName().equals("DESCRIPTION") && xmlaClient.isSAP())
                    cubeInfo.setName(XMLAClient.getTextFromDOMElement(nlRow.item(j)));
                else
                if(nlRow.item(j).getNodeName().equals("CUBE_CAPTION"))
                    cubeInfo.setName(XMLAClient.getTextFromDOMElement(nlRow.item(j)));
    ...
    private CellInfo parseValueNode(Node node, String strCoord[])
    {
        ...
        if(map.getLength() > 0)
        {
            ...
            if(type != null)
            {
                ...
                if(value.equals("xsd:decimal") || value.equals("xsd:integer") || value.equals("xsd:double") || value.equals("xsd:int") || value.equals("xsd:unsignedint"))
                    doubleType = true;
            }
        }
    ...
    private String getCharacteristicVariableString(PropertyInfo info)
    {
        ...
        if(selectionType == 1)
        {
            result = (new StringBuilder(String.valueOf(result))).append(" ").append(idString).append(" INCLUDING ").toString();
            result = (new StringBuilder(String.valueOf(result))).append(XMLADimensionInfo.transformId(selectedText)).toString();
        } else
        if(selectionType == 2)
        {
            ...
            if(uniqueName[0].equals(uniqueName[1]))
                result = (new StringBuilder(String.valueOf(result))).append(XMLADimensionInfo.transformId(uniqueName[0])).toString();
            else
                result = (new StringBuilder(String.valueOf(result))).append(XMLADimensionInfo.transformId(uniqueName[0])).append(":").append(XMLADimensionInfo.transformId(uniqueName[1])).toString();
        } else
        if(selectionType == 3)
        {
            StringTokenizer tok = new StringTokenizer(selectedText, "\n");
            StringBuffer tempResult = new StringBuffer();
            for(; tok.hasMoreTokens(); tempResult.append((new StringBuilder(" ")).append(idString).append(" INCLUDING ").append(XMLADimensionInfo.transformId(tok.nextToken())).toString()));
    ...
    private String getFloatingPointVariableString(PropertyInfo info)
    {
        ...
        {
            PropertyInfo idProp = info.getChild("ID");
            return (new StringBuilder(" ")).append(XMLADimensionInfo.transformId(idProp.getValue())).append(" INCLUDING ").append(text).toString();
        }
    }
    ...
    private String analyzeVariable(PropertyInfo info)
    {
        ...
        for(int i = 0; i < j; i++)
        {
            ...
            if(pi.getId().equals("DATATYPE"))
            {
                ...
                System.out.println((new StringBuilder("Unknown variable datatype: ")).append(pi.getValue()).toString());
            }
        }
    ...
    private String applyDatabaseSpecificMDX(String query, XMLAClient client, XMLACubeInfo cube)
    {
        ...
        {
            PropertyInfo varDef = xmlaConnection.getTypedPropertyLoader(cube).load("SAP_VARIABLE_DEF");
            if(varDef == null)
                return query;
            ...
            if(varInfos != null && varInfos.length > 0)
            {
                ...
                for(int n = varInfos.length; i < n; i++)
                {
                    ...
                    if(text.length() != 0)
                    {
                        if(!added)
                        {
                            query = (new StringBuilder(String.valueOf(query))).append(" SAP VARIABLES").toString();
                            added = true;
                        }
                        if(!first)
                            query = (new StringBuilder(String.valueOf(query))).append(",\n").toString();
                        query = (new StringBuilder(String.valueOf(query))).append(text).toString();
                        first = false;
                    }
                }
    ...
    {
        CellInfo result[] = getDataBulk(conName, client, cube, new XMLAElementInfo[][] {
            coordinates
        }, connection);
    ...
    public CellInfo[] getDataArray(String conName, XMLAClient client, XMLACubeInfo cube, XMLAElementInfo elements[][], XMLAConnection connection)
    {
        ...
        for(int i = 0; i < dimensionSize; i++)
        {
            ...
            {
                querySize[i] = 1;
                String defName = ((XMLADimensionInfo)elements[i][0].getDimension()).getDefaultElementName();
            ...
            } else
            {
                ...
                if(!first)
                    sb.append(", ");
                sb.append((new StringBuilder("{")).append(axes[i]).append("} on Axis (").append(axisCounter).append(")").toString());
                axisCounter++;
                first = false;
            }
        }
        ...
        where.append(")");
        sb.append((new StringBuilder(" FROM [")).append(cube.getId()).append("]").toString());
        if(where.length() > 2)
        ...
        for(int i = 0; i < elements.length; i++)
        {
            for(int j = 0; j < elements[i].length; j++)
                elementMapper.put(elements[i][j].getUniqueName(), ((Object) (new Object[] {
                    elements[i][j], Integer.valueOf(i), elementPositions.get(elements[i][j].getUniqueName())
                })));

        }

        if(sb.length() > 0x186a0)
            sb = shortenQuery(client, cube, sb, null, null, axes, dimensionSize, where, elementMapper);
                ...
                if(cl.item(i).getNodeType() == 1)
                {
                    ...
                    if(ordinal != null)
                    {
                        ...
                        for(int j = 0; j < nl.getLength(); j++)
                            if(nl.item(j).getNodeType() == 1 && nl.item(j).getNodeName().equals("Value"))
                                resultValues[transformCellOrdinal(cellOrdinal, resultCoordinates, elementMapper, querySize)] = parseValueNode(nl.item(j), new String[] {
                                    "a", "b"
                                });
    ...
    private StringBuffer shortenQuery(XMLAClient client, XMLACubeInfo cube, StringBuffer sb, int querySize[], HashMap elementIndexMap[], StringBuffer axes[], int dimensionSize, 
            StringBuffer where, HashMap mapper)
    {
        ...
        for(int n = dimensionSize; i < n; i++)
            if(axes[i].length() > 0)
            {
                if(!axes[i].toString().equals("-") && !axes[i].toString().equals("+"))
                {
                    if(!first)
                        sb.append(", ");
                    sb.append((new StringBuilder("{")).append(axes[i]).append("} on Axis (").append(axisCounter).append(")\n").toString());
                    first = false;
                    axisCounter++;
                }
            } else
            if(!axes[i].toString().equals("+"))
            {
                DimensionInfo dimInfo[] = BuilderRegistry.getInstance().getDimensionInfoBuilder().getDimensionInfo(xmlaConnection, client, (XMLADatabaseInfo)cube.getDatabase(), cube);
                XMLADimensionInfo dim = (XMLADimensionInfo)dimInfo[i];
                if(!first)
                    sb.append(", ");
                first = false;
                if(dim.getHierarchyUniqueName() != null)
                    sb.append((new StringBuilder("{")).append(dim.getHierarchyUniqueName()).append(".Members} on Axis (").append(axisCounter).append(")").toString());
                else
                    sb.append((new StringBuilder("{")).append(dim.getName()).append(".Members} on Axis (").append(axisCounter).append(")").toString());
                int counter = 0;
                ...
                for(int k = 0; k < l; k++)
                {
                    ...
                    mapper.put(((XMLAElementInfo)el).getUniqueName(), ((Object) (new Object[] {
                        el, Integer.valueOf(i), Integer.valueOf(counter)
                    })));
                    counter++;
                }
                ...
                if(elementIndexMap != null)
                {
                    ...
                    XMLAElementInfo els[] = BuilderRegistry.getInstance().getElementInfoBuilder().getElements(xmlaConnection, client, cube.getId(), dim);
    ...
    {
        return (new String[] {
            (new StringBuilder()).append(ordinal).toString()
        });
    }
    ...
    private final String[][] parseResultCoordinates(Document doc)
    {
                                                            ...
                                                            if(eel.item(l).getNodeType() == 1)
                                                            {
                                                                ...
                                                                if(eeel != null && eeel.getLength() > 0)
                                                                {
                                                                    for(int m = 0; m < eeel.getLength(); m++)
                                                                        if(eeel.item(m).getNodeType() == 1 && eeel.item(m).getNodeName().equals("UName"))
                                                                        {
        ...
        for(int n = resultCoordinatesArray.size(); i < n; i++)
            resultCoordinates[i] = (String[])((ArrayList)resultCoordinatesArray.get(i)).toArray(new String[0]);
    ...
    public ExtendedCellInfo readWholeCube(String conName, XMLAClient client, XMLACubeInfo cube, XMLAConnection connection)
    {
    	try
      {
        ...
        sb = new StringBuffer("SELECT ");
        dimInfo = BuilderRegistry.getInstance().getDimensionInfoBuilder().getDimensionInfo(xmlaConnection, client, (XMLADatabaseInfo)cube.getDatabase(), cube);
        boolean first = true;
        for(int i = 0; i < dimInfo.length; i++)
        {
            ...
            first = false;
            if(dim.getHierarchyUniqueName() != null)
                sb.append((new StringBuilder("NON EMPTY ")).append(dim.getHierarchyUniqueName()).append(" on Axis (").append(i).append(")").toString());
            else
                sb.append((new StringBuilder("NON EMPTY ")).append(dim.getName()).append(" on Axis (").append(i).append(")").toString());
        }
            ...
            for(int i = 0; i < cl.getLength(); i++)
                if(cl.item(i).getNodeType() == 1)
                {
                    ...
                    if(ordinal != null)
                    {
                        ...
                        for(int j = 0; j < nl.getLength(); j++)
                            if(nl.item(j).getNodeType() == 1 && nl.item(j).getNodeName().equals("Value"))
                                resultValues.add(parseValueNode(nl.item(j), convertOrdinal(cellOrdinal, dimInfo)));

                    }
                }

        }
        return new ExtendedCellInfo((CellInfo[])resultValues.toArray(new CellInfo[0]), resultCoordinates);
      }catch(IOException e){
    ...
    private final int transformCellOrdinal(int ordinal, String resultCoordinates[][], HashMap mapper, int elementLengths[])
    {
    ...
    public CellInfo[] getDataBulk(String conName, XMLAClient client, XMLACubeInfo cube, XMLAElementInfo elements[][], XMLAConnection connection)
    {
        ...
        for(int i = 0; i < dimensionSize; i++)
        {
            ...
            for(int m = elements.length; j < m; j++)
                if(!axisSet.contains(elements[j][i]))
                {
                    ...
                    elementIndexMap[i].put(elements[j][i].getId(), new Integer(axisSet.size() - 1));
                    if(!first)
                        axes[i].append(", ");
                    elementPositions.put(elements[j][i].getUniqueName(), Integer.valueOf(querySize[i]));
                    querySize[i]++;
            ...
            if(axisSet.size() > 1)
            {
                String xmlaDimName = ((XMLADimensionInfo)elements[0][i].getDimension()).getDimensionUniqueName();
        ...
        for(int i = 0; i < dimensionSize; i++)
            ...
            {
                String xmlaDimName = ((XMLADimensionInfo)elements[0][i].getDimension()).getDimensionUniqueName();
                String defName = ((XMLADimensionInfo)elements[0][i].getDimension()).getDefaultElementName();
        ...
        for(int i = 0; i < elements.length; i++)
        {
            for(int j = 0; j < elements[i].length; j++)
                elementMapper.put(elements[i][j].getUniqueName(), ((Object) (new Object[] {
                    elements[i][j], Integer.valueOf(j), elementPositions.get(elements[i][j].getUniqueName())
                })));

        }

        if(sb.length() > 0x186a0)
            sb = shortenQuery(client, cube, sb, querySize, elementIndexMap, axes, dimensionSize, where, elementMapper);
        sb = new StringBuffer(applyDatabaseSpecificMDX(sb.toString(), client, cube));
        if(XMLAClient.isDebug())
