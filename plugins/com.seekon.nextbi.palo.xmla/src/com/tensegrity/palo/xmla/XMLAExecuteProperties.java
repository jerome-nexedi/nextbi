...
public class XMLAExecuteProperties extends XMLAProperties
{
  ...
  protected String getPropertyListXML(String paramString)
  {
    ...
    if ((getDataSourceInfo() != null) && (getDataSourceInfo().trim().length() > 0))
      localStringBuffer.append("\n" + paramString + elementIndent + "<DataSourceInfo>" + getDataSourceInfo().trim() + "</DataSourceInfo>");
    if ((getCatalog() != null) && (getCatalog().trim().length() > 0))
      localStringBuffer.append("\n" + paramString + elementIndent + "<Catalog>" + getCatalog().trim() + "</Catalog>");
    if ((getFormat() != null) && (getFormat().trim().length() > 0))
      localStringBuffer.append("\n" + paramString + elementIndent + "<Format>" + getFormat().trim() + "</Format>");
    if ((getContent() != null) && (getContent().trim().length() > 0))
      localStringBuffer.append("\n" + paramString + elementIndent + "<Content>" + getContent().trim() + "</Content>");
    if ((this.axisFormat != null) && (this.axisFormat.trim().length() > 0))
      localStringBuffer.append("\n" + paramString + elementIndent + "<AxisFormat>" + this.axisFormat.trim() + "</AxisFormat>");
    localStringBuffer.append("\n" + paramString + "</PropertyList>");
 ...
 * Qualified Name:     com.tensegrity.palo.xmla.XMLAExecuteProperties
