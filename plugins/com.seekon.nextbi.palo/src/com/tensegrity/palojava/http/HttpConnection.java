...
/*      */     }
/*      */ 
/*  222 */     this.connectionInfo.setData("com.tensegrity.palojava.pass#" + user, pass);
/*      */     try
/*      */     {
...
/*  230 */       throw new PaloException("Failed to create encrypted password for user '" + 
/*  231 */         user + "'!", ex);
/*      */     }
/*      */ 
...
/*  273 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*  284 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*  295 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */   }
/*      */ 
/*      */   public final boolean addElements(DimensionInfo dimension, String[] names, int type, ElementInfo[][] children, double[][] weights)
/*      */   {
/*      */     try {
/*  316 */       setOwnChange(true);
/*  317 */       ElementHandler elHandler = this.handlerRegistry.getElementHandler();
/*  318 */       return elHandler.createBulk(dimension, names, type, children, weights);
/*      */     } catch (IOException e) {
/*  320 */       throw new PaloException(e.getLocalizedMessage(), e);
/*      */     }
/*      */   }
/*      */ 
/*      */   public final boolean addElements(DimensionInfo dimension, String[] names, int[] types, ElementInfo[][] children, double[][] weights)
/*      */   {
/*  326 */     if (getServerInfo().getMajor() < 3) {
/*  327 */       throw new PaloException("The method 'addElements(...int [] types...)' is only available for Palo Server version 3 or above. Please use 'addElements(...int type...)' instead.");
/*      */     }
/*      */ 
/*      */     try
/*      */     {
...
/*  334 */       return elHandler.createBulk(dimension, names, types, children, weights);
/*      */     } catch (IOException e) {
...
/*  343 */       (getServerInfo().getMajor() == 3) && (getServerInfo().getMinor() == 0) && (getServerInfo().getBuildNumber() < 555))) {
/*  344 */       throw new PaloException("The method 'Hierarchy.updateConsolidations(...)' is only available for Palo Server version 3 or above.");
/*      */     }
/*      */     try
/*      */     {
/*  348 */       setOwnChange(true);
/*  349 */       ElementHandler elHandler = this.handlerRegistry.getElementHandler();
/*  350 */       return elHandler.replaceBulk(dimension, elements, type, children, weights);
/*      */     } catch (IOException e) {
...
/*  361 */       return elHandler.create(hierarchy.getDimension(), name, type, children, weights);
/*      */     } catch (IOException e) {
...
/*  381 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*  440 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*  450 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */     try {
/*  459 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/*  468 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/*  477 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */   {
/*      */     try {
/*  565 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */   {
/*      */     try {
/*  585 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */   {
/*      */     try {
/*  595 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */     try {
/*  604 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */     try {
/*  613 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */     try
/*      */     {
/*  649 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/*  694 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/*  732 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/*  741 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*  760 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*  781 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*  811 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */   }
/*      */ 
/*      */   public final RuleInfo createRule(CubeInfo cube, String definition, String externalIdentifier, boolean useIt, String comment, boolean activate)
/*      */   {
/*      */     try
/*      */     {
...
/*      */     } catch (IOException e) {
/* 1009 */       throw new PaloException("Failed to receive rule for cell from cube: " + 
/* 1010 */         cube.getName(), e);
/*      */     }
/*      */   }
/*      */ 
...
/* 1045 */         return null;
/* 1046 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/*      */     try {
/* 1055 */       DatabaseHandler dbHandler = this.handlerRegistry.getDatabaseHandler();
...
/*      */     try {
/* 1073 */       DimensionHandler dimHandler = this.handlerRegistry.getDimensionHandler();
...
/* 1200 */     String key = HttpElementLoader.class.getName() + "#" + dbId + "#" + hierarchy.getDimension().getId();
...
/* 1232 */     String key = HttpRuleLoader.class.getName() + "#" + dbId + "#" + cube.getId();
...
/* 1363 */       return true;
/*      */     } catch (IOException e) {
/* 1365 */       throw new PaloException(e.getLocalizedMessage(), e); } catch (PaloException pe) {
/*      */     }
/* 1367 */     return false;
/*      */   }
/*      */ 
 ...
 * Qualified Name:     com.tensegrity.palojava.http.HttpConnection
