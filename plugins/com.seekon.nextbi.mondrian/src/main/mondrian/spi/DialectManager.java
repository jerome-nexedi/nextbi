 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/spi/DialectManager.java#5 $
 * @since Jan 13, 2009
 */
public abstract class DialectManager {
	...
	static DialectFactory createFactoryForDialect(
		...
		try {
			final Constructor<? extends Dialect> constructor = dialectClass
	 ...
	 * 
	 * <p>
	 * <code>DialectManagerImpl</code> has a non-static method for each public
	 * static method in <code>DialectManager</code>.
	 */
	private static class DialectManagerImpl {
		 ...
		 * 
		 * <p>
		 * Loads all dialects that can be found on the classpath according to the
		 * JAR service provider specification. (See
		 * {@link mondrian.util.ServiceDiscovery} for more details.)
		 */
		DialectManagerImpl() {
			final List<DialectFactory> list = new ArrayList<DialectFactory>();
			final List<Class<Dialect>> dialectClasses = ServiceDiscovery.forClass(
					Dialect.class).getImplementor();
			for (Class<Dialect> dialectClass : dialectClasses) {
				DialectFactory factory = createFactoryForDialect(dialectClass);
				if (factory != null) {
					list.add(factory);
				}
			}
			...
			final DialectFactory fallbackFactory = new DialectFactory() {
				public Dialect createDialect(DataSource dataSource,
					...
					try {
						...
						throw Util.newError(e, "Error while creating a generic dialect for"
			...
			factory = new CachingDialectFactory(new ChainDialectFactory(
					Arrays.asList(registeredFactory, fallbackFactory)));
		}
		...
		/**
		 * Implements
		 * {@link DialectManager#createDialect(javax.sql.DataSource,java.sql.Connection)}
		 * .
		 * 
		 * <p>
		 * The method synchronizes on a singleton class, so prevents two threads
		 * from accessing any dialect factory simultaneously.
		 * 
		 * @param dataSource
		 *          Data source
		 * @param connection
		 ...
		 */
		synchronized Dialect createDialect(DataSource dataSource,
				Connection connection) {
			...
			if (dialect == null) {
				throw Util.newError("Cannot create dialect for JDBC connection"
						+ connection);
			}
			return dialect;
		}
	...
	/**
	 * Implementation of {@link DialectFactory} that tries to create a Dialect
	 * using a succession of underlying factories.
	 */
	static class ChainDialectFactory implements DialectFactory {
		 ...
		 * 
		 * @param dialectFactoryList
		 *          List of underlying factories
		 */
	...
	/**
	 * Implementation of {@link DialectFactory} that calls a class's
	 * {@code public &lt;init&gt;(Connection connection)} constructor.
	 */
	static class ConstructorDialectFactory implements DialectFactory {
		...
		ConstructorDialectFactory(Constructor<? extends Dialect> constructor) {
			...
			assert constructor.getParameterTypes()[0] == java.sql.Connection.class;
		...
		public Dialect createDialect(DataSource dataSource, Connection connection) {
			...
			try {
				...
				throw Util.newError(e, "Error while instantiating dialect of class "
	...
	/**
	 * Implementation of {@link mondrian.spi.DialectFactory} that caches dialects
