 ...
 * 
 * <p>
 * If you are writing a class for a specific database dialect, we recommend that
 * you use this as a base class, so your dialect class will be
 ...
 * 
 * <p>
 * Mondrian uses JdbcDialectImpl as a fallback if it cannot find a more specific
 * dialect. JdbcDialectImpl reads properties from the JDBC driver's metadata, so
 * can deduce some of the dialect's behavior.
 * </p>
 * 
 * @author jhyde
 * @version $Id:
...
public class JdbcDialectImpl implements Dialect {
	...
	/**
	 * Indicates whether the database allows selection of columns not listed in
	 * the group by clause.
	 */
	...
	private static final int[] CONCURRENCY_VALUES = { ResultSet.CONCUR_READ_ONLY,
			ResultSet.CONCUR_UPDATABLE };

	/**
	 * The size required to add quotes around a string - this ought to be large
	 * enough to prevent a reallocation.
	 */
	 ...
	 * 
	 * <p>
	 * To prevent connection leaks, this constructor does not hold a reference to
	 * the connection after the call returns. It makes a copy of everything useful
	 * during the call. Derived classes must do the same.
	 * </p>
	 * 
	 * @param connection
	 *          Connection
	...
	protected String deduceIdentifierQuoteString(DatabaseMetaData databaseMetaData) {
		try {
			final String quoteIdentifierString = databaseMetaData
			...
			for (int concurrency : CONCURRENCY_VALUES) {
				try {
					if (databaseMetaData.supportsResultSetConcurrency(type, concurrency)) {
						...
						supports.add(new ArrayList<Integer>(Arrays
								.asList(type, concurrency)));
					}
				} catch (SQLException e) {
	...
	/**
	 * <p>
	 * Detects whether the database is configured to permit queries that include
	 * columns in the SELECT that are not also in the GROUP BY. MySQL is an
	 * example of one that does, though this is configurable.
	 * </p>
	 * 
	 * <p>
	 * The expectation is that this will not change while Mondrian is running,
	 * though some databases (MySQL) allow changing it on the fly.
	 * </p>
	 * 
	 * @param conn
	...
	/**
	 * Generic algorithm to generate inline values list, using an optional FROM
	 * clause, specified by the caller of this method, appropriate to the dialect
	 * of SQL.
	 * 
	 * @param columnNames
	 *          Column names
	...
	protected String generateInlineGeneric(List<String> columnNames,
			List<String> columnTypes, List<String[]> valueList, String fromClause,
			boolean cast) {
	 ...
	 * 
	 * <p>
	 * If NULL values are present, we use a CAST to ensure that they have the same
	 * type as other columns:
	 * 
	 * <blockquote><code>SELECT * FROM
	 ...
	 * @return Expression that returns the given values
	 */
	public String generateInlineForAnsi(String alias, List<String> columnNames,
			List<String> columnTypes, List<String[]> valueList, boolean cast) {
		final StringBuilder buf = new StringBuilder();
	...
	/**
	 * Guesses the type of a column based upon (a) its basic type, (b) a list of
	 * values.
	 * 
	 * @param basicType
	 *          Basic type
	 ...
	 * @return SQL type
	 */
	private static String guessSqlType(String basicType,
			List<String[]> valueList, int column) {
	 ...
	 * 
	 * <p>
	 * This default implementation makes use of the ANSI SQL 1999
	 * CASE-WHEN-THEN-ELSE in conjunction with IS NULL syntax. The resulting SQL
	 * will look something like this:
	 * 
	 * <p>
	 * <code>CASE WHEN "expr" IS NULL THEN 0 ELSE 1 END</code>
	 * 
	 * <p>
	 * You can override this method for a particular database to use something
	 * more efficient, like ISNULL().
	 * 
	 * <p>
	 * ANSI SQL provides the syntax "ASC/DESC NULLS LAST" and
	 * "ASC/DESC NULLS FIRST". If your database supports the ANSI syntax,
	 * implement this method by calling {@link #generateOrderByNullsAnsi}.
	 * 
	 * <p>
	 * This method is only called from
	 * {@link #generateOrderItem(String, boolean, boolean, boolean)}. Some
	 * dialects override that method and therefore never call this method.
	 * 
	 * @param expr
	 *          Expression.
	 * @param ascending
	 ...
	 *          Whether nulls should appear first or last.
	...
	protected String generateOrderByNulls(String expr, boolean ascending,
		...
		if (collateNullsLast) {
			...
			if (ascending) {
				return "CASE WHEN " + expr + " IS NULL THEN 0 ELSE 1 END, " + expr
						+ " ASC";
			} else {
				return "CASE WHEN " + expr + " IS NULL THEN 0 ELSE 1 END, " + expr
						+ " DESC";
	...
	/**
	 * Implementation for the {@link #generateOrderByNulls} method that uses the
	 * ANSI syntax "expr direction NULLS LAST" and "expr direction NULLS FIRST".
	 * 
	 * @param expr
	 *          Expression
	 * @param ascending
	 ...
	 */
	public static DatabaseProduct getProduct(String productName,
			String productVersion) {
		...
		if (productName.equals("ACCESS")) {
			...
			if (productName.startsWith("DB2 UDB for AS/400")) {
				...
				/*
				 * if (version_release.length > 2 && "04".compareTo(version_release[0])
				 * > 0 || ("04".compareTo(version_release[0]) == 0 &&
				 * "03".compareTo(version_release[1]) >= 0)) return true;
				 */
