 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/spi/impl/OracleDialect.java#14 $
 * @since Nov 23, 2008
 */
public class OracleDialect extends JdbcDialectImpl {
  ...
  public static final JdbcDialectFactory FACTORY = new JdbcDialectFactory(
    OracleDialect.class, DatabaseProduct.ORACLE);
  ...
  public String generateInline(List<String> columnNames, List<String> columnTypes,
    ...
    return generateInlineGeneric(columnNames, columnTypes, valueList, " from dual",
      false);
  }
  ...
  public String generateRegularExpression(String source, String javaRegex) {
    ...
    if (escapeMatcher.matches()) {
      ...
      javaRegex = javaRegex.replace(escapeMatcher.group(1), sequence);
    }
  ...
  public void quoteDateLiteral(StringBuilder buf, String value) {
    ...
    try {
      /*
       * The format of the 'value' parameter is not certain. Some JDBC drivers
       * will return a timestamp even though we ask for a date (oracle is one of
       * them). We must try to convert both formats.
       */
