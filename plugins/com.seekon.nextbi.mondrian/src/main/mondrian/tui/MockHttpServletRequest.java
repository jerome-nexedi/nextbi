...
public class MockHttpServletRequest implements HttpServletRequest {
	...
	public BufferedReader getReader() throws IOException {
		return (bodyContent == null) ? null : new BufferedReader(new StringReader(
				bodyContent));
	}
	...
	public String getRealPath(String path) {
		...
		return (session == null) ? null : session.getServletContext().getRealPath(
				path);
	}
	 ...
	 * for example, "BASIC" or "SSL," or null if the servlet was not protected.
	 * 
	 */
	...
	public long getDateHeader(String name) {
		...
		try {
			Date dateValue = new SimpleDateFormat(DATE_FORMAT_HEADER, Locale.US)
					.parse(header);
	...
	public String getHeader(String name) {
		...
		return ((headerList == null) || (headerList.size() == 0)) ? null
				: headerList.get(0);
	}
