...
class MondrianOlap4jStatement extends StatementImpl implements OlapStatement,
	...
	ResultSet executeQuery2(String mdx, boolean advanced, String tabFields,
		...
		if (parseTree instanceof DrillThrough) {
			...
			final MondrianOlap4jCell cell = (MondrianOlap4jCell) cellSet
					.getCell(coords);
			ResultSet resultSet = cell.drillThroughInternal(
					drillThrough.getMaxRowCount(), drillThrough.getFirstRowOrdinal(),
					null, true, null, rowCountSlot);
			if (resultSet == null) {
			...
			return olap4jConnection.factory.newFixedResultSet(olap4jConnection,
					...
					Collections.singletonList(Collections.<Object> singletonList(plan)));
		} else {
	...
	protected Pair<Query, MondrianOlap4jCellSetMetaData> parseQuery(
