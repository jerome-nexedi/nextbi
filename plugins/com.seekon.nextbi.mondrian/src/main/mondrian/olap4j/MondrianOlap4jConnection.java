...
abstract class MondrianOlap4jConnection implements OlapConnection {
	 ...
	 */
	MondrianOlap4jConnection(Factory factory, MondrianOlap4jDriver driver,
		...
		st = new StringTokenizer(String.valueOf(dbpropsMap
		...
		final MondrianOlap4jDatabase database = new MondrianOlap4jDatabase(this,
				olap4jCatalogs, String.valueOf(dbpropsMap.get("DataSourceName")),
				String.valueOf(dbpropsMap.get("DataSourceDescription")),
				String.valueOf(dbpropsMap.get("ProviderName")),
				String.valueOf(dbpropsMap.get("URL")), String.valueOf(dbpropsMap
						.get("DataSourceInfo")), pTypes, aModes);
	...
	MondrianOlap4jDimension toOlap4j(mondrian.olap.Dimension dimension) {
		return new MondrianOlap4jDimension(toOlap4j(dimension.getSchema()),
				dimension);
	}
	...
	Type toOlap4j(mondrian.olap.type.Type type) {
		if (type instanceof mondrian.olap.type.BooleanType) {
			...
			return new MemberType(toOlap4j(memberType.getDimension()),
					toOlap4j(memberType.getHierarchy()), toOlap4j(memberType.getLevel()),
	...
	MondrianOlap4jMember toOlap4j(mondrian.olap.Member member) {
		...
		if (member instanceof RolapMeasure) {
			...
			return new MondrianOlap4jMeasure(toOlap4j(member.getDimension()
					.getSchema()), measure);
		}
		return new MondrianOlap4jMember(
				toOlap4j(member.getDimension().getSchema()), member);
	}
	...
	MondrianOlap4jLevel toOlap4j(mondrian.olap.Level level) {
		...
		return new MondrianOlap4jLevel(toOlap4j(level.getDimension().getSchema()),
				level);
	}
	...
	MondrianOlap4jHierarchy toOlap4j(mondrian.olap.Hierarchy hierarchy) {
		...
		return new MondrianOlap4jHierarchy(toOlap4j(hierarchy.getDimension()
				.getSchema()), hierarchy);
	}
	...
	public List<String> getAvailableRoleNames() throws OlapException {
		return UnmodifiableArrayList.of(getMondrianConnection().getSchema()
				.roleNames());
	}
	...
	private static class MondrianToOlap4jNodeConverter {
		...
		public SelectNode toOlap4j(Query query) {
			...
			return new SelectNode(null, toOlap4j(query.getFormulas()),
					toOlap4j(query.getAxes()), new CubeNode(null,
							olap4jConnection.toOlap4j(query.getCube())),
					query.getSlicerAxis() == null ? null
							: toOlap4j(query.getSlicerAxis()), list);
		}
		...
		private ParseTreeNode toOlap4j(Exp exp) {
			...
			if (exp instanceof LevelExpr) {
				...
				return new LevelNode(null, olap4jConnection.toOlap4j(levelExpr
						.getLevel()));
			}
			if (exp instanceof MemberExpr) {
				MemberExpr memberExpr = (MemberExpr) exp;
				return new MemberNode(null, olap4jConnection.toOlap4j(memberExpr
						.getMember()));
			}
		...
		private ParseTreeNode toOlap4j(ResolvedFunCall call) {
			final CallNode callNode = new CallNode(null, call.getFunName(),
					toOlap4j(call.getSyntax()), toOlap4j(Arrays.asList(call.getArgs())));
		...
		private static IdentifierNode toOlap4j(Id id) {
			...
			return new IdentifierNode(
					list.toArray(new IdentifierSegment[list.size()]));
		}
	}
