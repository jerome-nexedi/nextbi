/*
// $Id: //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jDatabaseMetaData.java#13 $
...
/**
 * Implementation of {@link org.olap4j.OlapDatabaseMetaData} for the Mondrian
 * OLAP engine.
 * 
 * <p>
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jDatabaseMetaData
 *          .java#13 $
 * @since May 23, 2007
 */
abstract class MondrianOlap4jDatabaseMetaData implements OlapDatabaseMetaData {
  ...
  private static final Comparator<Catalog> CATALOG_COMP = new Comparator<Catalog>() {
    public int compare(Catalog o1, Catalog o2) {
      return o1.getName().compareTo(o2.getName());
    }
  };

  private static final Comparator<Schema> SCHEMA_COMP = new Comparator<Schema>() {
   ...
   * 
   * <p>
   * The XMLA specification usually specifies that the returned list is ordered
   * by particular attributes. XMLA notwithstanding, the result from this method
   * is always ordered.
   * 
   * @param methodName
   *          Name of the metadata request. Corresponds to the XMLA method name,
   *          e.g. "MDSCHEMA_CUBES"
   * 
   * @param patternValues
  ...
  private ResultSet getMetadata(String methodName, Object... patternValues)
    ...
    XmlaUtil.MetadataRowset rowset = XmlaUtil.getMetadataRowset(olap4jConnection,
      methodName, restrictionMap);
    return olap4jConnection.factory.newFixedResultSet(olap4jConnection,
      rowset.headerList, rowset.rowList);
  }

  /**
   * Wraps a string in an object that indicates that it is to be treated as a
   * wildcard pattern, not a literal match.
   * 
   * @param pattern
   *          Pattern
  ...
  public ResultSet getSchemas() throws OlapException {
    ...
    for (Catalog catalog : Util.sort(olap4jConnection.getOlapCatalogs(),
      CATALOG_COMP)) {
      for (Schema schema : Util.sort(catalog.getSchemas(), SCHEMA_COMP)) {
        rowList.add(Arrays.<Object> asList(schema.getName(), catalog.getName()));
      }
    }
    return olap4jConnection.factory.newFixedResultSet(olap4jConnection, headerList,
      rowList);
  }
  ...
  public ResultSet getCatalogs() throws OlapException {
    ...
    for (Catalog catalog : Util.sort(olap4jConnection.getOlapCatalogs(),
      CATALOG_COMP)) {
      rowList.add(Collections.<Object> singletonList(catalog.getName()));
    }
    return olap4jConnection.factory.newFixedResultSet(olap4jConnection, headerList,
      rowList);
  }
  ...
  public <T> T unwrap(Class<T> iface) throws SQLException {
    ...
    throw olap4jConnection.helper.createException("does not implement '" + iface
      + "'");
  }
  ...
  public ResultSet getActions(String catalog, String schemaPattern,
    ...
    return getMetadata("MDSCHEMA_ACTIONS", "SCHEMA_NAME", wildcard(schemaPattern),
      "CUBE_NAME", wildcard(cubeNamePattern), "ACTION_NAME",
  ...
  public ResultSet getProperties(String catalog, String schemaPattern,
    String cubeNamePattern, String dimensionUniqueName, String hierarchyUniqueName,
    String levelUniqueName, String memberUniqueName, String propertyNamePattern)
    throws OlapException {
    return getMetadata("MDSCHEMA_PROPERTIES", "CATALOG_NAME", catalog,
      "SCHEMA_NAME", wildcard(schemaPattern), "CUBE_NAME",
      wildcard(cubeNamePattern), "DIMENSION_UNIQUE_NAME", dimensionUniqueName,
      "HIERARCHY_UNIQUE_NAME", hierarchyUniqueName, "LEVEL_UNIQUE_NAME",
      levelUniqueName, "MEMBER_UNIQUE_NAME", memberUniqueName, "PROPERTY_NAME",
  ...
  public ResultSet getCubes(String catalog, String schemaPattern,
    ...
    return getMetadata("MDSCHEMA_CUBES", "CATALOG_NAME", catalog, "SCHEMA_NAME",
  ...
  public ResultSet getDimensions(String catalog, String schemaPattern,
    String cubeNamePattern, String dimensionNamePattern) throws OlapException {
    return getMetadata("MDSCHEMA_DIMENSIONS", "SCHEMA_NAME",
      wildcard(schemaPattern), "CUBE_NAME", wildcard(cubeNamePattern),
  ...
  public ResultSet getOlapFunctions(String functionNamePattern) throws OlapException {
    return getMetadata("MDSCHEMA_FUNCTIONS", "FUNCTION_NAME",
      wildcard(functionNamePattern));
  }

  public ResultSet getHierarchies(String catalog, String schemaPattern,
    String cubeNamePattern, String dimensionUniqueName, String hierarchyNamePattern)
    throws OlapException {
    return getMetadata("MDSCHEMA_HIERARCHIES", "CATALOG_NAME", catalog,
      "SCHEMA_NAME", wildcard(schemaPattern), "CUBE_NAME",
      wildcard(cubeNamePattern), "DIMENSION_UNIQUE_NAME", dimensionUniqueName,
  ...
  public ResultSet getMeasures(String catalog, String schemaPattern,
    String cubeNamePattern, String measureNamePattern, String measureUniqueName)
    throws OlapException {
    return getMetadata("MDSCHEMA_MEASURES", "CATALOG_NAME", catalog, "SCHEMA_NAME",
      wildcard(schemaPattern), "CUBE_NAME", wildcard(cubeNamePattern),
      "MEASURE_NAME", wildcard(measureNamePattern), "MEASURE_UNIQUE_NAME",
      measureUniqueName);
  }

  public ResultSet getMembers(String catalog, String schemaPattern,
    String cubeNamePattern, String dimensionUniqueName, String hierarchyUniqueName,
    String levelUniqueName, String memberUniqueName, Set<Member.TreeOp> treeOps)
    throws OlapException {
    String treeOpString;
    if (treeOps != null) {
      final int mask = Member.TreeOp.getDictionary().toMask(treeOps);
      treeOpString = String.valueOf(mask);
    } else {
      treeOpString = null;
    }
    return getMetadata("MDSCHEMA_MEMBERS", "CATALOG_NAME", catalog, "SCHEMA_NAME",
      wildcard(schemaPattern), "CUBE_NAME", wildcard(cubeNamePattern),
      "DIMENSION_UNIQUE_NAME", dimensionUniqueName, "HIERARCHY_UNIQUE_NAME",
  ...
  public ResultSet getLevels(String catalog, String schemaPattern,
    ...
    return getMetadata("MDSCHEMA_LEVELS", "CATALOG_NAME", catalog, "SCHEMA_NAME",
      wildcard(schemaPattern), "CUBE_NAME", wildcard(cubeNamePattern),
      "DIMENSION_UNIQUE_NAME", dimensionUniqueName, "HIERARCHY_UNIQUE_NAME",
  ...
  public ResultSet getSets(String catalog, String schemaPattern,
    ...
    return getMetadata("MDSCHEMA_SETS", "CATALOG_NAME", catalog, "SCHEMA_NAME",
      wildcard(schemaPattern), "CUBE_NAME", wildcard(cubeNamePattern), "SET_NAME",
      wildcard(setNamePattern));
  }
}
