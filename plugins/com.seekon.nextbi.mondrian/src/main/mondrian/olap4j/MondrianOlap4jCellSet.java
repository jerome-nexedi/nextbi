...
abstract class MondrianOlap4jCellSet extends Execution implements CellSet {
	...
	void execute() throws OlapException {
		result = olap4jStatement.olap4jConnection.getMondrianConnection().execute(
				this);
		...
		for (int i = 0; i < axes.length; i++) {
			...
			axisList.add(new MondrianOlap4jCellSetAxis(this, queryAxis,
					(RolapAxis) axis));
		}
		...
		if (queryAxis == null) {
			// Dummy slicer axis.
			queryAxis = new QueryAxis(false, null,
					AxisOrdinal.StandardAxisOrdinal.SLICER,
					QueryAxis.SubtotalVisibility.Undefined);
		}
		filterAxis = new MondrianOlap4jCellSetAxis(this, queryAxis,
				(RolapAxis) axis);
	}
		...
		try {
			...
			if (e.getMessage().indexOf("coordinates out of range") >= 0) {
				...
				throw new IllegalArgumentException(
						"Cell coordinates should have dimension " + axisList.size());
			} else {
				throw e;
			}
		}
		...
		for (CellSetAxis axis : axes) {
			...
			if (coordinate < 0 || coordinate >= axis.getPositionCount()) {
				throw new IndexOutOfBoundsException("Coordinate " + coordinate
						+ " of axis " + k + " is out of range (" + getBoundsAsString()
						+ ")");
			}
