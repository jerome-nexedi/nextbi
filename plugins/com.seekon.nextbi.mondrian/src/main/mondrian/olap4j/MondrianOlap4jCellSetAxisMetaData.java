...
/**
 * Implementation of {@link org.olap4j.CellSetMetaData} for the Mondrian OLAP
 * engine.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jCellSetAxisMetaData
 *          .java#11 $
 * @since Nov 17, 2007
 */
class MondrianOlap4jCellSetAxisMetaData implements CellSetAxisMetaData {
   ...
   *          Cell set axis metadata
   * @param queryAxis
   *          Query axis
   */
  MondrianOlap4jCellSetAxisMetaData(MondrianOlap4jCellSetMetaData cellSetMetaData,
    QueryAxis queryAxis) {
    if (queryAxis == null) {
    ...
    for (Id id : queryAxis.getDimensionProperties()) {
      ...
      if (names.length == 1) {
        olap4jProperty = Util
          .lookup(Property.StandardMemberProperty.class, names[0]);
      }
      if (olap4jProperty == null) {
        final UnresolvedFunCall call = (UnresolvedFunCall) Util.lookup(
          cellSetMetaData.query, id.getSegments(), true);
        olap4jProperty = new MondrianOlap4jProperty(Util.lookupProperty(
          ((LevelExpr) call.getArg(0)).getLevel(), call.getFunName()));
      }
  ...
  private List<Hierarchy> getHierarchiesNonFilter() {
    ...
    for (mondrian.olap.Hierarchy hierarchy : TypeUtil.getHierarchies(exp.getType())) {
      hierarchyList.add(cellSetMetaData.olap4jStatement.olap4jConnection
        .toOlap4j(hierarchy));
    }
