 ...
 * 
 * <p>
 * The public wrapper for this parser is {@link JavaccParserValidatorImpl}.
 * 
 * @author jhyde
 * @since Dec 14, 2010
 * @version $Id: //open/mondrian/src/main/mondrian/parser/MdxParser.jj#3 $
 */
@SuppressWarnings({ "ConstantIfStatement", "UnnecessarySemicolon",
		"UnnecessaryLabelOnBreakStatement", "RedundantIfStatement" })
public class MdxParserImpl implements MdxParserImplConstants {
	...
	Exp recursivelyParseExp(String s) throws ParseException {
		MdxParserImpl parser = new MdxParserImpl(factory, statement, s, false,
				funTable, strictValidation);
	...
	private static String stripQuotes(String s, String prefix, String suffix,
			String quoted) {
	...
	private Exp createCall(Exp left, Id.Segment segment, List<Exp> argList) {
		if (argList != null) {
			if (left != null) {
				...
				argList.add(0, left);
				return new UnresolvedFunCall(segment.name, Syntax.Method,
						toExpArray(argList));
			} else {
				// Function syntax: "foo(arg1, arg2)" or "foo()"
				return new UnresolvedFunCall(segment.name, Syntax.Function,
						toExpArray(argList));
			}
		} else {
			...
			case UNQUOTED:
				...
				call = funTable.isProperty(segment.name);
				break;
			case QUOTED:
			...
			if (left instanceof Id && !call) {
				...
				return new UnresolvedFunCall(segment.name, syntax, new Exp[] { left });
			}
		}
	}
	...
	final public Id.Segment identifier() throws ParseException {
		...
		case PROPERTIES:
			...
			{
				if (true)
					return new Id.Segment(id, Id.Quoting.UNQUOTED);
			}
			break;
		case ID:
			jj_consume_token(ID);
			{
				if (true)
					return new Id.Segment(token.image, Id.Quoting.UNQUOTED);
			}
			break;
		case QUOTED_ID:
			...
			{
				if (true)
					return new Id.Segment(stripQuotes(token.image, "[", "]", "]]"),
							Id.Quoting.QUOTED);
			}
			break;
		case AMP_QUOTED_ID:
			...
			default:
				...
				if (jj_2_2(2)) {
					...
					case IS:
						...
						x = new UnresolvedFunCall("IS EMPTY", Syntax.Postfix,
								new Exp[] { x });
						break;
					case MATCHES:
						jj_consume_token(MATCHES);
						y = term2();
						x = new UnresolvedFunCall("MATCHES", Syntax.Infix,
								new Exp[] { x, y });
						break;
					default:
						jj_la1[9] = jj_gen;
						if (jj_2_4(2)) {
							...
							jj_consume_token(MATCHES);
							y = term2();
							x = new UnresolvedFunCall("NOT", Syntax.Prefix,
							...
							case IN:
								jj_consume_token(IN);
								y = term2();
								x = new UnresolvedFunCall("IN", Syntax.Infix,
										new Exp[] { x, y });
								break;
							case NOT:
								jj_consume_token(NOT);
	...
	final public Exp atom() throws ParseException {
		...
		case CAST:
			...
			{
				if (true)
					return new UnresolvedFunCall("CAST", Syntax.Cast, new Exp[] { e,
		...
		case LPAREN:
			...
			{
				if (true)
					return new UnresolvedFunCall("()", Syntax.Parentheses,
							toExpArray(lis));
			}
			break;
		case LBRACE:
	...
	final public Exp caseExpression() throws ParseException {
		...
		if (match) {
			{
				if (true)
					return new UnresolvedFunCall("_CaseMatch", Syntax.Case,
							toExpArray(list));
			}
		} else {
			{
				if (true)
					return new UnresolvedFunCall("_CaseTest", Syntax.Case,
							toExpArray(list));
			}
		}
		throw new Error("Missing return statement in function");
	...
	final public Query selectStatement() throws ParseException {
		...
		case PROPERTIES:
			...
			jj_consume_token(PROPERTIES);
			p = compoundId();
			cellPropList.add(new CellProperty(p));
			label_15: while (true) {
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case COMMA:
					;
		...
		{
			if (true)
				return factory.makeQuery(statement, toFormulaArray(f),
						toQueryAxisArray(a), cubeName, w, toQueryPartArray(cellPropList),
						strictValidation);
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public QueryAxis axisSpecification() throws ParseException {
		...
		case DECIMAL_NUMERIC_LITERAL:
			...
			if (number.doubleValue() < 0 || number.doubleValue() != number.intValue()) {
				{
					if (true)
						throw MondrianResource.instance().InvalidAxis.ex(number
								.doubleValue());
				}
			}

			axis = AxisOrdinal.StandardAxisOrdinal.forLogicalOrdinal(number
					.intValue());
			break;
		default:
			jj_la1[48] = jj_gen;
		...
		{
			if (true)
				return new QueryAxis(nonEmpty, e, axis,
						QueryAxis.SubtotalVisibility.Undefined, toIdArray(dp));
		}
		throw new Error("Missing return statement in function");
	}
	...
	private boolean jj_3R_44() {
		if (jj_scan_token(AMP_QUOTED_ID))
			return true;
		return false;
	}
	...
	private boolean jj_3R_27() {
		if (jj_scan_token(DOUBLE_QUOTED_STRING))
			return true;
		return false;
	}

	private boolean jj_3R_26() {
		if (jj_scan_token(SINGLE_QUOTED_STRING))
			return true;
		return false;
	}
	...
	private boolean jj_3R_40() {
		if (jj_scan_token(APPROX_NUMERIC_LITERAL))
			return true;
		return false;
	}

	private boolean jj_3R_39() {
		if (jj_scan_token(UNSIGNED_INTEGER_LITERAL))
			return true;
		return false;
	}
	...
	private boolean jj_3R_38() {
		if (jj_scan_token(DECIMAL_NUMERIC_LITERAL))
			return true;
		return false;
	}
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x40000200, 0x40000200, 0x8000000, 0x8000000, 0x2,
				0x43000230, 0x11c0000, 0x0, 0x0, 0x180000, 0x1040000, 0x0, 0x0, 0x0,
				0x0, 0x42000230, 0x0, 0x0, 0x0, 0x43000230, 0x0, 0x0, 0x42000230,
				0x43000230, 0x0, 0x800, 0x4, 0x43000230, 0x0, 0x0, 0x4400, 0x400,
				0x400000, 0x400000, 0x0, 0x0, 0x43800230, 0x0, 0x40, 0x0, 0x40000040,
				0x0, 0x43000230, 0x800000, 0x200, 0x0, 0x40000200, 0x8, 0x10000188,
				0x200000, 0x8000, 0x80000000, 0x400, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x1000080, 0x1000080, 0x0, 0x80000000,
				0x70000000, 0x70000000, 0x70000000, 0x0, 0x0, 0x4000000, 0x4000000,
				0x400000, 0x400000, 0x80000000, 0x8000000, 0x0, 0x0, 0x80000000,
				0x800000, 0x0, 0x80000000, 0x80000000, 0x20, 0x0, 0x0, 0x80000000,
				0x2000000, 0x2000000, 0x104, 0x104, 0x8, 0x8, 0x100, 0x2000000,
				0x80000000, 0x40, 0x0, 0x2000000, 0x0, 0x2000000, 0x80000000, 0x0, 0x0,
				0x2000000, 0x0, 0x0, 0x3, 0x0, 0x0, 0x0, 0x104, };
	}

	private static void jj_la1_init_2() {
		jj_la1_2 = new int[] { 0x70000, 0x0, 0x0, 0x0, 0x0, 0x76e2a, 0x15, 0x15,
				0x15, 0x0, 0x0, 0x28, 0x28, 0x100, 0x100, 0x76e2a, 0x0, 0x2, 0xe00,
				0x76e2a, 0x0, 0x2, 0x76e02, 0x76e2a, 0x0, 0x0, 0x0, 0x76e2a, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x76e2a, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x76e2a, 0x0, 0x0, 0x0, 0x0, 0xe00, 0xe00, 0x0, 0x0, 0x0, 0x0, };
	}
