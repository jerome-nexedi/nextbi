/*
// $Id: //open/mondrian/src/main/mondrian/parser/JavaccParserValidatorImpl.java#3 $
...
/**
 * Default implementation of {@link MdxParserValidator}, using the <a
 * href="http://java.net/projects/javacc/">JavaCC</a> parser generator.
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/parser/JavaccParserValidatorImpl
 *          .java#3 $
 * @author jhyde
 */
public class JavaccParserValidatorImpl implements MdxParserValidator {
	...
	/**
	 * Creates a JavaccParserValidatorImpl with an explicit factory for parse tree
	 * nodes.
	 * 
	 * @param factory
	...
	public QueryPart parseInternal(Statement statement, String queryString,
		...
		final MdxParserImpl mdxParser = new MdxParserImpl(factory, statement,
	...
	public Exp parseExpression(Statement statement, String queryString,
		...
		final MdxParserImpl mdxParser = new MdxParserImpl(factory, statement,
				queryString, debug, funTable, false);
		try {
	 ...
	 *          MDX query string
	 * @param pe
	 *          JavaCC parse exception
	 * @return Wrapped exception
	 */
	private RuntimeException convertException(String queryString,
			ParseException pe) {
		Exception e;
		if (pe.getMessage().startsWith("Encountered ")) {
			e = new MondrianException("Syntax error at line "
					+ pe.currentToken.next.beginLine + ", column "
					+ pe.currentToken.next.beginColumn + ", token '"
