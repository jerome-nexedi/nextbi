/*
// $Id: //open/mondrian/src/main/mondrian/olap/Scanner.java#34 $
...
public class Scanner {
   ...
   */
  private static final String[][] commentDelim = { new String[] { "//", null },
  ...
  /**
   * The {@link java.math.BigDecimal} value 0. Note that BigDecimal.ZERO does
   * not exist until JDK 1.5.
   */
   ...
   * 
   * <p>
   * lookahead(0) returns the current char (nextChar). lookahead(1) returns the
   * next char (was lookaheadChar, same as lookahead());
   */
  ...
  /**
   * Deduces the line and column (0-based) of a symbol. Called by
   * {@link Parser#syntax_error}.
   */
  ...
  private Symbol trace(Symbol s) {
    if (debug) {
      ...
      LOGGER.error("Scanner returns #" + s.sym + (name == null ? "" : ":" + name)
  ...
  private Symbol makeId(String s, boolean quoted, boolean ampersand) {
    return makeSymbol(quoted && ampersand ? ParserSym.AMP_QUOTED_ID
      : quoted ? ParserSym.QUOTED_ID : ParserSym.ID, s);
  }
  ...
  /**
   * Eats a delimited comment. The type of delimiters are kept in commentDelim.
   * The current comment type is indicated by commentType. end of file
   * terminates a comment without error.
   */
  private void skipComment(final String startDelim, final String endDelim)
    throws IOException {
    int depth = 1;
