/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/OpeningClosingPeriodFunDef.java#16 $
 ...
 * @since 2005/8/14
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/OpeningClosingPeriodFunDef
 *          .java#16 $
 */
class OpeningClosingPeriodFunDef extends FunDefBase {
  private final boolean opening;

  static final Resolver OpeningPeriodResolver = new MultiResolver("OpeningPeriod",
    "OpeningPeriod([<Level>[, <Member>]])",
    "Returns the first descendant of a member at a level.", new String[] { "fm",
      "fml", "fmlm" }) {
  ...
  static final Resolver ClosingPeriodResolver = new MultiResolver("ClosingPeriod",
    "ClosingPeriod([<Level>[, <Member>]])",
    "Returns the last descendant of a member at a level.", new String[] { "fm",
      "fml", "fmlm", "fmm" }) {
    protected FunDef createFunDef(Exp[] args, FunDef dummyFunDef) {
  ...
  public Type getResultType(Validator validator, Exp[] args) {
    if (args.length == 0) {
      ...
      // dimension.
      RolapHierarchy defaultTimeHierarchy = ((RolapCube) validator.getQuery()
        .getCube()).getTimeHierarchy(getName());
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    switch (args.length) {
    case 0:
      defaultTimeHierarchy = ((RolapCube) compiler.getEvaluator().getCube())
        .getTimeHierarchy(getName());
      memberCalc = new HierarchyCurrentMemberFunDef.FixedCalcImpl(new DummyExp(
        MemberType.forHierarchy(defaultTimeHierarchy)), defaultTimeHierarchy);
      levelCalc = null;
      break;
    case 1:
      defaultTimeHierarchy = ((RolapCube) compiler.getEvaluator().getCube())
    ...
    if (levelCalc != null) {
      ...
      if (!memberDimension.equals(levelDimension)) {
        throw MondrianResource.instance().FunctionMbrAndLevelHierarchyMismatch.ex(
          opening ? "OpeningPeriod" : "ClosingPeriod", levelDimension
            .getUniqueName(), memberDimension.getUniqueName());
      }
    }
   ...
   * 
   * @param schemaReader
   *          The schema reader to use to evaluate the function.
   * @param member
   *          The member from which the descendant is to be found.
   * @param targetLevel
   *          The level to stop at.
   * @param returnFirstDescendant
   *          Flag indicating whether to return the first or last descendant of
   *          the member.
   * @return A member.
   * @pre member.getLevel().getDepth() < level.getDepth();
   */
  static Member getDescendant(SchemaReader schemaReader, Member member,
    Level targetLevel, boolean returnFirstDescendant) {
