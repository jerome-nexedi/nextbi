/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/TopBottomCountFunDef.java#21 $
...
/**
 * Definition of the <code>TopCount</code> and <code>BottomCount</code> MDX
 * builtin functions.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/TopBottomCountFunDef.
 *          java#21 $
 * @since Mar 23, 2006
 */
class TopBottomCountFunDef extends FunDefBase {
	...
	public Calc compileCall(final ResolvedFunCall call, ExpCompiler compiler) {
		...
		final Calc orderCalc = call.getArgCount() > 2 ? compiler.compileScalar(
				call.getArg(2), true) : null;
		...
		return new AbstractListCalc(call, new Calc[] { listCalc, integerCalc,
			...
			public TupleList evaluateList(Evaluator evaluator) {
				...
				return partiallySortList(evaluator, list, hasHighCardDimension(list), n);
			}

			private TupleList partiallySortList(Evaluator evaluator, TupleList list,
					boolean highCard, int n) {
				if (highCard) {
					...
					for (int i = 0, next; i < list.size(); i = next) {
						...
						TupleList chunkResult = partiallySortList(evaluator, chunk, false,
								n);
				...
				case 1:
					final List<Member> members = partiallySortMembers(evaluator.push(),
							list.slice(0), orderCalc, n, top);
					evaluator.restore(savepoint);
					return new UnaryTupleList(members);
				default:
					final List<List<Member>> tuples = partiallySortTuples(
							evaluator.push(), list, orderCalc, n, top);
					evaluator.restore(savepoint);
