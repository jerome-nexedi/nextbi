...
public class NativizeSetFunDef extends FunDefBase {
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    return new NativeListCalc(call, calcs, compiler, substitutionMap, originalExp);
  }
  ...
  private boolean isHighCardinality(Exp funArg, Evaluator evaluator) {
    ...
    if (level != null) {
      ...
      logHighCardinality(ESTIMATE_MESSAGE, minThreshold, cardinality, isHighCard);
      return isHighCard;
    }
    return false;
  }
  ...
  private static void logHighCardinality(final String estimateMessage,
    long nativizeMinThreshold, long estimatedCardinality, boolean highCardinality) {
  ...
  public static class NativeListCalc extends AbstractListCalc {
    ...
    private TupleList evaluateNonNative(Evaluator evaluator) {
      ...
      ListCalc calc = compiler.compileList(getOriginalExp(evaluator.getQuery()));
      final int savepoint = evaluator.savepoint();
    ...
    private boolean isHighCardinality(Evaluator evaluator, TupleList simplifiedList) {
      ...
      for (Member member : tuple) {
        ...
        if (memberName.startsWith(MEMBER_NAME_PREFIX)) {
          ...
          Level hierarchyLevel = Util.lookupHierarchyLevel(hierarchy, levelName);
    ...
    private TupleList evaluateJoinExpression(Evaluator evaluator,
      ...
      Exp unresolved = evaluator.getQuery().getConnection().parseExpression(
        crossJoinExpression);
  ...
  static class AddFormulasVisitor extends MdxVisitorImpl {
    ...
    private Formula createSentinelFormula(Level level) {
      ...
      LOGGER.debug("createSentinelFormula memberId=" + memberId + " memberExpr="
        + memberExpr);
    ...
    private Formula createNamedSetFormula(Level level, Formula memberFormula) {
      ...
      LOGGER.debug("createNamedSetFormula setId=" + setId + " setExpr=" + setExpr);
  ...
  static class TransformToFormulasVisitor extends MdxVisitorImpl {
    ...
    private Object flattenSetFunDef(ResolvedFunCall call) {
      ...
      if (newArgs.size() != call.getArgCount()) {
        return new ResolvedFunCall(call.getFunDef(), newArgs.toArray(new Exp[newArgs
          .size()]), call.getType());
      }
      return null;
    }
  ...
  public static class CrossJoinAnalyzer {
    ...
    public CrossJoinAnalyzer(TupleList simplifiedList,
      ...
      resultLimit = nativizeMaxResults <= 0 ? Integer.MAX_VALUE : (int) Math.min(
        nativizeMaxResults, Integer.MAX_VALUE);
    ...
    private ReassemblyCommand zz(SubstitutionMap substitutionMap, Member mbr) {
      ...
      if (substitutionMap.contains(mbr)) {
        ...
        NativeElementType nativeType = !isNativeCompatible(mbr) ? NON_NATIVE : mbr
          .getMemberType() == Member.MemberType.REGULAR ? ENUMERATED_VALUE
          : OTHER_NATIVE;
    ...
    private String formatCrossJoin(List<Collection<String>> memberLists) {
      ...
      if (left.length() == 0) {
        ...
        if (right.length() == 0) {
          ...
          buf.append("CrossJoin(").append("{").append(left).append("},").append(
            right).append(")");
        }
      }
    ...
    public TupleList adaptList(final TupleList sourceList, final int destSize,
      ...
      if (LOGGER.isDebugEnabled()) {
        ...
        LOGGER.debug(String.format(
          "returning native %s<%s> without copying to new list.", sourceListType,
          sourceElementType));
      }
      return destList;
    }
  ...
  public static class RangeIterator implements Iterator<Range> {
    ...
    private Range next(int cursor) {
      return (cursor >= parent.to) ? null : parent.subRangeStartingAt(cursor, col);
    }
  ...
  private static class ReassemblyGuide {
    ...
    private ReassemblyCommand currentCommand(List<ReassemblyCommand> commandTuple) {
      ...
      ReassemblyCommand prev = commands.isEmpty() ? null : commands.get(commands
        .size() - 1);
  ...
  private static String createMangledName(Level level, String prefix) {
    return prefix
      + level.getUniqueName().replaceAll("[\\[\\]]", "").replaceAll("\\.", "_")
      + "_";
  }

  private static void dumpListToLog(String heading, TupleList list) {
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(String.format("%s created with %,d rows.", heading, list.size()));
