/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/NativizeSetFunDef.java#25 $
 ...
 * @author jrand
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/NativizeSetFunDef.java#25
 *          $
 * @since Oct 14, 2009
 */
public class NativizeSetFunDef extends FunDefBase {
	...
	protected static final Logger LOGGER = Logger
			.getLogger(NativizeSetFunDef.class);
	...
	private static final List<Class<? extends FunDef>> functionWhitelist = Arrays
			.<Class<? extends FunDef>> asList(CacheFunDef.class, SetFunDef.class,
					CrossJoinFunDef.class, NativizeSetFunDef.class);

	static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
			"NativizeSet", "NativizeSet(<Set>)", "Tries to natively evaluate <Set>.",
			new String[] { "fxx" }, NativizeSetFunDef.class);
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		if (arity == 1 || substitutionMap.isEmpty()) {
			...
			final boolean highCardinality = arity == 1
		...
		if (isFirstCompileCall) {
			...
			call.accept(new AddFormulasVisitor(query, substitutionMap, dimensions));
			call.accept(new TransformToFormulasVisitor(query));
			query.resolve();
		}
		return new NativeListCalc(call, calcs, compiler, substitutionMap,
				originalExp);
	}
	...
	private boolean isHighCardinality(Exp funArg, Evaluator evaluator) {
		...
		if (level != null) {
			int cardinality = evaluator.getSchemaReader().getLevelCardinality(level,
					false, true);
			final int minThreshold = MondrianProperties.instance().NativizeMinThreshold
					.get();
			final boolean isHighCard = cardinality > minThreshold;
			logHighCardinality(ESTIMATE_MESSAGE, minThreshold, cardinality,
					isHighCard);
			return isHighCard;
		}
		return false;
	...
	private static void logHighCardinality(final String estimateMessage,
			long nativizeMinThreshold, long estimatedCardinality,
			boolean highCardinality) {
		LOGGER.debug(String.format(estimateMessage, highCardinality,
	...
	static class NonNativeCalc implements Calc {
		 ...
		 * {@inheritDoc}
		 * 
		 * Default implementation just casts to TargetClass. Subtypes that are
		 * wrappers should override.
		 */
	...
	public static class NativeListCalc extends AbstractListCalc {
		...
		protected NativeListCalc(ResolvedFunCall call, Calc[] calcs,
				ExpCompiler compiler, SubstitutionMap substitutionMap, Exp originalExp) {
			super(call, calcs);
		...
		private TupleList evaluateSimplifiedList(Evaluator evaluator) {
			...
			TupleList simplifiedList = simpleCalc.evaluateList(evaluator);
		...
		private TupleList evaluateNonNative(Evaluator evaluator) {
			LOGGER.debug("Disabling native evaluation. originalExp=" + originalExp);
			ListCalc calc = compiler
					.compileList(getOriginalExp(evaluator.getQuery()));
			final int savepoint = evaluator.savepoint();
		...
		private TupleList evaluateNative(Evaluator evaluator,
				TupleList simplifiedList) {
			CrossJoinAnalyzer analyzer = new CrossJoinAnalyzer(simplifiedList,
					substitutionMap);
			...
			LOGGER.debug("crossjoin reconstituted from simplified list: "
					+ String.format("%n" + crossJoin.replaceAll(",", "%n, ")));
			final int savepoint = evaluator.savepoint();
			evaluator.setNonEmpty(true);
			evaluator.setNativeEnabled(true);

			TupleList members = analyzer.mergeCalcMembers(evaluateJoinExpression(
					evaluator, crossJoin));
			evaluator.restore(savepoint);
			return members;
		}
		...
		private Exp getOriginalExp(final Query query) {
			originalExp.accept(new TransformFromFormulasVisitor(query, compiler));
		...
		private boolean isHighCardinality(Evaluator evaluator,
				TupleList simplifiedList) {
			...
			long nativizeMinThreshold = MondrianProperties.instance().NativizeMinThreshold
					.get();
			...
			for (Member member : tuple) {
				...
				if (memberName.startsWith(MEMBER_NAME_PREFIX)) {
					...
					Level hierarchyLevel = Util
							.lookupHierarchyLevel(hierarchy, levelName);
					long levelCardinality = getLevelCardinality(schema, hierarchyLevel);
					estimatedCardinality *= levelCardinality;
					if (estimatedCardinality >= nativizeMinThreshold) {
						logHighCardinality(PARTIAL_ESTIMATE_MESSAGE, nativizeMinThreshold,
								estimatedCardinality, true);
						return true;
					}
				}
			}
			...
			logHighCardinality(ESTIMATE_MESSAGE, nativizeMinThreshold,
		...
		private TupleList evaluateJoinExpression(Evaluator evaluator,
				String crossJoinExpression) {
	...
	static class FindLevelsVisitor extends MdxVisitorImpl {
		...
		public FindLevelsVisitor(SubstitutionMap substitutionMap,
	...
	static class AddFormulasVisitor extends MdxVisitorImpl {
		...
		public AddFormulasVisitor(Query query, SubstitutionMap substitutionMap,
		...
		private Formula createSentinelFormula(Level level) {
			...
			Exp memberExpr = query.getConnection().parseExpression("101010");

			LOGGER.debug("createSentinelFormula memberId=" + memberId
		...
		private Formula createDefaultMemberFormula(Level level) {
			...
			Exp memberExpr = new UnresolvedFunCall("DEFAULTMEMBER", Syntax.Property,
					new Exp[] { hierarchyId(level) });

			LOGGER.debug("createLevelMembersFormulas memberId=" + memberId
					+ " memberExpr=" + memberExpr);
			return new Formula(memberId, memberExpr, new MemberProperty[0]);
		}
		...
		private Formula createNamedSetFormula(Level level, Formula memberFormula) {
			...
			Exp setExpr = query.getConnection().parseExpression(
					"{" + memberFormula.getIdentifier().toString() + "}");

			LOGGER.debug("createNamedSetFormula setId=" + setId + " setExpr="
					+ setExpr);
	...
	static class TransformToFormulasVisitor extends MdxVisitorImpl {
		...
		private Object flattenSetFunDef(ResolvedFunCall call) {
			...
			if (newArgs.size() != call.getArgCount()) {
				return new ResolvedFunCall(call.getFunDef(),
						newArgs.toArray(new Exp[newArgs.size()]), call.getType());
			}
			return null;
		}
			...
			for (int i = 1; i < args.size(); i++) {
				...
				if (prev.toString().equals(curr.toString())) {
					...
					if (element != null) {
						...
						Id memberId = createSentinelId(level);
						Formula formula = query.findFormula(memberId.toString());
	...
	static class TransformFromFormulasVisitor extends MdxVisitorImpl {
		...
		public Object visit(NamedSetExpr namedSetExpr) {
			...
			if (exprName.contains(SET_NAME_PREFIX)) {
				String levelMembers = exprName.replaceAll(SET_NAME_PREFIX, "\\[")
						.replaceAll("_$", "\\]").replaceAll("_", "\\]\\.\\[")
						+ ".members";
				membersExpr = query.getConnection().parseExpression(levelMembers);
				membersExpr = compiler.getValidator().validate(membersExpr, false);
			} else {
	...
	public static class CrossJoinAnalyzer {
		...
		public CrossJoinAnalyzer(TupleList simplifiedList,
			...
			long nativizeMaxResults = MondrianProperties.instance().NativizeMaxResults
					.get();
			arity = simplifiedList.getArity();
			tempTuple = new Member[arity];
			tempTupleAsList = Arrays.asList(tempTuple);
			resultLimit = nativizeMaxResults <= 0 ? Integer.MAX_VALUE : (int) Math
					.min(nativizeMaxResults, Integer.MAX_VALUE);
		...
		public ReassemblyGuide classifyMembers(TupleList simplifiedList,
				SubstitutionMap substitutionMap) {
			ReassemblyGuide guide = new ReassemblyGuide(0);

			List<ReassemblyCommand> cmdTuple = new ArrayList<ReassemblyCommand>(arity);
		...
		private ReassemblyCommand zz(SubstitutionMap substitutionMap, Member mbr) {
			...
			if (substitutionMap.contains(mbr)) {
				c = new ReassemblyCommand(substitutionMap.get(mbr), LEVEL_MEMBERS);
			} else if (mbr.getName().startsWith(SENTINEL_PREFIX)) {
				c = new ReassemblyCommand(mbr, SENTINEL);
			} else {
				NativeElementType nativeType = !isNativeCompatible(mbr) ? NON_NATIVE
						: mbr.getMemberType() == Member.MemberType.REGULAR ? ENUMERATED_VALUE
								: OTHER_NATIVE;
		...
		private List<Collection<String>> findNativeMembers() {
			List<Collection<String>> nativeMembers = new ArrayList<Collection<String>>(
					arity);
		...
		private void findNativeMembers(ReassemblyGuide guide,
				List<Collection<String>> nativeMembers) {
			List<ReassemblyCommand> commands = guide.getCommands();
			Set<NativeElementType> typesToAdd = ReassemblyCommand
					.getMemberTypes(commands);
		...
		private boolean isNativeCompatible(Member member) {
			return member.isParentChildLeaf()
					|| (!member.isMeasure() && !member.isCalculated() && !member.isAll());
		}
		...
		private String formatCrossJoin(List<Collection<String>> memberLists) {
			...
			String right = memberLists.size() == 1 ? "" : formatCrossJoin(memberLists
					.subList(1, memberLists.size()));
			...
			if (left.length() == 0) {
				...
				if (right.length() == 0) {
					...
					buf.append("CrossJoin(").append("{").append(left).append("},")
							.append(right).append(")");
				}
			}
		...
		private TupleList mergeCalcMembers(TupleList nativeValues) {
			TupleList nativeList = adaptList(nativeValues, arity, nativeIndices);
		...
		private void mergeCalcMembers(ReassemblyGuide guide, Range range,
			...
			for (ReassemblyCommand command : guide.getCommands()) {
				...
				case NON_NATIVE:
					...
					mergeCalcMembers(nextGuide, range,
							(history == null ? new HashSet<List<Member>>() : history));
					break;
		...
		private void appendMembers(ReassemblyGuide guide, Range range,
			...
			for (ReassemblyCommand command : guide.getCommands()) {
				...
				case ENUMERATED_VALUE:
					...
					Range valueRange = range.subRangeForValue(value, col);
					if (!valueRange.isEmpty()) {
						appendTuple(valueRange.getTuple(), tempTupleAsList, history);
					}
					break;
				case LEVEL_MEMBERS:
				case OTHER_NATIVE:
		...
		private List<Member> copyOfTuple(List<Member> natives,
				List<Member> nonNatives) {
			Member[] copy = new Member[arity];
			for (int i = 0; i < arity; i++) {
				copy[i] = (nonNatives.get(i) == null) ? natives.get(i) : nonNatives
						.get(i);
			}
		 ...
		 * <p/>
		 * It didn't seem appropriate to use the existing Mondrian ResultLimit
		 * property, since the meaning and use of that property seems to be a bit
		 * ambiguous, otherwise we could simply call Util.checkCJResultLimit.
		 * 
		 * @param resultSize
		 *          Result limit
		...
		public TupleList adaptList(final TupleList sourceList, final int destSize,
			...
			TupleList destList = new DelegatingTupleList(destSize,
					new AbstractList<List<Member>>() {
						@Override
						public List<Member> get(int index) {
							final List<Member> sourceTuple = sourceList.get(index);
			...
			if (LOGGER.isDebugEnabled()) {
				...
				LOGGER.debug(String.format(
						...
						sourceListType, sourceElementType));
			}
			return destList;
		}
	}
	...
	public static class RangeIterator implements Iterator<Range> {
		...
		private Range next(int cursor) {
			return (cursor >= parent.to) ? null : parent.subRangeStartingAt(cursor,
					col);
		}
	...
	private static class ReassemblyGuide {
		...
		private ReassemblyCommand currentCommand(
			...
			ReassemblyCommand prev = commands.isEmpty() ? null : commands
					.get(commands.size() - 1);
		...
		public String toString() {
			return "" + index + ":"
					+ commands.toString().replaceAll("=null", "").replaceAll("=", " ")
					+ " ";
		}
	}
	...
	private static class ReassemblyCommand {
		...
		public static Set<NativeElementType> getMemberTypes(
			...
			Set<NativeElementType> types = Olap4jUtil
					.enumSetNoneOf(NativeElementType.class);
			for (ReassemblyCommand command : commands) {
	...
	enum NativeElementType {
		LEVEL_MEMBERS(true), ENUMERATED_VALUE(true), OTHER_NATIVE(true), NON_NATIVE(
	...
	private static Id createSentinelId(Level level) {
		return hierarchyId(level).append(
				q(createMangledName(level, SENTINEL_PREFIX)));
	}

	private static Id createMemberId(Level level) {
		return hierarchyId(level).append(
				q(createMangledName(level, MEMBER_NAME_PREFIX)));
	}
	...
	private static void dumpListToLog(String heading, TupleList list) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(String.format("%s created with %,d rows.", heading,
					list.size()));
