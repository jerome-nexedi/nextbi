/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/DrilldownLevelFunDef.java#9 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/DrilldownLevelFunDef.
 *          java#9 $
 * @since Mar 23, 2006
 */
class DrilldownLevelFunDef extends FunDefBase {
  static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
    ...
    new String[] { "fxx", "fxxl", "fxxen" }, DrilldownLevelFunDef.class);
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    final LevelCalc levelCalc = call.getArgCount() > 1
      && call.getArg(1).getType() instanceof mondrian.olap.type.LevelType ? compiler
      .compileLevel(call.getArg(1)) : null;
    final IntegerCalc indexCalc = call.getArgCount() > 2 ? compiler
      ...
      return new AbstractListCalc(call, new Calc[] { listCalc, indexCalc }) {
        public TupleList evaluateList(Evaluator evaluator) {
          ...
          for (List<Member> tuple : list) {
            result.add(tuple);
            final List<Member> children = schemaReader.getMemberChildren(tuple
              .get(index));
  ...
  List<Member> drill(int searchDepth, List<Member> list, Evaluator evaluator) {
    ...
    for (int i = 0, m = list.size(); i < m; i++) {
      ...
      Member nextMember = i == (m - 1) ? null : list.get(i + 1);
      ...
      if (member.getLevel().getDepth() == searchDepth
        ...
        final List<Member> childMembers = evaluator.getSchemaReader()
