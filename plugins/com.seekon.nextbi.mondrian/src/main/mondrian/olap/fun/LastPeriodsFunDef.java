 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/LastPeriodsFunDef.java#12
 *          $
 * @since Mar 23, 2006
 */
class LastPeriodsFunDef extends FunDefBase {
  static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
    "LastPeriods", "LastPeriods(<Index> [, <Member>])",
    "Returns a set of members prior to and including a specified member.",
    new String[] { "fxn", "fxnm" }, LastPeriodsFunDef.class);
  ...
  /**
   * If Index is positive, returns the set of Index members ending with Member
   * and starting with the member lagging Index - 1 from Member.
   * 
   * <p>
  ...
  List<Member> lastPeriods(Member member, Evaluator evaluator, int indexValue) {
    ...
    Member endMember;
    if (indexValue > 0) {
      startMember = evaluator.getSchemaReader().getLeadMember(member,
        -(indexValue - 1));
      endMember = member;
      if (startMember.isNull()) {
        List<Member> members = evaluator.getSchemaReader().getLevelMembers(
          member.getLevel(), false);
        startMember = members.get(0);
      }
    } else {
      startMember = member;
      endMember = evaluator.getSchemaReader().getLeadMember(member,
        -(indexValue + 1));
      if (endMember.isNull()) {
        List<Member> members = evaluator.getSchemaReader().getLevelMembers(
          member.getLevel(), false);
    ...
    evaluator.getSchemaReader().getMemberRange(member.getLevel(), startMember,
      endMember, list);
    return list;
  }
}
