/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/AbstractAggregateFunDef.java#27 $
 ...
 * @since 2005/8/14
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/AbstractAggregateFunDef
 *          .java#27 $
 */
public class AbstractAggregateFunDef extends FunDefBase {
		...
		if (i == 0) {
			if (MondrianProperties.instance().EnableExpCache.get()) {
				...
				if (FunUtil.worthCaching(arg)) {
					final Exp cacheCall = new UnresolvedFunCall(CacheFunDef.NAME,
							Syntax.Function, new Exp[] { arg });
	...
	/**
	 * Evaluates the list of members or tuples used in computing the aggregate. If
	 * the measure for aggregation has to ignore unrelated dimensions this method
	 * will push unrelated dimension members to top level member. This behaviour
	 * is driven by the ignoreUnrelatedDimensions property on a base cube usage
	 * specified in the virtual cube.Keeps track of the number of iterations that
	 * will be required to iterate over the members or tuples needed to compute
	 * the aggregate within the current context. In doing so, also determines if
	 * the cross product of all iterations across all parent evaluation contexts
	 * will exceed the limit set in the properties file.
	 * 
	 * @param listCalc
	 *          calculator used to evaluate the member list
	 * @param evaluator
	...
	protected TupleIterable evaluateCurrentIterable(IterCalc iterCalc,
			Evaluator evaluator) {
		final int savepoint = evaluator.savepoint();
		evaluator.setNonEmpty(false);
	...
	private static void crossProd(Evaluator evaluator, int currLen) {
		...
		if (iterationLimit > 0) {
			if (productLen > iterationLimit) {
				throw MondrianResource.instance().IterationLimitExceeded
						.ex(iterationLimit);
			}
		}
	 ...
	 * joining dimension exists in the aggregation list then return an empty list
	 * else return the original list.
	 * 
	 * @param tuplesForAggregation
	 *          is a list of members or tuples used in computing the aggregate
	 * @param evaluator
	 *          Evaluator
	 * @return list of members or tuples
	 */
	private static TupleList processUnrelatedDimensions(
			TupleList tuplesForAggregation, Evaluator evaluator) {
		...
		if (virtualCube.isVirtual() && baseCube != null) {
			...
			} else if (MondrianProperties.instance().IgnoreMeasureForNonJoiningDimension
					.get()) {
				return ignoreMeasureForNonJoiningDimension(tuplesForAggregation,
						baseCube);
			}
		}
		return tuplesForAggregation;
	...
	/**
	 * If a non joining dimension exists in the aggregation list then return an
	 * empty list else return the original list.
	 * 
	 * @param tuplesForAggregation
	 *          is a list of members or tuples used in computing the aggregate
	 * @param baseCube
	...
	private static TupleList ignoreMeasureForNonJoiningDimension(
			TupleList tuplesForAggregation, RolapCube baseCube) {
		Set<Dimension> nonJoiningDimensions = nonJoiningDimensions(baseCube,
	...
	private static TupleList ignoreUnrelatedDimensions(
		...
		Set<Dimension> nonJoiningDimensions = nonJoiningDimensions(baseCube,
				tuplesForAggregation);
		final Set<List<Member>> processedTuples = new LinkedHashSet<List<Member>>(
				tuplesForAggregation.size());
		...
		return new DelegatingTupleList(tuplesForAggregation.getArity(),
				new ArrayList<List<Member>>(processedTuples));
	}

	private static Set<Dimension> nonJoiningDimensions(RolapCube baseCube,
			TupleList tuplesForAggregation) {
		List<Member> tuple = tuplesForAggregation.get(0);
		return baseCube
				.nonJoiningDimensions(tuple.toArray(new Member[tuple.size()]));
	}
}
