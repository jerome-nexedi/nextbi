/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/CrossJoinFunDef.java#78 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/CrossJoinFunDef.java#78 $
 * @since Mar 23, 2006
 */
public class CrossJoinFunDef extends FunDefBase {
	static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
			"Crossjoin", "Crossjoin(<Set1>, <Set2>)",
			"Returns the cross product of two sets.", new String[] { "fxxx" },
	...
	private Calc toIter(ExpCompiler compiler, final Exp exp) {
		...
		if (type instanceof SetType) {
			...
			return compiler.compileAs(exp, null,
					ResultStyle.ITERABLE_LIST_MUTABLELIST);
		} else {
			// this always returns an IterCalc
			return new SetFunDef.ExprIterCalc(new DummyExp(new SetType(type)),
					new Exp[] { exp }, compiler, ResultStyle.ITERABLE_LIST_MUTABLELIST);
		}
	}
	...
	class CrossJoinIterCalc extends AbstractIterCalc {
		...
		public TupleIterable evaluateIterable(Evaluator evaluator) {
			...
			NativeEvaluator nativeEvaluator = schemaReader.getNativeSetEvaluator(
		...
		protected TupleIterable makeIterable(final TupleIterable it1,
	...
	private ListCalc toList(ExpCompiler compiler, final Exp exp) {
		...
		final Type type = exp.getType();
		if (type instanceof SetType) {
			final Calc calc = compiler.compileAs(exp, null,
			...
			return new SetFunDef.SetListCalc(new DummyExp(new SetType(type)),
					new Exp[] { exp }, compiler, ResultStyle.LIST_MUTABLELIST);
		}
	}
	...
	class ImmutableListCalc extends BaseListCalc {
		...
		protected TupleList makeList(final TupleList l1, final TupleList l2) {
			...
			return new DelegatingTupleList(arity, new AbstractList<List<Member>>() {
				final List<List<List<Member>>> lists = Arrays
						.<List<List<Member>>> asList(l1, l2);
				final Member[] members = new Member[arity];

				final CartesianProductList cartesianProductList = new CartesianProductList<List<Member>>(
						lists);
	...
	class MutableListCalc extends BaseListCalc {
		...
		protected TupleList makeList(final TupleList l1, final TupleList l2) {
			final int arity = l1.getArity() + l2.getArity();
			final List<Member> members = new ArrayList<Member>(arity * l1.size()
					* l2.size());
	...
	protected TupleList nonEmptyOptimizeList(Evaluator evaluator, TupleList list,
	...
	private static void cartesianProductRecurse(int i, List<TupleList> lists,
			List<Member> partial, Member[] partialArray, int partialSize,
			List<Member> result) {
		...
		while (cursor.forward()) {
			...
			if (i == lists.size() - 1) {
				...
				cartesianProductRecurse(iNext, lists, partial, partialArray,
						partialSizeNext, result);
			}
		}
	}
	...
	/**
	 * Traverses the function call tree of the non empty crossjoin function and
	 * populates the queryMeasureSet with base measures
	 */
	 ...
	 * Customers. The theory here, is if there is no data for a particular Member
	 * of the list where all other Members not part of the list are span their
	 * complete hierarchy, then there is certainly no data for Members of that
	 * Hierarchy at a more specific Level (more on this below).
	 * 
	 * <p>
	 ...
	 * the query... yes and no. For Calculated Measures one must also get all of
	 * the non-Calculated Measures that make up each Calculated Measure. Thus, one
	 * ends up iterating across all Calculated and non-Calculated Measures that
	 * are explicitly mentioned in the query as well as all Calculated and
	 ...
	 * possible Measures that might yield a non-null value for the List-Members
	 * and that is what we what to find. It might be a super set, but thats ok; we
	 * just do not want to miss anything.
	 * 
	 * <p>
	 ...
	 * Measure. For a dense dataset this may not be a problem or even apparent,
	 * but for a sparse dataset, the first Measure may, in fact, have not data but
	 * other Measures associated with the query might. Hence, the solution here is
	 * to identify all Measures associated with the query and then for each Member
	 * of the list, determine if there is any data iterating across all Measures
	 * until non-null data is found or the end of the Measures is reached.
	 * 
	 * <p>
	 * This is a non-optimistic implementation. This means that an element of the
	 * input parameter List is only not included in the returned result List if
	 * for no combination of Measures, non-All Members (for Hierarchies that have
	 * no All Members) and evaluator default Members did the element evaluate to
	 * non-null.
	 * 
	 * @param evaluator
	 *          Evaluator
	 ...
	 * 
	 * @return List of elements from the input parameter list that have evaluated
	 *         to non-null.
	 */
	protected TupleList nonEmptyList(Evaluator evaluator, TupleList list,
		...
		Member[][] nonAllMembers = (Member[][]) query
				.getEvalCache(nonAllMembersKey);
			...
			for (Member evalMember : evalMembers) {
				...
				if (isSlicerMember || !em.isAll()) {
					...
					if (h.hasAll()) {
						...
						Member[] rootMembers = rootMemberList
								.toArray(new Member[rootMemberList.size()]);
						nonAllMemberList.add(rootMembers);
					}
				}
			}
			nonAllMembers = nonAllMemberList.toArray(new Member[nonAllMemberList
					.size()][]);
		...
		while (cursor.forward()) {
			...
			if (checkData(nonAllMembers, nonAllMembers.length - 1, measureSet,
					evaluator)) {
	 ...
	 * 
	 * @param nonAllMembers
	 *          array of Member arrays of top-level Members for Hierarchies that
	 *          have no All Member.
	 * @param cnt
	 ...
	 */
	private static boolean checkData(Member[][] nonAllMembers, int cnt,
			Set<Member> measureSet, Evaluator evaluator) {
		if (cnt < 0) {
	...
	private static class StarCrossJoinResolver extends MultiResolver {
		public StarCrossJoinResolver() {
			super("*", "<Set1> * <Set2>", "Returns the cross product of two sets.",
