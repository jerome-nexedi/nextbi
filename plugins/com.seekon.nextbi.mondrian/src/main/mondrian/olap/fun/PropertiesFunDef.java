/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/PropertiesFunDef.java#23 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/PropertiesFunDef.java#23
 *          $
 * @since Mar 23, 2006
 */
class PropertiesFunDef extends FunDefBase {
	...
	public PropertiesFunDef(String name, String signature, String description,
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		return new GenericCalc(call) {
			public Object evaluate(Evaluator evaluator) {
				return properties(memberCalc.evaluateMember(evaluator),
	...
	static Object properties(Member member, String s) {
		...
		if (o == null) {
			if (!Util.isValidProperty(s, member.getLevel())) {
				throw new MondrianEvaluationException("Property '" + s
	...
	private static class ResolverImpl extends ResolverBase {
		...
		public FunDef resolve(Exp[] args, Validator validator,
			...
			if ((args.length != 2) || (memberExp.getCategory() != Category.Member)
					|| (propertyNameExp.getCategory() != Category.String)) {
				return null;
			}
			int returnType = deducePropertyCategory(memberExp, propertyNameExp);
			return new PropertiesFunDef(getName(), getSignature(), getDescription(),
					getSyntax(), returnType, argTypes);
		}

		/**
		 * Deduces the category of a property. This is possible only if the name is
		 * a string literal, and the member's hierarchy is unambigous. If the type
		 * cannot be deduced, returns {@link Category#Value}.
		 * 
		 * @param memberExp
		 *          Expression for the member
		 * @param propertyNameExp
		 *          Expression for the name of the property
		...
		private int deducePropertyCategory(Exp memberExp, Exp propertyNameExp) {
			...
			Property property = lookupProperty(levels[levels.length - 1],
					propertyName);
			if (property == null) {
