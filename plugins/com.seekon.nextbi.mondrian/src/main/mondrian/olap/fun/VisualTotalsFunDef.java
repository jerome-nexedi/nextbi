/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/VisualTotalsFunDef.java#22 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/VisualTotalsFunDef.java
 *          #22 $
 * @since Jan 16, 2006
 */
public class VisualTotalsFunDef extends FunDefBase {
	static final Resolver Resolver = new ReflectiveMultiResolver(
			...
			new String[] { "fxx", "fxxS" }, VisualTotalsFunDef.class);
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		final StringCalc stringCalc = call.getArgCount() > 1 ? compiler
				.compileString(call.getArg(1)) : null;
	...
	private static class CalcImpl extends AbstractListCalc {
		...
		public TupleList evaluateList(Evaluator evaluator) {
			final List<Member> list = listCalc.evaluateList(evaluator).slice(0);
			...
			for (int i = memberCount - 1; i >= 0; --i) {
				...
				if (i + 1 < memberCount) {
					...
					if (nextMember != member && nextMember.isChildOrEqualTo(member)) {
						resultList.set(i, createMember(member, i, resultList, evaluator));
					}
				}
			}
		...
		private VisualTotalMember createMember(Member member, int i,
				final List<Member> list, Evaluator evaluator) {
			final String name;
			if (stringCalc != null) {
			...
			final List<Member> childMemberList = followingDescendants(member, i + 1,
					list);
		...
		private List<Member> followingDescendants(Member member, int i,
		...
		private Exp makeExpr(final List childMemberList) {
			...
			return new UnresolvedFunCall("Aggregate",
					new Exp[] { new UnresolvedFunCall("{}", Syntax.Braces, memberExprs) });
		}
	}
	 ...
	 * <li>its value is a calculation computed by aggregating all of the members
	 * which occur following it in the list
	 * </ul>
	 * </p>
	 */
	public static class VisualTotalMember extends RolapMemberBase {
		...
		VisualTotalMember(Member member, String name, final Exp exp) {
			super((RolapMember) member.getParentMember(), (RolapLevel) member
					.getLevel(), null, name, MemberType.FORMULA);
			this.member = member;
			this.exp = exp;
		}
		...
		public boolean equals(Object o) {
			...
			return o instanceof VisualTotalMember
					...
					|| o instanceof Member && this.member.equals(o);
		}
	 ...
	 * <p/>
	 * 
	 * Asterisks are replaced with the name, double-asterisks are replaced with a
	 ...
	 *          Pattern
	 * @param name
	 *          Name to substitute into pattern
