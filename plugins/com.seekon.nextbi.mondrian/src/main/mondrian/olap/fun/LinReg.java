/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/LinReg.java#20 $
 ...
 *      <li>X and Y don't really correlate at all, and you just happened to
 *      observe such a strong correlation by chance. The P value determines how
 ...
 *      determination, but scientists call it r squared. It is has a value that
 *      ranges from zero to one, and is the fraction of the variance in the two
 *      variables that is shared. For example, if r2=0.59, then 59% of the
 *      variance in X can be explained by variation in Y. &nbsp;Likewise, 59% of
 *      the variance in Y can be explained by (or goes along with) variation in
 *      X. More simply, 59% of the variance is shared between X and Y.
 *      </p>
 * 
 *      <p>
 *      (<a href=
 ...
 *      <p>
 *      Also see: <a
 *      href="http://mathworld.wolfram.com/LeastSquaresFitting.html">least
 *      squares fitting</a>.
 */
...
public abstract class LinReg extends FunDefBase {
	...
	static final Resolver InterceptResolver = new ReflectiveMultiResolver(
			...
			new String[] { "fnxn", "fnxnn" }, InterceptFunDef.class);
	...
	static final Resolver PointResolver = new ReflectiveMultiResolver(
			...
			new String[] { "fnnxn", "fnnxnn" }, PointFunDef.class);
	...
	static final Resolver SlopeResolver = new ReflectiveMultiResolver(
			...
			new String[] { "fnxn", "fnxnn" }, SlopeFunDef.class);
	...
	static final Resolver R2Resolver = new ReflectiveMultiResolver(
			...
			new String[] { "fnxn", "fnxnn" }, R2FunDef.class);
	...
	static final Resolver VarianceResolver = new ReflectiveMultiResolver(
			...
			new String[] { "fnxn", "fnxnn" }, VarianceFunDef.class);

	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		final ListCalc listCalc = compiler.compileList(call.getArg(0));
		final DoubleCalc yCalc = compiler.compileDouble(call.getArg(1));
		final DoubleCalc xCalc = call.getArgCount() > 2 ? compiler
				.compileDouble(call.getArg(2)) : new ValueCalc(call);
	...
	static class Value {
		...
		public String toString() {
			return "LinReg.Value: slope of " + slope + " and an intercept of "
					+ intercept + ". That is, y=" + intercept
					+ (slope > 0.0 ? " +" : " ") + slope + " * x.";
		}
	}
	...
	public static class PointFunDef extends LinReg {
		...
		public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
			...
			final DoubleCalc xCalc = call.getArgCount() > 3 ? compiler
					.compileDouble(call.getArg(3)) : new ValueCalc(call);
			return new PointCalc(call, xPointCalc, listCalc, yCalc, xCalc);
		}
	}
	...
	private static class PointCalc extends AbstractDoubleCalc {
		...
		public PointCalc(ResolvedFunCall call, DoubleCalc xPointCalc,
				ListCalc listCalc, DoubleCalc yCalc, DoubleCalc xCalc) {
	...
	protected static LinReg.Value process(Evaluator evaluator, ListCalc listCalc,
			DoubleCalc yCalc, DoubleCalc xCalc) {
		...
		SetWrapper[] sws = evaluateSet(evaluator, members, new DoubleCalc[] {
				yCalc, xCalc });
		evaluator.restore(savepoint);
	...
	public static LinReg.Value linearReg(List xlist, List ylist) {
		...
		debug("LinReg.linearReg", "(n*sumXX - sumX*sumX)="
				+ (n * sumXX - sumX * sumX));
	...
	private static class LinRegCalc extends AbstractDoubleCalc {
		...
		public LinRegCalc(ResolvedFunCall call, ListCalc listCalc,
				DoubleCalc yCalc, DoubleCalc xCalc, int regType) {
