/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/UdfResolver.java#28 $
 ...
 * @author jhyde
 * @since 2.0
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/UdfResolver.java#28
 *          $
 */
public class UdfResolver implements Resolver {
	...
	public String getSignature() {
		...
		return getSyntax().getSignature(getName(), returnCategory,
				parameterCategories);
	}
	...
	/**
	 * Adapter which converts a {@link UserDefinedFunction} into a {@link FunDef}.
	 */
	private class UdfFunDef extends FunDefBase {
		private Type returnType;

		public UdfFunDef(int[] parameterCategories, Type returnType) {
			super(UdfResolver.this, TypeUtil.typeToCategory(returnType),
					parameterCategories);
		...
		public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
			...
			for (int i = 0; i < args.length; i++) {
				Exp arg = args[i];
				final Calc calc = compiler.compileAs(arg,
	...
	private static class ScalarCalcImpl extends GenericCalc {
		...
		public ScalarCalcImpl(ResolvedFunCall call, Calc[] calcs,
				UserDefinedFunction udf, UserDefinedFunction.Argument[] args) {
			super(call);
			this.calcs = calcs;
			this.udf = udf;
			this.args = args;
		...
		public Object evaluate(Evaluator evaluator) {
			try {
				...
				return FunUtil.newEvalException("Exception while executing function "
						+ udf.getName(), e);
			}
		}
	...
	private static class ListCalcImpl extends AbstractListCalc {
		...
		public ListCalcImpl(ResolvedFunCall call, Calc[] calcs,
				UserDefinedFunction udf, UserDefinedFunction.Argument[] args) {
			super(call, calcs);
			this.udf = udf;
			this.args = args;
		...
		public TupleList evaluateList(Evaluator evaluator) {
			...
			if (getType().getArity() == 1) {
				...
				return new ListTupleList(arity, new AbstractList<Member>() {
					@Override
					public Member get(int index) {
						return memberArrayList.get(index / arity)[index % arity];
					}
	...
	/**
	 * Wrapper around a {@link Calc} to make it appear as an {@link Exp}. Only the
	 * {@link #evaluate(mondrian.olap.Evaluator)} and
	 * {@link #evaluateScalar(mondrian.olap.Evaluator)} methods are supported.
	 */
	private static class CalcExp implements UserDefinedFunction.Argument {
		 ...
		 * @param listCalc
		 *          Compiled expression that evaluates an MDX set to a java list
		 * @param iterCalc
		 *          Compiled expression that evaluates an MDX set to
		 */
		public CalcExp(Calc calc, Calc scalarCalc, ListCalc listCalc,
				IterCalc iterCalc) {
			this.calc = calc;
		 ...
		 * calculator expressions to the old style, that returned either members or
		 * arrays of members.
		 * 
		 * @param o
		 *          Output of calc
		 * @return Output in new format (lists and iterables over lists of members)
		 */
	 ...
	 * 
	 * <p>
	 * This factory is required because a user-defined function is allowed to
	 * store state in itself. Therefore it is unsanitary to use the same UDF in
	 * the function table for validation and runtime. In the function table there
	 * is a factory. We use one instance of instance of the UDF to validate, and
	 * create another for the runtime plan.
	 * </p>
	 */
	...
	/**
	 * Implementation of {@link UdfFactory} that instantiates a given class using
	...
	public static class ClassUdfFactory implements UdfResolver.UdfFactory {
		 ...
		 *          Name
		 */
		public ClassUdfFactory(Class<? extends UserDefinedFunction> clazz,
				String name) {
			this.clazz = clazz;
			this.name = name;
			assert clazz != null;
