/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/AddCalculatedMembersFunDef.java#10 $
 ...
 * 
 * <p>
 * AddCalculatedMembers adds calculated members that are siblings of the members
 * in the set. The set is limited to one dimension.
 * 
 * <p>
 * Syntax:
 * 
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/AddCalculatedMembersFunDef
 *          .java#10 $
 * @since Mar 23, 2006
 */
class AddCalculatedMembersFunDef extends FunDefBase {
	...
	private List<Member> addCalculatedMembers(List<Member> memberList,
			Evaluator evaluator) {
			...
			if (hierarchy == null) {
				...
				throw newEvalException(
						...
						"Only members from the same hierarchy are allowed in the "
								+ "AddCalculatedMembers set: " + hierarchy + " vs "
								+ member.getHierarchy());
			}
		...
		final SchemaReader schemaReader = evaluator.getQuery()
				.getSchemaReader(true);
	...
	private static class ResolverImpl extends MultiResolver {
		public ResolverImpl() {
			super(instance.getName(), instance.getSignature(), instance
					.getDescription(), new String[] { FLAG });
		}
			...
			if (args.length == 1) {
				...
				if (type1 instanceof SetType) {
					...
					if (type.getElementType() instanceof MemberType) {
						...
						throw newEvalException(instance,
