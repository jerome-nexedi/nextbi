...
public class SetFunDef extends FunDefBase {
    ...
    if (type instanceof SetType) {
      ...
      return new AbstractIterCalc(call, new Calc[] { tupleCalc }) {
        public TupleIterable evaluateIterable(Evaluator evaluator) {
          ...
          return new ListTupleList(tupleCalc.getType().getArity(), Arrays
            .asList(members));
        }
      ...
      return new AbstractTupleIterable(getType().getArity()) {
        public TupleCursor tupleCursor() {
          return new AbstractTupleCursor(arity) {
            Iterator<IterCalc> calcIterator = Arrays.asList(iterCalcs).iterator();
  ...
  private static class ResolverImpl extends ResolverBase {
    public ResolverImpl() {
      super("{}", "{<Member> [, <Member>...]}", "Brace operator constructs a set.",
        Syntax.Braces);
    }
