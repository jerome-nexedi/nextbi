/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/SetItemFunDef.java#17 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/SetItemFunDef.java#17 $
 * @since Mar 23, 2006
 */
class SetItemFunDef extends FunDefBase {
	static final Resolver intResolver = new ReflectiveMultiResolver(
			...
			new String[] { "mmxn" }, SetItemFunDef.class);
	...
	static final Resolver stringResolver = new ResolverBase(
		...
		public FunDef resolve(Exp[] args, Validator validator,
			...
			for (int i = 1; i < args.length; i++) {
				if (!validator.canConvert(i, args[i], Category.String, conversions)) {
					return null;
				}
			}
			if (args.length - 1 != arity) {
				throw Util.newError("Argument count does not match set's cardinality "
						+ arity);
			}
			...
			if (isString) {
				return new AbstractTupleCalc(call, calcs) {
					public Member[] evaluateTuple(Evaluator evaluator) {
						...
						listLoop: for (List<Member> members : list) {
			...
			if (isString) {
				return new AbstractMemberCalc(call, calcs) {
					public Member evaluateMember(Evaluator evaluator) {
						...
						assert list != null;
						evaluator.restore(savepoint);
						final String result = stringCalcs[0].evaluateString(evaluator);
