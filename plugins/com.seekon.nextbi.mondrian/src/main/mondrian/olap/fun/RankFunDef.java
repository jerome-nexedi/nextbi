/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/RankFunDef.java#26 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/RankFunDef.java#26 $
 */
public class RankFunDef extends FunDefBase {
	static final boolean debug = false;
	static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
			"Rank", "Rank(<Tuple>, <Set> [, <Calc Expression>])",
			"Returns the one-based rank of a tuple in a set.", new String[] { "fitx",
					"fitxn", "fimx", "fimxn" }, RankFunDef.class);
	...
	public Calc compileCall3(ResolvedFunCall call, ExpCompiler compiler) {
		...
		final ExpCacheDescriptor cacheDescriptor = new ExpCacheDescriptor(call,
				sortedListCalc, compiler.getEvaluator());
	...
	public Calc compileCall2(ResolvedFunCall call, ExpCompiler compiler) {
		...
		if (MondrianProperties.instance().EnableExpCache.get()) {
			final ExpCacheDescriptor key = new ExpCacheDescriptor(listExp, listCalc1,
	...
	private static class Rank2TupleCalc extends AbstractIntegerCalc {
		...
		public Rank2TupleCalc(ResolvedFunCall call, TupleCalc tupleCalc,
				Calc listCalc) {
		...
		public int evaluateInteger(Evaluator evaluator) {
			...
			final RankedTupleList rankedTupleList = (RankedTupleList) listCalc
					.evaluate(evaluator);
	...
	private static class Rank2MemberCalc extends AbstractIntegerCalc {
		...
		public Rank2MemberCalc(ResolvedFunCall call, MemberCalc memberCalc,
				Calc listCalc) {
		...
		public int evaluateInteger(Evaluator evaluator) {
			...
			// not valid - in the Rank function". We will naturally return 0,
			// which I think is better.
			RankedMemberList rankedMemberList = (RankedMemberList) listCalc
					.evaluate(evaluator);
	...
	private static class Rank3TupleCalc extends AbstractIntegerCalc {
		...
		public Rank3TupleCalc(ResolvedFunCall call, TupleCalc tupleCalc,
				Calc sortCalc, ExpCacheDescriptor cacheDescriptor) {
		...
		public int evaluateInteger(Evaluator evaluator) {
			...
			final TupleSortResult sortResult = (TupleSortResult) evaluator
					.getCachedResult(cacheDescriptor);
			if (debug) {
			...
			int j = Arrays.binarySearch(sortResult.values, value,
	...
	private static class Rank3MemberCalc extends AbstractIntegerCalc {
		...
		public Rank3MemberCalc(ResolvedFunCall call, MemberCalc memberCalc,
				Calc sortCalc, ExpCacheDescriptor cacheDescriptor) {
		...
		public int evaluateInteger(Evaluator evaluator) {
			...
			// list, propagate it up.
			final MemberSortResult sortResult = (MemberSortResult) evaluator
					.getCachedResult(cacheDescriptor);
			if (debug) {
				sortResult.print(new PrintWriter(System.out));
			}
	...
	/**
	 * Calc which evaluates an expression to form a list of tuples, evaluates a
	 * scalar expression at each tuple, then sorts the list of values. The result
	 * is a value of type {@link SortResult}, and can be used to implement the
	...
	private static class SortedListCalc extends AbstractCalc {
		...
		public Object evaluate(Evaluator evaluator) {
			...
			assert list != null;
			if (list.isEmpty()) {
				return list.getArity() == 1 ? new MemberSortResult(new Object[0],
						Collections.<Member, Integer> emptyMap()) : new TupleSortResult(
						new Object[0], Collections.<List<Member>, Integer> emptyMap());
			}
			RuntimeException exception = null;
			// noinspection unchecked
			final Map<Object, Integer> uniqueValueCounterMap = new TreeMap<Object, Integer>(
			...
			// noinspection unchecked
			final Map<Object, Integer> uniqueValueRankMap = new TreeMap<Object, Integer>(
					Collections.<Object> reverseOrder());
			...
			if (list.getArity() == 1) {
				final Map<Member, Integer> rankMap = new HashMap<Member, Integer>();
				...
				final Map<List<Member>, Integer> rankMap = new HashMap<List<Member>, Integer>();
	...
	/**
	 * Holder for the result of sorting a set of values. It provides simple
	 * interface to look up the rank for a member or a tuple.
	 */
	private static abstract class SortResult {
		/**
		 * All values in sorted order; Duplicates are not removed. E.g. Set
		 * (15,15,5,0) 10 should be ranked 3.
		 * 
		 * <p>
	 ...
	 * {@link mondrian.olap.fun.RankFunDef.RankedTupleList}, or null if the list
	 * is empty.
	 */
