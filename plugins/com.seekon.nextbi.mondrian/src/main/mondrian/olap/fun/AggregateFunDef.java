/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/AggregateFunDef.java#38 $
 ...
 * @since 2005/8/14
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/AggregateFunDef.java#38 $
 */
public class AggregateFunDef extends AbstractAggregateFunDef {
  ...
  static final ReflectiveMultiResolver resolver = new ReflectiveMultiResolver(
    ...
    new String[] { "fnx", "fnxn" }, AggregateFunDef.class);
  ...
  public static class AggregateCalc extends GenericCalc {
    ...
    /**
     * Computes an expression for each element of a list, and aggregates the
     * result according to the evaluation context's current aggregation
     * strategy.
     * 
     * @param calc
     *          Compiled expression to evaluate a scalar
     * @param evaluator
     *          Evaluation context
     * @param tupleList
    ...
    public static Object aggregate(Calc calc, Evaluator evaluator,
      ...
      if (evaluator instanceof RolapEvaluator
        && ((RolapEvaluator) evaluator).getDialect().supportsUnlimitedValueList()) {
    ...
    /**
     * Analyzes a list of tuples and determines if the list can be safely
     * optimized. If a member of the tuple list is on a hierarchy for which a
     * rollup policy of PARTIAL is set, it is not safe to optimize that list.
     */
    private static boolean canOptimize(Evaluator evaluator, TupleList tupleList) {
      ...
      for (List<Member> tupleMembers : tupleList) {
        for (Member member : tupleMembers) {
          final RollupPolicy policy = evaluator.getSchemaReader().getRole()
            .getAccessDetails(member.getHierarchy()).getRollupPolicy();
    ...
    public static TupleList optimizeTupleList(Evaluator evaluator,
      ...
      tupleList = optimizeChildren(tupleList, evaluator.getSchemaReader(), evaluator
        .getMeasureCube());
    ...
    /**
     * In case of distinct count aggregation if a tuple which is a super set of
     * other tuples in the set exists then the child tuples can be ignored.
     * 
     * <p>
     * For example. A list consisting of: (Gender.[All Gender], [Product].[All
     * Products]), (Gender.[F], [Product].[Drink]), (Gender.[M],
     * [Product].[Food]) Can be optimized to: (Gender.[All Gender],
        ...
        if (trimmedList.isEmpty()) {
          ...
          while (iterator.forward()) {
            ...
            if (isSuperSet(tuple1, tuple2)) {
              iterator.remove();
            } else if (isSuperSet(tuple2, tuple1) || isEqual(tuple1, tuple2)) {
              ignore = true;
              break;
            }
          }
    ...
    private static void checkIfAggregationSizeIsTooLarge(List list) {
      final IntegerProperty property = MondrianProperties.instance().MaxConstraints;
      final int maxConstraints = property.get();
      if (list.size() > maxConstraints) {
        throw newEvalException(null, "Aggregation is not supported over a list"
          + " with more than " + maxConstraints + " predicates" + " (see property "
          + property.getPath() + ")");
      }
    }
    ...
    /**
     * In distinct Count aggregation, if tuple list is a result m.children *
     * n.children then it can be optimized to m * n
     * 
     * <p>
     * E.g. List consist of: (Gender.[F], [Store].[USA]), (Gender.[F],
     * [Store].[USA].[OR]), (Gender.[F], [Store].[USA].[CA]), (Gender.[F],
     * [Store].[USA].[WA]), (Gender.[F], [Store].[CANADA]) (Gender.[M],
     * [Store].[USA]), (Gender.[M], [Store].[USA].[OR]), (Gender.[M],
     * [Store].[USA].[CA]), (Gender.[M], [Store].[USA].[WA]), (Gender.[M],
     * [Store].[CANADA]) Can be optimized to: (Gender.[All Gender],
     * [Store].[USA]) (Gender.[All Gender], [Store].[CANADA])
     * 
     * 
     * @param tuples
     *          Tuples
     ...
     * @return xxxx
     */
    public static TupleList optimizeChildren(TupleList tuples, SchemaReader reader,
      Cube baseCubeForMeasure) {
      Map<Member, Integer>[] membersOccurencesInTuple = membersVersusOccurencesInTuple(tuples);
      ...
      for (int i = 0; i < tupleLength; i++) {
        if (areOccurencesEqual(membersOccurencesInTuple[i].values())) {
          ...
          sets[i] = optimizeMemberSet(new LinkedHashSet<Member>(members), reader,
            baseCubeForMeasure);
    ...
    /**
     * Finds member occurrences in tuple and generates a map of Members versus
     * their occurrences in tuples.
     * 
     * @param tupleList
     *          List of tuples
    ...
    private static Set<Member> optimizeMemberSet(Set<Member> members,
      SchemaReader reader, Cube baseCubeForMeasure) {
      boolean didOptimize;
      ...
      while (members.size() > 0) {
        ...
        while (iterator.hasNext()) {
          ...
          if (firstParentMember == null
            ...
            || (firstParentMember != null && firstParentMember
              .equals(currentParentMember))) {
    ...
    private static boolean canOptimize(Member parentMember, Cube baseCube) {
      return dimensionJoinsToBaseCube(parentMember.getDimension(), baseCube)
        || !parentMember.isAll();
    }
    ...
    private static boolean dimensionJoinsToBaseCube(Dimension dimension,
      Cube baseCube) {
