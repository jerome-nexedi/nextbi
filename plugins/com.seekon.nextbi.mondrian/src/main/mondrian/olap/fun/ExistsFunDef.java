/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/ExistsFunDef.java#7 $
 ...
 * 
 * @author kvu
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/ExistsFunDef.java#7
 *          $
 * @since Mar 23, 2008
 */
class ExistsFunDef extends FunDefBase {
	static final Resolver resolver = new ReflectiveMultiResolver(
			...
			new String[] { "fxxx" }, ExistsFunDef.class);
		...
		return new AbstractListCalc(call, new Calc[] { listCalc1, listCalc2 }) {
			public TupleList evaluateList(Evaluator evaluator) {
				...
				leftLoop: for (List<Member> leftTuple : leftTuples) {
					rightLoop: for (List<Member> rightTuple : rightTuples) {
						for (int i = 0; i < rightSize; i++) {
