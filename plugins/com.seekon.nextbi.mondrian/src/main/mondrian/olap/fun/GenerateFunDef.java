/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/GenerateFunDef.java#14 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/GenerateFunDef.java#14 $
 * @since Mar 23, 2006
 */
class GenerateFunDef extends FunDefBase {
  static final ReflectiveMultiResolver ListResolver = new ReflectiveMultiResolver(
    "Generate",
    "Generate(<Set1>, <Set2>[, ALL])",
    "Applies a set to each member of another set and joins the resulting sets by union.",
    new String[] { "fxxx", "fxxxy" }, GenerateFunDef.class);

  static final ReflectiveMultiResolver StringResolver = new ReflectiveMultiResolver(
    "Generate",
    "Generate(<Set>, <String>[, <String>])",
    "Applies a set to a string expression and joins resulting sets by string concatenation.",
    new String[] { "fSxS", "fSxSS" }, GenerateFunDef.class);
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    if (call.getArg(1).getType() instanceof StringType) {
      ...
      return new GenerateStringCalcImpl(call, (IterCalc) iterCalc, stringCalc,
        delimCalc);
    } else {
  ...
  private static class GenerateListCalcImpl extends AbstractListCalc {
    ...
    public GenerateListCalcImpl(ResolvedFunCall call, IterCalc iterCalc,
      ListCalc listCalc2, int arityOut, boolean all) {
    ...
    public TupleList evaluateList(Evaluator evaluator) {
      ...
      final TupleIterable iterable1 = iterCalc1.evaluateIterable(evaluator);
      ...
      if (all) {
        ...
        final Set<List<Member>> emitted = new HashSet<List<Member>>();
        final TupleCursor cursor = iterable1.tupleCursor();
        while (cursor.forward()) {
          cursor.setContext(evaluator);
          final TupleList result2 = listCalc2.evaluateList(evaluator);
  ...
  private static class GenerateStringCalcImpl extends AbstractStringCalc {
    ...
    public GenerateStringCalcImpl(ResolvedFunCall call, IterCalc iterCalc,
      StringCalc stringCalc, StringCalc sepCalc) {
    ...
    public String evaluateString(Evaluator evaluator) {
      ...
      int k = 0;
      final TupleIterable iter11 = iterCalc.evaluateIterable(evaluator);
      final int savepoint = evaluator.savepoint();
      ...
      while (cursor.forward()) {
        ...
        final String result2 = stringCalc.evaluateString(evaluator);
        buf.append(result2);
      }
      evaluator.restore(savepoint);
      return buf.toString();
