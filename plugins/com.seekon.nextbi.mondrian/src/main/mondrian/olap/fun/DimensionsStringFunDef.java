/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/DimensionsStringFunDef.java#1 $
...
/**
 * Definition of the <code>Dimensions(&lt;String Expression&gt;)</code> MDX
 * builtin function.
 * 
 * <p>
 * NOTE: Actually returns a hierarchy. This is consistent with Analysis
 * Services.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/DimensionsStringFunDef
 *          .java#1 $
 * @since Jul 20, 2009
 */
class DimensionsStringFunDef extends FunDefBase {
  ...
  private DimensionsStringFunDef() {
    super("Dimensions",
      "Returns the hierarchy whose name is specified by a string.", "fhS");
  }
  ...
  Hierarchy findHierarchy(String name, Evaluator evaluator) {
    ...
    OlapElement o = evaluator.getSchemaReader().lookupCompound(evaluator.getCube(),
      parseIdentifier(name), false, Category.Hierarchy);
    if (o instanceof Hierarchy) {
      return (Hierarchy) o;
    } else if (o == null) {
      throw newEvalException(this, "Hierarchy '" + name + "' not found");
    } else {
      throw newEvalException(this, "Hierarchy(" + name + ") found " + o);
    }
  }
}
