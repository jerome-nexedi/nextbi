/*
// $Id: //open/mondrian/src/main/mondrian/olap/CacheControl.java#10 $
...
/**
 * API for controlling the contents of the cell cache and the member cache. A
 * {@link CellRegion} denotes a portion of the cell cache, and a
 ...
 * {@link mondrian.olap.Connection#getCacheControl}.
 * </p>
 * 
 * <p>
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/CacheControl.java#10 $
 * @since Sep 27, 2006
 */
public interface CacheControl {
	 ...
	 *          the member
	 * @param descendants
	 *          When true, include descendants of the member in the region.
	 ...
	 * 
	 * <p>
	 * The members must belong to the same level of the same hierarchy. One of the
	 * bounds may be null.
	 * 
	 * <p>
	 * For example, given
	 ...
	 *          whether the the range includes the lower bound; ignored if the
	 *          lower bound is not specified
	 * @param lowerMember
	 *          lower bound member. If null, takes all preceding members
	 * @param upperInclusive
	 *          whether the the range includes the upper bound; ignored if the
	 *          upper bound is not specified
	 * @param upperMember
	 *          upper bound member. If null, takes all preceding members
	 * @param descendants
	 *          when true, include descendants of the member in the region
	 * @return the new cell region
	 */
	CellRegion createMemberRegion(boolean lowerInclusive, Member lowerMember,
	...
	/**
	 * Forms the union of two or more cell regions. The regions must have the same
	 * dimensionality.
	 * 
	 * @param regions
	 *          the operands
	...
	/**
	 * Creates a member set containing either a single member, or a member and its
	 * descendants.
	 * 
	 * @param member
	 *          a member
	 * @param descendants
	 *          when true, include descendants in the set
	 * @return the set
	 */
	MemberSet createMemberSet(Member member, boolean descendants);

	/**
	 * Creates a member set consisting of a range between two members. The members
	 * must belong to the same level of the same hierarchy. One of the bounds may
	 * be null. (Similar to
	 ...
	 * @return the set
	 */
	MemberSet createMemberSet(boolean lowerInclusive, Member lowerMember,
			boolean upperInclusive, Member upperMember, boolean descendants);
	...
	/**
	 * Atomically flushes all members in the member cache which belong to a given
	 * set.
	 * 
	 * @param set
	 *          a set of members
	 ...
	 * 
	 * @param cmds
	 *          a list of the component commands
	 ...
	 * 
	 * <p>
	 * The ordinal position of the new member among its siblings is implied by its
	 * properties.
	 * </p>
	 * 
	 * @param member
	 ...
	 */
	MemberEditCommand createAddCommand(Member member)
			throws IllegalArgumentException;
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *           if member is null, or loc is null, or member belongs to a
	 *           parent-child hierarchy, or if loc is incompatible with member
	 */
	MemberEditCommand createMoveCommand(Member member, Member loc)
			throws IllegalArgumentException;
	 ...
	 */
	MemberEditCommand createSetPropertyCommand(Member member, String name,
			Object value) throws IllegalArgumentException;

	/**
	 * Creates a command to several properties changes over a set of members. All
	 ...
	 * @return the command
	 * @throws IllegalArgumentException
	 *           for an invalid property, or if all members in the set do not
	 *           belong to the same Level.
	 */
	MemberEditCommand createSetPropertyCommand(MemberSet set,
	 ...
	 * 
	 * <p>
	 * This cache is referenced only when creating a new connection, so existing
	 * connections will continue to use the same schema definition.
	 * 
	 * <p>
	...
	/**
	 * Flushes the given Schema instance from the pool. It resolves the schema to
	 * flush by using its catalog URL, connection key and JDBC username.
	 * 
	 * <p>
	 * Flushing the schema cache will flush all aggregations and segments
	 * associated to it as well.
	 */
	void flushSchema(final String catalogUrl, final String connectionKey,
			final String jdbcUser, String dataSourceStr);

	/**
	 * Flushes the given Schema instance from the pool. It resolves the schema to
	 * flush by using its catalog URL and DataSource object.
	 * 
	 * <p>
	 * Flushing the schema cache will flush all aggregations and segments
	 * associated to it as well.
	...
	/**
	 * An operation to be applied to the member cache. The operation does not take
	 * effect until {@link CacheControl#execute} is called.
	 */
