/*
// $Id: //open/mondrian/src/main/mondrian/olap/CubeBase.java#37 $
 ...
 * @author jhyde
 * @since 6 August, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/olap/CubeBase.java#37 $
 */
public abstract class CubeBase extends OlapElementBase implements Cube {
	 ...
	 * 
	 * http://msdn.microsoft.com/library/psdk/dasdk/mdx8h4k.htm check
	 * "Restrictions in the MEMBER Rowset" under MEMBER Rowset section
	 */
	 ...
	 *          List of dimensions
	 */
	protected CubeBase(String name, String caption, boolean visible,
			String description, Dimension[] dimensions) {
		this.name = name;
	...
	public OlapElement lookupChild(SchemaReader schemaReader, Id.Segment s,
		...
		if (MondrianProperties.instance().SsasCompatibleNaming.get()
			...
			for (Dimension dimension : dimensions) {
				...
				for (Hierarchy hierarchy : schemaReader
						.getDimensionHierarchies(dimension)) {
					if (Util.equalName(s.name,
							dimension.getName() + "." + hierarchy.getName())) {
						return hierarchy;
					}
				}
			}
		...
		for (Dimension dimension : dimensions) {
			OlapElement mdxElement = dimension
