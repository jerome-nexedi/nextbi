/*
// $Id: //open/mondrian/src/main/mondrian/olap/Formula.java#55 $
...
public class Formula extends QueryPart {
  ...
  /**
   * <code>true</code> is this is a member, <code>false</code> if it is a set.
   */
  ...
  Formula(boolean isMember, Id id, Exp exp, MemberProperty[] memberProperties,
    Member mdxMember, NamedSet mdxSet) {
  ...
  public Object clone() {
    return new Formula(isMember, id, exp.clone(), MemberProperty
      .cloneArray(memberProperties), mdxMember, mdxSet);
  }
   ...
   * 
   * @param validator
   *          Validation context to resolve the identifiers in this formula
   */
  void accept(Validator validator) {
    ...
    if (isMember) {
      ...
      final List<MemberProperty> memberPropertyList = new ArrayList<MemberProperty>(
        Arrays.asList(memberProperties));
  ...
  void createElement(Query q) {
    ...
    if (isMember) {
      ...
      for (int i = 0; i < segments.size(); i++) {
        ...
        if (mdxElement == null || i == segments.size() - 1) {
          ...
          if (parent instanceof Member) {
            ...
            if (level == null) {
              throw Util.newError("The '" + segment.name
                + "' calculated member cannot be created "
                + "because its parent is at the lowest level " + "in the "
                + parentMember.getHierarchy().getUniqueName() + " hierarchy.");
            }
          } else {
            ...
            if (hierarchy == null) {
              throw MondrianResource.instance().MdxCalculatedHierarchyError.ex(id
                .toString());
            }
            level = hierarchy.getLevels()[0];
          }
          if (parentMember != null && parentMember.isCalculated()) {
            throw Util.newError("The '" + parent
              + "' calculated member cannot be used as a parent"
              + " of another calculated member.");
          }
          Member mdxMember = level.getHierarchy().createMember(parentMember, level,
            segment.name, this);
      ...
      mdxSet = new SetBase(segments.get(0).name, null, null, exp, false, Collections
  ...
  public Object[] getChildren() {
    ...
    children[0] = exp;
    System.arraycopy(memberProperties, 0, children, 1, memberProperties.length);
    return children;
  }
  ...
  public String getName() {
    return (isMember) ? mdxMember.getName() : mdxSet.getName();
  }

  /** Returns this formula's caption. */
  public String getCaption() {
    return (isMember) ? mdxMember.getCaption() : mdxSet.getName();
  }

  /**
   * Changes the last part of the name to <code>newName</code>. For example,
   * <code>[Abc].[Def].[Ghi]</code> becomes <code>[Abc].[Def].[Xyz]</code>; and
   * the member or set is renamed from <code>Ghi</code> to <code>Xyz</code>.
   */
  void rename(String newName) {
    String oldName = getElement().getName();
    final List<Id.Segment> segments = this.id.getSegments();
    Util
      .assertTrue(segments.get(segments.size() - 1).name.equalsIgnoreCase(oldName));
  ...
  String getUniqueName() {
    return (isMember) ? mdxMember.getUniqueName() : mdxSet.getUniqueName();
  }

  OlapElement getElement() {
    return (isMember) ? (OlapElement) mdxMember : (OlapElement) mdxSet;
  }
   ...
   * 
   * @pre isMember()
   * @return Solve order, or null if SOLVE_ORDER property is not specified or is
   *         not a number or is not constant
   */
  ...
  /**
   * Returns the integer value of a given constant. If the property is not set,
   * or its value is not an integer, or its value is not a constant, returns
   * null.
   * 
   * @param name
   *          Property name
   ...
   * @param exp
   *          Expression
   * @return Result as a number, or null if the expression is not a constant or
   *         not a number.
   */
  ...
  /**
   * Deduces a formatting expression for this calculated member. First it looks
   * for properties called "format", "format_string", etc. Then it looks inside
   * the expression, and returns the formatting expression for the first member
   * it finds.
   * 
   * @param validator
   */
  ...
  /**
   * Accepts a visitor to this Formula. The default implementation dispatches to
   * the {@link MdxVisitor#visit(Formula)} method.
   * 
   * @param visitor
   *          Visitor
   */
  ...
  private static class FormatFinder extends MdxVisitorImpl {
    ...
    public Object visit(MemberExpr memberExpr) {
      ...
      if (member.isCalculated() && member instanceof RolapCalculatedMember
     ...
     * 
     * @param expr
     * @return true if there is cyclic reference in expression. This check is
