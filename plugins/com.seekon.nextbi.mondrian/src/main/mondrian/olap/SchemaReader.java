/*
// $Id: //open/mondrian/src/main/mondrian/olap/SchemaReader.java#41 $
 ...
 * 
 * <p>
 * It is generally created using {@link Connection#getSchemaReader}, but also
 * via {@link Cube#getSchemaReader(Role)}.
 * </p>
 * 
 * <p>
 * SchemaReader is deprecated for code outside of mondrian. For new code, use
 ...
 * {@link java.util.EmptyStackException} indicating that mondrian cannot deduce
 * the current locus (statement context). If you get that error, call
 ...
 * @author jhyde
 * @since Feb 24, 2003
 * @version $Id: //open/mondrian/src/main/mondrian/olap/SchemaReader.java#41 $
 */
public interface SchemaReader {
  ...
  /**
   * Returns the access-control profile that this <code>SchemaReader</code> is
   * implementing.
   */
  Role getRole();
   ...
   * @param materialize
   *          Whether to go to disk if no approximation for the count is
   *          available and the members are not in cache. If false, returns
   ...
   * control policy of this SchemaReader.
   */
  ...
  /**
   * Returns direct children of <code>member</code>, optimized for NON EMPTY.
   * <p>
   * If <code>context == null</code> then there is no context and all members
   * are returned - then its identical to {@link #getMemberChildren(Member)}. If
   * <code>context</code> is not null, the resulting members <em>may</em> be
   * restricted to those members that have a non empty row in the fact table for
   * <code>context</code>. Wether or not optimization is possible depends on the
   * SchemaReader implementation.
   */
  ...
  /**
   * Returns direct children of each element of <code>members</code> which is
   * not empty in <code>context</code>.
   * 
   * @param members
   *          Array of members
   * @param context
   ...
   * 
   * @param dataMember
   *          Data member for a member of the parent-child hierarcy
   * @param hierarchy
   *          Hierarchy
   * @param list
   *          List of members to populate
   */
  void getParentChildContributingChildren(Member dataMember, Hierarchy hierarchy,
    List<Member> list);
   ...
   * {@link Level#getDepth getDepth}()</code> for three reasons:
   * <ol>
   * <li><b>Access control</b>. The most senior <em>visible</em> member has
   * level 0. If the client is not allowed to see the "All" and "Nation" levels
   * of the "Store" hierarchy, then members of the "State" level will have depth
   * 0.</li>
   ...
   * {@link #getMemberByUniqueName(java.util.List, boolean, MatchType)} with
  ...
  /**
   * Looks up an MDX object by name, specifying how to match if no object
   ...
   */
  OlapElement lookupCompound(OlapElement parent, List<Id.Segment> names,
   ...
   * and <code>endMember</code> (inclusive) which belong to <code>level</code>.
   */
  void getMemberRange(Level level, Member startMember, Member endMember,
    List<Member> list);
   ...
   * traversal. (that is, it is an ancestor or a earlier), is a sibling, or
   * comes later in a prefix traversal.
   * 
   ...
   *         <code>m2</code>; zero if <code>m1</code> is a sibling of
   *         <code>m2</code>; a positive integer if <code>m1</code> comes later
   *         in the prefix traversal then <code>m2</code>.
   */
  int compareMembersHierarchically(Member m1, Member m2);

  /**
   * Looks up the child of <code>parent</code> called <code>name</code>, or an
   * approximation according to <code>matchType</code>, returning null if no
   * element is found.
   * 
   * @param parent
   *          Parent element to search in
  ...
  /**
   * Looks up the child of <code>parent</code> <code>name</code>, returning null
  ...
  /**
   * Returns an object which can evaluate an expression in native SQL, or null
   ...
   * @param evaluator
   *          Evaluator, provides context
   * @param calc
   */
  NativeEvaluator getNativeSetEvaluator(FunDef fun, Exp[] args, Evaluator evaluator,
    Calc calc);
   ...
   * 
   * <p>
   * It is less efficient; use this only if the operation is occurring outside
   * the context of a statement. If you get the internal error "no locus",
   * that's a sign you should use this method.
   * </p>
   * 
   ...
   * 
   * <p>
   * For example, a schema reader from the perspective of a cube will return
   * cube and schema namespaces.
   * </p>
   * 
