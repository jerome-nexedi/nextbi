/*
// $Id: //open/mondrian/src/main/mondrian/olap/type/TypeUtil.java#16 $
 ...
 * @author jhyde
 * @since Feb 17, 2005
 * @version $Id: //open/mondrian/src/main/mondrian/olap/type/TypeUtil.java#16 $
 */
public class TypeUtil {
	/**
	 * Given a set type, returns the element type. Or its element type, if it is a
	 * set type. And so on.
	 * 
	 * @param type
	 *          Type
	...
	/**
	 * Converts a type to a member type. If it is a set, strips the set. If it is
	 * a member type, returns the type unchanged. If it is a dimension, hierarchy
	 * or level type, converts it to a member type. If it is a tuple, number,
	 * string, or boolean, returns null.
	 * 
	 * @param type
	 *          Type
	...
	public static MemberType toMemberType(Type type) {
		...
		} else if (type instanceof DimensionType || type instanceof HierarchyType
	...
	/**
	 * Returns whether this type is union-compatible with another. In general, to
	 * be union-compatible, types must have the same dimensionality.
	 * 
	 * @param type1
	 *          First type
	 * @param type2
			...
			if (type2 instanceof TupleType) {
				...
				if (tupleType1.elementTypes.length == tupleType2.elementTypes.length) {
					for (int i = 0; i < tupleType1.elementTypes.length; i++) {
						if (!isUnionCompatible(tupleType1.elementTypes[i],
	 ...
	 */
	private static boolean equal(final Hierarchy hierarchy1,
			final Hierarchy hierarchy2) {
		return hierarchy1 == null || hierarchy2 == null
				|| hierarchy2.getUniqueName().equals(hierarchy1.getUniqueName());
	}
	 ...
	 * <li>Member and tuple expressions can be interpreted as a scalar value. The
	 * expression is evaluated to establish the context where a measure can be
	 * evaluated.
	 * <li>Hierarchy and dimension expressions are implicitly converted into the
	 * current member, and evaluated as above.
	...
	public static boolean couldBeMember(Type type) {
		return type instanceof MemberType || type instanceof HierarchyType
	...
	/**
	 * Returns a type sufficiently broad to hold any value of several types, but
	 * as narrow as possible. If there is no such type, returns null.
	 * 
	 * <p>
	 * The result is equivalent to calling
	 * {@link Type#computeCommonType(Type, int[])} pairwise.
	 * 
	 * @param allowConversions
	 *          Whether to allow implicit conversions
	 * @param types
	 *          Array of types
	...
	/**
	 * Returns whether we can convert an argument of a given category to a given
	 * parameter category.
	 * 
	 * @param ordinal
	 *          argument ordinal
	 ...
	 * @param to
	 *          formal parameter category
	 * @param conversions
	 *          list of implicit conversions required (out)
	 * @return whether can convert from 'from' to 'to'
	 */
	public static boolean canConvert(int ordinal, Type fromType, int to,
		...
		case Category.Empty:
			return false;
		default:
			throw Util.newInternal("unknown category " + from + " for type "
					+ fromType);
		}
	}

	static <T> T neq(T t1, T t2) {
		return t1 == null ? t2 : t2 == null ? t1 : t1.equals(t2) ? t1 : null;
	}
	...
	private static class ConversionImpl implements Resolver.Conversion {
		...
		final int to;
		/**
		 * Which argument. Arguments are 0-based, and in particular the 'this' of a
		 * call of member or method call syntax is argument 0. Argument -1 is the
		 * return.
		 */
		final int ordinal;

		/**
		 * Score of the conversion. A higher value is more onerous and therefore a
		 * call using such a conversion is less likly to be chosen.
		 */
		final int cost;
		 ...
		 *          Exception
		 */
		public ConversionImpl(int from, int to, int ordinal, int cost,
				RuntimeException e) {
			this.from = from;
			this.to = to;
		...
		public void apply(Validator validator, List<Exp> args) {
			...
			case Category.Tuple:
				switch (to) {
				case Category.Set:
					final Exp newArg = validator.validate(new UnresolvedFunCall("{}",
		...
		public String toString() {
			return "Conversion(from=" + Category.instance().getName(from) + ", to="
					+ Category.instance().getName(to) + ", ordinal=" + ordinal
					+ ", cost=" + cost + ", e=" + e + ")";
		}
	}
}
