...
public class RoleImpl implements Role {
	...
	public Access getAccess(Dimension dimension) {
		...
		if (access == Access.CUSTOM) {
			...
			for (Hierarchy hierarchy : dimension.getHierarchies()) {
				final HierarchyAccessImpl hierarchyAccess = hierarchyGrants
						.get(hierarchy);
		...
		for (Map.Entry<Cube, Access> cubeGrant : cubeGrants.entrySet()) {
			...
			for (Dimension dimension1 : dimensions) {
				...
				if (dimension instanceof RolapCubeDimension
						...
						&& !((RolapCubeDimension) dimension1).getCube().equals(
								cubeGrant.getKey())) {
					continue;
				}
	...
	public void grant(Hierarchy hierarchy, Access access, Level topLevel,
		...
		hierarchyGrants.put(hierarchy, new HierarchyAccessImpl(this, hierarchy,
				access, topLevel, bottomLevel, rollupPolicy));
		// Cascade the access right to 'custom' on the parent dim if necessary
		final Access dimAccess = toAccess(dimensionGrants.get(hierarchy
				.getDimension()));
	...
	public HierarchyAccess getAccessDetails(Hierarchy hierarchy) {
		...
		final Access schemaGrant = schemaGrants.get(hierarchy.getDimension()
				.getSchema());
		if (schemaGrant != null) {
		...
		return new HierarchyAccessImpl(this, hierarchy, hierarchyAccess, null,
				null, RollupPolicy.HIDDEN);
	}

	public Access getAccess(Level level) {
		assert level != null;
		HierarchyAccessImpl hierarchyAccess = hierarchyGrants.get(level
				.getHierarchy());
		if (hierarchyAccess != null && hierarchyAccess.access != Access.NONE) {
	...
	public void grant(Member member, Access access) {
		...
		HierarchyAccessImpl hierarchyAccess = hierarchyGrants.get(member
				.getHierarchy());
	...
	public static HierarchyAccess createAllAccess(Hierarchy hierarchy) {
		return new HierarchyAccessImpl(Util.createRootRole(hierarchy.getDimension()
				.getSchema()), hierarchy, Access.ALL, null, null,
				Role.RollupPolicy.FULL);
	}
	...
	private static class HierarchyAccessImpl implements Role.HierarchyAccess {
		...
		public HierarchyAccess clone() {
			HierarchyAccessImpl hierarchyAccess = new HierarchyAccessImpl(role,
		...
		void grant(RoleImpl role, Member member, Access access) {
			...
			if (access == Access.NONE) {
				...
				for (Member m = member.getParentMember(); m != null; m = m
						.getParentMember()) {
				...
				if (hierarchyAccess == Access.NONE) {
					...
					role.grant(hierarchy, Access.CUSTOM, topLevel, bottomLevel,
							rollupPolicy);
				}
			}
		}
	 ...
	 */
	public static abstract class DelegatingHierarchyAccess implements
			HierarchyAccess {
