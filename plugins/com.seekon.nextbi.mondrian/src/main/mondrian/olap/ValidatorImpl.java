 ...
 * 
 * <p>
 * Uses a stack to help us guess the type of our parent expression before we've
 * completely resolved our children -- necessary, unfortunately, when figuring
 * out whether the "*" operator denotes multiplication or crossjoin.
 * 
 * <p>
 * Keeps track of which nodes have already been resolved, so we don't try to
 * resolve nodes which have already been resolved. (That would not be wrong, but
 * can cause resolution to be an <code>O(2^N)</code> operation.)
 * 
 * <p>
 * The concrete implementing class needs to implement {@link #getQuery()} and
 * {@link #defineParameter(Parameter)}.
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/ValidatorImpl.java#7 $
 */
abstract class ValidatorImpl implements Validator {
  ...
  public Exp validate(Exp exp, boolean scalar) {
    ...
    try {
      ...
      throw Util.newInternal(e, "Infinite recursion encountered while validating '"
        + Util.unparse(exp) + "'");
    }
    if (resolved == null) {
      try {
  ...
  public FunDef getDef(Exp[] args, String funName, Syntax syntax) {
    ...
    final String signature = syntax.getSignature(funName, Category.Unknown, ExpBase
      .getTypes(args));
    ...
    default:
      ...
      throw MondrianResource.instance().MoreThanOneFunctionMatchesSignature.ex(
  ...
  /**
   * Returns whether the <code>k</code>th argument to a function call has to be
   * an expression.
   */
  boolean requiresExpression(UnresolvedFunCall funCall, int k) {
    ...
    // expected.
    List<Resolver> resolvers = funTable.getResolvers(funCall.getFunName(), funCall
      .getSyntax());
  ...
  public Parameter createOrLookupParam(boolean definition, String name, Type type,
    Exp defaultExp, String description) {
