...
abstract class ValidatorImpl implements Validator {
	...
	public void validate(MemberProperty memberProperty) {
		MemberProperty resolved = (MemberProperty) resolvedNodes
				.get(memberProperty);
		if (resolved != null) {
			return; // already resolved
		}
	...
	public FunDef getDef(Exp[] args, String funName, Syntax syntax) {
		...
		final String signature = syntax.getSignature(funName, Category.Unknown,
		...
		for (Resolver resolver : resolvers) {
			...
			if (def != null) {
				...
				if (conversionCost < minConversionCost) {
					...
					matchDefs.clear();
					matchDefs.add(def);
					matchConversionList = new ArrayList<Resolver.Conversion>(
							conversionList);
		...
		case 0:
			throw MondrianResource.instance().NoFunctionMatchesSignature
					.ex(signature);
		case 1:
			break;
		default:
	...
	boolean requiresExpression(UnresolvedFunCall funCall, int k) {
		...
		// expected.
		List<Resolver> resolvers = funTable.getResolvers(funCall.getFunName(),
				funCall.getSyntax());
