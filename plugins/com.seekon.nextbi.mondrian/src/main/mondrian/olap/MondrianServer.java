 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/MondrianServer.java#10 $
 * @since Jun 25, 2006
 */
public abstract class MondrianServer {
  ...
  public static MondrianServer createWithRepository(
    RepositoryContentFinder contentFinder, CatalogLocator catalogLocator) {
    return MondrianServerRegistry.INSTANCE.createWithRepository(contentFinder,
      catalogLocator);
  }
   ...
   * 
   * <p>
   * If id is null, returns the catalog-less server. (The catalog-less server
   * can also be acquired using its id.)
   * 
  ...
  public static void dispose(String instanceId) {
    final MondrianServer server = forId(instanceId);
    if (server != null) {
      server.shutdown();
    }
  }
   ...
   * {@link MondrianServer#getLockBox()} and pass in the moniker for the
   * generated lock box entry. The server will retrieve the role from the
   * moniker.
   * 
   * @param catalogName
   *          Catalog name
  ...
  public abstract OlapConnection getConnection(String catalogName,
    String schemaName, String roleName) throws SQLException, SecurityException;

  /**
   * Extended version of
   * {@link MondrianServer#getConnection(String, String, String)} taking a list
   * of properties to pass down to the native connection.
   * 
   * <p>
   * Gets a Connection given a catalog (and implicitly the catalog's data
   * source) and the name of a user role.
   ...
   */
  public abstract OlapConnection getConnection(String catalogName,
    String schemaName, String roleName, Properties props) throws SQLException,
    SecurityException;

  /**
   * Returns a list of the databases in this server. One element per database,
   * each element a map whose keys are the XMLA fields describing a data source:
   * "DataSourceName", "DataSourceDescription", "URL", etc. Unrecognized fields
   * are ignored.
   * 
  ...
  /**
   * Called when the server must terminate all background tasks and cleanup all
  ...
  public interface MondrianVersion {
     ...
     * 
     * <p>
     * For example, if the full version string is "2.3.0", the major version is
     * 2.
     * 
     ...
     * 
     * <p>
     * For example, if the full version string is "2.3.0", the minor version is
     * 3.
     * 
