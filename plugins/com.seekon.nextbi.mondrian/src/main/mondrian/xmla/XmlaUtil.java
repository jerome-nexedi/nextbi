/*
// $Id: //open/mondrian/src/main/mondrian/xmla/XmlaUtil.java#33 $
 ...
 * 
 * @author Gang Chen
 * @version $Id: //open/mondrian/src/main/mondrian/xmla/XmlaUtil.java#33 $
 */
public class XmlaUtil implements XmlaConstants {
	 ...
	 * [#x10000-#x10FFFF] S ::= (#x20 | #x9 | #xD | #xA)+ NameChar ::= Letter |
	 * Digit | '.' | '-' | '_' | ':' | CombiningChar | Extender Name ::= (Letter |
	 * '_' | ':') (NameChar)* Names ::= Name (#x20 Name)* Nmtoken ::= (NameChar)+
	 ...
	 * 
	 * <p>
	 * Quoth the <a href="http://xmla.org">XML/A specification</a>, version 1.1:
	 * <blockquote> XML does not allow certain characters as element and attribute
	 * names. XML for Analysis supports encoding as defined by SQL Server 2000 to
	 * address this XML constraint. For column names that contain invalid XML name
	 * characters (according to the XML 1.0 specification), the nonvalid Unicode
	 * characters are encoded using the corresponding hexadecimal values. These
	 * are escaped as _x<i>HHHH_</i> where <i>HHHH</i> stands for the four-digit
	 * hexadecimal UCS-2 code for the character in most-significant bit first
	 * order. For example, the name "Order Details" is encoded as
	 * Order_<i>x0020</i>_Details, where the space character is replaced by the
	 * corresponding hexadecimal code. </blockquote>
	 * 
	 * @param name
	 *          Name of XML element
	 * @return encoded name
	...
	public static Element[] filterChildElements(Element parent, String ns,
			String lname) {
		/*
		 * way too noisy if (LOGGER.isDebugEnabled()) { StringBuilder buf = new
		 * StringBuilder(100); buf.append("XmlaUtil.filterChildElements: ");
		 * buf.append(" ns=\""); buf.append(ns); buf.append("\", lname=\"");
		 * buf.append(lname); buf.append("\""); LOGGER.debug(buf.toString()); }
		 */
	...
	/**
	 * Finds root MondrianException in exception chain if exists, otherwise the
	 * input throwable.
	 * 
	 * @param throwable
	 *          Exception
	...
	/**
	 * Corrects for the differences between numeric strings arising because JDBC
	 * drivers use different representations for numbers ({@link Double} vs.
	 * {@link java.math.BigDecimal}) and these have different toString() behavior.
	 * 
	 * <p>
	 * If it contains a decimal point, then strip off trailing '0's. After
	 * stripping off the '0's, if there is nothing right of the decimal point,
	 * then strip off decimal point.
	 * 
	 * @param numericStr
	 *          Numeric string
	 ...
	 * 
	 * <p/>
	 * Leverages mondrian's implementation of the XML/A specification, and is
	 * exposed here for use by mondrian's olap4j driver.
	 * 
	 * @param connection
	 *          Connection
	 * @param methodName
	 *          Metadata method name per XMLA (e.g. "MDSCHEMA_CUBES")
	 * @param restrictionMap
	 *          Restrictions
	 * @return Set of rows and column headings
	 */
	public static MetadataRowset getMetadataRowset(
			final OlapConnection connection, String methodName,
			final Map<String, Object> restrictionMap) throws OlapException {
		...
		final Rowset rowset = rowsetDefinition.getRowset(request, new XmlaHandler(
				connectionFactory, "xmla") {
			@Override
		...
		rowset.populate(new DefaultXmlaResponse(new ByteArrayOutputStream(),
				Charset.defaultCharset().name(), Enumeration.ResponseMimeType.SOAP),
				connection, rowList);
	 ...
	 * 
	 * <p>
	 * The header can contain a list of mime types and optional qualities, for
	 ...
	 * 
	 * <p>
	 * According to the XMLA spec, it should not. But we allow the client to
	 * specify different behavior. In particular, the olap4j driver for XMLA may
	 * need to access invisible members.
	 * 
	 * <p>
	 * Returns true if the EmitInvisibleMembers property is specified and equal to
	 * "true".
	 * 
	 * @param request
	 *          XMLA request
	...
	public static boolean shouldEmitInvisibleMembers(XmlaRequest request) {
		final String value = request.getProperties().get(
	...
	/**
	 * Wrapper which indicates that a restriction is to be treated as a SQL-style
	 * wildcard match.
	 */
