/*
// $Id: //open/mondrian/src/main/mondrian/xmla/impl/DefaultSaxWriter.java#14 $
...
public class DefaultSaxWriter implements SaxWriter {
   ...
   * 
   * <p>
   * If <code>writer</code> is a {@link java.io.PrintWriter},
  ...
  private void _startElement(String namespaceURI, String localName, String qName,
    Attributes atts) {
    _checkTag();
    if (indent > 0) {
      writer.println();
  ...
  private void _characters(char ch[], int start, int length) {
    ...
    if (XOMUtil.stringHasXMLSpecials(s)) {
      ...
      /*
       * if (s.indexOf("]]>") < 0) { writer.print("<![CDATA["); writer.print(s);
       * writer.print("]]>"); } else { XMLUtil.stringEncodeXML(s, writer); }
       */
    } else {
      writer.print(s);
    }
