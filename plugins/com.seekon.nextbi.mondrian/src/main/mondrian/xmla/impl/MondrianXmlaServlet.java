 ...
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/xmla/impl/MondrianXmlaServlet.
 *          java#1 $
 * @author jhyde
 */
public class MondrianXmlaServlet extends DefaultXmlaServlet {
	...
	protected XmlaHandler.ConnectionFactory createConnectionFactory(
		...
		if (server == null) {
			...
			server = MondrianServer.createWithRepository(contentFinder,
					catalogLocator);
		}
	 ...
	 * 
	 * <p>
	 * If there is an initParameter called "DataSourcesConfig" get its value,
	 * replace any "${key}" content with "value" where "key/value" are System
	 * properties, and try to create a URL instance out of it. If that fails, then
	 * assume its a real filepath and if the file exists then create a URL from it
	 * (but only if the file exists). If there is no initParameter with that name,
	 * then attempt to find the file called "datasources.xml" under "WEB-INF/" and
	 * if it exists, use it.
	 * 
	 * @param servletConfig
	 *          Servlet config
	...
	protected String makeDataSourcesUrl(ServletConfig servletConfig) {
		String paramValue = servletConfig
				.getInitParameter(PARAM_DATASOURCES_CONFIG);
		// if false, then do not throw exception if the file/url
		// can not be found
		boolean optional = getBooleanInitParameter(servletConfig,
		...
		try {
			if (paramValue == null) {
				...
				try {
					...
					// not a valid url
					mue = e;
					dataSourcesConfigUrl = this.getClass().getResource(paramValue);
				}
