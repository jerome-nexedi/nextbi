/*
// $Id: //open/mondrian/src/main/mondrian/xmla/impl/Olap4jXmlaServlet.java#5 $
 ...
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/xmla/impl/Olap4jXmlaServlet.java#5
 *          $
 * @author Julian Hyde
...
public class Olap4jXmlaServlet extends DefaultXmlaServlet {
	private static final Logger LOGGER = Logger
			.getLogger(Olap4jXmlaServlet.class);
	...
	private static final int DEFAULT_IDLE_CONNECTIONS_TIMEOUT_MS = 5 * 60 * 1000;
	...
	protected XmlaHandler.ConnectionFactory createConnectionFactory(
		...
		final String olap4jDriverClassName = servletConfig
				.getInitParameter(OLAP_DRIVER_CLASS_NAME_PARAM);
		final String olap4jDriverConnectionString = servletConfig
				.getInitParameter(OLAP_DRIVER_CONNECTION_STRING_PARAM);
		final String olap4jUsePreConfiguredDiscoverDatasourcesRes = servletConfig
				.getInitParameter(OLAP_DRIVER_PRECONFIGURED_DISCOVER_DATASOURCES_RESPONSE);
		boolean hardcodedDiscoverDatasources = olap4jUsePreConfiguredDiscoverDatasourcesRes != null
				&& Boolean.parseBoolean(olap4jUsePreConfiguredDiscoverDatasourcesRes);

		final String idleConnTimeoutStr = servletConfig
				.getInitParameter(OLAP_DRIVER_IDLE_CONNECTIONS_TIMEOUT_MINUTES);
		final int idleConnectionsCleanupTimeoutMs = idleConnTimeoutStr != null ? Integer
				.parseInt(idleConnTimeoutStr) * 60 * 1000
				: DEFAULT_IDLE_CONNECTIONS_TIMEOUT_MS;

		final String maxNumConnPerUserStr = servletConfig
				.getInitParameter(OLAP_DRIVER_MAX_NUM_CONNECTIONS_PER_USER);
		int maxNumConnectionsPerUser = maxNumConnPerUserStr != null ? Integer
				.parseInt(maxNumConnPerUserStr) : 1;
		try {
			Map<String, String> connectionProperties = getOlap4jConnectionProperties(
					servletConfig, OLAP_DRIVER_CONNECTION_PROPERTIES_PREFIX);
			final Map<String, Object> ddhcRes;
			if (hardcodedDiscoverDatasources) {
				ddhcRes = getDiscoverDatasourcesPreConfiguredResponse(servletConfig);
			} else {
				ddhcRes = null;
			}

			return new Olap4jPoolingConnectionFactory(olap4jDriverClassName,
					olap4jDriverConnectionString, connectionProperties,
					idleConnectionsCleanupTimeoutMs, maxNumConnectionsPerUser, ddhcRes);
		} catch (Exception ex) {
			String msg = "Exception [" + ex + "] while trying to create "
					+ "olap4j connection to [" + olap4jDriverConnectionString
					+ "] using driver " + "[" + olap4jDriverClassName + "]";
			LOGGER.error(msg, ex);
	...
	private static Map<String, Object> getDiscoverDatasourcesPreConfiguredResponse(
		...
		foo(map, "DataSourceDescription", servletConfig, "dataSourceDescription");
	...
	private static void foo(Map<String, Object> map, String targetProp,
			ServletConfig servletConfig, String sourceProp) {
		final String value = servletConfig
				.getInitParameter(OLAP_DRIVER_PRECONFIGURED_DISCOVER_DATASOURCES_PREFIX
						+ sourceProp);
		map.put(targetProp, value);
	}

	private static class Olap4jPoolingConnectionFactory implements
		 ...
		 * 
		 * @param olap4jDriverClassName
		 *          Driver class name
		 * @param olap4jDriverConnectionString
		 *          Connect string
		 * @param connectionProperties
		 *          Connection properties
		 * @param maxPerUserConnectionCount
		 *          max number of connections to create for every different username
		 * @param idleConnectionsCleanupTimeoutMs
		 *          pooled connections inactive for longer than this period of time
		 *          can be cleaned up
		 * @param discoverDatasourcesResponse
		 *          Pre-configured response to DISCOVER_DATASOURCES request, or null
		...
		public OlapConnection getConnection(String catalog, String schema,
			...
			synchronized (datasourcesPool) {
				...
				if (bds == null) {
					...
					bds.setMinEvictableIdleTimeMillis(idleConnectionsCleanupTimeoutMs);
			...
			OlapConnection olapConnection = unwrap(underlyingOlapConnection,
					OlapConnection.class);

			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Obtained connection object [" + olapConnection
	...
	/**
	 * Obtains connection properties from the ServletConfig init parameters and
	 * from System properties.
	 * 
	 * <p>
	 * The properties found in the System properties override the ones in the
	 * ServletConfig.
	 * 
	 * <p>
	 * copies the values of init parameters / properties which start with the
	 * given prefix to a target Map object stripping out the configured prefix
	 ...
	 * 
	 * <p>
	 * This will result in a connection properties object with entries
	...
	private static Map<String, String> getOlap4jConnectionProperties(
			final ServletConfig servletConfig, final String prefix) {
		...
		Bug.olap4jUpgrade("move Util.toMap to Olap4jUtil; this file should no longer "
	...
	/**
	 * Returns something that implements {@link OlapConnection} but still behaves
	 * as the wrapper returned by the connection pool.
	 * 
	 * <p>
	 * In other words we want the "close" method to play nice and do all the
	...
	private static OlapConnection createDelegatingOlapConnection(
			final Connection connection, final OlapConnection olapConnection) {
		return (OlapConnection) Proxy.newProxyInstance(olapConnection.getClass()
				.getClassLoader(), new Class[] { OlapConnection.class },
				new InvocationHandler() {
					public Object invoke(Object proxy, Method method, Object[] args)
							throws Throwable {
						if ("unwrap".equals(method.getName())
								|| OlapConnection.class.isAssignableFrom(method
										.getDeclaringClass())) {
