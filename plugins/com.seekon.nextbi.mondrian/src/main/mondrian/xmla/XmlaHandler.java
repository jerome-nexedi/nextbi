/*
// $Id: //open/mondrian/src/main/mondrian/xmla/XmlaHandler.java#81 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/xmla/XmlaHandler.java#81 $
 * @since 27 April, 2003
 */
public class XmlaHandler {
	...
	public OlapConnection getConnection(XmlaRequest request,
		...
		LOGGER.debug("Creating new connection for user [" + request.getUsername()
		...
		final String catalogName = request.getProperties().get(
				PropertyDefinition.Catalog.name());

		return getConnection(databaseName, catalogName, request.getRoleName(),
				props);
	}
	...
	private static final String EMPTY_ROW_SET_XML_SCHEMA = computeEmptyXsd(SetType.ROW_SET);

	private static final String MD_DATA_SET_XML_SCHEMA = computeXsd(SetType.MD_DATA_SET);

	private static final String EMPTY_MD_DATA_SET_XML_SCHEMA = computeEmptyXsd(SetType.MD_DATA_SET);
	 ...
	 * with value java.lang.Integer (and failing that an XSD_LONG (java.lang.Long)
	 * or XSD_INTEGER (java.math.BigInteger)). Worst case is the value loses
	 * precision with any integral representation and must be returned as a
	 * decimal type (Double or java.math.BigDecimal).
	 * <p>
	...
	static class ValueInfo {
		...
		static String getValueTypeHint(final String dataType) {
			if (dataType != null) {
				return (dataType.equals("Integer")) ? XSD_INT : ((dataType
						.equals("Numeric")) ? XSD_DOUBLE : XSD_STRING);
			} else {
				return null;
			}
		}
		...
		ValueInfo(final String dataType, final Object inputValue) {
			...
			if (valueTypeHint != null) {
				...
				if (valueTypeHint.equals(XSD_STRING)) {
					...
					} else if (inputValue instanceof Byte || inputValue instanceof Short
							|| inputValue instanceof Integer || inputValue instanceof Long) {
	...
	public void process(XmlaRequest request, XmlaResponse response)
		...
		default:
			throw new XmlaException(CLIENT_FAULT_FC, HSB_BAD_METHOD_CODE,
					HSB_BAD_METHOD_FAULT_FS, new IllegalArgumentException(
	...
	private void checkFormat(XmlaRequest request) throws XmlaException {
		...
		if (request.isDrillThrough()) {
			...
			final String formatName = properties
					.get(PropertyDefinition.Format.name());
			if (formatName != null) {
				Format format = getFormat(request, null);
			...
			final String axisFormatName = properties
					.get(PropertyDefinition.AxisFormat.name());
			if (axisFormatName != null) {
				AxisFormat axisFormat = Util.lookup(AxisFormat.class, axisFormatName,
						null);
	...
	private void execute(XmlaRequest request, XmlaResponse response)
		...
		final String contentName = properties
				.get(PropertyDefinition.Content.name());
		Content content = Util.lookup(Content.class, contentName,
				responseMimeType == Enumeration.ResponseMimeType.JSON ? Content.Data
						: Content.DEFAULT);
		...
		try {
			...
			writer.startElement(prefix + ":ExecuteResponse", "xmlns:" + prefix,
					NS_XMLA);
			writer.startElement(prefix + ":return");
			boolean rowset = request.isDrillThrough()
					|| Format.Tabular.name().equals(
							request.getProperties().get(PropertyDefinition.Format.name()));
			writer.startElement("root", "xmlns", result == null ? NS_XMLA_EMPTY
					: rowset ? NS_XMLA_ROWSET : NS_XMLA_MDDATASET, "xmlns:xsi", NS_XSI,
			...
			try {
				...
				throw new XmlaException(SERVER_FAULT_FC, HSB_EXECUTE_UNPARSE_CODE,
						HSB_EXECUTE_UNPARSE_FAULT_FS, t);
			} finally {
	...
	static void writeDatasetXmlSchema(SaxWriter writer, SetType settype) {
		String setNsXmla = (settype == SetType.ROW_SET) ? NS_XMLA_ROWSET
				: NS_XMLA_MDDATASET;

		writer.startElement("xsd:schema", "xmlns:xsd", NS_XSD, "targetNamespace",
				setNsXmla, "xmlns", setNsXmla, "xmlns:xsi", NS_XSI, "xmlns:sql",
				NS_XML_SQL, "elementFormDefault", "qualified");
		...
		writer.element("xsd:element", "name", "DisplayInfo", "type",
				XSD_UNSIGNED_INT);
		writer.startElement("xsd:sequence", "maxOccurs", "unbounded", "minOccurs",
				0);
		writer.element("xsd:any", "processContents", "lax", "maxOccurs",
				"unbounded");
		...
		{ // <CubeInfo>
			...
			{ // <Cube>
				writer.startElement("xsd:element", "name", "Cube", "maxOccurs",
						"unbounded");
				writer.startElement("xsd:complexType");
				writer.startElement("xsd:sequence");
		...
		{ // <AxesInfo>
			...
			writer.startElement("xsd:sequence");
			{ // <AxisInfo>
				writer.startElement("xsd:element", "name", "AxisInfo", "maxOccurs",
						"unbounded");
				writer.startElement("xsd:complexType");
				writer.startElement("xsd:sequence");

				{ // <HierarchyInfo>
					writer.startElement("xsd:element", "name", "HierarchyInfo",
					...
					writer.element("xsd:element", "name", "DisplayInfo", "type",
							"PropType", "minOccurs", 0, "maxOccurs", "unbounded");
					if (false)
						writer.element("xsd:element", "name", "PARENT_MEMBER_NAME", "type",
								"PropType", "minOccurs", 0, "maxOccurs", "unbounded");
					writer.endElement(); // xsd:sequence

					// This is the Depth element for JPivot??
					writer.startElement("xsd:sequence");
					writer.element("xsd:any", "processContents", "lax", "minOccurs", 0,
							"maxOccurs", "unbounded");
					writer.endElement(); // xsd:sequence
		...
		{ // <CellInfo>
			...
			writer.startElement("xsd:complexType");
			writer.startElement("xsd:sequence");
			writer.startElement("xsd:sequence", "minOccurs", 0, "maxOccurs",
					"unbounded");
			...
			writer.element("xsd:element", "name", "NonEmptyBehavior", "type",
					"PropType");
		...
		{ // <Axis>
			...
			writer.startElement("xsd:complexType");
			writer.startElement("xsd:choice", "minOccurs", 0, "maxOccurs",
					"unbounded");
			writer.element("xsd:element", "name", "CrossProduct", "type",
					"CrossProductType");
			writer.element("xsd:element", "name", "Tuples", "type", "TuplesType");
			writer.element("xsd:element", "name", "Members", "type", "MembersType");
			writer.endElement(); // xsd:choice
		...
		writer.startElement("xsd:sequence");
		{ // <Cell>
			writer.startElement("xsd:element", "name", "Cell", "minOccurs", 0,
					"maxOccurs", "unbounded");
			writer.startElement("xsd:complexType");
			writer.startElement("xsd:sequence", "maxOccurs", "unbounded");
			writer.startElement("xsd:choice");
			...
			writer.element("xsd:element", "name", "BackColor", "type",
					XSD_UNSIGNED_INT);
			writer.element("xsd:element", "name", "ForeColor", "type",
					XSD_UNSIGNED_INT);
			writer.element("xsd:element", "name", "FontName", "type", XSD_STRING);
			writer.element("xsd:element", "name", "FontSize", "type",
					"xsd:unsignedShort");
			writer.element("xsd:element", "name", "FontFlags", "type",
					XSD_UNSIGNED_INT);
			writer.element("xsd:element", "name", "FormatString", "type", XSD_STRING);
			writer.element("xsd:element", "name", "NonEmptyBehavior", "type",
					"xsd:unsignedShort");
			writer.element("xsd:element", "name", "SolveOrder", "type",
					XSD_UNSIGNED_INT);
			writer.element("xsd:element", "name", "Updateable", "type",
					XSD_UNSIGNED_INT);
			writer
					.element("xsd:element", "name", "Visible", "type", XSD_UNSIGNED_INT);
			writer.element("xsd:element", "name", "Expression", "type", XSD_STRING);
			writer.endElement(); // xsd:choice
			writer.endElement(); // xsd:sequence
			writer.element("xsd:attribute", "name", "CellOrdinal", "type",
					XSD_UNSIGNED_INT, "use", "required");
			writer.endElement(); // xsd:complexType
	...
	private QueryResult executeDrillThroughQuery(XmlaRequest request)
		...
		final String advancedFlag = properties.get(PropertyDefinition.AdvancedFlag
				.name());
		final boolean advanced = Boolean.parseBoolean(advancedFlag);
		final boolean enableRowCount = MondrianProperties.instance().EnableTotalCount
				.booleanValue();
		...
		try {
			...
			resultSet = getExtra(connection).executeDrillthrough(statement,
					request.getStatement(), advanced, tabFields, rowCountSlot);
			int rowCount = enableRowCount ? rowCountSlot[0] : -1;
			return new TabularRowSet(resultSet, rowCount);
		} catch (XmlaException xex) {
			throw xex;
		} catch (SQLException sqle) {
			throw new XmlaException(SERVER_FAULT_FC, HSB_DRILL_THROUGH_SQL_CODE,
					HSB_DRILL_THROUGH_SQL_FAULT_FS, Util.newError(sqle,
							"Error in drill through"));
		} catch (RuntimeException e) {
			// NOTE: One important error is "cannot drill through on the cell"
			throw new XmlaException(SERVER_FAULT_FC, HSB_DRILL_THROUGH_SQL_CODE,
					HSB_DRILL_THROUGH_SQL_FAULT_FS, e);
		} finally {
			if (resultSet != null) {
				try {
					resultSet.close();
	...
	static class TabularRowSet implements QueryResult {
		 ...
		 *          Result set
		 * @param totalCount
		 *          Total number of rows. If >= 0, writes the "totalCount" attribute
		 *          into the XMLA response.
		 * 
		 * @throws SQLException
		 *           on error
		 */
		public TabularRowSet(ResultSet rs, int totalCount) throws SQLException {
			...
			for (int i = 0; i < columnCount; i++) {
				columns.add(new Column(md.getColumnLabel(i + 1), md
						.getColumnType(i + 1), md.getScale(i + 1)));
			}
		 ...
		 * 
		 * @param tableFieldMap
		 *          Map from table name to a list of the names of the fields in the
		 *          table
		 * @param tableList
		 *          List of table names
		 */
		public TabularRowSet(Map<String, List<String>> tableFieldMap,
			...
			for (String tableName : tableList) {
				...
				for (String fieldName : fieldNames) {
					columns.add(new Column(tableName + "." + fieldName, Types.VARCHAR, // don't
																																							// know
																																							// the
																																							// real
																																							// type
		...
		public void unparse(SaxWriter writer) throws SAXException {
			...
			for (Object[] row : rows) {
				...
				for (int i = 0; i < row.length; i++) {
					writer.startElement(columns.get(i).encodedName, new Object[] {
							"xsi:type", columns.get(i).xsdType });
					Object value = row[i];
					if (value == null) {
		...
		public void metadata(SaxWriter writer) {
			writer.startElement("xsd:schema", "xmlns:xsd", NS_XSD, "targetNamespace",
					NS_XMLA_ROWSET, "xmlns", NS_XMLA_ROWSET, "xmlns:xsi", NS_XSI,
			...
			{ // <root>
				...
				writer.startElement("xsd:complexType");
				writer.startElement("xsd:sequence");
				writer.element("xsd:element", "maxOccurs", "unbounded", "minOccurs", 0,
						"name", "row", "type", "row");
				writer.endElement(); // xsd:sequence
				writer.endElement(); // xsd:complexType
				writer.endElement(); // xsd:element name=root
			...
			{ // xsd:complexType name="row"
				...
				writer.startElement("xsd:sequence");
				for (Column column : columns) {
					writer.element("xsd:element", "minOccurs", 0, "name",
							column.encodedName, "sql:field", column.name, "type",
							column.xsdType);
				}
	...
	private static String sqlToXsdType(final int sqlType, final int scale) {
		...
		case Types.DECIMAL:
			/*
			 * Oracle reports all numbers as NUMERIC. We check the scale of the column
			 * and pick the right XSD type.
			 */
			if (scale == 0) {
				return XSD_INT;
			} else {
	...
	private QueryResult executeQuery(XmlaRequest request) throws XmlaException {
		...
		try {
			...
			try {
				...
				throw new XmlaException(CLIENT_FAULT_FC, HSB_PARSE_QUERY_CODE,
						HSB_PARSE_QUERY_FAULT_FS, ex);
			}
			try {
				...
				if (format == Format.Multidimensional) {
					dataSet = new MDDataSet_Multidimensional(cellSet,
				...
				throw new XmlaException(SERVER_FAULT_FC, HSB_EXECUTE_QUERY_CODE,
						HSB_EXECUTE_QUERY_FAULT_FS, ex);
			}
		} finally {
			if (!success) {
				if (cellSet != null) {
	...
	private static Content getContent(XmlaRequest request) {
		final String contentName = request.getProperties().get(
	...
	static abstract class MDDataSet implements QueryResult {
		...
		protected static final List<Property> cellProps = Arrays.asList(
		...
		protected static final List<StandardCellProperty> cellPropLongs = Arrays
	...
	static class MDDataSet_Multidimensional extends MDDataSet {
		...
		protected MDDataSet_Multidimensional(CellSet cellSet,
				boolean omitDefaultSlicerInfo, boolean json) throws SQLException {
			super(cellSet);
		...
		private void olapInfo(SaxWriter writer) throws OlapException {
			...
			if (omitDefaultSlicerInfo) {
				...
				List<Dimension> unseenDimensionList = new ArrayList<Dimension>(
						cube.getDimensions());
			...
			cellProperty(writer, StandardCellProperty.FORMATTED_VALUE, true,
					"FmtValue");
			cellProperty(writer, StandardCellProperty.FORMAT_STRING, true,
					"FormatString");
		...
		private void cellProperty(SaxWriter writer,
				StandardCellProperty cellProperty, boolean evenEmpty, String elementName) {
			if (extra.shouldReturnCellProperty(cellSet, cellProperty, evenEmpty)) {
				writer.element(elementName, "name", cellProperty.getName());
			}
		}

		private List<Hierarchy> axisInfo(SaxWriter writer, CellSetAxis axis,
				String axisName) {
		...
		private void writeHierarchyInfo(SaxWriter writer,
				List<Hierarchy> hierarchies, List<Property> props) {
		...
		private void axes(SaxWriter writer) throws OlapException {
			...
			if (omitDefaultSlicerInfo) {
				...
				axis(writer, slicerAxis, getProps(slicerAxis.getAxisMetaData()),
						"SlicerAxis");
			} else {
				...
				final List<Member> slicerMembers = slicerPositions.isEmpty() ? Collections
						.<Member> emptyList() : slicerPositions.get(0).getMembers();
				for (Hierarchy hierarchy : hierarchies) {
					...
					if (member != null) {
						if (positionMember != null) {
							writeMember(writer, positionMember, null, slicerPositions.get(0),
									indexPosition, getProps(slicerAxis.getAxisMetaData()));
						} else {
		...
		private void axis(SaxWriter writer, CellSetAxis axis, List<Property> props,
		...
		private void writeMember(SaxWriter writer, Member member,
				Position prevPosition, Position nextPosition, int k,
				List<Property> props) throws OlapException {
			writer.startElement("Member", "Hierarchy", member.getHierarchy()
					.getName());
			for (Property prop : props) {
				...
				if (longProp == StandardMemberProperty.DISPLAY_INFO) {
					Integer childrenCard = (Integer) member
							.getPropertyValue(StandardMemberProperty.CHILDREN_CARDINALITY);
					value = calculateDisplayInfo(prevPosition, nextPosition, member, k,
							childrenCard);
		...
		private void slicerAxis(SaxWriter writer, Member member,
			...
			for (Property prop : props) {
				...
				if (longProp == StandardMemberProperty.DISPLAY_INFO) {
					...
					/*
					 * int displayInfo = calculateDisplayInfo((j == 0 ? null : positions[j
					 * - 1]), (j + 1 == positions.length ? null : positions[j + 1]),
		...
		private int calculateDisplayInfo(Position prevPosition,
				Position nextPosition, Member currentMember, int memberOrdinal,
				int childrenCount) {
		...
		private void recurse(SaxWriter writer, List<Integer> pos, int axisOrdinal,
				int[] cellOrdinal) {
			if (axisOrdinal < 0) {
		...
		private void emitCell(SaxWriter writer, List<Integer> pos, int ordinal) {
			...
			for (int i = 0; i < cellProps.size(); i++) {
				...
				if (!json && cellPropLong == StandardCellProperty.VALUE) {
					...
					final String dataType = (String) cell
							.getPropertyValue(StandardCellProperty.DATATYPE);
					...
					writer
							.startElement(cellProps.get(i).getName(), "xsi:type", valueType);
					writer.characters(valueString);
					writer.endElement();
				} else {
	 ...
	 * a property (e.g. [Store].[Store State].[MEMBER_UNIQUE_NAME]) in a flattened
	 * dataset.
	 */
	static class CellColumnHandler extends ColumnHandler {
		...
		public void metadata(SaxWriter writer) {
			writer.element("xsd:element", "minOccurs", 0, "name", encodedName,
					"sql:field", name);
		}
	...
	static class MemberColumnHandler extends ColumnHandler {
		...
		public void metadata(SaxWriter writer) {
			writer.element("xsd:element", "minOccurs", 0, "name", encodedName,
					"sql:field", name, "type", XSD_STRING);
		}
	...
	static class MDDataSet_Tabular extends MDDataSet {
		...
		private static final List<Property> MemberCaptionIdArray = Collections
				.<Property> singletonList(StandardMemberProperty.MEMBER_CAPTION);
		...
		public MDDataSet_Tabular(CellSet cellSet) {
			...
			if (!empty) {
				for (int i = axes.size() - 1; i > 0; i--) {
					...
					for (Position position : positions) {
						...
						for (Member member : position.getMembers()) {
							if (jj == 0
									|| member.getLevel().getDepth() > levels[memberOrdinal]
											.getDepth()) {
					...
					for (int j = z0; j < memberOrdinal; j++) {
						...
						for (int k = 0; k <= level.getDepth(); k++) {
							...
							for (Property dimProp : dimProps) {
								columnHandlerList.add(new MemberColumnHandler(dimProp, level2,
										j));
							}
						}
					}
			...
			this.columnHandlers = columnHandlerList
					.toArray(new ColumnHandler[columnHandlerList.size()]);
		}
		...
		private void iterate(SaxWriter writer, int axis, final int xxx)
				throws OlapException {
			final List<Position> positions = cellSet.getAxes().get(axis)
					.getPositions();
	...
	private void discover(XmlaRequest request, XmlaResponse response)
			throws XmlaException {
		final RowsetDefinition rowsetDefinition = RowsetDefinition.valueOf(request
				.getRequestType());
		Rowset rowset = rowsetDefinition.getRowset(request, this);

		Format format = getFormat(request, Format.Tabular);
		if (format != Format.Tabular) {
			throw new XmlaException(CLIENT_FAULT_FC, HSB_DISCOVER_FORMAT_CODE,
					HSB_DISCOVER_FORMAT_FAULT_FS, new UnsupportedOperationException(
		...
		writer.startElement(prefix + ":DiscoverResponse", "xmlns:" + prefix,
				NS_XMLA);
		writer.startElement(prefix + ":return");
		writer.startElement("root", "xmlns", NS_XMLA_ROWSET, "xmlns:xsi", NS_XSI,
				"xmlns:xsd", NS_XSD, "xmlns:EX", NS_XMLA_EX);
		...
		try {
			...
			throw new XmlaException(SERVER_FAULT_FC, HSB_DISCOVER_UNPARSE_CODE,
					HSB_DISCOVER_UNPARSE_FAULT_FS, t);
		} finally {
	 ...
	 *           If error occurs
	 */
	protected OlapConnection getConnection(String catalog, String schema,
			final String role) throws XmlaException {
	 ...
	 */
	protected OlapConnection getConnection(String catalog, String schema,
			final String role, Properties props) throws XmlaException {
		try {
			return connectionFactory.getConnection(catalog, schema, role, props);
		} catch (SecurityException e) {
			throw new XmlaException(CLIENT_FAULT_FC, HSB_ACCESS_DENIED_CODE,
					HSB_ACCESS_DENIED_FAULT_FS, e);
		} catch (SQLException e) {
	...
	/**
	 * Extra support for XMLA server. If a connection provides this interface, the
	 * XMLA server will call methods in this interface instead of relying on the
	 * core olap4j interface.
	 * 
	 * <p>
	 * The {@link mondrian.xmla.XmlaHandler.XmlaExtraImpl} class provides a
	 * default implementation that uses the olap4j interface exclusively.
	 */
	public interface XmlaExtra {

		ResultSet executeDrillthrough(OlapStatement olapStatement, String mdx,
				boolean advanced, String tabFields, int[] rowCountSlot)
				throws SQLException;
		 ...
		 */
		boolean shouldReturnCellProperty(CellSet cellSet, Property cellProperty,
				boolean evenEmpty);

		/**
		 * Returns a list of names of roles in the given schema to which the current
		 * user belongs.
		 * 
		 * @param schema
		 *          Schema
		...
		/**
		 * Returns a list of the data sources in this server. One element per data
		 * source, each element a map whose keys are the XMLA fields describing a
		 * data source: "DataSourceName", "DataSourceDescription", "URL", etc.
		...
		class FunctionDefinition {
			...
			public FunctionDefinition(String functionName, String description,
					String parameterList, int returnType, int origin,
					String interfaceName, String caption) {
	...
	private static class XmlaExtraImpl implements XmlaExtra {
		public ResultSet executeDrillthrough(OlapStatement olapStatement,
				String mdx, boolean advanced, String tabFields, int[] rowCountSlot)
				throws SQLException {
		...
		public void getSchemaFunctionList(List<FunctionDefinition> funDefs,
				Schema schema, Util.Functor1<Boolean, String> functionFilter) {
		...
		public boolean shouldReturnCellProperty(CellSet cellSet,
				Property cellProperty, boolean evenEmpty) {
			return true;
		}
		...
		public List<Map<String, Object>> getDataSources(OlapConnection connection)
			...
			return Collections.singletonList(Olap4jUtil.mapOf("DataSourceName",
					(Object) olapDb.getName(), "DataSourceDescription",
					olapDb.getDescription(), "URL", olapDb.getURL(), "DataSourceInfo",
					olapDb.getDataSourceInfo(), "ProviderName", olapDb.getProviderName(),
					"ProviderType", providerTypes, "AuthenticationMode", modes));
		}
	}
	 ...
	 * {@link javax.sql.DataSource} or a connect string, but it is important that
	 * the connection is assigned to the correct catalog, schema and role
	...
	public interface ConnectionFactory {
		 ...
		 * @param props
		 *          Properties to be passed to the underlying native driver.
		 * @return An OlapConnection object.
		 * @throws SQLException
		 *           on error
		 */
		OlapConnection getConnection(String catalog, String schema,
				String roleName, Properties props) throws SQLException;

		/**
		 * Returns a map of property name-value pairs with which to populate the
		 * response to the DISCOVER_DATASOURCES request.
		 * 
		 * <p>
		 * Properties correspond to the columns of that request: ""DataSourceName",
		 * et cetera.
		 * </p>
		 * 
		 * <p>
		 * Returns null if there is no pre-configured response; in which case, the
		 * driver will have to connect to get a response.
		 * </p>
		 * 
		 * @return Column names and values for the DISCOVER_DATASOURCES response
		 */
