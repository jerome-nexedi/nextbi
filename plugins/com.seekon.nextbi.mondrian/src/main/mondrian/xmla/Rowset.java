...
abstract class Rowset implements XmlaConstants {
	...
	Rowset(RowsetDefinition definition, XmlaRequest request, XmlaHandler handler) {
		...
		this.restrictedColumns = list.toArray(new RowsetDefinition.Column[list
				.size()]);
		boolean deep = false;
		for (Map.Entry<String, String> propertyEntry : properties.entrySet()) {
			String propertyName = propertyEntry.getKey();
			final PropertyDefinition propertyDef = Util.lookup(
					PropertyDefinition.class, propertyName);
			if (propertyDef == null) {
				throw Util.newError("Rowset '" + definition.name()
	...
	protected abstract void populateImpl(XmlaResponse response,
			OlapConnection connection, List<Row> rows) throws XmlaException,
			SQLException;
	...
	protected void emit(Row row, XmlaResponse response) throws XmlaException,
		...
		for (RowsetDefinition.Column column : rowsetDefinition.columnDefinitions) {
			...
			if (value == null) {
				if (!column.nullable) {
					throw new XmlaException(CLIENT_FAULT_FC,
							HSB_BAD_NON_NULLABLE_COLUMN_CODE,
							HSB_BAD_NON_NULLABLE_COLUMN_FAULT_FS,
							Util.newInternal("Value required for column " + column.name
	...
	protected Util.Functor1<Boolean, Catalog> catNameCond() {
		...
		final String catalogName = properties
