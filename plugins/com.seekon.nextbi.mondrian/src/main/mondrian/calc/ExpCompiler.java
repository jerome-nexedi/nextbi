/*
// $Id: //open/mondrian/src/main/mondrian/calc/ExpCompiler.java#19 $
...
/**
 * Mediates the compilation of an expression ({@link mondrian.olap.Exp}) into a
 * compiled expression ({@link Calc}).
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/calc/ExpCompiler.java#19 $
 * @since Sep 28, 2005
 */
public interface ExpCompiler {
	 ...
	 * 
	 * <p>
	 * If <code>resultType</code> is not null, casts the expression to that type.
	 * Throws an exception if that conversion is not allowed by the type system.
	 * 
	 * <p>
	 * The <code>preferredResultStyles</code> parameter specifies a list of
	 * desired result styles. It must not be null, but may be empty.
	 * 
	 * @param exp
	 *          Expression
	 * 
	 * @param resultType
	 *          Desired result type, or null to use expression's current type
	 * 
	 * @param preferredResultStyles
	 *          List of result types, in descending order of preference. Never
	 *          null.
	 * 
	 * @return Compiled expression, or null if none can satisfy
	 */
	Calc compileAs(Exp exp, Type resultType,
	...
	/**
	 * Compiles an expression which yields an <code>int</code> result. The
	 * expression is implicitly converted into a scalar.
	 */
	IntegerCalc compileInteger(Exp exp);

	/**
	 * Compiles an expression which yields a {@link String} result. The expression
	 * is implicitly converted into a scalar.
	 */
	StringCalc compileString(Exp exp);

	/**
	 * Compiles an expression which yields a {@link java.util.Date} result. The
	 * expression is implicitly converted into a scalar.
	 */
	DateTimeCalc compileDateTime(Exp exp);

	/**
	 * Compiles an expression which yields an immutable {@link TupleList} result.
	 * 
	 * <p>
	 * Always equivalent to <code>{@link #compileList}(exp, false)</code>.
	 */
	 ...
	 * 
	 * <p>
	 * Such an expression is generally a list of {@link Member} objects or a list
	 * of tuples (each represented by a {@link Member} array).
	 * 
	 * <p>
	 * See {@link #compileList(mondrian.olap.Exp)}.
	 * 
	 * @param exp
	 *          Expression
	 * @param mutable
	 ...
	 * automatically apply that member or tuple to the current dimensional context
	 * and return the value of the current measure.
	 * 
	 * @param exp
	 *          Expression
	 * @param specific
	 *          Whether to try to use the specific compile method for scalar
	 *          types. For example, if <code>specific</code> is true and
	...
	/**
	 * Returns a list of the {@link ResultStyle}s acceptable to the caller.
	 */
	List<ResultStyle> getAcceptableResultStyles();

	/**
	 * The <code>ExpCompiler.Factory</code> is used to access
	 * <code>ExpCompiler</code> implementations. Each call returns a new instance.
	 * This factory supports overriding the default instance by use of a
	 * <code>ThreadLocal</code> and by defining a <code>System</code> property
	 * with the <code>ExpCompiler</code> class name.
	 */
	public static final class Factory extends ObjectFactory<ExpCompiler> {
		...
		static {
			factory = new Factory();
			CLASS_ARRAY = new Class[] { Evaluator.class, Validator.class,
					ResultStyle[].class, };
		}

		/**
		 * Create a <code>ExpCompiler</code> instance, each call returns a new
		 * compiler.
		 * 
		 * @param evaluator
		 *          the <code>Evaluator</code> to use with the compiler
		 * @param validator
		...
		public static ExpCompiler getExpCompiler(final Evaluator evaluator,
				final Validator validator) throws CreationException {
		 ...
		 *          the <code>Validator</code> to use with the compiler
		 * @param resultStyles
		 *          the initial <code>ResultStyle</code> array for the compiler
		 * @return the new <code>ExpCompiler</code> compiler
		 * @throws CreationException
		 *           if the compiler can not be created
		 */
		public static ExpCompiler getExpCompiler(final Evaluator evaluator,
				final Validator validator, final List<ResultStyle> resultStyles)
				throws CreationException {
			return factory.getObject(CLASS_ARRAY, new Object[] { evaluator,
					validator, resultStyles });
		}

		/**
		 * <code>ThreadLocal</code> used to hold the class name of an
		 * <code>ExpCompiler</code> implementation. Generally, this should only be
		 * used for testing.
		 */
		private static final ThreadLocal<String> ClassName = new ThreadLocal<String>();

		/**
		 * Get the class name of a <code>ExpCompiler</code> implementation or null.
		 * 
		...
		/**
		 * Sets the class name of a <code>ExpCompiler</code> implementation. This
		 * should be called (obviously) before calling the
		 * <code>ExpCompiler.Factory</code> <code>getExpCompiler</code> method to
		 * get the <code>ExpCompiler</code> implementation. Generally, this is only
		 * used for testing.
		 * 
		 * @param className
		 *          Class name
		...
		/**
		 * Clears the class name (regardless of whether a class name was set). When
		 * a class name is set using <code>setThreadLocalClassName</code>, the
		 * setting whould be done in a try-block and a call to this clear method
		 * should be in the finally-clause of that try-block.
		 */
		...
		/**
		 * The constructor for the <code>ExpCompiler.Factory</code>. This passes the
		 * <code>ExpCompiler</code> class to the <code>ObjectFactory</code> base
		 * class.
		 */
		private Factory() {
		 ...
		 * <code>ExpCompiler</code> instance.
		 * 
		 * @param parameterTypes
		 *          array of classes: Evaluator, Validator and ResultStyle
		 * @param parameterValues
		 *          the Evaluator, Validator and ResultStyle values
		 * @return <code>ExpCompiler</code> instance
		 * @throws CreationException
		 *           if the <code>ExpCompiler</code> can not be created.
		 */
		 ...
		 * <code>ThreadLocal</code> and <code>System</code> <code>Properties</code>.
		 * This class captures and clears the current values for both in the
		 * constructor and then replaces them in the <code>restore</code> method.
		 * <p>
		 * This is for testing only.
		 */
		public static class Context implements ObjectFactory.Context {
			 ...
			 */
			Context() {
				this.threadLocalClassName = ExpCompiler.Factory
						.getThreadLocalClassName();
				if (this.threadLocalClassName != null) {
					ExpCompiler.Factory.clearThreadLocalClassName();
				}

				this.systemPropertyClassName = System.getProperty(ExpCompiler.class
						.getName());
			...
			private void restore() {
				if (this.threadLocalClassName != null) {
					ExpCompiler.Factory
							.setThreadLocalClassName(this.threadLocalClassName);
				}
				if (this.systemPropertyClassName != null) {
