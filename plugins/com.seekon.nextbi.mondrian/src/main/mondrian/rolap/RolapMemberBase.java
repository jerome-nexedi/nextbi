...
public class RolapMemberBase extends MemberBase implements RolapMember {
	 ...
	 *          Type of member
	 */
	protected RolapMemberBase(RolapMember parentMember, RolapLevel level,
			Object key, String name, MemberType memberType) {
		...
		if (parentMember == null
			...
			if (dimension.getDimensionType() != null
					&& (dimension.getDimensionType().equals(
							DimensionType.MeasuresDimension) && hierarchy.getName().equals(
							dimension.getName()))) {
				...
				if (name.equals(level.getName())) {
					this.uniqueName = Util
							.makeFqName(
									Util.makeFqName(hierarchy.getUniqueName(), level.getName()),
									name);
				} else {
	...
	public int compareTo(Object o) {
		...
		if (this.key.getClass().equals(other.key.getClass())) {
			if (this.key instanceof String) {
				...
				return Util.caseSensitiveCompareName((String) this.key,
						(String) other.key);
			} else {
	...
	public boolean isHidden() {
		...
		case IfParentsName: {
			...
			final String name = getName();
			return (parentName == null ? "" : parentName).equals(name == null ? ""
					: name);
		}
	...
	private static int setOrdinal(Member member, int ordinal) {
		if (member instanceof RolapMemberBase) {
			...
			LOGGER.warn("RolapMember.setAllChildren: NOT RolapMember "
					+ "member.name=" + member.getName() + ", member.class="
					+ member.getClass().getName() + ", ordinal=" + ordinal);
			ordinal++;
		}
		return ordinal;
	}
	 ...
	 *          Member
	 */
	private static void setOrdinalsTopDown(SchemaReader schemaReader,
			Member member) {
		long start = System.currentTimeMillis();
	...
	public static final class DefaultPropertyValueMapFactory implements
		...
		public Map<String, Object> create(Member member) {
			...
			if ((member instanceof RolapMeasure) || (props == null)
					|| (props.length > 3)) {
