...
public class RolapUtil {
  ...
  public static SchemaReader locusSchemaReader(RolapConnection connection,
    ...
    return (SchemaReader) Proxy.newProxyInstance(
      SchemaReader.class.getClassLoader(), new Class[] { SchemaReader.class },
  ...
  private final static class RolapUtilComparable implements Comparable, Serializable {
  ...
  public static void alertNonNative(String functionName, String reason)
    ...
    String alertMsg = "Unable to use native SQL evaluation for '" + functionName
      + "'; reason:  " + reason;
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (loadedDrivers.add(jdbcDriver)) {
        try {
          ...
          LOGGER
            .info("Mondrian: JDBC driver " + jdbcDriver + " loaded successfully");
  ...
  public static Member findBestMemberMatch(List<? extends Member> members,
    ...
    for (Member member : members) {
      ...
      if (matchType == MatchType.BEFORE) {
        if (rc < 0
          && (bestMatch == null || FunUtil.compareSiblingMembers(member, bestMatch) > 0)) {
          bestMatch = member;
        }
  ...
  public static MondrianDef.Relation convertInlineTableToRelation(
    ...
    view.addCode("generic", dialect.generateInline(columnNames, columnTypes,
      valueList));
    return view;
  }
