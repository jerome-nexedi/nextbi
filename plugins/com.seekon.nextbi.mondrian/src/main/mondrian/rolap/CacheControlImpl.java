/*
// $Id: //open/mondrian/src/main/mondrian/rolap/CacheControlImpl.java#26 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/CacheControlImpl.java#26 $
 * @since Sep 27, 2006
 */
public class CacheControlImpl implements CacheControl {
   ...
   * 
   * <p>
   * The "member cache" is a figure of speech: each RolapHierarchy has its own
   * MemberCache object. But to provide transparently serialized access to the
   * "member cache" via the interface CacheControl, provide a common lock here.
   * 
   * <p>
   * NOTE: static member is a little too wide a scope for this lock, because in
   * theory a JVM can contain multiple independent instances of mondrian.
   */
  ...
  public CellRegion createMemberRegion(boolean lowerInclusive, Member lowerMember,
    ...
    return new MemberRangeCellRegion((RolapMember) lowerMember, lowerInclusive,
      (RolapMember) upperMember, upperInclusive, descendants);
  }
  ...
  public CellRegion createCrossjoinRegion(CellRegion... regions) {
    ...
    for (CellRegion region : regions) {
      ...
      if (set.size() < prevSize + dimensionality.size()) {
        throw MondrianResource.instance().CacheFlushCrossjoinDimensionsInCommon
          .ex(getDimensionalityList(regions));
      }
  ...
  public CellRegion createUnionRegion(CellRegion... regions) {
    ...
    final List<CellRegionImpl> list = new ArrayList<CellRegionImpl>();
    for (CellRegion region : regions) {
      if (!region.getDimensionality().equals(regions[0].getDimensionality())) {
        throw MondrianResource.instance().CacheFlushUnionDimensionalityMismatch.ex(
          regions[0].getDimensionality().toString(), region.getDimensionality()
            .toString());
      }
  ...
  public CellRegion createMeasuresRegion(Cube cube) {
    ...
    if (measuresDimension == null) {
      throw new MondrianException("No measures dimension found for cube "
        + cube.getName());
    }
    final List<Member> measures = cube.getSchemaReader(null).withLocus()
      .getLevelMembers(measuresDimension.getHierarchy().getLevels()[0], false);
    if (measures.size() == 0) {
  ...
  protected void flushRegionList(List<CellRegion> cellRegionList) {
    ...
    case 1:
      ...
      break;
    default:
      final CellRegion[] cellRegions = cellRegionList
        .toArray(new CellRegion[cellRegionList.size()]);
  ...
  // todo: document
  public void flushSchema(String catalogUrl, String connectionKey, String jdbcUser,
    String dataSourceStr) {
    RolapSchema.Pool.instance().remove(catalogUrl, connectionKey, jdbcUser,
      dataSourceStr);
  }
    ...
    for (int i = 0; i < nonUnionList.size(); i++) {
      while (true) {
        ...
        for (CellRegionImpl unionComponent : firstUnion.regions) {
          ...
          CellRegionImpl cj = copyReplacing(nonUnionRegion, firstUnion,
            unionComponent);
          list.add(cj);
        }
  ...
  private CellRegionImpl copyReplacing(CellRegionImpl region, CellRegionImpl seek,
  ...
  static List<RolapStar> getStarList(CellRegion region) {
    ...
    for (Member measure : measuresList) {
      if (measure instanceof RolapStoredMeasure) {
        ...
        final RolapStar.Measure starMeasure = (RolapStar.Measure) storedMeasure
          .getStarMeasure();
  ...
  public void printCacheState(PrintWriter pw, CellRegion region) {
  ...
  public MemberSet createMemberSet(Member member, boolean descendants) {
    return new SimpleMemberSet(Collections.singletonList((RolapMember) member),
      descendants);
  }

  public MemberSet createMemberSet(boolean lowerInclusive, Member lowerMember,
    boolean upperInclusive, Member upperMember, boolean descendants) {
    ...
    return new RangeMemberSet(stripMember((RolapMember) lowerMember),
      lowerInclusive, stripMember((RolapMember) upperMember), upperInclusive,
      descendants);
  }
  ...
  public MemberEditCommand createCompoundCommand(List<MemberEditCommand> commandList) {
    // noinspection unchecked
    return new CompoundCommand((List) commandList);
  }

  public MemberEditCommand createCompoundCommand(MemberEditCommand... commands) {
    // noinspection unchecked
  ...
  public MemberEditCommand createAddCommand(Member member)
  ...
  public MemberEditCommand createSetPropertyCommand(Member member, String name,
    ...
    return new ChangeMemberPropsCommand(new SimpleMemberSet(Collections
      .singletonList((RolapMember) member), false), Collections.singletonMap(name,
      value));
  }

  public MemberEditCommand createSetPropertyCommand(MemberSet members,
    Map<String, Object> propertyValues) throws IllegalArgumentException {
   ...
   *          Member set
   * @throws IllegalArgumentException
   *           if members are from more than one level
   */
  ...
  public void execute(MemberEditCommand cmd) {
    ...
    if (prop.get()) {
      throw new IllegalArgumentException(
        "Member cache control operations are not allowed unless " + "property "
          + prop.getPath() + " is false");
    }
    synchronized (MEMBER_CACHE_LOCK) {
      ...
      Locus.push(new Locus(execution, "CacheControlImpl.execute",
            ...
            try {
              ...
              if (e.getCause() instanceof InvocationTargetException) {
                ...
                if (ite.getTargetException() instanceof MondrianException) {
                  final MondrianException me = (MondrianException) ite
                    .getTargetException();
                  if (me.getMessage().matches(
  ...
  private static MemberCache getMemberCache(RolapMember member) {
    final MemberReader memberReader = member.getHierarchy().getMemberReader();
  ...
  static class MemberRangeCellRegion implements CellRegionImpl {
    ...
    MemberRangeCellRegion(RolapMember lowerMember, boolean lowerInclusive,
      RolapMember upperMember, boolean upperInclusive, boolean descendants) {
      ...
      this.level = lowerMember == null ? upperMember.getLevel() : lowerMember
        .getLevel();
    }
  ...
  static class CrossjoinCellRegion implements CellRegionImpl {
    ...
    private static void compute(List<CellRegionImpl> regions,
      List<CellRegionImpl> components, List<Dimension> dimensionality) {
      final Set<Dimension> dimensionSet = new HashSet<Dimension>();
      for (CellRegionImpl region : regions) {
        addComponents(region, components);

        final List<Dimension> regionDimensionality = region.getDimensionality();
    ...
    private static void addComponents(CellRegionImpl region,
  ...
  private static class UnionCellRegion implements CellRegionImpl {
    ...
    UnionCellRegion(List<CellRegionImpl> regions) {
      ...
      for (int i = 1; i < regions.size(); i++) {
        ...
        assert region0.getDimensionality().equals(region.getDimensionality());
      }
    }
  ...
  interface MemberEditCommandPlus extends MemberEditCommand {
    /**
     * Executes this command, and gathers a list of cell regions affected in the
     * {@code cellRegionList} parameter. The caller will flush the cell regions
     * later.
     * 
     * @param cellRegionList
     *          Populated with a list of cell regions which are invalidated by
     *          this action
     */
  ...
  interface MemberSetPlus extends MemberSet {
    ...
    /**
     * Filters this member set, returning a member set containing all members at
     * a given Level. When applicable, returns this member set unchanged.
     * 
     * @param level
     *          Level
   ...
   * 
   * <p>
   * The default implementation may not be efficient. For example, if flushing a
   * range of members from the cache, you may not wish to fetch all of the
   * members into the cache in order to flush them.
   */
  public static abstract class MemberSetVisitorImpl implements MemberSetVisitor {
    ...
    public void visit(RangeMemberSet s) {
      final MemberReader memberReader = s.level.getHierarchy().getMemberReader();
      visitRange(memberReader, s.level, s.lowerMember, s.upperMember, s.descendants);
    }

    protected void visitRange(MemberReader memberReader, RolapLevel level,
      RolapMember lowerMember, RolapMember upperMember, boolean recurse) {
      ...
      if (recurse) {
        ...
        visitRange(memberReader, level, lowerChild, upperChild, recurse);
      }
    }
  ...
  static class UnionMemberSet implements MemberSetPlus {
    ...
    public MemberSetPlus filter(RolapLevel level) {
      final List<MemberSetPlus> filteredItems = new ArrayList<MemberSetPlus>();
  ...
  static class RangeMemberSet implements MemberSetPlus {
    ...
    RangeMemberSet(RolapMember lowerMember, boolean lowerInclusive,
      RolapMember upperMember, boolean upperInclusive, boolean descendants) {
      assert lowerMember != null || upperMember != null;
      assert lowerMember == null || upperMember == null
        || lowerMember.getLevel() == upperMember.getLevel();
    ...
    public MemberSetPlus filter2(RolapLevel seekLevel, RolapLevel level,
      RolapMember lower, RolapMember upper) {
      if (level == seekLevel) {
        return new RangeMemberSet(lower, lowerInclusive, upper, upperInclusive,
          false);
      } else if (descendants && level.getHierarchy() == seekLevel.getHierarchy()
        && level.getDepth() < seekLevel.getDepth()) {
        final MemberReader memberReader = level.getHierarchy().getMemberReader();
        final List<RolapMember> list = new ArrayList<RolapMember>();
        ...
        RolapMember upperChild = list.get(list.size() - 1);
        return filter2(seekLevel, (RolapLevel) level.getChildLevel(), lowerChild,
          upperChild);
      } else {
        return EmptyMemberSet.INSTANCE;
      }
   ...
   */
  private class DeleteMemberCommand extends MemberSetVisitorImpl implements
    MemberEditCommandPlus {
    ...
    public void visit(RolapMember member) {
      this.callable = deleteMember(member, member.getParentMember(), cellRegionList);
    }
  ...
  private class AddMemberCommand implements MemberEditCommandPlus {
    ...
    public void execute(List<CellRegion> cellRegionList) {
      this.callable = addMember(member, member.getParentMember(), cellRegionList);
    }
  ...
  private class MoveMemberCommand implements MemberEditCommandPlus {
    ...
    public void execute(final List<CellRegion> cellRegionList) {
      this.callable1 = deleteMember(member, member.getParentMember(), cellRegionList);
      this.callable2 = addMember(member, newParent, cellRegionList);
    }
   ...
   */
  private class ChangeMemberPropsCommand extends MemberSetVisitorImpl implements
    MemberEditCommandPlus {
    ...
    ChangeMemberPropsCommand(MemberSetPlus memberSet,
    ...
    public String toString() {
      return "CreateMemberPropsCommand(" + memberSet + ", " + propertyValues + ")";
    }
    ...
    public void commit() {
      for (RolapMember member : members) {
        ...
        final Object cacheKey = memberCache.makeKey(member.getParentMember(), member
          .getKey());
  ...
  private Callable<Boolean> deleteMember(final RolapMember member,
    final RolapMember previousParent, List<CellRegion> cellRegionList) {
    ...
    return new Callable<Boolean>() {
      public Boolean call() throws Exception {
        final MemberCache memberCache = getMemberCache(member);
        final MemberChildrenConstraint memberConstraint = new ChildByNameConstraint(
          new Id.Segment(member.getName(), Quoting.QUOTED));

        // Remove the member from its parent's lists. First try the
        // unconstrained cache.
        final List<RolapMember> childrenList = memberCache.getChildrenFromCache(
          previousParent, DefaultMemberChildrenConstraint.instance());
        if (childrenList != null) {
          // A list existed before. Let's splice it.
          childrenList.remove(member);
          memberCache.putChildren(previousParent, DefaultMemberChildrenConstraint
            .instance(), childrenList);
        }
        ...
        final List<RolapMember> levelMembers = memberCache.getLevelMembersFromCache(
          member.getLevel(), DefaultTupleConstraint.instance());
        if (levelMembers != null) {
          levelMembers.remove(member);
          memberCache.putChildren(member.getLevel(), DefaultTupleConstraint
            .instance(), childrenList);
        }
   ...
   *          Member
   * @param parent
   *          Member's parent (generally equals member.getParentMember)
   * @param cellRegionList
   *          List of cell regions to be flushed
   */
  private Callable<Boolean> addMember(final RolapMember member,
    final RolapMember parent, List<CellRegion> cellRegionList) {
    ...
    return new Callable<Boolean>() {
      public Boolean call() throws Exception {
        ...
        List<RolapMember> childrenList = memberCache.getChildrenFromCache(parent,
        ...
        if (levelMembers != null) {
          ...
          memberCache.putChildren(member.getLevel(), DefaultTupleConstraint
            .instance(), levelMembers);
        }

        // Now add the member itself into cache
        final Object memberKey = memberCache.makeKey(member.getParentMember(),
          member.getKey());
   ...
   */
  private void flushMember(RolapMember member, List<CellRegion> cellRegionList) {
    final MemberCache memberCache = getMemberCache(member);
    final Object key = memberCache
      .makeKey(member.getParentMember(), member.getKey());
    memberCache.removeMember(key);
