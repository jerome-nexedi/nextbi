...
public class CacheControlImpl implements CacheControl {
	...
	public CellRegion createMemberRegion(boolean lowerInclusive,
			Member lowerMember, boolean upperInclusive, Member upperMember,
			boolean descendants) {
		if (lowerMember == null) {
	...
	public CellRegion createUnionRegion(CellRegion... regions) {
		...
		for (CellRegion region : regions) {
			if (!region.getDimensionality().equals(regions[0].getDimensionality())) {
				throw MondrianResource.instance().CacheFlushUnionDimensionalityMismatch
						.ex(regions[0].getDimensionality().toString(), region
	...
	public CellRegion createMeasuresRegion(Cube cube) {
		...
		final List<Member> measures = cube
	...
	// todo: document
	public void flushSchema(String catalogUrl, String connectionKey,
			String jdbcUser, String dataSourceStr) {
	...
	private CellRegionImpl copyReplacing(CellRegionImpl region,
			CellRegionImpl seek, CellRegionImpl replacement) {
		if (region == seek) {
			return replacement;
		}
	...
	public MemberEditCommand createSetPropertyCommand(Member member, String name,
		...
		return new ChangeMemberPropsCommand(new SimpleMemberSet(
				Collections.singletonList((RolapMember) member), false),
	...
	public static abstract class MemberSetVisitorImpl implements MemberSetVisitor {
		...
		public void visit(RangeMemberSet s) {
			final MemberReader memberReader = s.level.getHierarchy()
					.getMemberReader();
			visitRange(memberReader, s.level, s.lowerMember, s.upperMember,
					s.descendants);
		}
	...
	static class RangeMemberSet implements MemberSetPlus {
		...
		public MemberSetPlus filter2(RolapLevel seekLevel, RolapLevel level,
			...
			if (level == seekLevel) {
				...
				final MemberReader memberReader = level.getHierarchy()
						.getMemberReader();
				final List<RolapMember> list = new ArrayList<RolapMember>();
				...
				RolapMember upperChild = list.get(list.size() - 1);
				return filter2(seekLevel, (RolapLevel) level.getChildLevel(),
	...
	private class DeleteMemberCommand extends MemberSetVisitorImpl implements
		...
		public void visit(RolapMember member) {
			this.callable = deleteMember(member, member.getParentMember(),
					cellRegionList);
		}
	...
	private class AddMemberCommand implements MemberEditCommandPlus {
		...
		public void execute(List<CellRegion> cellRegionList) {
			this.callable = addMember(member, member.getParentMember(),
					cellRegionList);
		}
	...
	private class MoveMemberCommand implements MemberEditCommandPlus {
		...
		public void execute(final List<CellRegion> cellRegionList) {
			this.callable1 = deleteMember(member, member.getParentMember(),
					cellRegionList);
	...
	private class ChangeMemberPropsCommand extends MemberSetVisitorImpl implements
		...
		public void commit() {
			for (RolapMember member : members) {
				...
				final Object cacheKey = memberCache.makeKey(member.getParentMember(),
						member.getKey());
	...
	private Callable<Boolean> deleteMember(final RolapMember member,
		...
		return new Callable<Boolean>() {
			public Boolean call() throws Exception {
				...
				// unconstrained cache.
				final List<RolapMember> childrenList = memberCache
						.getChildrenFromCache(previousParent,
								DefaultMemberChildrenConstraint.instance());
				if (childrenList != null) {
					// A list existed before. Let's splice it.
					childrenList.remove(member);
					memberCache.putChildren(previousParent,
							DefaultMemberChildrenConstraint.instance(), childrenList);
				}
				...
				final List<RolapMember> levelMembers = memberCache
						.getLevelMembersFromCache(member.getLevel(),
								DefaultTupleConstraint.instance());
				if (levelMembers != null) {
					levelMembers.remove(member);
					memberCache.putChildren(member.getLevel(),
							DefaultTupleConstraint.instance(), childrenList);
				}
	...
	private Callable<Boolean> addMember(final RolapMember member,
		...
		return new Callable<Boolean>() {
			public Boolean call() throws Exception {
				...
				List<RolapMember> childrenList = memberCache.getChildrenFromCache(
						parent, DefaultMemberChildrenConstraint.instance());
				if (childrenList == null) {
				...
				if (levelMembers != null) {
					...
					memberCache.putChildren(member.getLevel(),
							DefaultTupleConstraint.instance(), levelMembers);
				}
	...
	private void flushMember(RolapMember member, List<CellRegion> cellRegionList) {
		final MemberCache memberCache = getMemberCache(member);
		final Object key = memberCache.makeKey(member.getParentMember(),
				member.getKey());
		memberCache.removeMember(key);
