/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/Aggregation.java#67 $
 ...
 * <li>tighten any restriction (e.g. year={1997,1998} becomes year={1997})</li>
 * <li>restrict an unrestricted column (e.g. year=* becomes year={1997})</li>
 * </ul>
 * 
 * <p>
 ...
 * </p>
 * 
 * Suppose we have a segment {year=1997, quarter={1,2,3}, state={CA,WA}}. We
 * want to roll up to a segment for {year=1997, state={CA,WA}}. We need to know
 * that we have all quarters. We don't. Because year and quarter are
 * independent, we know that we have all of the ...</p>
 * 
 * <p>
 * Suppose we have a segment specified by {region=West, state=*, year=*}, which
 * materializes to ({West}, {CA,WA,OR}, {1997,1998}). Because state=*, we can
 * rollup to {region=West, year=*} or {region=West, year=1997}.
 * </p>
 * 
 * <p>
 * The space required for a segment depends upon the dimensionality (d), cell
 * count (c) and the value count (v). We don't count the space required for the
 * actual values, which is the same in any scheme.
 * </p>
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/agg/Aggregation.java#67
 *          $
...
public class Aggregation {
	 ...
	 * this list is not synchronized in the code. This is the only mutable field
	 * in the class.
	 */
	...
	/**
	 * This is set in the load method and is used during the processing of a
	 * particular aggregate load.
	 */
	private RolapStar.Column[] columns;

	/**
	 * This is set in the load method and is used during the process of loading
	 * aggregations/segments from rollup operations.
	 */
	 ...
	 *          the key specifying the axes, the context and the RolapStar for
	 *          this Aggregation
	 */
	public Aggregation(AggregationKey aggregationKey) {
		...
		this.constrainedColumnsBitKey = aggregationKey
	...
	/**
	 * Loads a set of segments into this aggregation, one per measure, each
	 * constrained by the same set of column values, and each pinned once.
	 * 
	 * <p>
	 ...
	 * </blockquote>
	 */
	public void load(RolapStar.Column[] columns, RolapStar.Measure[] measures,
		...
		List<Segment> segments = addSegmentsToAggregation(measures, measureBitKey,
				axes, pinnedSegments);
		// The constrained columns are simply the level and foreign columns
		BitKey levelBitKey = getConstrainedColumnsBitKey();
		GroupingSet groupingSet = new GroupingSet(segments, levelBitKey,
				measureBitKey, axes, columns);
		if (groupingSetsCollector.useGroupingSets()) {
			...
			final List<GroupingSet> gsList = new ArrayList<GroupingSet>();
			gsList.add(groupingSet);
			new SegmentLoader().load(gsList, pinnedSegments, compoundPredicateList);
		}
	}

	private List<Segment> addSegmentsToAggregation(RolapStar.Measure[] measures,
			BitKey measureBitKey, Axis[] axes,
			RolapAggregationManager.PinSet pinnedSegments) {
		...
		for (RolapStar.Measure measure : measures) {
			...
			Segment segment = new Segment(this, measure, axes,
	...
	public StarColumnPredicate[] optimizePredicates(RolapStar.Column[] columns,
		...
		for (int i = 0; i < newPredicates.length; i++) {
			...
			final List<StarColumnPredicate> predicateList = newPredicate
					.getPredicates();
	...
	public void flush(CacheControl cacheControl, RolapCacheRegion cacheRegion) {
		...
		final boolean bitmapsIntersect = cacheRegion.getConstrainedColumnsBitKey()
				.intersects(getConstrainedColumnsBitKey());

		// New list of segments - will replace segmentRefs when we're done.
		List<SoftReference<Segment>> newSegmentRefs = new ArrayList<SoftReference<Segment>>();
		segmentLoop: for (SoftReference<Segment> segmentRef : segmentRefs) {
			...
			if (!bitmapsIntersect) {
				...
				cacheControl.trace("discard segment - it has no columns in common: "
						+ segment);
				continue;
			}
			...
			for (int i = 0; i < columns.length; i++) {
				...
				final BitSet axisKeepBitSet = axisKeepBitSets[i] = new BitSet(keyCount);
				...
				StarColumnPredicate flushPredicate = cacheRegion.getPredicate(column
						.getBitPosition());
			...
			for (StarPredicate predicate : cacheRegion.getPredicates()) {
				ValuePruner pruner = new ValuePruner(predicate, segment.axes,
						segment.getData());
			...
			for (int i = 0; i < columns.length; i++) {
				...
				float retention = (float) axisBitSet.cardinality()
			...
			List<StarColumnPredicate> regionPredicates = new ArrayList<StarColumnPredicate>();
			int cellCount = 1;
			...
			Segment.Region region = new Segment.Region(regionPredicates,
					new ArrayList<StarPredicate>(cacheRegion.getPredicates()), cellCount);
			...
			// list.
			final List<Segment.Region> excludedRegions = new ArrayList<Segment.Region>(
			...
			if (bestColumn >= 0) {
				...
				final int bitPosition = column.getBitPosition();
				StarColumnPredicate flushPredicate = cacheRegion
						.getPredicate(bitPosition);
			...
			final Segment newSegment = segment.createSubSegment(axisKeepBitSets,
					bestColumn, bestColumnPredicate, excludedRegions);

			newSegmentRefs.add(new SoftReference<Segment>(newSegment));
		}

		// Flush the external cache regions
		SegmentCacheWorker.flush(SegmentHeader.forCacheRegion(cacheRegion));
	...
	/**
	 * Retrieves the value identified by <code>keys</code>. If the requested cell
	 * is found in the loading segment, current Thread will be blocked until
	 * segment is loaded.
	 * 
	 * <p>
	 * If <code>pinSet</code> is not null, pins the segment which holds it.
	 * <code>pinSet</code> ensures that a segment is only pinned once.
	 * 
	 * <p>
	 ...
	 */
	public Object getCellValue(RolapStar.Measure measure, Object[] keys,
	...
	/**
	 * Returns an array of axis representing this aggregation. It is only
	 * available once the aggregation has been loaded.
	 * 
	 * @return An array of axis objects, or null if the aggregation has not been
	 *         loaded yet.
	 */
	public Axis[] getAxes() {
		return this.axes;
	...
	static class Axis {
		 ...
		 * 
		 * <p>
		 * TODO: Hold keys in a sorted array, then deduce ordinal by doing binary
		 * search.
		 */
		private final Map<Comparable<?>, Integer> mapKeyToOffset = new HashMap<Comparable<?>, Integer>();
		 ...
		 * 
		 * @param predicate
		 *          Predicate defining which keys should appear on axis. (If a key
		 *          passes the predicate but is not in the list, every cell with
		 *          that key is assumed to have a null value.)
		 */
		 ...
		 * @param hasNull
		 *          Whether the axis contains the null value, in addition to the
		 *          values in <code>valueSet</code>
		...
		/**
		 * Returns whether this axis contains a given key, or would contain it if it
		 * existed.
		 * 
		 * <p>
		 * For example, if this axis is unconstrained, then this method returns
		 * <code>true</code> for any value.
		 * 
		 * @param key
		 *          Key
	...
	/**
	 * Helper class to figure out which axis values evaluate to true at least once
	 * by a given predicate.
	 * 
	 * <p>
	 * Consider, for example, the flush predicate<blockquote><code>
	 * 
	 ...
	 * 
	 * </code></blockquote> and therefore we wish to eliminate these pairs
	 * from the segment. But we can eliminate a value only if <em>all</em> of its
	 * values are eliminated.
	 * 
	 * <p>
	 * In this case, year=1998 is the only value which can be eliminated from the
	 * segment.
	 */
	private static class ValuePruner {
		/**
		 * Multi-column predicate. If the predicate evaluates to true, a cell will
		 * be removed from the segment. But we can only eliminate a value if all of
		 * its cells are eliminated.
		 */
		...
		/**
		 * For each column, a bitmap of values for which the predicate is sometimes
		 * false. These values cannot be eliminated from the axis.
		 */
		private final BitSet[] keepBitSets;
		/**
		 * For each segment axis, the predicate column which depends on the axis, or
		 * -1.
		 */
		...
		/**
		 * Workspace which contains the ordinal of the current value of each column
		 * on its axis.
		 */
		 ...
		 * 
		 * @param flushPredicate
		 *          Multi-column predicate to test
		 * @param segmentAxes
		 *          Axes of the segment. (The columns that the predicate may not be
		 *          present, or may be in a different order.)
		 * @param data
		 *          Segment dataset, which allows pruner to determine whether a
		 *          particular cell is currently empty
		 */
		ValuePruner(StarPredicate flushPredicate, Axis[] segmentAxes,
				SegmentDataset data) {
			...
			for (int i = 0; i < arity; i++) {
				RolapStar.Column column = flushPredicate.getConstrainedColumnList()
						.get(i);
				int axisOrdinal = findAxis(segmentAxes, column.getBitPosition());
				if (axisOrdinal < 0) {
					this.axes[i] = null;
		...
		/**
		 * Applies this ValuePruner's predicate and sets bits in axisBitSets to
		 * indicate extra values which can be removed.
		 * 
		 * @param axisKeepBitSets
		 *          Array containing, for each axis, a bitset of values to keep (not
		 *          flush)
		 */
		 ...
		 * {@link #keepBitSets} if the predicate ever evaluates to false. The result
		 * is that discardBitSets[i] will be false for column #i if the predicate
		 * evaluates to true for all cells in the segment which have that column
		 * value.
		 * 
		 * @param axisOrdinal
		 *          Axis ordinal
			...
			if (axisOrdinal == arity) {
				...
				if (axis == null) {
					...
					for (int keyOrdinal = 0; keyOrdinal < axis.keys.length; keyOrdinal++) {
						...
						cellKey.setAxis(axisInverseOrdinals[axisOrdinal], keyOrdinal);
						evaluatePredicate(axisOrdinal + 1);
					}
				}
			}
	...
	private static class ConstraintComparator implements Comparator<Integer> {
		...
		public int compare(Integer o0, Integer o1) {
			...
			return (bloat0 == bloat1) ? 0 : (bloat0 < bloat1) ? 1 : -1;
		}
	}
