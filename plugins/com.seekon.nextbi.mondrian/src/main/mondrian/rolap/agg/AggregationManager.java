/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/AggregationManager.java#69 $
...
/**
 * <code>RolapAggregationManager</code> manages all {@link Aggregation}s in the
 * system. It is a singleton class.
 * 
 * @author jhyde
 * @since 30 August, 2001
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/AggregationManager.java
 *          #69 $
 */
public class AggregationManager extends RolapAggregationManager {

	private static final MondrianProperties properties = MondrianProperties
			.instance();
	...
	/**
	 * Called by FastBatchingCellReader.loadAggregation where the RolapStar
	 * creates an Aggregation if needed.
	 * 
	 * @param measures
	 *          Measures to load
	 * @param columns
	 *          this is the CellRequest's constrained columns
	 * @param aggregationKey
	 *          this is the CellRequest's constraint key
	 * @param predicates
	 *          Array of constraints on each column
	 * @param pinnedSegments
	 *          Set of pinned segments
	 * @param groupingSetsCollector
	 *          grouping sets collector
	 */
	public void loadAggregation(RolapStar.Measure[] measures,
			RolapStar.Column[] columns, AggregationKey aggregationKey,
			StarColumnPredicate[] predicates, PinSet pinnedSegments,
	...
	public Object getCellFromCache(CellRequest request, PinSet pinSet) {
		...
		final Aggregation aggregation = measure.getStar().lookupAggregation(
				aggregationKey);
		...
		if (aggregation == null) {
			...
			return aggregation.getCellValue(measure, request.getSingleValues(),
					pinSet);
		}
	}

	public String getDrillThroughSql(final CellRequest request,
			final StarPredicate starPrediateSlicer, final boolean countOnly) {
		DrillThroughQuerySpec spec = new DrillThroughQuerySpec(request,
				starPrediateSlicer, countOnly);
		Pair<String, List<SqlStatement.Type>> pair = spec.generateSqlQuery();

		if (getLogger().isDebugEnabled()) {
			getLogger().debug("DrillThroughSQL: " + pair.left + Util.nl);
		}
	...
	/**
	 * Generates the query to retrieve the cells for a list of segments. Called by
	 * Segment.load.
	 * 
	 * @return A pair consisting of a SQL statement and a list of suggested types
	 *         of columns
	 */
	public Pair<String, List<SqlStatement.Type>> generateSql(
		...
		if (MondrianProperties.instance().UseAggregates.get()
			...
			if (aggStar != null) {
				...
				AggQuerySpec aggQuerySpec = new AggQuerySpec(aggStar, rollup[0],
						groupingSetsList);
				Pair<String, List<Type>> sql = aggQuerySpec.generateSqlQuery();

				if (getLogger().isDebugEnabled()) {
					getLogger().debug("generateSqlQuery: sql=" + sql.left);
				}
		...
		// Fact table query
		SegmentArrayQuerySpec spec = new SegmentArrayQuerySpec(groupingSetsList,
				compoundPredicateList);

		Pair<String, List<SqlStatement.Type>> pair = spec.generateSqlQuery();

		if (getLogger().isDebugEnabled()) {
			getLogger().debug("generateSqlQuery: sql=" + pair.left);
		}
	...
	/**
	 * Finds an aggregate table in the given star which has the desired levels and
	 * measures. Returns null if no aggregate table is suitable.
	 * 
	 * <p>
	 * If there no aggregate is an exact match, returns a more granular aggregate
	 * which can be rolled up, and sets rollup to true. If one or more of the
	 * measures are distinct-count measures rollup is possible only in limited
	 * circumstances.
	 * 
	 * @param star
	 *          Star
	 ...
	 *          Set of measures
	 * @param rollup
	 *          Out parameter, is set to true if the aggregate is not an exact
	 *          match
	 * @return An aggregate, or null if none is suitable.
	 */
	public AggStar findAgg(RolapStar star, final BitKey levelBitKey,
			final BitKey measureBitKey, boolean[] rollup) {
		...
		for (AggStar aggStar : star.getAggStars()) {
			...
			boolean isDistinct = measureBitKey.intersects(aggStar
			...
			final BitKey distinctMeasuresBitKey = measureBitKey.and(aggStar
					.getDistinctMeasureBitKey());
			final BitSet distinctMeasures = distinctMeasuresBitKey.toBitSet();
			BitKey combinedLevelBitKey = null;
			for (int k = distinctMeasures.nextSetBit(0); k >= 0; k = distinctMeasures
					.nextSetBit(k + 1)) {
				final AggStar.FactTable.Measure distinctMeasure = aggStar
						.lookupMeasure(k);
			...
			if (aggStar.hasForeignKeys()) {
				/*
				 * StringBuilder buf = new StringBuilder(256); buf.append("");
				 * buf.append(star.getFactTable().getAlias()); buf.append(Util.nl);
				 * buf.append("foreign ="); buf.append(levelBitKey);
				 * buf.append(Util.nl); buf.append("measure =");
				 * buf.append(measureBitKey); buf.append(Util.nl);
				 * buf.append("aggstar ="); buf.append(aggStar.getBitKey());
				 * buf.append(Util.nl); buf.append("distinct=");
				 * buf.append(aggStar.getDistinctMeasureBitKey()); buf.append(Util.nl);
				 * buf.append("AggStar="); buf.append(aggStar.getFactTable().getName());
				 * buf.append(Util.nl); for (Iterator columnIter =
				 * aggStar.getFactTable().getColumns().iterator();
				 * columnIter.hasNext();) { AggStar.Table.Column column =
				 * (AggStar.Table.Column) columnIter.next(); buf.append("   ");
				 * buf.append(column); buf.append(Util.nl); }
			...
			if (!aggStar.select(levelBitKey, combinedLevelBitKey, measureBitKey)) {
				continue;
			}
	 ...
	 */
	public static class PinSetImpl extends HashSet<Segment> implements
