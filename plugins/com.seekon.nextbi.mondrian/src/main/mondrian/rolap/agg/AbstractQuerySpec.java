...
public abstract class AbstractQuerySpec implements QuerySpec {
  ...
  protected Map<String, String> nonDistinctGenerateSql(SqlQuery sqlQuery) {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (databaseProduct == Dialect.DatabaseProduct.DB2_AS400) {
        ...
        alias = sqlQuery
          .addSelect(expr, column.getInternalType(), getColumnAlias(i));
      }
   ...
   *         enabled.
   */
  protected Map<String, String> distinctGenerateSql(final SqlQuery outerSqlQuery,
    boolean countOnly) {
    ...
    for (int i = 0, count = getMeasureCount(); i < count; i++) {
      ...
      outerSqlQuery.addSelect(measure.getAggregator().getNonDistinctAggregator()
