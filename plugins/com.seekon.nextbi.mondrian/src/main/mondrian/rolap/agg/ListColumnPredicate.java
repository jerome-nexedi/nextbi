/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/ListColumnPredicate.java#12 $
 ...
 * to the same, single column. It evaluates to true if any of the predicates
 * evaluates to true.
 * 
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/ListColumnPredicate.
 *          java#12 $
 * @since Nov 2, 2006
 */
public class ListColumnPredicate extends AbstractColumnPredicate {
  ...
  /**
   * Hash map of children predicates, keyed off of the hash code of each child.
   * Each entry in the map is a list of predicates matching that hash code.
   */
  ...
  public boolean equalConstraint(StarPredicate that) {
    boolean isEqual = that instanceof ListColumnPredicate
    ...
    if (isEqual) {
      ...
      if (getPredicates().size() != thatPred.getPredicates().size()) {
        ...
        if (childrenHashMap == null) {
          childrenHashMap = new HashMap<Integer, List<StarColumnPredicate>>();
          for (StarColumnPredicate thisChild : getPredicates()) {
            Integer key = new Integer(thisChild.hashCode());
            List<StarColumnPredicate> predList = childrenHashMap.get(key);
            if (predList == null) {
        ...
        for (StarColumnPredicate thatChild : thatPred.getPredicates()) {
          List<StarColumnPredicate> predList = childrenHashMap.get(thatChild
            .hashCode());
          if (predList == null) {
            isEqual = false;
            break;
  ...
  public Overlap intersect(StarColumnPredicate predicate) {
    ...
    if (matchCount == 0) {
      ...
      float selectivity = (float) matchCount / (float) children.size();
  ...
  public StarColumnPredicate minus(StarPredicate predicate) {
    ...
    List<StarColumnPredicate> newChildren = new ArrayList<StarColumnPredicate>(
      children);
    int changeCount = 0;
  ...
  public StarColumnPredicate orColumn(StarColumnPredicate predicate) {
    ...
    if (predicate instanceof ListColumnPredicate) {
      ...
      final List<StarColumnPredicate> list = new ArrayList<StarColumnPredicate>(
        children);
      list.addAll(that.children);
      return new ListColumnPredicate(getConstrainedColumn(), list);
    } else {
      final List<StarColumnPredicate> list = new ArrayList<StarColumnPredicate>(
        children);
      list.add(predicate);
  ...
  public StarColumnPredicate cloneWithColumn(RolapStar.Column column) {
    return new ListColumnPredicate(column, cloneListWithColumn(column, children));
  }
  ...
  public void toSql(SqlQuery sqlQuery, StringBuilder buf) {
    ...
    for (StarColumnPredicate predicate1 : predicates) {
      final ValueColumnPredicate predicate2 = (ValueColumnPredicate) predicate1;
    ...
    case 1:
      ...
      buf.append(" = ");
      sqlQuery.getDialect()
        .quote(buf, firstNotNull.getValue(), column.getDatatype());
      buf.append(" or ");
      buf.append(expr);
      buf.append(" is null)");
      break;
