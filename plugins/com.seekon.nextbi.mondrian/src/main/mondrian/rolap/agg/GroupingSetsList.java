/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/GroupingSetsList.java#7 $
 ...
 * 
 * <p>
 * For example, suppose we have the 3 grouping sets (a, b, c), (a, b) and (b,
 * c).
 * <ul>
 ...
 * @author Thiyagu
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/GroupingSetsList.java#7
 *          $
 * @since 24 May 2007
 */
final class GroupingSetsList {
   ...
   * 
   * <p>
   * First element of the groupingSets list should be the detailed grouping set
   * (default grouping set), followed by grouping sets which can be rolled-up.
   * 
   * @param groupingSets
  ...
  List<RolapStar.Column[]> getGroupingColumnsList(List<GroupingSet> groupingSets) {
    List<RolapStar.Column[]> groupingColumns = new ArrayList<RolapStar.Column[]>();
    for (GroupingSet aggBatchDetail : groupingSets) {
      groupingColumns.add(aggBatchDetail.segment0.aggregation.getColumns());
    }
  ...
  private List<BitKey> loadGroupingColumnBitKeys() {
    ...
    for (RolapStar.Column[] groupingSetColumns : groupingSetsColumns) {
      ...
      Set<RolapStar.Column> columns = new HashSet<RolapStar.Column>(Arrays
        .asList(groupingSetColumns));
      int bitPosition = 0;
  ...
  private int[] loadRollupIndex() {
    ...
    for (int columnIndex = 0; columnIndex < detailedColumns.length; columnIndex++) {
      int rollupIndex = rollupColumns.indexOf(detailedColumns[columnIndex]);
  ...
  private List<RolapStar.Column> findRollupColumns() {
    ...
    for (RolapStar.Column[] groupingSetColumn : groupingSetsColumns) {
      Set<RolapStar.Column> summaryColumns = new HashSet<RolapStar.Column>(Arrays
        .asList(groupingSetColumn));
  ...
  /**
   * Collection of {@link mondrian.rolap.agg.SegmentDataset} that have the same
   * dimensionality and identical axis values. A cohort contains corresponding
   * cell values for set of measures.
   */
  static class Cohort {
    ...
    Cohort(List<SegmentDataset> segmentDatasetList, int arity) {
