...
public class SegmentHeader implements Serializable {
   ...
   */
  public SegmentHeader(String schemaName, String schemaChecksum, String cubeName,
    String measureName, ConstrainedColumn[] constrainedColumns,
  ...
  public SegmentHeader(String schemaName, String schemaChecksum, String cubeName,
    String measureName, ConstrainedColumn[] constrainedColumns,
    String[] compoundPredicates, String rolapStarFactTableName,
  ...
  public static ConstrainedColumn[] forCacheRegion(RolapCacheRegion region) {
    ...
    for (StarColumnPredicate predicate : region.getColumnPredicates()) {
      ...
      cc[i] = new SegmentHeader.ConstrainedColumn(predicate.getConstrainedColumn()
        .getExpression().getGenericExpression(), values.toArray());
      i++;
    }
    return cc;
  }
  ...
  public static SegmentHeader forSegment(Segment segment,
    ...
    for (int i = 0; i < segment.axes.length; i++) {
      ...
      cc[i] = new SegmentHeader.ConstrainedColumn(axis.getPredicate()
        .getConstrainedColumn().getExpression().getGenericExpression(), values
        .toArray());
    }
    ...
    return new SegmentHeader(segment.measure.getStar().getSchema().getName(),
      segment.measure.getStar().getSchema().getChecksum(), segment.measure
        .getCubeName(), segment.measure.getName(), cc, cp, segment.aggregation
        .getStar().getFactTable().getAlias(), segment.aggregation
  ...
  public boolean isSubset(Segment segment) {
    ...
    if (!segment.aggregation.getStar().getFactTable().getAlias().equals(
