/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/OrPredicate.java#9 $
...
/**
 * Predicate which is the union of a list of predicates. It evaluates to true if
 * any of the predicates evaluates to true.
 * 
 * @see OrPredicate
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/agg/OrPredicate.java#9
 *          $
 */
public class OrPredicate extends ListPredicate {
	...
	public StarPredicate or(StarPredicate predicate) {
		if (predicate instanceof OrPredicate
			...
			final List<StarPredicate> list = new ArrayList<StarPredicate>(children);
	 ...
	 * translated using AND/OR. This method identifies all the columns that can be
	 * part of IN and and categorizes this predicate based on number of column
	 * values to use in the IN list.
	 * 
	 * @param predicate
	 ...
	 *          Query
	 * @param predicateMap
	 *          the map containing predicates analyzed so far
	 */
	private void checkInListForPredicate(StarPredicate predicate,
			SqlQuery sqlQuery, Map<BitKey, List<StarPredicate>> predicateMap) {
		...
		if (predicate instanceof ValueColumnPredicate) {
			...
			inListRhsBitKey = ((ValueColumnPredicate) predicate)
					.checkInList(columnBitKey);
		} else if (predicate instanceof AndPredicate) {
			// OR of ANDs over a set of values over the same column set
			inListRhsBitKey = ((AndPredicate) predicate).checkInList(sqlQuery,
					columnBitKey);
		} else {
	...
	private void checkInList(SqlQuery sqlQuery,
	...
	/**
	 * Translates a list of predicates over the same set of columns into sql using
	 * IN list where possible.
	 * 
	 * @param sqlQuery
	 *          Query
	 * @param buf
	 *          buffer to build sql
	 * @param inListRhsBitKey
	 *          which column positions are included in the IN predicate; the non
	 *          included positions corresponde to columns that are nulls
	 * @param predicateList
	 *          the list of predicates to translate.
	 */
	private void toInListSql(SqlQuery sqlQuery, StringBuilder buf,
			BitKey inListRhsBitKey, List<StarPredicate> predicateList) {
		// Make a col position to column map to aid search.
		Map<Integer, RolapStar.Column> columnMap = new HashMap<Integer, RolapStar.Column>();
		...
		for (Integer colPos : getConstrainedColumnBitKey().andNot(inListRhsBitKey)) {
	...
	public void toSql(SqlQuery sqlQuery, StringBuilder buf) {
		...
		// super.toSql().
		//
		final Map<BitKey, List<StarPredicate>> predicateMap = new LinkedHashMap<BitKey, List<StarPredicate>>();
