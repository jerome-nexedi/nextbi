...
public class SegmentLoader {
		...
		if (segmentsLeft) {
			...
			try {
				...
				final Map<BitKey, GroupingSetsList.Cohort> groupingDataSetsMap = createDataSetsForGroupingSets(
						...
						rows.getTypes().subList(arity, rows.getTypes().size()));
	...
	private void loadSegmentsFromCacheRollup(List<GroupingSet> groupingSets,
		...
		for (final GroupingSet groupingSet : groupingSets) {
			...
			segLoop: while (iterator.hasNext()) {
				...
				for (final SegmentHeader header : SegmentCacheWorker
						.getSegmentHeaders()) {
	...
	void cacheSegmentData(List<GroupingSet> groupingSets,
		...
		for (final GroupingSet groupingSet : groupingSets) {
			for (Segment segment : groupingSet.getSegments()) {
				final SegmentHeader sh = SegmentHeader.forSegment(segment,
						compoundPredicates);
				final SegmentBody sb = segment.getData().createSegmentBody(
	...
	private void loadDataToDataSets(GroupingSetsList groupingSetsList,
			RowList rows, Map<BitKey, GroupingSetsList.Cohort> groupingDataSetMap) {
			...
			for (int j = 0, k = 0; j < arity; j++) {
				...
				case DOUBLE:
					...
					if (useGroupingSet
							...
							&& groupingBitKey.get(groupingSetsList
	...
	private Map<BitKey, GroupingSetsList.Cohort> createDataSetsForGroupingSets(
		...
		if (!groupingSetsList.useGroupingSets()) {
			final GroupingSetsList.Cohort datasets = createDataSets(sparse,
					...
					groupingSetsList.getDefaultAxes(), types);
		...
		for (int i = 0; i < groupingSets.size(); i++) {
			...
			GroupingSetsList.Cohort cohort = createDataSets(sparse,
					groupingSet.getSegments(), groupingSet.getAxes(), types);
	...
	private GroupingSetsList.Cohort createDataSets(boolean sparse,
			List<Segment> segments, Aggregation.Axis[] axes,
			List<SqlStatement.Type> types) {
		final List<SegmentDataset> datasets = new ArrayList<SegmentDataset>(
				segments.size());
		final int n;
		if (sparse) {
			n = 0;
	...
	SqlStatement createExecuteSql(GroupingSetsList groupingSetsList,
		...
		return RolapUtil.executeQuery(star.getDataSource(), pair.left, pair.right,
				0, 0, new Locus(Locus.peek().execution, "Segment.load",
	...
	RowList processData(SqlStatement stmt, final boolean[] axisContainsNull,
		...
		final List<SqlStatement.Type> types = stmt == null ? Collections.nCopies(
				rawRows.getMetaData().getColumnCount(), SqlStatement.Type.OBJECT)
				: stmt.guessTypes();
		...
		while (rawRows.next()) {
			...
			if (groupingSetsList.useGroupingSets()) {
				processedRows.setObject(
						columnIndex,
						getRollupBitKey(groupingSetsList.getRollupColumns().size(),
								rawRows, columnIndex));
			}
		}
	...
	private boolean isAggregateNull(ResultSet rowList,
			int groupingColumnStartIndex, GroupingSetsList groupingSetsList,
	...
	private static boolean useSparse(final double possibleCount,
		...
		if (possibleCount < countThreshold) {
			assert !sparse : "Should never use sparse if count is less "
					...
					+ actualCount + ", countThreshold=" + countThreshold
	...
	static protected class RowList {
		...
		static class IntColumn extends NativeColumn {
			...
			public boolean isNull(int row) {
				return ints[row] == 0 && nullIndicators != null
		...
		static class LongColumn extends NativeColumn {
			...
			public boolean isNull(int row) {
				return longs[row] == 0 && nullIndicators != null
						&& nullIndicators.get(row);
			}
