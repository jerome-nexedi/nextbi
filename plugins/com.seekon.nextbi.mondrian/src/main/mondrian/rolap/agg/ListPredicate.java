/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/ListPredicate.java#12 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/ListPredicate.java#12 $
 */
public abstract class ListPredicate implements StarPredicate {
  ...
  protected ListPredicate(List<StarPredicate> predicateList) {
    ...
    final SortedSet<RolapStar.Column> columnSet = new TreeSet<RolapStar.Column>(
  ...
  public BitKey getConstrainedColumnBitKey() {
    if (columnBitKey == null) {
      for (StarPredicate predicate : children) {
        if (columnBitKey == null) {
          columnBitKey = predicate.getConstrainedColumnBitKey().copy();
        } else {
          columnBitKey = columnBitKey.or(predicate.getConstrainedColumnBitKey());
        }
      }
    }
    return columnBitKey;
  ...
  public boolean equalConstraint(StarPredicate that) {
    boolean isEqual = that instanceof ListPredicate
    ...
    if (isEqual) {
      ...
      if (isEqual) {
        ...
        if (childrenHashMap == null) {
          childrenHashMap = new HashMap<Integer, List<StarPredicate>>();
        ...
        for (StarPredicate thatChild : thatPred.getChildren()) {
          List<StarPredicate> predList = childrenHashMap.get(thatChild.hashCode());
          if (predList == null) {
            isEqual = false;
            break;
          }
