/*
// $Id: //open/mondrian/src/main/mondrian/rolap/MemberSource.java#12 $
 ...
 * 
 * <p>
 * A <code>MemberSource</code> may optionally support writeback to a
 * {@link MemberCache}. During the initialization of a <code>MemberSource</code>
 * , the consumer calls {@link #setCache}; the return value indicates whether
 * the <code>MemberSource</code> supports cache-writeback.
 * 
 * <p>
 * A <dfn>custom member reader</dfn> is a user-defined class which implements
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/MemberSource.java#12 $
 */
public interface MemberSource {
   ...
   * 
   * <p>
   * Cache-writeback is optional (for example, {@link SqlMemberSource} supports
   * it, and {@link ArrayMemberSource} does not), and the return value from this
   * method indicates whether this object supports it.
   * 
   * <p>
   * If this method returns <code>true</code>, the {@link #getMembers},
   * {@link #getRootMembers} and {@link #getMemberChildren} methods must write
   * to the cache, in addition to returning members as a return value.
   * 
   * @param cache
   *          The <code>MemberCache</code> which the caller would like this
   *          <code>MemberSource</code> to write to.
   ...
   * 
   * <p>
   * If this object {@link #setCache supports cache-writeaback}, also writes
   * these members to the cache.
   */
  List<RolapMember> getMembers();

  /**
   * Returns all members of this hierarchy which do not have a parent, sorted by
   * ordinal.
   * 
   * <p>
   * If this object {@link #setCache supports cache-writeback}, also writes
   * these members to the cache.
   * 
