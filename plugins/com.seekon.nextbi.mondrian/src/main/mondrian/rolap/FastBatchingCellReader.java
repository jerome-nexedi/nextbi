...
public class FastBatchingCellReader implements CellReader {
	...
	class Batch implements Loadable {
		...
		private StarColumnPredicate[] initPredicates() {
			...
			for (int j = 0; j < columns.length; j++) {
				...
				if (valueSet == null) {
					...
					predicate = new ListColumnPredicate(columns[j],
		...
		private void generateAggregateSql() {
			...
			if (cube == null || cube.isVirtual()) {
				...
				final AggGen aggGen = new AggGen(cube.getName(), cube.getStar(),
						columns);
				if (aggGen.isReady()) {
					// PRINT TO STDOUT - DO NOT USE BATCH_LOGGER
					System.out.println("createLost:" + Util.nl + aggGen.createLost());
					System.out.println("insertIntoLost:" + Util.nl
							+ aggGen.insertIntoLost());
					System.out.println("createCollapsed:" + Util.nl
		...
		boolean canBatch(Batch other) {
			return hasOverlappingBitKeys(other) && constraintsMatch(other)
					...
					&& haveSameStarAndAggregation(other) && haveSameClosureColumns(other);
		}
		...
		boolean haveSameClosureColumns(Batch other) {
			...
			final BitKey closureColumns = this.batchKey.getConstrainedColumnsBitKey()
	...
	private static class CompositeBatchComparator implements
		...
		public int compare(CompositeBatch o1, CompositeBatch o2) {
			return BatchComparator.instance.compare(o1.detailedBatch,
					o2.detailedBatch);
		}
	}
