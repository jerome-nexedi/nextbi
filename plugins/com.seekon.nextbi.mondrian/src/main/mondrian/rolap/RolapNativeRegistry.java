...
/**
 * Composite of {@link RolapNative}s. Uses chain of responsibility to select the
 * appropriate {@link RolapNative} evaluator.
 */
public class RolapNativeRegistry extends RolapNative {
  ...
  /**
   * Returns the matching NativeEvaluator or null if <code>fun</code> can not be
   * executed in SQL for the given context and arguments.
   */
  public NativeEvaluator createEvaluator(RolapEvaluator evaluator, FunDef fun,
    Exp[] args) {
    if (!isEnabled()) {
      return null;
    }
