/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapStar.java#120 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapStar.java#120 $
 */
public class RolapStar {
  ...
  /**
   * Holds all pending aggregations of this star that are waiting to be pushed
   * into the global cache. They cannot be pushed yet, because the aggregates in
   * question are currently in use by other threads.
   */
  ...
  /**
   * Keeps track of the columns across all tables. Should have a number of
   * elements equal to columnCount.
   */
  ...
  /**
   * If true, then database aggregation information is cached, otherwise it is
   ...
   * {@link RolapStar}.
   */
  RolapStar(final RolapSchema schema, final DataSource dataSource,
  ...
  private static class StarNetworkNode {
    ...
    private StarNetworkNode(StarNetworkNode parent, String alias,
      MondrianDef.Relation origRel, String foreignKey, String joinKey) {
      this.parent = parent;
    ...
    private boolean isCompatible(StarNetworkNode compatibleParent,
      MondrianDef.Relation rel, String compatibleForeignKey, String compatibleJoinKey) {
      return parent == compatibleParent && origRel.getClass().equals(rel.getClass())
        && foreignKey.equals(compatibleForeignKey)
        && joinKey.equals(compatibleJoinKey);
    }
  }

  private MondrianDef.RelationOrJoin cloneRelation(MondrianDef.Relation rel,
    String possibleName) {
    if (rel instanceof MondrianDef.Table) {
      ...
      MondrianDef.InlineTable newInlineTable = new MondrianDef.InlineTable(
        inlineTable);
   ...
   * 
   * @param rel
   *          the relation needing uniqueness
   * @param factForeignKey
   *          the foreign key of the fact table
   * @param primaryKey
   *          the join key of the relation
  ...
  public MondrianDef.RelationOrJoin getUniqueRelation(
    MondrianDef.RelationOrJoin rel, String factForeignKey, String primaryKey,
    String primaryKeyTable) {
    return getUniqueRelation(factNode, rel, factForeignKey, primaryKey,
      primaryKeyTable);
  }

  private MondrianDef.RelationOrJoin getUniqueRelation(StarNetworkNode parent,
    MondrianDef.RelationOrJoin relOrJoin, String foreignKey, String joinKey,
    String joinKeyTable) {
    if (relOrJoin == null) {
      return null;
      ...
      if (join.getLeftAlias().equals(joinKeyTable)) {
        ...
        left = getUniqueRelation(parent, join.left, foreignKey, joinKey,
          joinKeyTable);
        parent = nodeLookup.get(((MondrianDef.Relation) left).getAlias());
        right = getUniqueRelation(parent, join.right, join.leftKey, join.rightKey,
          join.getRightAlias());
      } else if (join.getRightAlias().equals(joinKeyTable)) {
        // right side must equal
        right = getUniqueRelation(parent, join.right, foreignKey, joinKey,
          joinKeyTable);
        parent = nodeLookup.get(((MondrianDef.Relation) right).getAlias());
        left = getUniqueRelation(parent, join.left, join.rightKey, join.leftKey,
          join.getLeftAlias());
      } else {
      ...
      if (join.left != left || join.right != right) {
        join = new MondrianDef.Join(
          left instanceof MondrianDef.Relation ? ((MondrianDef.Relation) left)
            .getAlias() : null, join.leftKey, left,
          right instanceof MondrianDef.Relation ? ((MondrianDef.Relation) right)
            .getAlias() : null, join.rightKey, right);
      }
      return join;
    }
    return null;
  ...
  /**
   * This is used by the {@link Column} constructor to get a unique id (per its
   * parent {@link RolapStar}).
   */
  ...
  /**
   * This is used to decrement the column counter and is used if a newly created
   * column is found to already exist.
   */
  ...
  /**
   * This is a place holder in case in the future we wish to be able to reload
   * aggregates. In that case, if aggregates had already been loaded, i.e., this
   * star has some aggstars, then those aggstars are cleared.
   */
   ...
   * 
   * <p>
   * Internally the AggStars are added in sort order, smallest row count to
   * biggest, so that the most efficient AggStar is encountered first; ties do
   * not matter.
   */
  ...
  /**
   * Reorder the list of aggregate stars. This should be called if the algorithm
   * used to order the AggStars has been changed.
   */
  ...
  /**
   * Sets whether to cache database aggregation information; if false, cache is
   * flushed after each query.
   * 
   * <p>
   * This method is called only by the RolapCube and is only called if caching
   * is to be turned off. Note that the same RolapStar can be associated with
   * more than on RolapCube. If any one of those cubes has caching turned off,
   * then caching is turned off for all of them.
   * 
   * @param cacheAggregations
  ...
  /**
   * Looks up an aggregation or creates one if it does not exist in an atomic
   ...
   *          this is the contrained column bitkey
   */
  ...
  /**
   * Looks for an existing aggregation over a given set of columns, or returns
   ...
   * 
   * <p>
   * If so, a new thread local aggregation will be made and added after the
   * query has finished.
   * 
   * <p>
   * This method should be called before a query is executed and afterwards the
   * function {@link #pushAggregateModificationsToGlobalCache()} should be
   * called.
   */
  public void checkAggregateModifications() {
    ...
    if (changeListener != null) {
      if (cacheAggregations && !isCacheDisabled()) {
        synchronized (sharedAggregations) {
          for (Map.Entry<AggregationKey, Aggregation> e : sharedAggregations
            .entrySet()) {
   ...
   * 
   * <p>
   * The method checks whether there are other running queries that are using
   * the requested modifications. If this is the case, modifications are not
   * pushed yet.
   */
  public void pushAggregateModificationsToGlobalCache() {
    ...
    synchronized (this) {
      if (cacheAggregations && !isCacheDisabled()) {
        ...
        Iterator<Map.Entry<AggregationKey, Aggregation>> it = pendingAggregations
          .entrySet().iterator();
        while (it.hasNext()) {
        ...
        while (it.hasNext()) {
          ...
          pushAggregateModification(aggregationKey, aggregation, targetMap);
        }
        localAggregations.get().clear();
      }
  ...
  /**
   * Pushes aggregations in destination aggregations, replacing older entries.
   */
  private void pushAggregateModification(AggregationKey localAggregationKey,
    Aggregation localAggregation, Map<AggregationKey, Aggregation> destAggregations) {
    if (cacheAggregations && !isCacheDisabled()) {
      synchronized (destAggregations) {
        boolean found = false;
        Iterator<Map.Entry<AggregationKey, Aggregation>> it = destAggregations
          .entrySet().iterator();
        while (it.hasNext()) {
          Map.Entry<AggregationKey, Aggregation> e = it.next();
          AggregationKey aggregationKey = e.getKey();
  ...
  private void clearAggregationRequests() {
    synchronized (aggregationRequests) {
      ...
      Set<AggregationKey> localAggregationRequestSet = new HashSet<AggregationKey>(
  ...
  /**
   * Collects all columns in this table and its children. If
   * <code>joinColumn</code> is specified, only considers child tables joined by
   * the given column.
   */
   ...
   * 
   * <p>
   * The SQL statement is of the form {@code SELECT ... FROM ... JOIN ... GROUP
   * BY ...}. It is useful for populating an aggregate table.
   * 
   * @param columnList
   *          List of columns (attributes and measures)
   * @param columnNameList
   *          List of column names (must have same cardinality as {@code
   *          columnList})
   ...
   * @param structure
   *          Whether to print the structure of the star
   */
   ...
   *          Cache control API
   * @param region
   *          Predicate defining a region of cells
   */
  public void flush(CacheControl cacheControl, CacheControl.CellRegion region) {
    ...
    final RolapCacheRegion cacheRegion = RolapAggregationManager.makeCacheRegion(
      this, region);
  ...
  public static class Column {
    ...
    /**
     * When a Column is a column, and not a Measure, the parent column is the
     * coloumn associated with next highest Level.
     */
    private final Column parentColumn;

    /**
     * This is used during both aggregate table recognition and aggregate table
     * generation. For multiple dimension usages, multiple shared dimension or
     * unshared dimension with the same column names, this is used to
     * disambiguate aggregate column names.
     */
    private final String usagePrefix;

    /**
     * This is only used in RolapAggregationManager and adds non-constraining
     * columns making the drill-through queries easier for humans to understand.
     */
    ...
    /**
     * The estimated cardinality of the column. {@link Integer#MIN_VALUE} means
     * unknown.
     */
    private int approxCardinality = Integer.MIN_VALUE;

    private Column(String name, Table table, MondrianDef.Expression expression,
      Dialect.Datatype datatype) {
      this(name, table, expression, datatype, null, null, null, null,
        Integer.MIN_VALUE, table.star.nextColumnCount());
    }

    private Column(String name, Table table, MondrianDef.Expression expression,
      Dialect.Datatype datatype, SqlStatement.Type internalType, Column nameColumn,
      Column parentColumn, String usagePrefix, int approxCardinality, int bitPosition) {
      this.name = name;
      this.table = table;
    ...
    public boolean equals(Object obj) {
      ...
      return other.table == this.table
        ...
        && other.datatype == this.datatype && other.name.equals(this.name);
    }
     ...
     * <code><i>tableName</i>.<i>columnName</i></code>.
     */
    ...
    /**
     * Get column cardinality from the schema cache if possible; otherwise issue
     * a select count(distinct) query to retrieve the cardinality and stores it
     * in the cache.
     * 
    ...
    public int getCardinality() {
      if (approxCardinality == Integer.MIN_VALUE) {
        ...
        Integer card = schema.getCachedRelationExprCardinality(table.getRelation(),
          expression);
        ...
        if (card != null) {
          ...
          schema.putCachedRelationExprCardinality(table.getRelation(), expression,
            approxCardinality);
        }
      }
    ...
    private int getCardinality(DataSource dataSource) {
      ...
      if (sqlQuery.getDialect().allowsCountDistinct()) {
        ...
        sqlQuery.addSelect("count(distinct " + generateExprString(sqlQuery) + ")",
          null);
        ...
        boolean failIfExists = true, joinToParent = false;
      ...
      final SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql, new Locus(
        Locus.peek().execution, "RolapStar.Column.getCardinality",
     ...
     * 
     * <li>Only null values: <code>foo.bar is null</code></li>
     * 
     * <li>String values: <code>foo.bar in ('a', 'b', 'c')</code></li>
     * 
     * </ul>
     */
    public static String createInExpr(final String expr,
      StarColumnPredicate predicate, Dialect.Datatype datatype, SqlQuery sqlQuery) {
      ...
      if (!Bug.BugMondrian313Fixed || !Bug.BugMondrian314Fixed
    ...
    /**
     * Returns a string representation of the datatype of this column, in the
     * dialect specified. For example, 'DECIMAL(10, 2) NOT NULL'.
     * 
     * @param dialect
     *          Dialect
    ...
    public String getDatatypeString(Dialect dialect) {
      ...
      query
        .addFrom(table.star.factTable.relation, table.star.factTable.alias, false);
   ...
   * 
   * <p>
   * A measure is basically just a column; except that its {@link #aggregator}
  ...
  public static class Measure extends Column {
    ...
    public Measure(String name, String cubeName, RolapAggregator aggregator,
      Table table, MondrianDef.Expression expression, Dialect.Datatype datatype) {
    ...
    public void print(PrintWriter pw, String prefix) {
      ...
      pw.print(getBitPosition());
      pw.print("): ");
      pw.print(aggregator.getExpression(getExpression() == null ? null
   ...
   * 
   * <p>
   * A 'table' is defined by a {@link mondrian.olap.MondrianDef.RelationOrJoin}
   * so may, in fact, be a view.
   * 
   * <p>
   * Every table in the star schema except the fact table has a parent table,
   * and a condition which specifies how it is joined to its parent. So the star
   * schema is, in effect, a hierarchy with the fact table at its root.
   */
  public static class Table {
    ...
    private Table(RolapStar star, MondrianDef.Relation relation, Table parent,
      Condition joinCondition) {
      this.star = star;
      this.relation = relation;
      this.alias = chooseAlias();
      this.parent = parent;
      final AliasReplacer aliasReplacer = new AliasReplacer(relation.getAlias(),
        this.alias);
    ...
    /**
     * Adds to a list all columns of this table or a child table which are
     * present in a given bitKey.
     * 
     * <p>
     * Note: This method is slow, but that's acceptable because it is only used
     * for tracing. It would be more efficient to store an array in the
     * {@link RolapStar} mapping column ordinals to columns.
     */
    ...
    public Column[] lookupColumns(String columnName) {
      ...
      for (Column column : getColumns()) {
        if (column.getExpression() instanceof MondrianDef.Column) {
          MondrianDef.Column columnExpr = (MondrianDef.Column) column
            .getExpression();
          if (columnExpr.name.equals(columnName)) {
            l.add(column);
          }
        } else if (column.getExpression() instanceof MondrianDef.KeyExpression) {
          MondrianDef.KeyExpression columnExpr = (MondrianDef.KeyExpression) column
            .getExpression();
    ...
    /**
     * Given a MondrianDef.Expression return a column with that expression or
     * null.
     */
    ...
    /**
     * Look up a {@link Measure} by its name. Returns null if not found.
     */
    ...
    /**
     * Sometimes one need to get to the "real" name when the table has been
     * given an alias.
     */
    ...
    synchronized void makeMeasure(RolapBaseCubeMeasure measure) {
      ...
      RolapStar.Measure starMeasure = new RolapStar.Measure(measure.getName(),
        measure.getCube().getName(), measure.getAggregator(), this, measure
          .getMondrianDefExpression(), measure.getDatatype());
    ...
    /**
     * This is only called by RolapCube. If the RolapLevel has a non-null name
     * expression then two columns will be made, otherwise only one. Updates the
     * RolapLevel to RolapStar.Column mapping associated with this cube.
     * 
     * @param cube
     *          Cube
     * @param level
     ...
     *          Parent column
     */
    synchronized Column makeColumns(RolapCube cube, RolapCubeLevel level,
      Column parentColumn, String usagePrefix) {
      Column nameColumn = null;
      if (level.getNameExp() != null) {
        // make a column for the name expression
        nameColumn = makeColumnForLevelExpr(cube, level, level.getName(), level
          .getNameExp(), Dialect.Datatype.String, null, null, null, null);
      }

      // select the column's name depending upon whether or not a
      // "named" column, above, has been created.
      String name = (level.getNameExp() == null) ? level.getName() : level.getName()
        + " (Key)";

      // If the nameColumn is not null, then it is associated with this
      // column.
      Column column = makeColumnForLevelExpr(cube, level, name, level.getKeyExp(),
        level.getDatatype(), level.getInternalType(), nameColumn, parentColumn,
        usagePrefix);
    ...
    private Column makeColumnForLevelExpr(RolapCube cube, RolapLevel level,
      String name, MondrianDef.Expression xmlExpr, Dialect.Datatype datatype,
      SqlStatement.Type internalType, Column nameColumn, Column parentColumn,
      String usagePrefix) {
      Table table = this;
      if (xmlExpr instanceof MondrianDef.Column) {
        ...
        if (table == null) {
          throw Util.newError("Level '" + level.getUniqueName() + "' of cube '"
            + this + "' is invalid: table '" + tableName
            + "' is not found in current scope" + Util.nl + ", star:" + Util.nl
            + getStar());
        }
        RolapStar.AliasReplacer aliasReplacer = new RolapStar.AliasReplacer(
      ...
      if (c != null && !c.equals(nameColumn)) {
        ...
        column = new RolapStar.Column(name, table, xmlExpr, datatype, internalType,
          nameColumn, parentColumn, usagePrefix, level.getApproxRowCount(), star
            .nextColumnCount());
        addColumn(column);
      }
      return column;
    ...
    /**
     * Extends this 'leg' of the star by adding <code>relation</code> joined by
     * <code>joinCondition</code>. If the same expression is already present,
     * does not create it again. Stores the unaliased table names to
     * RolapStar.Table mapping associated with the input <code>cube</code>.
     */
    synchronized Table addJoin(RolapCube cube,
      MondrianDef.RelationOrJoin relationOrJoin, RolapStar.Condition joinCondition) {
      if (relationOrJoin instanceof MondrianDef.Relation) {
        ...
        joinCondition = new RolapStar.Condition(new MondrianDef.Column(leftAlias,
          join.leftKey), new MondrianDef.Column(rightAlias, join.rightKey));
        RolapStar.Table rightTable = leftTable.addJoin(cube, join.right,
          joinCondition);
        return rightTable;
      ...
      for (Table child : getChildren()) {
        if (child.relation.equals(relation)) {
          ...
          if (!Util.equalName(relation.getAlias(), child.alias)) {
            ...
            AliasReplacer aliasReplacer = new AliasReplacer(relation.getAlias(),
              child.alias);
     ...
     *          Query to add to
     * @param failIfExists
     *          Pass in false if you might have already added the table before
     *          and if that happens you want to do nothing.
     * @param joinToParent
     *          Pass in true if you are constraining a cell calculation, false
     *          if you are retrieving members.
     */
    ...
    /**
     * Finds the child table of the fact table with the given columnName used in
     * its left join condition. This is used by the AggTableManager while
     * characterizing the fact table columns.
     */
     ...
     * mapping from for the aggregate join condition is valid. It returns the
     * child table with the matching left join condition.
     */
    ...
    public boolean containsColumn(String columnName) {
      if (relation instanceof MondrianDef.Relation) {
        return star.containsColumn(((MondrianDef.Relation) relation).getAlias(),
          columnName);
      } else {
        // todo: Deal with join.
        return false;
  ...
  /**
   * Creates a copy of an expression, everywhere replacing one alias with
   * another.
   */
  public static class AliasReplacer {
    ...
    private Condition visit(Condition condition) {
      ...
      return new Condition(visit(condition.left), visit(condition.right));
    }
