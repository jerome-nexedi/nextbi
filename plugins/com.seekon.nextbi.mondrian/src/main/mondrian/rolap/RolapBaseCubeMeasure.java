/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapBaseCubeMeasure.java#18 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapBaseCubeMeasure.java#18
 *          $
 */
public class RolapBaseCubeMeasure extends RolapMemberBase implements
		RolapStoredMeasure {
	private static final List<String> datatypeList = Arrays.asList("Integer",
			"Numeric", "String");
	...
	/**
	 * Holds the {@link mondrian.rolap.RolapStar.Measure} from which this member
	 ...
	 *          Annotations
	 */
	RolapBaseCubeMeasure(RolapCube cube, RolapMember parentMember,
			RolapLevel level, String name, String caption, String description,
			String formatString, MondrianDef.Expression expression,
			String aggregatorName, String datatype,
		...
		if (description != null) {
			setProperty(Property.DESCRIPTION.name, description);
		}
		if (formatString == null) {
			formatString = "";
		}
		setProperty(Property.FORMAT_EXP.name, Literal.createString(formatString));

		// Validate aggregator.
		this.aggregator = RolapAggregator.enumeration.getValue(aggregatorName,
				false);
		if (this.aggregator == null) {
			...
			throw MondrianResource.instance().UnknownAggregator.ex(aggregatorName,
					buf.toString());
		}
