...
public class RolapCubeLevel extends RolapLevel {
  ...
  public RolapCubeLevel(RolapLevel level, RolapCubeHierarchy cubeHierarchy) {
    super(cubeHierarchy, level.getName(), level.getCaption(), level.isVisible(),
      level.getDescription(), level.getDepth(), level.getKeyExp(), level
        .getNameExp(), level.getCaptionExp(), level.getOrdinalExp(), level
        .getParentExp(), level.getNullParentValue(), null, level.getProperties(),
      level.getFlags(), level.getDatatype(), level.getInternalType(), level
        .getHideMemberCondition(), level.getLevelType(), ""
        + level.getApproxRowCount(), level.getAnnotationMap());
  ...
  void init(MondrianDef.CubeDimension xmlDimension) {
    if (isAll()) {
      ...
      if (!getCube().isVirtual()) {
        getCube().createUsage(
          (RolapCubeHierarchy) cubeDimension.getHierarchies()[0], xmlDimension);
      }
  ...
  private MondrianDef.Expression convertExpression(MondrianDef.Expression exp,
    ...
    if (getHierarchy().isUsingCubeFact()) {
      ...
      if (rel instanceof MondrianDef.Table) {
        return new MondrianDef.Column(((MondrianDef.Table) rel).getAlias(), col
          .getColumnName());
  ...
  static final class RegularLevelReader implements LevelReader {
    ...
    public void constrainRegion(StarColumnPredicate predicate, RolapCube baseCube,
      RolapCacheRegion cacheRegion) {
      RolapStar.Column column = cubeLevel.getBaseStarKeyColumn(baseCube);
      ...
      if (predicate instanceof MemberColumnPredicate) {
        ...
        MemberTuplePredicate predicate2 = new MemberTuplePredicate(baseCube,
          lowerMember, !rangeColumnPredicate.getLowerInclusive(), upperMember,
  ...
  static final class ParentChildLevelReaderImpl implements LevelReader {
    ...
    public boolean constrainRequest(RolapCubeMember member, RolapCube baseCube,
      ...
      if (member.getDataMember() == null) {
        ...
        return regularLevelReader.constrainRequest(member.getDataMember(), baseCube,
          request);
      } else {
        ...
        member = new RolapCubeMember(allMember, wrappedMember, closedPeerCubeLevel);
