...
public class RolapDependencyTestingEvaluator extends RolapEvaluator {
	...
	public Object evaluate(Calc calc, Hierarchy[] independentHierarchies,
		...
		if (false) {
			System.out.println("original=" + saveMember.getUniqueName() + ", member="
					+ otherMember.getUniqueName() + ", originalResult=" + result
					+ ", result=" + otherResult);
		}
		if (!equals(otherResult, result)) {
			...
			throw Util.newInternal("Expression '" + mdxString
					...
					+ buf.toString() + "}; First result: " + toString(result)
					+ ", Second result: " + toString(otherResult));
		}
		// Restore context.
	...
	static class DteRoot extends RolapResult.RolapResultEvaluatorRoot {
		 ...
		 */
		private Member chooseOtherMember(final Member save,
	...
	private static class DteScalarCalcImpl extends GenericCalc {
		...
		DteScalarCalcImpl(Calc calc, Hierarchy[] independentHierarchies,
				String mdxString) {
