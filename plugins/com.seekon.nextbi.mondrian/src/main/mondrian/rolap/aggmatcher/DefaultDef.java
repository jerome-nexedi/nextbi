...
// on Sun Apr 22 18:24:21 CST 2012
...
/**
 * This is the XML model for defining default aggregate table recognition and
 * level/measure mapping. Revision is $Id:
 * //open/mondrian/src/main/mondrian/rolap/aggmatcher/DefaultRulesSchema.xml#11
 * $
 * <p>
 * This class was generated from XOM model 'aggregates' on Sun Apr 22 18:24:21
 * CST 2012
 */
public class DefaultDef {
  ...
  public static String[] _elements = { "AggRules", "Base", "CaseMatcher",
    "NameMatcher", "FactCountMatch", "ForeignKeyMatch", "TableMatch", "Mapper",
    "Regex", "RegexMapper", "Ref", "LevelMapRef", "MeasureMapRef", "IgnoreMapRef",
    "FactCountMatchRef", "ForeignKeyMatchRef", "TableMatchRef", "LevelMap",
    "MeasureMap", "IgnoreMap", "AggRule" };

  /**
   * The set of "named" rules for matching aggregate tables. Only one rule can
   * be applied to a given connection. In addition, one rule must be set as the
   * default - this rule is always the choice when not selecting by name. It is
   * very important that the AggRules validate method is called prior to using
   * any of the object.
   */
  public static class AggRules extends org.eigenbase.xom.ElementDef {
    ...
    /**
     * All AggRules (at least one). Also, one of them must be marked with
     * default=true.
     */
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("tag", tag, _cother.tag, _out,
        _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("tableMatches", tableMatches,
          _cother.tableMatches, _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("factCountMatches", factCountMatches,
          _cother.factCountMatches, _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("foreignKeyMatches", foreignKeyMatches,
          _cother.foreignKeyMatches, _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("levelMaps", levelMaps, _cother.levelMaps, _out,
          _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("measureMaps", measureMaps, _cother.measureMaps,
          _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("ignoreMaps", ignoreMaps, _cother.ignoreMaps,
          _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("aggRules", aggRules, _cother.aggRules, _out,
          _indent + 1);
      return _diff;
    }
  ...
  /**
   * Base is the base class for all of the elements. All elements can be enabled
   * or not, have a tag, and can be validated.
   */
  public static abstract class Base extends org.eigenbase.xom.ElementDef {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("Base", new org.eigenbase.xom.XMLAttrVector().add("enabled",
        enabled));
      _out.endTag("Base");
    }
  ...
  /**
   * This is a base class for all elements that can match strings where the case
   * of the string is important. In addition, it has an id which services as its
   * tag.
   */
  public static abstract class CaseMatcher extends Base {
    ...
    public CaseMatcher(org.eigenbase.xom.DOMWrapper _def)
      ...
      try {
        ...
        charcase = (String) _parser.getAttribute("charcase", "String", "ignore",
          _charcase_values, false);
        enabled = (Boolean) _parser.getAttribute("enabled", "Boolean", "true", null,
          false);
      } catch (org.eigenbase.xom.XOMException _ex) {
        throw new org.eigenbase.xom.XOMException("In " + getName() + ": "
    ...
    public static final String[] _charcase_values = { "ignore", "exact", "upper",
      "lower" };
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("CaseMatcher", new org.eigenbase.xom.XMLAttrVector().add("id",
        id).add("charcase", charcase).add("enabled", enabled));
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      _diff = _diff
        && displayAttributeDiff("charcase", charcase, _cother.charcase, _out,
          _indent + 1);
      return _diff;
    }
  ...
  /**
   * A NameMatcher is a CaseMatcher that prepends and appends regular
   * expressions to a given string as part of creating the matching regular
   * expression. Both the pre/post regular expression can be null in which case
   * matches are applied simply against the name (modulo case considerations).
   * The purpose of this class is to allow aggregate tables to be identified
   * when their table names are formed by placing text before and/or after the
  ...
  public static abstract class NameMatcher extends CaseMatcher {
    ...
    public NameMatcher(org.eigenbase.xom.DOMWrapper _def)
      ...
      try {
        org.eigenbase.xom.DOMElementParser _parser = new org.eigenbase.xom.DOMElementParser(
          _def, "", DefaultDef.class);
        pretemplate = (String) _parser.getAttribute("pretemplate", "String", null,
          null, false);
        posttemplate = (String) _parser.getAttribute("posttemplate", "String", null,
          null, false);
        basename = (String) _parser.getAttribute("basename", "String", null, null,
          false);
        id = (String) _parser.getAttribute("id", "String", null, null, true);
        charcase = (String) _parser.getAttribute("charcase", "String", "ignore",
          _charcase_values, false);
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("NameMatcher", new org.eigenbase.xom.XMLAttrVector().add(
        "pretemplate", pretemplate).add("posttemplate", posttemplate).add(
        "basename", basename).add("id", id).add("charcase", charcase).add("enabled",
        enabled));
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("pretemplate", pretemplate,
        _cother.pretemplate, _out, _indent + 1);
      _diff = _diff
        && displayAttributeDiff("posttemplate", posttemplate, _cother.posttemplate,
          _out, _indent + 1);
      _diff = _diff
        && displayAttributeDiff("basename", basename, _cother.basename, _out,
          _indent + 1);
      return _diff;
    }
    ...
    public void validate(final AggRules rules,
      ...
      try {
        ...
        if (basename != null) {
          baseNamePattern = java.util.regex.Pattern.compile(basename);
        }
      } finally {
        msgRecorder.popContextName();
      }
    ...
    /**
     * Generates a regular expression string by prepending and appending regular
     * expression to the parameter tableName.
     * 
     * @param name
     *          Table name
    ...
    public String getRegex(final String name) {
      ...
      if (name != null) {
        ...
        if (baseNamePattern != null) {
          java.util.regex.Matcher matcher = baseNamePattern.matcher(name);
    ...
    protected Recognizer.Matcher getMatcher(String name) {
      ...
      return new Recognizer.Matcher() {
        ...
        private void debug(String name) {
          ...
          bf.append("\"");
          if ((pattern.flags() & java.util.regex.Pattern.CASE_INSENSITIVE) != 0) {
  ...
  /**
   * This is used to identify the "fact_count" column in an aggregate table. It
   * allows one to match using regular exprssions. The default is that the name
   * of the fact count colum is simply the string "fact_count".
   */
  public static class FactCountMatch extends NameMatcher {
    ...
    public FactCountMatch(org.eigenbase.xom.DOMWrapper _def)
      ...
      try {
        org.eigenbase.xom.DOMElementParser _parser = new org.eigenbase.xom.DOMElementParser(
          _def, "", DefaultDef.class);
        factCountName = (String) _parser.getAttribute("factCountName", "String",
          "fact_count", null, true);
        pretemplate = (String) _parser.getAttribute("pretemplate", "String", null,
          null, false);
        posttemplate = (String) _parser.getAttribute("posttemplate", "String", null,
          null, false);
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("FactCountMatch", new org.eigenbase.xom.XMLAttrVector().add(
        "factCountName", factCountName).add("pretemplate", pretemplate).add(
        "posttemplate", posttemplate).add("basename", basename).add("id", id).add(
        "charcase", charcase).add("enabled", enabled));
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("factCountName", factCountName,
        _cother.factCountName, _out, _indent + 1);
      return _diff;
    }
  ...
  /**
   * This is used to identify foreign key columns in a candidate aggregate table
   * given the name of a foreign key column of the base fact table. This allows
   * such foreign keys to be identified by using a regular exprsssion. The
   * default is to simply match the base fact table's foreign key column name.
   */
  public static class ForeignKeyMatch extends NameMatcher {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("ForeignKeyMatch", new org.eigenbase.xom.XMLAttrVector().add(
        "pretemplate", pretemplate).add("posttemplate", posttemplate).add(
        "basename", basename).add("id", id).add("charcase", charcase).add("enabled",
        enabled));
  ...
  /**
   * This is used to identify which tables in the database might be aggregate
   * table of a given fact table. It is expected that aggregate table names will
   * include the base fact table name with additional text before and/or after.
   * It is not allow for both the prepending and appending regular expression
   * text to be null (if it were, then only aggregate tables who names were the
   * same as (modulo case) would match - which is surely not allowed).
   */
  public static class TableMatch extends NameMatcher {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("TableMatch", new org.eigenbase.xom.XMLAttrVector().add(
        "pretemplate", pretemplate).add("posttemplate", posttemplate).add(
        "basename", basename).add("id", id).add("charcase", charcase).add("enabled",
        enabled));
  ...
  /**
   * This allows one to create an element that matches against a single
   * template, where the template is an attribute. While much loved, this is
   * currently not used.
   */
  public static abstract class Mapper extends CaseMatcher {
    ...
    public Mapper(org.eigenbase.xom.DOMWrapper _def)
      ...
      try {
        org.eigenbase.xom.DOMElementParser _parser = new org.eigenbase.xom.DOMElementParser(
          _def, "", DefaultDef.class);
        template = (String) _parser.getAttribute("template", "String", null, null,
          true);
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("Mapper", new org.eigenbase.xom.XMLAttrVector().add("template",
        template).add("space", space).add("dot", dot).add("id", id).add("charcase",
        charcase).add("enabled", enabled));
      _out.endTag("Mapper");
    }
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("template", template, _cother.template,
        _out, _indent + 1);
      _diff = _diff
        && displayAttributeDiff("space", space, _cother.space, _out, _indent + 1);
      _diff = _diff
        && displayAttributeDiff("dot", dot, _cother.dot, _out, _indent + 1);
      return _diff;
    }
    ...
    /**
     * It is hoped that no one will need to match more than 50 names in a
     * template. Currently, this implementation, requires only 3.
     */
        ...
        while (count < MAX_SIZE) {
          if (start == -1) {
            if (count == 0) {
              ...
              // is an error
              String msg = "Bad template \"" + template + "\", no ${} entries";
              msgRecorder.reportError(msg);
              return;
            }
          ...
          if (end == -1) {
            ...
            String msg = "Bad template \"" + template
              + "\", it had a \"${\", but no '}'";
            msgRecorder.reportError(msg);
            return;
          }
  ...
  /**
   * This element is used in a vector of child elements when one wishes to have
   * one or more regular expressions associated with matching a given string.
   * The parent element must initialize Regex object by calling its validate
   * method passing in an array of template names. The cdata content is a
   * regular expression with embedded template names. Each name must be
   * surrounded by "${" and "}". Each time this is used for a new set of names,
   * the names replace the template names in the regular expression. For
   * example, if the charcase="lower", the attribute dot="-" (the default dot
   * value is "_"), the template names are: "city", "state", and "country" and
   * the cdata is: .*_${country}_.*_${city} Then when the names:
   * "San Francisco", "California", and "U.S.A" are passed in, the regular
   * expression becomes: .*_u-s-a_.*_san_francisco Note that a given template
   * name can only appear ONCE in the template content, the cdata content. As an
   * example, the following cdata template is not supported:
  ...
  public static class Regex extends CaseMatcher {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("Regex", new org.eigenbase.xom.XMLAttrVector().add("space",
        space).add("dot", dot).add("id", id).add("charcase", charcase).add(
        "enabled", enabled));
      _out.cdata(cdata);
      _out.endTag("Regex");
    }
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("space", space, _cother.space, _out,
        _indent + 1);
      _diff = _diff
        && displayAttributeDiff("dot", dot, _cother.dot, _out, _indent + 1);
      _diff = _diff
    ...
    public void validate(final AggRules rules, final String[] templateNames,
      final mondrian.recorder.MessageRecorder msgRecorder) {
      msgRecorder.pushContextName(getName());
      try {
        super.validate(rules, msgRecorder);
        ...
        if (templateNames.length == 0) {
          if (start == -1) {
            ...
            String msg = "Bad template \"" + template
              + "\", no ${} entries but there are " + "template names";
            msgRecorder.reportError(msg);
          }
          return;
        }
        while (count < MAX_SIZE) {
          ...
          String name = template.substring(start + 2, end);
          int pos = convertNameToID(name, templateNames, msgRecorder);
          if (pos == BAD_ID) {
            return;
          }
    ...
    private int convertNameToID(final String name, final String[] templateNames,
      final mondrian.recorder.MessageRecorder msgRecorder) {
  ...
  public static abstract class RegexMapper extends Base {
    ...
    /**
     * This is an array of Regex. A match occurs if any one of the Regex
     * matches; it is the equivalent of or-ing the regular expressions together.
     * A candidate string is processed sequentially by each Regex in their
     * document order until one matches. In none match, well, none match.
     */
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("RegexMapper", new org.eigenbase.xom.XMLAttrVector().add("id",
        id).add("enabled", enabled));
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("id", id, _cother.id, _out, _indent + 1);
      _diff = _diff
        && displayElementArrayDiff("regexs", regexs, _cother.regexs, _out,
          _indent + 1);
      return _diff;
    }
    ...
    /**
     * This must be defined in derived classes. It returns an array of symbolic
     * names that are the symbolic names allowed to appear in the regular
    ...
    protected Recognizer.Matcher getMatcher(final String[] names) {
      ...
      final java.util.regex.Pattern[] patterns = new java.util.regex.Pattern[regexs.length];
      ...
      return new Recognizer.Matcher() {
        ...
        private void debug(String name, java.util.regex.Pattern p) {
          ...
          bf.append(p.pattern());
          bf.append("\"");
          if ((p.flags() & java.util.regex.Pattern.CASE_INSENSITIVE) != 0) {
            bf.append(" case_insensitive");
          }
  ...
  public static abstract class Ref extends Base {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("Ref", new org.eigenbase.xom.XMLAttrVector().add("refId", refId)
        .add("enabled", enabled));
      _out.endTag("Ref");
    }
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("refId", refId, _cother.refId, _out,
        _indent + 1);
      return _diff;
    }
  ...
  public static class LevelMapRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("LevelMapRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasLevelMap(getRefId())) {
          String msg = "No LevelMap has id equal to refid \"" + getRefId() + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
        msgRecorder.popContextName();
  ...
  public static class MeasureMapRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("MeasureMapRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasMeasureMap(getRefId())) {
          String msg = "No MeasureMap has id equal to refid \"" + getRefId() + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
        msgRecorder.popContextName();
  ...
  public static class IgnoreMapRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("IgnoreMapRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasIgnoreMap(getRefId())) {
          String msg = "No IgnoreMap has id equal to refid \"" + getRefId() + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
        msgRecorder.popContextName();
  ...
  public static class FactCountMatchRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("FactCountMatchRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasFactCountMatch(getRefId())) {
          String msg = "No FactCountMatch has id equal to refid \"" + getRefId()
            + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
  ...
  public static class ForeignKeyMatchRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("ForeignKeyMatchRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasForeignKeyMatch(getRefId())) {
          String msg = "No ForeignKeyMatch has id equal to refid \"" + getRefId()
            + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
  ...
  public static class TableMatchRef extends Ref {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("TableMatchRef", new org.eigenbase.xom.XMLAttrVector().add(
        "refId", refId).add("enabled", enabled));
    ...
    public void validate(final AggRules rules,
      ...
      try {
        if (!rules.hasTableMatch(getRefId())) {
          String msg = "No TableMatch has id equal to refid \"" + getRefId() + "\"";
          msgRecorder.reportError(msg);
        }
      } finally {
        msgRecorder.popContextName();
  ...
  /**
   * This is the template that maps from a combination of level usage_prefix
   * hierarchy_name level_name level_column_name
   */
  public static class LevelMap extends RegexMapper {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("LevelMap", new org.eigenbase.xom.XMLAttrVector().add("id", id)
        .add("enabled", enabled));
      displayXMLElementArray(_out, regexs);
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayElementArrayDiff("regexs", regexs, _cother.regexs,
        _out, _indent + 1);
      return _diff;
    }

    // BEGIN pass-through code block ---
    private static final String[] TEMPLATE_NAMES = new String[] { "usage_prefix",
      "hierarchy_name", "level_name", "level_column_name" };
    ...
    protected Recognizer.Matcher getMatcher(final String usagePrefix,
      final String hierarchyName, final String levelName,
      final String levelColumnName) {
      return getMatcher(new String[] { usagePrefix, hierarchyName, levelName,
        levelColumnName });
    }
    // END pass-through code block ---
  }
  ...
  /**
   * This is the template that maps from a combination of measure measure_name,
   * measure_column_name, and aggregate_name ("count", "sum", "avg", "min",
   * "max", "distinct-count").
   */
  public static class MeasureMap extends RegexMapper {
    ...
    // BEGIN pass-through code block ---
    private static final String[] TEMPLATE_NAMES = new String[] { "measure_name",
      "measure_column_name", "aggregate_name" };

    protected String[] getTemplateNames() {
      return TEMPLATE_NAMES;
    }

    protected Recognizer.Matcher getMatcher(final String measureName,
      final String measuerColumnName, final String aggregateName) {
      return getMatcher(new String[] { measureName, measuerColumnName, aggregateName });
    }
    // END pass-through code block ---
  }

  /**
   * This is the template used to specify columns to be ignored. There are NO
   * template names. One simply uses a regular expression.
   */
  public static class IgnoreMap extends RegexMapper {
    ...
    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("IgnoreMap", new org.eigenbase.xom.XMLAttrVector().add("id", id)
        .add("enabled", enabled));
      displayXMLElementArray(_out, regexs);
  ...
  /**
   * A RolapConnection uses one AggRule. If no name is specified, then the
   * AggRule which is marked as default==true is used (validation fails if one
   * and only one AggRule is not marked as the default). An AggRule has
   * manditory child elements for matching the aggregate table names, aggregate
   * table fact count column, foreign key columns, the measure columns, and the
   * hierarchy level columns. These child elements can be specified as direct
   * children of an AggRule element or by reference to elements defined as a
   * pier to the AggRule (using references allows reuse of the child elements
   * and with one quick edit the reference to use can be changed by changing the
  ...
  public static class AggRule extends Base {
    ...
    public AggRule(org.eigenbase.xom.DOMWrapper _def)
      ...
      try {
        org.eigenbase.xom.DOMElementParser _parser = new org.eigenbase.xom.DOMElementParser(
          _def, "", DefaultDef.class);
        tag = (String) _parser.getAttribute("tag", "String", null, null, true);
        countColumn = (String) _parser.getAttribute("countColumn", "String",
          "fact_count", null, true);
        enabled = (Boolean) _parser.getAttribute("enabled", "Boolean", "true", null,
          false);
        ...
        factCountMatch = (FactCountMatch) _parser.getElement(FactCountMatch.class,
          false);
        factCountMatchRef = (FactCountMatchRef) _parser.getElement(
          FactCountMatchRef.class, false);
        foreignKeyMatch = (ForeignKeyMatch) _parser.getElement(
          ForeignKeyMatch.class, false);
        foreignKeyMatchRef = (ForeignKeyMatchRef) _parser.getElement(
          ForeignKeyMatchRef.class, false);
        tableMatch = (TableMatch) _parser.getElement(TableMatch.class, false);
        tableMatchRef = (TableMatchRef) _parser.getElement(TableMatchRef.class,
          false);
        ...
        measureMapRef = (MeasureMapRef) _parser.getElement(MeasureMapRef.class,
          false);
      } catch (org.eigenbase.xom.XOMException _ex) {
        throw new org.eigenbase.xom.XOMException("In " + getName() + ": "
          + _ex.getMessage());
    ...
    public void display(java.io.PrintWriter _out, int _indent) {
      ...
      displayAttribute(_out, "enabled", enabled, _indent + 1);
      displayElement(_out, "ignoreMap", (org.eigenbase.xom.ElementDef) ignoreMap,
        _indent + 1);
      displayElement(_out, "ignoreMapRef",
        (org.eigenbase.xom.ElementDef) ignoreMapRef, _indent + 1);
      displayElement(_out, "factCountMatch",
        (org.eigenbase.xom.ElementDef) factCountMatch, _indent + 1);
      displayElement(_out, "factCountMatchRef",
        (org.eigenbase.xom.ElementDef) factCountMatchRef, _indent + 1);
      displayElement(_out, "foreignKeyMatch",
        (org.eigenbase.xom.ElementDef) foreignKeyMatch, _indent + 1);
      displayElement(_out, "foreignKeyMatchRef",
        (org.eigenbase.xom.ElementDef) foreignKeyMatchRef, _indent + 1);
      displayElement(_out, "tableMatch", (org.eigenbase.xom.ElementDef) tableMatch,
        _indent + 1);
      displayElement(_out, "tableMatchRef",
        (org.eigenbase.xom.ElementDef) tableMatchRef, _indent + 1);
      displayElement(_out, "levelMap", (org.eigenbase.xom.ElementDef) levelMap,
        _indent + 1);
      displayElement(_out, "levelMapRef",
        (org.eigenbase.xom.ElementDef) levelMapRef, _indent + 1);
      displayElement(_out, "measureMap", (org.eigenbase.xom.ElementDef) measureMap,
        _indent + 1);
      displayElement(_out, "measureMapRef",
        (org.eigenbase.xom.ElementDef) measureMapRef, _indent + 1);
    }

    public void displayXML(org.eigenbase.xom.XMLOutput _out, int _indent) {
      _out.beginTag("AggRule", new org.eigenbase.xom.XMLAttrVector().add("tag", tag)
        .add("countColumn", countColumn).add("enabled", enabled));
    ...
    public boolean displayDiff(org.eigenbase.xom.ElementDef _other,
      ...
      boolean _diff = displayAttributeDiff("tag", tag, _cother.tag, _out,
        _indent + 1);
      _diff = _diff
        && displayAttributeDiff("countColumn", countColumn, _cother.countColumn,
          _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("ignoreMap", ignoreMap, _cother.ignoreMap, _out,
          _indent + 1);
      _diff = _diff
        && displayElementDiff("ignoreMapRef", ignoreMapRef, _cother.ignoreMapRef,
          _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("factCountMatch", factCountMatch,
          _cother.factCountMatch, _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("factCountMatchRef", factCountMatchRef,
          _cother.factCountMatchRef, _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("foreignKeyMatch", foreignKeyMatch,
          _cother.foreignKeyMatch, _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("foreignKeyMatchRef", foreignKeyMatchRef,
          _cother.foreignKeyMatchRef, _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("tableMatch", tableMatch, _cother.tableMatch, _out,
          _indent + 1);
      _diff = _diff
        && displayElementDiff("tableMatchRef", tableMatchRef, _cother.tableMatchRef,
          _out, _indent + 1);
      _diff = _diff
        && displayElementDiff("levelMap", levelMap, _cother.levelMap, _out,
          _indent + 1);
      _diff = _diff
        && displayElementDiff("levelMapRef", levelMapRef, _cother.levelMapRef, _out,
          _indent + 1);
      _diff = _diff
        && displayElementDiff("measureMap", measureMap, _cother.measureMap, _out,
          _indent + 1);
      _diff = _diff
        && displayElementDiff("measureMapRef", measureMapRef, _cother.measureMapRef,
          _out, _indent + 1);
      return _diff;
    }
    ...
    private boolean isRef(final AggRules rules,
      final mondrian.recorder.MessageRecorder msgRecorder, final Base base,
      final Base baseRef, final String baseName) {
      if (!isOk(base)) {
        if (isOk(baseRef)) {
          baseRef.validate(rules, msgRecorder);
          return true;
        } else {
          String msg = "Neither base " + baseName + " or baseref " + baseName
            + "Ref is ok";
          msgRecorder.reportError(msg);
          return false;
        }
      } else if (isOk(baseRef)) {
        String msg = "Both base " + base.getName() + " and baseref "
          + baseRef.getName() + " are ok";
        msgRecorder.reportError(msg);
        return false;
      } else {
    ...
    public void validate(final AggRules rules,
      ...
      try {
        ...
        if (ignoreMap != null) {
          ...
          ignoreMap = rules.lookupIgnoreMap(ignoreMapRef.getRefId());
        }
        if (isRef(rules, msgRecorder, factCountMatch, factCountMatchRef,
          "FactCountMatch")) {
          factCountMatch = rules.lookupFactCountMatch(factCountMatchRef.getRefId());
        }
        if (isRef(rules, msgRecorder, foreignKeyMatch, foreignKeyMatchRef,
          "ForeignKeyMatch")) {
          foreignKeyMatch = rules.lookupForeignKeyMatch(foreignKeyMatchRef
            .getRefId());
        }
        if (isRef(rules, msgRecorder, tableMatch, tableMatchRef, "TableMatch")) {
          tableMatch = rules.lookupTableMatch(tableMatchRef.getRefId());
        }
        if (isRef(rules, msgRecorder, levelMap, levelMapRef, "LevelMap")) {
          levelMap = rules.lookupLevelMap(levelMapRef.getRefId());
        }
        if (isRef(rules, msgRecorder, measureMap, measureMapRef, "MeasureMap")) {
          measureMap = rules.lookupMeasureMap(measureMapRef.getRefId());
        }
      } finally {
        msgRecorder.popContextName();
      }
