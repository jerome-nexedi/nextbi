/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/JdbcSchema.java#33 $
 ...
 * 
 * <p>
 * Tables are created when calling code requests the set of available tables.
 * This call <code>getTables()</code> causes all tables to be loaded. But a
 * table's columnIter are not loaded until, on a table-by-table basis, a request
 * is made to get the set of columnIter associated with the table. Since, the
 * AggTableManager first attempts table name matches (recognition) most tables
 * do not match, so why load their columnIter. Of course, as a result, there are
 * a host of methods that can throw an {@link SQLException}, rats.
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/JdbcSchema.java
 *          #33 $
 */
public class JdbcSchema {
  ...
  /**
   * Every SWEEP_COUNT calls to this method, go through all elements of the
   * dbMap removing all that either have null values (null SoftReference) or
   * those with SoftReference with null content.
   */
   ...
   */
  enum UsageType {
    UNKNOWN, FOREIGN_KEY, MEASURE, LEVEL, FACT_COUNT, IGNORE
  }
   ...
   * 
   * @param javaType
   *          JDBC type code, as per {@link java.sql.Types}
   * @return Datatype
   */
  ...
  public class Table {
    ...
    public class Column {
      ...
      public class Usage {
        ...
        /**
         * The prefix (possibly null) to use during aggregate table generation
         * (See AggGen).
         */
        ...
        /**
         * Sets the symbolic (logical) name associated with this usage. For
         * example, this might be the measure's name.
         * 
         * @param symbolicName
         *          Symbolic name
         */
        ...
        /**
         * Sets the aggregator associated with this usage (if it is a measure
         * usage).
         * 
         * @param aggregator
         *          Aggregator
        ...
        /**
         * Returns the aggregator associated with this usage (if its a measure
       ...
       */
      private final Set<UsageType> usageTypes = Olap4jUtil
        .enumSetNoneOf(UsageType.class);

      private Column(final String name) {
        this.name = name;
        this.column = new MondrianDef.Column(JdbcSchema.Table.this.getName(), name);
      ...
      /**
       * How many usages does this column have. A column has between 0 and N
       * usages. It has no usages if usages is some administrative column. It
       * has one usage if, for example, its the fact_count column or a level
       * column (for a collapsed dimension aggregate). It might have 2 usages if
       * its a foreign key that is also used as a measure. If its a column used
       * in N measures, then usages will have N usages.
       */
      ...
      /**
       * Return true if the column has at least one usage of the given column
       * type.
       */
      ...
      public Iterator<Usage> getUsages(UsageType usageType) {
        ...
        class ColumnTypeIterator implements Iterator<Usage> {
          ...
          ColumnTypeIterator(final List<Usage> usages, final UsageType columnType) {
    ...
    /**
     * Whether the table is a fact, aggregate or other table type. Note: this
     * assumes that a table has only ONE usage.
     */
    private TableUsageType tableUsageType;

    /**
     * Typical table types are: "TABLE", "VIEW", "SYSTEM TABLE",
     * "GLOBAL TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM". (Depends what
     * comes out of JDBC.)
     */
    ...
    public void setTableUsageType(final TableUsageType tableUsageType) {
      ...
      if ((this.tableUsageType != TableUsageType.UNKNOWN)
        ...
        throw mres.AttemptToChangeTableUsage.ex(getName(), this.tableUsageType
          .name(), tableUsageType.name());
      }
      this.tableUsageType = tableUsageType;
    }
    ...
    /**
     * Returns all of the columnIter associated with a table and creates Column
  ...
  /**
   * Tables by name. We use a sorted map so {@link #getTables()}'s output is in
   * deterministic order.
   */
  ...
  /**
   * Used for testing allowing one to load tables and their columnIter from more
   * than one datasource
   */
  ...
  /**
   * Gets all of the tables (and views) in the database. If called a second
  ...
  private void loadTables() throws SQLException {
    ...
    try {
      ...
      if (databaseMetaData.getDatabaseProductName().toUpperCase().indexOf("VERTICA") >= 0) {
   ...
   * etc.)
   */
  private void loadTablesOfType(DatabaseMetaData databaseMetaData,
    ...
    ResultSet rs = null;
    try {
      rs = databaseMetaData.getTables(catalog, schema, tableName, tableTypes);
      if (rs == null) {
