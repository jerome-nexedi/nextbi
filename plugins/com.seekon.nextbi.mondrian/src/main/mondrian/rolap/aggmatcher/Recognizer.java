...
abstract class Recognizer {
	...
	protected Recognizer(final RolapStar star,
			final JdbcSchema.Table dbFactTable, final JdbcSchema.Table aggTable,
	...
	protected void checkFactCount() {
		...
		try {
				...
				if (factCountMatcher.matches(aggColumn.getName())) {
					if (aggColumn.getDatatype().isNumeric()) {
						...
						String msg = mres.NonNumericFactCountColumn.str(aggTable.getName(),
								dbFactTable.getName(), aggColumn.getName(),
			...
			if (nosOfFactCounts == 0) {
				String msg = mres.NoFactCountColumns.str(aggTable.getName(),
						dbFactTable.getName());
				msgRecorder.reportError(msg);
	...
	protected void checkNosMeasures(int nosMeasures) {
		...
		try {
			if (nosMeasures == 0) {
				String msg = mres.NoMeasureColumns.str(aggTable.getName(),
						dbFactTable.getName());
				msgRecorder.reportError(msg);
	...
	protected List<JdbcSchema.Table.Column.Usage> checkForeignKeys() {
		...
		try {
			...
			for (Iterator<JdbcSchema.Table.Column.Usage> it = dbFactTable
				...
				if (matchCount > 1) {
					String msg = mres.TooManyMatchingForeignKeyColumns.str(aggTable
							.getName(), dbFactTable.getName(), matchCount, factUsage
							.getColumn().getName());
					msgRecorder.reportError(msg);
				...
				for (Iterator<JdbcSchema.Table.Column.Usage> uit = aggColumn
					...
					if (!aggUsage.relation.equals(rel)
						...
						String msg = mres.DoubleMatchForLevel.str(aggTable.getName(),
								dbFactTable.getName(), aggColumn.getName(),
								aggUsage.relation.toString(), aggColumn.column.name,
								rel.toString(), cName);
						msgRecorder.reportError(msg);
	...
	protected void checkUnusedColumns() {
		...
		for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
			if (!aggColumn.hasUsage()) {
				String msg = mres.AggUnknownColumn.str(aggTable.getName(),
						dbFactTable.getName(), aggColumn.getName());
	...
	protected RolapAggregator convertAggregator(
			final JdbcSchema.Table.Column.Usage aggUsage,
			final RolapAggregator factAgg, final RolapAggregator siblingAgg) {
		...
		if (rollupAgg == null) {
			String msg = mres.NoAggregatorFound.str(aggUsage.getSymbolicName(),
					factAgg.getName(), siblingAgg.getName());
			msgRecorder.reportError(msg);
		}
