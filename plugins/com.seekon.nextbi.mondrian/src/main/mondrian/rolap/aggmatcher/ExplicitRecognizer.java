...
class ExplicitRecognizer extends Recognizer {
	...
	ExplicitRecognizer(final ExplicitRules.TableDef tableDef,
			final RolapStar star, RolapCube cube, final JdbcSchema.Table dbFactTable,
	...
	protected int checkMeasures() {
		...
		try {
			...
			for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
				...
				for (ExplicitRules.TableDef.Measure measure : getTableDef()
						.getMeasures()) {
	...
	protected void makeMeasure(final ExplicitRules.TableDef.Measure measure,
		...
		RolapAggregator ra = (factAgg == null) ? convertAggregator(aggUsage,
				rm.getAggregator()) : convertAggregator(aggUsage, factAgg,
				rm.getAggregator());
				...
				if (level.getName().equals(levelUniqueName)) {
					...
					for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
						if (aggColumn.getName().equals(columnName)) {
							makeLevel(aggColumn, hierarchy, hierarchyUsage,
									getColumnName(rlevel.getKeyExp()), columnName,
									rlevel.getName());
							return true;
						}
					}
