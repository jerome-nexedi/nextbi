/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggTableManager.java#38 $
 ...
 * <li>The {@link mondrian.rolap.RolapSchema} calls {@link #initialize()}, which
 * scans the JDBC catalog and identifies aggregate tables.
 * <li>For each aggregate table, it creates an {@link AggStar} and calls
 * {@link RolapStar#addAggStar(AggStar)}.
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggTableManager
 *          .java#38 $
 */
public class AggTableManager {
	...
	/**
	 * This is used to create forward references to triggers (so they do not get
	 * reaped until the RolapSchema is reaped).
	 */
	...
	/**
	 * This should ONLY be called if the AggTableManager is no longer going to be
	 * used. In fact, it should only be called indirectly by its associated
	 * RolapSchema object.
	 */
	public void finalCleanUp() {
		...
		if (getLogger().isDebugEnabled()) {
			getLogger().debug(
					"AggTableManager.finalCleanUp: schema=" + schema.getName());
		}
	}
	...
	/**
	 * Initializes this object, loading all aggregate tables and associating them
	 * with {@link RolapStar}s. This method should only be called once.
	 */
	...
	private void printResults() {
		/*
		 * This was too much information at the INFO level, compared to the rest of
		 * Mondrian
		 * 
		 * if (getLogger().isInfoEnabled()) { // print just Star table alias and
		 * AggStar table names StringBuilder buf = new StringBuilder(1024);
		 * buf.append(Util.nl); for (Iterator it = getStars(); it.hasNext();) {
		 * RolapStar star = (RolapStar) it.next();
		 * buf.append(star.getFactTable().getAlias()); buf.append(Util.nl); for
		 * (Iterator ait = star.getAggStars(); ait.hasNext();) { AggStar aggStar =
		 * (AggStar) ait.next(); buf.append("    ");
		 * buf.append(aggStar.getFactTable().getName()); buf.append(Util.nl); } }
	 ...
	 * <p>
	 * NOTE: At this point all RolapStars have been made for this schema (except
	 * for dynamically added cubes which I am going to ignore for right now). So,
	 * All stars have their columns and their BitKeys can be generated.
	 * 
	 * @throws SQLException
	 */
			...
			synchronized (db) {
				...
				loop: for (RolapStar star : getStars()) {
					...
					for (JdbcSchema.Table dbTable : db.getTables()) {
						...
						ExplicitRules.TableDef tableDef = ExplicitRules
								.getIncludeByTableDef(name, aggGroups);
						...
						if (tableDef != null) {
							// load columns
							dbTable.load();
							makeAggStar = tableDef.columnsOK(star, dbFactTable, dbTable,
									msgRecorder);
						...
						if (!makeAggStar) {
							...
							if (rules.matchesTableName(factTableName, name)) {
								// load columns
								dbTable.load();
								makeAggStar = rules.columnsOK(star, dbFactTable, dbTable,
										msgRecorder);
							}
						}
						...
						if (makeAggStar) {
							...
							if (aggStar.getSize() > 0) {
								...
								getLogger().warn(
										mres.AggTableZeroSize.str(aggStar.getFactTable().getName(),
												factTableName));
							}
						}
	 ...
	 * {@link mondrian.rolap.aggmatcher.ExplicitRules.Group} in every cubes in a
	 * given {@link RolapStar}.
	 */
	protected List<ExplicitRules.Group> getAggGroups(RolapStar star) {
		List<ExplicitRules.Group> aggGroups = new ArrayList<ExplicitRules.Group>();
	 ...
	 * example, a measure in the RolapStar becomes a measure usage for the column
	 * with the same name and a RolapStar foreign key column becomes a foreign key
	 * usage for the column with the same name.
	 * 
	 * @param dbFactTable
	 * @param star
	 * @param msgRecorder
	 */
	void bindToStar(final JdbcSchema.Table dbFactTable, final RolapStar star,
			...
			for (JdbcSchema.Table.Column factColumn : dbFactTable.getColumns()) {
				...
				for (RolapStar.Column rc : rcs) {
					...
					if (rc instanceof RolapStar.Measure) {
						...
						JdbcSchema.Table.Column.Usage usage = factColumn
				...
				RolapStar.Table rTable = star.getFactTable()
				...
				if (!factColumn.hasUsage() && getLogger().isDebugEnabled()) {
					getLogger().debug(
							mres.UnknownFactTableColumn.str(msgRecorder.getContext(),
									dbFactTable.getName(), factColumn.getName()));
				}
			}
		} finally {
