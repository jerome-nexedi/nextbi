/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/DefaultRecognizer.java#18 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/DefaultRecognizer
 *          .java#18 $
 */
class DefaultRecognizer extends Recognizer {
  ...
  DefaultRecognizer(final DefaultRules aggDefault, final RolapStar star,
  ...
  protected Recognizer.Matcher getMeasureMatcher(
    ...
    return getRules().getMeasureMatcher(measureName, measureColumnName,
      aggregateName);
  }
   ...
   * <p>
   * First, iterator through all fact table measure usages. Create a Matcher for
   * each such usage. Iterate through all aggregate table columns. For each
   * column that matches create a measure usage.
   * <p>
   * Per fact table measure usage, at most only one aggregate measure should be
   * created.
   * 
  ...
  protected int checkMeasures() {
    ...
    try {
      ...
      for (Iterator<JdbcSchema.Table.Column.Usage> it = dbFactTable
        ...
        if (matchCount > 1) {
          String msg = mres.AggMultipleMatchingMeasure.str(msgRecorder.getContext(),
            aggTable.getName(), dbFactTable.getName(), matchCount, factUsage
              .getSymbolicName(), factUsage.getColumn().getName(), factUsage
              .getAggregator().getName());
   ...
   * aggregate table's columns are searched for one that matches. For each that
   * matches a foreign key usage is created (thought if more than one is created
   * its is an error which is handled in the calling code.
   */
  protected int matchForeignKey(JdbcSchema.Table.Column.Usage factUsage) {
    ...
    Recognizer.Matcher matcher = getRules().getForeignKeyMatcher(
      factColumn.getName());
   ...
   * 
   * <p>
   * A Matcher is created using the Hierarchy's name, the RolapLevel name, and
   * the column name associated with the RolapLevel's key expression. The
   * aggregate table columns are search for the first match and, if found, a
   * level usage is created for that column and true is returned.
   */
  protected boolean matchLevel(final Hierarchy hierarchy,
    ...
    try {
      ...
      Recognizer.Matcher matcher = getRules().getLevelMatcher(usagePrefix, hierName,
        levelName, levelColumnName);

      for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
        if (matcher.matches(aggColumn.getName())) {
          makeLevel(aggColumn, hierarchy, hierarchyUsage, getColumnName(level
            .getKeyExp()), getColumnName(level.getKeyExp()), level.getName());
          return true;
        }
      }
      return false;
