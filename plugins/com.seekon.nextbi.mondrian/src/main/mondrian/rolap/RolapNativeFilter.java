/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapNativeFilter.java#33 $
 ...
 * @since Nov 21, 2005
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapNativeFilter.java#33 $
 */
public class RolapNativeFilter extends RolapNativeSet {
	...
	static class FilterConstraint extends SetConstraint {
		...
		public FilterConstraint(CrossJoinArg[] args, RolapEvaluator evaluator,
				Exp filterExpr) {
		 ...
		 * 
		 * <p>
		 * A FilterConstraint always needs to join the fact table because we want to
		 * evaluate the filter expression against a fact.
		 */
	...
	NativeEvaluator createEvaluator(RolapEvaluator evaluator, FunDef fun,
		...
		List<CrossJoinArg[]> allArgs = crossJoinArgFactory().checkCrossJoinArg(
				evaluator, args[0]);
		...
		RolapNativeSql sql = new RolapNativeSql(sqlQuery, null, evaluator,
				cjArgs[0].getLevel());
		...
		if (SqlConstraintUtils.containsCalculatedMember(evaluator
				.getNonAllMembers())) {
			return null;
		}
		...
		TupleConstraint constraint = new FilterConstraint(combinedArgs, evaluator,
				filterExpr);
