/*
// $Id: //open/mondrian/src/main/mondrian/rolap/sql/CrossJoinArgFactory.java#9 $
 ...
 * @author kwalker
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/sql/CrossJoinArgFactory.
 *          java#9 $
 * @since Dec 15, 2009
 */
public class CrossJoinArgFactory {
	protected static final Logger LOGGER = Logger
			.getLogger(CrossJoinArgFactory.class);
	...
	public Set<CrossJoinArg> buildConstraintFromAllAxes(
		...
		Set<CrossJoinArg> joinArgs = new LinkedHashSet<CrossJoinArg>();
		for (QueryAxis ax : evaluator.getQuery().getAxes()) {
			List<CrossJoinArg[]> axesArgs = checkCrossJoinArg(evaluator, ax.getSet(),
					true);
			if (axesArgs != null) {
	 ...
	 * @param exp
	 *          Expresssion
	 * @return List of CrossJoinArg arrays. The first array represent the CJ
	 *         CrossJoinArg and the second array represent the additional
	 *         constraints.
	 */
	...
	private CrossJoinArg[] checkConstrainedMeasures(RolapEvaluator evaluator,
			FunDef fun, Exp[] args) {
		if (isSetOfConstrainedMeasures(fun, args)) {
			HashMap<Dimension, List<RolapMember>> memberLists = new LinkedHashMap<Dimension, List<RolapMember>>();
			for (Exp arg : args) {
	...
	private boolean isConstrainedMeasure(Exp arg) {
		if (!(arg instanceof MemberExpr && ((MemberExpr) arg).getMember()
				.isMeasure())) {
		...
		if (member instanceof RolapCalculatedMember) {
			Exp calcExp = ((RolapCalculatedMember) member).getFormula()
					.getExpression();
			return ((calcExp instanceof ResolvedFunCall && ((ResolvedFunCall) calcExp)
					.getFunDef() instanceof TupleFunDef)) || calcExp instanceof Literal;
		}
		return false;
	}

	private void addConstrainingMembersToMap(Exp arg,
	...
	private Exp[] getCalculatedTupleArgs(Exp arg) {
		if (arg instanceof MemberExpr) {
			Member member = ((MemberExpr) arg).getMember();
			if (member instanceof RolapCalculatedMember) {
				Exp formulaExp = ((RolapCalculatedMember) member).getFormula()
						.getExpression();
	...
	private CrossJoinArg[] memberListCrossJoinArgArray(
			Map<Dimension, List<RolapMember>> memberLists, Exp[] args,
		...
		for (List<RolapMember> memberList : memberLists.values()) {
			if (memberList.size() == countNonLiteralMeasures(args)) {
				...
				// for this dimension.
				final CrossJoinArg cjArg = MemberListCrossJoinArg.create(evaluator,
						removeDuplicates(memberList), restrictMemberTypes(), false);
				if (cjArg != null) {
					argList.add(cjArg);
				}
			}
	...
	private int countNonLiteralMeasures(Exp[] length) {
		...
		for (Exp exp : length) {
			if (exp instanceof MemberExpr) {
				...
				count += countNonLiteralMeasures(((ResolvedFunCall) exp).getArgs());
			}
		}
		return count;
	}
	...
	/**
	 * Checks for <code>CrossJoin(&lt;set1&gt;, &lt;set2&gt;)</code>, where set1
	 * and set2 are one of <code>member.children</code>,
	 * <code>level.members</code> or <code>member.descendants</code>.
	 * 
	 * @param evaluator
	 *          Evaluator to use if inputs are to be evaluated
	 * @param fun
	 *          The function, either "CrossJoin" or "NonEmptyCrossJoin"
	 * @param args
	 *          Inputs to the CrossJoin
	 * @param returnAny
	 *          indicates we should return any valid crossjoin args
	 * @return array of CrossJoinArg representing the inputs
	 */
	public List<CrossJoinArg[]> checkCrossJoin(RolapEvaluator evaluator,
			FunDef fun, Exp[] args, final boolean returnAny) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (allArgsOneInput == null || allArgsOneInput.isEmpty()
		...
		List<CrossJoinArg[]> allArgsBothInputs = new ArrayList<CrossJoinArg[]>();
		// Now combine the cjArgs from both sides
		CrossJoinArg[] combinedCJArgs = Util.appendArrays(
				cjArgsBothInputs[0] == null ? CrossJoinArg.EMPTY_ARRAY
						: cjArgsBothInputs[0],
				cjArgsBothInputs[1] == null ? CrossJoinArg.EMPTY_ARRAY
						: cjArgsBothInputs[1]);
		allArgsBothInputs.add(combinedCJArgs);

		CrossJoinArg[] combinedPredicateArgs = Util.appendArrays(
				predicateArgsBothInputs[0] == null ? CrossJoinArg.EMPTY_ARRAY
						: predicateArgsBothInputs[0],
				predicateArgsBothInputs[1] == null ? CrossJoinArg.EMPTY_ARRAY
	 ...
	 * 
	 * @return an {@link mondrian.rolap.sql.CrossJoinArg} instance describing the
	 *         enumeration, or null if <code>fun</code> represents something else.
	 */
	private CrossJoinArg[] checkEnumeration(RolapEvaluator evaluator, FunDef fun,
			Exp[] args, boolean exclude) {
		...
		final CrossJoinArg cjArg = MemberListCrossJoinArg.create(evaluator,
				memberList, restrictMemberTypes(), exclude);
		if (cjArg == null) {
			return null;
		}
	 ...
	 * 
	 * @return an {@link mondrian.rolap.sql.CrossJoinArg} instance describing the
	 *         member.children function, or null if <code>fun</code> represents
	 *         something else.
	 */
	 ...
	 * 
	 * @return an {@link mondrian.rolap.sql.CrossJoinArg} instance describing the
	 *         Level.members function, or null if <code>fun</code> represents
	 *         something else.
	 */
	...
	private static boolean isArgSizeSupported(RolapEvaluator evaluator,
			int argSize) {
	 ...
	 * 
	 * @return an {@link mondrian.rolap.sql.CrossJoinArg} instance describing the
	 *         Descendants function, or null if <code>fun</code> represents
	 *         something else.
	 */
	...
	/**
	 * Check if a dimension filter can be natively evaluated. Currently, these
	 * types of filters can be natively evaluated: Filter(Set, Qualified
	 * Predicate) where Qualified Predicate is either CurrentMember reference IN
	 * {m1, m2}, CurrentMember reference Is m1, negation(NOT) of qualified
	 * predicate conjuction(AND) of qualified predicates and where currentmember
	 * reference is either a member or ancester of a member from the context,
	 * 
	 * @param evaluator
	 *          Evaluator
	 * @param fun
	...
	private List<CrossJoinArg[]> checkDimensionFilter(RolapEvaluator evaluator,
		...
		if (filterArgs[1] instanceof ResolvedFunCall) {
			...
			currentPredicateArgs = checkFilterPredicate(evaluator, predicateCall,
					exclude);
		} else {
		...
		if (previousPredicateArgs != null) {
			combinedPredicateArgs = Util.appendArrays(previousPredicateArgs,
	...
	/**
	 * Checks whether the filter predicate can be turned into native SQL. See
	 * comment for checkDimensionFilter for the types of predicates suported.
	 * 
	 * @param evaluator
	 *          Evaluator
	 * @param predicateCall
	 *          Call to predicate function (ANd, NOT or parentheses)
	 * @param exclude
	 *          Whether to exclude tuples that match the predicate
	 * @return if filter predicate can be natively evaluated, the CrossJoinArg
	 *         array representing the predicate; otherwise, null.
	 */
	private CrossJoinArg[] checkFilterPredicate(RolapEvaluator evaluator,
			ResolvedFunCall predicateCall, boolean exclude) {
		...
		if (predicateCall.getFunName().equals("()")) {
			...
			if (actualPredicateCall instanceof ResolvedFunCall) {
				return checkFilterPredicate(evaluator,
						(ResolvedFunCall) actualPredicateCall, exclude);
			} else {
				return null;
			}
		}
		...
		if (predicateCall.getFunName().equals("AND")) {
			...
			if (andArg0 instanceof ResolvedFunCall
				...
				andCJArgs0 = checkFilterPredicate(evaluator, (ResolvedFunCall) andArg0,
						exclude);
				if (andCJArgs0 != null) {
					andCJArgs1 = checkFilterPredicate(evaluator,
							(ResolvedFunCall) andArg1, exclude);
					if (andCJArgs1 != null) {
		...
		predicateCJArgs = checkFilterPredicateInIs(evaluator, predicateCall,
				exclude);
		return predicateCJArgs;
	}

	/**
	 * Check whether the predicate is an IN or IS predicate and can be natively
	 * evaluated.
	 * 
	 * @param evaluator
	 * @param predicateCall
	 ...
	 */
	private CrossJoinArg[] checkFilterPredicateInIs(RolapEvaluator evaluator,
			ResolvedFunCall predicateCall, boolean exclude) {
		final boolean useIs;
		...
		predCJArgs = checkEnumeration(evaluator, predSecondArgFun,
				predSecondArgList, exclude);
		return predCJArgs;
	}
	...
	private CrossJoinArg[] expandNonNative(RolapEvaluator evaluator, Exp exp) {
		...
		if (shouldExpandNonEmpty(exp)
			...
			if (tupleList.getArity() == 1) {
				...
				CrossJoinArg arg = MemberListCrossJoinArg.create(evaluator, list0,
	...
	private static final List<String> cheapFuns = Arrays.asList("LastChild",
			"FirstChild", "Lag");
	...
	private boolean isSet(Exp exp) {
		return ((exp instanceof ResolvedFunCall) && ((ResolvedFunCall) exp)
				.getFunName().equals("{}")) || (exp instanceof NamedSetExpr);
	}
}
