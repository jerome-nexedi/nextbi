...
public class SqlQuery {
	 ...
	 */
	boolean addFromTable(final String schema, final String name,
			final String alias, final String filter, final Map hints,
			final boolean failIfExists) {
		if (fromAliases.contains(alias)) {
			if (failIfExists) {
				throw Util.newInternal("query already contains alias '" + alias + "'");
	...
	public boolean addFrom(final MondrianDef.RelationOrJoin relation,
		...
		if (relation instanceof MondrianDef.View) {
			...
			final MondrianDef.Relation relation1 = RolapUtil
					.convertInlineTableToRelation((MondrianDef.InlineTable) relation,
							dialect);
			...
			return addFromTable(table.schema, table.name, tableAlias,
					table.getFilter(), table.getHintMap(), failIfExists);

		} else if (relation instanceof MondrianDef.Join) {
			final MondrianDef.Join join = (MondrianDef.Join) relation;
			return addJoin(join.left, join.getLeftAlias(), join.leftKey, join.right,
					join.getRightAlias(), join.rightKey, failIfExists);
		} else {
	...
	private void addJoinBetween(MondrianDef.RelationOrJoin root,
		...
		for (int i = max - 1; i >= min; i--) {
			...
			addJoin(
					relInfo.relation,
					relInfo.leftAlias != null ? relInfo.leftAlias : relInfo.relation
							.getAlias(),
					relInfo.leftKey,
					relations.get(i + 1).relation,
					relInfo.rightAlias != null ? relInfo.rightAlias : relations
							.get(i + 1).relation.getAlias(), relInfo.rightKey, false);
		}
	}
	...
	public String addSelect(final String expression,
			final SqlStatement.Type type, String alias) {
		buf.setLength(0);
	...
	public Pair<String, List<SqlStatement.Type>> toSqlAndTypes() {
		assert types.size() == select.size() + groupingFunctions.size() : types
				.size()
				+ " types, "
	...
	private void flatten(List<RelInfo> relations,
			MondrianDef.RelationOrJoin root, String leftKey, String leftAlias,
			String rightKey, String rightAlias) {
		if (root instanceof MondrianDef.Join) {
			MondrianDef.Join join = (MondrianDef.Join) root;
			flatten(relations, join.left, join.leftKey, join.leftAlias,
					join.rightKey, join.rightAlias);
			flatten(relations, join.right, leftKey, leftAlias, rightKey, rightAlias);
		} else {
			relations.add(new RelInfo((MondrianDef.Relation) root, leftKey,
					leftAlias, rightKey, rightAlias));
		}
	}
		...
		void appendJoin(final List<String> addedTables, final String from,
			...
			for (JoinOnClause joinOnClause : joinOnClauses) {
				...
				if ((addedTables.size() == 1
						&& addedTables.get(0).equals(joinOnClause.left) && joinOnClause.left
	...
	static class ClauseList extends ArrayList<String> {
		...
		static void listToBuffer(StringBuilder buf,
				List<ClauseList> clauseListList, boolean generateFormattedSql,
				String prefix, String first, String sep, String last) {
			first = foo(generateFormattedSql, prefix, first);
			sep = foo(generateFormattedSql, prefix, sep);
			buf.append(first);
			int n = 0;
	...
	private static class RelInfo {
		...
		public RelInfo(MondrianDef.Relation relation, String leftKey,
				String leftAlias, String rightKey, String rightAlias) {
