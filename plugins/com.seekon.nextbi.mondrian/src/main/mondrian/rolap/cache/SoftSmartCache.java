 ...
 * <p>
 * There is no contains(key) method because it makes no sense - after contains()
 * returns true, the garbage collector may remove the value that was contained.
 * Instead the code should call get() and keep a reference to the value to
 * prevent garbage collection.
 * 
 * @author av
 * @since Nov 3, 2005
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/cache/SoftSmartCache.java#9
 *          $
 */
public class SoftSmartCache<K, V> implements SmartCache<K, V> {
  ...
  /**
   * an entry in the cache that contains the key for the cache map to remove the
   * entry when its value has been garbage collected
   * 
   * @author rk
   * @since Nov 7, 2005
   */
   ...
   * (non-Javadoc)
   * 
   * @see mondrian.rolap.cache.SmartCache#put(java.lang.Object,
   * java.lang.Object)
   */
  ...
  public Iterator<Map.Entry<K, V>> iterator() {
    final Iterator<Map.Entry<K, CacheReference>> cacheIterator = cache.entrySet()
      .iterator();
    return new Iterator<Map.Entry<K, V>>() {
      ...
      public boolean hasNext() {
        ...
        while (cacheIterator.hasNext()) {
          Map.Entry<K, CacheReference> cacheEntry = cacheIterator.next();
