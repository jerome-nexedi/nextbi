...
public class SqlStatement {
	 ...
	 */
	SqlStatement(DataSource dataSource, String sql, List<Type> types,
			int maxRows, int firstRowOrdinal, Locus locus, int resultSetType,
	...
	public void execute() {
		...
		try {
			...
			if (RolapUtil.SQL_LOGGER.isDebugEnabled()) {
				...
				sqllog.append(currId).append(": ").append(locus.component)
	...
	public void close() {
		...
		locus.execution.getQueryTiming().markFull(TIMING_NAME + locus.component,
				totalMs);
	...
	public RuntimeException handle(Exception e) {
		RuntimeException runtimeException = Util.newError(e, locus.message
				+ "; sql=[" + sql + "]");
		try {
			close();
	 ...
	 *           on error
	 */
	public static Type guessType(Type suggestedType, ResultSetMetaData metaData,
	...
	public ResultSet getWrappedResultSet() {
		return (ResultSet) Proxy.newProxyInstance(null,
				new Class<?>[] { ResultSet.class }, new MyDelegatingInvocationHandler(
						this));
	}
