...
class SqlMemberSource implements MemberReader, SqlTupleReader.MemberBuilder {
	...
	SqlMemberSource(RolapHierarchy hierarchy) {
		...
		this.dataSource = hierarchy.getRolapSchema().getInternalConnection()
				.getDataSource();
		assignOrderKeys = MondrianProperties.instance().CompareSiblingsByOrderKey
				.get();
	...
	private int getMemberCount(RolapLevel level, DataSource dataSource) {
		...
		final SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql,
	...
	private String makeLevelMemberCountSql(RolapLevel level,
			DataSource dataSource, boolean[] mustCount) {
		mustCount[0] = false;
		...
		if (!sqlQuery.getDialect().allowsFromQuery()) {
			...
			if (mustCount[0]) {
				for (String colDef : columnList) {
					final String exp = sqlQuery.getDialect().generateCountExpression(
							colDef);
	...
	private List<RolapMember> getMembers(DataSource dataSource) {
		...
		SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql, types, 0, 0,
				new Locus(null, "SqlMemberSource.getMembers",
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (limit > 0 && limit < stmt.rowCount) {
					...
					throw stmt
							.handle(MondrianResource.instance().MemberFetchLimitExceeded
									.ex(limit));
				}
				...
				for (RolapLevel level : levels) {
					...
					member = map.get(key);
					if (member == null) {
						RolapMemberBase memberBase = new RolapMemberBase(parent, level,
								value);
	...
	public List<RolapMember> getMembersInLevel(RolapLevel level,
			int startOrdinal, int endOrdinal) {
	...
	private List<RolapMember> getMembersInLevel(RolapLevel level,
		...
		final TupleReader tupleReader = level.getDimension().isHighCardinality() ? new HighCardSqlTupleReader(
				constraint) : new SqlTupleReader(constraint);
	...
	Pair<String, List<SqlStatement.Type>> makeChildMemberSql(RolapMember member,
		...
		if (levelCollapsed) {
			...
			int bitPos = starColumn.getBitPosition();
			AggStar.Table.Column aggColumn = aggStar.lookupColumn(bitPos);
			RolapStar.Condition condition = new RolapStar.Condition(
					level.getKeyExp(), aggColumn.getExpression());
			...
			while (parentLevel != null && !parentLevel.isAll() && !isUnique) {
				...
				condition = new RolapStar.Condition(parentLevel.getKeyExp(),
	...
	private static AggStar chooseAggStar(MemberChildrenConstraint constraint,
		...
		RolapCubeLevel childLevel = (RolapCubeLevel) member.getLevel()
				.getChildLevel();
		...
		return AggregationManager.instance().findAgg(star, levelBitKey,
				measureBitKey, new boolean[] { false });
	}
	...
	public void getMemberChildren(List<RolapMember> parentMembers,
		...
		if (childLevel != null) {
			TupleConstraint lmc = sqlConstraintFactory.getDescendantsConstraint(
					parentMembers, mcc);
			List<RolapMember> list = getMembersInLevel(childLevel, 0,
					Integer.MAX_VALUE, lmc);
	...
	public RolapMember makeMember(RolapMember parentMember,
		...
		if (parentChild) {
			...
			// children.
			member = childLevel.hasClosedPeer() ? new RolapParentChildMember(
					parentMember, rolapChildLevel, value, member)
					: new RolapParentChildMemberNoClosure(parentMember, rolapChildLevel,
							value, member);
		}
