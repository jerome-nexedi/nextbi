...
public class RolapSchema implements Schema {
	...
	private static final Set<Access> schemaAllowed = Olap4jUtil.enumSetOf(
			Access.NONE, Access.ALL, Access.ALL_DIMENSIONS, Access.CUSTOM);

	private static final Set<Access> cubeAllowed = Olap4jUtil.enumSetOf(
	...
	private static final Set<Access> memberAllowed = Olap4jUtil.enumSetOf(
	...
	private RolapSchema(final String key, final Util.PropertyList connectInfo,
		...
		this.internalConnection = new RolapConnection(internalServer, connectInfo,
				this, dataSource);
	...
	private RolapSchema(String key, String catalogUrl,
	...
	static CellFormatter getCellFormatter(String className,
		...
		if (className != null) {
			...
			Class<CellFormatter> clazz = (Class<CellFormatter>) Class
					.forName(className);
	...
	protected void finalCleanUp() {
		final CacheControl cacheControl = AggregationManager.instance()
	...
	private void load(MondrianDef.Schema xmlSchema) {
		...
		this.annotationMap = RolapHierarchy
				.createAnnotationMap(xmlSchema.annotations);
		...
		if (xmlSchema.defaultRole != null) {
			...
			if (role == null) {
				error("Role '" + xmlSchema.defaultRole + "' not found",
		...
		for (MondrianDef.SchemaGrant schemaGrant : xmlRole.schemaGrants) {
			...
			for (MondrianDef.CubeGrant cubeGrant : schemaGrant.cubeGrants) {
				...
				for (MondrianDef.DimensionGrant dimensionGrant : cubeGrant.dimensionGrants) {
					Dimension dimension = (Dimension) schemaReader.lookupCompound(cube,
							Util.parseIdentifier(dimensionGrant.dimension), true,
							Category.Dimension);
				...
				for (MondrianDef.HierarchyGrant hierarchyGrant : cubeGrant.hierarchyGrants) {
					Hierarchy hierarchy = (Hierarchy) schemaReader.lookupCompound(cube,
							Util.parseIdentifier(hierarchyGrant.hierarchy), true,
							Category.Hierarchy);
					...
					if (hierarchyGrant.topLevel != null) {
						...
						topLevel = (Level) schemaReader.lookupCompound(cube,
								Util.parseIdentifier(hierarchyGrant.topLevel), true,
								Category.Level);
					}
					...
					if (hierarchyGrant.bottomLevel != null) {
						...
						bottomLevel = (Level) schemaReader.lookupCompound(cube,
								Util.parseIdentifier(hierarchyGrant.bottomLevel), true,
								Category.Level);
					}
					Role.RollupPolicy rollupPolicy;
					if (hierarchyGrant.rollupPolicy != null) {
						try {
							rollupPolicy = Role.RollupPolicy
									.valueOf(hierarchyGrant.rollupPolicy.toUpperCase());
					...
					for (MondrianDef.MemberGrant memberGrant : hierarchyGrant.memberGrants) {
						...
						if (member.getHierarchy() != hierarchy) {
							throw Util.newError("Member '" + member
	...
	public Cube createCube(String xml) {
		...
		try {
			...
			throw Util
					.newError(e, "Error while creating cube from XML [" + xml + "]");
		}
		return cube;
	}
	...
	static class Pool {
		...
		synchronized RolapSchema get(final String catalogUrl,
				final String connectionKey, final String jdbcUser,
		...
		private RolapSchema get(final String catalogUrl,
				final String connectionKey, final String jdbcUser,
			...
			if (!Util.isEmpty(dynProcName)) {
				...
				try {
					...
					final Class<DynamicSchemaProcessor> clazz = (Class<DynamicSchemaProcessor>) Class
							.forName(dynProcName);
					final Constructor<DynamicSchemaProcessor> ctor = clazz
							.getConstructor();
			...
			if (!useSchemaPool) {
				schema = new RolapSchema(key, null, catalogUrl, catalogStr,
				...
				if (schema == null || md5Bytes == null || schema.md5Bytes == null
					...
					SoftReference<RolapSchema> ref = new SoftReference<RolapSchema>(
							schema);
					if (md5Bytes != null) {
		...
		synchronized void remove(final String catalogUrl,
				final String connectionKey, final String jdbcUser,
				final String dataSourceStr) {
			final String key = makeKey(catalogUrl, connectionKey, jdbcUser,
					dataSourceStr);
			if (LOGGER.isDebugEnabled()) {
