/*
// $Id: //open/mondrian/src/main/mondrian/rolap/BitKey.java#24 $
 ...
 * 
 * <p>
 * Unlike {@link java.util.BitSet}, the number of bits cannot be changed after
 * the BitKey is created. This allows us to optimize.
 * 
 * <p>
 ...
 * 
 * <p>
 * One caution: if the maximum number assigned to one of the objects is large,
 * then this representation might be sparse and therefore not efficient.
 * 
 * @author Richard M. Emberson
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/BitKey.java#24 $
 */
public interface BitKey extends Serializable, Comparable<BitKey>,
		Iterable<Integer> {
	/**
	 ...
	 * currently set in this <code>BitKey</code>; otherwise, the result is
	 * <code>false</code>.
	 */
	 ...
	 * <code>this</code>. If one switches <code>this</code> with the parameter
	 * <code>bitKey</code> one gets the equivalent of isSubSetOf.
	 * 
	 * @param bitKey
	 *          Bit key
	 */
	 ...
	 * <code>BitSet</code> whose corresponding bit is NOT set in the specified
	 * <code>BitSet</code>.
	 */
	...
	/**
	 * Returns an empty BitKey of the same type. This is the same as calling
	 * {@link #copy} followed by {@link #clear()}.
	 * 
	...
	/**
	 * Returns true if this <code>BitKey</code> contains no bits that are set to
	 * <code>true</code>.
	 */
	boolean isEmpty();
	...
	/**
	 * An Iterator over the bit positions. For example, if the BitKey had
	 * positions 3 and 4 set, then the Iterator would return the values 3 and then
	 * 4. The bit positions returned by the iterator are in the order, from
	 ...
	 * occurs on or after the specified starting index. If no such bit exists then
	 * -1 is returned.
	 * 
	 * To iterate over the <code>true</code> bits in a <code>BitKey</code>, use
	 * the following loop:
	 * 
	 * <pre>
	...
	abstract class AbstractBitKey implements BitKey {
		...
		/**
		 * Returns which chunk a given bit falls into. Bits 0 to 63 fall in chunk 0,
		 * bits 64 to 127 fall into chunk 1.
		 */
		 ...
		 * 
		 * <p>
		 * (Copied from {@link java.lang.Long#bitCount(long)}, which was introduced
		 * in JDK 1.5, but we need the functionality in JDK 1.4.)
		 * 
		 ...
		 *          Byte
		 */
		protected static void copyFromLong(final BitSet bitSet, int pos, long x) {
			while (x != 0) {
		...
		protected IllegalArgumentException createException(BitKey bitKey) {
			final String msg = (bitKey == null) ? "Null BitKey" : "Bad BitKey type: "
					+ bitKey.getClass().getName();
		 ...
		 * @param a2
		 *          Second array
		 * @return -1 if a1 compares less to a2, 0 if a1 is equal to a2, 1 if a1 is
		 *         greater than a2
		 */
		 ...
		 * @param i2
		 *          Second value
		 * @return -1 if i1 is less than i2, 1 if i1 is greater than i2, 0 if i1
		 *         equals i2
		 */
	...
	public class Small extends AbstractBitKey {
		...
		public boolean isSuperSetOf(BitKey bitKey) {
			if (bitKey instanceof BitKey.Small) {
				...
				return ((this.bits | other.bits0) == this.bits) && (other.bits1 == 0);
		...
		/**
		 * To say that I am happy about this algorithm (or the variations of the
		 * algorithm used for the Mid128 and Big BitKey implementations) would be a
		 * stretch. It works but there has to be a more elegant and faster one but
		 * this is the best I could come up with in a couple of hours.
		 * 
		 */
	...
	public class Mid128 extends AbstractBitKey {
		...
		public void clear(int pos) {
			if (pos < 64) {
				...
				throw new IndexOutOfBoundsException("pos " + pos + " exceeds size "
						+ 128);
			}
		}
		...
		public int cardinality() {
			return bitCount(bits0) + bitCount(bits1);
		}
	...
	public class Big extends AbstractBitKey {
		...
		/**
		 * Returns the number of chunks, ignoring any chunks on the leading edge
		 * that are all zero.
		 * 
	...
	final static byte bitPositionTable[] = { -1, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0,
			2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0, 6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0,
			2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 7, 0, 1, 0,
			2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0, 5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0,
			2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0,
			2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
			2, 0, 1, 0 };
}
