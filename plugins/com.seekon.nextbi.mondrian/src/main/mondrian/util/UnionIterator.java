 ...
 * 
 * <p>
 * Try, for instance, using the {@link #over} helper method:
 * </p>
 * 
 * <blockquote> <code>
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/util/UnionIterator.java#6 $
 * @since Apr 28, 2008
 */
public class UnionIterator<T> implements Iterator<T> {
	...
	public UnionIterator(Collection<? extends T>... iterables) {
		List<Iterable<? extends T>> list = new ArrayList<Iterable<? extends T>>(
				iterables.length);
		for (Iterable<? extends T> iterable : iterables) {
			// noinspection unchecked
			list.add(new MyIterable(iterable));
	...
	/**
	 * Moves to the next iterator that has at least one element. Called after
	 * finishing an iterator, or at the start.
	 */
	 ...
	 * 
	 * <p>
	 * You can use it like this: <blockquote>
	 * 
	 * <pre>
	 ...
	 * Retroweaver has its own version of the {@link Iterable} interface, which is
	 * problematic since the {@link java.util.Collection} classes don't implement
	 * it. This method solves some of these problems by working in terms of
