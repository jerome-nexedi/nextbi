/*
// $Id: //open/mondrian/src/main/mondrian/util/PropertyUtil.java#3 $
...
/**
 * Utilities to generate MondrianProperties.java and mondrian.properties from
 * property definitions in MondrianProperties.xml.
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/util/PropertyUtil.java#3 $
 * @author jhyde
 */
public class PropertyUtil {
	...
	public static void main0(String[] args) throws IllegalAccessException {
		...
		for (Field field : properties1.getClass().getFields()) {
			...
			System.out.println("        <Description>" + o.getPath()
					+ "</Description>");
			System.out.println("        <Type>"
					+ (o instanceof BooleanProperty ? "boolean"
							: o instanceof IntegerProperty ? "int"
									: o instanceof DoubleProperty ? "double" : "String")
					+ "</Type>");
			if (o.getDefaultValue() != null) {
				System.out.println("        <Default>" + o.getDefaultValue()
						+ "</Default" + ">");
			}
			System.out.println("    </PropertyDefinition>");
		}
	...
	private void generate() {
		final File xmlFile = new File("src/main/mondrian/olap",
				"MondrianProperties.xml");
		final File javaFile = new File("src/main/mondrian/olap",
				"MondrianProperties.java");
		final File propertiesFile = new File("mondrian.properties.template");
		...
		try {
			...
			for (Node element : iter(propertyDefinitions)) {
				if (element.getNodeName().equals("PropertyDefinition")) {
					...
					propertyDefinitionMap.put(name, new PropertyDef(name, path, dflt,
							category, PropertyType.valueOf(type.toUpperCase()), core == null
									|| Boolean.valueOf(core), description));
				}
			}
		} catch (Throwable e) {
	...
	void doGenerate(Generator generator,
			SortedMap<String, PropertyDef> propertyDefinitionMap, File file) {
		FileWriter fw = null;
	...
	enum Generator {
		JAVA {
			@Override
			void generate(SortedMap<String, PropertyDef> propertyDefinitionMap,
				...
				String[] lines = {
						...
						"    private MondrianProperties() {", "        super(",
		...
		HTML {
			...
			void generate(SortedMap<String, PropertyDef> propertyDefinitionMap,
				...
				for (String category : categories) {
					out.println("    <tr>");
					out.println("      <td colspan='4'><b><br>" + category + "</b"
							+ "></td>");
					out.println("    </tr>");
					for (PropertyDef def : propertyDefinitionMap.values()) {
						...
						out.println("    <tr>");
						out.println("<td><code><a href='api/mondrian/olap/MondrianProperties.html#"
								+ def.name + "'>" + split(def.path) + "</a></code></td>");
						out.println("<td>" + def.propertyType.name().toLowerCase()
								+ "</td>");
						out.println("<td>" + split(def.defaultHtml()) + "</td>");
						out.println("<td>" + split(def.description) + "</td>");
						out.println("    </tr>");
					}
				}
				out.println("<table>");
	...
	private static void printJavadoc(PrintWriter out, String prefix,
			String content) {
		out.println(prefix + "/**");
	...
	private static void printComments(PrintWriter out, String offset,
	...
	private static String stripHtml(String s) {
		...
		s = s.replaceAll("</h3>", " ###</h3>");
		String[] strings = { "p", "code", "br", "ul", "li", "blockquote", "h3", "i" };
	...
	private static String replaceRegion(String s, String start, String end) {
		...
		while ((i = s.indexOf(start, i)) >= 0) {
			...
			s = s.substring(0, i) + s.substring(i + start.length(), j)
					+ s.substring(j + 1);
	...
	private static class PropertyDef {
		...
		PropertyDef(String name, String path, String defaultValue, String category,
				PropertyType propertyType, boolean core, String description) {
			this.name = name;
			this.path = path;
		...
		public String defaultHtml() {
			...
			case DOUBLE:
				return new DecimalFormat("#,###.#")
						.format(new BigDecimal(defaultValue));
			default:
				return defaultValue;
			}
		}
	...
	private enum PropertyType {
		INT("IntegerProperty"), STRING("StringProperty"), DOUBLE("DoubleProperty"), BOOLEAN(
				"BooleanProperty");
