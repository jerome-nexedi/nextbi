/*
// $Id: //open/mondrian/src/main/mondrian/util/FilteredIterableList.java#6 $
...
/**
 * Iterable list which filters undesirable elements. To be used instead of
 * removing elements from an iterable list.
 * 
 * @author Luis F. Canals
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/util/FilteredIterableList.java#6 $
 * @since december, 2007
 */
public class FilteredIterableList<T> extends AbstractSequentialList<T> {
  ...
  public FilteredIterableList(final List<? extends T> list, final Filter filter) {
    super();
    this.plainList = null;
    this.filter = filter;
    this.internal = list;
  ...
  /*
   * public List<T> subList(final int start, final int end) { return new
   * AbstractList<T>() { public T get(final int index) { return
   * FilteredIterableList.this.get(index-start); } public int size() { return
   * FilteredIterableList.this.size() - start; } }; }
   */
