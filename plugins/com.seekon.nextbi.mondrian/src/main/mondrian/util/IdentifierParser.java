/*
// $Id: //open/mondrian/src/main/mondrian/util/IdentifierParser.java#4 $
...
/**
 * Utilities for parsing fully-qualified member names, tuples, member lists, and
 * tuple lists.
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/util/IdentifierParser.java#4
 *          $
 * @author jhyde
 */
public class IdentifierParser extends org.olap4j.impl.IdentifierParser {
  ...
  public static class BuilderImpl extends MemberBuilder {
    ...
    BuilderImpl(SchemaReader schemaReader, Cube cube, List<Hierarchy> hierarchyList) {
      ...
      this.ignoreInvalid = (load ? props.IgnoreInvalidMembers.get()
        : props.IgnoreInvalidMembersDuringQuery.get());
    }

    protected Member resolveMember(Hierarchy expectedHierarchy) {
      final List<Id.Segment> mondrianSegmentList = Util.convert(this.segmentList);
      Member member = (Member) Util.lookupCompound(schemaReader, cube,
        mondrianSegmentList, !ignoreInvalid, Category.Member);
      if (member == null) {
        ...
        if (expectedHierarchy != null) {
          ...
          for (int i = mondrianSegmentList.size() - 1; i > 0; --i) {
            ...
            OlapElement olapElement = schemaReader.lookupCompound(cube, partialName,
              false, Category.Unknown);
            if (olapElement != null) {
              return olapElement.getHierarchy().getNullMember();
            }
          }
          throw MondrianResource.instance().MdxChildObjectNotFound.ex(Util
            .implode(mondrianSegmentList), cube.getQualifiedName());
        }
      }
  ...
  public static class TupleBuilder extends BuilderImpl {
    ...
    public TupleBuilder(SchemaReader schemaReader, Cube cube,
  ...
  public static class TupleListBuilder extends TupleBuilder {
    ...
    public TupleListBuilder(SchemaReader schemaReader, Cube cube,
      List<Hierarchy> hierarchyList) {
      super(schemaReader, cube, hierarchyList);
  ...
  public static class MemberListBuilder extends BuilderImpl {
    ...
    public MemberListBuilder(SchemaReader schemaReader, Cube cube,
      Hierarchy hierarchy) {
