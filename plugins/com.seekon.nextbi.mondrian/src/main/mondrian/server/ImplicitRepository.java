/*
// $Id: //open/mondrian/src/main/mondrian/server/ImplicitRepository.java#4 $
...
/**
 * Implementation of {@link Repository} for a server that doesn't have a
 * repository: each connection in the server has its own catalog (specified in
 * the connect string) and therefore the catalog and schema metadata will be
 * whatever pertains to that connection. (That's why the methods have a
 * connection parameter.)
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/server/ImplicitRepository.java#4 $
 * @author Julian Hyde
 */
public class ImplicitRepository implements Repository {
  ...
  public List<String> getCatalogNames(RolapConnection connection, String databaseName) {
    ...
    return Collections.singletonList(connection.getSchema().getName());
  }
  ...
  public List<Map<String, Object>> getDatabases(RolapConnection connection) {
    return Collections.singletonList(Olap4jUtil.<String, Object> mapOf(
      "DataSourceName", connection.getSchema().getName(), "DataSourceDescription",
      null, "URL", null, "DataSourceInfo", connection.getSchema().getName(),
      "ProviderName", "Mondrian", "ProviderType", "MDP", "AuthenticationMode",
      "Unauthenticated"));
  }
