/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/TestAggregationManager.java#72 $
 ...
 * @since 21 March, 2002
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/TestAggregationManager
 *          .java#72 $
 */
public class TestAggregationManager extends BatchTestCase {
  private static final Set<Dialect.DatabaseProduct> ACCESS_MYSQL = Olap4jUtil
    .enumSetOf(Dialect.DatabaseProduct.ACCESS, Dialect.DatabaseProduct.MYSQL);
  ...
  public void testFemaleUnitSales() {
    CellRequest request = createRequest("Sales", "[Measures].[Unit Sales]",
      "customer", "gender", "F");
    ...
    FastBatchingCellReader fbcr = new FastBatchingCellReader(getCube("Sales"));
  ...
  public void testFemaleCustomerCount() {
    CellRequest request = createRequest("Sales", "[Measures].[Customer Count]",
      "customer", "gender", "F");
    final RolapAggregationManager aggMan = AggregationManager.instance();
    FastBatchingCellReader fbcr = new FastBatchingCellReader(getCube("Sales"));
    Object value = aggMan.getCellFromCache(request);
  ...
  public void testFemaleCustomerCountWithConstraints() {
    ...
    CellRequest request1 = createRequest("Sales", "[Measures].[Customer Count]",
      "customer", "gender", "F", makeConstraintYearQuarterMonth(Q1M1));

    CellRequest request2 = createRequest("Sales", "[Measures].[Customer Count]",
      "customer", "gender", "F", makeConstraintYearQuarterMonth(Q2M5));

    CellRequest request3 = createRequest("Sales", "[Measures].[Customer Count]",
      "customer", "gender", "F", makeConstraintYearQuarterMonth(Q1M1Q2M5));
  ...
  /**
   * Tests that a request for ([Measures].[Unit Sales], [Gender].[F]) generates
   * the correct SQL.
   */
  public void testFemaleUnitSalesSql() {
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL,
      "select `agg_g_ms_pcat_sales_fact_1997`.`gender` as `c0`,"
        ...
        + "group by `agg_g_ms_pcat_sales_fact_1997`.`gender`", 26) };
  ...
  private void _testFemaleUnitSalesSql_withAggs() {
    ...
    SqlPattern[] patterns = { new SqlPattern(
      ...
      "select `customer`.`gender` as `c0`,"
        ...
        + "and `customer`.`gender` = 'F' " + "group by `customer`.`gender`", 26) };

    assertRequestSql(new CellRequest[] { request }, patterns);
  }

  /**
   * Test a batch containing multiple measures: (store_state=CA, gender=F,
   * measure=[Unit Sales]) (store_state=CA, gender=M, measure=[Store Sales])
  ...
  public void testMultipleMeasures() {
    ...
    CellRequest[] requests = new CellRequest[] {
      createRequest("Sales", "[Measures].[Unit Sales]", new String[] { "customer",
        "store" }, new String[] { "gender", "store_state" }, new String[] { "F",
        "CA" }),
      createRequest("Sales", "[Measures].[Store Sales]", new String[] { "customer",
        "store" }, new String[] { "gender", "store_state" }, new String[] { "M",
        "CA" }),
      createRequest("Sales", "[Measures].[Unit Sales]", new String[] { "customer",
        "store" }, new String[] { "gender", "store_state" }, new String[] { "F",
        "OR" }) };
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL,
      "select `store`.`store_state` as `c0`," + " `customer`.`gender` as `c1`,"
        ...
        + "group by `store`.`store_state`, " + "`customer`.`gender`", 29) };

    assertRequestSql(requests, patterns);
  }

  /**
   * As {@link #testMultipleMeasures()}, but with aggregate tables switched on.
   * 
   * TODO: Enable this test.
   */
  private void _testMultipleMeasures_withAgg() {
    ...
    SqlPattern[] patterns = { new SqlPattern(
      ...
      "select `customer`.`gender` as `c0`,"
        ...
        + " and `store`.`store_state` in ('CA', 'OR') "
        + "group by `customer`.`gender`, `store`.`store_state`", 26) };
  ...
  private CellRequest createMultipleMeasureCellRequest() {
    ...
    request.addConstrainedColumn(storeTypeColumn, new ValueColumnPredicate(
      storeTypeColumn, value));
    return request;
  }
  ...
  /**
   * Tests that if a level is marked 'unique members', then its parent is not
   * constrained.
   */
  public void testUniqueMembers() {
    // [Store].[Store State] is unique, so we don't expect to see any
    // references to country.
    final String mdxQuery = "select {[Measures].[Unit Sales]} on columns,"
      + " {[Store].[USA].[CA], [Store].[USA].[OR]} on rows " + "from [Sales]";
    SqlPattern[] patterns;
    String accessMysqlSql, derbySql;

    /*
     * Note: the following aggregate loading sqls contain no references to the
     * parent level column "store_country".
     */
    if (MondrianProperties.instance().UseAggregates.get()
      && MondrianProperties.instance().ReadAggregates.get()) {
      accessMysqlSql = "select `store`.`store_state` as `c0`,"
      ...
      derbySql = "select "
        ...
        + "\"agg_c_14_sales_fact_1997\".\"the_year\" = 1997 " + "group by "
        + "\"store\".\"store_state\", \"agg_c_14_sales_fact_1997\".\"the_year\"";

      patterns = new SqlPattern[] {
        new SqlPattern(ACCESS_MYSQL, accessMysqlSql, 50),
      ...
      derbySql = "select \"store\".\"store_state\" as \"c0\", \"time_by_day\".\"the_year\" as \"c1\", "
   ...
   * (Store Sales) instead of the default measure (Unit Sales) in the case where
   * the query only has one result axis. The setup here is necessarily elaborate
   * because the original bug was quite arbitrary.
   */
  public void testNonEmptyCrossJoinLoneAxis() {
    ...
    String derbySql = "select " + "\"store\".\"store_country\" as \"c0\", "
      + "\"time_by_day\".\"the_year\" as \"c1\", "
    ...
    SqlPattern[] patterns = { new SqlPattern(Dialect.DatabaseProduct.DERBY,
      derbySql, derbySql) };
  ...
  public void testHierarchyInFactTable() {
    CellRequest request = createRequest("Store", "[Measures].[Store Sqft]", "store",
      "store_type", "Supermarket");

    String accessMysqlSql = "select `store`.`store_type` as `c0`,"
      + " sum(`store`.`store_sqft`) as `m0` " + "from `store` as `store` "
      + "where `store`.`store_type` = 'Supermarket' "
      + "group by `store`.`store_type`";

    String derbySql = "select " + "\"store\".\"store_type\" as \"c0\", "
      + "sum(\"store\".\"store_sqft\") as \"m0\" " + "from "
      + "\"store\" as \"store\" " + "where "
      + "\"store\".\"store_type\" = 'Supermarket' "
      + "group by \"store\".\"store_type\"";

    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL, accessMysqlSql, 26),
      new SqlPattern(Dialect.DatabaseProduct.DERBY, derbySql, derbySql) };
  ...
  public void testCountDistinctAggMatch() {
    ...
    CellRequest request = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "time_by_day", "time_by_day", "time_by_day" }, new String[] {
        "the_year", "quarter", "month_of_year" }, new String[] { "1997", "Q1", "1" });

    String accessSql = "select " + "`agg_c_10_sales_fact_1997`.`the_year` as `c0`, "
      + "`agg_c_10_sales_fact_1997`.`quarter` as `c1`, "
      + "`agg_c_10_sales_fact_1997`.`month_of_year` as `c2`, "
      + "`agg_c_10_sales_fact_1997`.`customer_count` as `m0` " + "from "
      + "`agg_c_10_sales_fact_1997` as `agg_c_10_sales_fact_1997` " + "where "
    ...
    SqlPattern[] patterns = { new SqlPattern(Dialect.DatabaseProduct.ACCESS,
      accessSql, 26) };
  ...
  public void testCountDistinctCannotRollup() {
    ...
    // "month_of_year".
    CellRequest request = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "time_by_day", "time_by_day", "product_class" }, new String[] {
        "the_year", "quarter", "product_family" }, new String[] { "1997", "Q1",
        "Food" });

    SqlPattern[] patterns = {
      new SqlPattern(Dialect.DatabaseProduct.MYSQL, "select"
        ...
        + "group by `time_by_day`.`the_year`," + " `time_by_day`.`quarter`,"
        + " `product_class`.`product_family`", 23),
      new SqlPattern(Dialect.DatabaseProduct.ACCESS, "select" + " `d0` as `c0`,"
        + " `d1` as `c1`," + " `d2` as `c2`," + " count(`m0`) as `c3` " + "from ("
        + "select distinct `time_by_day`.`the_year` as `d0`,"
        + " `time_by_day`.`quarter` as `d1`,"
   ...
   * aggregate. But if you're rolling up along the dimension which the count is
   * counting, it's OK. In this case, you know that every member can only belong
   * to one group.
   */
  public void testCountDistinctRollupAlongDim() {
    ...
    CellRequest request = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "time_by_day", "time_by_day", "time_by_day", "product_class",
        "product_class", "product_class" },
      new String[] { "the_year", "quarter", "month_of_year", "product_family",
        "product_department", "product_category" }, new String[] { "1997", "Q1",
        "1", "Food", "Deli", "Meat" });
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL,
      "select `agg_g_ms_pcat_sales_fact_1997`.`the_year` as `c0`,"
        ...
        + " `agg_g_ms_pcat_sales_fact_1997`.`product_category`", 58) };
  ...
  public void testCountDistinctRollup2() {
    ...
    CellRequest request = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "time_by_day", "time_by_day", "time_by_day", "product_class",
        "product_class", "product_class", "customer" }, new String[] { "the_year",
        "quarter", "month_of_year", "product_family", "product_department",
        "product_category", "gender" }, new String[] { "1997", "Q1", "1", "Food",
        "Deli", "Meat", "F" });
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL,
      "select `agg_g_ms_pcat_sales_fact_1997`.`the_year` as `c0`,"
        ...
        + " `agg_g_ms_pcat_sales_fact_1997`.`gender`", 58) };

    assertRequestSql(new CellRequest[] { request }, patterns);
  }

  /*
   * Test that cells with the same compound member constraints are loaded in one
   * Sql statement.
   * 
  ...
  public void testCountDistinctBatchLoading() {
    ...
    CellRequest request1 = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "product_class" }, new String[] { "product_family" },
      new String[] { "Food" }, aggConstraint);

    CellRequest request2 = createRequest("Sales", "[Measures].[Customer Count]",
      new String[] { "product_class" }, new String[] { "product_family" },
      new String[] { "Drink" }, aggConstraint);

    String mysqlSql = "select `product_class`.`product_family` as `c0`, "
      + "count(distinct `sales_fact_1997`.`customer_id`) as `m0` "
    ...
    String derbySql = "select \"product_class\".\"product_family\" as \"c0\", "
      + "count(distinct \"sales_fact_1997\".\"customer_id\") as \"m0\" "
      + "from \"product_class\" as \"product_class\", \"product\" as \"product\", "
      + "\"sales_fact_1997\" as \"sales_fact_1997\", \"time_by_day\" as \"time_by_day\" "
      + "where \"sales_fact_1997\".\"product_id\" = \"product\".\"product_id\" and "
  ...
  public void testAggMembers() {
    ...
    SqlPattern[] patterns = {
      new SqlPattern(Dialect.DatabaseProduct.ACCESS,
  ...
  /**
   * This test case tests for a null pointer that was being thrown inside of
   * CellRequest.
   */
  public void testNoNullPtrInCellRequest() {
    ...
    testContext.assertQueryReturns("select {[Measures].[Unit Sales]} on columns, "
      + "Filter ({ " + "[Store2].[All Stores].[USA].[CA].[Beverly Hills], "
      + "[Store2].[All Stores].[USA].[CA].[Beverly Hills].[Gourmet Supermarket] "
      + "},[Measures].[Unit Sales] > 0) on rows " + "from [Sales] "
      + "where [Store Type].[Store Type].[Small Grocery]", "Axis #0:\n"
   ...
   * 
   * <p>
   * Test also that expressions with only table alias difference do not share
   * cardinality result.
   */
  public void testColumnCadinalityCache() {
    String query1 = "select " + "NonEmptyCrossJoin("
      + "[Product].[Product Family].Members, "
      + "[Gender].[Gender].Members) on columns " + "from [Sales]";

    String query2 = "select " + "NonEmptyCrossJoin("
      + "[Store].[Store Country].Members, "
      + "[Product].[Product Family].Members) on columns " + "from [Warehouse]";

    String cardinalitySqlDerby = "select "
      + "count(distinct \"product_class\".\"product_family\") "
      + "from \"product_class\" as \"product_class\"";

    String cardinalitySqlMySql = "select "
      + "count(distinct `product_class`.`product_family`) as `c0` "
      + "from `product_class` as `product_class`";

    SqlPattern[] patterns = new SqlPattern[] {
      new SqlPattern(Dialect.DatabaseProduct.DERBY, cardinalitySqlDerby,
        cardinalitySqlDerby),
      new SqlPattern(Dialect.DatabaseProduct.MYSQL, cardinalitySqlMySql,
        cardinalitySqlMySql) };
  ...
  public void testKeyExpressionCardinalityCache() {
    ...
    String cardinalitySqlDerby2 = "select count(distinct \"store_country\") from \"store_ragged\" as \"store_ragged\"";

    String cardinalitySqlMySql2 = "select count(distinct `store_country`) as `c0` from `store_ragged` as `store_ragged`";

    SqlPattern[] patterns1 = new SqlPattern[] {
      new SqlPattern(Dialect.DatabaseProduct.DERBY, cardinalitySqlDerby1,
        cardinalitySqlDerby1),
      new SqlPattern(Dialect.DatabaseProduct.MYSQL, cardinalitySqlMySql1,
        cardinalitySqlMySql1) };

    SqlPattern[] patterns2 = new SqlPattern[] {
      new SqlPattern(Dialect.DatabaseProduct.DERBY, cardinalitySqlDerby2,
        cardinalitySqlDerby2),
      new SqlPattern(Dialect.DatabaseProduct.MYSQL, cardinalitySqlMySql2,
        cardinalitySqlMySql2) };

    TestContext testContext = TestContext.instance().create(storeDim1 + storeDim2,
      salesCube1 + salesCube2, null, null, null, null);
  ...
  public void testCountDistinctWithConstraintAggMiss() {
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL, "select "
      ...
      + "`time_by_day`.`month_of_year` = 1) " + "group by "
      + "`product_class`.`product_family`, `product_class`.`product_department`, "
      + "`product_class`.`product_category`", 58) };
  ...
  public void testOrdinalExprAggTuplesAndChildren() {
    ...
    assertEquals("Food", result.getAxes()[1].getPositions().get(0).get(0)
      .getCaption());
  ...
  public void testAggregatingTuples() {
    ...
    SqlPattern[] patterns = { new SqlPattern(ACCESS_MYSQL, "select "
      ...
      + "`agg_g_ms_pcat_sales_fact_1997`.`marital_status` as `c1` " + "from "
      ...
      + "`agg_g_ms_pcat_sales_fact_1997`.`marital_status` " + "order by "
      ...
      + "`agg_g_ms_pcat_sales_fact_1997`.`marital_status` ASC", null) };
    ...
    SqlPattern[] patterns2 = { new SqlPattern(ACCESS_MYSQL, "select "
      + "`store`.`store_country` as `c0`, " + "`store`.`store_state` as `c1` "
      + "from " + "`store` as `store`, "
      + "`agg_c_14_sales_fact_1997` as `agg_c_14_sales_fact_1997` " + "where "
      + "`agg_c_14_sales_fact_1997`.`store_id` = `store`.`store_id` and "
      + "`agg_c_14_sales_fact_1997`.`the_year` = 1997 and "
      + "`agg_c_14_sales_fact_1997`.`quarter` = 'Q1' " + "group by "
      + "`store`.`store_country`, `store`.`store_state` " + "order by "
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-812"> bug
   * MONDRIAN-812</a>. Using a key expression for a level element would make
   * aggregate tables fail to be used.
   */
  public void testLevelKeyAsSqlExpWithAgg() {
    ...
    final String mdxQuery = "select non empty{[Promotions].[All Promotions].Children} ON rows, "
      + "non empty {[Store].[All Stores]} ON columns "
      + "from [Sales] "
      + "where {[Measures].[Unit Sales]}";
    // Provoke an error in the key resolution to prove it uses it.
    final String colName = TestContext.instance().getDialect().quoteIdentifier(
      "promotion_name");
    TestContext testContext = TestContext
      .instance()
      .createSubstitutingCube(
        ...
        "<Dimension name=\"Promotions\" foreignKey=\"promotion_id\">\n"
          ...
          + "      <KeyExpression><SQL>ERROR_TEST_FUNCTION_NAME(" + colName
          + ")</SQL></KeyExpression>\n" + "    </Level>\n" + "  </Hierarchy>\n"
          + "</Dimension>");
    ...
    testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "<Dimension name=\"Promotions\" foreignKey=\"promotion_id\">\n"
          ...
          + "    <Table name=\"promotion\"/>\n"
          + "    <Level name=\"Promotion Name\" column=\"promotion_name\" uniqueMembers=\"true\">\n"
          + "      <KeyExpression><SQL>RTRIM(" + colName
          + ")</SQL></KeyExpression>\n" + "    </Level>\n" + "  </Hierarchy>\n"
          + "</Dimension>");
    testContext.assertQueryReturns(
      "select non empty{[Promotions].[All Promotions].Children} ON rows, "
        + "non empty {[Store].[All Stores]} ON columns " + "from [Sales] "
        + "where {[Measures].[Unit Sales]}", "Axis #0:\n"
        + "{[Measures].[Unit Sales]}\n" + "Axis #1:\n" + "{[Store].[All Stores]}\n"
  ...
  /**
   * This is a test for MONDRIAN-918 and MONDRIAN-903. We have added an
   * attribute to AggName called approxRowCount so that the aggregation manager
   * can optimize the aggregation tables without having to issue a select
   * count() query.
   */
  public void testAggNameApproxRowCount() {
    ...
    assertQuerySqlOrNot(context, mdxQuery, new SqlPattern[] {
