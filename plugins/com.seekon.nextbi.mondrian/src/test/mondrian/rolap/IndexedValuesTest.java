/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/IndexedValuesTest.java#8 $
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/IndexedValuesTest.java#8
 *          $
 */
public class IndexedValuesTest extends FoodMartTestCase {
  ...
  public void testQueryWithIndex() {
    ...
    // Query using name
    assertQueryReturns("SELECT {[Measures].[Org Salary], [Measures].[Count]} "
      + "ON COLUMNS, " + "{[Employees].[Sheri Nowmer]} " + "ON ROWS FROM [HR]",
      desiredResult);

    // Query using key; expect same result.
    assertQueryReturns("SELECT {[Measures].[Org Salary], [Measures].[Count]} "
      + "ON COLUMNS, " + "{[Employees].&[1]} " + "ON ROWS FROM [HR]", desiredResult);
    ...
    // (We should fix this.)
    assertQueryReturns("SELECT {[Measures].[Org Salary], [Measures].[Count]} "
      + "ON COLUMNS, " + "{[Employees].&[4]} " + "ON ROWS FROM [HR]", "something");

    // "level.&key" syntax
    assertQueryReturns("SELECT [Measures] ON COLUMNS, "
      + "{[Product].[Product Name].&[9]} " + "ON ROWS FROM [Sales]", "something");
  }
}
