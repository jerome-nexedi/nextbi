/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/HighDimensionsTest.java#38 $
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/HighDimensionsTest.java
 *          #38 $
 * @since May, 2008
 */
public class HighDimensionsTest extends FoodMartTestCase {
  ...
  public void testBug1971406() throws Exception {
    ...
    Query query = connection.parseQuery("with set necj as "
    ...
    assertTrue("Query execution took " + elapsed + " milliseconds, "
      + "which is outside target of  " + target + " milliseconds", elapsed <= target);
  }
   ...
   * 
   * <p>
   * The method performs a benchmark computation, and returns is the ratio of
   * the elapsed time for the current system versus the baseline. If the current
   * system takes, say, 2 seconds to perform the benchmark computation, the
   * method returns 2.0, meaning that the system would be expected to take twice
   * as long to do a typical CPU-intensive task.
   * 
  ...
  public void testPromotionsTwoDimensions() throws Exception {
    ...
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      ...
      1, "Promotions", highCardResults, null, true, 51);
  }
  ...
  public void testHead() throws Exception {
    ...
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      ...
      + "on rows from [Sales Ragged]", 1, "Promotions", first40HighCardResults,
      null, true, 40);
  }
  ...
  public void _testTopCount() throws Exception {
    ...
    Query query = connection
      .parseQuery("select {[Measures].[Unit Sales]} on columns,\n"
        + "TopCount({[Promotions].[Promotion Name].Members},41, "
        + "[Measures].[Unit Sales]) " + "on rows from [Sales Ragged]");
    Result result = connection.execute(query);
    int i = 0;
    ...
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      ...
      + "[Measures].[Unit Sales]) " + "on rows from [Sales Ragged]", 1,
      "Promotions", topcount40HighCardResults, topcount40Cells, false, 40);
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      + "TopCount({[Promotions].[Promotion Name].Members},41, "
      + "[Measures].[Unit Sales]) " + "on rows from [Sales Ragged]", 1,
      "Promotions", topcount41HighCardResults, topcount41Cells, false, 41);
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      + "TopCount({[Promotions].[Promotion Name].Members},40, "
      + "[Measures].[Unit Sales]) " + "on rows from [Sales Ragged]", 1,
      "Promotions", topcount40HighCardResults, topcount40Cells, false, 40);
  ...
  public void testNonEmpty() throws Exception {
    ...
    execHighCardTest("select {[Measures].[Unit Sales]} on columns,\n"
      ...
      + "on rows from [Sales Ragged]", 1, "Promotions", nonEmptyHighCardResults,
      nonEmptyCells, true, 48);
  }
  ...
  public void testFilter() throws Exception {
    ...
    execHighCardTest("select [Measures].[Unit Sales] on columns, "
      ...
      + "[Measures].[Unit Sales]>0) " + "on rows from [Sales Ragged]", 1,
      "Promotions", nonEmptyHighCardResults, nonEmptyCells, true, 48);

    execHighCardTest("select [Measures].[Unit Sales] on columns, "
      + "filter([Promotions].[Promotion Name].Members, "
      + "[Measures].[Unit Sales]>4000) " + "on rows from [Sales Ragged]", 1,
      "Promotions", moreThan4000highCardResults, moreThan4000Cells, true, 3);
  }
  ...
  /**
   * Executes query test trying to [Promotions].[Promotion Name] elements into
   * an axis from the results.
   */
  private void execHighCardTest(final String queryString, final int axisIndex,
    final String highDimensionName, final String results, final String results2,
    final boolean shouldForget, final int resultLimit) throws Exception {
    ...
    final TestContext testContext = TestContext.instance().createSubstitutingCube(
      ...
      "<Dimension name=\"Promotions\" highCardinality=\"true\" "
        + "foreignKey=\"promotion_id\">" + "    <Hierarchy hasAll=\"true\" "
    ...
    final List<SoftReference> softReferences = new ArrayList<SoftReference>();
  ...
  private static final String first40HighCardResults = "[Promotions].[Bag Stuffers]"
    + "[Promotions].[Best Savings]" + "[Promotions].[Big Promo]"
    + "[Promotions].[Big Time Discounts]" + "[Promotions].[Big Time Savings]"
    + "[Promotions].[Bye Bye Baby]" + "[Promotions].[Cash Register Lottery]"
    + "[Promotions].[Coupon Spectacular]" + "[Promotions].[Dimes Off]"
    + "[Promotions].[Dollar Cutters]" + "[Promotions].[Dollar Days]"
    + "[Promotions].[Double Down Sale]" + "[Promotions].[Double Your Savings]"
    + "[Promotions].[Fantastic Discounts]" + "[Promotions].[Free For All]"
    + "[Promotions].[Go For It]" + "[Promotions].[Green Light Days]"
    + "[Promotions].[Green Light Special]" + "[Promotions].[High Roller Savings]"
    + "[Promotions].[I Cant Believe It Sale]" + "[Promotions].[Money Grabbers]"
    + "[Promotions].[Money Savers]" + "[Promotions].[Mystery Sale]"
    + "[Promotions].[No Promotion]" + "[Promotions].[One Day Sale]"
    + "[Promotions].[Pick Your Savings]" + "[Promotions].[Price Cutters]"
    + "[Promotions].[Price Destroyers]" + "[Promotions].[Price Savers]"
    + "[Promotions].[Price Slashers]" + "[Promotions].[Price Smashers]"
    + "[Promotions].[Price Winners]" + "[Promotions].[Sale Winners]"
    + "[Promotions].[Sales Days]" + "[Promotions].[Sales Galore]"
    + "[Promotions].[Save-It Sale]" + "[Promotions].[Saving Days]"
    + "[Promotions].[Savings Galore]" + "[Promotions].[Shelf Clearing Days]"
    + "[Promotions].[Shelf Emptiers]";

  private static final String nonEmptyHighCardResults = "[Promotions].[Bag Stuffers]"
  ...
  private static final String highCardResults = "[Promotions].[Bag Stuffers]"
    + "[Promotions].[Best Savings]" + "[Promotions].[Big Promo]"
    + "[Promotions].[Big Time Discounts]" + "[Promotions].[Big Time Savings]"
    + "[Promotions].[Bye Bye Baby]" + "[Promotions].[Cash Register Lottery]"
    + "[Promotions].[Coupon Spectacular]" + "[Promotions].[Dimes Off]"
    ...
    + "[Promotions].[Save-It Sale]" + "[Promotions].[Saving Days]"
    + "[Promotions].[Savings Galore]" + "[Promotions].[Shelf Clearing Days]"
    + "[Promotions].[Shelf Emptiers]" + "[Promotions].[Super Duper Savers]"
    + "[Promotions].[Super Savers]" + "[Promotions].[Super Wallet Savers]"
    + "[Promotions].[Three for One]" + "[Promotions].[Tip Top Savings]"
    + "[Promotions].[Two Day Sale]" + "[Promotions].[Two for One]"
    + "[Promotions].[Unbeatable Price Savers]" + "[Promotions].[Wallet Savers]"
    + "[Promotions].[Weekend Markdown]" + "[Promotions].[You Save Days]";

  private static final String moreThan4000highCardResults = "[Promotions].[Cash Register Lottery]"
    + "[Promotions].[No Promotion]" + "[Promotions].[Price Savers]";

  private static final String moreThan4000Cells = "4792.0195448.04094.0";

  private static final String nonEmptyCells = "901.02081.01789.0932.0700.0921.04792.01219.0"
  ...
  private static final String topcount40Cells = "195448.04792.04094.03145.02654.02628.02572.02497.0"
  ...
  private static final String topcount41Cells = "195448.04792.04094.03145.02654.02628.02572.02497.02203.0"
