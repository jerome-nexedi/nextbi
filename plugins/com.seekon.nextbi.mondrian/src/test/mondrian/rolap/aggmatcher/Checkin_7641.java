...
/**
 * Checkin 7641 attempted to correct a problem demonstrated by this junit. The
 * original problem involved implicit Time member usage in on axis and the use
 * of the default Time member in the other axis. This junit defines a hierarchy
 * Product with a default member 'Class2', The MDX in one axis explicitly uses
 * the {Product][Class1] member. Depending upon whether the 7641 code is used or
 * not (its use depends upon the existance of a System property) one gets
 * different answers when the mdx is evaluated.
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/aggmatcher/Checkin_7641
 *          .java#10 $
 */
public class Checkin_7641 extends CsvDBTestCase {
  ...
  public void testImplicitMember() throws Exception {
    ...
    String mdx = " select NON EMPTY Crossjoin("
      ...
      + "[Product].[Class1].Children)), " + " {[Measures].[Requested Value], "
      + " [Measures].[Shipped Value]}" + ") ON COLUMNS,"
      + " NON EMPTY Hierarchize(Union({[Geography].[All Regions]},"
      + "[Geography].[All Regions].Children)) ON ROWS" + " from [ImplicitMember]";
