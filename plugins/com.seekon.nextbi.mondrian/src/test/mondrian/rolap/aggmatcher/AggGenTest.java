/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/aggmatcher/AggGenTest.java#12 $
 ...
 * @author Sherman Wood
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/aggmatcher/AggGenTest
 *          .java#12 $
 */
public class AggGenTest extends FoodMartTestCase {
  ...
  public void testCallingLoadColumnsInAddCollapsedColumnOrAddzSpecialCollapsedColumn()
    throws Exception {
    ...
    Query query = rolapConn
      .parseQuery("select {[Measures].[Count]} on columns from [HR]");
    ...
    try {
      ...
      Pattern p = Pattern.compile("DEBUG - Init: Column: [^:]+: `(\\w+)`.`(\\w+)`"
        + Util.nl + "WARN - Can not find column: \\2");
      Matcher m = p.matcher(log);

      while (m.find()) {
        ResultSet rs = dbmeta.getColumns(catalogName, schemaName, m.group(1), m
          .group(2));
        assertTrue(!rs.next());
      }
    } finally {
