/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/BatchTestCase.java#39 $
 ...
 * @author Thiyagu
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/BatchTestCase.java#39 $
 * @since 06-Jun-2007
 */
public class BatchTestCase extends FoodMartTestCase {
  ...
  protected final String[] fieldValuesProductFamily = { "Food", "Non-Consumable",
    "Drink" };

  protected final String[] fieldValueProductDepartment = { "Alcoholic Beverages",
    "Baked Goods", "Baking Goods", "Beverages", "Breakfast Foods", "Canned Foods",
    "Canned Products", "Carousel", "Checkout", "Dairy", "Deli", "Eggs",
    "Frozen Foods", "Health and Hygiene", "Household", "Meat", "Packaged Foods",
  ...
  protected FastBatchingCellReader.Batch createBatch(FastBatchingCellReader fbcr,
    ...
    FastBatchingCellReader.Batch batch = fbcr.new Batch(createRequest(cubeName,
      measure, tableNames, fieldNames, values.toArray(new String[values.size()])));

    addRequests(batch, cubeName, measure, tableNames, fieldNames, fieldValues,
  ...
  protected FastBatchingCellReader.Batch createBatch(FastBatchingCellReader fbcr,
    ...
    addRequests(batch, cubeName, measure, tableNames, fieldNames, fieldValues,
      new ArrayList<String>(), 0, constraint);
    return batch;
  }

  private void addRequests(FastBatchingCellReader.Batch batch, String cubeName,
    String measure, String[] tableNames, String[] fieldNames,
    String[][] fieldValues, List<String> selectedValues, int currPos) {
    if (currPos < fieldNames.length) {
      for (int j = 0; j < fieldValues[currPos].length; j++) {
        selectedValues.add(fieldValues[currPos][j]);
        addRequests(batch, cubeName, measure, tableNames, fieldNames, fieldValues,
          selectedValues, currPos + 1);
        selectedValues.remove(fieldValues[currPos][j]);
      }
    } else {
      batch.add(createRequest(cubeName, measure, tableNames, fieldNames,
  ...
  private void addRequests(FastBatchingCellReader.Batch batch, String cubeName,
    ...
    if (currPos < fieldNames.length) {
      for (int j = 0; j < fieldValues[currPos].length; j++) {
        selectedValues.add(fieldValues[currPos][j]);
        addRequests(batch, cubeName, measure, tableNames, fieldNames, fieldValues,
          selectedValues, currPos + 1, constraint);
        selectedValues.remove(fieldValues[currPos][j]);
      }
    } else {
      batch.add(createRequest(cubeName, measure, tableNames, fieldNames,
  ...
  protected GroupingSet getGroupingSet(String[] tableNames, String[] fieldNames,
    String[][] fieldValues, String cubeName, String measure) {
    FastBatchingCellReader.Batch batch = createBatch(new FastBatchingCellReader(
      getCube(cubeName)), tableNames, fieldNames, fieldValues, cubeName, measure);
  ...
  /**
   * Checks that a given sequence of cell requests results in a particular SQL
   ...
   * 
   * <p>
   * Runs the requests once for each SQL pattern in the current dialect. If
   * there are multiple patterns, runs the MDX query multiple times, and expects
   * to see each SQL statement appear. If there are no patterns in this dialect,
   * the test trivially succeeds.
   * 
   * @param requests
   *          Sequence of cell requests
   * @param patterns
   *          Set of patterns
   */
   ...
   *          Set of patterns
   * @param negative
   *          Set to false in order to 'expect' a query or true to 'forbid' a
   *          query.
   */
  protected void assertRequestSql(CellRequest[] requests, SqlPattern[] patterns,
    boolean negative) {
    ...
    for (SqlPattern pattern : patterns) {
      ...
      Bomb bomb;
      final Locus locus = new Locus(new Execution(null, 1000), "BatchTestCase",
        "BatchTestCase");
      try {
        FastBatchingCellReader fbcr = new FastBatchingCellReader(getCube(cubeName));
    ...
    // dialect.
    if (!patternFound) {
      String warnDialect = MondrianProperties.instance().WarnIfNoPatternForDialect
        .get();

      if (warnDialect.equals(d.toString())) {
        System.out.println("[No expected SQL statements found for dialect \""
          + sqlDialect.toString() + "\" and test not run]");
      }
    }
  }
  ...
  /**
   * Checks that a given MDX query results in a particular SQL statement being
   * generated.
   * 
   * @param mdxQuery
   *          MDX query
   ...
   * 
   * @param testContext
   *          non-default test context if required
   * @param mdxQuery
   *          MDX query
   * @param patterns
   *          Set of patterns for expected SQL statements
   */
  protected void assertQuerySql(TestContext testContext, String mdxQuery,
    SqlPattern[] patterns) {
    assertQuerySqlOrNot(testContext, mdxQuery, patterns, false, false, true);
  }

  /**
   * Checks that a given MDX query does not result in a particular SQL statement
   * being generated.
   * 
   * @param mdxQuery
   *          MDX query
  ...
  protected void assertQuerySql(String mdxQuery, SqlPattern[] patterns,
    boolean clearCache) {
    assertQuerySqlOrNot(getTestContext(), mdxQuery, patterns, false, false,
      clearCache);
  }

  /**
   * During MDX query parse and execution, checks that the query results (or
   * does not result) in a particular SQL statement being generated.
   * 
   * <p>
   * Parses and executes the MDX query once for each SQL pattern in the current
   * dialect. If there are multiple patterns, runs the MDX query multiple times,
   * and expects to see each SQL statement appear. If there are no patterns in
   * this dialect, the test trivially succeeds.
   * 
   * @param testContext
   *          non-default test context if required
   * @param mdxQuery
   ...
   *          Set of patterns
   * @param negative
   *          false to assert if SQL is generated; true to assert if SQL is NOT
   *          generated
   * @param bypassSchemaCache
   *          whether to grab a new connection and bypass the schema cache
   *          before parsing the MDX query
   * @param clearCache
   *          whether to clear cache before executing the MDX query
   */
  protected void assertQuerySqlOrNot(TestContext testContext, String mdxQuery,
    SqlPattern[] patterns, boolean negative, boolean bypassSchemaCache,
    boolean clearCache) {
    ...
    if (!patternFound) {
      ...
      if (warnDialect.equals(d.toString())) {
        System.out.println("[No expected SQL statements found for dialect \""
          + dialect.toString() + "\" and test not run]");
      }
    }
  }
  ...
  protected String dialectize(Dialect.DatabaseProduct d, String sql) {
    ...
    case ACCESS:
      return sql.replaceAll("ISNULL\\(([^)]*)\\)", "Iif($1 IS NULL, 1, 0)");
    default:
      return sql;
    }
  }
  ...
  private void clearCache(RolapCube cube) {
    ...
    RolapHierarchy hierarchy = (RolapHierarchy) salesCube.lookupHierarchy(
  ...
  protected CellRequest createRequest(final String cube, final String measure,
    final String table, final String column, final String value) {
    return createRequest(cube, measure, new String[] { table },
      new String[] { column }, new String[] { value });
  }

  protected CellRequest createRequest(final String cube, final String measureName,
  ...
  protected CellRequest createRequest(final String cube, final String measure,
    ...
    return createRequest(cube, measure, new String[] { table },
      new String[] { column }, new String[] { value }, aggConstraint);
  }
  ...
  protected CellRequest createRequest(final String cube, final String measureName,
    ...
    request.addAggregateList(aggConstraint.getBitKey(star), aggConstraint
      .toPredicate(star));
  ...
  static CellRequestConstraint makeConstraintYearQuarterMonth(List<String[]> values) {
    String[] aggConstraintTables = new String[] { "time_by_day", "time_by_day",
      "time_by_day" };
    String[] aggConstraintColumns = new String[] { "the_year", "quarter",
      "month_of_year" };
    ...
    return new CellRequestConstraint(aggConstraintTables, aggConstraintColumns,
      aggConstraintValues);
  }
  ...
  static CellRequestConstraint makeConstraintProductFamilyDepartment(
    ...
    String[] aggConstraintColumns = new String[] { "product_family",
      "product_department" };
    List<String[]> aggConstraintValues = new ArrayList<String[]>();
  ...
  protected void checkNotNative(int rowCount, String mdx, String expectedResult) {
    ...
    if (expectedResult != null) {
      ...
      if (!nonNativeResult.equals(expectedResult)) {
        TestContext.assertEqualsVerbose(expectedResult, nonNativeResult, false,
   ...
   * 
   * @param resultLimit
   *          Maximum result size of all the MDX operations in this query. This
   *          might be hard to estimate as it is usually larger than the
   *          rowCount of the final result. Setting it to 0 will cause this
   *          limit to be ignored.
   * @param rowCount
   *          Number of rows returned
   * @param mdx
   ...
   * 
   * <p>
   * Optionally the query can be run with fresh connection. This is useful if
   * the test case sets its certain mondrian properties, e.g. native properties
   * like: mondrian.native.filter.enable
   * 
   * @param resultLimit
   *          Maximum result size of all the MDX operations in this query. This
   *          might be hard to estimate as it is usually larger than the
   ...
   *          limit to be ignored.
   * @param rowCount
   *          Number of rows returned. (That is, the number of positions on the
   *          last axis of the query.)
   * @param mdx
   *          Query
   * @param expectedResult
   *          Expected result string
   * @param freshConnection
   *          Whether fresh connection is required
   */
  protected void checkNative(int resultLimit, int rowCount, String mdx,
    String expectedResult, boolean freshConnection) {
    ...
    try {
      ...
      Connection con = getTestContext().withSchemaPool(reuseConnection)
        .getConnection();
      RolapNativeRegistry reg = getRegistry(con);
      ...
      if (expectedResult != null) {
        TestContext.assertEqualsVerbose(expectedResult, nativeResult, false,
          "Native implementation returned different result than " + "expected; MDX="
            + mdx);
        TestContext.assertEqualsVerbose(expectedResult, interpretedResult, false,
          "Interpreter implementation returned different result than "
            + "expected; MDX=" + mdx);
      }

      if (!nativeResult.equals(interpretedResult)) {
        TestContext.assertEqualsVerbose(interpretedResult, nativeResult, false,
  ...
  public static void checkNotNative(String mdx, Result expectedResult) {
    ...
    test.checkNotNative(getRowCount(expectedResult), mdx, TestContext
      .toString(expectedResult));
  }

  public static void checkNative(String mdx, Result expectedResult) {
    BatchTestCase test = new BatchTestCase();
    test.checkNative(0, getRowCount(expectedResult), mdx, TestContext
      .toString(expectedResult), true);
  }
  ...
  protected class TestCase {
    /**
     * Maximum number of rows to be read from SQL. If more than this number of
     * rows are read, the test will fail.
     */
  ...
  /**
   * Fake exception to interrupt the test when we see the desired query. It is
   * an {@link Error} because we need it to be unchecked ({@link Exception} is
   * checked), and we don't want handlers to handle it.
   */
  ...
  static class CellRequestConstraint {
    ...
    StarPredicate toPredicate(RolapStar star) {
      ...
      for (String[] values : valueList) {
        ...
        final StarPredicate predicate = andPredList.size() == 1 ? andPredList.get(0)
          : new AndPredicate(andPredList);
        orPredList.add(predicate);
      }

      return orPredList.size() == 1 ? orPredList.get(0)
