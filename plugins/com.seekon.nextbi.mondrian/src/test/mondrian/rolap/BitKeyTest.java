/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/BitKeyTest.java#20 $
 ...
 * @version $Id: //open/mondrian/testsrc/main/mondrian/rolap/BitKeyTest.java#20
 *          $
 */
public class BitKeyTest extends TestCase {
  ...
  public void testEquals() {
    int[][] positionsArray0 = { new int[] { 0, 1, 2, 3, },
      new int[] { 3, 17, 33, 63 }, new int[] { 1, 2, 3, 20, 21, 33, 61, 62, 63 }, };
    ...
    int[][] positionsArray1 = { new int[] { 0, 1, 2, 3, },
      new int[] { 3, 17, 33, 63 }, new int[] { 1, 2, 3, 20, 21, 33, 61, 62, 63 },
    ...
    int[][] positionsArray2 = { new int[] { 0, 1, 2, 3, },
      new int[] { 1, 2, 3, 20, 21, 33, 61, 62, 55, 56, 127, 128 },
      new int[] { 1, 2, 499 }, new int[] { 1, 2, 200, 300, 499 }, };
   ...
   * including reflexive (comparing a key to itself) and symmetric (comparing in
   * both directions). All keys must be equal (albeit different
   * representations).
   */
  ...
  public void testNotEquals() {
    int[] positions0 = { 0, 1, 2, 3, 4 };
    int[] positions1 = { 0, 1, 2, 3, };
  ...
  public void testClear() {
    ...
    assertTrue("BitKey 0 not equals after clear to 0", (bitKey0.equals(bitKey_0)));
    assertTrue("BitKey 0 not equals after clear to 64", (bitKey0.equals(bitKey_64)));
    assertTrue("BitKey 0 not equals after clear to 128",
      (bitKey0.equals(bitKey_128)));

    int size1 = 68;
    int[] positions1 = { 0, 1, 2, 3, 4, 45, 67 };
    BitKey bitKey1 = makeAndSet(size1, positions1);
    bitKey1.clear();

    assertTrue("BitKey 1 not equals after clear to 0", (bitKey1.equals(bitKey_0)));
    assertTrue("BitKey 1 not equals after clear to 64", (bitKey1.equals(bitKey_64)));
    assertTrue("BitKey 1 not equals after clear to 128",
      (bitKey1.equals(bitKey_128)));

    int size2 = 400;
    int[] positions2 = { 0, 1, 2, 3, 4, 45, 67, 213, 333 };
    BitKey bitKey2 = makeAndSet(size2, positions2);
    bitKey2.clear();

    assertTrue("BitKey 2 not equals after clear to 0", (bitKey2.equals(bitKey_0)));
    assertTrue("BitKey 2 not equals after clear to 64", (bitKey2.equals(bitKey_64)));
  ...
  public void testIsSuperSetOf() {
    int size0 = 20;
    int[] positions0 = { 0, 2, 3, 4, 23, 30 };
    BitKey bitKey0 = makeAndSet(size0, positions0);

    int size1 = 20;
    int[] positions1 = { 0, 2, 23 };
    BitKey bitKey1 = makeAndSet(size1, positions1);

    assertTrue("BitKey 1 not subset of 0", (bitKey0.isSuperSetOf(bitKey1)));

    assertTrue("BitKey 0 is subset of 1", (!bitKey1.isSuperSetOf(bitKey0)));

    int size2 = 65;
    int[] positions2 = { 0, 1, 2, 3, 4, 23, 30, 113 };
    BitKey bitKey2 = makeAndSet(size2, positions2);

    assertTrue("BitKey 0 not subset of 2", (bitKey2.isSuperSetOf(bitKey0)));
    assertTrue("BitKey 1 not subset of 2", (bitKey2.isSuperSetOf(bitKey1)));

    assertTrue("BitKey 2 is subset of 0", (!bitKey0.isSuperSetOf(bitKey2)));
    assertTrue("BitKey 2 is subset of 1", (!bitKey1.isSuperSetOf(bitKey2)));

    int size3 = 213;
    int[] positions3 = { 0, 1, 2, 3, 4, 23, 30, 113, 145, 233, 234 };
    BitKey bitKey3 = makeAndSet(size3, positions3);

    assertTrue("BitKey 0 not subset of 3", (bitKey3.isSuperSetOf(bitKey0)));
    assertTrue("BitKey 1 not subset of 3", (bitKey3.isSuperSetOf(bitKey1)));
    assertTrue("BitKey 2 not subset of 3", (bitKey3.isSuperSetOf(bitKey2)));

    assertTrue("BitKey 3 is subset of 0", (!bitKey0.isSuperSetOf(bitKey3)));
    assertTrue("BitKey 3 is subset of 1", (!bitKey1.isSuperSetOf(bitKey3)));
    assertTrue("BitKey 3 is subset of 2", (!bitKey2.isSuperSetOf(bitKey3)));
  }
    ...
    doTestOp(new Checker() {
      public void check(int size0, int[] positions0, int size1, int[] positions1) {
        ...
        for (int pos = 0; pos <= max; pos++) {
          boolean expected = contains(positions0, pos) || contains(positions1, pos);
  ...
  private interface Checker {
    void check(int size0, int[] positions0, int size1, int[] positions1);
  }
  ...
  public void testCreateFromBitSet() {
    ...
    assertEquals(
      "0x0000000000000000000000000000000000000000000000000000100000101100", bitKey
        .toString());
  ...
  public void testIterator() {
    /*
     * printBitPositions(0); printBitPositions(1); printBitPositions(2);
     * printBitPositions(3); printBitPositions(4); printBitPositions(5);
     * printBitPositions(6); printBitPositions(7); printBitPositions(8);
     * printBitPositions(9);
     */
    // BitKey.Small
  ...
  private void doTestEquals(int size0, int size1, int[][] positionsArray) {
    for (int i = 0; i < positionsArray.length; i++) {
      ...
      assertTrue("BitKey not equals size0=" + size0 + ", size1=" + size1 + ", i="
        + i, (bitKey0.equals(bitKey1)));
    }
  }

  private void doTestNotEquals(int size0, int[] positions0, int size1,
    int[] positions1) {
    BitKey bitKey0 = makeAndSet(size0, positions0);
    BitKey bitKey1 = makeAndSet(size1, positions1);

    assertTrue("BitKey not equals size0=" + size0 + ", size1=" + size1, (!bitKey0
      .equals(bitKey1)));
  }
  ...
  public void testCompareUnsignedLongArrays() {
    ...
    assertEquals(0, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] {},
      new long[] {}));
    // empty array does not equal other
    assertEquals(-1, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] {},
      new long[] { 1 }));
    // empty array with left-padding
    assertEquals(0, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] {},
      new long[] { 0, 0 }));
    assertEquals(0, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 0 },
      new long[] {}));
    assertEquals(0, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 0, 0 },
      new long[] { 0, 0 }));
    // 0x00000050000001 > 00000040000002
    assertEquals(1, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 1, 5 },
      new long[] { 2, 4 }));
    // 0x00000050000001 < 00000050000002
    assertEquals(-1, BitKey.AbstractBitKey.compareUnsignedArrays(
      new long[] { 1, 5 }, new long[] { 2, 5 }));
    // as above, with zero padding
    assertEquals(-1, BitKey.AbstractBitKey.compareUnsignedArrays(
      new long[] { 1, 5 }, new long[] { 2, 5, 0, 0 }));
    assertEquals(-1, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 1, 5,
      0, 0, 0 }, new long[] { 2, 5, 0, 0 }));
    assertEquals(-1, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 1, 5,
      0, 0, 0 }, new long[] { 2, 5 }));
    // negative numbers are interpreted as large unsigned
    assertEquals(1, BitKey.AbstractBitKey.compareUnsignedArrays(new long[] { 1, 5 },
