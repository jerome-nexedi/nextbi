/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/sql/SelectNotInGroupByTest.java#5 $
...
/**
 * Test that various values of {@link Dialect#allowsSelectNotInGroupBy} produce
 * correctly optimized SQL.
 * 
 * @author Eric McDermid
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/sql/SelectNotInGroupByTest
 *          .java#5 $
 */
public class SelectNotInGroupByTest extends BatchTestCase {
  ...
  public void testDependentPropertySkipped() {
    ...
    SqlPattern[] sqlPatterns = { new SqlPattern(Dialect.DatabaseProduct.MYSQL,
      sqlpat, sqlpat) };

    // Use dimension with level-dependent property
    TestContext tc = TestContext.instance().create(storeDimensionLevelDependent,
      cubeA, null, null, null, null);
    assertQuerySqlOrNot(tc, queryCubeA, sqlPatterns, false, false, true);
  }

  public void testIndependentPropertyNotSkipped() {
    SqlPattern[] sqlPatterns = { new SqlPattern(Dialect.DatabaseProduct.MYSQL,
      sqlWithAllGroupBy, sqlWithAllGroupBy) };

    // Use dimension with level-independent property
    TestContext tc = TestContext.instance().create(storeDimensionLevelIndependent,
      cubeA, null, null, null, null);
    assertQuerySqlOrNot(tc, queryCubeA, sqlPatterns, false, false, true);
  }
  ...
  public void testGroupByNotSkippedIfIndependentProperty() {
    ...
    TestContext tc = TestContext.instance().create(
      storeDimensionUniqueLevelIndependentProp, cubeA, null, null, null, null);
    assertQuerySqlOrNot(tc, queryCubeA, sqlPatterns, false, false, true);
  }
