/*
// $Id: //open/mondrian/testsrc/main/mondrian/xmla/XmlaBaseTestCase.java#23 $
 ...
 * @author mkambol
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/xmla/XmlaBaseTestCase.java#23
 *          $
 */
public abstract class XmlaBaseTestCase extends FoodMartTestCase {
  protected static final String LAST_SCHEMA_UPDATE_DATE = "xxxx-xx-xxTxx:xx:xx";
  ...
  /**
   * Cache servlet instances between test invocations. Prevents creation of many
   * spurious MondrianServer instances.
   */
  private static final HashMap<List<String>, WeakReference<Servlet>> SERVLET_CACHE = new HashMap<List<String>, WeakReference<Servlet>>();
  ...
  protected void validate(byte[] bytes, Document expectedDoc,
    TestContext testContext, boolean replace) throws Exception {
    ...
    try {
      ...
      if (replace) {
        gotStr = gotStr.replaceAll(" SessionId=\"[^\"]*\" ",
  ...
  protected void doTest(MockHttpServletRequest req, Properties props)
    throws Exception {
    String requestText = generateRequestString(props);
  ...
  protected void helperTestExpect(boolean doSessionId) {
    ...
    req.setHeader(XmlaRequestCallback.EXPECT,
  ...
  static class CallBack implements XmlaRequestCallback {
    ...
    public boolean processHttpHeader(HttpServletRequest request,
      HttpServletResponse response, Map<String, Object> context) throws Exception {
      return true;
    }

    public void preAction(HttpServletRequest request, Element[] requestSoapParts,
    ...
    public void postAction(HttpServletRequest request, HttpServletResponse response,
      byte[][] responseSoapParts, Map<String, Object> context) throws Exception {
    }
  }
  ...
  protected Document replaceLastSchemaUpdateDate(Document doc) {
    ...
    for (int i = 0; i < elements.getLength(); i++) {
      ...
      node.getFirstChild().setNodeValue(LAST_SCHEMA_UPDATE_DATE);
    }
    return doc;
  }

  private String ignoreLastUpdateDate(String document) {
    return document.replaceAll("\"LAST_SCHEMA_UPDATE\": \"....-..-..T..:..:..\"",
  ...
  protected Map<String, String> getCatalogNameUrls(TestContext testContext) {
    if (catalogNameUrls == null) {
      ...
      String catalog = connectProperties.get(RolapConnectionProperties.Catalog
        .name());
  ...
  protected Servlet getServlet(TestContext testContext) throws IOException,
    ...
    if (servlet == null) {
      ...
      servlet = XmlaSupport.makeServlet(connectString, catalogNameUrls,
  ...
  protected Document fileToDocument(String filename) throws IOException,
    SAXException {
  ...
  /**
   * Filters the content of a test resource. The default implementation returns
   * the content unchanged, but a derived class might override this method to
   * change the content.
   * 
   * @param testCaseName
   *          Name of current test case, e.g. "testFoo"
   * @param filename
   *          Name of requested content, e.g. "${request}"
   * @param content
   *          Content
  ...
  /**
   * Executes an XMLA request, reading the text of the request and the response
   * from attributes in {@link #getDiffRepos()}.
   * 
   * @param requestType
   ...
   *           on error
   */
  public void doTest(String requestType, Properties props, TestContext testContext)
    throws Exception {
  ...
  public void doTest(String requestType, Properties props, TestContext testContext,
    ...
    doTestInline(requestType, requestText, "response", props, testContext, role);
  }

  public void doTestInline(String requestType, String requestText,
    String respFileName, Properties props, TestContext testContext) throws Exception {
    doTestInline(requestType, requestText, respFileName, props, testContext, null);
  }

  public void doTestInline(String requestType, String requestText,
    String respFileName, Properties props, TestContext testContext, Role role)
    throws Exception {
    String connectString = testContext.getConnectString();
    Map<String, String> catalogNameUrls = getCatalogNameUrls(testContext);
    ...
    if (!xml) {
      String responseStr = (respFileName != null) ? fileToString(respFileName)
        : null;
      doTestsJson(requestText, props, testContext, connectString, catalogNameUrls,
        responseStr, XmlaConstants.Content.Data, role);
      return;
    }

    final Document responseDoc = (respFileName != null) ? fileToDocument(respFileName)
      : null;
    ...
    expectedDoc = (responseDoc != null) ? XmlaSupport.transformSoapXmla(responseDoc,
      new String[][] { { "content", "schemadata" } }, ns) : null;
    doTests(requestText, props, testContext, connectString, catalogNameUrls,
   ...
   * 
   * @param soapRequestText
   *          SOAP request
   * @param props
   *          Name/value pairs to substitute in the request
   * @param testContext
   *          Test context
   * @param connectString
   *          Connect string
   * @param catalogNameUrls
   *          Map from catalog names to URL
   * @param expectedDoc
   ...
   *          TestName.log.xml if there is an exception. If you are running the
   *          same request with different content types and the same reference
   *          log, you should pass {@code true} for the content type that has
   *          the most information (generally
   *          {@link org.olap4j.metadata.XmlaConstants.Content#SchemaData})
   * @throws Exception
   *           on error
   */
  protected void doTests(String soapRequestText, Properties props,
    TestContext testContext, String connectString,
    Map<String, String> catalogNameUrls, Document expectedDoc,
    XmlaConstants.Content content, Role role, boolean replace) throws Exception {
    if (content != null) {
    ...
    if (role != null) {
      final MondrianServer mondrianServer = MondrianServer.forConnection(testContext
        .getConnection());
    ...
    bytes = XmlaSupport.processSoapXmla(soapReqDoc, connectString, catalogNameUrls,
      callBackClassName, role, SERVLET_CACHE);

    if (DEBUG) {
      System.out.println("XmlaBaseTestCase.doTests: soap response="
        + new String(bytes));
    }
  ...
  protected void doTestsJson(String soapRequestText, Properties props,
    TestContext testContext, String connectString,
    Map<String, String> catalogNameUrls, String expectedStr,
    XmlaConstants.Content content, Role role) throws Exception {
    if (content != null) {
      props.setProperty(XmlaBasicTest.CONTENT_PROP, content.name());
    }
    if (role != null) {
      final MondrianServer mondrianServer = MondrianServer.forConnection(testContext
        .getConnection());
      final LockBox.Entry entry = mondrianServer.getLockBox().register(role);
      props.setProperty(XmlaBaseTestCase.ROLE_PROP, entry.getMoniker());
    }
    soapRequestText = Util.replaceProperties(soapRequestText, Util.toMap(props));
  ...
  protected static abstract class XmlaRequestCallbackImpl implements
    XmlaRequestCallback {
    ...
    public boolean processHttpHeader(HttpServletRequest request,
      ...
      if ((expect != null)
        ...
        Helper.generatedExpectResponse(request, response, context);
        return false;
      } else {
        return true;
      }
    ...
    private void setSessionId(Map<String, Object> context) {
      context.put(MY_SESSION_ID, getSessionId(name, Action.CREATE));
    }
