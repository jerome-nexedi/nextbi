/*
// $Id: //open/mondrian/testsrc/main/mondrian/xmla/XmlaExcel2007Test.java#2 $
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/xmla/XmlaExcel2007Test.java#2
 *          $
 */
public class XmlaExcel2007Test extends XmlaBaseTestCase {
  ...
  protected String filter(String testCaseName, String filename, String content) {
    if ((testCaseName.startsWith("testMemberPropertiesAndSlicer") || testCaseName
      .equals("testBugMondrian761"))
  ...
  private String foo(String content, String tag, String from, String to) {
    String start = "<" + tag + ">";
    String end = "</" + tag + ">";
    final String s = Util.replace(content, start + from + end, start + to + end);
  ...
  /**
   * <p>
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-679"> bug
   * MONDRIAN-679, "VisualTotals gives ClassCastException when called via
   * XMLA"</a>.
   */
  public void test01() {
    helperTest(false);
  ...
  /**
   * Test that checks that (a) member properties are in correct format for Excel
   * 2007, (b) the slicer axis is in the correct format for Excel 2007.
   */
  ...
  /**
   * Tests that executed MDX query with CELL PROPERTIES included; bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-708">MONDRIAN-708, "After
   * change 13351 all Excel pivots fail to update. CellInfo element in XMLA
   * response is wrong"</a>.
   * 
   * <p>
   * CellInfo element should always contain all requested cell properties. Cell
   * itself can contain fewer properties than requested.
   * 
   * <p>
   * Currently most properties are not implemented or not defined. If they get
   * implemented then test needs to be changed.
   */
