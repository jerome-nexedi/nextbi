 ...
 * 
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/util/MemoryMonitorTest.java#13
 *          $
...
public class MemoryMonitorTest extends FoodMartTestCase {
  ...
  /**
   * Get the difference between the maximum memory and the used memory and
   * divide that by 1000. This is the size of allocation chunks. Keep allocating
   * chunks until an <code>OutOfMemoryError</code> is created.
   */
  ...
  /*
   * Does not work without the notify on add feature. public void
   * testZeroUsage() throws Exception { if (Util.PreJdk15 || !enabled) { return;
   * } class Listener implements MemoryMonitor.Listener { boolean wasNotified =
   * false; Listener() { } public void memoryUsageNotification(long used, long
   * max) { wasNotified = true; } } Listener listener = new Listener();
   * MemoryMonitor mm = MemoryMonitorFactory.getMemoryMonitor(); try { // We use
   * a percentage of '0' because we know that value is // less than or equal to
   * the lowest JVM memory usage. mm.addListener(listener, 0); if (!
   * listener.wasNotified) { fail("Listener callback not called"); } } finally {
  ...
  /*
   * Does not work without the notify on add feature. public void
   * testUpdatePercent() throws Exception { if (Util.PreJdk15 || !enabled) {
   * return; } class Listener implements MemoryMonitor.Listener { boolean
   * wasNotified = false; Listener() { } public void
   * memoryUsageNotification(long used, long max) { wasNotified = true; } }
   * Listener listener = new Listener(); // we will set a percentage well above
   * the current // used level, and then allocate an object, and then // update
   * percentage to below new usage level. long maxMemory = mm.getMaxMemory();
   * long usedMemory = mm.getUsedMemory(); int currentPercentage =
   * convertThresholdToPercentage(usedMemory, maxMemory); int delta = (int)
   * (maxMemory - usedMemory)/10; int percentage =
   * convertThresholdToPercentage(delta, maxMemory); try {
   * mm.addListener(listener, 2 * percentage + currentPercentage); byte[] bytes
   * = new byte[(int) (1.5 * delta)]; if (listener.wasNotified) {
   * fail("Listener callback was called"); }
   * mm.updateListenerThreshold(listener, percentage + currentPercentage); if (!
   * listener.wasNotified) { fail("Listener callback was not called"); } }
   * finally { mm.removeListener(listener); } }
   */
  ...
  /**
   * Run this by itself and it works across 2 orders of magnitude. Run it with
   * other tests and its hard to pick the right values for the percentage and
   * how much to allocate for it to always work.
   * 
   * @throws Exception
   */
  public void _testQuery() throws Exception {
    ...
    try {
      ...
      if (level > usedMemory) {
        ...
        // int currentPercentage = convertThresholdToPercentage(level,
        // maxMemory);
