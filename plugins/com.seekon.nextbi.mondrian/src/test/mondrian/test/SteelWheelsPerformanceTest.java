/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/SteelWheelsPerformanceTest.java#3 $
...
/**
 * Those performance tests use the steel wheels schema and are not meant to be
 * run as part of the CI test suite. They must be deactivated by default.
 * 
 * @author LBoudreau
 */
public class SteelWheelsPerformanceTest extends TestCase {
  ...
  private static final Logger LOGGER = Logger
    .getLogger(SteelWheelsPerformanceTest.class);
  ...
  /**
   * This test execute a specially crafted query with tons of filters and sort
   * to test the performance of some bug fixes before/after.
   */
  public void testComplexFilters() throws Exception {
    if (!LOGGER.isDebugEnabled()) {
      return;
    }
    final String query = "with set [*NATIVE_CJ_SET] as 'NonEmptyCrossJoin([*BASE_MEMBERS_Product], NonEmptyCrossJoin([*BASE_MEMBERS_Markets], NonEmptyCrossJoin([*BASE_MEMBERS_Customers], NonEmptyCrossJoin([*BASE_MEMBERS_Time], [*BASE_MEMBERS_Order Status]))))'\n"
