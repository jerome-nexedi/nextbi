/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/DialectTest.java#38 $
...
/**
 * Unit test which checks that {@link mondrian.spi.Dialect} accurately
 * represents the capabilities of the underlying database.
 * 
 * <p>
 * The existing mondrian tests, when run on various databases and drivers, make
 * sure that Dialect never over-states the capabilities of a particular
 ...
 * 
 * <p>
 * <strong>NOTE: If you see failures in this test, let the mondrian developers
 * know!</strong> You may be running a version of a database which no one has
 * tried before, and which has more capabilities than we expect. If you tell us
 * about them, we can change mondrian to use those features.
 * </p>
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/DialectTest.java#38
 *          $
 * @since May 18, 2007
 */
public class DialectTest extends TestCase {
  ...
  private static final String INFOBRIGHT_UNSUPPORTED = "The query includes syntax that is not supported by the Infobright"
  ...
  private static final String NEOVIEW_SYNTAX_ERROR = "(?s).* ERROR\\[15001\\] A syntax error occurred at or before: .*";
  ...
  public void testAllowsCompoundCountDistinct() {
    String sql = dialectize("select count(distinct [customer_id], [product_id])\n"
  ...
  public void testAllowsCountDistinct() {
    String sql1 = dialectize("select count(distinct [customer_id]) from [sales_fact_1997]");
    // one distinct-count and one nondistinct-agg
    String sql2 = dialectize("select count(distinct [customer_id]),\n"
      + " sum([time_id])\n" + "from [sales_fact_1997]");
  ...
  public void testAllowsMultipleCountDistinct() {
    ...
    String sql1 = dialectize("select count(distinct [customer_id]),\n"
      + " count(distinct [time_id])\n" + "from [sales_fact_1997]");
    // multiple distinct-counts with group by and other aggs
    String sql3 = dialectize("select [unit_sales],\n"
  ...
  public void testAllowsFromQuery() {
    String sql = dialectize("select * from (select * from [sales_fact_1997]) as [x]");
  ...
  public void testRequiresFromQueryAlias() {
    ...
    String sql = dialectize("select * from (select * from [sales_fact_1997])");
    if (getDialect().requiresAliasForFromQuery()) {
      String[] errs = {
      // mysql
  ...
  public void testAllowsOrderByAlias() {
    String sql = dialectize("SELECT [unit_sales] as [x],\n"
  ...
  public void testRequiresUnionOrderByExprToBeInSelectClause() {
    String sql = dialectize("SELECT [unit_sales], [store_sales]\n"
      + "FROM [sales_fact_1997]\n" + "UNION ALL\n"
  ...
  public void testSupportsGroupByExpressions() {
    String sql = dialectize("SELECT sum([unit_sales] + 3) + 8\n"
  ...
  /**
   * Tests that the {@link mondrian.spi.Dialect#supportsGroupingSets()} dialect
   * property is accurate.
   */
  public void testAllowsGroupingSets() {
    String sql = dialectize("SELECT [customer_id],\n" + " SUM([store_sales]),\n"
  ...
  public void testResultSetConcurrency() {
    ...
    int[] Concurs = { ResultSet.CONCUR_READ_ONLY, ResultSet.CONCUR_UPDATABLE };
    String sql = dialectize("SELECT [unit_sales] FROM [sales_fact_1997]");
    for (int type : Types) {
      for (int concur : Concurs) {
        boolean b = getDialect().supportsResultSetConcurrency(type, concur);
        Statement stmt = null;
        try {
          ...
          if (!b) {
            ...
            // But don't fail.
            Util.discard("expected to fail for type=" + type + ", concur=" + concur);
          }
        } catch (SQLException e) {
          if (b) {
            fail("expected to succeed for type=" + type + ", concur=" + concur);
  ...
  public void testGenerateInline() throws SQLException {
    ...
    assertInline(nameList, typeList, new String[] { "a", "1" });

    assertInline(nameList, typeList, new String[] { "a", "1" }, new String[] { "bb",
      "2" });
    ...
    assertInline(nameList, typeList, new String[] { "can't stop", "1" },
      new String[] { "back\\slash", "2" });

    // date value
    final List<String> typeList2 = Arrays.asList("String", "Date");
    assertInline(nameList, typeList2, new String[] { "a", "2008-04-29" },
      new String[] { "b", "2007-01-02" });
  }
   ...
   *          Whether ascending
   * @param nullsLast
   *          Force nulls last or not.
   */
  private void checkForceNullCollation(boolean ascending, boolean nullsLast)
    throws SQLException {
    ...
    String query = "select "
      ...
      + " order by "
      + dialect.generateOrderItem(dialect.quoteIdentifier("store_manager"), true,
        ascending, nullsLast);
    if (ascending) {
      if (nullsLast) {
  ...
  private void assertFirstLast(String query, String expectedFirst,
  ...
  private void assertInline(List<String> nameList, List<String> typeList,
    String[]... valueList) throws SQLException {
    String sql = getDialect().generateInline(nameList, typeList,
   ...
   *          SQL query
   * @param patterns
   *          Array of expected patterns, generally one for each SQL dialect for
   *          which the test is expected to fail
   */
  protected void assertQueryFails(String sql, String[] patterns) {
    ...
    try {
      ...
      try {
        ...
        throw new AssertionFailedError("error [" + message
  ...
  public void testAllowsSelectNotInGroupBy() throws SQLException {
    Dialect dialect = getDialect();
    String sql = "select " + dialect.quoteIdentifier("time_id") + ", "
      + dialect.quoteIdentifier("the_month") + " from "
      + dialect.quoteIdentifier("time_by_day") + " group by "
      + dialect.quoteIdentifier("time_id");
    if (dialect.allowsSelectNotInGroupBy()) {
      final ResultSet resultSet = getConnection().createStatement()
        .executeQuery(sql);
      assertTrue(resultSet.next());
      resultSet.close();
    } else {
  ...
  public void testHavingRequiresAlias() throws Exception {
    Dialect dialect = getDialect();
    StringBuilder sb = new StringBuilder("select upper("
      + dialect.quoteIdentifier("customer", "fname") + ") as c from "
      + dialect.quoteIdentifier("customer") + " group by "
      + dialect.quoteIdentifier("customer", "fname") + " having "
      + dialect.quoteIdentifier("customer", "fname") + " LIKE ");
    dialect.quoteStringLiteral(sb, "%");
    if (!dialect.requiresHavingAlias()) {
      final ResultSet resultSet = getConnection().createStatement().executeQuery(
        sb.toString());
      assertTrue(resultSet.next());
      resultSet.close();
    } else {
  ...
  public void testAllowsRegularExpressionInWhereClause() throws Exception {
    ...
    if (dialect.allowsRegularExpressionInWhereClause()) {
      assertNotNull(dialect.generateRegularExpression(dialect.quoteIdentifier(
        "customer", "fname"), "(?i).*\\QJeanne\\E.*"));
      StringBuilder sb = new StringBuilder("select "
        ...
        + dialect.quoteIdentifier("customer", "fname")
        + " having "
        + dialect.generateRegularExpression(dialect.quoteIdentifier("customer",
          "fname"), "(?i).*\\QJeanne\\E.*"));
      final ResultSet resultSet = getConnection().createStatement().executeQuery(
        sb.toString());
      assertTrue(resultSet.next());
  ...
  public void testRegularExpressionSqlInjection() throws SQLException {
    ...
    boolean couldTranslate = checkRegex("(?i).*\\Qa\"\"\\); window.alert(\"\"woot');\\E.*");
    ...
    try {
      couldTranslate = checkRegex("\"(?i).*\\Qa\"\"\\); window.alert(\"\"woot');\\E.*\"");
    } catch (SQLException e) {
      throwable = e;
    }
    switch (getDialect().getDatabaseProduct()) {
  ...
  /**
   * Translates a regular expression into SQL and executes the query. Returns
   * whether the dialect was able to translate the regex.
   * 
   * @param regex
  ...
  private boolean checkRegex(String regex) throws SQLException {
    Dialect dialect = getDialect();
    final String sqlRegex = dialect.generateRegularExpression(dialect
      .quoteIdentifier("customer", "fname"), regex);
    if (sqlRegex != null) {
      String sql = "select * from " + dialect.quoteIdentifier("customer")
        + " where " + sqlRegex;
      final ResultSet resultSet = getConnection().createStatement()
        .executeQuery(sql);
