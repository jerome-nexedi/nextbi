/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/CompoundSlicerTest.java#10 $
 ...
 * @since 15 May, 2009
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/CompoundSlicerTest.java
 *          #10 $
 */
public class CompoundSlicerTest extends FoodMartTestCase {
   ...
   * 
   * <p>
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-637"> Bug
   * MONDRIAN-637, "Using Except in the slicer makes no sense"</a>.
   */
  public void testCompoundSlicerExcept() {
    ...
    assertQueryReturns("with member [Promotion Media].[Except Daily Paper] as\n"
      + "  Aggregate(\n" + "    Except(\n" + "      [Promotion Media].Children,\n"
      + "      {[Promotion Media].[Daily Paper]}))\n"
  ...
  /**
   * Tests a query with a compond slicer over tuples. (Multiple rows, each of
  ...
  /**
   * Test case for a basic query with more than one member of the same hierarchy
   * in the WHERE clause.
   */
  public void testCompoundSlicer() {
    ...
    assertQueryReturns("select [Measures].[Unit Sales] on 0,\n"
      ...
      Bug.BugMondrian555Fixed ? "Axis #0:\n" + "{[Product].[Drink]}\n"
        + "{[Product].[Food]}\n" + "{[Product].[Drink]}\n" + "Axis #1:\n"
        + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n" + "{[Gender].[All Gender]}\n"
        + "{[Gender].[F]}\n" + "{[Gender].[M]}\n"
  ...
  /**
   * Slicer that is a member expression that evaluates to null. SSAS 2005 allows
   * this, and returns null cells.
   */
  ...
  /**
   * Slicer that is literal null. SSAS 2005 allows this, and returns null cells;
   * Mondrian currently gives an error.
   */
  public void testSlicerContainsLiteralNull() {
    final String mdx = "select [Measures].[Unit Sales] on 0,\n"
      + "[Gender].Members on 1\n" + "from [Sales]\n" + "where null";
    if (Bug.Ssas2005Compatible) {
      // SSAS returns a cell set containing null cells.
      assertQueryReturns(mdx, "xxx");
    } else {
      ...
      // behavior.
      assertQueryThrows(mdx, "Function does not support NULL member parameter");
    }
  }

  /**
   * Slicer that is a tuple and one of the members evaluates to null; that makes
   * it a null tuple, and it is eliminated from the list. SSAS 2005 allows this,
   * and returns null cells.
   */
  ...
  public void testCompoundSlicerWithDistinctCount() {
    ...
    // 332 + 48 = 380 > 352
    // 1069 + 155 = 1224 > 1175
    assertQueryReturns("select [Measures].[Customer Count] on 0,\n"
   ...
   * 
   * <p>
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-675"> Bug
   * MONDRIAN-675,
  ...
  /**
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-899"> Bug
   * MONDRIAN-899,
  ...
  public void testBugMondrian899() {
    ...
    assertQueryReturns(
      "select NON EMPTY {[Measures].[Unit Sales]} ON COLUMNS, \n"
        ...
        + "from [Sales] \n"
        + "where ([Time].[1997].[Q1].[2] : [Time].[1997].[Q4].[11])", expected);
  ...
  /**
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-900"> Bug
   * MONDRIAN-900,
