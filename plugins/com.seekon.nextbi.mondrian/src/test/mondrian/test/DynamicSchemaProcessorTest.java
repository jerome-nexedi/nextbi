/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/DynamicSchemaProcessorTest.java#3 $
 ...
 * 
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/DynamicSchemaProcessorTest
 *          .java#3 $
 * @author ngoodman
 */
public class DynamicSchemaProcessorTest extends TestCase {
  ...
  public static final String FRAGMENT_TWO = "\">\n" + "<Cube name=\"Sales\">\n"
    ...
    + "   <AggExclude name=\"agg_c_10_sales_fact_1997\" />" + "</Table>"
    + " <Dimension name=\"Fake\"><Hierarchy hasAll=\"true\">"
    + "  <Level name=\"blah\" column=\"store_id\"/>" + " </Hierarchy></Dimension>"
    + " <Measure name=\"c\" column=\"store_id\" aggregator=\"count\"/>"
    + "</Cube>\n" + "</Schema>\n";

  public static final String TEMPLATE_SCHEMA = FRAGMENT_ONE + "REPLACEME"
    + FRAGMENT_TWO;
  ...
  /**
   * Tests to make sure that our base DynamicSchemaProcessor works, and Mondrian
   * is able to parse and connect to FoodMart with it
   */
  public void testFoodmartDsp() {
    final Connection monConnection = TestContext.instance().withSchemaProcessor(
      BaseDsp.class).getConnection();
  ...
  public static class BaseDsp implements DynamicSchemaProcessor {
    ...
    public String getSchema() throws Exception {
      return DynamicSchemaProcessorTest.TEMPLATE_SCHEMA.replaceAll("REPLACEME",
        this.replaceToken);
    }
  }
  ...
  public void testProviderTestDSP() {
    Connection monConnection = TestContext.instance().withSchemaProcessor(
      ProviderTestDSP.class).getConnection();
    assertEquals(monConnection.getSchema().getName(), "mondrian");
  }

  /**
   * DSP that checks that replaces the Schema Name with the name of the Provider
   * property
   * 
   * @author ngoodman
   * 
  ...
  public void testDBInfoDSP() {
    Connection monConnection = TestContext.instance().withSchemaProcessor(
      FoodMartCatalogDsp.class).getConnection();
    assertEquals(monConnection.getSchema().getName(),
  ...
  public void testCheckJdbcPropertyDsp() {
    Connection monConnection = TestContext.instance().withSchemaProcessor(
      CheckJdbcPropertyDsp.class).getConnection();
    assertEquals(monConnection.getSchema().getName(),
   ...
   * multiple databases all we check in schema name is the first four characters
   * (JDBC)
   * 
   * @author ngoodman
   * 
