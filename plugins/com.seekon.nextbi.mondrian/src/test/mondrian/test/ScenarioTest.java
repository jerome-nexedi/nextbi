/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/ScenarioTest.java#8 $
 ...
 * @author jhyde
 * @since 24 April, 2009
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/ScenarioTest.java#8
 *          $
 */
public class ScenarioTest extends FoodMartTestCase {
  ...
  public void testSetCell() throws SQLException {
    ...
    try {
      ...
      final Scenario scenario = connection.createScenario();
      connection.setScenario(scenario);
      connection.prepareOlapStatement("select {[Measures].[Unit Sales]} on 0,\n"
  ...
  public void testSetCellWithoutScenarioFails() throws SQLException {
    ...
    try {
      assertNull(connection.getScenario());
      final PreparedOlapStatement pstmt = connection
  ...
  public void testSetCellCalcError() throws SQLException {
    ...
    PreparedOlapStatement pstmt = connection
      .prepareOlapStatement("with member [Measures].[Unit Sales Plus One]\n"
    ...
    try {
      ...
      TestContext.checkThrowable(e, "Cannot write to cell: one of the coordinates "
        + "([Measures].[Unit Sales Plus One]) is a calculated member");
    }

    // Calc member on non-measures dimension
    cellSet = pstmt.executeOlapQuery("with member [Product].[FoodDrink]\n"
  ...
  public void testUnsupportedAllocationPolicyFails() throws SQLException {
    ...
    for (AllocationPolicy policy : AllocationPolicy.values()) {
      ...
      try {
        ...
        TestContext.checkThrowable(e, "Allocation policy " + policy
          + " is not supported");
      }
    }
    try {
  ...
  private void assertAllocation(final AllocationPolicy allocationPolicy)
    throws SQLException {
    ...
    final TestContext testContext = TestContext.instance().createSubstitutingCube(
      ...
      "<Dimension name='Scenario' foreignKey='time_id'>\n"
        ...
        + "</Dimension>", "<Measure name='Atomic Cell Count' aggregator='count'/>")
      .withScenario();
    final OlapConnection connection = testContext.getOlap4jConnection();
    ...
    cellSet = stmt.executeOlapQuery("select {[Measures].[Unit Sales]} on 0,\n"
      + "{[Product].Children} on 1\n" + "from [Sales]\n" + "where [Scenario].["
      + id2 + "]");
  ...
  /**
   * Test case for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-815">MONDRIAN-815</a>,
  ...
  public void testBugMondrian815() throws SQLException {
    ...
    final PreparedOlapStatement pstmt = connection
      .prepareOlapStatement("select NON EMPTY [Gender].Members ON COLUMNS,\n"
    ...
    TestContext.assertEqualsVerbose("Axis #0:\n"
      ...
      + "Row #0: 2\n" + "Row #1: 4\n" + "Row #1: 2\n" + "Row #1: 2\n", TestContext
      .toString(cellSet));
    cellSet.getCell(Arrays.asList(0, 1)).setValue(10,
      AllocationPolicy.EQUAL_ALLOCATION);
    cellSet.getCell(Arrays.asList(1, 0)).setValue(999,
      AllocationPolicy.EQUAL_ALLOCATION);
