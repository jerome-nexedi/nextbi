/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/ParentChildHierarchyTest.java#59 $
 ...
 * @since Mar 6, 2003
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/ParentChildHierarchyTest
 *          .java#59 $
 */
public class ParentChildHierarchyTest extends FoodMartTestCase {
   ...
   * closure of the parent/child relation, ie it represents ancestor/descendant,
   * having a row for each ancestor/descendant pair.
   * 
   * <p>
  ...
  /**
   * Returns a TestContext in which the "HR" cube contains an extra dimension,
   * "EmployeesSnowFlake", which is a joined hierarchy with a closure. this is
   * almost identical to employee, except we do a join with store to validate
   * joins with closures work
   */
  ...
  private TestContext getEmpSharedClosureTestContext() {
    String sharedClosureDimension = "<Dimension name=\"SharedEmployee\">"
    ...
    return TestContext.instance().create(sharedClosureDimension, cube, null, null,
      null, null);
  }
  ...
  private TestContext getEmpNonClosureTestContext() {
    return TestContext.instance().createSubstitutingCube(
      ...
      "<Dimension name=\"EmployeesNonClosure\" foreignKey=\"employee_id\">"
        + "<Hierarchy hasAll=\"true\" allMemberName=\"All Employees\""
        + "    primaryKey=\"employee_id\">" + "  <Table name=\"employee\"/>"
        ...
        + "    <Property name=\"Education Level\" column=\"education_level\"/>"
        + "    <Property name=\"Management Role\" column=\"management_role\"/>"
        + "  </Level>" + "</Hierarchy>" + "</Dimension>", null);
  }
    ...
    context
      .assertQueryReturns(
        "Select "
          ...
          + "from EmployeeSharedClosureCube", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void _testNonClosureParentChildHierarchy() {
    getEmpNonClosureTestContext().assertQueryReturns(
      "Select " + "{[EmployeesNonClosure].[Sheri Nowmer].children} on columns,"
        + "{[Time].[1997]} ON rows " + "from HR",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testDistinctAll() {
    ...
    assertQueryReturns("select {[Measures].[Count], [Measures].[Org Salary], \n"
      + "[Measures].[Number Of Employees], [Measures].[Avg Salary]} on columns,\n"
      + "{[Employees]} on rows\n" + "from [HR]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Count]}\n" + "{[Measures].[Org Salary]}\n"
      + "{[Measures].[Number of Employees]}\n" + "{[Measures].[Avg Salary]}\n"
  ...
  /**
   * Script That Uses the LEAVES Flag to Return the Bottom 10 Dimension Members,
   * from <a
   * href="http://www.winscriptingsolutions.com/Files/09/27139/Listing_01.txt"
   * >here</a>.
   */
  ...
  /**
   * Script from <a
   * href="http://www.winscriptingsolutions.com/Files/09/27139/Listing_02.txt"
   * >here</a>.
   */
  ...
  /**
   * The recursion cyclicity check kicks in when the recursion depth reachs the
   * number of dimensions in the cube. So create a cube with fewer dimensions
   * (3) than the depth of the emp dimension (6).
   */
  public void testHierarchyFalseCycle() {
    ...
    assertQueryReturns("SELECT {[Employees].[All Employees].Children} on columns,\n"
  ...
  public void testParentChildDrillThrough() {
    ...
    checkDrillThroughSql(result, 0, extendedContext, "[Employees].[All Employees]",
      "$39,431.67", "select" + " `time_by_day`.`the_year` as `Year`,"
        + " `salary`.`salary_paid` as `Org Salary` "
        + "from `time_by_day` =as= `time_by_day`," + " `salary` =as= `salary` "
        + "where `salary`.`pay_date` = `time_by_day`.`the_date`"
        + " and `time_by_day`.`the_year` = 1997 "
        + "order by `time_by_day`.`the_year` ASC", 7392);
    ...
    checkDrillThroughSql(result, 1, extendedContext, "[Employees].[Sheri Nowmer]",
      "$39,431.67", "select `time_by_day`.`the_year` as `Year`,"
        ...
        + "order by `time_by_day`.`the_year` ASC, `employee`.`employee_id` ASC", 12);

    // Drill-through for row #2, [Employees].[All].[Sheri Nowmer].
    // Note that the SQL does not contain the employee_closure table.
    checkDrillThroughSql(result, 2, extendedContext,
      "[Employees].[Sheri Nowmer].[Derrick Whelply]", "$36,494.07",
  ...
  public void testParentChildDrillThroughWithContext() {
    ...
    checkDrillThroughSql(result, 2, extendedContext,
      "[Employees].[Sheri Nowmer].[Derrick Whelply]", "$36,494.07", "select"
        ...
        + " `employee`.`employee_id` as `Employee Id (Key)`,"
        + " `salary`.`salary_paid` as `Org Salary` " + "from"
        + " `time_by_day` =as= `time_by_day`," + " `salary` =as= `salary`,"
        + " `store` =as= `store`," + " `employee` =as= `employee`,"
        + " `position` =as= `position`," + " `department` =as= `department` "
        + "where" + " `salary`.`pay_date` = `time_by_day`.`the_date`"
        + " and `time_by_day`.`the_year` = 1997"
        + " and `salary`.`employee_id` = `employee`.`employee_id`"
        ...
        + " and `employee`.`employee_id` = 2 " + "order by"
        ...
        + " `position`.`pay_type` ASC," + " `store`.`store_type` ASC,"
        + " `employee`.`management_role` ASC," + " `employee`.`position_title` ASC,"
        + " `department`.`department_id` ASC," + " `employee`.`full_name` ASC,"
        + " `employee`.`employee_id` ASC", 12);
  }

  private void checkDrillThroughSql(Result result, int row, boolean extendedContext,
    String expectedMember, String expectedCell, String expectedSql, int expectedRows) {
   ...
   * "ordinalColumn" attribute is included in its definition. Testcase for <a
   * href="http://jira.pentaho.org/browse/MONDRIAN-203">MONDRIAN-203,
  ...
  public void testLevelMembers() {
    ...
    // <Level>.MEMBERS
    testContext
      .assertExprReturns("[Employees].[Employee Id].Members.Count", "1,155");
    // <Member>.CHILDREN
    ...
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        "<Cube name=\"CustomSales\">"
          ...
          + "</CalculatedMember>" + "</VirtualCube>", null, null, null);

    testContext.assertQueryReturns("select "
      + "[Employees].[All Employees].[Sheri Nowmer].[Rebecca Kanagaki].Children"
      + " ON COLUMNS, "
      + "{[Measures].[Org Salary]} ON ROWS from [CustomSalesAndHR]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n"
      + "{[Employees].[Sheri Nowmer].[Rebecca Kanagaki].[Juanita Sharp]}\n"
      + "{[Employees].[Sheri Nowmer].[Rebecca Kanagaki].[Sandra Brunner]}\n"
      + "Axis #2:\n" + "{[Measures].[Org Salary]}\n" + "Row #0: 152.76\n"
  ...
  /**
   * Verifies the fix for <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-519">MONDRIAN-519</a>, a
   * class cast exception when using non-closure parent child hierarchies.
   */
  public void testClosureVsNoClosure() {
    if (Bug.avoidSlowTestOnLucidDB(getTestContext().getDialect())) {
      return;
    }
    String cubestart = "<Cube name=\"HR4C\">\n"
      + "  <Table name=\"salary\"/>\n"
      + "  <Dimension name=\"Employees\" foreignKey=\"employee_id\">\n"
    ...
    final TestContext testClosureContext = TestContext.instance().create(null,
      cubestart + closure + cubeend, null, null, null, null);
    final TestContext testNoClosureContext = TestContext.instance().create(null,
    ...
    mdx = "select {[Measures].[Count]} ON COLUMNS,\n"
      + " NON EMPTY {[Employees].AllMembers} ON ROWS\n" + "from [HR4C]";
    expected = TestContext.toString(testClosureContext.executeQuery(mdx));
    assertTrue(expected, TestContext.unfold(expected).contains("Row #0: 21,252\n"));
    ...
    testNoClosureContext.assertQueryReturns(mdx, TestContext.unfold(expected));
  ...
  public void testSchemaReaderLevelMembers() {
    final SchemaReader schemaReader = TestContext.instance().getConnection()
    ...
    for (Cube cube : schemaReader.getCubes()) {
      ...
      for (Dimension dimension : schemaReader.getCubeDimensions(cube)) {
        for (Hierarchy hierarchy : schemaReader.getDimensionHierarchies(dimension)) {
          ...
          final List<Member> memberList = schemaReader.getLevelMembers(level, true);
          assertEquals(1155, memberList.size());
          assertEquals("[Employees].[Sheri Nowmer]", memberList.get(0)
            .getUniqueName());
          assertEquals("[Employees].[Sheri Nowmer].[Derrick Whelply]", memberList
            .get(1).getUniqueName());
        }
      }
    }
