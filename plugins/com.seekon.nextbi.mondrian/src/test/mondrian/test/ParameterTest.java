/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/ParameterTest.java#35 $
 ...
 * @since Feb 13, 2003
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/ParameterTest.java#35 $
 */
public class ParameterTest extends FoodMartTestCase {
  ...
  private void assertSetPropertyFails(String propName, String scope) {
    ...
    try {
      ...
      assertTrue(e.getMessage().indexOf(
        "Parameter '" + propName + "' (defined at '" + scope
          + "' scope) is not modifiable") >= 0);
    }
  }
  ...
  public void testParameterInFormatString() {
    assertQueryReturns("with member [Measures].[X] as '[Measures].[Store Sales]',\n"
  ...
  public void testParameterInFormatString_Bug1584439() {
    String queryString = "with member [Measures].[X] as '[Measures].[Store Sales]',\n"
      + "format_string = Parameter(\"fmtstrpara\", STRING, \"#\")\n"
      + "select {[Measures].[X]} ON COLUMNS\n" + "from [Sales]";
  ...
  public void testStringParameter() {
    String s = executeExpr("Parameter(\"S\",STRING,\"x\" || \"y\","
      + "\"A string parameter\")");
  ...
  public void testStringParameterNull() {
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', STRING, 'default')", "xxx", "foo", "xxx");
    // explicitly set parameter to null and you should not get default value
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', STRING, 'default')", "", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Len(Parameter('foo', STRING, 'default'))", "0", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', STRING, 'default') = 'default'", "false", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', STRING, 'default') = ''", "false", "foo", null);
  }

  public void testNumericParameterNull() {
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', NUMERIC, 12.3)", "234", "foo", 234);
    // explicitly set parameter to null and you should not get default value
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', NUMERIC, 12.3)", "", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', NUMERIC, 12.3) * 10", "", "foo", null);
  }

  public void testMemberParameterNull() {
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]).Name", "M", "foo", "[Gender].[M]");
    // explicitly set parameter to null and you should not get default value
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]).Name", "#null", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]).Hierarchy.Name", "Gender", "foo",
      null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]) is null", "true", "foo", null);
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]) is [Gender].Parent", "true", "foo",
      null);

    // assign null then assign something else
    getTestContext().assertParameterizedExprReturns(
      "Parameter('foo', [Gender], [Gender].[F]).Name", "M", "foo", null, "foo",
      "[Gender].[All Gender].[M]");
  }

  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-745">MONDRIAN-745,
   * "NullPointerException when passing in null param value"</a>.
   */
  public void testNullStrToMember() {
    Connection connection = getConnection();
    Query query = connection
      .parseQuery("select NON EMPTY {[Time].[1997]} ON COLUMNS, "
  ...
  public void testSetUnsetParameter() {
    Connection connection = getConnection();
    Query query = connection.parseQuery("with member [Measures].[Foo] as\n"
  ...
  public void testParameterDimension() {
    assertExprReturns("Parameter(\"Foo\",[Time],[Time].[1997],\"Foo\").Name", "1997");
    assertExprReturns(
      "Parameter(\"Foo\",[Time],[Time].[1997].[Q2].[5],\"Foo\").Name", "5");
    // wrong dimension
    assertExprThrows(
  ...
  public void testParameterHierarchy() {
    ...
    final String levelName = MondrianProperties.instance().SsasCompatibleNaming
  ...
  public void testParameterLevel() {
    assertExprReturns(
      "Parameter(\"Foo\",[Time].[Quarter], [Time].[1997].[Q3], \"Foo\").Name", "Q3");
    assertExprThrows(
  ...
  /**
   * Non-trivial default value. Example shows how to set the parameter to the
   * last month that someone in Bellflower, CA had a good beer. You can use it
   * to solve the more common problem "How do I automatically set the time
   * dimension to the latest date for which there are transactions?".
   */
  ...
  /**
   * Tests a parameter derived from another parameter. OK as long as it is not
   * cyclic.
   */
  public void testDerivedParameter() {
    assertExprReturns("Parameter(\"X\", NUMERIC, Parameter(\"Y\", NUMERIC, 1) + 2)",
      "3");
  }
  ...
  public void dontTestParamRef() {
    String s = executeExpr("Parameter(\"X\",STRING,\"x\",\"A string\") || "
      + "ParamRef(\"Y\") || " + "\".\" ||" + "ParamRef(\"X\") || "
  ...
  public void testParamDefinedTwiceFails() {
    assertQueryThrows("select {[Measures].[Unit Sales]} on rows,\n"
  ...
  public void testParamCyclicOk() {
    assertExprReturns("Parameter(\"P\", NUMERIC, ParamRef(\"Q\") + 1) + "
      + "Parameter(\"Q\", NUMERIC, Iif(1 = 0, ParamRef(\"P\"), 2))", "5");
  }

  public void testParamCyclicFails() {
    assertExprThrows("Parameter(\"P\", NUMERIC, ParamRef(\"Q\") + 1) + "
  ...
  public void testParameterMetadata() {
    Connection connection = getConnection();
    Query query = connection
      .parseQuery("with member [Measures].[A string] as \n"
    ...
    final Member member = query.getSchemaReader(true).getMemberByUniqueName(
  ...
  public void testAssignNumericParameter() {
    ...
    assertAssignParameter(para, false, new Date(),
      "' for parameter 'x', type NUMERIC");
    assertAssignParameter(para, false, new Timestamp(new Date().getTime()),
      "' for parameter 'x', type NUMERIC");
    assertAssignParameter(para, false, new Time(new Date().getTime()),
      "' for parameter 'x', type NUMERIC");
    // OK to assign null
  ...
  /**
   * Positive and negative tests assigning values to a parameter whose type is a
   * member.
   */
  public void testAssignMemberParameter() {
    final String para = "Parameter(\"x\", [Customers], [Customers].[USA])";
    assertAssignParameter(para, false, "8",
      "MDX object '[8]' not found in cube 'Sales'");
    assertAssignParameter(para, false, "8.24",
      "MDX object '[8].[24]' not found in cube 'Sales'");
    assertAssignParameter(para, false, 8, "Invalid value '8' for parameter 'x',"
      + " type MemberType<hierarchy=[Customers]>");
    assertAssignParameter(para, false, -8.56,
      "Invalid value '-8.56' for parameter 'x',"
        + " type MemberType<hierarchy=[Customers]>");
    assertAssignParameter(para, false, new BigDecimal("12.345"),
    ...
    assertAssignParameter(para, false, "[Customers].[All Customers].[Canada].[BC]",
      null);
    // non-existent member
    assertAssignParameter(para, false, "[Customers].[Canada].[Bear Province]",
    ...
    SchemaReader sr = TestContext.instance().getConnection().parseQuery(
      "select from [Sales]").getSchemaReader(true).withLocus();

    // Member of wrong hierarchy.
    assertAssignParameter(para, false, sr.getMemberByUniqueName(Id.Segment.toList(
      "Time", "1997", "Q2", "5"), true),
    ...
    assertAssignParameter(
      "Parameter(\"x\", [Customers].[State Province], [Customers].[USA].[CA])",
      false, "[Customers].[USA]", "Invalid value '[Customers].[USA]' for parameter "
        + "'x', type MemberType<level=[Customers].[State Province]>");

    // Member of right level.
    assertAssignParameter(
      "Parameter(\"x\", [Customers].[State Province], [Customers].[USA].[CA])",
      false, sr.getMemberByUniqueName(Id.Segment.toList("Customers", "USA", "OR"),
        true), null);
  }
  ...
  public void testAssignSetParameter() {
    ...
    assertAssignParameter(para, true, "8",
      "MDX object '[8]' not found in cube 'Sales'");
    assertAssignParameter(para, true, "foobar",
    ...
    assertAssignParameter(para, true, new Date(),
      "' for parameter 'x', type SetType<MemberType<hierarchy=[Customers]>");
    assertAssignParameter(para, true, new Timestamp(new Date().getTime()),
      "' for parameter 'x', type SetType<MemberType<hierarchy=[Customers]>");
    assertAssignParameter(para, true, new Time(new Date().getTime()),
      "' for parameter 'x', type SetType<MemberType<hierarchy=[Customers]>");

    // strings are OK
    assertAssignParameter(para, true,
      "{[Customers].[USA], [Customers].[All Customers].[Canada].[BC]}", null);
    // also OK without braces
    assertAssignParameter(para, true,
      "[Customers].[USA], [Customers].[All Customers].[Canada].[BC]", null);
    // also OK with non-standard spacing
    assertAssignParameter(para, true,
      "[Customers] . [USA] , [Customers].[Canada].[BC],[Customers].[Mexico]", null);
    ...
    list = Arrays.asList(sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "Mexico"), true), sr.getMemberByUniqueName(Id.Segment.toList("Time", "1997",
      "Q2", "5"), true));
    assertAssignParameter(para, true, list,
      "Invalid value '[Time].[1997].[Q2].[5]' for parameter 'x', "
        + "type MemberType<hierarchy=[Customers]>");
    ...
    list = Arrays.asList(sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "Mexico"), true), sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "Canada"), true));
    assertAssignParameter(para, true, list, null);

    // List that contains member of wrong level of right hierarchy.
    list = Arrays.asList(sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "USA", "CA"), true), sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "Mexico"), true));
    assertAssignParameter(
      "Parameter(\"x\", [Customers].[State Province], {[Customers].[USA].[CA]})",
      true, list, "Invalid value '[Customers].[Mexico]' for parameter "
        + "'x', type MemberType<level=[Customers].[State Province]>");

    // as above, strings
    assertAssignParameter(
      "Parameter(\"x\", [Customers].[State Province], {[Customers].[USA].[CA]})",
      true, "{[Customers].[USA].[CA], [Customers].[Mexico]}",
      "Invalid value '[Customers].[Mexico]' for parameter "
        + "'x', type MemberType<level=[Customers].[State Province]>");

    // List that contains members of right level, and a null member.
    list = Arrays.asList(sr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "USA", "CA"), true), null, sr.getMemberByUniqueName(Id.Segment.toList(
      "Customers", "USA", "OR"), true));
    assertAssignParameter(
      "Parameter(\"x\", [Customers].[State Province], {[Customers].[USA].[CA]})",
      true, list, null);
  }
  ...
  /**
   * Checks that assigning a given value to a parameter does (or, if {@code
   * expectedMsg} is null, does not) give an error.
   * 
   * @param parameterMdx
   *          MDX expression declaring parameter
   * @param set
   *          Whether parameter is a set (as opposed to a member or scalar)
   * @param value
   *          Value to assign to parameter
   * @param expectedMsg
   *          Expected message, or null if it should succeed
   */
  private void assertAssignParameter(String parameterMdx, boolean set, Object value,
    String expectedMsg) {
    Connection connection = getTestContext().getConnection();
    try {
      String mdx = set ? "with set [Foo] as " + parameterMdx + " \n"
  ...
  public void testParamSet() {
    Connection connection = getTestContext().getConnection();
    try {
      String mdx = "select [Measures].[Unit Sales] on 0,\n"
        + " Parameter(\"Foo\", [Time], {}, \"Foo\") on 1\n" + "from [Sales]";
      Query query = connection.parseQuery(mdx);
      SchemaReader sr = query.getSchemaReader(false);
      Member m1 = sr.getMemberByUniqueName(Id.Segment.toList("Time", "1997", "Q2",
        "5"), true);
      Member m2 = sr.getMemberByUniqueName(Id.Segment.toList("Time", "1997", "Q3"),
        true);
      Parameter p = sr.getParameter("Foo");
  ...
  public void testConnectionPropsCannotBeOverridden() {
    Set<RolapConnectionProperties> overrideableProps = Olap4jUtil.enumSetOf(
      RolapConnectionProperties.Catalog, RolapConnectionProperties.Locale);
    for (RolapConnectionProperties prop : RolapConnectionProperties.class
      .getEnumConstants()) {
  ...
  public void testSystemPropsGet() {
    for (Property property : MondrianProperties.instance().getPropertyList()) {
      assertExprReturns("ParamRef(" + Util.singleQuoteString(property.getPath())
  ...
  public void testSystemPropsGetJava() {
    ...
    assertExprReturns("ParamRef(\"java.version\")", javaVersion);
  }
  ...
  public void testMondrianPropsGetJava() {
    final String jdbcDrivers = MondrianProperties.instance().JdbcDrivers.get();
    assertExprReturns("ParamRef(\"mondrian.jdbcDrivers\")", jdbcDrivers);
  }
  ...
  public void testSchemaProp() {
    final TestContext tc = TestContext.instance().create(
      "<Parameter name=\"prop\" type=\"String\" "
        + "defaultValue=\" 'foo bar' \" />", null, null, null, null, null);
  ...
  public void testSchemaPropIllegalTypeFails() {
    final TestContext tc = TestContext.instance().create(
      "<Parameter name=\"foo\" type=\"Bad type\" defaultValue=\"1\" />", null, null,
      null, null, null);
    tc
      .assertExprThrows(
        "1",
  ...
  /**
   * Tests that a schema property fails if it references dimensions which are
   * not available.
   */
