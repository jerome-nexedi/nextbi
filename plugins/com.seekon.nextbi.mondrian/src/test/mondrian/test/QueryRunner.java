/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/QueryRunner.java#13 $
...
/**
 * Thread which runs an MDX query and checks it against an expected result. It
 * is used for concurrency testing.
 * 
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/QueryRunner.java#13
 *          $
 */
public class QueryRunner extends Thread {
  ...
  public void run() {
    ...
    try {
      Connection cxn = DriverManager
        .getConnection(StandAlone.ConnectionString, null);
      int queryIndex = -1;
      ...
      while (System.currentTimeMillis() - mStartTime < mRunTime) {
        try {
          ...
          mExceptions.add(new Exception("Exception occurred on iteration "
            + queryIndex, e));
        }
      }
  ...
  public void report(PrintStream out) {
    String message = MessageFormat.format(
      "Thread {0} ran {1} queries, {2} successfully in {3} milliseconds", mMyId,
  ...
  public static void main(String[] args) {
    if (args.length != 3) {
      ...
      runTest(Integer.parseInt(args[0]), Integer.parseInt(args[1]), Boolean
        .valueOf(args[2]));
    }
  }
}
