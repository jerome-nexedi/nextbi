/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/Main.java#124 $
 ...
 * 
 * <p>
 * The {@link #suite()} method returns a suite which contains all other Mondrian
 * tests.
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/Main.java#124 $
 */
public class Main extends TestSuite {
  ...
  /**
   * Checks to see if the tests are running one user, one iteration. Some tests
   * are not thread safe so have to be skipped if this is not true.
   * 
   * @return whether the tests are run with one user, one iteration
   */
  private static boolean isRunOnce() {
    final MondrianProperties properties = MondrianProperties.instance();
    return !properties.Warmup.get() && properties.VUsers.get() == 1
  ...
  private static void addTest(TestSuite suite, Class<? extends TestCase> testClass)
    throws Exception {
  ...
  private static void addTest(TestSuite suite, Test tests, String testClassName)
    throws Exception {
    int startTestCount = suite.countTestCases();
    suite.addTest(tests);
    int endTestCount = suite.countTestCases();
  ...
  private static void printTestInfo(TestSuite suite, String testClassName,
    int startCount, int endCount) {
    String testInfo = testSuiteInfo.get(suite);
    String newTestInfo = "[" + startCount + " - " + endCount + "] : "
      + testClassName + "\n";
    if (testInfo == null) {
