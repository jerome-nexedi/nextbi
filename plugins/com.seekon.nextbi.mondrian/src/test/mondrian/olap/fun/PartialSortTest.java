/*
// $Id: //open/mondrian/testsrc/main/mondrian/olap/fun/PartialSortTest.java#6 $
 ...
 * @since Nov 2008
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/fun/PartialSortTest.java
 *          #6 $
 */
public class PartialSortTest extends TestCase {
  ...
  private static <T extends Comparable> boolean isPartiallySorted(T[] vec,
    int limit, boolean descending) {
    // noinspection unchecked
    return isPartiallySorted(vec, limit, (Comparator<T>) ComparatorUtils
      .naturalComparator(), descending);
  }

  // same predicate generalized: uses a given Comparator
  private static <T> boolean isPartiallySorted(T[] vec, int limit,
  ...
  private static <T> boolean isPartiallySorted(T[] vec, int limit,
    Comparator<? super T> order, boolean descending, Comparator<? super T> tieBreaker) {
    // elements {0 <= i < limit} are sorted
    for (int i = 1; i < limit; i++) {
  ...
  private void speedTest(Logger logger, int length, int limit) {
    ...
    @SuppressWarnings( { "unchecked" })
    Comparator<Integer> comp = new ReverseComparator(ComparatorUtils
      .naturalComparator());
