/*
// $Id: //open/mondrian/testsrc/main/mondrian/olap/fun/SortTest.java#8 $
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/testsrc/main/mondrian/olap/fun/SortTest.java#8
 *          $
 * @since Sep 21, 2006
 */
public class SortTest extends FoodMartTestCase {
  public void testFoo() {
    ...
    double[] values = { Double.NEGATIVE_INFINITY, FunUtil.DoubleNull, -34.5, -0.001,
      0, 0.00000567, 1, 3.14, Double.NaN, Double.POSITIVE_INFINITY, };
    for (int i = 0; i < values.length; i++) {
      for (int j = 0; j < values.length; j++) {
        int expected = i < j ? -1 : i == j ? 0 : 1;
        assertEquals("values[" + i + "]=" + values[i] + ", values[" + j + "]="
          + values[j], expected, FunUtil.compareValues(values[i], values[j]));
      }
    }
  }
