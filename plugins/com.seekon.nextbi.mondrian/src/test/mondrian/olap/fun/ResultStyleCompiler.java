/*
// $Id: //open/mondrian/testsrc/main/mondrian/olap/fun/ResultStyleCompiler.java#13 $
...
/**
 * The <code>ResultStyleCompiler</code> can be used to assure that the use of
 * the container ResultStyle: ITERABLE, LIST and MUTABLE_LIST; can be requested
 * by any Calc. This ExpCompiler injects into the Exp hierarchy a special Calc,
 * the MultiCalc, that evaluates its three child Calc's (one for ITERABLE, LIST
 * and MUTABLE_LIST) and compares the lists returned to make sure that they are
 * the same. This comparison can only be done when the Member evaluation stage
 * of query evaluation is begin done the last time. [Think about it - how can
 * you tell when the evaluation is happening for the last time.] Evaluation is
 * called from the RolapResult's constructor calling the method executeAxis.
 * This happens one or more times in the while-loop. These evaluations may not
 * be complete; you can not necessarily compare results. Then, evaluation occurs
 * just below the while-loop, again calling executeAxis. In this case the
 * evaluation is complete. The trick is to llok a the stack and when one changes
 * the line number from which one is being called, then one knows one is being
 * called by the second executeAxis call in the RolapResult constructor.
 * 
 * 
 * @author Richard M. Emberson
 * @since Feb 10 2007
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/fun/ResultStyleCompiler
 *          .java#13 $
 */
public class ResultStyleCompiler extends DelegatingExpCompiler {
  ...
  private static ExpCompiler generateCompiler(Evaluator evaluator,
    Validator validator, List<ResultStyle> resultStyles) {
    // pop and then push class name
    Object context = ExpCompiler.Factory.getFactory().removeContext();
    try {
      return ExpCompiler.Factory.getExpCompiler(evaluator, validator, resultStyles);
    } finally {
  ...
  /**
   * Constructor which uses the ExpCompiler.Factory to get the default
   * ExpCompiler as an instance variable - ResultStyleCompiler is a wrapper.
   * 
   */
  ...
  public Calc compileAs(Exp exp, Type resultType, List<ResultStyle> resultStyles) {
    ...
    if (resultStyles.size() > 0) {
      ...
      if (foundIterable || foundList || foundMutableList) {
        Calc calcIter = compileAs(exp, resultType, ResultStyle.ITERABLE_ONLY);
        Calc calcList = compileAs(exp, resultType, ResultStyle.LIST_ONLY);
        Calc calcMList = compileAs(exp, resultType, ResultStyle.MUTABLELIST_ONLY);
  ...
  /**
   * Calc with three child Calcs, one for ITERABLE, LIST and MUTABLE_LIST, which
   * are evaluated during the normal evaluation process.
   */
  static class MultiCalc implements Calc {
    ...
    /**
     * Returns whether this is a final evaluation; the one that takes place
     * after the while-loop in the RolapResult constructor.
     * 
          ...
          if (o1 == null) {
            ...
            if (o1 instanceof Member) {
              ...
              if (!Arrays.equals(ma1, ma2)) {
                 ...
                 * System.out.println("MultiCalc.compare: o1="+o1); print(ma1);
                 * System.out.println("MultiCalc.compare: o2="+o2); print(ma2);
                 */
                return false;
              }
            }
