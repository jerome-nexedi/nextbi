...
/*     */{
  ...
  /*     */{
    ...
    /* 60 */synchronized (this.pools) {
      /* 61 */String serverId = account.getLoginName() + ":"
  ...
  /*     */{
    ...
    /* 82 */synchronized (this.pools) {
      /* 83 */if (account.getConnection() == null) {
        /* 84 */return null;
        /*     */}
      /* 86 */String serverId = account.getLoginName() + ":"
        + account.getConnection().getHost() + ":"
        + account.getConnection().getService();
  ...
  /*     */
  /*     */public synchronized void disconnect(Account account, String sessionId,
    String from)
  /*     */{
    /* 94 */if (!(account instanceof PaloAccount))
      /*     */return;
    /* 96 */synchronized (this.pools) {
      /* 97 */String serverId = account.getLoginName() + ":"
        + account.getConnection().getHost() + ":"
        + account.getConnection().getService();
      /* 98 */ServerConnectionPool pool = (ServerConnectionPool) this.pools
        .get(serverId);
      /* 99 */if (pool == null) {
 ...
 * .jar Qualified Name: org.palo.viewapi.internal.ConnectionPoolManager JD-Core
 * Version: 0.5.4
 */