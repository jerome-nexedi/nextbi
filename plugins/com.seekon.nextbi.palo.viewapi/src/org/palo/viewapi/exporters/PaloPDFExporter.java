...
public class PaloPDFExporter
    implements CubeViewExporter
{
    private static class BorderPdfPCellEvent
    ...
    /*      */ 
    /*      */     public int hashCode() {
    /* 1003 */       return (this.parentInPrevHier == null) ? 17 : this.parentInPrevHier.hashCode() + 
    /* 1004 */         7 * this.col;
    /*      */     }
    /*      */ 
    ...
    /*      */       }
    /* 1015 */       return (ip.parentInPrevHier.equals(this.parentInPrevHier)) && (this.col == ip.col);
    /*      */     }
    /*      */   }
    ...
    private class PDFPageEventHandler extends PdfPageEventHelper
    {
        ...
        public void onEndPage(PdfWriter writer, Document document)
        {
            ...
            if(config.isShowPageNumbers())
            {
                ...
                ColumnText.showTextAligned(cb, 1, footerPhr, (document.right() - document.left()) / 2.0F + document.leftMargin(), textBase, 0.0F);
        ...
        private String getPageText(PdfWriter writer)
        {
            ...
            {
                extPageCount++;
                return (new StringBuilder(String.valueOf(Integer.toString(lastPage)))).append(NumberUtil.getCharNumber(extPageCount)).toString();
            }
        }
    ...
    public PaloPDFExporter(PaloPDFExporterConfiguration config)
    {
        ...
        isExtPage = false;
        firstCell = null;
        this.config = config == null ? PaloPDFExporterConfiguration.createDefault() : config;
        if(config == null)
        {
    ...
    {
        columsMaxWidth = BASE_FONT_CELL.getWidthPoint((new StringBuilder(String.valueOf(cmw))).append("9").toString(), FONT_CELL.getSize());
    ...
    public void export(CubeView view)
    {
        ...
        try
        {
            ...
            firstSiteYOffSet = document.topMargin() + (config.isShowTitle() ? FONT_H1.getSize() : 0.0F);
        ...
        throws DocumentException
    {
        ...
        for(int i = 0; i < cubeDims.length; i++)
        {
            AxisHierarchy axisHierarchy = selAxis.getAxisHierarchy(cubeDims[i].getDefaultHierarchy());
        ...
        try
        {
            rows = new AxisFlatModel(view.getAxis("rows"), 1, rr != null && Boolean.parseBoolean((new StringBuilder()).append(rr).toString()));
        }
        catch(Throwable t)
        {
            doc.close();
        ...
        AxisFlatModel cols = new AxisFlatModel(view.getAxis("cols"), rc != null && Boolean.parseBoolean((new StringBuilder()).append(rc).toString()));
        ...
        for(int r = 0; r < rowsModel.length; r++)
        {
            for(int c = 0; c < colsModel[colLeaf].length; c++)
            {
                ...
                cellIndexMap.put(key, new Integer[] {
                    Integer.valueOf(r), Integer.valueOf(c)
                });
                indexCellMap.put((new StringBuilder(String.valueOf(r))).append(";").append(c).toString(), cellCoords[cellIndex]);
                cellIndex++;
            }
            ...
            for(int k = 0; k < i1; k++)
            {
                ...
                if(r.getCells() != null)
                {
                    ...
                    for(int j1 = 0; j1 < l1; j1++)
                    {
                        ...
                        if(rcCoord != null)
                            allCoordinates.add(new IndexRangeInfo(f, rcCoord[0].intValue(), rcCoord[1].intValue()));
                    }
        ...
        indexRanges = (IndexRangeInfo[])allCoordinates.toArray(new IndexRangeInfo[allCoordinates.size()]);
        ...
        selAxisHeight = config.isShowPOV() ? calcSelectionAxisHeight(doc, selAxis) : 0.0F;
        if(config.isShowPOV())
    ...
    private final float calcVerticalAxisWidth(float selAxisHeight, float horizontalAxisHeight, Document doc, int numberOfDims)
    {
        ...
        float pdfHeight = doc.getPageSize().getHeight() - firstSiteYOffSet - 9F - horizontalAxisHeight - selAxisHeight - headerRows;
    ...
    private final void addVerticalAxisHeader(Document doc, AxisItem colsModel[][], AxisItem rowsModel[][], float horizontalAxisHeight, float selAxisHeight)
    {
        ...
        g2d.dispose();
        float yoff = doc.getPageSize().getHeight() - firstSiteYOffSet - 9F - horizontalAxisHeight - selAxisHeight - headerRows;
    ...
    private void addTable(PdfWriter writer, Document doc, PdfPTable table, float colHeight, float widths[], float yOffset, int rows, 
            int colHeaderRows, float selAxisHeight)
    {
        ...
        for(int remainingRows = rows; remainingRows > 0;)
        {
            ...
            do
            {
                ...
                isExtPage = cStart != 0;
                table.writeSelectedRows(cStart, cEnd, startRow, startRow + rowsPerPage, doc.leftMargin(), sy, writer.getDirectContent());
                doc.newPage();
                cStart = cEnd;
            } while(true);
    ...
    private final void fill(Element coord[], AxisItem item, HashMap hierIndex)
    {
        ...
        {
            coord[((Integer)hierIndex.get(item.getHierarchy())).intValue()] = item.getElement();
    ...
    private final String getString(Element coord[])
    {
        ...
        for(int i = 0; i < j; i++)
        {
            ...
            if(e != null)
                b.append((new StringBuilder(String.valueOf(e.hashCode()))).append(",").toString());
        }
    ...
    private File getOutFileChecked(File dir)
    {
        ...
        File f ;
        try{
        f = File.createTempFile((new StringBuilder(String.valueOf(name))).append("_(").toString(), ").pdf", dir);
        return f;
        }catch(IOException e){
        e.printStackTrace();
        f = new File(dir, (new StringBuilder(String.valueOf(name))).append(".pdf").toString());
        if(!f.exists())
            return f;
        int n = 2;
        do
        {
            name = (new StringBuilder(String.valueOf(view.getName().replace(" ", "_")))).append("_").append(Integer.toString(n++)).append(".pdf").toString();
            f = new File(dir, name);
        } while(f.exists());
        }
        return f;
    ...
    private PdfPCell createHCell(String h, int colSpan, AxisItem item, boolean row)
    {
        ...
        if(row)
        {
            ...
            for(float rhmw = BASE_FONT_CELL_H.getWidthPoint(buf.toString(), FONT_CELL_H.getSize()) + rowHeadersMaxWidth; BASE_FONT_CELL_H.getWidthPoint(h, FONT_CELL_H.getSize()) > rhmw && length > 2; h = (new StringBuilder(String.valueOf(orig.substring(0, length)))).append("...").toString())
                length--;

        } else
        {
            for(; BASE_FONT_CELL_H.getWidthPoint(h, FONT_CELL_H.getSize()) > columsMaxWidth && length > 2; h = (new StringBuilder(String.valueOf(orig.substring(0, length)))).append("...").toString())
                length--;
                ...
                if(item.hasState(2))
                {
                    ...
                    try
                    {
                        ...
                        if(config.isShowExpansionStates())
                        {
                            ...
                            ph.add(new Chunk((new StringBuilder(" ")).append(h).toString(), FONT_CELL_H));
                        } else
                        {
    ...
    public void traverseVisible(AxisItem item, AxisItem parent, AxisItem parentInPrevHierarchy, ArrayList result, boolean start, HashMap emptyItems)
    {
        ...
        {
            if(!emptyItems.containsKey(item) || !((Boolean)emptyItems.get(item)).booleanValue())
    ...
    private final void createHeaderCells(PdfPTable table, AxisItem rowModel[][], AxisItem colModel[][], ArrayList emptyColumns)
    {
        ...
        for(int c = 0; c < colModel[colLeaf].length; c++)
            emptyItems.put(colModel[colLeaf][c], Boolean.valueOf(emptyColumns.contains(Integer.valueOf(c))));
        ...
        for(int c = 0; c < colModel[colLeaf].length; c++)
        {
            ...
            if(!emptyColumns.contains(Integer.valueOf(c)))
            {
                ...
                for(int j = 0; j < colModel.length; j++)
                {
                    ...
                    {
                        isEmpty.put(ip, Integer.valueOf(((Integer)isEmpty.get(ip)).intValue() - 1));
                    } else
                    {
    ...
    {
        String orig = h;
        for(int length = h.length(); BASE_FONT_CELL_H.getWidthPoint(h, FONT_CELL_H.getSize()) > columsMaxWidth && length > 2; h = (new StringBuilder(String.valueOf(orig.substring(0, length)))).append("...").toString())
            length--;
    ...
    private final void createRowCells(PdfPTable table, AxisItem rowModel[][], AxisItem colModel[][], ArrayList emptyColumns, ArrayList emptyRows, Cell cellValues[])
    {
        int cellPosition = 0;
        ...
        for(int i = 0; i < rowModel.length; i++)
            emptyItems.put(rowModel[i][rowModel[i].length - 1], Boolean.valueOf(emptyRows.contains(isEmpty)));
        ...
        for(int i = 0; i < rowModel.length; i++)
        {
            ...
            } else
            {
                ...
                for(int j = 0; j < rowModel[i].length; j++)
                {
                    ...
                    {
                        isEmpty.put(Integer.valueOf(j), Integer.valueOf(((Integer)isEmpty.get(Integer.valueOf(j))).intValue() - 1));
                ...
                for(int c = start; c < end; c++)
                    ...
                    } else
                    {
                        ...
                        PdfPCell cell = createCell(tv, getFormat(i, c - start, (Element[])indexCellMap.get((new StringBuilder(String.valueOf(i))).append(";").append(c - start).toString())));
        ...
        PdfPCell c;
        int bits;
        for(Iterator iterator = lastRow.iterator(); iterator.hasNext(); c.setBorder(bits))
        {
        ...
        for(Iterator iterator1 = allCells.iterator(); iterator1.hasNext(); table.addCell(c))
    ...
    {
        if(cHeight == -1F)
            cHeight = FONT_CELL_H.getSize() + cell.getPaddingBottom() + cell.getPaddingTop() + cell.getBorderWidth();
        float cm = cell.getBorderWidth() + cell.getPaddingLeft() + cell.getPaddingRight();
    ...
    private void calcColWidth(boolean header, int column, PdfPCell cell, String tv, int rowDepth)
    {
        ...
        if(header)
            if(rowDepth > -1)
            {
                ...
                for(float rhmw = BASE_FONT_CELL_H.getWidthPoint(buf.toString(), FONT_CELL_H.getSize()) + rowHeadersMaxWidth; BASE_FONT_CELL_H.getWidthPoint(tv, FONT_CELL_H.getSize()) > rhmw && length > 2; tv = (new StringBuilder(String.valueOf(orig.substring(0, length)))).append("...").toString())
                    length--;

            } else
            {
                for(; BASE_FONT_CELL_H.getWidthPoint(tv, FONT_CELL_H.getSize()) > columsMaxWidth && length > 2; tv = (new StringBuilder(String.valueOf(orig.substring(0, length)))).append("...").toString())
                    length--;
        ...
        if(cell.getPhrase().getChunks().size() >= 2 && config.isShowExpansionStates())
            imWidth = 9;
        float w = header ? BASE_FONT_CELL_H.getWidthPoint(tv, FONT_CELL_H.getSize()) + (float)imWidth : BASE_FONT_CELL.getWidthPoint(formatNumber(tv, null), FONT_CELL.getSize());
        w += cm;
    ...
    private final Format getFormat(int row, int col, Element coords[])
    {
        ...
        if(coords != null)
        {
            ...
            for(int j = 0; j < l; j++)
            {
                ...
                {
                    valid = false;
                    for(Iterator iterator = ((ArrayList)allDimensionLevels.get(e.getDimension())).iterator(); iterator.hasNext();)
                    {
