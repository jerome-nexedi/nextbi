...
public class ModellerChildLoader implements ChildLoader {
  ...
  public boolean accepts(XObject parent) {
    return parent instanceof XServer || parent instanceof XDatabase
      || parent instanceof XCube || parent instanceof XFolder
  ...
  public XObject[] loadChildren(XObject parent, UserSession userSession) {
    ...
    // AuthUser user =
    // (AuthUser)
    // XObjectMatcher.getNativeObject(((XModellerObject)parent).getUser());
    //
    ...
    // ret = new XFolder[2];
    //
    // ret[0] = addStaticFolder(parent, "Dimensions", xdb.getDbId(),
    // TYPE_STATIC_FOLDER_DIMENSIONS);
    // ret[1] = addStaticFolder(parent, "Cubes", xdb.getId(),
    ...
    // } else if
    // (folder.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_DIMENSIONS)
    // ||
  ...
  private Connection getConnection() {
    ConnectionConfiguration config = ConnectionFactory.getInstance()
  ...
  private XFolder addStaticFolder(XObject parent, String label, String dbId,
    String folderType) {
  ...
  private XObject[] addCubes(XFolder parent, Database db) {
    ...
    for (int i = 0; i < allCubeCnt; i++) {
      ...
      if ((parent.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_CUBES) && isDataCube(cube))
        || (parent.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_SYSTEMCUBES) && cube
          .isSystemCube())
        || (parent.getFolderType().equalsIgnoreCase(
          TYPE_STATIC_FOLDER_ATTRIBUTECUBES) && cube.isAttributeCube())) {
        cubeCnt++;
      }
    }
    ...
    if (parent.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_CUBES)) {
      ret[curCube] = addStaticFolder(parent, "Attribute Cubes", db.getId(),
        TYPE_STATIC_FOLDER_ATTRIBUTECUBES);
      ret[curCube + 1] = addStaticFolder(parent, "System Cubes", db.getId(),
  ...
  private boolean isDataCube(Cube cube) {
    return (!cube.isSystemCube() && !cube.isAttributeCube() && !cube
      .isUserInfoCube());
  }

  private boolean isDataDimension(Dimension dim) {
    return (!dim.isSystemDimension() && !dim.isAttributeDimension() && !dim
      .isUserInfoDimension());
  }
  ...
  private XObject[] addDimensions(XFolder parent, Database db) {
    ...
    for (int i = 0; i < allDimCnt; i++) {
      ...
      if ((parent.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_DIMENSIONS) && isDataDimension(dim))
        || (parent.getFolderType().equalsIgnoreCase(
          TYPE_STATIC_FOLDER_SYSTEMDIMENSIONS) && dim.isSystemDimension())
        || (parent.getFolderType().equalsIgnoreCase(
          TYPE_STATIC_FOLDER_ATTRIBUTEDIMENSIONS) && dim.isAttributeDimension())) {
        dimCnt++;
      }
    }
    ...
    if (parent.getFolderType().equalsIgnoreCase(TYPE_STATIC_FOLDER_DIMENSIONS)) {
      ret[curDim] = addStaticFolder(parent, "Attribute Dimension", db.getId(),
        TYPE_STATIC_FOLDER_ATTRIBUTEDIMENSIONS);
      ret[curDim + 1] = addStaticFolder(parent, "System Dimension", db.getId(),
