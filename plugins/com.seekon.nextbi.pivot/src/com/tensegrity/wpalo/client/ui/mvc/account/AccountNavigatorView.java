...
/**
 * <code>AccountNavigatorView</code> TODO DOCUMENT ME
 * 
 * @version $Id: AccountNavigatorView.java,v 1.35 2010/03/02 08:59:12
 *          PhilippBouillon Exp $
 **/
  ...
  protected void handleEvent(AppEvent<?> event) {
    ...
    case WPaloEvent.SAVED_CONNECTION_ITEM:
      ...
      if (event.type == WPaloEvent.SAVED_CONNECTION_ITEM) {
        ...
        WPaloAdminServiceProvider.getInstance().hasAccount(admin.getSessionId(),
          lastCreatedConnection, new AsyncCallback<Boolean>() {
            public void onFailure(Throwable arg0) {
            }

            public void onSuccess(Boolean result) {
              if (!result) {
                AdminHelpDialog dia = new AdminHelpDialog(constants
                  .adminHintAccountCreation(), adminUserId);
                dia.showDialog();
              }
            }
          });
    ...
    del.addSelectionListener(new SelectionListener<ComponentEvent>() {
      ...
      public final void handleEvent(ComponentEvent be) {
        ...
        if (type.equals(XAccount.class.getName())) {
          ...
          confirmMsg = messages.deleteAccount(name + " - "
            + acc.getConnection().getName());
        } else if (type.equals(XConnection.class.getName()))
...
class TreeNodeBinder extends TreeBinder<TreeNode> {
  ...
  protected TreeItem createItem(TreeNode model) {
    ...
    if (type.equals(XAccount.TYPE)) {
      ...
      item.setText(xAcc.getUser().getLogin() + " - "
        + xAcc.getConnection().getName());
