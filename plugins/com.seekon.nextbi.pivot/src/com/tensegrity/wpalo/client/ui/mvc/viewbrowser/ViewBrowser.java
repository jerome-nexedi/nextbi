...
public class ViewBrowser extends View {
  protected static transient final ILocalConstants constants = Resources
    .getInstance().getConstants();

  protected static transient final ILocalMessages messages = Resources.getInstance()
    .getMessages();
  ...
  private static final boolean isNewerVersionAvailable(String buildNumber,
  ...
  public static final void displayAboutDialog(final String buildNumber,
    final String[] buildInformation) {
    WPaloPropertyServiceProvider.getInstance().getStringProperty("appName",
      new AsyncCallback<String>() {
        private final void showAbout(String appName) {
          final String aboutString = messages.aboutMessage(appName, 3, 2, 0,
            buildNumber);
          ...
          LargeQueryWarningDialog.readThresholds(new AsyncCallback<String[]>() {
            ...
            public void onSuccess(String[] result) {
              ...
              String thInfo = messages.thresholds(result[0], result[1], result[2],
              ...
              if (buildInformation != null) {
                if (isNewerVersionAvailable(buildNumber, buildInformation)) {
                  displayInfo += messages.newerVersionExists(buildInformation[0],
                    buildInformation[1])
                    + "</br>";
                } else {
                  ...
                  displayInfo += messages.noNewerVersionAvailable(infoLink)
                    + "<br/>";
                }
              }
  ...
  private final ToolBar initUI(XUser user) {
    ...
    navigator.addListener(Events.Expand, new Listener<ComponentEvent>() {
      public void handleEvent(ComponentEvent be) {
        Dispatcher.get().dispatch(WPaloEvent.EXPANDED_VIEWBROWSER_SECTION);
      }
    });
    ...
    createViews.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
    ...
    importViews.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        TreeNode node = getSelectedNode();
        importViews(node);
      }
    ...
    // public void componentSelected(ComponentEvent ce) {
    // WPaloPropertyServiceProvider.getInstance().getBuildNumber(new
    ...
    editViewProperties.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        if (targetView != null) {
          ...
          // int permission = user.getId().equals(targetView.getOwnerId()) ?
          // 2 : 16;
          int permission = user.isAdmin() ? 0 : (user.getId().equals(
            targetView.getOwnerId()) ? 16 : 22);
          WPaloCubeViewServiceProvider.getInstance().checkPermission(sessionId,
            permission, new Callback<Boolean>() {
              public void onSuccess(Boolean result) {
                if (result) {
                  final EditViewPropertiesDialog dlg = new EditViewPropertiesDialog(
                    viewToModify);
                    ...
                    new Listener<BaseEvent>() {
                      public void handleEvent(BaseEvent be) {
                        ...
                        WPaloCubeViewServiceProvider.getInstance().setVisibility(
                          sessionId, (XFolderElement) xObject, pub, edi, ownerId,
                          accId, dbId, cubeId, new Callback<XView>() {
                            public void onSuccess(XView view) {
                              XFolderElement xObj = (XFolderElement) browserModel
                                .getWrappedXObject(nodeOfView);
                  ...
                  MessageBox.alert(constants.notEnoughRights(), constants
                    .notEnoughRightsToModifyVisibility(), null);
                }
              }
            });
        }
        ...
        if (treeBinder != null && treeBinder.getTree() != null) {
          ...
          if (selection != null) {
            ...
            if (xObj instanceof XFolderElement) {
              ...
              if (source != null && source instanceof XView) {
                final CreateDirectLinkDialog cdl = new CreateDirectLinkDialog(
                  (XView) source);
                cdl.show();
              }
            }
      ...
      new Callback<Boolean>() {
        public void onSuccess(Boolean result) {
          if (result) {
            ...
            MessageBox.alert(constants.notEnoughRights(), constants
              .notEnoughRightsToCreateNewFolders(), null);
          }
        }
      });
  }
  ...
  private final void deleteFolderElement(final XFolderElement xFolderElement,
    ...
    if (xView != null) {
      String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
        .getSessionId();
      WPaloCubeViewServiceProvider.getInstance().checkPermission(sessionId,
        xView.getId(), CubeViewEditor.RIGHT_DELETE, new Callback<Boolean>() {
          public void onSuccess(Boolean result) {
            if (result) {
              if (confirm) {
                MessageBox.confirm(constants.confirmDelete(), messages
                  .sureToDeleteView(ViewBrowserModel
                    .modify(xFolderElement.getName())), new Listener<WindowEvent>() {
                  public void handleEvent(WindowEvent be) {
              ...
              MessageBox.alert(constants.notEnoughRights(), constants
                .notEnoughRightsToDeleteView(), null);
            }
          }
        });
    } else {
  ...
  private final void deleteFolder(final XStaticFolder xFolder, final TreeNode node) {
    XUser user = ((Workbench) Registry.get(Workbench.ID)).getUser();
    WPaloCubeViewServiceProvider.getInstance().checkPermission(user.getSessionId(),
      4, new Callback<Boolean>() {
        public void onSuccess(Boolean result) {
          if (result) {
            MessageBox.confirm(constants.confirmDelete(), messages
              .sureToDeleteFolder(ViewBrowserModel.modify(xFolder.getName())),
            ...
            MessageBox.alert(constants.notEnoughRights(), constants
              .notEnoughRightsToDeleteFolder(), null);
          }
        }
      });
  }
  ...
  private final void createViews(final TreeNode node) {
    if (createRight == -1) {
      String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
        .getSessionId();
      WPaloCubeViewServiceProvider.getInstance().checkPermission(sessionId,
        CubeViewEditor.RIGHT_CREATE, new Callback<Boolean>() {
          public void onSuccess(Boolean result) {
            if (result) {
              createRight = 1;
              ...
              createRight = 0;
              MessageBox.alert(constants.notEnoughRights(), constants
                .notEnoughRightsToCreateNewViews(), null);
            }
          }
        });
  ...
  private final void doOpen(TreeNode selection) {
    ...
    if (xObj instanceof XFolderElement) {
      fireEvent(new AppEvent<TreeNode>(WPaloEvent.EDIT_VIEWBROWSER_VIEW, selection));
    }
  }
  ...
  private final void delete(final XFolderElement xFolderElement, final TreeNode node) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloFolderServiceProvider.getInstance().deleteFolderElement(
      sessionId,
      xFolderElement,
      new Callback<Void>(messages.couldNotDeleteView(ViewBrowserModel
        .modify(xFolderElement.getName()))) {
  ...
  private final void delete(final XView xView, final TreeNode node) {
    if (xView == null) {
      ...
      String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
        .getSessionId();
      WPaloCubeViewServiceProvider.getInstance().deleteView(
        sessionId,
        xView,
        new Callback<Void>(messages.couldNotDeleteView(ViewBrowserModel.modify(xView
          .getName()))) {
          public void onSuccess(Void arg0) {
            fireViewDeleted(node);
          }
  ...
  private final void fireViewDeleted(TreeNode node) {
    fireEvent(new AppEvent<TreeNode>(WPaloEvent.DELETED_VIEWBROWSER_VIEW, node));
  ...
  private final void delete(final XStaticFolder folder, final TreeNode node) {
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloFolderServiceProvider.getInstance().deleteFolder(
      sessionId,
      folder,
      new Callback<Void>(messages.couldNotDeleteFolder(ViewBrowserModel
        .modify(folder.getName()))) {
        public void onSuccess(Void arg0) {
          browserModel.delete(node);
        }
      });
  ...
  private final void doRename(final TreeItem item) {
        ...
        new Callback<Boolean>() {
          public void onSuccess(Boolean result) {
            if (result) {
              ...
              MessageBox.alert(constants.notEnoughRights(), constants
                .notEnoughRightsToRenameFolder(), null);
            }
          }
        });
      return;
    ...
    if (xView != null) {
      String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
        .getSessionId();
      WPaloCubeViewServiceProvider.getInstance().checkPermission(sessionId,
        xView.getId(), CubeViewEditor.RIGHT_WRITE, new Callback<Boolean>() {
          public void onSuccess(Boolean result) {
            if (result) {
              rename(xFolderElement, xView, item);
            } else {
              MessageBox.alert(constants.notEnoughRights(), constants
                .notEnoughRightsToRenameView(), null);
            }
          }
  ...
  private final void rename(final XFolderElement xFolderElement, final XView xView,
    final TreeItem item) {
    final String newName = demodify(item.getText());
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloFolderServiceProvider.getInstance().renameFolderElement(sessionId,
      xFolderElement, newName, new Callback<Void>(constants.failedToRenameView()) {
  ...
  private final void rename(final XStaticFolder folder, final TreeItem item) {
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloFolderServiceProvider.getInstance().renameFolder(sessionId, folder,
      newName, new Callback<Void>(constants.failedToRenameFolder()) {
        public void onSuccess(Void arg0) {
          TreeNode node = (TreeNode) item.getModel();
    ...
    WPaloCubeViewServiceProvider.getInstance().renameView(sessionId, xView, newName,
      new Callback<Void>(constants.failedToRenameView()) {
        public void onSuccess(Void arg0) {
          ...
          browserModel.refresh(node);
          // checkStyle(item);
          fireEvent(new AppEvent<TreeNode>(WPaloEvent.RENAMED_VIEWBROWSER_VIEW, node));
        }
      });
  }

  @SuppressWarnings("unchecked")
  private final void dropToStaticFolder(XStaticFolder toFolder, TreeItem target,
    DNDEvent e) {
  ...
  private final void addDragAndDrop(final TreeBinder<TreeNode> binder) {
    ...
    dropTarget.addDNDListener(new DNDListener() {
      public void dragDrop(final DNDEvent e) {
        ...
        if (target != null) {
          XObject wrappedObj = browserModel.getWrappedXObject((TreeNode) target
            .getModel());
  ...
  private final XObject[] getMoveableElementsFrom(List nodes, TreeNode newParent) {
  ...
  private final void createFolder(String name, final TreeNode parent) {
    ...
    WPaloFolderServiceProvider.getInstance().createFolder(sessionId, name,
      parentFolder, new Callback<XStaticFolder>(constants.couldNotAddFolder()) {
