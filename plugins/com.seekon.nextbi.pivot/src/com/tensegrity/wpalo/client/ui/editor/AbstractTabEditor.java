...
/**
 * <code>AdminEditor</code> TODO DOCUMENT ME
 * 
 * @version $Id: AbstractTabEditor.java,v 1.29 2010/04/15 09:55:22
 *          PhilippBouillon Exp $
 **/
public abstract class AbstractTabEditor implements IEditor {
  ...
  public void beforeClose(final AsyncCallback<Boolean> cb) {
    if (isDirty()) {
      MessageBoxUtils.yesNoCancel(constants.saveEditor(), messages
        .saveEditorBeforeClosing(getTitle()), new Listener<WindowEvent>() {
  ...
  public final void doSave(final AsyncCallback<Boolean> callback) {
    ...
    if (admin != null) {
      ...
      for (EditorTab tab : tabs) {
        if (!tab.save(xObj)) {
          MessageBox.alert(constants.error(), messages.failedToSave(getTitle()),
            null);
          return;
        }
      }
      ...
      // final XUser loggedInUser =
      // ((Workbench)Registry.get(Workbench.ID)).getUser();
      ...
      // }
      // WPaloAdminServiceProvider.getInstance().saveXObject(loggedInUser.getSessionId(),
      // xObj,
      ...
      // });
      // } else {
      WPaloAdminServiceProvider.getInstance().saveXObject(
        wb.getUser().getSessionId(), xObj, new Callback<XObject>() {
          public void onFailure(Throwable t) {
            if (xObj != null && xObj.getName() != null) {
              MessageBox.alert(constants.error(), messages
                .failedToSave(ViewBrowserModel.modify(xObj.getName()))
                + t.getMessage(), null);
            } else {
              String id = xObj == null ? "" : xObj.getId();
              MessageBox.alert(constants.error(), messages.failedToSave(id)
                + t.getMessage(), null);
            }
            saved(false);
            if (callback != null) {
