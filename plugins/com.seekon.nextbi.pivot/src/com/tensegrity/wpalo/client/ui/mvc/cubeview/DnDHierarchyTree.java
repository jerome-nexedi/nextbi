...
/**
 * <code>DnDHierarchyTree</code> TODO DOCUMENT ME
 * 
 * @version $Id: DnDHierarchyTree.java,v 1.20 2010/03/11 10:43:45
 *          PhilippBouillon Exp $
 **/
public class DnDHierarchyTree {
  ...
  void applyAlias(XAxisHierarchy hierarchy) {
    ...
    traverse(new FastMSTreeItemVisitor() {
      public boolean visit(FastMSTreeItem item, FastMSTreeItem parent) {
        if (item.getXObjectModel().getXObject() instanceof XElementNode) {
          allXElementNodes.add((XElementNode) item.getXObjectModel().getXObject());
        }
        return true;
      }
    });
    ...
    WPaloCubeViewServiceProvider.getInstance().applyAlias(sessionId,
      ...
      hierarchy.getActiveAlias(), allXElementNodes.toArray(new XElementNode[0]),
  ...
  private final void add(XElementNode node, XObjectModel parent, String[] paths) {
  ...
  private final XObjectModel determineParent(Map<Integer, XObjectModel> nodeMap,
    TreeNode node) {
    TreeNode nd = node;
  ...
  public final void moveDownSelection() {
    ArrayList<FastMSTreeItem> selection = new ArrayList<FastMSTreeItem>(tree
      .getSelectedItems());
    if (!selection.isEmpty()) {
  ...
  private final void traverse(List<FastMSTreeItem> items, FastMSTreeItem parent,
    FastMSTreeItemVisitor visitor) {
