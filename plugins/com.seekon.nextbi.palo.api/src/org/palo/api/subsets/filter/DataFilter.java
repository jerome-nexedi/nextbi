...
/*     */{
  ...
  /*     */{
    ...
    /* 160 */for (Dimension d : srcCube.getDimensions()) {
      ...
      /*     */}
    /* 163 */Hierarchy[] hierarchies = (Hierarchy[]) allHiers
      .toArray(new Hierarchy[allHiers.size()]);
    ...
    /* 191 */new DataCellComparator(this.setting.getCellOperator().getValue()
      .intValue()));
    /*     */
    ...
    /* 195 */if (top < 0) {
      ...
      /*     */}
    /* 198 */double upper = this.setting.getUpperPercentage().getValue()
      .doubleValue();
    /* 199 */double lower = this.setting.getLowerPercentage().getValue()
      .doubleValue();
    /*     */
    /* 202 */int count = 0;
    ...
    /* 214 */if ((upper > 0.0D) || (lower > 0.0D)) {
      ...
      /* 216 */if (upper > 0.0D) {
        ...
        /* 219 */for (int i = sortedValues.size() - 1; i >= 0; --i) {
          ...
          /* 222 */if (lower < 0.0D) {
            ...
            /* 227 */partial = partial
              +
              /* 227 */((Double) cell.getValue(this.setting.getCellOperator()
                .getValue().intValue())).doubleValue();
            /*     */} else {
            ...
            /* 233 */partial = partial
              +
              /* 233 */((Double) cell.getValue(this.setting.getCellOperator()
                .getValue().intValue())).doubleValue();
            /*     */}
          /*     */}
        /*     */}
      ...
      /* 238 */if (lower > 0.0D) {
        ...
        /* 241 */int i = 0;
        for (int n = sortedValues.size(); i < n; ++i) {
          ...
          /* 250 */partial = partial
            +
            /* 250 */((Double) cell.getValue(this.setting.getCellOperator()
              .getValue().intValue())).doubleValue();
          /*     */}
        /*     */}
      /*     */}
  ...
  /*     */public final void validateSettings() throws PaloIOException {
    ...
    /* 263 */Cube srcCube = database.getCubeById(this.setting.getSourceCube()
      .getValue());
    /* 264 */if (srcCube == null)
      /* 265 */throw new PaloIOException(
        "DataFilter: unknown source cube with id '" + srcCube + "'!");
  ...
  /*     */{
    ...
    /* 281 */if (dataElement == null) {
      ...
      /*     */}
    /* 284 */return dataElement.getValue(
      this.setting.getCellOperator().getValue().intValue()).toString();
    /*     */}
  ...
  /*     */private final double getTotalValue(DataCell[] dataCells) {
    ...
    /* 289 */for (DataCell cell : dataCells) {
      /* 290 */if (!cell.isString())
        /* 291 */total += ((Double) cell.getValue(this.setting.getCellOperator()
          .getValue().intValue())).doubleValue();
      /*     */}
    /* 293 */return total;
    /*     */}
 ...
 * Qualified Name: org.palo.api.subsets.filter.DataFilter JD-Core Version: 0.5.4
 */