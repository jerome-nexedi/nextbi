...
/*      */ 
/*      */   static final CubeImpl create(ConnectionImpl connection, Database database, CubeInfo cubeInfo)
/*      */   {
/*  130 */     return create(connection, database, cubeInfo, null);
/*      */   }
/*      */ 
/*      */   static final CubeImpl create(ConnectionImpl connection, Database database, CubeInfo cubeInfo, Dimension[] dimensions) {
...
/*  186 */     this.viewStorageHandler = ((DatabaseImpl)database).getViewStorageHandler();
/*  187 */     this.key = 
/*  188 */       new CompoundKey(new Object[] { CubeImpl.class, connection, 
/*  188 */       cubeInfo.getDatabase().getId(), cubeInfo.getId() });
/*  189 */     this.propertyLoader = this.dbConnection.getTypedPropertyLoader(cubeInfo);
/*      */   }
/*      */ 
...
/*      */     {
/*  231 */       PaloIOException pio = new PaloIOException("CubeView loading failed!", e);
...
/*      */     try
/*      */     {
/*  329 */       CellInfo cell = this.dbConnection.getData(this.cubeInfo, getCoordinates(coordinates));
...
/*      */       }
/*  357 */       CellInfo[] cells = this.dbConnection.getDataArea(this.cubeInfo, elInfos);
...
/*      */       }
/*  376 */       CellInfo[] cells = this.dbConnection.getDataArea(this.cubeInfo, elInfos);
...
/*      */       }
/*  398 */       CellInfo[] cells = this.dbConnection.getDataArray(this.cubeInfo, coords);
...
/*  428 */     reloadDims();
/*  429 */     if ((this.dimensions == null) || (index < 0) || (index >= this.dimensions.length))
/*  430 */       return null;
...
/*  463 */     reloadDims();
/*  464 */     return (this.dimensions == null) ? null : (Dimension[])this.dimensions.clone();
/*      */   }
/*      */ 
...
/*  484 */     int result = this.dbConnection.convert(this.cubeInfo, getInfoType(type));
...
/*      */   }
/*      */ 
/*      */   public final void addDataArray(Element[][] coordinates, Object[] values, int splashMode)
/*      */   {
/*  551 */     setDataArray(coordinates, values, true, splashMode, true);
/*      */   }
/*      */ 
/*      */   public final void setDataArray(Element[][] coordinates, Object[] values, int splashMode)
/*      */   {
...
/*      */       }
/*  569 */       this.dbConnection.setDataArray(this.cubeInfo, coords, values, add, splashMode, notifyEventProcessors);
/*      */     } catch (PaloException pex) {
...
/*  584 */         double newVal = Double.parseDouble(valStr.substring(1, valStr.length() - 1));
...
/*      */   }
/*      */ 
/*      */   public final void setData(Element[] coordinates, Object value, NumberFormat formatter)
/*      */   {
...
/*      */   }
/*      */ 
/*      */   public final void setDataSplashed(String[] coordinates, Object value, int splashMode)
/*      */   {
/*  647 */     if (value == null)
/*  648 */       return;
/*      */     try
...
/*      */   }
/*      */ 
/*      */   public final void setDataSplashed(Element[] coordinates, Object value, int splashMode)
/*      */   {
/*  662 */     if (value == null)
/*  663 */       return;
/*      */     try
...
/*      */   }
/*      */ 
/*      */   public final Rule addRule(String definition, String externalIdentifier, boolean useIt, String comment)
/*      */   {
/*  733 */     return addRule(definition, externalIdentifier, useIt, comment, true);
/*      */   }
/*      */ 
/*      */   public final Rule addRule(String definition, String externalIdentifier, boolean useIt, String comment, boolean activate)
/*      */   {
...
/*      */       }
/*  885 */       CellInfo[] _cells = this.dbConnection.getDataArray(this.cubeInfo, coords);
...
/*  898 */       throw new PaloAPIException("Could not match cell coordinates to cube data. Has a dimension been removed or added to this cube?", ex);
/*      */     } catch (RuntimeException e) {
...
/*      */       }
/*  933 */       CellInfo[] _cells = this.dbConnection.getDataArray(this.cubeInfo, coords);
...
/*      */     } catch (NullPointerException ex) {
/*  948 */       throw new PaloAPIException("Could not match cell coordinates to cube data. Has a dimension been removed or added to this cube?", ex);
/*      */     } catch (RuntimeException e) {
...
/*      */       }
/*  962 */       CellInfo[] _cells = this.dbConnection.getDataArea(this.cubeInfo, coords);
...
/*      */     try {
/* 1002 */       if (lock != null)
/* 1003 */         return this.dbConnection.commit(this.cubeInfo, ((LockImpl)lock).getInfo());
/*      */     }
/*      */     catch (Exception localException) {
/*      */     }
/* 1007 */     return false;
...
/*      */   }
/*      */ 
/*      */   private final void setDataInternal(ElementInfo[] coordinates, Object value, int splashMode)
/*      */   {
/* 1177 */     if (value == null)
/* 1178 */       return;
/*      */     try
...
/* 1182 */         this.dbConnection.setDataNumericSplashed(this.cubeInfo, coordinates, 
...
/* 1202 */         (ElementImpl)dimension.getDefaultHierarchy().getElementByName(names[i]);
/* 1203 */       if (element == null)
/* 1204 */         throw new PaloAPIException("Element not found! Dimension '" + dimension.getName() + "' contains no element with name '" + names[i] + "'!!");
...
/* 1216 */       coords[i] = 
/* 1217 */         ((ElementImpl)dimension.getDefaultHierarchy().getElementByName(names[i])).getInfo();
/*      */     }
/* 1219 */     return coords;
/*      */   }
/*      */ 
...
/*      */   {
/* 1277 */     int i = 0; for (int n = this.viewObservers.size(); i < n; ++i) {
...
/*      */   {
/* 1286 */     int i = 0; for (int n = this.viewObservers.size(); i < n; ++i) {
...
/* 1294 */     int i = 0; for (int n = this.viewObservers.size(); i < n; ++i) {
...
/*      */   }
/*      */ 
/*      */   private final void setPercentage(Element[] coordinates, String valStr, NumberFormat formatter)
/*      */   {
...
/* 1349 */       return null;
/*      */     }
/* 1351 */     Property2 property = (Property2)this.loadedProperties.get(propInfo.getId());
...
/*      */   {
/* 1404 */     ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(), 
/* 1405 */       getDatabase(), 
...
/* 1413 */     ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(), 
/* 1414 */       getDatabase(), 19, 
/* 1415 */       rules);
...
/* 1420 */     ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(), 
/* 1421 */       getDatabase(), 20, 
/* 1422 */       rules);
...
/* 1427 */     ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(), 
/* 1428 */       getDatabase(), 21, 
/* 1429 */       new Rule[] { rule });
...
/* 1485 */       int i = 0; for (int n = chars.length; i < n; ++i) {
/* 1486 */         char c = chars[i];
/* 1487 */         if ((Character.isDigit(c)) || (c == '.') || (c == ',') || (c == '-') || (c == '+') || (c == 'e') || (c == 'E')) {
 ...
 * Qualified Name:     org.palo.api.impl.CubeImpl
