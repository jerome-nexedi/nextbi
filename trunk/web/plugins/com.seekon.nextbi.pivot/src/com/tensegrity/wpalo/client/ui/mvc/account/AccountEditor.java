...
/**
 * <code>AccountEditor</code> TODO DOCUMENT ME
 * 
 * @version $Id: AccountEditor.java,v 1.22 2010/04/15 09:55:22 PhilippBouillon
 *          Exp $
 **/
public class AccountEditor extends AbstractTabEditor {
  ...
  public EditorTab[] getEditorTabs() {
    return new EditorTab[] { new AccountPropertiesTab(this), };
  }
...
class AccountPropertiesTab extends EditorTab {
  ...
  private final ContentPanel createPropertiesPanel() {
    ...
    selectionListener = new SelectionChangedListener<XObjectWrapper>() {
      public void selectionChanged(SelectionChangedEvent<XObjectWrapper> se) {
        editor.markDirty();
      }
    };
    userSelectionListener = new SelectionChangedListener<XObjectWrapper>() {
      public void selectionChanged(SelectionChangedEvent<XObjectWrapper> se) {
        editor.markDirty();
  ...
  private final void fillConnections(final XConnection connection,
    final XUser forUser) {
    connStore.removeAll();
              ...
              new Callback<XConnection[]>(constants.loadingAllConnectionsFailed()) {
                public void onSuccess(XConnection[] connections) {
                  ...
                  for (XConnection conn : connections) {
                    ...
                    if (selection == null && prefillCon != null
                      && conn.equals(prefillCon)) {
                      selection = w;
                    }
                  }
                  if (selection != null) {
                  ...
                  if (selection == null && !connStore.getModels().isEmpty()
                    && doSetC) {
      ...
      new Callback<XUser[]>(constants.loadingAllUsersFailed()) {
        public void onSuccess(XUser[] users) {
          ...
          for (XUser xUser : users) {
            ...
            if (selection == null && prefillUser != null
              && xUser.equals(prefillUser)) {
              selection = w;
            }
          }
          if (selection != null) {
