...
public class WPaloServiceImpl extends BasePaloServiceServlet implements WPaloService {
  ...
  public List<TreeNode> loadChildren(String sessionId, String parentType,
    String viewId, String axisId, String parentId, String parentPath)
    throws SessionExpiredException, PaloGwtCoreException {
    XObject[] result = ChildLoaderManager.getInstance().loadChildren(parentType,
      viewId, axisId, parentId, getUserSession(sessionId));
    ArrayList<TreeNode> nodeList = new ArrayList<TreeNode>();
    try {
  ...
  public List<TreeNode> loadHierarchyTree(String sessionId, String hierarchyId,
    String viewId, String axisId, int level) throws SessionExpiredException,
    PaloGwtCoreException {
    List<TreeNode> result = loadTreeNodesFrom(getUserSession(sessionId),
      hierarchyId, viewId, axisId, level);
    return result;
  }
  ...
  private final List<XElementNode> loadElementsFrom(UserSession userSession,
    ...
    CubeViewController viewController = getViewController(userSession, xHierarchy
      .getViewId());
  ...
  private final String fastCreatePath(TreeNode tNode, XElement xKidElement,
  ...
  private final void addChildren(int currentDepth, int level,
    ElementNode[] rootNodes, List<TreeNode> nodes, String hierarchyId,
    String viewId, HashMap<ElementNode, TreeNode> parents) {
    ...
    for (ElementNode root : rootNodes) {
      ...
      XElementNode parentX = parent == null ? null : (XElementNode) parent
        .getXObject();
      HashMap<XElement, Integer> repCounter = new HashMap<XElement, Integer>();
    ...
    if (nextLevel.size() != 0 && ((currentDepth + 1) < level || level == -1)) {
      addChildren(currentDepth + 1, level, nextLevel.toArray(new ElementNode[0]),
        nodes, hierarchyId, viewId, parents);
    }
  }

  private final List<TreeNode> loadTreeNodes(CubeViewController cvc,
    ...
    if (axisHierarchy != null) {
      ...
      for (ElementNode rootElement : rootNodes) {
        XElementNode xRoot = XElementFactory.createX(rootElement, hierarchyId,
          viewId);
      ...
      int currentDepth = 1;
      try {
        addChildren(currentDepth, level, rootNodes, nodes, hierarchyId, viewId,
          parents);
      } catch (Throwable t) {
        logger.error(t.getMessage());
        t.printStackTrace();
  ...
  public List<XElement> loadRootElements(XHierarchy hierarchy) {
    ...
    // Connection connection = getConnection();
    // Database db =
    // connection.getDatabaseById(hierarchy.getDimension().getDatabase().getId());
  ...
  // ConnectionFactory.getInstance().getConfiguration("localhost",
  // "7777", "admin", "admin");
  ...
  public XWorksheet loadWorksheet(XTemplate template) {
    ...
    // }
    //
    // WSSApplication tensegrityApp =
    // temp.getConnection().getSystemApplication();
    // XApplication xapp = new XApplication(template.getUser(),
    // template.getAccount(),
  ...
  // // XApplication app = new XApplication(user, xAcc,
  // wb.getApplication().getId(), true, false);
  ...
  public XWorksheet loadWorksheet(XReport report, XUser user) {
    // if
    // (!report.getReceiverType().equals(XConstants.TYPE_FOLDER_ELEMENT_SHEET))
    // {
    // return null;
    // }
    // FolderElement el = (FolderElement)
    // XObjectMatcher.getNativeObject(report);
    // WSSWorkbook wb = (WSSWorkbook)
    // ReportFolderChildLoader.getSourceObjectFromElement(
    // user, el);
    ...
    // XWorkbook xwb = new XWorkbook(wb.getId(), wb.getName(), xApp,
    // ws.getName());
    // xwb.setHasChildren(false);
    // XWorksheet xws = new XWorksheet(ws.getId(), ws.getName(), xwb);
    // xws.setHasChildren(false);
  ...
  public XStaticReportFolder createStaticFolder(String name, XObject parent,
    XUser usr) {
  ...
  public XDynamicReportFolder createDynamicFolder(String name, XObject parent,
    XUser usr) {
    // Object o = XObjectMatcher.getNativeObject(parent);
    // AuthUser user = (AuthUser) XObjectMatcher.getNativeObject(usr);
    // FolderService folderService = ServiceProvider.getFolderService(user);
  ...
  public XReport[] addReceivers(XReportFolder parent, XObject[] receivers, XUser usr) {
    ...
    // // XReport xr = new XReport(name, fe.getId(), type, elements.toArray(new
    // XElement[0]));
  ...
  protected void myInitDbConnection(ServletContext globalContext,
  ...
  private final XDirectLinkData parseMultipleViews(String locale, AuthUser authUser,
    String link, XDirectLinkData data) {
    try {
      ...
      while ((index = link.indexOf("[", index + 1)) != -1) {
        ...
        if (neededAccount == null) {
          data.addError(UserSession.trans(locale, "noAccountForView", v.getName(), v
            .getId()));
          continue;
        }
        ...
        // data.addError("The view " + v.getName() + " (" + v.getId() +
        // ") cannot be seen by this user.");
        // continue;
        // }
        ...
        if (!allowed) {
          data.addError(UserSession.trans(locale, "viewCannotBeSeen", v.getName(), v
            .getId()));
          continue;
        }
      ...
      logger.error(t.getMessage());
      data.addError(UserSession.trans(locale, "errorWhileOpeningMultipleViews", t
        .getLocalizedMessage()));
    }
    return data;
  }

  private final XDirectLinkData parseSingleView(String locale, AuthUser authUser,
    String link, XDirectLinkData data) {
    ...
    try {
      ...
      if (v != null) {
        ...
        if (existingView == null) {
          v = CubeViewConverter.createDefaultView(cubeName, cube, accountId,
            authUser, "", null);
          viewService.save(v);
        } else {
          v = existingView;
      ...
      logger.error(t.getMessage());
      data.addError(UserSession.trans(locale, "errorWhileOpeningDirectLinkView",
        view, t.getLocalizedMessage()));
    }
    return data;
  }

  public XDirectLinkData openViewAfterLogin(String locale, String sessionId,
    String link) {
    ...
    if (user == null) {
      ...
      try {
        ...
        data
          .addError("You specified an invalid username/password combination. The user in the direct link could not be authenticated.");
      }
      return data;
    }
  ...
  private static final String decodePass(String key, String user,
    String pass_enc_base64, String viewid, SimpleLogger log) throws Exception {
  ...
  private final PaloSuiteData initPaloSuiteData(SimpleLogger log) throws IOException {
    ...
    if (!file.exists()) {
      throw new FileNotFoundException("Configuration file does not exist here: "
  ...
  private final boolean checkChecksum(String link, String sharedSecret,
    SimpleLogger log) {
    int index;
  ...
  private final List<ConnectionDescriptor> readConnections(String user, String pass,
    PaloSuiteData psd, String viewId, XDirectLinkData data, SimpleLogger log) {
    ConnectionConfiguration cfg = ConnectionFactory.getInstance().getConfiguration(
      psd.host, psd.port, user, pass);
    ...
    try {
      ...
      if (db != null) {
        ...
        if (c != null) {
          ...
          ArrayList<ConnectionDescriptor> connections = new ArrayList<ConnectionDescriptor>();
          ...
          for (int i = 0; i < conDim.getElementCount(); i++) {
            if (counter >= result.length) {
              log
                .debug("Found connection to which this user has no rights. Skipping it (or them).");
              break;
            }
            ...
            ConnectionDescriptor cd = new ConnectionDescriptor(type, name, host,
              port, uuser, ppass, active, useLogin);
            connections.add(cd);
            log.debug("Adding connection descriptor: " + type + ", " + name + ", "
              + host + ", " + port + ", " + user + ", " + pass + ", " + active
              + ", " + useLogin);
          }
          return connections;
        }
  ...
  public XAccount[] loadPaloSuiteAccounts(String sessionId, String link)
    throws SessionExpiredException {
  ...
  public XDirectLinkData openPaloSuiteView(String locale, String link) {
    ...
    try {
      ...
      if (psd.host == null) {
        data
          .addError("No palo configuration database host specified. Can't continue.<br/>");
      }
      if (psd.port == null) {
        data
          .addError("No palo configuration database port specified. Can't continue.<br/>");
      }
      ...
      if (decPass == null
        || (data.getErrors() != null && data.getErrors().length > 0)) {
        return data;
      }
  ...
  class ConnectionDescriptor {
    ConnectionDescriptor(String type, String name, String host, String port,
      String user, String pass, String active, String useLoginCredentials) {
      this.type = type;
      this.name = name;
      this.host = host;
      this.port = port;
      ...
      this.useLoginCredentials = useLoginCredentials != null
        && useLoginCredentials.equals("1");
    }
  ...
  private final void addElement(ArrayList<Element> attributes, Dimension dim,
    String name) {
    Element e = dim.getElementByName(name);
    if (e == null) {
      throw new IllegalArgumentException("Element name " + name
        + " does not exist in attribute dimension.");
    }
    attributes.add(e);
  }

  private final User getUser(AdministrationService admService, AuthUser admin,
    String user, String pass) throws SQLException, OperationFailedException {
    ...
    if (viewApiUser == null) {
      ...
      IUserRoleManagement urAssoc = MapperRegistry.getInstance()
  ...
  private final void attachConnections(AdministrationService admService,
    AuthUser admin, ArrayList<ConnectionDescriptor> connections, String host,
    String port, SimpleLogger log) throws OperationFailedException {
    for (ConnectionDescriptor cd : connections) {
      boolean found = false;
      for (PaloConnection conn : admService.getConnections()) {
        if (cd.host.equals(conn.getHost()) && cd.port.equals(conn.getService())) {
          found = true;
          break;
        }
      }
      if (!found) {
        if (cd.type.equalsIgnoreCase("palo")) {
          if (cd.useLoginCredentials) {
            PaloConnection con = admService.createConnection(cd.name, cd.host,
              cd.port, PaloConnection.TYPE_HTTP);
            admService.save(con);
          } else {
            PaloConnection con = admService.createConnection(cd.name, cd.host,
              cd.port, PaloConnection.TYPE_HTTP);
  ...
  private final AuthUser attachAccounts(String userName, String decodedPass,
    AdministrationService admService, AuthUser admin, User user, String pass,
    ArrayList<ConnectionDescriptor> connections, SimpleLogger log)
    throws SQLException, OperationFailedException, AuthenticationFailedException {
    AuthUser authenticatedUser = null;
    try {
      if (user.getLoginName().equals("admin")) {
        authenticatedUser = ServiceProvider.getAuthenticationService()
          .authenticateAdmin();
      } else {
        authenticatedUser = ServiceProvider.getAuthenticationService().authenticate(
          user.getLoginName(), pass);
      }
      if (!user.getLoginName().equals("admin")) {
    ...
    if (authenticatedUser == null) {
      log.error("Authentication of PaloPivot user failed! Aborting.",
    ...
    for (ConnectionDescriptor cd : connections) {
      ...
      for (Account acc : authenticatedUser.getAccounts()) {
        if (acc.getConnection().getHost() != null
          && acc.getConnection().getHost().equals(cd.host)
          && acc.getConnection().getService() != null
          && acc.getConnection().getService().equals(cd.port)) {
          // if (acc.getConnection() != null &&
          // acc.getConnection().getName().equals(cd.name)) {
          // found = true;
          // break;
          // }
          ...
          log.debug("Account already exists: " + acc.getConnection().getHost()
      ...
      if (!found) {
        ...
        if (con == null) {
          log.warn("No connection found for " + cd.host + ":" + cd.port
            + ". Ignoring.");
          continue;
        }
        Account acc = null;
        if (cd.type.equalsIgnoreCase("palo")) {
          if (cd.useLoginCredentials) {
            log.debug("Creating account (dynamic) " + userName + ", " + decodedPass
              + " for user " + user.getLoginName());
            admService.createAccount(userName, decodedPass, authenticatedUser, con);
          } else {
            log.debug("Creating account (palo) " + cd.user + ", " + cd.pass
              + " for user " + user.getLoginName());
            admService.createAccount(cd.user, cd.pass, authenticatedUser, con);
          }
        }
        // else if (cd.type.equalsIgnoreCase("dynamic")) {
        // log.debug("Creating account (dynamic) " + userName + ", " +
        // decodedPass + " for user " + user.getLoginName());
        // admService.createAccount(userName, decodedPass, authenticatedUser,
        // con);
        // }
        admService.save(acc);
        admService.save(user);
    ...
    log
      .debug("  ==> After account creation, the authenticated user has the following accounts:");
    for (Account a : authenticatedUser.getAccounts()) {
      log.debug("       => " + a.getLoginName() + ", " + a.getConnection().getHost()
  ...
  private final boolean checkPaloSuiteId(View v, String viewId, SimpleLogger log) {
    ...
    int index;
    if (def != null
      && (index = def.indexOf("<property id=\"paloSuiteID\" value=")) != -1) {
  ...
  private final XDirectLinkData findPaloSuiteView(AuthUser authenticatedUser,
    String viewId, SimpleLogger log, XDirectLinkData data,
    ArrayList<ConnectionDescriptor> connections, String locale) {
    ...
    try {
      for (View v : mgmt.listViews()) {
        ...
        if (checkPaloSuiteId(v, viewId, log)) {
          log
            .debug("   => it has the correct paloSuiteId, so go on... List accounts:");
          viewIdFound = v;
          for (Account acc : authenticatedUser.getAccounts()) {
            ...
            for (ConnectionDescriptor desc : connections) {
              if (desc.host.equals(host) && desc.port.equals(service)) {
                found = true;
                break;
              }
            }
            ...
            log
              .debug("       account: " + acc.getLoginName() + ", "
                + acc.getPassword() + " [" + host + ", " + service + ", " + user
                + "]");
            log.debug("       looking for: " + c.getHost() + ", " + c.getService()
              + ", " + v.getAccount().getUser().getLoginName());
    ...
    if (accountToUse == null && bestMatch == null) {
      log
        .debug("No account or no view found. Either the view does not exist, or something's rotten in the state of Denmark.");
    }

    if (accountToUse == null && bestMatch != null) {
      log.debug("No exact account match found, but something close enough: "
        + bestMatch.getLoginName() + ", " + bestMatch.getConnection().getHost()
        + ", " + bestMatch.getConnection().getService());
    ...
    if (viewIdFound != null && accountToUse == null) {
      data.addError(UserSession.trans(locale, "viewCannotBeSeen", viewIdFound
        .getName(), viewIdFound.getId()));
    }
    return data;
  }

  private final XDirectLinkData parsePaloStudioConnectionData(String paloSuiteUser,
    String paloSuitePass, String paloSuiteHost, String paloSuitePort,
    Database configDb, String viewId, SimpleLogger log, XDirectLinkData data,
    String locale) {
    ...
    if (c != null) {
      ...
      for (int i = 0; i < conDim.getElementCount(); i++) {
        ...
        String active = result[counter++].toString();
        String useLogin = result[counter++].toString();
        ConnectionDescriptor cd = new ConnectionDescriptor(type, name, host, port,
          user, pass, active, useLogin);
        connections.add(cd);
        log.debug("Adding connection descriptor: " + type + ", " + name + ", "
          + host + ", " + port + ", " + user + ", " + pass + ", " + active + ", "
          + useLogin);
      ...
      try {
        AuthUser admin = ServiceProvider.getAuthenticationService()
          .authenticateAdmin();
        AdministrationService admService = ServiceProvider
          .getAdministrationService(admin);
        ...
        attachConnections(admService, admin, connections, paloSuiteHost,
          paloSuitePort, log);

        // Step 3: Make sure all accounts exist, add new ones:
        AuthUser authUser = attachAccounts(paloSuiteUser, paloSuitePass, admService,
          admin, viewApiUser, paloSuitePass, connections, log);

        // Step 4: Now find the view with the Id x and open it...
        if (authUser != null) {
          log.debug("AuthUser != null; " + authUser.getLoginName()
            + " => finding Views.");
  ...
  private final XDirectLinkData legacyAuthentication(Connection con, String user,
    String pass, PaloSuiteData psd, String viewId, XDirectLinkData data,
    SimpleLogger log) throws OperationFailedException, SQLException,
    ...
    if (con.getDatabaseCount() > 0) {
      ...
      for (PaloConnection conn : admService.getConnections()) {
        if (psd.host.equals(conn.getHost()) && psd.port.equals(conn.getService())) {
          conToUse = conn;
          break;
        }
      }
      if (conToUse == null) {
        conToUse = admService.createConnection("PaloCon", psd.host, psd.port,
          PaloConnection.TYPE_HTTP);
      ...
      try {
        if (viewApiUser.getLoginName().equals("admin")) {
          ...
          authenticatedUser = ServiceProvider.getAuthenticationService()
            .authenticate(viewApiUser.getLoginName(), pass);
        }
        if (!viewApiUser.getLoginName().equals("admin")) {
  ...
  private final XDirectLinkData readConfigForUser(String user, String pass,
    PaloSuiteData psd, String viewId, XDirectLinkData data, SimpleLogger log,
    String locale) {
    ...
    try {
      ...
      Database db = con.getDatabaseByName("Config");
      if (db != null) {
        return parsePaloStudioConnectionData(user, pass, psd.host, psd.port, db,
          viewId, log, data, locale);
      } else {
        log.error("No configuration database found...", new NullPointerException());
        return null;
      }
  ...
  public XDirectLinkData openViewDirectly(String locale, String link) {
    ...
    try {
      authUser = ServiceProvider.getAuthenticationService().authenticateHash(user,
        pass);
    } catch (AuthenticationFailedException e) {
      logger.error(e.getMessage());
    }
  ...
  public boolean assignSubsetOrDimension(XDynamicReportFolder folder,
    ...
    // FolderService folderService = ServiceProvider.getFolderService(user);
    //		
    // DynamicFolder df = (DynamicFolder)
    // XObjectMatcher.getNativeObject(folder);
    // if (df == null) {
    // return false;
    // }
    //		
  ...
  public XVariableDescriptor getVariables(XReport report, XUser usr) {
    ...
    // // desc.addMapping(createXHierarchy((Hierarchy) o, usr, user),
    // el.getVariableMapping((Hierarchy) o));
    // // } else if (o instanceof Subset2) {
    // // desc.addMapping(createXSubset((Subset2) o, usr, user),
    // el.getVariableMapping((Subset2) o));
    // // }
    // // }
    // return desc;
    return null;
  ...
  private final XWorkbook createWorkbook(XApplication application, String name) {
    // WSSApplication app = (WSSApplication)
    // XObjectMatcher.getNativeObject(application);
    // if (app == null) {
    // return null;
    // }
    // app.select();
    // WSSWorkbook wb = app.addWorkbook(name);
    // XWorkbook xwb = new XWorkbook(wb.getId(), wb.getName(), application,
    // null);
  ...
  private final XWorkbook createWorkbookTemplate(XAccount account, String name) {
    ...
    // if (xapp == null) {
    // xapp = new XApplication(account.getUser(), account, account.getId(),
    // false, true);
  ...
  public boolean applyMapping(XReport input, XObject[] keys, String[] values,
    XUser usr) {
    // AuthUser user = (AuthUser) XObjectMatcher.getNativeObject(usr);
    // FolderService folderService = ServiceProvider.getFolderService(user);
  ...
  // private static final XElement createXElement(Element e, XUser xUser,
  // AuthUser user) {
  // if (e == null) {
  // return null;
  // }
  ...
  // }
  //	
  // private static final XDatabase createXDatabase(Database db, XAccount xacc,
  // XUser xUser) {
  ...
  // }
  //	
  // private static final XDimension createXDimension(Dimension dim, XDatabase
  // xdb, XUser xUser) {
  ...
  // }
  //	
  // private static final XHierarchy createXHierarchy(Hierarchy hier, XDimension
  // xdim, XUser xUser) {
  ...
  // }
  //	
  // private static final XSubset createXSubset(Subset2 sub, XHierarchy hier,
  // XUser xUser) {
  ...
  // }
  //	
  // public static final XHierarchy createXHierarchy(Hierarchy hier, XUser
  // xUser, AuthUser user) {
  // if (hier == null) {
  // return null;
  // }
  ...
  // }
  //	
  // public static final XSubset createXSubset(Subset2 subset, XUser xUser,
  // AuthUser user) {
  // if (subset == null) {
  // return null;
  // }
  ...
  // }
  //	
  // private static final XAccount createXAccount(Connection con, XUser xUser,
  // AuthUser user) {
  // XAccount xacc = null;
