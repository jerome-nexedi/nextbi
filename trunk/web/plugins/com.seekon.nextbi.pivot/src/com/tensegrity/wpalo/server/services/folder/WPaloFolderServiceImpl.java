...
public class WPaloFolderServiceImpl extends BasePaloServiceServlet implements
  ...
  public XStaticFolder loadFolderRoot(String sessionId)
    ...
    if (folderRoot instanceof StaticFolder) {
      XStaticFolder xRoot = new XStaticFolder(folderRoot.getId(), folderRoot
        .getName());
  ...
  public XStaticFolder createFolder(String sessionId, String name,
    XStaticFolder xParent) throws DbOperationFailedException,
    ...
    try {
      ...
      // .getId());
      StaticFolder staticFolder = folderService.createStaticFolder(name, parentNode,
        null);
      // folderService.save(staticFolder.getRoot());
      XStaticFolder xStaticFolder = new XStaticFolder(staticFolder.getId(),
        staticFolder.getName());
      ...
      throw new DbOperationFailedException(userSession.translate(
        "couldNotCreateFolder", name, e.getLocalizedMessage()), e);
    } catch (PaloIOException e) {
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate(
        "couldNotCreateFolder", name, e.getLocalizedMessage()), e);
  ...
  public void deleteFolder(String sessionId, XStaticFolder xFolder)
    ...
    try {
      ...
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
        xFolder.getName(), e.getLocalizedMessage()), e);
    } catch (PaloIOException e) {
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
        xFolder.getName(), e.getLocalizedMessage()), e);
  ...
  private final void deleteFolder(String sessionId, FolderService folderService,
    StaticFolder folder) throws OperationFailedException, SessionExpiredException {
    if (folder != null) {
      deleteContentof(sessionId, folder);
      folderService.delete(folder);
    }
  ...
  private final void delete(String sessionId, View view)
    throws OperationFailedException, SessionExpiredException {
  ...
  public void deleteFolderElement(String sessionId, XFolderElement xFolderElement)
    ...
    try {
      ...
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
        xFolderElement.getName(), e.getLocalizedMessage()), e);
    } catch (PaloIOException e) {
      e.printStackTrace();
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate("couldNotDelete",
  ...
  public XFolderElement createFolderElement(String sessionId, XView xView,
    XStaticFolder xParentFolder, boolean isPublic, boolean isEditable)
    throws DbOperationFailedException, SessionExpiredException {
    // ExplorerTreeNode parentFolder = getFolder(xParentFolder);
    FolderElement folderElement = createFolderElement(sessionId, xView,
      xParentFolder, true, isPublic, isEditable);
    return createXFolderElement(folderElement, xView);
  }

  public XFolderElement[] importViewsAsFolderElements(String sessionId,
    XView[] views, XStaticFolder xParentFolder, boolean isPublic, boolean isEditable)
    throws DbOperationFailedException, SessionExpiredException {
    ...
    for (XView xView : views) {
      ...
      if (xParentFolder != null) {
        FolderElement folderElement = createFolderElement(sessionId, importedXView,
          xParentFolder, true, isPublic, isEditable);
        xFolderElements.add(createXFolderElement(folderElement, importedXView));
  ...
  public XView importView(String sessionId, XView view)
    throws DbOperationFailedException, SessionExpiredException {
    WPaloCubeViewConverter converter = new WPaloCubeViewConverter(
      getLoggedInUser(sessionId));
  ...
  private final XView importViewWith(String sessionId,
    WPaloCubeViewConverter converter, XView xView) throws DbOperationFailedException {
    try {
      ...
      if (userSession != null) {
        throw new DbOperationFailedException(userSession.translate(
          "couldNotImportView", xView.getName(), t.getLocalizedMessage()), t);
      } else {
  ...
  private final FolderElement createFolderElement(String sessionId, XView xView,
    XStaticFolder parentFolder, boolean p, boolean isPublic, boolean isEditable)
    throws DbOperationFailedException, SessionExpiredException {
    ...
    try {
      ...
      fe = folderService.createFolderElement(xView.getName(), parent, null);
  ...
  private final View getView(String sessionId, XView xView)
    throws SessionExpiredException {
    ViewService viewService = ServiceProvider
      .getViewService(getLoggedInUser(sessionId));
  ...
  private final XFolderElement createXFolderElement(FolderElement folderElement,
    XObject xSourceObject) {
    XFolderElement xFolderElement = new XFolderElement(folderElement.getId(),
      folderElement.getName(), xSourceObject);
  ...
  public final void move(String sessionId, XObject[] xObjects,
    ...
    try {
      ...
      UserSession userSession = getUserSession(sessionId);
      throw new DbOperationFailedException(userSession.translate(
        "couldNotMoveFolderElements", ope.getLocalizedMessage()), ope);
    }
  }

  // private final ExplorerTreeNode getTreeNode(String sessionId, XFolderElement
  // xf) throws
  // OperationFailedException, SessionExpiredException {
  // FolderService folderService =
  // ServiceProvider.getFolderService(getLoggedInUser(sessionId));
  ...
  public void renameFolderElement(String sessionId, XFolderElement xFolderElement,
    String newName) throws DbOperationFailedException, SessionExpiredException {
    FolderService folderService = ServiceProvider
      .getFolderService(getLoggedInUser(sessionId));
    // ExplorerTreeNode folder = folderService.getTreeNode(xFolderElement
    // .getId());
