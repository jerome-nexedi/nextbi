...
class TemplateTab extends EditorTab {
  ...
  private final ToolBar createButtonBar() {
    ...
    TextToolItem assignView = new TextToolItem("Add to Folder",
      "icon-assign-to-folder");
    ...
    assignView.addSelectionListener(new SelectionListener<ComponentEvent>() {
      public void componentSelected(ComponentEvent ce) {
        ...
        for (TreeItem it : items) {
          ...
          if (!(obj instanceof XTemplate) && !(obj instanceof XWorkbook)
        ...
        WPaloServiceProvider.getInstance().addReceivers(xrf,
          objs.toArray(new XObject[0]), user, new Callback<XReport[]>() {
            public void onSuccess(XReport[] reports) {
              if (reports != null && reports.length > 0) {
                ReportStructureEditor.view.getTreeLoader().loadChildren(
                  (TreeNode) ti.getModel());
  ...
  class TreeNodeBinder extends TreeBinder<TreeNode> {
    ...
    protected TreeItem createItem(TreeNode model) {
      ...
      // if (model.getXObject() != null && model.getXObject() instanceof
      // XAccount) {
