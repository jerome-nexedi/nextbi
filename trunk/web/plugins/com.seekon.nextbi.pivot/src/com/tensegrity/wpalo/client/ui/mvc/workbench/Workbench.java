...
public class Workbench extends AbstractTopLevelView {
  ...
  public final void checkOpen(IEditor newEditor,
    ...
    if (currentEditor != null && !currentEditor.isDirty()
      && currentEditor instanceof ViewBrowserEditor
      && !(newEditor instanceof ViewBrowserEditor)) {
    ...
    if (currentEditor != null
      ...
      String messageText = messages
        .saveEditorBeforeClosing(currentEditor.getTitle());
    ...
    dlg.addListener(Events.Hide, new Listener<BoxComponentEvent>() {
      public void handleEvent(BoxComponentEvent be) {
        ...
        if (directLink == null) {
          ...
          WPaloServiceProvider.getInstance().openViewAfterLogin(locale,
            user.getSessionId(), directLink, new AsyncCallback<XDirectLinkData>() {
                ...
                if (!data.isAuthenticated()) {
                  ...
                  for (int i = 1; i < xViews.length; i++) {
                    ((WorkbenchController) getController())
                      .getViewBrowserController().addViewToLoad(xViews[i]);
                  }
