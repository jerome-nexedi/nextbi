 ...
 *          ArndHouben Exp $
 **/
public class WPaloControllerServiceImpl extends BasePaloServiceServlet implements
  ...
  public final XUser login(String login, String password, String locale)
    ...
    // try to authenticate:
    try {
      AuthUser usr = ServiceProvider.getAuthenticationService().authenticate(login,
        password);
      // AuthUser usr =
      // ServiceProvider.getAuthenticationService().authenticate("admin",
      // "admin");
      registerListeners();
  ...
  public final XUser loginHash(String login, String password, String locale)
    ...
    try {
      ...
      AuthUser usr = ServiceProvider.getAuthenticationService().authenticateHash(
        login, password);
  ...
  private final DbConnection createConnection() {
    try {
      String useMySql = ResourceBundle.getBundle("deploy", Locale.ITALIAN)
        .getString("use.mysql");
      if (useMySql != null) {
        USE_MYSQL = Boolean.valueOf(ResourceBundle.getBundle("deploy",
          Locale.ITALIAN).getString("use.mysql"));
      }
    } catch (Exception e) {
