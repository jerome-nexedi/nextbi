...
/**
 * <code>UserChildLoader</code> TODO DOCUMENT ME
 * 
 * @version $Id: AdminChildLoader.java,v 1.9 2009/12/17 16:14:20 PhilippBouillon
 *          Exp $
 **/
public class AdminChildLoader implements ChildLoader {
  ...
  private final XObject[] getUsers(AdministrationService adminSrv) {
    ...
    for (int i = 0; i < ret.length; ++i) {
      ...
      // xUser.setAccounts(WPaloAdminCache.getAccounts(accounts));
      // //XConverter.createAccounts(accounts, xUser));
      // // groups:
      // List<Group> groups = adminSrv.getGroups(usr);
      // xUser.setGroups(WPaloAdminCache.getGroups(groups));
      // //XConverter.createGroups(groups));
      // // roles:
      // List<Role> roles = adminSrv.getRoles(usr);
      // xUser.setRoles(WPaloAdminCache.getRoles(roles));
      // //XConverter.createRoles(roles));
      // ret[i] = xUser;
  ...
  private final XObject[] getGroups(List<Group> groups) {
    ...
    for (int i = 0; i < ret.length; ++i) {
      ...
      // ret[i] = (XGroup) WPaloAdminCache.getXObject(grp);
      // //XConverter.createGroup(grp);
  ...
  private final XObject[] getRoles(List<Role> roles) {
    ...
    for (int i = 0; i < ret.length; ++i) {
      ...
      // ret[i] = (XRole) WPaloAdminCache.getXObject(role);
      // //XConverter.createRole(role);
