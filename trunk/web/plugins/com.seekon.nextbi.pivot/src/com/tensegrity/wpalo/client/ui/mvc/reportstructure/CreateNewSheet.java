...
public class CreateNewSheet extends SelectionListener<ComponentEvent> {
  ...
  public void componentSelected(ComponentEvent ce) {
    ...
    RequestNameDialog rnd = new RequestNameDialog(title, "Name",
      new ResultListener<String>() {
        public void requestCancelled() {
        }

        public void requestFinished(String result) {
          WPaloServiceProvider.getInstance().createWorkbook(parent, result,
            new Callback<XWorkbook>() {
              public void onSuccess(XWorkbook workbook) {
                if (workbook == null) {
                  System.err.println("Returned workbook == null");
                  return;
                }
                Dispatcher.get().dispatch(WPaloEvent.UPDATE_WORKBOOKS, parent);
              }
            });
        }
      });
