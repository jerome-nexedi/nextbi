...
public class JpaLockingStrategy implements LockingStrategy {
  ...
  public boolean acquire() {
    Lock lock;
    try {
      lock = entityManager.find(Lock.class, applicationId,
        LockModeType.PESSIMISTIC_WRITE);
    } catch (PersistenceException e) {
      logger.debug("{} failed querying for {} lock.", new Object[] { uniqueId,
        applicationId, e });
      return false;
    }
    ...
    if (lock != null) {
      ...
      if (lock.getUniqueId() == null) {
        ...
        logger.debug("{} trying to acquire expired {} lock.", uniqueId,
          applicationId);
        result = acquire(entityManager, lock);
      }
    } else {
      ...
      logger
        .debug("Creating {} lock initially held by {}.", applicationId, uniqueId);
  ...
  private boolean acquire(final EntityManager em, Lock lock) {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("{} could not obtain {} lock.", new Object[] { uniqueId,
          applicationId, e });
      } else {
