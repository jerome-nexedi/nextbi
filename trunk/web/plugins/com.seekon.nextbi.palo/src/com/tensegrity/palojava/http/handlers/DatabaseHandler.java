...
public class DatabaseHandler extends HttpHandler {
  ...
  public final void rename(DatabaseInfo database, String newName) throws IOException {
    StringBuffer query = new StringBuffer();
  ...
  // PRIVATE METHODS
  //
  private final CubeInfo[] getCubesInternal(DatabaseInfo database, boolean getAll,
  ...
  private final DimensionInfo[] getDimensionsInternal(DatabaseInfo database,
    ...
    if (database.getStatus() == DatabaseInfo.STATUS_UNLOADED) {
      System.err.println("WARNING: database '" + database.getName()
        + "' not loaded!!");
      //			if(load(database))
    ...
    DimensionInfoBuilder dimensionBuilder = builderReg.getDimensionBuilder();
