...
public abstract class ElementLoader extends PaloInfoLoader {
   ...
   */
  public final ElementInfo create(String name, int type, ElementInfo[] children,
    double[] weights) {
    ElementInfo elInfo = paloConnection.addElement(hierarchy, name, type, children,
      weights);
    loaded(elInfo);
    return elInfo;
  }
  ...
  public final ElementInfo[] createBulk(String[] names, int type,
    ...
    if (paloConnection.addElements(hierarchy.getDimension(), names, type, children,
      weights)) {
      ElementInfo[] allElements = paloConnection.getElements(hierarchy
        .getDimension());
  ...
  public final ElementInfo[] createBulk(String[] names, int[] types,
    ElementInfo[][] children, double[][] weights) {
    List<ElementInfo> newElements = new ArrayList<ElementInfo>(names.length);
    if (paloConnection.addElements(hierarchy.getDimension(), names, types, children,
      weights)) {
      ElementInfo[] allElements = paloConnection.getElements(hierarchy
        .getDimension());
      Set<String> reqNames = new HashSet<String>(Arrays.asList(names));
  ...
  public final ElementInfo[] replaceBulk(ElementInfo[] elements, int type,
    ElementInfo[][] children, Double[][] weights) {
    if (paloConnection.replaceBulk(hierarchy.getDimension(), elements, type,
      children, weights)) {
