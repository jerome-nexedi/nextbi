/*
// $Id: //open/mondrian/testsrc/main/mondrian/xmla/XmlaBasicTest.java#69 $
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/xmla/XmlaBasicTest.java#69 $
 */
public class XmlaBasicTest extends XmlaBaseTestCase {
  ...
  /**
   * Tests the output of the MDSCHEMA_FUNCTIONS call in JDK 1.5 or later. In JDK
   * 1.4, does nothing and trivially succeeds. See
   * {@link #testMDFunctionsJdk14()}.
   * 
   * @throws Exception
   *           on error
   */
   ...
   * different because metadata such as function name and description is encoded
   * using Java annotations, and hence is not available until JDK 1.5. In JDK
   * 1.5 and later, does nothing and trivially succeeds. See
   * {@link #testMDFunctions()}.
   * 
   * @throws Exception
   *           on error
   */
  ...
  protected String filter(String testCaseName, String filename, String content) {
    if (testCaseName.startsWith("testDrillThrough") && filename.equals("response")) {
      ...
      case ORACLE:
        content = Util.replace(content, " type=\"xsd:double\"",
          " type=\"xsd:decimal\"");
        content = Util.replace(content, " type=\"xsd:integer\"",
          " type=\"xsd:decimal\"");
        break;
      case POSTGRESQL:
      ...
      case TERADATA:
        ...
        content = Util.replace(content, " sql:field=\"Week\" type=\"xsd:decimal\"",
          " sql:field=\"Week\" type=\"xsd:integer\"");
        content = Util.replace(content, " sql:field=\"Day\" type=\"xsd:decimal\"",
  ...
  public void testExecuteCrossjoin() throws Exception {
    ...
    String requestType = "EXECUTE";
    String query = "SELECT CrossJoin({[Product].[All Products].children}, "
    ...
    String request = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
      ...
      + "</soapenv:Body>\n" + "</soapenv:Envelope>";
    Properties props = getDefaultRequestProperties(requestType);
    doTestInline(requestType, request, "response", props, TestContext.instance());
  }

  /**
   * This test returns the same result as testExecuteCrossjoin above except that
   * the Role used disables accessing [Customers].[All Customers].[Mexico].
   */
  ...
  private void doTestExecuteContent(XmlaConstants.Content content) throws Exception {
    String requestType = "EXECUTE";
    Properties props = getDefaultRequestProperties(requestType);
    ...
    expectedDoc = (responseDoc != null) ? XmlaSupport.transformSoapXmla(responseDoc,
      new String[][] { { "content", content.name() } }, ns) : null;
