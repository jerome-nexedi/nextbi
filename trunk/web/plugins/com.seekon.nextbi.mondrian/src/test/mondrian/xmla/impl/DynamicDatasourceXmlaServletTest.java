 ...
 * 
 * @author Thiyagu, Ajit
 * @version $Id: //open/mondrian/testsrc/main/mondrian/xmla/impl/
 *          DynamicDatasourceXmlaServletTest.java#14 $
 * @since Mar 30, 2007
 */
public class DynamicDatasourceXmlaServletTest extends TestCase {
  ...
  private static final String CATALOG_0_DEFINITION = "<Catalog name='"
    + CATALOG_0_NAME + "'>"
    + "<Definition>Provider=mondrian;Jdbc=jdbc:odbc:MondrianFoodMart0;"
    + "JdbcDrivers=sun.jdbc.odbc.JdbcOdbcDriver" + "</Definition>" + "</Catalog>";

  private static final String CATALOG_0_UPDATED_DEFINITION = "<Catalog name='"
    + CATALOG_0_NAME + "'>"
    + "<Definition>Provider=mondrian;Jdbc=jdbc:odbc:MondrianFoodMart0.0;"
    + "JdbcDrivers=sun.jdbc.odbc.JdbcOdbcDriver" + "</Definition>" + "</Catalog>";

  private static final String CATALOG_1_DEFINITION = "<Catalog name='"
    + CATALOG_1_NAME + "'>"
    + "<Definition>Provider=mondrian;Jdbc=jdbc:odbc:MondrianFoodMart1;"
    + "JdbcDrivers=sun.jdbc.odbc.JdbcOdbcDriver" + "</Definition>" + "</Catalog>";

  private static final String CATALOG_2_DEFINITION = "<Catalog name='"
    + CATALOG_2_NAME + "'>"
  ...
  public void testFlushObsoleteCatalogsForNewCatalog() throws Exception {
    DataSourcesConfig.DataSources newDataSources = getDataSources(
      CATALOG_0_DEFINITION, CATALOG_1_DEFINITION);
    final MockDynamicContentFinder finder = new MockDynamicContentFinder("inline:"
      + getDataSourceContent(CATALOG_0_DEFINITION));
  ...
  public void testMergeDataSourcesForAlteringCatalogAcrossDataSources()
    throws Exception {
    DataSourcesConfig.DataSources newDataSources = getDataSources(Olap4jUtil.mapOf(
      DATASOURCE_1_NAME, new String[] { CATALOG_0_UPDATED_DEFINITION,
        CATALOG_1_DEFINITION }, DATASOURCE_2_NAME,
      new String[] { CATALOG_2_DEFINITION }));
    final MockDynamicContentFinder finder = new MockDynamicContentFinder("inline:"
      + getDataSourceContent(CATALOG_0_DEFINITION));
    finder.flushObsoleteCatalogs(newDataSources);
    assertTrue(finder.flushCatalogList().contains(CATALOG_0_NAME));
  ...
  public void testAreCatalogsEqual() throws Exception {
    DataSourcesConfig.DataSources newDataSources = getDataSources(
      CATALOG_0_DEFINITION, CATALOG_0_UPDATED_DEFINITION, CATALOG_1_DEFINITION,
    ...
    assertFalse(DynamicContentFinder.areCatalogsEqual(datasource, catalog0,
      datasource, catalog0Updated));
    assertTrue(DynamicContentFinder.areCatalogsEqual(datasource, catalog0,
      datasource, catalog0));
    assertFalse(DynamicContentFinder.areCatalogsEqual(datasource, catalog1,
      datasource, catalog2));
  }

  private static DataSourcesConfig.DataSources getDataSources(String... catalogs)
    throws XOMException {
  ...
  private static String getDataSourceContent(String... catalogs) {
    return getDataSourceString(Olap4jUtil.mapOf(DATASOURCE_1_NAME, catalogs));
  }

  private static DataSourcesConfig.DataSources getDataSources(
    Map<String, String[]> dsCatalog) throws XOMException {
  ...
  private static String getDataSourceString(Map<String, String[]> dsCatalog) {
    ...
    for (Map.Entry<String, String[]> entry : dsCatalog.entrySet()) {
      ...
      ds.append("       <DataSourceDescription>" + "DATASOURCE_DESCRIPTION"
        + "</DataSourceDescription>");
      ds.append("       <URL>http://localhost:8080/mondrian/xmla</URL>");
      ds.append("       <DataSourceInfo>Provider=mondrian;").append(
        "Jdbc=jdbc:oracle:thin:foodmart/foodmart@").append(
        "//marmalade.hydromatic.net:1521/XE;").append(
        "JdbcUser=foodmart;JdbcPassword=foodmart;").append(
        "JdbcDrivers=oracle.jdbc.OracleDriver;").append(
        "Catalog=/WEB-INF/queries/FoodMart.xml").append("</DataSourceInfo>");
      ds.append("       <ProviderName>Mondrian</ProviderName>");
      ds.append("       <ProviderType>MDP</ProviderType>");
      ds.append("       <AuthenticationMode>").append("Unauthenticated").append(
        "</AuthenticationMode>");
      ds.append("       <Catalogs>");
  ...
  public void testReloadDataSources() throws Exception {
    DataSourcesConfig.DataSources ds1 = getDataSources(CATALOG_0_DEFINITION,
      CATALOG_1_DEFINITION);
    DataSourcesConfig.DataSources ds2 = getDataSources(CATALOG_1_DEFINITION,
      CATALOG_2_DEFINITION);

    File dsFile = File.createTempFile(getClass().getName() + "-datasources", ".xml");
    dsFile.deleteOnExit();
    ...
    final MockDynamicContentFinder finder = new MockDynamicContentFinder(dsFile
      .toURL().toString());
    ...
    assertTrue(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_1_NAME));
    assertTrue(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_2_NAME));
    assertFalse(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_0_NAME));
    ...
    assertTrue(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_0_NAME));
    assertTrue(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_1_NAME));
    assertFalse(finder.containsCatalog(DATASOURCE_1_NAME, CATALOG_2_NAME));
    finder.shutdown();
  }
  ...
  public void testAutoReloadDataSources() throws Exception {
    ...
    Thread
      .sleep(MondrianProperties.instance().XmlaSchemaRefreshInterval.get() + 1000);
  ...
  private static class MockDynamicContentFinder extends DynamicContentFinder {
    ...
    public synchronized DataSourcesConfig.Catalog locateCatalog(
      String datasourceName, String catalogName) {
