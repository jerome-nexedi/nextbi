...
public class XmlaTest extends TestCase {
  ...
  protected void runTest() throws Exception {
    ...
    Element requestElem = XmlaUtil.text2Element(XmlaTestContext.xmlFromTemplate(
    ...
    String actualResponse = TestContext.instance().upgradeActual(
  ...
  private static Element executeRequest(Element requestElem) {
    ...
    MondrianServer server = MondrianServer.createWithRepository(
      new StringRepositoryContentFinder(context.getDataSourcesString()),
      XmlaTestContext.CATALOG_LOCATOR);
    XmlaHandler handler = new XmlaHandler((XmlaHandler.ConnectionFactory) server,
      "xmla");
  ...
  public static class OtherTest extends TestCase {
    ...
    public void testAccept() {
      // simple
      assertEquals(Enumeration.ResponseMimeType.SOAP, XmlaUtil
        .chooseResponseMimeType("application/xml"));

      // deal with ",q=<n>" quality codes by ignoring them
      assertEquals(Enumeration.ResponseMimeType.SOAP, XmlaUtil
        .chooseResponseMimeType("text/html,application/xhtml+xml,"
      ...
      assertEquals(Enumeration.ResponseMimeType.JSON, XmlaUtil
        .chooseResponseMimeType("text/html;q=0.9," + "application/xhtml+xml;q=0.9,"
          + "application/json;q=0.9," + "application/xml;q=0.9," + "*/*;q=0.8"));

      // allow application/soap+xml as synonym for application/xml
      assertEquals(Enumeration.ResponseMimeType.SOAP, XmlaUtil
        .chooseResponseMimeType("text/html,application/soap+xml"));
