/*
// $Id: //open/mondrian/testsrc/main/mondrian/olap/fun/FunctionTest.java#185 $
 ...
 * @author gjohnson
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/fun/FunctionTest.java#185
 *          $
 */
public class FunctionTest extends FoodMartTestCase {
  ...
  private static final String months = "[Time].[1997].[Q1].[1]\n"
    + "[Time].[1997].[Q1].[2]\n" + "[Time].[1997].[Q1].[3]\n"
    + "[Time].[1997].[Q2].[4]\n" + "[Time].[1997].[Q2].[5]\n"
    + "[Time].[1997].[Q2].[6]\n" + "[Time].[1997].[Q3].[7]\n"
    + "[Time].[1997].[Q3].[8]\n" + "[Time].[1997].[Q3].[9]\n"
  ...
  private static final String quarters = "[Time].[1997].[Q1]\n"
    + "[Time].[1997].[Q2]\n" + "[Time].[1997].[Q3]\n" + "[Time].[1997].[Q4]";

  private static final String year1997 = "[Time].[1997]";

  private static final String hierarchized1997 = year1997 + "\n"
    + "[Time].[1997].[Q1]\n" + "[Time].[1997].[Q1].[1]\n"
    + "[Time].[1997].[Q1].[2]\n" + "[Time].[1997].[Q1].[3]\n"
    + "[Time].[1997].[Q2]\n" + "[Time].[1997].[Q2].[4]\n"
    + "[Time].[1997].[Q2].[5]\n" + "[Time].[1997].[Q2].[6]\n"
  ...
  private static final String TimeWeekly = MondrianProperties.instance().SsasCompatibleNaming
    .get() ? "[Time].[Weekly]" : "[Time.Weekly]";
  ...
  public void testParallelPeriodMinValue() {
    executeQuery("with " + "member [measures].[foo] as "
      + "'([Measures].[unit sales],"
      + "ParallelPeriod([Time].[Quarter], -2147483648))' " + "select "
      + "[measures].[foo] on columns, " + "[time].[1997].children on rows "
      + "from [sales]");
  }
  ...
  public void testLagMinValue() {
    executeQuery("with " + "member [measures].[foo] as "
      ...
      + "[time].[1997].children on rows " + "from [sales]");
  }
  ...
  public void testParallelPeriodWithSlicer() {
    assertQueryReturns(
      "With "
        ...
        + "From [Sales] " + "Where ([Product].[*FILTER_MEMBER])", "Axis #0:\n"
  ...
  public void testNullMember() {
    ...
    // MSAS succeeds too
    assertExprReturns("[Gender].[All Gender].Parent.Children.Count", "0");
    ...
    if (isDefaultNullMemberRepresentation()) {
      ...
      // MSAS returns "" here.
      assertExprReturns("[Gender].[All Gender].Parent.Name", "#null");
    }
  }

  /**
   * Tests use of NULL literal to generate a null cell value. Testcase is from
   * bug 1440344.
   */
  ...
  public void testLevelsTooSmall() {
    assertExprThrows("[Time].[Time].Levels(-1).Name", "Index '-1' out of bounds");
  }

  public void testLevelsTooLarge() {
    assertExprThrows("[Time].[Time].Levels(8).Name", "Index '8' out of bounds");
  }

  public void testHierarchyLevelsString() {
    assertExprReturns("[Time].[Time].Levels(\"Year\").UniqueName", "[Time].[Year]");
  }

  public void testHierarchyLevelsStringFail() {
    assertExprThrows("[Time].[Time].Levels(\"nonexistent\").UniqueName",
  ...
  public void testIsEmptyWithAggregate() {
    assertQueryReturns(
      "WITH MEMBER [gender].[foo] AS 'isEmpty(Aggregate({[Gender].m}))' "
        + "SELECT {Gender.foo} on 0 from sales", "Axis #0:\n" + "{}\n"
        + "Axis #1:\n" + "{[Gender].[foo]}\n" + "Row #0: false\n");
  }
  ...
  public void testIsEmpty() {
    ...
    assertBooleanExprReturns("[Gender].CurrentMember.Parent.NextMember IS NULL",
      true);
    ...
    assertBooleanExprReturns(
      "IsEmpty(([Time].currentMember.Parent, [Measures].[Unit Sales]))", false);
    assertBooleanExprReturns(
      "IsEmpty(([Time].currentMember, [Measures].[Unit Sales]))", false);
    ...
    assertQueryReturns("WITH SET [empty set] AS '{}'\n"
  ...
  public void testValidMeasureDepends() {
    ...
    getTestContext().assertExprDependsOn("ValidMeasure([Measures].[Unit Sales])",
      s12);
    ...
    getTestContext().assertExprDependsOn(
      "ValidMeasure(([Measures].[Unit Sales], "
        + "[Time].[Time].CurrentMember.Parent))", s1);
  }
  ...
  public void testAncestorNumeric() {
    ...
    final TestContext testContextRagged = getTestContext()
      .withCube("[Sales Ragged]");
    member = testContextRagged
      .executeSingletonAxis("Ancestor([Store].[All Stores].[Vatican], 1)");
    Assert.assertEquals("All Stores", member.getName());

    member = testContextRagged
      .executeSingletonAxis("Ancestor([Store].[USA].[Washington], 1)");
    Assert.assertEquals("USA", member.getName());

    // complicated way to say "1".
    member = testContextRagged
      .executeSingletonAxis("Ancestor([Store].[USA].[Washington], 7 * 6 - 41)");
    Assert.assertEquals("USA", member.getName());

    member = testContextRagged
      .executeSingletonAxis("Ancestor([Store].[All Stores].[Vatican], 2)");
    Assert.assertNull("Ancestor at 2 must be null", member);

    member = testContextRagged
      .executeSingletonAxis("Ancestor([Store].[All Stores].[Vatican], -5)");
  ...
  public void testAncestorWithHiddenParent() {
    ...
    Member member = testContext
      .executeSingletonAxis("Ancestor([Store].[All Stores].[Israel].[Haifa], "
        + "[Store].[Store Country])");
  ...
  public void testAncestorDepends() {
    getTestContext().assertExprDependsOn(
      "Ancestor([Store].CurrentMember, [Store].[Store Country]).Name", "{[Store]}");
  ...
  public void testOrdinal() {
    ...
    cell = testContext
      .executeExprRaw("[Store].[All Stores].[USA].[Washington].ordinal");
    assertEquals("Washington is at level 3.", 3, ((Number) cell.getValue())
      .intValue());
  }
  ...
  public void testClosingPeriodLevel() {
    getTestContext().assertMemberExprDependsOn("ClosingPeriod([Time].[Year])",
      "{[Time]}");
    getTestContext().assertMemberExprDependsOn(
      "([Measures].[Unit Sales], ClosingPeriod([Time].[Month]))", "{[Time]}");
    ...
    assertQueryReturns("with member [Measures].[Closing Unit Sales] as "
  ...
  public void testClosingPeriod() {
    getTestContext().assertMemberExprDependsOn(
      "ClosingPeriod([Time].[Month], [Time].[Time].CurrentMember)", "{[Time]}");
    ...
    getTestContext().assertExprDependsOn(
      "(([Measures].[Store Sales],"
        ...
        + "([Measures].[Store Cost],"
        + " ClosingPeriod([Time].[Month], [Time].[Time].CurrentMember)))", s1);
    ...
    assertAxisReturns("ClosingPeriod([Time].[Year], [Time].[1997])", "[Time].[1997]");
    ...
    assertAxisReturns("ClosingPeriod([Time].[Month])", "[Time].[1997].[Q4].[12]");
  ...
  public void testCousinOverrun() {
    Member member = executeSingletonAxis("Cousin([Customers].[USA].[CA].[San Jose],"
      + " [Customers].[USA].[OR])");
  ...
  public void testCousinThreeDown() {
    Member member = executeSingletonAxis("Cousin([Customers].[USA].[CA].[Berkeley].[Barbara Combs],"
      + " [Customers].[Mexico])");
    ...
    // of the 1st child (DF)
    // of Mexico
    Assert.assertEquals("[Customers].[Mexico].[DF].[Tixapan].[Annmarie Hill]",
  ...
  public void testCousinWrongHierarchy() {
    assertAxisThrows("Cousin([Time].[1997], [Gender].[M])", MondrianResource
      .instance().CousinHierarchyMismatch.str("[Time].[1997]", "[Gender].[M]"));
  }

  public void testParent() {
    getTestContext().assertMemberExprDependsOn("[Gender].Parent", "{[Gender]}");
    getTestContext().assertMemberExprDependsOn("[Gender].[M].Parent", "{}");
    assertAxisReturns("{[Store].[USA].[CA].Parent}", "[Store].[USA]");
  ...
  public void testParentPC() {
    ...
    testContext.assertAxisReturns("[Employees].[Sheri Nowmer].Parent",
      "[Employees].[All Employees]");
    testContext.assertAxisReturns(
      "[Employees].[Sheri Nowmer].[Derrick Whelply].Parent",
      "[Employees].[Sheri Nowmer]");
    testContext.assertAxisReturns("[Employees].Members.Item(3)",
      "[Employees].[Sheri Nowmer].[Derrick Whelply].[Beverly Baker]");
    testContext.assertAxisReturns("[Employees].Members.Item(3).Parent",
      "[Employees].[Sheri Nowmer].[Derrick Whelply]");
    testContext.assertAxisReturns("[Employees].AllMembers.Item(3).Parent",
      "[Employees].[Sheri Nowmer].[Derrick Whelply]");
  ...
  public void testMembers() {
    // <Level>.members
    assertAxisReturns("{[Customers].[Country].Members}", "[Customers].[Canada]\n"
      + "[Customers].[Mexico]\n" + "[Customers].[USA]");

    // <Level>.members applied to 'all' level
    assertAxisReturns("{[Customers].[(All)].Members}", "[Customers].[All Customers]");
    ...
    assertAxisReturns("{[Measures].Members}", "[Measures].[Unit Sales]\n"
      + "[Measures].[Store Cost]\n" + "[Measures].[Store Sales]\n"
      + "[Measures].[Sales Count]\n" + "[Measures].[Customer Count]\n"
      + "[Measures].[Promotion Sales]");
    ...
    default:
      assertQueryReturns(
        "with member [Measures].[Xxx] AS ' [Measures].[Unit Sales] '"
          + "select {[Measures].members} on columns from [Sales]", "Axis #0:\n"
          + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
          + "{[Measures].[Store Cost]}\n" + "{[Measures].[Store Sales]}\n"
          + "{[Measures].[Sales Count]}\n" + "{[Measures].[Customer Count]}\n"
          + "{[Measures].[Promotion Sales]}\n" + "Row #0: 266,773\n"
  ...
  public void testHierarchyMembers() {
    assertAxisReturns("Head({[Time.Weekly].Members}, 10)",
    ...
    assertAxisReturns("Tail({[Time.Weekly].Members}, 5)",
  ...
  public void testAllMembers() {
    // <Level>.allmembers
    assertAxisReturns("{[Customers].[Country].allmembers}", "[Customers].[Canada]\n"
      + "[Customers].[Mexico]\n" + "[Customers].[USA]");
    ...
    assertAxisReturns("{[Measures].allmembers}", "[Measures].[Unit Sales]\n"
      + "[Measures].[Store Cost]\n" + "[Measures].[Store Sales]\n"
      + "[Measures].[Sales Count]\n" + "[Measures].[Customer Count]\n"
      + "[Measures].[Promotion Sales]\n" + "[Measures].[Profit]\n"
      + "[Measures].[Profit Growth]\n" + "[Measures].[Profit last Period]");
    ...
    default:
      assertQueryReturns(
        "with member [Measures].[Xxx] AS ' [Measures].[Unit Sales] '"
          + "select {[Measures].allmembers} on columns from [Sales]", "Axis #0:\n"
          + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
          + "{[Measures].[Store Cost]}\n" + "{[Measures].[Store Sales]}\n"
          + "{[Measures].[Sales Count]}\n" + "{[Measures].[Customer Count]}\n"
          + "{[Measures].[Promotion Sales]}\n" + "{[Measures].[Profit]}\n"
    ...
    default:
      assertQueryReturns(
        "WITH MEMBER [Measures].[Unit to Sales ratio] as\n"
          ...
          + "non empty({[Store].[Store State].Members}) ON ROWS " + "FROM Sales "
  ...
  public void testStripCalculatedMembers() {
    ...
    getTestContext().assertSetExprDependsOn(
      "StripCalculatedMembers([Customers].CurrentMember.Children)", "{[Customers]}");

    // ----------------------------------------------------
    // Calc members in dimension based on level stripped
    // Actual members in measures left alone
    // ----------------------------------------------------
    assertQueryReturns("WITH MEMBER [Store].[USA].[CA plus OR] AS "
      + "'AGGREGATE({[Store].[USA].[CA], [Store].[USA].[OR]})' "
      + "SELECT StripCalculatedMembers({[Measures].[Unit Sales], "
      + "[Measures].[Store Sales]}) ON COLUMNS," + "StripCalculatedMembers("
      + "AddCalculatedMembers([Store].[USA].Children)) ON ROWS " + "FROM Sales "
      + "WHERE ([1997].[Q1])", "Axis #0:\n" + "{[Time].[1997].[Q1]}\n"
      + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
      + "{[Measures].[Store Sales]}\n" + "Axis #2:\n" + "{[Store].[USA].[CA]}\n"
      + "{[Store].[USA].[OR]}\n" + "{[Store].[USA].[WA]}\n" + "Row #0: 16,890\n"
  ...
  public void testCurrentMemberDepends() {
    getTestContext().assertMemberExprDependsOn("[Gender].CurrentMember",
      "{[Gender]}");
    ...
    // expression
    getTestContext().assertMemberExprDependsOn("[Gender].[M].Dimension",
      "{[Gender]}");
    ...
    getTestContext().assertExprDependsOn("[Customers]", TestContext.allHiers());
  }
  ...
  public void testCurrentMemberMultiHierarchy() {
    final String hierarchyName = MondrianProperties.instance().SsasCompatibleNaming
      .get() ? "Weekly" : "Time.Weekly";
    ...
    result = executeQuery(queryString1 + " {[Time.Weekly].[1997]} ON ROWS "
      + queryString2);
    Assert.assertEquals("-12,885", result.getCell(coords).getFormattedValue());
  }
  ...
  public void testDefaultMember() {
    ...
    Result result = executeQuery("select {[Time].[Time].DefaultMember} on columns\n"
      + "from Sales");
    Assert.assertEquals("1997", result.getAxes()[0].getPositions().get(0).get(0)
      .getName());
    ...
    result = executeQuery("select {[Time.Weekly].DefaultMember} on columns\n"
      + "from Sales");
    Assert.assertEquals(
      MondrianProperties.instance().SsasCompatibleNaming.get() ? "All Weeklys"
        : "All Time.Weeklys", result.getAxes()[0].getPositions().get(0).get(0)
        .getName());

    final String memberUname = MondrianProperties.instance().SsasCompatibleNaming
      .get() ? "[Time2].[Weekly].[1997].[23]" : "[Time2.Weekly].[1997].[23]";
    ...
    // member.
    result = testContext
      .executeQuery("select {[Time2.Weekly].DefaultMember} on columns\n"
        + "from Sales");
    Assert.assertEquals("23", result.getAxes()[0].getPositions().get(0).get(0)
      .getName());
  }
  ...
  /**
   * When evaluating a calculated member, MSOLAP regards that calculated member
   * as the current member of that dimension, so it cycles in this case. But I
   * disagree; it is the previous current member, before the calculated member
   * was expanded.
   */
  ...
  public void testNamedSetCurrentOrdinalWithNonNamedSetFails() {
    ...
    // fails
    assertQueryThrows("with set [Time Members] as [Time].Members\n"
    ...
    assertQueryThrows("with member [Measures].[Foo] as\n"
  ...
  public void testDrilldownLevelTop() {
    ...
    // <set>, <n>
    assertAxisReturns("DrilldownLevelTop({[Store].[USA]}, 2)", "[Store].[USA]\n"
      + "[Store].[USA].[WA]\n" + "[Store].[USA].[CA]");
    ...
    // <n> negative
    assertAxisReturns("DrilldownLevelTop({[Store].[USA]}, 2 - 3)", "[Store].[USA]");

    // <n> zero
    assertAxisReturns("DrilldownLevelTop({[Store].[USA]}, 2 - 2)", "[Store].[USA]");

    // <n> null
    assertAxisReturns("DrilldownLevelTop({[Store].[USA]}, null)", "[Store].[USA]");

    // mixed bag, no level, all expanded
    assertAxisReturns("DrilldownLevelTop({[Store].[USA], "
      + "[Store].[USA].[CA].[San Francisco], " + "[Store].[All Stores], "
      + "[Store].[Canada].[BC]}, " + "2)", "[Store].[USA]\n"
      + "[Store].[USA].[WA]\n" + "[Store].[USA].[CA]\n"
      + "[Store].[USA].[CA].[San Francisco]\n"
    ...
    assertAxisReturns("DrilldownLevelTop({[Store].[USA], "
      + "[Store].[USA].[CA].[San Francisco], " + "[Store].[All Stores], "
      + "[Store].[Canada].[BC]}, 2, [Store].[Store City])", "[Store].[USA]\n"
      + "[Store].[USA].[CA].[San Francisco]\n"
      + "[Store].[USA].[CA].[San Francisco].[Store 14]\n" + "[Store].[All Stores]\n"
      + "[Store].[Canada].[BC]");
  ...
  public void testDrilldownMemberEmptyExpr() {
    ...
    // reverse expression
    assertAxisReturns("DrilldownLevelTop("
      + "{[Store].[USA]}, 2, , - [Measures].[Unit Sales])", "[Store].[USA]\n"
      + "[Store].[USA].[OR]\n" + "[Store].[USA].[CA]");
  }
  ...
  public void testBasic5() {
    Result result = executeQuery("select{ [Product].[All Products].[Drink].Parent} on columns "
      + "from Sales");
    assertEquals("All Products", result.getAxes()[0].getPositions().get(0).get(0)
      .getName());
  }

  public void testFirstInLevel5() {
    Result result = executeQuery("select {[Time].[1997].[Q2].[4].Parent} on columns,"
      + "{[Gender].[M]} on rows from Sales");
    assertEquals("Q2", result.getAxes()[0].getPositions().get(0).get(0).getName());
  }

  public void testAll5() {
    Result result = executeQuery("select {[Time].[1997].[Q2].Parent} on columns,"
      + "{[Gender].[M]} on rows from Sales");
    // previous to [Gender].[All] is null, so no members are returned
  ...
  public void testAggregateDepends() {
    ...
    getTestContext().assertExprDependsOn("([Measures].[Unit Sales], [Gender].[F])",
      s12);
    ...
    getTestContext().assertExprDependsOn("Aggregate([Customers].Members)", s11);
  ...
  public void testAggregateWithIIF() {
    assertQueryReturns("with member store.foo as 'iif(3>1,"
      ...
      + "select {store.foo} on 0 from sales", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Store].[foo]}\n" + "Row #0: 67,659\n");
  }
   ...
   * 
   * <p>
   * Mosha discusses this behavior in the article <a
   * href="http://www.mosha.com/msolap/articles/mdxmultiselectcalcs.htm">
   * Multiselect friendly MDX calculations</a>.
   * 
   * <p>
   * Mondrian's behavior is consistent with MSAS 2K: it returns zeroes. SSAS
   * 2005 returns an error, which can be fixed by reformulating the calculated
   * members.
   * 
  ...
  public void testCount() {
    ...
    getTestContext().assertExprDependsOn(
      "count(Crossjoin([Store].[All Stores].[USA].Children, "
        + "{[Gender].children}), EXCLUDEEMPTY)", s1);
  ...
  public void testCountExcludeEmptyNull() {
    assertQueryReturns("WITH MEMBER [Measures].[Foo] AS\n" + "    Iif("
      ...
      + "    Iif(" + TestContext.hierarchyName("Time", "Time")
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-710"> bug
   * MONDRIAN-710, "Count with ExcludeEmpty throws an exception when the cube
   * does not have a factCountMeasure"</a>.
   */
  public void testCountExcludeEmptyOnCubeWithNoCountFacts() {
    assertQueryReturns("WITH " + "  MEMBER [Measures].[count] AS '"
      + "    COUNT([Store Type].[Store Type].MEMBERS, EXCLUDEEMPTY)'" + " SELECT "
      + "  {[Measures].[count]} ON AXIS(0)" + " FROM [Warehouse]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[count]}\n" + "Row #0: 5\n");
  }

  public void testCountExcludeEmptyOnVirtualCubeWithNoCountFacts() {
    assertQueryReturns("WITH " + "  MEMBER [Measures].[count] AS '"
      + "    COUNT([Store].MEMBERS, EXCLUDEEMPTY)'" + " SELECT "
      + "  {[Measures].[count]} ON AXIS(0)" + " FROM [Warehouse and Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[count]}\n"
        + "Row #0: 31\n");
  }
  ...
  public void testIIfNumeric() {
    ...
    assertExprReturns(
      "IIf([Measures].[Unit Sales] > [Measures].[Store Sales], 45, 32)", "32");
  }
  ...
  public void testMedian() {
    assertExprReturns("MEDIAN({[Store].[All Stores].[USA].children},"
      + "[Measures].[Store Sales])", "159,167.84");
  }
  ...
  public void testValue() {
    ...
    String s1 = TestContext.allHiersExcept("[Measures]");
    getTestContext().assertExprDependsOn("[Measures].[Store Sales].VALUE", s1);
   ...
   */
  public void testAs() {
    assertAxisReturns("Filter([Customers].Children as t,\n"
      + "t.Current.Name = 'USA')", "[Customers].[USA]");
  ...
  public void testAs2() {
    ...
    assertQueryThrows("with member Measures.Foo as 'x.Current.Name'\n" + "select\n"
    ...
    // should be the same.
    assertQueryThrows("with member Measures.Foo as 'z.Current.Name'\n" + "select\n"
      + "  {Measures.[Unit Sales], Measures.Foo} on 0,\n" + "  generate(\n"
      + "    [Marital Status].\n" + "    Children as s,\n" + "    filter(\n"
      + "      Gender.Members as s,\n"
    ...
    assertQueryThrows("select Measures.[Unit Sales] on 0,\n" + "  filter(\n"
      ...
      + "from [Sales]", "Syntax error at line 3, column 46, token ''foo''");

    // 'set AS numeric' is invalid
    assertQueryThrows("select Measures.[Unit Sales] on 0,\n" + "  filter(\n"
      + "    (Time.Month.Members * Gender.Members) as 1234,\n"
      + "    (s.Current.Item(0).Parent, [Marital Status].[S]) > 50000) on 1\n"
      + "from [Sales]", "Syntax error at line 3, column 46, token '1234'");
  ...
  public void testAscendants() {
    assertAxisReturns("Ascendants([Store].[USA].[CA])", "[Store].[USA].[CA]\n"
      + "[Store].[USA]\n" + "[Store].[All Stores]");
  }

  public void testAscendantsAll() {
    assertAxisReturns("Ascendants([Store].DefaultMember)", "[Store].[All Stores]");
  }
  ...
  /**
   * Tests that Except() successfully removes crossjoined tuples from the axis
   * results. Previously, this would fail by returning all tuples in the first
   * argument to Except. bug 1439627
   */
  public void testExceptCrossjoin() {
    assertAxisReturns("Except(CROSSJOIN({[Promotion Media].[All Media]},\n"
  ...
  /**
   * Tests that TopPercent() operates succesfully on a axis of crossjoined
   * tuples. previously, this would fail with a ClassCastException in
   * FunUtil.java. bug 1440306
   */
   ...
   * The problem occurs when a calculated member that references itself is
   * referenced in a crossjoin.
   */
  ...
  /**
   * Test case for bug 1911832, "Exception converting immutable list to array in
   * JDK 1.5".
   */
  ...
  public void testCrossjoinDupHierarchyFails() {
    assertQueryThrows("select [Measures].[Unit Sales] ON COLUMNS,\n"
      ...
      + "from [Sales]", "Tuple contains more than one member of hierarchy '[Time]'.");
  ...
  /**
   * Tests cases of different hierarchies in the same dimension. (Compare to
   * {@link #testCrossjoinDupHierarchyFails()}). Not an error.
   */
  public void testCrossjoinDupDimensionOk() {
    ...
    assertQueryReturns("select [Measures].[Unit Sales] ON COLUMNS,\n"
      + " CrossJoin({[Time].[Quarter].[Q1]}, {" + timeWeekly
      + ".[1997].[10]}) ON ROWS\n" + "from [Sales]", expectedResult);

    // now with Item, for kicks
    assertQueryReturns("select [Measures].[Unit Sales] ON COLUMNS,\n"
      + " CrossJoin({[Time].[Quarter].[Q1]}, {" + timeWeekly
      + ".[1997].[10]}).Item(0) ON ROWS\n" + "from [Sales]", expectedResult);
  ...
  public void testDescendantsM() {
    assertAxisReturns("Descendants([Time].[1997].[Q1])", "[Time].[1997].[Q1]\n"
      + "[Time].[1997].[Q1].[1]\n" + "[Time].[1997].[Q1].[2]\n"
      + "[Time].[1997].[Q1].[3]");
  }
  ...
  public void testDescendantsDepends() {
    getTestContext().assertSetExprDependsOn(
      "Descendants([Time].[Time].CurrentMember)", "{[Time]}");
  }

  public void testDescendantsML() {
    assertAxisReturns("Descendants([Time].[1997], [Time].[Month])", months);
  }

  public void testDescendantsMLSelf() {
    assertAxisReturns("Descendants([Time].[1997], [Time].[Quarter], SELF)", quarters);
  }
  ...
  public void testDescendantsMLLeaves() {
    ...
    assertAxisReturns("Descendants([Time].[1997], [Time].[Month], LEAVES)", months);
  ...
  public void testDescendantsMLLeavesRagged() {
    ...
    final TestContext raggedContext = getTestContext().withCube("[Sales Ragged]");
  ...
  public void testDescendantsMLSelfBefore() {
    assertAxisReturns(
      "Descendants([Time].[1997], [Time].[Quarter], SELF_AND_BEFORE)", year1997
        + "\n" + quarters);
  }
  ...
  public void testDescendantsMLBeforeAfter() {
    assertAxisReturns(
      "Descendants([Time].[1997], [Time].[Quarter], BEFORE_AND_AFTER)", year1997
        + "\n" + months);
  }

  public void testDescendantsMLAfter() {
    assertAxisReturns("Descendants([Time].[1997], [Time].[Quarter], AFTER)", months);
  }
  ...
  public void testDescendantsM0() {
    assertAxisReturns("Descendants([Time].[1997], 0)", year1997);
  }

  public void testDescendantsM2() {
    assertAxisReturns("Descendants([Time].[1997], 2)", months);
  }

  public void testDescendantsM2Self() {
    assertAxisReturns("Descendants([Time].[1997], 2, Self)", months);
  }

  public void testDescendantsM2Leaves() {
    assertAxisReturns("Descendants([Time].[1997], 2, Leaves)", months);
  }

  public void testDescendantsMFarLeaves() {
    assertAxisReturns("Descendants([Time].[1997], 10000, Leaves)", months);
  }

  public void testDescendantsMEmptyLeaves() {
    assertAxisReturns("Descendants([Time].[1997], , Leaves)", months);
  }

  public void testDescendantsMEmptyLeavesFail() {
    assertAxisThrows("Descendants([Time].[1997],)",
      "No function matches signature 'Descendants(<Member>, <Empty>)");
  }

  public void testDescendantsMEmptyLeavesFail2() {
    assertAxisThrows("Descendants([Time].[1997], , AFTER)",
  ...
  public void testDescendantsMNY() {
    assertAxisReturns("Descendants([Time].[1997], 1, BEFORE_AND_AFTER)", year1997
      + "\n" + months);
  }
  ...
  public void testDescendantsParentChildLeaves() {
    ...
    testContext.assertAxisReturns("Descendants([Employees], 2, LEAVES)",
    ...
    testContext.assertAxisReturns("Descendants([Employees], 3, LEAVES)",
      "[Employees].[Sheri Nowmer].[Roberta Damstra].[Jennifer Cooper]\n"
        + "[Employees].[Sheri Nowmer].[Roberta Damstra].[Peggy Petty]\n"
        + "[Employees].[Sheri Nowmer].[Roberta Damstra].[Jessica Olguin]\n"
        + "[Employees].[Sheri Nowmer].[Roberta Damstra].[Phyllis Burchett]\n"
    ...
    testContext
      .assertExprReturns("Count(Descendants([Employees], 2, LEAVES))", "16");
    testContext
      .assertExprReturns("Count(Descendants([Employees], 3, LEAVES))", "16");
    testContext
      .assertExprReturns("Count(Descendants([Employees], 4, LEAVES))", "63");
    testContext.assertExprReturns("Count(Descendants([Employees], 999, LEAVES))",
      "1,044");

    // Negative depth acts like +infinity (per MSAS). Run the test several
    // times because we had a non-deterministic bug here.
    for (int i = 0; i < 100; ++i) {
      testContext.assertExprReturns("Count(Descendants([Employees], -1, LEAVES))",
        "1,044");
    }
  }
  ...
  public void testDescendantsSetEmpty() {
    assertAxisThrows("Descendants({}, 1)", "Cannot deduce type of set");
    assertAxisReturns("Descendants(Filter({[Time].[Time].Members}, 1=0), 1)", "");
  }
  ...
  /**
   * Large dimensions use a different member reader, therefore need to be tested
   * separately.
   */
  ...
  public void testRangeBoundedByAll() {
    assertAxisReturns("[Gender] : [Gender]", "[Gender].[All Gender]");
  }
  ...
  public void testBug715177() {
    assertQueryReturns("WITH MEMBER [Product].[Non-Consumable].[Other] AS\n"
  ...
  public void testBug715177c() {
    assertAxisReturns("Order(TopCount({[Store].[USA].[CA].children},"
  ...
  public void testFormatMember() {
    assertExprReturns("Format([Store].[USA].[CA], \"#,#\" || \"#0.00\")",
      "74,748.00");
  }
  ...
  public void testIsEmptyWithNull() {
    assertExprReturns("iif (isempty(null), \"is empty\", \"not is empty\")",
      "is empty");
  ...
  public void testIIfLevel() {
    assertExprReturns(
      "IIf(1 > 2, [Store].[Store Country],[Store].[Store City]).Name", "Store City");
  }

  public void testIIfHierarchy() {
    assertExprReturns("IIf(1 > 2, [Time], [Store]).Name", "Store");
    ...
    assertExprReturns("IIf(1 > 2, [Time], [Time.Weekly]).Name", "Time");
  }

  public void testIIfDimension() {
    assertExprReturns("IIf(1 > 2, [Store], [Time]).Name", "Time");
  }
  ...
  public void testCoalesceEmpty() {
    ...
    checkDataResults(new Double[][] { new Double[] { null, null },
      new Double[] { new Double(263793.22), new Double(124366) } }, result, 0.001);
    ...
    checkDataResults(new Double[][] { new Double[] { null, null },
      new Double[] { new Double(8.963), new Double(8.963) } }, result, 0.001);
  ...
  public void testSetItemInt() {
    ...
    assertAxisReturns(
      "{([Time].[1997].[Q1].[1], [Customers].[All Customers].[USA]),"
        ...
        + ".Item(100 / 50 - 1)", "{[Time].[1997].[Q1].[2], [Customers].[USA].[WA]}");
  ...
  public void testSetItemString() {
    assertAxisReturns("{[Gender].[M], [Gender].[F]}.Item(\"M\")", "[Gender].[M]");
  ...
  public void testTuple() {
    assertExprReturns("([Gender].[M], " + "[Time].[Time].Children.Item(2), "
      + "[Measures].[Unit Sales])", "33,249");
  ...
  public void testTupleArgTypes() {
    ...
    assertExprReturns("([Gender], [Time].[Time])", "266,773");

    // can coerce hierarchy to member
    assertExprReturns("([Gender].[M], " + TimeWeekly + ")", "135,215");

    // cannot coerce level to member
    assertAxisThrows("{([Gender].[M], [Store].[Store City])}",
  ...
  public void testTupleItem() {
    ...
    assertAxisReturns("{[Time].[1997].[Q1].[1]}.item(0)", "[Time].[1997].[Q1].[1]");

    assertAxisReturns("{[Time].[1997].[Q1].[1]}.Item(0).Item(0)",
      "[Time].[1997].[Q1].[1]");
    ...
    // empty set
    assertExprReturns("Filter([Gender].members, 1 = 0).Item(0)", "");

    // empty set of unknown type
    assertExprReturns("{}.Item(3)", "");

    // past end of set
    assertExprReturns("{[Gender].members}.Item(4)", "");

    // negative index
    assertExprReturns("{[Gender].members}.Item(-50)", "");
  }
  ...
  public void testTupleDepends() {
    getTestContext()
      .assertMemberExprDependsOn("([Store].[USA], [Gender].[F])", "{}");

    getTestContext().assertMemberExprDependsOn("([Store].[USA], [Gender])",
      "{[Gender]}");

    // in a scalar context, the expression depends on everything except
    // the explicitly stated dimensions
    getTestContext().assertExprDependsOn("([Store].[USA], [Gender])",
    ...
    getTestContext().assertExprDependsOn(
      "(Dimensions('Store').CurrentMember, [Gender].[F])", TestContext.allHiers());
  }
  ...
  public void testItemNull() {
    ...
    assertExprReturns("Filter([Gender].members, 1 = 0).Item(0).Dimension.Name",
      "Gender");

    // MSAS returns error here.
    assertExprReturns("Filter([Gender].members, 1 = 0).Item(0).Parent", "");
    assertExprReturns("(Filter([Store].members, 0 = 0).Item(0).Item(0),"
      + "Filter([Store].members, 0 = 0).Item(0).Item(0))", "266,773");

    if (isDefaultNullMemberRepresentation()) {
      // MSAS returns error here.
      assertExprReturns("Filter([Gender].members, 1 = 0).Item(0).Name", "#null");
    }
  }
  ...
  public void testTupleNull() {
    ...
    // null
    assertAxisReturns("([Gender].parent, [Marital Status]),\n" // part null
    ...
    if (isDefaultNullMemberRepresentation()) {
      ...
      // tuple is null.
      assertExprReturns("([Gender].parent, [Marital Status]).Item(0).Name", "#null");
      assertExprReturns("([Gender].parent, [Marital Status]).Item(1).Name", "#null");
    }
  }

  private void checkDataResults(Double[][] expected, Result result,
    ...
    for (int row = 0; row < expected.length; row++) {
      ...
      for (int col = 0; col < expected[0].length; col++) {
        ...
        if (expectedValue == null) {
          ...
          fail("Cell at (" + row + ", " + col + ") was null, but was expecting "
            + expectedValue);
        } else {
          assertEquals("Incorrect value returned at (" + row + ", " + col + ")",
            expectedValue, ((Number) cell.getValue()).doubleValue(), tolerance);
        }
      }
    }
  }
  ...
  public void testCaseTestMatchElse() {
    assertExprReturns("CASE WHEN 1=0 THEN \"first\" ELSE \"fourth\" END", "fourth");
  }
  ...
  public void testCaseMatchElse() {
    assertExprReturns("CASE 7 WHEN 1 THEN \"first\" ELSE \"fourth\" END", "fourth");
  }
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-853"> bug
   * MONDRIAN-853, "When using CASE WHEN in a CalculatedMember values are not
   * returned the way expected"</a>.
   */
  public void testCaseTuple() {
    ...
    if (false)
      assertExprReturns("case 1 when 0 then 1.5\n"
        + " else ([Gender].[M], [Measures].[Unit Sales]) end", "135,215");

    // "case when" variant always worked
    assertExprReturns("case when 1=0 then 1.5\n"
      + " else ([Gender].[M], [Measures].[Unit Sales]) end", "135,215");
    ...
    if (false)
      assertExprReturns(
        "case when 1=0 then ([Gender].[M], [Measures].[Store Sales])\n"
          + " else ([Gender].[M], [Measures].[Unit Sales]) end", "xxx");

    // case 3: mixture of member & tuple. Should be able to deduce that
    // result type is an expression.
    if (false)
      assertExprReturns("case when 1=0 then ([Measures].[Store Sales])\n"
        + " else ([Gender].[M], [Measures].[Unit Sales]) end", "xxx");
  }
  ...
  public void testPropertyInCalculatedMember() {
    ...
    Assert.assertEquals("[Store].[USA].[WA].[Bellingham].[Store 2]", member
      .getUniqueName());
    ...
    Assert.assertEquals("[Store].[Mexico].[DF].[San Andres].[Store 21]", member
      .getUniqueName());
  ...
  public void testOpeningPeriod() {
    ...
    assertAxisReturns("OpeningPeriod([Time].[Year], [Time].[1997])", "[Time].[1997]");
    ...
    // Default member is [Time].[1997].
    assertAxisReturns("OpeningPeriod([Time].[Month])", "[Time].[1997].[Q1].[1]");
  ...
  public void testLastPeriods() {
    ...
    assertAxisReturns("LastPeriods(1, [Time].[1998])", "[Time].[1998]");
    assertAxisReturns("LastPeriods(-1, [Time].[1998])", "[Time].[1998]");
    assertAxisReturns("LastPeriods(2, [Time].[1998])", "[Time].[1997]\n"
      + "[Time].[1998]");
    assertAxisReturns("LastPeriods(-2, [Time].[1997])", "[Time].[1997]\n"
      + "[Time].[1998]");
    assertAxisReturns("LastPeriods(5000, [Time].[1998])", "[Time].[1997]\n"
      + "[Time].[1998]");
    assertAxisReturns("LastPeriods(-5000, [Time].[1997])", "[Time].[1997]\n"
      + "[Time].[1998]");
    assertAxisReturns("LastPeriods(2, [Time].[1998].[Q2])", "[Time].[1998].[Q1]\n"
      + "[Time].[1998].[Q2]");
    assertAxisReturns("LastPeriods(4, [Time].[1998].[Q2])", "[Time].[1997].[Q3]\n"
      + "[Time].[1997].[Q4]\n" + "[Time].[1998].[Q1]\n" + "[Time].[1998].[Q2]");
    assertAxisReturns("LastPeriods(-2, [Time].[1997].[Q2])", "[Time].[1997].[Q2]\n"
      + "[Time].[1997].[Q3]");
    assertAxisReturns("LastPeriods(-4, [Time].[1997].[Q2])", "[Time].[1997].[Q2]\n"
      + "[Time].[1997].[Q3]\n" + "[Time].[1997].[Q4]\n" + "[Time].[1998].[Q1]");
    ...
    assertAxisReturns("LastPeriods(-12, [Time].[1997].[Q2].[6])",
      "[Time].[1997].[Q2].[6]\n" + "[Time].[1997].[Q3].[7]\n"
        ...
        + "[Time].[1998].[Q1].[2]\n" + "[Time].[1998].[Q1].[3]\n"
        + "[Time].[1998].[Q2].[4]\n" + "[Time].[1998].[Q2].[5]");
    assertAxisReturns("LastPeriods(2, [Gender].[M])", "[Gender].[F]\n"
      + "[Gender].[M]");
    assertAxisReturns("LastPeriods(-2, [Gender].[F])", "[Gender].[F]\n"
      + "[Gender].[M]");
    assertAxisReturns("LastPeriods(2, [Gender])", "[Gender].[All Gender]");
  ...
  public void testParallelPeriodDepends() {
    ...
    getTestContext().assertMemberExprDependsOn("ParallelPeriod()", "{[Time]}");
    getTestContext().assertMemberExprDependsOn("ParallelPeriod([Product].[Food])",
      "{[Product]}");
    ...
    getTestContext().assertSetExprDependsOn(
      "parallelperiod([Time].[Time].CurrentMember)", "{[Time]}");
  }
  ...
  public void testParallelPeriodLevelLag() {
    assertQueryReturns(
      "with member [Measures].[Prev Unit Sales] as "
        ...
        + "    {[Time].[1997].[Q3]} on rows " + "from  " + "    [Sales] ",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testParallelPeriodLevel() {
    assertQueryReturns(
      "with "
        ...
        + "    {[Time].[1997].[Q3].[8]} on rows " + "from  " + "    [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n"
        + "{[Measures].[Unit Sales], [Marital Status].[M]}\n"
        + "{[Measures].[Prev Unit Sales], [Marital Status].[M]}\n" + "Axis #2:\n"
        + "{[Time].[1997].[Q3].[8]}\n" + "Row #0: 10,957\n" + "Row #0: 10,280\n");
  ...
  public void testDivide() {
    ...
    boolean origNullDenominatorProducesNull = MondrianProperties.instance().NullDenominatorProducesNull
      .get();
    try {
      // default behavior
  ...
  public void testMod() {
    ...
    assertExprThrows("mod(4, 0)", "java.lang.ArithmeticException: / by zero");
    assertExprThrows("mod(0, 0)", "java.lang.ArithmeticException: / by zero");
  }
  ...
  public void testString() {
    ...
    // string to a char.
    assertQueryReturns("with member measures.x as 'String(3, \"yahoo\")'\n"
    ...
    assertExprThrows("String(-1, 'x')", "NegativeArraySizeException"); // SSAS
    // agrees
    assertExprThrows("String(-200, 'x')", "NegativeArraySizeException"); // SSAS
    // agrees
  }

  public void testStringConcat() {
    assertExprReturns(" \"foo\" || \"bar\"  ", "foobar");
  }

  public void testStringConcat2() {
    assertExprReturns(" \"foo\" || [Gender].[M].Name || \"\" ", "fooM");
  }
  ...
  public void testIsTuple() {
    ...
    assertBooleanExprReturns(
      " (Store.[USA], Gender.[M]) IS (Gender.[M], Store.[USA]) "
        + "OR [Gender] IS NULL", true);
    assertBooleanExprReturns(
      " (Store.[USA], Gender.[M]) IS (Gender.[M], Store.[USA]) "
        + "AND [Gender] IS NULL", false);
    assertBooleanExprReturns(
      " (Store.[USA], Gender.[M]) IS (Store.[USA], Gender.[F])", false);
  ...
  public void testIsLevel() {
    ...
    assertBooleanExprReturns(" Store.[USA].[CA].level IS Store.[Store Country] ",
      false);
  }

  public void testIsHierarchy() {
    assertBooleanExprReturns(" Store.[USA].hierarchy IS Store.[Mexico].hierarchy ",
      true);
    assertBooleanExprReturns(" Store.[USA].hierarchy IS Gender.[M].hierarchy ",
      false);
  }
  ...
  private void checkNullOp(final String op) {
    ...
    assertBooleanExprReturns(NullNumericExpr + " " + op + " " + NullNumericExpr,
      false);
  }
   ...
   * select filter([Customers].members, [Unit Sales] > 100) from sales where
   * ([Time].[1998])
   */
  ...
  public void testGenerateNonSet() {
    ...
    assertAxisReturns("Generate([Store].[USA], [Store].PrevMember, ALL)",
      "[Store].[Mexico]");
  }
  ...
  public void testGenerateAll() {
    assertAxisReturns(
      "Generate({[Store].[USA].[CA], [Store].[USA].[OR].[Portland]},"
        + " Ascendants([Store].CurrentMember)," + " ALL)", "[Store].[USA].[CA]\n"
        + "[Store].[USA]\n" + "[Store].[All Stores]\n"
        + "[Store].[USA].[OR].[Portland]\n" + "[Store].[USA].[OR]\n"
        + "[Store].[USA]\n" + "[Store].[All Stores]");
  }
  ...
  public void testGenerateUnique() {
    assertAxisReturns(
      "Generate({[Store].[USA].[CA], [Store].[USA].[OR].[Portland]},"
        + " Ascendants([Store].CurrentMember))", "[Store].[USA].[CA]\n"
        + "[Store].[USA]\n" + "[Store].[All Stores]\n"
        + "[Store].[USA].[OR].[Portland]\n" + "[Store].[USA].[OR]");
  }

  public void testGenerateUniqueTuple() {
    assertAxisReturns("Generate({([Store].[USA].[CA],[Product].[All Products]), "
  ...
  public void testGenerateString() {
    assertExprReturns("Generate({Time.[1997], Time.[1998]},"
      + " Time.[Time].CurrentMember.Name)", "19971998");
    assertExprReturns("Generate({Time.[1997], Time.[1998]},"
      + " Time.[Time].CurrentMember.Name, \" and \")", "1997 and 1998");
  }

  public void testHead() {
    assertAxisReturns("Head([Store].Children, 2)", "[Store].[Canada]\n"
      + "[Store].[Mexico]");
  }
  ...
  public void testHeadDefault() {
    assertAxisReturns("Head([Store].Children)", "[Store].[Canada]");
  }

  public void testHeadOvershoot() {
    assertAxisReturns("Head([Store].Children, 2 + 2)", "[Store].[Canada]\n"
      + "[Store].[Mexico]\n" + "[Store].[USA]");
  }
  ...
  /**
   * Test case for bug 2488492, "Union between calc mem and head function throws
   * exception"
   */
  public void testHeadBug() {
    ...
    assertAxisReturns("Union(\n" + "  Union(\n"
      ...
      + "  Tail([Customers].[USA].[OR].Children, 2)," + "  ALL)",
      "[Customers].[USA].[CA].[West Covina]\n"
        + "[Customers].[USA].[CA].[Woodland Hills]\n"
        + "[Customers].[USA].[WA].[Anacortes]\n"
        + "[Customers].[USA].[WA].[Ballard]\n"
   ...
   * whose ordering is determined by an 'ordinal' property. TODO: fix this test
   * (bug 1220787)
   * 
   * WG: Note that this is disabled right now due to its impact on other tests
   * later on within the test suite, specifically XMLA tests that return a list
   * of cubes. We could run this test after XMLA, or clear out the cache to
   * solve this.
   */
  public void testHierarchizeOrdinal() {
    ...
    context.assertAxisReturns("Hierarchize([Time_Alphabetical].members)",
    ...
    context.assertAxisReturns("Hierarchize([Month_Alphabetical].members)",
  ...
  public void testOrderDepends() {
    ...
    String s11 = TestContext.allHiersExcept("[Product]", "[Measures]",
      "[Marital Status]", "[Gender]");
    getTestContext().assertSetExprDependsOn(
      "Order(" + " Crossjoin([Gender].MEMBERS, [Product].MEMBERS),"
        + " ([Measures].[Unit Sales], [Marital Status].[S])," + " ASC)", s11);

    // Depends upon everything EXCEPT [Product], [Measures],
    // [Marital Status]. Does depend upon [Gender].
    String s12 = TestContext.allHiersExcept("[Product]", "[Measures]",
      "[Marital Status]");
    getTestContext().assertSetExprDependsOn(
      "Order(" + " Crossjoin({[Gender].CurrentMember}, [Product].MEMBERS),"
        + " ([Measures].[Unit Sales], [Marital Status].[S])," + " ASC)", s12);

    // Depends upon everything except [Measures].
    String s13 = TestContext.allHiersExcept("[Measures]");
    getTestContext().assertSetExprDependsOn(
      "Order(" + "  Crossjoin(" + "    [Gender].CurrentMember.Children, "
        + "    [Marital Status].CurrentMember.Children), "
        + "  [Measures].[Unit Sales], " + "  BDESC)", s13);

    String s1 = TestContext.allHiersExcept("[Measures]", "[Store]", "[Product]",
      "[Time]");
    getTestContext().assertSetExprDependsOn(
      "  Order(\n" + "    CrossJoin(\n"
  ...
  /**
   * Verifies that the order function works with a defined member. See this
   * forum post for additional information:
   * http://forums.pentaho.com/showthread.php?p=179473#post179473
   */
  public void testOrderWithMember() {
    assertQueryReturns("with member [Measures].[Product Name Length] as "
  ...
  public void testOrderNonEmpty() {
    assertQueryReturns("select NON EMPTY [Gender].Members ON COLUMNS,\n"
  ...
  public void testOrderParentsMissing() {
    ...
    assertQueryReturns("select {[Measures].[Unit Sales]} on columns," + " order({\n"
      + "  [Product].[All Products].[Drink].[Alcoholic Beverages],\n"
      + "  [Product].[All Products].[Food].[Eggs]},\n"
  ...
  public void testOrderCrossJoin() {
    ...
    assertQueryReturns("select CrossJoin(\n" + "    {[Time].[1997],\n"
      + "     [Time].[1997].[Q1]},\n"
      + "    {[Measures].[Unit Sales]}) on columns,\n" + "  Order(\n"
      + "    CrossJoin(\n" + "      {[Product].[All Products].[Food].[Eggs],\n"
      + "       [Product].[All Products].[Food].[Seafood],\n"
  ...
  public void testUnorder() {
    assertAxisReturns("Unorder([Gender].members)", "[Gender].[All Gender]\n"
      + "[Gender].[F]\n" + "[Gender].[M]");
    assertAxisReturns("Unorder(Order([Gender].members, -[Measures].[Unit Sales]))",
      "[Gender].[All Gender]\n" + "[Gender].[M]\n" + "[Gender].[F]");
    assertAxisReturns(
    ...
    assertAxisReturns("Unorder([Gender].[M])", "[Gender].[M]");

    assertAxisThrows("Unorder(1 + 3)",
      "No function matches signature 'Unorder(<Numeric Expression>)'");
    assertAxisThrows("Unorder([Gender].[M], 1 + 3)",
  ...
  public void testSiblingsA() {
    assertAxisReturns("{[Time].[1997].Siblings}", "[Time].[1997]\n"
      + "[Time].[1998]");
  }

  public void testSiblingsB() {
    assertAxisReturns("{[Store].Siblings}", "[Store].[All Stores]");
  }

  public void testSiblingsC() {
    assertAxisReturns("{[Store].[USA].[CA].Siblings}", "[Store].[USA].[CA]\n"
      + "[Store].[USA].[OR]\n" + "[Store].[USA].[WA]");
  }
  ...
  public void testTail() {
    assertAxisReturns("Tail([Store].Children, 2)", "[Store].[Mexico]\n"
      + "[Store].[USA]");
  }
  ...
  public void testTailDefault() {
    assertAxisReturns("Tail([Store].Children)", "[Store].[USA]");
  }

  public void testTailOvershoot() {
    assertAxisReturns("Tail([Store].Children, 2 + 2)", "[Store].[Canada]\n"
      + "[Store].[Mexico]\n" + "[Store].[USA]");
  }
  ...
  public void testToggleDrillState() {
    assertAxisReturns("ToggleDrillState({[Customers].[USA],[Customers].[Canada]},"
      + "{[Customers].[USA],[Customers].[USA].[CA]})", "[Customers].[USA]\n"
      + "[Customers].[USA].[CA]\n" + "[Customers].[USA].[OR]\n"
      + "[Customers].[USA].[WA]\n" + "[Customers].[Canada]");
  }

  public void testToggleDrillState2() {
    assertAxisReturns("ToggleDrillState([Product].[Product Department].members, "
  ...
  public void testToggleDrillState3() {
    assertAxisReturns("ToggleDrillState(" + "{[Time].[1997].[Q1],"
      + " [Time].[1997].[Q2]," + " [Time].[1997].[Q2].[4],"
      + " [Time].[1997].[Q2].[6]," + " [Time].[1997].[Q3]},"
  ...
  private void checkTopBottomCountPercentDepends(String fun) {
    String s1 = TestContext.allHiersExcept("[Measures]", "[Promotion Media]");
    getTestContext().assertSetExprDependsOn(
      fun + "({[Promotion Media].[Media Type].members}, "
        + "2, [Measures].[Unit Sales])", s1);
   ...
   * machine, the 1st query took 14.5 secs, the 2nd query took 5.0 secs. After
   * optimizing, who knows?
   */
  ...
  public void testUnionAll() {
    assertAxisReturns("Union({[Gender].[M]}, {[Gender].[F]}, ALL)", "[Gender].[M]\n"
  ...
  public void testUnion() {
    assertAxisReturns("Union({[Store].[USA], [Store].[USA], [Store].[USA].[OR]}, "
      + "{[Store].[USA].[CA], [Store].[USA]})", "[Store].[USA]\n"
      + "[Store].[USA].[OR]\n" + "[Store].[USA].[CA]");
  }
  ...
  public void testUnionEmptyRight() {
    assertAxisReturns("Union({[Gender].[M]}, {})", "[Gender].[M]");
  }

  public void testUnionTuple() {
    assertAxisReturns("Union({" + " ([Gender].[M], [Marital Status].[S]),"
      + " ([Gender].[F], [Marital Status].[S])" + "}, {"
      + " ([Gender].[M], [Marital Status].[M]),"
      + " ([Gender].[M], [Marital Status].[S])" + "})",
  ...
  public void testUnionTupleDistinct() {
    assertAxisReturns("Union({" + " ([Gender].[M], [Marital Status].[S]),"
      + " ([Gender].[F], [Marital Status].[S])" + "}, {"
      + " ([Gender].[M], [Marital Status].[M]),"
      + " ([Gender].[M], [Marital Status].[S])" + "}, Distinct)",
  ...
  public void testUnionQuery() {
    Result result = executeQuery("select {[Measures].[Unit Sales], "
      + "[Measures].[Store Cost], "
      + "[Measures].[Store Sales]} on columns,\n"
      + " Hierarchize(\n"
      + "   Union(\n"
  ...
  public void testStrToMember() {
    assertExprReturns("StrToMember(\"[Time].[1997].[Q2].[4]\").Name", "4");
  }

  public void testStrToMemberUniqueName() {
    assertExprReturns("StrToMember(\"[Store].[USA].[CA]\").Name", "CA");
  }

  public void testStrToMemberFullyQualifiedName() {
    assertExprReturns("StrToMember(\"[Store].[All Stores].[USA].[CA]\").Name", "CA");
  }

  public void testStrToMemberNull() {
    // SSAS 2005 gives "#Error An MDX expression was expected. An empty
    // expression was specified."
    assertExprThrows("StrToMember(null).Name",
      "An MDX expression was expected. An empty expression was specified");
    assertExprThrows("StrToSet(null, [Gender]).Count",
      "An MDX expression was expected. An empty expression was specified");
    assertExprThrows("StrToTuple(null, [Gender]).Name",
      "An MDX expression was expected. An empty expression was specified");
  }

  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-560"> bug
  ...
  public void testStrToMemberIgnoreInvalidMembers() {
    ...
    // [Time].
    final String timeWeekly = TestContext.hierarchyName("Time", "Weekly");
    assertExprReturns("StrToMember(\"" + timeWeekly
      + ".[1996].[Q1]\").Hierarchy.UniqueName", timeWeekly);

    // If hierarchy is invalid, throw an error even though
    // IgnoreInvalidMembersDuringQuery is set.
    assertExprThrows("StrToMember(\"[Unknown Hierarchy].[Invalid].[Member]\").Name",
      "MDX object '[Unknown Hierarchy].[Invalid].[Member]' not found in cube 'Sales'");
    assertExprThrows("StrToMember(\"[Unknown Hierarchy].[Invalid]\").Name",
      "MDX object '[Unknown Hierarchy].[Invalid]' not found in cube 'Sales'");
    assertExprThrows("StrToMember(\"[Unknown Hierarchy]\").Name",
    ...
    assertExprThrows("StrToMember(\"[Marital Status].[Separated]\").Hierarchy.Name",
  ...
  public void testStrToTupleIgnoreInvalidMembers() {
    ...
    assertAxisReturns("StrToTuple(\"([Gender].[M], [Marital Status].[Separated])\","
  ...
  public void testStrToTupleDupHierInSameDimensions() {
    assertAxisThrows("{StrToTuple(" + "\"([Gender].[F], " + "[Time].[1997].[Q2], "
      + "[Time].[Weekly].[1997].[10])\"," + " [Gender], "
      + TestContext.hierarchyName("Time", "Weekly") + ", [Gender])}",
      "Tuple contains more than one member of hierarchy '[Gender]'.");
  }
  ...
  public void testStrToSet() {
    ...
    assertAxisReturns("StrToSet(" + " \"{[Gender].[F], [Gender].[M]}\","
      + " [Gender])", "[Gender].[F]\n" + "[Gender].[M]");

    assertAxisThrows("StrToSet(" + " \"{[Gender].[F], [Time].[1997]}\","
      + " [Gender])", "member is of wrong hierarchy");
    ...
    assertAxisReturns("StrToSet(" + "\"" + "{"
      + " ([Gender].[F], [Time].[1997].[Q2]), " + " ([Gender].[M], [Time].[1997])"
      + "}" + "\"," + " [Gender]," + " [Time])",
      "{[Gender].[F], [Time].[1997].[Q2]}\n" + "{[Gender].[M], [Time].[1997]}");

    // matches unique name
    assertAxisReturns("StrToSet(" + "\"" + "{" + " [Store].[USA].[CA], "
  ...
  public void testStrToSetDupDimensionsFails() {
    assertAxisThrows("StrToSet(" + "\"" + "{"
      ...
      + " ([Gender].[M], [Time].[1997], [Gender].[F])" + "}" + "\"," + " [Gender],"
      + " [Time]," + " [Gender])",
      "Tuple contains more than one member of hierarchy '[Gender]'.");
  }
  ...
  public void testStrToSetIgnoreInvalidMembers() {
    final MondrianProperties properties = MondrianProperties.instance();
    propSaver.set(properties.IgnoreInvalidMembersDuringQuery, true);
    assertAxisReturns("StrToSet(" + "\"" + "{" + " [Product].[Food],"
      + " [Product].[Food].[You wouldn't like],"
      + " [Product].[Drink].[You would like]," + " [Product].[Drink].[Dairy]" + "}"
    ...
    assertAxisReturns("StrToSet(" + "\"" + "{"
      ...
      + " ([Gender].[F], [Product].[Drink].[Dairy])" + "}" + "\","
      + " [Gender], [Product])", "{[Gender].[M], [Product].[Food]}\n"
      + "{[Gender].[F], [Product].[Drink].[Dairy]}");
  }

  public void testYtd() {
    assertAxisReturns("Ytd()", "[Time].[1997]");
    assertAxisReturns("Ytd([Time].[1997].[Q3])", "[Time].[1997].[Q1]\n"
      + "[Time].[1997].[Q2]\n" + "[Time].[1997].[Q3]");
    assertAxisReturns("Ytd([Time].[1997].[Q2].[4])", "[Time].[1997].[Q1].[1]\n"
      + "[Time].[1997].[Q1].[2]\n" + "[Time].[1997].[Q1].[3]\n"
      + "[Time].[1997].[Q2].[4]");
    assertAxisThrows("Ytd([Store])",
      "Argument to function 'Ytd' must belong to Time hierarchy");
    getTestContext().assertSetExprDependsOn("Ytd()", "{[Time], " + TimeWeekly + "}");
  ...
  public void testGeneratePlusXtd() {
    ...
    assertAxisReturns("generate(\n"
      ...
      + "[Time].[1997].[Q2].[4]\n" + "[Time].[1997].[Q2].[5]\n"
      + "[Time].[1997].[Q2].[6]\n" + "[Time].[1997].[Q3].[7]");
    assertExprReturns("count(generate({[Time].[1997].[Q4].[11]},"
      + " {Qtd( [Time].[Time].currentMember)}))", 2, 0);
    assertExprReturns("count(generate({[Time].[1997].[Q4].[11]},"
      + " {Mtd( [Time].[Time].currentMember)}))", 1, 0);
  }

  public void testQtd() {
    // zero args
    assertQueryReturns("with member [Measures].[Foo] as ' SetToStr(Qtd()) '\n"
      + "select {[Measures].[Foo]} on columns\n" + "from [Sales]\n"
      + "where [Time].[1997].[Q2].[5]", "Axis #0:\n" + "{[Time].[1997].[Q2].[5]}\n"
      + "Axis #1:\n" + "{[Measures].[Foo]}\n"
    ...
    // one arg, a month
    assertAxisReturns("Qtd([Time].[1997].[Q2].[5])", "[Time].[1997].[Q2].[4]\n"
      + "[Time].[1997].[Q2].[5]");

    // one arg, a quarter
    assertAxisReturns("Qtd([Time].[1997].[Q2])", "[Time].[1997].[Q2]");
  ...
  public void testMtd() {
    // zero args
    assertQueryReturns("with member [Measures].[Foo] as ' SetToStr(Mtd()) '\n"
      + "select {[Measures].[Foo]} on columns\n" + "from [Sales]\n"
      + "where [Time].[1997].[Q2].[5]", "Axis #0:\n" + "{[Time].[1997].[Q2].[5]}\n"
      + "Axis #1:\n" + "{[Measures].[Foo]}\n" + "Row #0: {[Time].[1997].[Q2].[5]}\n");

    // one arg, a month
    assertAxisReturns("Mtd([Time].[1997].[Q2].[5])", "[Time].[1997].[Q2].[5]");
  ...
  public void testPeriodsToDate() {
    ...
    getTestContext().assertSetExprDependsOn("PeriodsToDate([Time].[Year])",
      "{[Time]}");
    getTestContext().assertSetExprDependsOn(
      "PeriodsToDate([Time].[Year], [Time].[1997].[Q2].[5])", "{}");

    // two args
    assertAxisReturns("PeriodsToDate([Time].[Quarter], [Time].[1997].[Q2].[5])",
      "[Time].[1997].[Q2].[4]\n" + "[Time].[1997].[Q2].[5]");

    // equivalent to above
    assertAxisReturns("TopCount(" + "  Descendants(" + "    Ancestor("
      + "      [Time].[1997].[Q2].[5], [Time].[Quarter]),"
      + "    [Time].[1997].[Q2].[5].Level),"
      + "  1).Item(0) : [Time].[1997].[Q2].[5]", "[Time].[1997].[Q2].[4]\n"
      + "[Time].[1997].[Q2].[5]");
    ...
    assertQueryReturns("with member [Measures].[Position] as\n" + " 'Sum("
      + "PeriodsToDate([Time].[Time].Levels(0)," + " [Time].[Time].CurrentMember), "
      + "[Measures].[Store Sales])'\n" + "select {[Time].[1997],\n"
      + " [Time].[1997].[Q1],\n" + " [Time].[1997].[Q1].[1],\n"
  ...
  public void testSetToStr() {
    assertExprReturns("SetToStr([Time].[Time].children)",
  ...
  public void testTupleToStr() {
    ...
    assertExprReturns("TupleToStr([Product])", "[Product].[All Products]");

    // Applied to a dimension (invalid because has no default hierarchy)
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      assertExprThrows("TupleToStr([Time])",
        "The 'Time' dimension contains more than one hierarchy, "
          + "therefore the hierarchy must be explicitly specified.");
    } else {
      assertExprReturns("TupleToStr([Time])", "[Time].[1997]");
    }

    // Applied to a hierarchy
    assertExprReturns("TupleToStr([Time].[Time])", "[Time].[1997]");

    // Applied to a member
    assertExprReturns("TupleToStr([Store].[USA].[OR])", "[Store].[USA].[OR]");
   ...
   * For example, <code>assertExprReturns("1 + 2", "3")</code> should succeed.
   */
  ...
  /**
   * Executes a scalar expression, and asserts that the result is within delta
   * of the expected result.
   * 
   * @param expr
   *          MDX scalar expression
   * @param expected
   *          Expected value
   * @param delta
  ...
  public void assertExprReturns(String expr, double expected, double delta) {
    ...
    try {
      ...
      throw new ComparisonFailure(msg, Double.toString(expected), String
        .valueOf(value));
    }
  }
  ...
  public void testRank() {
    // Member within set
    assertExprReturns("Rank([Store].[USA].[CA], " + "{[Store].[USA].[OR],"
      + " [Store].[USA].[CA]," + " [Store].[USA]})", "2");
    // Member not in set
    assertExprReturns("Rank([Store].[USA].[WA], " + "{[Store].[USA].[OR],"
      + " [Store].[USA].[CA]," + " [Store].[USA]})", "0");
    // Member not in empty set
    assertExprReturns("Rank([Store].[USA].[WA], {})", "0");
    // Null member not in set returns null.
    assertExprReturns("Rank([Store].Parent, " + "{[Store].[USA].[OR],"
    ...
    assertExprReturns("Rank([Store].[USA].[WA], \n" + "{[Store].[USA].[WA],"
      + " [Store].[USA].[CA]," + " [Store].[USA]," + " [Store].[USA].[WA]})", "1");
    // Tuple not in set
    assertExprReturns("Rank(([Gender].[F], [Marital Status].[M]), \n"
      + "{([Gender].[F], [Marital Status].[S]),\n"
      + " ([Gender].[M], [Marital Status].[S]),\n"
    ...
    assertExprReturns("Rank(([Gender].[F], [Marital Status].Parent), \n"
      + "{([Gender].[F], [Marital Status].[S]),\n"
      + " ([Gender].[M], [Marital Status].[S]),\n"
      + " ([Gender].[F], [Marital Status].[M])})", "");
  }
  ...
  public void testRankWithExpr2() {
    ...
    // F 131,558
    // M 135,215
    assertExprReturns("Rank([Gender].[All Gender]," + " {[Gender].Members},"
      + " [Measures].[Unit Sales])", "1");
    assertExprReturns("Rank([Gender].[F]," + " {[Gender].Members},"
      + " [Measures].[Unit Sales])", "3");
    assertExprReturns("Rank([Gender].[M]," + " {[Gender].Members},"
    ...
    assertExprReturns("Rank([Gender].[All Gender].Parent," + " {[Gender].Members},"
      + " [Measures].[Unit Sales])", "");
    ...
    assertExprReturns("Rank([Gender].[M]," + " {}," + " [Measures].[Unit Sales])",
      "1");
    // As above, but SSAS can type-check this.
    assertExprReturns("Rank([Gender].[M]," + " Filter(Gender.Members, 1 = 0),"
      + " [Measures].[Unit Sales])", "1");
    // Member is not in set
    assertExprReturns("Rank([Gender].[M],"
      + " {[Gender].[All Gender], [Gender].[F]})", "0");
    // Even though M is not in the set, its value lies between [All Gender]
    // and [F].
    assertExprReturns("Rank([Gender].[M],"
      + " {[Gender].[All Gender], [Gender].[F]}," + " [Measures].[Unit Sales])", "2");
    // Expr evaluates to null for some values of set.
    assertExprReturns("Rank([Product].[Non-Consumable].[Household],"
  ...
  /**
   * Tests the 3-arg version of the RANK function with a value which returns
   * null within a set of nulls.
   */
  public void testRankWithNulls() {
    assertQueryReturns("with member [Measures].[X] as "
      + "'iif([Measures].[Store Sales]=777," + "[Measures].[Store Sales],Null)'\n"
      + "member [Measures].[Y] as 'Rank([Gender].[M],"
      + "{[Measures].[X],[Measures].[X],[Measures].[X]},"
      + " [Marital Status].[All Marital Status].Parent)'"
      + "select {[Measures].[Y]} on columns from Sales", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Y]}\n" + "Row #0: 1\n");
  }

  /**
   * Tests a RANK function which is so large that we need to use caching in
  ...
  /**
   * Tests all of the linear regression functions, as suggested by <a
   * href="http://support.microsoft.com/kb/q307276/">a Microsoft knowledge base
   * article</a>.
   */
  ...
  public void testLinRegIntercept() {
    assertExprReturns("LinRegIntercept([Time].[Month].members,"
      + " [Measures].[Unit Sales], [Measures].[Store Sales])", -126.65, 0.50);
    ...
    if (false) {
      assertExprReturns("LinRegIntercept({[Time].Parent},"
        + " [Measures].[Unit Sales], [Measures].[Store Sales])", "-1.#IND"); // MSAS
      // returns
      // -1.#IND
      // (whatever
      // that
    ...
    // format does not add '$'
    assertExprReturns("LinRegIntercept([Time].[Month].members,"
      + " 7, [Measures].[Store Sales])", 7.00, 0.01);

    // Mondrian can not return "missing data" value -1.#IND
    // second expr constant
    if (false) {
      assertExprReturns("LinRegIntercept([Time].[Month].members,"
        + " [Measures].[Unit Sales], 4)", "-1.#IND"); // MSAS returns -1.#IND
      // (whatever that means)
    }
  }
  ...
  public void testLinRegSlope() {
    assertExprReturns("LinRegSlope([Time].[Month].members,"
      + " [Measures].[Unit Sales], [Measures].[Store Sales])", 0.4746, 0.50);
    ...
    assertExprReturns("LinRegSlope([Time].[Month].members,"
      + " 7, [Measures].[Store Sales])", 0.00, 0.01);
  ...
  public void testLinRegPoint() {
    ...
    if (false) {
      assertExprReturns("LinRegPoint([Measures].[Unit Sales],"
        ...
        + " [Measures].[Unit Sales], [Measures].[Store Sales])", "0.4746");
    }

    // Mondrian can not return "missing data" value -1.#IND
    // empty set
    if (false) {
      assertExprReturns("LinRegPoint([Measures].[Unit Sales]," + " {[Time].Parent},"
        + " [Measures].[Unit Sales], [Measures].[Store Sales])", "-1.#IND"); // MSAS
      // returns
      // -1.#IND
      // (whatever
    ...
    if (false) {
      assertExprReturns("LinRegPoint(-1," + " [Time].[Month].members,"
    ...
    if (false) {
      assertExprReturns("LinRegPoint([Measures].[Unit Sales],"
        + " [Time].[Month].members," + " 7, [Measures].[Store Sales])", "$7.00");
    }

    // format does not add '$'
    assertExprReturns("LinRegPoint([Measures].[Unit Sales],"
      + " [Time].[Month].members," + " 7, [Measures].[Store Sales])", 7.00, 0.01);

    // Mondrian can not return "missing data" value -1.#IND
    // second expr constant
    if (false) {
      assertExprReturns("LinRegPoint([Measures].[Unit Sales],"
        + " [Time].[Month].members," + " [Measures].[Unit Sales], 4)", "-1.#IND"); // MSAS returns -1.#IND (whatever that means)
    }
  }
  ...
  public void testVisualTotalsNoPattern() {
    assertAxisReturns("VisualTotals("
      + "    {[Product].[All Products].[Food].[Baked Goods].[Bread],"
      + "     [Product].[All Products].[Food].[Baked Goods].[Bread].[Bagels],"
      + "     [Product].[All Products].[Food].[Baked Goods].[Bread].[Muffins]})",
  ...
  public void testVisualTotalsWithFilter() {
    assertQueryReturns("select {[Measures].[Unit Sales]} on columns, " + "{Filter("
      + "    VisualTotals("
      + "        {[Product].[All Products].[Food].[Baked Goods].[Bread],"
      + "         [Product].[All Products].[Food].[Baked Goods].[Bread].[Bagels],"
      + "         [Product].[All Products].[Food].[Baked Goods].[Bread].[Muffins]},"
      + "        \"**Subtotal - *\")," + "[Measures].[Unit Sales] > 3400)} on rows "
      + "from [Sales]",
  ...
  public void testVisualTotalsNested() {
    assertQueryReturns(
      "select {[Measures].[Unit Sales]} on columns, "
        ...
        + "    \"Second total - *\")} on rows " + "from [Sales]",
  ...
  /**
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-615">MONDRIAN-615</a>,
  ...
  /**
   * Test case involving a named set and query pivoted. Suggested in <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-615">MONDRIAN-615</a>,
   * "VisualTotals doesn't work for the all member".
   */
  public void testVisualTotalsWithNamedSetAndPivot() {
    assertQueryReturns("WITH SET [CA_OR] AS\n" + "    VisualTotals(\n"
      + "        {[Customers].[All Customers],\n" + "         [Customers].[USA],\n"
      + "         [Customers].[USA].[CA],\n" + "         [Customers].[USA].[OR]})\n"
   ...
   * 
   * <p>
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-295"> bug
   * MONDRIAN-295, "Query generated by Excel 2007 gives incorrect results"</a>.
   */
  ...
  /**
   * <p>
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-682"> bug
   * MONDRIAN-682, "VisualTotals + Distinct-count measure gives wrong
   * results"</a>.
   */
  ...
  /**
   * <p>
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-761"> bug
   * MONDRIAN-761, "VisualTotalMember cannot be cast to RolapCubeMember"</a>.
   */
  ...
  /**
   * <p>
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-678"> bug
   * MONDRIAN-678, "VisualTotals gives UnsupportedOperationException calling
   * getOrdinal"</a>. Key difference from previous test is that there are
   * multiple hierarchies in Named set.
   */
   ...
   * 
   * <p>
   * The bug is not currently fixed, so it is a negative test case. Row #2 cell
   * #1 contains an exception, but should be "**Subtotal - Bread : Product
   * Subcategory".
   */
  ...
  public void testCast() {
    ...
    assertExprReturns("'xxx' || Cast(1 = 1 AND 1 = 2 AS String)", "xxxfalse");

    // To boolean (trivial)
    assertExprReturns("1=1 AND Cast((1 = 1 AND 1 = 2) AS Boolean)", "false");

    assertExprReturns("1=1 OR Cast(1 = 1 AND 1 = 2 AS Boolean)", "true");
    ...
    assertExprThrows("Cast(1 AS 5)", "Syntax error at line 1, column 11, token '5'");
  ...
  public void testCastBug524() {
    assertExprReturns("Cast(Int([Measures].[Store Sales] / 3600) as String)", "157");
  }

  /**
   * Tests {@link mondrian.olap.FunTable#getFunInfoList()}, but more
   * importantly, generates an HTML table of all implemented functions into a
   * file called "functions.html". You can manually include that table in the <a
  ...
  public void testDumpFunctions() throws IOException {
    ...
    // Add some UDFs.
    funInfoList.add(new FunInfo(new UdfResolver(new UdfResolver.ClassUdfFactory(
      CurrentDateMemberExactUdf.class, null))));
    funInfoList.add(new FunInfo(new UdfResolver(new UdfResolver.ClassUdfFactory(
      CurrentDateMemberUdf.class, null))));
    funInfoList.add(new FunInfo(new UdfResolver(new UdfResolver.ClassUdfFactory(
      CurrentDateStringUdf.class, null))));
  ...
  public void testComplexOrExpr() {
    ...
    assertQueryReturns(
      "with set [*NATIVE_CJ_SET] as '[Store].[Store Country].members' "
        ...
        + "select [*GENERATED_MEMBERS_Measures] ON COLUMNS, " + "NON EMPTY "
        + "    Filter(" + "        Generate(" + "        [*NATIVE_CJ_SET], "
        + "        {[Store].CurrentMember}), "
        ...
        + "            (NOT IsEmpty([Measures].[Promotion Sales])))) " + "on rows "
        + "from [Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
        + "{[Measures].[Unit Sales]}\n" + "{[Measures].[Store Cost]}\n"
        + "{[Measures].[Sales Count]}\n" + "{[Measures].[Customer Count]}\n"
        + "{[Measures].[Promotion Sales]}\n" + "Axis #2:\n" + "{[Store].[USA]}\n"
  ...
  public void testLeftFunctionWithLengthValueZero() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      ...
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\") on 0 from sales", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
      + "Row #0: 2,237\n");
  }
  ...
  public void testLeftFunctionWithLengthValueEqualToStringLength() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "Left([Store].CURRENTMEMBER.Name, 10)=\"Bellingham\") " + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testLeftFunctionWithLengthMoreThanStringLength() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "Left([Store].CURRENTMEMBER.Name, 20)=\"Bellingham\") " + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testLeftFunctionWithZeroLengthString() {
    assertQueryReturns("select filter([Store].MEMBERS,Left(\"\", 20)=\"\" "
      + "And [Store].CURRENTMEMBER.Name = \"Bellingham\") " + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testLeftFunctionWithNegativeLength() {
    assertQueryThrows(
      "select filter([Store].MEMBERS,"
        ...
        + "on 0 from sales", Util.IBM_JVM ? "StringIndexOutOfBoundsException: null"
  ...
  public void testMidFunctionWithValidArguments() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      ...
      + "And Mid(\"Bellingham\", 4, 6) = \"lingha\")" + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testMidFunctionWithZeroLengthStringArgument() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"\", 4, 6) = \"\")" + "on 0 from sales", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n" + "Row #0: 2,237\n");
  }
  ...
  public void testMidFunctionWithLengthArgumentLargerThanStringLength() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", 4, 20) = \"lingham\")" + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testMidFunctionWithStartIndexGreaterThanStringLength() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", 20, 2) = \"\")" + "on 0 from sales", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
      + "Row #0: 2,237\n");
  }
  ...
  public void testMidFunctionWithStartIndexZeroFails() {
    ...
    if (Bug.Ssas2005Compatible) {
      assertQueryReturns("select filter([Store].MEMBERS,"
        + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
        + "And Mid(\"Bellingham\", 0, 2) = \"Be\")" + "on 0 from sales",
        "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
          + "Row #0: 2,237\n");
    } else {
      assertQueryThrows("select filter([Store].MEMBERS,"
        + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
        + "And Mid(\"Bellingham\", 0, 2) = \"Be\")" + "on 0 from sales",
        "Invalid parameter. Start parameter of Mid function must be " + "positive");
    }
  }
  ...
  public void testMidFunctionWithStartIndexOne() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", 1, 2) = \"Be\")" + "on 0 from sales", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
      + "Row #0: 2,237\n");
  }
  ...
  public void testMidFunctionWithNegativeStartIndex() {
    assertQueryThrows("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", -20, 2) = \"\")" + "on 0 from sales",
      "Invalid parameter. " + "Start parameter of Mid function must be positive");
  }
  ...
  public void testMidFunctionWithNegativeLength() {
    assertQueryThrows("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", 2, -2) = \"\")" + "on 0 from sales",
      "Invalid parameter. "
  ...
  public void testMidFunctionWithoutLength() {
    assertQueryReturns("select filter([Store].MEMBERS,"
      + "[Store].CURRENTMEMBER.Name = \"Bellingham\""
      + "And Mid(\"Bellingham\", 2) = \"ellingham\")" + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testLenFunctionWithNonEmptyString() {
    assertQueryReturns("select filter([Store].MEMBERS, "
      + "Len([Store].CURRENTMEMBER.Name) = 3) on 0 from sales", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA]}\n" + "Row #0: 266,773\n");
  }
  ...
  public void testUCaseWithNonEmptyString() {
    assertQueryReturns("select filter([Store].MEMBERS, "
      + " UCase([Store].CURRENTMEMBER.Name) = \"BELLINGHAM\") " + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }

  public void testUCaseWithEmptyString() {
    assertQueryReturns("select filter([Store].MEMBERS, " + " UCase(\"\") = \"\" "
      + "And [Store].CURRENTMEMBER.Name = \"Bellingham\") " + "on 0 from sales",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[WA].[Bellingham]}\n"
        + "Row #0: 2,237\n");
  }
  ...
  public void testIifFWithBooleanBooleanAndNumericParameterForReturningTruePart() {
    assertQueryReturns("SELECT Filter(Store.allmembers, "
      ...
      + "[store].currentMember.NAME = \"USA\", 0)) on 0 FROM SALES", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA]}\n" + "Row #0: 266,773\n");
  }

  public void testIifWithBooleanBooleanAndNumericParameterForReturningFalsePart() {
    assertQueryReturns("SELECT Filter([Store].[USA].[CA].[Beverly Hills].children, "
      + "iif(measures.profit > 400000,"
      + "[store].currentMember.NAME = \"USA\", 1)) on 0 FROM SALES", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[CA].[Beverly Hills].[Store 6]}\n"
      + "Row #0: 21,333\n");
  }
  ...
  public void testCache() {
    ...
    assertAxisReturns("[Gender].Children", "[Gender].[F]\n" + "[Gender].[M]");
  ...
  /**
   * Test for bug MONDRIAN-296, "Cube getTimeDimension use when Cube has no Time
   * dimension".
   */
  public void testCubeTimeDimensionFails() {
    assertQueryThrows("select LastPeriods(1) on columns from [Store]",
      "'LastPeriods', no time dimension");
    assertQueryThrows("select OpeningPeriod() on columns from [Store]",
      "'OpeningPeriod', no time dimension");
    assertQueryThrows("select OpeningPeriod([Store Type]) on columns from [Store]",
      "'OpeningPeriod', no time dimension");
    assertQueryThrows("select ClosingPeriod() on columns from [Store]",
      "'ClosingPeriod', no time dimension");
    assertQueryThrows("select ClosingPeriod([Store Type]) on columns from [Store]",
      "'ClosingPeriod', no time dimension");
    assertQueryThrows("select ParallelPeriod() on columns from [Store]",
      "'ParallelPeriod', no time dimension");
    assertQueryThrows("select PeriodsToDate() on columns from [Store]",
      "'PeriodsToDate', no time dimension");
    assertQueryThrows("select Mtd() on columns from [Store]",
  ...
  public void testFilterEmpty() {
    ...
    assertAxisReturns("Filter({}, 1=0)", "");
    assertAxisReturns("Filter({[Time].[Time].Children}, 1=0)", "");
  }
  ...
  public void testExistsMembersAll() {
    assertQueryReturns("select exists(\n" + "  {[Customers].[All Customers],\n"
  ...
  /**
   * Executes a query that has a complex parse tree. Goal is to find algorithmic
   * complexity bugs in the validator which would make the query run extremely
   * slowly.
   */
   ...
   *          Max recursion depth
   * @param breadth
   *          Number of iterations at each depth
   */
  private void generateComplex(StringBuilder buf, String indent, int depth,
    int depthLimit, int breadth) {
    ...
    for (int i = 0; i < breadth; i++) {
      if (depth < depthLimit) {
        ...
        generateComplex(buf, indent + "      ", depth + 1, depthLimit, breadth);
        buf.append("},\n");
        buf.append(indent + "      2),\n");
        buf.append(indent + "    1),\n");
      } else {
