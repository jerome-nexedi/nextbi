 ...
 * @author jrand
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/fun/NativizeSetFunDefTest
 *          .java#19 $
 * @since Oct 14, 2009
 */
public class NativizeSetFunDefTest extends BatchTestCase {
  ...
  public void testIsNoOpWithAggregatesTablesOn() {
    ...
    propSaver.set(MondrianProperties.instance().UseAggregates, true);
    checkNotNative("with  member [gender].[agg] as"
      + "  'aggregate({[gender].[gender].members},[measures].[unit sales])'"
      + "select NativizeSet(CrossJoin( " + "{gender.gender.members, gender.agg}, "
  ...
  public void testLevelHierarchyHighCardinality() {
    ...
    String mdx = "select NativizeSet(" + "CrossJoin( " + "gender.gender.members, "
      + "CrossJoin(" + "{ measures.[unit sales] }, " + "[Time].[Month].members"
      + "))) on 0" + "from sales";
    checkNative(mdx);
  }
  ...
  public void testNamedSetLowCardinality() {
    ...
    checkNotNative("with "
      ...
      + "select  nativizeSet([levelMembers]) on 0 " + "from [warehouse and sales]");
  }

  public void testCrossjoinWithNamedSetLowCardinality() {
    propSaver.set(MondrianProperties.instance().NativizeMinThreshold,
      Integer.MAX_VALUE);
    checkNotNative("with " + "set [genderMembers] as 'gender.gender.members'"
  ...
  public void testMeasureInCrossJoinWithTwoDimensions() {
    checkNative("select NativizeSet(" + "CrossJoin( " + "gender.gender.members, "
  ...
  public void testNativeResultLimitAtZero() {
    ...
    String mdx = "with  member [gender].[agg] as"
      + "  'aggregate({[gender].[gender].members},[measures].[unit sales])'"
      + "select NativizeSet(CrossJoin( " + "{gender.gender.members, gender.agg}, "
      + "{[marital status].[marital status].members}" + ")) on 0 from sales";
  ...
  public void testMeasureAndDimensionInCrossJoin() {
    ...
    // the correct result.
    "select NativizeSet(" + "CrossJoin(" + "{ measures.[unit sales] }, "
      + "[marital status].[marital status].members" + ")) on 0" + "from sales");
  }
  ...
  public void testDimensionAndMeasureInCrossJoin() {
    ...
    "select NativizeSet(" + "CrossJoin("
      + "[marital status].[marital status].members, " + "{ measures.[unit sales] }"
      + ")) on 0" + "from sales");
  }
  ...
  public void testAllByAll() {
    ...
    // This test is here to ensure that "NativizeSet" still returns
    // the correct result.
    "select NativizeSet(" + "CrossJoin(" + "{ [gender].[all gender] }, "
      + "{ [marital status].[all marital status] } " + ")) on 0" + "from sales");
  }
  ...
  public void testAllByAllByAll() {
    ...
    // This test is here to ensure that "NativizeSet" still returns
    // the correct result.
    "select NativizeSet(" + "CrossJoin(" + "{ [product].[all products] }, "
      + "CrossJoin(" + "{ [gender].[all gender] }, "
      + "{ [marital status].[all marital status] } " + "))) on 0" + "from sales");
  }

  public void testNativizeTwoAxes() {
    String mdx = "select " + "NativizeSet(" + "CrossJoin("
      + "{ [gender].[gender].members }, "
      + "{ [marital status].[marital status].members } " + ")) on 0,"
      + "NativizeSet(" + "CrossJoin(" + "{ [measures].[unit sales] }, "
    ...
    // evaluated.
    propSaver.set(MondrianProperties.instance().NativizeMinThreshold, 200000);
    checkNotNative(mdx);
  }
  ...
  public void testCurrentMemberAsFunArg() {
    checkNative("with "
      ...
      + "       Aggregate(gender.gender.members), 101010)' " + "select "
      + "NativizeSet(" + "crossjoin(" + "{time.year.members}, " + "crossjoin("
      + "{gender.x}," + "[marital status].[marital status].members" + "))) "
      + "on axis(0) " + "from [warehouse and sales]");
  }
  ...
  public void testOnlyMeasureIsLiteral() {
    ...
    // ////////////////////////////////////////////////////////////////
    "with " + "member [measures].[cog_oqp_int_t1] as '1', solve_order = 65535 "
      + "select NativizeSet(CrossJoin("
      + "   [marital status].[marital status].members, "
      + "   [gender].[gender].members " + ")) on 1, "
      + "{ [measures].[cog_oqp_int_t1] } " + "on 0 " + "from [warehouse and sales]");
  }
  ...
  public void testTwoLiteralMeasuresAndUnitAndStoreSales() {
    ...
    "with " + "member [measures].[cog_oqp_int_t1] as '1', solve_order = 65535 "
      ...
      + "      [marital status].[marital status].members, "
      + "      [gender].[gender].members " + "    ))" + "on 1, " + "{ "
      + "   { [measures].[cog_oqp_int_t1] }, " + "   { [measures].[unit sales] }, "
      + "   { [measures].[cog_oqp_int_t2] }, " + "   { [measures].[store sales] } "
      + "} " + " on 0 " + "from [warehouse and sales]");
  }
  ...
  public void testLagOnMeasures() {
    checkNotNative("with "
      ...
      // ///////////////////////////////////////////
      + "member [measures].[cog_oqp_int_t1] "
      + "   as 'measures.[store sales].lag(1)', solve_order = 65535 " + "select "
  ...
  public void testRangeOfMeasures() {
    checkNotNative("select " + "   NativizeSet(CrossJoin("
      + "      [marital status].[marital status].members, "
      + "      [gender].[gender].members " + "    ))"
      + "on 1, "
      + "{ "
  ...
  public void testOrderOnMeasures() {
    checkNative("with "
      ...
      + "measures.[store sales])', solve_order = 65535 " + "select "
      + "   NativizeSet(CrossJoin("
      + "      [marital status].[marital status].members, "
      + "      [gender].[gender].members " + "   ))" + "on 1, " + "{ "
      + "   measures.[cog_oqp_int_t1]," + "   measures.[unit sales]" + "} "
  ...
  public void testLiteralMeasureAndUnitSalesUsingSet() {
    ...
    "with " // measure will bring in a base cube.
      ...
      + "      [gender].[gender].members " + "    )'"
      + "select "
      + "   NativizeSet([cog_oqp_int_s1])" + "on 1, "
      + "{ "
      + "   [measures].[cog_oqp_int_t1], "
      + "   [measures].[unit sales], "
  ...
  public void testNoSubstitutionsArityTwo() {
    checkNotNative("SELECT NativizeSet(CrossJoin(" + "{Gender.F, Gender.M}, "
      + "{ [Marital Status].M } " + ")) on 0 from sales");
  }

  public void testExplicitCurrentMonth() {
    checkNative("SELECT NativizeSet(CrossJoin( "
      + "   { [Time].[Month].currentmember }, " + "   Gender.Gender.members )) "
      + "on 0 from sales");
  }

  public void disabled_testCalculatedCurrentMonth() {
    checkNative("WITH " + "SET [Current Month] AS 'tail([Time].[month].members, 1)'"
      + "SELECT NativizeSet(CrossJoin( " + "   { [Current Month] }, "
  ...
  public void testAcceptsCrossJoinAsInput() {
    checkNative("SELECT NativizeSet( CrossJoin({ Gender.F, Gender.M }, "
      + "{[Marital Status].[Marital Status].members})) on 0 from sales");
  }
  ...
  public void testRedundantEnumMembersFirst() {
    ...
    "SELECT NativizeSet( CrossJoin("
      ...
      + "  { [Marital Status].S } " + "}," + "CrossJoin( "
      + "{ gender.gender.members }, " + "{ time.quarter.members } "
      + "))) on 0 from sales");
  }
  ...
  public void testRedundantEnumMembersMiddle() {
    ...
    "SELECT NativizeSet( CrossJoin("
      ...
      + "{ { gender.F, gender.M , gender.M}, " + "  { gender.M } " + "}, "
      + "{ time.quarter.members } " + "))) on 0 from sales");
  }
  ...
  public void testRedundantEnumMembersLast() {
    ...
    "SELECT NativizeSet( CrossJoin("
      + "{  [Marital Status].[Marital Status].members }," + "CrossJoin( "
      + "{ gender.gender.members }, " + "{ { time.[1997].Q1, time.[1997].Q2 }, "
      + "  { time.[1997].Q2 } " + "} " + "))) on 0 from sales");
  }
  ...
  public void testRedundantLevelMembersMiddle() {
    ...
    "SELECT NativizeSet( CrossJoin("
      + "{  [Marital Status].[Marital Status].members }," + "CrossJoin( "
      + "{ gender.gender.members, " + "  { gender.gender.members } " + "}, "
      + "{ time.quarter.members } " + "))) on 0 from sales");
  }
  ...
  public void testLevelMembersAndAllWhereOrderMatters() {
    checkNative("select NativizeSet (" + "crossjoin( "
      + "  { gender.gender.members, gender.[all gender] }, "
      + "  { [marital status].S, [marital status].M } " + ")) on 0 from sales");
  }
  ...
  public void testMaritalStatusByGenderMembersAndAgg() {
    checkNative("with member gender.agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( "
      + "  [marital status].[marital status].members, "
      + "  { gender.gender.members, gender.[agg] } " + ")) on 0 from sales");
  }
  ...
  public void testMaritalStatusByGenderAggAndMembers() {
    checkNative("with member gender.agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( "
      + "  [marital status].[marital status].members, "
      + "  { gender.[agg], gender.gender.members } " + ")) on 0 from sales");
  }
  ...
  public void testCrossjoinArgWithMultipleElementTypes() {
    ...
    "with member [gender].agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( " + "{ time.quarter.members }, "
      + "CrossJoin( "
  ...
  public void testProductFamilyMembers() {
    checkNative("select non empty NativizeSet(" + "crossjoin( "
      + "  [product].[product family].members, " + "  { [gender].F } "
      + ")) on 0 from sales");
  }
  ...
  public void testNestedCrossJoinWhereAllColsHaveNative() {
    checkNative("with "
      ...
      + "  [Education Level].[Education Level].members " + "))) on 0 from sales");
  }
  ...
  public void testNestedCrossJoinWhereFirstColumnNonNative() {
    checkNative("with "
      ...
      + "member [marital status].agg as 'Aggregate( [marital status].[marital status].members )' "
      + "select NativizeSet(" + "crossjoin( "
      + "  { gender.[all gender], gender.[agg] }, " + "  crossjoin("
      + "  { [marital status].[marital status].members, [marital status].[agg] },"
      + "  [Education Level].[Education Level].members " + "))) on 0 from sales");
  }
  ...
  public void testNestedCrossJoinWhereMiddleColumnNonNative() {
    checkNative("with "
      ...
      + "select NativizeSet(" + "crossjoin( "
      + "  { [marital status].[marital status].members, [marital status].[agg] },"
      + "  crossjoin(" + "  { gender.[all gender], gender.[agg] }, "
      + "  [Education Level].[Education Level].members " + "))) on 0 from sales");
  }
  ...
  public void testNestedCrossJoinWhereLastColumnNonNative() {
    checkNative("with "
      ...
      + "  { gender.[all gender], gender.[agg] } " + "))) on 0 from sales");
  }
  ...
  public void testGenderAggByMaritalStatus() {
    ...
    "with member gender.agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( " + "  { gender.[agg] }, "
      + "  [marital status].[marital status].members " + ")) on 0 from sales");
  }
  ...
  public void testGenderAggTwiceByMaritalStatus() {
    checkNotNative(
    // NativizeSet removes the crossjoin, so not native
    "with " + "member gender.agg1 as 'Aggregate( { gender.M } )' "
      + "member gender.agg2 as 'Aggregate( { gender.F } )' " + "select NativizeSet("
      + "crossjoin( " + "  { gender.[agg1], gender.[agg2] }, "
      + "  [marital status].[marital status].members " + ")) on 0 from sales");
  }
  ...
  public void testSameGenderAggTwiceByMaritalStatus() {
    ...
    // NativizeSet removes the crossjoin, so not native
    "with " + "member gender.agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( " + "  { gender.[agg], gender.[agg] }, "
      + "  [marital status].[marital status].members " + ")) on 0 from sales");
  }
  ...
  public void testMaritalStatusByGenderAgg() {
    ...
    "with member gender.agg as 'Aggregate( gender.gender.members )' "
      + "select NativizeSet(" + "crossjoin( "
      + "  [marital status].[marital status].members, " + "  { gender.[agg] } "
      + ")) on 0 from sales");
  }
  ...
  public void testMaritalStatusByTwoGenderAggs() {
    ...
    "with " + "member gender.agg1 as 'Aggregate( { gender.M } )' "
      + "member gender.agg2 as 'Aggregate( { gender.F } )' " + "select NativizeSet("
      + "crossjoin( " + "  [marital status].[marital status].members, "
      + "  { gender.[agg1], gender.[agg2] } " + ")) on 0 from sales");
  }

  public void testMaritalStatusBySameGenderAggTwice() {
    checkNotNative(
    // NativizeSet removes the crossjoin, so not native
    "with " + "member gender.agg as 'Aggregate( { gender.M } )' "
      + "select NativizeSet(" + "crossjoin( "
      + "  [marital status].[marital status].members, "
      + "  { gender.[agg], gender.[agg] } " + ")) on 0 from sales");
  }
  ...
  public void testMultipleLevelsOfSameDimInConcatenatedJoins() {
    ...
    // mdx in that test.
    "select NativizeSet( {" + "CrossJoin(" + "  { [Time].[Year].members },"
      + "  { gender.F, gender. M } )," + "CrossJoin("
      + "  { [Time].[Quarter].members }," + "  { gender.F, gender. M } )"
      + "} ) on 0 from sales");
  }
  ...
  public void testDoesNoHarmToPlainEnumeratedMembers() {
    ...
    assertQueryIsReWritten(
      "SELECT NativizeSet({Gender.M,Gender.F}) on 0 from sales", "select "
        + "NativizeSet({[Gender].[M], [Gender].[F]}) " + "ON COLUMNS\n"
        + "from [Sales]\n");
  }
  ...
  public void testTopCountDoesNotGetTransformed() {
    assertQueryIsReWritten(
      "select "
        ...
        + " ) on 0," + "{[Measures].[Unit Sales]} on 1 FROM [Sales]",
  ...
  public void testEvaluationIsNonNativeWhenBelowHighcardThreshoold() {
    ...
    String mdxQuery = "select non empty NativizeSet("
  ...
  public void testCalculatedLevelsDoNotCauseException() {
    String mdx = "SELECT " + "  Nativizeset" + "  (" + "    {"
      + "      [Store].Levels(0).MEMBERS" + "    }" + "  ) ON COLUMNS"
      + " FROM [Sales]";
    checkNotNative(mdx);
  }
  ...
  public void testAxisWithNamedSetArityOneIsNotNativelyEvaluated() {
    checkNotNative("with " + "set [COG_OQP_INT_s1] as "
      + "'Intersect({[Gender].[Gender].Members}, {[Gender].[Gender].[M]})' "
      + "select NON EMPTY " + "NativizeSet([COG_OQP_INT_s1]) ON COLUMNS "
      + "from [Sales]");
  }
  ...
  public void testOneAxisHighAndOneLowGetsNativeEvaluation() {
    ...
    checkNative("select NativizeSet(" + "Crossjoin([Gender].[Gender].members,"
      + "[Marital Status].[Marital Status].members)) on 0," + "NativizeSet("
  ...
  public void disabled_testAggregatesInSparseResultsGetSortedCorrectly() {
    propSaver.set(MondrianProperties.instance().NativizeMinThreshold, 0);
    checkNative("select non empty NativizeSet("
      + "Crossjoin({[Store Type].[Store Type].members,[Store Type].[all store types]},"
      + "{ [Promotion Media].[Media Type].members }" + ")) on 0 from sales");
  }
  ...
  public void testAggregatedCrossjoinWithZeroMembersInNativeList() {
    ...
    checkNative("with"
      ...
      + "select" + " non empty " + " NativizeSet(" + "Crossjoin("
      + "{[Marital Status].[Marital Status].members,[Marital Status].[agg]},"
      + "{[Gender].[Gender].members,[gender].[agg]}" + ")) on 0 " + " from sales "
  ...
  public void testCardinalityQueriesOnlyExecuteOnce() {
    SqlPattern[] patterns = {
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, "select count(*) as \"c0\" "
        + "from (select " + "distinct \"customer\".\"gender\" as \"c0\" "
        + "from \"customer\" \"customer\") \"init\"", 108),
      new SqlPattern(Dialect.DatabaseProduct.ACCESS, "select count(*) as `c0` "
        + "from (select " + "distinct `customer`.`gender` as `c0` "
        + "from `customer` as `customer`) as `init`", 108) };
    String mdxQuery = "select" + " non empty" + " NativizeSet(Crossjoin("
  ...
  public void testSingleLevelDotMembersIsNativelyEvaluated() {
    String mdx1 = "with member [Customers].[agg] as '"
      ...
      + "non empty NativizeSet(" + "Crossjoin({[Gender].[Gender].[M]},"
      + "[Measures].[Unit Sales])) on 1 " + "from Sales";
    String mdx2 = "select non empty NativizeSet({[Customers].[name].members}) on 0,"
      + "non empty NativizeSet(" + "Crossjoin({[Gender].[Gender].[M]},"
      + "[Measures].[Unit Sales])) on 1 " + "from Sales";
    ...
    SqlPattern oraclePattern = new SqlPattern(Dialect.DatabaseProduct.ORACLE, sql,
      sql.length());
  ...
  private static String removeNativize(String mdx) {
    ...
    assertFalse("Query does use NativizeSet", mdx.equals(mdxWithoutNativize));
    return mdxWithoutNativize;
  }

  private void assertQueryIsReWritten(final String query, final String expectedQuery) {
    assertQueryIsReWritten(getTestContext(), query, expectedQuery);
  }

  private void assertQueryIsReWritten(TestContext testContext, final String query,
    final String expectedQuery) {
    String actualOutput = testContext.getConnection().parseQuery(query).toString();
    if (!Util.nl.equals("\n")) {
