/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/MondrianServerTest.java#4 $
 ...
 * @since 2010/11/22
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/MondrianServerTest.java#4
 *          $
 */
public class MondrianServerTest extends TestCase {
  ...
  public void testEmbedded() {
    ...
    final MondrianServer server = MondrianServer.forConnection(testContext
      .getConnection());
  ...
  public void testStringRepository() throws MalformedURLException {
    final MondrianServer server = MondrianServer.createWithRepository(
      new StringRepositoryContentFinder("foo bar"), null);
    final String id = server.getId();
    assertNotNull(id);
    server.shutdown();
  }
  ...
  public void testRepository() throws MalformedURLException, SQLException {
    ...
    final MondrianServer server = MondrianServer.createWithRepository(
      new UrlRepositoryContentFinder("inline:"
        + xmlaTestContext.getDataSourcesString()), null);
    final String id = server.getId();
    assertNotNull(id);
