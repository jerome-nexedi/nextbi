/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/loader/DBLoader.java#27 $
 ...
 * returns an array of Table instances. A Table instance is used to define a
 * database table and its content (its rows or data). A concrete subclass of
 * this class can define how the array of Tables are created (CSV files, XML
 * files, by reading another database, etc.) and then using this class load
 * thoses Tables into a database. In addition, rather than loading a database,
 * this class can be used to generate files containing the sql that can be used
 ...
 * SqlQuery.Dialect) and loading the database itself.</li>
 * <li>
 * JDBC URL: How to connect to the database.</li>
 * <li>
 * User Name: The database user name (optional).</li>
 * <li>
 * Password: The database password (optional).</li>
 * <li>
 ...
 * generated. The names of the files are based upon the table name plus a
 * suffix. Each table has four files for the different SQL operations: drop the
 * table, drop the rows in the table, create the table and load the rows in the
 * table. The suffixes have default values which can be overriden the the value
 * of System properties:
 * <ul>
 * <li>
 * The property "mondrian.test.loader.drop.table.suffix" can be used to define
 * the drop table suffix. The default value is "drop.sql".</li>
 * <li>
 * The property "mondrian.test.loader.drop.table.rows.suffix" can be used to
 * define the drop rows table suffix. The default value is "droprows.sql".</li>
 * <li>
 * The property "mondrian.test.loader.create.table.suffix" can be used to define
 * the create table suffix. The default value is "create.sql".</li>
 * <li>
 * The property "mondrian.test.loader.table.rows.suffix" can be used to define
 * the create table suffix. The default value is "loadrows.sql".</li>
 * </ul>
 * </li>
 * <li>
 * Force: If files are being generated and if they already exist, setting the
 * force flag to true instructs this class to over-write the existing file. If
 * the force flag is false and a file already exists and exception is thrown.</li>
 * </ul>
 * <p>
 ...
 * <ul>
 * <li>
 * DropTable: If true, the table is dropped from the database (Default value is
 * true).</li>
 * <li>
 * DropRows: If true, the rows in the table are dropped (Default value is true).
 * </li>
 * <li>
 * CreateTable: If true, the table is created in the database (Default value is
 * true).</li>
 * <li>
 * LoadRows: If true, the rows are loaded into the table (Default value is
 * true).</li>
 * </ul>
 * <p>
 * The Table.Controller must also have its RowStream object defined. A RowStream
 * produces a set of Row objects (see the Row interface below) which in turn has
 * an array of Objects that represent the values of a row in the Table. The
 * default RowStream is an emtpy RowStrean, no rows. The user must implement the
 * RowStrean interface. One such implementation might be a RowStrean containing
 * a list of rows. In this case, all of the rows would be in memory. Another
 * implementation might read each row's data from a file or another database. In
 * this case, the row data is not in memory allowing one to load much larger
 * tables.
 * <p>
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/loader/DBLoader.java#27 $
 */
public abstract class DBLoader {
  ...
  final static Pattern decimalDataTypeRegex = Pattern
    .compile("DECIMAL\\((.*),(.*)\\)");

  final static Pattern varcharDataTypeRegex = Pattern.compile("VARCHAR\\((.*)\\)");

  final static DecimalFormat integerFormatter = new DecimalFormat(decimalFormat(15,
    0));
  ...
  final static DateFormat dateFormatter = new SimpleDateFormat(dateFormatString);

  /**
   * Generate an appropriate number format string for doubles etc to be used to
  ...
  /**
   * The RowStream interface allows one to load large sets of rows by streaming
   * them in, one does not have to have all of the row data in memory.
   */
   ...
   * <p/>
   * 
   * Specific databases will represent this their own particular physical type,
   * for example "TINYINT(1)", "BOOLEAN" or "BIT"; see
  ...
  public static class Type {
    ...
    public String toPhysical(Dialect dialect) {
      if (this == Integer || this == Decimal || this == Smallint
  ...
  public void initialize() throws Exception {
    ...
    if (this.connection == null) {
      ...
      if (this.userName == null) {
        ...
        this.connection = DriverManager.getConnection(this.jdbcURL, this.userName,
          this.password);
      }
    }
  ...
  protected void generateBeforeActions(Table table) {
    ...
    for (int i = 0; i < dropIndexList.size(); i++) {
      ...
      String dropIndexStmt = "DROP INDEX " + quotedIndexName + " ON "
        + quotedTableName;
  ...
  protected void generateAfterActions(Table table) {
    ...
    for (int i = 0; i < createIndexList.size(); i++) {
      ...
      String createIndexStmt = "CREATE INDEX " + quotedIndexName + " ON "
  ...
  protected boolean makeFileWriter(Table table, String suffix) throws Exception {
    if (this.outputDirectory != null) {
      ...
      if (file.exists()) {
        if (this.force) {
          if (!file.delete()) {
            throw new Exception("Table file \"" + fileName
              + "\" could not be deleted");
          }
        } else {
          throw new Exception("Table file \"" + fileName + "\" already exists"
  ...
  /**
   * Undoes all of the database table creations performed when the load method
   * was called.
   */
  public void clear() {
  }
   ...
   * 
   * <p>
   * Call this method when the load process is finished and the connection is no
  ...
  protected void executeBeforeActions(Table table) throws Exception {
    ...
    try {
      ...
      LOGGER
        .debug("Before Table actions of " + table.getName() + " failed. Ignored");
    } finally {
      closeFileWriter();
    }
  }
    ...
    try {
      ...
      if (controller.loadRows()) {
        ...
        while (it.hasNext()) {
          ...
          String insertStatement = createInsertStatement(table, values);
  ...
  protected String columnValue(Column column, Object value) {
    ...
    if (type == Type.Integer) {
       ...
       */
    } else if ((type == Type.Varchar30) || (type == Type.Varchar255)
      || (type == Type.Varchar60) || typeName.startsWith("VARCHAR(")) {
      if (value instanceof String) {
      ...
      /*
       * Output for a BOOLEAN - TINYINT(1) (MySQL) } else if
       * (columnType.startsWith("TINYINT(1)")) { Boolean result = (Boolean) obj;
       * if (result.booleanValue()) { return "1"; } else { return "0"; }
       */
    }
    throw new RuntimeException("Unknown column type: " + typeName + " for column: "
      + column.getName());
  }
   ...
   */
  protected int writeBatch(String[] batch, int batchSize) throws IOException,
    SQLException {
    if (this.fileWriter != null) {
      ...
      if (batchSize == 1) {
        ...
        if (updates < batchSize) {
          throw new RuntimeException("Failed to execute batch: " + batchSize
            + " versus " + updates);
        }
      }
      if (connection.getMetaData().supportsTransactions()) {
