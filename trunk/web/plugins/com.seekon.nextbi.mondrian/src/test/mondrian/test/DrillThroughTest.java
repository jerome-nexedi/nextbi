/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/DrillThroughTest.java#42 $
...
/**
 * Test generation of SQL to access the fact table data underlying an MDX result
 * set.
 * 
 * @author jhyde
 * @since May 10, 2006
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/DrillThroughTest.java#42
 *          $
 */
public class DrillThroughTest extends FoodMartTestCase {
  ...
  public void testTrivialCalcMemberDrillThrough() {
    Result result = executeQuery("WITH MEMBER [Measures].[Formatted Unit Sales]"
    ...
    assertNotNull(sql);
    expectedSql = "select `time_by_day`.`the_year` as `Year`,"
      + " `product_class`.`product_family` as `Product Family`,"
      + " `sales_fact_1997`.`unit_sales` as `Unit Sales` "
      + "from `time_by_day` =as= `time_by_day`,"
      + " `sales_fact_1997` =as= `sales_fact_1997`,"
  ...
  public void testTrivialCalcMemberNotMeasure() {
    ...
    // First, on ROWS axis.
    Result result = executeQuery("with member [Product].[My Food]\n"
    ...
    // Next, on filter axis.
    result = executeQuery("with member [Product].[My Food]\n"
      + " AS [Product].[Food], FORMAT_STRING = '#,###'\n"
      + "SELECT [Measures].[Unit Sales] * [Gender].[M] on 0,\n"
    ...
    result = executeQuery("with member [Product].[My Food Drink]\n"
  ...
  private String getNameExp(Result result, String hierName, String levelName) {
    ...
    Hierarchy h = cube.lookupHierarchy(
  ...
  public void testDrillThrough2() {
    ...
    String expectedSql = "select `store`.`store_country` as `Store Country`,"
   ...
   * MONDRIAN-180, "Drillthrough fails, if Aggregate in MDX-query"</a>. The
   * problem actually occurs with any calculated member, not just Aggregate. The
   * bug was causing a syntactically invalid constraint to be added to the WHERE
   * clause; after the fix, we do not constrain on the member at all.
   */
  public void testDrillThroughBugMondrian180() {
    Result result = executeQuery("with set [Date Range] as '{[Time].[1997].[Q1], [Time].[1997].[Q2]}'\n"
    ...
    final String expectedSql = "select" + " `store`.`store_state` as `Store State`,"
      + " `store`.`store_city` as `Store City`,"
      + " `store`.`store_name` as `Store Name`,"
      + " `store`.`store_sqft` as `Store Sqft`,"
      + " `store`.`store_type` as `Store Type`,"
  ...
  /**
   * Tests that proper SQL is being generated for a Measure specified as an
   * expression.
   */
  public void testDrillThroughMeasureExp() {
    ...
    final String caseStmt = " \\(case when `sales_fact_1997`.`promotion_id` = 0 then 0"
      + " else `sales_fact_1997`.`store_sales` end\\)";
    switch (dialect.getDatabaseProduct()) {
    case ACCESS:
      expectedSql = expectedSql.replaceAll(caseStmt,
  ...
  public void testDrillThroughDupKeys() {
    /*
     * Note here that the type on the Store Id level is Integer or Numeric. The
     * default, of course, would be String.
     * 
     * For DB2 and Derby, we need the Integer type, otherwise the generated SQL
    ...
    Result result = testContext
      .executeQuery("SELECT {[Store2].[Store Id].Members} on columns,\n"
  ...
  public void testDrillThroughVirtualCube() {
    Result result = executeQuery("select Crossjoin([Customers].[All Customers].[USA].[OR].Children, {[Measures].[Unit Sales]}) ON COLUMNS, "
    ...
    String expectedSql = "select `time_by_day`.`the_year` as `Year`,"
      ...
      + " `time_by_day`.month_of_year` as `Month`,"
      + " `customer`.`state_province` as `State Province`,"
      + " `customer`.`city` as `City`," + " `customer`.`gender` as `Gender`,"
      + " `sales_fact_1997`.`unit_sales` as `Unit Sales`"
      + " from `time_by_day` =as= `time_by_day`,"
      ...
      + " `customer`.`city` = 'Albany' and" + " `customer`.`gender` = 'F'"
      + " order by `time_by_day`.`the_year` ASC," + " `time_by_day`.`quarter` ASC,"
      + " `time_by_day`.`month_of_year` ASC," + " `customer`.`state_province` ASC,"
      + " `customer`.`city` ASC," + " `customer`.`gender` ASC";
   ...
   * 
   * <p>
   * Testcase for bug 1893959, "Generated drill-through columns too long for
   * DBMS".
   * 
   * @throws Exception
   *           on error
  ...
  public void testDrillThroughExprs() {
    assertCanDrillThrough(true, "Sales",
    ...
    // cube
    assertCanDrillThrough(true, "Warehouse and Sales", "2.0");
    assertCanDrillThrough(true, "Warehouse and Sales",
      "[Measures].[Unit Sales] * 2.0");
    // in virtual cube, mixture of measures from two cubes is not drillable
    assertCanDrillThrough(false, "Warehouse and Sales",
    ...
    assertCanDrillThrough(false, "Sales", "Sum([Product].Children)");
    // Sets of members not drillable
    assertCanDrillThrough(false, "Sales",
  ...
  private void assertMaxRows(String firstMaxRow, int expectedCount)
    throws SQLException {
    final ResultSet resultSet = getTestContext()
      .executeStatement(
        "drillthrough\n"
          + firstMaxRow
          + " select\n"
  ...
  public void testDrillthroughNegativeMaxRowsFails() throws SQLException {
    try {
      ...
      TestContext.checkThrowable(e, "Syntax error at line 1, column 22, token '-'");
    }
  }
  ...
  public void testDrillThroughNotDrillableFails() {
    try {
      ...
      TestContext.checkThrowable(e, "Cannot do DrillThrough operation on the cell");
    }
  }
   ...
   * drillability.
   * 
   * @param canDrillThrough
   *          Whether we expect the cell to be drillable
   * @param cubeName
   *          Cube name
   * @param expr
   *          Scalar expression
   */
  private void assertCanDrillThrough(boolean canDrillThrough, String cubeName,
    String expr) {
  ...
  public void testDrillThroughOneAxis() {
    Result result = executeQuery("SELECT [Measures].[Unit Sales] on 0\n"
      + "from Sales");
   ...
   * MONDRIAN-751, "Drill SQL does not include slicer members in WHERE clause".
   */
  public void testDrillThroughCalcMemberInSlicer() {
    Result result = executeQuery("WITH MEMBER [Product].[Aggregate Food Drink] AS \n"
  ...
  public void testDrillThroughMultiPositionCompoundSlicer() {
    ...
    case MYSQL:
      expectedSql = "select sales_fact_1997.unit_sales as Unit Sales from time_by_day as time_by_day, sales_fact_1997 as sales_fact_1997 where sales_fact_1997.time_id = time_by_day.time_id and (((time_by_day.the_year, time_by_day.quarter) in ((1997, 'Q1'), (1997, 'Q2'))))";
      break;
    case ORACLE:
      expectedSql = "select sales_fact_1997.unit_sales as Unit Sales from time_by_day time_by_day, sales_fact_1997 sales_fact_1997 where sales_fact_1997.time_id = time_by_day.time_id and ((time_by_day.quarter = 'Q1' and time_by_day.the_year = 1997) or (time_by_day.quarter = 'Q2' and time_by_day.the_year = 1997))";
      break;
    default:
      return;
    }
    ...
    result = executeQuery("SELECT {[Measures].[Unit Sales]} ON COLUMNS,\n"
      + " {[Product].[All Products]} ON ROWS\n" + "FROM [Sales]\n"
    ...
    switch (getTestContext().getDialect().getDatabaseProduct()) {
    case MYSQL:
      expectedSql = "select customer.gender as Gender, customer.marital_status as Marital Status, sales_fact_1997.unit_sales as Unit Sales from customer as customer, sales_fact_1997 as sales_fact_1997, time_by_day as time_by_day where sales_fact_1997.customer_id = customer.customer_id and customer.gender = 'F' and customer.marital_status = 'M' and sales_fact_1997.time_id = time_by_day.time_id and (((time_by_day.the_year, time_by_day.quarter) in ((1997, 'Q1'), (1997, 'Q2')))) order by customer.gender ASC, customer.marital_status ASC";
      break;
    case ORACLE:
    ...
    switch (getTestContext().getDialect().getDatabaseProduct()) {
    case MYSQL:
      expectedSql = "select sales_fact_1997.unit_sales as Unit Sales from time_by_day as time_by_day, sales_fact_1997 as sales_fact_1997, customer as customer where sales_fact_1997.time_id = time_by_day.time_id and sales_fact_1997.customer_id = customer.customer_id and (((time_by_day.the_year, time_by_day.quarter, customer.gender) in ((1997, 'Q1', 'F'), (1997, 'Q2', 'M'))))";
      break;
    case ORACLE:
    ...
    switch (getTestContext().getDialect().getDatabaseProduct()) {
    case MYSQL:
      expectedSql = "select sales_fact_1997.unit_sales as Unit Sales from time_by_day as time_by_day, sales_fact_1997 as sales_fact_1997 where sales_fact_1997.time_id = time_by_day.time_id and ((time_by_day.quarter = 'Q1' and time_by_day.the_year = 1997) or (time_by_day.month_of_year = 1 and time_by_day.quarter = 'Q1' and time_by_day.the_year = 1997))";
      break;
    case ORACLE:
    ...
    switch (getTestContext().getDialect().getDatabaseProduct()) {
    case MYSQL:
      expectedSql = "select sales_fact_1997.unit_sales as Unit Sales from time_by_day as time_by_day, sales_fact_1997 as sales_fact_1997 where sales_fact_1997.time_id = time_by_day.time_id and ((time_by_day.month_of_year = 1 and time_by_day.quarter = 'Q1' and time_by_day.the_year = 1997) or (time_by_day.quarter = 'Q2' and time_by_day.the_year = 1997))";
      break;
    case ORACLE:
