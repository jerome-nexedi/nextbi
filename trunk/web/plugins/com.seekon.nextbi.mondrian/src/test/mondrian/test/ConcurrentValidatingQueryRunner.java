...
/**
 * Utility class to run set of MDX queries in multiple threads and validate the
 * results. Queries are run against the FoodMart database.
 * 
 * @author Thiyagu, Ajit
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/ConcurrentValidatingQueryRunner
 *          .java#10 $
 */
...
public class ConcurrentValidatingQueryRunner extends Thread {
  ...
  /**
   * Runs concurrent queries without flushing cache. This constructor provides
   * backward compatibilty for usage in {@link ConcurrentMdxTest}.
   * 
   * @param numSeconds
   *          Running time
   * @param useRandomQuery
   *          If set to <code>true</code>, the runner will pick a random query
   *          from the set. If set to <code>false</code>, the runner will circle
   *          through queries sequentially
   * @param queriesAndResults
   *          The array of pairs of query and expected result
   */
  public ConcurrentValidatingQueryRunner(int numSeconds, boolean useRandomQuery,
   ...
   *          from the set. If set to <code>false</code>, the runner will circle
   *          through queries sequentially
   * @param randomCacheFlush
   *          If set to <code>true</code>, the runner will do a coin toss before
   *          running the query. If the result of the experiment is favorable,
   *          runner will flush a random region of aggregation cache
   * @param queriesAndResults
   *          The array of pairs of query and expected result
   */
  public ConcurrentValidatingQueryRunner(int numSeconds, boolean useRandomQuery,
    boolean randomCacheFlush, FoodMartTestCase.QueryAndResult[] queriesAndResults) {
    this.mdxQueries = queriesAndResults;
    mRunTime = numSeconds * 1000;
    mRandomQueries = useRandomQuery;
  ...
  /**
   * Runs a number of queries until time expires. For each iteration, if cache
   * is to be flushed, do it before running the query.
   */
  public void run() {
    ...
    try {
      ...
      while (System.currentTimeMillis() - mStartTime < mRunTime) {
        try {
          ...
          synchronized (lock) {
            concurrentMdxTest.assertQueryReturns(mdxQueries[queryIndex].query,
              mdxQueries[queryIndex].result);
            mSuccessCount++;
          }
        } catch (Exception e) {
          mExceptions.add(new Exception("Exception occurred in iteration "
            + mRunCount + " of thread " + Thread.currentThread().getName(), e));
        }
      }
  ...
  private void report(PrintStream out) {
    String message = MessageFormat.format(
      " {0} ran {1} queries, {2} successfully in {3} milliseconds", threadName,
      mRunCount, mSuccessCount, mStopTime - mStartTime);
  ...
  /**
   * Creates and runs concurrent threads of tests without flushing cache. This
   * method provides backward compatibilty for usage in
   ...
   */
  static List<Throwable> runTest(int numThreads, int runTimeInSeconds,
    boolean randomQueries, boolean printReport,
    FoodMartTestCase.QueryAndResult[] queriesAndResults) {
    return runTest(numThreads, runTimeInSeconds, randomQueries, false, printReport,
      queriesAndResults);
  }
   ...
   *          Whether to pick queries in random or in sequence
   * @param randomCacheFlush
   *          Whether to flush cache before running queries
   * @param printReport
   *          Whether to print report
   * @param queriesAndResults
   *          Array of pairs of query and expected result
   ...
   */
  static List<Throwable> runTest(int numThreads, int runTimeInSeconds,
    boolean randomQueries, boolean randomCacheFlush, boolean printReport,
    FoodMartTestCase.QueryAndResult[] queriesAndResults) {
    ...
    for (int idx = 0; idx < runners.length; idx++) {
      runners[idx] = new ConcurrentValidatingQueryRunner(runTimeInSeconds,
        randomQueries, randomCacheFlush, queriesAndResults);
    }
  ...
  private void flushSchema() {
    Connection connection = concurrentMdxTest.getConnection();
    CacheControl cacheControl = connection.getCacheControl(null);

    Cube salesCube = connection.getSchema().lookupCube("Sales", true);
    CacheControl.CellRegion measuresRegion = cacheControl
      .createMeasuresRegion(salesCube);
    cacheControl.flush(measuresRegion);

    Cube whsalesCube = connection.getSchema()
      .lookupCube("Warehouse and Sales", true);
  ...
  private void flushRandomRegionOfCache() {
    ...
    try {
      ...
      Member memberTime97 = schemaReader.getMemberByUniqueName(tid.getSegments(),
        false);
      CacheControl.CellRegion regionTime97 = cacheControl.createMemberRegion(
        memberTime97, true);

      String[] states = { "CA", "OR", "WA" };
      int idx = (int) (Math.random() * states.length);

      String[] ssegments = new String[] { "Customers", "All Customers", "USA",
        states[idx] };
      Id sid = new Id(Id.Segment.toList(ssegments));

      Member memberCustomerState = schemaReader.getMemberByUniqueName(sid
        .getSegments(), false);
      CacheControl.CellRegion regionCustomerState = cacheControl.createMemberRegion(
        memberCustomerState, true);

      CacheControl.CellRegion region97State = cacheControl.createCrossjoinRegion(
        measuresRegion, regionTime97, regionCustomerState);
