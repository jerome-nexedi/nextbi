/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/TupleListTest.java#1 $
 ...
 * 
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/TupleListTest.java#1
 *          $
 * @author jhyde
 */
public class TupleListTest extends FoodMartTestCase {
  ...
  public void testDelegatingTupleList() {
    ...
    assertEquals("[[[Gender].[F], [Store].[USA]], [[Gender].[M], [Store].[USA]]]",
      fm.toString());
  ...
  private void checkProject(TupleList fm) {
    ...
    assertEquals("[[[Gender].[F]], [[Gender].[M]]]", fm.project(new int[] { 0 })
      .toString());
    assertEquals("[[[Store].[USA]], [[Store].[USA]]]", fm.project(new int[] { 1 })
      .toString());
  ...
  private Member xxx(String memberName) {
    ...
    return schemaReader
      .getMemberByUniqueName(Util.parseIdentifier(memberName), true);
  }
}
