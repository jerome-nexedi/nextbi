/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/CompatibilityTest.java#37 $
...
/**
 * <code>CompatibilityTest</code> is a test case which tests MDX syntax
 * compatibility with Microsoft and SAS servers. There is no MDX spec document
 * per se, so compatibility with de-facto standards from the major vendors is
 * important. Uses the FoodMart database.
 * 
 * @author sasebb
 * @since March 30, 2005
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/CompatibilityTest.java#37
 *          $
 */
public class CompatibilityTest extends FoodMartTestCase {
  ...
  public void testReservedWord() {
    ...
    assertQueryReturns("with member [Measures].[ordinal] as '1'\n"
  ...
  private void checkSolveOrder(String keyword) {
    assertQueryReturns("WITH\n" + "   MEMBER [Store].[StoreCalc] as '0', " + keyword
  ...
  /**
   * Hierarchy names of the form [Dim].[Hier], [Dim.Hier], and Dim.Hier are
   * accepted.
   */
  public void testHierarchyNames() {
    checkAxis("[Customers].[All Customers]", "[Customers].[All Customers]");
    checkAxis("[Customers].[All Customers]",
  ...
  /**
   * Tests that a #null member on a Hiearchy Level of type String can still be
   * looked up when case sensitive is off.
   */
  ...
  /**
   * Tests that data in Hierarchy.Level attribute "nameColumn" can be null. This
   * will map to the #null memeber.
   */
  public void testNullNameColumn() {
    ...
    testContext.assertQueryReturns("select {" + "[Alternative Promotion].[#null], "
  ...
  /**
   * Tests that NULL values sort last on all platforms. On some platforms, such
   * as MySQL, NULLs naturally come before other values, so we have to generate
   * a modified ORDER BY clause.
   */
   ...
   * 
   * <p>
   * The test does not alter this property: for testing coverage, we assume that
   * you run the test once with mondrian.olap.case.sensitive=true, and once with
  ...
  private void assertAxisWithDimensionPrefix(boolean prefixNeeded) {
    propSaver.set(props.NeedDimensionPrefix, prefixNeeded);
