/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/MultipleHierarchyTest.java#19 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/MultipleHierarchyTest
 *          .java#19 $
 * @since Dec 15, 2005
 */
public class MultipleHierarchyTest extends FoodMartTestCase {
  ...
  private static final String timeTime = TestContext.hierarchyName("Time", "Time");
  ...
  public void testWeekly() {
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      ...
      assertAxisReturns("{[Time].[Time].CurrentMember}", "[Time].[1997]");
      assertAxisReturns("{[Time].[Weekly].CurrentMember}",
  ...
  public void testMembersOfHierarchiesInSameDimensionInSlicer() {
    assertQueryReturns(
      "select {[Measures].[Unit Sales]} on columns,\n"
        + " {[Store].children} on rows\n" + "from [Sales]\n"
        + "where ([Gender].[M], " + TestContext.hierarchyName("Time", "Weekly")
        + ".[1997], [Time].[1997].[Q1])", "Axis #0:\n"
  ...
  public void testCalcMember() {
    ...
    assertQueryReturns("with member [Measures].[Sales to Date] as \n"
      + " ' Sum(PeriodsToDate(" + timeWeekly + ".[Year], " + timeWeekly
      + ".CurrentMember), [Measures].[Unit Sales])'\n"
      + "select {[Measures].[Sales to Date]} on columns,\n" + " {" + timeWeekly
      + ".[1997].[14] : " + timeWeekly + ".[1997].[16]} on rows\n" + "from [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Sales to Date]}\n"
        + "Axis #2:\n" + "{[Time].[Weekly].[1997].[14]}\n"
        + "{[Time].[Weekly].[1997].[15]}\n" + "{[Time].[Weekly].[1997].[16]}\n"
        + "Row #0: 81,670\n" + "Row #1: 86,300\n" + "Row #2: 90,139\n");
  ...
  /**
   * Tests <a href="http://jira.pentaho.com/browse/MONDRIAN-191"> bug
   * MONDRIAN-191, "Properties not working with multiple hierarchies"</a>.
   */
  public void testProperty() {
    ...
    testContext.assertQueryReturns("with member [Measures].[Store level] as '"
      + nuStore + ".CurrentMember.Level.Name'\n"
      ...
      + "[Measures].[Store Sales], " + "[Measures].[Store level], "
      + "[Measures].[Store type], " + "[Measures].[Store Sqft]" + "} ON COLUMNS,\n"
      + "{" + nuStore + ".[All NuStores], " + nuStore + ".[Canada], " + nuStore
      + ".[Canada].[BC], " + nuStore + ".[Canada].[BC].[Vancouver], " + nuStore
      + ".[Canada].[BC].[Vancouver].[Store 19], " + nuStore
      + ".[Canada].[BC].[Victoria], " + nuStore + ".[Mexico], " + nuStore + ".[USA]"
   ...
   * at compile time. (SSAS detects at run time, and generates a cell error, but
   * this is better.)
   */
  public void testAmbiguousHierarchyInCalcMember() {
    final String query = "with member [Measures].[Time Child Count] as\n"
  ...
  /**
   * Tests <a href="http://jira.pentaho.com/browse/MONDRIAN-750"> bug
   * MONDRIAN-750, "... multiple hierarchies beneath a single dimension throws
   * exception"</a>.
   */
  public void testDefaultNamedHierarchy() {
    ...
    testContext.assertQueryReturns(
      "with set [*NATIVE_CJ_SET] as '[*BASE_MEMBERS_NuStore]' "
        + "set [*SORTED_ROW_AXIS] as 'Order([*CJ_ROW_AXIS], " + nuStore
        + ".CurrentMember.OrderKey, BASC)' " + "set [*BASE_MEMBERS_NuStore] as '"
        + nuStore + ".[NuStore Country].Members' "
        + "set [*BASE_MEMBERS_Measures] as '{[Measures].[*ZERO]}' "
        + "set [*CJ_ROW_AXIS] as 'Generate([*NATIVE_CJ_SET], {" + nuStore
        + ".CurrentMember})' " + "set [*CJ_COL_AXIS] as '[*NATIVE_CJ_SET]' "
        + "member [Measures].[*ZERO] as '0.0', SOLVE_ORDER = 0.0 "
        + "select [*BASE_MEMBERS_Measures] ON COLUMNS, "
        + "[*SORTED_ROW_AXIS] ON ROWS " + "from [Sales]", "Axis #0:\n" + "{}\n"
        + "Axis #1:\n" + "{[Measures].[*ZERO]}\n" + "Axis #2:\n"
        + "{[NuStore].[Canada]}\n" + "{[NuStore].[Mexico]}\n"
        + "{[NuStore].[USA]}\n" + "Row #0: 0\n" + "Row #1: 0\n" + "Row #2: 0\n");
  }
