/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/UdfTest.java#53 $
...
/**
 * Unit-test for {@link UserDefinedFunction user-defined functions}. Also tests
 * {@link mondrian.spi.CellFormatter cell formatters} and
 * {@link mondrian.spi.MemberFormatter member formatters}.
 * 
 * <p>
 * TODO: 1. test that function which does not return a name, description etc.
 * gets a sensible error 2. document UDFs
 * 
 * @author jhyde
 * @since Apr 29, 2005
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/UdfTest.java#53 $
 */
public class UdfTest extends FoodMartTestCase {
   ...
   */
  private final TestContext tc = udfTestContext("<UserDefinedFunction name=\"PlusOne\" className=\""
  ...
  private TestContext measureTestContext(String xmlMeasure) {
    return TestContext.instance().createSubstitutingCube("Sales", null, xmlMeasure,
      null, null);
  }
  ...
  private TestContext calcMemberTestContext(String xmlCalcMember) {
    return TestContext.instance().createSubstitutingCube("Sales", null, null,
      xmlCalcMember, null);
  }
   ...
   * generates a cache miss, and the next iteration reads precisely one cell. So
   * the query soon exceeds the {@link MondrianProperties#MaxEvalDepth}
   * property.
   */
  ...
  public void testBadFun() {
    final TestContext tc = udfTestContext("<UserDefinedFunction name=\"BadPlusOne\" className=\""
  ...
  public void testGenericFun() {
    final TestContext tc = udfTestContext("<UserDefinedFunction name=\"GenericPlusOne\" className=\""
  ...
  public void testException() {
    ...
    getTestContext().assertMatchesVerbose(
      Pattern
        .compile("(?s).*Invalid value for inverse normal distribution: 1.4708.*"),
      cell.getValue().toString());
    cell = result.getCell(new int[] { 0, 5 });
    assertTrue(cell.isError());
    getTestContext().assertMatchesVerbose(
      Pattern
        .compile("(?s).*Invalid value for inverse normal distribution: 1.4435.*"),
      cell.getValue().toString());
  }
  ...
  public void testCurrentDateString() {
    ...
    String expected = dateString.substring(0, 11)
      + dateString.substring(dateString.length() - 4);
    assertEquals(expected, actual);
  }

  public void testCurrentDateMemberBefore() {
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[yyyy]\\.[Qq]\\.[m]\", BEFORE)} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Time].[1998].[Q4].[12]}\n"
        + "Row #0: \n");
  }
  ...
  public void testCurrentDateMemberBeforeUsingQuotes() {
    assertAxisReturns(
      MondrianProperties.instance().SsasCompatibleNaming.get() ? "CurrentDateMember([Time].[Time], "
  ...
  public void testCurrentDateMemberAfter() {
    ...
    // FoodMart is from '98
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[yyyy]\\.[Qq]\\.[m]\", AFTER)} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n");
  }
  ...
  public void testCurrentDateMemberNoFindArg() {
    ...
    // FoodMart is from '98
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[yyyy]\\.[Qq]\\.[m]\")} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n");
  }

  public void testCurrentDateMemberHierarchy() {
    final String query = MondrianProperties.instance().SsasCompatibleNaming.get() ? "SELECT { CurrentDateMember([Time.Weekly], "
  ...
  public void testCurrentDateMemberHierarchyNullReturn() {
    ...
    // than a dimension
    assertQueryReturns("SELECT { CurrentDateMember([Time.Weekly], "
      + "\"[Ti\\me]\\.[yyyy]\\.[Qq]\\.[m]\")} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n");
  }
  ...
  public void testCurrentDateMemberRealAfter() {
    ...
    // after logic
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[1996]\\.[Q4]\", after)} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Time].[1997].[Q1]}\n"
        + "Row #0: 66,291\n");
  }
  ...
  public void testCurrentDateMemberRealExact1() {
    ...
    // exact logic
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[1997]\")} " + "ON COLUMNS FROM [Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Time].[1997]}\n" + "Row #0: 266,773\n");
  }
  ...
  public void testCurrentDateMemberRealExact2() {
    ...
    // exact logic
    assertQueryReturns("SELECT { CurrentDateMember([Time].[Time], "
      + "\"[Ti\\me]\\.[1997]\\.[Q2]\\.[5]\")} " + "ON COLUMNS FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Time].[1997].[Q2].[5]}\n"
        + "Row #0: 21,081\n");
  }
  ...
  public void testNotMatches() {
    assertQueryReturns("SELECT {[Measures].[Store Sales]} ON COLUMNS, "
      + "Filter({[Store Type].MEMBERS}, "
      + "[Store Type].CurrentMember.Name NOT MATCHES " + "'.*Grocery.*') ON ROWS "
      + "FROM [Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testIn() {
    assertQueryReturns("SELECT {[Measures].[Unit Sales]} ON COLUMNS, "
      ...
      + "[Product].[All Products].lastChild}) ON ROWS " + "FROM [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
  ...
  public void testChildMemberIn() {
    assertQueryReturns("SELECT {[Measures].[Store Sales]} ON COLUMNS, "
      + "{[Store].[Store Name].MEMBERS} ON ROWS " + "FROM [Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Measures].[Store Sales]}\n" + "Axis #2:\n"
    ...
    assertQueryReturns("SELECT {[Measures].[Store Sales]} ON COLUMNS, "
      ...
      + "{[Store].[All Stores].[Mexico], " + "[Store].[All Stores].[USA]}) ON ROWS "
      + "FROM [Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Store Sales]}\n" + "Axis #2:\n");
  }

  /**
   * Tests that the inferred return type is correct for a UDF whose return type
   * is not the same as would be guessed by the default implementation of
   * {@link mondrian.olap.fun.FunDefBase#getResultType}, which simply guesses
   * based on the type of the first argument.
   */
  public void testNonGuessableReturnType() {
    TestContext tc = udfTestContext("<UserDefinedFunction name=\"StringMult\" className=\""
  ...
  /**
   * Tests a UDF whose return type is not the same as its first parameter. The
   * return type needs to have full dimensional information; in this case,
   * HierarchyType(dimension=Time, hierarchy=unknown).
   * 
   * <p>
   * Also tests applying a UDF to arguments of coercible type. In this case,
   * applies f(member,dimension) to args(member,hierarchy).
   */
  public void testAnotherMemberFun() {
    final TestContext tc = udfTestContext("<UserDefinedFunction name=\"PlusOne\" className=\""
      + PlusOneUdf.class.getName()
      + "\"/>\n"
  ...
  public void testCachingCurrentDate() {
    assertQueryReturns("SELECT {filter([Time].[Month].Members, "
      + "[Time].[Time].CurrentMember in {CurrentDateMember([Time]"
      + ".[Time], '[\"Time\"]\\.[yyyy]\\.[\"Q\"q]\\.[m]', "
      + "BEFORE)})} ON COLUMNS " + "from [Sales]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Time].[1998].[Q4].[12]}\n" + "Row #0: \n");
  }
   ...
   * <p>
   * Test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-588">MONDRIAN-588,
   * "UDF returning List works under 2.4, fails under 3.1.1"</a>.
   * 
   * <p>
   * Also test case for bug <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-589">MONDRIAN-589, "UDF
   * expecting List gets anonymous mondrian.rolap.RolapNamedSetEvaluator$1
   * instead"</a>.
   */
   ...
   * 
   * @param functionClass
   *          Class that implements the "Reverse" function.
   */
  private void checkListUdf(final Class<? extends ReverseFunction> functionClass) {
    TestContext tc = udfTestContext("<UserDefinedFunction name=\"Reverse\" className=\""
    ...
    tc.assertQueryReturns("with set [Foo] as Reverse([Gender].Members)\n"
      + "select [Foo] on 0\n" + "from [Sales]", expectedResult);
    // UDF applied to calc set -- exhibited MONDRIAN-589
    tc.assertQueryReturns("with set [Foo] as [Gender].Members\n"
  ...
  public void testNonStaticUdfFails() {
    ...
    TestContext tc = udfTestContext("<UserDefinedFunction name=\"Reverse2\" className=\""
  ...
  public void testMemberUdfDoesNotEvaluateToScalar() {
    TestContext tc = udfTestContext("<UserDefinedFunction name=\"MemberName\" className=\""
  ...
  public void testUdfNeitherScriptNorClassname() {
    ...
    tc.assertQueryThrows("select from [Sales]",
      "Must specify either className attribute or Script element");
  }

  /**
   * Unit test that ensures that a UDF does not have both a script and a
   * className.
   */
  ...
  public void testScriptUdfFactorial() {
    ...
    TestContext tc = udfTestContext("<UserDefinedFunction name='Factorial'>\n"
      ...
      + "  </Script>\n" + "</UserDefinedFunction>\n");
    tc.assertExprReturns("Factorial(4 + 2)", "720");
  }
  ...
  public void testScriptUdfInvalid() {
    ...
    getTestContext().assertMatchesVerbose(
      Pattern.compile("(?s).*ReferenceError: \"factorial_xx\" is not defined..*"),
      cell.getValue().toString());
  }
  ...
  public void testCellFormatter() {
    ...
    // is ignored.
    TestContext tc = measureTestContext("<Measure name='Unit Sales Foo Bar' column='unit_sales'\n"
  ...
  /**
   * As {@link #testCellFormatter()}, but using new-style nested CellFormatter
   * element.
   */
  ...
  /**
   * Unit test for a cell formatter defined against a calculated member, using
   * the old syntax (a member property called "CELL_FORMATTER").
   */
  ...
  /**
   * Unit test for a cell formatter defined against a calculated member, using
   * the new syntax (a nested CellFormatter element).
   */
  public void testCellFormatterOnCalcMemberNested() {
    TestContext tc = calcMemberTestContext("<CalculatedMember\n"
      + "  name='Unit Sales Foo Bar'\n" + "      dimension='Measures'>\n"
      + "  <Formula>[Measures].[Unit Sales]</Formula>\n"
      + "  <CellFormatter className='" + FooBarCellFormatter.class.getName()
      + "'/>\n" + "</CalculatedMember>");
    tc.assertQueryReturns("select {[Measures].[Unit Sales],\n"
      + "      [Measures].[Unit Sales Foo Bar]} on 0\n" + "from [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
  ...
  /**
   * Unit test for a cell formatter defined against a calculated member, using a
   * script.
   */
    ...
    TestContext tc = TestContext
      ...
      .createSubstitutingCube(
        ...
        "  <Dimension name='Promotion Media2' foreignKey='promotion_id'>\n"
          ...
          + "  </Dimension>");
    tc.assertExprReturns("[Promotion Media2].FirstChild.Caption", "fooBulk Mailbar");
  }
  ...
  /**
   * Unit test for a property formatter defined in the old way -- a 'formatter'
    ...
    TestContext tc = TestContext
      ...
      .createSubstitutingCube(
        ...
        "<Dimension name='Promotions2' foreignKey='promotion_id'>\n"
          ...
          + "  </Hierarchy>\n" + "</Dimension>");
    final CellSet result = tc
      .executeOlap4jQuery("select [Promotions2].Children on 0\n" + "from [Sales]");
    final org.olap4j.metadata.Member member = result.getAxes().get(0).getPositions()
      .get(0).getMembers().get(0);
    final Property property = member.getProperties().get("Medium");
    assertEquals("foo0/Medium/No Mediabar", member
  ...
  /**
   * A user-defined function which, depending on its given name, either adds one
   * to, or subtracts one from, its argument.
   */
  public static class PlusOrMinusOneUdf implements UserDefinedFunction {
    private final String name;

    public PlusOrMinusOneUdf(String name) {
      if (!(name.equals("GenericPlusOne") || name.equals("GenericMinusOne"))) {
    ...
    public String getDescription() {
      return "A user-defined function which, depending on its given name, "
  ...
  /**
   * The "TimesString" user-defined function. We wanted a function whose actual
   * return type (string) is not the same as the guessed return type (integer).
   */
  ...
  /**
   * Cell formatter for test purposes. Returns value of the cell prefixed with
   * "foo" and suffixed with "bar".
   */
  ...
  public static class FooBarPropertyFormatter implements PropertyFormatter {
    public String formatProperty(Member member, String propertyName,
      ...
      return "foo" + member.getName() + "/" + propertyName + "/" + propertyValue
        + "bar";
    }
  }
}
