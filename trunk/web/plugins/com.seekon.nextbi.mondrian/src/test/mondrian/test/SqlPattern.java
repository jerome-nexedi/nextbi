/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/SqlPattern.java#8 $
 ...
 * 
 * <p>
 * A pattern contains a dialect. This allows a test to run against different
 * dialects.
 * 
 * @see mondrian.spi.Dialect
 * 
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/SqlPattern.java#8 $
 * @author jhyde
 */
public class SqlPattern {
  ...
  /**
   * Creates a pattern which applies to a collection of dialects and is
   * triggered by the first N characters of the expected statement.
   * 
   * @param databaseProducts
   *          Set of dialects
   * @param sql
   *          SQL statement
   * @param startsWithLen
   ...
   */
  public SqlPattern(Set<Dialect.DatabaseProduct> databaseProducts, String sql,
    int startsWithLen) {
    this(databaseProducts, sql, sql.substring(0, startsWithLen));
  }

  /**
   * Creates a pattern which applies to one or more dialects and is triggered by
   * the first N characters of the expected statement.
   * 
   ...
   *          Length of prefix of statement to consider
   */
  public SqlPattern(Dialect.DatabaseProduct databaseProduct, final String sql,
   ...
   *          SQL statement
   * @param triggerSql
   *          Prefix of SQL statement which triggers a match; null means whole
   *          statement
   */
  public SqlPattern(Dialect.DatabaseProduct databaseProduct, final String sql,
