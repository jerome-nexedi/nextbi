/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/PropertiesTest.java#18 $
 ...
 * @since 5 July, 2005
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/PropertiesTest.java#18 $
 */
public class PropertiesTest extends FoodMartTestCase {
  ...
  public void testMandatoryMemberProperties() {
    ...
    Member member = scr.getMemberByUniqueName(Id.Segment.toList("Customers",
      "All Customers", "USA", "CA"), true);
    ...
    if (false) {
      ...
      assertEquals(Integer.valueOf(scr.getMemberChildren(member).size()),
        intPropValue);
    }

    intPropValue = (Integer) member.getPropertyValue("PARENT_LEVEL");
    assertEquals(Integer.valueOf(member.getParentMember().getLevel().getDepth()),
      intPropValue);
  ...
  public void testGetChildCardinalityPropertyValue() {
    ...
    Member memberForCardinalityTest = scr.getMemberByUniqueName(Id.Segment.toList(
      "Marital Status", "All Marital Status"), true);
    Integer intPropValue = (Integer) memberForCardinalityTest
      .getPropertyValue("CHILDREN_CARDINALITY");
    assertEquals(Integer.valueOf(111), intPropValue);
  }

  /**
   * Tests the ability of MDX parser to pass requested member properties to
   * Result object.
   */
  public void testPropertiesMDX() {
    ...
    String[] props = { "CATALOG_NAME", "SCHEMA_NAME", "CUBE_NAME",
      ...
      "PARENT_LEVEL", "PARENT_UNIQUE_NAME", "PARENT_COUNT", "DESCRIPTION" };
  ...
  public void testMandatoryCellProperties() {
    ...
    assertEquals(135215.0, ((Number) cell.getPropertyValue("VALUE")).doubleValue(),
      0.1);
    ...
    if (MondrianProperties.instance().CaseSensitive.get()) {
      ...
      assertEquals("135,215", cell.getPropertyValue("formatted_value"));
      assertEquals(0, cell.getPropertyValue("solve_order"));
      assertEquals(135215.0,
        ((Number) cell.getPropertyValue("value")).doubleValue(), 0.1);
    }
  }
}
