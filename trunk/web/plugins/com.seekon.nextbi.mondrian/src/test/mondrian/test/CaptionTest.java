/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/CaptionTest.java#19 $
 ...
 * 
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/CaptionTest.java#19
 *          $
 * @author hhaas
 */
public class CaptionTest extends TestCase {
  ...
  public void testMeasureCaption() {
    final Connection monConnection = TestContext.instance().withSchemaProcessor(
      MyFoodmart.class).getConnection();
    String mdxQuery = "SELECT {[Measures].[Unit Sales]} ON COLUMNS, "
  ...
  public void testDimCaptionShared() {
    String mdxQuery = "SELECT {[Measures].[Unit Sales]} ON COLUMNS, "
      + "{[Store Size in SQFT].[All Store Size in SQFTs]} ON ROWS " + "FROM [Sales]";
    final Connection monConnection = TestContext.instance().withSchemaProcessor(
      MyFoodmart.class).getConnection();
    mondrian.olap.Query monQuery = monConnection.parseQuery(mdxQuery);
    mondrian.olap.Result monResult = monConnection.execute(monQuery);
  ...
  /**
   * Tests the &lt;CaptionExpression&gt; element. The caption for [Time].[1997]
   * should be "1997-12-31".
   * 
   * <p>
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-236">Bug
   * MONDRIAN-683,
  ...
  public void testLevelCaptionExpression() {
    ...
    final Connection monConnection = tc.withSchemaProcessor(MyFoodmart.class)
      .getConnection();
    String mdxQuery = "SELECT {[Measures].[Unit Sales]} ON COLUMNS, "
  ...
  public static class MyFoodmart implements DynamicSchemaProcessor {
    final static String foodmart = "<?xml version=\"1.0\"?>\n"
    ...
    public String processSchema(String schemaUrl, Util.PropertyList properties)
      throws Exception {
      return foodmart;
    }
  }
