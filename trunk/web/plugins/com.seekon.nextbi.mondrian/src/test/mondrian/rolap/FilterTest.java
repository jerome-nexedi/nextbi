 ...
 * @author Rushan Chen
 * @since April 28, 2009
 * @version $Id: //open/mondrian/testsrc/main/mondrian/rolap/FilterTest.java#10
 *          $
 */
public class FilterTest extends BatchTestCase {
  ...
  public void testInFilterSimple() throws Exception {
    ...
    String query = "With "
      ...
      + "Non Empty [*CJ_ROW_AXIS] on rows " + "From [Sales]";
  ...
  /**
   * Here the filter is above (rather than as inputs to) the NECJ. These types
   * of filters are currently not natively evaluated.
   * 
   * <p>
   * To expand on this case, RolapNativeFilter needs to be improved so it knows
   * how to represent the dimension filter constraint. Currently the
  ...
  /**
   * Test that if Null member is not explicitly excluded, then the native filter
   * SQL should not filter out null members.
   * 
   * @throws Exception
   */
  public void testNotInFilterKeepNullMember() throws Exception {
    ...
    String query = "With "
      ...
      + "Select " + "{[Measures].[Customer Count]} on columns, "
      + "Non Empty [*ORDERED_CJ_ROW_AXIS] on rows " + "From [Sales]";
  ...
  /**
   * Test that if Null member is explicitly excluded, then the native filter SQL
  ...
  /**
   * Test that null members are included when the filter excludes members that
   * contain multiple levels, but none being null.
   */
  public void testNotInMultiLevelMemberConstraintNonNullParent() {
    ...
    String necjSqlDerby = "select \"customer\".\"country\", \"time_by_day\".\"the_year\", "
    ...
    String necjSqlMySql = "select `customer`.`country` as `c0`, `time_by_day`.`the_year` as `c1`, "
  ...
  /**
   * Test that null members are included when the filter excludes members that
   * contain multiple levels, but none being null. The members have the same
   * parent.
   */
  public void testNotInMultiLevelMemberConstraintNonNullSameParent() {
    ...
    String necjSqlMySql = "select `customer`.`country` as `c0`, `time_by_day`.`the_year` as "
  ...
  public void testNotInMultiLevelMemberConstraintMixedNullNonNullParent() {
    ...
    String necjSqlDerby = "select \"warehouse\".\"warehouse_fax\", \"warehouse\".\"wa_address1\", "
  ...
  /**
   * Test that null members are included when the filter explicitly excludes a
   * single member that has a null. The members span multiple levels.
   */
  public void testNotInMultiLevelMemberConstraintSingleNullParent() {
    ...
    String necjSqlMySql = "select `warehouse`.`warehouse_fax` as `c0`, "
  ...
  public void testNativeFilter() {
    ...
    checkNative(
      ...
      "select {[Measures].[Store Sales]} ON COLUMNS, "
        ...
        + "from [Sales] " + "where ([Time].[1997])", null, requestFreshConnection);
  }
  ...
  public void testCmNativeFilter() {
    ...
    checkNative(
      ...
      "with member [Measures].[Rendite] as '([Measures].[Store Sales] - [Measures].[Store Cost]) / [Measures].[Store Cost]' "
        ...
        + "from [Sales] " + "where ([Store].[All Stores].[USA].[CA], [Time].[1997])",
      "Axis #0:\n"
  ...
  public void testNativeFilterNonEmpty() {
    ...
    // is not refreshed for this parameter.
    boolean requestFreshConnection = true;
    checkNative(0, 20, "select Filter(CrossJoin([Store].[Store Name].members, "
      + "                        "
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-706">bug
   * MONDRIAN-706, "SQL using hierarchy attribute 'Column Name' instead of
   * 'Column' in the filter"</a>.
   */
  public void testBugMondrian706() {
    ...
    final SqlPattern[] badPatterns = { new SqlPattern(Dialect.DatabaseProduct.MYSQL,
      badMysqlSQL, null) };
    final SqlPattern[] goodPatterns = { new SqlPattern(
      Dialect.DatabaseProduct.MYSQL, goodMysqlSQL, null) };
  ...
  /**
   * Tests the bug MONDRIAN-779. The {@link MemberListCrossJoinArg} was not
   * considering the 'exclude' attribute in its hash code. This resulted in two
   * filters being chained within two different named sets to register a cache
   * element with the same key, even though they were the different because of
   * the added "NOT" keyword.
   */
  public void testBug779() {
    ...
    final String expectedResult1 = "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
      + "{[Product].[Drink].[Dairy]}\n" + "Row #0: 4,186\n";
