/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/agg/AggregationOnDistinctCountMeasuresTest.java#42 $
...
/**
 * <code>AggregationOnDistinctCountMeasureTest</code> tests the Distinct Count
 * functionality with tuples and members.
 * 
 * @author ajogleka
 * @version $Id: //open/mondrian/testsrc/main/mondrian/rolap/agg/
 *          AggregationOnDistinctCountMeasuresTest.java#42 $
...
public class AggregationOnDistinctCountMeasuresTest extends BatchTestCase {
  ...
  protected void setUp() throws Exception {
    schemaReader = getTestContext().getConnection().getSchemaReader().withLocus();
  ...
  public void testCrossJoinMembersWithASingleMember() {
    String query = "WITH MEMBER GENDER.X AS 'AGGREGATE({[GENDER].[GENDER].members} * "
  ...
  public void testCrossJoinParticularMembersFromTwoDimensions() {
    assertQueryReturns("WITH MEMBER GENDER.X AS 'AGGREGATE({[GENDER].M} * "
      + "{[STORE].[ALL STORES].[USA].[CA]})', solve_order=100 "
      + "SELECT GENDER.X ON 0, [MEASURES].[CUSTOMER COUNT] ON 1 FROM SALES",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Gender].[X]}\n" + "Axis #2:\n"
        + "{[Measures].[Customer Count]}\n" + "Row #0: 1,389\n");
  ...
  public void testDistinctCountWithAMeasureAsPartOfTuple() {
    assertQueryReturns("SELECT [STORE].[ALL STORES].[USA].[CA] ON 0, "
      + "([MEASURES].[CUSTOMER COUNT], [Gender].[m]) ON 1 FROM SALES", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA].[CA]}\n" + "Axis #2:\n"
  ...
  public void testDistinctCountOnSetOfMembers() {
    assertQueryReturns(
      "WITH MEMBER STORE.X as 'Aggregate({[STORE].[ALL STORES].[USA].[CA],"
        + "[STORE].[ALL STORES].[USA].[WA]})'" + "SELECT STORE.X  ON ROWS, "
  ...
  public void testDistinctCountOnTuplesWithSomeNonJoiningDimensions() {
    propSaver.set(props.IgnoreMeasureForNonJoiningDimension, false);
  ...
  public void testAggregationOverLargeListGeneratesError() {
    ...
    assertQueryReturns(
      "WITH MEMBER PRODUCT.X as 'Aggregate({"
        ...
        + "{[MEASURES].[CUSTOMER COUNT]} ON COLUMNS\n"
        + "FROM [WAREHOUSE AND SALES2]", result);
  ...
  public void testMultiLevelMembersNullParents() {
    ...
    String query = "with set [Filtered Warehouse Set] as "
    ...
    String necjSqlDerby = "select count(distinct \"inventory_fact_1997\".\"warehouse_cost\") as \"m0\" "
    ...
    String necjSqlMySql = "select count(distinct `inventory_fact_1997`.`warehouse_cost`) as `m0` "
  ...
  public void testCanNotBatchForDifferentCompoundPredicate() {
    ...
    String oraTeraSqlForAgg = "select \"time_by_day\".\"the_year\" as \"c0\", "
      + "count(distinct \"sales_fact_1997\".\"customer_id\") as \"m0\" "
    ...
    String oraTeraSqlForDetail = "select \"store\".\"store_state\" as \"c0\", "
      + "\"time_by_day\".\"the_year\" as \"c1\", "
      + "count(distinct \"sales_fact_1997\".\"customer_id\") as \"m0\" "
      + "from \"store\" =as= \"store\", "
      + "\"sales_fact_1997\" =as= \"sales_fact_1997\", "
    ...
    SqlPattern[] patterns = {
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, oraTeraSqlForAgg,
        oraTeraSqlForAgg),
      new SqlPattern(Dialect.DatabaseProduct.TERADATA, oraTeraSqlForAgg,
        oraTeraSqlForAgg),
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, oraTeraSqlForDetail,
        oraTeraSqlForDetail),
      new SqlPattern(Dialect.DatabaseProduct.TERADATA, oraTeraSqlForDetail,
        oraTeraSqlForDetail), };
  ...
  /**
   * Test distinct count agg happens in non gs query for subset of members with
   * mixed measures.
   */
  public void testDistinctCountInNonGroupingSetsQuery() {
    ...
    String oraTeraSqlForDistinctCountAgg = "select \"time_by_day\".\"the_year\" as \"c0\", "
      + "count(distinct \"sales_fact_1997\".\"customer_id\") as \"m0\" "
      + "from \"time_by_day\" =as= \"time_by_day\", "
      + "\"sales_fact_1997\" =as= \"sales_fact_1997\", \"store\" =as= \"store\" "
      + "where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\" "
    ...
    SqlPattern[] patterns = {
      ...
      new SqlPattern(Dialect.DatabaseProduct.TERADATA, oraTeraSqlForDetail,
        oraTeraSqlForDetail),
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, oraTeraSqlForDistinctCountAgg,
        oraTeraSqlForDistinctCountAgg),
      new SqlPattern(Dialect.DatabaseProduct.TERADATA,
        oraTeraSqlForDistinctCountAgg, oraTeraSqlForDistinctCountAgg), };
  ...
  public void testOptimizeChildren() {
    String query = "with member gender.x as " + "'aggregate("
    ...
    // unlimited IN list.
    String luciddbSql = "select \"time_by_day\".\"the_year\" as \"c0\", "
      + "count(distinct \"sales_fact_1997\".\"customer_id\") as \"m0\" "
  ...
  public void testOptimizeListWhenTuplesAreFormedWithDifferentLevels() {
    ...
    String accessSql = "select `d0` as `c0`, count(`m0`) as `c1` from (select distinct `time_by_day`.`the_year` as `d0`, `sales_fact_1997`.`customer_id` as `m0` "
  ...
  public void testOptimizeChildrenForTuplesWithLength1() {
    ...
    assertTrue(tuppleListContains(tuples, member(Id.Segment.toList("Product",
      "All Products", "Non-Consumable", "Household", "Kitchen Products",
      "Pot Scrubbers", "Cormorant"), salesCubeSchemaReader)));
    assertFalse(tuppleListContains(tuples, member(Id.Segment.toList("Product",
      "All Products", "Non-Consumable", "Household", "Kitchen Products",
      "Pot Scrubbers"), salesCubeSchemaReader)));
    assertFalse(tuppleListContains(tuples, member(Id.Segment.toList("Product",
      "All Products", "Non-Consumable", "Household", "Kitchen Products",
      "Pots and Pans", "Cormorant"), salesCubeSchemaReader)));
    assertTrue(tuppleListContains(tuples, member(Id.Segment.toList("Product",
      "All Products", "Non-Consumable", "Household", "Kitchen Products",
      "Pots and Pans"), salesCubeSchemaReader)));
    assertEquals(4, tuples.size());
  }

  public void testOptimizeChildrenForTuplesWithLength3() {
    TupleList memberList = CrossJoinFunDef.mutableCrossJoin(
    ...
    memberList = CrossJoinFunDef.mutableCrossJoin(memberList,
      storeMembersCAAndOR(salesCubeSchemaReader));
    TupleList tuples = optimizeChildren(memberList);
    assertFalse(tuppleListContains(tuples, member(Id.Segment.toList("Store",
      "All Stores", "USA", "OR", "Portland"), salesCubeSchemaReader)));
    assertTrue(tuppleListContains(tuples, member(Id.Segment.toList("Store",
      "All Stores", "USA", "OR"), salesCubeSchemaReader)));
  ...
  public void testShouldNotRemoveDuplicateTuples() {
    Member maleChildMember = member(Id.Segment.toList("Gender", "All Gender", "M"),
  ...
  public void testMemberCountIsSameForAllMembersInTuple() {
    TupleList memberList = CrossJoinFunDef.mutableCrossJoin(
      genderMembersIncludingAll(false, salesCubeSchemaReader, salesCube),
      storeMembersUsaAndCanada(false, salesCubeSchemaReader, salesCube));
    Map<Member, Integer>[] memberCounterMap = AggregateFunDef.AggregateCalc
      .membersVersusOccurencesInTuple(memberList);
  ...
  public void testMemberCountIsNotSameForAllMembersInTuple() {
    Member maleChild = member(Id.Segment.toList("Gender", "All Gender", "M"),
      salesCubeSchemaReader);
    Member femaleChild = member(Id.Segment.toList("Gender", "All Gender", "F"),
      salesCubeSchemaReader);
    Member mexicoMember = member(Id.Segment.toList("Store", "All Stores", "Mexico"),
      salesCubeSchemaReader);
  ...
  public void testAggregatesAtTheSameLevelForNormalAndDistinctCountMeasure() {
    ...
    try {
      assertQueryReturns("WITH "
        ...
        + "MEMBER GENDER.AGG2 AS 'AGGREGATE({ GENDER.[M] })' " + "SELECT "
  ...
  public void testDistinctCountForAggregatesAtTheSameLevel() {
    ...
    try {
      assertQueryReturns("WITH "
        ...
        + "SELECT " + "{MEASURES.[CUSTOMER COUNT]} ON 0, " + "{GENDER.AGG } ON 1 "
        + "FROM SALES", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
        + "{[Measures].[Customer Count]}\n" + "Axis #2:\n" + "{[Gender].[AGG]}\n"
        + "Row #0: 5,581\n");
    } finally {
  ...
  private boolean tuppleListContains(TupleList tuples, Member memberByUniqueName) {
    if (tuples.getArity() == 1) {
      return tuples.contains(Collections.singletonList(memberByUniqueName));
    }
  ...
  private TupleList optimizeChildren(final TupleList memberList) {
    return Locus.execute(Execution.NONE, "AggregationOnDistinctCountMeasuresTest",
      new Locus.Action<TupleList>() {
        public TupleList execute() {
          return AggregateFunDef.AggregateCalc.optimizeChildren(memberList,
            schemaReader, salesCube);
        }
      });
  }
}
