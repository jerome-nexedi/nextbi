/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/aggmatcher/Checkin_7634.java#10 $
...
/**
 * Checkin 7634 attempted to correct a problem demonstrated by this junit. The
 * CrossJoinFunDef class has an optimization that kicks in when the combined
 * lists sizes are greater than 1000. I create a property here which, if set,
 * can be used to change that size from 1000 to, in this case, 2. Also, there is
 * a property that disables the use of the optimization altogether and another
 * that permits the use of the old optimization, currently the nonEmptyListOld
 * method in the CrossJoinFunDef class, and the new, checkin 7634, version of
 * the method called nonEmptyList.
 * 
 * <p>
 * The old optimization only looked at the default measure while the new version
 * looks at all measures appearing in the query. The example Cube and data for
 * the junit is such that there is no data for the default measure. Thus the old
 * optimization fails to produce the correct result.
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/aggmatcher/Checkin_7634
 *          .java#10 $
 */
public class Checkin_7634 extends CsvDBTestCase {
  ...
  public void testCrossJoin() throws Exception {
    ...
    String mdx = "select {[Measures].[Requested Value]} ON COLUMNS,"
      + " NON EMPTY Crossjoin(" + " {[Geography].[All Regions].Children},"
      + " {[Product].[All Products].Children}" + ") ON ROWS"
      + " from [Checkin_7634]";

    // Execute query but do not used the CrossJoin nonEmptyList optimization
    propSaver.set(MondrianProperties.instance().CrossJoinOptimizerSize,
      Integer.MAX_VALUE);
