/*
// $Id: //open/mondrian/src/main/mondrian/xmla/Rowset.java#40 $
 ...
 * @since May 2, 2003
 * @version $Id: //open/mondrian/src/main/mondrian/xmla/Rowset.java#40 $
 */
abstract class Rowset implements XmlaConstants {
   ...
   * 
   * <p>
   * The exceptions thrown in this constructor are not produced during the
  ...
  Rowset(RowsetDefinition definition, XmlaRequest request, XmlaHandler handler) {
    ...
    for (Map.Entry<String, Object> restrictionEntry : restrictions.entrySet()) {
      ...
      final RowsetDefinition.Column column = definition
        .lookupColumn(restrictedColumn);
      if (column == null) {
        throw Util.newError("Rowset '" + definition.name()
          + "' does not contain column '" + restrictedColumn + "'");
      }
      if (!column.restriction) {
        throw Util.newError("Rowset '" + definition.name() + "' column '"
          + restrictedColumn + "' does not allow restrictions");
      }
      ...
      if (restriction instanceof List && ((List) restriction).size() > 1) {
        ...
        default:
          throw Util.newError("Rowset '" + definition.name() + "' column '"
            + restrictedColumn + "' can only be restricted on one value at a time");
        }
      }
      list.add(column);
    }
    ...
    for (Map.Entry<String, String> propertyEntry : properties.entrySet()) {
      ...
      final PropertyDefinition propertyDef = Util.lookup(PropertyDefinition.class,
        propertyName);
      if (propertyDef == null) {
        throw Util.newError("Rowset '" + definition.name()
   ...
   * <p/>
   * 
   * A derived class should override this method and intercept each property it
   * supports. Any property it does not support, it should forward to the base
   * class method, which will probably throw an error.
   * <p/>
   */
  protected void setProperty(PropertyDefinition propertyDef, String value) {
    ...
    default:
      LOGGER.warn("Warning: Rowset '" + rowsetDefinition.name()
        + "' does not support property '" + propertyDef.name() + "' (value is '"
        + value + "')");
    }
  }
   ...
   */
  public final void populate(XmlaResponse response, OlapConnection connection,
  ...
  protected abstract void populateImpl(XmlaResponse response,
    OlapConnection connection, List<Row> rows) throws XmlaException, SQLException;
      ...
      if (value == null) {
        if (!column.nullable) {
          throw new XmlaException(CLIENT_FAULT_FC, HSB_BAD_NON_NULLABLE_COLUMN_CODE,
            HSB_BAD_NON_NULLABLE_COLUMN_FAULT_FS, Util
              .newInternal("Value required for column " + column.name
                + " of rowset " + rowsetDefinition.name()));
        }
   ...
   */
  protected <E> void populate(Class<E> clazz, List<Row> rows,
    final Comparator<E> comparator) throws XmlaException {
   ...
   * 
   * <p>
   * A condition is a {@link mondrian.olap.Util.Functor1} whose return type is
   * boolean.
   * 
   ...
   * (String) or an array of values (String[]). So, creating the conditions just
   * once at the beginning is faster than having to determine the restriction
   * status each time it is needed.
   * 
   * @param column
   *          Metadata column
   * @param <E>
   *          Element type, e.g. {@link org.olap4j.metadata.Catalog} or
  ...
  <E> Util.Functor1<Boolean, E> makeCondition(RowsetDefinition.Column column) {
    return makeCondition(Util.<E> identityFunctor(), column);
  }
   ...
   * 
   * <p>
   * The accessor gets a particular property of the element in question for the
   * column restrictions to act upon.
   * 
   * @param getter
   *          Attribute accessor
   * @param column
   ...
   *          {@link org.olap4j.metadata.Level}
   * @param <V>
   *          Value type; often {@link String}, since many restrictions work on
   *          the name or unique name of elements
   * @return Condition functor
   */
  <E, V> Util.Functor1<Boolean, E> makeCondition(
    final Util.Functor1<V, ? super E> getter, RowsetDefinition.Column column) {
    ...
    if (restriction == null) {
      ...
      String regexp = Util.wildcardToRegexp(Collections
      ...
      throw Util.newInternal("unexpected restriction type: "
   ...
   * attempt two determine if the restriction is an array of Strings if all
   * members of the array have the same value (in which case one could return,
   * again, simply a single String).
   */
  ...
  /**
   * Returns a column's restriction as an <code>int</code> if it exists, -1
   * otherwise.
   */
    ...
    if (restriction instanceof List) {
      ...
      if (rval.size() == 1) {
        try {
          ...
          LOGGER.info("Rowset.getRestrictionValue: " + "bad integer restriction \""
            + rval + "\"");
          return -1;
        }
      }
  ...
  /**
   * A set of name/value pairs, which can be output using {@link Rowset#addRow}.
   * This uses less memory than simply using a HashMap and for very big data
   * sets memory is a concern.
   */
  protected static class Row {
    ...
    /**
     * Retrieves the value of a field with a given name, or null if the field's
  ...
  protected static class XmlElement {
    ...
    private XmlElement(String tag, Object[] attributes, String text,
      XmlElement[] children) {
