/*
// $Id: //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jCellSet.java#17 $
...
/**
 * Implementation of {@link CellSet} for the Mondrian OLAP engine.
 * 
 * <p>
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jCellSet.java
 *          #17 $
 * @since May 24, 2007
 */
abstract class MondrianOlap4jCellSet extends Execution implements CellSet {
  ...
  private final List<CellSetAxis> axisList = new ArrayList<CellSetAxis>();
   ...
   *          Statement
   */
  public MondrianOlap4jCellSet(MondrianOlap4jStatement olap4jStatement) {
    ...
    if (olap4jStatement instanceof MondrianOlap4jPreparedStatement) {
      this.metaData = ((MondrianOlap4jPreparedStatement) olap4jStatement).cellSetMetaData;
    } else {
      this.metaData = new MondrianOlap4jCellSetMetaData(olap4jStatement, query);
    }
  }
   ...
   * 
   * <p>
   * This method may take some time. While it is executing, a client may execute
   * {@link MondrianOlap4jStatement#cancel()}.
   * 
  ...
  void execute() throws OlapException {
    ...
    if (queryAxis == null) {
      // Dummy slicer axis.
      queryAxis = new QueryAxis(false, null, AxisOrdinal.StandardAxisOrdinal.SLICER,
    ...
    try {
      ...
      if (e.getMessage().indexOf("coordinates out of range") >= 0) {
        ...
        throw new IndexOutOfBoundsException("Cell coordinates ("
          + getCoordsAsString(pos) + ") fall outside CellSet bounds ("
          + getCoordsAsString(dimensions) + ")");
      } else if (e.getMessage().indexOf("coordinates should have dimension") >= 0) {
        throw new IllegalArgumentException("Cell coordinates should have dimension "
          + axisList.size());
      } else {
        throw e;
      }
  ...
  public int coordinatesToOrdinal(List<Integer> coordinates) {
    ...
    if (coordinates.size() != axes.size()) {
      throw new IllegalArgumentException("Coordinates have different dimension "
    ...
    for (CellSetAxis axis : axes) {
      ...
      if (coordinate < 0 || coordinate >= axis.getPositionCount()) {
        throw new IndexOutOfBoundsException("Coordinate " + coordinate + " of axis "
          + k + " is out of range (" + getBoundsAsString() + ")");
      }
