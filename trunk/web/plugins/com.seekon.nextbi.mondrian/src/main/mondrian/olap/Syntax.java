/*
// $Id: //open/mondrian/src/main/mondrian/olap/Syntax.java#20 $
 ...
 * @author jhyde
 * @since 21 July, 2003
 * @version $Id: //open/mondrian/src/main/mondrian/olap/Syntax.java#20 $
 */
public enum Syntax {
  /**
   ...
   * <code>object.METHOD()</code> or <code>object.METHOD(args)</code>.
   */
  Method {
    public void unparse(String fun, Exp[] args, PrintWriter pw) {
      Util.assertTrue(args.length >= 1);
    ...
    public String getSignature(String name, int returnType, int[] argTypes) {
      ...
      return (returnType == Category.Unknown ? "" : getTypeDescription(returnType)
        + " ")
        + getTypeDescription(argTypes[0])
        + "."
        + name
        + "("
        + getTypeDescriptionCommaList(argTypes, 1) + ")";
    }
  },
  ...
  /**
   * Defines syntax for expression invoked as <code>OPERATOR arg</code> (like
   * unary '-').
   */
  Prefix {
    public void unparse(String fun, Exp[] args, PrintWriter pw) {
  ...
  /**
   * Defines syntax for expression invoked as <code>arg OPERATOR</code> (like
   * <code>IS EMPTY</code>).
   */
  Postfix {
    public void unparse(String fun, Exp[] args, PrintWriter pw) {
   ...
   * <code>{ARG, &#46;&#46;&#46;}</code>; that is, the set construction
   * operator.
   */
  Braces {
    public String getSignature(String name, int returnType, int[] argTypes) {
  ...
  /**
   * Defines syntax for expression generated by the Mondrian system which cannot
   * be specified syntactically.
   */
  Internal,

  /**
   * Defines syntax for a CAST expression <code>CAST(expression AS type)</code>.
   */
  Cast {
    public void unparse(String fun, Exp[] args, PrintWriter pw) {
      pw.print("CAST(");
  ...
  /**
   * Defines syntax for expression invoked <code>object&#46;&PROPERTY</code> (a
   * variant of {@link #Property}).
   */
  QuotedProperty,
  ...
  /**
   * Returns a description of the signature of a function call, for example,
   * "CoalesceEmpty(<Numeric Expression>, <String Expression>)".
   * 
   * @param name
   *          Function name
   * @param returnType
   *          Function's return category
   * @param argTypes
  ...
  private static boolean needParen(Exp[] args) {
    return !(args.length == 1 && args[0] instanceof FunCall && ((FunCall) args[0])
      .getSyntax() == Syntax.Parentheses);
  }
  ...
  private static String getTypeDescriptionCommaList(int[] types, int start) {
    ...
    for (int i = start; i < types.length; i++) {
      ...
      sb.append("<").append(
        Category.instance.getDescription(types[i] & Category.Mask)).append(">");
    }
    return sb.toString();
  }
}
