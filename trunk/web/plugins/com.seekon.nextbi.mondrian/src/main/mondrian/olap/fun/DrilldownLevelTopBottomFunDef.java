/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/DrilldownLevelTopBottomFunDef.java#12 $
 ...
 * Syntax:
 * 
 * <blockquote> DrilldownLevelTop(Set_Expression, Count [, [Level_Expression][,
 * Numeric_Expression]])<br/>
 * DrilldownLevelBottom(Set_Expression, Count [, [Level_Expression][,
 * Numeric_Expression]]) </blockquote>
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/DrilldownLevelTopBottomFunDef
 *          .java#12 $
 * @since Oct 18, 2007
 */
class DrilldownLevelTopBottomFunDef extends FunDefBase {
  ...
  public Calc compileCall(final ResolvedFunCall call, ExpCompiler compiler) {
    ...
    final LevelCalc levelCalc = call.getArgCount() > 2
      && call.getArg(2).getCategory() != Category.Empty ? compiler.compileLevel(call
      .getArg(2)) : null;
    final Calc orderCalc = call.getArgCount() > 3 ? compiler.compileScalar(call
      .getArg(3), true) : new ValueCalc(new DummyExp(new ScalarType()));
    return new AbstractListCalc(call,
        ...
        for (Member member : list.slice(0)) {
          ...
          if (level != null && member.getLevel() != level) {
            if (level.getDimension() != member.getDimension()) {
              throw newEvalException(DrilldownLevelTopBottomFunDef.this, "Level '"
                + level.getUniqueName() + "' not compatible with member '"
          ...
          final List<Member> sortedChildren = sortMembers(evaluator, children,
            children, orderCalc, top, true);
