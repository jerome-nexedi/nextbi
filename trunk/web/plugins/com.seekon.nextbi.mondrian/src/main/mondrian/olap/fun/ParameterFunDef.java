/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/ParameterFunDef.java#20 $
 ...
 * fleetingly, and is then converted into a {@link mondrian.olap.Parameter}. For
 * internal use only.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/ParameterFunDef.java#20 $
 * @since Feb 14, 2003
 */
public class ParameterFunDef extends FunDefBase {
  ...
  ParameterFunDef(FunDef funDef, String parameterName, Type type,
    int returnCategory, Exp exp, String description) {
    super(funDef.getName(), funDef.getSignature(), funDef.getDescription(), funDef
      .getSyntax(), returnCategory, funDef.getParameterCategories());
  ...
  public Exp createCall(Validator validator, Exp[] args) {
    Parameter parameter = validator.createOrLookupParam(this.getName().equals(
      "Parameter"), parameterName, type, exp, parameterDescription);
  ...
  public static String getParameterName(Exp[] args) {
    if (args[0] instanceof Literal && args[0].getCategory() == Category.String) {
   ...
   * Does not use the default value expression, so this method can safely be
   * used before the expression has been validated.
   */
    ...
    protected FunDef createFunDef(Exp[] args, FunDef dummyFunDef) {
      ...
      case Category.Level:
        ...
        if (!isConstant(typeArg)) {
          throw newEvalException(dummyFunDef, "Invalid parameter '" + parameterName
            + "'. Type must be a NUMERIC, STRING, or a dimension, "
            + "hierarchy or level");
        }
        if (dimension == null) {
          throw newEvalException(dummyFunDef, "Invalid dimension for parameter '"
            + parameterName + "'");
        }
        type = new MemberType(type.getDimension(), type.getHierarchy(), type
          .getLevel(), null);
      ...
      default:
        ...
        // type-checked.
        throw newEvalException(dummyFunDef, "Invalid type for parameter '"
          + parameterName + "'; expecting NUMERIC, STRING or a hierarchy");
      }

      // Default value
      Exp exp = args[2];
      Validator validator = createSimpleValidator(BuiltinFunTable.instance());
      ...
      if (!validator.canConvert(2, exp, category, conversionList)) {
        throw newEvalException(dummyFunDef, "Default value of parameter '"
          + parameterName + "' is inconsistent with its type, " + typeName);
      }
      if (exp.getCategory() == Category.Set && category == Category.Member) {
      ...
      if (category == Category.Member) {
        ...
        if (distinctFrom(type.getDimension(), expType.getDimension())
          ...
          throw newEvalException(dummyFunDef, "Default value of parameter '"
            + parameterName + "' is not consistent with the parameter type '" + type);
        }
      }

      String parameterDescription = null;
      if (args.length > 3) {
        if (args[3] instanceof Literal && args[3].getCategory() == Category.String) {
          parameterDescription = (String) ((Literal) args[3]).getValue();
        } else {
          throw newEvalException(dummyFunDef, "Description of parameter '"
            + parameterName + "' must be a string constant");
        }
      }

      return new ParameterFunDef(dummyFunDef, parameterName, type, category, exp,
  ...
  public static class ParamRefResolver extends MultiResolver {
    ...
    protected FunDef createFunDef(Exp[] args, FunDef dummyFunDef) {
      String parameterName = getParameterName(args);
      return new ParameterFunDef(dummyFunDef, parameterName, null, Category.Unknown,
        null, null);
    }
  }
}
