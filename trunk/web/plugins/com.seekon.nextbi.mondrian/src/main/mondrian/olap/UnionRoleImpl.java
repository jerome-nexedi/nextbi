...
/**
 * Implementation of {@link Role} which combines the privileges of several roles
 * and has the superset of their privileges.
 * 
 * @see mondrian.olap.RoleImpl#union(java.util.List)
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/UnionRoleImpl.java#3 $
 * @since Nov 26, 2007
 */
class UnionRoleImpl implements Role {
  ...
  /**
   * Returns the larger of two enum values. Useful if the enums are sorted so
   * that more permissive values come after less permissive values.
   * 
   * @param t1
   *          First value
   * @param t2
  ...
  public HierarchyAccess getAccessDetails(final Hierarchy hierarchy) {
    ...
    for (Role role : roleList) {
      final HierarchyAccess accessDetails = role.getAccessDetails(hierarchy);
    ...
    HierarchyAccess hierarchyAccess = new UnionHierarchyAccessImpl(hierarchy, list);
    if (list.size() > 5) {
  ...
  public Access getAccess(Member member) {
    assert member != null;
    HierarchyAccess hierarchyAccess = getAccessDetails(member.getHierarchy());
  ...
  /**
   * Implementation of {@link mondrian.olap.Role.HierarchyAccess} that gives
   * access to an object if any one of the constituent hierarchy accesses has
  ...
  private class UnionHierarchyAccessImpl implements HierarchyAccess {
     ...
     *          Hierarchy
     * @param list
     *          List of underlying hierarchy accesses
     */
    UnionHierarchyAccessImpl(Hierarchy hierarchy, List<HierarchyAccess> list) {
    ...
    public int getTopLevelDepth() {
      ...
      for (HierarchyAccess hierarchyAccess : list) {
        access = Math.min(access, hierarchyAccess.getTopLevelDepth());
        if (access == 0) {
          break;
        }
      }
    ...
    public RollupPolicy getRollupPolicy() {
      ...
      for (HierarchyAccess hierarchyAccess : list) {
        rollupPolicy = max(rollupPolicy, hierarchyAccess.getRollupPolicy());
