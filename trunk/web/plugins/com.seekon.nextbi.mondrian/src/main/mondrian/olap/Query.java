/*
// $Id: //open/mondrian/src/main/mondrian/olap/Query.java#143 $
 ...
 * 
 * <p>
 * It is created by calling {@link Connection#parseQuery}, and executed by
 * calling {@link Connection#execute}, to return a {@link Result}.
 * </p>
 * 
 * <h3>Query control</h3>
 * 
 * <p>
 * Most queries are model citizens, executing quickly (often using cached
 ...
 * three ways to control rogue queries:
 * <ul>
 * 
 * <li>You can set a query timeout by setting the
 * {@link MondrianProperties#QueryTimeout} parameter. If the query takes longer
 * to execute than the value of this parameter, the system will kill it.</li>
 * 
 * <li>The {@link MondrianProperties#QueryLimit} parameter limits the number of
 * cells returned by a query.</li>
 * 
 * <li>At any time while a query is executing, another thread can cancel the
 * query by calling {@link #getStatement()}.{@link Statement#cancel() cancel()}.
 * The call to {@link Connection#execute(Query)} will throw an exception.</li>
 * 
 * </ul>
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/Query.java#143 $
 */
public class Query extends QueryPart {
  ...
  /**
   * Set of FunDefs for which alerts about non-native evaluation have already
   * been posted.
   */
  Set<FunDef> alertedNonNativeFunDefs;

  /**
   * Unique list of members referenced from the measures dimension. Will be used
   * to determine if cross joins can be processed natively for virtual cubes.
   */
  private Set<Member> measuresMembers;

  /**
   * If true, virtual cubes can be processed using native cross joins. It
   * defaults to true, unless functions are applied on measures.
   */
  private boolean nativeCrossJoinVirtualCube;

  /**
   * Used for virtual cubes. Comtains a list of base cubes related to a virtual
   * cube
   */
  ...
  /**
   * How should the query be returned? Valid values are: ResultStyle.ITERABLE
   * ResultStyle.LIST ResultStyle.MUTABLE_LIST For java4, use LIST
   */
  private ResultStyle resultStyle = Util.Retrowoven ? ResultStyle.LIST
   ...
   * Creates a Query.
   */
  public Query(Statement statement, Formula[] formulas, QueryAxis[] axes,
    String cube, QueryAxis slicerAxis, QueryPart[] cellProps,
    boolean strictValidation) {
    this(statement, Util.lookupCube(statement.getSchemaReader(), cube, true),
      formulas, axes, slicerAxis, cellProps, new Parameter[0], strictValidation);
  }

  /**
   * Creates a Query.
   */
  public Query(Statement statement, Cube mdxCube, Formula[] formulas,
    QueryAxis[] axes, QueryAxis slicerAxis, QueryPart[] cellProps,
  ...
  /**
   * Adds a new formula specifying a member or a set to an existing query;
   * resolve is called after the formula has been added.
   * 
   * @param formula
  ...
  /**
   * Adds some number of new formulas specifying members or sets to an existing
   * query; resolve is only called once, after all the new members have been
   * added to the query.
   * 
   * @param additions
  ...
  public Validator createValidator() {
    return createValidator(statement.getSchema().getFunTable(), false);
  }
   ...
   * @param functionTable
   *          Function table
   * @param alwaysResolveFunDef
   *          Whether to always resolve function definitions (see
   *          {@link Validator#alwaysResolveFunDef()})
   * @return Validator
   */
  public Validator createValidator(FunTable functionTable,
    boolean alwaysResolveFunDef) {
    return new QueryValidator(functionTable, alwaysResolveFunDef, Query.this);
  }
  ...
  public Query clone() {
    return new Query(statement, cube, Formula.cloneArray(formulas), QueryAxis
      .cloneArray(axes), (slicerAxis == null) ? null : (QueryAxis) slicerAxis
      .clone(), cellProps, parameters.toArray(new Parameter[parameters.size()]),
      strictValidation);
  }
  ...
  /**
   * Issues a cancel request on this Query object. Once the thread running the
   * query detects the cancel request, the query execution will throw an
   * exception. See <code>BasicQueryTest.testCancel</code> for an example of
  ...
  private void normalizeAxes() {
    for (int i = 0; i < axes.length; i++) {
      AxisOrdinal correctOrdinal = AxisOrdinal.StandardAxisOrdinal
        .forLogicalOrdinal(i);
  ...
  /**
   * Performs type-checking and validates internal consistency of a query, using
   * the default resolver.
   * 
   * <p>
   * This method is called automatically when a query is created; you need to
   * call this method manually if you have modified the query's expression tree
   * in any way.
   */
  public void resolve() {
    ...
    ExpCompiler compiler = createCompiler(evaluator, validator, Collections
      .singletonList(resultStyle));
    compile(compiler);
  }

  /**
   * @return true if the relevant property for ignoring invalid members is set
   *         to true for this query's environment (a different property is
   ...
   * ResultStyle.ITERABLE ResultStyle.LIST ResultStyle.MUTABLE_LIST
   * 
   * @param resultStyle
   */
  public void setResultStyle(ResultStyle resultStyle) {
    ...
    default:
      throw ResultStyleException.generateBadType(
        ResultStyle.ITERABLE_LIST_MUTABLELIST, resultStyle);
    }
  }
  ...
  public void resolve(Validator validator) {
    ...
    if (axes != null) {
      ...
      for (QueryAxis axis : axes) {
        if (!axisNames.contains(seekOrdinal)) {
          AxisOrdinal axisName = AxisOrdinal.StandardAxisOrdinal
            .forLogicalOrdinal(seekOrdinal);
          throw MondrianResource.instance().NonContiguousAxis.ex(seekOrdinal,
            axisName.name());
        }
        ++seekOrdinal;
      }
    ...
    for (Hierarchy hierarchy : ((RolapCube) getCube()).getHierarchies()) {
      ...
      if (useCount > 1) {
        throw MondrianResource.instance().HierarchyInIndependentAxes.ex(hierarchy
          .getUniqueName());
      }
    }
  }
  ...
  private Collection<QueryAxis> allAxes() {
    if (slicerAxis == null) {
      ...
      return new CompositeCollection(new Collection[] {
        Collections.singletonList(slicerAxis), Arrays.asList(axes) });
    }
  }
   ...
   * 
   * <p>
   * If the expression's type is a dimension with several hierarchies, assumes
   * that the expression yields a member of the first (default) hierarchy of the
   * dimension.
   * 
   * <p>
   * For example, the expression <blockquote><code>Crossjoin(
   *   Hierarchize(
   *     Union(
  ...
  public void setParameter(final String parameterName, final Object value) {
    ...
    final Parameter param = getSchemaReader(false).getParameter(parameterName);
    if (param == null) {
      throw MondrianResource.instance().UnknownParameter.ex(parameterName);
    }
    if (!param.isModifiable()) {
      throw MondrianResource.instance().ParameterIsNotModifiable.ex(parameterName,
        param.getScope().name());
    }
    final Object value2 = Locus.execute(new Execution(statement, 0),
      "Query.quickParse", new Locus.Action<Object>() {
   ...
   * <li>For sets of members, requires a list of members or strings and converts
   * each element to a member.
   * </ul>
   * 
   * @param type
   *          Type
   ...
   *           If value needs to be a number but isn't
   */
  private static Object quickParse(String parameterName, Type type, Object value,
    Query query) throws NumberFormatException {
    ...
    case Category.Numeric:
      ...
      throw Util.newInternal("Invalid value '" + value + "' for parameter '"
    ...
    case Category.Set:
      ...
      for (Object o : list) {
        ...
        final Member member = (Member) quickParse(parameterName, elementType, o,
          query);
        expList.add(member);
      }
      return expList;
   ...
   *          If true, schema reader returns only elements which are accessible
   *          to the statement's current role
   * 
  ...
  public Member lookupMemberFromCache(String memberUniqueName) {
    ...
    for (Member member : getDefinedMembers()) {
      if (Util.equalName(member.getUniqueName(), memberUniqueName)
        || Util.equalName(getUniqueNameWithoutAll(member), memberUniqueName)) {
        return member;
      }
    }
    return null;
  ...
  private String getUniqueNameWithoutAll(Member member) {
    ...
    if ((parentMember != null) && !parentMember.isAll()) {
      return Util
        .makeFqName(getUniqueNameWithoutAll(parentMember), member.getName());
    } else {
  ...
  private NamedSet lookupNamedSet(String name) {
    for (Formula formula : formulas) {
      if (!formula.isMember() && formula.getElement() != null
   ...
   * 
   * @param nameParts
   *          Multi-part identifier for set
   * @param scopeList
   *          Parse tree node where name is used (last in list) and
   */
  ScopedNamedSet lookupScopedNamedSet(List<Id.Segment> nameParts,
  ...
  /**
   * Remove a formula from the query. If <code>failIfUsedInQuery</code> is true,
   * checks and throws an error if formula is used somewhere in the query.
   */
  public void removeFormula(String uniqueName, boolean failIfUsedInQuery) {
    ...
    if (failIfUsedInQuery && formula != null) {
      ...
      while (walker.hasMoreElements()) {
        ...
        // or formula
        String formulaType = formula.isMember() ? MondrianResource.instance().CalculatedMember
          .str()
          ...
          if (grandParent instanceof Query) {
            if (parent instanceof Axis) {
              throw MondrianResource.instance().MdxCalculatedFormulaUsedOnAxis.ex(
                formulaType, uniqueName, ((QueryAxis) parent).getAxisName());

            } else if (parent instanceof Formula) {
              String parentFormulaType = ((Formula) parent).isMember() ? MondrianResource
                .instance().CalculatedMember.str()
                : MondrianResource.instance().CalculatedSet.str();
              throw MondrianResource.instance().MdxCalculatedFormulaUsedInFormula
                .ex(formulaType, uniqueName, parentFormulaType, ((Formula) parent)
                  .getUniqueName());
  ...
  public void renameFormula(String uniqueName, String newName) {
    Formula formula = findFormula(uniqueName);
    if (formula == null) {
      throw MondrianResource.instance().MdxFormulaNotFound.ex("formula", uniqueName,
        Util.unparse(this));
    }
  ...
  public Hierarchy[] getMdxHierarchiesOnAxis(AxisOrdinal axis) {
    ...
    QueryAxis queryAxis = axis.isFilter() ? slicerAxis : axes[axis.logicalOrdinal()];
   ...
   * @param resultStyle
   *          Preferred result style; if null, use query's default result style;
   *          ignored if expression is scalar
   * @return compiled expression
   */
  public Calc compileExpression(Exp exp, boolean scalar, ResultStyle resultStyle) {
    final Statement statement = ((ConnectionBase) getConnection())
    ...
    try {
      ...
      resultStyleList = Collections.singletonList(resultStyle != null ? resultStyle
        : this.resultStyle);
  ...
  public ExpCompiler createCompiler() {
    ...
    try {
      ...
      return createCompiler(evaluator, validator, Collections
        .singletonList(resultStyle));
    } finally {
      statement.close();
    }
  ...
  private ExpCompiler createCompiler(final Evaluator evaluator,
    final Validator validator, List<ResultStyle> resultStyleList) {
    ExpCompiler compiler = ExpCompiler.Factory.getExpCompiler(evaluator, validator,
      resultStyleList);
  ...
  /**
   * @return set of members from the measures dimension referenced within this
   *         query
   */
  ...
  /**
   * Indicates that the query cannot use native cross joins to process this
   * virtual cube
   */
  ...
  /**
   * @return true if the query can use native cross joins on a virtual cube
   */
  ...
  /**
   * Saves away the base cubes related to the virtual cube referenced in this
   * query
   * 
   * @param baseCubes
   *          set of base cubes
  ...
  /**
   * return the set of base cubes associated with the virtual cube referenced in
   * this query
   * 
  ...
  /**
   * Put an Object value into the evaluation cache with given key. This is used
   * by Calc's to store information between iterations (rather than re-generate
   * each time).
   * 
   * @param key
   *          the cache key
   ...
   * 
   * <p>
   * Releases any resources held. Writes statistics to log if profiling is
   * enabled.
   * 
   * <p>
   * This method is idempotent.
   * 
   * @deprecated Call {@code Query.getStatement().close()}. This method will be
   *             removed in mondrian-4.0.
   */
  public void close() {
    statement.close();
  }
   ...
   * 
   * <p>
   * Note especially that {@link #getCalculatedMember(java.util.List)} returns
   * the calculated members defined in this query. It does not perform access
   * control; all calculated members defined in a query are visible to everyone.
   */
  private static class QuerySchemaReader extends DelegatingSchemaReader implements
    ...
    public SchemaReader withoutAccessControl() {
      return new QuerySchemaReader(schemaReader.withoutAccessControl(), query);
    }
    ...
    private static boolean match(Member member, List<Id.Segment> nameParts) {
      ...
      if (segment.matches(member.getName())) {
        return Util.equalName(member.getHierarchy().getUniqueName(), Util
          .implode(nameParts.subList(0, nameParts.size() - 1)));
    ...
    public List<Member> getCalculatedMembers(Level level) {
      List<Member> hierarchyMembers = getCalculatedMembers(level.getHierarchy());
      List<Member> result = new ArrayList<Member>();
    ...
    public OlapElement getElementChild(OlapElement parent, Id.Segment s,
      MatchType matchType) {
      // first look in cube
      OlapElement mdxElement = schemaReader.getElementChild(parent, s, matchType);
      if (mdxElement != null) {
        return mdxElement;
      }
      ...
      for (Formula formula : query.formulas) {
        ...
        if (id.getSegments().size() == 1 && id.getSegments().get(0).matches(s.name)) {
    ...
    public OlapElement lookupCompoundInternal(OlapElement parent,
      ...
      if (matchType == MatchType.EXACT) {
        OlapElement oe = lookupCompound(parent, names, failIfNotFound, category,
      ...
      OlapElement olapElement = super.lookupCompoundInternal(parent, names,
        failIfNotFound, category, matchType);
      if (olapElement instanceof Member) {
        Member member = (Member) olapElement;
        final Formula formula = (Formula) member
          .getPropertyValue(Property.FORMULA.name);
        if (formula != null) {
   ...
   * 
   * <p>
   * It's unlikely that we would want a validator that is NOT within a
   * particular query, but by organizing the code this way, with the majority of
   * the code in {@link mondrian.olap.ValidatorImpl}, the dependencies between
   * Validator and Query are explicit.
   */
  private static class QueryValidator extends ValidatorImpl {
     ...
     * @param alwaysResolveFunDef
     *          Whether to always resolve function definitions (see
     *          {@link #alwaysResolveFunDef()})
     * @param query
     *          Query
     */
    public QueryValidator(FunTable functionTable, boolean alwaysResolveFunDef,
      Query query) {
      super(functionTable);
  ...
  /**
   * Schema reader that depends on the current scope during the validation of a
   * query. Depending on the scope, different calculated sets may be visible.
   * The scope is represented by the expression stack inside the validator.
   */
  private static class ScopedSchemaReader extends DelegatingSchemaReader implements
    NameResolver.Namespace {
     ...
     *          Validator that is being used to validate the query
     * @param accessControlled
     *          Access controlled
     */
    private ScopedSchemaReader(QueryValidator queryValidator,
    ...
    @Override
    public OlapElement lookupCompoundInternal(OlapElement parent,
      final List<Id.Segment> names, boolean failIfNotFound, int category,
      MatchType matchType) {
      switch (category) {
      case Category.Set:
      case Category.Unknown:
        final ScopedNamedSet namedSet = queryValidator.getQuery()
          .lookupScopedNamedSet(names, queryValidator.getScopeStack());
        if (namedSet != null) {
          return namedSet;
        }
      }
      return super.lookupCompoundInternal(parent, names, failIfNotFound, category,
        matchType);
    }
  ...
  public static class ScopedNamedSet implements NamedSet {
     ...
     *          Name
     * @param scope
     *          Scope of named set (the function call that encloses the 'expr AS
     *          name', often GENERATE or FILTER)
     * @param expr
     *          Expression that defines the set
     */
    ...
    public NamedSet validate(Validator validator) {
      ...
      if (type instanceof MemberType || type instanceof TupleType) {
        newExpr = new UnresolvedFunCall("{}", Syntax.Braces, new Exp[] { newExpr })
          .accept(validator);
      }
      this.expr = newExpr;
      return this;
    ...
    public Object visit(UnresolvedFunCall call) {
      if (call.getFunName().equals("Parameter")) {
        ...
        if (parametersByName.get(parameterName) != null) {
          throw MondrianResource.instance().ParameterDefinedMoreThanOnce
            .ex(parameterName);
        }
        ...
        Parameter parameter = new ParameterImpl(parameterName, Literal.nullValue,
          null, type);
        parameters.add(parameter);
  ...
  /**
   * Visitor that locates and registers all aliased expressions ('expr AS
   * alias') as named sets. The resulting named sets have scope, therefore they
   * can only be seen and used within that scope.
   */
  private class AliasedExpressionFinder extends MdxVisitorImpl {
    ...
    /**
     * Registers a named set if an expression is of the form "expr AS alias".
     * 
     * @param parent
     *          Parent node
     * @param exp
     *          Expression that may be an "AS"
     */
    private void registerAlias(QueryPart parent, Exp exp) {
      if (exp instanceof FunCall) {
        ...
        if (call2.getSyntax() == Syntax.Infix && call2.getFunName().equals("AS")) {
          ...
          createScopedNamedSet(id.getSegments().get(0).name, parent, call2.getArg(0));
        }
      }
    }
  }
