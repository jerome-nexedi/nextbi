/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/IntersectFunDef.java#17 $
...
class IntersectFunDef extends FunDefBase {
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    return new AbstractListCalc(call, new Calc[] { listCalc1, listCalc2 }) {
      public TupleList evaluateList(Evaluator evaluator) {
        ...
        RetrievableSet<List<Member>> rightSet = new RetrievableHashSet<List<Member>>(
          rightList.size());
        for (List<Member> tuple : rightList) {
          rightSet.add(tuple);
        }

        final TupleList result = TupleCollections.createList(arity, Math.min(
        ...
        for (List<Member> leftTuple : leftList) {
          ...
          result
            .add(copyTupleWithVisualTotalsMembersOverriding(leftTuple, rightKey));
        }
        return result;
      }
       ...
       * overridden by any corresponding members from {@code rightKey} that
       * happen to be visual totals members.
       * 
       * <p>
       * Returns the original tuple if there are no visual totals members on the
       * RHS.
       * 
       * @param leftTuple
       *          Original tuple
       * @param rightKey
       *          Right tuple
       * @return Copy of original tuple, with any VisualTotalMembers from right
       *         tuple overriding
       */
   ...
   * {@link Object#equals(Object)} and {@link Object#hashCode()}, per the set
   * contract) and you wish to distinguish them after they have been added to
   * the set.
   * 
   * @param <E>
   *          element type
  ...
  private interface RetrievableSet<E> {
    /**
     * Returns the key in this set that compares equal to a given object, or
  ...
  private static class RetrievableHashSet<E> extends HashMap<E, E> implements
    RetrievableSet<E> {
