/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapCell.java#50 $
 ...
 * {@link RolapResult}.
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapCell.java#50 $
 */
public class RolapCell implements Cell {
  ...
  /**
   * Create an sql query that, when executed, will return the drill through data
   * for this cell. If the parameter extendedContext is true, then the query
   * will include all the levels (i.e. columns) of non-constraining members
   * (i.e. members which are at the "All" level). If the parameter
   * extendedContext is false, the query will exclude the levels (columns) of
   * non-constraining members.
   */
  public String getDrillThroughSQL(boolean extendedContext) {
    ...
    StarPredicate starPredicateSlicer = buildDrillthroughSlicerPredicate(
      currentMembers, result.getSlicerAxis());
    CellRequest cellRequest = RolapAggregationManager.makeDrillThroughRequest(
      currentMembers, extendedContext, result.getCube());
    return (cellRequest == null) ? null : aggMan.getDrillThroughSql(cellRequest,
      starPredicateSlicer, false);
  }
  ...
  public int getDrillThroughCount() {
    ...
    RolapConnection connection = (RolapConnection) result.getQuery().getConnection();
    final String sql = aggMan.getDrillThroughSql(cellRequest, starPredicateSlicer,
      true);

    final SqlStatement stmt = RolapUtil.executeQuery(connection.getDataSource(),
      sql, new Locus(new Execution(connection.createDummyStatement(), 0),
        "RolapCell.getDrillThroughCount", "Error while counting drill-through"));
    try {
   ...
   * <ol>
   * <li>Modify the passed-in array if any Member is overly restrictive. This
   * can happen if the slicer specifies multiple members in the same hierarchy.
   * In this scenario, the array of Members will contain an element for only the
   * last selected member in the hierarchy. This method will replace that Member
   * with the "All" Member from that hierarchy.</li>
   * <li>Create a {@link StarPredicate} representing the Positions indicated by
   * the slicer axis.</li>
   * </ol>
   * 
   * @param membersForDrillthrough
   *          the array of Members returned by
   *          {@link #getMembersForDrillThrough()}
   * @param slicerAxis
   *          the slicer {@link Axis}
   * @return an instance of <code>StarPredicate</code> representing all of the
   *         the positions from the slicer if it has more than one, or
   *         <code>null</code> otherwise.
   */
  private StarPredicate buildDrillthroughSlicerPredicate(
    Member[] membersForDrillthrough, Axis slicerAxis) {
    ...
    for (Position position : listOfPositions) {
      for (Member member : position) {
        ...
        if (!membersForDrillthrough[rolapHierarchy.getOrdinalInCube()]
          ...
          membersForDrillthrough[rolapHierarchy.getOrdinalInCube()] = rolapHierarchy
            .getAllMember();
        }
      }
    }
    ...
    // slicer axis
    List<StarPredicate> listOfStarPredicatesForSlicerPositions = new ArrayList<StarPredicate>();
    ...
    for (Position position : listOfPositions) {
      ...
      List<StarPredicate> listOfStarPredicatesForCurrentPosition = new ArrayList<StarPredicate>();
        ...
        if (!membersForDrillthrough[rolapHierarchy.getOrdinalInCube()]
          ...
          while (memberWalk != null && !memberWalk.isAll()) {
            ...
            if (memberWalk.getLevel() != levelLast) {
              ...
              RolapStar.Column column = rolapCubeMember.getLevel()
                .getBaseStarKeyColumn(result.getCube());
              // Add a predicate for the member at this level
              listOfStarPredicatesForCurrentPosition.add(new MemberColumnPredicate(
                column, rolapCubeMember));
            }
      ...
      StarPredicate starPredicateForCurrentSlicerPosition = new AndPredicate(
        listOfStarPredicatesForCurrentPosition);
  ...
  /**
   * Returns whether it is possible to drill through this cell. Drill-through is
   * possible if the measure is a stored measure and not possible for calculated
   * measures.
   * 
  ...
  public static RolapCube chooseDrillThroughCube(Member[] currentMembers,
    ...
    final DrillThroughVisitor visitor = new DrillThroughVisitor();
    try {
   ...
   * 
   * <p>
   * If tabFields is not null, returns the specified columns. (This option is
   * deprecated.)
   * 
   * @param maxRowCount
   ...
   */
  public SqlStatement drillThroughInternal(int maxRowCount, int firstRowOrdinal,
    ...
    return RolapUtil.executeQuery(connection.getDataSource(), sql, null,
      maxRowCount, firstRowOrdinal, new Locus(execution, "RolapCell.drillThrough",
        "Error in drill through"), resultSetType, resultSetConcurrency);
  }
  ...
  public Object getPropertyValue(String propertyName) {
    final boolean matchCase = MondrianProperties.instance().CaseSensitive.get();
  ...
  public void setValue(Scenario scenario, Object newValue,
    ...
    for (int i = 0; i < members.length; i++) {
      ...
      if (ScenarioImpl.isScenario(member.getHierarchy())) {
        ...
        throw Util.newError("Cannot write to cell: one of the coordinates ("
          + member.getUniqueName() + ") is a calculated member");
      }
    }
    if (scenario == null) {
    ...
    ((ScenarioImpl) scenario).setCellValue(result.getQuery().getConnection(), Arrays
      .asList(members), doubleNewValue, doubleCurrentValue, allocationPolicy,
      allocationArgs);
  }

  /**
   * Visitor that walks over a cell's expression and checks whether the cell
   * should allow drill-through. If not, throws the {@link #bomb} exception.
   * 
   * <p>
   * Examples:
   * </p>
   ...
   * <li>Calculated member with expression [Measures].[Unit Sales] + 1 is
   * drillable</li>
   * <li>Calculated member with expression ([Measures].[Unit Sales],
   * [Time].PrevMember) is not drillable</li>
   * </ul>
   */
  private static class DrillThroughVisitor extends MdxVisitorImpl {
    ...
    public Object visit(ResolvedFunCall call) {
      ...
      if (name.equals("+") || name.equals("-") || name.equals("/")
        || name.equals("*") || name.equals("CoalesceEmpty")
