/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapAggregationManager.java#61 $
 ...
 * {@link mondrian.rolap.agg.Aggregation}s in the system. It is a singleton
 * class.
 * 
 * <p>
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapAggregationManager.
 *          java#61 $
 */
public abstract class RolapAggregationManager {
  ...
  /**
   * Creates a request to evaluate the cell identified by <code>members</code>.
   * 
   * <p>
   * If any of the members is the null member, returns null, since there is no
   * cell. If the measure is calculated, returns null.
   * 
   * @param members
   *          Set of members which constrain the cell
   ...
   * @return Cell request, or null if the requst is unsatisfiable
   */
  public static CellRequest makeDrillThroughRequest(final Member[] members,
    final boolean extendedContext, RolapCube cube) {
    assert cube != null;
   ...
   * 
   * <p>
   * If any of the members from the context is the null member, returns null,
   * since there is no cell. If the measure is calculated, returns null.
   * 
   * @param evaluator
   *          the cell specified by the evaluator context
   * @return Cell request, or null if the requst is unsatisfiable
   */
  public static CellRequest makeRequest(RolapEvaluator evaluator) {
    final Member[] currentMembers = evaluator.getNonAllMembers();
    final List<List<List<Member>>> aggregationLists = evaluator
      .getAggregationLists();

    final RolapStoredMeasure measure = (RolapStoredMeasure) currentMembers[0];
    final RolapStar.Measure starMeasure = (RolapStar.Measure) measure
      .getStarMeasure();
    assert starMeasure != null;
    int starColumnCount = starMeasure.getStar().getColumnCount();

    CellRequest request = makeCellRequest(currentMembers, false, false, null);

    /*
     * Now setting the compound keys. First find out the columns referenced in
     * the aggregateMemberList. Each list defines a compound member.
     */
    ...
    for (List<List<Member>> aggregationList : aggregationLists) {
      ...
      // into groups.
      List<List<RolapMember>> rolapAggregationList = new ArrayList<List<RolapMember>>();
      ...
      unsatisfiable = makeCompoundGroup(starColumnCount, measure.getCube(),
        rolapAggregationList, compoundGroupMap);

      if (unsatisfiable) {
        return null;
      }
      compoundPredicate = makeCompoundPredicate(compoundGroupMap, measure.getCube());
  ...
  private static CellRequest makeCellRequest(final Member[] members,
    boolean drillThrough, final boolean extendedContext, RolapCube cube) {
    ...
    final RolapStar.Measure starMeasure = (RolapStar.Measure) measure
      .getStarMeasure();
    assert starMeasure != null;
    final CellRequest request = new CellRequest(starMeasure, extendedContext,
      drillThrough);
    ...
    if (extendedContext) {
      for (int i = 1; i < members.length; i++) {
        ...
        if (member.getHierarchy().getRolapHierarchy().closureFor != null) {
          continue;
        }
        addNonConstrainingColumns(member, measure.getCube(), request);

        final RolapCubeLevel level = member.getLevel();
        final boolean needToReturnNull = level.getLevelReader().constrainRequest(
  ...
  /**
   * Adds the key columns as non-constraining columns. For example, if they
   * asked for [Gender].[M], [Store].[USA].[CA] then the following levels are in
   * play:
   * <ul>
   * <li>Gender = 'M'
   ...
   * 
   * <p>
   * Note that [Marital Status] column is present by virtue of the implicit
   * [Marital Status].[All] member. Hence the SQL
   * 
   * <blockquote>
   * 
   * <pre>
   ...
   *          Cell request
   */
  private static void addNonConstrainingColumns(final RolapCubeMember member,
    final RolapCube baseCube, final CellRequest request) {
    final RolapCubeHierarchy hierarchy = member.getHierarchy();
    final RolapCubeLevel[] levels = hierarchy.getLevels();
    for (int j = levels.length - 1, depth = member.getLevel().getDepth(); j > depth; j--) {
   ...
   * 
   * <p>
   * This function returns a boolean value indicating if any constraint can be
   * created from the aggregationList. It is possible that only part of the
   * aggregationList can be applied, which still leads to a (partial) constraint
   * that is represented by the compoundGroupMap.
   */
  private static boolean makeCompoundGroup(int starColumnCount, RolapCube baseCube,
    ...
    for (List<RolapMember> aggregation : aggregationList) {
      ...
      if (aggregation.size() > 0
        && (aggregation.get(0) instanceof RolapCubeMember || aggregation.get(0) instanceof VisualTotalMember)) {
        isTuple = true;
      } else {
  ...
  private static void addTupleToCompoundGroupMap(RolapCubeMember[] tuple,
    BitKey bitKey, Map<BitKey, List<RolapCubeMember[]>> compoundGroupMap) {
  ...
  private static boolean makeCompoundGroupForMember(RolapCubeMember member,
    RolapCube baseCube, BitKey bitKey) {
   ...
   * compound member into {@link ListPredicate} by traversing a list of members
   * or tuples.
   * 
   * <p>
   * 1. The example below is for list of tuples
   * 
   * <blockquote> group 1: [Gender].[M], [Store].[USA].[CA]<br/>
   * group 2: [Gender].[F], [Store].[USA].[CA] </blockquote>
   * 
   * is translated into
   * 
   * <blockquote> (Gender=M AND Store_State=CA AND Store_Country=USA)<br/>
   * OR<br/>
   * (Gender=F AND Store_State=CA AND Store_Country=USA) </blockquote>
   * 
   * <p>
   * The caller of this method will translate this representation into
   * appropriate SQL form as <blockquote> where (gender = 'M'<br/>
   * and Store_State = 'CA'<br/>
   * AND Store_Country = 'USA')<br/>
   * OR (Gender = 'F'<br/>
   * and Store_State = 'CA'<br/>
   * AND Store_Country = 'USA') </blockquote>
   * 
   * <p>
   * 2. The example below for a list of members <blockquote> group 1:
   * [USA].[CA], [Canada].[BC]<br/>
   * group 2: [USA].[CA].[San Francisco], [USA].[OR].[Portland] </blockquote>
   * 
   * is translated into:
   * 
   * <blockquote> (Country=USA AND State=CA)<br/>
   * OR (Country=Canada AND State=BC)<br/>
   * OR (Country=USA AND State=CA AND City=San Francisco)<br/>
   * OR (Country=USA AND State=OR AND City=Portland) </blockquote>
   * 
   * <p>
   * The caller of this method will translate this representation into
   ...
   * 
   * <blockquote> where (country, state, city) IN ((USA, CA, San Francisco),
   * (USA, OR, Portland)) </blockquote>
   * 
   * or, if the DB does not support multi-value IN list:
   * 
   * <blockquote> where country=USA AND ((state=CA AND city = San Francisco) OR
   * (state=OR AND city=Portland)) </blockquote>
   * 
   * @param compoundGroupMap
   *          Map from dimensionality to groups
   * @param baseCube
   *          base cube if virtual
   * @return compound predicate for a tuple or a member
   */
  private static StarPredicate makeCompoundPredicate(
    Map<BitKey, List<RolapCubeMember[]>> compoundGroupMap, RolapCube baseCube) {
    ...
    for (List<RolapCubeMember[]> group : compoundGroupMap.values()) {
      ...
      for (RolapCubeMember[] tuple : group) {
        ...
        for (RolapCubeMember member : tuple) {
          tuplePredicate = makeCompoundPredicateForMember(member, baseCube,
            tuplePredicate);
        }
      ...
      if (compoundGroupPredicate != null) {
        /*
         * Sometimes the compound member list does not constrain any columns;
         * for example, if only AllLevel is present.
         */
  ...
  private static StarPredicate makeCompoundPredicateForMember(
    RolapCubeMember member, RolapCube baseCube, StarPredicate memberPredicate) {
    while (member != null) {
      RolapCubeLevel level = member.getLevel();
      if (!level.isAll()) {
        RolapStar.Column column = level.getBaseStarKeyColumn(baseCube);
        if (memberPredicate == null) {
          memberPredicate = new ValueColumnPredicate(column, member.getKey());
        } else {
          memberPredicate = memberPredicate.and(new ValueColumnPredicate(column,
            member.getKey()));
        }
      }
   ...
   * @return Cell value, or null if cell is not in any aggregation in cache, or
   *         {@link Util#nullValue} if cell's value is null
   */
  public abstract Object getCellFromCache(CellRequest request);

  public abstract Object getCellFromCache(CellRequest request, PinSet pinSet);
   ...
   * @param starPredicateSlicer
   *          A StarPredicate representing slicer positions that could not be
   *          represented by the CellRequest, or <code>null</code> if no
  ...
  public static RolapCacheRegion makeCacheRegion(final RolapStar star,
    final CacheControl.CellRegion region) {
    final List<Member> measureList = CacheControlImpl.findMeasures(region);
    final List<RolapStar.Measure> starMeasureList = new ArrayList<RolapStar.Measure>();
    ...
    final RolapCacheRegion cacheRegion = new RolapCacheRegion(star, starMeasureList);
    if (region instanceof CacheControlImpl.CrossjoinCellRegion) {
      final CacheControlImpl.CrossjoinCellRegion crossjoin = (CacheControlImpl.CrossjoinCellRegion) region;
  ...
  private static void constrainCacheRegion(final RolapCacheRegion cacheRegion,
    final RolapCube baseCube, final CacheControl.CellRegion region) {
    if (region instanceof CacheControlImpl.MemberCellRegion) {
      final CacheControlImpl.MemberCellRegion memberCellRegion = (CacheControlImpl.MemberCellRegion) region;
      ...
      for (Member member : memberList) {
        ...
        if (member instanceof RolapCubeMember) {
          ...
          rolapMember = (RolapCubeMember) baseCube.getSchemaReader()
            .getMemberByUniqueName(Util.parseIdentifier(member.getUniqueName()),
              true);
        }
        ...
        level.getLevelReader().constrainRegion(
          new MemberColumnPredicate(column, rolapMember), baseCube, cacheRegion);
      }
    } else if (region instanceof CacheControlImpl.MemberRangeCellRegion) {
      final CacheControlImpl.MemberRangeCellRegion rangeRegion = (CacheControlImpl.MemberRangeCellRegion) region;
      final RolapCubeLevel level = (RolapCubeLevel) rangeRegion.getLevel();
      RolapStar.Column column = level.getBaseStarKeyColumn(baseCube);

      level.getLevelReader().constrainRegion(
        new RangeColumnPredicate(column, rangeRegion.getLowerInclusive(),
          (rangeRegion.getLowerBound() == null ? null : new MemberColumnPredicate(
            column, rangeRegion.getLowerBound())), rangeRegion.getUpperInclusive(),
          (rangeRegion.getUpperBound() == null ? null : new MemberColumnPredicate(
            column, rangeRegion.getUpperBound()))), baseCube, cacheRegion);
    } else {
  ...
  /**
   * A set of segments which are pinned (prevented from garbage collection) for
   * a short duration as a result of a cache inquiry.
   */
