/*
// $Id: //open/mondrian/src/main/mondrian/rolap/sql/MemberListCrossJoinArg.java#5 $
...
/**
 * Represents an enumeration {member1, member2, ...}. All members must to the
 * same level and are non-calculated.
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/sql/MemberListCrossJoinArg
 *          .java#5 $
 */
public class MemberListCrossJoinArg implements CrossJoinArg {
  ...
  private MemberListCrossJoinArg(RolapLevel level, List<RolapMember> members,
    boolean restrictMemberTypes, boolean hasCalcMembers, boolean hasNonCalcMembers,
    boolean hasAllMember, boolean exclude) {
    this.level = level;
  ...
  /**
   * Creates an instance of {@link CrossJoinArg}, or returns null if the
   * arguments are invalid. This method also records properties of the member
   * list such as containing calc/non calc members, and containing the All
   * member.
   * 
   * <p>
   * If restrictMemberTypes is set, then the resulting argument could contain
   * calculated members. The newly created CrossJoinArg is marked appropriately
   * for special handling downstream.
   * 
   * <p>
   * If restrictMemberTypes is false, then the resulting argument contains
   * non-calculated members of the same level (after filtering out any null
   * members).
   * 
   * @param evaluator
   ...
   *          whether calculated members are allowed
   * @param exclude
   *          Whether to exclude tuples that match the predicate
   * @return MemberListCrossJoinArg if member list is well formed, null if not.
   */
  public static CrossJoinArg create(RolapEvaluator evaluator,
    final List<RolapMember> args, final boolean restrictMemberTypes, boolean exclude) {
    // First check that the member list will not result in a predicate
    // longer than the underlying DB could support.
    ...
    if ((level != null) && (!level.isSimple() && !supportedParentChild(level, args))) {
      return null;
    }
    ...
    return new MemberListCrossJoinArg(level, members, restrictMemberTypes,
      hasCalcMembers, hasNonCalcMembers, hasAllMember, exclude);
  }

  private static boolean supportedParentChild(RolapLevel level,
  ...
  public void addConstraint(SqlQuery sqlQuery, RolapCube baseCube, AggStar aggStar) {
    SqlConstraintUtils.addMemberConstraint(sqlQuery, baseCube, aggStar, members,
      restrictMemberTypes, true, exclude);
  }
   ...
   * 
   * <p>
   * This is used to selectively push down empty input arg into the native
   * evaluator.
   * 
