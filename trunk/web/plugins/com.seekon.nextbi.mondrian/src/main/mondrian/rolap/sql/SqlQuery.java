/*
// $Id: //open/mondrian/src/main/mondrian/rolap/sql/SqlQuery.java#122 $
...
/**
 * <code>SqlQuery</code> allows us to build a <code>select</code> statement and
 * generate it in database-specific SQL syntax.
 * 
 * <p>
 ...
 * <dd>Oracle (and all JDBC-compliant drivers) uses double-quotes, for example
 * <code>select * from "emp"</code>. Access prefers brackets, for example
 * <code>select * from [emp]</code>. mySQL allows single- and double-quotes for
 * string literals, and therefore does not allow identifiers to be quoted, for
 * example <code>select 'foo', "bar" from
 * emp</code>.</dd>
 * 
 ...
 * <dd>Some databases require that every column in the select list has a valid
 * alias. If the expression is an expression containing non-alphanumeric
 * characters, an explicit alias is needed. For example, Oracle will barfs at
 * <code>select empno + 1 from emp</code>.</dd>
 * 
 ...
 * <dd>In PostgreSQL and Oracle, index names must be unique within the database;
 * in Access and hsqldb, they must merely be unique within their table</dd>
 * 
 * <dt>Datatypes</dt>
 * <dd>In Oracle, BIT is CHAR(1), TIMESTAMP is DATE. In PostgreSQL, DOUBLE is
 * DOUBLE PRECISION, BIT is BOOL.</dd>
 * </ul>
 * 
 * <p>
 * NOTE: Instances of this class are NOT thread safe so the user must make sure
 * this is accessed by only one thread at a time.
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/sql/SqlQuery.java#122 $
 */
public class SqlQuery {
  ...
  /**
   * This list is used to keep track of what aliases have been used in the FROM
   * clause. One might think that a java.util.Set would be a more appropriate
   * Collection type, but if you only have a couple of "from aliases", then
   * iterating over a list is faster than doing a hash lookup (as is used in
   * java.util.HashSet).
   */
   ...
   * preferences from {@link MondrianProperties#GenerateFormattedSql} property.
   * 
   * @param dialect
   *          Dialect
   */
  ...
  /**
   * Chooses whether table optimization hints may be used (assuming the dialect
   * supports it).
   * 
   * @param t
  ...
  /**
   * Adds a subquery to the FROM clause of this Query with a given alias. If the
   * query already exists it either, depending on <code>failIfExists</code>,
   * throws an exception or does not add the query and returns false.
   * 
   * @param query
   *          Subquery
   * @param alias
   ...
   *          table name
   * @param alias
   *          table alias, may not be null (if not null, must not be zero
   *          length).
   * @param filter
   *          Extra filter condition, or null
   * @param hints
   *          table optimization hints, if any
   * @param failIfExists
   *          Whether to throw a RuntimeException if from clause already
   *          contains this alias
   * 
   * @pre alias != null
   * @return true if table was added
   */
  boolean addFromTable(final String schema, final String name, final String alias,
    final String filter, final Map hints, final boolean failIfExists) {
    if (fromAliases.contains(alias)) {
      if (failIfExists) {
        throw Util.newInternal("query already contains alias '" + alias + "'");
      } else {
  ...
  /**
   * Adds a relation to a query, adding appropriate join conditions, unless it
   * is already present.
   * 
   * <p>
   ...
   * @param failIfExists
   *          Whether to fail if relation is already present
   * @return true, if relation *was* added to query
   */
  public boolean addFrom(final MondrianDef.RelationOrJoin relation,
    final String alias, final boolean failIfExists) {
    ...
    if (relation instanceof MondrianDef.View) {
      ...
      final MondrianDef.Relation relation1 = RolapUtil.convertInlineTableToRelation(
      ...
      return addFromTable(table.schema, table.name, tableAlias, table.getFilter(),
        table.getHintMap(), failIfExists);

    } else if (relation instanceof MondrianDef.Join) {
      final MondrianDef.Join join = (MondrianDef.Join) relation;
      return addJoin(join.left, join.getLeftAlias(), join.leftKey, join.right, join
        .getRightAlias(), join.rightKey, failIfExists);
    } else {
      throw Util.newInternal("bad relation type " + relation);
    }
  }

  private boolean addJoin(MondrianDef.RelationOrJoin left, String leftAlias,
    String leftKey, MondrianDef.RelationOrJoin right, String rightAlias,
    String rightKey, boolean failIfExists) {
    ...
    if (added) {
      ...
      if (dialect.allowsJoinOn()) {
        from.addOn(leftAlias, leftKey, rightAlias, rightKey, condition);
      } else {
        addWhere(condition);
      }
    }
  ...
  private void addJoinBetween(MondrianDef.RelationOrJoin root,
    MondrianDef.Relation relation1, MondrianDef.Relation relation2) {
    ...
    for (int i = max - 1; i >= min; i--) {
      ...
      addJoin(relInfo.relation, relInfo.leftAlias != null ? relInfo.leftAlias
        : relInfo.relation.getAlias(), relInfo.leftKey,
        relations.get(i + 1).relation,
        relInfo.rightAlias != null ? relInfo.rightAlias
          : relations.get(i + 1).relation.getAlias(), relInfo.rightKey, false);
    }
  }
  ...
  /**
   * Adds an expression to the select clause, automatically creating a column
   * alias.
   */
   ...
   */
  public String addSelectGroupBy(final String expression, SqlStatement.Type type) {
  ...
  /**
   * Adds an expression to the select clause, with a specified type and column
   * alias.
   * 
   * @param expression
   *          Expression
   * @param type
   *          Java type to be used to hold cursor column
   * @param alias
   *          Column alias (or null for no alias)
   * @return Column alias
   */
  public String addSelect(final String expression, final SqlStatement.Type type,
    String alias) {
    buf.setLength(0);
  ...
  public void addWhere(final String exprLeft, final String exprMid,
  ...
  public void addWhere(RolapStar.Condition joinCondition) {
    ...
    if (fromAliases.contains(left) && fromAliases.contains(right)) {
      addWhere(joinCondition.getLeft(this), " = ", joinCondition.getRight(this));
    }
  }
   ...
   * 
   * @param expr
   *          the expr to order by
   * @param ascending
   *          sort direction
   * @param prepend
   *          whether to prepend to the current list of items
   * @param nullable
   ...
   */
  public void addOrderBy(String expr, boolean ascending, boolean prepend,
    boolean nullable, boolean collateNullsLast) {
  ...
  public void toBuffer(StringBuilder buf, String prefix) {
    ...
    if (groupingSets.isEmpty()) {
      groupBy
        .toBuffer(buf, generateFormattedSql, prefix, " group by ", ", ", "", "");
    } else {
      ClauseList.listToBuffer(buf, groupingSets, generateFormattedSql, prefix,
  ...
  public Pair<String, List<SqlStatement.Type>> toSqlAndTypes() {
    assert types.size() == select.size() + groupingFunctions.size() : types.size()
      + " types, " + (select.size() + groupingFunctions.size())
  ...
  private void flatten(List<RelInfo> relations, MondrianDef.RelationOrJoin root,
    String leftKey, String leftAlias, String rightKey, String rightAlias) {
    if (root instanceof MondrianDef.Join) {
      MondrianDef.Join join = (MondrianDef.Join) root;
      flatten(relations, join.left, join.leftKey, join.leftAlias, join.rightKey,
        join.rightAlias);
      flatten(relations, join.right, leftKey, leftAlias, rightKey, rightAlias);
    } else {
      relations.add(new RelInfo((MondrianDef.Relation) root, leftKey, leftAlias,
        rightKey, rightAlias));
    }
  }
  ...
  static class FromClauseList extends ClauseList {
    ...
    public void addOn(String leftAlias, String leftKey, String rightAlias,
      String rightKey, String condition) {
      ...
      joinOnClauses.add(new JoinOnClause(condition, leftAlias, rightAlias));
    }
    ...
    void appendJoin(final List<String> addedTables, final String from,
      final String alias, final StringBuilder buf) {
      int n = 0;
      ...
      for (JoinOnClause joinOnClause : joinOnClauses) {
        ...
        if ((addedTables.size() == 1 && addedTables.get(0).equals(joinOnClause.left) && joinOnClause.left
          .equals(joinOnClause.right))
          || (alias.equals(joinOnClause.left) && addedTables
            .contains(joinOnClause.right))
          || (alias.equals(joinOnClause.right) && addedTables
            .contains(joinOnClause.left))) {
          if (n++ == 0) {
      ...
      if (n == 0) {
        ...
        buf.append(joinOnClauses.isEmpty() ? ", " : " cross join ").append(from);
      }
    }
  }
  ...
  static class ClauseList extends ArrayList<String> {
    ...
    /**
     * Adds an element to this ClauseList if either duplicates are allowed or if
    ...
    final void toBuffer(StringBuilder buf, boolean generateFormattedSql,
      String prefix, String first, String sep, String last, String empty) {
      if (isEmpty()) {
        buf.append(empty);
        return;
      }
    ...
    static String foo(boolean generateFormattedSql, String prefix, String s) {
      if (generateFormattedSql) {
        ...
        if (s.endsWith(" ")) {
          // E.g. ", "
          s = s.substring(0, s.length() - 1) + Util.nl + prefix + INDENT;
        } else if (s.endsWith("(")) {
          // E.g. "("
    ...
    final void toBuffer(final StringBuilder buf, final String first,
    ...
    static void listToBuffer(StringBuilder buf, List<ClauseList> clauseListList,
      boolean generateFormattedSql, String prefix, String first, String sep,
      String last) {
      first = foo(generateFormattedSql, prefix, first);
      sep = foo(generateFormattedSql, prefix, sep);
      buf.append(first);
  ...
  private static class RelInfo {
    ...
    public RelInfo(MondrianDef.Relation relation, String leftKey, String leftAlias,
      String rightKey, String rightAlias) {
