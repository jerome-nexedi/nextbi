/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapDependencyTestingEvaluator.java#20 $
 ...
 * 
 * <p>
 * For each expression evaluation, this valuator evaluates each expression more
 * times, and makes sure that the results of the expression are independent of
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapDependencyTestingEvaluator
 *          .java#20 $
 */
public class RolapDependencyTestingEvaluator extends RolapEvaluator {
   ...
   *          Parent evaluator
   */
  private RolapDependencyTestingEvaluator(RolapEvaluatorRoot root,
    RolapDependencyTestingEvaluator evaluator, List<List<Member>> aggregationList) {
    super(root, evaluator, aggregationList);
  }

  public Object evaluate(Calc calc, Hierarchy[] independentHierarchies,
    String mdxString) {
    final DteRoot dteRoot = (DteRoot) root;
    if (dteRoot.faking) {
    ...
    if (dteRoot.disabled
      || dteRoot.faking
      || isNonEmpty()
      || (double) dteRoot.fakeCallCount > (double) dteRoot.callCount
        * dteRoot.random.nextDouble() * 2 * dteRoot.expDeps) {
      return result;
    }
    ...
    final Member otherMember = dteRoot.chooseOtherMember(saveMember, getQuery()
      .getSchemaReader(false));
    setContext(otherMember);
    final Object otherResult = calc.evaluate(this);
    if (false) {
      System.out.println("original=" + saveMember.getUniqueName() + ", member="
        + otherMember.getUniqueName() + ", originalResult=" + result + ", result="
        + otherResult);
    }
    if (!equals(otherResult, result)) {
      ...
      throw Util.newInternal("Expression '" + mdxString
        + "' claims to be independent of dimension " + saveMember.getDimension()
        + " but is not; context is {" + buf.toString() + "}; First result: "
        + toString(result) + ", Second result: " + toString(otherResult));
    }
    // Restore context.
  ...
  private boolean equals(Object o1, Object o2) {
    ...
    if (o1 instanceof List) {
      return o2 instanceof List
        && equals(((List) o1).toArray(), ((List) o2).toArray());
    }
  ...
  static class DteRoot extends RolapResult.RolapResultEvaluatorRoot {
    ...
    final Random random = Util.createRandom(MondrianProperties.instance().TestSeed
      .get());
    ...
    /**
     * Chooses another member of the same hierarchy. The member will come from
     * all levels with the same probability. Calculated members are not
     * included.
     * 
     * @param save
     *          Previous member
     ...
     */
    private Member chooseOtherMember(final Member save, SchemaReader schemaReader) {
  ...
  /**
   * Expression which checks dependencies and list immutability of an underlying
  ...
  private static class DteIterCalcImpl extends GenericIterCalc {
    ...
    DteIterCalcImpl(Calc calc, Hierarchy[] independentHierarchies,
      boolean mutableList, String mdxString) {
      super(new DummyExp(calc.getType()));
      this.calc = calc;
      this.independentHierarchies = independentHierarchies;
  ...
  static class DteCompiler extends DelegatingExpCompiler {
    ...
    /**
     * Returns the dimensions an expression does not depend on. If the current
     * member of any of these dimensions changes, the expression will return the
     * same result.
     * 
     * @param calc
     *          Expression
    ...
    private Hierarchy[] getIndependentHierarchies(Calc calc) {
      ...
      final List<RolapHierarchy> hierarchies = ((RolapCube) getValidator()
        .getQuery().getCube()).getHierarchies();
