/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapSchemaReader.java#69 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapSchemaReader.java#69 $
 * @since Feb 24, 2003
 */
public class RolapSchemaReader implements SchemaReader,
  RolapNativeSet.SchemaReaderWithMemberReaderAvailable, NameResolver.Namespace {
  ...
  public void getMemberRange(Level level, Member startMember, Member endMember,
    List<Member> list) {
    getMemberReader(level.getHierarchy()).getMemberRange((RolapLevel) level,
      (RolapMember) startMember, (RolapMember) endMember,
      Util.<RolapMember> cast(list));
  }
  ...
  public int getMemberDepth(Member member) {
    final Role.HierarchyAccess hierarchyAccess = role.getAccessDetails(member
      .getHierarchy());
    if (hierarchyAccess != null) {
  ...
  public List<Member> getMemberChildren(Member member, Evaluator context) {
    MemberChildrenConstraint constraint = sqlConstraintFactory
      .getMemberChildrenConstraint(context);
   ...
   */
  private List<RolapMember> internalGetMemberChildren(Member member,
  ...
  public void getParentChildContributingChildren(Member dataMember,
    Hierarchy hierarchy, List<Member> list) {
  ...
  public int getChildrenCountFromCache(Member member) {
    ...
    if (memberReader instanceof RolapCubeHierarchy.RolapCubeHierarchyMemberReader) {
      List list = ((RolapCubeHierarchy.RolapCubeHierarchyMemberReader) memberReader)
        .getRolapCubeMemberCacheHelper().getChildrenFromCache((RolapMember) member,
          null);
      if (list == null) {
        return -1;
      }
    ...
    List list = ((MemberCache) memberReader).getChildrenFromCache(
  ...
  /**
   * Returns number of members in a level, if the information can be retrieved
   * from cache. Otherwise {@link Integer#MIN_VALUE}.
   * 
   * @param level
   *          Level
  ...
  private int getLevelCardinalityFromCache(Level level) {
    ...
    if (memberReader instanceof RolapCubeHierarchy.RolapCubeHierarchyMemberReader) {
      ...
      final List<RolapMember> list = cache.getLevelMembersFromCache(
    ...
    if (memberReader instanceof SmartMemberReader) {
      List<RolapMember> list = ((SmartMemberReader) memberReader).getMemberCache()
    ...
    if (memberReader instanceof MemberCache) {
      List<RolapMember> list = ((MemberCache) memberReader)
        .getLevelMembersFromCache((RolapLevel) level, null);
      if (list == null) {
        return Integer.MIN_VALUE;
      }
  ...
  public int getLevelCardinality(Level level, boolean approximate,
    boolean materialize) {
  ...
  public SchemaReader withoutAccessControl() {
    assert this.getClass() == RolapSchemaReader.class : "Subclass " + getClass()
      + " must override";
  ...
  public final Member getMemberByUniqueName(List<Id.Segment> uniqueNameParts,
    ...
    return getMemberByUniqueName(uniqueNameParts, failIfNotFound, MatchType.EXACT);
  }
  ...
  public OlapElement lookupCompound(OlapElement parent, List<Id.Segment> names,
  ...
  public final OlapElement lookupCompound(OlapElement parent,
    List<Id.Segment> names, boolean failIfNotFound, int category, MatchType matchType) {
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      return new NameResolver().resolve(parent, Util.toOlap4j(names),
        failIfNotFound, category, matchType, getNamespaces());
    }
    return lookupCompoundInternal(parent, names, failIfNotFound, category, matchType);
  }
  ...
  public OlapElement lookupChild(OlapElement parent, IdentifierSegment segment,
    MatchType matchType) {
    OlapElement element = getElementChild(parent, Util.convert(segment), matchType);
    if (element != null) {
      return element;
    }
  ...
  public Member lookupMemberChildByName(Member parent, Id.Segment childName,
    ...
    try {
      ...
      if (children.size() > 0) {
        return RolapUtil.findBestMemberMatch(children, (RolapMember) parent,
          children.get(0).getLevel(), childName, matchType, true);
      }
      ...
      LOGGER.debug("NumberFormatException in lookupMemberChildByName "
        + "for parent = \"" + parent + "\", childName=\"" + childName
  ...
  public List<Member> getLevelMembers(Level level, Evaluator context) {
    TupleConstraint constraint = sqlConstraintFactory.getLevelMembersConstraint(
      context, new Level[] { level });
    final MemberReader memberReader = getMemberReader(level.getHierarchy());
    List<RolapMember> membersInLevel = memberReader.getMembersInLevel(
  ...
  public List<Level> getHierarchyLevels(Hierarchy hierarchy) {
    ...
    List<Level> restrictedLevels = Arrays.asList(levels).subList(
  ...
  public boolean isDrillable(Member member) {
    ...
    if (level.getParentExp() != null) {
      ...
      return (childLevel != null) && (role.getAccess(childLevel) != Access.NONE);
    }
  }
  ...
  public NativeEvaluator getNativeSetEvaluator(FunDef fun, Exp[] args,
    Evaluator evaluator, Calc calc) {
    RolapEvaluator revaluator = (RolapEvaluator) AbstractCalc.simplifyEvaluator(
      calc, evaluator);
  ...
  public SchemaReader withLocus() {
    return RolapUtil.locusSchemaReader(schema.getInternalConnection(), this);
  }

  /**
   * Implementation of {@link Parameter} which is sourced from system propertes
   * (see {@link System#getProperties()} or mondrian properties (see
   * {@link MondrianProperties}.
   * <p/>
   * <p>
  ...
  private static class SystemPropertyParameter extends ParameterImpl {
    /**
     * true if source is a system property; false if source is a mondrian
     * property.
     */
    ...
    public SystemPropertyParameter(String name, boolean system) {
      super(name, Literal.nullValue, "System property '" + name + "'",
        new StringType());
      this.system = system;
      this.propertyDefinition = system ? null : MondrianProperties.instance()
