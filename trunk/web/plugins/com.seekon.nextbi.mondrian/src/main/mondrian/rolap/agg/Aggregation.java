...
public class Aggregation {
  ...
  public void load(RolapStar.Column[] columns, RolapStar.Measure[] measures,
    StarColumnPredicate[] predicates, RolapAggregationManager.PinSet pinnedSegments,
    ...
    List<Segment> segments = addSegmentsToAggregation(measures, measureBitKey, axes,
      pinnedSegments);
    // The constrained columns are simply the level and foreign columns
    BitKey levelBitKey = getConstrainedColumnsBitKey();
    GroupingSet groupingSet = new GroupingSet(segments, levelBitKey, measureBitKey,
      axes, columns);
  ...
  private List<Segment> addSegmentsToAggregation(RolapStar.Measure[] measures,
    BitKey measureBitKey, Axis[] axes, RolapAggregationManager.PinSet pinnedSegments) {
    ...
    for (RolapStar.Measure measure : measures) {
      ...
      Segment segment = new Segment(this, measure, axes, Collections
        .<Segment.Region> emptyList());
  ...
  public void flush(CacheControl cacheControl, RolapCacheRegion cacheRegion) {
    ...
    segmentLoop: for (SoftReference<Segment> segmentRef : segmentRefs) {
      ...
      for (StarPredicate predicate : cacheRegion.getPredicates()) {
        ValuePruner pruner = new ValuePruner(predicate, segment.axes, segment
          .getData());
   ...
   * 
   * </code>
   * </blockquote>applied to the segment <blockquote><code>
   * 
