/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/AndPredicate.java#11 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/AndPredicate.java#11 $
 */
public class AndPredicate extends ListPredicate {
  ...
  public BitKey checkInList(SqlQuery sqlQuery, BitKey inListLHSBitKey) {
    ...
    if (!getConstrainedColumnBitKey().equals(inListLHSBitKey)
      || (children.size() > 1 && !sqlQuery.getDialect().supportsMultiValueInExpr())) {
      ...
      for (StarPredicate predicate : children) {
        ...
        if (predicate instanceof ValueColumnPredicate) {
          ...
          if (columnPred.getValue() == RolapUtil.sqlNullValue) {
            ...
            inListRHSBitKey
              .clear(columnPred.getConstrainedColumn().getBitPosition());
          }
  ...
  /*
   * Generate value list for this predicate to be used in an IN-list sql
   * predicate.
   * 
   * The values in a multi-column IN list predicates are generated in the same
   * order, based on the bit position from the columnBitKey.
   */
  public void toInListSql(SqlQuery sqlQuery, StringBuilder buf,
    ...
    buf.append("(");
    /*
     * Arranging children according to the bit position. This is required as RHS
     * of IN list needs to list the column values in the same order.
     */
    Set<ValueColumnPredicate> sortedPredicates = new TreeSet<ValueColumnPredicate>();
    ...
    for (StarPredicate predicate : children) {
      ...
      if (inListRHSBitKey.get(((ValueColumnPredicate) predicate)
        .getConstrainedColumn().getBitPosition())) {
    ...
    for (ValueColumnPredicate predicate : sortedPredicates) {
      ...
      sqlQuery.getDialect().quote(buf, predicate.getValue(),
