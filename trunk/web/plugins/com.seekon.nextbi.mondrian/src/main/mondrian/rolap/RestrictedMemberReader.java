/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RestrictedMemberReader.java#19 $
 ...
 * @since Feb 26, 2003
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RestrictedMemberReader.java
 *          #19 $
 */
class RestrictedMemberReader extends DelegatingMemberReader {
   ...
   * 
   * <p>
   * There's no filtering to be done unless either the role has restrictions on
   * this hierarchy, or the hierarchy is ragged; there's a pre-condition to this
   * effect.
   * </p>
   * 
   * @param memberReader
   ...
   * @param role
   *          Role whose access profile to obey. The role must have restrictions
   *          on this hierarchy
  ...
  private void processMemberChildren(List<RolapMember> fullChildren,
    List<RolapMember> children, MemberChildrenConstraint constraint) {
   ...
   *          Output list
   */
  private void filterMembers(List<RolapMember> members,
  ...
  public synchronized void getMemberChildren(List<RolapMember> parentMembers,
    List<RolapMember> children, MemberChildrenConstraint constraint) {
  ...
  public List<RolapMember> getRootMembers() {
    ...
    if (topLevelDepth > 0) {
      ...
      final List<RolapMember> memberList = getMembersInLevel(topLevel, 0,
        Integer.MAX_VALUE);
      if (memberList.isEmpty()) {
        throw MondrianResource.instance().HierarchyHasNoAccessibleMembers
  ...
  public List<RolapMember> getMembersInLevel(RolapLevel level, int startOrdinal,
    ...
    final List<RolapMember> membersInLevel = memberReader.getMembersInLevel(level,
