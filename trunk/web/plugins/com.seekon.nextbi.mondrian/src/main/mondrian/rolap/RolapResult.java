...
public class RolapResult extends ResultBase {
  ...
  RolapResult(final Execution execution, boolean execute) {
    ...
    try {
      ...
      RolapEvaluator slicerEvaluator;
      do {
        TupleIterable tupleIterable = evalExecute(nonAllMembers, nonAllMembers
          .size() - 1, savedEvaluator, query.getSlicerAxis(), query.slicerCalc);
        ...
        if (tupleList.size() > 1) {
          ...
          final Calc calc = new GenericCalc(new DummyExp(query.slicerCalc.getType())) {
            public Object evaluate(Evaluator evaluator) {
              return AggregateFunDef.AggregateCalc.aggregate(valueCalc, evaluator,
                tupleList1);
            }
          };
          ...
          evaluator.addCalculation(new RolapTupleCalculation(hierarchyList, calc),
            true);
        }
      ...
      do {
        ...
        do {
          ...
          for (int i = 0; i < axes.length; i++) {
            ...
            this.axes[i] = new RolapAxis(TupleCollections.materialize(tupleIterable,
              false));
          }
        } while (redo);
      } while (batchingReader.loadAggregations(query));
  ...
  protected void loadMembers(List<List<Member>> nonAllMembers,
    RolapEvaluator evaluator, QueryAxis axis, Calc calc, AxisMember axisMembers) {
    int attempt = 0;
    ...
    while (true) {
      ...
      if (count++ > maxEvalDepth) {
        if (evaluator instanceof RolapDependencyTestingEvaluator) {
          ...
          throw Util
            .newInternal("Query required more than " + count + " iterations");
        }
      }
  ...
  protected static class RolapResultEvaluatorRoot extends RolapEvaluatorRoot {
    ...
    protected Evaluator.NamedSetEvaluator evaluateNamedSet(final NamedSet namedSet,
      boolean create) {
    ...
    public Object getParameterValue(ParameterSlot slot) {
      ...
      if (liftedValue != null) {
        if (liftedValue == CycleSentinel) {
          throw MondrianResource.instance().CycleDuringParameterEvaluation.ex(slot
