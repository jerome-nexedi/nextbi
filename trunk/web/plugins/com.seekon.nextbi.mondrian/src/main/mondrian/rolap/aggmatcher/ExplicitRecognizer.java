/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/ExplicitRecognizer.java#21 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/ExplicitRecognizer
 *          .java#21 $
 */
class ExplicitRecognizer extends Recognizer {
  ...
  ExplicitRecognizer(final ExplicitRules.TableDef tableDef, final RolapStar star,
    RolapCube cube, final JdbcSchema.Table dbFactTable,
   ...
   * <p>
   * First, iterate through all of the columns in the table. For each column,
   * iterate through all of the tableDef measures, the explicit definitions of a
   * measure. If the table's column name matches the column name in the measure
   * definition, then make a measure. Next, look through all of the fact table
   * column usage measures. For each such measure usage that has a sibling
   * foreign key usage see if the tableDef has a foreign key defined with the
   * same name. If so, then, for free, we can make a measure for the aggregate
   * using its foreign key.
   * <p>
   * 
      ...
      for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
        ...
        for (ExplicitRules.TableDef.Measure measure : getTableDef().getMeasures()) {
          ...
          if (measure.getColumnName().equalsIgnoreCase(aggColumnName)) {
            ...
            RolapStar.Measure m = star.getFactTable().lookupMeasureByName(
  ...
  protected void makeMeasure(final ExplicitRules.TableDef.Measure measure,
    RolapAggregator factAgg, final JdbcSchema.Table.Column aggColumn) {
    ...
    RolapAggregator ra = (factAgg == null) ? convertAggregator(aggUsage, rm
      .getAggregator()) : convertAggregator(aggUsage, factAgg, rm.getAggregator());
   ...
   * search for a foreign key definition in the ExplicitRules.tableDef. If not
   * found, thats ok, it is just a lost dimension. If found, look for a column
   * in the aggregate table with that name and make a foreign key usage.
   */
   ...
   * aggregate table's columns for one with that name and make a level usage for
   * the column. Return true if the aggregate table's column was found.
   */
  protected boolean matchLevel(final Hierarchy hierarchy,
    final HierarchyUsage hierarchyUsage, final RolapLevel rlevel) {
        ...
        if (level.getName().equals(levelUniqueName)) {
          ...
          for (JdbcSchema.Table.Column aggColumn : aggTable.getColumns()) {
            if (aggColumn.getName().equals(columnName)) {
              makeLevel(aggColumn, hierarchy, hierarchyUsage, getColumnName(rlevel
                .getKeyExp()), columnName, rlevel.getName());
              return true;
            }
          }
        }
