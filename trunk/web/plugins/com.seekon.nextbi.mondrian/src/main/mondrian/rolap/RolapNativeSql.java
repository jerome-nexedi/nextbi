 ...
 * @author av
 * @since Nov 17, 2005
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapNativeSql.java#31
 *          $
 */
public class RolapNativeSql {
  ...
  /**
   * We remember one of the measures so we can generate the constraints from
   * RolapAggregationManager. Also make sure all measures live in the same star.
   * 
  ...
  interface SqlCompiler {
    /**
     * Returns SQL. If <code>exp</code> can not be compiled into SQL, returns
     * null.
     * 
     * @param exp
     *          Expression
  ...
  /**
   * Implementation of {@link SqlCompiler} that uses chain of responsibility to
   * find a matching sql compiler.
   */
  ...
  class NumberSqlCompiler implements SqlCompiler {
    public String compile(Exp exp) {
      ...
      if (dialect.getDatabaseProduct().getFamily() == Dialect.DatabaseProduct.DB2) {
        expr = "FLOAT(" + expr + ")";
      }
      return expr;
    }
  ...
  /**
   * Compiles a measure into SQL, the measure will be aggregated like
   * <code>sum(measure)</code>.
   */
  class StoredMeasureSqlCompiler extends MemberSqlCompiler {
    ...
    public String compile(Exp exp) {
      ...
      if (aggStar != null && measure.getStarMeasure() instanceof RolapStar.Column) {
  ...
  class MatchingSqlCompiler extends FunCallSqlCompilerBase {
    ...
    public String compile(Exp exp) {
      ...
      if (!dialect.allowsRegularExpressionInWhereClause()
        || !(exp instanceof ResolvedFunCall) || evaluator == null) {
        return null;
      }
      ...
      if (!(arg0 instanceof ResolvedFunCall)
        ...
        || (!((ResolvedFunCall) arg0).getFunName().equals("Name") && !((ResolvedFunCall) arg0)
      ...
      if (dimExpr instanceof DimensionExpr) {
        dimension = (RolapCubeDimension) evaluator
          .getCachedResult(new ExpCacheDescriptor(dimExpr, evaluator));
      } else if (dimExpr instanceof HierarchyExpr) {
        final RolapCubeHierarchy hierarchy = (RolapCubeHierarchy) evaluator
          .getCachedResult(new ExpCacheDescriptor(dimExpr, evaluator));
        dimension = (RolapCubeDimension) hierarchy.getDimension();
      } else if (dimExpr instanceof LevelExpr) {
        final RolapCubeLevel level = (RolapCubeLevel) evaluator
          .getCachedResult(new ExpCacheDescriptor(dimExpr, evaluator));
      ...
      if (rolapLevel != null && dimension.equals(rolapLevel.getDimension())) {
        ...
        MondrianDef.Expression expression = useCaption ? rolapLevel.captionExp == null ? rolapLevel.nameExp == null ? rolapLevel.keyExp
          : rolapLevel.nameExp
          : rolapLevel.captionExp
          : rolapLevel.nameExp == null ? rolapLevel.keyExp : rolapLevel.nameExp;
        /*
         * If an aggregation table is used, it might be more efficient to use
         * only the aggregate table and not the hierarchy table. Try to lookup
         * the column bit key. If that fails, we will link the aggregate table
         * to the hierarchy table. If no aggregate table is used, we can use the
         * column expression directly.
         */
        String sourceExp;
        if (aggStar != null && rolapLevel instanceof RolapCubeLevel
          ...
          int bitPos = ((RolapCubeLevel) rolapLevel).getStarKeyColumn()
            .getBitPosition();
          mondrian.rolap.aggmatcher.AggStar.Table.Column col = aggStar
            .lookupColumn(bitPos);
          if (col != null) {
        ...
        return dialect.generateRegularExpression(sourceExp, String.valueOf(evaluator
          .getCachedResult(new ExpCacheDescriptor(arg1, evaluator))));
      } else {
        return null;
      }
    }
  ...
  abstract class FunCallSqlCompilerBase implements SqlCompiler {
     ...
     * 
     * @return array of expressions or null if either exp does not match or any
     *         argument could not be compiled.
     */
  ...
  class UnaryOpSqlCompiler extends FunCallSqlCompiler {
    protected UnaryOpSqlCompiler(int category, String mdx, String sql,
      SqlCompiler argumentCompiler) {
  ...
  /**
   * Compiles an <code>IsEmpty(measure)</code> expression into SQL
   * <code>measure is null</code>.
   */
   ...
   * 
   * @param sqlQuery
   *          the query which is needed for different SQL dialects - it is not
   *          modified
   */
  public RolapNativeSql(SqlQuery sqlQuery, AggStar aggStar, Evaluator evaluator,
    ...
    numericCompiler
      .add(new ParenthesisSqlCompiler(Category.Numeric, numericCompiler));
    numericCompiler.add(new InfixOpSqlCompiler(Category.Numeric, "+", "+",
      numericCompiler));
    numericCompiler.add(new InfixOpSqlCompiler(Category.Numeric, "-", "-",
      numericCompiler));
    numericCompiler.add(new InfixOpSqlCompiler(Category.Numeric, "/", "/",
    ...
    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, "<", "<",
      numericCompiler));
    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, "<=", "<=",
      numericCompiler));
    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, ">", ">",
    ...
    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, "<>", "<>",
      numericCompiler));
    booleanCompiler.add(new IsEmptySqlCompiler(Category.Logical, "IsEmpty",
      numericCompiler));

    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, "and", "AND",
      booleanCompiler));
    booleanCompiler.add(new InfixOpSqlCompiler(Category.Logical, "or", "OR",
      booleanCompiler));
    booleanCompiler.add(new UnaryOpSqlCompiler(Category.Logical, "not", "NOT",
      booleanCompiler));
    booleanCompiler.add(new MatchingSqlCompiler());
    booleanCompiler
      .add(new ParenthesisSqlCompiler(Category.Logical, booleanCompiler));
    booleanCompiler.add(new IifSqlCompiler(Category.Logical, booleanCompiler));
  }

  /**
   * Generates an aggregate of a measure, e.g. "sum(Store_Sales)" for TopCount.
   * The returned expr will be added to the select list and to the order by
   * clause.
   */
