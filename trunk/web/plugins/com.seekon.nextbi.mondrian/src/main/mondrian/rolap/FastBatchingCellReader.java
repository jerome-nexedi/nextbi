/*
// $Id: //open/mondrian/src/main/mondrian/rolap/FastBatchingCellReader.java#78 $
 ...
 * to look up the values of stored measures, it lies, and records the fact that
 * the value was asked for. Later, we can look over the values which are
 * required, fetch them in an efficient way, and re-run the evaluation with a
 * real evaluator.
 * 
 * <p>
 * NOTE: When it doesn't know the answer, it lies by returning an error object.
 * The calling code must be able to deal with that.
 * </p>
 * 
 * <p>
 * This class tries to minimize the amount of storage needed to record the fact
 * that a cell was requested.
 * </p>
 */
public class FastBatchingCellReader implements CellReader {

  private static final Logger LOGGER = Logger
    .getLogger(FastBatchingCellReader.class);
  ...
  /**
   * Records the number of requests. The field is used for correctness: if the
   * request count stays the same during an operation, you know that the
  ...
  private final RolapAggregationManager.PinSet pinnedSegments = aggMgr
    .createPinSet();
  ...
  public Object get(RolapEvaluator evaluator) {
    final CellRequest request = RolapAggregationManager.makeRequest(evaluator);
  ...
  public final void recordCellRequest(CellRequest request) {
    ...
    if (batch == null) {
      ...
      if (LOGGER.isDebugEnabled()) {
        ...
        final RolapStar.Column[] columns = request.getConstrainedColumns();
  ...
  /**
   * Returns whether this reader has told a lie. This is the case if there are
   * pending batches to load or if {@link #setDirty(boolean)} has been called.
   */
   ...
   * 
   * @param query
   *          the parent query object that initiated this call
   * 
  ...
  List<CompositeBatch> groupBatches(List<Batch> batchList) {
    Map<AggregationKey, CompositeBatch> batchGroups = new HashMap<AggregationKey, CompositeBatch>();
    ...
    final CompositeBatch[] compositeBatches = batchGroups.values().toArray(
  ...
  private void wrapNonBatchedBatchesWithCompositeBatches(List<Batch> batchList,
  ...
  void addToCompositeBatch(Map<AggregationKey, CompositeBatch> batchGroups,
    ...
    FastBatchingCellReader.CompositeBatch compositeBatchOfSummaryBatch = batchGroups
      .remove(summaryBatch.batchKey);
  ...
  class CompositeBatch implements Loadable {
    ...
    public void loadAggregation() {
      ...
      getSegmentLoader().load(batchCollector.getGroupingSets(), pinnedSegments,
  ...
  /**
   * Encapsulates a common property of {@link Batch} and {@link CompositeBatch},
   * namely, that they can be asked to load their aggregations into the cache.
   */
  interface Loadable {
  ...
  class Batch implements Loadable {
    ...
    public String toString() {
      if (string == null) {
        ...
        buf.append("Batch {\n").append("  columns={").append(
          Arrays.toString(columns)).append("}\n").append("  measures={").append(
          measuresList).append("}\n").append("  valueSets={").append(
          Arrays.toString(valueSets)).append("}\n").append("  batchKey=").append(
          batchKey).append("}\n").append("}");
        string = buf.toString();
      }
      return string;
    ...
    public final void add(CellRequest request) {
      ...
      if (!measuresList.contains(measure)) {
        assert (measuresList.size() == 0)
          || (measure.getStar() == (measuresList.get(0)).getStar()) : "Measure must belong to same star as other measures";
     ...
     * 
     * <p>
     * This method can only be called after the {@link #add} method has been
     * called.
     * 
    ...
    public final void loadAggregation() {
      GroupingSetsCollector collectorWithGroupingSetsTurnedOff = new GroupingSetsCollector(
        false);
    ...
    final void loadAggregation(GroupingSetsCollector groupingSetsCollector) {
      ...
      boolean tooManyDistinctMeasures = distinctMeasureCount > 0
        && !dialect.allowsCountDistinct() || distinctMeasureCount > 1
      ...
      if (!dialect.allowsMultipleDistinctSqlMeasures()) {
        ...
        List<RolapStar.Measure> distinctSqlMeasureList = getDistinctSqlMeasures(measuresList);
        for (RolapStar.Measure measure : distinctSqlMeasureList) {
          RolapStar.Measure[] measures = { measure };
          aggmgr.loadAggregation(measures, columns, batchKey, predicates,
      ...
      if (measureCount > 0) {
        final RolapStar.Measure[] measures = measuresList
          .toArray(new RolapStar.Measure[measureCount]);
        aggmgr.loadAggregation(measures, columns, batchKey, predicates,
          pinnedSegments, groupingSetsCollector);
      }
    ...
    private void doSpecialHandlingOfDistinctCountMeasures(AggregationManager aggmgr,
      StarColumnPredicate[] predicates, GroupingSetsCollector groupingSetsCollector) {
      while (true) {
        // Scan for a measure based upon a distinct aggregation.
        final RolapStar.Measure distinctMeasure = getFirstDistinctMeasure(measuresList);
        if (distinctMeasure == null) {
          break;
        }
        final String expr = distinctMeasure.getExpression().getGenericExpression();
        ...
        // together
        final RolapStar.Measure[] measures = distinctMeasuresList
          .toArray(new RolapStar.Measure[distinctMeasuresList.size()]);
        aggmgr.loadAggregation(measures, columns, batchKey, predicates,
          pinnedSegments, groupingSetsCollector);
      }
    }
    ...
    private StarColumnPredicate[] initPredicates() {
      ...
      for (int j = 0; j < columns.length; j++) {
        ...
        if (valueSet == null) {
          ...
          ValueColumnPredicate[] values = valueSet
            .toArray(new ValueColumnPredicate[valueSet.size()]);
          // Sort array to achieve determinism in generated SQL.
          Arrays.sort(values, ValueColumnConstraintComparator.instance);

          predicate = new ListColumnPredicate(columns[j], Arrays
            .asList((StarColumnPredicate[]) values));
        }
    ...
    private void generateAggregateSql() {
      ...
      if (cube == null || cube.isVirtual()) {
        ...
        buf.append("AggGen: Sorry, can not create SQL for virtual Cube \"").append(
          cube.getName()).append("\", operation not currently supported");
        BATCH_LOGGER.error(buf.toString());

      } else {
        final AggGen aggGen = new AggGen(cube.getName(), cube.getStar(), columns);
        if (aggGen.isReady()) {
          ...
          System.out
            .println("createCollapsed:" + Util.nl + aggGen.createCollapsed());
          System.out.println("insertIntoCollapsed:" + Util.nl
    ...
    /**
     * Returns the list of measures based upon a distinct aggregation containing
     * SQL measure expressions(as opposed to column expressions).
     * 
    ...
    private List<RolapStar.Measure> getDistinctSqlMeasures(
      List<RolapStar.Measure> measuresList) {
      List<RolapStar.Measure> distinctSqlMeasureList = new ArrayList<RolapStar.Measure>();
      for (RolapStar.Measure measure : measuresList) {
        if (measure.getAggregator().isDistinct()
          && measure.getExpression() instanceof MondrianDef.MeasureExpression) {
          MondrianDef.MeasureExpression measureExpr = (MondrianDef.MeasureExpression) measure
            .getExpression();
     ...
     * <p>
     * This is possible if:
     * <li>columns list is super set of other batch's constraint columns; and
     * <li>both have same Fact Table; and
     * <li>matching columns of this and other batch has the same value; and
     * <li>non matching columns of this batch have ALL VALUES </ul>
     */
    boolean canBatch(Batch other) {
      return hasOverlappingBitKeys(other) && constraintsMatch(other)
        && hasSameMeasureList(other) && !hasDistinctCountMeasure()
        && !other.hasDistinctCountMeasure() && haveSameStarAndAggregation(other)
        && haveSameClosureColumns(other);
    }

    /**
     * Returns whether the constraints on this Batch subsume the constraints on
     * another Batch and therefore the other Batch can be subsumed into this one
     * for GROUPING SETS purposes. Not symmetric.
     * 
     * @param other
     *          Other batch
    ...
    private boolean constraintsMatch(Batch other) {
      if (areBothDistinctCountBatches(other)) {
        if (getConstrainedColumnsBitKey()
          .equals(other.getConstrainedColumnsBitKey())) {
          return hasSameCompoundPredicate(other) && haveSameValues(other);
        } else {
          return hasSameCompoundPredicate(other)
            || (other.batchKey.getCompoundPredicateList().isEmpty() || equalConstraint(
              batchKey.getCompoundPredicateList(), other.batchKey
                .getCompoundPredicateList())) && haveSameValues(other);
        }
      } else {
    ...
    private boolean equalConstraint(List<StarPredicate> predList1,
    ...
    private boolean areBothDistinctCountBatches(Batch other) {
      return this.hasDistinctCountMeasure() && !this.hasNormalMeasures()
        && other.hasDistinctCountMeasure() && !other.hasNormalMeasures();
    }
    ...
    boolean hasOverlappingBitKeys(Batch other) {
      return getConstrainedColumnsBitKey().isSuperSetOf(
    ...
    private StarPredicate compoundPredicate() {
      ...
      for (StarPredicate starPredicate : batchKey.getCompoundPredicateList()) {
        if (predicate == null) {
     ...
     * 
     * <p>
     * Ensures that we do not group together a batch that includes a level of a
     * parent-child closure dimension with a batch that does not. It is not safe
     * to roll up from a parent-child closure level; due to multiple accounting,
     * the 'all' level is less than the sum of the members of the closure level.
     * 
     * @param other
     *          Other batch
    ...
    boolean haveSameClosureColumns(Batch other) {
      ...
      final BitKey closureColumns = this.batchKey.getConstrainedColumnsBitKey().and(
        cubeClosureColumnBitKey);
      final BitKey otherClosureColumns = other.batchKey
        .getConstrainedColumnsBitKey().and(cubeClosureColumnBitKey);
    ...
    private AggStar getAgg(boolean[] rollup) {
      ...
      AggStar star = aggregationManager.findAgg(getStar(),
        getConstrainedColumnsBitKey(), makeMeasureBitKey(), rollup);
      return star;
    }
    ...
    /**
     * Return whether have same values for overlapping columns or has all
     * children for others.
     */
    boolean haveSameValues(Batch other) {
      for (int j = 0; j < columns.length; j++) {
        ...
        if (!isCommonColumn && !hasAllValues(columns[j], valueSets[j])) {
          return false;
        }
      }
      return true;
  ...
  private static class CompositeBatchComparator implements
  ...
  private static class BatchComparator implements Comparator<Batch> {
    ...
    public int compare(Batch o1, Batch o2) {
      ...
      for (int i = 0; i < o1.columns.length; i++) {
        int c = o1.columns[i].getName().compareTo(o2.columns[i].getName());
        if (c != 0) {
          return c;
        }
      }
  ...
  private static class ValueColumnConstraintComparator implements
    Comparator<ValueColumnPredicate> {
