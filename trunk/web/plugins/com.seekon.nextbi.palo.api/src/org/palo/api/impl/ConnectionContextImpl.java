...
class ConnectionContextImpl implements ConnectionContext {
  ...
  ConnectionContextImpl(ConnectionImpl connection) {
    ...
    this.serverInfo = new ServerInfoImpl(connection, connection
      .getConnectionInternal().getServerInfo());
    setContext();
  }
  ...
  private final void setContext() {
    com.tensegrity.palojava.ServerInfo srvInfo = connection.getConnectionInternal()
      .getServerInfo();
    setDoSupportSubset2((srvInfo.getMajor() >= 2 && srvInfo.getBuildNumber() > 2400));
  }
