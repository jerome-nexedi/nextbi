  ...
  protected void registerStartHandlers() {
    ...
    registerStartHandler(new IPaloStartHandler() {
      ...
      public void startElement(String uri, String localName, String qName,
        ...
        for (int i = 0; i < dimIds.length; ++i) {
          ...
          if (dimension == null) {
            addError("CubeViewReader: unknown dimension id '" + dimIds[i]
              + "' in database '" + database.getName() + "'!!", cubeView.getId(),
              cubeView, database, dimIds[i], PersistenceError.UNKNOWN_DIMENSION,
          ...
          if (dimension != null && hierIds != null) {
            ...
            if (hier == null) {
              addError("CubeViewReader: unknown hierarchy id '" + hierIds[i]
                + "' in database '" + database.getName() + "'!!", cubeView.getId(),
                cubeView, database, hierIds[i], PersistenceError.UNKNOWN_DIMENSION,
                currAxis, PersistenceError.TARGET_GENERAL);
            }
            currAxis.add(hier);
          } else {
      ...
      public void startElement(String uri, String localName, String qName,
        ...
        if (ids.indexOf(CubeViewPersistence.DIM_HIER_DELIMITER) != -1) {
          ...
          for (int i = 0; i < dimHierIds.length; ++i) {
            String[] bothIds = dimHierIds[i]
              .split(CubeViewPersistence.DIM_HIER_DELIMITER);
            Dimension dimension = database.getDimensionById(bothIds[0]);
            if (dimension == null) {
              addError("CubeViewReader: unknown dimension id '" + bothIds[0]
                + "' in database '" + database.getName() + "'!!", cubeView.getId(),
                cubeView, database, bothIds[0], PersistenceError.UNKNOWN_DIMENSION,
                currAxis, PersistenceError.TARGET_GENERAL);
            } else {
              Hierarchy hier = dimension.getHierarchyById(bothIds[1]);
              if (hier == null) {
                addError("CubeViewReader: unknown hierarchy id '" + bothIds[1]
                  + "' in database '" + database.getName() + "'!!",
                  cubeView.getId(), cubeView, database, bothIds[1],
          ...
          for (int i = 0; i < dimIds.length; ++i) {
            ...
            if (dimension == null) {
              ...
              if (hierIds != null) {
                ...
                if (hier == null) {
                  addError("CubeViewReader: unknown hierarchy id '" + hierIds[i]
                    + "' in database '" + database.getName() + "'!!", cubeView
                    .getId(), cubeView, database, hierIds[i],
                    PersistenceError.UNKNOWN_DIMENSION, currAxis,
                    PersistenceError.TARGET_GENERAL);
                }
                currAxis.add(hier);
    ...
    registerStartHandler(new IPaloStartHandler() {
      ...
      public void startElement(String uri, String localName, String qName,
        ...
        try {
          ElementPath expandedPath = ElementPath.restore(hierarchies, paths);
          currAxis.addExpanded(expandedPath);
        } catch (PaloAPIException ex) {
          addError(ex.getMessage(), cubeView.getId(), cubeView, ((Object[]) ex
            .getData())[0], ((Object[]) ex.getData())[1].toString(),
