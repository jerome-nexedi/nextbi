  ...
  public CubeView findCubeViewById(Connection connection, String dbId,
    ...
    for (int j = 0; j < dbs.length; j++) {
      if (dbs[j].getId().equals(dbId)) {
        ...
        if (c != null) {
          final ArrayList<CubeView> views = new ArrayList<CubeView>();
  ...
  public FavoriteViewXMLHandler(final Connection connection) {
    ...
    putStartHandler("folder", new StartHandler() {
      public void startElement(String uri, String localName, String qName,
        ...
        if (folderName != null && folderName.length() > 0) {
          ...
          FavoriteViewsFolder fold = new FavoriteViewsFolderImpl(folderName,
            connection, pos);
    ...
    putStartHandler("bookmark", new StartHandler() {
      public void startElement(String uri, String localName, String qName,
        ...
        CubeView view = findCubeViewById(connection, databaseId, cubeId, viewId);
        FavoriteViewImpl bm = new FavoriteViewImpl(name, view, Integer
          .parseInt(position));
