...
class DimensionImpl implements Dimension {
  ...
  // FACTORY
  //
  final static DimensionImpl create(ConnectionImpl connection, Database database,
    DimensionInfo dimInfo, boolean doEvents) {
  ...
  private DimensionImpl(ConnectionImpl connection, Database database,
    ...
    this.key = new CompoundKey(new Object[] { DimensionImpl.class, connection,
  ...
  public final void rename(String name) {
    ...
    reloadRules();
    //create event:
    fireDimensionsRenamed(new Dimension[] { this, attrDim }, new String[] { oldName,
      oldAttrDimName });
  }
  ...
  private final void fireDimensionsRenamed(Object[] dimensions, Object oldValue) {
    ConnectionEvent ev = new ConnectionEvent(getDatabase().getConnection(),
      getDatabase(), ConnectionEvent.CONNECTION_EVENT_DIMENSIONS_RENAMED, dimensions);
  ...
  public Hierarchy getHierarchyById(String id) {
    if (!loadedHierarchies.containsKey(id)) {
      ...
      HierarchyImpl hier = HierarchyImpl.create(connection, this, hierInfo, true);
  ...
  final void reloadHierarchyInfos() {
    HierarchyInfo[] _hierInfos = dbConnection.getHierarchies(dimInfo);
