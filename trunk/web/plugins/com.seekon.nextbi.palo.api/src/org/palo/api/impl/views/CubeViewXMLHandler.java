...
class CubeViewXMLHandler extends BaseXMLHandler {
  ...
  CubeViewXMLHandler(Database database, String key) {
    registerHandler(CubeViewHandler.LEGACY, new CubeViewHandlerLegacy(database, key));
  ...
  public final void processingInstruction(String target, String data)
    ...
    if (target.equals("paloview")) {
      ...
      CubeViewHandler handler = (CubeViewHandler) handlers.get(version);
      ...
      if (handler == null && !version.equalsIgnoreCase("legacy")) {
        ...
        String[] versions = (String[]) handlers.keySet().toArray(new String[0]);
