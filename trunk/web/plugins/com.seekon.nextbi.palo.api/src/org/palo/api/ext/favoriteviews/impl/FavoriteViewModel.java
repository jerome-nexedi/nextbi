...
public class FavoriteViewModel {
  ...
  final protected FavoriteViewTreeNode fromXML(String input, Connection con) {
    ...
    try {
      ByteArrayInputStream bin = new ByteArrayInputStream(input.getBytes("UTF-8")); //$NON-NLS-1$
  ...
  public Cube createBookmarkCubeInExistingDatabase(Database db) {
    ...
    if (user == null) {
      ...
      // element.
      if (user.getDefaultHierarchy().getElementByName(
  ...
  public FavoriteViewTreeNode loadFavoriteViews(Connection con) {
    if (con.getType() == Connection.TYPE_XMLA) {
      ...
      String xmlData = "";
      try {
        xmlData = sql.loadFavoriteView(con.getServer(), con.getService(), con
          .getUsername());
      } finally {
        sql.close();
      }
    ...
    if (cube == null) {
      return new FavoriteViewTreeNode(FavoriteViewFactory.getInstance()
        .createFolder("Root", con));
    }
    ...
    if (userDim.getDefaultHierarchy().getElementByName(con.getUsername()) == null) {
      try {
        userDim.getDefaultHierarchy().addElement(con.getUsername(),
          Element.ELEMENTTYPE_STRING);
      } catch (Exception e) {
    ...
    try {
      ...
      String xmlData = (String) cube.getData(new String[] { con.getUsername(),
        "Bookmarks" });
      if (xmlData != null) {
        if (xmlData.length() > 0) {
          return fromXML(xmlData, con);
  ...
  public synchronized void storeFavoriteViews(Connection con,
    ...
    if (con.getType() == Connection.TYPE_XMLA) {
      SQLConnection sql = new SQLConnection();
      try {
        sql.writeFavoriteViews(con.getServer(), con.getService(), con.getUsername(),
          xmlBookmarkText);
      } finally {
        sql.close();
      }
