...
abstract class CubeViewHandler {
  ...
  protected final Element[] getPath(String path, Hierarchy hier, Object section,
    int sectionType) throws PaloAPIException {
    ...
    for (int i = 0; i < paths.length; ++i) {
      ...
      if (elements[i] == null) {
        addError("CubeViewReader: unknown element id '" + paths[i] + "'!!", cubeView
          .getId(), cubeView, hier, paths[i], PersistenceError.UNKNOWN_ELEMENT,
          section, sectionType);
      }
    }
    return elements;
