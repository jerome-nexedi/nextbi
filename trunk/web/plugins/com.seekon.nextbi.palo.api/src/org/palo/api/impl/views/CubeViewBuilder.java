  ...
  final synchronized CubeView createView(CubeViewHandler viewHandler) {
    if (id == null || name == null || cube == null) {
      PersistenceError error = new PersistenceErrorImpl(
        "Could not create cube view, insufficient information", id, cube, null,
        null, PersistenceError.LOADING_FAILED, null, PersistenceError.TARGET_GENERAL);
    ...
    if (n > 0) {
      ...
      for (int i = 0; i < n; i++) {
        props[i] = new Property(keys[i], (String) properties.get(keys[i]));
        params[3 + i] = props[i];
      }
    }
