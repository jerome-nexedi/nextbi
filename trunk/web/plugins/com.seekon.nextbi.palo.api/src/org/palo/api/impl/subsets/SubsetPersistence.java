...
public class SubsetPersistence {
  ...
  static final String DIMENSION_SUBSET_COLUMNS = SYSTEM_PREFIX + "subsetcolumns"; //$NON-NLS-1$

  static final String DIMENSION_SUBSET_ROWS = SYSTEM_PREFIX + "subsetrows"; //$NON-NLS-1$
  ...
  final void save(final Subset subset) {
    try {
      ...
      throw new PaloAPIException(
        "Could not save subset '" + subset.getName() + "'!", e);
    }
  }
  ...
  private boolean deleteInternal(Subset subset) throws Exception {
    ...
    do {
      ...
      for (int i = 0; i < elements.length; ++i) {
        Element element = elements[i];
        String xmlDef = cube.getData(new String[] { COL_DEF, element.getName() })
          .toString();
        ByteArrayInputStream bin = new ByteArrayInputStream(xmlDef.getBytes("UTF-8")); //$NON-NLS-1$
        Subset xmlSub = SubsetReader.getInstance().fromXML(bin, element.getName(),
          database);
        if (xmlSub != null) {
  ...
  private void saveInternal(Subset subset) throws Exception {
    ...
    Cube cube;
    if ((cube = database.getCubeByName(CUBE_SUBSETS)) == null) {
      cube = database.addCube(CUBE_SUBSETS, new Dimension[] { columns, rows });
    }
    ...
    cube
      .setData(new String[] { COL_DEF, element.getName() }, bout.toString("UTF-8")); //$NON-NLS-1$
  }
  ...
  private final Subset loadSubset(Database database, Cube subsetCube,
    ...
    try {
      subset = SubsetReader.getInstance().fromXML(bin, subsetElement.getName(),
        database);
    } finally {
      bin.close();
    }
  ...
  private final Subset[] loadBulky(Database database) {
    ...
    for (int i = 0; i < values.length; ++i) {
      try {
        if (values[i] != null) {
          Subset subset = loadFromDefinition(values[i].toString(), elements[i]
            .getName(), database);
          if (subset != null) {
            subsets.add(subset);
          }
        ...
        // TODO nested errors?
        System.err.println("Failed to load subset with id " + elements[i].getName());
      }
    }
    return subsets.toArray(new Subset[subsets.size()]);
  }

  private final Subset loadFromDefinition(String def, String subId, Database database)
    throws IOException {
    Subset subset = null;
    // PR 6566: skip subsets without definitions, can occur during
    // first subset creation while loading required cubes...
