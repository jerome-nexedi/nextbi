...
class AxisImpl implements Axis {
  ...
  private final Map<Hierarchy, Map<String, Element[]>> expanded = new HashMap<Hierarchy, Map<String, Element[]>>();
  ...
  private final Map<Dimension, Subset> activeSubs = new HashMap<Dimension, Subset>();

  private final Map<Dimension, Subset2> activeSubs2 = new HashMap<Dimension, Subset2>();

  private final Map<Hierarchy, Element> selectedElements = new HashMap<Hierarchy, Element>();

  private final Map<Hierarchy, Map<String, ArrayListInt>> repetitions = new HashMap<Hierarchy, Map<String, ArrayListInt>>();
  ...
  public final void removeExpanded(Hierarchy hierarchy, Element[] path,
    int repetition) {
    Map<String, Element[]> paths = expanded.get(hierarchy);
    if (paths == null)
      return;
  ...
  public final void removeExpanded(Dimension dimension, Element[] path,
    int repetition) {
  ...
  public final boolean equals(Object obj) {
    if (obj instanceof AxisImpl) {
      ...
      return other.getId().equals(id) && other.view.getId().equals(view.getId());
    }
    return false;
  }
  ...
  public final ElementPath[] getExpandedPaths() {
    return expandedPaths.toArray(new ElementPath[expandedPaths.size()]);
  }
