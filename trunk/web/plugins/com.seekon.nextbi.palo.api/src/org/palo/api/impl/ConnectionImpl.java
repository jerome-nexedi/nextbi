...
* @author Arnd Houben
*
* @version $Id: ConnectionImpl.java,v 1.101 2010/07/19 11:13:18 PhilippBouillon Exp $
*
*/
...
public class ConnectionImpl implements Connection, ServerListener {
  ...
  public final LicenseInfo getLicenseInfo() {
    return dbConnection.getLicenseInfo();
  }
  ...
  public final void ping() {
    try {
      ...
      throw new PaloException("Server is not reachable.");
    }
  }
  ...
  public synchronized final void storeFavoriteViews(
    FavoriteViewTreeNode favoriteViews) {
    FavoriteViewModel bm = new FavoriteViewModel();
  ...
  public final void serverStructureChanged(ServerEvent event) {
    ...
    if (evType == ServerEvent.SERVER_DOWN)
      ...
      //if(evType == ServerEvent.SERVER_CHANGED)
  ...
  final synchronized void reloadAlllDatabases(boolean doEvents) {
    HashMap<String, DatabaseImpl> oldDatabases = new HashMap<String, DatabaseImpl>(
      loadedDatabases);
    loadedDatabases.clear();
    dbLoader.reset();
    ...
    if (doEvents) {
      LinkedHashSet removedDatabases = new LinkedHashSet(oldDatabases.values());
      removedDatabases.removeAll(loadedDatabases.values());
      if (removedDatabases.size() > 0) {
        fireEvent(new ConnectionEvent(this, this,
          ConnectionEvent.CONNECTION_EVENT_DATABASES_REMOVED, removedDatabases
            .toArray()));
      }
    }

    if (doEvents) {
      LinkedHashSet addedDatabases = new LinkedHashSet(loadedDatabases.values());
      addedDatabases.removeAll(oldDatabases.values());
      if (addedDatabases.size() > 0) {
        fireEvent(new ConnectionEvent(this, this,
          ConnectionEvent.CONNECTION_EVENT_DATABASES_ADDED, addedDatabases.toArray()));
      }
    }
  ...
  private final DatabaseImpl createDatabase(DatabaseInfo dbInfo, boolean fireEvent) {
    try {
      DatabaseImpl database = DatabaseImpl.create(this, dbInfo, fireEvent);
