...
class SubsetConverter {
    ...
    for (Subset subset : subsets) {
      ...
      if (remove) {
        ...
        try {
          ...
          System.err.println("failed to remove legacy subset '" + subset.getName()
            + "'!");
        }
      }
    }
    if (!failedSubsets.isEmpty()) {
      PaloIOException paloEx = new PaloIOException(
        "Some subset transformation failed!");
  ...
  private final Subset2 toNewSubset(Subset subset, int type) throws PaloIOException {
    ...
    if (newSubset != null) {
      ...
      if (stateId.equals(HierarchicalState.ID)) {
        ...
        SubsetFilter sortFilter = newSubset.getFilter(SubsetFilter.TYPE_SORTING);
        if (sortFilter == null) {
          sortFilter = new SortingFilter(subset.getDimension());
          newSubset.add(sortFilter);
        }
        SortingFilterSetting settings = (SortingFilterSetting) sortFilter
          .getSettings();
        settings.setSortCriteria(SortingFilterSetting.SORT_CRITERIA_DEFINITION);
        settings
          .setHierarchicalMode(SortingFilterSetting.HIERARCHICAL_MODE_SHOW_CHILDREN);
      } else if (stateId.equals(FlatState.ID)) {
        //PR 6906 try to get same order as in legacy flat subset...
        org.palo.api.ext.subsets.SubsetHandler handler = SubsetHandlerRegistry
          .getInstance().getHandler(subset, FlatState.ID);
        List<ElementNode> nodes = handler != null ? handler
          .getVisibleRootNodesAsList() : new ArrayList<ElementNode>();
  ...
  private final void addHierarchicalFilter(Subset2 newSubset, Subset subset) {
    ...
    sortFilter.getSettings().setHierarchicalMode(
