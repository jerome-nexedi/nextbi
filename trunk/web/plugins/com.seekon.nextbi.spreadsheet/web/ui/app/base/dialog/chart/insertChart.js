Jedox.wss.dialog.chart.openChartDialog = function(mode, chartID) {
	...
	var onBeforeDestroyDialog = function(panel) {
		...
		Jedox.wss.app.unload(Jedox.wss.app.dynJSRegistry.chart)
	};
	...
	var collectProperties = function() {
		...
		var vportPos = activeBook.getViewportPos();
		...
		var chartProps = {
			props : {
				...
				Title : {
					General : {
						HasTitle : Ext.getCmp("chartTitle").getValue() ? true
								: false,
	...
	var doInsertChart = function() {
		...
		if (chartProps != false) {
			var chartID = Jedox.wss.backend.ha.wsel("add_chart", chartProps);
			Jedox.wss.dialog.chart.createChart(false, winID, chartID,
					chartProps.n_location, chartProps.pos_offsets,
					chartProps.size, chartProps.props.ChartType.General.Type)
		}
			...
			Ext.MessageBox
					.show( {
						...
						fn : function() {
							...
							Jedox.wss.app.lastInputMode = Jedox.wss.grid.GridMode.READY;
							Jedox.wss.app
									.unload(Jedox.wss.app.dynJSRegistry.chart)
						}
					});
			dialogWindow.destroy();
			return
	...
	var tplChartCategories = new Ext.XTemplate(
			'<div class="charttypes" style="width: 92%;">Categories</div>',
			'<tpl for=".">',
	...
	var insChartForm = new Ext.FormPanel(
			{
									...
									new Ext.Panel(
											{
												...
												items : [
														...
														},
														{
															xtype : "combo",
															id : "firstColLabels",
															fieldLabel : "Use category labels"
																	.localize(),
															store : labelPosItems,
															displayField : "itemName",
															valueField : "itemValue",
				...
				buttons : [ {
					text : "OK".localize(),
					handler : (mode == "insert") ? doInsertChart : doEditChart,
					formBind : true,
					tabIndex : 3
				} ]
			});
	...
	Ext.getCmp("chartcategories").select("All");
	if (mode == "edit") {
