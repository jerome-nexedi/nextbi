Jedox.wss.range = new function() {
	...
	this.drawDependingCells = function(formulastr) {
		var env = Jedox.wss.app.environment, refs = Jedox.wss.formula.parse(
				formulastr, Jedox.wss.app.activePane.getCellNFs(
		...
		for ( var area, area_tok, states = Jedox.wss.range.AreaState, areas = env.formulaSelection
		...
		var activeSheet = Jedox.wss.app.activeBook.getSheetSelector()
				.getActiveSheetName(), Point = Jedox.wss.cls.Point;
		for (key in toks) {
	...
	this.getCellCoord = function(cell) {
		return [
				[ Jedox.util.offsetLeft(cell),
						Jedox.util.offsetLeft(cell) + cell.offsetWidth ],
				[ Jedox.util.offsetTop(cell),
						Jedox.util.offsetTop(cell) + cell.offsetHeight ] ]
	}
};