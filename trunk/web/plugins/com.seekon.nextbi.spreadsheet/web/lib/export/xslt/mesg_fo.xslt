...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdt="http://www.w3.org/2005/xpath-datatypes" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fo="http://www.w3.org/1999/XSL/Format">
					...
					<fo:block>
						<fo:table table-layout="fixed" width="100%" border-spacing="2pt">
							...
							<fo:table-body start-indent="0pt">
								<fo:table-row>
									<fo:table-cell padding="2pt" height="500" text-align="center" display-align="center">
										<fo:block>
											<xsl:for-each select="$XML">
												<xsl:for-each select="message">
													<xsl:for-each select="img">
														<fo:external-graphic>
															<xsl:variable name="sHeight" select="@h"/>
															<xsl:if test="$sHeight != ''">
																<xsl:attribute name="height"><xsl:value-of select="$sHeight"/></xsl:attribute>
															</xsl:if>
															<xsl:attribute name="src"><xsl:text>url(</xsl:text><xsl:call-template name="double-backslash"><xsl:with-param name="text"><xsl:value-of select="string(@path)"/></xsl:with-param><xsl:with-param name="text-length"><xsl:value-of select="string-length(string(@path))"/></xsl:with-param></xsl:call-template><xsl:text>)</xsl:text></xsl:attribute>
														</fo:external-graphic>
													</xsl:for-each>
												</xsl:for-each>
											</xsl:for-each>
	...
	<xsl:template match="*|@*|text()" mode="second-step">
		<xsl:copy>
			<xsl:apply-templates select="*|@*|text()" mode="second-step"/>
		</xsl:copy>
	</xsl:template>
</xsl:stylesheet>
