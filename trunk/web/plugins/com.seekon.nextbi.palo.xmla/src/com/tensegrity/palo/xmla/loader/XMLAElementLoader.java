...
public class XMLAElementLoader extends ElementLoader {
  ...
  public ElementInfo[] getElementsAtDepth(int depth) {
    ...
    elementInfos = BuilderRegistry
      .getInstance()
      .getElementInfoBuilder()
      .getElements((XMLAConnection) paloConnection, xmlaClient,
        ((XMLADimensionInfo) hierarchy.getDimension()).getCubeId(), hierarchy, depth);
  ...
  public ElementInfo loadByName(String name) {
    if (LOAD_ALL_ELEMENTS) {
      ...
      if (!loadedInfo.containsKey(cleanedText)) {
        info = BuilderRegistry.getInstance().getElementInfoBuilder().getElementInfo(
          (XMLAConnection) paloConnection, xmlaClient, hierarchy, name);
        loaded(info);
      } else {
    ...
    if (children == null || children.length == 0) {
      if (!_parent.hasChildren())
        ...
        elementInfos = BuilderRegistry.getInstance().getElementInfoBuilder()
          .getChildren((XMLAConnection) paloConnection, xmlaClient,
  ...
  private void loadAllElementIds() {
    ...
    try {
      ...
      String cleanId = XMLADimensionInfo.transformId(hierarchy.getDimension()
        .getId());
  ...
  private final void loadElements() {
    ...
    elementInfos = BuilderRegistry.getInstance().getElementInfoBuilder()
      .getElements((XMLAConnection) paloConnection, xmlaClient,
        ((XMLADimensionInfo) hierarchy.getDimension()).getCubeId(), hierarchy);
    XMLAElementLoader otherLoader = null;
    if (hierarchy != null) {
      otherLoader = (XMLAElementLoader) ((XMLAConnection) paloConnection)
        .getElementLoader(hierarchy.getDimension());
    }
    for (XMLAElementInfo info : elementInfos) {
