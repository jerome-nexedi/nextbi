...
*
* @version $Id: XMLAConnection.java,v 1.53 2010/07/13 09:21:06 PhilippBouillon Exp $
*
*/
...
public class XMLAConnection implements DbConnection {
  ...
  XMLAConnection(String host, String service, String user, String pass, int timeout) {
    ...
    try {
      xmlaClient = new XMLAClient(host, service, user, pass, timeout);
      connected = true;
  ...
  public ElementInfo addElement(DimensionInfo dimension, String name, int type,
    ElementInfo[] children, double[] weights) {
    throw new PaloException("XMLAConnections cannot add elements.");
  }

  public ElementInfo addElement(HierarchyInfo hierarchy, String name, int type,
    ElementInfo[] children, double[] weights) {
    throw new PaloException("XMLAConnections cannot add elements.");
  }
  ...
  public CellInfo getData(CubeInfo cube, ElementInfo[] coordinate) {
    ...
    return BuilderRegistry.getInstance().getCubeInfoBuilder().getData(
      xmlaClient.getConnections()[0].getName(), xmlaClient, (XMLACubeInfo) cube,
      els, this);
  }
  ...
  public CellInfo[] getDataArea(CubeInfo cube, ElementInfo[][] coordinates) {
    ...
    return BuilderRegistry.getInstance().getCubeInfoBuilder().getDataArray(
      xmlaClient.getConnections()[0].getName(), xmlaClient, (XMLACubeInfo) cube,
      els, this);
  }
  ...
  public CellInfo[] getDataArray(CubeInfo cube, ElementInfo[][] coordinates) {
    ...
    return BuilderRegistry.getInstance().getCubeInfoBuilder().getDataBulk(
      xmlaClient.getConnections()[0].getName(), xmlaClient, (XMLACubeInfo) cube,
      els, this);
  }

  public Object getWholeCube(CubeInfo cube) {
    return BuilderRegistry.getInstance().getCubeInfoBuilder().readWholeCube(
      xmlaClient.getConnections()[0].getName(), xmlaClient, (XMLACubeInfo) cube,
      this);
  }
  ...
  private final CellInfo[] performCopy(ArrayList<CoordinateStorage> allCoords,
    DimensionInfo[] origDims, CubeInfo cube, boolean ignoreEmptyCells) {
    ...
    for (CellInfo r : result) {
      counter++;
      if (ignoreEmptyCells) {
        if (r == null || r.getValue().toString().equals("0.0")
          || r.getValue().toString().equals("")) {
          continue;
        }
      }
      try {
  ...
  private final CellInfo[] collectCoordinates(CubeInfo cube,
    ...
    ArrayList<CoordinateStorage> allCoords = new ArrayList<CoordinateStorage>();
    while (!finished) {
      ...
      if (!finished) {
        ...
        if (cellCounter >= 10000) {
          completeResult.addAll(Arrays.asList(performCopy(allCoords, origDims, cube,
            context.ignoreEmptyCells())));
          allCoords.clear();
          cellCounter = 0;
        }
      }
  ...
  public XMLAElementInfo[] getCubeElements(XMLACubeInfo cube,
    XMLADimensionInfo dimension) {
    ElementLoader el = getElementLoader(dimension.getDefaultHierarchy());
    String[] ids = el.getAllElementIds();
    ArrayList<XMLAElementInfo> cubeElements = new ArrayList<XMLAElementInfo>();
    for (int i = 0, n = ids.length; i < n; i++) {
      XMLAElementInfo elInfo = (XMLAElementInfo) el.load(ids[i]);
      if (((XMLADimensionInfo) elInfo.getDimension()).getCubeId().equals(
        cube.getId())) {
  ...
  public String getRule(CubeInfo cube, ElementInfo[] coordinate) {
    return BuilderRegistry.getInstance().getRuleInfoBuilder().getRule(
      (XMLACubeInfo) cube, coordinate);
  }

  public RuleInfo[] getRules(CubeInfo cube) {
    return BuilderRegistry.getInstance().getRuleInfoBuilder().getRules(this,
      xmlaClient, (XMLACubeInfo) cube);
  ...
  public ServerInfo getServerInfo() {
    ...
    if (cons == null || cons.length < 1) {
      throw new PaloException("Could not login to xmla server '"
        + xmlaClient.getServer() + "' as user '" + xmlaClient.getUsername() + "'!!");
    }
    return cons[0];
  }
  ...
  public String listFunctions() {
    return BuilderRegistry.getInstance().getRuleInfoBuilder().getFunctions(
      xmlaClient);
  }

  public String listFunctionNames() {
    return BuilderRegistry.getInstance().getRuleInfoBuilder().getFunctionNames(
      xmlaClient);
  }
  ...
  public RuleInfo getRule(CubeInfo cube, String id) {
    ...
    if (rules == null) {
      rules = BuilderRegistry.getInstance().getRuleInfoBuilder().getRules(this,
        xmlaClient, (XMLACubeInfo) cube);
  ...
  public PropertyInfo getProperty(String id) {
    if (id.equals(PROPERTY_SAP_VARIABLES)) {
      ...
      PropertyInfo info = new PropertyInfoImpl(id, Boolean.toString(isSap), null,
  ...
  public HierarchyInfo[] getHierarchies(DimensionInfo dimension) {
    XMLAHierarchyRequestor req = new XMLAHierarchyRequestor(
      (XMLADimensionInfo) dimension, (XMLADatabaseInfo) dimension.getDatabase(),
      this);
    req.setCubeNameRestriction(((XMLADimensionInfo) dimension).getCubeId());
    req.setCatalogNameRestriction(((XMLADimensionInfo) dimension).getDatabase()
      .getId());
    req.setDimensionUniqueNameRestriction(((XMLADimensionInfo) dimension)
  ...
  public void testElementMDX(String databaseId, String dimensionId,
    String hierarchyId) {
    /*BuilderRegistry.getInstance().getElementInfoBuilder().
    	getElementsTest(this, xmlaClient, databaseId, dimensionId, hierarchyId);*/
  }

  public boolean addElements(DimensionInfo dimension, String[] names, int type,
    ElementInfo[][] children, double[][] weights) {
    throw new PaloException("XMLAConnections cannot add elements.");
  }
  ...
  public boolean addElements(DimensionInfo dimension, String[] names, int[] types,
    ElementInfo[][] children, double[][] weights) {
    throw new PaloException("XMLAConnections cannot add elements.");
  }
  ...
  public boolean copyCell(CubeInfo cube, ElementInfo[] src, ElementInfo[] dest,
    double value) {
    throw new PaloException("XMLAConnections cannot copy cells.");
  }
