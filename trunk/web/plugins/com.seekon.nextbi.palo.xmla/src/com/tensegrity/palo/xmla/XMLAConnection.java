...
public class XMLAConnection implements DbConnection {
  ...
  XMLAConnection(String host, String service, String user, String pass) {
    ...
    try {
      ...
      xmlaClient = new XMLAClient(host, service, user, pass/*
                                                            * ,
  ...
  public void addConsolidations(ElementInfo elementinfo, ElementInfo aelementinfo[],
    double ad[]) {
  }
  ...
  private final CellInfo[] performCopy(ArrayList allCoords,
    ...
    for (int i = 0; i < j; i++) {
      ...
      counter++;
      if (ignoreEmptyCells
        && (r == null || r.getValue().toString().equals("0.0") || r.getValue()
          .toString().equals("")))
        continue;
      try {
  ...
  public ServerInfo getServerInfo() {
    ...
    if (cons == null || cons.length < 1)
      throw new PaloException(
        (new StringBuilder("Could not login to xmla server '")).append(
          xmlaClient.getServer()).append("' as user '").append(
          xmlaClient.getUsername()).append("'!!").toString());
    else
      return cons[0];
  }
  ...
  public String parseRule(CubeInfo cube, String ruleDefinition, String functions) {
    ...
    for (i = 0; i < dimensions.length; i++) {
      definition.append((new StringBuilder("'")).append(
        dimensions[i].getId().replaceAll("'", "''")).append("':''").toString());
  ...
  public void update(ElementInfo elementinfo, int i, String as[], double ad[],
  ...
  public PropertyInfo getProperty(String id) {
    if (id.equals("SAP_VARIABLES")) {
      ...
      PropertyInfo info = new PropertyInfoImpl(id, Boolean.toString(isSap), null, 3,
        true);
      return info;
    } else {
      return null;
  ...
  public String[] getAllKnownPropertyIds() {
    return (new String[] { "SAP_VARIABLES" });
  }
 ...
 * Qualified Name: com.tensegrity.palo.xmla.XMLAConnection JD-Core Version:
 * 0.5.4
 */