...
public class DatabaseInfoBuilder {
  ...
  public XMLADatabaseInfo getDatabaseInfo(XMLAConnection paramXMLAConnection,
    XMLAClient paramXMLAClient, String paramString) {
    if (paramString == null)
      return null;
    XMLADatabaseRequestor localXMLADatabaseRequestor = new XMLADatabaseRequestor(
      paramXMLAConnection);
      ...
      for (int j = 0; j < localNodeList1.getLength(); ++j) {
        ...
        for (int k = 0; k < localNodeList2.getLength(); ++k) {
          if ((localNodeList2.item(k).getNodeType() != 1)
            || (!localNodeList2.item(k).getNodeName().equals("CUBE_NAME")))
            continue;
          ++i;
        }
      }
  ...
  private int requestDimensions(String paramString) {
    ...
    try {
      ...
      localXMLAProperties.setCatalog(paramString);
      localXMLARestrictions.setCatalog(paramString);
      Document localDocument = this.xmlaClient.getDimensionList(
      ...
      while (j < k) {
        ...
        for (int l = 0; l < localNodeList2.getLength(); ++l) {
          if ((localNodeList2.item(l).getNodeType() != 1)
            || (!localNodeList2.item(l).getNodeName()
              .equals("DIMENSION_UNIQUE_NAME")))
            continue;
          ++i;
        }
        ++j;
 ...
 * Qualified Name: com.tensegrity.palo.xmla.builders.DatabaseInfoBuilder JD-Core
 * Version: 0.5.4
 */