...
public class XMLACubeRequestor extends AbstractXMLARequestor {
  ...
  public XMLACubeRequestor(XMLAConnection paramXMLAConnection,
    XMLADatabaseInfo paramXMLADatabaseInfo) {
  ...
  public XMLACubeInfo[] requestCubes(XMLAClient paramXMLAClient) {
    ...
    try {
      ...
      Document localDocument = paramXMLAClient.getCubeList(localXMLARestrictions,
        localXMLAProperties);
  ...
  protected void parseResult(HashMap<String, String> paramHashMap,
    ...
    XMLACubeInfo localXMLACubeInfo = new XMLACubeInfo(str, str, this.database,
      paramString, paramXMLAClient, this.connection);
    BigInteger localBigInteger = new BigInteger("1");
    XMLADimensionRequestor localXMLADimensionRequestor = new XMLADimensionRequestor(
      localXMLACubeInfo, this.connection);
    ...
    XMLADimensionInfo[] arrayOfXMLADimensionInfo = localXMLADimensionRequestor
      .requestDimensions(paramXMLAClient);
    ...
    localXMLACubeInfo
      .setDimensions((String[]) localArrayList.toArray(new String[0]));
    localXMLACubeInfo.setCellCount(localBigInteger);
    localXMLACubeInfo.setFilledCellCount(localBigInteger);

    PropertyInfo propertyInfo = this.connection.getPropertyLoader().load(
      "SAP_VARIABLES");
    if ((propertyInfo != null) && (Boolean.parseBoolean((propertyInfo).getValue()))) {
      XMLAVariableInfo[] localObject2 = BuilderRegistry.getInstance()
        .getVariableInfoBuilder().requestVariables(paramXMLAClient,
          (XMLADatabaseInfo) localXMLACubeInfo.getDatabase(),
          localXMLACubeInfo.getId());
      localXMLACubeInfo.setVariables(localObject2);
      if ((XMLAClient.IGNORE_VARIABLE_CUBES) && (localObject2 != null)
        && (localObject2.length > 0))
        return;
    }
 ...
 * Qualified Name: com.tensegrity.palo.xmla.parsers.XMLACubeRequestor JD-Core
 * Version: 0.5.4
 */