...
public class CubeInfoBuilder {
  ...
  private CellInfo parseValueNode(Node node, String[] strCoord) {
    ...
    if (doubleType) {
      CellInfoImpl cell = new CellInfoImpl(CellInfo.TYPE_NUMERIC, true, new Double(
        text));
  ...
  private String getCharacteristicVariableString(PropertyInfo info) {
    int selectionType;
    PropertyInfo prop = info
      .getChild(XMLAConnection.PROPERTY_SAP_VAR_SELECTED_VALUES);
    if (prop == null) {
      return "";
    }
    ...
    PropertyInfo selType = info
      .getChild(XMLAConnection.PROPERTY_SAP_VAR_SELECTIONTYPE);
    if (selType == null) {
      return "";
    }
    try {
    ...
    if (selectionType == VariableInfo.VAR_SELECTION_TYPE_VALUE) {
      ...
      if (uniqueName[0].equals(uniqueName[1])) {
        ...
        result += XMLADimensionInfo.transformId(uniqueName[0]) + ":"
          + XMLADimensionInfo.transformId(uniqueName[1]);
      }
  ...
  private String getFloatingPointVariableString(PropertyInfo info) {
    ...
    PropertyInfo idProp = info.getChild(XMLAConnection.PROPERTY_SAP_VAR_ID);
    return " " + XMLADimensionInfo.transformId(idProp.getValue()) + " INCLUDING "
      + text;
  }
    ...
    try {
      ...
      for (int i = 0; i < dimensionSize; i++) {
        ...
        if (i > 1 && elements[i].length == 1) {
          ...
          elementPositions
            .put(((XMLAElementInfo) elements[i][0]).getUniqueName(), 0);
          axes[i] = new StringBuffer("-");
        } else {
          for (int j = 0, m = elements[i].length; j < m; j++) {
            String uniqueName = ((XMLAElementInfo) elements[i][j]).getUniqueName();
      ...
      HashMap<String, Object[]> elementMapper = new HashMap<String, Object[]>();
  ...
  private StringBuffer shortenQuery(XMLAClient client, XMLACubeInfo cube,
    StringBuffer sb, int[] querySize, final HashMap[] elementIndexMap,
    final StringBuffer[] axes, final int dimensionSize, StringBuffer where,
    ...
    do {
      ...
      for (int i = 0; i < dimensionSize; i++) {
        if (querySize != null) {
          ...
          if (axes[i].length() > max && !axes[i].toString().equals("-")
            && !axes[i].toString().equals("+")) {
            max = axes[i].length();
            pos = i;
          }
    ...
    for (int i = 0, n = dimensionSize; i < n; i++) {
      if (axes[i].length() > 0) {
        ...
        if (dim.getHierarchyUniqueName() != null) {
          sb.append("{" + dim.getHierarchyUniqueName() + ".Members} on Axis ("
            + axisCounter + ")");
        } else {
        ...
        for (ElementInfo el : xmlaConnection.getElements(dim)) {
          mapper.put(((XMLAElementInfo) el).getUniqueName(), new Object[] { el, i,
            counter });
          counter++;
        }
        if (querySize != null) {
  ...
  private final String[][] parseResultCoordinates(Document doc) {
    ...
    ArrayList<ArrayList<String>> resultCoordinatesArray = new ArrayList<ArrayList<String>>();
    if (ad != null) {
      if (ad.item(0) != null) {
  ...
  public ExtendedCellInfo readWholeCube(String conName, XMLAClient client,
    ...
    try {
      ...
      return new ExtendedCellInfo(resultValues.toArray(new CellInfo[0]),
        resultCoordinates);
    } catch (IOException e) {
  ...
  private final int transformCellOrdinal(int ordinal, String[][] resultCoordinates,
    HashMap<String, Object[]> mapper, int[] elementLengths) {
    int tempOrdinal = ordinal;
    int newCellOrdinal = 0;
    int[] offset = new int[resultCoordinates.length];
    ...
    try {
      ...
      for (int i = 0, n = elements.length; i < n; i++) {
        ...
        for (int j = 0; j < dimensionSize; j++) {
          Integer iOffset = ((Integer) elementIndexMap[j]
            .get(elements[i][j].getId()));
          if (iOffset != null) {
