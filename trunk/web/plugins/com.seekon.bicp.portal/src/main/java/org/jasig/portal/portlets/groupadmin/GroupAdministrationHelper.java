 ...
 * administration webflows. These methods include convenience methods for
 * populating and editing form objects, as well as saving information supplied
 * to a group form.
 * 
...
public class GroupAdministrationHelper {
   ...
   * 
   * @param key
   *          key of the group to be deleted
   * @param user
  ...
  public void deleteGroup(String key, IPerson deleter) {
    ...
    // groups
    @SuppressWarnings("unchecked")
    Iterator<IEntityGroup> iter = (Iterator<IEntityGroup>) group
      .getContainingGroups();
    while (iter.hasNext()) {
   ...
   * 
   * @param groupForm
   *          Form representing the new group configuration
   * @param updater
   *          Updating user
   */
  public void updateGroupDetails(GroupForm groupForm, IPerson updater) {
    ...
    if (!canEditGroup(updater, groupForm.getKey())) {
      throw new RuntimeAuthorizationException(updater, EDIT_PERMISSION, groupForm
        .getKey());
    }
  ...
  public void updateGroupMembers(GroupForm groupForm, IPerson updater) {
    ...
    for (JsonEntityBean child : groupForm.getMembers()) {
      ...
      if (type.isGroup()) {
        ...
        IGroupMember member = GroupService.getGroupMember(child.getId(), type
          .getClazz());
        group.addMember(member);
      }
    }
   ...
   * 
   * @param groupForm
   *          form object representing the new group
   * @param parent
   *          parent group for this new group
   * @param creator
   *          the uPortal user creating the new group
   */
  public void createGroup(GroupForm groupForm, JsonEntityBean parent, IPerson creator) {
    ...
    if (!canCreateMemberGroup(creator, parent.getId())) {
      throw new RuntimeAuthorizationException(creator, CREATE_PERMISSION, groupForm
        .getKey());
    }

    if (log.isDebugEnabled()) {
      log.debug("Creating new group for group form [" + groupForm.toString()
        + "] and parent [" + parent.toString() + "]");
    }
