...
public interface IUserLayout {
  ...
  /**
   * Writes user layout content (with appropriate markings) into a
  ...
  /**
   * Writes subtree of a user layout (with appropriate markings) defined by a
   * particular node into a <code>Document</code>
   * 
   * @param nodeId
   *          a <code>String</code> a node determining a user layout subtree.
   * @param document
   ...
   *          The expression to execute against the layout DOM
   * @return The ID of the resolved node, null if there is no match
   */
  public String findNodeId(XPathExpression xpathExpression);

  /**
   * This method is for the same use as {@link #findNodeId(XPathExpression)},
   * but since DOM traversal is much faster than XPath evaluation, sometimes it
   * is worth to implement manual document traversing.
   * 
   * @param finder
   *          {@link INodeIdResolver} to use against the layout DOM.
   * @return The ID of the resolved node, null if there is no match.
   */
  ...
  /**
   * @return The composite {@link IStylesheetUserPreferences} based on the
   *         preferences from the incorporated fragments
   */
