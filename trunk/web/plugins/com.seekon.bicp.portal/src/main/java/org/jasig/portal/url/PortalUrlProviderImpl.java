...
/**
 * Generates {@link IPortalUrlBuilder} objects based on various layout and
 * portlet targets.
 * 
...
public class PortalUrlProviderImpl implements IPortalUrlProvider {
  private static final String PORTAL_ACTION_URL_BUILDER = PortalUrlProviderImpl.class
    .getName()
    + ".PORTAL_ACTION_URL_BUILDER";
  ...
  @Autowired
  public void setPortletDelegationManager(
   ...
   * 
   * @see
   * org.jasig.portal.url.IPortalUrlProvider#getDefaultUrl(javax.servlet.http
   * .HttpServletRequest)
   */
  @Override
  public IPortalUrlBuilder getDefaultUrl(HttpServletRequest request) {
    ...
    final String defaultLayoutNodeId = urlNodeSyntaxHelper
      .getDefaultLayoutNodeId(request);
    return this.getPortalUrlBuilderByLayoutNode(request, defaultLayoutNodeId,
      UrlType.RENDER);
  }
   ...
   * 
   * @see
   * org.jasig.portal.url.IPortalUrlProvider#getPortalUrlBuilderByLayoutNode
   * (javax.servlet.http.HttpServletRequest, java.lang.String,
   * org.jasig.portal.url.UrlType)
   */
  @Override
  public IPortalUrlBuilder getPortalUrlBuilderByLayoutNode(
    HttpServletRequest request, String layoutNodeId, UrlType urlType) {
    final IPortletWindowId portletWindowId = getPortletWindowId(request,
      layoutNodeId);

    return new PortalUrlBuilder(this.urlSyntaxProvider, request, layoutNodeId,
      portletWindowId, urlType);
  }

  private IPortletWindowId getPortletWindowId(HttpServletRequest request,
    String layoutNodeId) {
    if (layoutNodeId == null) {
      return null;
    }

    final LayoutNodeType layoutNodeType = this.getLayoutNodeType(request,
      layoutNodeId);
    if (layoutNodeType == null) {
      throw new IllegalArgumentException("No layout node exists for id: "
        + layoutNodeId);
    }
   ...
   * 
   * @see
   * org.jasig.portal.url.IPortalUrlProvider#getPortalUrlBuilderByPortletWindow
   * (javax.servlet.http.HttpServletRequest,
   * org.jasig.portal.portlet.om.IPortletWindowId, org.jasig.portal.url.UrlType)
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.url.IPortalUrlProvider#getPortalUrlBuilderByPortletFName
   * (javax.servlet.http.HttpServletRequest, java.lang.String,
   * org.jasig.portal.url.UrlType)
   */
  @Override
  public IPortalUrlBuilder getPortalUrlBuilderByPortletFName(
    ...
    final IPortletWindow portletWindow = this.portletWindowRegistry
      .getOrCreateDefaultPortletWindowByFname(request, portletFName);
    if (portletWindow == null) {
      throw new IllegalArgumentException("Could not find PortletWindow for fname="
        + portletFName + " to create IPortalUrlBuilder");
    }
  ...
  protected IPortalUrlBuilder getPortalUrlBuilderByPortletWindow(
    ...
    final IPortletWindowId parentPortletWindowId = portletWindow
      .getDelegationParentId();
    if (parentPortletWindowId != null) {
      // Get the portal url builder that targets the parent
      final IPortalUrlBuilder portalUrlBuilder = this
        .getPortalUrlBuilderByPortletWindow(request, parentPortletWindowId, urlType);

      // See if there is additional delegation request data that needs to be
      // added to the URL
      final DelegationRequest delegationRequest = this.portletDelegationManager
        .getDelegationRequest(request, portletWindowId);
      if (delegationRequest != null) {
        final IPortletUrlBuilder parentPortletUrlBuilder = portalUrlBuilder
          .getPortletUrlBuilder(parentPortletWindowId);

        final Map<String, List<String>> parentParameters = delegationRequest
          .getParentParameters();
        if (parentParameters != null) {
          parentPortletUrlBuilder.setParameters(parentParameters);
        }

        final PortletMode parentPortletMode = delegationRequest
          .getParentPortletMode();
        if (parentPortletMode != null) {
          parentPortletUrlBuilder.setPortletMode(parentPortletMode);
        }

        final WindowState parentWindowState = delegationRequest
  ...
  /**
   * Verify the requested portlet window corresponds to a node in the user's
   * layout and return the corresponding layout node id
   */
  protected String verifyPortletWindowId(HttpServletRequest request,
    ...
    final IPortletWindow portletWindow = this.portletWindowRegistry
      .getPortletWindow(request, portletWindowId);
    final IPortletWindowId delegationParentWindowId = portletWindow
      .getDelegationParentId();
    ...
    final IUserLayoutNodeDescription node = userLayoutManager
      .getNode(channelSubscribeId);
    if (node == null) {
      throw new IllegalArgumentException("No layout node exists for id "
        + channelSubscribeId + " of window " + portletWindowId);
    }
  ...
  /**
   * Verify the requested node exists in the user's layout. Also if the node
   * exists see if it is a portlet node and if it is return the
   * {@link IPortletWindowId} of the corresponding portlet.
   */
  protected LayoutNodeType getLayoutNodeType(HttpServletRequest request,
    String folderNodeId) {
    final IUserInstance userInstance = this.userInstanceManager
      .getUserInstance(request);
    final IUserPreferencesManager preferencesManager = userInstance
