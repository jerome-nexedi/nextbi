...
/**
 * Base implementation of CharacterEventSource that deals with reading the
 * element data off the stream.
 * 
...
public abstract class BasePlaceholderEventSource implements CharacterEventSource {
  ...
  protected void generateCharacterEvents(HttpServletRequest servletRequest,
  ...
  /**
   * Read {@link XMLEvent}s off the {@link XMLEventReader} until the
   * corresponding {@link EndElement} is found.
   */
  protected final void readToEndElement(XMLEventReader eventReader,
    ...
    while (!elementStack.isEmpty()) {
      ...
      if (nextEvent.isStartElement()) {
        ...
        if (!lastStart.equals(endElement.getName())) {
          throw new XMLStreamException("Invalid XML Structure, expected EndElement "
            + lastStart + " but found EndElment " + endElement.getName());
        }
      }
    }
  }
