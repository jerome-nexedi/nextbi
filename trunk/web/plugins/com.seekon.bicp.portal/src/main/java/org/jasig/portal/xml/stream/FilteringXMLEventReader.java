...
/**
 * Base class for {@link XMLEventReader}s that want to modify or remove events
 * from the reader stream. If a {@link StartElement} event is removed the
 * subclass's {@link #filterEvent(XMLEvent, boolean)} will not see any events
 * until after the matching {@link EndElement} event.
 * 
...
public abstract class FilteringXMLEventReader extends BaseXMLEventReader {
  ...
  protected final XMLEvent internalNext(boolean peek) throws XMLStreamException {
    ...
    do {
      ...
      // If there are pruned elements in the queue filtering events is still
      // needed
      if (!prunedElements.isEmpty()) {
        ...
        // If end element pop the newest name of the queue and double check that
        // the start/end elements match up
          ...
          if (!startElementName.equals(endElementName)) {
            throw new IllegalArgumentException(
              "Malformed XMLEvent stream. Expected end element for "
                + startElementName + " but found end element for " + endElementName);
          }
        }
        ...
        // If the event is being removed and it is a start element all elements
        // until the matching
   ...
   *          The current event
   * @param peek
   *          If the event is from a {@link #peek()} call
   * @return The event to return, if null is returned the event is dropped from
   *         the stream and the next event will be used.
   */
