...
public class ChannelRegistryPropertyFinder implements IEntityPropertyFinder {
  ...
  private static final Log log = LogFactory
    .getLog(ChannelRegistryPropertyFinder.class);
  ...
  public static void init() {
    ...
    /*
     * this array should hold the desired attributes of a channel element as
     * defined in channelRegistry.dtd
     */
    names = new String[5];
    names[0] = "Name";
    names[1] = "Title";
  ...
  public String getProperty(EntityIdentifier entityID, String name) {
    ...
    if (entityID.getType().equals(chan)) {
      try {
        // Element c =
        // ChannelRegistryManager.getChannel("chan"+entityID.getKey());
        // r = c.getAttribute(name);
        IPortletDefinition cd = PortletDefinitionRegistryLocator
          .getPortletDefinitionRegistry().getPortletDefinition(entityID.getKey());
        if (name.equals("Name")) {
          r = cd.getName();
        }
        ...
        log.error("ChannelRegistryPropertyFinder.getProperty(" + entityID.getKey()
          + "-" + entityID.getType().getName() + "," + name + ") :", e);
      }
    }
    return r;
  }
