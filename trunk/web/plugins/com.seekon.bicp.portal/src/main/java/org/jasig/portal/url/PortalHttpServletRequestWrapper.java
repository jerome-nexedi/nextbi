...
/**
 * Portal wide request wrapper. Provides portal specific information for request
 * parameters, attributes, user and role.
 * 
 * @author Peter Kharchenko: pkharchenko at unicon.net
 * @version $Revision: 11911 $
 */
public class PortalHttpServletRequestWrapper extends
  AbstractHttpServletRequestWrapper {
  /**
   * {@link javax.servlet.http.HttpServletRequest} attribute that this
   * {@link HttpServletRequest} object will be available.
   */
  public static final String ATTRIBUTE__HTTP_SERVLET_REQUEST = PortalHttpServletRequestWrapper.class
    .getName()
    + ".PORTAL_HTTP_SERVLET_REQUEST";

  /**
   * {@link javax.servlet.http.HttpServletRequest} attribute that the
   * {@link HttpServletResponse} object will be available.
   */
  public static final String ATTRIBUTE__HTTP_SERVLET_RESPONSE = PortalHttpServletRequestWrapper.class
    .getName()
    + ".PORTAL_HTTP_SERVLET_RESPONSE";
  ...
  public PortalHttpServletRequestWrapper(HttpServletRequest request,
    HttpServletResponse response, IUserInstanceManager userInstanceManager) {
    super(request);
  ...
  public long getDateHeader(String name) {
    ...
    throw new IllegalArgumentException("Header '" + name
      + "' cannot be converted to long: '" + value + "' is of type "
  ...
  public Enumeration getHeaderNames() {
    ...
    for (final Enumeration<?> headerNamesEnum = super.getHeaderNames(); headerNamesEnum
      .hasMoreElements();) {
  ...
  public int getIntHeader(String name) {
    ...
    throw new IllegalArgumentException("Header '" + name
      + "' cannot be converted to int: '" + value + "' is of type "
      + value.getClass().getName());
  }
  ...
  /**
   * Determines whether or not the user is in the given role. The wrapped
   * request is consulted first then the {@link GroupService} is used to
   * determine if a group exists for the specified role and if the user is a
   * member of it.
   * 
  ...
  public boolean isUserInRole(String role) {
    ...
    if (groupForRole == null) {
      final EntityIdentifier[] results = GroupService.searchForGroups(role,
        GroupService.IS, IPerson.class);
      if (results == null || results.length == 0) {
        return false;
      }

      if (results.length > 1) {
        this.logger.warn(results.length + " groups were found for role '" + role
    ...
    final IGroupMember personGroupMember = GroupService
      .getGroupMember(personEntityId);
