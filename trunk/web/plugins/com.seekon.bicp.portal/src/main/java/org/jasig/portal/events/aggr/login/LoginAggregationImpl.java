...
@SequenceGenerator(name = "UP_LOGIN_EVENT_AGGREGATE_GEN", sequenceName = "UP_LOGIN_EVENT_AGGREGATE_SEQ", allocationSize = 100)
@TableGenerator(name = "UP_LOGIN_EVENT_AGGREGATE_GEN", pkColumnValue = "UP_LOGIN_EVENT_AGGREGATE_PROP", allocationSize = 100)
@Cacheable
...
public class LoginAggregationImpl implements LoginAggregation, Serializable {
  ...
  @JoinTable(name = "UP_LOGIN_EVENT_AGGREGATE__UIDS", joinColumns = @JoinColumn(name = "LOGIN_AGGR_ID"))
  ...
  private void checkState() {
    if (this.loginCount > 0 && this.uniqueUserNames.isEmpty()) {
      throw new IllegalStateException(
        "intervalComplete has been called, countUser can no longer be called");
    }
  }
  ...
  public int hashCode() {
    ...
    result = prime * result
      + ((aggregatedGroup == null) ? 0 : aggregatedGroup.hashCode());
  ...
  public String toString() {
    return "LoginAggregationImpl [timeDimension=" + timeDimension
      + ", dateDimension=" + dateDimension + ", interval=" + interval
      + ", groupName=" + aggregatedGroup + ", duration=" + duration
      + ", loginCount=" + loginCount + ", uniqueLoginCount=" + uniqueLoginCount
      + "]";
  }
}
