...
public class PersonDirectoryUserInfoService implements UserInfoService {
  ...
  /**
   * @param personAttributeDao
   *          the personAttributeDao to set
   */
  @Autowired
   ...
   * 
   * @see
   * org.apache.pluto.spi.optional.UserInfoService#getUserInfo(javax.portlet
   * .PortletRequest, org.apache.pluto.PortletWindow)
   */
  public Map<String, String> getUserInfo(PortletRequest request,
    PortletWindow plutoPortletWindow) throws PortletContainerException {
    // Get the remote user
  ...
  /**
   * Commons logic to get a subset of the user's attributes for the specified
   * portlet window.
   * 
   * @param remoteUser
   *          The user to get attributes for.
   * @param httpServletRequest
   *          The current, underlying httpServletRequest
   * @param portletWindow
   *          The window to filter attributes for
   * @return A Map of user attributes for the user and windows
   * @throws PortletContainerException
   */
  protected Map<String, String> getUserInfo(String remoteUser,
    HttpServletRequest httpServletRequest, IPortletWindow portletWindow)
    throws PortletContainerException {
    ...
    final IPersonAttributes personAttributes = this.personAttributeDao
      .getPerson(remoteUser);
    if (personAttributes == null) {
      return Collections.emptyMap();
    }
    final List<? extends UserAttribute> expectedUserAttributes = this
      .getExpectedUserAttributes(httpServletRequest, portletWindow);

    final Map<String, String> portletUserAttributes = this.generateUserInfo(
      personAttributes, expectedUserAttributes, httpServletRequest);
    return portletUserAttributes;
  }

  /**
   * Using the Map of portal user attributes and a List of expected attributes
   * generate the USER_INFO map for the portlet
   * 
   * @param portalUserAttributes
   *          All the attributes the portal knows about the user
   * @param expectedUserAttributes
   *          The attributes the portlet expects to get
  ...
  protected Map<String, String> generateUserInfo(
    ...
    final Map<String, String> portletUserAttributes = new HashMap<String, String>(
      expectedUserAttributes.size());
    ...
    for (final UserAttribute userAttributeDD : expectedUserAttributes) {
      ...
      // TODO a personAttributes.hasAttribute(String) API is needed here, if
      // hasAttribute and null then put the key with no value in the returned
      // map
  ...
  /**
   * Converts the full portal user attribute Map to a USER_INFO map for the
   * portlet
   * 
   * @param portalUserAttributes
   *          All the attributes the portal knows about the user
   ...
   */
  protected Map<String, String> generateUserInfo(
    final Map<String, Object> portalUserAttributes) {
    final Map<String, String> portletUserAttributes = new HashMap<String, String>(
      portalUserAttributes.size());

    // Copy expected attributes to the USER_INFO Map
    for (final Map.Entry<String, Object> portalUserAttributeEntry : portalUserAttributes
      .entrySet()) {
   ...
   *          The current request.
   * @param portletWindow
   *          The window to get the expected user attributes for.
   * @return The List of expected user attributes for the portlet
   * @throws PortletContainerException
   *           If expected attributes cannot be determined
   */
  protected List<? extends UserAttribute> getExpectedUserAttributes(
    ...
    final PortletApplicationDefinition portletApplicationDescriptor = this.portletDefinitionRegistry
      .getParentPortletApplicationDescriptor(portletDefinition
