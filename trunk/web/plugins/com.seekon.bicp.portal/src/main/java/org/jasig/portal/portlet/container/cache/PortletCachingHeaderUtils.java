...
public final class PortletCachingHeaderUtils {
   ...
   *          The {@link CachedPortletData} to base the headers on
   * @param portletResourceOutputHandler
   *          The handler to write the headers to
   * 
   * @see #setCachingHeaders(int, boolean, long, PortletResourceOutputHandler)
   * @see #setETag(String, PortletResourceOutputHandler)
   */
  public static void setCachingHeaders(CachedPortletData<?> cachedPortletData,
    PortletResourceOutputHandler portletResourceOutputHandler) {
    final long expirationTime = cachedPortletData.getExpirationTime();
    final int maxAge = (int) TimeUnit.MILLISECONDS.toSeconds(expirationTime
  ...
  /**
   * Set the Last-Modified, CacheControl, and Expires headers based on the
   * maxAge, publicScope and lastModified.
   * 
   * @param maxAge
   *          Maximum age for the content, follows the portlet rules (-1 cache
   *          forever, 0 cache never, N cache seconds)
   * @param publicScope
   *          If the content is public
   * @param lastModified
   *          The last modification timestamp of the content
   * @param portletResourceOutputHandler
   *          The handler to write the headers to
   */
  public static void setCachingHeaders(int maxAge, boolean publicScope,
    long lastModified, PortletResourceOutputHandler portletResourceOutputHandler) {
    if (maxAge != 0) {
      ...
      portletResourceOutputHandler.setDateHeader("Expires", System
        .currentTimeMillis()
        + TimeUnit.SECONDS.toMillis(maxAge));
   ...
   *          ETag value
   * @param portletResourceOutputHandler
   *          The handler to write the header to
   */
  public static void setETag(String token,
    PortletResourceOutputHandler portletResourceOutputHandler) {
