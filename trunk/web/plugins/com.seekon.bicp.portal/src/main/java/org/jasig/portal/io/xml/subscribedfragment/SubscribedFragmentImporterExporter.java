...
public class SubscribedFragmentImporterExporter extends
  ...
  public Set<PortalDataKey> getImportDataKeys() {
    return Collections
      .singleton(SubscribedFragmentPortalDataType.IMPORT_40_DATA_KEY);
  }
  ...
  public Iterable<? extends IPortalData> getPortalData() {
    final List<String> allUsersWithActiveSubscriptions = this.userFragmentSubscriptionDao
      .getAllUsersWithActiveSubscriptions();

    return Collections2.transform(allUsersWithActiveSubscriptions,
  ...
  public void importData(ExternalSubscribedFragments data) {
    ...
    for (final SubscribedFragmentType subscribedFragmentType : data
      .getSubscribedFragments()) {
      ...
      if (fragmentPerson == null) {
        throw new IllegalArgumentException("No fragmentOwner " + fragmentOwner
          + " exists to subscribe to, be sure to import all fragment owners first");
      }

      final IUserFragmentSubscription userFragmentSubscription = this.userFragmentSubscriptionDao
        .getUserFragmentInfo(person, fragmentPerson);
      if (userFragmentSubscription == null) {
        this.userFragmentSubscriptionDao.createUserFragmentInfo(person,
          fragmentPerson);
      }
    }
  }
  ...
  private ExternalSubscribedFragments exportInternal(final IPerson person) {
    ...
    data
      .setVersion(SubscribedFragmentPortalDataType.IMPORT_40_DATA_KEY.getVersion());

    final List<SubscribedFragmentType> subscribedFragments = data
      .getSubscribedFragments();

    for (final IUserFragmentSubscription userFragmentSubscription : this.userFragmentSubscriptionDao
      ...
      if (userFragmentSubscription.isActive()) {
        ...
        subscribedFragmentType.setFragmentOwner(userFragmentSubscription
          .getFragmentOwner());
  ...
  public ExternalSubscribedFragments deleteData(String id) {
    ...
    for (final IUserFragmentSubscription userFragmentSubscription : this.userFragmentSubscriptionDao
      .getUserFragmentInfo(person)) {
      this.userFragmentSubscriptionDao
        .deleteUserFragmentInfo(userFragmentSubscription);
    }
