...
/**
 * This is a bootstrap class and facade for the IEntityLockService
 * implementation. It presents a simple api for acquiring lock objects,
 * <code>IEntityLocks</code>, that can be used to control concurrent access to
 * portal entities in a multi-server environment. (See
 * org.jasig.portal.concurrency.IEntityLockService for a fuller description.)
 * <p>
 ...
 *   lock.renew();<br>
  * </code>
 * <p>
 * A READ lock guarantees shared access; other clients can get READ locks but
 * not WRITE locks. A WRITE lock guarantees exclusive access; no other clients
 * can get either READ or WRITE locks on the entity.
 * 
...
public class EntityLockService {
  ...
  private void initialize() throws LockingException {
    ...
    String factoryName = PropertiesManager.getProperty(
      "org.jasig.portal.concurrency.IEntityLockServiceFactory", null);

    if (factoryName == null) {
      log
        .warn("Property org.jasig.portal.concurrency.IEntityLockServiceFactory not configured in PropertiesManager.  Defaulting to  org.jasig.portal.concurrency.locking.ReferenceEntityLockServiceFactory");
      factoryName = "org.jasig.portal.concurrency.locking.ReferenceEntityLockServiceFactory";
    }

    try {
      IEntityLockServiceFactory lockServiceFactory = (IEntityLockServiceFactory) Class
  ...
  public IEntityLock newReadLock(Class entityType, String entityKey, String owner)
    ...
    return lockService.newLock(entityType, entityKey, IEntityLockService.READ_LOCK,
      owner);
  }
   ...
   */
  public IEntityLock newReadLock(Class entityType, String entityKey, String owner,
    int duration) throws LockingException {
    return lockService.newLock(entityType, entityKey, IEntityLockService.READ_LOCK,
      owner, duration);
  }
  ...
  public IEntityLock newReadLock(EntityIdentifier entityID, String owner)
    throws LockingException {
    return lockService.newLock(entityID.getType(), entityID.getKey(),
      IEntityLockService.READ_LOCK, owner);
  }
  ...
  public IEntityLock newReadLock(EntityIdentifier entityID, String owner,
    ...
    return lockService.newLock(entityID.getType(), entityID.getKey(),
      IEntityLockService.READ_LOCK, owner, durationSecs);
  }
  ...
  public IEntityLock newWriteLock(Class entityType, String entityKey, String owner)
    throws LockingException {
    return lockService.newLock(entityType, entityKey, IEntityLockService.WRITE_LOCK,
      owner);
  }
  ...
  public IEntityLock newWriteLock(EntityIdentifier entityID, String owner)
    throws LockingException {
    return lockService.newLock(entityID.getType(), entityID.getKey(),
      IEntityLockService.WRITE_LOCK, owner);
  }
  ...
  public IEntityLock newWriteLock(EntityIdentifier entityID, String owner,
    int durationSecs) throws LockingException {
    return lockService.newLock(entityID.getType(), entityID.getKey(),
      IEntityLockService.WRITE_LOCK, owner, durationSecs);
  }
}
