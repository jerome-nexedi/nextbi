...
@SequenceGenerator(name = "UP_DLM_EVALUATOR_GEN", sequenceName = "UP_DLM_EVALUATOR_SEQ", allocationSize = 1)
@TableGenerator(name = "UP_DLM_EVALUATOR_GEN", pkColumnValue = "UP_DLM_EVALUATOR", allocationSize = 1)
@Cacheable
...
public abstract class Evaluator {
  ...
  /**
   * Serializes this {@link Evaluator} into the same XML format supported by
   * dlm.xml. <b>NOTE:</b> this method will only yield usable XML if invoked on
   * an instance of {@link FragmentDefinition}; all other subclasses will return
   * only XML fragments.
   * 
   * @param parent
   *          The XML structure (starting with &lt;dlm:fragment&gt;) so far
   */
