...
/**
 * Spring managed version of the Xalan Elements helper class used during portal
 * XSL transformations.
 * 
...
public class XalanAuthorizationHelperBean implements IXalanAuthorizationHelper {
   ...
   * 
   * @see
   * org.jasig.portal.security.xslt.IAuthorizationHelper#canRender(java.lang
    ...
    try {
      ...
      if (portletDefinition == null) {
        if (this.logger.isInfoEnabled()) {
          this.logger.info("No PortletDefinition for fname='" + fname
            + "', returning false.");
        }
      ...
      this.logger.warn("Could not find PortletDefinition for fname='" + fname
        + "' while checking if user '" + userName
        + "' can render it. Returning FALSE.", e);
      return false;
    }
