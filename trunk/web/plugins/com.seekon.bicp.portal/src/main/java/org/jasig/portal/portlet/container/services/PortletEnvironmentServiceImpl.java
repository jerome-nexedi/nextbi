...
/**
 * Provides custom portlet session instance to use a different scoping attribute
 * value
 * 
...
public class PortletEnvironmentServiceImpl extends
   ...
   * (non-Javadoc)
   * 
   * @seeorg.apache.pluto.container.impl.PortletEnvironmentServiceImpl#
   * createPortletSession(javax.portlet.PortletContext,
   * org.apache.pluto.container.PortletWindow, javax.servlet.http.HttpSession)
   */
  @Override
  public PortletSession createPortletSession(PortletContext portletContext,
    PortletWindow portletWindow, HttpSession session) {
    // TODO pluto 1.1 PortletEnvironmentService#createPortletSession passed in
    // the request; now use IPortalRequestUtils#getCurrentPortalRequest()?
    final HttpServletRequest request = portalRequestUtils.getCurrentPortalRequest();
    final IPortletWindow internalPortletWindow = this.portletWindowRegistry
      .convertPortletWindow(request, portletWindow);
    final IPortletEntity portletEntity = internalPortletWindow.getPortletEntity();
    final IPortletEntityId portletEntityId = portletEntity.getPortletEntityId();

    return new ScopingPortletSessionImpl(portletEntityId, portletContext,
      portletWindow, session);
  }
