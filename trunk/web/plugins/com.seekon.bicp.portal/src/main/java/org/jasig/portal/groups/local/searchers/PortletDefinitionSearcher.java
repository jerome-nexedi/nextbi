...
public class PortletDefinitionSearcher implements ITypedEntitySearcher {
  ...
  public EntityIdentifier[] searchForEntities(String query, int method)
    ...
    if (log.isDebugEnabled()) {
      log.debug("Found " + definitions.size() + " matching definitions for query "
        + query);
    }
    ...
    for (final ListIterator<IPortletDefinition> defIter = definitions.listIterator(); defIter
      .hasNext();) {
      final IPortletDefinition definition = defIter.next();
      identifiers[defIter.previousIndex()] = new EntityIdentifier(definition
        .getPortletDefinitionId().getStringId(), this.getType());
    }
