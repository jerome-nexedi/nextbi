...
public interface IPortalEventAggregator<E extends PortalEvent> {
   ...
   * 
   * @param e
   *          The event to aggregate
   * @param eventSession
   *          Information about the event session associated with the event, MAY
   *          BE NULL!
   * @param currentIntervals
   *          Information about all of the intervals the event exists in.
   */
  void aggregateEvent(E e, EventSession eventSession,
    Map<AggregationInterval, AggregationIntervalInfo> currentIntervals);

  /**
   * Handle crossing over an interval boundary, called after the LAST event of
   * the interval is processed.
   * 
   * @param interval
   *          The type of interval that was crossed
   * @param intervals
   *          Information about all intervals that the previous set of events
   *          was part of
   */
  void handleIntervalBoundary(AggregationInterval interval,
