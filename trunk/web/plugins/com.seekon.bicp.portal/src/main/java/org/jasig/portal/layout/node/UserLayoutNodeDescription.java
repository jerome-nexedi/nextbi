...
/**
 * An class describing common features of user layout nodes, that is channels
 * and folders
 * 
 ...
 * @version 1.0
 */
public abstract class UserLayoutNodeDescription implements
  ...
  /**
   * Returns the precedence value for this node. The precedence is 0.0 for a
   * user owned node and the value of the node's owning fragment's precedence
   * for a node incorporated from another fragment. Added by SCT for DLM.
   */
  ...
  /**
   * Set the precedence of a node. See getPrecedence for more information. Added
   * by SCT for DLM.
   */
  ...
  /**
   * Returns true if a child node may be added to the node. Added by SCT for
   * DLM.
   */
  ...
  /**
   * Set whether or not child nodes can be added to this node. Added by SCT for
   * DLM.
   */
  ...
  /**
   * Returns a node Id. The Id has to be unique in the entire user layout
   * document.
   * 
  ...
  /**
   * Set a new node Id. The Id has to be unique in the entire user layout
   * document.
   * 
   */
  ...
  public void addNodeAttributes(Element node) {
    ...
    if (!this.isAddChildAllowed())
      node
        .setAttributeNS(Constants.NS_URI, Constants.ATT_ADD_CHILD_ALLOWED, "false");
    if (this.getPrecedence() != 0.0)
      node.setAttributeNS(Constants.NS_URI, Constants.ATT_PRECEDENCE, Double
        .toString(this.getPrecedence()));
  }

  /**
   * A factory method to create a <code>UserLayoutNodeDescription</code>
   * instance, based on the information provided in the user layout
   * <code>Element</code>.
   * 
   * @param xmlNode
   *          a user layout DTD folder/channel <code>Element</code> value
