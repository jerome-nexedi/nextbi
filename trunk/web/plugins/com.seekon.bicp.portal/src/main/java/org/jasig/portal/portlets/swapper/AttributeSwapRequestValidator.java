...
public class AttributeSwapRequestValidator {
  ...
  /**
   * {@link IAttributeSwapperHelper} to use for getting configuration and query
   * information from for validation.
   */
  public void setAttributeSwapperHelper(
    IAttributeSwapperHelper attributeSwapperHelper) {
  ...
  public void validateAttributesForm(AttributeSwapRequest attributeSwapRequest,
    ...
    final Set<String> swappableAttributes = this.attributeSwapperHelper
      .getSwappableAttributes(externalContext);

    final Map<String, Attribute> currentAttributes = attributeSwapRequest
      .getCurrentAttributes();
    this.checkAttributesMap(context, "currentAttributes", swappableAttributes,
      currentAttributes);

    final Map<String, Attribute> attributesToCopy = attributeSwapRequest
      .getAttributesToCopy();
    this.checkAttributesMap(context, "attributesToCopy", swappableAttributes,
      attributesToCopy);
  }

  /**
   * Checks the keys in the Map on the model against a Set to ensure there are
   * no values in the Map that aren't also in the Set
   */
  protected void checkAttributesMap(MessageContext context, String basePath,
    Set<String> swappableAttributes, Map<String, Attribute> attributesToCopy) {
