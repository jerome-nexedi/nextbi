...
public interface IUserLayoutStore {
   ...
   * 
   * @param Person
   *          an <code>IPerson</code> object specifying the user
   * @param profile
   *          a user profile
   * @return a <code>Document</code> containing user layout (conforms to
   *         userLayout.dtd)
   * @exception Exception
   *              if an error occurs
   */
   ...
   *          An <code>IPerson</code> object specifying the user
   * @param profile
   *          A valid profile for <code>person</code>
   ...
   * 
   * @param layout
   *          XML representing a user's layout and <code>UserPreferences</code>
   */
   ...
   * @param layoutXML
   *          a <code>Document</code> containing user layout (conforming to
   *          userLayout.dtd)
   * @param channelsAdded
   *          a boolean flag specifying if new channels have been added to the
   *          current user layout (for performance optimization purposes)
   * @exception Exception
   *              if an error occurs
   */
  ...
  /**
   * Creates a new user profile in the database. In the process, new profileId
   * is assigned to the profile
   * 
   * @param person
   *          User
   * @param profile
   *          profile object (profile id in this object will be overwritten)
   ...
   * 
   * @param person
   *          an <code>IPerson</code> object representing the user
   * @param profileId
   *          profile id
   */
   ...
   * @param person
   *          User
   * @return a <code>Hashtable</code> mapping user profile ids (
   *         <code>Integer</code> objects) to the {@link UserProfile} objects
   */
  ...
  /**
   * add a new system profile to the database. During this process, a new
   * profile id will be assigned to the profile.
   * 
   * @param profile
   ...
   * 
   * @return a <code>Hashtable</code> mapping system profile ids (
   *         <code>Integer</code> objects) to the {@link UserProfile} objects
   */
   ...
   * @param userAgent
   *          User-Agent header string
   * @param systemProfileId
   *          profile id of a profile to which given user-agent will be mapped
   */
  ...
  /**
   * Returns an object suitable for identifying channel attribute and parameter
   * values in a user's layout that differ from the values on the same element
   * in a fragment. This is used by the layout manager to know which ones must
   * be persisted.
   * 
   * @param sId
   ...
   * layout that differ from the values on the same element in a fragment. This
   * is used by the layout manager to know which ones must be persisted.
   * 
   * @param sId
   ...
   * fragment in the dlm.xml. Precedence is actually based on two elements in a
   * fragment definition: the precedence and the index of the fragment
   * definition in the dlm.xml file. If two fragments are given equal precedence
   * then the index if relied upon to resolve conflicts with UI elements.
   */
