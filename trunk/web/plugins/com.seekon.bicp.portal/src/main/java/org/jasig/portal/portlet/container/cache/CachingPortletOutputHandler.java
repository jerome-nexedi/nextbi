...
/**
 * Captures the output of a portlet for later re-use. The maximumSize field
 * allows for setting a max number of bytes/chars to cache before giving up on
 * the caching.
 * 
...
public class CachingPortletOutputHandler implements PortletOutputHandler {
  ...
  public CachingPortletOutputHandler(PortletOutputHandler portletOutputHandler,
    int maximumSize) {
  ...
  public <T extends Serializable> CachedPortletData<T> getCachedPortletData(
    ...
    return new CachedPortletData<T>(portletResult,
      this.cachingWriter != null ? this.cachingWriter.toString() : null,
      this.cachingOutputStream != null ? this.cachingOutputStream.toByteArray()
        : null, contentType, cacheControl.isPublicScope(), cacheControl.getETag(),
  ...
  public PrintWriter getPrintWriter() throws IOException {
    ...
    if (this.printWriter == null) {
      ...
      // Create the limiting tee writer to write to the actual PrintWriter and
      // the cachingWriter
  ...
  public OutputStream getOutputStream() throws IOException {
    ...
    if (this.teeStream == null) {
      final OutputStream delegateOutputStream = this.portletOutputHandler
        .getOutputStream();
      this.cachingOutputStream = new ByteArrayOutputStream();

      // Create the limiting tee output stream to the actual OutputStream and
      // the cachingOutputStream
      this.teeStream = new LimitingTeeOutputStream(this.maximumSize,
        delegateOutputStream, this.cachingOutputStream,
