...
/**
 * Utility functions for applying user changes to the user's plf prior to
 * persisting.
 * 
...
public class HandlerUtils {
   ...
   * null is returned. If the create is true then an attempt is made to create
   * the node along with any necessary ancestor nodes needed to represent the
   * path along the tree.
   */
  public static Element getPLFNode(Element compViewNode, IPerson person,
    boolean create, boolean includeChildNodes) throws PortalException {
    ...
    if (create == true)
      return createPlfNodeAndPath(compViewNode, includeChildNodes, person);
    return null;
  }

  /**
   * Creates a copy of the passed in ILF node in the PLF if not already there as
   * well as creating any ancestor nodes along the path from this node up to the
   * layout root if they are not there.
   */
  public static Element createPlfNodeAndPath(Element compViewNode,
    boolean includeChildNodes, IPerson person) throws PortalException {
    ...
    if (compViewNode.getAttribute(Constants.ATT_ID).startsWith(
      ...
      return createILFCopy(compViewNode, compViewParent, includeChildNodes, plf,
        plfParent, person);
    return createOrMovePLFOwnedNode(compViewNode, compViewParent, true, // create
      // if
      // not
      // found
   ...
   * attributes.
   */
  private static Element createILFCopy(Element compViewNode, Element compViewParent,
    boolean includeChildNodes, Document plf, Element plfParent, IPerson person)
    ...
    if (plfNode.getAttribute(Constants.ATT_PLF_ID).equals("")) {
      ...
      try {
        if (!plfNode.getAttribute(Constants.ATT_CHANNEL_ID).equals("")) // dealing
          // with
          // a
          // channel
        ...
        throw new PortalException("Exception encountered while "
          + "generating new user layout node " + "Id for userId=" + person.getID(),
          e);
      }
  ...
  /**
   * Creates or moves the plf copy of a node in the composite view and inserting
   * it before its next highest sibling so that if dlm is not used then the
   * model ends up exactly like the original non-dlm persistance version. The
   * position set is also updated and if no ilf copy nodes are found in the
   * sibling list the set is cleared if it exists.
   */
  static Element createOrMovePLFOwnedNode(Element compViewNode,
    Element compViewParent, boolean createIfNotFound, boolean createChildNodes,
    Document plf, Element plfParent, IPerson person) throws PortalException {
