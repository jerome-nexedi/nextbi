...
public class PortletWindowAttributeSource implements AttributeSource, BeanNameAware {
  ...
  public final Iterator<Attribute> getAdditionalAttributes(
    ...
    final Tuple<IPortletWindow, StartElement> portletWindowAndElement = this.portletWindowRegistry
      .getPortletWindow(request, event);
    if (portletWindowAndElement == null) {
      this.logger.warn("No IPortletWindow could be found or created for element: "
        + event);
      return null;
    }
    ...
    final Attribute windowStateAttribute = xmlEventFactory.createAttribute(
      "windowState", windowState.toString());
    attributes.add(windowStateAttribute);

    // Add portlet mode data
    final PortletMode portletMode = portletWindow.getPortletMode();
    final Attribute portletModeAttribute = xmlEventFactory.createAttribute(
      "portletMode", portletMode.toString());
  ...
  public final CacheKey getCacheKey(HttpServletRequest request,
    ...
    final Set<IPortletWindow> portletWindows = this.portletWindowRegistry
      .getAllLayoutPortletWindows(request);
    ...
    for (final IPortletWindow portletWindow : portletWindows) {
      if (portletWindow != null) {
        ...
        final WindowState windowState = portletWindow.getWindowState();
        final PortletMode portletMode = portletWindow.getPortletMode();
        cacheKeyBuilder.addAll(portletWindowId, windowState.toString(), portletMode
          .toString());
      } else {
        this.logger
          .warn("portletWindowRegistry#getAllLayoutPortletWindows() returned a null portletWindow");
      }
    }
