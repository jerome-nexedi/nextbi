 ...
 * credentials against an LDAP directory. It expects to be able to bind to the
 * LDAP directory as the user so that it can authenticate the user.
 * </p>
 * <p>
 * By implementing the
 * {@link org.jasig.portal.security.IConfigurableSecurityContext} interface this
 * context may have properties set on it. The one property the
 * <code>SimpleLdapSecurityContext</code> looks for is defined by the String
 * {@link #LDAP_PROPERTIES_CONNECTION_NAME} "connection". This property allows a
 * specific, named, LDAP connection to be used by the context. If no
 * "connection" property is specified the default LDAP connection returned by
 * {@link org.jasig.portal.ldap.LdapServices} is used.
 * </p>
 * 
 * @author Russell Tokuyama (University of Hawaii)
 * @version $Revision$
 */
public class SimpleLdapSecurityContext extends ChainingSecurityContext implements
  ...
  public int getAuthType() {
    /*
     * What is this for? No one would know what to do with the value returned.
     * Subclasses might know but our getAuthType() doesn't return anything
     * easily useful.
     */
  ...
  public synchronized void authenticate() throws PortalSecurityException {
    ...
    if (this.myPrincipal.UID != null && !this.myPrincipal.UID.trim().equals("")
      ...
      try {
        ...
        if (conn != null) {
          try {
            ...
            if (results != null) {
              if (!results.hasMore())
                log.error("SimpleLdapSecurityContext: user not found , "
              ...
              while (results != null && results.hasMore()) {
                ...
                conn
                  .removeFromEnvironment(javax.naming.Context.SECURITY_CREDENTIALS);
              ...
              log.error("SimpleLdapSecurityContext: No such user: "
                + this.myPrincipal.UID);
            }
            ...
            log.error("SimpleLdapSecurityContext: LDAP Error with user: "
              + this.myPrincipal.UID + "; ", e);
            throw new PortalSecurityException(
              "SimpleLdapSecurityContext: LDAP Error" + e + " with user: "
                + this.myPrincipal.UID);
          } finally {
      ...
      log
        .error("Principal or OpaqueCredentials not initialized prior to authenticate");
    }
  ...
  /**
   * <p>
   * Return a single value of an attribute from possibly multiple values,
   * grossly ignoring anything else. If there are no values, then return an
   * empty string.
   * </p>
   * 
   * @param attrs
