...
/**
 * Provides LDAP access in a way similar to a relational DBMS. This class was
 * modified for the 2.4 release to function more like
 * {@link org.jasig.portal.RDBMServices}. The class should be used via the
 * static {@link #getDefaultLdapServer()} and {@link #getLdapServer(String name)}
 * methods. <br>
 * <br>
 ...
 * @version $Revision$
 * @deprecated The prefered way to access configured ldap servers is using
 *             dependency injection and accessing the LdapContext instances in
 *             the spring context.
 */
public final class LdapServices {
   ...
   * @return An {@link ILdapServer} with the specified name, <code>null</code>
  ...
  /**
   * Get a {@link Map} of {@link ILdapServer} instances from the spring
   * configuration.
   * 
  ...
  public static Map<String, ILdapServer> getLdapServerMap() {
    final ApplicationContext applicationContext = PortalApplicationContextLocator
      .getApplicationContext();
    final Map<String, ILdapServer> ldapServers = applicationContext
      .getBeansOfType(ILdapServer.class);
