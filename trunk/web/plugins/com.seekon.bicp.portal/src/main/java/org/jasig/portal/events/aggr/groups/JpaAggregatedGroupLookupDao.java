 ...
 */
@Repository
public class JpaAggregatedGroupLookupDao extends BaseJpaDao implements
  ...
  public void afterPropertiesSet() throws Exception {
    this.groupServiceParameter = this.createParameterExpression(String.class,
      "groupService");
    this.groupNameParameter = this.createParameterExpression(String.class,
      "groupName");

    this.findGroupMappingByServiceAndNameQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<AggregatedGroupMappingImpl>>() {
        @Override
        public CriteriaQuery<AggregatedGroupMappingImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<AggregatedGroupMappingImpl> criteriaQuery = cb
            .createQuery(AggregatedGroupMappingImpl.class);
          final Root<AggregatedGroupMappingImpl> root = criteriaQuery
            .from(AggregatedGroupMappingImpl.class);
          criteriaQuery.select(root);
          criteriaQuery.where(cb.and(cb.equal(root
            .get(AggregatedGroupMappingImpl_.groupService), groupServiceParameter),
            cb.equal(root.get(AggregatedGroupMappingImpl_.groupName),
              groupNameParameter)));
  ...
  @Override
  public AggregatedGroupMapping getGroupMapping(final String groupService,
    final String groupName) {
    final TypedQuery<AggregatedGroupMappingImpl> query = this
      .createCachedQuery(this.findGroupMappingByServiceAndNameQuery);
    ...
    return this.transactionOperations
      .execute(new TransactionCallback<AggregatedGroupMapping>() {
        @Override
        public AggregatedGroupMapping doInTransaction(TransactionStatus status) {
          final AggregatedGroupMappingImpl aggregatedGroupMapping = new AggregatedGroupMappingImpl(
