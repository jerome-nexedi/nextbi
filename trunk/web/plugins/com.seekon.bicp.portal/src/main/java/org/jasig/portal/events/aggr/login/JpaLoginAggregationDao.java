 ...
 */
@Repository
public class JpaLoginAggregationDao extends BaseJpaDao implements
  ...
  public void afterPropertiesSet() throws Exception {
    this.timeDimensionParameter = this.createParameterExpression(
      TimeDimension.class, "timeDimension");
    this.dateDimensionParameter = this.createParameterExpression(
      DateDimension.class, "dateDimension");
    this.intervalParameter = this.createParameterExpression(
      AggregationInterval.class, "interval");
    this.aggregatedGroupParameter = this.createParameterExpression(
      AggregatedGroupMapping.class, "aggregatedGroup");
    this.aggregatedGroupsParameter = this.createParameterExpression(Set.class,
      "aggregatedGroups");
    ...
    this.findLoginAggregationByDateTimeIntervalQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<LoginAggregationImpl>>() {
        @Override
        public CriteriaQuery<LoginAggregationImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<LoginAggregationImpl> criteriaQuery = cb
            .createQuery(LoginAggregationImpl.class);
          final Root<LoginAggregationImpl> root = criteriaQuery
            .from(LoginAggregationImpl.class);
          criteriaQuery.select(root);
          root.fetch(LoginAggregationImpl_.uniqueUserNames, JoinType.LEFT);
          criteriaQuery.where(cb.and(cb.equal(root
            .get(LoginAggregationImpl_.dateDimension), dateDimensionParameter), cb
            .equal(root.get(LoginAggregationImpl_.timeDimension),
              timeDimensionParameter), cb.equal(root
            .get(LoginAggregationImpl_.interval), intervalParameter)));

          return criteriaQuery;
        }
      });

    this.findLoginAggregationByDateTimeIntervalGroupQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<LoginAggregationImpl>>() {
        @Override
        public CriteriaQuery<LoginAggregationImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<LoginAggregationImpl> criteriaQuery = cb
          ...
          criteriaQuery.where(cb.and(cb.equal(root
            ...
            .get(LoginAggregationImpl_.interval), intervalParameter)
          // //,
            // //cb.equal(root.get(LoginAggregationImpl_.aggregatedGroup),
    ...
    this.findLoginAggregationsByDateRangeQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<LoginAggregationImpl>>() {
        @Override
        public CriteriaQuery<LoginAggregationImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<LoginAggregationImpl> criteriaQuery = cb
          ...
          final Root<DateDimensionImpl> root = criteriaQuery
            .from(DateDimensionImpl.class);
          final CollectionJoin<DateDimensionImpl, LoginAggregationImpl> loginAggrJoin = root
            .join(DateDimensionImpl_.loginAggregations, JoinType.LEFT);

          criteriaQuery.select(loginAggrJoin);
          criteriaQuery.where(cb.and(cb.between(root.get(DateDimensionImpl_.date),
            startDate, endDate), cb.equal(loginAggrJoin
            .get(LoginAggregationImpl_.interval), intervalParameter)
          // //,
  ...
  @Override
  public List<LoginAggregation> getLoginAggregations(DateMidnight start,
    DateMidnight end, AggregationInterval interval,
    AggregatedGroupMapping... aggregatedGroupMapping) {
    final TypedQuery<LoginAggregationImpl> query = this
      .createQuery(findLoginAggregationsByDateRangeQuery);
    ...
    query.setParameter(this.aggregatedGroupsParameter, ImmutableSet
      .copyOf(aggregatedGroupMapping));

    return new ArrayList<LoginAggregation>(query.getResultList());
  }

  @Override
  public Set<LoginAggregationImpl> getLoginAggregationsForInterval(
    DateDimension dateDimension, TimeDimension timeDimension,
    AggregationInterval interval) {
    final TypedQuery<LoginAggregationImpl> query = this
      .createCachedQuery(this.findLoginAggregationByDateTimeIntervalQuery);
  ...
  @Override
  public LoginAggregationImpl getLoginAggregation(DateDimension dateDimension,
    TimeDimension timeDimension, AggregationInterval interval,
    AggregatedGroupMapping aggregatedGroup) {
    final TypedQuery<LoginAggregationImpl> query = this
      .createCachedQuery(this.findLoginAggregationByDateTimeIntervalGroupQuery);
    query.setParameter(this.dateDimensionParameter, dateDimension);
    query.setParameter(this.timeDimensionParameter, timeDimension);
    query.setParameter(this.intervalParameter, interval);
  ...
  @Override
  public LoginAggregationImpl createLoginAggregation(DateDimension dateDimension,
    TimeDimension timeDimension, AggregationInterval interval,
    AggregatedGroupMapping aggregatedGroup) {
    final LoginAggregationImpl loginAggregation = new LoginAggregationImpl(
