 ...
 * retrieved based on the information provided in security.properties, including
 * all relevant subcontexts. A typical sequence would be:
 * 
 * <pre>
...
public class InitialSecurityContextFactory {
  ...
  private static final Log log = LogFactory
    .getLog(InitialSecurityContextFactory.class);

  private static final String CONTEXT_PROPERTY_PREFIX = "securityContextProperty";

  /**
   * Used to store the configuration for each initial context, this allows a the
   * ISecurityContext chain to be created more quickly since the properties file
   * doesn't need to be parsed at each getInitialContext call.
   */
  ...
  public static ISecurityContext getInitialContext(final String rootContext)
    ...
    synchronized (contextConfigCache) {
      contextConfigBase = (BaseContextConfiguration) contextConfigCache
        .get(rootContext);
      ...
      if (contextConfigBase == null) {
        ...
        if (rootContext.indexOf('.') != -1) {
          PortalSecurityException ep = new PortalSecurityException(
            "Initial Context can't be compound");
    ...
    /*
     * Changing the synchronized object will minimize blocking in the case of
     * different root contexts. The config initialization code is thread safe as
     * long as each thread is initializing a different config.
     */

    /*
     * Synchronize on the contextConfig, this ensures two threads don't try to
     * intialize the config in parallel. Only one will be allowed into the
     * synchronized block at a time, if the config has not been initialized it
     * will initialize it and set the flag to true then exit the block. The
     * waiting thread(s) will have a reference to a now initialized config and
     * skip the initalization code.
     */
    synchronized (contextConfigBase) {
      if (!contextConfigBase.initialized) {
        ...
        try {
          securityPropertiesStream = InitialSecurityContextFactory.class
        ...
        contextConfigBase.rootConfig = loadContextConfigurationChain(rootContext,
          securityProperties);
  ...
  /**
   * Recursivly parses the tree of {@link ContextConfiguration} objects to
   * create a tree (chain) of {@link ISecurityContext}s. The root context is
   * returned by the method after all of it's sub-contexts have been created and
   * configured.
   * 
   * @param contextConfig
   *          The {@link ContextConfiguration} to use as the root
   * @return A configured {@link ISecurityContext}
   * @throws PortalSecurityException
   *           If an excetion is thrown by the
   *           {@link ISecurityContext#addSubContext(String, ISecurityContext)}
   *           method.
   */
  private static ISecurityContext createSecurityContextChain(
    ...
    final ISecurityContext securityContext = contextConfig.contextFactory
      .getSecurityContext();
    ...
    for (int index = 0; index < contextConfig.subConfigs.length; index++) {
      ...
      securityContext.addSubContext(contextConfig.subConfigs[index].contextName,
        subSecurityContext);
    }
   ...
   * named then the {@link Properties} are parsed to find all context
   * configuration properties and sub-contexts for this context. For each
   * sub-context this method is called recursivly.
   * 
   * @param fullContextName
   *          The fully qualified name of the context to configure.
   * @param securtiyProperties
   *          The {@link Properties} to use for configuration.
   * @return A fully configured {@link ContextConfiguration} object.
   * @throws PortalSecurityException
   *           If no context with the specified named exists or the factory
   *           cannot be created.
   */
  private static ContextConfiguration loadContextConfigurationChain(
    ...
    if (factoryName == null) {
      final PortalSecurityException ep = new PortalSecurityException(
        "No such security context " + fullContextName);
    ...
    if (lastDotIndex >= 0) {
      try {
        ...
        final PortalSecurityException pse = new PortalSecurityException(
    ...
    for (final Enumeration ctxnames = securtiyProperties.propertyNames(); ctxnames
      ...
      if (securityPropName.startsWith(fullContextName)
        ...
        final ContextConfiguration subContextConfig = loadContextConfigurationChain(
          securityPropName, securtiyProperties);
      ...
      // <PropertyName> cannot contain .
        ...
        final String propName = securityPropName
          .substring(contextConfigPropertyPrefix.length() + 1);
    ...
    contextConfig.subConfigs = (ContextConfiguration[]) subContexts
      .toArray(new ContextConfiguration[subContexts.size()]);
