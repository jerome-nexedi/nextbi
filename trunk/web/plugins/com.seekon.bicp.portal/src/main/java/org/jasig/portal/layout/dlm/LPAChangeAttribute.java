...
public class LPAChangeAttribute implements ILayoutProcessingAction {
  ...
  public void perform() throws PortalException {
    ...
    if (nodeId.startsWith(Constants.FRAGMENT_ID_USER_PREFIX)) {
      ...
      Element plfNode = HandlerUtils.getPLFNode(ilfNode, person, true, // create
        // node
        // if
        // not
        // found
      ...
      /*
       * add directive to hold override value. This is not necessary for folder
       * names since they are overridden at render time with their locale
       * specific version and persisted via a different mechanism.
       */
    ...
    /*
     * push the change into the ILF if not the name attribute. For names the
     * rendering will inject the localized name via a special processor. So it
     * doesn't matter what is in the ILF's folder name attribute.
     */
