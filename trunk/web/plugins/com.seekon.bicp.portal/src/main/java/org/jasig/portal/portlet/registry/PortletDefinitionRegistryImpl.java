...
/**
 * Implementation of the definition registry, pulls together the related parts
 * of the framework for creation and access of {@link IPortletDefinition}s.
 * 
...
public class PortletDefinitionRegistryImpl implements IPortletDefinitionRegistry,
  ...
  /**
   * @param portletDefinitionDao
   *          the portletDefinitionDao to set
   */
  @Autowired
   ...
   * (non-Javadoc)
   * 
   * @seeorg.jasig.portal.portlet.registry.IPortletDefinitionRegistry#
   * getPortletDefinition(org.jasig.portal.portlet.om.IPortletDefinitionId)
   */
  @Override
  ...
  public IPortletDefinition getPortletDefinition(String portletDefinitionIdString) {
    Validate.notNull(portletDefinitionIdString,
      "portletDefinitionId can not be null");
  ...
  @Override
  public IPortletDefinition createPortletDefinition(IPortletType portletType,
    String fname, String name, String title, String applicationId,
    String portletName, boolean isFramework) {
    return this.portletDefinitionDao.createPortletDefinition(portletType, fname,
   ...
   * 
   * @seeorg.jasig.portal.portlet.registry.IPortletDefinitionRegistry#
   * savePortletDefinition(org.jasig.portal.portlet.om.IPortletDefinition)
   */
  @Override
  public IPortletDefinition updatePortletDefinition(
  ...
  public PortletApplicationDefinition getParentPortletApplicationDescriptor(
    IPortletDefinitionId portletDefinitionId) {
    final IPortletDefinition portletDefinition = this
    ...
    final Tuple<String, String> portletDescriptorKeys = this
      .getPortletDescriptorKeys(portletDefinition);
    ...
    try {
      return portletRegistryService
        .getPortletApplication(portletDescriptorKeys.first);
   ...
   */
  @Override
  public PortletDefinition getParentPortletDescriptor(
    IPortletDefinitionId portletDefinitionId) {
    final IPortletDefinition portletDefinition = this
      .getPortletDefinition(portletDefinitionId);
    if (portletDefinition == null) {
    ...
    final PortletRegistryService portletRegistryService = this.portalDriverContainerServices
      .getPortletRegistryService();
    try {
      return portletRegistryService.getPortlet(portletDescriptorKeys.first,
        portletDescriptorKeys.second);
    } catch (PortletContainerException e) {
      if (this.logger.isDebugEnabled()) {
        ...
        this.logger
          .warn("No portlet descriptor could be found for the portlet definition, null will be returned: "
  ...
  /**
   * Get the portletApplicationId and portletName for the specified channel
   * definition id. The portletApplicationId will be {@link Tuple#first} and the
   * portletName will be {@link Tuple#second}
   */
  @Override
  public Tuple<String, String> getPortletDescriptorKeys(
    IPortletDefinition portletDefinition) {
    ...
    if (portletDefinition.getPortletDescriptorKey().isFrameworkPortlet()) {
      ...
      portletApplicationId = portletDefinition.getPortletDescriptorKey()
        .getWebAppName();
    }

    final String portletName = portletDefinition.getPortletDescriptorKey()
      .getPortletName();
