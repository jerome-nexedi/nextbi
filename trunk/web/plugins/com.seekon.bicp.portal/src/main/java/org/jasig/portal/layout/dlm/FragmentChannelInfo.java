...
public class FragmentChannelInfo extends FragmentNodeInfo {
  ...
  /**
   * Returns true if a parameter can be modified for a channel. This is based on
   * a single dlm:editAllowed attribute for each channel parameter in fragments.
   * If not included then edits to that channel parameter are allowed. If
   * included with a value other than true then edits to that parameter for that
  ...
  public boolean canOverrideParameter(String name) {
    ...
    for (int i = 0; i < parms.getLength(); i++) {
      ...
      if (parm.getTagName().equals(Constants.ELM_PARAMETER)) {
        String parmName = parm.getAttribute(Constants.ATT_NAME);
        if (parmName.equals(name)) {
          Attr editAllowed = parm.getAttributeNodeNS(Constants.NS_URI,
