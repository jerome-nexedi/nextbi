...
public interface IPortalDataHandlerService {
  ...
  public class BatchOptions {
    ...
    /**
     * @return The directory to log the export to, if null a temp directory is
     *         used.
     */
   ...
   * 
   * @param directory
   *          Base directory to import from
   * @param pattern
   *          Optional ant path matcher pattern used for matching files to
   *          import. If not specified the default pattern set is used
   * @param options
   *          Optional set of options to better control the import
   */
  public void importData(File directory, String pattern, BatchImportOptions options);

  /**
   * Import data from the specified resource, uses a {@link ResourceLoader} find
   * the data file
   */
  ...
  /**
   * @return All portal data for a specific portal data type, some types may
   *         return an empty set even if they contain data due size constraints.
   */
  public Iterable<? extends IPortalData> getPortalData(String typeId);

  /**
   * Export the portal data for the specified type and id writing it to the
   * provided XML Transformer Result
   * 
   * @param typeId
   *          Type of the portal data to export
   * @param dataId
   *          Id of the data to export
   * @param result
   *          XML Result to write the exported data to
   * @return The recommended file name to use for the result
   */
  public String exportData(String typeId, String dataId, Result result);

  /**
   * Export the portal data for the specified type and save it to the specified
   * directory
   * 
   * @param typeId
   *          Type of the portal data to export
   ...
   *          Id of the data to export
   * @param directory
   *          Directory to save exported data to
   * @return True if the specified data was found, false if no data exists for
   *         the type and data ids
   */
  public boolean exportData(String typeId, String dataId, File directory);

  /**
   * Export all the portal data for each type and save it to the specified
   * directory
   * 
   * @param typeIds
   *          TypeIds from {@link #getExportPortalDataTypes()} to export all
   *          data for
   * @param directory
   *          Directory to save exported data to
   * @param options
   *          Optional set of options to better control the export
   */
  public void exportAllDataOfType(Set<String> typeIds, File directory,
    BatchExportOptions options);

  /**
   * Export all portal data for all data type and save it to the specified
   * directory
   * 
   * @param directory
   *          Directory to save exported data to
   ...
   * 
   * @param typeId
   *          id of the portal type to delete data from
   * @param dataId
