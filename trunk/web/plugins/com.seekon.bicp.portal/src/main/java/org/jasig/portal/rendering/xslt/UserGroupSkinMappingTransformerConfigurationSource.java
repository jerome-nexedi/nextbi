 ...
 * @version $Revision$
 */
public class UserGroupSkinMappingTransformerConfigurationSource extends
  ...
  protected String getSkinName(HttpServletRequest request) {
    ...
    final Map<IGroupMember, String> groupMemberToSkinMapping = groupMemberToSkinMappingCreator
      .get();
    for (final Entry<IGroupMember, String> groupToSkinEntry : groupMemberToSkinMapping
      .entrySet()) {
  ...
  private final class GroupMemberMappingCreator extends
    SingletonDoubleCheckedCreator<Map<IGroupMember, String>> {
    @Override
    protected Map<IGroupMember, String> createSingleton(Object... args) {
      final Map<IGroupMember, String> groupMemberToSkinMapping = new LinkedHashMap<IGroupMember, String>(
        groupToSkinMap.size());
    ...
    protected IGroupMember findGroup(String group) {
      ...
      final EntityIdentifier[] results = GroupService.searchForGroups(group,
        GroupService.IS, IPerson.class);
      if (results == null || results.length == 0) {
        this.logger.warn("Configured group '" + group
      ...
      if (results.length > 1) {
        this.logger.warn(results.length
          + " groups were found for skin mapping group '" + group
          + "'. The first result will be used. " + Arrays.toString(results));
      }
