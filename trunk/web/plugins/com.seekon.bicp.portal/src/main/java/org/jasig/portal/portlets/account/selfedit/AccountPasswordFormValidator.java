  ...
  public void validateEnterPassword(AccountPasswordForm form, MessageContext context) {
    ...
    if (StringUtils.isBlank(form.getCurrentPassword())) {
      context.addMessage(new MessageBuilder().error().source("currentPassword")
        .code("please.enter.current.password").defaultText(
          "Please enter your current password").build());
    }
    ...
    // password
      ...
      if (!passwordService.validatePassword(form.getCurrentPassword(), account
        .getPassword())) {
        context
          .addMessage(new MessageBuilder().error().source("currentPassword").code(
            "current.password.doesnt.match").defaultText(
            "Provided password does not match the current account password").build());
      }
    ...
    if (StringUtils.isBlank(form.getNewPassword())) {
      context.addMessage(new MessageBuilder().error().source("newPassword").code(
        "please.enter.new.password").defaultText("Please enter a new password")
        .build());
    }
    ...
    if (StringUtils.isBlank(form.getConfirmNewPassword())) {
      context.addMessage(new MessageBuilder().error().source("confirmNewPassword")
        .code("please.enter.confirm.password").defaultText(
          "Please confirm your new password").build());
    }
    ...
    if (StringUtils.isNotBlank(form.getNewPassword())
      ...
      context.addMessage(new MessageBuilder().error().source("confirmPassword")
        .code("passwords.must.match").defaultText("Passwords must match").build());
    }
