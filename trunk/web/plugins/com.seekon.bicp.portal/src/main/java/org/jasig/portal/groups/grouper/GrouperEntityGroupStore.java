 ...
 * @version $Revision$
 */
public class GrouperEntityGroupStore implements IEntityGroupStore, IEntityStore,
  IEntitySearcher {
  ...
  public boolean contains(IEntityGroup group, IGroupMember member)
    ...
    gcHasMember
      .addSubjectLookup(new WsSubjectLookup(null, "g:gsa", groupMemberName));
    ...
    if (GrouperClientUtils.length(wsHasMemberResults.getResults()) == 1) {
      ...
      return StringUtils.equals("IS_MEMBER", wsHasMemberResult.getResultMetadata()
        .getResultCode());
    }
    return false;
  }
  ...
  public IEntityGroup find(String key) throws GroupsException {
    ...
    try {
      ...
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("Retrieved group from the Grouper server matching key " + key
          + ": " + group.toString());
      }
      ...
      LOGGER.warn("Exception while attempting to retrieve " + "group with key "
        + key + " from Grouper web services: " + e.getMessage());
      return null;
    }
  ...
  public Iterator findContainingGroups(IGroupMember gm) throws GroupsException {
    ...
    try {
      ...
      LOGGER.warn("Exception while attempting to retrieve "
        + "parents for entity with key " + key + " from Grouper web services: "
        + e.getMessage());
    ...
    try {
      ...
      for (WsSubject gInfo : gInfos) {
        ...
        if (!StringUtils.equals(gInfo.getSourceId(), "g:gsa")) {
          ...
          if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("creating leaf member:" + gInfo.getId() + " and name: "
              + gInfo.getName() + " from group: " + group.getLocalKey());
          }
    ...
    try {
      ...
      if (results != null && results.getWsSubjects() != null) {
        ...
        for (WsSubject wsSubject : results.getWsSubjects()) {
          entityIdentifiers.add(new EntityIdentifier(wsSubject.getName(),
