...
class PermissionActivityImpl implements IPermissionActivity, Serializable {
  ...
  public String toString() {
    return "PermissionActivityImpl [id=" + this.id + ", entityVersion="
      + this.entityVersion + ", fname=" + this.fname + ", name=" + this.name
      + ", description=" + this.description + ", targetProviderKey="
      + this.targetProviderKey + "]";
  }

  @Override
  public int compareTo(IPermissionActivity activity) {
    return new CompareToBuilder().append(this.name, activity.getName()).append(
      this.targetProviderKey, activity.getTargetProviderKey()).append(this.fname,
      activity.getFname()).toComparison();
  }
}
