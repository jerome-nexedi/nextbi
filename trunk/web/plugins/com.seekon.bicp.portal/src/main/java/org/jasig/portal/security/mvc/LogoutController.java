...
/**
 * Simple servlet to handle user logout. When a user logs out, their session
 * gets invalidated and they are returned to the guest page.
 * 
...
public class LogoutController implements InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    try {
      ...
      String key;
      final Properties props = ResourceLoader.getResourceAsProperties(
  ...
  public void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (session != null) {
      ...
      try {
        ...
        log.error("Exception recording logout " + "associated with request "
          + request, e);
      }

      final String originalUid = (String) session
        .getAttribute(LoginController.SWAP_ORIGINAL_UID);
      ...
      if (originalUid != null) {
        ...
        try {
          ...
          // This is fine. LogoutController is looking to guarantee the logged
          // out session is invalid;
  ...
  /**
   * The redirect is determined based upon the context that passed
   * authentication The LogoutController looks at each authenticated context and
   * determines if a redirect exists for that context in the redirectMap
   * variable (loaded from security.properties file). The redirect is returned
   * for the first authenticated context that has an associated redirect string.
   * If such a context is not found, we use the default DEFAULT_REDIRECT that
   * was originally setup.
   * 
   * NOTE: This will work or not work based upon the logic in the root context.
   * At this time, all known security contexts extend the
   * ChainingSecurityContext class. If a context has the variable
   * stopWhenAuthenticated set to false, the user may be logged into multiple
   * security contexts. If this is the case, the logout process currently
   * implemented does not accommodate multiple logouts. As a reference
   * implemention, the current implementation assumes only one security context
   * has been authenticated. Modifications to perform multiple logouts should be
   * considered when a concrete need arises and can be handled by this class or
   * through a change in the ISecurityConext API where a context knows how to
   * perform it's own logout.
   * 
   * @param request
  ...
  private String getRedirectionUrl(HttpServletRequest request) {
    ...
    try {
      ...
      if (person != null) {
        ...
        while (subCtxNames.hasMoreElements()) {
          ...
          if (log.isDebugEnabled()) {
            log.debug("LogoutController::getRedirectionUrl() " + " subCtxName = "
              + subCtxName);
          }
    ...
    if (log.isDebugEnabled()) {
      log.debug("LogoutController::getRedirectionUrl()" + " redirectionURL = "
        + redirect);
    }
    return redirect;
  }
