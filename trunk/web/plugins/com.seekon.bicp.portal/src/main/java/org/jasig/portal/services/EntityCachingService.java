 ...
 * that lets clients cache and retrieve <code>IBasicEntities</code>. It hides
 * such details as the physical structure of the cache and whether it is running
 * in a multi- or single-JVM environment.
 * <p>
 * An <code>IBasicEntity</code> can answer its type and key. (See
 * org.jasig.portal.groups.EntityTypes).
 * <p>
 * Caching consists of asking the service to add, retrieve, update and remove
 * elements from the cache, e.g.,
 * <p>
 * <code>
 ...
 * @version $Revision$
 * @deprecated Where possible the Spring managed {@link IEntityCachingService}
 *             bean should be injected instead of using this lookup class.
 */
@Deprecated
public class EntityCachingService implements IEntityCachingService {
  ...
  public IBasicEntity get(EntityIdentifier entityID) throws CachingException {
    return EntityCachingServiceLocator.getEntityCachingService().get(
      entityID.getType(), entityID.getKey());
  }
   ...
   * @exception org.jasig.portal.concurrency.CachingException
   */
  public void remove(Class<? extends IBasicEntity> type, String key)
    throws CachingException {
