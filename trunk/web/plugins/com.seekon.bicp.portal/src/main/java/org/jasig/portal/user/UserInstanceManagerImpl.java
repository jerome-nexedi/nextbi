...
public class UserInstanceManagerImpl implements IUserInstanceManager {
  private static final String KEY = UserInstanceManagerImpl.class.getName()
    + ".USER_INSTANCE";
  ...
  public IUserInstance getUserInstance(HttpServletRequest request)
    ...
    final IUserProfile userProfile = this.getUserProfile(request, person,
      localeManager, userAgent);

    // Create the user layout manager and user instance object
    IUserLayoutManager userLayoutManager = userLayoutManagerFactory
      .getUserLayoutManager(person, userProfile);
    if (person.isGuest()) {
      userLayoutManager = userLayoutManagerFactory
        .immutableUserLayoutManager(userLayoutManager);
    }

    final UserPreferencesManager userPreferencesManager = new UserPreferencesManager(
      person, userProfile, userLayoutManager);
  ...
  protected IUserProfile getUserProfile(HttpServletRequest request, IPerson person,
    LocaleManager localeManager, String userAgent) {
    final String profileFname = profileMapper.getProfileFname(person, request);
    IUserProfile userProfile = userLayoutStore.getUserProfileByFname(person,
      profileFname);
  ...
  /**
   * <p>
   * Serializable wrapper class so the UserInstance object can be indirectly
   * stored in the session. The manager can deal with this class returning a
   * null value and its field is transient so the session can be serialized
   * successfully with the UserInstance object in it.
   * </p>
   * <p>
   * Implements HttpSessionBindingListener and delegates those methods to the
   * wrapped UserInstance, if present.
   * </p>
   */
