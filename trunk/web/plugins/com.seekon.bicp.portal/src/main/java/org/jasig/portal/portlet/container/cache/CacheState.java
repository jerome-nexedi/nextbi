...
public class CacheState<D extends CachedPortletResultHolder<T>, T extends Serializable> {
  ...
  /**
   * @return true If the cached portlet data should be replayed. If true
   *         {@link #getCachedPortletData()} will not be false
   */
  ...
  /**
   * @return true If the browser's If-None-Match or If-Modified-Since header
   *         values mean the browser's cache matches the portal's cache
   */
  ...
  /**
   * @return true If the browser set an ETag. If true
   *         {@link CacheControl#getETag()} will return the browser set ETag
   */
  ...
  /**
   * @return The private cahe key for the portlet, may be null if publicly
   *         scoped cache data was found
   */
  ...
  protected void setPrivatePortletCacheKey(
    PrivatePortletCacheKey privatePortletCacheKey) {
