...
public class AdminEvaluator {
  ...
  /**
   * Determines if the passed-in IPerson represents a user that is a member of
   * the administrator group or any of its sub groups.
   * 
   * @param p
   * @return
   */
  public static boolean isAdmin(IPerson p) {
    IAuthorizationPrincipal iap = AuthorizationService.instance().newPrincipal(
      p.getEntityIdentifier().getKey(), p.getEntityIdentifier().getType());

    return isAdmin(iap);
  }

  /**
   * Determines if the passed-in authorization principal represents a user that
   * is a member of the administrator group or any of its sub groups.
   * 
   * @param p
   * @return
   */
  ...
  /**
   * Determines if the passed-in group member represents a user that is a member
