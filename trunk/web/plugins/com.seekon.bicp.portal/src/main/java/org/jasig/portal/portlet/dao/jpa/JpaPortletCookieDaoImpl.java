...
public class JpaPortletCookieDaoImpl extends BaseJpaDao implements IPortletCookieDao {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    this.deletePortalCookieQueryString = "DELETE FROM "
      + PortalCookieImpl.class.getName() + " e " + "WHERE e."
      + PortalCookieImpl_.expires.getName() + " <= :" + this.nowParameter.getName();

    this.deletePortletCookieQueryString = "DELETE FROM "
      + PortletCookieImpl.class.getName() + " e " + "WHERE e."
      + PortletCookieImpl_.expires.getName() + " <= :" + this.nowParameter.getName();

    this.findPortalCookieByValueQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<PortalCookieImpl>>() {
        @Override
        public CriteriaQuery<PortalCookieImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<PortalCookieImpl> criteriaQuery = cb
            .createQuery(PortalCookieImpl.class);
          final Root<PortalCookieImpl> typeRoot = criteriaQuery
            .from(PortalCookieImpl.class);
          criteriaQuery.select(typeRoot);
          typeRoot.fetch(PortalCookieImpl_.portletCookies, JoinType.LEFT);
          criteriaQuery.where(cb.equal(typeRoot.get(PortalCookieImpl_.value),
            valueParameter));
    ...
    this.findExpiredByParentPortletCookiesQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<PortletCookieImpl>>() {
        ...
        public CriteriaQuery<PortletCookieImpl> apply(CriteriaBuilder cb) {
          ...
          final Root<PortletCookieImpl> typeRoot = criteriaQuery
            .from(PortletCookieImpl.class);
          criteriaQuery.select(typeRoot);
          criteriaQuery.where(cb.lessThanOrEqualTo(typeRoot.get(
            PortletCookieImpl_.portalCookie).get(PortalCookieImpl_.expires),
            nowParameter));
   ...
   * 
   * @see
   * org.jasig.portal.portlet.cookies.IPortalCookieDao#createPortalCookie(javax
   * .servlet.http.HttpServletRequest)
   */
  @Override
  @Transactional
  public IPortalCookie createPortalCookie(int maxAge) {
    // Make sure our unique ID doesn't already exist by really small random
    // chance
    String uniqueId;
    do {
   ...
   * 
   * @see
   * org.jasig.portal.portlet.cookies.IPortalCookieDao#getPortalCookie(java.
   * lang.String)
   */
  @Override
  public IPortalCookie getPortalCookie(String portalCookieValue) {
    final TypedQuery<PortalCookieImpl> query = this
      .createCachedQuery(this.findPortalCookieByValueQuery);
  ...
  @Override
  @Transactional
  public IPortalCookie updatePortalCookieExpiration(IPortalCookie portalCookie,
    int maxAge) {
  ...
  public void purgeExpiredCookies() {
    ...
    final Query deletePortletCookieQuery = this.entityManager
      .createQuery(this.deletePortletCookieQueryString);
    ...
    if (log.isDebugEnabled()) {
      log.debug("finished purging " + deletedPortletCookies
        + " directly expired portlet cookies");
    }
    ...
    final List<PortletCookieImpl> indirectlyExpiredCookies = expiredByParentCookiesQuery
      .getResultList();
    ...
    if (log.isDebugEnabled()) {
      log.debug("finished purging " + indirectlyExpiredCookies.size()
        + " indirectly expired portlet cookies");
    }

    log.debug("begin portal cookie expiration");

    final Query deletePortalCookieQuery = this.entityManager
      .createQuery(this.deletePortalCookieQueryString);
    ...
    if (log.isDebugEnabled()) {
      log.debug("finished purging " + deletedPortalCookies
        + " portal cookies, begin portlet cookie expiration");
    }
  }
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletCookieDao#updatePortletCookie(org.
   * jasig.portal.portlet.om.IPortalCookie, javax.servlet.http.Cookie)
   */
  @Override
  @Transactional
  public IPortalCookie addOrUpdatePortletCookie(IPortalCookie portalCookie,
    Cookie cookie) {
    final Set<IPortletCookie> portletCookies = portalCookie.getPortletCookies();

    boolean found = false;
    final String name = cookie.getName();
    for (final Iterator<IPortletCookie> portletCookieItr = portletCookies.iterator(); portletCookieItr
      .hasNext();) {
