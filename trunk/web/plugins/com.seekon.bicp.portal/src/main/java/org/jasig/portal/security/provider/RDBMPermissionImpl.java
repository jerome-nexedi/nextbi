...
/**
 * Reference implementation of IPermissionStore. Performs CRUD operations on the
 * UP_Permission table.
 * 
...
public class RDBMPermissionImpl implements IPermissionStore {
  ...
  /**
   * Signifies that the principal is an {@see IEntityGroup}. The {@see
   * IPermission} contract does not define a method like
   * <code>setPrincipalType()</code>, but {@see RDBMPermissionImpl} (this class)
   * <i>does</i> have a notion of principal type. Specify
  ...
  /**
   * Signifies that the principal is an {@see IPerson}. The {@see IPermission}
   * contract does not define a method like <code>setPrincipalType()</code>, but
   * {@see RDBMPermissionImpl} (this class) <i>does</i> have a notion of
   * principal type. Specify
   * <code>(PRINCIPAL_TYPE_GROUP|PRINCIPAL_TYPE_USER)PRINCIPAL_SEPARATOR&lt;principal.key&gt;</code>
   * to {@see IPermission.setPrincipal} for use with {@see RDBMPermissionImpl}.
   */
  ...
  public void delete(IPermission[] perms) throws AuthorizationException {
    if (perms.length > 0) {
      try {
        ...
        throw new AuthorizationException("Exception deleting permissions "
          + Arrays.toString(perms), ex);
      }
    }
  }
   ...
   */
  public boolean existsInDatabase(IPermission perm) throws AuthorizationException,
    SQLException {
    Connection conn = null;
    try {
      conn = RDBMServices.getConnection();
      ...
      log.error("Exception determining whether " + "permission [" + perm
        + "] exists in database.", ex);
      throw new AuthorizationException("RDBMPermissionImpl.existsInDatabase(): "
        + ex);
    } finally {
      RDBMServices.releaseConnection(conn);
    }
  ...
  private String getPrincipalKey(String principalString) {
    return principalString
      .substring(principalString.indexOf(PRINCIPAL_SEPARATOR) + 1);
  }
  ...
  private int getPrincipalType(String principalString) {
    return Integer.parseInt(principalString.substring(0, principalString
      .indexOf(PRINCIPAL_SEPARATOR)));
  }
  ...
  private IPermission instanceFromResultSet(ResultSet rs) throws SQLException {
    ...
    perm.setPrincipal(rs.getString(PRINCIPAL_TYPE_COLUMN) + "."
      + rs.getString(PRINCIPAL_KEY_COLUMN));
   ...
   *          org.jasig.portal.security.IPermission
   * @param ps
   *          java.sql.PreparedStatement - the PreparedStatement for inserting a
   *          Permission row.
   * @exception Exception
   */
   ...
   *          org.jasig.portal.security.IPermission
   * @param ps
   *          java.sql.PreparedStatement - the PreparedStatement for deleting a
   *          Permission row.
   ...
   *          org.jasig.portal.security.IPermission
   * @param ps
   *          java.sql.PreparedStatement - the PreparedStatement for updating a
   *          Permission row.
   * @return int - the return code from the PreparedStatement
   * @exception Exception
   */
  ...
  private void prepareSelectQuery(PreparedStatement stmt, String owner,
    String principal, String activity, String target, String type)
    throws SQLException {
    int i = 1;
  ...
  private String getSelectQuery(String owner, String principal, String activity,
    ...
    if (log.isTraceEnabled()) {
      log.trace("Computed SQL query [" + sqlQuery + "] for owner=[" + owner
        + "] and principal=[" + principal + "] and activity=[" + activity
        + "] and target=[" + target + "] and type=[" + type + "]");
    }
   ...
   * 
   * @param owner
   *          String - the Permission owner
   * @param principal
   *          String - the Permission principal
   * @param activity
   *          String - the Permission activity
   * @exception org.jasig.portal.AuthorizationException
   *              - wraps an Exception specific to the store.
   */
  public IPermission[] select(String owner, String principal, String activity,
    String target, String type) throws AuthorizationException {
    Connection conn = null;
    ...
    try {
      ...
      throw new AuthorizationException("Problem retrieving Permissions ["
        + sqle.getMessage() + "] for query=[" + query + "] for owner=[" + owner
        + "] and principal=[" + principal + "] and activity=[" + activity
    ...
    if (log.isTraceEnabled()) {
      log.trace("RDBMPermissionImpl.select(): [" + query + "] for owner=[" + owner
        + "] and principal=[" + principal + "] and activity=[" + activity
        + "] and target=[" + target + "] and type=[" + type
        + "] returned permissions [" + perms + "]");
    }
