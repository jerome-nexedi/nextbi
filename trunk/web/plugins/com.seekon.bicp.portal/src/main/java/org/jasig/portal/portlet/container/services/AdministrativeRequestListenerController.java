...
/**
 * AdministrativeRequestListener that delegates to another
 * AdministrativeRequestListener based on a request attribute.
 * 
 * <br>
 * Configuration:
 * <table border="1">
 ...
 * <td>
 * The PortletRequest attribute to get the key for the
 * {@link AdministrativeRequestListener} from.</td>
 * <td valign="top">No</td>
 * <td valign="top">
 * org.jasig.portal.portlet.common.AdministrativeRequestListenerControler
 * .LISTENER_KEY</td>
 * </tr>
 * <tr>
 * <td align="right" valign="top">listeners</td>
 * <td>
 * The {@link Map} of {@link String} keys to
 * {@link AdministrativeRequestListener}s.</td>
 * <td valign="top">No</td>
 ...
 * <td>
 * If an exception should be thrown if a <code>listenerKeyAttribute</code>
 * request attribute is not found. If false a warning will be logged and the
 * method will return.</td>
 * <td valign="top">No</td>
 ...
 * <td>
 * If an exception should be thrown if a {@link AdministrativeRequestListener}
 * isn't found for the request attribute key. If false a warning will be logged
 * and the method will return.</td>
 * <td valign="top">No</td>
 * <td valign="top">false</td>
 * </tr>
 * </table>
 ...
 * @version $Revision$
 */
public class AdministrativeRequestListenerController implements
  org.apache.pluto.container.driver.AdministrativeRequestListener {
  public static final String DEFAULT_LISTENER_KEY_ATTRIBUTE = AdministrativeRequestListenerController.class
    .getName()
    + ".LISTENER_KEY";
  ...
  /**
   * @param failOnKeyNotFound
   *          the failOnKeyNotFound to set
   */
  ...
  /**
   * @param failOnListenerNotFound
   *          the failOnListenerNotFound to set
   */
  ...
  /**
   * @param listenerKeyAttribute
   *          the listenerKeyAttribute to set
   */
  ...
  /**
   * @see org.apache.pluto.spi.optional.AdministrativeRequestListener#administer(javax.portlet.PortletRequest,
   *      javax.portlet.PortletResponse)
   */
  public void administer(PortletRequest request, PortletResponse response) {
    final String listenerKey = (String) request
      .getAttribute(this.listenerKeyAttribute);
    if (this.logger.isDebugEnabled()) {
      this.logger.debug("Found key '" + listenerKey
        + "' from PortletRequest attribute '" + this.listenerKeyAttribute + "'");
    }

    if (listenerKey == null) {
      if (this.failOnKeyNotFound) {
        this.logger.error("Failed to find PortletRequest attribute '"
      ...
      this.logger.warn("Failed to find PortletRequest attribute '"
        + this.listenerKeyAttribute + "'");
      return;
    }

    final AdministrativeRequestListener administrativeRequestListener = this.listeners
      .get(listenerKey);
    if (this.logger.isDebugEnabled()) {
      this.logger.debug("Found AdministrativeRequestListener '"
        + administrativeRequestListener + "' for key '" + listenerKey + "'");
    }

    if (administrativeRequestListener == null) {
      if (this.failOnListenerNotFound) {
        this.logger.error("Failed to find AdministrativeRequestListener for key '"
          + listenerKey + "'");
        throw new IllegalArgumentException(
      ...
      this.logger.warn("Failed to find AdministrativeRequestListener for key '"
        + listenerKey + "'");
      return;
    }
