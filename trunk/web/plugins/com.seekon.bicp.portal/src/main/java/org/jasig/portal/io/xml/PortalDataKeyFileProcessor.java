...
public final class PortalDataKeyFileProcessor implements Function<Resource, Object> {
  ...
  PortalDataKeyFileProcessor(Map<PortalDataKey, IPortalDataType> dataKeyTypes,
    ...
    // Set the input buffer to 2k bytes. This appears to work for reading just
    // enough to get the start element event for
    // all of the data files in a single read operation.
    this.xmlInputFactory
      .setProperty(WstxInputProperties.P_INPUT_BUFFER_LENGTH, 2000);
    this.xmlInputFactory.setProperty(XMLInputFactory2.P_LAZY_PARSING, true); // Do
    // as
    // little
    // parsing
    // as
    ...
    // basic
    // info
    this.xmlInputFactory.setProperty(XMLInputFactory.IS_VALIDATING, false); // Don't
    // do
    // any
    // validation
    // here
    this.xmlInputFactory.setProperty(XMLInputFactory.SUPPORT_DTD, false); // Don't
    // load
    // referenced
    // DTDs
    // here
  ...
  public Object apply(Resource input) {
    ...
    try {
      ...
      logger.warn("Failed to create InputStream, resource will be ignored: {}",
        input);
      return null;
    }
    ...
    try {
      xmlEventReader = new BufferedXMLEventReader(this.xmlInputFactory
        .createXMLEventReader(fis), -1);
    ...
    // Allow the PortalDataType to do any necessary post-processing of the
    // input, needed as some types require extra work
    final String resourceUri = ResourceUtils.getResourceUri(input);
    final Set<PortalDataKey> processedPortalDataKeys = portalDataType
      .postProcessPortalDataKey(resourceUri, portalDataKey, xmlEventReader);
    xmlEventReader.reset();
    ...
    for (final PortalDataKey processedPortalDataKey : processedPortalDataKeys) {
      ...
      if (queue == null) {
        queue = ConcurrentMapUtils.putIfAbsent(this.dataToImport,
          processedPortalDataKey, new ConcurrentLinkedQueue<Resource>());
      }
      queue.offer(input);
