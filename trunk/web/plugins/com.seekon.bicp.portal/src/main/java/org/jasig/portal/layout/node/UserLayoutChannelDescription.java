...
public class UserLayoutChannelDescription extends UserLayoutNodeDescription
  ...
  private static final Log log = LogFactory
    .getLog(UserLayoutChannelDescription.class);
  ...
  public UserLayoutChannelDescription(IPortletDefinition definition) {
    ...
    this.channelPublishId = String.valueOf(definition.getPortletDefinitionId()
      .getStringId());
   ...
   * 
   * @param xmlNode
   *          a user layout channel <code>Element</code> value
   ...
   * 
   * @param v
   *          Value to assign to functionalName.
   */
   ...
   * 
   * @param v
   *          Value to assign to channelSubscribeId.
   */
   ...
   * 
   * @param v
   *          Value to assign to channelTypeId.
   */
   ...
   * 
   * @param v
   *          Value to assign to channelPublishId.
   */
  ...
  /**
   * Reset a channel parameter value. Since parameter changes by channels can be
   * persisted if override is allowed this method enables resetting to the
   * original value or, if the parameter is ad-hoc meaning that the channel
   * definition does not provide a value for this parameter, then the parameter
   * value is removed.
   * 
   * @param parameterName
   *          a <code>String</code> value
  ...
  public void resetParameter(String parameterName) throws PortalException {
    /*
     * Since original channel definition parameter values that were overridden
     * are not maintained at this level we must consult the channel definition
     * to determine the original value and restore it or discover that this is
     * an ad-hoc parameter and can be removed.
     */
    try {
      ...
      throw new PortalException("Unable to reset parameter [" + parameterName
        + "] for channel [" + getTitle() + "].", e);
    }
  }
   ...
   * 
   * @param parameterName
   *          a <code>String</code> parameter name.
  ...
  /**
   * Creates a <code>org.w3c.dom.Element</code> representation of the current
   * channel.
   * 
   * @param root
   *          a <code>Document</code> for which the <code>Element</code> should
   *          be created.
