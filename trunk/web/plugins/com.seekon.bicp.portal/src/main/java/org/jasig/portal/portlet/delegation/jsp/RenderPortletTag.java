...
public class RenderPortletTag extends TagSupport {
  ...
  public int doEndTag() throws JspException {
    ...
    final RenderRequest renderRequest = (RenderRequest) this.pageContext
      .getAttribute("renderRequest");
    final RenderResponse renderResponse = (RenderResponse) this.pageContext
      .getAttribute("renderResponse");

    final PortletDelegationLocator portletDelegationLocator = (PortletDelegationLocator) renderRequest
      .getAttribute(PortletDelegationLocator.PORTLET_DELECATION_LOCATOR_ATTR);

    final String sessionKey = this.sessionKeyPrefix + this.fname;

    final PortletSession portletSession = renderRequest.getPortletSession();
    IPortletWindowId portletWindowId = (IPortletWindowId) portletSession
    ...
    if (portletWindowId == null) {
      portletDelegationDispatcher = portletDelegationLocator
        .createRequestDispatcher(renderRequest, this.fname);
    ...
    // id in session, get the old dispatcher
      ...
      portletDelegationDispatcher = portletDelegationLocator.getRequestDispatcher(
        renderRequest, portletWindowId);
      delegateState = null;
    }
    ...
    try {
      portletDelegationDispatcher.doRender(renderRequest, renderResponse,
        delegationRequest, new JspWriterPortletOutputHandler(out));
    } catch (IOException e) {
      throw new JspException("Failed to execute delegate render on portlet '"
        + this.fname + "'", e);
    }
