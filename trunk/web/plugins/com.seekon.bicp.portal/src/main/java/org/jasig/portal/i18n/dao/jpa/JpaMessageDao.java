    ...
    this.findMessageByCodeAndLocaleQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<MessageImpl>>() {
        ...
        public CriteriaQuery<MessageImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<MessageImpl> criteriaQuery = cb
            .createQuery(MessageImpl.class);
          final Root<MessageImpl> root = criteriaQuery.from(MessageImpl.class);
          criteriaQuery.select(root);
          criteriaQuery.where(cb
            .and(cb.equal(root.get(MessageImpl_.code), codeParameter), cb.equal(root
              .get(MessageImpl_.locale), localeParameter)));
          criteriaQuery.orderBy(cb.asc(root.get(MessageImpl_.code)), cb.asc(root
            .get(MessageImpl_.locale)));
    ...
    this.findMessageByLocaleQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<MessageImpl>>() {
        ...
        public CriteriaQuery<MessageImpl> apply(CriteriaBuilder cb) {
          ...
          final Root<MessageImpl> root = criteriaQuery.from(MessageImpl.class);
          criteriaQuery.select(root);
          criteriaQuery.where(cb.equal(root.get(MessageImpl_.locale),
            localeParameter));
