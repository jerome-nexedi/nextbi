...
/**
 * <p>
 * This is the basic abstract class for all security contexts that should chain
 * to children security contexts.
 * </p>
 * 
 * @author Andrew Newman, newman@yale.edu
 * @version $Revision$
 * @author Don Fracapane (df7@columbia.edu) Added a new method named
 *         getSubContextNames() that returns an Enumeration of names for the
 *         subcontexts.
 */
public abstract class ChainingSecurityContext implements ISecurityContext {
  ...
  /**
   * Default value for stopWhenAuthenticated. This value will be used when the
   * corresponding property cannot be loaded.
   */
  private static final boolean DEFAULT_STOP_WHEN_AUTHENTICATED = true;

  protected static boolean stopWhenAuthenticated = PropertiesManager
   ...
   * from the parent. Note that the contexts themselves should resist actually
   * performing the assignment if an assignment has already been made to either
   * the credentials or the UID.
   */
  ...
  public synchronized void authenticate() throws PortalSecurityException {
    ...
    while (e.hasMoreElements()) {
      ...
      // The principal and credential are now set for all subcontexts in
      // Authentication
      try {
      ...
      // Stop attempting to authenticate if authenticated and if the property
      // flag is set
    ...
    if (error && !this.isauth)
      throw new PortalSecurityException(
        "One of the security subcontexts threw an exception");
    return;
  }
  ...
  public synchronized ISecurityContext getSubContext(String name) {
    ...
    PortalSecurityException ep = new PortalSecurityException("No such subcontext: "
      + name);
  ...
  public synchronized void addSubContext(String name, ISecurityContext ctx)
    ...
    if (doesSubContextExist(name)) {
      PortalSecurityException ep = new PortalSecurityException(
        "Subcontext already exists: " + name);
  ...
  protected class ChainingPrincipal implements IPrincipal {
    ...
    /**
     * Original, no-arg constructor used by the
     * <code>ChainingSecurityContext</code>.
     * 
     */
    ...
    /**
     * Creates a new <code>ChainingPrincipal</code> from the specified
     * <code>IPrincipal</code>.
     * 
     */
  ...
  /**
   * A default, placeholder implementation of IAdditionalDescriptor an instance
   * of which is the default value for the instance variable
   * "myAdditionalDescriptor" of instances of this class.
   */
