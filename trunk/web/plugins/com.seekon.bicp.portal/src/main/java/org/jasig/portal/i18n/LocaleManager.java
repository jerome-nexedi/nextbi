...
/**
 * Manages locales on behalf of a user. This class currently keeps track of
 * locales at the following levels:<br>
 * <ol>
 ...
 * <li>Session's locale preferences (set via the portal request parameter
 * uP_locales)</li>
 * <li>Portal's locale preferences (set in portal.properties)</li>
 * </ol>
 * Eventually, this class will also keep track of locale preferences at the
 * following levels:<br>
 * <ol>
...
public class LocaleManager implements Serializable {
  ...
  /**
   * Default value for localeAware. This value will be used when the
   * corresponding property cannot be loaded.
   */
  public static final boolean DEFAULT_LOCALE_AWARE = false;

  private static boolean localeAware = PropertiesManager.getPropertyAsBoolean(
    "org.jasig.portal.i18n.LocaleManager.locale_aware", DEFAULT_LOCALE_AWARE);
  ...
  /**
   * Constructor that sets up locales according to the
   * <code>Accept-Language</code> request header from a user's browser.
   * 
   * @param person
   *          the user
   * @param acceptLanguage
   *          the Accept-Language request header from a user's browser
   */
  ...
  /**
   * Read and parse portal_locales from portal.properties. portal_locales will
   * be in the form of a comma-separated list, e.g. en_US,ja_JP,sv_SE
   */
  private Locale[] loadPortalLocales() {
    String portalLocalesString = PropertiesManager
      .getProperty("org.jasig.portal.i18n.LocaleManager.portal_locales");
    return parseLocales(portalLocalesString);
  }

  /**
   * Produces a sorted list of locales according to locale preferences obtained
   * from several places. The following priority is given: session, user,
   * browser, portal, and jvm.
   * 
  ...
  /**
   * Helper method to produce a <code>java.util.Locale</code> array from a
   * comma-delimited locale string list, e.g. "en_US,ja_JP"
   * 
   * @param localeStringList
  ...
  /**
   * Helper method to produce a <code>java.util.Locale</code> object from a
   * locale string such as en_US or ja_JP.
   * 
   * @param localeString
  ...
  /**
   * Constructs a comma-delimited list of locales that could be parsed back into
   * a Locale array with parseLocales(String localeStringList).
   * 
   * @param locales
   *          the list of locales
  ...
  /**
   * Creates an XML representation of a list of locales. If a selected locale is
   * supplied, the XML element representing the selected locale will have an
   * attribute of selected with value of true. This is helpful when constructing
   * user interfaces that indicate which locale is selected.
   * 
   * @param locales
   *          the locale list
  ...
  public static Document xmlValueOf(Locale[] locales, Locale selectedLocale) {
    ...
    for (int i = 0; i < locales.length; i++) {
      ...
      languageE.setAttribute("displayName", locales[i]
        .getDisplayLanguage(locales[0]));
