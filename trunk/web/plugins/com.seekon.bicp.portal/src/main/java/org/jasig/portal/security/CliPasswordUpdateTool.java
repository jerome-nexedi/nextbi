...
/**
 * <p>
 * A utility class that demonstrates changing and locking md5 passwords in the
 * UP_PERSON_DIR table. The program accepts two optional flags <code>-c</code>
...
public class CliPasswordUpdateTool implements IPasswordUpdateTool {
   ...
   * 
   * @see
   * org.jasig.portal.security.IPasswordUpdateTool#updatePassword(java.lang.
   * String, boolean)
   */
  @Override
  @Transactional
  public boolean updatePassword(String user, boolean create) throws IOException {
    ...
    if (StringUtils.isBlank(user)) {
      System.err
        .println("You did not specify a valid user name.  Please try again.");
      return false;
    }
    ...
    final String encryptedPassword = this.portalPasswordService
      .encryptPassword(spass);
