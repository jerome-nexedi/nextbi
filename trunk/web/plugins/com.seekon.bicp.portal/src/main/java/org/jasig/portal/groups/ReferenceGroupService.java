 ...
 * @version $Revision$
 * @deprecated Use instead an {@link ICompositeGroupService} implementation.
 */
@Deprecated
public class ReferenceGroupService implements ILockableGroupService {
  ...
  /**
   * Default value for cacheInUse. This value will be used when the
  ...
  */
  ...
  public IEntityGroup getDistinguishedGroup(String name) throws GroupsException {
    ...
    String key = PropertiesManager.getProperty(
      "org.jasig.portal.groups.ReferenceGroupService.key_" + name, null);
    if (key != null) {
      return findGroup(key);
    } else {
      throw new GroupsException(
        "ReferenceGroupService.getDistinguishedGroup(): no key found to match requested name ["
          + name + "]");
    }
  }
  ...
  private void initialize() throws GroupsException {
    ...
    try {
      groupFactory = (IEntityGroupStore) Class.forName(groupFactoryName)
        .newInstance();
    } catch (Exception e) {
      eMsg = "ReferenceGroupService.initialize(): Failed to instantiate "
        + groupFactoryName;
      log.error(eMsg);
  ...
  */
  ...
  public void updateGroupMembers(IEntityGroup group) throws GroupsException {
    if (cacheInUse()) {
      try {
        ...
        throw new GroupsException("Problem updating members for group "
          + group.getKey(), ce);
      }
    }
  ...
  public EntityIdentifier[] searchForEntities(String query, int method, Class type)
    throws GroupsException {
    final IEntitySearcher entitySearcher = ReferenceEntitySearcherLocator
      .getReferenceEntitySearcher();
