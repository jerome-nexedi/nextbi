...
public class TemplatesBuilder implements Loader<Templates>, ResourceLoaderAware {
   ...
   * 
   * @see
   * org.jasig.portal.utils.cache.resource.ResourceBuilder#buildResource(org
   * .springframework.core.io.Resource, java.io.InputStream)
   */
  @Override
  public LoadedResource<Templates> loadResource(Resource resource)
    throws IOException {
    final TransformerFactory transformerFactory = TransformerFactory.newInstance();

    if (this.transformerAttributes != null) {
      for (final Map.Entry<String, Object> attributeEntry : this.transformerAttributes
        .entrySet()) {
        transformerFactory.setAttribute(attributeEntry.getKey(), attributeEntry
          .getValue());
      }
    }

    final ResourceTrackingURIResolver uriResolver = new ResourceTrackingURIResolver(
      this.resourceLoader);
  ...
  private static class ResourceTrackingURIResolver extends ResourceLoaderURIResolver {
    ...
    @Override
    protected Resource resolveResource(String href, String base)
