...
/**
 * FlowViewResolver that can use a list of both ViewResolver and
 * FlowViewResolvers
 * 
...
public class FallbackDelegatingFlowViewResolver extends DelegatingFlowViewResolver {
   ...
   * 
   * @see
   * org.springframework.webflow.mvc.builder.DelegatingFlowViewResolver#resolveView
   * (java.lang.String, org.springframework.webflow.execution.RequestContext)
   */
  @Override
  public View resolveView(String viewId, RequestContext context) {
    ...
    for (final FlowViewResolver flowViewResolver : this.flowViewResolvers) {
      try {
        final String viewIdByConvention = flowViewResolver
          .getViewIdByConvention(viewId);
