...
/**
 * Handles entity management for transient portlets, defined as portlets that
 * aren't permanent parts of the user's layout. Portlet preferences are still
 * persisted but are associated with a transient rendering of the portlet.
 * Transient portlets are detected by checking the channel subscribe ID against
 * the {@link TransientUserLayoutManagerWrapper#SUBSCRIBE_PREFIX} prefix.
 * 
...
public class TransientPortletEntityDao implements IPortletEntityDao {
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#createPortletEntity(org.
   * jasig.portal.portlet.om.IPortletDefinitionId, java.lang.String, int)
   */
  @Override
  public IPortletEntity createPortletEntity(
    ...
    if (layoutNodeId.startsWith(TransientUserLayoutManagerWrapper.SUBSCRIBE_PREFIX)) {
      ...
      final IPortletEntity portletEntity = this.delegatePortletEntityDao
        .createPortletEntity(portletDefinitionId, layoutNodeId, userId);
      return new TransientPortletEntity(portletEntity, transientLayoutNodeId);
    }

    return this.delegatePortletEntityDao.createPortletEntity(portletDefinitionId,
      layoutNodeId, userId);
  }
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#deletePortletEntity(org.
   * jasig.portal.portlet.om.IPortletEntity)
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#getPortletEntities(org.jasig
   * .portal.portlet.om.IPortletDefinitionId)
   */
  @Override
  public Set<IPortletEntity> getPortletEntities(
    IPortletDefinitionId portletDefinitionId) {
    final Set<IPortletEntity> portletEntities = this.delegatePortletEntityDao
      .getPortletEntities(portletDefinitionId);
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#getPortletEntitiesForUser
   * (int)
   */
  @Override
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#getPortletEntity(org.jasig
   * .portal.portlet.om.IPortletEntityId)
   */
  @Override
  public IPortletEntity getPortletEntity(IPortletEntityId portletEntityId) {
    final IPortletEntity portletEntity = this.delegatePortletEntityDao
      .getPortletEntity(portletEntityId);
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#getPortletEntity(java.lang
   * .String, int)
   */
  @Override
  public IPortletEntity getPortletEntity(String layoutNodeId, int userId) {
    if (layoutNodeId.startsWith(TransientUserLayoutManagerWrapper.SUBSCRIBE_PREFIX)) {
      final String databaseChannelSubscribeId = this
        .determineDatabaseChannelSubscribeId(layoutNodeId);

      final IPortletEntity portletEntity = this.delegatePortletEntityDao
        .getPortletEntity(databaseChannelSubscribeId, userId);
      return this.wrapEntity(portletEntity);
    }
   ...
   * 
   * @see
   * org.jasig.portal.portlet.dao.IPortletEntityDao#updatePortletEntity(org.
   * jasig.portal.portlet.om.IPortletEntity)
   */
  @Override
  ...
  /**
   * Returns the unwrapped entity if it is an instance of
   * TransientPortletEntity. If not the original entity is returned.
   */
   ...
   * If the specified entity is transient but no transient subscribe id has been
   * registered for it yet in the transientIdMap null is returned. If no
  ...
  protected IPortletEntity wrapEntity(IPortletEntity portletEntity) {
    ...
    if (persistentLayoutNodeId
      .startsWith(TransientUserLayoutManagerWrapper.SUBSCRIBE_PREFIX)) {
      final IUserLayoutManager userLayoutManager = this.getUserLayoutManager();
      if (userLayoutManager == null) {
        this.logger
          .warn("Could not find IUserLayoutManager when trying to wrap transient portlet entity: "
            + portletEntity);
        return portletEntity;
      }
  ...
  /**
   * Calles {@link #wrapEntity(IPortletEntity)} on each entry in the Set, wrap
   * calls that return null result in the entitiy being dropped from the
   * returned Set
   */
  ...
  protected String determineDatabaseChannelSubscribeId(String layoutNodeId) {
    ...
    final IPortletDefinition portletDefinition = this.portletDefinitionRegistry
      .getPortletDefinition(portletPublishId);

    // Generate the subscribe ID used for the database
    return this
      .getPersistentLayoutNodeId(portletDefinition.getPortletDefinitionId());
  }
