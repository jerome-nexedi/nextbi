...
/**
 * Maps a user attribute to a skin. The user's attribute named by
 * {@link #setSkinAttributeName(String)} is used to look up a skin name via the
 * {@link #setAttributeToSkinMap(Map)} map and the skin name is set to in the
 * transformer using the {@link #setSkinParameterName(String)} parameter.
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public abstract class SkinMappingTransformerConfigurationSource extends
  TransformerConfigurationSourceAdapter implements BeanNameAware {
  ...
  /**
   * The name of the transformer parameter used for the skin name, defaults to
   * "skin"
   */
  ...
  /**
   * If true the result of {@link #getSkinName(HttpServletRequest)} will be
   * cached in the user's session and re-used. If false
   * {@link #getSkinName(HttpServletRequest)} will be called on every execution.
   */
  ...
  /**
   * Set of theme stylesheet descriptor names that the skin-default should be
   * set for. If not set the skin-default will be set for all stylesheets
   */
  ...
  @Override
  public final Map<String, Object> getParameters(HttpServletRequest request,
  ...
  @Override
  public final CacheKey getCacheKey(HttpServletRequest request,
    HttpServletResponse response) {
    final String skinName = this.getSkinNameInternal(request);
  ...
  private String getSkinNameInternal(HttpServletRequest request) {
    ...
    SkinNameHolder skinNameHolder = (SkinNameHolder) session
      .getAttribute(skinNameAttribute);
  ...
  protected boolean shouldOverrideSkin(HttpServletRequest request) {
    if (this.stylesheetDescriptorNames == null
      || this.stylesheetDescriptorNames.isEmpty()) {
      return true;
    }

    final IStylesheetDescriptor stylesheetDescriptor = stylesheetUserPreferencesService
      .getStylesheetDescriptor(request, PreferencesScope.THEME);
