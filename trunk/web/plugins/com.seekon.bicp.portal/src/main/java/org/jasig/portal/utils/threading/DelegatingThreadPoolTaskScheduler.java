...
public class DelegatingThreadPoolTaskScheduler extends ThreadPoolTaskScheduler
   ...
   * 
   * @param initialDelay
   *          Delay before starting ANY scheduled task
   */
  ...
  protected long getAdditionalStartDelay() {
    // Only bother recalculating the start delay if the last time it was
    // resulted in a delay
    if (this.lastStartDelay != 0) {
      this.lastStartDelay = Math.max(0, this.initialDelay
        - (System.currentTimeMillis() - this.initialized));
  ...
  protected Date getDelayedStartDate(Date startDate) {
    ...
    if (additionalStartDelay > 0) {
      ...
      logger.debug("Updated startDate with additionalStartDelay from " + startDate
        + " to " + newStartDate);
      return newStartDate;
    }
  ...
  public void execute(Runnable task, long startTimeout) {
    final DelegatingRunnable delegatingRunnable = new DelegatingRunnable(
      this.executorService, task);
  ...
  public <T> Future<T> submit(Callable<T> task) {
    final DelegatingCallable<T> delegatingCallable = new DelegatingCallable<T>(
      this.executorService, task);
  ...
  public ScheduledFuture<Object> schedule(Runnable task, final Trigger trigger) {
    ...
    final ScheduledFuture<ScheduledFuture<Object>> future = super.schedule(
      delegatingRunnable, wrappedTrigger);
  ...
  public ScheduledFuture<Object> schedule(Runnable task, Date startTime) {
    ...
    @SuppressWarnings("unchecked")
    final ScheduledFuture<ScheduledFuture<Object>> future = super.schedule(
      delegatingRunnable, startTime);
    return new DelegatingForwardingScheduledFuture<Object>(future);
  }

  @Override
  public ScheduledFuture<Object> scheduleAtFixedRate(Runnable task, Date startTime,
    long period) {
    startTime = getDelayedStartDate(startTime);
    ...
    final DelegatingRunnable delegatingRunnable = new DelegatingRunnable(
      this.executorService, task);
    @SuppressWarnings("unchecked")
    final ScheduledFuture<ScheduledFuture<Object>> future = super
      .scheduleAtFixedRate(delegatingRunnable, startTime, period);
    return new DelegatingForwardingScheduledFuture<Object>(future);
  }
  ...
  public ScheduledFuture<Object> scheduleAtFixedRate(Runnable task, long period) {
    final long additionalStartDelay = this.getAdditionalStartDelay();
    if (additionalStartDelay > 0) {
      // If there is an additional delay use the alternate call which includes a
      // startTime
    ...
    @SuppressWarnings("unchecked")
    final ScheduledFuture<ScheduledFuture<Object>> future = super
      .scheduleAtFixedRate(delegatingRunnable, period);
    return new DelegatingForwardingScheduledFuture<Object>(future);
  }

  @Override
  public ScheduledFuture<Object> scheduleWithFixedDelay(Runnable task,
    ...
    @SuppressWarnings("unchecked")
    final ScheduledFuture<ScheduledFuture<Object>> future = super
      .scheduleWithFixedDelay(delegatingRunnable, startTime, delay);
    return new DelegatingForwardingScheduledFuture<Object>(future);
  }
  ...
  public ScheduledFuture<Object> scheduleWithFixedDelay(Runnable task, long delay) {
    ...
    @SuppressWarnings("unchecked")
    final ScheduledFuture<ScheduledFuture<Object>> future = super
      .scheduleWithFixedDelay(delegatingRunnable, delay);
    return new DelegatingForwardingScheduledFuture<Object>(future);
  }
  ...
  private static class DelegatingForwardingScheduledFuture<V> extends
    DelegatingForwardingFuture<V> implements ScheduledFuture<V> {
