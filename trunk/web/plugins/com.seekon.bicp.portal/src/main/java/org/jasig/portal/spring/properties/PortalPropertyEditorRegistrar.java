...
public class PortalPropertyEditorRegistrar implements PropertyEditorRegistrar {
  ...
  public void registerCustomEditors(PropertyEditorRegistry registry) {
    if (this.propertyEditors == null) {
      this.logger
        .warn("No PropertyEditors Map configured, returning with no action taken.");
      return;
    }

    for (final Map.Entry<Class<?>, PropertyEditor> editorEntry : this.propertyEditors
      .entrySet()) {
      final Class<?> requiredType = editorEntry.getKey();
      final PropertyEditor editor = editorEntry.getValue();

      if (this.logger.isTraceEnabled()) {
        this.logger.trace("Registering PropertyEditor '" + editor + "' for type '"
          + requiredType + "'");
      }
