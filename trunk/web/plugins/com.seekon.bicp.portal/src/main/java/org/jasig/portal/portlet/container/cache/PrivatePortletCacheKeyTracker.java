...
/**
 * Used to track keys by session id and portlet window id for a cache. Clients
 * need to call
 * {@link PrivatePortletCacheKeyTracker#initPrivateKeyCache(HttpSession)} on a
 * session before keys will be tracked for that session. At any point clients
 * can call
...
final class PrivatePortletCacheKeyTracker extends CacheEventListenerAdapter {
  ...
  /**
   * Map from {@link HttpSession#getId()} to LoadingCache of private portlet
   * cache keys. Weak values are used so that when the {@link HttpSession} that
   * holds the LoadingCache is invalidated the reference is freed
   */
  ...
  public void initPrivateKeyCache(HttpSession session) {
    // Sync to make sure only one instance of the LoadingCache is created per
    // session
    synchronized (WebUtils.getSessionMutex(session)) {
      if (session.getAttribute(sessionKey) == null) {
        final LoadingCache<IPortletWindowId, Set<PrivatePortletCacheKey>> privateCacheKeysMap = CacheBuilder
          .newBuilder().build(
            new CacheLoader<IPortletWindowId, Set<PrivatePortletCacheKey>>() {
              @Override
              public Set<PrivatePortletCacheKey> load(IPortletWindowId key)
                throws Exception {
        ...
        logger.debug("Initiated key tracking for session {} and tracker {}",
  ...
  public void destroyPrivateKeyCache(HttpSession session) {
    ...
    logger.debug("Removed all cache keys for session {} to tracker {}", sessionId,
      this.sessionKey);
  }

  public Set<PrivatePortletCacheKey> getCacheKeys(HttpSession session,
    IPortletWindowId portletWindowId) {
    final LoadingCache<IPortletWindowId, Set<PrivatePortletCacheKey>> privatePortletCacheKeys = privatePortletCacheKeysBySession
      .get(session.getId());
    if (privatePortletCacheKeys == null) {
      return Collections.emptySet();
    }

    final Set<PrivatePortletCacheKey> keySet = privatePortletCacheKeys
      .getIfPresent(portletWindowId);
    if (keySet == null) {
      return Collections.emptySet();
    }
  ...
  protected void removeEntry(Element element) {
    ...
    if (privatePortletCacheKeys != null) {
      final Set<PrivatePortletCacheKey> keySet = privatePortletCacheKeys
        .getIfPresent(key.getPortletWindowId());
      if (keySet != null) {
