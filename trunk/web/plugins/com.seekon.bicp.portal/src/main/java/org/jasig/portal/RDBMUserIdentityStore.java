 ...
 * 
 * @author Susan Bramhall, Yale University (modify by Julien Marchal, University
 *         Nancy 2; Eric Dalquist - edalquist@unicon.net)
 * @version $Revision$
 */
...
public class RDBMUserIdentityStore implements IUserIdentityStore {
  ...
  public void setPortletEntityDao(
    @Qualifier("persistence") IPortletEntityDao portletEntityDao) {
  ...
  public void setUserLockCache(Ehcache userLockCache) {
    this.userLockCache = new SelfPopulatingCache(userLockCache,
  ...
  /**
   * getuPortalUID - return a unique uPortal key for a user. calls alternate
   * signature with createPortalData set to false.
   * 
   * @param person
   *          the person object
  ...
  public void removePortalUID(final String userName) {
    this.transactionOperations.execute(new TransactionCallbackWithoutResult() {
      ...
      protected void doInTransactionWithoutResult(TransactionStatus arg0) {
        ...
        final int userId = jdbcOperations.queryForInt(
          "SELECT USER_ID FROM UP_USER WHERE USER_NAME=?", userName);

        final int type = jdbcOperations.queryForInt(
        ...
        final List<Integer> groupIds = jdbcOperations.queryForList(
          "SELECT M.GROUP_ID "
            ...
            + "  AND  M.MEMBER_KEY =? AND  M.MEMBER_IS_GROUP = 'F'", Integer.class,
          userName);
        ...
        jdbcOperations.update("DELETE FROM UP_USER            WHERE USER_ID = ?",
          userId);
        jdbcOperations.update("DELETE FROM UP_USER_LAYOUT     WHERE USER_ID = ?",
          userId);
        jdbcOperations.update("DELETE FROM UP_USER_PROFILE    WHERE USER_ID = ?",
          userId);
        jdbcOperations.update("DELETE FROM UP_LAYOUT_PARAM    WHERE USER_ID = ?",
          userId);
        jdbcOperations.update("DELETE FROM UP_LAYOUT_STRUCT   WHERE USER_ID = ?",
          userId);
        jdbcOperations.update("DELETE FROM UP_USER_LOCALE     WHERE USER_ID = ?",
          userId);

        // Purge all portlet entity data
        final Set<IPortletEntity> portletEntities = portletEntityDao
        ...
        for (final IStylesheetUserPreferences stylesheetUserPreference : stylesheetUserPreferences) {
          stylesheetUserPreferencesDao
            .deleteStylesheetUserPreferences(stylesheetUserPreference);
        }
  ...
  public void removePortalUID(final int uPortalUID) {
    ...
    this.transactionOperations.execute(new TransactionCallbackWithoutResult() {
      @Override
      protected void doInTransactionWithoutResult(TransactionStatus arg0) {
        final String name = jdbcOperations.queryForObject(
          "SELECT USER_NAME FROM UP_USER WHERE USER_ID=?", String.class, uPortalUID);
        if (name == null) {
          log.warn("No user exists for id " + uPortalUID
   ...
   *          indicating whether to try to create all uPortal data for this user
   *          from template prototype
   * @return uPortalUID number or -1 if unable to create user.
   * @throws AuthorizationException
   *           if createPortalData is false and no user is found or if a sql
  ...
  public boolean isDefaultUser(String username) {
    final int defaultUserCount = this.jdbcOperations.queryForInt(
      IS_DEFAULT_USER_QUERY, username);
    ...
    try {
      ...
      if (createPortalData) {
        ...
        if (portalUser != null) {
          ...
          // User hasn't logged in before, some data needs to be created for
          // them based on their template user

          // Retrieve the information for the template user
          TemplateUser templateUser = getTemplateUser(templateName);
          if (templateUser == null) {
            throw new AuthorizationException(
              "No information found for template user = " + templateName
                + ". Cannot create new account for " + userName);
          }
        ...
        throw new AuthorizationException("No portal information exists for user "
          + userName);
      }
  ...
  protected PortalUser getPortalUser(final String userName) throws Exception {
    return jdbcOperations.execute(new ConnectionCallback<PortalUser>() {
      @Override
      public PortalUser doInConnection(Connection con) throws SQLException,
        DataAccessException {
        ...
        try {
          ...
          try {
            if (log.isDebugEnabled())
              log.debug("RDBMUserIdentityStore::getPortalUID(userName=" + userName
                + "): " + query);
  ...
  /**
   * Gets the TemplateUser data store object for the specified template user
   * name.
   * 
   ...
   * @throws Exception
   */
  protected TemplateUser getTemplateUser(final String templateUserName)
    throws Exception {
    return jdbcOperations.execute(new ConnectionCallback<TemplateUser>() {
      @Override
      public TemplateUser doInConnection(Connection con) throws SQLException,
        DataAccessException {
        ...
        try {
          ...
          try {
            if (log.isDebugEnabled())
              log.debug("RDBMUserIdentityStore::getTemplateUser(templateUserName="
                + templateUserName + "): " + query);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    return jdbcOperations.execute(new ConnectionCallback<Boolean>() {
      ...
      public Boolean doInConnection(Connection con) throws SQLException,
        ...
        try {
          ...
          try {
            if (log.isDebugEnabled())
              log.debug("RDBMUserIdentityStore::getTemplateUser(userId=" + userId
                + "): " + query);
            rs = pstmt.executeQuery();
            if (rs.next()) {
  ...
  private ILockableEntityGroup getSafeLockableGroup(IEntityGroup eg, IGroupMember gm) {
    ...
    try {
      ...
      log.error(
        "Unable to create lockable group for group/member: " + eg + "/" + gm, e);
    }
   ...
   * 
   * @param person
   *          person to be removed (used for logging)
   * @param me
   *          member representing the person
   * @param eg
   *          group from which the user should be removed
   */
  ...
  /**
   * Add a person to a group. This method catches and logs exceptions
   * encountered performing the removal.
   * 
   * @param person
   *          person to be added (used for logging)
   * @param me
   *          member representing the person
   * @param eg
  ...
  protected void updateUser(final int userId, final IPerson person,
    final TemplateUser templateUser) throws Exception {
      ...
      public Object doInTransaction(TransactionStatus status) {
        return jdbcOperations.execute(new ConnectionCallback<Object>() {
          ...
          public Object doInConnection(Connection con) throws SQLException,
            ...
            try {
              // Update UP_USER
              String update = "UPDATE UP_USER " + "SET USER_DFLT_USR_ID=?, "
                + "USER_DFLT_LAY_ID=?, " + "NEXT_STRUCT_ID=null "
                + "WHERE USER_ID=?";
                ...
                while (rs.next()) {
                  ...
                  if (log.isDebugEnabled())
                    log.debug("RDBMUserIdentityStore::updateUser(USER_ID=" + userId
                      ...
                      + profileName + ", DESCRIPTION=" + description + "): "
                      + insert);
                  insertStmt.executeUpdate();
                }
                rs.close();
  ...
  protected int addNewUser(final int newUID, final IPerson person,
    final TemplateUser templateUser) throws Exception {
    // Copy template user's groups memberships
    IGroupMember me = GroupService.getGroupMember(person.getEntityIdentifier());
    IGroupMember template = GroupService.getEntity(templateUser.getUserName(), Class
      .forName("org.jasig.portal.security.IPerson"));
    Iterator templateGroups = template.getContainingGroups();
    while (templateGroups.hasNext()) {
      IEntityGroup eg = (IEntityGroup) templateGroups.next();
      ILockableEntityGroup leg = getSafeLockableGroup(eg, me);
            ...
            try {
              ...
              if (log.isDebugEnabled())
                log.debug("RDBMUserIdentityStore::addNewUser(USER_ID=" + newUID
                  + ", USER_NAME=" + userName + ", USER_DFLT_USR_ID="
                  + templateUser.getUserId() + ", USER_DFLT_LAY_ID="
                  + templateUser.getDefaultLayoutId() + "): " + insert);
              insertStmt.executeUpdate();
              insertStmt.close();
              insertStmt = null;
