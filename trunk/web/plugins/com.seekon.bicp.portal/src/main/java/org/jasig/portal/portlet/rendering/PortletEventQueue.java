...
/**
 * Used to track events generated for a specific portlet window in a thread safe
 * manner. The event source adds events to the queue for a specific
 * {@link IPortletWindowId} and the event consumer can iterate over the
 * {@link IPortletWindowId}s that have events and get a {@link Queue} of
...
public class PortletEventQueue implements Iterable<IPortletWindowId> {
  /**
   * Queue of portlet events that have not been resolved to target specific
   * portlet windows
   */
  private final Queue<QueuedEvent> rawEventQueue = new ConcurrentLinkedQueue<QueuedEvent>();

  /**
   * Map for portlet windows where the value is a queue of events that need to
   * be dispatched to the portlet
   */
  ...
  public void offerEvent(IPortletWindowId portletWindowId, QueuedEvent event) {
    ...
    if (events == null) {
      events = ConcurrentMapUtils.putIfAbsent(resolvedEventQueues, portletWindowId,
  ...
  /**
   * Remove the oldest {@link Event} from the Queue for the specified
  ...
  /**
   * Get an {@link Iterator} of all {@link IPortletWindowId}s that have
   * {@link Event}s queued.
   */
  @Override
