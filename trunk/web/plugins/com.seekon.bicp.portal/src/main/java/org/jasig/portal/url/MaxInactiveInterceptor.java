...
public class MaxInactiveInterceptor extends HandlerInterceptorAdapter {
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
    ...
    final IPerson person = this.personManager
    ...
    if (securityContext != null && securityContext.isAuthenticated()) {
      // We have an authenticated user... let's see if any MAX_INACTIVE settings
      // apply...
      IAuthorizationPrincipal principal = authorizationService.newPrincipal(
        (String) person.getAttribute(IPerson.USERNAME), IPerson.class);
      Integer rulingGrant = null;
      Integer rulingDeny = null;
      IPermission[] permissions = authorizationService
        .getAllPermissionsForPrincipal(principal, null, "MAX_INACTIVE", null);
      for (IPermission p : permissions) {
        ...
        if (p.getType().equals(IPermission.PERMISSION_TYPE_GRANT)) {
          try {
            ...
            if (rulingGrant == null || grantEntry.intValue() < 0 /*
                                                                  * Any
                                                                  * negative
                                                                  * number
                                                                  * trumps all
      ...
      if (rulingDeny != null && rulingDeny.intValue() < 0) {
        ...
        // clear it.
        log
          .warn("A MAX_INACTIVE DENY entry improperly specified a negative target:  "
            + rulingDeny.intValue());
        rulingDeny = null;
      }
      if (rulingGrant != null || rulingDeny != null) {
        ...
        // govern.
        int maxInactive = rulingGrant != null ? rulingGrant.intValue() : 0; // If
        // rulingGrant
        // is
        // null,
        // rulingDeny
        ...
        // be...
        if (rulingDeny != null) {
          // Applying DENY entries is tricky b/c GRANT entries may be
          // negative...
