...
public interface IPermissionTargetProvider {
  ...
  /**
   * Return the permission target associated with the specified key under this
   * provider. If no target with the given key is valid for this provider,
  ...
  /**
   * Search this provider for a particular target using a single string search
   * term. Each target provider implementation is responsible for determining
   * the definition a "matching" target.
   * 
   * @param term
   *          search term
