 ...
 * {@link org.jasig.portal.security.provider.cas.ICasSecurityContext} that reads
 * the Assertion from the ThreadLocal. The Assertion stored in a ThreadLocal is
 * an artifact of the Jasig CAS Client for Java 3.x library.
 * 
 * @author Scott Battaglia
 * @version $Revision$ $Date$
 * @since 3.2
 */
public class CasAssertionSecurityContext extends ChainingSecurityContext implements
  ICasSecurityContext {
  ...
  /**
   * Exposes a template post-authentication method for subclasses to implement
   * their custom logic in.
   * <p>
   * NOTE: This is called BEFORE super.authenticate();
   * 
   * @param assertion
   *          the Assertion that was retrieved from the ThreadLocal. CANNOT be
   *          NULL.
   */
  ...
  public final void authenticate() throws PortalSecurityException {
    ...
    if (this.assertion != null) {
      ...
      log.debug("CASContext authenticated [" + this.myPrincipal.getUID()
        + "] using assertion [" + this.assertion + "]");
  ...
  public final String getCasServiceToken(final String target)
    ...
    if (log.isTraceEnabled()) {
      log.trace("Attempting to retrieve proxy ticket for target [" + target
        + "] by using CAS Assertion [" + assertion + "]");
    }
    ...
    final String proxyTicket = this.assertion.getPrincipal().getProxyTicketFor(
      target);

    if (proxyTicket == null) {
      log.error("Failed to retrieve proxy ticket for assertion [" + assertion
