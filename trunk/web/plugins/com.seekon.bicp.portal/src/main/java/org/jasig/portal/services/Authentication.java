...
/**
 * Attempts to authenticate a user and retrieve attributes associated with the
 * user.
 * 
 * @author Ken Weiner, kweiner@unicon.net
 * @author Don Fracapane (df7@columbia.edu) Added properties in the security
 *         properties file that hold the tokens used to represent the principal
 *         and credential for each security context. This version differs in the
 *         way the principal and credentials are set (all contexts are set up
 *         front after evaluating the tokens). See setContextParameters() also.
 * @version $Revision$ Changes put in to allow credentials and principals to be
 *          defined and held by each context.
 */
@Service
public class Authentication {
  ...
  /**
   * Attempts to authenticate a given IPerson based on a set of principals and
   * credentials
   * 
   * @param principals
   * @param credentials
  ...
  public void authenticate(HttpServletRequest request,
    Map<String, String> principals, Map<String, String> credentials, IPerson person)
    ...
    this.configureSecurityContextChain(principals, credentials, person,
      securityContext, BASE_CONTEXT_NAME);
    ...
    // credentials for each context. It then retrieves the values from the
    // request
    ...
    if (securityContext.isAuthenticated()) {
      ...
      final IAdditionalDescriptor addInfo = person.getSecurityContext()
      ...
      if (addInfo != null) {
        ...
        // additional descriptor is an IPerson object created by the security
        // context
        // NOTE: This is not the preferred method, creation of IPerson objects
        // should be
        ...
        if (addInfo instanceof IPerson) {
          ...
          for (final Enumeration<String> e = newPerson.getAttributeNames(); e
            .hasMoreElements();) {
            final String attributeName = e.nextElement();
            person
              .setAttribute(attributeName, newPerson.getAttribute(attributeName));
          }
        ...
        // simply copy all of these additional attributes into the IPerson
          ...
          for (final Iterator<?> keys = additionalAttributes.keySet().iterator(); keys
            .hasNext();) {
            // Get a key
      ...
      if (PropertiesManager
        .getPropertyAsBoolean("org.jasig.portal.services.Authentication.usePersonDirectory")) {
        ...
        if (personAttributes != null) {
          // attribs may be null. IPersonAttributeDao returns null when it does
          // not recognize a user at all, as
          // distinguished from returning an empty Map of attributes when it
          // recognizes a user has having no
          // attributes.
      ...
      if (person.getFullName() == null) {
        ...
        if (person.getFullName() == null) {
          person.setFullName("Unrecognized person: "
            + person.getAttribute(IPerson.USERNAME));
        }
      }
      // Find the uPortal userid for this user or flunk authentication if not
      // found
      // The template username should actually be derived from directory
      // information.
      // The reference implemenatation sets the uPortalTemplateUserName to the
      // default in
      ...
      // If we are going to be auto creating accounts then we must find the
   ...
   * @param person
   */
  public void setContextParameters(Map<String, String> principals,
    Map<String, String> credentials, String ctxName,
    ISecurityContext securityContext, IPerson person) {

    if (log.isDebugEnabled()) {
      final StringBuilder msg = new StringBuilder();
      msg
        .append("Preparing to authenticate;  setting parameters for context name '")
        .append(ctxName).append("', context class '").append(
          securityContext.getClass().getName()).append("'");
      ...
      // Keep credentialTokens secret, but indicate whether they were
      // provided...
      ...
      for (final Object o : credentials.entrySet()) {
        ...
        msg.append("\n\t\t").append(y.getKey()).append(" was ").append(
          valWasSpecified);
      }
    ...
    username = username != null ? username : (String) principals
      .get(BASE_CONTEXT_NAME);
    credential = credential != null ? credential : (String) credentials
      .get(BASE_CONTEXT_NAME);
    if (log.isDebugEnabled()) {
    ...
    final IOpaqueCredentials credentialsInstance = securityContext
  ...
  /**
   * Recureses through the {@link ISecurityContext} chain, setting the
   * credentials for each. TODO This functionality should be moved into the
   * {@link org.jasig.portal.security.provider.ChainingSecurityContext}.
   * 
   * @param principals
   * @param credentials
   * @param person
  ...
  private void configureSecurityContextChain(final Map<String, String> principals,
    final Map<String, String> credentials, final IPerson person,
    final ISecurityContext securityContext, final String baseContextName)
    throws PortalSecurityException {
    this.setContextParameters(principals, credentials, baseContextName,
      securityContext, person);

    // load principals and credentials for the subContexts
    for (final Enumeration<String> subCtxNames = securityContext
      .getSubContextNames(); subCtxNames.hasMoreElements();) {
