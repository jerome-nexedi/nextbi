...
public interface IPortletExecutionWorker<V> extends IPortletExecutionContext {
  ...
  /**
   * Submit the worker for execution. The worker may start immediately or submit
   * itself to a thread pool or service for execution at a future time.
   * 
   ...
   * 
   * @param timeout
   *          The maximum time in ms to wait for the worker to start
   * @return The time in ms that the worker actually started, 0 if it has not
   *         started
   */
   ...
   * 
   * @param timeout
   *          The maximum time in ms to wait for the worker to complete, if the
   *          worker has not started yet this includes time spent waiting to
   *          start
   * @return The object returned by the portlet execution
   * @throws Exception
   *           The exception thrown by the portlet during execution if any
   */
