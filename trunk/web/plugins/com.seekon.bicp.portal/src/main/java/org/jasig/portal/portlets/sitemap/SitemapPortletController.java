...
/**
 * SitemapPortletController produces a sitemap view of the current user's portal
 * layout.
 * 
...
public class SitemapPortletController {
  ...
  /**
   * Name of XSL parameter indicating whether to use tab groups or not
  ...
  /**
   * We must use {@link StAXPipelineComponent} over the user layout document,
   * because layout document doesn't contain required attributes for tab group
   * functionality. Those attributes must be incorporated into user's layout
  ...
  /**
   * Whether to use tab groups or not. The value of this attribute will be
   * passed to XSL using {@value #USE_TAB_GROUPS} as parameter name.
   */
   ...
   */
  @RequestMapping
  public ModelAndView displaySitemap(PortletRequest request)
    throws XMLStreamException {

    Map<String, Object> model = new HashMap<String, Object>();

    // retrieve the user layout with structure attributes applied (required in
    // order to display tab groups)
    final HttpServletRequest httpServletRequest = this.portalRequestUtils
      .getPortletHttpRequest(request);
    final HttpServletResponse httpServletResponse = this.portalRequestUtils
    ...
    model.put(XsltPortalUrlProvider.XSLT_PORTAL_URL_PROVIDER,
      this.xsltPortalUrlProvider);
