...
public class AggregationIntervalHelperImpl implements AggregationIntervalHelper {
   ...
   * 
   * @see
   * org.jasig.portal.stats.IntervalHelper#getIntervalDates(org.jasig.portal
   * .stats.Interval, java.util.Date)
   */
  @Override
  public AggregationIntervalInfo getIntervalInfo(AggregationInterval interval,
    DateTime date) {
    // Chop off everything below the minutes (seconds, millis)
    final DateTime instant = date.minuteOfHour().roundFloorCopy();

    // TODO cache this resolution ... best place would be in the current jpa
    // session
    ...
    case CALENDAR_QUARTER: {
      final List<QuarterDetail> quartersDetails = this.eventAggregationManagementDao
        .getQuartersDetails();
      final QuarterDetail quarterDetail = EventDateTimeUtils.findDateRangeSorted(
        instant, quartersDetails);
    ...
    case ACADEMIC_TERM: {
      ...
      final AcademicTermDetail academicTermDetail = EventDateTimeUtils
    ...
    final TimeDimension startTimeDimension = this.timeDimensionDao
      .getTimeDimensionByTime(startTime);

    final DateMidnight startDateMidnight = start.toDateMidnight();
    final DateDimension startDateDimension = this.dateDimensionDao
      .getDateDimensionByDate(startDateMidnight);

    return new AggregationIntervalInfo(interval, start, end, startDateDimension,
      startTimeDimension);
  }
  ...
  protected DateTime determineStart(AggregationInterval interval, DateTime instant) {
    ...
    if (interval == AggregationInterval.FIVE_MINUTE) {
      return instant.hourOfDay().roundFloorCopy().plusMinutes(
        (instant.getMinuteOfHour() / 5) * 5);
    }
