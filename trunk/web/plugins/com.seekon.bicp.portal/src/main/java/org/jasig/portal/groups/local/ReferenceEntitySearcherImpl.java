...
/**
 * An IEntitySearcher implementation for the local portal group service. Uses
 * implementations of ITypedEntitySearcher to do the dirty work.
 * 
 * @author Alex Vigdor
 * @version $Revision$
 */
  ...
  public EntityIdentifier[] searchForEntities(String query, int method, Class type)
    ...
    for (final ITypedEntitySearcher typedEntitySearcher : this.typedEntitySearchers) {
      if (typedEntitySearcher.getType().equals(type)) {
        final EntityIdentifier[] results = typedEntitySearcher.searchForEntities(
          query, method);
