...
public class JpaClusterLockDao extends BaseJpaDao implements IClusterLockDao {
  ...
  /**
   * Maximum age of the {@link ClusterMutex#getLastUpdate()} field for a locked
   * mutex. A ClusterMutex with an old lastUpdate value will be considered
   * abandoned and be forcibly unlocked. Defaults to 5 seconds.
   * <p/>
   * IMPORTANT: this value must be larger than the maximum possible clock skew
  ...
  public void setPlatformTransactionManager(
    ...
    this.newTransactionTemplate
      .setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRES_NEW);
    this.newTransactionTemplate.afterPropertiesSet();

    this.defaultTransactionTemplate = new TransactionTemplate(
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    this.clusterLockByNameQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<ClusterMutex>>() {
        @Override
        public CriteriaQuery<ClusterMutex> apply(CriteriaBuilder cb) {
          final CriteriaQuery<ClusterMutex> criteriaQuery = cb
            .createQuery(ClusterMutex.class);
          final Root<ClusterMutex> definitionRoot = criteriaQuery
            .from(ClusterMutex.class);
          criteriaQuery.select(definitionRoot);
          criteriaQuery.where(cb.equal(definitionRoot.get(ClusterMutex_.name),
            nameParameter));
   ...
   * 
   * @see
   * org.jasig.portal.concurrency.locking.ClusterLockDao#getClusterMutex(java
   * .lang.String)
   */
  @Override
  public ClusterMutex getClusterMutex(final String mutexName) {
    ...
    throw new IllegalStateException("Failed to find or create ClusterMutex "
      + mutexName);
  }
    ...
    this.executeIgnoreRollback(new TransactionCallbackWithoutResult() {
      ...
      protected void doInTransactionWithoutResult(TransactionStatus status) {
        ...
        try {
          ...
          final IllegalMonitorStateException imse = new IllegalMonitorStateException(
            "Failed to update " + mutexName
  ...
  protected ClusterMutex getClusterMutexInternal(final String mutexName) {
    return this.defaultTransactionTemplate
      .execute(new TransactionCallback<ClusterMutex>() {
        @Override
  ...
  /**
   * Creates a new ClusterMutex with the specified name. Returns the created
   * mutex or null if the mutex already exists.
   */
  ...
  protected void validateLockedMutex(ClusterMutex clusterMutex) {
    ...
    if (!serverName.equals(clusterMutex.getServerId())) {
      throw new IllegalMonitorStateException(
        "Mutex is currently locked by another server: " + clusterMutex);
    }
  }
    ...
    this.executeIgnoreRollback(new TransactionCallbackWithoutResult() {
      ...
      protected void doInTransactionWithoutResult(TransactionStatus status) {
        ...
        try {
          ...
          logger.trace("Abandoned mutex {} was cleared by another thread or server",
            mutexName);
        }
      }
    });
  ...
  protected boolean isLockAbandoned(final ClusterMutex clusterMutex) {
    return clusterMutex.getLastUpdate() < (System.currentTimeMillis() - abandonedLockAge
      .getMillis());
  }
  ...
  /**
   * Utility for using TransactionTemplate when we know that a rollback might
   * happen and just want to ignore it
   */
  protected <T> T executeIgnoreRollback(TransactionCallback<T> action,
    T rollbackValue) {
    try {
