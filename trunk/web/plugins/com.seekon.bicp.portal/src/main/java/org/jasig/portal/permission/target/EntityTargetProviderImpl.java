 ...
 * of this implementation may indicate which entity types may be used as
 * targets. Target keys will be the key of the underlying entity itself, while
 * the target human-readable name will similarly be the name of the entity.
 * 
 * TODO: This implementation currently has a number of problems. The code uses
 * the EntityEnum class and is hardcoded to only recognize four types of
 * entities: uPortal person groups, person entities, portlet categories, and
 * portlet entities. This code also may perform poorly for large portal
 * installations for searches that return many results.
 * 
 * @author Jen Bourey, jbourey@unicon.net
 * @version $Revision$
 * @since 3.3
 */
public class EntityTargetProviderImpl implements IPermissionTargetProvider,
  Serializable {
  ...
  /**
   * Construct a new instance of targets matching the set of allowed target
   * entity types.
   * 
   ...
   * 
   * @see
   * org.jasig.portal.permission.target.IPermissionTargetProvider#getTarget(
   * java.lang.String)
   */
  public IPermissionTarget getTarget(String key) {

    /*
     * If the specified key matches one of the "all entity" style targets, just
     ...
     * implementation will return the first entity that it finds. If the portal
     * contains duplicate entity keys across multiple types, it's possible that
     * this implementation would demonstrate inconsistent behavior.
     */

    for (String type : allowedEntityTypes) {
      JsonEntityBean entity = groupListHelper.getEntity(type, key, false);
      if (entity != null) {
        IPermissionTarget target = new PermissionTargetImpl(entity.getId(), entity
          .getName());
        return target;
      }
    }
   ...
   * 
   * @see
   * org.jasig.portal.permission.target.IPermissionTargetProvider#searchTargets
   * (java.lang.String)
   */
  public Collection<IPermissionTarget> searchTargets(String term) {
    ...
    /*
     * Attempt to find matching entities for each allowed entity type. Any
     * matching entities will be added to our collection.
     */
   ...
   * represent any generic category-type entity, allowing administrators to set
   * permissions on ungrouped entity items.
   * 
   ...
   * represent any generic person group-type entity, allowing administrators to
   * set permissions on ungrouped entity items.
   * 
   ...
   * represent any generic portlet-type entity, allowing administrators to set
   * permissions on ungrouped entity items.
   * 
