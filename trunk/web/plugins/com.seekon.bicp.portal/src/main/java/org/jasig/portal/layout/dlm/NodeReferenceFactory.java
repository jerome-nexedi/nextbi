...
public final class NodeReferenceFactory {
  ...
  private final static Pattern USER_NODE_PATTERN = Pattern
    .compile("\\A([a-zA-Z]\\d*)\\z");

  private final static Pattern DLM_NODE_PATTERN = Pattern
    .compile("u(\\d+)l\\d+([ns]\\d+)");
  ...
  /**
   * Returns a valid {@link Noderef} based on the specified arguments or <code>null</null> if that's not possible.<br/>
   * 
   * <strong>This method returns <code>null</code> if the pathref cannot be resolved to a node on
   * the specified layout.</strong> It is the responsibility of calling code to
   * handle this case appropriately.
   * 
   * @return a valid {@link Noderef} or <code>null</null>
   */
  public Noderef getNoderefFromPathref(String layoutOwner, String pathref,
    String fname, boolean isStructRef, org.dom4j.Element layoutElement) {
    ...
    if (log.isTraceEnabled()) {
      ...
      msg.append("getDlmNoderef: [layoutOwner='").append(layoutOwner).append(
        "', pathref='").append(pathref).append("', fname='").append(fname).append(
        "', isStructRef='").append(isStructRef).append("']");
    ...
    if (pathTokens[0].equals(layoutOwner)) {
      ...
      final org.dom4j.Element target = (org.dom4j.Element) layoutElement
        .selectSingleNode(pathTokens[1]);
      if (target != null) {
        return new Noderef(target.valueOf("@ID"));
      }

      this.log.warn("Unable to resolve pathref '" + pathref + "' for layoutOwner '"
        + layoutOwner + "'");
      return null;
    }
    ...
    final Integer layoutOwnerUserId = this.userIdentityStore
      .getPortalUserId(layoutOwnerName);
    if (layoutOwnerUserId == null) {
      this.log.warn("Unable to resolve pathref '" + pathref + "' for layoutOwner '"
        + layoutOwner + "', no userId found for userName: " + layoutOwnerName);
      return null;
    }

    final Tuple<String, DistributedUserLayout> userLayoutInfo = getUserLayoutTuple(
      layoutOwnerName, layoutOwnerUserId);
    final Document userLayout = userLayoutInfo.second.getLayout();

    final Node targetNode = this.xPathOperations.evaluate(layoutPath, userLayout,
      XPathConstants.NODE);
    if (targetNode == null) {
    ...
    if (fname != null) {
      ...
      if (fnameAttr == null) {
        this.log.warn("Layout node for pathref does not have fname attribute: "
          + pathref);
        return null;
      }
      ...
      if (!fname.equals(nodeFname)) {
        this.log.warn("fname '" + nodeFname
          + "' on layout node not match specified fname '" + fname
          + "' for pathref: " + pathref);
        return null;
      }
    }
    ...
    if (structIdAttr != null) {
      ...
      if (isStructRef) {
        return new Noderef(layoutOwnerUserId, 1 /*
  ...
  /**
   * Returns a valid {@link Pathref} based on the specified arguments or <code>null</null> if that's not possible.
   * 
   ...
   */
  public Pathref getPathrefFromNoderef(String layoutOwnerUsername,
    String dlmNoderef, org.dom4j.Element layout) {

    Validate.notNull(layoutOwnerUsername,
      "Argument 'layoutOwnerUsername' cannot be null.");
    Validate.notNull(dlmNoderef, "Argument 'dlmNoderef' cannot be null.");

    if (log.isTraceEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg.append("createPathref: [layoutOwnerUsername='")
        .append(layoutOwnerUsername).append("', dlmNoderef='").append(dlmNoderef)
        .append("']");
      log.trace(msg.toString());
    }
    ...
    if (dlmNodeMatcher.matches()) {
      ...
      if (userLayoutInfo.second == null) {
        this.log.warn("no layout for fragment user '" + userLayoutInfo.first
          + "' Specified dlmNoderef " + dlmNoderef + " cannot be resolved.");
        return null;
      }

      final Document fragmentLayout = userLayoutInfo.second.getLayout();
      final Node targetElement = this.xPathOperations.evaluate("//*[@ID = $nodeId]",
        Collections.singletonMap("nodeId", nodeId), fragmentLayout,
        XPathConstants.NODE);
  ...
  /**
   * Provides a {@link Tuple} containing the &quot;fragmentized&quot; version of
   * a DLM fragment owner's layout, together with the username. This version of
   * the layout consistent with what DLM uses internally for fragments, and is
   * created by FragmentActivator.fragmentizeLayout. It's important that the
   * version returned by this method matches what DLM uses internally because it
   * will be used to establish relationships between fragment layout nodes and
   * user customizations of DLM fragments.
   * 
   * @param userId
   * @return
   */
  /* TODO: make private */Tuple<String, DistributedUserLayout> getUserLayoutTuple(
    ...
    final IUserProfile profile = layoutStore.getUserProfileByFname(person,
      UserProfile.DEFAULT_PROFILE_FNAME);
    final DistributedUserLayout userLayout = layoutStore.getUserLayout(person,
      (UserProfile) profile);
