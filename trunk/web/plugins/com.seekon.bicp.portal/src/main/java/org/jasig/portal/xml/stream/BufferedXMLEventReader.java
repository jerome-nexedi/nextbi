...
public class BufferedXMLEventReader extends BaseXMLEventReader {
  ...
  /**
   * Create new buffering reader, no buffering is done until {@link #mark(int)}
   * is called.
   */
  ...
  /**
   * Create new buffering reader. Calls {@link #mark(int)} with the specified
   * event limit
   * 
   * @see #mark(int)
   */
  ...
  protected XMLEvent internalNextEvent() throws XMLStreamException {
    ...
    if (this.bufferReader != null) {
      ...
      // If nothing left in the iterator, remove the reference and fall through
      // to direct reading
  ...
  public void mark(int eventLimit) {
    ...
    // Buffering limited set of events, lets trim the buffer if needed
      ...
      // If there is an iterator check its current position and calculate the
      // new iterator start position
      int iteratorIndex = 0;
      if (this.bufferReader != null) {
        final int nextIndex = this.bufferReader.nextIndex();
        iteratorIndex = Math.max(0, nextIndex
          - (this.eventBuffer.size() - this.eventLimit));
      }
  ...
  /**
   * If reading from the buffer after a {@link #reset()} call an
