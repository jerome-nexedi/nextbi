...
/**
 * Applies and updates position specifiers for child nodes in the composite
 * layout.
 * 
...
public class PositionManager {
   ...
   * according to the order specified in the passed in position set and return
   * via the passed in result set whether the personal layout fragment (one
   * portion of which is the position set) or the incoporated layouts fragment
   * (one portion of which is the compViewParent) were changed.
   * 
   * This may also include pulling nodes in from other parents under certain
   * circumstances. For example, if allowed a user can move nodes that are not
   * part of their personal layout fragment or PLF; the UI elements that they
   * own. These node do not exist in their layout in the database but instead
   * are merged in with their owned elements at log in and other times. So to
   * move them during subsequent merges a position set can contain a position
   * directive indicating the id of the node to be moved into a specific
   * position in the sibling list and that well may refer to a node not in the
   * sibling list to begin with. If the node no longer exists in the composite
   * view then that position directive can safely be discarded.
   * 
   * Positioning is meant to preserve as much as possible the user's specified
   * ordering of user interface elements but always respecting movement
   * restrictions placed on those elements that are incorporated by their
   * owners. So the following rules apply from most important to least.
   * 
   * 1) nodes with moveAllowed="false" prevent nodes of lower precedence from
   * being to their left or higher with left or higher defined as having a lower
   * index in the sibling list. (applyLowerPrecRestriction)
   * 
   * 2) nodes with moveAllowed="false" prevent nodes of equal precedence from
   * moving from one side of this node to the other from their position as found
   * in the compViewParent initially and prevents nodes with the same precedence
   * from moving from other parents into this parent prior to the restricted
   * node. Prior to implies a lower sibling index. (applyHoppingRestriction)
   * 
   * 3) nodes with moveAllowed="false" prevent nodes of equal precedence lower
   * in the sibling list from being reparented. (ie: moving to another parent)
   * However, they can be deleted. (applyReparentingCheck)
   * 
   * 4) nodes should be ordered as much as possible in the order specified by
   * the user but in view of the above conditions. So if a user has moved nodes
   * thus specifying some order and the owner of some node in that set then
   * locks one of those nodes some of those nodes will have to move back to
   * their orinial positions to conform with the rules above but for the
   * remaining nodes they should be found in the same relative order specified
   * by the user. (getOrder)
   * 
   * 5) nodes not included in the order specified by the user (ie: nodes added
   * since the user last ordered them) should maintain their relative order as
   * much as possible and be appended to the end of the sibling list after all
   * others rules have been applied. (getOrder)
   * 
   * Each of these rules is applied by a call to a method 5 being first and 1
   * being last so that 1 has the highest precedence and last say. Once the
   * final ordering is specified then it is applied to the children of the
   * compViewParent and returned.
   */
   ...
   * restrictions ended up making changes to the compViewParent or the original
   * position set. If changes are applicable to the CVP then they are applied.
   * If the position set changed then the original stored in the PLF is updated.
   */
  static void evaluateAndApply(ArrayList order, Element compViewParent,
    Element positionSet, IntegrationResult result) throws PortalException {
  ...
  /**
   * This method trims down the position set to the position directives on the
   * node info elements still having a position directive. Any directives that
   * violated restrictions were removed from the node info objects so the
   * position set should be made to match the order of those still having one.
   */
  static void adjustPositionSet(ArrayList order, Element positionSet,
    IntegrationResult result) {
    Node nodeToMatch = positionSet.getFirstChild();
    Element nodeToInsertBefore = positionSet.getOwnerDocument().createElement(
      "INSERT_POINT");
  ...
  /**
   * This method compares the children by id in the order list with the order in
   * the compViewParent's ui visible children and returns true if the ordering
   * differs indicating that the positioning if needed.
   */
  static boolean hasAffectOnCVP(ArrayList order, Element compViewParent) {
    ...
    while (child != null) {
      if (child.getAttribute("hidden").equals("false")
        && (!child.getAttribute("chanID").equals("") || child.getAttribute("type")
          .equals("regular"))) {
  ...
  /**
   * This method applies the ordering specified in the passed in order list to
   * the child nodes of the compViewParent. Nodes specified in the list but
   * located elsewhere are pulled in.
   */
  ...
  /**
   * This method is responsible for preventing nodes with lower precedence from
   * being located to the left (lower sibling order) of nodes having a higher
   * precedence and moveAllowed="false".
   */
  static void applyLowerPrecedence(ArrayList order, Element compViewParent,
    Element positionSet) {
    for (int i = 0; i < order.size(); i++) {
      ...
      if (ni.node.getAttribute(Constants.ATT_MOVE_ALLOWED).equals("false")) {
        for (int j = 0; j < i; j++) {
          ...
          if (lefty.precedence == null || lefty.precedence.isLessThan(ni.precedence)) {
            order.remove(j);
            order.add(i, lefty);
          }
        }
  ...
  /**
   * This method is responsible for preventing nodes with identical precedence
   * in the same parent from hopping over each other so that a layout fragment
   * can lock two tabs that are next to each other and they can only be
   * separated by tabs with higher precedence.
   * 
   * If this situation is detected then the positioning of all nodes currently
   * in the compViewParent is left as they are found in the CVP with any nodes
   * brought in from other parents appended at the end with their relative order
   * preserved.
   */
  static void applyNoHopping(ArrayList order, Element compViewParent,
    Element positionSet) {
  ...
  /**
   * This method determines if any illegal hopping is being specified. To
   * determine if the positioning is specifying an ordering that will result in
   * hopping I need to determine for each node n in the list if any of the nodes
   * to be positioned to its right currently lie to its left in the CVP and have
   * moveAllowed="false" and have the same precedence or if any of the nodes to
   * be positioned to its left currently lie to its right in the CVP and have
  ...
  static boolean isIllegalHoppingSpecified(ArrayList order) {
    for (int i = 0; i < order.size(); i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (niSib.precedence.isEqualTo(ni.precedence) && (niSib.indexInCVP == -1 || // from another parent
   ...
   * those that are not in the passed in compViewParent. For those it then looks
   * in its current parent and checks to see if there are any down- stream
   * (higher sibling index) siblings that have moveAllowed="false". If any such
   * sibling is found then the node is not allowed to be reparented and is
   * removed from the list.
   */
  static void applyNoReparenting(ArrayList order, Element compViewParent,
    Element positionSet) {
    int i = 0;
    while (i < order.size()) {
      NodeInfo ni = (NodeInfo) order.get(i);
  ...
  private static boolean isNotReparentable(NodeInfo ni) {
    ...
    while (node != null) {
      if (node.getAttribute(Constants.ATT_MOVE_ALLOWED).equals("false")) {
        Precedence p = Precedence.newInstance(node
          .getAttribute(Constants.ATT_FRAGMENT));
   ...
   * nodes still exist in the composite view and then by any remaining children
   * in the compViewParent.
   */
  static void applyOrdering(ArrayList order, Element compViewParent,
    Element positionSet) {
  ...
  /**
   * This method updates the positions recorded in a position set to reflect the
   * ids of the nodes in the composite view of the layout. Any position nodes
   * already in existence are reused to reduce database interaction needed to
   * generate a new ID attribute. If any are left over after updating those
   * position elements are removed. If no position set existed a new one is
   * created for the parent. If no ILF nodes are found in the parent node then
   * the position set as a whole is reclaimed.
   */
   ...
   * passed in plfParent or if not found it will create one automatically and
   * return it if the passed in create flag is true.
   */
  private static Element getPositionSet(Element plfParent, IPerson person,
    boolean create) throws PortalException {
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new position set node " + "Id for userId=" + person.getID(), e);
    }
  ...
  /**
   * Create, append to the passed in position set, and return a position element
   * that references the passed in elementID.
   */
  private static Element createAndAppendPosition(String elementID,
    Element positions, IPerson person) throws PortalException {
    if (LOG.isDebugEnabled())
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new position node " + "Id for userId=" + person.getID(), e);
    }
  ...
  static class NodeInfoComparator implements Comparator {
