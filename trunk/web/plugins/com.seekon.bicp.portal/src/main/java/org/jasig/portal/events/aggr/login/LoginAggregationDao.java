...
/**
 * DAO used to query information about login aggregates: Total Logins and Unique
 * Logins per date,time,interval,group
 * 
...
public interface LoginAggregationDao<T extends LoginAggregation> {
   ...
   */
  List<LoginAggregation> getLoginAggregations(DateMidnight start, DateMidnight end,
   ...
   */
  Set<T> getLoginAggregationsForInterval(DateDimension dateDimension,
    TimeDimension timeDimension, AggregationInterval interval);

  LoginAggregation getLoginAggregation(DateDimension dateDimension,
    TimeDimension timeDimension, AggregationInterval interval,
