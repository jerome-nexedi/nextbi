...
/**
 * After each request processed by a portlet the portlets session (if one
 * exists) is stored in a Map in the Portal's session. When a portal session is
 * invalidated the {@link PortletSession#invalidate()} method is called on all
 * portlet sessions in the Map.
 * 
...
public class PortletSessionExpirationManager implements PortletInvocationListener,
  ApplicationListener<HttpSessionDestroyedEvent> {
  public static final String PORTLET_SESSIONS_MAP = PortletSessionExpirationManager.class
    .getName()
    + ".PORTLET_SESSIONS";
   ...
   * 
   * @see
   * org.apache.pluto.spi.optional.PortletInvocationListener#onEnd(org.apache
   * .pluto.spi.optional.PortletInvocationEvent)
   */
    ...
    if (portalSession != null) {
      ...
      synchronized (WebUtils.getSessionMutex(portalSession)) {
        ...
        if (portletSessions == null || !portletSessions.isValid()) {
          portletSessions = new NonSerializableMapHolder(
  ...
  public void onApplicationEvent(HttpSessionDestroyedEvent event) {
    ...
    final Map<String, PortletSession> portletSessions = (Map<String, PortletSession>) session
      .getAttribute(PORTLET_SESSIONS_MAP);
    if (portletSessions == null) {
      return;
    }

    for (final Map.Entry<String, PortletSession> portletSessionEntry : portletSessions
      .entrySet()) {
      ...
      try {
        ...
        this.logger.info("PortletSession with id '" + portletSession.getId()
          + "' for context '" + contextPath + "' has already been invalidated.");
      } catch (Exception e) {
        this.logger.warn("Failed to invalidate PortletSession with id '"
          + portletSession.getId() + "' for context '" + contextPath + "'", e);
      }
    }
  }
   ...
   * 
   * @see
   * org.apache.pluto.spi.optional.PortletInvocationListener#onBegin(org.apache
   * .pluto.spi.optional.PortletInvocationEvent)
   */
   ...
   * 
   * @see
   * org.apache.pluto.spi.optional.PortletInvocationListener#onError(org.apache
   * .pluto.spi.optional.PortletInvocationEvent, java.lang.Throwable)
   */
  ...
  /**
   * Map implementation that holds the Map reference passed into the constructor
   * in a transient field. This allows a Map of non-serializable objects to be
   * stored in the session but skipped during session persistence.
   */
  private static final class NonSerializableMapHolder<K, V> implements Map<K, V>,
    Serializable {
