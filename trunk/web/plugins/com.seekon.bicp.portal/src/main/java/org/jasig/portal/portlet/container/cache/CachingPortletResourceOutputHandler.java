...
/**
 * Extention of {@link CachingPortletOutputHandler} that adds support for
 * resource requests
 * 
...
public class CachingPortletResourceOutputHandler extends CachingPortletOutputHandler
  ...
  public <T extends Serializable> CachedPortletResourceData<T> getCachedPortletResourceData(
    ...
    final CachedPortletData<T> cachedPortletData = super.getCachedPortletData(
      portletResult, cacheControl);
    if (cachedPortletData == null) {
      // Hit the caching limit, nothing to return
      return null;
    }

    return new CachedPortletResourceData<T>(cachedPortletData, headers, status,
      characterEncoding, contentLength, locale);
  }
