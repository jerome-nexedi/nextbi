...
public class EntityNameFinderService {
  ...
  public IEntityNameFinder getNameFinder(Class type) throws GroupsException {
    ...
    if (finder == null) {
      throw new GroupsException("Name finder for " + type.getName()
  ...
  /**
   * Gets all the entity types and tries to instantiate and cache a finder for
   * each one. There needn't be a finder for every entity type, so if there's no
   * entry in the portal.properties, we just log the fact and continue.
   */
  private synchronized void initialize() {
    ...
    while (types.hasNext()) {
      ...
      if (type != Object.class) {
        String factoryKey = "org.jasig.portal.services.EntityNameFinderService.NameFinderFactory.implementation_"
          + type.getName();
        try {
        ...
        if (factoryName != null) {
          try {
            IEntityNameFinderFactory factory = (IEntityNameFinderFactory) Class
