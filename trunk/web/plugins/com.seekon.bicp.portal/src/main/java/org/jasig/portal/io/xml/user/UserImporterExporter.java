...
public class UserImporterExporter extends AbstractJaxbDataHandler<UserType> {
  ...
  private static final ImmutableSet<PortalDataKey> IMPORT_DATA_KEYS = ImmutableSet
    .of(UserPortalDataType.IMPORT_40_DATA_KEY,
  ...
  public Iterable<? extends IPortalData> getPortalData() {
    final List<String> userList = this.jdbcOperations.queryForList(
      "SELECT USER_NAME FROM UP_USER", String.class);
  ...
  public void importData(UserType userType) {
    ...
    final Integer defaultUserId = defaultUserInfo != null ? defaultUserInfo.first
      : null;

    // Update or Insert
    final int rowsUpdated = this.jdbcOperations.update("UPDATE UP_USER \n"
      + "SET USER_DFLT_USR_ID = ?, USER_DFLT_LAY_ID=1, NEXT_STRUCT_ID=? \n"
      + "WHERE USER_NAME = ?", defaultUserId, nextStructId, username);
      ...
      this.jdbcOperations
        .update(
          "INSERT INTO UP_USER(USER_ID, USER_DFLT_USR_ID, USER_DFLT_LAY_ID, NEXT_STRUCT_ID, USER_NAME) \n"
            + "VALUES(?, ?, 1, ?, ?)", userId, defaultUserId, nextStructId, username);
    }
  ...
  protected Long getNextStructId(final String username,
    final Tuple<Integer, Long> defaultUserInfo) {
    final List<Long> maxStructIdResults = this.jdbcOperations.queryForList(
      "SELECT MAX(UPLS.STRUCT_ID) AS MAX_STRUCT_ID " + "FROM UP_USER UPU "
        + "    LEFT JOIN UP_LAYOUT_STRUCT UPLS ON UPU.USER_ID = UPLS.USER_ID "
        + "WHERE UPU.USER_NAME = ?", Long.class, username);
  ...
  protected Tuple<Integer, Long> getDefaultUserInfo(String defaultUsername) {
    ...
    final List<Tuple<Integer, Long>> defaultUserInfoResults = this.jdbcOperations
      .query("SELECT USER_ID, NEXT_STRUCT_ID " + "FROM UP_USER "
        + "WHERE USER_NAME = ?", new RowMapper<Tuple<Integer, Long>>() {
        @Override
        public Tuple<Integer, Long> mapRow(ResultSet rs, int rowNum)
          ...
          return new Tuple<Integer, Long>(userId, nullNextStructId ? null
            : nextStructId);
        }
      }, defaultUsername);
    try {
      ...
      throw new RuntimeException("No user data found for default-user: "
        + defaultUsername, e);
    }
  }
  ...
  public UserType exportData(String userName) {
    ...
    final ILocalAccountPerson localAccountPerson = this.localAccountDao
      .getPerson(userName);
    if (localAccountPerson != null) {
      ...
      for (final Map.Entry<String, List<Object>> attributeEntry : localAccountPerson
  ...
  protected String getDefaultUserName(String userName) {
    final List<Integer> defaultUserIdResults = this.jdbcOperations.queryForList(
      "SELECT user_dflt_usr_id FROM up_user WHERE user_name = ?", Integer.class,
      userName);
    ...
    if (defaultUserId != null) {
      final List<String> defaultUserNameResults = this.jdbcOperations.queryForList(
        "SELECT user_name FROM up_user WHERE user_id = ?", String.class,
        defaultUserId);
