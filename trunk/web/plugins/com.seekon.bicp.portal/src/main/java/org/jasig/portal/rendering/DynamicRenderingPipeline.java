...
/**
 * Top level class that initiates rendering via a
 * {@link CharacterPipelineComponent}
 * 
...
public class DynamicRenderingPipeline implements IPortalRenderingPipeline {
  ...
  /**
   * The root element in the rendering pipeline. This element MUST only return
   * {@link CharacterEventTypes#CHARACTER} type events.
   */
  ...
  public void renderState(HttpServletRequest req, HttpServletResponse res)
    ...
    final PipelineEventReader<CharacterEventReader, CharacterEvent> pipelineEventReader = this.pipeline
      .getEventReader(req, res);
    ...
    for (final CharacterEvent event : pipelineEventReader) {
      if (CharacterEventTypes.CHARACTER != event.getEventType()) {
        throw new RenderingPipelineConfigurationException("Only "
          + CharacterEventTypes.CHARACTER
          + " events are supported in the top level renderer. "
          + event.getEventType() + " is not supported.");
      }
    ...
    final IPortalRequestInfo portalRequestInfo = this.urlSyntaxProvider
      .getPortalRequestInfo(req);
    this.portalEventFactory.publishPortalRenderEvent(req, this, req.getPathInfo(),
      executionTime, portalRequestInfo);
  }

  /**
   * Determine the media type to use for the response
   */
  protected String getMediaType(HttpServletRequest req, HttpServletResponse res,
    PipelineEventReader<CharacterEventReader, CharacterEvent> pipelineEventReader) {
    final String mediaType = pipelineEventReader
    ...
    this.logger
      .warn("No mediaType was specified in the pipeline output properties, defaulting to "
        + DEFAULT_MEDIA_TYPE);
