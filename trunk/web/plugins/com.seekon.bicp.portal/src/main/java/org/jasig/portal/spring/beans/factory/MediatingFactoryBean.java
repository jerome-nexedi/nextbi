...
/**
 * Takes a list of bean names and attempts to load each in order. If the loading
 * of a bean throws an Exception it is logged and the next bean name in the list
 * is tried. If no beans can be loaded a {@link BeanCreationException} is
 * thrown.
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public class MediatingFactoryBean<T> extends AbstractFactoryBean<T> implements
  BeanFactoryAware {
  ...
  /**
   * @param delegateBeanNames
   *          the delegateBeanNames to set
   */
  ...
  public void afterPropertiesSet() throws Exception {
    Assert
      .notEmpty(this.delegateBeanNames, "delegateBeanNames list cannot be empty");
    ...
    for (final String beanName : this.delegateBeanNames) {
      try {
        ...
        if (this.logger.isDebugEnabled()) {
          ...
          this.logger.info(msg + " " + e.getMessage()
