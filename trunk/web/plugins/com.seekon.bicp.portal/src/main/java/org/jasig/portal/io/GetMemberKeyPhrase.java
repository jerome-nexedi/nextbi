...
public class GetMemberKeyPhrase implements Phrase {
  ...
  public static final Reagent ELEMENT = new SimpleReagent("ELEMENT",
    "descendant-or-self::text()", ReagentType.PHRASE, Element.class,
  ...
  public Object evaluate(TaskRequest req, TaskResponse res) {
    ...
    try {
      ...
      if (e.getName().equals("channel")) {
        IPortletDefinition def = PortletDefinitionRegistryLocator
          .getPortletDefinitionRegistry().getPortletDefinitionByFname(e.getText());
      ...
      for (int i = 0; i < leafTypes.length && rslt == null; i++) {
        EntityIdentifier[] eis = GroupService.searchForGroups(e.getText(),
          IGroupConstants.IS, leafTypes[i]);
        if (eis.length == 1) {
          // Match!
