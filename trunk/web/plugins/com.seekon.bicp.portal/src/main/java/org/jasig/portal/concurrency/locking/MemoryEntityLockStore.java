...
public class MemoryEntityLockStore implements IEntityLockStore {
  ...
  /**
   * Returns an IEntityLock[] based on the params, any or all of which may be
   * null. A null param means any value, so
   * <code>find(myType,myKey,null,null,null)</code> will return all
   * <code>IEntityLocks</code> for myType and myKey.
   * 
  ...
  public IEntityLock[] find(Class entityType, String entityKey, Integer lockType,
    java.util.Date expiration, String lockOwner) throws LockingException {
   ...
   * will return all <code>IEntityLocks</code> for myType and myKey unexpired as
   * of <code>expir</code>.
   * 
   * @param expiration
   *          Date
   ...
   *              - wraps an Exception specific to the store.
   */
  public IEntityLock[] findUnexpired(java.util.Date expiration, Class entityType,
  ...
  */
  ...
  /**
   * Make sure the store has a reference to the lock, and then add the lock to
   * refresh the SmartCache wrapper.
   * 
   * @param lock
   *          org.jasig.portal.concurrency.locking.IEntityLock
   * @param newExpiration
  ...
  public void update(IEntityLock lock, java.util.Date newExpiration,
    ...
    if (find(lock) == null) {
      throw new LockingException("Problem updating " + lock
        + " : not found in store.");
    }
