...
public class ReferenceEntityLockService implements IEntityLockService {
  ...
  /*
   * Fudge factor in milliseconds, extends the apparent expiration times of
   * potentially conflicting locks beyond their actual expirations. We only use
   * it when checking for locking conflicts and then only if inMemory == false.
   * Defaults to 5000.
   */
  ...
  public void convert(IEntityLock lock, int newType, int newDuration)
    ...
    if (lock.getLockType() == newType) {
      throw new LockingException("Could not convert " + lock
        + " : old and new lock TYPEs are the same.");
    }

    if (!isValidLockType(newType)) {
      throw new LockingException("Could not convert " + lock + " : lock TYPE "
        + newType + " is invalid.");
    }
    ...
    if (newType == WRITE_LOCK
      ...
      throw new LockingException("Could not convert " + lock
        + " : another lock already exists.");
    }

    if (newType == READ_LOCK) { /* Can always convert to READ */
    }
  ...
  public boolean existsInStore(IEntityLock lock) throws LockingException {
    ...
    IEntityLock[] lockArray = getLockStore().find(entityType, key, lockType,
      expiration, owner);
  ...
  private void initialize() throws LockingException {
    ...
    try {
      multiServer = PropertiesManager.getPropertyAsBoolean(
        "org.jasig.portal.concurrency.multiServer", false);

      lockStore = (multiServer) ? RDBMEntityLockStore.singleton()
        : MemoryEntityLockStore.singleton();
    } catch (Exception e) {
      eMsg = "ReferenceEntityLockingService.initialize(): Failed to instantiate entity lock store. "
        + e;
      log.error(eMsg);
    ...
    try {
      int lockDuration = PropertiesManager
        .getPropertyAsInt("org.jasig.portal.concurrency.IEntityLockService.defaultLockDuration");
    ...
    if (multiServer) {
      try {
        int lockTolerance = PropertiesManager
          .getPropertyAsInt("org.jasig.portal.concurrency.clockTolerance");
   ...
   * @exception org.jasig.portal.concurrency.LockingException
   */
  private boolean isLocked(Class entityType, String entityKey)
    throws LockingException {
    return isLocked(entityType, entityKey, null);
  }

  /**
   * Answers if the entity represented by entityType and entityKey has one or
   * more locks. Param <code>lockType</code> can be null.
   * 
   * @param entityType
   * @param entityKey
   * @param lockType
  ...
  */
  ...
  /**
   * Returns a lock for the entity, lock type and owner if no conflicting locks
   * exist.
   * 
   * @param entityType
   * @param entityKey
  ...
  public IEntityLock newLock(Class entityType, String entityKey, int lockType,
    String owner) throws LockingException {
   ...
   * @exception LockingException
   * 
   *              Retrieves potentially conflicting locks and checks them before
   *              adding the new lock to the store. The add of a write lock will
   *              fail if any other lock exists for the entity. The add of a
   *              read lock will fail if a write lock exists for the entity.
   *              After we add a write lock we check the store a second time and
   *              roll back if any other lock has snuck in. I think this is
   *              slightly safer than depending on the db isolation level for
   *              transactional integrity.
   */
  public IEntityLock newLock(Class entityType, String entityKey, int lockType,
    String owner, int durationSecs) throws LockingException {
    int expirationSecs = durationSecs;
    Date expires = getNewExpiration(expirationSecs);
    IEntityLock newLock = new EntityLockImpl(entityType, entityKey, lockType,
      expires, owner, this);
      ...
      for (int i = 0; i < locks.length; i++) {
        if (locks[i].getLockType() == WRITE_LOCK) {
          ...
          if (locks[i].equals(newLock)) {
            ...
            newLock = new EntityLockImpl(entityType, entityKey, lockType, expires,
              owner, this);
          }
        }
      }
  ...
  public IEntityLock newLock(EntityIdentifier entityID, int lockType, String owner)
    throws LockingException {
    return newLock(entityID.getType(), entityID.getKey(), lockType, owner,
      defaultLockPeriod);
  }
   ...
   */
  public IEntityLock newLock(EntityIdentifier entityID, int lockType, String owner,
    int durationSecs) throws LockingException {
    return newLock(entityID.getType(), entityID.getKey(), lockType, owner,
      durationSecs);
  }
  ...
  /**
   * Returns an IEntityLock[] containing unexpired locks for the entityType,
   * entityKey and lockType. Param <code>lockType</code> can be null.
   * 
   * @param entityType
   * @param entityKey
   * @param lockType
   ...
   * @exception LockingException
   */
  private IEntityLock[] retrieveLocks(Class entityType, String entityKey,
    Integer lockType) throws LockingException {
    Date expiration = (multiServer) ? new Date(System.currentTimeMillis()
      - getLockToleranceMillis()) : new Date();

    return getLockStore().findUnexpired(expiration, entityType, entityKey, lockType,
      null);
  }

  /**
   * @param newDefaultLockPeriod
   *          int
   */
