...
public class LPAChangeParameter implements ILayoutProcessingAction {
  ...
  /**
   * Look in the passed-in element for a parameter child element whose value is
   * to be changed to the passed-in value.
   * 
   * @param node
  ...
  static void changeParameterChild(Element node, String name, String value) {
    if (node != null) {
      ...
      for (Element parm = (Element) node.getFirstChild(); parm != null; parm = (Element) parm
        .getNextSibling()) {
