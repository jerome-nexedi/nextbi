...
/**
 * Extension of SmartApplicationListener that allows configuration of a set of
 * supported event classes
 * 
 * extend GenericApplicationListenerAdapter
 */
public class ClassFilteringApplicationListener<E extends ApplicationEvent>
  implements ApplicationEventFilter<E>, InitializingBean {
  protected final Logger logger = LoggerFactory.getLogger(this.getClass());

  private final Map<Class<? extends ApplicationEvent>, Boolean> supportedEventsCache = new MapMaker()
    .weakKeys().makeMap();
  ...
  /**
   * If no <code>supportedEvents</code> {@link Collection} is configured all
   * {@link ApplicationEvent} sub-classes are supported otherwise matching is
   * done. The property defaults to null (all event types)
   * 
   * @param supportedEvents
   *          the supportedEvents to set
   */
  public void setSupportedEvents(
    Collection<Class<? extends ApplicationEvent>> supportedEvents) {
    this.supportedEventsCache.clear();
    if (supportedEvents == null) {
      this.supportedEvents = null;
    } else {
      this.supportedEvents = new LinkedHashSet<Class<? extends ApplicationEvent>>(
        supportedEvents);
    }
  }
  ...
  public void afterPropertiesSet() throws Exception {
    // Figure out the generic type's Class, cribbed from
    // SmartApplicationListener
    Class<?> typeArg = GenericTypeResolver.resolveTypeArgument(this.getClass(),
      ApplicationListener.class);
    if (typeArg == null || typeArg.equals(ApplicationEvent.class)) {
      Class<?> targetClass = AopUtils.getTargetClass(this);
      if (targetClass != this.getClass()) {
        typeArg = GenericTypeResolver.resolveTypeArgument(targetClass,
          ApplicationListener.class);
      }
    }
