...
/**
 * Looks for, applies against the ilf, and updates accordingly the delete set
 * within a plf.
 * 
...
public class DeleteManager {
   ...
   * removing any that fail from the delete set so that the delete set is self
   * cleaning.
   */
  static void applyAndUpdateDeleteSet(Document plf, Document ilf,
  ...
  /**
   * Attempt to apply a single delete command and return true if it succeeds or
   * false otherwise. If the delete is disallowed or the target element no
   * longer exists in the document the delete command fails and returns false.
   */
  ...
  /**
   * Get the delete set if any stored in the root of the document or create it
   * is passed in create flag is true.
   */
  private static Element getDeleteSet(Document plf, IPerson person, boolean create)
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new delete set node " + "Id for userId=" + person.getID(), e);
    }
  ...
  /**
   * Create and append a delete directive to delete the node identified by the
   * passed in element id. If this node contains any incorporated elements then
   * they must also have a delete directive added in here to prevent
   * incorporated channels originating in another column from reappearing in
   * that column because the position set entry that pulled them into this
   * column was now removed. (ie: the user moved an inc'd channel to this column
   * and then deleted the column means that the inc'd channel should be deleted
   * also.) This was designed to add a delete directive for each nested element
   * having an ID so as to work for the future case of a tree view.
   */
  public static void addDeleteDirective(Element compViewNode, String elementID,
   ...
   * deleted as well.
   */
  private static void addDeleteDirective(Element compViewNode, String elementID,
    IPerson person, Document plf, Element delSet) throws PortalException {
    String ID = null;
    ...
    try {
      ...
      throw new PortalException("Exception encountered while "
        + "generating new delete node " + "Id for userId=" + person.getID(), e);
    }
