...
/**
 * Provides access to actions involving the PortletSession. Refer to
 * {@link SessionAction} for supported actions.
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public class PortletSessionAdministrativeRequestListener implements
  ...
  public void administer(PortletRequest request, PortletResponse response) {
    ...
    if (argumentCount != action.getArgumentCount()) {
      throw new IllegalArgumentException("SessionAction " + action + " requires "
        + action.getArgumentCount() + " arguments but " + argumentCount
        + " arguments were provided.");
    }

    // Get the session according to the action
    final PortletSession portletSession = request.getPortletSession(action
      .isRequiresCreation());

    switch (action) {
    case CLEAR: {
      if (portletSession != null) {
        for (final Enumeration<String> attributeNames = (Enumeration<String>) portletSession
          .getAttributeNames(scope); attributeNames.hasMoreElements();) {
