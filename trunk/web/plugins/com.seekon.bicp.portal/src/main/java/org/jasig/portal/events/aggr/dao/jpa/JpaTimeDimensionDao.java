...
public class JpaTimeDimensionDao extends BaseJpaDao implements TimeDimensionDao {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    this.findAllTimeDimensionsQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<TimeDimensionImpl>>() {
        @Override
        public CriteriaQuery<TimeDimensionImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<TimeDimensionImpl> criteriaQuery = cb
            .createQuery(TimeDimensionImpl.class);
          final Root<TimeDimensionImpl> dimensionRoot = criteriaQuery
            .from(TimeDimensionImpl.class);
    ...
    this.findTimeDimensionByHourMinuteQuery = this
      .createCriteriaQuery(new Function<CriteriaBuilder, CriteriaQuery<TimeDimensionImpl>>() {
        @Override
        public CriteriaQuery<TimeDimensionImpl> apply(CriteriaBuilder cb) {
          final CriteriaQuery<TimeDimensionImpl> criteriaQuery = cb
          ...
          final Root<TimeDimensionImpl> dimensionRoot = criteriaQuery
            .from(TimeDimensionImpl.class);
          criteriaQuery.select(dimensionRoot);
          criteriaQuery.where(cb.equal(dimensionRoot.get(TimeDimensionImpl_.time),
            timeParameter));
  ...
  public List<TimeDimension> getTimeDimensions() {
    final TypedQuery<TimeDimensionImpl> query = this
      .createCachedQuery(this.findAllTimeDimensionsQuery);
  ...
  public TimeDimension getTimeDimensionById(long key) {
    final TimeDimension timeDimension = this.entityManager.find(
      TimeDimensionImpl.class, key);
  ...
  public TimeDimension getTimeDimensionByTime(LocalTime localTime) {
    final TypedQuery<TimeDimensionImpl> query = this
      .createCachedQuery(this.findTimeDimensionByHourMinuteQuery);
    query
      .setParameter(this.timeParameter, localTime.minuteOfHour().roundFloorCopy());
