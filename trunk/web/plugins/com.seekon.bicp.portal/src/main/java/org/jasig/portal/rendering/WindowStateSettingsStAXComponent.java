...
/**
 * Enforces a specific WindowState based on the "dashboardForcedWindowState"
 * {@link IStylesheetDescriptor} parameter. For renderings where a specific
 * portlet is not specified and the stylesheet descriptor specifies a
 * dashboardForcedWindowState then all portlets in the pipeline will have their
 * window state set to the specified value. If the IStylesheetDescriptor is not
 * specified a check is made to ensure the window states are not
 * {@link WindowState#MAXIMIZED}, {@link IPortletRenderer#DETACHED}, or
...
public class WindowStateSettingsStAXComponent extends StAXPipelineComponentWrapper {
  ...
  public void setStylesheetAttributeSource(
    StylesheetAttributeSource stylesheetAttributeSource) {
   ...
   * 
   * @see
   * org.jasig.portal.rendering.PipelineComponent#getCacheKey(javax.servlet.
   ...
   * 
   * @see
   * org.jasig.portal.rendering.PipelineComponent#getEventReader(javax.servlet
   * .http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
   */
  @Override
  public PipelineEventReader<XMLEventReader, XMLEvent> getEventReader(
    ...
    final IStylesheetDescriptor stylesheetDescriptor = stylesheetAttributeSource
      .getStylesheetDescriptor(request);
    ...
    if (portalRequestInfo.getTargetedPortletWindowId() == null) {
      ...
      if (defaultWindowStateParam != null) {
        ...
        final WindowState windowState = PortletUtils
          .getWindowState(defaultWindowStateParam.getDefaultValue());
        filteredEventReader = new SinglePortletWindowStateSettingXMLEventReader(
          request, eventReader, windowState);
      } else {
        // Make sure there aren't any portlets in a "targeted" window state
        filteredEventReader = new NonTargetedPortletWindowStateSettingXMLEventReader(
          request, eventReader);
      }
    } else {
    ...
    final Map<String, String> outputProperties = pipelineEventReader
      .getOutputProperties();
    return new PipelineEventReaderImpl<XMLEventReader, XMLEvent>(
      filteredEventReader, outputProperties);
  }

  private abstract class PortletWindowStateSettingXMLEventReader extends
    FilteringXMLEventReader {
    private final HttpServletRequest request;

    public PortletWindowStateSettingXMLEventReader(HttpServletRequest request,
    ...
    protected final XMLEvent filterEvent(XMLEvent event, boolean peek) {
      if (event.isStartElement()) {
        ...
        if (IUserLayoutManager.CHANNEL.equals(localName)
          ...
          final Tuple<IPortletWindow, StartElement> portletWindowAndElement = portletWindowRegistry
            .getPortletWindow(request, startElement);
          ...
          if (!windowState.equals(currentWindowState)) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug("Changing WindowState from {} to {} for {}",
  ...
  private class SinglePortletWindowStateSettingXMLEventReader extends
    PortletWindowStateSettingXMLEventReader {
    private final WindowState state;

    public SinglePortletWindowStateSettingXMLEventReader(HttpServletRequest request,
      XMLEventReader reader, WindowState state) {
  ...
  private class NonTargetedPortletWindowStateSettingXMLEventReader extends
    PortletWindowStateSettingXMLEventReader {
