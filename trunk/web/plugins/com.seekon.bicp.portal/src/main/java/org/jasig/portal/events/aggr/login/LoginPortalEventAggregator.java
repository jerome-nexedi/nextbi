...
/**
 * Event aggregator that uses {@link LoginAggregationPrivateDao} to aggregate
 * login events
 * 
 ...
 */
@Service
public class LoginPortalEventAggregator implements
  IPortalEventAggregator<LoginEvent> {
  ...
  public void aggregateEvent(LoginEvent e, EventSession eventSession,
    ...
    for (Map.Entry<AggregationInterval, AggregationIntervalInfo> intervalInfoEntry : currentIntervals
      .entrySet()) {
      ...
      final Set<LoginAggregationImpl> loginAggregations = this.loginAggregationDao
        .getLoginAggregationsForInterval(dateDimension, timeDimension, interval);
      for (final LoginAggregationImpl loginAggregation : loginAggregations) {
        // Remove the aggregation from the group set to mark that it has been
        // updated
      ...
      if (!groupMappings.isEmpty()) {
        for (final AggregatedGroupMapping aggregatedGroup : groupMappings) {
          final LoginAggregationImpl loginAggregation = loginAggregationDao
            .createLoginAggregation(dateDimension, timeDimension, interval,
              aggregatedGroup);
          updateAggregation(e, intervalInfo, loginAggregation);
        }
      }
  ...
  private void updateAggregation(LoginEvent e,
    final AggregationIntervalInfo intervalInfo,
    final LoginAggregationImpl loginAggregation) {
  ...
  @Transactional("aggrEvents")
  @Override
  public void handleIntervalBoundary(AggregationInterval interval,
