...
/**
 * Base implementation of layout attribute source that feeds off of
 * {@link IStylesheetDescriptor} and {@link IStylesheetUserPreferences} data
 * 
 * @author Eric Dalquist
 * @version $Revision$
 */
public abstract class StylesheetAttributeSource implements AttributeSource,
  BeanNameAware {
  ...
  public final Iterator<Attribute> getAdditionalAttributes(
    ...
    final IStylesheetDescriptor stylesheetDescriptor = this
      .getStylesheetDescriptor(request);

    final PreferencesScope stylesheetPreferencesScope = this
      .getStylesheetPreferencesScope(request);
    ...
    for (final ILayoutAttributeDescriptor layoutAttributeDescriptor : stylesheetDescriptor
      ...
      final Set<String> targetElementNames = layoutAttributeDescriptor
        .getTargetElementNames();
      ...
      if (targetElementNames.contains(localEventName)) {
        final Attribute subscribeIdAttr = event
        ...
        String value = this.stylesheetUserPreferencesService.getLayoutAttribute(
  ...
  public final CacheKey getCacheKey(HttpServletRequest request,
    ...
    final CacheKeyBuilder<String, String> cacheKeyBuilder = CacheKey
      .builder(this.name);

    final Iterable<String> layoutAttributeNodeIds = this.stylesheetUserPreferencesService
      .getAllLayoutAttributeNodeIds(request, stylesheetPreferencesScope);
    for (final String nodeId : layoutAttributeNodeIds) {
      cacheKeyBuilder.add(nodeId);
      this.stylesheetUserPreferencesService.populateLayoutAttributes(request,
        stylesheetPreferencesScope, nodeId, cacheKeyBuilder);
    }
  ...
  public abstract PreferencesScope getStylesheetPreferencesScope(
