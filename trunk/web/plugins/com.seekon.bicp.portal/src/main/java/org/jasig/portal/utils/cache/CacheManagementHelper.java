...
public class CacheManagementHelper {
  private static final class CaseInsenstivieStringComparator implements
    Comparator<String> {
  ...
  public Map<String, CacheStatistics> getAllCacheStatistics() {
    final Map<String, CacheStatistics> allCacheStatistics = new TreeMap<String, CacheStatistics>(
      CaseInsenstivieStringComparator.INSTANCE);
    ...
    for (final String cacheName : this.cacheManager.getCacheNames()) {
      ...
      if (null != cache && Status.STATUS_ALIVE.equals(cache.getStatus())) {
        ...
        cacheStatistics.maxSize = cacheConfiguration.getMaxElementsInMemory()
   ...
   * @param cacheName
   * @return the {@link Statistics} for the specified cache; returns null of
   *         cache is not alive or doesn't exist
   */
  ...
  /**
   * Call {@link Cache#removeAll()} on the specified cache, if it exists and is
   * alive.
   * 
   * @see Status#STATUS_ALIVE
   * @see Cache#getStatus()
