...
/**
 * Implementation of {@link XPathOperations} that uses a
 * {@link GenericKeyedObjectPool} to pool compiled {@link XPathExpression}
 * instances.
 * 
...
public class XPathPoolImpl implements XPathOperations, DisposableBean {
  ...
  public XPathPoolImpl(NamespaceContext namespaceContext) {
    ...
    this.xpathExpressionfactory = new XPathExpressionFactory(this.namespaceContext,
      variableResolver);
  ...
  public <T> T doWithExpression(String expression,
    Function<XPathExpression, T> callback) {
    return this.<T> doWithExpression(expression, null, callback);
  }

  @Override
  public <T> T doWithExpression(String expression, Map<String, ?> variables,
    Function<XPathExpression, T> callback) {
    try {
      final XPathExpression xPathExpression = (XPathExpression) this.pool
        .borrowObject(expression);
      try {
      ...
      throw new IllegalStateException("Exception of type " + e.getClass().getName()
        + " is not expected", e);
    }
  }
  ...
  @Override
  public <T> T evaluate(String expression, Map<String, ?> variables,
    final Object item, final QName returnType) {
    return this.doWithExpression(expression, variables,
      new Function<XPathExpression, T>() {
        ...
        public T apply(XPathExpression xpathExpression) {
          try {
            ...
            throw new RuntimeException("Failed to execute XPathExpression '"
              + xpathExpression + "'", e);
          }
        }
      });
