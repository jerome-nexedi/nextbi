...
public class TableFormatter {
  public static final class TableEntry<T> {
    ...
    public String toString() {
      return "TableEntry [flags=" + flags + ", conversion=" + conversion
        + ", value=" + value + "]";
    }
  }
  ...
  public TableFormatter(Formatter f, TableEntry<?> firstHeading,
    TableEntry<?>... headings) {
    Validate.notNull(firstHeading);

    // Setup scratch objects used for formatting data
    this.scratchBuilder = new StringBuilder();
    this.scratchFormatter = new Formatter(scratchBuilder, f != null ? f.locale()
      : null);
  ...
  public void addRow(TableEntry<?> firstValue, TableEntry<?>... values) {
    ...
    if (1 + values.length != this.columnWidths.size()) {
      throw new IllegalArgumentException("Inconsistent column count. Expected "
        + this.columnWidths.size() + " but was " + (1 + values.length));
    }

    final List<TableEntry<?>> row = new ArrayList<TableEntry<?>>(this.columnWidths
      .size());
  ...
  private void updateColumnSize(int index, TableEntry<?> value) {
    ...
    if (this.columnWidths.size() == index) {
      ...
      this.columnWidths.set(index, Math.max(this.columnWidths.get(index),
        valueLength));
    }
  }
