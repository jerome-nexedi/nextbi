 ...
 * <code>entityKey</code>. It guarantees some degree of exclusive access to the
 * entity, depending on <code>lockType</code> and <code>expirationTime</code>.
 * 
...
public class EntityLockImpl implements IEntityLock {
  ...
  *
  */
  protected EntityLockImpl(Class newEntityType, String newEntityKey,
    int newLockType, Date newExpirationTime, String newLockOwner) {
    super();
  ...
  */
  ...
  /**
   * This method is supported primarily for hash tables, such as those provided
   * in java.util.
   * 
   ...
   */
  public int hashCode() {
    return getEntityType().hashCode() + getEntityKey().hashCode()
      + getLockOwner().hashCode();
  }
  ...
  public String toString() {
    return "EntityLockImpl for " + getEntityType().getName() + "(" + getEntityKey()
      + ")" + " type " + getLockType() + " owner " + getLockOwner() + " "
      + getExpirationTime();
  }
}
