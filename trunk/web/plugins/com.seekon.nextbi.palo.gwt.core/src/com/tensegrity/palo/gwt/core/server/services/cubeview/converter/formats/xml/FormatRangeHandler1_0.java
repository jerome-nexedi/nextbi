 ...
 * 
 * @author Philipp Bouillon
 * @version $Id: FormatRangeHandler1_0.java,v 1.4 2009/12/17 16:14:29
 *          PhilippBouillon Exp $
 **/
class FormatRangeHandler1_0 extends FormatRangeHandler {
  ...
  private FormatRangeInfo createFormatRange(String from, String to) {
    ...
    for (int i = 0; i < axes.length; i++) {
      if (!isRowOrColumn(axes[i]))
        fillCoordinateWithSelectedElements(coord, axes[i].getAxisHierarchies());
    }
  ...
  private final boolean isRowOrColumn(Axis axis) {
    ...
    return axisId.equals(CubeView.ROW_AXIS) || axisId.equals(CubeView.COLUMN_AXIS);
  }
  ...
  private final List<Element[]> fill(Element[] coord, int[] from, int[] to,
  ...
  private final void fill(Element[] coord, int elIndex, Hierarchy[] hierarchies,
    int hierIndex) {
    if (hierIndex < 0)
      return;
  ...
  private final void addRangeProperty(FormatRangeInfo range, String from, String to) {
