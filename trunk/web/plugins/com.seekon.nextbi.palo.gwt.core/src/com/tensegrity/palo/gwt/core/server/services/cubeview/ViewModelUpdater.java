...
/**
 * <code>ViewModelUpdater</code> TODO DOCUMENT ME
 * 
 * @version $Id: ViewModelUpdater.java,v 1.20 2010/03/11 10:42:20
 *          PhilippBouillon Exp $
 **/
public class ViewModelUpdater {

  static final void updateNative(ViewModel viewModel, XViewModel xViewModel,
    boolean updateLocalFilter) {
    updateNativeAxes(viewModel, xViewModel, true, true); // updateLocalFilter);
  }

  static final void updateNativeWithoutTable(ViewModel viewModel,
    XViewModel xViewModel) {
    updateNativeAxes(viewModel, xViewModel, false, true);
  }

  private static final void updateNativeAxes(ViewModel viewModel,
    XViewModel xViewModel, boolean withTable, boolean updateLocalFilter) {
    Map<String, AxisHierarchy> allAxisHierarchies = collectAxisHierarchies(viewModel);
    if (withTable) {
      updateNativeAxis(viewModel.getRowAxis(), xViewModel.getRowAxis(),
        allAxisHierarchies, !xViewModel.needsRestore());
      updateNativeAxis(viewModel.getColumnAxis(), xViewModel.getColumnAxis(),
        allAxisHierarchies, !xViewModel.needsRestore());
    }
    updateNativeAxis(viewModel.getSelectionAxis(), xViewModel.getSelectionAxis(),
      allAxisHierarchies, !xViewModel.needsRestore());
    updateNativeAxis(viewModel.getRepositoryAxis(), xViewModel.getRepositoryAxis(),
      allAxisHierarchies, !xViewModel.needsRestore());
    if (withTable) {
  ...
  private static final Map<String, AxisHierarchy> collectAxisHierarchies(
    ViewModel viewModel) {
  ...
  private static final void add(AxisHierarchy[] axisHierarchies,
    Map<String, AxisHierarchy> toCollection) {
    for (AxisHierarchy axisHierarchy : axisHierarchies)
      toCollection.put(axisHierarchy.getHierarchy().getId(), axisHierarchy);
  }

  private static final void updateNativeAxis(AxisModel axisModel, XAxis xAxis,
    Map<String, AxisHierarchy> allAxisHierarchies, boolean updateLocalFilter) {
    updateNativeAxis(axisModel.getAxis(), xAxis, allAxisHierarchies,
      updateLocalFilter);
    axisModel.refresh();
  }

  private static final void updateNativeAxis(Axis axis, XAxis xAxis,
    Map<String, AxisHierarchy> allAxisHierarchies, boolean updateLocalFilter) {
    axis.removeAll();
  ...
  private static final void updateExpandedItems(ViewModel viewModel,
    XViewModel xViewModel) {
  ...
  private static final Map<String, List<XAxisItem>> collectExpandedItems(
    XViewModel xViewModel) {
  ...
  private static final void addExpandedItemsOf(XAxis axis,
    Map<String, List<XAxisItem>> toMap) {
  ...
  private static final List<XAxisItem> getItemsList(String id,
    Map<String, List<XAxisItem>> fromMap) {
  ...
  private static final void updateExpandedItems(AxisModel inAxis,
    Map<String, List<XAxisItem>> items) {
    int index = 0;
    for (Hierarchy hierarchy : inAxis.getHierarchies()) {
      ...
      if (items.containsKey(hierarchyID)) {
        ...
        for (XAxisItem item : expandedItems) {
          ...
          try {
            ElementPath elementPath = ElementPath.restore(inAxis.getAxis()
              .getHierarchies(), item.getPath());
  ...
  private static final String modifyPath(String path, HashSet<String> ids,
    int axisIndex) {
    int size = ids.size();
  ...
  private static final void expandItem(String path, int index, AxisModel axis,
    String hierarchyId, HashSet<String> ids, int axisIndex) {
