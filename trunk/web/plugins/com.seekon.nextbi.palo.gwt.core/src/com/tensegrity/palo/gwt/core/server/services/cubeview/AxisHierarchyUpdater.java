...
/**
 * <code>AxisHierarchyUpdater</code> TODO DOCUMENT ME
 * 
 * @version $Id: AxisHierarchyUpdater.java,v 1.20 2010/04/15 09:54:49
 *          PhilippBouillon Exp $
 **/
public class AxisHierarchyUpdater {
  ...
  static XElement update(AxisHierarchy axisHierarchy, XAxisHierarchy xAxisHierarchy) {
    ...
    if (xAxisHierarchy.getSelectedElement() == null) {
      ...
      if (nodes != null && nodes.length > 0 && nodes[0] != null) {
        ...
        XElement x = new XElement(nodes[0].getElement().getId(), nodes[0]
          .getElement().getName(), XElementType.fromString(nodes[0].getElement()
          .getTypeAsString()));
    ...
    XElement elem = setAliasForSelectedElement(axisHierarchy, xAxisHierarchy
      .getSelectedElement());
    return elem;
  }
  ...
  private static void updateSubset(AxisHierarchy hierarchy, XSubset subset) {
    if (subset != null) {
      ...
      int type = subset.getSubsetType() == XSubsetType.LOCAL ? Subset2.TYPE_LOCAL
        : Subset2.TYPE_GLOBAL;
  ...
  private static final void updateSelectedElement(AxisHierarchy hierarchy,
    XElement selectedElement) {
    Element element = selectedElement == null ? null : hierarchy.getHierarchy()
  ...
  private static final XElement setAliasForSelectedElement(AxisHierarchy hierarchy,
    XElement element) {
    if (element != null) {
      ...
      if (p == null || p.getValue() == null) {
        ...
        String result = hierarchy.getHierarchy().getElementById(element.getId())
          .getName();
        if (name != null) {
  ...
  private static final void updateNativeProperties(AxisHierarchy axis,
    XAxisHierarchy xAxis) {
