...
/**
 * <code>ViewModelController2</code> TODO DOCUMENT ME
 * 
 * @version $Id: ViewModelController.java,v 1.20 2010/03/11 10:42:20
 *          PhilippBouillon Exp $
 **/
public class ViewModelController {
  ...
  private final void updateAxisItemStructure(ViewModel model) {
        ...
        if (item.hasChildren() && item.hasState(AxisItem.EXPANDED)) {
          for (AxisItem child : item.getChildren()) {
            ...
            if (item.getParentInPrevHierarchy() != null
              && child.getParentInPrevHierarchy() == null) {
    ...
    AxisItem[] items;
    if (model.getRowAxis() != null
      && (items = model.getRowAxis().getRoots()) != null) {
      traverser1.traverseVisible(items, visitor1);
    }
    if (model.getColumnAxis() != null
      && (items = model.getColumnAxis().getRoots()) != null) {
      traverser1.traverseVisible(items, visitor1);
    }
  }
  ...
  public final void proceedSwapAxes() {
    ...
    String[] rowExpanded = ((AxisTreeModel) viewModel.getRowAxis())
      .getExpandedNodes();
    String[] colExpanded = ((AxisTreeModel) viewModel.getColumnAxis())
      .getExpandedNodes();
  ...
  private final void addAll(ElementNode[] roots,
    HashMap<ElementNode, Integer> parentId, ArrayList<String> result,
    HashMap<Element, String> origNodes) {
  ...
  public final String[] hideItem(XAxisItem xItem, List<XAxisItem> xItemRoots,
          ...
          if (it.isExpanded) {
            ...
            if (parent == null) {
              ...
              for (ElementNode k : kids) {
                ElementNode c = new ElementNode(k.getElement(),
                  k.getConsolidation(), k.getIndex());
  ...
  private final int willExpand(XAxisItem xItem, AxisItem item, AxisModel axis) {
    ...
    if (isFirstExpand) {
      ...
      ViewModelUtils.determineLeafIndexes(axis); // TODO this should be done by
      // AxisModel!!!!
  ...
  private final void calculateLeafs(boolean expandedColumn) {
    AxisModel otherAxis = expandedColumn ? viewModel.getRowAxis() : viewModel
      .getColumnAxis();
    ...
    if (expandedItems != null && expandedItems.length > 0) {
      ...
      // expanded cells:
      rowLeafs = expandedColumn ? ViewModelUtils.getVisibleLeafs(otherAxis)
        : ViewModelUtils.getVisibleLeafs(roots);
      columnLeafs = expandedColumn ? ViewModelUtils.getVisibleLeafs(roots)
        : ViewModelUtils.getVisibleLeafs(otherAxis);
    }
  }
  ...
  /**
   * Proceeds with expand action, i.e. marks all cells as loaded which need to
   * be loaded due to this expand
   * 
  ...
  /**
   * Cancels a former started expand action, i.e. the expanded item is collapsed
   * again and unloaded if it was loaded during expand
   */
  ...
  private final void undo(Expanded expand) {
    ...
    if (expand.firstTime()) {
      ...
      AxisItem item = expand.getItem(); // getAxisItem(expand.getXItem(),
      // expand.getAxis());
      if (item != null) {
  ...
  // public final int willSetExpandState(String axisId, XAxisItem[] rootItems,
  // int expandDepth) {
  public final int willSetExpandState(XAxisItem[] expanded, XAxisItem[] collapsed,
    String axisId, int expandDepth) {
  ...
  private final int willExpand(XAxisItem xItem, AxisItem item, AxisModel axis,
    ...
    if (item != null && item.getLevel() < expandDepth && item.hasChildren()) {
      ...
      if (xItem != null && xItem.isLoaded)
        return cells; // when item is loaded its unloaded children are expanded
      // by another call of willExpand()!!!
      for (AxisItem child : item.getChildren())
...
class Expanded {
  ...
  Expanded(XAxisItem xItem, AxisItem item, boolean firstTime, int insertIndex,
    AxisModel axis, boolean column) {
    this.axis = axis;
    this.item = item;
    this.xItem = xItem;
    this.column = column;
