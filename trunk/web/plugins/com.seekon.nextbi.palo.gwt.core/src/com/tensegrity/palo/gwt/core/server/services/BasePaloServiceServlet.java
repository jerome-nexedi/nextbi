...
/**
 * <code>CorePaloService</code> TODO DOCUMENT ME
 * 
 * @version $Id: BasePaloServiceServlet.java,v 1.27 2010/04/13 09:45:58
 *          PhilippBouillon Exp $
 **/
public abstract class BasePaloServiceServlet extends RemoteServiceServlet {
  // MAIN SERVLET. PROVIDES PER SESSION DATA AND GLOBAL CONTEXT DATA, LIKE
  // CONNECTION POOLS
  ...
  static final String SESSION_USER = BasePaloServiceServlet.class.getName()
    + "authuser";

  // GLOBAL DATA ATTRIBUTES:
  protected static final String SQL_CONNECTION = BasePaloServiceServlet.class
    .getName()
    + "sqlconnection";

  private ISessionListener sessionListener;

  private static final HashMap<String, String> userBinding = new HashMap<String, String>();

  private static final HashMap<String, Long> lastAccessTime = new HashMap<String, Long>();

  private static final HashMap<String, Integer> maxInactiveTime = new HashMap<String, Integer>();

  private static final HashMap<String, UserSession> sessionMap = new HashMap<String, UserSession>();
  ...
  private final synchronized void checkUserSessionTimeout(String sessionId,
    HttpSession session, UserSession userSession) throws SessionExpiredException {
    ...
    if (!maxInactiveTime.containsKey(sessionId)) {
      ...
      // System.err.println("Setting session timeout for session [" + sessionId
    ...
    if (deltaTime > maxInactive) {
      System.err.println("Session [" + sessionId + "] timed out after " + deltaTime
        + " inactive seconds. (Timeout was: " + maxInactive + "s)");
  ...
  public final synchronized UserSession getUserSession(String id)
  ...
  // validateCookie(userBinding.getUserSession().getUser(),
  // userBinding.getUserSession().getId());
  // lastAccessTime.put(userBinding.getUserSession().getId(),
  ...
  public final AuthUser getLoggedInUser(String sessionId)
    throws SessionExpiredException {
    UserSession userSession = getUserSession(sessionId);
    return userSession.getUser();
  }

  protected final void overrideLoggedInUser(String sessionId, AuthUser user)
    throws SessionExpiredException {
    UserSession userSession = getUserSession(sessionId);
    userSession.setUser(user);
  }

  public final NumberFormat getNumberFormat(String sessionId)
    throws SessionExpiredException {
    UserSession userSession = getUserSession(sessionId);
  ...
  protected final UserSession setLoggedInUser(AuthUser user) {
    HttpSession session = getThreadLocalRequest().getSession();// //startNewSession();
    UserSession userSession = new UserSession(user);
    updateCookies(user.getLoginName(), "", userSession.getSessionId());
    // session.setMaxInactiveInterval(30);
    // System.err.println("!!!IMPORTANT -- REMOVE BEFORE BUILD (BasePaloServiceServlet) Session Timeout: "
    // + session.getMaxInactiveInterval() + "s");
    bindUserToSession(userSession, session);
  ...
  protected final void setNumberFormat(String sessionId, AuthUser user,
    NumberFormat format) {
    try {
  ...
  private final void displayCookie(String msg) {
    ...
    System.err.println("[" + msg + "]" + " cookie: " + loginName + ", " + password
      + ", " + id);
  }
  ...
  protected Object[] getAuthUserAndIdFromCookies() {
    ...
    System.err.println("Examining cookie: " + loginName + ", " + password + ", "
      + id);

    try {
      return new Object[] {
        ServiceProvider.getAuthenticationService().authenticateHash(loginName,
          password), id };
