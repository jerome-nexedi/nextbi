...
public class JavaScriptMinificationTag extends BodyTagSupport {
  ...
  protected final ErrorReporter jsErrorReporter = new CommonsLogErrorReporter(
    this.log);
    ...
    if (isCompressionEnabled()) {
      ...
      try {
        final JavaScriptCompressor jsCompressor = new JavaScriptCompressor(
          bodyReader, this.jsErrorReporter);
        jsCompressor.compress(out, this.lineBreakColumnNumber, this.obfuscate,
          false, this.preserveAllSemiColons, this.disableOptimizations);

        scriptWritten = true;
      } catch (EvaluatorException e) {
        log.warn(
          "Failed to parse JS data to minify, falling back to non-minified JS.", e);
        bc.clearBody();
      } catch (IOException e) {
        log.warn(
          "Failed to read or write JS data, falling back to non-minified JS.", e);
        bc.clearBody();
      }
    }
  ...
  protected boolean isCompressionEnabled() {
    ...
    final ResourcesElementsProvider resourcesElementsProvider = ResourcesElementsProviderUtils
