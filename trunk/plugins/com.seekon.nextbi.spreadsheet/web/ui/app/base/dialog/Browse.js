Jedox.wss.dialog.Browse = function(OSHflag, closeWinFnc, preselectMode) {
	var that = this;
	var title;
	var btn;
	var typeCmb;
	...
	var folderTypeData = [ [ "My Workbook Documents".localize(),
			"my_workbook_documents" ] ];
	var folderTypeNavigationStore = new Ext.data.SimpleStore( {
		fields : [ "label", "icon" ],
		data : folderTypeData
	});
	var fileTypes = [ [ "0", "Work Sheet Files".localize() ],
			[ "1", "All Files".localize() ] ];
				...
				if (that.activeGroup != record.data) {
					var wssStudioHandler = {
						treeSetGroup : function(result) {
							if (!result) {
								that
										.showMessageERROR("Database read error"
												.localize(), "read_data_err"
												.localize());
								return false
							}
								...
								if (node.attributes.type) {
									if (that.activeHierarchy.id != node.attributes.id) {
										var wssStudioStub = new Studio(
												wssStudioHandler);
										wssStudioStub.treeSetHierarchy("wss",
												node.attributes.id);
										_nodeClick_cb = true;
	...
	that.hierarchyTP.on("click", function(n) {
		var sn = that.hierarchyTP.getSelectionModel().getSelectedNode() || {};
		if (n.id != sn.id && n.id != "root") {
			if (n.attributes.type) {
				initFilesData(n.attributes.type)
			} else {
				initFilesData(n.attributes.n_uid)
			}
		} else {
			if (n.id == "root") {
				clear()
	...
	var mainListView = new Ext.DataView(
			{
				...
				tpl : new Ext.XTemplate(
						...
						'<img class="{img_src}" src="../lib/ext/resources/images/default/s.gif" width="16" height="16"></div>',
				...
				listeners : {
					dblclick : function(dataView, index, node, e) {
						onDblClick(dataView.store.getAt(index).get("id"),
					...
					click : function(dataView, index, node, e) {
						...
						if (that.fStore.getAt(index).get("iconCls") != "w3s_folder") {
							...
							that.selectionId = that.fStore.getAt(index).get(
									"id");
							if (!isClicked(value)) {
								fileNameData.unshift(new Array());
								fileNameData[0].push(value);
	...
	var mainDetailsView = new Ext.grid.GridPanel( {
		...
		listeners : {
			rowdblclick : function(gridView, index, e) {
				onDblClick(gridView.store.getAt(index).get("id"),
	...
	function setView(m, pressed) {
		...
		if (pressed) {
			...
			switch (m.text) {
			case "List".localize():
				mvp.layout.setActiveItem("main-list-view");
	...
	function initFilesData(node) {
		var wssStudioHandler = {
			treeMngNode : function(result) {
				if (!result) {
					that.showMessageERROR("Database read error".localize(),
							"read_data_err".localize());
					return false
				}
				var fData = [];
				for ( var i = 0; i < result.length; i++) {
					fData.push( [ result[i].id, result[i].text, result[i].leaf,
							result[i].qtip, result[i].iconCls,
							img_src(result[i].iconCls) ])
				}
	...
	function onDblClick(id, iconCls) {
		if (iconCls === "w3s_folder") {
			...
			if (!that._isHyperlink) {
				that.openWB(id)
			}
		}
	}
	function propagateSelection(n_uid) {
		var id = that.hierarchyTP.getNodeById(n_uid) ? n_uid
				: that.activeHierarchy.id + "_" + n_uid;
		that.hierarchyTP.fireEvent("select", id)
	}
	function formatTitle(value, p, record) {
		return String.format(
				'<div class="topic"><img src="{0}"/>&nbsp;&nbsp;{1}</div>',
				record.data.img_src, value)
	}
	function filterFunc(record, id) {
		return record.get("iconCls") === "w3s_workbook"
				|| record.get("iconCls") === "w3s_folder" ? true : false
	}
	...
	function remove() {
		...
		Jedox.wss.backend.studio.treeMngNode("file", parentId, "removeNode",
				that.selectionId);
		initFilesData(parentId)
	}
	Jedox.wss.dialog.Browse.superclass.constructor.call(this, {
		...
		items : [ {
			...
			x : 0,
			y : 260,
			items : [ that.fileNameCmb, typeCmb ]
		} ]
	});
...
Ext
		.extend(
				Jedox.wss.dialog.Browse,
				Ext.Panel,
				{
					initComponent : function() {
						var that = this;
						Jedox.wss.dialog.Browse.superclass.initComponent
								.call(this);
						this.onOpen = function() {
							var name = that.fileNameCmb.getValue();
							var id = that.getIdByName(name);
							that.openWB(id)
						};
						this.onSave = function(cb) {
							...
							var fileName = that.trim(that.fileNameCmb
									.getValue());
							var s = fileName.toLowerCase().indexOf(".wss");
							if (s > 0 && s === fileName.length - 4) {
								fileName = that.trim(fileName.slice(0,
										length - 4))
							}
							if (that.validateName(fileName)) {
								that.closeFnc();
								var oldMeta = Jedox.wss.workspace
										.getMetaByWinId(Jedox.wss.app.activeBook
												.getWinId());
								try {
									for ( var triggers = Jedox.wss.events.triggers.saveAsWorkbook_before, i = triggers.length - 1, wbMeta = oldMeta; i >= 0; i--) {
										triggers[i][0]["saveAsWorkbook_before"]
								...
								var saveRes = Jedox.wss.backend.ha
										.saveWorkbook(group, hierarchy,
												fileName, parentId);
								if (!saveRes[0]) {
									Ext.MessageBox.show( {
										title : "Save As".localize(),
									...
									var activeBook = Jedox.wss.app.activeBook, newWbMeta = {
										g : saveRes[1].group,
										h : saveRes[1].hierarchy,
										n : saveRes[1].node,
										p : saveRes[1].perm
									};
									activeBook.setWinCaption(fileName);
									Jedox.wss.workspace.updateMeta(activeBook
											.getWinId(), fileName, newWbMeta);
									activeBook.setWbId(saveRes[1].wbid);
									Jedox.wss.action.adjustToACL();
									try {
										for ( var triggers = Jedox.wss.events.triggers.saveAsWorkbook_after, i = triggers.length - 1, wbMeta = newWbMeta; i >= 0; i--) {
											triggers[i][0]["saveAsWorkbook_after"]
													.call(parent,
															triggers[i][1], {
																g : wbMeta.g,
																h : wbMeta.h,
						...
						this.onInsertHyperlink = function() {
							var g = that.activeGroup.id;
							var h = that.activeHierarchy.id;
							var n = that.selectionId;
							if (g && h && n) {
								var result = Jedox.wss.backend.studio
										.getNodeHyperlinkPropertiesData(g, h, n);
								if (result) {
									return result
								} else {
									return false
						...
						this.preselectPath = function(ghn) {
							that
									.initGroups(function(result) {
										if (result) {
											for ( var i = 0, count = that.gStore
													.getCount(); i < count; i++) {
												if (that.gStore.getAt(i).get(
														"id") == ghn.g) {
													break
												}
											}
					...
					getRelativePath : function(id) {
						var parent = this.hierarchyTP.getNodeById(
								this.activeHierarchy.id + "_"
										+ this.getParentId()).getPath("text");
						var fileName = this.getFileName(this.fileNameCmb
								.getValue());
					...
					openWB : function(id) {
						...
						try {
							...
							for ( var triggers = Jedox.wss.events.triggers.openWorkbook_before, i = triggers.length - 1; i >= 0; i--) {
								triggers[i][0]["openWorkbook_before"].call(
										parent, triggers[i][1], {
											g : group,
											h : hierarchy,
											n : id
					...
					validateName : function(name) {
						...
						for ( var i = 0; i < that.fStore.getCount(); i++) {
							if (name === that.fStore.getAt(i).get("text")
									.toLowerCase()
									&& that.fStore.getAt(i).get("iconCls") == "w3s_workbook") {
								return false
							}
						}
						return true
					...
					initGroups : function(cb) {
						...
						var wssStudioHandler = {
							treeMngGroup : function(result) {
								if (!result) {
									that.showMessageERROR("Database read error"
											.localize(), "read_data_err"
											.localize());
									if (cb) {
										cb(false)
					...
					initHierarchies : function(group, cb) {
						var that = this;
						var nodes = [];
						this.hierarchyTP.root.eachChild(function(node) {
							nodes.push(node)
						}, [ this.hierarchyTP ]);
											...
											.appendChild(new Ext.tree.AsyncTreeNode(
													{
														...
														loader : new Ext.tree.TreeLoader(
																{
																	dataUrl : Jedox.wss.backend.studio.dispatcher.serverUrl
																			.concat(
																					"&wam=",
																					Jedox.wss.app.appModeS,
																					"&c=",
																					Jedox.wss.backend.studio.className,
																					"&m=treeDump"),
																	baseParams : {
																		type : "wss",
																		hierarchy : i,
								...
								that.hierarchyTP.root
										.expand(
												...
												function(gNode) {
													...
													if (that.hierarchyTP.root
															.hasChildNodes()) {
														var hNode;
														if (that.hierarchyTP.root
																.hasChildNodes()) {
															hNode = that.hierarchyTP
																	.getNodeById(Jedox.wss.app.defaultFiles.hierarchy)
																	|| that.hierarchyTP
																			.getNodeById(that.hierarchyTP.root.firstChild.attributes.id)
														} else {
															hNode = that.hierarchyTP
																	.getNodeById(that.hierarchyTP.root.attributes.id)
														}
														if (hNode) {
															hNode
																	.expand(
						...
						var wssStudioStub = new Studio(wssStudioHandler);
						wssStudioStub
								.treeMngHierarchy("wss", "listHierarchies")
					},
					selectNode : function(n) {
						...
						this.fileNameCmb.setValue(this.fStore.getAt(i).get(
								"text"))
					}
				});