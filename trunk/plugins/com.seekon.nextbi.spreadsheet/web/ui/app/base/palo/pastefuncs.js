Jedox.wss.palo.PasteDataFunctions = function(pv) {
	...
	var win, listCubes;
	var panelMain, panelbtnPaste, panelbtnClose, panelcmbDb, panelcmbPaloFuncs, panelchbAttrCubes, panelchbGuessArgs;
	var lblcmbDb, cmbDb, cmbPaloFuncs, chbAttrCubes, chbGuessArgs, btnPaste, btnClose;
	var selectedElem, cmbPaloFuncsState, cmbDbState, dataMode, numOfRequests;
	...
	this.init = function() {
		...
		dataMode = 0;
		numOfRequests = 0;
		var paloFuncList = [ [ "PALO.DATA" ], [ "PALO.DATAC" ],
				[ "PALO.DATAV" ] ];
			...
			listeners : {
				click : function() {
					if (listCubes.getSelectionCount() > 0) {
						var cube = listCubes.getSelectedRecords()[0]
								.get("name");
						if (Jedox.wss.app.activeBook) {
							...
							var lowerRighCoords = env.defaultSelection
									.getActiveRange().getLowerRight();
							var settings = [ upperLeftCoords.getX(),
									upperLeftCoords.getY(),
									lowerRighCoords.getX(),
									lowerRighCoords.getY(), servId, dbName,
									chbGuessArgs.getValue() ];
							activeBook.cb("palo_handlerPasteDataFunctions", [ [
									settings, cube ] ])
						} else {
							var settings = [ 5, 5, 10, 10, servId, dbName,
									chbGuessArgs.getValue() ];
							phpPaloServer.handlerPasteDataFunctions( [
									settings, cube ])
						}
					}
					win.close()
		...
		cmbPaloFuncs.setValue(storePaloFuncs.getAt(cmbPaloFuncsState).get(
				"name"));
			...
			listeners : {
				check : function() {
					...
					if (chbAttrCubes.getValue()) {
						...
						if (tmpType == "database") {
							phpPaloServer
									.getCubeNames(servId, dbName, dataMode)
						}
					} else {
						dataMode = 0;
						if (tmpType == "database") {
		...
		panelMain = new Ext.Panel( {
			...
			items : [ panellistCubes, lbllistCubes, panelbtnPaste,
					panelbtnClose, panelcmbDb, lblcmbDb, panelcmbPaloFuncs,
					panelchbAttrCubes, panelchbGuessArgs ]
		});
		win = new Ext.Window(
				{
					id : "pastedatafunctionsWizardWindow",
					layout : "fit",
					cls : "default-format-window",
					title : "Paste Data Functions".localize(),
					width : _config.pdfWinW,
					height : _config.pdfWinH,
					minWidth : _config.pdfWinW,
					minHeight : _config.pdfWinH,
					closeAction : "close",
					autoDestroy : true,
					plain : true,
					modal : true,
					...
					listeners : {
						...
						close : function() {
							Jedox.wss.app
									.unload(Jedox.wss.app.dynJSRegistry.pasteDataFunctions);
	...
	var _resizeAll = function() {
		if (panelMain.rendered) {
			...
			panelcmbPaloFuncs.setPosition(marginSize, 5 * marginSize + 3
					* lineH + listH);
			if (chbAttrCubes.rendered && chbGuessArgs.rendered) {
				var widthShow = Ext.util.TextMetrics.measure(chbAttrCubes.id,
						"Attribute Cubes".localize());
				panelchbAttrCubes.setPosition(w - widthShow.width - 35, 4
						* marginSize + 2 * lineH);
				var widthShow2 = Ext.util.TextMetrics.measure(chbGuessArgs.id,
						"Guess Arguments".localize());
				panelchbGuessArgs.setPosition(w - widthShow2.width - 35, 5
						* marginSize + 3 * lineH + listH + 2)
			}
			if (panelbtnClose.rendered && panelbtnPaste.rendered
				...
				panelbtnPaste.setPosition(w - btnClose.getEl().getBox().width
						- btnPaste.getEl().getBox().width - 2 * marginSize, h
						- lineH - marginSize)
			}
		}
	};
