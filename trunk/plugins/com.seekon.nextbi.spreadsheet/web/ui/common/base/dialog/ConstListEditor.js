var ConstListEditor = Ext
		.extend(
				Ext.Panel,
				{
					imgPath : "/ui/studio/res/img/rt/",
					cmnImgPath : "/ui/common/res/img/palo/",
					width : 350,
					height : 255,
					border : "1px solid",
					initComponent : function() {
						...
						var ConstListDataViewClass = Ext
								.extend(
										Ext.DataView,
										{
						...
						var ConstInlineEditor = function(cfg, field) {
							ConstInlineEditor.superclass.constructor.call(this,
										...
										Ext.Editor,
										{
											...
											onDblClick : function(e, target) {
												if (!this.disabled
														...
														&& !e.shiftKey) {
													var item = this.view
															.findItemFromChild(target);
													e.stopEvent();
													var record = this.view.store
															.getAt(this.view
																	.indexOf(item));
													this
															.startEdit(
						...
						this.setList = function(list) {
							...
							if (list
									&& (list.constructor.toString().indexOf(
											"Array") != -1)) {
						...
						var ciePlugin = new ConstInlineEditor(
								{
									...
									listeners : {
										beforecomplete : function(thisEd,
												value, startValue) {
											if (value && (value != "")) {
												thisEd.activeRecord
														.set(thisEd.dataIndex,
																value)
											} else {
												if (value == "") {
										...
										canceledit : function(thisEd, value,
												startValue) {
											if (startValue == "New Constant"
													.localize()) {
												thisEd.activeRecord.store
														.remove(thisEd.activeRecord)
											}
						...
						var topBtnHandler = function() {
							var storeSize = storeConsts.getCount();
							var numOfSelectedConsts = viewConsts
									.getSelectionCount();
							for ( var tmpRec, i = 0, j = 0; i < storeSize; i++) {
								if (viewConsts
										.isSelected(viewConsts.getNode(i))) {
						...
						var upBtnHandler = function() {
							var tmpElem;
							var tmpSelectedIds = viewConsts
									.getSelectedIndexes();
							var storeSize = storeConsts.getCount();
							for ( var i = 1; i < storeSize; i++) {
								if (viewConsts
										.isSelected(viewConsts.getNode(i))) {
									if (!viewConsts.isSelected(viewConsts
											.getNode(i - 1))) {
							...
							for ( var tmpNode, i = 0; i < storeSize; i++) {
								...
								if (viewConsts.isSelected(tmpNode)) {
									tmpNode.scrollIntoView(viewConsts.body,
											false);
									break
								}
							}
						...
						var panel_btnAcctions = new Ext.Panel(
								{
									...
									width : 31,
									autoHeight : true,
									items : [ btnAcctions.top, btnAcctions.up,
											btnAcctions.down,
											btnAcctions.bottom ]
								});
						var addConst = function() {
							...
							ciePlugin.startEdit(newNode,
									newRec.data[ciePlugin.dataIndex])
						};
							...
							editB : new Ext.Button(
									{
										...
										handler : function() {
											var sIndexes = viewConsts
													.getSelectedIndexes();
											if (sIndexes.length > 0) {
												var firstNode = viewConsts
														.getNode(sIndexes[0]);
												var scrollToElem = Ext
														.get(firstNode);
												firstNode.scrollIntoView(
														viewConsts.body, false);
												var firstRec = viewConsts
														.getRecord(firstNode);
						...
						viewConsts.on("selectionchange", function(thisDv,
								selections) {
