...
public final class HttpClient implements Serializable, DisposableBean {
  ...
  private static ExecutorService EXECUTOR_SERVICE = Executors
  ...
  public boolean sendMessageToEndPoint(final String url, final String message,
    final boolean async) {
    final Future<Boolean> result = EXECUTOR_SERVICE.submit(new MessageSender(url,
      message, this.readTimeout, this.connectionTimeout));
    ...
    try {
      ...
      if (responseCode == 500) {
        ...
        log.error(String.format(
          "There was an error contacting the endpoint: %s; The error was:\n%s", url
            .toExternalForm(), value));
      }
  ...
  private static final class MessageSender implements Callable<Boolean> {
    ...
    public MessageSender(final String url, final String message,
    ...
    public Boolean call() throws Exception {
      ...
      try {
        ...
        connection.setRequestProperty("Content-Length", Integer.toString(output
          .getBytes().length));
        ...
        log.warn("Error Sending message to url endpoint [" + url + "].  Error is ["
          + e.getMessage() + "]");
        return false;
      } finally {
        if (in != null) {
