...
public class Saml10SuccessResponseView extends AbstractCasView {
  ...
  protected void renderMergedOutputModel(final Map model,
    ...
    try {
      ...
      final Authentication authentication = assertion.getChainedAuthentications()
        .get(0);
      ...
      final SAMLResponse samlResponse = new SAMLResponse(null, service.getId(),
        new ArrayList<Object>(), null);
      final boolean isRemembered = (authentication.getAttributes().get(
        RememberMeCredentials.AUTHENTICATION_ATTRIBUTE_REMEMBER_ME) == Boolean.TRUE && !assertion
        .isFromNewLogin());
      ...
      samlAuthenticationStatement
        .setAuthMethod(authenticationMethod != null ? authenticationMethod
      ...
      if (!authentication.getPrincipal().getAttributes().isEmpty() || isRemembered) {
        ...
        for (final Entry<String, Object> e : authentication.getPrincipal()
