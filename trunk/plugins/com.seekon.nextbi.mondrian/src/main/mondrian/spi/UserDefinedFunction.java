 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/spi/UserDefinedFunction.java#11 $
 */
public interface UserDefinedFunction {
	/**
	 * Returns the name with which the user-defined function will be used from
	 * within MDX expressions.
	 */
	...
	/**
	 * Returns the syntactic type of the user-defined function. Usually
	 ...
	 * 
	 * @param evaluator
	 *          Evaluator containts the runtime context, in particular the current
	 *          member of each dimension.
	 * @param arguments
	 *          Expressions which yield the arguments of this function. Most
	 *          user-defined functions will evaluate all arguments before using
	...
	/**
	 * Returns a list of reserved words used by this function. May return an empty
	 * array or null if this function does not require any reserved words.
	 */
	...
	interface Argument {
		 ...
		 * 
		 * <p>
		 * The effect is the same as {@link #evaluate(mondrian.olap.Evaluator)}
		 * except if the argument evaluates to a member or tuple. This method will
		 * set the context to the member or tuple and evaluate the current measure,
		 * whereas {@code evaluate} would return the member or tuple.
		 * 
		 * <p>
		 ...
		 * 
		 * <p>
		 * Note how {@code [Measures].[Previous Period]} is defined as a tuple, but
		 ...
		 * 
		 * <p>
		 * If the argument is a set of members or tuples, this method may return
		 * either a {@link List} or an {@link Iterable}. It is not safe to blindly
		 * cast to {@code List}. For guaranteed type, call
