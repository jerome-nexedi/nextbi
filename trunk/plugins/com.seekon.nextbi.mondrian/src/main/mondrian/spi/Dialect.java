 ...
 * 
 * <p>
 * A dialect is instantiated via a {@link DialectFactory}.
 * </p>
 * 
 * <p>
 * In JDBC terms, a dialect is analogous to a {@link java.sql.Connection}, and a
 * dialect factory is analogous to a {@link java.sql.Driver}, in the sense that
 * the JDBC driver manager maintains a chain of registered drivers, and each
 * driver in turn is given the opportunity to create a connection that can
 * handle a particular JDBC connect string. For dialects, each registered
 * dialect factory is given the chance to create a dialect that matches a
 ...
 * 
 * <li>If there is no explicit factory, Mondrian requires that the class has a
 * public constructor that takes a {@link java.sql.Connection} as its sole
 * parameter, and the {@link DialectManager} creates an <em>implicit
 ...
 * 
 * <p>
 * Dialect factories can also be the means for caching or pooling dialects. See
 * {@link #allowsDialectSharing} and {@link mondrian.spi.DialectFactory} for
 * more details.
 * </p>
 * 
 ...
 * 
 * <p>
 * A dialect needs to be registered with the system in order to be used. Call
 * {@link mondrian.spi.DialectManager#register(DialectFactory)} to register a
 * dialect factory, or {@link mondrian.spi.DialectManager#register(Class)} to
 ...
 * <code>META-INF/services/mondrian.spi.Dialect</code>", containing the name of
 * your dialect class.</li>
 ...
 * 
 * <p>
 * To implement a dialect, write a class that implements the {@code Dialect}
 * interface. It is recommended that you subclass
 * {@link mondrian.spi.impl.JdbcDialectImpl}, to help to make your dialect is
 * forwards compatible, but it is not mandatory.
 * </p>
 * 
 * <p>
 * A dialects should be immutable. Mondrian assumes that dialects can safely be
 * shared between threads that use the same {@link java.sql.Connection JDBC
 * connection} without synchronization. If {@link #allowsDialectSharing()}
 * returns true, Mondrian may use the same dialect for different connections
 * from the same {@link javax.sql.DataSource JDBC data source}.
 * </p>
 * 
 * <p>
...
public interface Dialect {
   ...
   * 
   * <p>
   * For example, for MySQL, {@code toUpper("foo.bar")} returns {@code
   * "UPPER(foo.bar)"}.
   * </p>
   * 
   * @param expr
   *          SQL expression
   * 
   * @return SQL syntax that converts <code>expr</code> into upper case.
   */
   ...
   * 
   * <p>
   * For example, {@code caseWhenElse("b", "1", "0")} returns {@code
   * "case when b then 1 else 0 end"} on Oracle, {@code "Iif(b, 1, 0)"} on
   * Access.
   * 
   * @param cond
   ...
   * 
   * <p>
   * For example, <code>quoteIdentifier("emp")</code> yields a string containing
   * <code>"emp"</code> in Oracle, and a string containing <code>[emp]</code> in
   * Access.
   * 
   * @param val
   *          Identifier
   ...
   * 
   * @param val
   *          identifier to quote (must not be null).
   * @param buf
   *          Buffer
   */
   ...
   * double-quotes, <code>quoteIdentifier("schema","table")</code> yields a
   * string containing <code>"schema"."table"</code>.
   * 
   * @param qual
   *          Qualifier. If it is not null, <code>"<em>qual</em>".</code> is
   *          prepended.
   * @param name
   *          Name to be quoted.
   * @return Quoted identifier
  ...
  /**
   * Returns the character which is used to quote identifiers, or null if
   ...
   * <code>quoteStringLiteral(buf, "Can't")</code> appends " <code>'Can''t'</code>" to
   * <code>buf</code>.
   * 
   * @param buf
   *          Buffer to append to
   ...
   * <p>
   * For example, in the default dialect,
   * <code>quoteStringLiteral(buf, "1969-03-17")</code> appends <code>DATE '1969-03-17'</code>.
   * 
   * @param buf
   *          Buffer to append to
   * @param value
   ...
   * <p>
   * For example, in the default dialect,
   * <code>quoteStringLiteral(buf, "12:34:56")</code> appends <code>TIME '12:34:56'</code>.
   * 
   * @param buf
   *          Buffer to append to
   * @param value
   ...
   * <p>
   * For example, in the default dialect,
   * <code>quoteStringLiteral(buf, "1969-03-17 12:34:56")</code> appends <code>TIMESTAMP '1969-03-17 12:34:56'</code>.
   * 
   * @param buf
   *          Buffer to append to
   * @param value
   ...
   * 
   * <p>
   * For example, Access does not allow <blockquote>
   * <code>select count(distinct x) from t</code> </blockquote>
   * 
   ...
   * 
   * <p>
   * In Derby 10.1, <blockquote> <code>select couunt(distinct x) from t</code>
   * </blockquote> is OK, but <blockquote>
   ...
   * @return SQL string
   */
  String generateInline(List<String> columnNames, List<String> columnTypes,
   ...
   * (DDL) statements such as <code>CREATE TABLE</code> and
   ...
   * direction, and ensuring that NULL values collate either before or after all
   * non-NULL values, depending on the <code>collateNullsLast</code> parameter.
   * 
   * @param expr
   *          Expression
   * @param nullable
   ...
   * @param collateNullsLast
   *          Whether the null values should be sorted first or last.
   * 
  ...
  /**
   * Returns whether this Dialect allows the GROUPING SETS construct in the
   * GROUP BY clause. Currently Greenplum, IBM DB2, Oracle, and Teradata.
   * 
  ...
  /**
   * Returns whether this Dialect places no limit on the number of rows which
   * can appear as elements of an IN or VALUES expression.
   * 
   ...
   * 
   * <p>
   * For example, in such a dialect, <blockquote>
   * <code>SELECT x, x FROM t GROUP BY x</code> </blockquote> would be illegal,
   * but <blockquote> <code>SELECT x AS a, x AS b FROM t ORDER BY a, b</code>
   * </blockquote>
   * 
   * would be legal.
   * </p>
   ...
   *         only by adding an expression to the SELECT clause and using its
   *         alias
   */
   ...
   * <p>
   * For example, in such a dialect, <blockquote>
   * <code>SELECT x FROM t ORDER BY x + y</code> </blockquote> would be illegal,
   * but <blockquote> <code>SELECT x, x + y AS z FROM t ORDER BY z</code>
   * </blockquote>
   * 
   * would be legal.
   * </p>
   ...
   * </blockquote> would be illegal, but <blockquote>
   ...
   * <p>
   * For example, in such a dialect, <blockquote>
   * <code>SELECT x, x + y AS z FROM t ORDER BY z</code> </blockquote>
   * 
   * would be legal.
   * </p>
   * 
   * <p>
   * MySQL, DB2 and Ingres are examples of dialects where this is true; Access
   * is a dialect where this is false.
   * </p>
   * 
  ...
  /**
   * Returns true if this dialect allows only integers in the ORDER BY clause of
   * a UNION (or other set operation) query.
   * 
   * <p>
   * For example,
   * 
   ...
   * 
   * @return whether this dialect allows only integers in the ORDER BY clause of
   *         a UNION (or other set operation) query
   */
  boolean requiresUnionOrderByOrdinal();

  /**
   * Returns true if this dialect allows an expression in the ORDER BY clause of
   * a UNION (or other set operation) query only if it occurs in the SELECT
   * clause.
   * 
   * <p>
   * For example,
   ...
   * UNION ALL<br/>
   * SELECT x, y, z FROM t<br/>
   * ORDER BY y + z</code> <code>SELECT x, y, z FROM t ORDER BY y + z</code>
   * 
   * is not.
   * 
   * <p>
   ...
   * 
   * @return whether this dialect allows an expression in the ORDER BY clause of
   *         a UNION (or other set operation) query only if it occurs in the
   *         SELECT clause
   */
   ...
   * except that the JdbcOdbc bridge in JDK 1.6 overstates its abilities. See
   * bug 1690406.
   * 
   * @param type
   *          defined in {@link java.sql.ResultSet}
   * @param concurrency
   *          type defined in {@link java.sql.ResultSet}
   ...
   * {@link mondrian.spi.Dialect.DatabaseProduct#UNKNOWN} if the database is not
   * a common database.
   * 
   * @return Database
   */
   ...
   * after the FROM clause in a SELECT statement, based on any hints provided.
   * Any unrecognized or unsupported hints will be ignored.
   * 
   * @param buf
   *          The Stringbuffer to which the dialect-specific syntax for any
  ...
  /**
   * Returns whether this Dialect object can be used for all connections from
   * the same data source.
   * 
   * <p>
   * The default implementation returns {@code true}, and this allows dialects
   * to be cached and reused in environments where connections are allocated
   * from a pool based on the same data source.
   * </p>
   * 
   * <p>
   ...
   * </p>
   * 
   * @see mondrian.spi.DialectFactory#createDialect(javax.sql.DataSource,
   *      java.sql.Connection)
   * 
   ...
   * SQL standard allows this if the database can deduce that the expression is
   * functionally dependent on columns in the GROUP BY clause.
   * 
   * <p>
   * For example, {@code SELECT empno, first_name || ' ' || last_name FROM emps
   * GROUP BY empno} is valid because {@code empno} is the primary key of the
  ...
  /**
   * Returns whether this dialect supports "ANSI-style JOIN syntax", {@code FROM
   * leftTable JOIN rightTable ON conditon}.
   * 
  ...
  /**
   * Some databases, like Greenplum, don't include nulls as part of the results
   * of a COUNT sql call. This allows dialects to wrap the count expression in
   ...
   * might be a column identifier or some other identifier, but the
   * implementation must presume that it is already escaped and fit for use. The
   * regular expression is not escaped and must be adapted to the proper dialect
   * rules.
   * <p>
   ...
   * particular regular expression into something that the database would
   * support.
   * </p>
   * 
   * @param source
   ...
   * @return A dialect specific matching operation, or null if the dialect
   *         cannot convert that particular regular expression into something
   *         that the database would support.
   */
   ...
   * differently for different databases. However, since the capabilities of a
   * database can change between versions, it is recommended that conditional
   * code is in terms of capabilities methods in {@link mondrian.spi.Dialect}.
   * 
   * <p>
   * Because there are so many differences between various versions and ports of
   * DB2, we represent them as 3 separate products. If you want to treat them
   * all as one product, note that the {@link #getFamily()} method for
   * {@link #DB2_AS400} and {@link #DB2_OLD_AS400} returns {@link #DB2}.
   */
  enum DatabaseProduct {
    ACCESS, UNKNOWN, DERBY, DB2_OLD_AS400, DB2_AS400, DB2, FIREBIRD, GREENPLUM, HIVE, HSQLDB, INFORMIX, INFOBRIGHT, INGRES, INTERBASE, LUCIDDB, MSSQL, NETEZZA, NEOVIEW, ORACLE, POSTGRESQL, MYSQL, SQLSTREAM, SYBASE, TERADATA, VERTICA, VECTORWISE;

    /**
     * Return the root of the family of products this database product belongs
     * to.
     * 
     * <p>
     * For {@link #DB2_AS400} and {@link #DB2_OLD_AS400} returns {@link #DB2};
     * for all other database products, returns the same product.
     * 
  ...
  enum Datatype {
    ...
    /**
     * Appends to a buffer a value of this type, in the appropriate format for
     * this dialect.
     * 
     * @param buf
     *          Buffer
