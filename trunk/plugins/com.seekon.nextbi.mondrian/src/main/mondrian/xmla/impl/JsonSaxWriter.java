/*
// $Id: //open/mondrian/src/main/mondrian/xmla/impl/JsonSaxWriter.java#1 $
...
/**
 * Implementation of <code>SaxWriter</code> which, perversely, generates a JSON
 * (JavaScript Object Notation) document.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/xmla/impl/JsonSaxWriter.java#1 $
 */
class JsonSaxWriter implements SaxWriter {
	...
	public void startSequence(String name, String subName) {
		...
		if (stack.peek().name != null) {
			assert name.equals(stack.peek().name) : "In sequence [" + stack.peek()
					+ "], element name [" + name + "]";
			buf.append("[");
		} else {
	...
	/**
	 * Writes a value with appropriate quoting for a JavaScript constant of that
	 * type.
	 * 
	 * <p>
	 * Examples: {@code "a \"quoted\" string"} (string), {@code 12} (int),
	 * {@code 12.345} (float), {@code null} (null value).
	 * 
	 * @param value
	 *          Value
	 */
