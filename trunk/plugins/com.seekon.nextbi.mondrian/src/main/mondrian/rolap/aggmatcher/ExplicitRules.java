...
public class ExplicitRules {
	...
	public static ExplicitRules.TableDef getIncludeByTableDef(
			final String tableName, final List<Group> aggGroups) {
		for (Group group : aggGroups) {
	...
	private static Exclude make(final MondrianDef.AggExclude aggExclude) {
		return (aggExclude.getNameAttribute() != null) ? new ExcludeName(
				aggExclude.getNameAttribute(), aggExclude.isIgnoreCase())
	...
	public static abstract class TableDef {
		...
		private static void addTo(final ExplicitRules.TableDef tableDef,
			...
			addLevelTo(tableDef, aggLevel.getNameAttribute(),
					aggLevel.getColumnName());
		}

		private static void addTo(final ExplicitRules.TableDef tableDef,
				final MondrianDef.AggMeasure aggMeasure) {
			addMeasureTo(tableDef, aggMeasure.getNameAttribute(),
		...
		class Level {
			...
			public void validate(final MessageRecorder msgRecorder) {
				...
				try {
					...
					if (names.size() != 2) {
						msgRecorder.reportError(mres.BadLevelNameFormat.str(
								msgRecorder.getContext(), name));
					} else {
						RolapCube cube = ExplicitRules.TableDef.this.getCube();
						SchemaReader schemaReader = cube.getSchemaReader();
						RolapLevel level = (RolapLevel) schemaReader.lookupCompound(cube,
								names, false, Category.Level);
						if (level == null) {
							Hierarchy hierarchy = (Hierarchy) schemaReader.lookupCompound(
									cube, names.subList(0, 1), false, Category.Hierarchy);
							if (hierarchy == null) {
								msgRecorder.reportError(mres.UnknownHierarchyName.str(
										msgRecorder.getContext(), names.get(0).name));
							} else {
								msgRecorder.reportError(mres.UnknownLevelName.str(
										msgRecorder.getContext(), names.get(0).name,
										names.get(1).name));
							}
						}
		...
		class Measure {
			...
			public void validate(final MessageRecorder msgRecorder) {
				...
				try {
					...
					List<Id.Segment> names = Util.parseIdentifier(name);
					if (names.size() != 2) {
						msgRecorder.reportError(mres.BadMeasureNameFormat.str(
								msgRecorder.getContext(), name));
					} else {
						RolapCube cube = ExplicitRules.TableDef.this.getCube();
						SchemaReader schemaReader = cube.getSchemaReader();
						Member member = (Member) schemaReader.lookupCompound(cube, names,
								false, Category.Member);
						if (member == null) {
							if (!names.get(0).name.equals("Measures")) {
								msgRecorder.reportError(mres.BadMeasures.str(
										msgRecorder.getContext(), names.get(0).name));
							} else {
								msgRecorder.reportError(mres.UnknownMeasureName.str(
										msgRecorder.getContext(), names.get(1).name));
							}
						}
						...
						if (rolapMeasure == null) {
							msgRecorder.reportError(mres.BadMeasureName.str(
									msgRecorder.getContext(), names.get(1).name, cube.getName()));
						}
		...
		public boolean columnsOK(final RolapStar star,
			...
			Recognizer cb = new ExplicitRecognizer(this, star, getCube(),
					dbFactTable, dbTable, msgRecorder);
			return cb.check();
		}
			...
			try {
				...
				for (Level level : levels) {
					...
					if (namesToObjects.containsKey(level.getName())) {
						msgRecorder.reportError(mres.DuplicateLevelNames.str(
								msgRecorder.getContext(), level.getName()));
					} else {
					...
					if (columnsToObjects.containsKey(level.getColumnName())) {
						...
						msgRecorder.reportError(mres.DuplicateLevelColumnNames.str(
								msgRecorder.getContext(), level.getName(), l.getName(),
								level.getColumnName()));
					} else {
				...
				for (Measure measure : measures) {
					...
					if (namesToObjects.containsKey(measure.getName())) {
						msgRecorder.reportError(mres.DuplicateMeasureNames.str(
								msgRecorder.getContext(), measure.getName()));
						continue;
					} else {
					...
					if (columnsToObjects.containsKey(measure.getColumnName())) {
						...
						if (o instanceof Measure) {
							...
							msgRecorder.reportError(mres.DuplicateMeasureColumnNames.str(
									msgRecorder.getContext(), measure.getName(), m.getName(),
									measure.getColumnName()));
						} else {
							Level l = (Level) o;
							msgRecorder.reportError(mres.DuplicateLevelMeasureColumnNames
									.str(msgRecorder.getContext(), l.getName(),
											measure.getName(), measure.getColumnName()));
						}
				...
				for (Map.Entry<String, String> e : foreignKeyMap.entrySet()) {
					...
					if (namesToObjects.containsKey(baseFKName)) {
						msgRecorder.reportError(mres.DuplicateFactForeignKey.str(
								msgRecorder.getContext(), baseFKName, aggFKName));
					} else {
					...
					if (factTable.findTableWithLeftCondition(c) == null) {
						msgRecorder.reportError(mres.UnknownLeftJoinCondition.str(
								msgRecorder.getContext(), tableName, baseFKName));
					}
				}
			} finally {
				msgRecorder.popContextName();
	...
	static class NameTableDef extends ExplicitRules.TableDef {
		...
		static ExplicitRules.NameTableDef make(final MondrianDef.AggName aggName,
				final ExplicitRules.Group group) {
			ExplicitRules.NameTableDef name = new ExplicitRules.NameTableDef(
					aggName.getNameAttribute(), aggName.getApproxRowCountAttribute(),
					aggName.isIgnoreCase(), group);
		...
		public boolean matches(final String tableName) {
			return (this.ignoreCase) ? this.name.equalsIgnoreCase(tableName)
