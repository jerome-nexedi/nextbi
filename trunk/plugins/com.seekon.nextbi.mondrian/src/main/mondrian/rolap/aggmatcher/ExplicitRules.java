/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/ExplicitRules.java#26 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/ExplicitRules
 *          .java#26 $
 */
public class ExplicitRules {
  ...
  /**
   * Returns the {@link TableDef} for a tableName that is a candidate aggregate
   * table. If null is returned, then the default rules are used otherwise if
   * not null, then the ExplicitRules.TableDef is used.
   */
  public static ExplicitRules.TableDef getIncludeByTableDef(final String tableName,
    final List<Group> aggGroups) {
    for (Group group : aggGroups) {
  ...
  /**
   * This class forms a collection of aggregate table explicit rules for a given
   * cube.
   * 
   */
  public static class Group {
    ...
    public static ExplicitRules.Group make(final RolapCube cube,
      ...
      if (relation instanceof MondrianDef.Table) {
        MondrianDef.AggExclude[] aggExcludes = ((MondrianDef.Table) relation)
          .getAggExcludes();
        if (aggExcludes != null) {
        ...
        MondrianDef.AggTable[] aggTables = ((MondrianDef.Table) relation)
          .getAggTables();
        if (aggTables != null) {
        ...
        LOGGER.warn(mres.CubeRelationNotTable.str(cube.getName(), relation
          .getClass().getName()));
      }
    ...
    /**
     * Get the database table name associated with this Group's RolapStar's fact
     * table.
     */
    ...
    /**
     * Get the database catalog name associated with this Group's RolapStar's
     * fact table. Note: this currently this always returns null.
     */
  ...
  private static Exclude make(final MondrianDef.AggExclude aggExclude) {
    return (aggExclude.getNameAttribute() != null) ? new ExcludeName(aggExclude
      .getNameAttribute(), aggExclude.isIgnoreCase())
      : (Exclude) new ExcludePattern(aggExclude.getPattern(), aggExclude
        .isIgnoreCase());
  }
  ...
  private static class ExcludeName implements Exclude {
    ...
    public boolean isExcluded(final String tableName) {
      return (this.ignoreCase) ? this.name.equals(tableName) : this.name
    ...
    public void validate(final MessageRecorder msgRecorder) {
      ...
      try {
        ...
        /*
         * RME TODO // If name does not match the PatternTableDef pattern, //
         * then issue warning. // Why, because no table with the exclude's name
         * will // ever match the pattern, so the exclude is superfluous. //
         * This is best effort. Pattern pattern =
         * ExplicitRules.PatternTableDef.this.getPattern(); boolean
         * patternIgnoreCase =
         * ExplicitRules.PatternTableDef.this.isIgnoreCase(); boolean ignoreCase
         * = isIgnoreCase();
         * 
         * // If pattern is ignoreCase and name is any case or pattern // is not
         * ignoreCase and name is not ignoreCase, then simply // see if name
         * matches. // Else pattern in not ignoreCase and name is ignoreCase, //
         * then pattern could be "AB.*" and name "abc". // Here "abc" would
         * name, but not pattern - but who cares if (patternIgnoreCase || !
         * ignoreCase) { if (! pattern.matcher(name).matches()) {
         * msgRecorder.reportWarning( mres.getSuperfluousExludeName(
         * msgRecorder.getContext(), name, pattern.pattern())); } }
         */
      } finally {
        msgRecorder.popContextName();
      }
  ...
  private static class ExcludePattern implements Exclude {
    ...
    private ExcludePattern(final String pattern, final boolean ignoreCase) {
      this.pattern = (ignoreCase) ? Pattern.compile(pattern,
   ...
   * aggregate table mapping definitions. It contains the mappings for the fact
   * count column, optional ignore columns, foreign key mappings, measure column
   * mappings and level column mappings.
   */
  public static abstract class TableDef {

    /**
     * Given a MondrianDef.AggTable instance create a TableDef instance which is
     * either a NameTableDef or PatternTableDef.
     */
    static ExplicitRules.TableDef make(final MondrianDef.AggTable aggTable,
      final ExplicitRules.Group group) {
      return (aggTable instanceof MondrianDef.AggName) ? ExplicitRules.NameTableDef
        .make((MondrianDef.AggName) aggTable, group)
        : (ExplicitRules.TableDef) ExplicitRules.PatternTableDef.make(
          (MondrianDef.AggPattern) aggTable, group);
    }

    /**
     * This method extracts information from the MondrianDef.AggTable and places
     * it in the ExplicitRules.TableDef. This code is used for both the
     * NameTableDef and PatternTableDef subclasses of TableDef (it extracts
    ...
    private static void addTo(final ExplicitRules.TableDef tableDef,
      ...
      addLevelTo(tableDef, aggLevel.getNameAttribute(), aggLevel.getColumnName());
    }
    ...
    public static void addLevelTo(final ExplicitRules.TableDef tableDef,
      final String name, final String columnName) {
      Level level = tableDef.new Level(name, columnName);
      tableDef.add(level);
    }

    public static void addMeasureTo(final ExplicitRules.TableDef tableDef,
      final String name, final String column) {
      Measure measure = tableDef.new Measure(name, column);
      tableDef.add(measure);
    }

    /**
     * This class is used to map from a Level's symbolic name, [Time]&#46;[Year]
     * to the aggregate table's column name, TIME_YEAR.
     */
    class Level {
      private final String name;
       ...
       * 
       * <p>
       * The level name must be of the form <code>[hierarchy usage
       * name].[level name]</code>.
       * 
       * <p>
       * This method checks that is of length 2, starts with a hierarchy and the
       * "level name" exists.
       */
      public void validate(final MessageRecorder msgRecorder) {
        ...
        try {
          ...
          if (names.size() != 2) {
            msgRecorder.reportError(mres.BadLevelNameFormat.str(msgRecorder
              .getContext(), name));
          } else {
            RolapCube cube = ExplicitRules.TableDef.this.getCube();
            SchemaReader schemaReader = cube.getSchemaReader();
            RolapLevel level = (RolapLevel) schemaReader.lookupCompound(cube, names,
              false, Category.Level);
            if (level == null) {
              Hierarchy hierarchy = (Hierarchy) schemaReader.lookupCompound(cube,
                names.subList(0, 1), false, Category.Hierarchy);
              if (hierarchy == null) {
                msgRecorder.reportError(mres.UnknownHierarchyName.str(msgRecorder
                  .getContext(), names.get(0).name));
              } else {
                msgRecorder.reportError(mres.UnknownLevelName.str(msgRecorder
                  .getContext(), names.get(0).name, names.get(1).name));
              }
            }
            rlevel = level;
          }
     ...
     * [Measures]&amp;#46;[Unit Sales] to the aggregate table's column name,
     * UNIT_SALES_SUM.
     */
    class Measure {
      private final String name;
       ...
       * 
       * <p>
       * The measure name must be of the form <blockquote>
       * <code>[Measures].[measure name]</code></blockquote>
       * 
       * <p>
       * This method checks that is of length 2, starts with "Measures" and the
       * "measure name" exists.
       */
      public void validate(final MessageRecorder msgRecorder) {
        ...
        try {
          ...
          List<Id.Segment> names = Util.parseIdentifier(name);
          if (names.size() != 2) {
            msgRecorder.reportError(mres.BadMeasureNameFormat.str(msgRecorder
              .getContext(), name));
          } else {
            RolapCube cube = ExplicitRules.TableDef.this.getCube();
            SchemaReader schemaReader = cube.getSchemaReader();
            Member member = (Member) schemaReader.lookupCompound(cube, names, false,
              Category.Member);
            if (member == null) {
              if (!names.get(0).name.equals("Measures")) {
                msgRecorder.reportError(mres.BadMeasures.str(msgRecorder
                  .getContext(), names.get(0).name));
              } else {
                msgRecorder.reportError(mres.UnknownMeasureName.str(msgRecorder
                  .getContext(), names.get(1).name));
              }
            }
            RolapStar star = cube.getStar();
            rolapMeasure = star.getFactTable().lookupMeasureByName(cube.getName(),
              names.get(1).name);
            if (rolapMeasure == null) {
              msgRecorder.reportError(mres.BadMeasureName.str(msgRecorder
                .getContext(), names.get(1).name, cube.getName()));
            }
    ...
    protected TableDef(final boolean ignoreCase, final ExplicitRules.Group aggGroup) {
      this.id = nextId();
      this.ignoreCase = ignoreCase;
    ...
    /**
     * Returns an approximate number of rows in this table. A negative value
     * indicates that no estimate is available.
     * 
     * @return An estimated row count, or a negative value if no row count
    ...
    protected Recognizer.Matcher getIgnoreMatcher() {
      return new Recognizer.Matcher() {
        public boolean matches(final String name) {
          for (Iterator<String> it = ExplicitRules.TableDef.this
            .getIgnoreColumnNames(); it.hasNext();) {
    ...
    public boolean columnsOK(final RolapStar star,
      final JdbcSchema.Table dbFactTable, final JdbcSchema.Table dbTable,
      final MessageRecorder msgRecorder) {
      Recognizer cb = new ExplicitRecognizer(this, star, getCube(), dbFactTable,
        dbTable, msgRecorder);
      return cb.check();
    }
    ...
    /**
     * Add foreign key mapping entry (maps from fact table foreign key column
     * name to aggregate table foreign key column name).
     */
    protected void addFK(final MondrianDef.AggForeignKey fk) {
      ...
      this.foreignKeyMap
        .put(fk.getFactFKColumnName(), fk.getAggregateFKColumnName());
    }

    /**
     * Get the name of the aggregate table's foreign key column that matches the
     * base fact table's foreign key column or return null.
     */
    ...
    /**
     * Validate the Levels and Measures, also make sure each definition is
     * different, both name and column.
     */
    public void validate(final MessageRecorder msgRecorder) {
      msgRecorder.pushContextName("TableDef");
      try {
        // used to detect duplicates
        Map<String, Object> namesToObjects = new HashMap<String, Object>();
        // used to detect duplicates
        Map<String, Object> columnsToObjects = new HashMap<String, Object>();
        ...
        for (Level level : levels) {
          ...
          if (namesToObjects.containsKey(level.getName())) {
            msgRecorder.reportError(mres.DuplicateLevelNames.str(msgRecorder
              .getContext(), level.getName()));
          } else {
          ...
          if (columnsToObjects.containsKey(level.getColumnName())) {
            ...
            msgRecorder.reportError(mres.DuplicateLevelColumnNames.str(msgRecorder
              .getContext(), level.getName(), l.getName(), level.getColumnName()));
          } else {
        ...
        for (Measure measure : measures) {
          ...
          if (namesToObjects.containsKey(measure.getName())) {
            msgRecorder.reportError(mres.DuplicateMeasureNames.str(msgRecorder
              .getContext(), measure.getName()));
            continue;
          } else {
          ...
          if (columnsToObjects.containsKey(measure.getColumnName())) {
            ...
            if (o instanceof Measure) {
              Measure m = (Measure) o;
              msgRecorder.reportError(mres.DuplicateMeasureColumnNames.str(
                msgRecorder.getContext(), measure.getName(), m.getName(), measure
                  .getColumnName()));
            } else {
              Level l = (Level) o;
              msgRecorder.reportError(mres.DuplicateLevelMeasureColumnNames.str(
                msgRecorder.getContext(), l.getName(), measure.getName(), measure
                  .getColumnName()));
            }
        ...
        for (Map.Entry<String, String> e : foreignKeyMap.entrySet()) {
          ...
          if (namesToObjects.containsKey(baseFKName)) {
            msgRecorder.reportError(mres.DuplicateFactForeignKey.str(msgRecorder
              .getContext(), baseFKName, aggFKName));
          } else {
          ...
          MondrianDef.Column c = new MondrianDef.Column(tableName, baseFKName);
          if (factTable.findTableWithLeftCondition(c) == null) {
            msgRecorder.reportError(mres.UnknownLeftJoinCondition.str(msgRecorder
              .getContext(), tableName, baseFKName));
          }
        }
      } finally {
        msgRecorder.popContextName();
  ...
  static class NameTableDef extends ExplicitRules.TableDef {
    ...
    static ExplicitRules.NameTableDef make(final MondrianDef.AggName aggName,
      final ExplicitRules.Group group) {
      ExplicitRules.NameTableDef name = new ExplicitRules.NameTableDef(aggName
        .getNameAttribute(), aggName.getApproxRowCountAttribute(), aggName
        .isIgnoreCase(), group);
    ...
    public NameTableDef(final String name, final String approxRowCount,
      final boolean ignoreCase, final ExplicitRules.Group group) {
    ...
    private int loadApproxRowCount(String approxRowCount) {
      boolean notNullAndNumeric = approxRowCount != null
    ...
    /**
     * Does the given tableName match this NameTableDef (either exact match or,
     * if set, a case insensitive match).
     */
    public boolean matches(final String tableName) {
      return (this.ignoreCase) ? this.name.equalsIgnoreCase(tableName) : this.name
        .equals(tableName);
    }
  ...
  public static class PatternTableDef extends ExplicitRules.TableDef {
    ...
    static ExplicitRules.PatternTableDef make(
      final MondrianDef.AggPattern aggPattern, final ExplicitRules.Group group) {
      ExplicitRules.PatternTableDef pattern = new ExplicitRules.PatternTableDef(
        aggPattern.getPattern(), aggPattern.isIgnoreCase(), group);
    ...
    public PatternTableDef(final String pattern, final boolean ignoreCase,
      final ExplicitRules.Group group) {
      super(ignoreCase, group);
      this.pattern = (this.ignoreCase) ? Pattern.compile(pattern,
        Pattern.CASE_INSENSITIVE) : Pattern.compile(pattern);
      this.excludes = Collections.emptyList();
    }
    ...
    /**
     * Return true if the tableName 1) matches the pattern and 2) is not matched
     * by any of the Excludes.
     */
    public boolean matches(final String tableName) {
  ...
  private static void checkAttributeString(final MessageRecorder msgRecorder,
    final String attrValue, final String attrName) {
    if (attrValue == null) {
      msgRecorder.reportError(mres.NullAttributeString.str(msgRecorder.getContext(),
        attrName));
    } else if (attrValue.length() == 0) {
      msgRecorder.reportError(mres.EmptyAttributeString.str(
        msgRecorder.getContext(), attrName));
    }
  }
