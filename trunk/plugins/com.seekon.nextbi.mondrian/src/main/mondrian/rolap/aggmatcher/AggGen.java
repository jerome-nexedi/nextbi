/*
// $Id: //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggGen.java#26 $
...
/**
 * This class is used to create "lost" and "collapsed" aggregate table creation
 * sql (creates the rdbms table and inserts into it from the base fact table).
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/aggmatcher/AggGen.java#26 $
 */
public class AggGen {
	...
	/**
	 * Return true if this instance is ready to generate the sql. If false, then
	 * something went wrong as it was trying to understand the columns.
	 */
	...
	protected JdbcSchema.Table getTable(JdbcSchema db, RolapStar.Table rt) {
		...
		return (jt == null) ? getTable(db, rt.getTableName()) : jt;
	}
	...
	protected void addForeignKeyToNotLostColumnUsages(
		...
		if (column.hasUsage(JdbcSchema.UsageType.FOREIGN_KEY)) {
			Iterator<JdbcSchema.Table.Column.Usage> it = column
					.getUsages(JdbcSchema.UsageType.FOREIGN_KEY);
			it.hasNext();
			usage = it.next();
		} else {
	 ...
	 * request. This is what happens. First, for each column, walk up the column's
	 * table until one level below the base fact table. The left join condition
	 * contains the base fact table and the foreign key column name. This column
	 * should not be lost. Get the base fact table's measure columns. With a list
	 * of columns that should not be lost and measure, one can create lost create
	 * and insert commands.
	 */
	private void init() {
		...
		if (factTable == null) {
			getLogger().warn(
					"Init: " + "No fact table with name \"" + getFactTableName() + "\"");
			return;
		}
		try {
			factTable.load();
		...
		if (getLogger().isDebugEnabled()) {
			getLogger().debug(
					"Init: " + "RolapStar:" + Util.nl + getFactTable() + Util.nl
							+ "FactTable:" + Util.nl + factTable);
		}
		...
		for (RolapStar.Column column : columns) {
			...
			if (table.getParentTable() == null) {
				...
				if (expr instanceof MondrianDef.Column) {
					...
					if (c == null) {
						getLogger().warn(
								"Init: " + "FactTable:" + getFactTableName() + Util.nl
										+ "No Column with name \"" + name + "\"");
						return;
					}
					if (getLogger().isDebugEnabled()) {
		...
		for (RolapStar.Column rColumn : getFactTable().getColumns()) {
			...
			if (name == null) {
				getLogger().warn(
						"Init: " + "For fact table \"" + getFactTableName()
			...
			if (c == null) {
				getLogger().warn(
						"For RolapStar: \"" + getFactTable().getAlias()
								+ "\" measure with name, " + name + ", is not a column name. "
			...
			if (c.hasUsage(JdbcSchema.UsageType.MEASURE)) {
				for (Iterator<JdbcSchema.Table.Column.Usage> uit = c
						.getUsages(JdbcSchema.UsageType.MEASURE); uit.hasNext();) {
	...
	private boolean addSpecialCollapsedColumn(final JdbcSchema db,
		...
		if (rname == null) {
			getLogger().warn(
					"Adding Special Collapsed Column: " + "For fact table \""
							+ getFactTableName()
							+ "\", could not get column name for RolapStar.Column: "
							+ rColumn);
			return false;
		...
		if (jt == null) {
			getLogger().warn(
					"Adding Special Collapsed Column: " + "For fact table \""
							+ getFactTableName() + "\", could not get jdbc schema table "
		...
		if (c == null) {
			getLogger().warn(
					"Adding Special Collapsed Column: " + "For fact table \""
							+ getFactTableName() + "\", could not get jdbc schema column "
							+ "for RolapStar.Table with alias \"" + rt.getAlias()
							+ "\" and column name \"" + rname + "\"");
			return false;
		}
	...
	private boolean addCollapsedColumn(final JdbcSchema db,
		...
		if (rname == null) {
			getLogger().warn(
					"Adding Collapsed Column: " + "For fact table \""
							+ getFactTableName()
							+ "\", could not get column name for RolapStar.Column: "
							+ rColumn);
			return false;
		...
		List<JdbcSchema.Table.Column.Usage> list = new ArrayList<JdbcSchema.Table.Column.Usage>();
		for (RolapStar.Column rc : rt.getColumns()) {
			...
			if (name == null) {
				getLogger()
						.warn(
								"Adding Collapsed Column: " + "For fact table \""
										+ getFactTableName() + "\", could not get column name"
										+ " for RolapStar.Column \"" + rc
										+ "\" for RolapStar.Table with alias \"" + rt.getAlias()
										+ "\"");
				return false;
			}
			...
			JdbcSchema.Table.Column.Usage usage = c
	...
	public String insertIntoLost() {
		...
		for (JdbcSchema.Table.Column.Usage usage : notLostColumnUsages) {
			...
			pw.print(prefix);
			pw.print(sqlQuery.getDialect()
					.quoteIdentifier(factTableName, c.getName()));
			pw.print(" AS ");
		...
		for (JdbcSchema.Table.Column.Usage usage : measures) {
			...
			pw.print(prefix);
			pw.print(agg.getExpression(sqlQuery.getDialect().quoteIdentifier(
	...
	public String createCollapsed() {
		...
		// do foreign keys
		for (List<JdbcSchema.Table.Column.Usage> list : collapsedColumnUsages
				.values()) {
	...
	/**
	 * Return the sql code to populate a collapsed dimension table from the fact
	 * table.
	 */
	public String insertIntoCollapsed() {
		...
		for (List<JdbcSchema.Table.Column.Usage> list : collapsedColumnUsages
			...
			for (JdbcSchema.Table.Column.Usage usage : list) {
				...
				pw.print(prefix);
				pw.print(sqlQuery.getDialect()
						.quoteIdentifier(t.getName(), c.getName()));
				pw.print(" AS ");
				String n = (usage.usagePrefix == null) ? c.getName()
	...
	private void addColumnCreate(final PrintWriter pw, final String prefix,
