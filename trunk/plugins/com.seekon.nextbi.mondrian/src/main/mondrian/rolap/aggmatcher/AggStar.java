...
public class AggStar {
	...
	public int getSize() {
		return MondrianProperties.instance().ChooseAggregateByVolume.get() ? getFactTable()
				.getVolume() : getFactTable().getNumberOfRows();
	}
	...
	public abstract class Table {
		...
		public class JoinCondition {
			...
			public void print(final PrintWriter pw, final String prefix) {
				...
				if (left instanceof MondrianDef.Column) {
					...
					mondrian.rolap.RolapStar.Column col = getTable().getAggStar()
							.getStar().getFactTable().lookupColumn(c.name);
					if (col != null) {
						pw.print(" (");
		...
		protected AggStar.DimTable convertTable(final RolapStar.Table rTable,
			...
			// method).
			mondrian.rolap.RolapStar.Column col = getAggStar().getStar()
					.getFactTable().lookupColumn(left.name);
			if (col != null) {
	...
	public class FactTable extends Table {
		...
		public class Measure extends Table.Column {
			...
			private Measure(final String name,
				...
				this.rollableLevelBitKey = BitKey.Factory.makeBitKey(star
						.getColumnCount());
		...
		private void loadFactCount(final JdbcSchema.Table.Column.Usage usage) {
			...
			Column aggColumn = new Column(symbolicName, expression, datatype,
					bitPosition);
		...
		private void makeNumberOfRows() {
			...
			try {
				...
				if (resultSet.next()) {
					...
					getLogger().warn(
							mres.SqlQueryFailed.str("AggStar.FactTable.makeNumberOfRows",
									query.toString()));
