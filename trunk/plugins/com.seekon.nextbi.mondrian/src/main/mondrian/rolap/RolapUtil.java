/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapUtil.java#76 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapUtil.java#76 $
 */
public class RolapUtil {
	...
	public static final Logger PROFILE_LOGGER = Logger
			.getLogger("mondrian.profile");
	...
	/*
	 * Hook to run when a query is executed. This should not be used at runtime
	...
	/**
	 * Wraps a schema reader in a proxy so that each call to schema reader has a
	 * locus for profiling purposes.
	 * 
	 * @param connection
	 *          Connection
	 * @param schemaReader
	...
	private final static class RolapUtilComparable implements Comparable,
			Serializable {
	...
	public static void reloadNullLiteral() {
		mdxNullLiteral = MondrianProperties.instance().NullMemberRepresentation
				.get();
	}
	 ...
	 * 
	 * <p>
	 * NOTE: Synchronization policy: Lock the {@link RolapConnection} class before
	 * modifying or using this member.
	 */
	private static final Set<String> loadedDrivers = new HashSet<String>();

	static RolapMember[] toArray(List<RolapMember> v) {
		return v.isEmpty() ? new RolapMember[0] : v.toArray(new RolapMember[v
				.size()]);
	}

	static RolapMember lookupMember(MemberReader reader,
			List<Id.Segment> uniqueNameParts, boolean failIfNotFound) {
		RolapMember member = lookupMemberInternal(uniqueNameParts, null, reader,
				failIfNotFound);
		if (member != null) {
			return member;
		}
		...
		if (rootMembers.size() == 1) {
			...
			if (rootMember.isAll()) {
				member = lookupMemberInternal(uniqueNameParts, rootMember, reader,
						failIfNotFound);
			}
		}
		return member;
	...
	private static RolapMember lookupMemberInternal(List<Id.Segment> segments,
			RolapMember member, MemberReader reader, boolean failIfNotFound) {
		...
		if (member == null && failIfNotFound) {
			throw MondrianResource.instance().MdxCantFindMember.ex(Util
					.implode(segments));
		}
		return member;
	}
	 ...
	 * 
	 * <p>
	 * If it succeeds, the caller must call the {@link SqlStatement#close} method
	 * of the returned {@link SqlStatement}.
	 * 
	 * @param dataSource
	 *          DataSource
	 * @param sql
	 ...
	 * @return ResultSet
	 */
	public static SqlStatement executeQuery(DataSource dataSource, String sql,
			Locus locus) {
	 ...
	 * @param resultSetType
	 *          Result set type, or -1 to use default
	 * @param resultSetConcurrency
	 *          Result set concurrency, or -1 to use default
	 * @return ResultSet
	 */
	public static SqlStatement executeQuery(DataSource dataSource, String sql,
			List<SqlStatement.Type> types, int maxRowCount, int firstRowOrdinal,
			Locus locus, int resultSetType, int resultSetConcurrency) {
		SqlStatement stmt = new SqlStatement(dataSource, sql, types, maxRowCount,
				firstRowOrdinal, locus, resultSetType, resultSetConcurrency);
		stmt.execute();
		return stmt;
	}

	/**
	 * Raises an alert that native SQL evaluation could not be used in a case
	 * where it might have been beneficial, but some limitation in Mondrian's
	 * implementation prevented it. (Do not call this in cases where native
	 * evaluation would have been wasted effort.)
	 * 
	 * @param functionName
	 *          name of function for which native evaluation was skipped
	 * 
	 * @param reason
	 *          reason why native evaluation was skipped
	 */
	public static void alertNonNative(String functionName, String reason)
			throws NativeEvaluationUnsupportedException {
		// No i18n for log message, but yes for excn
		String alertMsg = "Unable to use native SQL evaluation for '"
				+ functionName + "'; reason:  " + reason;
		...
		} else if (alertValue.equalsIgnoreCase(org.apache.log4j.Level.ERROR
				.toString())) {
			LOGGER.error(alertMsg);
			throw MondrianResource.instance().NativeEvaluationUnsupported
					.ex(functionName);
		}
	}
	 ...
	 * 
	 * @param jdbcDrivers
	 *          A string consisting of the comma-separated names of JDBC driver
	 *          classes. For example
		...
		while (tok.hasMoreTokens()) {
			...
			if (loadedDrivers.add(jdbcDriver)) {
				try {
					...
					LOGGER.info("Mondrian: JDBC driver " + jdbcDriver
							+ " loaded successfully");
				} catch (ClassNotFoundException e) {
					LOGGER.warn("Mondrian: Warning: JDBC driver " + jdbcDriver
							+ " not found");
				}
			}
		}
	...
	/**
	 * Creates a compiler which will generate programs which will test whether the
	 * dependencies declared via {@link mondrian.calc.Calc#dependsOn(Hierarchy)}
	 * are accurate.
	 */
	...
	/**
	 * Locates a member specified by its member name, from an array of members. If
	 * an exact match isn't found, but a matchType of BEFORE or AFTER is
	 * specified, then the closest matching member is returned.
	 * 
	 * @param members
	 *          array of members to search from
	 * @param parent
	 *          parent member corresponding to the member being searched for
	 * @param level
	 *          level of the member
	 * @param searchName
	 *          member name
	 ...
	 *          if true, use case insensitive search (if applicable) when when
	 *          doing exact searches
	 * 
	 * @return matching member (if it exists) or the closest matching one in the
	 *         case of a BEFORE or AFTER search
	 */
	public static Member findBestMemberMatch(List<? extends Member> members,
			RolapMember parent, RolapLevel level, Id.Segment searchName,
			MatchType matchType, boolean caseInsensitive) {
		switch (matchType) {
		case FIRST:
		...
		// the members array
		Member searchMember = level.getHierarchy().createMember(parent, level,
		...
		for (Member member : members) {
			...
			if (matchType == MatchType.BEFORE) {
				if (rc < 0
						&& (bestMatch == null || FunUtil.compareSiblingMembers(member,
								bestMatch) > 0)) {
					bestMatch = member;
				}
	...
	public static MondrianDef.Relation convertInlineTableToRelation(
			MondrianDef.InlineTable inlineTable, final Dialect dialect) {
		...
		view.addCode("generic",
				dialect.generateInline(columnNames, columnTypes, valueList));
		return view;
	}
	 ...
	 * 
	 * <p>
	 * Given a semaphore initialized with <code>count</code>, no more than
