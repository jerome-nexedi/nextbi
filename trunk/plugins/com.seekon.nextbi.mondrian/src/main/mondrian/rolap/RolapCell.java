...
public class RolapCell implements Cell {
	...
	public String getDrillThroughSQL(boolean extendedContext) {
		...
		return (cellRequest == null) ? null : aggMan.getDrillThroughSql(
				cellRequest, starPredicateSlicer, false);
	}
	...
	public int getDrillThroughCount() {
		...
		RolapConnection connection = (RolapConnection) result.getQuery()
				.getConnection();
		final String sql = aggMan.getDrillThroughSql(cellRequest,
				starPredicateSlicer, true);

		final SqlStatement stmt = RolapUtil.executeQuery(
				connection.getDataSource(), sql,
				new Locus(new Execution(connection.createDummyStatement(), 0),
					...
					while (memberWalk != null && !memberWalk.isAll()) {
						...
						if (memberWalk.getLevel() != levelLast) {
							...
							listOfStarPredicatesForCurrentPosition
									.add(new MemberColumnPredicate(column, rolapCubeMember));
						}
	...
	public SqlStatement drillThroughInternal(int maxRowCount,
			int firstRowOrdinal, String tabFields, boolean extendedContext,
			Logger logger) {
		if (!canDrillThrough()) {
		...
		if (!dialect.supportsResultSetConcurrency(resultSetType,
				resultSetConcurrency) || firstRowOrdinal <= 1) {
		...
		return RolapUtil.executeQuery(connection.getDataSource(), sql, null,
				maxRowCount, firstRowOrdinal, new Locus(execution,
						"RolapCell.drillThrough", "Error in drill through"), resultSetType,
	...
	public void setValue(Scenario scenario, Object newValue,
		...
		((ScenarioImpl) scenario).setCellValue(result.getQuery().getConnection(),
				Arrays.asList(members), doubleNewValue, doubleCurrentValue,
				allocationPolicy, allocationArgs);
	}
