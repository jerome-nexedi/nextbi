...
class SqlMemberSource implements MemberReader, SqlTupleReader.MemberBuilder {
  ...
  SqlMemberSource(RolapHierarchy hierarchy) {
    ...
    this.dataSource = hierarchy.getRolapSchema().getInternalConnection()
      .getDataSource();
    assignOrderKeys = MondrianProperties.instance().CompareSiblingsByOrderKey.get();
  ...
  private int getMemberCount(RolapLevel level, DataSource dataSource) {
    ...
    final SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql, new Locus(
      Locus.peek().execution, "SqlMemberSource.getLevelMemberCount",
  ...
  /**
   * Generates the SQL statement to count the members in <code>level</code>. For
   * example, <blockquote>
   * 
   * <pre>
   * SELECT count(*) FROM (
   ...
   * </pre>
   * 
   * </blockquote> counts the non-leaf "state_province" level. MySQL doesn't
   * allow SELECT-in-FROM, so we use the syntax<blockquote>
   * 
   * <pre>
   ...
   */
  private String makeLevelMemberCountSql(RolapLevel level, DataSource dataSource,
    boolean[] mustCount) {
    mustCount[0] = false;
    SqlQuery sqlQuery = SqlQuery.newQuery(dataSource,
    ...
    if (!sqlQuery.getDialect().allowsFromQuery()) {
      ...
      for (int i = levelDepth; i >= 0; i--) {
        ...
        if (columnCount > 0
          ...
          && sqlQuery.getDialect().getDatabaseProduct() == Dialect.DatabaseProduct.SYBASE) {
      ...
      SqlQuery outerQuery = SqlQuery.newQuery(dataSource,
        "while generating query to count members in level " + level);
  ...
  private List<RolapMember> getMembers(DataSource dataSource) {
    ...
    RolapLevel[] levels = (RolapLevel[]) hierarchy.getLevels();
    SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql, types, 0, 0,
      new Locus(null, "SqlMemberSource.getMembers", "while building member cache"),
      -1, -1);
    try {
      ...
      while (resultSet.next()) {
        ...
        if (limit > 0 && limit < stmt.rowCount) {
          ...
          throw stmt.handle(MondrianResource.instance().MemberFetchLimitExceeded
            .ex(limit));
        }
        ...
        for (RolapLevel level : levels) {
          ...
          if (member == null) {
            ...
            member = memberBase;
            /*
             * RME is this right if (level.getOrdinalExp() != level.getKeyExp())
          ...
          for (Property property : properties) {
            /*
             * REVIEW emcdermid 9-Jul-2009: Should we also look up the value in
             * the pool here, rather than setting it directly? Presumably the
             * value is already in the pool as a result of makeMember().
             */
  ...
  /**
   * Adds <code>member</code> just before the first element in <code>list</code>
  ...
  public List<RolapMember> getMembersInLevel(RolapLevel level, int startOrdinal,
    int endOrdinal) {
    TupleConstraint constraint = sqlConstraintFactory
      .getLevelMembersConstraint(null);
  ...
  private List<RolapMember> getMembersInLevel(RolapLevel level,
    TupleConstraint constraint) {
    final TupleReader tupleReader = level.getDimension().isHighCardinality() ? new HighCardSqlTupleReader(
      constraint)
      : new SqlTupleReader(constraint);
    tupleReader.addLevelMembers(level, this, null);
    final TupleList tupleList = tupleReader.readTuples(dataSource, null, null);
  ...
  /**
   * Generates the SQL statement to access the children of <code>member</code>.
   ...
   * <code>[Canada].[BC]</code>.
   * <p>
   * Note that this method is never called in the context of virtual cubes, it
   * is only called on regular cubes.
   * 
   * <p>
   * See also {@link SqlTupleReader#makeLevelMembersSql}.
   */
  Pair<String, List<SqlStatement.Type>> makeChildMemberSql(RolapMember member,
    DataSource dataSource, MemberChildrenConstraint constraint) {
    SqlQuery sqlQuery = SqlQuery.newQuery(dataSource,
    ...
    if (levelCollapsed) {
      ...
      int bitPos = starColumn.getBitPosition();
      AggStar.Table.Column aggColumn = aggStar.lookupColumn(bitPos);
      RolapStar.Condition condition = new RolapStar.Condition(level.getKeyExp(),
      ...
      while (parentLevel != null && !parentLevel.isAll() && !isUnique) {
        ...
        condition = new RolapStar.Condition(parentLevel.getKeyExp(), aggColumn
          .getExpression());
        sqlQuery.addWhere(condition.toString(sqlQuery));
  ...
  private static AggStar chooseAggStar(MemberChildrenConstraint constraint,
    RolapMember member) {
    ...
    int bitPosition = ((RolapStar.Measure) measure.getStarMeasure())
      .getBitPosition();
    ...
    return AggregationManager.instance().findAgg(star, levelBitKey, measureBitKey,
      new boolean[] { false });
  }

  /**
   * Determine if a level contains more than a single column for its data, such
   * as an ordinal column or property column
   * 
   * @param level
   *          the level to check
  ...
  /**
   * Determine if the given aggregate table has the dimension level specified
   * within in (AggStar.FactTable) it, aka collapsed, or associated with foreign
   * keys (AggStar.DimTable)
   * 
   * @param aggStar
   *          aggregate star if exists
   * @param level
   *          level
  ...
  public void getMemberChildren(List<RolapMember> parentMembers,
    List<RolapMember> children, MemberChildrenConstraint mcc) {
    // try to fetch all children at once
    RolapLevel childLevel = getCommonChildLevelForDescendants(parentMembers);
    if (childLevel != null) {
      TupleConstraint lmc = sqlConstraintFactory.getDescendantsConstraint(
        parentMembers, mcc);
      List<RolapMember> list = getMembersInLevel(childLevel, 0, Integer.MAX_VALUE,
        lmc);
   ...
   * returns that level; this indicates that all member children can be fetched
   * at once. Otherwise returns null.
   */
  ...
  private void getMemberChildren2(RolapMember parentMember,
    List<RolapMember> children, MemberChildrenConstraint constraint) {
    ...
    SqlStatement stmt = RolapUtil.executeQuery(dataSource, sql, types, 0, 0,
      new Locus(Locus.peek().execution, "SqlMemberSource.getMemberChildren",
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        if (member == null) {
          member = makeMember(parentMember2, childLevel, value, captionValue,
  ...
  public RolapMember makeMember(RolapMember parentMember, RolapLevel childLevel,
    ...
    RolapMemberBase member = new RolapMemberBase(parentMember, rolapChildLevel,
      value);
    ...
    if (parentChild) {
      ...
      // children.
      member = childLevel.hasClosedPeer() ? new RolapParentChildMember(parentMember,
        rolapChildLevel, value, member) : new RolapParentChildMemberNoClosure(
    ...
    for (int j = 0; j < properties.length; j++) {
      ...
      member.setProperty(property.getName(), getPooledValue(accessors.get(
        columnOffset + j).get()));
    }
  ...
  public RolapMember allMember() {
    final RolapHierarchy rolapHierarchy = hierarchy instanceof RolapCubeHierarchy ? ((RolapCubeHierarchy) hierarchy)
      .getRolapHierarchy()
      : hierarchy;
  ...
  /**
   * <p>
   * Looks up an object (and if needed, stores it) in a cached value pool. This
   * permits us to reuse references to an existing object rather than create new
   * references to what are essentially duplicates. The intent is to allow the
   * duplicate object to be garbage collected earlier, thus keeping overall
   ...
   * is not set, then valuePool will be null and no attempt to cache the value
   * will be made. The method will simply return the incoming object reference.
   * </p>
   * 
   * @param incoming
   *          An object to look up. Must be immutable in usage, even if not
   *          declared as such.
   * @return a reference to a cached object equal to the incoming object, or to
   *         the incoming object if either no cached object was found, or
   *         caching is disabled.
   */
  ...
  /**
   * Generates the SQL to find all root members of a parent-child hierarchy. For
   * example, <blockquote>
   * 
   * <pre>
   * SELECT "employee_id"
   ...
   * 
   * <p>
   * Currently, parent-child hierarchies may have only one level (plus the 'All'
   * level).
   */
  private Pair<String, List<SqlStatement.Type>> makeChildMemberSql_PCRoot(
    ...
    Util.assertTrue(level.isUnique(), "parent-child level '" + level
      + "' must be unique");
  ...
  /**
   * Generates the SQL statement to access the children of <code>member</code>
   * in a parent-child hierarchy. For example, <blockquote>
   * 
   * <pre>
   * SELECT "employee_id"
   * FROM "employee"
  ...
  private Pair<String, List<SqlStatement.Type>> makeChildMemberSqlPC(
    ...
    Util.assertTrue(!level.isAll(), "all level cannot be parent-child");
    Util.assertTrue(level.isUnique(), "parent-child level '" + level + "' must be "
      + "unique");
  ...
  public void getMemberRange(RolapLevel level, RolapMember startMember,
    RolapMember endMember, List<RolapMember> memberList) {
    throw new UnsupportedOperationException();
  }
   ...
   * 
   * <p>
   * When looking up cells, this member will automatically be converted to a
   * corresponding member of the auxiliary dimension which maps onto the closure
   * table.
   */
  private static class RolapParentChildMember extends RolapMemberBase {
    ...
    public RolapParentChildMember(RolapMember parentMember, RolapLevel childLevel,
      Object value, RolapMember dataMember) {
   ...
   * 
   * <p>
   * This member is calculated. When you ask for its value, it returns an
   * expression which aggregates the values of its child members. This
   * calculation is very inefficient, and we can only support aggregatable
   * measures ("count distinct" is non-aggregatable). Unfortunately it's the
   * best we can do without a closure table.
   */
  private static class RolapParentChildMemberNoClosure extends
  ...
  /**
   * <p>
   * Interface definition for the pluggable factory used to decide which
   * implementation of {@link java.util.Map} to use to pool reusable values.
   * </p>
   */
  public interface ValuePoolFactory {
    /**
     * <p>
     * Create a new {@link java.util.Map} to be used to pool values. The value
     * pool permits us to reuse references to existing objects rather than
     * create new references to what are essentially duplicates of the same
     * object. The intent is to allow the duplicate object to be garbage
     * collected earlier, thus keeping overall memory requirements down.
     * </p>
     * 
     * @param source
     *          The {@link SqlMemberSource} in which values are being pooled.
  ...
  public static final class NullValuePoolFactory implements ValuePoolFactory {
     ...
     * {@inheritDoc}
     * <p>
     * This version returns null, meaning that by default values will not be
     * pooled.
     * </p>
     * 
     * @param source
  ...
  /**
   * <p>
   * Creates the ValuePoolFactory which is in turn used to create property-value
   * maps for member properties.
   * </p>
   * 
   * <p>
   ...
   * </p>
   */
  public static final class ValuePoolFactoryFactory extends
    ObjectFactory.Singleton<ValuePoolFactory> {
    /**
    ...
    /**
     * The constructor for the <code>ValuePoolFactoryFactory</code>. This passes
     * the <code>ValuePoolFactory</code> class to the <code>ObjectFactory</code>
     * base class.
     */
    ...
    protected ValuePoolFactory getDefault(Class[] parameterTypes,
