/*
// $Id: //open/mondrian/src/main/mondrian/rolap/SqlConstraintUtils.java#62 $
 ...
 * {@link mondrian.rolap.sql.SqlConstraint}, used to generate constraints into
 * {@link mondrian.rolap.sql.SqlQuery}.
 * 
 * @author av
 * @since Nov 21, 2005
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/SqlConstraintUtils.java#62 $
 */
public class SqlConstraintUtils {
  ...
  /**
   * For every restricting member in the current context, generates a WHERE
   * condition and a join to the fact table.
   * 
   * @param sqlQuery
   *          the query to modify
   * @param aggStar
   ...
   *          defines the behavior if the current context contains calculated
   *          members. If true, thows an exception.
   * @param evaluator
   *          Evaluator
   */
  public static void addContextConstraint(SqlQuery sqlQuery, AggStar aggStar,
    Evaluator evaluator, boolean restrictMemberTypes) {
    ...
    if (restrictMemberTypes) {
      ...
      if (hasMultiPositionSlicer(evaluator)) {
        throw Util
          .newInternal("can not restrict SQL to context with multi-position slicer");
      }
    } else {
  ...
  /**
   * Looks at the given <code>evaluator</code> to determine if it has more than
   * one slicer member from any particular hierarchy.
   * 
   * @param evaluator
   *          the evaluator to look at
  ...
  protected static Member[] removeMultiPositionSlicerMembers(Member[] members,
    Evaluator evaluator) {
   ...
   * 
   * <p>
   * This is required only if the default member is not the ALL member. The time
   * dimension for example, has 1997 as default member. When we evaluate the
   * query
   * 
   * <pre>
   ...
   * table for 1998 not 1997. So we do not restrict the time dimension and fetch
   * all children.
   * 
   * <p>
  ...
  private static Member[] removeCalculatedAndDefaultMembers(Member[] members) {
    ...
    for (int i = 0; i < members.length; ++i) {
      ...
      if (i > 0 && member.getHierarchy().getDefaultMember().equals(member)) {
        continue;
      }
  ...
  static List<Member> removeCalculatedMembers(List<Member> members) {
    return new FilteredIterableList<Member>(members,
  ...
  public static void joinLevelTableToFactTable(SqlQuery sqlQuery,
    RolapCube baseCube, AggStar aggStar, Evaluator e, RolapCubeLevel level) {
   ...
   * @param restrictMemberTypes
   *          defines the behavior if <code>parent</code> is a calculated
   *          member. If true, an exception is thrown
   */
  public static void addMemberConstraint(SqlQuery sqlQuery, RolapCube baseCube,
    AggStar aggStar, RolapMember parent, boolean restrictMemberTypes) {
    List<RolapMember> list = Collections.singletonList(parent);
    boolean exclude = false;
    addMemberConstraint(sqlQuery, baseCube, aggStar, list, restrictMemberTypes,
      false, exclude);
  }

  /**
   * Creates a "WHERE exp IN (...)" condition containing the values of all
   * parents. All parents must belong to the same level.
   * 
   * <p>
   * If this constraint is part of a native cross join, there are multiple
   * constraining members, and the members comprise the cross product of all
   * unique member keys referenced at each level, then generating IN expressions
   * would result in incorrect results. In that case, "WHERE ((level1 = val1a
   * AND level2 = val2a AND ...) OR (level1 = val1b AND level2 = val2b AND ...)
   * OR ..." is generated instead.
   * 
   * @param sqlQuery
   *          the query to modify
   * @param baseCube
   ...
   * @param restrictMemberTypes
   *          defines the behavior if <code>parents</code> contains calculated
   *          members. If true, and one of the members is calculated, an
   *          exception is thrown.
   * @param crossJoin
   *          true if constraint is being generated as part of a native
   *          crossjoin
   * @param exclude
   *          whether to exclude the members in the SQL predicate. e.g. not in {
   *          member list}.
   */
  public static void addMemberConstraint(SqlQuery sqlQuery, RolapCube baseCube,
    AggStar aggStar, List<RolapMember> members, boolean restrictMemberTypes,
    boolean crossJoin, boolean exclude) {
    if (members.size() == 0) {
    ...
    for (; firstUniqueParent != null && !firstUniqueParent.getLevel().isUnique(); firstUniqueParent = firstUniqueParent
      .getParentMember()) {
    }
    ...
    if (crossJoin && !memberLevel.isUnique() && !membersAreCrossProduct(members)) {
      ...
      condition += constrainMultiLevelMembers(sqlQuery, baseCube, aggStar, members,
        firstUniqueParentLevel, restrictMemberTypes, exclude);
    } else {
      condition += generateSingleValueInExpr(sqlQuery, baseCube, aggStar, members,
        firstUniqueParentLevel, restrictMemberTypes, exclude);
    }
  ...
  private static StarColumnPredicate getColumnPredicates(RolapStar.Column column,
  ...
  /**
   * Adds to the where clause of a query expression matching a specified list of
   * members
   * 
   * @param sqlQuery
   ...
   * 
   * @return a non-empty String if SQL is generated for the multi-level member
   *         list.
   */
  private static String constrainMultiLevelMembers(SqlQuery sqlQuery,
    RolapCube baseCube, AggStar aggStar, List<RolapMember> members,
    RolapLevel fromLevel, boolean restrictMemberTypes, boolean exclude) {
    ...
    if (sqlQuery.getDialect().supportsMultiValueInExpr()) {
      condition1.append(generateMultiValueInExpr(sqlQuery, baseCube, aggStar,
        members, fromLevel, restrictMemberTypes, parentChildrenMap));
      ...
      if (parentChildrenMap.isEmpty()) {
        ...
        if (exclude) {
          ...
          condition.append(")");
          condition.append(" or ");
          condition.append(generateMultiValueIsNullExprs(sqlQuery, baseCube, members
            .get(0), fromLevel));
        }
    ...
    for (RolapMember p : parentChildrenMap.keySet()) {
      ...
      for (RolapMember gp = p; gp != null; gp = gp.getParentMember()) {
        ...
        condition2.append(constrainLevel(level, sqlQuery, baseCube, aggStar,
          getColumnValue(gp.getKey(), sqlQuery.getDialect(), level.getDatatype()),
          false));
      ...
      if (!children.isEmpty()) {
        ...
        if (sqlQuery.getDialect().supportsMultiValueInExpr()
          ...
          condition2.append(generateMultiValueInExpr(sqlQuery, baseCube, aggStar,
            children, childrenLevel, restrictMemberTypes, tmpParentChildrenMap));
          ...
          condition2.append(generateSingleValueInExpr(sqlQuery, baseCube, aggStar,
            children, childrenLevel, restrictMemberTypes, false));
        }
      }
   ...
   *          list of members
   * 
   * @return true if the members comprise the cross product of all unique member
   *         keys referenced at each level
   */
  private static boolean membersAreCrossProduct(List<RolapMember> members) {
    int crossProdSize = getNumUniqueMemberKeys(members);
    for (Collection<RolapMember> parents = getUniqueParentMembers(members); !parents
      .isEmpty(); parents = getUniqueParentMembers(parents)) {
   ...
   */
  private static String getColumnValue(Object key, Dialect dialect,
   ...
   */
  public static String constrainLevel(RolapLevel level, SqlQuery query,
    RolapCube baseCube, AggStar aggStar, String columnValue, boolean caseSensitive) {
  ...
  /**
   * Generates a multi-value IN expression corresponding to a list of member
   * expressions, and adds the expression to the WHERE clause of a query,
   * provided the member values are all non-null
   * 
   * @param sqlQuery
   *          query containing the where clause
   * @param baseCube
   ...
   *          upon return this map contains members that have Null values in its
   *          (parent) levels
   * @return a non-empty String if multi-value IN list was generated for some
   *         members
   */
  private static String generateMultiValueInExpr(SqlQuery sqlQuery,
    RolapCube baseCube, AggStar aggStar, List<RolapMember> members,
    ...
    for (RolapMember m : members) {
      ...
      for (RolapMember p = m; p != null; p = p.getParentMember()) {
        ...
        String value = getColumnValue(p.getKey(), sqlQuery.getDialect(), level
          .getDatatype());
  ...
  /**
   * Generates an expression that is an OR of IS NULL expressions, one per level
   * in a RolapMember.
   * 
   * @param sqlQuery
   ...
   */
  private static String generateMultiValueIsNullExprs(SqlQuery sqlQuery,
    RolapCube baseCube, RolapMember member, RolapLevel fromLevel) {
   ...
   */
  private static String generateSingleValueInExpr(SqlQuery sqlQuery,
    RolapCube baseCube, AggStar aggStar, List<RolapMember> members,
    RolapLevel fromLevel, boolean restrictMemberTypes, boolean exclude) {
    ...
    for (Collection<RolapMember> c = members; !c.isEmpty(); c = getUniqueParentMembers(c)) {
      ...
      if (column != null) {
        if (aggStar != null) {
          ...
          AggStar.Table.Column aggColumn = aggStar.lookupColumn(bitPos);
          if (aggColumn == null) {
            throw Util.newInternal("AggStar " + aggStar + " has no column for "
      ...
      if (!dialect.supportsUnlimitedValueList() && cc instanceof ListColumnPredicate
        ...
        String where = RolapStar.Column.createInExpr(q, cc, level.getDatatype(),
          sqlQuery);
