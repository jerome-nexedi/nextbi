...
public class SqlConstraintUtils {
	...
	public static boolean hasMultiPositionSlicer(Evaluator evaluator) {
		if (evaluator instanceof RolapEvaluator) {
			...
			for (Member slicerMember : ((RolapEvaluator) evaluator)
					.getSlicerMembers()) {
	...
	public static void addMemberConstraint(SqlQuery sqlQuery, RolapCube baseCube,
		...
		for (; firstUniqueParent != null
				&& !firstUniqueParent.getLevel().isUnique(); firstUniqueParent = firstUniqueParent
				.getParentMember()) {
		}
		...
		if (crossJoin && !memberLevel.isUnique()
				&& !membersAreCrossProduct(members)) {
			assert (member.getParentMember() != null);
			condition += constrainMultiLevelMembers(sqlQuery, baseCube, aggStar,
					members, firstUniqueParentLevel, restrictMemberTypes, exclude);
		} else {
			condition += generateSingleValueInExpr(sqlQuery, baseCube, aggStar,
					members, firstUniqueParentLevel, restrictMemberTypes, exclude);
		}
	...
	private static StarColumnPredicate getColumnPredicates(
			RolapStar.Column column, Collection<RolapMember> members) {
		switch (members.size()) {
		case 0:
	...
	private static String constrainMultiLevelMembers(SqlQuery sqlQuery,
		...
		if (sqlQuery.getDialect().supportsMultiValueInExpr()) {
			...
			if (parentChildrenMap.isEmpty()) {
				...
				if (exclude) {
					...
					condition.append(")");
					condition.append(" or ");
					condition.append(generateMultiValueIsNullExprs(sqlQuery, baseCube,
							members.get(0), fromLevel));
				}
		...
		for (RolapMember p : parentChildrenMap.keySet()) {
			...
			for (RolapMember gp = p; gp != null; gp = gp.getParentMember()) {
				...
				condition2.append(constrainLevel(
						level,
						sqlQuery,
						baseCube,
						aggStar,
						getColumnValue(gp.getKey(), sqlQuery.getDialect(),
								level.getDatatype()), false));
			...
			if (!children.isEmpty()) {
				...
				if (sqlQuery.getDialect().supportsMultiValueInExpr()
					...
					condition2.append(generateMultiValueInExpr(sqlQuery, baseCube,
							aggStar, children, childrenLevel, restrictMemberTypes,
					...
					condition2.append(generateSingleValueInExpr(sqlQuery, baseCube,
							aggStar, children, childrenLevel, restrictMemberTypes, false));
				}
			}
	...
	public static String constrainLevel(RolapLevel level, SqlQuery query,
			RolapCube baseCube, AggStar aggStar, String columnValue,
	...
	private static String generateMultiValueInExpr(SqlQuery sqlQuery,
		...
		for (RolapMember m : members) {
			...
			for (RolapMember p = m; p != null; p = p.getParentMember()) {
				...
				String value = getColumnValue(p.getKey(), sqlQuery.getDialect(),
						level.getDatatype());
	...
	private static String generateSingleValueInExpr(SqlQuery sqlQuery,
		...
		for (Collection<RolapMember> c = members; !c.isEmpty(); c = getUniqueParentMembers(c)) {
			...
			if (!dialect.supportsUnlimitedValueList()
				...
				String where = RolapStar.Column.createInExpr(q, cc,
						level.getDatatype(), sqlQuery);
