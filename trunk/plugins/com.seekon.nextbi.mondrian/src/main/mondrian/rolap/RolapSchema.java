/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapSchema.java#144 $
 ...
 * @author jhyde
 * @since 26 July, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapSchema.java#144 $
 */
public class RolapSchema implements Schema {
  private static final Logger LOGGER = Logger.getLogger(RolapSchema.class);

  private static final Set<Access> schemaAllowed = Olap4jUtil.enumSetOf(Access.NONE,
    Access.ALL, Access.ALL_DIMENSIONS, Access.CUSTOM);

  private static final Set<Access> cubeAllowed = Olap4jUtil.enumSetOf(Access.NONE,
    Access.ALL, Access.CUSTOM);

  private static final Set<Access> dimensionAllowed = Olap4jUtil.enumSetOf(
    Access.NONE, Access.ALL, Access.CUSTOM);

  private static final Set<Access> hierarchyAllowed = Olap4jUtil.enumSetOf(
    Access.NONE, Access.ALL, Access.CUSTOM);

  private static final Set<Access> memberAllowed = Olap4jUtil.enumSetOf(Access.NONE,
    Access.ALL);
  ...
  /**
   * Maps {@link String names of shared hierarchies} to {@link RolapHierarchy
   * the canonical instance of those hierarchies}.
   */
  ...
  /**
   * Map containing column cardinality. The combination of Mondrianef.Relation
   * and MondrianDef.Expression uniquely identifies a relational expression(e.g.
   * a column) specified in the xml schema.
   */
  private final Map<MondrianDef.Relation, Map<MondrianDef.Expression, Integer>> relationExprCardinalityMap;

  /**
   * List of warnings. Populated when a schema is created by a connection that
   * has {@link mondrian.rolap.RolapConnectionProperties#Ignore Ignore}=true.
   */
  ...
  /**
   * Unique schema instance id that will be used to inform clients when the
   * schema has changed.
   * 
   * <p>
   ...
   *          MD5 hash
   */
  private RolapSchema(final String key, final Util.PropertyList connectInfo,
    final DataSource dataSource, final String md5Bytes) {
    ...
    this.internalConnection = new RolapConnection(internalServer, connectInfo, this,
      dataSource);
    ...
    this.relationExprCardinalityMap = new HashMap<MondrianDef.Relation, Map<MondrianDef.Expression, Integer>>();
  }
   ...
   */
  private RolapSchema(String key, String md5Bytes, String catalogUrl,
    String catalogStr, Util.PropertyList connectInfo, DataSource dataSource) {
    this(key, connectInfo, dataSource, md5Bytes);
    load(catalogUrl, catalogStr);
  }

  private RolapSchema(String key, String catalogUrl, Util.PropertyList connectInfo,
  ...
  static CellFormatter getCellFormatter(String className,
    Scripts.ScriptDefinition script) throws Exception {
    if (className == null && script == null) {
      throw Util
        .newError("Must specify either className attribute or Script element");
    }
    if (className != null && script != null) {
      throw Util
        .newError("Must not specify both className attribute and Script element");
    }
    if (className != null) {
      @SuppressWarnings( { "unchecked" })
      Class<CellFormatter> clazz = (Class<CellFormatter>) Class.forName(className);
  ...
  static MemberFormatter getMemberFormatter(String className,
    ...
    if (className != null) {
      @SuppressWarnings( { "unchecked" })
      Class<MemberFormatter> clazz = (Class<MemberFormatter>) Class
        .forName(className);
   ...
   * 
   * @param className
   *          Name of property formatter class
   * @param script
   *          Script
  ...
  static PropertyFormatter createPropertyFormatter(String className,
    ...
    if (className != null) {
      @SuppressWarnings( { "unchecked" })
      Class<PropertyFormatter> clazz = (Class<PropertyFormatter>) Class
        .forName(className);
  ...
  protected void finalCleanUp() {
    final CacheControl cacheControl = AggregationManager.instance().getCacheControl(
      null);
    for (Cube cube : getCubes()) {
      CellRegion cr = cacheControl.createMeasuresRegion(cube);
   ...
   * 
   * <p>
   * NOTE: This method is not cheap. The implementation gets a connection from
   * the connection pool.
   * 
   * @return dialect
   */
  ...
  private void load(MondrianDef.Schema xmlSchema) {
    ...
    final RolapSchemaFunctionTable funTable = new RolapSchemaFunctionTable(
      mapNameToUdf.values());
    funTable.init();
    ...
    for (MondrianDef.Dimension xmlDimension : xmlSchema.dimensions) {
      if (xmlDimension.foreignKey != null) {
        throw MondrianResource.instance().PublicDimensionMustNotHaveForeignKey
          .ex(xmlDimension.name);
      }
    }
    ...
    for (MondrianDef.Parameter xmlParameter : xmlSchema.parameters) {
      ...
      RolapSchemaParameter param = new RolapSchemaParameter(this, name,
        defaultValue, description, type, modifiable);
      Util.discard(param);
    }
    ...
    if (xmlSchema.defaultRole != null) {
      ...
      if (role == null) {
        error("Role '" + xmlSchema.defaultRole + "' not found", locate(xmlSchema,
          "defaultRole"));
      } else {
  ...
  static Scripts.ScriptDefinition toScriptDef(MondrianDef.Script script) {
    ...
    final Scripts.ScriptLanguage language = Scripts.ScriptLanguage
      .lookup(script.language);
    if (language == null) {
  ...
  /**
   * Reports an error. If we are tolerant of errors (see
   * {@link mondrian.rolap.RolapConnectionProperties#Ignore}), adds it to the
   * stack, overwise throws. A thrown exception will typically abort the attempt
   * to create the exception.
   * 
   * @param message
   *          Message
   * @param xmlLocation
   *          Location of XML element or attribute that caused the error, or
   *          null
   */
  void error(String message, XmlLocation xmlLocation) {
    final RuntimeException ex = new RuntimeException(message);
    if (internalConnection != null
      && "true".equals(internalConnection
        .getProperty(RolapConnectionProperties.Ignore.name()))) {
      warningList.add(ex);
    } else {
      throw ex;
    }
  ...
  private NamedSet createNamedSet(MondrianDef.NamedSet xmlNamedSet) {
    ...
    try {
      ...
      throw MondrianResource.instance().NamedSetHasBadFormula
        .ex(xmlNamedSet.name, e);
    }
    final Formula formula = new Formula(new Id(new Id.Segment(xmlNamedSet.name,
      Id.Quoting.UNQUOTED)), exp);
    ...
    for (MondrianDef.SchemaGrant schemaGrant : xmlRole.schemaGrants) {
      ...
      for (MondrianDef.CubeGrant cubeGrant : schemaGrant.cubeGrants) {
        ...
        for (MondrianDef.DimensionGrant dimensionGrant : cubeGrant.dimensionGrants) {
          Dimension dimension = (Dimension) schemaReader.lookupCompound(cube, Util
            .parseIdentifier(dimensionGrant.dimension), true, Category.Dimension);
        ...
        for (MondrianDef.HierarchyGrant hierarchyGrant : cubeGrant.hierarchyGrants) {
          Hierarchy hierarchy = (Hierarchy) schemaReader.lookupCompound(cube, Util
            .parseIdentifier(hierarchyGrant.hierarchy), true, Category.Hierarchy);
          final Access hierarchyAccess = getAccess(hierarchyGrant.access,
            hierarchyAllowed);
          Level topLevel = null;
          if (hierarchyGrant.topLevel != null) {
            if (hierarchyAccess != Access.CUSTOM) {
              throw Util.newError("You may only specify 'topLevel' if "
                + "access='custom'");
            }
            topLevel = (Level) schemaReader.lookupCompound(cube, Util
              .parseIdentifier(hierarchyGrant.topLevel), true, Category.Level);
          }
          Level bottomLevel = null;
          if (hierarchyGrant.bottomLevel != null) {
            if (hierarchyAccess != Access.CUSTOM) {
              throw Util.newError("You may only specify 'bottomLevel' if "
                + "access='custom'");
            }
            bottomLevel = (Level) schemaReader.lookupCompound(cube, Util
              .parseIdentifier(hierarchyGrant.bottomLevel), true, Category.Level);
          }
          Role.RollupPolicy rollupPolicy;
          if (hierarchyGrant.rollupPolicy != null) {
            try {
              rollupPolicy = Role.RollupPolicy.valueOf(hierarchyGrant.rollupPolicy
                .toUpperCase());
          ...
          role
            .grant(hierarchy, hierarchyAccess, topLevel, bottomLevel, rollupPolicy);
          for (MondrianDef.MemberGrant memberGrant : hierarchyGrant.memberGrants) {
            if (hierarchyAccess != Access.CUSTOM) {
              throw Util.newError("You may only specify <MemberGrant> if "
                + "<Hierarchy> has access='custom'");
            }
            final boolean ignoreInvalidMembers = MondrianProperties.instance().IgnoreInvalidMembers
              .get();
            Member member = schemaReader.getMemberByUniqueName(Util
              .parseIdentifier(memberGrant.member), !ignoreInvalidMembers);
            if (member == null) {
            ...
            if (member.getHierarchy() != hierarchy) {
              throw Util.newError("Member '" + member + "' is not in hierarchy '"
                + hierarchy + "'");
            }
  ...
  public Dimension createDimension(Cube cube, String xml) {
    ...
    try {
      ...
      throw Util.newError(e, "Error while adding dimension to cube '" + cube
        + "' from XML [" + xml + "]");
    }
  ...
  /**
   * A collection of schemas, identified by their connection properties (catalog
   * name, JDBC URL, and so forth).
   * 
   * <p>
   * To lookup a schema, call <code>Pool.instance().{@link #get}</code>.
   */
  static class Pool {
     ...
     * 
     * @param value
     *          String to create one way hash upon.
     * @return MD5 hash.
     */
    ...
    synchronized RolapSchema get(final String catalogUrl,
      final String connectionKey, final String jdbcUser, final String dataSourceStr,
      final Util.PropertyList connectInfo) {
      return get(catalogUrl, connectionKey, jdbcUser, dataSourceStr, null,
        connectInfo);
    }

    synchronized RolapSchema get(final String catalogUrl,
      final DataSource dataSource, final Util.PropertyList connectInfo) {
      return get(catalogUrl, null, null, null, dataSource, connectInfo);
    }

    private RolapSchema get(final String catalogUrl, final String connectionKey,
      final String jdbcUser, final String dataSourceStr,
      final DataSource dataSource, final Util.PropertyList connectInfo) {
      String key = (dataSource == null) ? makeKey(catalogUrl, connectionKey,
        jdbcUser, dataSourceStr) : makeKey(catalogUrl, dataSource);

      RolapSchema schema = null;

      String dynProcName = connectInfo
        .get(RolapConnectionProperties.DynamicSchemaProcessor.name());

      String catalogStr = connectInfo.get(RolapConnectionProperties.CatalogContent
        .name());
      ...
      final boolean useContentChecksum = Boolean.parseBoolean(connectInfo
        .get(RolapConnectionProperties.UseContentChecksum.name()));

      // Use the schema pool unless "UseSchemaPool" is explicitly false.
      final boolean useSchemaPool = Boolean.parseBoolean(connectInfo.get(
        RolapConnectionProperties.UseSchemaPool.name(), "true"));
      ...
      if (!Util.isEmpty(dynProcName)) {
        ...
        try {
          @SuppressWarnings("unchecked")
          final Class<DynamicSchemaProcessor> clazz = (Class<DynamicSchemaProcessor>) Class
            .forName(dynProcName);
          final Constructor<DynamicSchemaProcessor> ctor = clazz.getConstructor();
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("Pool.get: create schema \"" + catalogUrl
            + "\" using dynamic processor");
        }
      }

      if (!useSchemaPool) {
        schema = new RolapSchema(key, null, catalogUrl, catalogStr, connectInfo,
          dataSource);
        ...
        if (md5Bytes != null) {
          SoftReference<RolapSchema> ref = mapUrlToSchema.get(md5Bytes);
          if (ref != null) {
            schema = ref.get();
            if (schema == null) {
        ...
        if (schema == null || md5Bytes == null || schema.md5Bytes == null
          || !schema.md5Bytes.equals(md5Bytes)) {
          schema = new RolapSchema(key, md5Bytes, catalogUrl, catalogStr,
            connectInfo, dataSource);
          ...
          if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Pool.get: create schema \"" + catalogUrl + "\" with MD5");
          }

        } else if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("Pool.get: schema \"" + catalogUrl
        ...
        if (schema == null) {
          ...
          LOGGER.debug("Pool.get: schema \"" + catalogUrl + "\" exists already ");
        }
      }
      return schema;
    }

    synchronized void remove(final String catalogUrl, final String connectionKey,
      final String jdbcUser, final String dataSourceStr) {
      final String key = makeKey(catalogUrl, connectionKey, jdbcUser, dataSourceStr);
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("Pool.remove: schema \"" + catalogUrl
          + "\" and datasource string \"" + dataSourceStr + "\"");
      }
      remove(key);
    }

    synchronized void remove(final String catalogUrl, final DataSource dataSource) {
    ...
    synchronized void remove(RolapSchema schema) {
      if (schema != null) {
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("Pool.remove: schema \"" + schema.name
            + "\" and datasource object");
        }
        remove(schema.key);
      }
    ...
    synchronized List<RolapSchema> getRolapSchemas() {
      ...
      for (Iterator<SoftReference<RolapSchema>> it = mapUrlToSchema.values()
        .iterator(); it.hasNext();) {
     ...
     */
    private static String makeKey(final String catalogUrl,
      final String connectionKey, final String jdbcUser, final String dataSourceStr) {
  ...
  protected MondrianDef.CalculatedMember lookupXmlCalculatedMember(
    final String calcMemberName, final String cubeName) {
  ...
  private String calcMemberFqName(MondrianDef.CalculatedMember xmlCalcMember) {
    if (xmlCalcMember.dimension != null) {
      return Util.makeFqName(Util.quoteMdxIdentifier(xmlCalcMember.dimension),
        xmlCalcMember.name);
    } else {
  ...
  public Hierarchy[] getSharedHierarchies() {
    Collection<RolapHierarchy> hierarchies = mapSharedHierarchyNameToHierarchy
      .values();
   ...
   * 
   * <p>
   * If the function is not valid, throws an error.
   * 
   * @param name
   *          Name of the function.
   * @param className
   *          Name of the class which implements the function. The class must
   *          implement {@link mondrian.spi.UserDefinedFunction} (otherwise it
   *          is a user-error).
   */
  private void defineFunction(Map<String, UdfResolver.UdfFactory> mapNameToUdf,
    final String name, String className, final Scripts.ScriptDefinition script) {
    if (className == null && script == null) {
      throw Util
        .newError("Must specify either className attribute or Script element");
    }
  ...
  /**
   * Throws an error if a user-defined function does not adhere to the API.
   */
  private void validateFunction(UdfResolver.UdfFactory udfFactory) {
    ...
    for (int i = 0; i < parameterTypes.length; i++) {
      ...
      if (parameterType == null) {
        throw Util.newInternal("Invalid user-defined function '" + udfName
   ...
   * hierarchy is shared (<code>sharedName</code> is not null), looks up a
   * reader from a cache, or creates one if necessary.
   * 
   * <p>
   * Synchronization: thread safe
   */
  synchronized MemberReader createMemberReader(final String sharedName,
    final RolapHierarchy hierarchy, final String memberReaderClass) {
    MemberReader reader;
    if (sharedName != null) {
      ...
      if (reader == null) {
         ...
         * } else { mapSharedHierarchyNameToHierarchy.put(sharedName,
         * hierarchy); }
         */
   ...
   */
  private MemberReader createMemberReader(final RolapHierarchy hierarchy,
    ...
    if (memberReaderClass != null) {
      ...
      try {
        ...
        Constructor<?> constructor = clazz.getConstructor(RolapHierarchy.class,
          Properties.class);
      ...
      throw Util.newInternal(e2, "while instantiating member reader '"
        + memberReaderClass);
    } else {
      SqlMemberSource source = new SqlMemberSource(hierarchy);
      if (hierarchy.getDimension().isHighCardinality()) {
        LOGGER.debug("High cardinality for " + hierarchy.getDimension());
  ...
  /**
   * Creates a {@link DataSourceChangeListener} with which to detect changes to
   * datasources.
   */
  private DataSourceChangeListener createDataSourceChangeListener(
    ...
    // everything else. In particular, ignore the dynamic schema
    // processor.
    String dataSourceChangeListenerStr = connectInfo
      .get(RolapConnectionProperties.DataSourceChangeListener.name());
    ...
    if (!Util.isEmpty(dataSourceChangeListenerStr)) {
      try {
         ...
         * Class.forName(dataSourceChangeListenerStr); final
         * Constructor<DataSourceChangeListener> ctor = clazz.getConstructor();
      ...
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("RolapSchema.createDataSourceChangeListener: "
          + "create datasource change listener \"" + dataSourceChangeListenerStr);
      }
    }
  ...
  /**
   * Returns the checksum of this schema. Returns <code>null</code>
  ...
  /**
   * Connection for purposes of parsing and validation. Careful! It won't have
   * the correct locale or access-control profile.
   */
  ...
  /**
   * Returns the cached cardinality for the column. The cache is stored in the
   * schema so that queries on different cubes can share them.
   * 
  ...
  Integer getCachedRelationExprCardinality(MondrianDef.Relation relation,
    ...
    synchronized (relationExprCardinalityMap) {
      Map<MondrianDef.Expression, Integer> exprCardinalityMap = relationExprCardinalityMap
        .get(relation);
   ...
   * 
   * @param relation
   *          the relation associated with the column expression
   * @param columnExpr
   *          the column expression to cache the cardinality for
   * @param cardinality
   *          the cardinality for the column expression
   */
  void putCachedRelationExprCardinality(MondrianDef.Relation relation,
    MondrianDef.Expression columnExpr, Integer cardinality) {
    synchronized (relationExprCardinalityMap) {
      Map<MondrianDef.Expression, Integer> exprCardinalityMap = relationExprCardinalityMap
        .get(relation);
      if (exprCardinalityMap == null) {
        exprCardinalityMap = new HashMap<MondrianDef.Expression, Integer>();
  ...
  /**
   * Pushes all modifications of the aggregations to global cache, so other
