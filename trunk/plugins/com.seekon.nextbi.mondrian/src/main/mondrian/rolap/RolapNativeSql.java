...
public class RolapNativeSql {
	...
	class MatchingSqlCompiler extends FunCallSqlCompilerBase {
		...
		public String compile(Exp exp) {
			...
			if (!(currMemberExpr instanceof ResolvedFunCall)
					...
					|| !((ResolvedFunCall) currMemberExpr).getFunName().equals(
							"CurrentMember")) {
				return null;
			}
			...
			if (rolapLevel != null && dimension.equals(rolapLevel.getDimension())) {
				...
				MondrianDef.Expression expression = useCaption ? rolapLevel.captionExp == null ? rolapLevel.nameExp == null ? rolapLevel.keyExp
						...
						: rolapLevel.nameExp == null ? rolapLevel.keyExp
								: rolapLevel.nameExp;
				/*
				...
				return dialect.generateRegularExpression(sourceExp, String
						.valueOf(evaluator.getCachedResult(new ExpCacheDescriptor(arg1,
								evaluator))));
			} else {
				return null;
			}
	 ...
	 *          modified
	 */
	public RolapNativeSql(SqlQuery sqlQuery, AggStar aggStar,
			Evaluator evaluator, RolapLevel rolapLevel) {
		...
		numericCompiler.add(new ParenthesisSqlCompiler(Category.Numeric,
				numericCompiler));
		...
		booleanCompiler.add(new ParenthesisSqlCompiler(Category.Logical,
				booleanCompiler));
