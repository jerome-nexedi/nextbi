/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapNamedSetEvaluator.java#10 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapNamedSetEvaluator.java
 *          #10 $
 */
class RolapNamedSetEvaluator implements Evaluator.NamedSetEvaluator,
    ...
    if (list != null) {
      if (list == DUMMY_LIST) {
        throw rrer.result.slicerEvaluator.newEvalException(null,
          "Illegal attempt to reference value of named set '" + namedSet.getName()
            + "' while evaluating itself");
      }
      return;
    }
    if (RolapResult.LOGGER.isDebugEnabled()) {
      RolapResult.LOGGER.debug("Named set " + namedSet.getName()
        + ": starting evaluation");
    }
    list = DUMMY_LIST; // recursion detection
    final Calc calc = rrer.getCompiled(namedSet.getExp(), false,
      ResultStyle.ITERABLE);
    TupleIterable iterable = (TupleIterable) rrer.result.evaluateExp(calc,
