...
public class RolapStar {
	...
	private static class StarNetworkNode {
		...
		private boolean isCompatible(StarNetworkNode compatibleParent,
				MondrianDef.Relation rel, String compatibleForeignKey,
	...
	private MondrianDef.RelationOrJoin getUniqueRelation(StarNetworkNode parent,
		...
		if (relOrJoin == null) {
			...
			if (join.getLeftAlias().equals(joinKeyTable)) {
				...
				right = getUniqueRelation(parent, join.right, join.leftKey,
				...
				left = getUniqueRelation(parent, join.left, join.rightKey,
			...
			if (join.left != left || join.right != right) {
				join = new MondrianDef.Join(
						left instanceof MondrianDef.Relation ? ((MondrianDef.Relation) left).getAlias()
								: null,
						join.leftKey,
						left,
	...
	private boolean containsColumn(String tableName, String columnName) {
		...
		try {
			...
			throw Util.newInternal(e1,
					"Error while creating connection from data source");
		}
		try {
	 ...
	 *          List of column names (must have same cardinality as
	 *          {@code columnList})
	...
	public void flush(CacheControl cacheControl, CacheControl.CellRegion region) {
		...
		final RolapCacheRegion cacheRegion = RolapAggregationManager
				.makeCacheRegion(this, region);
	...
	public static class Column {
		...
		private Column(String name, Table table, MondrianDef.Expression expression,
				Dialect.Datatype datatype, SqlStatement.Type internalType,
		...
		protected Column(Dialect.Datatype datatype) {
			this(null, null, null, datatype, null, null, null, null,
					Integer.MIN_VALUE, 0);
		}
		...
		public int getCardinality() {
			if (approxCardinality == Integer.MIN_VALUE) {
				...
				Integer card = schema.getCachedRelationExprCardinality(
						table.getRelation(), expression);
				...
				if (card != null) {
					...
					schema.putCachedRelationExprCardinality(table.getRelation(),
							expression, approxCardinality);
				}
			}
		...
		private int getCardinality(DataSource dataSource) {
			...
			final SqlStatement stmt = RolapUtil.executeQuery(
					dataSource,
					sql,
					new Locus(Locus.peek().execution, "RolapStar.Column.getCardinality",
		...
		public static String createInExpr(final String expr,
				StarColumnPredicate predicate, Dialect.Datatype datatype,
				SqlQuery sqlQuery) {
		...
		public String getDatatypeString(Dialect dialect) {
			...
			query.addFrom(table.star.factTable.relation, table.star.factTable.alias,
					false);
			...
			try {
				...
				throw Util.newError(e, "Error while deriving type of column "
						+ toString());
			} finally {
	...
	public static class Measure extends Column {
		...
		public Measure(String name, String cubeName, RolapAggregator aggregator,
				Table table, MondrianDef.Expression expression,
				Dialect.Datatype datatype) {
	...
	public static class Table {
		...
		private Table(RolapStar star, MondrianDef.Relation relation, Table parent,
			...
			this.parent = parent;
			final AliasReplacer aliasReplacer = new AliasReplacer(
					relation.getAlias(), this.alias);
		...
		synchronized void makeMeasure(RolapBaseCubeMeasure measure) {
			...
			RolapStar.Measure starMeasure = new RolapStar.Measure(measure.getName(),
					measure.getCube().getName(), measure.getAggregator(), this,
					measure.getMondrianDefExpression(), measure.getDatatype());
		...
		synchronized Column makeColumns(RolapCube cube, RolapCubeLevel level,
			...
			if (level.getNameExp() != null) {
				...
				nameColumn = makeColumnForLevelExpr(cube, level, level.getName(),
						level.getNameExp(), Dialect.Datatype.String, null, null, null, null);
			}

			// select the column's name depending upon whether or not a
			// "named" column, above, has been created.
			String name = (level.getNameExp() == null) ? level.getName() : level
					.getName() + " (Key)";

			// If the nameColumn is not null, then it is associated with this
			// column.
			Column column = makeColumnForLevelExpr(cube, level, name,
					level.getKeyExp(), level.getDatatype(), level.getInternalType(),
					nameColumn, parentColumn, usagePrefix);
		...
		private Column makeColumnForLevelExpr(RolapCube cube, RolapLevel level,
			...
			if (xmlExpr instanceof MondrianDef.Column) {
				...
				if (table == null) {
					throw Util.newError("Level '" + level.getUniqueName() + "' of cube '"
							...
							+ "' is not found in current scope" + Util.nl + ", star:"
							+ Util.nl + getStar());
				}
			...
			if (c != null && !c.equals(nameColumn)) {
				...
				column = new RolapStar.Column(name, table, xmlExpr, datatype,
						internalType, nameColumn, parentColumn, usagePrefix,
						level.getApproxRowCount(), star.nextColumnCount());
				addColumn(column);
			}
			return column;
		}
		...
		synchronized Table addJoin(RolapCube cube,
			...
			if (relationOrJoin instanceof MondrianDef.Relation) {
				...
				joinCondition = new RolapStar.Condition(new MondrianDef.Column(
						leftAlias, join.leftKey), new MondrianDef.Column(rightAlias,
						join.rightKey));
		 ...
		 */
		public void addToFrom(SqlQuery query, boolean failIfExists,
		...
		public boolean containsColumn(String columnName) {
			if (relation instanceof MondrianDef.Relation) {
				return star.containsColumn(
						((MondrianDef.Relation) relation).getAlias(), columnName);
			} else {
				// todo: Deal with join.
				return false;
			}
