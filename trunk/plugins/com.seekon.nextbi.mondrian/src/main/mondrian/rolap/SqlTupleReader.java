/*
// $Id: //open/mondrian/src/main/mondrian/rolap/SqlTupleReader.java#89 $
 ...
 * 
 * <p>
 * Allows the result to be restricted by a {@link TupleConstraint}. So the
 * SqlTupleReader can also read Member.Descendants (which is level.members
 ...
 * although it may do so to restrict the result. Also it is permitted to cache
 * the parent/children from all members in MemberCache, so
 * {@link TupleConstraint#getMemberChildrenConstraint(RolapMember)} should not
 * return null.</li>
 * 
 * <li>When reading multiple levels (i.e. we are performing a crossjoin), then
 * we can not store the parent/child pairs in the MemberCache and
 * {@link TupleConstraint#getMemberChildrenConstraint(RolapMember)} must return
 * null. Also
 ...
 * @author av
 * @since Nov 11, 2005
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/SqlTupleReader.java#89
 *          $
 */
public class SqlTupleReader implements TupleReader {
	private final static Logger LOGGER = Logger.getLogger(SqlTupleReader.class);
	 ...
	 * passes and will terminate in success when missedMemberCount == 0 at the end
	 * of a pass, or failure if a pass generates failures but does not manage to
	 * load any more members.
	 */
	private int missedMemberCount;
	private static final String UNION = " union ";

	/**
	 * Helper class for SqlTupleReader; keeps track of target levels and
	 * constraints for adding to sql query.
	 */
		...
		int internalAddRow(SqlStatement stmt, int column) throws SQLException {
			...
			if (getCurrMember() != null) {
				...
				for (int i = 0; i <= levelDepth; i++) {
					...
					if (parentChild) {
						Object parentValue = accessors.get(column++).get();
						if (parentValue == null) {
							...
							Object parentKey = cache.makeKey(member, parentValue);
							parentMember = cache.getMember(parentKey);
							if (parentMember == null) {
								LOGGER
										.warn(MondrianResource.instance().LevelTableParentNotFound
												.str(childLevel.getUniqueName(),
					...
					if (member == null) {
						...
						if (member == null) {
							member = memberBuilder.makeMember(parentMember, childLevel,
									value, captionValue, parentChild, stmt, key, column);
						}
					}
					...
					if (member != members.get(i)) {
						...
						if (children != null) {
							MemberChildrenConstraint mcc = constraint
									.getMemberChildrenConstraint(members.get(i));
							if (mcc != null) {
						...
						List<RolapMember> cachedChildren = cache.getChildrenFromCache(
								member, mcc);
	...
	protected void prepareTuples(DataSource dataSource, TupleList partialResult,
		...
		try {
			if (execQuery) {
				...
				stmt = RolapUtil.executeQuery(dataSource, sql, types, maxRows, 0,
						new Locus(Locus.peek().execution, "SqlTupleReader.readTuples "
			...
			while (moreRows) {
				...
				if (enumTargetCount == 0) {
					...
					addTargets(0, firstEnumTarget, enumTargetCount, srcMemberIdxes, stmt,
							message);
	...
	public TupleList readMembers(DataSource dataSource, TupleList partialResult,
			List<List<RolapMember>> newPartialResult) {
	...
	public TupleList readTuples(DataSource jdbcConnection,
			TupleList partialResult, List<List<RolapMember>> newPartialResult) {
		...
		TupleList tupleList = n == 1 ? new UnaryTupleList(members)
	...
	/**
	 * Sets the current member for those targets that retrieve their column values
	 * from native sql
	 * 
	 * @param partialRow
	 *          if set, previously cached result set
	 */
	...
	/**
	 * Recursively forms the cross product of a row retrieved through sql with
	 * each of the targets that contains an enumerated set of members.
	 * 
	 * @param currEnumTargetIdx
	 *          current enum target that recursion is being applied on
	 * @param currTargetIdx
	 *          index within the list of a targets that currEnumTargetIdx
	 *          corresponds to
	 * @param nEnumTargets
	 *          number of targets that have enumerated members
	 * @param srcMemberIdxes
	 *          for each enumerated target, the current member to be retrieved to
	 *          form the current cross product row
	 * @param stmt
	 *          Statement containing the result set corresponding to rows
	 *          retrieved through native SQL
	 * @param message
	 *          Message to issue on failure
	 */
	private void addTargets(int currEnumTargetIdx, int currTargetIdx,
			int nEnumTargets, int[] srcMemberIdxes, SqlStatement stmt, String message) {
		...
		for (int i = 0; i < currTarget.srcMembers.size(); i++) {
			...
			if (currEnumTargetIdx < nEnumTargets - 1) {
				...
				addTargets(currEnumTargetIdx + 1, nextTargetIdx, nEnumTargets,
				...
				for (TargetBase target : targets) {
					if (target.srcMembers == null) {
						...
						RolapMember member = target.srcMembers
								.get(srcMemberIdxes[enumTargetIdx++]);
	...
	/**
	 * Retrieves the current members fetched from the targets executed through sql
	 * and form tuples, adding them to partialResult
	 * 
	 * @param partialResult
	 *          list containing the columns and rows corresponding to data fetched
	 *          through sql
	 */
	...
	Pair<String, List<SqlStatement.Type>> makeLevelMembersSql(
		...
		if (virtualCube) {
			...
			Collection<RolapCube> fullyJoiningBaseCubes = getFullyJoiningBaseCubes(baseCubes);
			...
			for (RolapCube baseCube : fullyJoiningBaseCubes) {
				...
				WhichSelect whichSelect = (++k == fullyJoiningBaseCubes.size() - 1) ? WhichSelect.LAST
						: WhichSelect.NOT_LAST;
				selectString.append(prependString);
				final Pair<String, List<SqlStatement.Type>> pair = generateSelectForLevels(
	...
	private Collection<RolapCube> getFullyJoiningBaseCubes(
		...
		final Collection<RolapCube> fullyJoiningCubes = new ArrayList<RolapCube>();
	...
	Collection<RolapCube> getBaseCubeCollection(final Query query) {
		...
		Collection<RolapCube> baseCubes = new TreeSet<RolapCube>(cubeComparator);
	 ...
	 * 
	 * @param dataSource
	 *          jdbc connection that they query will execute against
	 * @param baseCube
	 *          this is the cube object for regular cubes, and the underlying base
	 *          cube for virtual cubes
	 * @param whichSelect
	...
	Pair<String, List<SqlStatement.Type>> generateSelectForLevels(
			DataSource dataSource, RolapCube baseCube, WhichSelect whichSelect) {
		...
		for (TargetBase target : targets) {
			...
			if (target.getSrcMembers() == null) {
				addLevelMemberSql(sqlQuery, target.getLevel(), baseCube, whichSelect,
						aggStar);
			}
		}
	...
	boolean targetIsOnBaseCube(TargetBase target, RolapCube baseCube) {
		return baseCube == null
				|| baseCube.findBaseCubeHierarchy(target.getLevel().getHierarchy()) != null;
	}
	 ...
	 * properties in the included levels are functionally dependent on their level
	 * values.
	 * </p>
	 * 
	 * 
	 ...
	 * @param levelDepth
	 *          Level depth at which the query is occuring
	 * @return whether the GROUP BY is needed
	 * 
	 */
	private boolean isGroupByNeeded(SqlQuery sqlQuery, RolapHierarchy hierarchy,
			RolapLevel[] levels, int levelDepth) {
		/*
		 * Figure out if we need to generate GROUP BY at all. It may only be
		 * eliminated if we are at a depth that includes the unique key level, and
		 * all properties of included levels depend on the level value.
		 */
	 ...
	 * </pre>
	 * 
	 * </blockquote> accesses the "City" level of the "Customers" hierarchy. Note
	 * that:
	 * <ul>
	 * 
	 ...
	 *          the query object being constructed
	 * @param level
	 *          level to be added to the sql query
	 * @param baseCube
	 *          this is the cube object for regular cubes, and the underlying base
	 *          cube for virtual cubes
	 * @param whichSelect
	 ...
	 */
	protected void addLevelMemberSql(SqlQuery sqlQuery, RolapLevel level,
			RolapCube baseCube, WhichSelect whichSelect, AggStar aggStar) {
		...
		boolean needsGroupBy = isGroupByNeeded(sqlQuery, hierarchy, levels,
				levelDepth);
		...
		for (int i = 0; i <= levelDepth; i++) {
			...
			boolean multipleCols = SqlMemberSource
					.levelContainsMultipleColumns(currLevel);
			...
			if (levelCollapsed && !multipleCols) {
				...
				RolapStar.Column starColumn = ((RolapCubeLevel) currLevel)
						.getStarKeyColumn();
			...
			if (levelCollapsed) {
				...
				int bitPos = starColumn.getBitPosition();
				AggStar.Table.Column aggColumn = aggStar.lookupColumn(bitPos);
				RolapStar.Condition condition = new RolapStar.Condition(keyExp,
			...
			case LAST:
				...
				sqlQuery.addOrderBy(
						Integer.toString(sqlQuery.getCurrentSelectListSize()), true, false,
						nullable);
			...
			for (RolapProperty property : properties) {
				...
				if (propExp instanceof MondrianDef.Column) {
					...
					propSql = sqlQuery.getDialect().quoteIdentifier(
							currLevel.getTableAlias(), ((MondrianDef.Column) propExp).name);
				} else {
	...
	/**
	 * Obtains the evaluator used to find an aggregate table to support the Tuple
	 * constraint.
	 * 
	 * @param constraint
	 *          Constraint
	...
	protected Evaluator getEvaluator(TupleConstraint constraint) {
		...
		if (constraint instanceof DescendantsConstraint) {
			...
			MemberChildrenConstraint mcc = descConstraint
	...
	/**
	 * Obtains the AggStar instance which corresponds to an aggregate table which
	 * can be used to support the member constraint.
	 * 
	 * @param constraint
	 * @param evaluator
	 *          the current evaluator to obtain the cube and members to be queried @return
	...
	AggStar chooseAggStar(TupleConstraint constraint, Evaluator evaluator) {
			...
			for (CrossJoinArg arg : necj.args) {
				if (arg instanceof DescendantsCrossJoinArg
					...
					if (level != null && !level.isAll()) {
						RolapStar.Column column = ((RolapCubeLevel) level)
								.getStarKeyColumn();
						levelBitKey.set(column.getBitPosition());
					}
				}
		...
		AggStar aggStar = AggregationManager.instance().findAgg(star, levelBitKey,
	...
	/**
	 * Description of the position of a SELECT statement in a UNION. Queries on
	 * virtual cubes tend to generate unions.
	 */
	enum WhichSelect {
		/**
		...
		ONLY,
		/**
		 * Select statement belongs to a UNION, but is not the last. Typically this
		 * occurs when querying a virtual cube.
		 */
		NOT_LAST,
		/**
		 * Select statement is the last in a UNION. Typically this occurs when
		 * querying a virtual cube.
		 */
		LAST
	}
}
