/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapResult.java#150 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapResult.java#150 $
 */
public class RolapResult extends ResultBase {
	...
	private final CellReader aggregatingReader = AggregationManager.instance()
			.getCacheCellReader();
	private Modulos modulos = null;
	private final int maxEvalDepth = MondrianProperties.instance().MaxEvalDepth
			.get();
	...
	RolapResult(final Execution execution, boolean execute) {
		...
		this.cellInfos = (query.axes.length > 4) ? new CellInfoMap(point)
		...
		try {
			...
			loadMembers(emptyNonAllMembers, evaluator, query.getSlicerAxis(),
					query.slicerCalc, axisMembers);
			...
			RolapEvaluator slicerEvaluator;
			do {
				TupleIterable tupleIterable = evalExecute(nonAllMembers,
						nonAllMembers.size() - 1, savedEvaluator, query.getSlicerAxis(),
						query.slicerCalc);
				...
				if (tupleList.size() > 1) {
					tupleList = AggregateFunDef.AggregateCalc.optimizeTupleList(
							slicerEvaluator, tupleList);

					final Calc valueCalc = new ValueCalc(new DummyExp(new ScalarType()));
					final TupleList tupleList1 = tupleList;
					final Calc calc = new GenericCalc(new DummyExp(
							query.slicerCalc.getType())) {
						public Object evaluate(Evaluator evaluator) {
							return AggregateFunDef.AggregateCalc.aggregate(valueCalc,
									evaluator, tupleList1);
						}
					};
					...
					evaluator.addCalculation(new RolapTupleCalculation(hierarchyList,
							calc), true);
				}
			...
			do {
				...
				do {
					...
					for (int i = 0; i < axes.length; i++) {
						...
						final Calc calc = query.axisCalcs[i];
						TupleIterable tupleIterable = evalExecute(nonAllMembers,
								nonAllMembers.size() - 1, evaluator, axis, calc);
						...
						this.axes[i] = new RolapAxis(TupleCollections.materialize(
								tupleIterable, false));
					}
				} while (redo);
			} while (batchingReader.loadAggregations(query));
	...
	protected boolean replaceNonAllMembers(List<List<Member>> nonAllMembers,
		...
		for (ListIterator<List<Member>> it = nonAllMembers.listIterator(); it
				.hasNext();) {
	...
	protected void loadMembers(List<List<Member>> nonAllMembers,
		...
		while (true) {
			...
			evalLoad(nonAllMembers, nonAllMembers.size() - 1, evaluator, axis, calc,
					axisMembers);
	...
	void evalLoad(List<List<Member>> nonAllMembers, int cnt, Evaluator evaluator,
			QueryAxis axis, Calc calc, AxisMember axisMembers) {
		if (cnt < 0) {
			final int savepoint = evaluator.savepoint();
	...
	TupleIterable evalExecute(List<List<Member>> nonAllMembers, int cnt,
			RolapEvaluator evaluator, QueryAxis queryAxis, Calc calc) {
		final int arity = calc == null ? 0 : calc.getType().getArity();
		if (cnt < 0) {
			final int savepoint = evaluator.savepoint();
			final TupleIterable axis = executeAxis(evaluator, queryAxis, calc, true,
					null);
			evaluator.restore(savepoint);
			return axis;
			...
			for (Member m : nonAllMembers.get(cnt)) {
				evaluator.setContext(m);
				TupleIterable axis = evalExecute(nonAllMembers, cnt - 1, evaluator,
						queryAxis, calc);
	...
	/**
	 * Finds all root Members 1) whose Hierarchy does not have an ALL Member, 2)
	 * whose default Member is not the ALL Member and 3) all Measures.
	 * 
	 * @param nonDefaultAllMembers
	 *          List of all root Members for Hierarchies whose default Member is
	 *          not the ALL Member.
	 * @param nonAllMembers
	...
	protected void loadSpecialMembers(List<Member> nonDefaultAllMembers,
			List<List<Member>> nonAllMembers, List<Member> measureMembers) {
	...
	private TupleIterable executeAxis(Evaluator evaluator, QueryAxis queryAxis,
			Calc axisCalc, boolean construct, AxisMember axisMembers) {
		if (queryAxis == null) {
			// Create an axis containing one position with no members (not
			// the same as an empty axis).
			return new DelegatingTupleList(0, Collections.singletonList(Collections
					.<Member> emptyList()));
		}
		final int savepoint = evaluator.savepoint();
		evaluator.setNonEmpty(queryAxis.isNonEmpty());
		evaluator.setEvalAxes(true);
		final TupleIterable iterable = ((IterCalc) axisCalc)
	...
	private void executeBody(RolapEvaluator evaluator, Query query,
			final int[] pos) {
			...
			if (count++ > maxEvalDepth) {
				if (evaluator instanceof RolapDependencyTestingEvaluator) {
					...
					if (count > maxEvalDepth * 2) {
						throw Util.newInternal("Query required more than " + count
								+ " iterations");
					}
				} else {
					throw Util.newInternal("Query required more than " + count
		...
		if (axisOrdinal < 0) {
			...
			for (List<Member> members : tupleList) {
				...
				try {
					...
					RolapMeasure m = (RolapMeasure) revaluator
							.getContext(measuresHierarchy);
			...
			if (isAxisHighCardinality(axisOrdinal, tupleList)) {
				...
				for (List<Member> tuple : tupleList) {
					List<Member> measures = new ArrayList<Member>(statement.getQuery()
							.getMeasuresMembers());
	...
	private boolean isAxisHighCardinality(int axisOrdinal, TupleList tupleList) {
		...
		if (highCardinality == null) {
			...
			for (List<Member> tuple : tupleList) {
				if (!tuple.isEmpty()) {
					highCardinality = tuple.get(0).getDimension().isHighCardinality();
				}
				break;
			}
	...
	/*
	 * Distinct counts are aggregated separately from other measures. We need to
	 * apply filters to each level in the query.
	 * 
	 * Replace VisualTotalMember expressions with new expressions where all leaf
	 * level members are included.
	 * 
	 * Example: For MDX query: WITH SET [XL_Row_Dim_0] AS VisualTotals( Distinct(
	 * Hierarchize( {Ascendants([Store].[All Stores].[USA].[CA]),
	 * Descendants([Store].[All Stores].[USA].[CA])}))) select NON EMPTY
	 * Hierarchize( Intersect( {DrilldownLevel({[Store].[All Stores]})},
	 * [XL_Row_Dim_0])) ON COLUMNS from [HR] where [Measures].[Number of
	 * Employees]
	 * 
	 * For member [Store].[All Stores]: we replace aggregate expression -
	 * Aggregate({[Store].[All Stores].[USA]}) with - Aggregate({[Store].[All
	 * Stores].[USA].[CA].[Alameda].[HQ], [Store].[All Stores].[USA].[CA].[Beverly
	 * Hills].[Store 6], [Store].[All Stores].[USA].[CA].[Los Angeles].[Store 7],
	 * [Store].[All Stores].[USA].[CA].[San Diego].[Store 24], [Store].[All
	 * Stores].[USA].[CA].[San Francisco].[Store 14] }) TODO: Can be optimized.
	 * For that particular query we don't need to go to the lowest level. We can
	 * simply replace it with: - Aggregate({[Store].[All Stores].[USA].[CA]})
	 * Because all children of [Store].[All Stores].[USA].[CA] are included.
	 */
	private List<Member> processDistinctMeasureExpr(List<Member> tuple,
	...
	/**
	 * Called only by RolapCell. Use this when creating an Evaluator is not
	 * required.
	 * 
	 * @param pos
	 *          Coordinates of cell
	...
	/**
	 * Counts and collects Members found of the axes. This class does two things.
	 * First it collects all Members found during the Member-Determination phase.
	 * Secondly, it counts how many Members are on each axis and forms the
	 * product, the totalCellCount which is checked against the ResultLimit
	 * property value.
	 */
	private static class AxisMember implements Iterable<Member> {
			...
			if (this.limit > 0) {
				...
				if (this.totalCellCount > this.limit) {
					throw MondrianResource.instance().TotalMembersLimitExceeded.ex(
							this.totalCellCount, this.limit);
				}
				this.axisCount = 0;
			}
		}
		...
		private void mergeTuple(final TupleCursor cursor) {
			 ...
			 * cursor.currentToArray(members1, 0); if
			 * (FunUtil.tupleContainsNullMember(members1)) { return; }
			 */
	...
	/**
	 * Extension to {@link RolapEvaluatorRoot} which is capable of evaluating
	 * named sets.
	 * <p/>
	 * 
	...
	protected static class RolapResultEvaluatorRoot extends RolapEvaluatorRoot {
		...
		protected Evaluator.NamedSetEvaluator evaluateNamedSet(
				final NamedSet namedSet, boolean create) {
		...
		public Object getParameterValue(ParameterSlot slot) {
			...
			if (liftedValue != null) {
				if (liftedValue == CycleSentinel) {
					throw MondrianResource.instance().CycleDuringParameterEvaluation
							.ex(slot.getParameter().getName());
				}
			...
			value = result.evaluateExp(slot.getDefaultValueCalc(),
					result.slicerEvaluator);
			if (value == null) {
	 ...
	 * 
	 * <p>
	 * Every Cell has a value, a format string (or CellFormatter) and a formatted
	 * value string. There are a wide range of possible values (pick a Double, any
	 * Double - its a value). Because there are lots of possible values, there are
	 * also lots of possible formatted value strings. On the other hand, there are
	 * only a very small number of format strings and CellFormatter's. These
	 * formatters are to be cached in a synchronized HashMaps in order to limit
	 * how many copies need to be kept around.
	 * 
	 * <p>
	...
	/**
	 * A CellFormatterValueFormatter uses a user-defined {@link CellFormatter} to
	 * format values.
	 */
	...
	/**
	 * A FormatValueFormatter takes a {@link Locale} as a parameter and uses it to
	 * get the {@link mondrian.util.Format} to be used in formatting an Object
	 * value with a given format string.
	 */
	...
	/*
	 * Generate a long ordinal based upon the values of the integers stored in the
	 * cell position array. With this mechanism, the Cell information can be
	 * stored using a long key (rather than the array integer of positions) thus
	 * saving memory. The trick is to use a 'large number' per axis in order to
	 * convert from position array to long key where the 'large number' is greater
	 * than the number of members in the axis. The largest 'long' is
	 * java.lang.Long.MAX_VALUE which is 9,223,372,036,854,776,000. The product of
	 * the maximum number of members per axis must be less than this maximum
	 * 'long' value (otherwise one gets hashing collisions). <p> For a single
	 * axis, the maximum number of members is equal to the max 'long' number,
	 * 9,223,372,036,854,776,000. <p> For two axes, the maximum number of members
	 * is the square root of the max 'long' number, 9,223,372,036,854,776,000,
	 * which is slightly bigger than 2,147,483,647 (which is the maximum integer).
	 * <p> For three axes, the maximum number of members per axis is the cube root
	 * of the max 'long' which is about 2,000,000 <p> For four axes the forth root
	 * is about 50,000. <p> For five or more axes, the maximum number of members
	 * per axis based upon the root of the maximum 'long' number, start getting
	 * too small to guarantee that it will be smaller than the number of members
	 * on a given axis and so we must resort to the Map-base Cell container.
	 */

	/**
	 * Synchronized Map from Locale to ValueFormatter. It is expected that there
	 * will be only a small number of Locale's. Should these be a WeakHashMap?
	 */
	...
	/**
	 * A CellInfo contains all of the information that a Cell requires. It is
	 * placed in the cellInfos map during evaluation and serves as a constructor
	 * parameter for {@link RolapCell}.
	 * 
	 * <p>
	...
	static class CellInfo {
		...
		/**
		 * Creates a CellInfo with position, value, format string and formatter of a
		 * cell.
		 * 
		 * @param key
		 *          Ordinal representing the position of a cell
		 ...
		 */
		CellInfo(long key, Object value, String formatString,
				ValueFormatter valueFormatter) {
			this.key = key;
			this.value = value;
	...
	/**
	 * API for the creation and lookup of {@link CellInfo} objects. There are two
	 * implementations, one that uses a Map for storage and the other uses an
	 * ObjectPool.
	 */
	interface CellInfoContainer {
		 ...
		 * current entries. This should be called after all CellInfo objects have
		 * been added to container.
		 */
		void trimToSize();

		/**
		 * Removes all CellInfo objects from container. Does not change the size of
		 * the internal data structures.
		 */
		void clear();

		/**
		 * Creates a new CellInfo object, adds it to the container a location
		 * <code>pos</code> and returns it.
		 * 
		 * @param pos
	 ...
	 * 
	 * <p>
	 * Note that the CellKey point instance variable is the same Object (NOT a
	 * copy) that is used and modified during the recursive calls to executeStripe
	 * - the <code>create</code> method relies on this fact.
	 */
	 ...
	 * 
	 * <p>
	 * There is an inner interface (<code>CellKeyMaker</code>) and implementations
	 * for 0 through 4 axes that convert the Cell position integer array into a
	 * long.
	 * 
	 * <p>
	 * It should be noted that there is an alternate approach. As the
	 * <code>executeStripe</code> method is recursively called, at each call it is
	 * known which axis is being iterated across and it is known whether or not
	 * the Position object for that axis is a List or just an Iterable. It it is a
	 * List, then one knows the real size of the axis. If it is an Iterable, then
	 * one has to use one of the MAX_AXIS_SIZE values. Given that this information
	 * is available when one recursives down to the next
	 * <code>executeStripe</code> call, the Cell ordinal, the position integer
	 * array could converted to an <code>long</code>, could be generated on the
	 * call stack!! Just a thought for the future.
	 */
	static class CellInfoPool implements CellInfoContainer {
		/**
		 * The maximum number of Members, 2,147,483,647, that can be any given Axis
		 * when the number of Axes is 2.
		 */
		protected static final long MAX_AXIS_SIZE_2 = 2147483647;
		/**
		 * The maximum number of Members, 2,000,000, that can be any given Axis when
		 * the number of Axes is 3.
		 */
		protected static final long MAX_AXIS_SIZE_3 = 2000000;
		/**
		 * The maximum number of Members, 50,000, that can be any given Axis when
		 * the number of Axes is 4.
		 */
		protected static final long MAX_AXIS_SIZE_4 = 50000;

		/**
		 * Implementations of CellKeyMaker convert the Cell position integer array
		 * to a <code>long</code>.
		 */
		...
		private static CellKeyMaker createCellKeyMaker(int axisLength) {
			...
			case 4:
				return new Four();
			default:
				throw new RuntimeException("Creating CellInfoPool with axisLength="
						+ axisLength);
			}
		}
	...
	static TupleList mergeAxes(TupleList axis1, TupleIterable axis2,
			boolean ordered) {
