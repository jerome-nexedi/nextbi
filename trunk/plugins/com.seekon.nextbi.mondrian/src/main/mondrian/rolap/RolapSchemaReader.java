...
public class RolapSchemaReader implements SchemaReader,
	...
	private static final Logger LOGGER = Logger
			.getLogger(RolapSchemaReader.class);
	...
	public int getChildrenCountFromCache(Member member) {
		...
		if (memberReader instanceof RolapCubeHierarchy.RolapCubeHierarchyMemberReader) {
			List list = ((RolapCubeHierarchy.RolapCubeHierarchyMemberReader) memberReader)
					.getRolapCubeMemberCacheHelper().getChildrenFromCache(
	...
	private int getLevelCardinalityFromCache(Level level) {
		...
		if (memberReader instanceof SmartMemberReader) {
			List<RolapMember> list = ((SmartMemberReader) memberReader)
					.getMemberCache().getLevelMembersFromCache((RolapLevel) level, null);
			if (list == null) {
				return Integer.MIN_VALUE;
			}
			return list.size();
	...
	public SchemaReader withoutAccessControl() {
		assert this.getClass() == RolapSchemaReader.class : "Subclass "
				+ getClass() + " must override";
	...
	public final Member getMemberByUniqueName(List<Id.Segment> uniqueNameParts,
		...
		return getMemberByUniqueName(uniqueNameParts, failIfNotFound,
				MatchType.EXACT);
	}
	...
	public OlapElement lookupCompound(OlapElement parent, List<Id.Segment> names,
		...
		return lookupCompound(parent, names, failIfNotFound, category,
				MatchType.EXACT);
	}
	...
	public final OlapElement lookupCompound(OlapElement parent,
		...
		return lookupCompoundInternal(parent, names, failIfNotFound, category,
				matchType);
	}
	...
	public OlapElement lookupChild(OlapElement parent, IdentifierSegment segment,
			MatchType matchType) {
		OlapElement element = getElementChild(parent, Util.convert(segment),
				matchType);
		if (element != null) {
			return element;
		}
	...
	public List<Member> getLevelMembers(Level level, Evaluator context) {
		TupleConstraint constraint = sqlConstraintFactory
				.getLevelMembersConstraint(context, new Level[] { level });
		final MemberReader memberReader = getMemberReader(level.getHierarchy());
	...
	public NativeEvaluator getNativeSetEvaluator(FunDef fun, Exp[] args,
		...
		RolapEvaluator revaluator = (RolapEvaluator) AbstractCalc
	...
	public Parameter getParameter(String name) {
		...
		List<Property> propertyList = MondrianProperties.instance()
				.getPropertyList();
