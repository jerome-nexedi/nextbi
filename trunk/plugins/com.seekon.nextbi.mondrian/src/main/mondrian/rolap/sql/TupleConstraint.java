/*
// $Id: //open/mondrian/src/main/mondrian/rolap/sql/TupleConstraint.java#16 $
 ...
 * @author av
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/sql/TupleConstraint.java#16
 *          $
 */
public interface TupleConstraint extends SqlConstraint {
	 ...
	 * @param baseCube
	 *          base cube for virtual cube constraints
	 */
	public void addConstraint(SqlQuery sqlQuery, RolapCube baseCube,
			AggStar aggStar);

	/**
	 * Will be called multiple times for every "group by" level in Level.Members
	 * query, i.e. the level that contains the members and all parent levels
	 * except All. If the condition requires so, it may join the levels table to
	 * the fact table.
	 * 
	 * @param query
	 *          the query to modify
	...
	/**
	 * When the members of a level are fetched, the result is grouped by into
	 * parents and their children. These parent/children are stored in the
	 * parent/children cache, whose key consists of the parent and the
	 * MemberChildrenConstraint#hashKey(). So we need a matching
	 * MemberChildrenConstraint to store the parent with its children into the
	...
	/**
	 * @return the evaluator currently associated with the constraint; null if
	 *         there is no associated evaluator
	 */
