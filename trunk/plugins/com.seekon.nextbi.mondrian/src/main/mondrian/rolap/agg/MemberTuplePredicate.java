/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/MemberTuplePredicate.java#10 $
...
/**
 * Predicate which constrains a column to a particular member, or a range above
 * or below a member, or a range between two members.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/MemberTuplePredicate
 *          .java#10 $
 */
public class MemberTuplePredicate implements StarPredicate {
  ...
  /**
   * Creates a MemberTuplePredicate which evaluates to true for a given range of
   * members.
   * 
   * <p>
   ...
   * @param lower
   *          Member which forms the lower bound, or null if range is open below
   * @param lowerStrict
   *          Whether lower bound of range is strict
   * @param upper
   *          Member which forms the upper bound, or null if range is open above
   * @param upperStrict
   *          Whether upper bound of range is strict
   */
  public MemberTuplePredicate(RolapCube baseCube, RolapMember lower,
    boolean lowerStrict, RolapMember upper, boolean upperStrict) {
    columnBitKey = null;
    this.columnList = computeColumnList(lower != null ? lower : upper, baseCube);

    if (lower == null) {
      assert upper != null;
      bounds = new Bound[] { new Bound(upper, upperStrict ? RelOp.LT : RelOp.LE) };
    } else if (upper == null) {
      bounds = new Bound[] { new Bound(lower, lowerStrict ? RelOp.GT : RelOp.GE) };
    } else {
      bounds = new Bound[] { new Bound(lower, lowerStrict ? RelOp.GT : RelOp.GE),
  ...
  private List<RolapStar.Column> computeColumnList(RolapMember member,
    RolapCube baseCube) {
    ...
    while (true) {
      ...
      if (columnBitKey == null) {
        columnBitKey = BitKey.Factory.makeBitKey(column.getStar().getColumnCount());
        columnBitKey.clear();
      }
  ...
  private enum RelOp {
    ...
    /**
     * If this is a strict operator (LT, GT) returns the non-strict equivalent
     * (LE, GE); otherwise returns this operator.
     * 
