/*
// $Id: //open/mondrian/src/main/mondrian/rolap/agg/AbstractQuerySpec.java#26 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/agg/AbstractQuerySpec.java
 *          #26 $
 */
public abstract class AbstractQuerySpec implements QuerySpec {
	 ...
	 * 
	 * @param star
	 *          Star which defines columns of interest and their relationships
	 * 
	 * @param countOnly
	 *          If true, generate no GROUP BY clause, so the query returns a
	 *          single row containing a grand total
	 */
	...
	protected void addMeasure(final int i, final SqlQuery sqlQuery) {
		...
		String exprInner = measure.getExpression() == null ? "*" : measure
				.generateExprString(sqlQuery);
		String exprOuter = measure.getAggregator().getExpression(exprInner);
		sqlQuery
				.addSelect(exprOuter, measure.getInternalType(), getMeasureAlias(i));
	}
	...
	/**
	 * Whether to add an ORDER BY clause to make results deterministic. Necessary
	 * if query returns more than one row and results are for human consumption.
	 * 
	...
	/**
	 * Generates a SQL query to retrieve the values in this segment using an
	 * algorithm which converts distinct-aggregates to non-distinct aggregates
	 * over subqueries.
	 * 
	 * @param outerSqlQuery
	 *          Query to modify
	 * @param countOnly
	 *          If true, only generate a single row: no need to generate a GROUP
	 *          BY clause or put any constraining columns in the SELECT clause
	 * @return A map of aliases used in the inner query if grouping sets were
	 *         enabled.
	 */
	protected Map<String, String> distinctGenerateSql(
			final SqlQuery outerSqlQuery, boolean countOnly) {
		...
		for (int i = 0; i < arity; i++) {
			...
			StarColumnPredicate predicate = getColumnPredicate(i);
			final String where = RolapStar.Column.createInExpr(expr, predicate,
					column.getDatatype(), innerSqlQuery);
			if (!where.equals("true")) {
			...
			groupingSetsAliases.put(expr,
					dialect.quoteIdentifier("dummyname." + alias));
		}
		...
		for (int i = 0, count = getMeasureCount(); i < count; i++) {
			...
			outerSqlQuery.addSelect(
					measure.getAggregator().getNonDistinctAggregator()
							.getExpression(dialect.quoteIdentifier(alias)), null);
		}
