/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapEvaluatorRoot.java#16 $
 ...
 * 
 * <p>
 * Contains the context that does not change as evaluation context is
 * pushed/popped.
 * 
 * @author jhyde
 * @since Nov 11, 2008
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapEvaluatorRoot.java#16 $
 */
class RolapEvaluatorRoot {
   ...
   * Finding the default member is moderately expensive, but happens very often.
   */
  ...
  final SolveOrderMode solveOrderMode = Util.lookup(SolveOrderMode.class,
  ...
  private RolapEvaluatorRoot(Statement statement, Execution execution) {
    ...
    for (RolapHierarchy hierarchy : cube.getHierarchies()) {
      RolapMember defaultMember = (RolapMember) schemaReader
    ...
    this.currentDialect = DialectManager.createDialect(schemaReader.getDataSource(),
      null);
  ...
  /**
   * Just a simple key of Exp/scalar/resultStyle, used for keeping compiled
   * expressions. Previous to the introduction of this class, the key was a list
   * constructed as Arrays.asList(exp, scalar, resultStyle) and having poorer
   * performance on equals, hashCode, and construction.
   */
  private static class CompiledExpKey {
    ...
    public boolean equals(Object other) {
      ...
      return this.scalar == otherKey.scalar
        && this.resultStyle == otherKey.resultStyle && this.exp.equals(otherKey.exp);
    }
  ...
  /**
   * Returns the value of a parameter, evaluating its default expression if
   * necessary.
   * 
   * <p>
   * The default implementation throws {@link UnsupportedOperationException}.
