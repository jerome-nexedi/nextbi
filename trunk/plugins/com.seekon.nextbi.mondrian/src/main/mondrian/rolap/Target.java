/*
// $Id: //open/mondrian/src/main/mondrian/rolap/Target.java#17 $
...
/**
 * Helper class for {@link mondrian.rolap.HighCardSqlTupleReader} that keeps
 * track of target levels and constraints for adding to SQL query.
 * 
 * @author luis f. canals, Kurtis Walker
 * @since July 23, 2009
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/Target.java#17 $
 */
public class Target extends TargetBase {
	...
	public Target(final RolapLevel level,
			...
			final List<RolapMember> srcMembers, final TupleConstraint constraint,
			...
			public int size() {
				while (this.moreRows) {
					...
					if (limit > 0 && !asList && getList().size() > limit) {
						System.out.println("Target: 199, Ouch! Toooo big array..."
								+ hashCode());
