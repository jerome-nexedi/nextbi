...
public abstract class RolapAggregationManager {
	...
	public static CellRequest makeRequest(RolapEvaluator evaluator) {
		...
		for (List<List<Member>> aggregationList : aggregationLists) {
			...
			compoundPredicate = makeCompoundPredicate(compoundGroupMap,
					measure.getCube());
	...
	private static CellRequest makeCellRequest(final Member[] members,
		...
		if (extendedContext) {
			for (int i = 1; i < members.length; i++) {
				...
				final boolean needToReturnNull = level.getLevelReader()
						.constrainRequest(member, measure.getCube(), request);
				if (needToReturnNull) {
					return null;
				}
			}
	 ...
	 */
	private static boolean makeCompoundGroup(int starColumnCount,
			RolapCube baseCube, List<List<RolapMember>> aggregationList,
			Map<BitKey, List<RolapCubeMember[]>> compoundGroupMap) {
		...
		for (List<RolapMember> aggregation : aggregationList) {
			...
			if (aggregation.size() > 0
					&& (aggregation.get(0) instanceof RolapCubeMember || aggregation
							.get(0) instanceof VisualTotalMember)) {
				isTuple = true;
			} else {
			...
			for (RolapCubeMember member : tuple) {
				...
				tupleUnsatisfiable = makeCompoundGroupForMember(member, baseCube,
						bitKey);
		...
		while (member != null) {
			...
			if (!level.isAll()) {
				...
				if (memberPredicate == null) {
					...
					memberPredicate = memberPredicate.and(new ValueColumnPredicate(
	...
	public static RolapCacheRegion makeCacheRegion(final RolapStar star,
		...
		final RolapCacheRegion cacheRegion = new RolapCacheRegion(star,
				starMeasureList);
		...
		if (region instanceof CacheControlImpl.MemberCellRegion) {
				...
				if (member instanceof RolapCubeMember) {
					...
					rolapMember = (RolapCubeMember) baseCube.getSchemaReader()
							.getMemberByUniqueName(
									Util.parseIdentifier(member.getUniqueName()), true);
				}
				final RolapCubeLevel level = rolapMember.getLevel();
				RolapStar.Column column = level.getBaseStarKeyColumn(baseCube);

				level.getLevelReader().constrainRegion(
						new MemberColumnPredicate(column, rolapMember), baseCube,
						cacheRegion);
			}
			...
			level.getLevelReader().constrainRegion(
					new RangeColumnPredicate(column, rangeRegion.getLowerInclusive(),
							(rangeRegion.getLowerBound() == null ? null
									: new MemberColumnPredicate(column, rangeRegion
											.getLowerBound())),
							rangeRegion.getUpperInclusive(),
							(rangeRegion.getUpperBound() == null ? null
									: new MemberColumnPredicate(column, rangeRegion
											.getUpperBound()))), baseCube, cacheRegion);
		} else {
