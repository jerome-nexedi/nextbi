...
public class NoCacheMemberReader implements MemberReader, MemberCache {
	private static final Logger LOGGER = Logger
			.getLogger(NoCacheMemberReader.class);
	...
	NoCacheMemberReader(MemberReader source) {
		...
		if (!source.setCache(this)) {
			throw Util.newInternal("MemberSource (" + source + ", "
					+ source.getClass() + ") does not support cache-writeback");
		}
	}
	...
	public void putChildren(final RolapLevel level,
			final TupleConstraint constraint, final List<RolapMember> children) {
	}
	...
	public void getMemberRange(final RolapLevel level,
			final RolapMember startMember, final RolapMember endMember,
	...
	class SiblingIterator {
		...
		SiblingIterator(MemberReader reader, RolapMember member) {
			...
			if (this.position == -1) {
				throw Util.newInternal("member " + member
						+ " not found among its siblings");
			}
		}
