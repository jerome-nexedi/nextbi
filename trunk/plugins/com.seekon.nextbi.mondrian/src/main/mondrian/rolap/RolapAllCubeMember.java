/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapAllCubeMember.java#2 $
 ...
 * 
 * <p>
 * A minor extension to {@link mondrian.rolap.RolapCubeMember} because the
 * naming rules are different.
 * 
 * @author Will Gorman
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapAllCubeMember.java#2 $
 */
class RolapAllCubeMember extends RolapCubeMember {
	...
	public RolapAllCubeMember(RolapMember member, RolapCubeLevel cubeLevel) {
		...
		if (member.getHierarchy().getName().equals(getHierarchy().getName())) {
			...
			String replacement = getHierarchy().getName()
					.replaceAll("\\$", "\\\\\\$");

			// convert string to regular expression
			String memberLevelName = member.getHierarchy().getName()
					.replaceAll("\\.", "\\\\.");
