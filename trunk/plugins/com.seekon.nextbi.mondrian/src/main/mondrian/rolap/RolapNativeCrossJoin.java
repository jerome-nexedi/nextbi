 ...
 * CrossJoin in SQL. The generated SQL will join the dimension tables with the
 * fact table and return all combinations that have a corresponding row in the
 * fact table. The current context (slicer) is used for filtering (WHERE clause
 * in SQL). This very effective computes queries like
 * 
 * <pre>
 *   SELECT ...
 *   NON EMTPY Crossjoin(
 ...
 * 
 * <p>
 * The implementation currently can not handle sets containting parent/child
 * hierarchies, ragged hierarchies, calculated members and the ALL member.
 * Otherwise all
 * 
 * @author av
 * @since Nov 21, 2005
...
public class RolapNativeCrossJoin extends RolapNativeSet {
	 ...
	 * 
	 * <p>
	 * If the current context contains calculated members, silently ignores them.
	 * This means means that too many members are returned, but this does not
	 * matter, because the {@link RolapConnection.NonEmptyResult} will filter out
	 * these later.
	 * </p>
	 */
		...
		public RolapMember findMember(Object key) {
			for (CrossJoinArg arg : args) {
				if (arg instanceof MemberListCrossJoinArg) {
					final MemberListCrossJoinArg crossJoinArg = (MemberListCrossJoinArg) arg;
					final List<RolapMember> memberList = crossJoinArg.getMembers();
	...
	NativeEvaluator createEvaluator(RolapEvaluator evaluator, FunDef fun,
			Exp[] args) {
		if (!isEnabled()) {
		...
		List<CrossJoinArg[]> allArgs = crossJoinArgFactory().checkCrossJoin(
				evaluator, fun, args, false);
		...
		if (allArgs == null || allArgs.isEmpty() || allArgs.get(0) == null) {
			...
			alertCrossJoinNonNative(evaluator, fun, "arguments not supported");
			return null;
		}
		...
		if (countNonNativeInputArg == cjArgs.length) {
			...
			alertCrossJoinNonNative(evaluator, fun,
		...
		if (!NonEmptyCrossJoinConstraint.isValidContext(evaluator, false,
				levels.toArray(new RolapLevel[levels.size()]), restrictMemberTypes())) {
		...
		for (RolapLevel level : levels) {
			...
			for (int i = 0; i < evalMembers.length; ++i) {
				Dimension evalMemberDimension = evalMembers[i].getHierarchy()
						.getDimension();
	...
	private TupleConstraint buildConstraint(final RolapEvaluator evaluator,
			final FunDef fun, final CrossJoinArg[] cargs) {
		CrossJoinArg[] myArgs;
	...
	private CrossJoinArg[] buildArgs(final RolapEvaluator evaluator,
			final CrossJoinArg[] cargs) {
		Set<CrossJoinArg> joinArgs = crossJoinArgFactory()
				.buildConstraintFromAllAxes(evaluator);
	...
	private void alertCrossJoinNonNative(RolapEvaluator evaluator, FunDef fun,
			String reason) {
