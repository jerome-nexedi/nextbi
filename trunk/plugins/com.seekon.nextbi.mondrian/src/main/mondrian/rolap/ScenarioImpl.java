/*
// $Id: //open/mondrian/src/main/mondrian/rolap/ScenarioImpl.java#9 $
 ...
 * @author jhyde
 * @since 24 April, 2009
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/ScenarioImpl.java#9 $
 */
public final class ScenarioImpl implements Scenario {
	...
	public boolean equals(Object obj) {
		return obj instanceof ScenarioImpl && id == ((ScenarioImpl) obj).id;
	}
	 ...
	 *          Allocation policy
	 * @param allocationArgs
	 *          Additional arguments of allocation policy
	 */
	public void setCellValue(Connection connection, List<RolapMember> members,
			double newValue, double currentValue, AllocationPolicy allocationPolicy,
		...
		case EQUAL_INCREMENT:
			if (allocationArgs.length != 0) {
				throw Util
						.newError("Allocation policy " + allocationPolicy
								+ " takes 0 arguments; " + allocationArgs.length
								+ " were supplied");
			}
			break;
		default:
			throw Util.newError("Allocation policy " + allocationPolicy
					+ " is not supported");
		}
		...
		final BitKey constrainedColumnsBitKey = BitKey.Factory
				.makeBitKey(starColumnCount);
		Object[] keyValues = new Object[starColumnCount];
		for (int i = 1; i < members.size(); i++) {
			Member member = members.get(i);
			for (RolapCubeMember m = (RolapCubeMember) member; m != null
					&& !m.isAll(); m = m.getParentMember()) {
		...
		// CellRequest.
		Object[] compactKeyValues = new Object[constrainedColumnsBitKey
				.cardinality()];
		int k = 0;
		...
		writebackCells.add(new WritebackCell(baseCube, new ArrayList<RolapMember>(
				members), constrainedColumnsBitKey, compactKeyValues, newValue,
				currentValue, allocationPolicy));
	}
	 ...
	 * For example, applied to [Scenario].[1], returns the Scenario object
	 * representing scenario #1.
	 * 
	 * @param member
	 *          Wrapper member
	...
	static Scenario forMember(final RolapMember member) {
		...
		final ResolvedFunCall resolvedFunCall = (ResolvedFunCall) formula
				.getExpression();
	 ...
	 * [Scenario].[{id}] for each cube that has writeback enabled. (Currently a
	 * cube has writeback enabled iff it has a dimension called "Scenario".)
	 * 
	 * @param schema
	 *          Schema
	 */
		...
		for (RolapCube cube : schema.getCubeList()) {
			for (RolapHierarchy hierarchy : cube.getHierarchies()) {
				if (isScenario(hierarchy)) {
					member = cube.createCalculatedMember(hierarchy, getId() + "",
	...
	/**
	 * Returns the number of atomic cells that contribute to the current cell.
	 * 
	 * @param evaluator
	 *          Evaluator
	...
	/**
	 * Computes the number of atomic cells in a cell identified by a list of
	 * members.
	 * 
	 * <p>
	 * The method may be expensive. If the value is not in the cache, computes it
	 * immediately using a database access. It uses an aggregate table if
	 * applicable, and puts the value into the cache.
	 * 
	 * <p>
	 * Compare with {@link #evaluateAtomicCellCount(RolapEvaluator)}, which gets
	 * the value from the cache but may lie (and generate a cache miss) if the
	 * value is not present.
	 * 
	 * @param cube
	 *          Cube
	 * @param memberList
	 *          Coordinate members of cell
	 * @return Number of atomic cells in cell
	 */
	private static double computeAtomicCellCount(RolapCube cube,
	 ...
	 * scenario. Including that member in the slicer will automatically use this
	 * scenario.
	 * 
	 * <p>
	 * The result is not null, provided that {@link #register(RolapSchema)} has
	 * been called.
	 * 
	 ...
	 * 
	 * <p>
	 * From this, other cell values can be modified as they are read into cache.
	 * Only the cells specifically modified by the client have a
	 * {@code CellValueOverride}.
	 * 
	 * <p>
	 * In future, a {@link ScenarioImpl} could be persisted by serializing all
	 * {@code WritebackCell}s to a file.
	 */
	private static class WritebackCell {
		 ...
		 *          Cube
		 * @param members
		 *          Members that form context
		 * @param constrainedColumnsBitKey
		 *          Bitmap of columns which have values
		 * @param keyValues
		 *          List of values, by bit position
		 * @param newValue
		 *          New value
		 * @param currentValue
		 *          Current value
		 ...
		 *          Allocation policy
		 */
		WritebackCell(RolapCube cube, List<RolapMember> members,
				BitKey constrainedColumnsBitKey, Object[] keyValues, double newValue,
				double currentValue, AllocationPolicy allocationPolicy) {
		 ...
		 * 
		 * <p>
		 * Assumes that {@code members} contains an entry for each dimension in the
		 * cube.
		 * 
		 * @param members
		 *          Co-ordinates of another cell
		 * @return Relation of this writeback cell to other co-ordinate, never null
		 */
	 ...
	 * 
	 * <p>
	 * When evaluated, replaces the value of a cell with the value overridden by a
	 * writeback value, per
	 * {@link org.olap4j.Cell#setValue(Object, org.olap4j.AllocationPolicy, Object...)}
	...
	private static class ScenarioCalc extends GenericCalc {
		 ...
		 * 
		 * @param scenario
		 *          Scenario whose writeback values should be substituted for the
		...
		public Object evaluate(Evaluator evaluator) {
			...
			final Member defaultMember = scenario.member.getHierarchy()
					.getDefaultMember();
			final int savepoint = evaluator.savepoint();
			...
			for (ScenarioImpl.WritebackCell writebackCell : scenario.writebackCells) {
				CellRelation relation = writebackCell.getRelationTo(evaluator
						.getMembers());
				switch (relation) {
				case ABOVE:
					...
					case EQUAL_ALLOCATION:
						...
						break;
					case EQUAL_INCREMENT:
						d += writebackCell.getOffset() * atomicCellCount
								/ writebackCell.atomicCellCount;
						break;
					default:
