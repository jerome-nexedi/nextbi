...
public class RolapCube extends CubeBase {
   ...
   *          Annotations
   */
  private RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema, String name,
    boolean visible, String caption, String description, boolean isCache,
    MondrianDef.Relation fact, MondrianDef.CubeDimension[] dimensions, boolean load,
    ...
    RolapDimension measuresDimension = new RolapDimension(schema,
      Dimension.MEASURES_NAME, null, true, null, DimensionType.MeasuresDimension,
      false, Collections.<String, Annotation> emptyMap());
    ...
    for (int i = 0; i < dimensions.length; i++) {
      ...
      RolapCubeDimension dimension = getOrCreateDimension(xmlCubeDimension, schema,
        xmlSchema, i + 1, hierarchyList);
      if (getLogger().isDebugEnabled()) {
  ...
  RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema,
    ...
    this(schema, xmlSchema, xmlCube.name, xmlCube.visible, xmlCube.caption,
      xmlCube.description, xmlCube.cache, xmlCube.fact, xmlCube.dimensions, load,
    ...
    if (factCountMeasure == null) {
      ...
      factCountMeasure = createMeasure(xmlCube, measuresLevel, measureList.size(),
        xmlMeasure);
      measureList.add(factCountMeasure);
    }

    setMeasuresHierarchyMemberReader(new CacheMemberReader(new MeasureMemberSource(
      this.measuresHierarchy, measureList)));
  ...
  private RolapBaseCubeMeasure createMeasure(MondrianDef.Cube xmlCube,
    RolapLevel measuresLevel, int ordinal, final MondrianDef.Measure xmlMeasure) {
    ...
    final RolapBaseCubeMeasure measure = new RolapBaseCubeMeasure(this, null,
      measuresLevel, xmlMeasure.name, xmlMeasure.caption, xmlMeasure.description,
      xmlMeasure.formatString, measureExp, aggregator, xmlMeasure.datatype,
    ...
    if (xmlMeasure.cellFormatter != null) {
      ...
      scriptDefinition = RolapSchema.toScriptDef(xmlMeasure.cellFormatter.script);
    } else {
  ...
  RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema,
        ...
        if (cubeMeasure.getUniqueName().equals(xmlMeasure.name)) {
          ...
          if (cubeMeasure instanceof RolapCalculatedMember) {
            ...
            RolapVirtualCubeMeasure virtualCubeMeasure = new RolapVirtualCubeMeasure(
              null, measuresLevel, (RolapStoredMeasure) cubeMeasure, RolapHierarchy
                .createAnnotationMap(xmlMeasure.annotations));
            ...
            virtualCubeMeasure.setProperty(Property.CAPTION.name, cubeMeasure
              .getCaption());
    ...
    for (Object o : calculatedMembersMap.keySet()) {
      ...
      List<MondrianDef.CalculatedMember> xmlCalculatedMemberList = calculatedMembersMap
        .get(baseCube);
      Query queryExp = resolveCalcMembers(xmlCalculatedMemberList, Collections
        .<MondrianDef.NamedSet> emptyList(), baseCube, false);
    ...
    setMeasuresHierarchyMemberReader(new CacheMemberReader(new MeasureMemberSource(
      this.measuresHierarchy, Util.<RolapMember> cast(modifiedMeasureList))));

    createCalcMembersAndNamedSets(xmlCalculatedMemberList, Arrays
      .asList(xmlVirtualCube.namedSets), new ArrayList<RolapMember>(),
    ...
    setMeasuresHierarchyMemberReader(new CacheMemberReader(new MeasureMemberSource(
      this.measuresHierarchy, Util.<RolapMember> cast(origMeasureList))));

    this.measuresHierarchy.setDefaultMember(defaultMeasure);

    List<MondrianDef.CalculatedMember> xmlVirtualCubeCalculatedMemberList = Arrays
      .asList(xmlVirtualCube.calculatedMembers);
    if (!vcHasAllCalcMembers(origCalcMeasureList, xmlVirtualCubeCalculatedMemberList)) {
      ...
      for (Formula calculatedMember : calculatedMemberListCopy) {
        ...
        findOriginalMembers(calculatedMember, xmlVirtualCubeCalculatedMemberList,
          calculatedMemberList);
      }
    }
    ...
    setMeasuresHierarchyMemberReader(new CacheMemberReader(new MeasureMemberSource(
      this.measuresHierarchy, Util.<RolapMember> cast(finalMeasureMembers))));
  ...
  private RolapCubeDimension getOrCreateDimension(
    ...
    if (xmlCubeDimension instanceof MondrianDef.DimensionUsage) {
      ...
      final RolapHierarchy sharedHierarchy = schema.getSharedHierarchy(usage.source);
  ...
  private void init(MondrianDef.Cube xmlCube, final List<RolapMember> memberList) {
    ...
    createCalcMembersAndNamedSets(Arrays.asList(xmlCube.calculatedMembers), Arrays
      .asList(xmlCube.namedSets), memberList, formulaList, this, true);
  }
  ...
  private void createCalcMembersAndNamedSets(
    ...
    final Query queryExp = resolveCalcMembers(xmlCalcMembers, xmlNamedSets, cube,
      errOnDups);
    if (queryExp == null) {
      return;
    }
  ...
  private Query resolveCalcMembers(
    ...
    try {
      ...
      throw MondrianResource.instance().UnknownNamedSetHasBadFormula
        .ex(getName(), e);
    }
    queryExp.resolve();
    return queryExp;
  ...
  private void preNamedSet(MondrianDef.NamedSet xmlNamedSet, Set<String> nameSet,
    StringBuilder buf) {
    ...
    buf.append("SET ").append(Util.makeFqName(xmlNamedSet.name)).append(Util.nl)
      .append(" AS ");
    Util.singleQuoteString(xmlNamedSet.getFormula(), buf);
    buf.append(Util.nl);
  }

  private void postCalcMember(List<MondrianDef.CalculatedMember> xmlCalcMembers,
    int i, final Query queryExp, List<RolapMember> memberList) {
  ...
  private void preCalcMember(List<MondrianDef.CalculatedMember> xmlCalcMembers,
    int j, StringBuilder buf, RolapCube cube, boolean errOnDup, Set<String> fqNames) {
    ...
    for (int i = 0; i < calculatedMemberList.size(); i++) {
      ...
      if (formula.getName().equals(xmlCalcMember.name)
        && formula.getMdxMember().getHierarchy().equals(hierarchy)) {
        if (errOnDup) {
          throw MondrianResource.instance().CalcMemberNotUnique
            .ex(fqName, getName());
        } else {
    ...
    if (!fqNames.add(fqName)) {
      throw MondrianResource.instance().CalcMemberNotUnique.ex(fqName, getName());
    }
    ...
    if (!propNames.contains(Property.MEMBER_ORDINAL.getName())) {
      buf.append(",").append(Util.nl).append(Property.MEMBER_ORDINAL).append(" = ")
  ...
  void processFormatStringAttribute(MondrianDef.CalculatedMember xmlCalcMember,
    ...
    if (xmlCalcMember.formatString != null) {
      buf.append(",").append(Util.nl).append(Property.FORMAT_STRING.name).append(
  ...
  synchronized void createUsage(RolapCubeHierarchy hierarchy,
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("RolapCube.createUsage: " + "cube=" + getName() + ", hierarchy="
        + hierarchy.getName() + ", usage=" + usage);
    }
    ...
    for (HierarchyUsage hu : hierarchyUsages) {
      ...
      if ((s != null) && s.equals(source)) {
        if (list != null) {
          if (getLogger().isDebugEnabled()) {
            getLogger().debug(
              "RolapCube.getUsagesBySource: " + "add list HierarchyUsage.name="
                + hu.getName());
          }
          list.add(hu);
        } else if (hierUsage == null) {
        ...
        if (levelName != null) {
          for (RolapCubeLevel level : levels) {
            if (level.getKeyExp() != null) {
              parentColumn = makeColumns(table, level, parentColumn, usagePrefix);
            }
    ...
    if (tableName != null) {
      if (table.getAlias().equals(tableName)) {
        ...
        if (t != null) {
          ...
          getLogger().warn(
            "RolapCube.makeColumns: for cube \"" + getName() + "\" the Level \""
              + level.getName() + "\" has a table name attribute \"" + tableName
  ...
  private static void format(MondrianDef.RelationOrJoin relation, StringBuilder buf,
    String indent) {
  ...
  private static void topToBottom(MondrianDef.RelationOrJoin relation) {
    if (relation instanceof MondrianDef.Table) {
      ...
      while (join.left instanceof MondrianDef.Join) {
        ...
        join.right = new MondrianDef.Join(join.leftAlias, join.leftKey, jleft.right,
          join.rightAlias, join.rightKey, join.right);
      ...
      if (dimensionName.equals(levelDimName)
        ...
        for (Hierarchy hier : dimension.getHierarchies()) {
          ...
          if (hierarchyName.equals(levelHierName)
            ...
            if (isClosure) {
              final RolapCubeLevel baseLevel = ((RolapCubeLevel) hier.getLevels()[1])
                .getClosedPeer();
  ...
  RolapCubeDimension createDimension(MondrianDef.CubeDimension xmlCubeDimension,
    ...
    RolapCubeDimension dimension = getOrCreateDimension(xmlCubeDimension, schema,
      xmlSchema, dimensions.length, hierarchyList);
  ...
  public Member createCalculatedMember(String xml) {
    ...
    try {
      ...
      throw Util.newError(e, "Error while creating calculated member from XML ["
        + xml + "]");
    }
  ...
  RolapMember createCalculatedMember(RolapHierarchy hierarchy, String name, Calc calc) {
    ...
    final Formula formula = new Formula(new Id(segmentList), createDummyExp(calc),
    ...
    public Object visit(MemberExpr memberExpr) {
      ...
      if (member instanceof RolapCalculatedMember) {
        ...
        MondrianDef.CalculatedMember xmlCalcMember = schema
          .lookupXmlCalculatedMember(calcMember.getUniqueName(), baseCube.name);
        createCalcMembersAndNamedSets(Collections.singletonList(xmlCalcMember),
          Collections.<MondrianDef.NamedSet> emptyList(),
          new ArrayList<RolapMember>(), new ArrayList<Formula>(), virtualCube, false);
        return null;

      } else if (member instanceof RolapBaseCubeMeasure) {
        RolapBaseCubeMeasure baseMeasure = (RolapBaseCubeMeasure) member;
        RolapVirtualCubeMeasure virtualCubeMeasure = new RolapVirtualCubeMeasure(
          null, measuresLevel, baseMeasure, Collections
            .<String, Annotation> emptyMap());
