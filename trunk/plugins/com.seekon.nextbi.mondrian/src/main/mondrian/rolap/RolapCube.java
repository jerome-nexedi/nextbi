/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapCube.java#165 $
 ...
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapCube.java#165 $
 */
public class RolapCube extends CubeBase {
	 ...
	 */
	private final SoftSmartCache<Role, List<Member>> roleToAccessibleCalculatedMembers = new SoftSmartCache<Role, List<Member>>();
	...
	final List<RolapHierarchy> hierarchyList = new ArrayList<RolapHierarchy>();
	 ...
	 *          Annotations
	 */
	private RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema,
			String name, boolean visible, String caption, String description,
			boolean isCache, MondrianDef.Relation fact,
			MondrianDef.CubeDimension[] dimensions, boolean load,
			Map<String, Annotation> annotationMap) {
		super(name, caption, visible, description,
		...
		if (!isVirtual()) {
			...
			closureColumnBitKey = BitKey.Factory.makeBitKey(star.getColumnCount());
		} else {
		...
		RolapDimension measuresDimension = new RolapDimension(schema,
				Dimension.MEASURES_NAME, null, true, null,
				DimensionType.MeasuresDimension, false,
				Collections.<String, Annotation> emptyMap());

		this.dimensions[0] = measuresDimension;

		this.measuresHierarchy = measuresDimension.newHierarchy(null, false, null);
		...
		for (int i = 0; i < dimensions.length; i++) {
			...
			RolapCubeDimension dimension = getOrCreateDimension(xmlCubeDimension,
					schema, xmlSchema, i + 1, hierarchyList);
			if (getLogger().isDebugEnabled()) {
	 ...
	 */
	RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema,
			MondrianDef.Cube xmlCube, boolean load) {
		this(schema, xmlSchema, xmlCube.name, xmlCube.visible, xmlCube.caption,
				xmlCube.description, xmlCube.cache, xmlCube.fact, xmlCube.dimensions,
				load, RolapHierarchy.createAnnotationMap(xmlCube.annotations));

		if (fact == null) {
			throw Util
					.newError("Must specify fact table of cube '" + getName() + "'");
		}

		if (fact.getAlias() == null) {
			throw Util.newError("Must specify alias for fact table of cube '"
					+ getName() + "'");
		}
		...
		for (int i = 0; i < xmlCube.measures.length; i++) {
			RolapBaseCubeMeasure measure = createMeasure(xmlCube, measuresLevel, i,
					xmlCube.measures[i]);
		...
		if (factCountMeasure == null) {
			...
			factCountMeasure = createMeasure(xmlCube, measuresLevel,
					measureList.size(), xmlMeasure);
	 ...
	 *          XML cube
	 * @param measuresLevel
	 *          Member that all measures belong to
	 * @param ordinal
	 *          Ordinal of measure
	 * @param xmlMeasure
	 *          XML measure
	 * @return Measure
	 */
	private RolapBaseCubeMeasure createMeasure(MondrianDef.Cube xmlCube,
			RolapLevel measuresLevel, int ordinal,
		...
		if (xmlMeasure.column != null) {
			if (xmlMeasure.measureExp != null) {
				throw MondrianResource.instance().BadMeasureSource.ex(xmlCube.name,
						xmlMeasure.name);
			}
			measureExp = new MondrianDef.Column(fact.getAlias(), xmlMeasure.column);
		...
		final RolapBaseCubeMeasure measure = new RolapBaseCubeMeasure(this, null,
				measuresLevel, xmlMeasure.name, xmlMeasure.caption,
				xmlMeasure.description, xmlMeasure.formatString, measureExp,
		...
		if (xmlMeasure.cellFormatter != null) {
			...
			scriptDefinition = RolapSchema
					.toScriptDef(xmlMeasure.cellFormatter.script);
		} else {
		...
		if (cellFormatterClassName != null || scriptDefinition != null) {
			try {
				CellFormatter cellFormatter = RolapSchema.getCellFormatter(
						cellFormatterClassName, scriptDefinition);
		...
		validateMemberProps(xmlMeasure.memberProperties, propNames, propExprs,
				xmlMeasure.name);
	...
	/**
	 * Makes sure that the schemaReader cache is invalidated. Problems can occur
	 * if the measure hierarchy member reader is out of sync with the cache.
	 * 
	 * @param memberReader
	 *          new member reader for measures hierarchy
	 */
	 ...
	 */
	RolapCube(RolapSchema schema, MondrianDef.Schema xmlSchema,
			MondrianDef.VirtualCube xmlVirtualCube, boolean load) {
		this(schema, xmlSchema, xmlVirtualCube.name, xmlVirtualCube.visible,
				xmlVirtualCube.caption, xmlVirtualCube.description, true, null,
				xmlVirtualCube.dimensions, load, RolapHierarchy
						.createAnnotationMap(xmlVirtualCube.annotations));
		...
		List<RolapVirtualCubeMeasure> origMeasureList = new ArrayList<RolapVirtualCubeMeasure>();
		List<MondrianDef.CalculatedMember> origCalcMeasureList = new ArrayList<MondrianDef.CalculatedMember>();
		CubeComparator cubeComparator = new CubeComparator();
		Map<RolapCube, List<MondrianDef.CalculatedMember>> calculatedMembersMap = new TreeMap<RolapCube, List<MondrianDef.CalculatedMember>>(
				cubeComparator);
		Member defaultMeasure = null;
		...
		for (MondrianDef.VirtualCubeMeasure xmlMeasure : xmlVirtualCube.measures) {
			...
			for (Member cubeMeasure : cubeMeasures) {
				if (cubeMeasure.getUniqueName().equals(xmlMeasure.name)) {
					...
					if (cubeMeasure instanceof RolapCalculatedMember) {
						...
						MondrianDef.CalculatedMember calcMember = schema
								.lookupXmlCalculatedMember(xmlMeasure.name, xmlMeasure.cubeName);
						if (calcMember == null) {
							throw Util.newInternal("Could not find XML Calculated Member '"
									+ xmlMeasure.name + "' in XML cube '" + xmlMeasure.cubeName
									+ "'");
						}
						List<MondrianDef.CalculatedMember> memberList = calculatedMembersMap
								.get(cube);
						if (memberList == null) {
						...
						RolapVirtualCubeMeasure virtualCubeMeasure = new RolapVirtualCubeMeasure(
								null, measuresLevel, (RolapStoredMeasure) cubeMeasure,
								RolapHierarchy.createAnnotationMap(xmlMeasure.annotations));
			...
			if (!found) {
				throw Util.newInternal("could not find measure '" + xmlMeasure.name
		...
		List<RolapVirtualCubeMeasure> modifiedMeasureList = new ArrayList<RolapVirtualCubeMeasure>(
				origMeasureList);
		for (Object o : calculatedMembersMap.keySet()) {
			RolapCube baseCube = (RolapCube) o;
			List<MondrianDef.CalculatedMember> xmlCalculatedMemberList = calculatedMembersMap
					.get(baseCube);
			Query queryExp = resolveCalcMembers(xmlCalculatedMemberList,
					Collections.<MondrianDef.NamedSet> emptyList(), baseCube, false);
			MeasureFinder measureFinder = new MeasureFinder(this, baseCube,
					measuresLevel);
		...
		// list of calculated members
		List<MondrianDef.CalculatedMember> xmlCalculatedMemberList = new ArrayList<MondrianDef.CalculatedMember>();
		for (Object o : calculatedMembersMap.keySet()) {
			RolapCube baseCube = (RolapCube) o;
			xmlCalculatedMemberList.addAll(calculatedMembersMap.get(baseCube));
		}
		xmlCalculatedMemberList.addAll(Arrays
				.asList(xmlVirtualCube.calculatedMembers));
		...
		createCalcMembersAndNamedSets(xmlCalculatedMemberList,
				Arrays.asList(xmlVirtualCube.namedSets), new ArrayList<RolapMember>(),
				new ArrayList<Formula>(), this, false);
		...
		List<MondrianDef.CalculatedMember> xmlVirtualCubeCalculatedMemberList = Arrays
				.asList(xmlVirtualCube.calculatedMembers);
		if (!vcHasAllCalcMembers(origCalcMeasureList,
			...
			List<Formula> calculatedMemberListCopy = new ArrayList<Formula>(
					calculatedMemberList);
			calculatedMemberList.clear();
			for (Formula calculatedMember : calculatedMemberListCopy) {
				if (findOriginalMembers(calculatedMember, origCalcMeasureList,
						calculatedMemberList)) {
					continue;
				}
				findOriginalMembers(calculatedMember,
						xmlVirtualCubeCalculatedMemberList, calculatedMemberList);
			}
		}
		...
		final List<RolapMember> finalMeasureMembers = new ArrayList<RolapMember>();
		...
		for (Formula formula : calculatedMemberList) {
			finalMeasureMembers.add((RolapMember) formula.getMdxMember());
		}
		setMeasuresHierarchyMemberReader(new CacheMemberReader(
				new MeasureMemberSource(this.measuresHierarchy,
	...
	private boolean vcHasAllCalcMembers(
		...
		return calculatedMemberList.size() == (origCalcMeasureList.size() + xmlVirtualCubeCalculatedMemberList
				.size());
	}

	private boolean findOriginalMembers(Formula formula,
		...
		for (MondrianDef.CalculatedMember xmlCalcMember : xmlCalcMemberList) {
			...
			if (xmlCalcMember.dimension != null) {
				Dimension dimension = lookupDimension(new Id.Segment(
						xmlCalcMember.dimension, Id.Quoting.UNQUOTED));
				if (dimension != null && dimension.getHierarchy() != null) {
					hierarchy = dimension.getHierarchy();
				}
			} else if (xmlCalcMember.hierarchy != null) {
				hierarchy = lookupHierarchy(new Id.Segment(xmlCalcMember.hierarchy,
						Id.Quoting.UNQUOTED), true);
			}
	...
	/**
	 * Creates a dimension from its XML definition. If the XML definition is a
	 * &lt;DimensionUsage&gt;, and the shared dimension is cached in the schema,
	 * returns that.
	 * 
	 * @param xmlCubeDimension
	 *          XML Dimension or DimensionUsage
	 * @param schema
	 *          Schema
	 * @param xmlSchema
	 *          XML Schema
	 ...
	 * @param cubeHierarchyList
	 *          List of hierarchies in cube
	 * @return A dimension
	 */
	private RolapCubeDimension getOrCreateDimension(
			MondrianDef.CubeDimension xmlCubeDimension, RolapSchema schema,
			MondrianDef.Schema xmlSchema, int dimensionOrdinal,
		...
		if (xmlCubeDimension instanceof MondrianDef.DimensionUsage) {
			...
			final RolapHierarchy sharedHierarchy = schema
					.getSharedHierarchy(usage.source);
		...
		if (dimension == null) {
			MondrianDef.Dimension xmlDimension = xmlCubeDimension
					.getDimension(xmlSchema);
		...
		return new RolapCubeDimension(this, dimension, xmlCubeDimension,
				xmlCubeDimension.name, dimensionOrdinal, cubeHierarchyList,
	 ...
	 */
	private void init(MondrianDef.Cube xmlCube, final List<RolapMember> memberList) {
		...
		createCalcMembersAndNamedSets(Arrays.asList(xmlCube.calculatedMembers),
				Arrays.asList(xmlCube.namedSets), memberList, formulaList, this, true);
	}
	 ...
	 * 
	 * @param cubeName
	 *          name of the cube (required for error messages)
	 * @param measures
	 *          measure list
	 */
	private void checkOrdinals(String cubeName, List<RolapMember> measures) {
		...
		for (RolapMember measure : measures) {
			...
			if (!ordinals.containsKey(ordinal)) {
				...
				throw MondrianResource.instance().MeasureOrdinalsNotUnique.ex(cubeName,
	...
	/**
	 * Adds a collection of calculated members and named sets to this cube. The
	 * members and sets can refer to each other.
	 * 
	 * @param xmlCalcMembers
	 *          XML objects representing members
	 * @param xmlNamedSets
	 *          Array of XML definition of named set
	 * @param memberList
	 *          Output list of {@link mondrian.olap.Member} objects
	 * @param formulaList
	 *          Output list of {@link mondrian.olap.Formula} objects
	 * @param cube
	 *          the cube that the calculated members originate from
	 * @param errOnDups
	...
	private void createCalcMembersAndNamedSets(
			...
			List<MondrianDef.NamedSet> xmlNamedSets, List<RolapMember> memberList,
			List<Formula> formulaList, RolapCube cube, boolean errOnDups) {
		final Query queryExp = resolveCalcMembers(xmlCalcMembers, xmlNamedSets,
				cube, errOnDups);
		if (queryExp == null) {
			return;
		}
		...
		Util.assertTrue(queryExp.getFormulas().length == xmlCalcMembers.size()
				+ xmlNamedSets.size());
		for (int i = 0; i < xmlCalcMembers.size(); i++) {
			postCalcMember(xmlCalcMembers, i, queryExp, memberList);
		}
		for (int i = 0; i < xmlNamedSets.size(); i++) {
			postNamedSet(xmlNamedSets, xmlCalcMembers.size(), i, queryExp,
					formulaList);
		}
	}
	...
	private Query resolveCalcMembers(
			List<MondrianDef.CalculatedMember> xmlCalcMembers,
			List<MondrianDef.NamedSet> xmlNamedSets, RolapCube cube, boolean errOnDups) {
	...
	private void postNamedSet(List<MondrianDef.NamedSet> xmlNamedSets,
			final int offset, int i, final Query queryExp, List<Formula> formulaList) {
		...
		namedSet.setAnnotationMap(RolapHierarchy
				.createAnnotationMap(xmlNamedSet.annotations));

		namedSetList.add(formula);
		formulaList.add(formula);
	}

	private void preNamedSet(MondrianDef.NamedSet xmlNamedSet,
			Set<String> nameSet, StringBuilder buf) {
		if (!nameSet.add(xmlNamedSet.name)) {
			throw MondrianResource.instance().NamedSetNotUnique.ex(xmlNamedSet.name,
					getName());
		}

		buf.append("SET ").append(Util.makeFqName(xmlNamedSet.name))
				.append(Util.nl).append(" AS ");
		Util.singleQuoteString(xmlNamedSet.getFormula(), buf);
		buf.append(Util.nl);
	}

	private void postCalcMember(
			List<MondrianDef.CalculatedMember> xmlCalcMembers, int i,
			final Query queryExp, List<RolapMember> memberList) {
		...
		((RolapCalculatedMember) member1).setAnnotationMap(RolapHierarchy
				.createAnnotationMap(xmlCalcMember.annotations));

		memberList.add(member);
	}

	private void preCalcMember(List<MondrianDef.CalculatedMember> xmlCalcMembers,
			int j, StringBuilder buf, RolapCube cube, boolean errOnDup,
			Set<String> fqNames) {
		...
		if (xmlCalcMember.dimension != null) {
			...
			final Dimension dimension = lookupDimension(new Id.Segment(
					xmlCalcMember.dimension, Id.Quoting.UNQUOTED));
			if (dimension != null) {
				hierarchy = dimension.getHierarchy();
			}
			...
			hierarchy = (Hierarchy) getSchemaReader().lookupCompound(this,
					Util.parseIdentifier(dimName), false, Category.Hierarchy);
		}
		if (hierarchy == null) {
			throw MondrianResource.instance().CalcMemberHasBadDimension.ex(dimName,
		...
		for (int i = 0; i < calculatedMemberList.size(); i++) {
			...
			if (formula.getName().equals(xmlCalcMember.name)
					&& formula.getMdxMember().getHierarchy().equals(hierarchy)) {
				if (errOnDup) {
					throw MondrianResource.instance().CalcMemberNotUnique.ex(fqName,
							getName());
				} else {
		...
		final MondrianDef.CalculatedMemberProperty[] xmlProperties = xmlCalcMember.memberProperties;
		List<String> propNames = new ArrayList<String>();
		List<String> propExprs = new ArrayList<String>();
		validateMemberProps(xmlProperties, propNames, propExprs, xmlCalcMember.name);

		final int measureCount = cube.measuresHierarchy.getMemberReader()
				.getMemberCount();
		...
		if (xmlCalcMember.cellFormatter != null) {
			...
			if (xmlCalcMember.cellFormatter.script != null) {
				if (xmlCalcMember.cellFormatter.script.language != null) {
					...
					propExprs.add(Util
							.quoteForMdx(xmlCalcMember.cellFormatter.script.language));
				}
				propNames.add(Property.CELL_FORMATTER_SCRIPT.name);
				propExprs.add(Util
						.quoteForMdx(xmlCalcMember.cellFormatter.script.cdata));
			}
		}
		...
		if (!propNames.contains(Property.MEMBER_ORDINAL.getName())) {
			buf.append(",").append(Util.nl).append(Property.MEMBER_ORDINAL)
					.append(" = ").append(measureCount + j);
		}
		buf.append(Util.nl);
	}

	private String removeSurroundingQuotesIfNumericProperty(String name,
			String expr) {
	...
	void processFormatStringAttribute(MondrianDef.CalculatedMember xmlCalcMember,
		...
		if (xmlCalcMember.formatString != null) {
			buf.append(",").append(Util.nl).append(Property.FORMAT_STRING.name)
					.append(" = ").append(Util.quoteForMdx(xmlCalcMember.formatString));
		}
	}

	/**
	 * Validates an array of member properties, and populates a list of names and
	 * expressions, one for each property.
	 * 
	 * @param xmlProperties
	 ...
	 * @param propExprs
	 *          Output array of property expressions.
	 * @param memberName
	...
	private void validateMemberProps(
			...
			List<String> propNames, List<String> propExprs, String memberName) {
		...
		for (MondrianDef.CalculatedMemberProperty xmlProperty : xmlProperties) {
			if (xmlProperty.expression == null && xmlProperty.value == null) {
				throw MondrianResource.instance().NeitherExprNorValueForCalcMemberProperty
						.ex(xmlProperty.name, memberName, getName());
			}
			if (xmlProperty.expression != null && xmlProperty.value != null) {
				throw MondrianResource.instance().ExprAndValueForMemberProperty.ex(
	 ...
	 * context. schemaReader is cached, and needs to stay in sync with any changes
	 * to the cube.
	 * 
	 * @post return != null
	...
	public synchronized SchemaReader getSchemaReader() {
		if (schemaReader == null) {
			schemaReader = new RolapCubeSchemaReader(Util.createRootRole(schema));
		}
		return schemaReader;
	}
	...
	private void register() {
		...
		List<RolapBaseCubeMeasure> storedMeasures = new ArrayList<RolapBaseCubeMeasure>();
	...
	/**
	 * Returns true if this Cube is either virtual or if the Cube's RolapStar is
	 * caching aggregates.
	 * 
	...
	/**
	 * Clear the in memory aggregate cache associated with this Cube, but only if
	 * Disabling Caching has been enabled.
	 */
	...
	/**
	 * Push all modifications of the aggregations to global cache, so other
	 * queries can start using the new cache
	 */
	...
	private void createUsages(RolapCubeDimension dimension,
		...
		RolapCubeHierarchy[] hierarchies = (RolapCubeHierarchy[]) dimension
				.getHierarchies();
		...
		if (hierarchies.length == 1) {
			...
			for (RolapCubeHierarchy hierarchy : hierarchies) {
				if (getLogger().isDebugEnabled()) {
					getLogger()
							.debug("RolapCube<init>: hierarchy=" + hierarchy.getName());
				}
				RolapLevel joinLevel = (RolapLevel) Util.lookupHierarchyLevel(
						hierarchy, du.level);
				if (joinLevel == null) {
					continue;
				}
	...
	synchronized void createUsage(RolapCubeHierarchy hierarchy,
			MondrianDef.CubeDimension cubeDim) {
		HierarchyUsage usage = new HierarchyUsage(this, hierarchy, cubeDim);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("RolapCube.createUsage: " + "cube=" + getName()
					+ ", hierarchy=" + hierarchy.getName() + ", usage=" + usage);
		}
	...
	/**
	 * A Hierarchy may have one or more HierarchyUsages. This method returns an
	 * array holding the one or more usages associated with a Hierarchy. The
	 * HierarchyUsages hierarchyName attribute always equals the name attribute of
	 * the Hierarchy.
	 * 
	 * @param hierarchy
	 *          Hierarchy
		...
		for (HierarchyUsage hu : hierarchyUsages) {
			if (hu.getHierarchyName().equals(name)) {
				if (list != null) {
					if (getLogger().isDebugEnabled()) {
						getLogger().debug(
								"RolapCube.getUsages: " + "add list HierarchyUsage.name="
										+ hu.getName());
					}
					list.add(hu);
					...
					if (getLogger().isDebugEnabled()) {
						getLogger().debug(
								"RolapCube.getUsages: " + "add list hierUsage.name="
										+ hierUsage.getName() + ", hu.name=" + hu.getName());
					}
					list.add(hierUsage);
					list.add(hu);
					hierUsage = null;
	...
	/**
	 * Looks up all of the HierarchyUsages with the same "source" returning an
	 * array of HierarchyUsage of length 0 or more.
	 * 
		...
		for (HierarchyUsage hu : hierarchyUsages) {
			...
			if ((s != null) && s.equals(source)) {
				if (list != null) {
					...
					if (getLogger().isDebugEnabled()) {
						getLogger().debug(
								"RolapCube.getUsagesBySource: " + "add list hierUsage.name="
										+ hierUsage.getName() + ", hu.name=" + hu.getName());
					}
					list.add(hierUsage);
					list.add(hu);
			...
			for (HierarchyUsage hierarchyUsage : hierarchyUsages) {
				...
				if (!relation.equals(table.getRelation())) {
					...
					if (hierarchyUsage.getForeignKey() == null) {
						throw MondrianResource.instance().HierarchyMustHaveForeignKey.ex(
								hierarchy.getName(), getName());
					}
					// jhyde: check is disabled until we handle <View> correctly
					if (false && !star.getFactTable().containsColumn(
							hierarchyUsage.getForeignKey())) {
						throw MondrianResource.instance().HierarchyInvalidForeignKey.ex(
								hierarchyUsage.getForeignKey(), hierarchy.getName(), getName());
					}
					// parameters:
					// fact table,
					// fact table foreign key,
					MondrianDef.Column column = new MondrianDef.Column(table.getAlias(),
							hierarchyUsage.getForeignKey());
					// parameters:
					// left column
					// right column
					RolapStar.Condition joinCondition = new RolapStar.Condition(column,
				...
				if (levelName != null) {
					for (RolapCubeLevel level : levels) {
						if (level.getKeyExp() != null) {
							parentColumn = makeColumns(table, level, parentColumn,
									usagePrefix);
						}
	...
	/**
	 * Adds a column to the appropriate table in the {@link RolapStar}. Note that
	 * if the RolapLevel has a table attribute, then the associated column needs
	 * to be associated with that table.
	 */
	protected RolapStar.Column makeColumns(RolapStar.Table table,
			RolapCubeLevel level, RolapStar.Column parentColumn, String usagePrefix) {
		...
		if (tableName != null) {
			if (table.getAlias().equals(tableName)) {
				parentColumn = table
						.makeColumns(this, level, parentColumn, usagePrefix);
				...
				if (t != null) {
					...
					getLogger().warn(
							"RolapCube.makeColumns: for cube \"" + getName()
									+ "\" the Level \"" + level.getName()
									+ "\" has a table name attribute \"" + tableName
									+ "\" but the associated RolapStar does not"
									+ " have a table with that name.");

					parentColumn = table.makeColumns(this, level, parentColumn,
							usagePrefix);
				}
			}
		} else {
	...
	/**
	 * Formats a {@link mondrian.olap.MondrianDef.RelationOrJoin}, indenting joins
	 * for readability.
	 * 
	 * @param relation
	 */
	...
	private static void format(MondrianDef.RelationOrJoin relation,
	...
	/**
	 * This method tells us if unrelated dimensions to measures from the input
	 * base cube should be pushed to default member or not during aggregation.
	 * 
	 * @param baseCubeName
	 *          name of the base cube for which we want to check this property
	 * @return boolean
	 */
	...
	public boolean isLoadInProgress() {
		return loadInProgress || getSchema().getSchemaLoadDate() == null;
	}

	/**
	 * Association between a MondrianDef.Table with its associated level's depth.
	 * This is used to rank tables in a snowflake so that the table with the
	 * lowest rank, level depth, is furthest from the base fact table in the
	 * RolapStar.
	 */
	private static class RelNode {

		/**
		 * Finds a RelNode by table name or, if that fails, by table alias from a
		 * map of RelNodes.
		 * 
		 * @param table
		 * @param map
		 */
		private static RelNode lookup(MondrianDef.Relation table,
	 ...
	 * 
	 * <p>
	 * What is the canonical form? It is only relevant when the relation is a
	 * snowflake (nested joins), not simply a table. The canonical form has lower
	 * levels to the left of higher levels (Day before Month before Quarter before
	 * Year) and the nested joins are always on the right side of the parent join.
	 * 
	 * <p>
	 ...
	 * lowest level table (the Day table) which joins to the next level (the Month
	 * table) which joins to the next (the Quarter table) which joins to the top
	 * level table (the Year table).
	 * <p>
	 * This method supports the transformation of a subset of all possible
	 * join/table relation trees (and anyone who whats to generalize it is welcome
	 * to). It will take any of the following and convert them to the canonical.
	 * 
	 * <pre>
	 *            |
	 *    ----------------
	 ...
	 * </pre>
	 * <p>
	 * In addition, at any join node, it can exchange the left and right child
	 * relations so that the lower level depth is to the left. For example, it can
	 * also transform the following:
	 * 
	 * <pre>
	 *                |
	 *         ----------------
	 ...
	 * </pre>
	 * <p>
	 * What it can not handle are cases where on both the left and right side of a
	 * join there are child joins:
	 * 
	 * <pre>
	 *                |
	 *         ----------------
	 ...
	 * </pre>
	 * <p>
	 * When does this method do nothing? 1) when there are less than 2 levels, 2)
	 * when any level does not have a table name, and 3) when for every table in
	 * the relation there is not a level. In these cases, this method simply
	...
	private static MondrianDef.RelationOrJoin reorder(
			MondrianDef.RelationOrJoin relation, RolapLevel[] levels) {
	...
	/**
	 * The map has to be validated against the relation because there are certain
	 * cases where we do not want to (read: can not) do reordering, for instance,
	 * when closures are involved.
	 * 
	 * @param relation
	 * @param map
	 */
	private static boolean validateNodes(MondrianDef.RelationOrJoin relation,
			Map<String, RelNode> map) {
	...
	/**
	 * Transforms the Relation moving the tables associated with lower levels
	 * (greater level depth, i.e., Day is lower than Month) to the left of tables
	 * with high levels.
	 * 
	 * @param relation
	 * @param map
	 */
	private static int leftToRight(MondrianDef.RelationOrJoin relation,
			Map<String, RelNode> map) {
		if (relation instanceof MondrianDef.Relation) {
			MondrianDef.Relation table = (MondrianDef.Relation) relation;
	...
	/**
	 * Transforms so that all joins have a table as their left child and either a
	...
	private static void topToBottom(MondrianDef.RelationOrJoin relation) {
		if (relation instanceof MondrianDef.Table) {
			...
			while (join.left instanceof MondrianDef.Join) {
				...
				join.right = new MondrianDef.Join(join.leftAlias, join.leftKey,
						jleft.right, join.rightAlias, join.rightKey, join.right);
	...
	/**
	 * Takes a relation in canonical form and snips off the the tables with the
	 * given tableName (or table alias). The matching table only appears once in
	 * the relation.
	 * 
	 * @param relation
	 * @param tableName
	...
	private static MondrianDef.RelationOrJoin snip(
			MondrianDef.RelationOrJoin relation, String tableName) {
		if (relation instanceof MondrianDef.Table) {
			MondrianDef.Table table = (MondrianDef.Table) relation;
			// Return null if the table's name or alias matches tableName
			return ((table.alias != null) && table.alias.equals(tableName)) ? null
	...
	/**
	 * Finds out non joining dimensions for this cube. Useful for finding out non
	 * joining dimensions for a stored measure from a base cube.
	 * 
	 * @param tuple
	 *          array of members
	 ...
	 * dimensions is done based on the unique name. Object equality can't be used.
	 * 
	 * @param otherDims
	 *          Set of dimensions to be tested for existence in this cube
	 * @return Set of dimensions that do not exist (non joining) in this cube
	 */
	...
	/**
	 * Returns whether this cube is virtual. We use the fact that virtual cubes do
	 * not have fact tables.
	 */
	...
	/**
	 * Returns the system measure that counts the number of fact table rows in a
	 * given cell.
	 * 
	 * <p>
	 * Never null, because if there is no count measure explicitly defined, the
	 * system creates one.
	 */
	...
	/**
	 * Returns the system measure that counts the number of atomic cells in a
	 * given cell.
	 * 
	 * <p>
	 * A cell is atomic if all dimensions are at their lowest level. If the fact
	 * table has a primary key, this measure is equivalent to the
	...
	/**
	 * Locates the base cube hierarchy for a particular virtual hierarchy. If not
	 * found, return null. This may be converted to a map lookup or cached in some
	 * way in the future to increase performance with cubes that have large
	 * numbers of hierarchies
	 * 
	 * @param hierarchy
	 *          virtual hierarchy
	...
	/**
	 * Locates the base cube level for a particular virtual level. If not found,
	 * return null. This may be converted to a map lookup or cached in some way in
	 * the future to increase performance with cubes that have large numbers of
			...
			if (dimensionName.equals(levelDimName)
				...
				for (Hierarchy hier : dimension.getHierarchies()) {
					...
					if (hierarchyName.equals(levelHierName)
						...
						if (isClosure) {
							final RolapCubeLevel baseLevel = ((RolapCubeLevel) hier
									.getLevels()[1]).getClosedPeer();
	...
	RolapCubeDimension createDimension(
			MondrianDef.CubeDimension xmlCubeDimension, MondrianDef.Schema xmlSchema) {
		RolapCubeDimension dimension = getOrCreateDimension(xmlCubeDimension,
				schema, xmlSchema, dimensions.length, hierarchyList);
	...
	public Member createCalculatedMember(String xml) {
		...
		try {
			...
			throw Util.newError(e,
					"Error while creating calculated member from XML [" + xml + "]");
		}

		try {
			loadInProgress = true;
			final List<RolapMember> memberList = new ArrayList<RolapMember>();
			createCalcMembersAndNamedSets(Collections.singletonList(xmlCalcMember),
					Collections.<MondrianDef.NamedSet> emptyList(), memberList,
					new ArrayList<Formula>(), this, true);
	 ...
	 * 
	 * @param hierarchy
	 *          Hierarchy the calculated member belongs to
	 * @param name
	 *          Name of member
	 * @param calc
	 *          Compiled expression
	 */
	RolapMember createCalculatedMember(RolapHierarchy hierarchy, String name,
			Calc calc) {
		...
		final Formula formula = new Formula(new Id(segmentList),
				createDummyExp(calc), new MemberProperty[0]);
		final Statement statement = schema.getInternalConnection()
				.createDummyStatement();
		try {
			final Query query = new Query(statement, this, new Formula[] { formula },
					new QueryAxis[0], null, new QueryPart[0], new Parameter[0], false);
	...
	/**
	 * Schema reader which works from the perspective of a particular cube (and
	 * hence includes calculated members defined in that cube) and also applies
	 * the access-rights of a given role.
	 */
	private class RolapCubeSchemaReader extends RolapSchemaReader implements
		...
		public Member getCalculatedMember(List<Id.Segment> nameParts) {
			...
			for (Formula formula : calculatedMemberList) {
				final String formulaUniqueName = formula.getMdxMember().getUniqueName();
		...
		public SchemaReader withoutAccessControl() {
			assert getClass() == RolapCubeSchemaReader.class : "Derived class "
					+ getClass() + " must override method";
			return RolapCube.this.getSchemaReader();
		}

		public Member getMemberByUniqueName(List<Id.Segment> uniqueNameParts,
				boolean failIfNotFound, MatchType matchType) {
			Member member = (Member) lookupCompound(RolapCube.this, uniqueNameParts,
					failIfNotFound, Category.Member, matchType);
			if (member == null) {
			...
			if (getRole().canAccess(member)) {
				...
				if (failIfNotFound) {
					throw Util.newElementNotFoundException(Category.Member,
							new IdentifierNode(Util.toOlap4j(uniqueNameParts)));
				}
				return null;
			}
		}
		...
		public OlapElement lookupChild(OlapElement parent,
				IdentifierSegment segment, MatchType matchType) {
			// ignore matchType
	...
	/**
	 * Visitor that walks an MDX parse tree containing formulas associated with
	 * calculated members defined in a base cube but referenced from a virtual
	 * cube. When walking the tree, look for other calculated members as well as
	 * stored measures. Keep track of all stored measures found, and for the
	 * calculated members, once the formula of that calculated member has been
	 * visited, resolve the calculated member relative to the virtual cube.
	 */
	private class MeasureFinder extends MdxVisitorImpl {
		/**
		 * The virtual cube where the original calculated member was referenced from
		 */
		...
		public MeasureFinder(RolapCube virtualCube, RolapCube baseCube,
		...
		public Object visit(MemberExpr memberExpr) {
			...
			if (member instanceof RolapCalculatedMember) {
				...
				virtualCube.setMeasuresHierarchyMemberReader(new CacheMemberReader(
						new MeasureMemberSource(virtualCube.measuresHierarchy, Util
								.<RolapMember> cast(measuresFound))));

				MondrianDef.CalculatedMember xmlCalcMember = schema
						.lookupXmlCalculatedMember(calcMember.getUniqueName(),
								baseCube.name);
				createCalcMembersAndNamedSets(Collections.singletonList(xmlCalcMember),
						Collections.<MondrianDef.NamedSet> emptyList(),
						new ArrayList<RolapMember>(), new ArrayList<Formula>(),
						virtualCube, false);
				return null;
				...
				RolapVirtualCubeMeasure virtualCubeMeasure = new RolapVirtualCubeMeasure(
						null, measuresLevel, baseMeasure,
						Collections.<String, Annotation> emptyMap());
	 ...
	 * 
	 * <p>
	 * Use this for synthetic expressions that do not correspond to anything in an
	 * MDX parse tree, and just need to compile to a particular compiled
