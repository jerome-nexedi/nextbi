/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapAggregator.java#23 $
 ...
 * @author jhyde
 * @since Jul 9, 2003
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapAggregator.java#23
 *          $
 */
public abstract class RolapAggregator extends EnumeratedValues.BasicValue
		implements Aggregator {
	private static int index = 0;

	public static final RolapAggregator Sum = new RolapAggregator("sum", index++,
			false) {
	...
	public static final RolapAggregator Count = new RolapAggregator("count",
			index++, false) {
	...
	public static final RolapAggregator Min = new RolapAggregator("min", index++,
			false) {
		public Object aggregate(Evaluator evaluator, TupleList members, Calc exp) {
	...
	public static final RolapAggregator Max = new RolapAggregator("max", index++,
			false) {
		public Object aggregate(Evaluator evaluator, TupleList members, Calc exp) {
	...
	public static final RolapAggregator Avg = new RolapAggregator("avg", index++,
			false) {
		public Aggregator getRollup() {
			return new RolapAggregator("avg", index, false) {
				public Object aggregate(Evaluator evaluator, TupleList members,
						Calc calc) {
	...
	public static final RolapAggregator DistinctCount = new RolapAggregator(
			"distinct-count", index++, true) {
		public Aggregator getRollup() {
	 ...
	 */
	public static final EnumeratedValues<RolapAggregator> enumeration = new EnumeratedValues<RolapAggregator>(
	 ...
	 * that these require not oly the operand to create the aggregation String
	 ...
	 * 
	 * <p>
	 * It uses the aggregate table fact_count column and a sum measure to create
	 * the query used to generate an average: <blockquote> <code>
	 ...
	 * create the query used to generate an average: <blockquote> <code>
	 ...
	 * <code>getExpression("emp.sal")</code> returns <code>"sum(emp.sal)"</code>.
	 */
	public String getExpression(String operand) {
		StringBuilder buf = new StringBuilder(64);
		buf.append(name);
