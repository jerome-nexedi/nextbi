/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapConnectionPool.java#14 $
...
/**
 * Singleton class that holds a connection pool. Call
 * RolapConnectionPool.instance().getPoolingDataSource(connectionFactory) to get
 * a DataSource in return that is a pooled data source.
 * 
 * @author jhyde
 * @author Robin Bagot
 * @since 7 July, 2003
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapConnectionPool.java#14
 *          $
 */
class RolapConnectionPool {
	...
	/**
	 * Sets up a pooling data source for connection pooling. This can be used if
	 * the application server does not have a pooling dataSource already
	 * configured.
	 * 
	 * <p>
	 * This takes a normal jdbc connection string, and requires a jdbc driver to
	 * be loaded, and then uses a {@link DriverManagerConnectionFactory} to create
	 ...
	 * 
	 * @param key
	 *          Identifies which connection factory to use. A typical key is a
	 *          JDBC connect string, since each JDBC connect string requires a
	 *          different connection factory.
	 * @param connectionFactory
	 *          Creates connections from an underlying JDBC connect string or
	 *          DataSource
	...
	public synchronized DataSource getDriverManagerPoolingDataSource(
			String jdbcConnectString, Properties jdbcProperties) {
		...
		List<Object> key = Arrays.<Object> asList("DriverManagerPoolingDataSource",
		...
		try {
			...
			dataSource = getPoolingDataSource(jdbcConnectString + propertyString,
					connectionFactory);
		} catch (Throwable e) {
			throw Util.newInternal(e,
					"Error while creating connection pool (with URI " + jdbcConnectString
							+ ")");
		}
	...
	public synchronized DataSource getDataSourcePoolingDataSource(
			DataSource dataSource, String dataSourceName, String jdbcUser,
			String jdbcPassword) {
		// First look for a data source with identical specification. This in
		// turn helps us to use the cache of Dialect objects.
		List<Object> key = Arrays.asList("DataSourcePoolingDataSource", dataSource,
		...
		if (jdbcUser != null || jdbcPassword != null) {
			connectionFactory = new DataSourceConnectionFactory(dataSource, jdbcUser,
					jdbcPassword);
		} else {
		...
		try {
			...
			throw Util.newInternal(e,
					"Error while creating connection pool (with URI " + dataSourceName
							+ ")");
		}
	...
	/**
	 * Gets or creates a connection pool for a particular connect specification.
	 */
	private synchronized ObjectPool getPool(Object key,
		...
		if (connectionPool == null) {
			...
			connectionPool = new GenericObjectPool(null, // PoolableObjectFactory, can
																										// be null
					50, // max active
