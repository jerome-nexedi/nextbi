/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapEvaluator.java#111 $
 ...
 * 
 * <p>
 * The context contains a member (which may be the default member) for every
 * dimension in the current cube. Certain operations, such as evaluating a
 * calculated member or a tuple, change the current context.
 * 
 * <p>
 * There are two ways of preserving context.
 * 
 * <p>
 * First, the {@link #push} method creates a verbatim copy of the evaluator. Use
 * that copy for computations, and any changes of state will be made only to the
 * copy.
 * 
 * <p>
 * Second, the {@link #savepoint()} method tells the evaluator to create a
 * checkpoint of its state, and returns an {@code int} value that can later be
 * passed to {@link #restore(int)}.
 * 
 * <p>
 * The {@code savepoint} method is recommended for most purposes, because the
 ...
 * 
 * <p>
 * One occasion that you would use {@code push} is when creating an iterator,
 * and the iterator needs its own evaluator context, even if the code that
 * created the iterator later reverts the context. In this case, the iterator's
 * constructor should call {@code push}.
 * 
 * <h3>Developers note</h3>
 * 
 * <p>
 * Many of the methods in this class are performance-critical. Where possible
 * they are declared 'final' so that the JVM can optimize calls to these
 * methods. If future functionality requires it, the 'final' modifier can be
 * removed and these methods can be overridden.
 * 
 * @author jhyde
 * @since 10 August, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapEvaluator.java#111
 *          $
 */
public class RolapEvaluator implements Evaluator {
	...
	/**
	 * List of lists of tuples or members, rarely used, but overrides the ordinary
	 * dimensional context if set when a cell value comes to be evaluated.
	 */
	...
	/**
	 * States of the finite state machine for determining the max solve order for
	 * the "scoped" behavior.
	 */
	 ...
	 * 
	 * @param root
	 *          Root context for stack of evaluators (contains information which
	...
	public RolapEvaluator(RolapEvaluatorRoot root) {
		...
		nonEmpty = false;
		nativeEnabled = MondrianProperties.instance().EnableNativeNonEmpty.get()
	...
	public boolean mightReturnNullForUnrelatedDimension() {
		if (!MondrianProperties.instance().IgnoreMeasureForNonJoiningDimension
				.get()) {
			return false;
		}
	...
	public boolean needToReturnNullForUnrelatedDimension(Member[] members) {
		...
		Set<Dimension> nonJoiningDimensions = baseCube
	...
	/**
	 * Adds a command to the stack that ensures that the state after restoring is
	...
	/**
	 * Returns true if the other object is a {@link RolapEvaluator} with identical
	 * context.
	 */
	...
	/**
	 * Adds a slicer member to the evaluator context, and remember it as part of
	 * the slicer. The slicer members are passed onto derived evaluators so that
	 * functions using those evaluators can choose to ignore the slicer members.
	 * One such function is CrossJoin emptiness check.
	 * 
	 * @param member
	 *          a member in the slicer
	 */
	 ...
	 * @param ordinal
	 *          Hierarchy ordinal
	 * @return Whether there is a member with the given hierarchy ordinal on the
	 *         stack
	 */
	...
	/**
	 * More specific version of {@link #getContext(mondrian.olap.Hierarchy)}, for
	 * internal code.
	 * 
	 * @param hierarchy
	 *          Hierarchy
	...
	void setExpanding(Member member) {
		...
		if (totalCommandCount > root.recursionCheckCommandCount) {
			...
			// recursion.
			root.recursionCheckCommandCount = totalCommandCount
					+ (root.defaultMembers.length << 4);
		}
	}
		...
		for (Member member : getNonAllMembers()) {
			...
			if (member == null) {
				if (getLogger().isDebugEnabled()) {
					getLogger().debug(
							"RolapEvaluator.getProperty: member == null " + " , count=" + i);
				}
				continue;
			}
	...
	public final String getFormatString() {
		final Exp formatExp = (Exp) getProperty(Property.FORMAT_EXP.name, null);
		if (formatExp == null) {
			return "Standard";
		}
	...
	/**
	 * Creates a key which uniquely identifes an expression and its context. The
	 * context includes members of dimensions which the expression is dependent
	 * upon.
	 */
	private Object getExpResultCacheKey(ExpCacheDescriptor descriptor) {
		...
		if (nonEmpty) {
			...
			final int[] hierarchyOrdinals = descriptor
	...
	public final Object getCachedResult(ExpCacheDescriptor cacheDescriptor) {
		...
		if (result == null) {
			...
			root.putCacheResult(key, result == null ? nullResult : result,
					isValidResult);
	...
	public final NamedSetEvaluator getNamedSetEvaluator(NamedSet namedSet,
			boolean create) {
	 ...
	 * 
	 * <p>
	 * The SOLVE_ORDER value is absolute regardless of where it is defined; e.g. a
	 * query defined calculated member with a SOLVE_ORDER of 1 always takes
	...
	/**
	 * Returns the member with the highest solve order according to AS2005 scoping
	 * rules.
	 * 
	 * <p>
	 * By default, cube calculated members are resolved before any session scope
	 ...
	 * 
	 * <p>
	 * The aggregate function is always applied to base members; i.e. as if
	...
	/**
	 * Returns whether a given calculation expands before another. A calculation
	 * expands before another if its solve order is higher, or if its solve order
	 * is the same and its dimension ordinal is lower.
	 * 
	 * @param calc1
	...
	private boolean expandsBefore(RolapCalculation calc1, RolapCalculation calc2) {
		...
		if (solveOrder1 > solveOrder2) {
			...
			return solveOrder1 == solveOrder2
					&& calc1.getHierarchyOrdinal() < calc2.getHierarchyOrdinal();
		}
	}
	...
	private int checksumState() {
		...
		h = h
				* 31
				+ new HashSet<RolapCalculation>(Arrays.asList(calculations).subList(0,
						calculationCount)).hashCode();
		...
		h = h * 31 + (nonEmpty ? 0x1 : 0x2) + (nativeEnabled ? 0x4 : 0x8)
				+ (firstExpanding ? 0x10 : 0x20) + (evalAxes ? 0x40 : 0x80);
		if (false) {
			// Enable this code block to debug checksum mismatches.
			System.err.println("h="
					+ h
					+ ": "
					+ Arrays.asList(Arrays.asList(currentMembers),
							new HashSet<RolapCalculation>(Arrays.asList(calculations)
									.subList(0, calculationCount)), expandingMember,
							aggregationLists, nonEmpty, nativeEnabled, firstExpanding,
							evalAxes));
		}
		return h;
	}
	...
	/**
	 * Checks if unrelated dimensions to the measure in the current context should
	 * be ignored.
	 * 
	 * @return boolean
	 */
	...
	private enum Command {
		...
		CHECKSUM(1) {
			...
			void execute(RolapEvaluator evaluator) {
				...
				assert value == currentState : "Current checksum " + currentState
