/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapCubeLevel.java#22 $
 ...
 * 
 * @author Will Gorman
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapCubeLevel.java#22
 *          $
 */
public class RolapCubeLevel extends RolapLevel {
	...
	/**
	 * For a parent-child hierarchy with a closure provided by the schema, the
	 * equivalent level in the closed hierarchy; otherwise null.
	 */
	...
	public RolapCubeLevel(RolapLevel level, RolapCubeHierarchy cubeHierarchy) {
		super(cubeHierarchy, level.getName(), level.getCaption(),
				level.isVisible(), level.getDescription(), level.getDepth(), level
						.getKeyExp(), level.getNameExp(), level.getCaptionExp(), level
						.getOrdinalExp(), level.getParentExp(), level.getNullParentValue(),
				null, level.getProperties(), level.getFlags(), level.getDatatype(),
				level.getInternalType(), level.getHideMemberCondition(), level
						.getLevelType(), "" + level.getApproxRowCount(), level
						.getAnnotationMap());
	...
	void init(MondrianDef.CubeDimension xmlDimension) {
		if (isAll()) {
			...
			RolapDimension dimension = (RolapDimension) rolapLevel.getClosedPeer()
					.getHierarchy().getDimension();

			RolapCubeDimension cubeDimension = new RolapCubeDimension(getCube(),
					dimension, xmlDimension, getDimension().getName() + "$Closure", -1,
					getCube().hierarchyList, getDimension().isHighCardinality());

			/*
			 * RME HACK WG: Note that the reason for registering this usage is so that
			...
			closedPeerCubeLevel = (RolapCubeLevel) cubeDimension.getHierarchies()[0]
					.getLevels()[1];

			if (!getCube().isVirtual()) {
				getCube().closureColumnBitKey.set(closedPeerCubeLevel.starKeyColumn
						.getBitPosition());
			}
	 ...
	 */
	private MondrianDef.Expression convertExpression(MondrianDef.Expression exp,
		...
		if (getHierarchy().isUsingCubeFact()) {
			...
			if (rel instanceof MondrianDef.Table) {
				return new MondrianDef.Column(((MondrianDef.Table) rel).getAlias(),
						col.getColumnName());
	...
	/**
	 * this method returns the RolapStar.Column if non-virtual, if virtual, find
	 * the base cube level and return it's column
	 * 
	 * @param baseCube
	...
	public boolean equals(RolapCubeLevel level) {
		...
		return super.equals(level) && getCube().equals(level.getCube());
	}
	...
	/**
	 * Encapsulation of the difference between levels in terms of how constraints
	 * are generated. There are implementations for 'all' levels, the 'null'
	 * level, parent-child levels and regular levels.
	 */
	interface LevelReader {
		 ...
		 * @param baseCube
		 *          base cube if virtual level
		 * @param request
		 *          Request to be constrained
		 * 
		 * @return true if request is unsatisfiable (e.g. if the member is the null
		 *         member)
		 */
		boolean constrainRequest(RolapCubeMember member, RolapCube baseCube,
				CellRequest request);
		 ...
		 */
		void constrainRegion(StarColumnPredicate predicate, RolapCube baseCube,
	...
	static final class RegularLevelReader implements LevelReader {
		...
		public boolean constrainRequest(RolapCubeMember member, RolapCube baseCube,
				CellRequest request) {
			...
			if (request.extendedContext && cubeLevel.getNameExp() != null) {
		...
		public void constrainRegion(StarColumnPredicate predicate,
				RolapCube baseCube, RolapCacheRegion cacheRegion) {
			RolapStar.Column column = cubeLevel.getBaseStarKeyColumn(baseCube);
			...
			if (predicate instanceof MemberColumnPredicate) {
				...
				final ValueColumnPredicate lowerBound = rangeColumnPredicate
						.getLowerBound();
				...
				final ValueColumnPredicate upperBound = rangeColumnPredicate
						.getUpperBound();
				...
				MemberTuplePredicate predicate2 = new MemberTuplePredicate(baseCube,
						lowerMember, !rangeColumnPredicate.getLowerInclusive(),
						upperMember, !rangeColumnPredicate.getUpperInclusive());
				// use the member as constraint, this will give us some
				// optimization potential
	...
	static final class ParentChildLevelReaderImpl implements LevelReader {
		...
		ParentChildLevelReaderImpl(RolapCubeLevel cubeLevel) {
			...
			this.wrappedAllMember = (RolapMember) closedPeerLevel.getHierarchy()
					.getDefaultMember();
			this.allMember = closedPeerCubeLevel.getHierarchy().getDefaultMember();
		...
		public boolean constrainRequest(RolapCubeMember member, RolapCube baseCube,
			...
			if (member.getDataMember() == null) {
				...
				return regularLevelReader.constrainRequest(member, baseCube, request);
			} else if (request.drillThrough) {
				return regularLevelReader.constrainRequest(member.getDataMember(),
						baseCube, request);
			} else {
				// isn't creating a member on the fly a bad idea?
				RolapMember wrappedMember = new RolapMemberBase(wrappedAllMember,
						closedPeerLevel, member.getKey());
				member = new RolapCubeMember(allMember, wrappedMember,
						closedPeerCubeLevel);

				return closedPeerCubeLevel.getLevelReader().constrainRequest(member,
						baseCube, request);
			}
		}
