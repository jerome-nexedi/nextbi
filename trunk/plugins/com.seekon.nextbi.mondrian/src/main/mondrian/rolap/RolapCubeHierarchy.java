/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapCubeHierarchy.java#32 $
 ...
 * @author Will Gorman
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/rolap/RolapCubeHierarchy.java#32 $
 */
public class RolapCubeHierarchy extends RolapHierarchy {

  private final boolean cachingEnabled = MondrianProperties.instance().EnableRolapCubeMemberCache
    .get();
  ...
  /**
   * Length of prefix to be removed when translating member unique names, or 0
   ...
   *          Wrapped hierarchy
   * @param subName
   *          Name of hierarchy within dimension
   * @param ordinal
   *          Ordinal of hierarchy within cube
   */
  public RolapCubeHierarchy(RolapCubeDimension cubeDimension,
    MondrianDef.CubeDimension cubeDim, RolapHierarchy rolapHierarchy,
    String subName, int ordinal) {
    super(cubeDimension, subName, applyPrefix(cubeDim, rolapHierarchy.getCaption()),
      rolapHierarchy.isVisible(), applyPrefix(cubeDim, rolapHierarchy
        .getDescription()), rolapHierarchy.hasAll(), null, rolapHierarchy
        .getAnnotationMap());
    this.ordinal = ordinal;
    if (!cubeDimension.getCube().isVirtual()) {
      this.usage = new HierarchyUsage(cubeDimension.getCube(), rolapHierarchy,
        cubeDim);
    }
    ...
    usingCubeFact = (cubeDimension.getCube().getFact() == null || cubeDimension
      .getCube().getFact().equals(rolapHierarchy.getRelation()));

    // re-alias names if necessary
    if (!usingCubeFact) {
      // join expressions are columns only
      assert (usage.getJoinExp() instanceof MondrianDef.Column);
      currentRelation = this.cubeDimension.getCube().getStar().getUniqueRelation(
        rolapHierarchy.getRelation(), usage.getForeignKey(),
    ...
    for (int i = 0; i < rolapHierarchy.getLevels().length; i++) {
      this.cubeLevels[i] = new RolapCubeLevel((RolapLevel) rolapHierarchy
        .getLevels()[i], this);
      if (i == 0) {
        if (rolapHierarchy.getAllMember() != null) {
          ...
          if (hasAll()) {
            ...
            allLevel = new RolapCubeLevel(rolapHierarchy.getAllMember().getLevel(),
              this);
            allLevel.init(cubeDimension.xmlDimension);
          }

          this.currentAllMember = new RolapAllCubeMember(rolapHierarchy
            .getAllMember(), allLevel);
        }
      }
    }
   ...
   * 
   * <p>
   * For example, if the [Time] dimension is imported as [Order Time] and [Ship
   * Time], then the [Time].[Weekly] hierarchy would have caption
   ...
   * 
   * <p>
   * If the dimension usage has a null name, or the name is the same as the
   * dimension, and no caption, then no prefix is applied.
   * 
   * @param cubeDim
   *          Cube dimension (maybe a usage of a shared dimension)
   * @param caption
  ...
  private static String applyPrefix(MondrianDef.CubeDimension cubeDim, String caption) {
    ...
    if (cubeDim instanceof MondrianDef.DimensionUsage) {
      final MondrianDef.DimensionUsage dimensionUsage = (MondrianDef.DimensionUsage) cubeDim;
   ...
   * 
   * @param oldrel
   *          Original relation, as defined in the schema
   * @param newrel
   *          New star relation, generated by RolapStar, canonical, and shared
   *          between all cubes with similar structure
   */
  protected void extractNewAliases(MondrianDef.RelationOrJoin oldrel,
    ...
    if (oldrel == null && newrel == null) {
      ...
      aliases.put(((MondrianDef.Relation) oldrel).getAlias(),
  ...
  public Member createMember(Member parent, Level level, String name, Formula formula) {
    ...
    if (formula == null) {
      ...
      RolapCalculatedMember member = new RolapCalculatedMember((RolapMember) parent,
        rolapLevel, name, formula);
      return new RolapCubeMember((RolapCubeMember) parent, member,
        (RolapCubeLevel) level);
    }
  }
  ...
  /**
   * The currentRelation object is derived from the shared relation object it is
   * generated via the RolapStar object, and contains unique aliases for it's
   * particular join path
   * 
  ...
  public final RolapCubeMember getDefaultMember() {
    if (currentDefaultMember == null) {
      ...
      currentDefaultMember = bootstrapLookup((RolapMember) rolapHierarchy
        .getDefaultMember());
    }
   ...
   * 
   * @param rolapMember
   *          Member of underlying hierarchy
   * @return Member of this hierarchy
   */
  private RolapCubeMember bootstrapLookup(RolapMember rolapMember) {
    RolapCubeMember parent = rolapMember.getParentMember() == null ? null
      : rolapMember.getParentMember().isAll() ? currentAllMember
  ...
  public Member getNullMember() {
    ...
    if (currentNullMember == null) {
      currentNullMember = new RolapCubeMember(null, (RolapMember) rolapHierarchy
        .getNullMember(), currentNullLevel);
    }
  ...
  public void setDefaultMember(Member defaultMeasure) {
    ...
    RolapCubeLevel level = new RolapCubeLevel((RolapLevel) rolapHierarchy
      .getDefaultMember().getLevel(), this);
    currentDefaultMember = new RolapCubeMember(null, (RolapMember) rolapHierarchy
      .getDefaultMember(), level);
  }
  ...
  /**
   * Converts the unique name of a member of the underlying hierarchy to the
   * appropriate name for this hierarchy.
   * 
   * <p>
  ...
  /**
   * TODO: Since this is part of a caching strategy, should be implemented as a
   * Strategy Pattern, avoiding hirarchy.
   */
  public static interface RolapCubeHierarchyMemberReader extends MemberReader {
    public RolapCubeMember lookupCubeMember(final RolapCubeMember parent,
      final RolapMember member, final RolapCubeLevel level);
   ...
   * 
   * - RolapHierarchy.SmartMemberReader.SmartCacheHelper -> This is the shared
   * cache across shared hierarchies. This member cache only contains members
   * loaded by non-cube specific member lookups. This cache should only contain
   * RolapMembers, not RolapCubeMembers
   * 
   ...
   * a join with the fact table is necessary, aka a SqlContextConstraint is
   * used. This cache may be redundant with rolapCubeCacheHelper.
   * 
   * - A Special note regarding RolapCubeHierarchyMemberReader.cubeSource - This
   * class was required for the special situation getMemberBuilder() method call
   * from RolapNativeSet. This class utilizes both the rolapCubeCacheHelper
   * class for storing RolapCubeMembers, and also the
   * RolapCubeHierarchyMemberReader's inherited SmartCacheHelper.
   ******/

  /**
   * member reader wrapper - uses existing member reader, but wraps and caches
   * all intermediate members.
   * 
   * <p>
   * Synchronization. Most synchronization takes place within SmartMemberReader.
   * All synchronization is done on the cacheHelper object.
   */
  public class CacheRolapCubeHierarchyMemberReader extends SmartMemberReader
    ...
    /**
     * this cache caches RolapCubeMembers that are light wrappers around shared
     * and non-shared Hierarchy RolapMembers. The inherited cacheHelper object
     * contains non-shared hierarchy RolapMembers. non-shared hierarchy
     * RolapMembers are created when a member lookup involves the Cube's fact
     * table.
     */
    protected MemberCacheHelper rolapCubeCacheHelper;

    private final boolean enableCache = MondrianProperties.instance().EnableRolapCubeMemberCache
      .get();

    public CacheRolapCubeHierarchyMemberReader() {
      super(new SqlMemberSource(RolapCubeHierarchy.this));
      rolapCubeCacheHelper = new MemberCacheHelper(RolapCubeHierarchy.this);

      cubeSource = new RolapCubeSqlMemberSource(this, RolapCubeHierarchy.this,
        rolapCubeCacheHelper, cacheHelper);
    ...
    public List<RolapMember> getRootMembers() {
      if (rootMembers == null) {
        rootMembers = getMembersInLevel(cubeLevels[0], 0, Integer.MAX_VALUE);
      }
      return rootMembers;
    }

    protected void readMemberChildren(List<RolapMember> parentMembers,
      List<RolapMember> children, MemberChildrenConstraint constraint) {
      List<RolapMember> rolapChildren = new ArrayList<RolapMember>();
      List<RolapMember> rolapParents = new ArrayList<RolapMember>();
      Map<String, RolapCubeMember> lookup = new HashMap<String, RolapCubeMember>();
      ...
      for (RolapMember currMember : rolapChildren) {
        RolapCubeMember parent = lookup.get(currMember.getParentMember()
          .getUniqueName());
        ...
        RolapCubeMember newmember = lookupCubeMember(parent, currMember, level);
      ...
      synchronized (cacheHelper) {
        for (Map.Entry<RolapMember, List<RolapMember>> entry : tempMap.entrySet()) {
          ...
          if (rolapCubeCacheHelper.getChildrenFromCache(member, constraint) == null) {
            final List<RolapMember> cacheList = entry.getValue();
            if (enableCache) {
              rolapCubeCacheHelper.putChildren(member, constraint, cacheList);
            }
          }
        }
      }
    ...
    public void getMemberChildren(List<RolapMember> parentMembers,
      ...
      synchronized (cacheHelper) {
        ...
        for (RolapMember parentMember : parentMembers) {
          List<RolapMember> list = rolapCubeCacheHelper.getChildrenFromCache(
    ...
    public List<RolapMember> getMembersInLevel(RolapLevel level, int startOrdinal,
      int endOrdinal, TupleConstraint constraint) {
      synchronized (cacheHelper) {
        checkCacheStatus();

        List<RolapMember> members = rolapCubeCacheHelper.getLevelMembersFromCache(
          level, constraint);
        if (members != null) {
          return members;
        }
        ...
        if (!joinReq) {
          list = rolapHierarchy.getMemberReader().getMembersInLevel(
            cubeLevel.getRolapLevel(), startOrdinal, endOrdinal, constraint);
        } else {
          list = super
            .getMembersInLevel(level, startOrdinal, endOrdinal, constraint);
        }
        ...
        for (RolapMember member : list) {
          ...
          if (false && member == rolapHierarchy.getAllMember()) {
            ...
            RolapCubeMember cubeMember = lookupCubeMemberWithParent(member,
              cubeLevel);
    ...
    private RolapCubeMember lookupCubeMemberWithParent(RolapMember member,
      ...
      if (parentMember == null) {
        ...
        // same level.
        final RolapCubeLevel parentLevel = parentMember.getLevel() == member
          .getLevel() ? cubeLevel : cubeLevel.getParentLevel();
        parentCubeMember = lookupCubeMemberWithParent(parentMember, parentLevel);
      }
      return lookupCubeMember(parentCubeMember, member, cubeLevel);
    }

    public RolapCubeMember lookupCubeMember(RolapCubeMember parent,
      RolapMember member, RolapCubeLevel level) {
      synchronized (cacheHelper) {
        ...
        if (enableCache) {
          ...
          cubeMember = (RolapCubeMember) rolapCubeCacheHelper.getMember(key, false);
          if (cubeMember == null) {
    ...
    protected void checkCacheStatus() {
      synchronized (cacheHelper) {
        ...
        if (cacheHelper.getChangeListener() != null) {
          if (cacheHelper.getChangeListener().isHierarchyChanged(getHierarchy())) {
            cacheHelper.flushCache();
            rolapCubeCacheHelper.flushCache();

            if (rolapHierarchy.getMemberReader() instanceof SmartMemberReader) {
              SmartMemberReader smartMemberReader = (SmartMemberReader) rolapHierarchy
                .getMemberReader();
              if (smartMemberReader.getMemberCache() instanceof MemberCacheHelper) {
                MemberCacheHelper helper = (MemberCacheHelper) smartMemberReader
                  .getMemberCache();
                helper.flushCache();
              }
            }
   ...
   * anything.
   */
  public class NoCacheRolapCubeHierarchyMemberReader extends NoCacheMemberReader
    implements RolapCubeHierarchyMemberReader {
    /**
     * cubeSource is passed as our member builder
     */
    ...
    public NoCacheRolapCubeHierarchyMemberReader() {
      super(new SqlMemberSource(RolapCubeHierarchy.this));
      rolapCubeCacheHelper = new MemberNoCacheHelper();
    ...
    protected void readMemberChildren(List<RolapMember> parentMembers,
      ...
      // extract RolapMembers from their RolapCubeMember objects
      // populate lookup for reconnecting parents and children
      final List<RolapCubeMember> parentRolapCubeMemberList = Util
        .cast(parentMembers);
    ...
    public List<RolapMember> getMembersInLevel(final RolapLevel level,
      ...
      if (!joinReq) {
        list = rolapHierarchy.getMemberReader().getMembersInLevel(
          ((RolapCubeLevel) level).getRolapLevel(), startOrdinal, endOrdinal,
          constraint);
      } else {
      ...
      return new UnsupportedList<RolapMember>() {
        ...
        private RolapMember mutate(final RolapMember member) {
          ...
          if (member.getParentMember() != null) {
            parent = createAncestorMembers((RolapCubeLevel) level.getParentLevel(),
              member.getParentMember());
          }
          return lookupCubeMember(parent, member, (RolapCubeLevel) level);
        }
      };
    }

    private RolapCubeMember createAncestorMembers(RolapCubeLevel level,
      RolapMember member) {
      RolapCubeMember parent = null;
      if (member.getParentMember() != null) {
        parent = createAncestorMembers(level.getParentLevel(), member
          .getParentMember());
      }
  ...
  public static class RolapCubeSqlMemberSource extends SqlMemberSource {
    ...
    public RolapCubeSqlMemberSource(RolapCubeHierarchyMemberReader memberReader,
      RolapCubeHierarchy hierarchy, MemberCacheHelper memberSourceCacheHelper,
    ...
    public RolapMember makeMember(RolapMember parentMember, RolapLevel childLevel,
      Object value, Object captionValue, boolean parentChild, SqlStatement stmt,
      Object key, int columnOffset) throws SQLException {
      final RolapCubeMember parentCubeMember = (RolapCubeMember) parentMember;
      ...
      RolapMember member = super.makeMember(parent, childCubeLevel.getRolapLevel(),
        value, captionValue, parentChild, stmt, key, columnOffset);
      return memberReader.lookupCubeMember(parentCubeMember, member, childCubeLevel);
    }
