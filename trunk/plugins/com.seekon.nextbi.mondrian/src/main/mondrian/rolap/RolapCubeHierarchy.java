...
public class RolapCubeHierarchy extends RolapHierarchy {
	...
	public RolapCubeHierarchy(RolapCubeDimension cubeDimension,
		...
		super(cubeDimension, subName, applyPrefix(cubeDim,
				rolapHierarchy.getCaption()), rolapHierarchy.isVisible(), applyPrefix(
				cubeDim, rolapHierarchy.getDescription()), rolapHierarchy.hasAll(),
				null, rolapHierarchy.getAnnotationMap());
		...
		if (!usingCubeFact) {
			...
			currentRelation = this.cubeDimension
					.getCube()
					.getStar()
					.getUniqueRelation(rolapHierarchy.getRelation(),
							usage.getForeignKey(),
		...
		for (int i = 0; i < rolapHierarchy.getLevels().length; i++) {
			this.cubeLevels[i] = new RolapCubeLevel(
			...
			if (i == 0) {
				if (rolapHierarchy.getAllMember() != null) {
					...
					if (hasAll()) {
						...
						allLevel = new RolapCubeLevel(rolapHierarchy.getAllMember()
								.getLevel(), this);
						allLevel.init(cubeDimension.xmlDimension);
					}

					this.currentAllMember = new RolapAllCubeMember(
	 ...
	 */
	private static String applyPrefix(MondrianDef.CubeDimension cubeDim,
			String caption) {
		if (caption == null) {
			return null;
		}
	...
	public Member getNullMember() {
		...
		if (currentNullMember == null) {
			currentNullMember = new RolapCubeMember(null,
					(RolapMember) rolapHierarchy.getNullMember(), currentNullLevel);
		}
	...
	public void setDefaultMember(Member defaultMeasure) {
		...
		currentDefaultMember = new RolapCubeMember(null,
	...
	public class CacheRolapCubeHierarchyMemberReader extends SmartMemberReader
		...
		protected void readMemberChildren(List<RolapMember> parentMembers,
			...
			synchronized (cacheHelper) {
				for (Map.Entry<RolapMember, List<RolapMember>> entry : tempMap
						.entrySet()) {
		...
		public List<RolapMember> getMembersInLevel(RolapLevel level,
			...
			synchronized (cacheHelper) {
				...
				List<RolapMember> members = rolapCubeCacheHelper
						.getLevelMembersFromCache(level, constraint);
				if (members != null) {
					return members;
				}
		...
		public RolapCubeMember lookupCubeMember(RolapCubeMember parent,
			...
			synchronized (cacheHelper) {
				...
				if (enableCache) {
					...
					cubeMember = (RolapCubeMember) rolapCubeCacheHelper.getMember(key,
							false);
					if (cubeMember == null) {
	 ...
	 * anything.
	 */
	public class NoCacheRolapCubeHierarchyMemberReader extends
			NoCacheMemberReader implements RolapCubeHierarchyMemberReader {
		/**
		 * cubeSource is passed as our member builder
		 */
		protected final RolapCubeSqlMemberSource cubeSource;
		...
		private RolapCubeMember createAncestorMembers(RolapCubeLevel level,
			...
			RolapCubeMember parent = null;
			if (member.getParentMember() != null) {
				parent = createAncestorMembers(level.getParentLevel(),
						member.getParentMember());
			}
	...
	public static class RolapCubeSqlMemberSource extends SqlMemberSource {
		...
		public RolapCubeSqlMemberSource(
				...
				MemberCacheHelper memberSourceCacheHelper, Object memberCacheLock) {
			super(hierarchy);
		...
		public RolapMember makeMember(RolapMember parentMember,
				RolapLevel childLevel, Object value, Object captionValue,
				boolean parentChild, SqlStatement stmt, Object key, int columnOffset)
				throws SQLException {
			...
			RolapMember member = super.makeMember(parent,
					childCubeLevel.getRolapLevel(), value, captionValue, parentChild,
					stmt, key, columnOffset);
			return memberReader.lookupCubeMember(parentCubeMember, member,
					childCubeLevel);
		}
