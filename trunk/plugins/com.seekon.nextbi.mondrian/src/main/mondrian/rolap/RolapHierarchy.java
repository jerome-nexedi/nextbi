/*
// $Id: //open/mondrian/src/main/mondrian/rolap/RolapHierarchy.java#114 $
 ...
 * 
 * <p>
 * NOTE: It is only valid to call that method on the measures hierarchy, and on
 * members of the {@link RolapCubeHierarchy} subclass. When the measures
 * hierarchy is of that class, we will move the method down.)
 * 
 * @author jhyde
 * @since 10 August, 2001
 * @version $Id: //open/mondrian/src/main/mondrian/rolap/RolapHierarchy.java#114
 *          $
 */
public class RolapHierarchy extends HierarchyBase {
	...
	/**
	 * The 'all' member of this hierarchy. This exists even if the hierarchy does
	 * not officially have an 'all' member.
	 */
	 ...
	 *          regular hierarchies
	 */
	RolapHierarchy(RolapDimension dimension, String subName, String caption,
			boolean visible, String description, boolean hasAll,
			RolapHierarchy closureFor, Map<String, Annotation> annotationMap) {
		super(dimension, subName, caption, visible, description, hasAll);
		this.annotationMap = annotationMap;
		this.allLevelName = "(All)";
		this.allMemberName = subName != null
				&& (MondrianProperties.instance().SsasCompatibleNaming.get() || name
						.equals(subName + "." + subName)) ? "All " + subName + "s" : "All "
				+ name + "s";
		this.closureFor = closureFor;
		if (hasAll) {
			this.levels = new RolapLevel[1];
			this.levels[0] = new RolapLevel(this, this.allLevelName, null, true,
					null, 0, null, null, null, null, null, null, null,
					RolapProperty.emptyArray, RolapLevel.FLAG_ALL
							| RolapLevel.FLAG_UNIQUE, null, null,
		...
		// the 'all' level.
		this.nullLevel = new RolapLevel(this, this.allLevelName, null, true, null,
				0, null, null, null, null, null, null, null, RolapProperty.emptyArray,
				RolapLevel.FLAG_ALL | RolapLevel.FLAG_UNIQUE, null, null,
				RolapLevel.HideMemberCondition.Never, LevelType.Null, "",
				Collections.<String, Annotation> emptyMap());
	}
	 ...
	 *          the xml object defining the cube dimension for this object
	 */
	RolapHierarchy(RolapDimension dimension, MondrianDef.Hierarchy xmlHierarchy,
			MondrianDef.CubeDimension xmlCubeDimension) {
		this(dimension, xmlHierarchy.name, xmlHierarchy.caption,
				xmlHierarchy.visible, xmlHierarchy.description, xmlHierarchy.hasAll,
				null, createAnnotationMap(xmlHierarchy.annotations));
		...
		if (xmlHierarchy.relation instanceof MondrianDef.InlineTable) {
			this.relation = RolapUtil.convertInlineTableToRelation(
					(MondrianDef.InlineTable) xmlHierarchy.relation, getRolapSchema()
							.getDialect());
		}
		...
		RolapLevel allLevel = new RolapLevel(this, this.allLevelName, null, true,
				null, 0, null, null, null, null, null, null, null,
				RolapProperty.emptyArray, RolapLevel.FLAG_ALL | RolapLevel.FLAG_UNIQUE,
				null, null, RolapLevel.HideMemberCondition.Never, LevelType.Regular,
				ALL_LEVEL_CARDINALITY, Collections.<String, Annotation> emptyMap());
		allLevel.init(xmlCubeDimension);
		this.allMember = new RolapMemberBase(null, allLevel, null, allMemberName,
		...
		if (xmlHierarchy.levels.length == 0) {
			throw MondrianResource.instance().HierarchyHasNoLevels
					.ex(getUniqueName());
		}
		...
		for (MondrianDef.Level level : xmlHierarchy.levels) {
			if (!levelNameSet.add(level.name)) {
				throw MondrianResource.instance().HierarchyLevelNamesNotUnique.ex(
		...
		if (hasAll) {
			...
			for (int i = 0; i < xmlHierarchy.levels.length; i++) {
				...
				if (xmlLevel.getKeyExp() == null
					...
					throw MondrianResource.instance().LevelMustHaveNameExpression
							.ex(xmlLevel.name);
				}
		...
		if (xmlCubeDimension instanceof MondrianDef.DimensionUsage) {
			String sharedDimensionName = ((MondrianDef.DimensionUsage) xmlCubeDimension).source;
		...
		if (xmlHierarchy.relation != null && xmlHierarchy.memberReaderClass != null) {
			throw MondrianResource.instance().HierarchyMustNotHaveMoreThanOneSource
					.ex(getUniqueName());
		}
	...
	public static Map<String, Annotation> createAnnotationMap(
		...
		final Map<String, Annotation> map = new LinkedHashMap<String, Annotation>();
	...
	protected int computeHashCode() {
		return super.computeHashCode()
				^ (sharedHierarchyName == null ? 0 : sharedHierarchyName.hashCode());
	}
	...
	void init(MondrianDef.CubeDimension xmlDimension) {
		...
		if (defaultMemberName != null) {
			...
			if (defaultMemberName.contains("[")) {
				...
				uniqueNameParts = Collections.singletonList(new Id.Segment(
						defaultMemberName, Id.Quoting.UNQUOTED));
			}

			// First look up from within this hierarchy. Works for unqualified
			// names, e.g. [USA].[CA].
			defaultMember = (Member) Util.lookupCompound(getRolapSchema()
					.getSchemaReader(), this, uniqueNameParts, false, Category.Member,
					MatchType.EXACT);
			...
			if (defaultMember == null) {
				defaultMember = (Member) Util.lookupCompound(getRolapSchema()
						.getSchemaReader(), new DummyElement(), uniqueNameParts, false,
						Category.Member, MatchType.EXACT);
			}
			if (defaultMember == null) {
				throw Util.newInternal("Can not find Default Member with name \""
						+ defaultMemberName + "\" in Hierarchy \"" + getName() + "\"");
			}
		}
	}
	...
	RolapLevel newMeasuresLevel() {
		RolapLevel level = new RolapLevel(this, "MeasuresLevel", null, true, null,
				this.levels.length, null, null, null, null, null, null, null,
				RolapProperty.emptyArray, 0, null, null,
				RolapLevel.HideMemberCondition.Never, LevelType.Regular, "",
				Collections.<String, Annotation> emptyMap());
	...
	/**
	 * If this hierarchy has precisely one table, returns that table; if this
	 * hierarchy has no table, return the cube's fact-table; otherwise, returns
	 * null.
	 */
	MondrianDef.Relation getUniqueTable() {
		if (relation instanceof MondrianDef.Relation) {
			...
			throw Util
					.newInternal("hierarchy's relation is a " + relation.getClass());
		}
	}
	...
	private static MondrianDef.Relation getTable(String tableName,
	...
	public Member getDefaultMember() {
		...
		if (defaultMember == null) {
			...
			final SchemaReader schemaReader = getRolapSchema().getSchemaReader();
			List<RolapMember> calcMemberList = Util.cast(schemaReader
					.getCalculatedMembers(getLevels()[0]));
			for (RolapMember rootMember : UnionIterator.over(rootMembers,
					calcMemberList)) {
			...
			if (defaultMember == null) {
				throw MondrianResource.instance().InvalidHierarchyCondition.ex(this
						.getUniqueName());
			}
		}
	...
	public Member createMember(Member parent, Level level, String name,
			Formula formula) {
		if (formula == null) {
			return new RolapMemberBase((RolapMember) parent, (RolapLevel) level, name);
		} else if (level.getDimension().isMeasures()) {
			return new RolapCalculatedMeasure((RolapMember) parent,
					(RolapLevel) level, name, formula);
		} else {
			return new RolapCalculatedMember((RolapMember) parent,
					(RolapLevel) level, name, formula);
		}
	}
	 ...
	 * 
	 * <p>
	 * If this hierarchy is a public -- that is, it belongs to a dimension which
	 * is a usage of a shared dimension -- then <code>sharedHierarchyName</code>
	 * holds the unique name of the shared hierarchy; otherwise it is null.
	 * 
	 * <p>
	 * Suppose this hierarchy is "Weekly" in the dimension "Order Date" of cube
	 * "Sales", and that "Order Date" is a usage of the "Time" dimension. Then
	 * <code>sharedHierarchyName</code> will be "[Time].[Weekly]".
	 */
	 ...
	 * 
	 * <p>
	 * This method is idempotent: if you call it more than once, it only adds the
	 * table(s) to the FROM clause once.
	 * 
	 * @param query
	 *          Query to add the hierarchy to
	 * @param expression
	 *          Level to qualify up to; if null, qualifies up to the topmost
	 *          ('all') expression, which may require more columns and more joins
	 */
	void addToFromInverse(SqlQuery query, MondrianDef.Expression expression) {
		if (relation == null) {
			throw Util.newError("cannot add hierarchy " + getUniqueName()
		...
		if (relation instanceof MondrianDef.Join) {
			if (expression != null) {
				subRelation = relationSubsetInverse(relation,
	...
	/**
	 * Adds to the FROM clause of the query the tables necessary to access the
	 * members of this hierarchy. If <code>expression</code> is not null, adds the
	 * tables necessary to compute that expression.
	 * 
	 * <p>
	 * This method is idempotent: if you call it more than once, it only adds the
	 * table(s) to the FROM clause once.
	...
	/**
	 * Adds a table to the FROM clause of the query. If <code>table</code> is not
	 * null, adds the table. Otherwise, add the relation on which this hierarchy
	 * is based on.
	 * 
	 * <p>
	 * This method is idempotent: if you call it more than once, it only adds the
	...
	void addToFrom(SqlQuery query, RolapStar.Table table) {
		...
		boolean tableAdded = query.addFrom(subRelation,
				table != null ? table.getAlias() : null, failIfExists);
	...
	/**
	 * Returns the smallest subset of <code>relation</code> which contains the
	 * relation <code>alias</code>, or null if these is no relation with such an
	 * alias, in inverse join order, used for agg tables.
	 * 
	 * @param relation
	 *          the relation in which to look for table by its alias
	 * @param alias
	 *          table alias to search for
	 * @return the smallest containing relation or null if no matching table is
	 *         found in <code>relation</code>
	 */
	private static MondrianDef.RelationOrJoin relationSubsetInverse(
			MondrianDef.RelationOrJoin relation, String alias) {
		if (relation instanceof MondrianDef.Relation) {
			...
			MondrianDef.RelationOrJoin leftRelation = relationSubsetInverse(
					join.left, alias);
			return (leftRelation == null) ? relationSubsetInverse(join.right, alias)
					: join;
	...
	private static MondrianDef.RelationOrJoin relationSubset(
		...
		if (relation instanceof MondrianDef.Relation) {
			...
			MondrianDef.Join join = (MondrianDef.Join) relation;
			MondrianDef.RelationOrJoin rightRelation = relationSubset(join.right,
					alias);
			return (rightRelation == null) ? relationSubset(join.left, alias)
					: MondrianProperties.instance().FilterChildlessSnowflakeMembers.get() ? join
							: rightRelation;
		} else {
			throw Util.newInternal("bad relation type " + relation);
		}
	...
	/**
	 * Returns the smallest subset of <code>relation</code> which contains the
	 * table <code>targetTable</code>, or null if the targetTable is not one of
	 * the joining table in <code>relation</code>.
	 * 
	 * @param relation
	 *          the relation in which to look for targetTable
	 * @param targetTable
	 *          table to add to the query
	 * @return the smallest containing relation or null if no matching table is
	 *         found in <code>relation</code>
	...
	private static MondrianDef.RelationOrJoin lookupRelationSubset(
			MondrianDef.RelationOrJoin relation, RolapStar.Table targetTable) {
		if (relation instanceof MondrianDef.Table) {
			...
			MondrianDef.Join join = (MondrianDef.Join) relation;
			MondrianDef.RelationOrJoin rightRelation = lookupRelationSubset(
					join.right, targetTable);
	 ...
	 * 
	 * <p>
	 * This method may not be efficient, so the caller should take care not to
	 * call it too often. A cache is a good idea.
	 * 
	 * @param role
	 *          Role
	...
	MemberReader createMemberReader(Role role) {
		...
		case ALL:
			return (isRagged()) ? new RestrictedMemberReader(getMemberReader(), role)
					: getMemberReader();
		...
		case CUSTOM:
			...
			case PARTIAL:
				Type memberType1 = new mondrian.olap.type.MemberType(getDimension(),
						this, null, null);
				SetType setType = new SetType(memberType1);
				ListCalc listCalc = new AbstractListCalc(new DummyExp(setType),
						new Calc[0]) {
					public TupleList evaluateList(Evaluator evaluator) {
						return new UnaryTupleList(FunUtil.getNonEmptyMemberChildren(
								evaluator, ((RolapEvaluator) evaluator).getExpanding()));
					}
				...
				final Calc partialCalc = new LimitedRollupAggregateCalc(returnType,
						listCalc);

				final Exp partialExp = new ResolvedFunCall(new FunDefBase("$x", "x",
						"In") {
				...
				return new LimitedRollupSubstitutingMemberReader(getMemberReader(),
	 ...
	 * 
	 * <p>
	 * It is efficient to share one expression between all calculated members in a
	 * parent-child hierarchy, so we only need need to validate the expression
	 * once.
	 */
	synchronized Exp getAggregateChildrenExpression() {
		if (aggregateChildrenExpression == null) {
			UnresolvedFunCall fc = new UnresolvedFunCall("$AggregateChildren",
					Syntax.Internal, new Exp[] { new HierarchyExpr(this) });
			Validator validator = Util.createSimpleValidator(BuiltinFunTable
					.instance());
	...
	/**
	 * Builds a dimension which maps onto a table holding the transitive closure
	 * of the relationship for this parent-child level.
	 * 
	 * <p>
	 * This method is triggered by the {@link mondrian.olap.MondrianDef.Closure}
	 * element in a schema, and is only meaningful for a parent-child hierarchy.
	 * 
	 * <p>
	 * When a Schema contains a parent-child Hierarchy that has an associated
	 * closure table, Mondrian creates a parallel internal Hierarchy, called a
	 * "closed peer", that refers to the closure table. This is indicated in the
	 * schema at the level of a Level, by including a Closure element. The closure
	 * table represents the transitive closure of the parent-child relationship.
	 * 
	 * <p>
	 * The peer dimension, with its single hierarchy, and 3 levels (all, closure,
	 * item) really 'belong to' the parent-child level. If a single hierarchy had
	 * two parent-child levels (however unlikely this might be) then each level
	 * would have its own auxiliary dimension.
	 * 
	 * <p>
	 * For example, in the demo schema the [HR].[Employee] dimension contains a
	 ...
	 * 
	 * <p>
	 * Note that the original Level with the Closure produces two Levels in the
	 * closed peer Hierarchy: a simple peer (Employee) and a closed peer
	 * (Closure).
	 * 
	 * @param src
	...
	RolapDimension createClosedPeerDimension(RolapLevel src,
			MondrianDef.Closure clos, MondrianDef.CubeDimension xmlDimension) {
		// REVIEW (mb): What about attribute primaryKeyTable?

		// Create a peer dimension.
		RolapDimension peerDimension = new RolapDimension(dimension.getSchema(),
				dimension.getName() + "$Closure", null, true,
				"Closure dimension for parent-child hierarchy " + getName(),
				DimensionType.StandardDimension, dimension.isHighCardinality(),
				Collections.<String, Annotation> emptyMap());
		...
		MondrianDef.Expression keyExp = new MondrianDef.Column(clos.table.name,
				clos.parentColumn);

		RolapLevel level = new RolapLevel(peerHier, "Closure", caption, true,
				description, index++, keyExp, null, null, null, null,
				null, // no longer a parent-child hierarchy
				null, RolapProperty.emptyArray, flags | RolapLevel.FLAG_UNIQUE,
				src.getDatatype(), null, src.getHideMemberCondition(),
				src.getLevelType(), "", Collections.<String, Annotation> emptyMap());
		...
		RolapLevel sublevel = new RolapLevel(peerHier, "Item", null, true, null,
				index++, keyExp, null, null, null, null,
				null, // no longer a parent-child hierarchy
				null, RolapProperty.emptyArray, flags, src.getDatatype(),
				src.getInternalType(), src.getHideMemberCondition(),
				src.getLevelType(), "", Collections.<String, Annotation> emptyMap());
	...
	/**
	 * <p>
	 * Gets "unique key level name" attribute of this Hierarchy, if set. If set,
	 * this property indicates that all level properties are functionally
	 * dependent (invariant) on their associated levels, and that the set of
	 * levels from the root to the named level (inclusive) effectively defines an
	 * alternate key.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * A <code>RolapNullMember</code> is the null member of its hierarchy. Every
	 * hierarchy has precisely one. They are yielded by operations such as
	 ...
	 * [Measures] dimension).
	 */
	protected static class RolapCalculatedMeasure extends RolapCalculatedMember
			implements RolapMeasure {
		private RolapResult.ValueFormatter cellFormatter;

		public RolapCalculatedMeasure(RolapMember parent, RolapLevel level,
				String name, Formula formula) {
		...
		public synchronized void setProperty(String name, Object value) {
			if (name.equals(Property.CELL_FORMATTER.getName())) {
				...
				try {
					CellFormatter formatter = RolapSchema.getCellFormatter(
							cellFormatterClass, null);
					this.cellFormatter = new RolapResult.CellFormatterValueFormatter(
							formatter);
				} catch (Exception e) {
			...
			if (name.equals(Property.CELL_FORMATTER_SCRIPT.name)) {
				String language = (String) getPropertyValue(Property.CELL_FORMATTER_SCRIPT_LANGUAGE.name);
				String scriptText = (String) value;
				try {
					final Scripts.ScriptDefinition script = new Scripts.ScriptDefinition(
							scriptText, Scripts.ScriptLanguage.lookup(language));
					CellFormatter formatter = RolapSchema.getCellFormatter(null, script);
					this.cellFormatter = new RolapResult.CellFormatterValueFormatter(
							formatter);
				} catch (Exception e) {
					throw MondrianResource.instance().CellFormatterLoadFailed.ex(
	...
	/**
	 * Substitute for a member in a hierarchy whose rollup policy is 'partial' or
	 * 'hidden'. The member is calculated using an expression which aggregates
	 * only visible descendants.
	 * 
	 * <p>
	 * Note that this class extends RolapCubeMember only because other code
	 * expects that all members in a RolapCubeHierarchy are RolapCubeMembers. As
	 * part of {@link mondrian.util.Bug#BugSegregateRolapCubeMemberFixed}, maybe
	 * make {@link mondrian.rolap.RolapCubeMember} an interface.
	 * 
	...
	public static class LimitedRollupMember extends RolapCubeMember {
		...
		LimitedRollupMember(RolapCubeMember member, Exp exp) {
			super(member.getParentMember(), member.getRolapMember(), member
					.getLevel());
	...
	/**
	 * Member reader which wraps a hierarchy's member reader, and if the role has
	 * limited access to the hierarchy, replaces members with dummy members which
	 * evaluate to the sum of only the accessible children.
	 */
	private static class LimitedRollupSubstitutingMemberReader extends
		 ...
		 * 
		 * @param memberReader
		 *          Underlying member reader
		 * @param role
		 *          Role to enforce
		 * @param hierarchyAccess
		 *          Access this role has to the hierarchy
		 * @param exp
		 *          Expression for hidden member
		 */
		public LimitedRollupSubstitutingMemberReader(MemberReader memberReader,
				Role role, Role.HierarchyAccess hierarchyAccess, Exp exp) {
		...
		public RolapMember substitute(final RolapMember member) {
			if (member != null
					&& (hierarchyAccess.getAccess(member) == Access.CUSTOM || hierarchyAccess
	 ...
	 */
	private static class LimitedRollupAggregateCalc extends
			AggregateFunDef.AggregateCalc {
		public LimitedRollupAggregateCalc(Type returnType, ListCalc listCalc) {
			super(new DummyExp(returnType), listCalc, new ValueCalc(new DummyExp(
					returnType)));
		}
	}
	 ...
	 * shared hierarchies. Acts like a cube that has a single child, the hierarchy
	 * in question.
	 */
