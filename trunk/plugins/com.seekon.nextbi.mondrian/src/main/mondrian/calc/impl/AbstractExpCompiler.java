/*
// $Id: //open/mondrian/src/main/mondrian/calc/impl/AbstractExpCompiler.java#41 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/calc/impl/AbstractExpCompiler.
 *          java#41 $
 * @since Sep 29, 2005
 */
public class AbstractExpCompiler implements ExpCompiler {
	...
	public AbstractExpCompiler(Evaluator evaluator, Validator validator,
		...
		this.resultStyles = (resultStyles == null) ? ResultStyle.ANY_LIST
				: resultStyles;
	}
	 ...
	 */
	public Calc compileAs(Exp exp, Type resultType,
		...
		if (Util.Retrowoven) {
			...
			List<ResultStyle> tmp = new ArrayList<ResultStyle>(
	...
	private MemberCalc hierarchyToMember(HierarchyCalc hierarchyCalc) {
		...
		if (hierarchy != null) {
			return new HierarchyCurrentMemberFunDef.FixedCalcImpl(new DummyExp(
					TypeUtil.toMemberType(hierarchyCalc.getType())), hierarchy);
		} else {
			return new HierarchyCurrentMemberFunDef.CalcImpl(new DummyExp(
					TypeUtil.toMemberType(hierarchyCalc.getType())), hierarchyCalc);
		}
	}
	...
	public LevelCalc compileLevel(Exp exp) {
		...
		if (type instanceof MemberType) {
			...
			return new MemberLevelFunDef.CalcImpl(new DummyExp(
					LevelType.forType(type)), memberCalc);
		}
	...
	public DimensionCalc compileDimension(Exp exp) {
		...
		if (type instanceof HierarchyType) {
			final HierarchyCalc hierarchyCalc = compileHierarchy(exp);
			return new HierarchyDimensionFunDef.CalcImpl(new DummyExp(
					new DimensionType(type.getDimension())), hierarchyCalc);
		}
	...
	public HierarchyCalc compileHierarchy(Exp exp) {
		...
		if (type instanceof DimensionType) {
			...
			if (dimension != null) {
				final Hierarchy hierarchy = FunUtil
						.getDimensionDefaultHierarchy(dimension);
				if (hierarchy != null) {
					...
					throw MondrianResource.instance().CannotImplicitlyConvertDimensionToHierarchy
							.ex(dimension.getName());
				}
			}
			final DimensionCalc dimensionCalc = compileDimension(exp);
			return new DimensionHierarchyCalc(new DummyExp(
					HierarchyType.forType(type)), dimensionCalc);
		}
		if (type instanceof MemberType) {
			// <Member> --> <Member>.Hierarchy
			final MemberCalc memberCalc = compileMember(exp);
			return new MemberHierarchyFunDef.CalcImpl(new DummyExp(
					HierarchyType.forType(type)), memberCalc);
		}
		if (type instanceof LevelType) {
			// <Level> --> <Level>.Hierarchy
			final LevelCalc levelCalc = compileLevel(exp);
			return new LevelHierarchyFunDef.CalcImpl(new DummyExp(
					HierarchyType.forType(type)), levelCalc);
		}
	...
	public IntegerCalc compileInteger(Exp exp) {
		...
		if (type instanceof DecimalType && ((DecimalType) type).getScale() == 0) {
			...
			if (calc instanceof ConstantCalc) {
				...
				return new ConstantCalc(new DecimalType(Integer.MAX_VALUE, 0),
	...
	public IterCalc compileIter(Exp exp) {
		IterCalc calc = (IterCalc) compileAs(exp, null, ResultStyle.ITERABLE_ONLY);
		if (calc == null) {
		...
		if (calc instanceof BooleanCalc) {
			if (calc instanceof ConstantCalc) {
				...
				if (!(o instanceof Boolean)) {
					return ConstantCalc.constantBoolean(CastFunDef.toBoolean(o,
							new BooleanType()));
				}
			}
	...
	public DoubleCalc compileDouble(Exp exp) {
		...
		if (calc instanceof ConstantCalc
			...
			return ConstantCalc.constantDouble(((ConstantCalc) calc)
					.evaluateDouble(null));
		}
	...
	public Calc compileScalar(Exp exp, boolean specific) {
		...
		if (type instanceof MemberType) {
			...
			final TupleValueCalc scalarCalc = new TupleValueCalc(new DummyExp(
					tupleType.getValueType()), tupleCalc, getEvaluator()
	...
	private Calc memberToScalar(MemberCalc memberCalc) {
		...
		return MemberValueCalc.create(new DummyExp(memberType.getValueType()),
				new MemberCalc[] { memberCalc }, getEvaluator()
						.mightReturnNullForUnrelatedDimension());
	}
	...
	public ParameterSlot registerParameter(Parameter parameter) {
		...
		if (type instanceof ScalarType) {
			if (!defaultExp.getType().equals(type)) {
				defaultExp = new UnresolvedFunCall("Cast", Syntax.Cast, new Exp[] {
						defaultExp,
						Literal.createSymbol(Category.instance.getName(TypeUtil
								.typeToCategory(type))) });
	...
	private static class ParameterSlotImpl implements ParameterSlot {
		 ...
		 * 
		 * @param calc
		 *          Compiled expression to compute default value of parameter
		 * 
	...
	private static class DimensionHierarchyCalc extends AbstractHierarchyCalc {
		...
		public Hierarchy evaluateHierarchy(Evaluator evaluator) {
			...
			throw FunUtil
					.newEvalException(MondrianResource.instance().CannotImplicitlyConvertDimensionToHierarchy
							.ex(dimension.getName()));
		}
	}
}
