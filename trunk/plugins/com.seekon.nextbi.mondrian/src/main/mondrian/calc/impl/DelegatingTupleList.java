/*
// $Id: //open/mondrian/src/main/mondrian/calc/impl/DelegatingTupleList.java#2 $
...
/**
 * Implementation of {@link mondrian.calc.TupleList} based on a list of {@code
 * List<Member>} tuples.
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/calc/impl/DelegatingTupleList.
 *          java#2 $
 * @author jhyde
 */
public class DelegatingTupleList extends AbstractTupleList {
  ...
  public DelegatingTupleList(int arity, List<List<Member>> list) {
    ...
    this.list = list;
    assert list.isEmpty()
      || (list.get(0) instanceof List && (list.get(0).isEmpty()
        || list.get(0).get(0) == null || list.get(0).get(0) instanceof Member)) : "sanity check failed: "
      + list;
  }
  ...
  public TupleList cloneList(int capacity) {
    return new DelegatingTupleList(arity,
      capacity < 0 ? new ArrayList<List<Member>>(list)
