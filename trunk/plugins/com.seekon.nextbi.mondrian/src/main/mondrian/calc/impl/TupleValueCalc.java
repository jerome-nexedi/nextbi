...
/**
 * Expression which evaluates a tuple expression, sets the dimensional context
 * to the result of that expression, then yields the value of the current
 * measure in the current dimensional context.
 * 
 * <p>
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/calc/impl/TupleValueCalc.java#11 $
 * @since Sep 27, 2005
 */
public class TupleValueCalc extends GenericCalc {
   ...
   *          Expression
   * @param tupleCalc
   *          Compiled expression to evaluate the tuple
   * @param nullCheck
  ...
  /**
   * Optimizes the scalar evaluation of a tuple. It evaluates the members of the
   * tuple, sets the context to these members, and evaluates the scalar result
   * in one step, without generating a tuple.
   * <p/>
   * 
  ...
  public Calc optimize() {
    if (tupleCalc instanceof TupleFunDef.CalcImpl) {
      ...
      return MemberValueCalc.create(new DummyExp(type), calc.getMemberCalcs(),
        nullCheck);
    }
    return this;
  }
