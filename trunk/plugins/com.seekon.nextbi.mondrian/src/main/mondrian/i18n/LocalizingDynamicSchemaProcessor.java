/*
// $Id: //open/mondrian/src/main/mondrian/i18n/LocalizingDynamicSchemaProcessor.java#12 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/i18n/LocalizingDynamicSchemaProcessor
 *          .java#12 $
 */
public class LocalizingDynamicSchemaProcessor extends
		FilterDynamicSchemaProcessor implements DynamicSchemaProcessor {
	private static final Logger LOGGER = Logger
			.getLogger(LocalizingDynamicSchemaProcessor.class);
	 ...
	 * 
	 * <p>
	 * The name of the property file is typically the name of a class, as per
	 * {@link ResourceBundle#getBundle(String)}. However, for backwards
	 * compatibility, the name can contain slashes (which are converted to dots)
	 * and end with ".properties" (which is removed). Therefore
	 * "com/acme/MyResource.properties" is equivalent to "com.acme.MyResource".
	 * 
	...
	void populate(String propFile) {
		if (propFile.endsWith(".properties")) {
			propFile = propFile.substring(0,
					propFile.length() - ".properties".length());
		}
		try {
			bundle = ResourceBundle.getBundle(propFile, Util.parseLocale(locale),
	...
	private String doRegExReplacements(String schema) {
		...
		while (match.find()) {
			...
			try {
				...
				LOGGER.error("missing resource key at substring(" + start + "," + end
						+ ")", e);
			}
		}
