/*
// $Id: //open/mondrian/src/main/mondrian/web/servlet/MdxQueryServlet.java#5 $
 ...
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/web/servlet/MdxQueryServlet.java#5
 *          $
 */
public class MdxQueryServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
			...
			for (int j = 0; j < columnWidth; j++) {
				...
				if (j == 0 && result.getAxes().length > 1) {
					...
					html.append("<td nowrap class='slicer' rowspan='")
							.append(columnWidth).append("' colspan='").append(rowWidth)
							.append("'>");
				...
				for (int i = 0; i < columns.size(); i++) {
					...
					html.append("<td nowrap class='columnheading' colspan='")
							.append(width).append("'>").append(member.getUniqueName())
							.append("</td>");
				}
		...
		getServletContext().getRequestDispatcher("/adhoc.jsp").include(request,
				response);
	}

	private void showCell(StringBuilder out, Cell cell) {
		out.append("<td class='cell'>").append(cell.getFormattedValue())
				.append("</td>");
	}

	private void processTransform(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		String queryName = request.getParameter("query");
		ResultCache rc = ResultCache.getInstance(request.getSession(),
				getServletContext(), queryName);
		...
		response.setHeader("Content-Type", "text/html");
		getServletContext().getRequestDispatcher(redirect).include(request,
				response);
	}
