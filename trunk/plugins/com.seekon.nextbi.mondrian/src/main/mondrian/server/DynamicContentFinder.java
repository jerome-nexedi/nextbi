/*
// $Id: //open/mondrian/src/main/mondrian/server/DynamicContentFinder.java#8 $
...
/**
 * Implementation of {@link RepositoryContentFinder} that periodically reloads
 * the content of the repository.
 * 
 * <p>
 * The updates are performed by a background thread. It is important to call
 * {@link DynamicContentFinder#shutdown()} once this object can be disposed of.
 * 
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/server/DynamicContentFinder.java#8
 *          $
 * @author Thiyagu Ajit
 * @author Luc Boudreau
 * @author Julian Hyde
...
public class DynamicContentFinder extends UrlRepositoryContentFinder {
	...
	public DynamicContentFinder(String dataSourcesConfigUrl) {
		...
		reloadDataSources();
		scheduledTask = executorService.scheduleWithFixedDelay(new Runnable() {
			public void run() {
				Object result;
				try {
	...
	public synchronized boolean reloadDataSources() {
		try {
			...
			DataSourcesConfig.DataSources newDataSources = XmlaSupport
					.parseDataSources(dataSourcesConfigString, LOGGER);
	...
	private Map<String, Pair<DataSourcesConfig.DataSource, DataSourcesConfig.Catalog>> createCatalogMap(
			DataSourcesConfig.DataSources newDataSources) {
		Map<String, Pair<DataSourcesConfig.DataSource, DataSourcesConfig.Catalog>> newDatasourceCatalogNames = new HashMap<String, Pair<DataSourcesConfig.DataSource, DataSourcesConfig.Catalog>>();
		for (DataSourcesConfig.DataSource dataSource : newDataSources.dataSources) {
			for (DataSourcesConfig.Catalog catalog : dataSource.catalogs.catalogs) {
				...
				newDatasourceCatalogNames.put(catalog.name,
						Pair.of(dataSource, catalog));
			}
		}
	...
	public synchronized void flushObsoleteCatalogs(
		...
		Map<String, Pair<DataSourcesConfig.DataSource, DataSourcesConfig.Catalog>> newDatasourceCatalogs = createCatalogMap(newDataSources);
		...
		for (DataSourcesConfig.DataSource oldDataSource : dataSources.dataSources) {
			for (DataSourcesConfig.Catalog oldCatalog : oldDataSource.catalogs.catalogs) {
				Pair<DataSourcesConfig.DataSource, DataSourcesConfig.Catalog> pair = newDatasourceCatalogs
						.get(oldCatalog.name);
				if (pair == null
						|| !areCatalogsEqual(oldDataSource, oldCatalog, pair.left,
								pair.right)) {
	...
	public static boolean areCatalogsEqual(
		...
		return Util.equals(dsi(dataSource1, catalog1), dsi(dataSource2, catalog2))
	...
	public static String dsi(DataSourcesConfig.DataSource dataSource,
		...
		return catalog.dataSourceInfo == null && dataSource != null ? dataSource.dataSourceInfo
