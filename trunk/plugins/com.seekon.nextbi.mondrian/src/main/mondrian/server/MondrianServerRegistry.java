/*
// $Id: //open/mondrian/src/main/mondrian/server/MondrianServerRegistry.java#2 $
 ...
 * 
 * <p>
 * This class is not a public API. User applications should use the methods in
 * {@link mondrian.olap.MondrianServer}.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/server/MondrianServerRegistry.
 *          java#2 $
 */
public class MondrianServerRegistry {
	...
	/**
	 * Looks up a server with a given id. If the id is null, returns the static
	 * server.
	 * 
	 * @param instanceId
	...
	public synchronized MondrianServer.MondrianVersion getOrLoadVersion() {
		if (version == null) {
			...
			if (false) {
				System.out.println("vendor=" + vendor + ", title=" + title
						+ ", versionString=" + versionString);
			}
			int dot1 = versionString.indexOf('.');
			final int majorVersion = dot1 < 0 ? 1 : Integer.valueOf(versionString
					.substring(0, dot1));
			int dot2 = versionString.indexOf('.', dot1 + 1);
			final int minorVersion = dot2 < 0 ? 0 : Integer.valueOf(versionString
					.substring(dot1 + 1, dot2));
	...
	private static String[] loadVersionFile() {
		...
		String title = null;
		String vendor = null;
		URL resource = MondrianServerImpl.class.getClassLoader().getResource(
				"DefaultRules.xml");
		if (resource != null) {
			try {
				String path = resource.getPath();
				String path2 = Util.replace(path, "DefaultRules.xml", "VERSION.txt");
				URL resource2 = new URL(resource.getProtocol(), resource.getHost(),
						path2);
