...
public class Formula extends QueryPart {
	...
	public Object clone() {
		return new Formula(isMember, id, exp.clone(),
				MemberProperty.cloneArray(memberProperties), mdxMember, mdxSet);
	}
	...
	void createElement(Query q) {
		...
		if (isMember) {
			...
			for (int i = 0; i < segments.size(); i++) {
				...
				if (mdxElement == null || i == segments.size() - 1) {
					...
					if (parent instanceof Member) {
						...
						if (hierarchy == null) {
							throw MondrianResource.instance().MdxCalculatedHierarchyError
									.ex(id.toString());
						}
					...
					Member mdxMember = level.getHierarchy().createMember(parentMember,
							level, segment.name, this);
			...
			mdxSet = new SetBase(segments.get(0).name, null, null, exp, false,
	...
	void rename(String newName) {
		...
		Util.assertTrue(segments.get(segments.size() - 1).name
