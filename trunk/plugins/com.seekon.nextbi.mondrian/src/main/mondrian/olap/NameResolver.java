/*
// $Id: //open/mondrian/src/main/mondrian/olap/NameResolver.java#7 $
 ...
 * 
 * @version $Id: //open/mondrian/src/main/mondrian/olap/NameResolver.java#7 $
 */
public final class NameResolver {
	 ...
	 * 
	 * @param parent
	 *          Parent element to search in, usually a cube
	 * @param segments
	...
	public OlapElement resolve(OlapElement parent,
			List<IdentifierSegment> segments, boolean failIfNotFound, int category,
			MatchType matchType, List<Namespace> namespaces) {
		OlapElement element;
		if (matchType == MatchType.EXACT) {
			element = resolveExact(parent, segments, namespaces);
		} else {
			element = resolveInexact(parent, segments, matchType, namespaces);
		}
		if (element != null) {
			element = nullify(category, element);
		}
		if (element == null && failIfNotFound) {
			throw Util.newElementNotFoundException(category, new IdentifierNode(
					segments));
		}
		return element;
	}

	private OlapElement resolveInexact(OlapElement parent,
			List<IdentifierSegment> segments, MatchType matchType,
	...
	// matchType == EXACT
	private OlapElement resolveExact(OlapElement parent,
			List<IdentifierSegment> segments, List<Namespace> namespaces) {
		OlapElement element = parent;
		for (final IdentifierSegment segment : segments) {
			assert element != null;
			OlapElement child = null;
	...
	/**
	 * Converts an element to the required type, converting if possible, returning
	 * null if it is not of the required type and cannot be converted.
	 * 
	 * @param category
	 *          Desired category of element
	 * @param element
	 *          Element
	...
	/**
	 * Returns whether a formula (representing a calculated member or named set)
	 * matches a given parent and name segment.
	 * 
	 * @param formula
	 *          Formula
	 * @param parent
	 ...
	 */
	public static boolean matches(Formula formula, OlapElement parent,
	 ...
	 * 
	 * <p>
	 * Elements' names are hierarchical, so elements are resolved one name segment
	 * at a time. It is possible for an element to be defined in a different
	 * namespace than its parent: for example, stored member [Dim].[Hier].[X].[Y]
	 * might have a child [Dim].[Hier].[X].[Y].[Z] which is a calculated member
	 * defined using a WITH MEMBER clause.
	 * </p>
	 */
	public interface Namespace {
		 ...
		 * 
		 * <p>
		 * If {@code matchType} is {@link MatchType#EXACT}, effect is identical to
		 * calling
		 ...
		 */
		OlapElement lookupChild(OlapElement parent, IdentifierSegment segment,
				MatchType matchType);
