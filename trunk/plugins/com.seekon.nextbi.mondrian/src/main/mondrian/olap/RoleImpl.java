/*
// $Id: //open/mondrian/src/main/mondrian/olap/RoleImpl.java#25 $
 ...
 * @since Oct 5, 2002
 * @version $Id: //open/mondrian/src/main/mondrian/olap/RoleImpl.java#25 $
 */
public class RoleImpl implements Role {
  ...
  protected RoleImpl clone() {
    ...
    for (Map.Entry<Hierarchy, HierarchyAccessImpl> entry : hierarchyGrants
      .entrySet()) {
      role.hierarchyGrants.put(entry.getKey(), (HierarchyAccessImpl) entry
        .getValue().clone());
    }
    return role;
  }
   ...
   * @param access
   *          An {@link Access access code}
   * 
   * @pre schema != null
   * @pre access == Access.ALL || access == Access.NONE || access ==
   ...
   * 
   * @param cube
   *          Cube whose access to grant/deny.
   * @param access
   *          An {@link Access access code}
   * 
   * @pre cube != null
  ...
  public void grant(Cube cube, Access access) {
    ...
    final Access schemaAccess = getAccess(cube.getSchema());
   ...
   * 
   * @param dimension
   *          Dimension whose access to grant/deny.
   * @param access
   *          An Access instance
   * 
   * @pre dimension != null
   * @pre access == Access.ALL || access == Access.CUSTOM || access ==
   *      Access.NONE
   * @pre isMutable()
   */
  ...
  public Access getAccess(Dimension dimension) {
    ...
    if (access == Access.CUSTOM) {
      ...
      for (Hierarchy hierarchy : dimension.getHierarchies()) {
        final HierarchyAccessImpl hierarchyAccess = hierarchyGrants.get(hierarchy);
        if (hierarchyAccess != null && hierarchyAccess.access != Access.NONE) {
          canAccess = true;
        }
      }
      if (canAccess) {
  ...
  /**
   * This method is used to check if the access rights over a dimension that
   * might be inherited from the parent cube.
   * <p>
   * It only checks for inherited access, and it presumes that there are no
   * dimension grants currently given to the dimension passed as an argument.
   */
    ...
    for (Map.Entry<Cube, Access> cubeGrant : cubeGrants.entrySet()) {
      ...
      for (Dimension dimension1 : dimensions) {
        ...
        if (dimension instanceof RolapCubeDimension && dimension.equals(dimension1)
          && !((RolapCubeDimension) dimension1).getCube().equals(cubeGrant.getKey())) {
          continue;
        }
   ...
   * 
   * @param hierarchy
   *          Hierarchy whose access to grant/deny.
   * @param access
   *          An {@link Access access code}
   * @param topLevel
   *          Top-most level which can be accessed, or null if the highest
   *          level. May only be specified if <code>access</code> is
   *          {@link mondrian.olap.Access#CUSTOM}.
   * @param bottomLevel
   *          Bottom-most level which can be accessed, or null if the lowest
   *          level. May only be specified if <code>access</code> is
   *          {@link mondrian.olap.Access#CUSTOM}.
   * 
   * @param rollupPolicy
   ...
   * @pre (access == Access.CUSTOM) || (topLevel == null &amp;&amp; bottomLevel
   *      == null)
   * @pre topLevel == null || topLevel.getHierarchy() == hierarchy
   * @pre bottomLevel == null || bottomLevel.getHierarchy() == hierarchy
   * @pre isMutable()
   */
  public void grant(Hierarchy hierarchy, Access access, Level topLevel,
    Level bottomLevel, RollupPolicy rollupPolicy) {
    ...
    hierarchyGrants.put(hierarchy, new HierarchyAccessImpl(this, hierarchy, access,
      topLevel, bottomLevel, rollupPolicy));
    // Cascade the access right to 'custom' on the parent dim if necessary
    final Access dimAccess = toAccess(dimensionGrants.get(hierarchy.getDimension()));
  ...
  public HierarchyAccess getAccessDetails(Hierarchy hierarchy) {
    ...
    final Access schemaGrant = schemaGrants
      .get(hierarchy.getDimension().getSchema());
    if (schemaGrant != null) {
    ...
    return new HierarchyAccessImpl(this, hierarchy, hierarchyAccess, null, null,
      RollupPolicy.HIDDEN);
  }

  public Access getAccess(Level level) {
    assert level != null;
    HierarchyAccessImpl hierarchyAccess = hierarchyGrants.get(level.getHierarchy());
    if (hierarchyAccess != null && hierarchyAccess.access != Access.NONE) {
      if (checkLevelIsOkWithRestrictions(hierarchyAccess, level)) {
  ...
  private static boolean checkLevelIsOkWithRestrictions(
    HierarchyAccessImpl hierarchyAccess, Level level) {
   ...
   * Notes:
   * <ol>
   * <li>The order of grants matters. If you grant/deny access to a member,
   * previous grants/denials to its descendants are ignored.</li>
  ...
  public Access getAccess(Member member) {
    ...
    // rules for this.
    final HierarchyAccessImpl hierarchyAccess = hierarchyGrants.get(member
      .getHierarchy());
    if (hierarchyAccess != null) {
  ...
  public static HierarchyAccess createAllAccess(Hierarchy hierarchy) {
    return new HierarchyAccessImpl(Util.createRootRole(hierarchy.getDimension()
      .getSchema()), hierarchy, Access.ALL, null, null, Role.RollupPolicy.FULL);
  }
  ...
  private static class HierarchyAccessImpl implements Role.HierarchyAccess {
     ...
     * @param hierarchy
     *          A hierarchy this object describes.
     * @param access
     *          The access granted to this role for this hierarchy.
     * @param topLevel
     *          The top level to restrict the role to, or null to grant access
     *          up top the top level of the hierarchy parameter.
     * @param bottomLevel
     *          The bottom level to restrict the role to, or null to grant
     *          access down to the bottom level of the hierarchy parameter.
     * @param rollupPolicy
     *          The rollup policy to apply.
     */
    HierarchyAccessImpl(Role role, Hierarchy hierarchy, Access access,
      Level topLevel, Level bottomLevel, RollupPolicy rollupPolicy) {
      assert role != null;
      assert hierarchy != null;
      assert access != null;
      assert rollupPolicy != null;
      ...
      this.bottomLevel = bottomLevel == null ? hierarchy.getLevels()[hierarchy
        .getLevels().length - 1] : bottomLevel;
    }

    public HierarchyAccess clone() {
      HierarchyAccessImpl hierarchyAccess = new HierarchyAccessImpl(role, hierarchy,
        access, topLevel, bottomLevel, rollupPolicy);
    ...
    void grant(RoleImpl role, Member member, Access access) {
      ...
      for (Iterator<Member> memberIter = memberGrants.keySet().iterator(); memberIter
        .hasNext();) {
      ...
      if (access == Access.NONE) {
        ...
        for (Member m = member.getParentMember(); m != null; m = m.getParentMember()) {
          ...
          if (memberAccess == Access.NONE
            && checkLevelIsOkWithRestrictions(this, m.getLevel())) {
        ...
        // child visible.
        for (Member m = member.getParentMember(); m != null; m = m.getParentMember()) {
          if (checkLevelIsOkWithRestrictions(this, m.getLevel())) {
            final Access parentAccess = toAccess(memberGrants.get(m));
        ...
        final Access hierarchyAccess = role.getAccess(member.getLevel()
          .getHierarchy());
        if (hierarchyAccess == Access.NONE) {
          // Upgrade to CUSTOM level.
          role.grant(hierarchy, Access.CUSTOM, topLevel, bottomLevel, rollupPolicy);
        }
      }
    }
    ...
    public Access getAccess(Member member) {
      ...
      if (memberGrants.size() == 0
        && checkLevelIsOkWithRestrictions(this, member.getLevel())) {
        return Access.ALL;
      }
      // No access
      return Access.NONE;
  ...
  /**
   * Implementation of {@link mondrian.olap.Role.HierarchyAccess} that delegates
   * all methods to an underlying hierarchy access.
   */
  public static abstract class DelegatingHierarchyAccess implements HierarchyAccess {
   ...
   * 
   * <p>
   * This reduces the number of calls to the underlying HierarchyAccess, which
   * is particularly useful for a union role which is based on many roles.
   * 
   * <p>
   * Caching uses two {@link java.util.WeakHashMap} objects, so should release
   * resources if memory is scarce. However, it may use up memory and cause
   * segments etc. to be removed from the cache when GC is triggered. For this
   * reason, you should only use this wrapper for a HierarchyAccess which would
   * otherwise have poor performance; currently used for union roles with 5 or
   * more member roles.
   */
