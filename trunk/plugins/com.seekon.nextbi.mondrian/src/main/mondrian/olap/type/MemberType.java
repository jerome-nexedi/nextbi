/*
// $Id: //open/mondrian/src/main/mondrian/olap/type/MemberType.java#17 $
 ...
 * @author jhyde
 * @since Feb 17, 2005
 * @version $Id: //open/mondrian/src/main/mondrian/olap/type/MemberType.java#17
 *          $
 */
public class MemberType implements Type {
	 ...
	 * 
	 * @param dimension
	 *          Dimension the member belongs to, or null if not known
	 * @param hierarchy
	 *          Hierarchy the member belongs to, or null if not known
	 * @param level
	 *          Level the member belongs to, or null if not known
	 * @param member
	 *          The precise member, or null if not known
	 */
	public MemberType(Dimension dimension, Hierarchy hierarchy, Level level,
			Member member) {
	...
	public boolean usesHierarchy(Hierarchy hierarchy, boolean definitely) {
		return this.hierarchy == hierarchy
				|| (!definitely && this.hierarchy == null && (this.dimension == null || this.dimension == hierarchy
						.getDimension()));
	}
	...
	public static MemberType forType(Type type) {
		if (type instanceof MemberType) {
			...
			return new MemberType(type.getDimension(), type.getHierarchy(),
					type.getLevel(), null);
		}
	}
	...
	public Type computeCommonType(Type type, int[] conversionCount) {
		...
		if (this.getLevel() != null && this.getLevel().equals(that.getLevel())) {
			return new MemberType(this.getDimension(), this.getHierarchy(),
					this.getLevel(), null);
		}
		if (this.getHierarchy() != null
				&& this.getHierarchy().equals(that.getHierarchy())) {
			return new MemberType(this.getDimension(), this.getHierarchy(), null,
					null);
		}
	...
	public boolean isInstance(Object value) {
		return value instanceof Member
				&& (level == null || ((Member) value).getLevel().equals(level))
				&& (hierarchy == null || ((Member) value).getHierarchy().equals(
						hierarchy))
				&& (dimension == null || ((Member) value).getDimension().equals(
						dimension));
	}
