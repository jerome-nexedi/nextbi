/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/MultiResolver.java#24 $
 ...
 * function. If one of these overloadings matches the actual arguments, it calls
 * the factory method {@link #createFunDef}.
 * 
 * @author jhyde
 * @since Feb 12, 2003
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/MultiResolver.java#24 $
 */
public abstract class MultiResolver extends FunUtil implements Resolver {
   ...
   * 
   * @param name
   *          Name of function or operator
   * @param signature
   *          Signature of function or operator
   * @param description
   *          Description of function or operator
   * @param signatures
   *          Array of possible signatures, each of which is an encoding of the
   *          syntactic type, return type, and parameter types of this operator.
   *          The "Members" operator has a syntactic type "pxd" which means "an
   *          operator with {@link Syntax#Property property} syntax (p) which
   *          returns a set (x) and takes a dimension (d) as its argument". See
   ...
   */
  protected MultiResolver(String name, String signature, String description,
    String[] signatures) {
    this.name = name;
  ...
  public FunDef resolve(Exp[] args, Validator validator, List<Conversion> conversions) {
    outer: for (String signature : signatures) {
  ...
  public boolean requiresExpression(int k) {
    for (String signature : signatures) {
      int[] parameterTypes = decodeParameterCategories(signature);
      if ((k < parameterTypes.length) && parameterTypes[k] == Category.Set) {
        return false;
      }
    }
    return true;
