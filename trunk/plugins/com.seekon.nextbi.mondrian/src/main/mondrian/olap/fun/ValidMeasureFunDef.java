/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/ValidMeasureFunDef.java#16 $
 ...
 * 
 * <p>
 * Returns a valid measure in a virtual cube by forcing inapplicable dimensions
 * to their top level.
 * 
 * <p>
 * Syntax: <blockquote><code>
 * ValidMeasure(&lt;Tuple&gt;)
 * </code></blockquote>
 * 
 * @author kwalker, mpflug
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/ValidMeasureFunDef.java
 *          #16 $
 */
	...
	private static class CalcImpl extends GenericCalc {
		...
		public Object evaluate(Evaluator evaluator) {
			...
			if (calc.isWrapperFor(MemberCalc.class)) {
				...
				memberList.add(calc.unwrap(MemberCalc.class).evaluateMember(evaluator));
			} else {
				final Member[] tupleMembers = calc.unwrap((TupleCalc.class))
						.evaluateTuple(evaluator);
			...
			baseCube = getBaseCubeofMeasure(evaluator,
					memberList.get(measurePosition), baseCube);
			List<Dimension> vMinusBDimensions = getDimensionsToForceToAllLevel(
					virtualCube, baseCube, memberList);
			// declare members array and fill in with all needed members
			final List<Member> validMeasureMembers = new ArrayList<Member>(memberList);
		...
		private RolapCube getBaseCubeofMeasure(Evaluator evaluator, Member member,
				RolapCube baseCube) {
		...
		private List<Dimension> getDimensionsToForceToAllLevel(
				RolapCube virtualCube, RolapCube baseCube, List<Member> memberList) {
			...
			Set<Dimension> nonJoiningDims = baseCube
					.nonJoiningDimensions(virtualCubeDims);
