/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/ExtractFunDef.java#10 $
 ...
 * 
 * <p>
 * Syntax: <blockquote><code>Extract(&lt;Set&gt;, &lt;Hierarchy&gt;[,
 * &lt;Hierarchy&gt;...])</code></blockquote>
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/ExtractFunDef.java#10 $
 * @since Jun 10, 2007
 */
class ExtractFunDef extends FunDefBase {
	static final ResolverBase Resolver = new ResolverBase(
		...
		public FunDef resolve(Exp[] args, Validator validator,
			...
			for (int i = 1; i < args.length; ++i) {
				if (!validator.canConvert(0, args[i], Category.Hierarchy, conversions)) {
					return null;
				}
			}
			...
			final List<Hierarchy> extractedHierarchies = new ArrayList<Hierarchy>();
			...
			Arrays.fill(parameterTypes, 1, parameterTypes.length, Category.Hierarchy);
	...
	public Type getResultType(Validator validator, Exp[] args) {
		...
		if (extractedHierarchies.size() == 1) {
			...
			return new SetType(new TupleType(typeList.toArray(new Type[typeList
					.size()])));
		}
	}

	private static void findExtractedHierarchies(Exp[] args,
			List<Hierarchy> extractedHierarchies, List<Integer> extractedOrdinals) {
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		findExtractedHierarchies(call.getArgs(), extractedHierarchyList,
				extractedOrdinalList);
		Util.assertTrue(extractedOrdinalList.size() == extractedHierarchyList
				.size());
