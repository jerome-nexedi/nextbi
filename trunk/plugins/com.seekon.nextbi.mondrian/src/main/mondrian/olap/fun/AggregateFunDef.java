...
public class AggregateFunDef extends AbstractAggregateFunDef {
	...
	private static final String TIMING_NAME = AggregateFunDef.class
			.getSimpleName();
	...
	public static class AggregateCalc extends GenericCalc {
		...
		public static TupleList optimizeTupleList(Evaluator evaluator,
			...
			tupleList = optimizeChildren(tupleList, evaluator.getSchemaReader(),
		...
		public static TupleList optimizeChildren(TupleList tuples,
			...
			for (int i = 0; i < tupleLength; i++) {
				if (areOccurencesEqual(membersOccurencesInTuple[i].values())) {
					...
					sets[i] = optimizeMemberSet(new LinkedHashSet<Member>(members),
		...
		private static int getChildCount(Member parentMember, SchemaReader reader) {
			int childrenCountFromCache = reader
					.getChildrenCountFromCache(parentMember);
