/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/ParallelPeriodFunDef.java#11 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/ParallelPeriodFunDef.
 *          java#11 $
 * @since Mar 23, 2006
 */
class ParallelPeriodFunDef extends FunDefBase {
	static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
			...
			new String[] { "fm", "fml", "fmln", "fmlnm" }, ParallelPeriodFunDef.class);
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		final IntegerCalc lagValueCalc = (args.length >= 2) ? compiler
				.compileInteger(args[1]) : ConstantCalc.constantInteger(1);

		// If level is not specified, we compute it from
		// member at runtime.
		final LevelCalc ancestorLevelCalc = args.length >= 1 ? compiler
				.compileLevel(args[0]) : null;
		...
		case 1:
			...
			if (hierarchy != null) {
				...
				memberCalc = new HierarchyCurrentMemberFunDef.FixedCalcImpl(call,
						hierarchy);
			} else {
				memberCalc = null;
			}
			break;
		default:
			final RolapHierarchy timeHierarchy = ((RolapCube) compiler.getEvaluator()
					.getCube()).getTimeHierarchy(getName());
			memberCalc = new HierarchyCurrentMemberFunDef.FixedCalcImpl(call,
					timeHierarchy);
			break;
		...
		return new AbstractMemberCalc(call, new Calc[] { memberCalc, lagValueCalc,
				ancestorLevelCalc }) {
	...
	Member parallelPeriod(Member member, Level ancestorLevel,
			Evaluator evaluator, int lagValue) {
		...
		Member ancestor = FunUtil.ancestor(evaluator, member, distance,
				ancestorLevel);
		Member inLaw = evaluator.getSchemaReader().getLeadMember(ancestor,
				-lagValue);
