...
class TopBottomPercentSumFunDef extends FunDefBase {
	...
	public TopBottomPercentSumFunDef(FunDef dummyFunDef, boolean top,
			boolean percent) {
		super(dummyFunDef);
		this.top = top;
	...
	private class CalcImpl extends AbstractListCalc {
		...
		public CalcImpl(ResolvedFunCall call, ListCalc listCalc,
				DoubleCalc doubleCalc, Calc calc) {
		...
		public TupleList evaluateList(Evaluator evaluator) {
			...
			Map<List<Member>, Object> mapMemberToValue = evaluateTuples(evaluator,
					calc, list);
			final int savepoint = evaluator.savepoint();
			evaluator.setNonEmpty(false);
			list = sortTuples(evaluator, list, list, calc, top, true, getType()
					.getArity());
			...
			if (memberCount > 0 && percent && nullCount == memberCount) {
				return top ? list.subList(0, 1) : list.subList(memberCount - 1,
						memberCount);
			}
			return list;
		}
