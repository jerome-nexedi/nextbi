/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/FunTableImpl.java#11 $
 ...
 * {@link #defineFunctions(mondrian.olap.FunTable.Builder)} to define each
 * function which will be recognized by this table. This method is called from
 * the constructor, after which point, no further functions can be added.
 */
public abstract class FunTableImpl implements FunTable {
  /**
   * Maps the upper-case name of a function plus its
   * {@link mondrian.olap.Syntax} to an array of {@link Resolver} objects for
   * that name.
   */
  ...
  public final void init() {
    ...
    this.mapNameToResolvers = Collections
      .unmodifiableMap(builder.mapNameToResolvers);
    this.reservedWordSet = builder.reservedWords;
    final String[] reservedWords = builder.reservedWords
      .toArray(new String[builder.reservedWords.size()]);
    Arrays.sort(reservedWords);
    this.reservedWordList = Collections.unmodifiableList(Arrays
      .asList(reservedWords));
    this.propertyWords = Collections.unmodifiableSet(builder.propertyWords);
  }

  /**
   * Creates a key to look up an operator in the resolver map. The key consists
   * of the uppercase function name and the syntax.
   * 
   * @param name
  ...
  /**
   * Implementation of {@link mondrian.olap.FunTable.Builder}. Functions are
   * added to lists each time {@link #define(Resolver)} is called, then
   * {@link #organizeFunctions()} sorts and indexes the map.
   */
  private class BuilderImpl implements Builder {
    ...
    protected void organizeFunctions() {
      ...
      final List<List<Resolver>> nonSingletonResolverLists = new ArrayList<List<Resolver>>();
      for (Resolver resolver : resolverList) {
        Pair<String, Syntax> key = makeResolverKey(resolver.getName(), resolver
          .getSyntax());
      ...
      final Comparator<Resolver> comparator = new Comparator<Resolver>() {
