/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/SetFunDef.java#41 $
 ...
 * @author jhyde
 * @since 3 March, 2002
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/SetFunDef.java#41 $
 */
public class SetFunDef extends FunDefBase {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (i == 0) {
					...
					if (!TypeUtil.isUnionCompatible(type0, type)) {
						throw MondrianResource.instance().ArgsMustHaveSameHierarchy
								.ex(getName());
					}
				}
			}
	 ...
	 * 
	 * <p>
	 * The set function can contain expressions which yield sets together with
	 * expressions which yield individual tuples, provided that they all have the
	 * same type. It automatically removes null or partially-null tuples from the
	 * list.
	 * 
	 * <p>
	...
	public static class SetListCalc extends AbstractListCalc {
		...
		public SetListCalc(Exp exp, Exp[] args, ExpCompiler compiler,
			...
			if (type instanceof SetType) {
				...
				return new AbstractVoidCalc(arg, new Calc[] { listCalc }) {
					public void evaluateVoid(Evaluator evaluator) {
						...
						outer: for (List<Member> members : list) {
				...
				return new AbstractVoidCalc(arg, new Calc[] { tupleCalc }) {
					public void evaluateVoid(Evaluator evaluator) {
						...
						if (members == null || tupleContainsNullMember(members)) {
							return;
						}
						result.addTuple(members);
					}
	...
	private static List<Calc> compileSelf(Exp[] args, ExpCompiler compiler,
			List<ResultStyle> resultStyles) {
	...
	private static IterCalc createCalc(Exp arg, ExpCompiler compiler,
			List<ResultStyle> resultStyles) {
		final Type type = arg.getType();
		if (type instanceof SetType) {
			...
			return new AbstractIterCalc(call, new Calc[] { tupleCalc }) {
				public TupleIterable evaluateIterable(Evaluator evaluator) {
					...
					return new ListTupleList(tupleCalc.getType().getArity(),
	...
	public static ResolvedFunCall wrapAsSet(Exp... args) {
		...
		return new ResolvedFunCall(new SetFunDef(Resolver, categories), args,
				new SetType(type));
	}
	...
	public static class ExprIterCalc extends AbstractIterCalc {
		...
		public ExprIterCalc(Exp exp, Exp[] args, ExpCompiler compiler,
				List<ResultStyle> resultStyles) {
			super(exp, null);
			...
			return new AbstractTupleIterable(getType().getArity()) {
				public TupleCursor tupleCursor() {
					return new AbstractTupleCursor(arity) {
						Iterator<IterCalc> calcIterator = Arrays.asList(iterCalcs)
								.iterator();
						TupleCursor currentCursor = TupleCollections.emptyList(1)
								.tupleCursor();
						...
						public boolean forward() {
							while (true) {
								...
								currentCursor = calcIterator.next().evaluateIterable(evaluator)
										.tupleCursor();
							}
						}
	...
	private static class ResolverImpl extends ResolverBase {
		public ResolverImpl() {
			super("{}", "{<Member> [, <Member>...]}",
					"Brace operator constructs a set.", Syntax.Braces);
		}
		...
		public FunDef resolve(Exp[] args, Validator validator,
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (validator.canConvert(i, args[i], Category.Tuple, conversions)) {
