/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/FilterFunDef.java#26 $
 ...
 * 
 * <p>
 * Syntax: <blockquote><code>Filter(&lt;Set&gt;, &lt;Search
 * Condition&gt;)</code></blockquote>
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/FilterFunDef.java#26
 *          $
 * @since Mar 23, 2006
 */
class FilterFunDef extends FunDefBase {
   ...
   * 
   * <p>
   * Here we would like to get either a IterCalc or ListCalc (mutable) from the
   * inner expression. For the IterCalc, its Iterator can be wrapped with
   * another Iterator that filters each element. For the mutable list, remove
   * all members that are filtered.
   * 
   * @param call
   *          Call
   * @param compiler
  ...
  protected IterCalc compileCallIterable(final ResolvedFunCall call,
    ...
    Calc imlcalc = compiler.compileAs(call.getArg(0), null,
  ...
  private static class IterIterCalc extends BaseIterCalc {
   ...
   */
  protected ListCalc compileCallList(final ResolvedFunCall call, ExpCompiler compiler) {
    ...
    throw ResultStyleException.generateBadType(ResultStyle.MUTABLELIST_LIST, ilcalc
      .getResultStyle());
  }
