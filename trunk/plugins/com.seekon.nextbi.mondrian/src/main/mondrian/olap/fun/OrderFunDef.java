/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/OrderFunDef.java#34 $
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/OrderFunDef.java#34
 *          $
 * @since Mar 23, 2006
 */
class OrderFunDef extends FunDefBase {
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		if (keySpecCount == 1) {
			if (expCalc.isWrapperFor(MemberValueCalc.class)
				...
				final MemberCalc[] calcs = (MemberCalc[]) ((AbstractCalc) expCalc)
						.getCalcs();
				for (MemberCalc memberCalc : calcs) {
					if (memberCalc.isWrapperFor(ConstantCalc.class)
							&& !listCalc.dependsOn(memberCalc.getType().getHierarchy())) {
				...
				if (constantList.isEmpty()) {
					...
					return new ContextCalc(calcs, new CalcImpl(call, calcList,
							keySpecList));
				} else {
					// Some members are constant. Evaluate these before
					// evaluating the list expression.
					calcList[1] = MemberValueCalc.create(new DummyExp(expCalc.getType()),
							variableList.toArray(new MemberCalc[variableList.size()]),
							compiler.getEvaluator().mightReturnNullForUnrelatedDimension());
					return new ContextCalc(
							constantList.toArray(new MemberCalc[constantList.size()]),
							new CalcImpl(call, calcList, keySpecList));
				}
			}
		}
	...
	private void buildKeySpecList(List<SortKeySpec> keySpecList,
	...
	private interface CalcWithDual extends Calc {
		public TupleList evaluateDual(Evaluator rootEvaluator,
				Evaluator subEvaluator);
	}

	private static class CalcImpl extends AbstractListCalc implements
			CalcWithDual {
		...
		public CalcImpl(ResolvedFunCall call, Calc[] calcList,
		...
		public TupleList evaluateDual(Evaluator rootEvaluator,
			...
			final TupleList list = iterable instanceof TupleList && false ? (TupleList) iterable
					: null;
			...
			if (arity == 1) {
				tupleList = new UnaryTupleList(sortMembers(subEvaluator,
						iterable.slice(0), list == null ? null : list.slice(0),
						sortKeyCalc, sortKeyDir.descending, sortKeyDir.brk));
			} else {
				tupleList = sortTuples(subEvaluator, iterable, list, sortKeyCalc,
						sortKeyDir.descending, sortKeyDir.brk, arity);
			}
		...
		public TupleList evaluateList(Evaluator evaluator) {
			...
			if (originalKeySpecCount == 1) {
				...
				if (arity == 1) {
					tupleList = new UnaryTupleList(sortMembers(evaluator,
							iterable.slice(0), list == null ? null : list.slice(0),
							sortKeyCalc, sortKeyDir.descending, sortKeyDir.brk));
				} else {
					tupleList = sortTuples(evaluator, iterable, list, sortKeyCalc,
							sortKeyDir.descending, sortKeyDir.brk, arity);
				}
				...
				if (arity == 1) {
					...
					tupleList = sortTuples(evaluator, iterable, list, keySpecList, arity);
				}
				evaluator.restore(savepoint);
				return tupleList;
			}
		...
		public void collectArguments(Map<String, Object> arguments) {
			...
			arguments.put("direction",
					(sortKeyDir.descending ? (sortKeyDir.brk ? Flag.BDESC : Flag.DESC)
		...
		private void purgeKeySpecList(List<SortKeySpec> keySpecList, TupleList list) {
			...
			List<Hierarchy> listHierarchies = new ArrayList<Hierarchy>(
					list.getArity());
			...
			while (iter.hasNext()) {
				...
				if (expCalc instanceof MemberOrderKeyFunDef.CalcImpl) {
					...
					if (memberCalc instanceof ConstantCalc
							|| !listHierarchies.contains(memberCalc.getType().getHierarchy())) {
						iter.remove();
					}
				}
			}
