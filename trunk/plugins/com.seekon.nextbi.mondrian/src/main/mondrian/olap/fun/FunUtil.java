/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/FunUtil.java#147 $
...
/**
 * {@code FunUtil} contains a set of methods useful within the
 * {@code mondrian.olap.fun} package.
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/olap/fun/FunUtil.java#147 $
 * @since 1.0
 */
public class FunUtil extends Util {
	...
	/**
	 * Special value which indicates that a {@code double} computation has
	 * returned the MDX null value. See {@link DoubleCalc}.
	 */
	public static final double DoubleNull = 0.000000012345;

	/**
	 * Special value which indicates that a {@code double} computation has
	 * returned the MDX EMPTY value. See {@link DoubleCalc}.
	 */
	public static final double DoubleEmpty = -0.000000012345;

	/**
	 * Special value which indicates that an {@code int} computation has returned
	 * the MDX null value. See {@link mondrian.calc.IntegerCalc}.
	 */
	public static final int IntegerNull = Integer.MIN_VALUE + 1;

	/**
	 * Null value in three-valued boolean logic. Actually, a placeholder until we
	 * actually implement 3VL.
	 */
	...
	public static RuntimeException newEvalException(Throwable throwable) {
		return new MondrianEvaluationException(throwable.getClass().getName()
	...
	public static void checkIterListResultStyles(Calc calc) {
		...
		default:
			throw ResultStyleException.generateBadType(
					ResultStyle.ITERABLE_LIST_MUTABLELIST, calc.getResultStyle());
		}
	}
	...
	public static void checkListResultStyles(Calc calc) {
		...
		case MUTABLE_LIST:
			break;
		default:
			throw ResultStyleException.generateBadType(ResultStyle.LIST_MUTABLELIST,
					calc.getResultStyle());
		}
	}
	 ...
	 */
	static String getLiteralArg(ResolvedFunCall call, int i, String defaultValue,
		...
		if (!(arg instanceof Literal) || arg.getCategory() != Category.Symbol) {
			throw newEvalException(call.getFunDef(), "Expected a symbol, found '"
					+ arg + "'");
		}
		...
		throw newEvalException(call.getFunDef(), "Allowed values are: {" + sb + "}");
	}

	/**
	 * Returns the ordinal of a literal argument. If the argument does not belong
	 * to the supplied enumeration, returns -1.
	 */
	static <E extends Enum<E>> E getLiteralArg(ResolvedFunCall call, int i,
			E defaultValue, Class<E> allowedValues) {
		if (i >= call.getArgCount()) {
			if (defaultValue == null) {
				throw newEvalException(call.getFunDef(), "Required argument is missing");
			} else {
	 ...
	 * @param left
	 * @param right
	 * @throws MondrianEvaluationException
	 *           if expressions don't have the same hierarchy
	 */
	...
	/**
	 * Adds every element of {@code right} which is not in {@code set} to both
	 * {@code set} and {@code left}.
	 */
	...
	static List<Member> addMembers(final SchemaReader schemaReader,
			final List<Member> members, final Hierarchy hierarchy) {
	...
	static List<Member> addMembers(SchemaReader schemaReader,
			List<Member> members, Level level) {
	...
	/**
	 * Removes every member from a list which is calculated. The list must not be
	 * null, and must consist only of members.
	 * 
	 * @param memberList
	 *          Member list
	...
	/**
	 * Removes every tuple from a list which is calculated. The list must not be
	 * null, and must consist only of members.
	 * 
	 * @param memberList
	 *          Member list
	...
	/**
	 * For each member in a list, evaluates an expression and creates a map from
	 * members to values.
	 * 
	 * <p>
	 ...
	 * @param memberIter
	 *          Iterable over the collection of members
	 * @param memberList
	 *          List to be populated with members, or null
	 * @param parentsToo
	 *          If true, evaluate the expression for all ancestors of the members
	 *          as well
	 * 
	 * @pre exp != null
	 * @pre exp.getType() instanceof ScalarType
	 */
	static Map<Member, Object> evaluateMembers(Evaluator evaluator, Calc exp,
			Iterable<Member> memberIter, List<Member> memberList, boolean parentsToo) {
	...
	/**
	 * For each tuple in a list, evaluates an expression and creates a map from
	 * tuples to values.
	 * 
	 * @param evaluator
	 *          Evaluation context
	 ...
	 * 
	 * <p>
	 * If you do not specify {@code memberList}, the method will build its own
	 * member list as it iterates over {@code memberIter}. It is acceptable if
	 * {@code memberList} and {@code memberIter} are the same list object.
	 * 
	 * <p>
	...
	static List<Member> sortMembers(Evaluator evaluator,
			Iterable<Member> memberIter, List<Member> memberList, Calc exp,
		...
		try {
			...
			if (memberList == null) {
				...
				mapMemberToValue = evaluateMembers(evaluator, exp, memberIter,
						memberList, parentsToo);
			} else {
				mapMemberToValue = evaluateMembers(evaluator, exp, memberIter, null,
						parentsToo);
			}
	...
	/**
	 * Sorts a list of members according to a list of SortKeySpecs. An in-place,
	 * Stable sort. Helper function for MDX OrderSet function.
	 * 
	 * <p>
	 * NOTE: Does not preserve the contents of the validator.
	 */
	static List<Member> sortMembers(Evaluator evaluator,
			Iterable<Member> memberIter, List<Member> memberList,
		...
		for (SortKeySpec key : keySpecList) {
			...
			MemberComparator comp;
			if (brk) {
				comp = new BreakMemberComparator(evaluator, key.key,
						key.direction.descending);
			} else {
				comp = new HierarchicalMemberComparator(evaluator, key.key,
						key.direction.descending);
			}
	 ...
	 * @param tupleList
	 *          List of tuples, if known, otherwise null
	 * @param exp
	 *          Expression to sort on
	 * @param desc
	 *          Whether to sort descending
	...
	public static TupleList sortTuples(Evaluator evaluator,
			TupleIterable tupleIterable, TupleList tupleList, Calc exp, boolean desc,
			boolean brk, int arity) {
		...
		List<Member>[] tuples = tupleArrayList.toArray(new List[tupleArrayList
				.size()]);
		final DelegatingTupleList result = new DelegatingTupleList(
				tupleIterable.getArity(), Arrays.asList(tuples));

		Comparator<List<Member>> comparator;
		if (brk) {
			BreakTupleComparator c = new BreakTupleComparator(evaluator, exp, arity);
	 ...
	 *          a list of members
	 * @param exp
	 *          a Calc applied to each member to find its sort-key
	 * @param evaluator
	 *          Evaluator
	 * @param limit
	 *          maximum count of members to return.
	 * @param desc
	...
	public static List<Member> partiallySortMembers(Evaluator evaluator,
			List<Member> list, Calc exp, int limit, boolean desc) {
		evaluator.getTiming().markStart(SORT_EVAL_TIMING_NAME);
		boolean timingEval = true;
		boolean timingSort = false;
		try {
			MemberComparator comp = new BreakMemberComparator(evaluator, exp, desc);
			Map<Member, Object> valueMap = evaluateMembers(evaluator, exp, list,
					null, false);
			evaluator.getTiming().markEnd(SORT_EVAL_TIMING_NAME);
			timingEval = false;
			evaluator.getTiming().markStart(SORT_TIMING_NAME);
	...
	/**
	 * Helper function to sort a list of tuples according to a list of expressions
	 * and a list of sorting flags.
	 * 
	 * <p>
	 * NOTE: This function does not preserve the contents of the validator.
	 */
	static TupleList sortTuples(Evaluator evaluator, TupleIterable tupleIter,
			TupleList tupleList, List<SortKeySpec> keySpecList, int arity) {
		if (tupleList == null) {
		...
		for (SortKeySpec key : keySpecList) {
			...
			if (brk) {
				TupleExpMemoComparator comp = new BreakTupleComparator(evaluator,
						key.key, arity);
				comp.preloadValues(tupleList);
				chain.addComparator(comp.wrap(), key.direction.descending);
			} else if (orderByKey) {
				TupleExpMemoComparator comp = new HierarchicalTupleKeyComparator(
						evaluator, key.key, arity);
				comp.preloadValues(tupleList);
				chain.addComparator(comp.wrap(), key.direction.descending);
			} else {
				TupleExpComparator comp = new HierarchicalTupleComparator(evaluator,
						key.key, arity, key.direction.descending);
				chain.addComparator(comp.wrap(), false);
			}
		}
	 ...
	 * 
	 * <p>
	 * NOTE: Does not preserve the contents of the validator. The returned list is
	 * immutable.
	 * 
	 * @param evaluator
	 *          Evaluator
	 ...
	 *          a list of tuples
	 * @param exp
	 *          a Calc applied to each tple to find its sort-key
	 * @param limit
	 *          maximum count of tuples to return.
	 * @param desc
	 *          true to sort descending (and find TopCount), false to sort
	 *          ascending (and find BottomCount).
	...
	public static List<List<Member>> partiallySortTuples(Evaluator evaluator,
			TupleList list, Calc exp, int limit, boolean desc) {
		Comparator<List<Member>> comp = new BreakTupleComparator(evaluator, exp,
				list.getArity()).wrap();
		if (desc) {
	...
	/**
	 * Sorts a list of members into hierarchical order. The members must belong to
	 * the same dimension.
	 * 
	 * @param memberList
	 *          List of members
	...
	public static TupleList hierarchizeTupleList(TupleList tupleList, boolean post) {
		...
		Comparator<List<Member>> comparator = new HierarchizeTupleComparator(
				fixedList.getArity(), post).wrap();
	 ...
	 * 
	 * <p>
	 * MDX requires a total order: <blockquote> -inf &lt; NULL &lt; ... &lt; -1
	 * &lt; ... &lt; 0 &lt; ... &lt; NaN &lt; +inf </blockquote> but this is
	 * different than Java semantics, specifically with regard to
	 * {@link Double#NaN}.
	 */
	 ...
	 * 
	 * <p>
	 * Nulls compare last, exceptions (including the object which indicates the
	 * the cell is not in the cache yet) next, then numbers and strings are
	 * compared by value.
	 * 
	 * @param value0
	 *          First cell value
	 * @param value1
	 *          Second cell value
	 * @return -1, 0, or 1, depending upon whether first cell value is less than,
	...
	public static int compareValues(Object value0, Object value1) {
		...
		if (value0 instanceof RuntimeException
			...
			return FunUtil.compareValues(((Number) value0).doubleValue(),
	...
	/**
	 * Turns the mapped values into relative values (percentages) for easy use by
	 * the general topOrBottom function. This might also be a useful function in
	 * itself.
	 */
	 ...
	 * 
	 * @param flags
	 *          A encoded string which represents an operator signature, as used
	 *          by the {@code flags} parameter used to construct a
	 *          {@link FunDefBase}.
	 * 
	 ...
	 * 
	 * <p>
	 * For example, <code>decodeReturnType("fnx")</code> returns
	 * <code>{@link Category#Numeric}</code>, indicating this function has a
	 * numeric return value.
	 * 
	 * @param flags
	 *          The signature of an operator, as used by the {@code flags}
	 *          parameter used to construct a {@link FunDefBase}.
	 * 
	...
	/**
	 * Decodes the {@code offset}th character of an encoded method signature into
	 * a type category.
	 * 
	 * <p>
	 * The codes are:
	 ...
	 * <tr>
	 * <td>a</td>
	 * <td>{@link Category#Array}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>d</td>
	 * <td>{@link Category#Dimension}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>h</td>
	 * <td>{@link Category#Hierarchy}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>l</td>
	 * <td>{@link Category#Level}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>b</td>
	 * <td>{@link Category#Logical}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>m</td>
	 * <td>{@link Category#Member}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>N</td>
	 * <td>Constant {@link Category#Numeric}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>n</td>
	 * <td>{@link Category#Numeric}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>x</td>
	 * <td>{@link Category#Set}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>#</td>
	 * <td>Constant {@link Category#String}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>S</td>
	 * <td>{@link Category#String}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>t</td>
	 * <td>{@link Category#Tuple}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>v</td>
	 * <td>{@link Category#Value}</td>
	 * </tr>
	 * 
	 * <tr>
	 * <td>y</td>
	 * <td>{@link Category#Symbol}</td>
	 * </tr>
	 * 
	 * </table>
	 * 
	 ...
	 * 
	 * <p>
	 * Each character is decoded using {@link #decodeCategory(String, int)}. For
	 * example, <code>decodeParameterTypes("nx")</code> returns
	...
	static double percentile(Evaluator evaluator, TupleList members, Calc exp,
			double p) {
		...
		/*
		 * The median is defined as the value that has exactly the same number of
		 * entries before it in the sorted list as after. So, if the number of
		 * entries in the list is odd, the median is the entry at (length-1)/2
		 * (using zero-based indexes). If the number of entries is even, the median
		 * is defined as the arithmetic mean of the two numbers in the middle of the
		 * list, or (entries[length/2 - 1] + entries[length/2]) / 2.
		 */
	 ...
	 * @pre range >= 1 && range <= 3
	 */
	protected static double quartile(Evaluator evaluator, TupleList members,
			Calc exp, int range) {
		...
		return dm == median && median < asArray.length - 1 ? (asArray[median] + asArray[median + 1]) / 2
				: asArray[median];
	}
	...
	static Object var(Evaluator evaluator, TupleList members, Calc exp,
			boolean biased) {
		SetWrapper sw = evaluateSet(evaluator, members, exp);
	...
	static double correlation(Evaluator evaluator, TupleList memberList,
			Calc exp1, Calc exp2) {
		...
		if ((covar instanceof Double) && (var1 instanceof Double)
	...
	static Object covariance(Evaluator evaluator, TupleList members, Calc exp1,
			Calc exp2, boolean biased) {
		final int savepoint = evaluator.savepoint();
	...
	private static Object _covariance(SetWrapper sw1, SetWrapper sw2,
			boolean biased) {
	...
	static Object stdev(Evaluator evaluator, TupleList members, Calc exp,
			boolean biased) {
		Object o = var(evaluator, members, exp, biased);
		return (o instanceof Double) ? new Double(Math.sqrt(((Double) o)
				.doubleValue())) : o;
	}

	public static Object avg(Evaluator evaluator, TupleList members, Calc calc) {
		SetWrapper sw = evaluateSet(evaluator, members, calc);
		return (sw.errorCount > 0) ? new Double(Double.NaN)
				: (sw.v.size() == 0) ? Util.nullValue : new Double(_avg(sw));
	}
	...
	public static double sumDouble(Evaluator evaluator, TupleList members,
			Calc exp) {
		SetWrapper sw = evaluateSet(evaluator, members, exp);
		if (sw.errorCount > 0) {
			return Double.NaN;
	...
	public static double sumDouble(Evaluator evaluator, TupleIterable iterable,
			Calc exp) {
	...
	public static int count(Evaluator evaluator, TupleIterable iterable,
	...
	/**
	 * Evaluates {@code exp} (if defined) over {@code members} to generate a
	 * {@link List} of {@link SetWrapper} objects, which contains a {@link Double}
	 * value and meta information, unlike {@link #evaluateMembers}, which only
	 * produces values.
	 * 
	 * @pre exp != null
	 */
	static SetWrapper evaluateSet(Evaluator evaluator, TupleIterable members,
			Calc calc) {
		assert members != null;
		assert calc != null;
	...
	/**
	 * Evaluates one or more expressions against the member list returning a
	 * SetWrapper array. Where this differs very significantly from the above
	 * evaluateSet methods is how it count null values and Throwables; this method
	 * adds nulls to the SetWrapper Vector rather than not adding anything - as
	 * the above method does. The impact of this is that if, for example, one was
	 * creating a list of x,y values then each list will have the same number of
	 * values (though some might be null) - this allows higher level code to
	 * determine how to handle the lack of data rather than having a non-equal
	 * number (if one is plotting x,y values it helps to have the same number and
	 * know where a potential gap is the data is.
	 */
	static SetWrapper[] evaluateSet(Evaluator evaluator, TupleList list,
			DoubleCalc[] calcs) {
	...
	static List<Member> periodsToDate(Evaluator evaluator, Level level,
			Member member) {
		if (member == null) {
	...
	static List<Member> memberRange(Evaluator evaluator, Member startMember,
			Member endMember) {
		final Level level = startMember.getLevel();
		assertTrue(level == endMember.getLevel());
		List<Member> members = new ArrayList<Member>();
		evaluator.getSchemaReader().getMemberRange(level, startMember, endMember,
				members);
		...
		if (members.isEmpty()) {
			...
			evaluator.getSchemaReader().getMemberRange(level, endMember, startMember,
					members);
		}
		return members;
	}
	 ...
	 *          The reader to use
	 * @param member
	 *          The member for which we'll find the cousin.
	 * @param ancestorMember
	 *          The cousin's ancestor.
	 * 
	 * @return The child of {@code ancestorMember} in the same position under
	 *         {@code ancestorMember} as {@code member} is under its parent.
	 */
	static Member cousin(SchemaReader schemaReader, Member member,
	...
	static private Member cousin2(SchemaReader schemaReader, Member member1,
			Member member2) {
	...
	/**
	 * Returns the ancestor of {@code member} at the given level or distance. It
	 * is assumed that any error checking required has been done prior to calling
	 * this function.
	 * 
	 * <p>
	 * This method takes into consideration the fact that there may be intervening
	 * hidden members between {@code member} and the ancestor. If
	 * {@code targetLevel} is not null, then the method will only return a member
	 * if the level at {@code distance} from the member is actually the
	 * {@code targetLevel} specified.
	 * 
	 * @param evaluator
	 *          The evaluation context
	 * @param member
	 *          The member for which the ancestor is to be found
	 * @param distance
	 *          The distance up the chain the ancestor is to be found.
	 * 
	 * @param targetLevel
	 *          The desired targetLevel of the ancestor. If {@code null}, then the
	 *          distance completely determines the desired ancestor.
	 * 
	 * @return The ancestor member, or {@code null} if no such ancestor exists.
	 */
	static Member ancestor(Evaluator evaluator, Member member, int distance,
			Level targetLevel) {
		if ((targetLevel != null)
	...
	/**
	 * Compares a pair of members according to their positions in a prefix-order
	 * (or postfix-order, if {@code post} is true) walk over a hierarchy.
	 * 
	 * @param m1
	 *          First member
	 * @param m2
	 ...
	 *          sortMembers immediately after its last child. If false, a parent
	 *          will sortMembers immediately before its first child.
	 * 
	 * @return -1 if m1 collates before m2, 0 if m1 equals m2, 1 if m1 collates
	 *         after m2
	 */
		...
		while (true) {
			...
			if (depth1 < depth2) {
				...
				if (equals(m1, m2)) {
					...
					assert c != 0 : "Members " + prev1 + ", " + prev2
	 ...
	 * 
	 * First, compare by ordinal. This is only valid now we know they're siblings,
	 * because ordinals are only unique within a parent. If the dimension does not
	 * use ordinals, both ordinals will be -1.
	 * 
	 * <p>
	 ...
	 * @param m2
	 *          Second member
	 * @return -1 if m1 collates less than m2, 1 if m1 collates after m2, 0 if m1
	 *         == m2.
	 */
	public static int compareSiblingMembers(Member m1, Member m2) {
		...
		if ((k1 != null) && (k2 != null)) {
			...
			return (ordinal1 == ordinal2) ? m1.compareTo(m2)
	 ...
	 * 
	 * @param validator
	 *          Validator used to validate function arguments and resolve the
	 *          function
	 * @param funDef
	 *          Function definition, or null to deduce definition from name,
	 *          syntax and argument types
	 * @param args
	 ...
	 */
	public static FunDef resolveFunArgs(Validator validator, FunDef funDef,
			Exp[] args, Exp[] newArgs, String name, Syntax syntax) {
	 ...
	 *          Arguments to the function
	 */
	private static void checkNativeCompatible(Validator validator, FunDef funDef,
			Exp[] args) {
		...
		if (!(funDef instanceof SetFunDef)
				&& !(funDef instanceof ParenthesesFunDef) && query != null
	...
	static FunDef createDummyFunDef(Resolver resolver, int returnCategory,
			Exp[] args) {
	...
	/**
	 * Returns members of a level which are not empty (according to the criteria
	 * expressed by the evaluator).
	 * 
	 * @param evaluator
	 *          Evaluator, determines non-empty criteria
	 * @param level
	 *          Level
	 * @param includeCalcMembers
	 *          Whether to include calculated members
	 */
	static List<Member> getNonEmptyLevelMembers(final Evaluator evaluator,
			final Level level, final boolean includeCalcMembers) {
		SchemaReader sr = evaluator.getSchemaReader();
		if (evaluator.isNonEmpty()) {
	...
	static TupleList levelMembers(final Level level, final Evaluator evaluator,
			final boolean includeCalcMembers) {
		List<Member> memberList = getNonEmptyLevelMembers(evaluator, level,
				includeCalcMembers);
		TupleList tupleList;
		if (!includeCalcMembers) {
	...
	static TupleList hierarchyMembers(Hierarchy hierarchy, Evaluator evaluator,
			final boolean includeCalcMembers) {
		...
		if (evaluator.isNonEmpty()) {
			...
			for (Level level : hierarchy.getLevels()) {
				List<Member> members = getNonEmptyLevelMembers(evaluator, level,
						includeCalcMembers);
				memberList.addAll(members);
			}
		} else {
			final List<Member> memberList1 = addMembers(evaluator.getSchemaReader(),
	 ...
	 * @param comp
	 *          a Comparator; null means use natural comparison
	 * @param limit
	 */
	 ...
	 */
	static <T> List<T> stablePartialSort(final List<T> list,
			final Comparator<T> comp, int limit) {
		assert limit >= 0;
	...
	static TupleList parseTupleList(Evaluator evaluator, String string,
			List<Hierarchy> hierarchies) {
		final IdentifierParser.TupleListBuilder builder = new IdentifierParser.TupleListBuilder(
				evaluator.getSchemaReader(), evaluator.getCube(), hierarchies);
		IdentifierParser.parseTupleList(builder, string);
		return builder.tupleList;
	}

	/**
	 * Parses a tuple, of the form '(member, member, ...)'. There must be
	 * precisely one member for each hierarchy.
	 * 
	 * 
	 * @param evaluator
	 *          Evaluator, provides a {@link mondrian.olap.SchemaReader} and
	 *          {@link mondrian.olap.Cube}
	 * @param string
	 *          String to parse
	 * @param i
	 *          Position to start parsing in string
	 * @param members
	 ...
	 */
	private static int parseTuple(final Evaluator evaluator, String string,
			int i, final Member[] members, List<Hierarchy> hierarchies) {
		final IdentifierParser.Builder builder = new IdentifierParser.TupleBuilder(
				evaluator.getSchemaReader(), evaluator.getCube(), hierarchies) {
	 ...
	 */
	static Member[] parseTuple(Evaluator evaluator, String string,
			List<Hierarchy> hierarchies) {
	...
	static List<Member> parseMemberList(Evaluator evaluator, String string,
			Hierarchy hierarchy) {
		IdentifierParser.MemberListBuilder builder = new IdentifierParser.MemberListBuilder(
	...
	private static int parseMember(Evaluator evaluator, String string, int i,
			final Member[] members, Hierarchy hierarchy) {
		IdentifierParser.MemberListBuilder builder = new IdentifierParser.MemberListBuilder(
				evaluator.getSchemaReader(), evaluator.getCube(), hierarchy) {
			@Override
	...
	static Member parseMember(Evaluator evaluator, String string,
			Hierarchy hierarchy) {
		Member[] members = { null };
		...
		if (member == null) {
			throw MondrianResource.instance().MdxChildObjectNotFound.ex(string,
	...
	public static boolean worthCaching(Exp exp) {
		...
		// caching.
		if (exp instanceof MemberExpr || exp instanceof LevelExpr
				|| exp instanceof HierarchyExpr || exp instanceof DimensionExpr) {
			return false;
		}
	...
	/**
	 * A functional for {@link FunUtil#partialSort}. Sorts or partially sorts an
	 * array in ascending order, using a Comparator.
	 * 
	 * <p>
	 * Algorithm: quicksort, or partial quicksort (alias "quickselect"),
	 * Hoare/Singleton. Partial quicksort is quicksort that recurs only on one
	 * side, which is thus tail-recursion. Picks pivot as median of three; falls
	 * back on insertion sort for small "subfiles". Partial quicksort is O(n + m
	 * log m), instead of O(n log n), where n is the input size, and m is the
	 * desired output size.
	 * 
	 * <p>
	 * See D Knuth, Art of Computer Programming, 5.2.2 (Algorithm Q); R.
	 * Sedgewick, Algorithms in C, ch 5. Good summary in
	...
	static class Quicksorter<T> {
		...
		/**
		 * Runs one pass of quicksort on array segment VEC[START .. END], dividing
		 * it into two parts, the left side VEC[START .. P] none greater than the
		 * pivot value VEC[P], and the right side VEC[P+1 .. END] none less than the
		...
		public void partialSort(int limit) {
			...
			select(limit, 0, n);
			if (traced) {
				traceStats("partial sort: quickselect phase for " + limit + "from " + n
						+ "items");
			}
			sort(0, limit - 1);
			if (traced) {
	 ...
	 * 
	 * <p>
	 * Could genericize this to <code>class&lt;T&gt; MemorizingComparator
	 * implements Comparator&lt;T&gt;</code>, but not if it adds a run time cost,
	 * since the comparator is at the heart of the sort algorithms.
	 */
	private static abstract class MemberComparator implements Comparator<Member> {
		private static final Logger LOGGER = Logger
				.getLogger(MemberComparator.class);
			...
			if (LOGGER.isDebugEnabled()) {
				return new Comparator<Member>() {
					public int compare(Member m1, Member m2) {
						...
						LOGGER.debug("compare " + m1.getUniqueName() + "(" + eval(m1)
		...
		protected final int compareHierarchicallyButSiblingsByValue(Member m1,
				Member m2) {
	 ...
	 */
	private static abstract class TupleComparator implements
			Comparator<List<Member>> {
		private static final Logger LOGGER = Logger
				.getLogger(TupleComparator.class);
		final int arity;
			...
			if (LOGGER.isDebugEnabled()) {
				return new Comparator<List<Member>>() {
					public int compare(List<Member> a1, List<Member> a2) {
						...
						LOGGER.debug("compare {" + a1 + "}, {" + a2 + "} yields " + c);
						return c;
					}
				};
			} else {
	...
	/**
	 * Extension to {@link TupleComparator} which compares tuples by evaluating an
	 * expression.
	 */
	...
	private static class HierarchicalTupleComparator extends TupleExpComparator {
		private final boolean desc;

		HierarchicalTupleComparator(Evaluator evaluator, Calc calc, int arity,
				boolean desc) {
	...
	private static class HierarchicalTupleKeyComparator extends
	 ...
	 */
	enum Flag {
		ASC(false, false), DESC(true, false), BASC(false, true), BDESC(true, true);
	 ...
	 * 
	 * <p>
	 * Similar to {@link Pair}, but saves boxing overhead of converting
	...
	public static class ObjIntPair<T> {
		...
		public boolean equals(Object obj) {
			return obj instanceof ObjIntPair && this.i == ((ObjIntPair) obj).i
