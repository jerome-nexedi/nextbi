...
public class FunUtil extends Util {
  ...
  public static void checkListResultStyles(Calc calc) {
    ...
    case MUTABLE_LIST:
      break;
    default:
      throw ResultStyleException.generateBadType(ResultStyle.LIST_MUTABLELIST, calc
        .getResultStyle());
    }
  }
  ...
  static List<Member> addMembers(SchemaReader schemaReader, List<Member> members,
    Level level) {
   ...
   * @pre exp.getType() instanceof ScalarType
   */
  static Map<List<Member>, Object> evaluateTuples(Evaluator evaluator, Calc exp,
    TupleList tuples) {
    final int savepoint = evaluator.savepoint();
   ...
   */
  static List<Member> sortMembers(Evaluator evaluator, Iterable<Member> memberIter,
    List<Member> memberList, Calc exp, boolean desc, boolean brk) {
    ...
    try {
      ...
      if (memberList == null) {
        ...
        mapMemberToValue = evaluateMembers(evaluator, exp, memberIter, memberList,
          parentsToo);
      } else {
   ...
   */
  static List<Member> sortMembers(Evaluator evaluator, Iterable<Member> memberIter,
    List<Member> memberList, List<SortKeySpec> keySpecList) {
    if ((memberList != null) && (memberList.size() <= 1)) {
      return memberList;
    }
    if (memberList == null) {
  ...
  public static TupleList sortTuples(Evaluator evaluator,
    ...
    List<Member>[] tuples = tupleArrayList.toArray(new List[tupleArrayList.size()]);
    final DelegatingTupleList result = new DelegatingTupleList(tupleIterable
      .getArity(), Arrays.asList(tuples));
  ...
  public static List<Member> partiallySortMembers(Evaluator evaluator,
    ...
    try {
      ...
      Map<Member, Object> valueMap = evaluateMembers(evaluator, exp, list, null,
        false);
      evaluator.getTiming().markEnd(SORT_EVAL_TIMING_NAME);
      timingEval = false;
      evaluator.getTiming().markStart(SORT_TIMING_NAME);
  ...
  static TupleList sortTuples(Evaluator evaluator, TupleIterable tupleIter,
    ...
    for (SortKeySpec key : keySpecList) {
      ...
      if (brk) {
        TupleExpMemoComparator comp = new BreakTupleComparator(evaluator, key.key,
          arity);
        comp.preloadValues(tupleList);
        chain.addComparator(comp.wrap(), key.direction.descending);
      } else if (orderByKey) {
        TupleExpMemoComparator comp = new HierarchicalTupleKeyComparator(evaluator,
          key.key, arity);
        comp.preloadValues(tupleList);
        chain.addComparator(comp.wrap(), key.direction.descending);
      } else {
  ...
  public static List<List<Member>> partiallySortTuples(Evaluator evaluator,
    ...
    Comparator<List<Member>> comp = new BreakTupleComparator(evaluator, exp, list
      .getArity()).wrap();
    if (desc) {
  ...
  public static TupleList hierarchizeTupleList(TupleList tupleList, boolean post) {
    ...
    Comparator<List<Member>> comparator = new HierarchizeTupleComparator(fixedList
      .getArity(), post).wrap();
   ...
   * @pre range >= 1 && range <= 3
   */
  protected static double quartile(Evaluator evaluator, TupleList members, Calc exp,
    int range) {
  ...
  static double correlation(Evaluator evaluator, TupleList memberList, Calc exp1,
    Calc exp2) {
  ...
  static Object stdev(Evaluator evaluator, TupleList members, Calc exp,
    ...
    return (o instanceof Double) ? new Double(Math.sqrt(((Double) o).doubleValue()))
      : o;
  }
  ...
  static Member cousin(SchemaReader schemaReader, Member member,
    ...
    if (member.getHierarchy() != ancestorMember.getHierarchy()) {
      throw MondrianResource.instance().CousinHierarchyMismatch.ex(member
        .getUniqueName(), ancestorMember.getUniqueName());
    }
   ...
   * hidden members between {@code member} and the ancestor. If {@code
   * targetLevel} is not null, then the method will only return a member if the
   * level at {@code distance} from the member is actually the {@code
  ...
  static Member ancestor(Evaluator evaluator, Member member, int distance,
    ...
    if ((targetLevel != null)
      ...
      throw MondrianResource.instance().MemberNotInLevelHierarchy.ex(member
        .getUniqueName(), targetLevel.getUniqueName());
    }
  ...
  public static int compareSiblingMembers(Member m1, Member m2) {
    ...
    if ((k1 != null) && (k2 != null)) {
      ...
      return (ordinal1 == ordinal2) ? m1.compareTo(m2) : (ordinal1 < ordinal2) ? -1
        : 1;
    }
  }
  ...
  private static void checkNativeCompatible(Validator validator, FunDef funDef,
    ...
    if (!(funDef instanceof SetFunDef) && !(funDef instanceof ParenthesesFunDef)
      && query != null && query.nativeCrossJoinVirtualCube()) {
   ...
   */
  static <T> List<T> stablePartialSort(final List<T> list, final Comparator<T> comp,
    int limit) {
    assert limit >= 0;
   ...
   */
  private static int parseTuple(final Evaluator evaluator, String string, int i,
    final Member[] members, List<Hierarchy> hierarchies) {
  ...
  static Member parseMember(Evaluator evaluator, String string, Hierarchy hierarchy) {
    ...
    if (member == null) {
      throw MondrianResource.instance().MdxChildObjectNotFound.ex(string, evaluator
        .getCube().getQualifiedName());
    }
    return member;
  }
        ...
        return new Comparator<Member>() {
          public int compare(Member m1, Member m2) {
            ...
            LOGGER.debug("compare " + m1.getUniqueName() + "(" + eval(m1) + "), "
              + m2.getUniqueName() + "(" + eval(m2) + ")" + " yields " + c);
            return c;
          }
        };
      } else {
   ...
   */
  private static abstract class TupleComparator implements Comparator<List<Member>> {
    private static final Logger LOGGER = Logger.getLogger(TupleComparator.class);
  ...
  private static class HierarchicalTupleKeyComparator extends TupleExpMemoComparator {
   ...
   * 
   * <p>
   * Similar to {@link Pair}, but saves boxing overhead of converting {@code
   * int} to {@link Integer}.
   */
