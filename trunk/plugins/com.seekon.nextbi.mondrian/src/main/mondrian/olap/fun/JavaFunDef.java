...
public class JavaFunDef extends FunDefBase {
	 ...
	 */
	public JavaFunDef(String name, String desc, Syntax syntax,
			int returnCategory, int[] paramCategories, Method method) {
	...
	private static FunDef generateFunDef(final Method method) {
		String name = getAnnotation(method, className + "$FunctionName",
				method.getName());
		String desc = getAnnotation(method, className + "$Description", "");
		Syntax syntax = getAnnotation(method, className + "$SyntaxDef",
				Syntax.Function);
		...
		if (clazz == String.class) {
			...
			return new AbstractCalc2(exp, stringCalc) {
				public Object evaluate(Evaluator evaluator) {
					...
					return Character
							.valueOf(string == null || string.length() < 1 ? (char) 0
									: string.charAt(0));
				}
			};
