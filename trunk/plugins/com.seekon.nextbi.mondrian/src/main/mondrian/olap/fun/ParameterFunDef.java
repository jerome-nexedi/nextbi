...
public class ParameterFunDef extends FunDefBase {
	...
	ParameterFunDef(FunDef funDef, String parameterName, Type type,
		...
		super(funDef.getName(), funDef.getSignature(), funDef.getDescription(),
				funDef.getSyntax(), returnCategory, funDef.getParameterCategories());
	...
	public Exp createCall(Validator validator, Exp[] args) {
		Parameter parameter = validator.createOrLookupParam(
				this.getName().equals("Parameter"), parameterName, type, exp,
		...
		protected FunDef createFunDef(Exp[] args, FunDef dummyFunDef) {
			...
			case Category.Level:
				...
				if (!isConstant(typeArg)) {
					throw newEvalException(dummyFunDef, "Invalid parameter '"
							+ parameterName
				...
				if (dimension == null) {
					throw newEvalException(dummyFunDef,
							"Invalid dimension for parameter '" + parameterName + "'");
				}
				type = new MemberType(type.getDimension(), type.getHierarchy(),
						type.getLevel(), null);
			...
			if (category == Category.Member) {
				...
				if (distinctFrom(type.getDimension(), expType.getDimension())
					...
					throw newEvalException(dummyFunDef, "Default value of parameter '"
							+ parameterName + "' is not consistent with the parameter type '"
							+ type);
				}
			}
			...
			return new ParameterFunDef(dummyFunDef, parameterName, type, category,
	...
	public static class ParamRefResolver extends MultiResolver {
		...
		protected FunDef createFunDef(Exp[] args, FunDef dummyFunDef) {
			String parameterName = getParameterName(args);
			return new ParameterFunDef(dummyFunDef, parameterName, null,
					Category.Unknown, null, null);
		}
	}
}
