/*
// $Id: //open/mondrian/src/main/mondrian/olap/fun/DescendantsFunDef.java#30 $
 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap/fun/DescendantsFunDef.java#30
 *          $
 * @since Mar 23, 2006
 */
class DescendantsFunDef extends FunDefBase {
	...
	static final ReflectiveMultiResolver Resolver = new ReflectiveMultiResolver(
			...
			DescendantsFunDef.class, Flag.getNames());

	static final ReflectiveMultiResolver Resolver2 = new ReflectiveMultiResolver(
			"Descendants",
	...
	public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
		...
		if (type0 instanceof SetType) {
			...
			descendantsArgs[0] = new UnresolvedFunCall("CurrentMember",
					Syntax.Property, new Exp[] { new HierarchyExpr(hierarchy) });
			final ResolvedFunCall generateCall = (ResolvedFunCall) compiler
					.getValidator().validate(
							new UnresolvedFunCall("Generate", new Exp[] { call.getArg(0),
									new UnresolvedFunCall("Descendants", descendantsArgs) }),
							false);
		...
		if (call.getArgCount() >= 2 && depthEmpty) {
			if (flag != Flag.LEAVES) {
				throw Util
						.newError("depth must be specified unless DESC_FLAG is LEAVES");
			}
		}
		if ((depthSpecified || depthEmpty) && flag.leaves) {
			final IntegerCalc depthCalc = depthSpecified ? compiler
					.compileInteger(call.getArg(1)) : null;
			...
			return new AbstractListCalc(call, new Calc[] { memberCalc, depthCalc }) {
				public TupleList evaluateList(Evaluator evaluator) {
					...
					final SchemaReader schemaReader = evaluator.getSchemaReader();
					descendantsByDepth(member, result, schemaReader, depth, flag1.before,
							flag1.self, flag1.after, evaluator);
					hierarchizeMemberList(result, false);
					return new UnaryTupleList(result);
				}
			};
		} else {
			final LevelCalc levelCalc = call.getArgCount() > 1 ? compiler
					.compileLevel(call.getArg(1)) : null;
			...
			return new AbstractListCalc(call, new Calc[] { memberCalc, levelCalc }) {
				public TupleList evaluateList(Evaluator evaluator) {
					...
					List<Member> result = new ArrayList<Member>();
					final SchemaReader schemaReader = evaluator.getSchemaReader();
					final Level level = levelCalc != null ? levelCalc
							.evaluateLevel(evaluator) : member.getLevel();
					descendantsByLevel(schemaReader, member, level, result, flag2.before,
							flag2.self, flag2.after, flag2.leaves, context);
					hierarchizeMemberList(result, false);
					return new UnaryTupleList(result);
				}
			};
	...
	private static void descendantsByDepth(Member member, List<Member> result,
	 ...
	 * constraint.
	 */
	private static void descendantsLeavesByDepth(final Member member,
			final List<Member> result, final SchemaReader schemaReader,
	...
	/**
	 * Finds all descendants of a member which are before/at/after a level, and/or
	 * are leaves (have no descendants) and adds them to a result list.
	 * 
	 * @param schemaReader
	 *          Member reader
	 * @param ancestor
	 ...
	 * @param self
	 *          Whether to output members at <code>level</code>
	 * @param after
	 *          Whether to output members below <code>level</code>
	 * @param leaves
	 *          Whether to output members which are leaves
	 * @param context
	 *          Evaluation context; determines criteria by which the result set
	 *          should be filtered
	 */
	static void descendantsByLevel(SchemaReader schemaReader, Member ancestor,
			Level level, List<Member> result, boolean before, boolean self,
			boolean after, boolean leaves, Evaluator context) {
		...
		if (leaves) {
			...
			do {
				...
				for (Member member : members) {
					...
					List<Member> childMembers = schemaReader.getMemberChildren(member,
							context);
			...
			do {
				...
				members = schemaReader.getMemberChildren(fertileMembers, context);
			} while (members.size() > 0);
		}
	}
	 ...
	 */
	enum Flag {
		SELF(true, false, false, false), AFTER(false, true, false, false), BEFORE(
				false, false, true, false), BEFORE_AND_AFTER(false, true, true, false), SELF_AND_AFTER(
				true, true, false, false), SELF_AND_BEFORE(true, false, true, false), SELF_BEFORE_AFTER(
				true, true, true, false), LEAVES(false, false, false, true);
