...
class DescendantsFunDef extends FunDefBase {
  ...
  public Calc compileCall(ResolvedFunCall call, ExpCompiler compiler) {
    ...
    if (type0 instanceof SetType) {
      ...
      descendantsArgs[0] = new UnresolvedFunCall("CurrentMember", Syntax.Property,
        new Exp[] { new HierarchyExpr(hierarchy) });
      final ResolvedFunCall generateCall = (ResolvedFunCall) compiler.getValidator()
        .validate(
          new UnresolvedFunCall("Generate", new Exp[] { call.getArg(0),
            new UnresolvedFunCall("Descendants", descendantsArgs) }), false);
    ...
    if ((depthSpecified || depthEmpty) && flag.leaves) {
      final IntegerCalc depthCalc = depthSpecified ? compiler.compileInteger(call
        .getArg(1)) : null;
      ...
      return new AbstractListCalc(call, new Calc[] { memberCalc, levelCalc }) {
        public TupleList evaluateList(Evaluator evaluator) {
          ...
          List<Member> result = new ArrayList<Member>();
          final SchemaReader schemaReader = evaluator.getSchemaReader();
          final Level level = levelCalc != null ? levelCalc.evaluateLevel(evaluator)
            : member.getLevel();
  ...
  private static void descendantsLeavesByDepth(final Member member,
    final List<Member> result, final SchemaReader schemaReader, final int depthLimit) {
  ...
  static void descendantsByLevel(SchemaReader schemaReader, Member ancestor,
    Level level, List<Member> result, boolean before, boolean self, boolean after,
    boolean leaves, Evaluator context) {
    ...
    if (leaves) {
      ...
      do {
        ...
        for (Member member : members) {
          ...
          List<Member> childMembers = schemaReader
            .getMemberChildren(member, context);
   ...
   */
  enum Flag {
    SELF(true, false, false, false), AFTER(false, true, false, false), BEFORE(false,
      false, true, false), BEFORE_AND_AFTER(false, true, true, false), SELF_AND_AFTER(
