/*
// $Id: //open/mondrian/src/main/mondrian/olap/Walker.java#9 $
...
public class Walker implements Enumeration {
  /**
   * The active parts of the tree from the root to nextNode are held in a stack.
   * When the stack is empty, the enumeration finishes. currentFrame holds the
   * frame of the 'current node' (the node last returned from nextElement())
   * because it may no longer be on the stack.
   */
  ...
  /**
   * get the ordinal within its parent node of the current node. Returns 0 for
   * the root element. Equivalent to getAncestorOrdinal(0).
   */
  public int getOrdinal() {
    // We can't use currentFrame.parent.iChild because moveToNext() may
    // have changed it.
    return currentFrame.parent == null ? 0 : arrayFind(currentFrame.parent.children,
      currentFrame.node);
  }
  ...
  public int getAncestorOrdinal(int iDepth) {
    Frame f = getAncestorFrame(iDepth);
    return f == null ? -1 : f.parent == null ? 0 : arrayFind(f.parent.children,
      f.node);
  }

  /**
   * Override this function to prune the tree, or to allow objects which are not
   * Walkable to have children.
   */
  ...
  public static void main(String[] args) {
    ...
    Region usa = new Region("USA", new Region[] {
      new Region("CA",
        new Region[] {
          new Region("San Francisco",
            new Region[] {
              new Region("WesternAddition",
                new Region[] { new Region("Haight", null) }),
              new Region("Soma", null) }), new Region("Los Angeles", null) }),
      new Region("WA", new Region[] { new Region("Seattle", null),
