...
public class Query extends QueryPart {
	...
	public Query(Statement statement, Formula[] formulas, QueryAxis[] axes,
		...
		this(statement, Util.lookupCube(statement.getSchemaReader(), cube, true),
				formulas, axes, slicerAxis, cellProps, new Parameter[0],
				strictValidation);
	}
	...
	public Query clone() {
		return new Query(statement, cube, Formula.cloneArray(formulas),
				QueryAxis.cloneArray(axes), (slicerAxis == null) ? null
						: (QueryAxis) slicerAxis.clone(), cellProps,
				parameters.toArray(new Parameter[parameters.size()]), strictValidation);
	}
	...
	public void setResultStyle(ResultStyle resultStyle) {
		...
		case ITERABLE:
			// For java4, use LIST
			this.resultStyle = (Util.Retrowoven) ? ResultStyle.LIST
					: ResultStyle.ITERABLE;
			break;
		case LIST:
		case MUTABLE_LIST:
	...
	public void resolve(Validator validator) {
		...
		if (axes != null) {
			...
			for (QueryAxis axis : axes) {
				...
				if (!axisNames.add(axis.getAxisOrdinal().logicalOrdinal())) {
					throw MondrianResource.instance().DuplicateAxis
							.ex(axis.getAxisName());
				}
			}
			...
			// 0 .. N-1 should exist.
			int seekOrdinal = AxisOrdinal.StandardAxisOrdinal.COLUMNS
					.logicalOrdinal();
			for (QueryAxis axis : axes) {
		...
		for (Hierarchy hierarchy : ((RolapCube) getCube()).getHierarchies()) {
			...
			if (useCount > 1) {
				throw MondrianResource.instance().HierarchyInIndependentAxes
	 ...
	 */
	private static Object quickParse(String parameterName, Type type,
			Object value, Query query) throws NumberFormatException {
		...
		case Category.Set:
			...
			for (Object o : list) {
				...
				final Member member = (Member) quickParse(parameterName, elementType,
						o, query);
				expList.add(member);
			}
			return expList;
		case Category.Member:
			...
			if (value instanceof List
				...
				final List<IdentifierSegment> olap4jSegmentList = Util
						.cast((List) value);
	...
	private String getUniqueNameWithoutAll(Member member) {
		...
		if ((parentMember != null) && !parentMember.isAll()) {
			return Util.makeFqName(getUniqueNameWithoutAll(parentMember),
					member.getName());
		} else {
	 ...
	 */
	public ScopedNamedSet createScopedNamedSet(String name, QueryPart scope,
			Exp expr) {
		...
		if (failIfUsedInQuery && formula != null) {
			...
			while (walker.hasMoreElements()) {
				...
				String formulaType = formula.isMember() ? MondrianResource.instance().CalculatedMember
						.str() : MondrianResource.instance().CalculatedSet.str();
					...
					if (grandParent instanceof Query) {
						if (parent instanceof Axis) {
							...
							String parentFormulaType = ((Formula) parent).isMember() ? MondrianResource
									.instance().CalculatedMember.str() : MondrianResource
									.instance().CalculatedSet.str();
							throw MondrianResource.instance().MdxCalculatedFormulaUsedInFormula
									.ex(formulaType, uniqueName, parentFormulaType,
											((Formula) parent).getUniqueName());

						} else {
							throw MondrianResource.instance().MdxCalculatedFormulaUsedOnSlicer
									.ex(formulaType, uniqueName);
						}
					}
					++i;
	...
	public void renameFormula(String uniqueName, String newName) {
		Formula formula = findFormula(uniqueName);
		if (formula == null) {
			throw MondrianResource.instance().MdxFormulaNotFound.ex("formula",
					uniqueName, Util.unparse(this));
		}
	...
	public Hierarchy[] getMdxHierarchiesOnAxis(AxisOrdinal axis) {
		...
		QueryAxis queryAxis = axis.isFilter() ? slicerAxis : axes[axis
				.logicalOrdinal()];
	...
	public Calc compileExpression(Exp exp, boolean scalar, ResultStyle resultStyle) {
		...
		try {
			...
			resultStyleList = Collections
					.singletonList(resultStyle != null ? resultStyle : this.resultStyle);
	...
	public ExpCompiler createCompiler() {
		...
		try {
			...
			return createCompiler(evaluator, validator,
					Collections.singletonList(resultStyle));
		} finally {
			statement.close();
		}
	...
	private ExpCompiler createCompiler(final Evaluator evaluator,
		...
		ExpCompiler compiler = ExpCompiler.Factory.getExpCompiler(evaluator,
				validator, resultStyleList);
	...
	private static class QuerySchemaReader extends DelegatingSchemaReader
		...
		private static boolean match(Member member, List<Id.Segment> nameParts) {
			...
			if (segment.matches(member.getName())) {
				return Util.equalName(member.getHierarchy().getUniqueName(),
		...
		public List<Member> getCalculatedMembers(Hierarchy hierarchy) {
			...
			final List<Member> calculatedMembers = super
		...
		public OlapElement lookupCompoundInternal(OlapElement parent,
			...
			if (matchType == MatchType.EXACT) {
				OlapElement oe = lookupCompound(parent, names, failIfNotFound,
						category, MatchType.EXACT_SCHEMA);
				if (oe != null) {
					return oe;
				}
			}
	...
	private static class ScopedSchemaReader extends DelegatingSchemaReader
		...
		public OlapElement lookupCompoundInternal(OlapElement parent,
			...
			return super.lookupCompoundInternal(parent, names, failIfNotFound,
					category, matchType);
		}
	...
	public static class ScopedNamedSet implements NamedSet {
		...
		public NamedSet validate(Validator validator) {
			...
			if (type instanceof MemberType || type instanceof TupleType) {
				newExpr = new UnresolvedFunCall("{}", Syntax.Braces,
						new Exp[] { newExpr }).accept(validator);
			}
			this.expr = newExpr;
			return this;
		}
	...
	private class ParameterFinder extends MdxVisitorImpl {
		...
		public Object visit(UnresolvedFunCall call) {
			if (call.getFunName().equals("Parameter")) {
				...
				Parameter parameter = new ParameterImpl(parameterName,
						Literal.nullValue, null, type);
				parameters.add(parameter);
		...
		private void registerAlias(QueryPart parent, Exp exp) {
			if (exp instanceof FunCall) {
				...
				if (call2.getSyntax() == Syntax.Infix
					...
					createScopedNamedSet(id.getSegments().get(0).name, parent,
							call2.getArg(0));
				}
			}
		}
