...
public class Util extends XOMUtil {
	 ...
	 */
	public static final boolean IBM_JVM = System.getProperties()
			.getProperty("java.vendor").equals("IBM Corporation");
	 ...
	 */
	public static final boolean Retrowoven = Access.class.getSuperclass()
			.getName().equals("net.sourceforge.retroweaver.runtime.java.lang.Enum");
	...
	static {
		...
		try {
			Class<UtilCompatible> clazz = (Class<UtilCompatible>) Class
					.forName(className);
	...
	public static String normalizeName(String s) {
		return MondrianProperties.instance().CaseSensitive.get() ? s : s
				.toUpperCase();
	}
	 ...
	 */
	public static StringBuilder replace(StringBuilder buf, int start,
	...
	public static OlapElement lookupCompound(SchemaReader schemaReader,
			...
			int category) {
		return lookupCompound(schemaReader, parent, names, failIfNotFound,
	...
	public static OlapElement lookupCompound(SchemaReader schemaReader,
		...
		for (int i = 0; i < names.size(); i++) {
			...
			if (child == null) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Util.lookupCompound: " + "parent.name="
							+ parent.getName() + " has no child with name=" + name);
				}
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Util.lookupCompound: " + "found child.name="
					+ parent.getName() + ", child.class=" + parent.getClass().getName());
		}
	 ...
	 */
	public static Exp lookup(Query q, List<Id.Segment> nameParts,
			boolean allowProp) {
	...
	public static Exp lookup(Query q, SchemaReader schemaReader,
		...
		OlapElement olapElement = schemaReaderSansAc.lookupCompound(cube,
				nameParts, false, Category.Unknown);
		if (olapElement != null) {
		...
		if (olapElement == null) {
			if (allowProp && nameParts.size() > 1) {
				List<Id.Segment> namePartsButOne = nameParts.subList(0,
						nameParts.size() - 1);
			...
			if (q.ignoreInvalidMembers()) {
				...
				while (nameLen > 0 && olapElement == null) {
					...
					olapElement = schemaReaderSansAc.lookupCompound(cube, partialName,
				...
				if (olapElement != null) {
					...
					throw MondrianResource.instance().MdxChildObjectNotFound.ex(fullName,
	 ...
	 */
	static Cube lookupCube(SchemaReader schemaReader, String cubeName,
			boolean fail) {
	...
	public static Member lookupHierarchyRootMember(SchemaReader reader,
		...
		return lookupHierarchyRootMember(reader, hierarchy, memberName,
				MatchType.EXACT);
	}
	...
	public static Member lookupHierarchyRootMember(SchemaReader reader,
		...
		if (!matchType.isExact() && !hierarchy.hasAll() && !rootMembers.isEmpty()) {
			searchMember = hierarchy.createMember(null,
					rootMembers.get(0).getLevel(), memberName.name, null);
		}
		...
		return (rootMembers.size() > 0 && rootMembers.get(0).isAll()) ? reader
				.lookupMemberChildByName(rootMembers.get(0), memberName, matchType)
				: null;
	}
	...
	public static int getMemberOrdinalInParent(SchemaReader reader, Member member) {
		...
		List<Member> siblings = (parent == null) ? reader
				.getHierarchyRootMembers(member.getHierarchy()) : reader
	...
	public static Property lookupProperty(Level level, String propertyName) {
		...
		if (property != null && property.isMemberProperty()
				&& property.isStandard()) {
			return property;
		}
		return null;
	...
	public static List<Member> addLevelCalculatedMembers(SchemaReader reader,
		...
		List<Member> calcMembers = reader
	...
	public static RuntimeException newElementNotFoundException(int category,
		...
		switch (category) {
		case Category.Member:
			return MondrianResource.instance().MemberNotFound.ex(identifierNode
					.toString());
		case Category.Unknown:
			type = "Element";
			break;
	...
	public static String getErrorMessage(Throwable err, boolean prependClassName) {
		...
		if ((errMsg == null) || (err instanceof RuntimeException)) {
			...
			return (prependClassName) ? err.getClass().getName() + ": " + errMsg
					: errMsg;
		}
	}
	 ...
	 * 
	 * <p>
	 * This method implements as much as possible of the <a href=
	 * "http://msdn.microsoft.com/library/en-us/oledb/htm/oledbconnectionstringsyntax.asp"
			...
			if (c == '"' || c == '\'') {
				...
				if (i >= n) {
					...
					throw new RuntimeException(
							"quoted value ended too soon, at position " + i + " in '" + s
									+ "'");
				}
			} else {
				String value;
	...
	/**
	 * Like <code>{@link java.util.Arrays}.copyOf(int[], int)</code>, but exists
	 * prior to JDK 1.6.
	 * 
	 * @param original
	 *          the array to be copied
	...
	/**
	 * Like <code>{@link java.util.Arrays}.copyOf(long[], int)</code>, but exists
	 * prior to JDK 1.6.
	 * 
	 * @param original
	 *          the array to be copied
	 ...
	 */
	public static <T> T compileScript(Class<T> iface, String script,
			String engineName) {
	...
	public static UserDefinedFunction createUdf(
		...
		if (constructor == null) {
			throw MondrianResource.instance().UdfClassWrongIface.ex(
					functionNameOrEmpty, className, UserDefinedFunction.class.getName());
		}
		// Instantiate class.
		try {
	...
	public static abstract class AbstractFlatList<T> implements List<T>,
			RandomAccess {
