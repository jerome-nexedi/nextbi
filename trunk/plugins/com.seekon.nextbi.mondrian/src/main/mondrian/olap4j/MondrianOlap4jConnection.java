/*
// $Id: //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jConnection.java#33 $
...
/**
 * Implementation of {@link org.olap4j.OlapConnection} for the Mondrian OLAP
 * engine.
 * 
 * <p>
 * This class has sub-classes which implement JDBC 3.0 and JDBC 4.0 APIs; it is
 * instantiated using
 * {@link Factory#newConnection(MondrianOlap4jDriver, String, java.util.Properties)}
 * .
 * </p>
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jConnection
 *          .java#33 $
 * @since May 23, 2007
 */
abstract class MondrianOlap4jConnection implements OlapConnection {
   ...
   * {@link #getMondrianConnection()} or {@link #getMondrianConnection2()}, and
   * these will throw if the connection has been closed.
   */
   ...
   * 
   * <p>
   * REVIEW: This assumes that a RolapSchema occurs at most once in a catalog.
   * It is possible for a schema to be mapped more than once, with different
   * names; the same RolapSchema object will be used.
   */
  final Map<mondrian.olap.Schema, MondrianOlap4jSchema> schemaMap = new HashMap<mondrian.olap.Schema, MondrianOlap4jSchema>();
   ...
   * {@link mondrian.olap4j.MondrianOlap4jDriver} for more details.
   * 
   * @param factory
   *          Factory
   * @param driver
   ...
   */
  MondrianOlap4jConnection(Factory factory, MondrianOlap4jDriver driver, String url,
    ...
    if (url.startsWith(ENGINE_CONNECT_STRING_PREFIX)) {
      ...
      throw new AssertionError("does not start with '" + CONNECT_STRING_PREFIX + "'");
    }
    ...
    this.mondrianConnection = (RolapConnection) mondrian.olap.DriverManager
      .getConnection(list, null);

    this.olap4jDatabaseMetaData = factory.newDatabaseMetaData(this,
      mondrianConnection);

    this.mondrianServer = MondrianServer.forConnection(mondrianConnection);
    final CatalogFinder catalogFinder = (CatalogFinder) mondrianServer;

    NamedList<MondrianOlap4jCatalog> olap4jCatalogs = new NamedListImpl<MondrianOlap4jCatalog>();
    this.olap4jDatabases = new NamedListImpl<MondrianOlap4jDatabase>();

    List<Map<String, Object>> dbpropsMaps = mondrianServer
      .getDatabases(mondrianConnection);
    if (dbpropsMaps.size() != 1) {
      throw new AssertionError();
    }
    Map<String, Object> dbpropsMap = dbpropsMaps.get(0);
    StringTokenizer st = new StringTokenizer(String.valueOf(dbpropsMap
      .get("ProviderType")), ",");
    List<ProviderType> pTypes = new ArrayList<ProviderType>();
    while (st.hasMoreTokens()) {
      pTypes.add(ProviderType.valueOf(st.nextToken()));
    }
    st = new StringTokenizer(String.valueOf(dbpropsMap.get("AuthenticationMode")),
      ",");
    List<AuthenticationMode> aModes = new ArrayList<AuthenticationMode>();
    while (st.hasMoreTokens()) {
      aModes.add(AuthenticationMode.valueOf(st.nextToken()));
    }
    final MondrianOlap4jDatabase database = new MondrianOlap4jDatabase(this,
      olap4jCatalogs, String.valueOf(dbpropsMap.get("DataSourceName")), String
        .valueOf(dbpropsMap.get("DataSourceDescription")), String.valueOf(dbpropsMap
        .get("ProviderName")), String.valueOf(dbpropsMap.get("URL")), String
        .valueOf(dbpropsMap.get("DataSourceInfo")), pTypes, aModes);
    this.olap4jDatabases.add(database);

    for (String catalogName : catalogFinder.getCatalogNames(mondrianConnection)) {
      final Map<String, RolapSchema> schemaMap = catalogFinder.getRolapSchemas(
        mondrianConnection, catalogName);
      olap4jCatalogs.add(new MondrianOlap4jCatalog(olap4jDatabaseMetaData,
        catalogName, database, schemaMap));
    }
  ...
  public boolean isWrapperFor(Class<?> iface) throws SQLException {
    return iface.isInstance(this) || iface.isInstance(mondrianConnection);
  }
  ...
  Type toOlap4j(mondrian.olap.type.Type type) {
    if (type instanceof mondrian.olap.type.BooleanType) {
      ...
      final mondrian.olap.Cube mondrianCube = ((mondrian.olap.type.CubeType) type)
        .getCube();
      return new CubeType(toOlap4j(mondrianCube));
    } else if (type instanceof mondrian.olap.type.DecimalType) {
      mondrian.olap.type.DecimalType decimalType = (mondrian.olap.type.DecimalType) type;
      ...
      return new DimensionType(toOlap4j(dimensionType.getDimension()));
      ...
      final mondrian.olap.type.MemberType memberType = (mondrian.olap.type.MemberType) type;
      return new MemberType(toOlap4j(memberType.getDimension()), toOlap4j(memberType
        .getHierarchy()), toOlap4j(memberType.getLevel()), toOlap4j(memberType
        .getMember()));
      ...
      final mondrian.olap.type.SetType setType = (mondrian.olap.type.SetType) type;
  ...
  MondrianOlap4jMember toOlap4j(mondrian.olap.Member member) {
    ...
    if (member instanceof RolapMeasure) {
      ...
      return new MondrianOlap4jMeasure(toOlap4j(member.getDimension().getSchema()),
        measure);
    }
    return new MondrianOlap4jMember(toOlap4j(member.getDimension().getSchema()),
      member);
  }
  ...
  MondrianOlap4jHierarchy toOlap4j(mondrian.olap.Hierarchy hierarchy) {
    ...
    return new MondrianOlap4jHierarchy(
      toOlap4j(hierarchy.getDimension().getSchema()), hierarchy);
  }
  ...
  MondrianOlap4jNamedSet toOlap4j(mondrian.olap.Cube cube,
  ...
  /**
   * Cop-out version of {@link #getMondrianConnection()} that doesn't throw a
   * checked exception. For those situations where the olap4j API doesn't
   * declare 'throws OlapException', but we need an open connection anyway. Use
   * {@link #getMondrianConnection()} where possible.
   * 
   ...
   */
  static <T> NamedList<T> unmodifiableNamedList(final NamedList<? extends T> list) {
    mondrian.util.Bug.olap4jUpgrade("remove this method and 2 classes");
    return list instanceof RandomAccess ? new UnmodifiableNamedRandomAccessList<T>(
      list) : new UnmodifiableNamedList<T>(list);
  }

  // inner classes

  /**
   * Package-private helper class which encapsulates policies which are common
   * throughout the driver. These policies include exception handling and
   * factory methods.
   */
  static class Helper {
    ...
    /**
     * Creates an exception in the context of a particular Cell and with a given
     * cause.
     * 
     * @param context
     *          Cell context for exception
    ...
    /**
     * Converts a SQLException to an OlapException. Casts the exception if it is
     * already an OlapException, wraps otherwise.
     * 
     * <p>
     * This method is typically used as an adapter for SQLException instances
     * coming from a base class, where derived interface declares that it throws
     * the more specific OlapException.
     * 
     * @param e
     *          Exception
  ...
  private static class MondrianOlap4jMdxValidator implements MdxValidator {
    ...
    public SelectNode validateSelect(SelectNode selectNode) throws OlapException {
      try {
        ...
        Query query = connection.mondrianConnection.parseQuery(mdx);
        query.resolve();
  ...
  private static class MondrianToOlap4jNodeConverter {
    ...
    public SelectNode toOlap4j(Query query) {
      ...
      return new SelectNode(null, toOlap4j(query.getFormulas()), toOlap4j(query
        .getAxes()), new CubeNode(null, olap4jConnection.toOlap4j(query.getCube())),
        query.getSlicerAxis() == null ? null : toOlap4j(query.getSlicerAxis()), list);
    }

    private AxisNode toOlap4j(QueryAxis axis) {
      return new AxisNode(null, axis.isNonEmpty(), Axis.Factory.forOrdinal(axis
        .getAxisOrdinal().logicalOrdinal()),
        toOlap4j(axis.getDimensionProperties()), toOlap4j(axis.getSet()));
    }
    ...
    private ParseTreeNode toOlap4j(Exp exp) {
      ...
      if (exp instanceof DimensionExpr) {
        ...
        return new DimensionNode(null, olap4jConnection.toOlap4j(dimensionExpr
          .getDimension()));
      }
      if (exp instanceof HierarchyExpr) {
        HierarchyExpr hierarchyExpr = (HierarchyExpr) exp;
        return new HierarchyNode(null, olap4jConnection.toOlap4j(hierarchyExpr
          .getHierarchy()));
      }
      if (exp instanceof LevelExpr) {
        LevelExpr levelExpr = (LevelExpr) exp;
        return new LevelNode(null, olap4jConnection.toOlap4j(levelExpr.getLevel()));
      }
      if (exp instanceof MemberExpr) {
        MemberExpr memberExpr = (MemberExpr) exp;
        return new MemberNode(null, olap4jConnection
          .toOlap4j(memberExpr.getMember()));
      }
      if (exp instanceof Literal) {
        Literal literal = (Literal) exp;
        final Object value = literal.getValue();
        if (literal.getCategory() == Category.Symbol) {
          return LiteralNode.createSymbol(null, (String) literal.getValue());
    ...
    private ParseTreeNode toOlap4j(ResolvedFunCall call) {
      final CallNode callNode = new CallNode(null, call.getFunName(), toOlap4j(call
        .getSyntax()), toOlap4j(Arrays.asList(call.getArgs())));
    ...
    private List<ParseTreeNode> toOlap4j(Formula[] formulas) {
      ...
      for (Formula formula : formulas) {
        if (formula.isMember()) {
          List<PropertyValueNode> memberPropertyList = new ArrayList<PropertyValueNode>();
          for (Object child : formula.getChildren()) {
            if (child instanceof MemberProperty) {
              MemberProperty memberProperty = (MemberProperty) child;
              memberPropertyList.add(new PropertyValueNode(null, memberProperty
                .getName(), toOlap4j(memberProperty.getExp())));
            }
          }
          list.add(new WithMemberNode(null, toOlap4j(formula.getIdentifier()),
            toOlap4j(formula.getExpression()), memberPropertyList));
        }
      }
      return list;
    }
    ...
    private static IdentifierNode toOlap4j(Id id) {
      ...
      return new IdentifierNode(list.toArray(new IdentifierSegment[list.size()]));
    }
  }
  ...
  private static class UnmodifiableNamedRandomAccessList<T> extends
    UnmodifiableNamedList<T> implements RandomAccess {
