...
/**
 * Provides access to internals of mondrian's olap4j driver that are not part of
 * the olap4j API.
 * 
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/olap4j/MondrianOlap4jExtra.java#5
 *          $
 * @since Nov 12, 2010
 */
class MondrianOlap4jExtra implements XmlaHandler.XmlaExtra {
	...
	public ResultSet executeDrillthrough(OlapStatement olapStatement, String mdx,
			...
			throws SQLException {
		return ((MondrianOlap4jStatement) olapStatement).executeQuery2(mdx,
				advanced, tabFields, rowCountSlot);
	}
	...
	public int getLevelCardinality(Level level) throws OlapException {
		if (level instanceof MondrianOlap4jLevel) {
			...
			final mondrian.olap.SchemaReader schemaReader = olap4jLevel.olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData.olap4jConnection
	...
	public void getSchemaFunctionList(List<FunctionDefinition> funDefs,
		...
		final FunTable funTable = ((MondrianOlap4jSchema) schema).schema
				.getFunTable();
		...
		for (FunInfo fi : funTable.getFunInfoList()) {
			...
			if ((paramCategories == null) || (paramCategories.length == 0)) {
				funDefs.add(new FunctionDefinition(fi.getName(), description, "(none)",
						1, 1,
				...
				for (int i = 0; i < paramCategories.length; i++) {
					...
					RowsetDefinition.MdschemaFunctionsRowset.VarType varType = RowsetDefinition.MdschemaFunctionsRowset.VarType
							.forCategory(returnCategory);
					funDefs.add(new FunctionDefinition(fi.getName(), description, buf
							.toString(),
	...
	public int getHierarchyCardinality(Hierarchy hierarchy) throws OlapException {
		final MondrianOlap4jHierarchy olap4jHierarchy = (MondrianOlap4jHierarchy) hierarchy;
		final mondrian.olap.SchemaReader schemaReader = olap4jHierarchy.olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData.olap4jConnection
				.getMondrianConnection().getSchemaReader().withLocus();
		return RolapMemberBase.getHierarchyCardinality(schemaReader,
				olap4jHierarchy.hierarchy);
	}
	...
	public int getMeasureAggregator(Member member) {
		...
		Object aggProp = olap4jMeasure.member
				.getPropertyValue(Property.AGGREGATION_TYPE.name);
		if (aggProp == null) {
			return RowsetDefinition.MdschemaMeasuresRowset.MDMEASURE_AGGR_CALCULATED;
		}
	...
	public void checkMemberOrdinal(Member member) throws OlapException {
		if (member.getOrdinal() == -1) {
			...
			final mondrian.olap.SchemaReader schemaReader = olap4jMember.olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData.olap4jConnection
					.getMondrianConnection().getSchemaReader().withLocus();
	...
	public boolean shouldReturnCellProperty(CellSet cellSet,
			org.olap4j.metadata.Property cellProperty, boolean evenEmpty) {
		MondrianOlap4jCellSet olap4jCellSet = (MondrianOlap4jCellSet) cellSet;
		Query query = olap4jCellSet.query;
		return (evenEmpty && query.isCellPropertyEmpty())
	...
	public String getCubeType(Cube cube) {
		return (cube instanceof MondrianOlap4jCube)
				&& ((RolapCube) ((MondrianOlap4jCube) cube).cube).isVirtual() ? RowsetDefinition.MdschemaCubesRowset.MD_CUBTYPE_VIRTUAL_CUBE
	...
	public List<Map<String, Object>> getDataSources(OlapConnection connection)
		...
		MondrianServer server = MondrianServer.forConnection(olap4jConnection
