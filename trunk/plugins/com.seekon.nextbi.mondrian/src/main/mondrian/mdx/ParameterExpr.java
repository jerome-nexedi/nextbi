/*
// $Id: //open/mondrian/src/main/mondrian/mdx/ParameterExpr.java#9 $
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/mdx/ParameterExpr.java#9 $
 */
public class ParameterExpr extends ExpBase {
  ...
  public Exp accept(Validator validator) {
    ...
    final SchemaReader schemaReader = validator.getQuery().getSchemaReader(false);
    Parameter p = schemaReader.getParameter(parameterName);
    if (p == null) {
      this.parameter = validator.createOrLookupParam(true, parameter.getName(),
        parameter.getType(), parameter.getDefaultExp(), parameter.getDescription());
    } else {
      this.parameter = p;
    }
    return this;
   ...
   * <code>Parameter(paramName, description, defaultValue)</code> function, and
   * subsequent usages become calls to <code>ParamRef(paramName)</code>
   * 
   * @param pw
   *          PrintWriter
   */
  ...
  /**
   * Returns the unique name of the level, hierarchy, or dimension of this type,
