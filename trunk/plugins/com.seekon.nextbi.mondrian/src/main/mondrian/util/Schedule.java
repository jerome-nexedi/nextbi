/*
// $Id: //open/mondrian/src/main/mondrian/util/Schedule.java#12 $
 ...
 * <li>{@link #createMonthlyByWeek}.</li>
 * </ul>
 * 
 * <p>
 * Then use the {@link #nextOccurrence} method to find the next occurrence after
 * a particular point in time.
 * 
 * <p>
 * The <code>begin</code> and <code>end</code> parameters represent the points
 * in time between which the schedule is active. Both are optional. However, if
 * a schedule type supports a <code>period</code> parameter, and you supply a
 * value greater than 1, <code>begin</code> is used to determine the start of
 * the cycle. If <code>begin</code> is not specified, the cycle starts at the
 * epoch (January 1st, 1970).
 * 
 * <p>
 * The {@link Date} parameters in this API -- <code>begin</code> and
 * <code>end</code>, the <code>time</code> parameter to {@link #createOnce}, and
 * the <code>earliestDate</code> parameter and value returned from
 * {@link #nextOccurrence} -- always represent a point in time (GMT), not a
 * local time. If a schedule is to start at 12 noon Tokyo time, April 1st, 2002,
 * it is the application's reponsibility to convert this into a UTC {@link Date}
 * value.
 * 
 * @author jhyde
 * @since May 7, 2002
 * @version $Id: //open/mondrian/src/main/mondrian/util/Schedule.java#12 $
 */
public class Schedule {
  ...
  static final int allDaysOfWeekBitmap = (1 << Calendar.MONDAY)
    | (1 << Calendar.TUESDAY) | (1 << Calendar.WEDNESDAY) | (1 << Calendar.THURSDAY)
    | (1 << Calendar.FRIDAY) | (1 << Calendar.SATURDAY) | (1 << Calendar.SUNDAY);
   ...
   * Schedule.
   */
  private Schedule(DateSchedule dateSchedule, TimeSchedule timeSchedule,
    TimeZone tz, Date begin, Date end) {
   ...
   * 
   * @param date
   *          date and time to fire, must be UTC
   * @param tz
   *          timezone
   * 
   * @pre tz != null
  ...
  public static Schedule createOnce(Date date, TimeZone tz) {
    ...
    Time timeOfDay = ScheduleUtil.createTime(calendar.get(Calendar.HOUR_OF_DAY),
   ...
   * @param end
   *          closed upper bound, may be null, must be UTC
   * @param tz
   *          timezone
   * @param timeOfDay
   *          time at which to fire
   * @param period
   *          causes the schedule to fire every <code>period</code> days. If
   *          <code>period</code> is greater than 1, the cycle starts at the
   *          begin point of the schedule, or at the epoch (1 January, 1970) if
   *          <code>begin</code> is not specified.
   * 
   * @pre tz != null
   * @pre period > 0
   * @post return != null
   */
  public static Schedule createDaily(Date begin, Date end, TimeZone tz,
    Time timeOfDay, int period) {
    DateSchedule dateSchedule = new DailyDateSchedule(begin == null ? null
      : ScheduleUtil.createCalendar(begin), period);
    return new Schedule(dateSchedule, new OnceTimeSchedule(ScheduleUtil
      .createTimeCalendar(timeOfDay)), tz, begin, end);
  }
   ...
   * @param tz
   *          timezone
   * @param daysOfWeekBitmap
   *          a bitmap of day values, for example
   *          <code>(1 << {@link Calendar#TUESDAY}) |
   *           (1 << {@link Calendar#THURSDAY})</code> to fire on Tuesdays and
   *          Thursdays
   * @param timeOfDay
   *          time at which to fire
   * @param begin
   ...
   * @param end
   *          closed upper bound, may be null
   * @param period
   *          causes the schedule to be active every <code>period</code> weeks.
   *          If <code>period</code> is greater than 1, the cycle starts at the
   *          begin point of the schedule, or at the epoch (1 January, 1970) if
   *          <code>begin</code> is not specified.
   * 
   * @pre tz != null
   ...
   * @post return != null
   */
  public static Schedule createWeekly(Date begin, Date end, TimeZone tz,
    Time timeOfDay, int period, int daysOfWeekBitmap) {
    DateSchedule dateSchedule = new WeeklyDateSchedule(begin == null ? null
      : ScheduleUtil.createCalendar(begin), period, daysOfWeekBitmap);
    return new Schedule(dateSchedule, new OnceTimeSchedule(ScheduleUtil
      .createTimeCalendar(timeOfDay)), tz, begin, end);
  }

  /**
   * Creates a calendar which fires on particular days of each month. For
   * example,<blockquote>
   * 
   * <pre>
   * createMonthlyByDay(
   *     null, null, TimeZone.getTimeZone("PST"), 1,
   *     (1 << 12) | (1 << 14) | (1 << {@link #LAST_DAY_OF_MONTH}))
   * </pre>
   * 
   * </blockquote> creates a schedule which fires on the 12th, 14th and last day
   * of the month.
   * 
   * @param begin
   *          open lower bound, may be null
   ...
   * @param tz
   *          timezone
   * @param daysOfMonthBitmap
   *          a bitmap of day values, may include {@link #LAST_DAY_OF_MONTH}
   * @param timeOfDay
   *          time at which to fire
   * @param period
   *          causes the schedule to be active every <code>period</code> months.
   *          If <code>period</code> is greater than 1, the cycle starts at the
   *          begin point of the schedule, or at the epoch (1 January, 1970) if
   *          <code>begin</code> is not specified.
   * 
   ...
   * @post return != null
   */
  public static Schedule createMonthlyByDay(Date begin, Date end, TimeZone tz,
    Time timeOfDay, int period, int daysOfMonthBitmap) {
    DateSchedule dateSchedule = new MonthlyByDayDateSchedule(begin == null ? null
      : ScheduleUtil.createCalendar(begin), period, daysOfMonthBitmap);
    return new Schedule(dateSchedule, new OnceTimeSchedule(ScheduleUtil
      .createTimeCalendar(timeOfDay)), tz, begin, end);
  }

  /**
   * Creates a calendar which fires on particular days of particular weeks of a
   * month. For example,<blockquote>
   * 
   * <pre>
   * createMonthlyByWeek(
   *     null, null, TimeZone.getTimeZone("PST"),
   *     (1 << Calendar.TUESDAY) | (1 << Calendar.THURSDAY),
   *     (1 << 2) | (1 << {@link #LAST_WEEK_OF_MONTH})
   * </pre>
   * 
   ...
   * @param weeksOfMonthBitmap
   *          a bitmap of week values (may include {@link #LAST_WEEK_OF_MONTH}
   * @param timeOfDay
   *          time at which to fire
   * @param period
   *          causes the schedule be active every <code>period</code> months. If
   *          <code>period</code> is greater than 1, the cycle starts at the
   *          begin point of the schedule, or at the epoch (1 January, 1970) if
   *          <code>begin</code> is not specified.
   * 
   ...
   * @post return != null
   */
  public static Schedule createMonthlyByWeek(Date begin, Date end, TimeZone tz,
    Time timeOfDay, int period, int daysOfWeekBitmap, int weeksOfMonthBitmap) {
    DateSchedule dateSchedule = new MonthlyByWeekDateSchedule(begin == null ? null
      : ScheduleUtil.createCalendar(begin), period, daysOfWeekBitmap,
      weeksOfMonthBitmap);
    return new Schedule(dateSchedule, new OnceTimeSchedule(ScheduleUtil
      .createTimeCalendar(timeOfDay)), tz, begin, end);
   ...
   * <code>after</code> is null, returns the first occurrence. If there are no
   * further occurrences, returns null.
   * 
   * @param after
   *          if not null, returns the first occurrence after this point in
   *          time; if null, returns the first occurrence ever.
   * @param strict
   *          If <code>after</code> is an occurrence, <code>strict</code>
   *          determines whether this method returns it, or the next occurrence.
   *          If <code>strict</code> is true, the value returned is strictly
   *          greater than <code>after</code>.
   */
...
interface TimeSchedule {
   ...
   * 
   * @param strict
   *          if true, return time must be after <code>after</code>, not equal
   *          to it
   */
...
class DailyDateSchedule implements DateSchedule {
  ...
  DailyDateSchedule(Calendar begin, int period) {
    ...
    this.beginOrdinal = ScheduleUtil.julianDay(begin == null ? ScheduleUtil.epochDay
      : begin);
  }
...
/**
 * A <code>WeeklyDateSchedule</code> fires every week. A bitmap indicates which
 * days of the week it fires.
 */
class WeeklyDateSchedule implements DateSchedule {
  ...
  WeeklyDateSchedule(Calendar begin, int period, int daysOfWeekBitmap) {
    ...
    ScheduleUtil.assertTrue((daysOfWeekBitmap & Schedule.allDaysOfWeekBitmap) != 0,
  ...
  public Calendar nextOccurrence(Calendar earliest, boolean strict) {
    ...
    throw ScheduleUtil
      .newInternal("weekly date schedule is looping -- maybe the bitmap is empty: "
        + daysOfWeekBitmap);
  }
}
...
class MonthlyByDayDateSchedule implements DateSchedule {
  ...
  MonthlyByDayDateSchedule(Calendar begin, int period, int daysOfMonthBitmap) {
    ...
    ScheduleUtil.assertTrue(
      (daysOfMonthBitmap & Schedule.allDaysOfMonthBitmap) == daysOfMonthBitmap,
  ...
  public Calendar nextOccurrence(Calendar earliest, boolean strict) {
    ...
    throw ScheduleUtil
      .newInternal("monthly-by-day date schedule is looping -- maybe "
        + "the bitmap is empty: " + daysOfMonthBitmap);
  }

  private static int monthOrdinal(Calendar earliest) {
    return earliest.get(Calendar.YEAR) * 12 + earliest.get(Calendar.MONTH);
  }
}
...
class MonthlyByWeekDateSchedule implements DateSchedule {
  ...
  MonthlyByWeekDateSchedule(Calendar begin, int period, int daysOfWeekBitmap,
    int weeksOfMonthBitmap) {
    this.period = period;
    ScheduleUtil.assertTrue(period > 0, "period must be positive");
    this.beginMonth = begin == null ? 0 : monthOrdinal(begin);
    ...
    ScheduleUtil.assertTrue(
      (weeksOfMonthBitmap & Schedule.allWeeksOfMonthBitmap) == weeksOfMonthBitmap,
      "week of month schedule has bad bits set: " + weeksOfMonthBitmap);
  }
  ...
  public Calendar nextOccurrence(Calendar earliest, boolean strict) {
    ...
    while (i-- > 0) {
      ...
      if ((daysOfWeekBitmap & (1 << dayOfWeek)) != 0) {
        ...
        if ((weeksOfMonthBitmap & (1 << Schedule.LAST_WEEK_OF_MONTH)) != 0) {
...
class ScheduleUtil {
  ...
  public static boolean isTime(Calendar calendar) {
    return calendar.get(Calendar.YEAR) == ScheduleUtil.epochDay.get(Calendar.YEAR)
      && calendar.get(Calendar.DAY_OF_YEAR) == ScheduleUtil.epochDay
  ...
  public static Calendar getTime(Calendar calendar) {
    ...
    return createTimeCalendar(calendar.get(Calendar.HOUR_OF_DAY), calendar
  ...
  /**
   * Creates a calendar in UTC, and initializes it to a given year, month, day,
   * hour, minute, second. <b>NOTE: month is 1-based</b>
   */
  ...
  public static Time createTime(int hour, int minutes, int second) {
    return new Time(createTimeCalendar(hour, minutes, second).getTime().getTime());
  }

  /**
   * Returns the julian day number of a given date. (Is there a better way to do
   * this?)
   */
  public static int julianDay(Calendar calendar) {
    int year = calendar.get(Calendar.YEAR), day = calendar.get(Calendar.DAY_OF_YEAR), leapDays = (year / 4)
      - (year / 100) + (year / 400);
  ...
  public static int timezoneOffset(TimeZone tz, Calendar calendar) {
    return tz.getOffset(calendar.get(Calendar.ERA), calendar.get(Calendar.YEAR),
      calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH), calendar
        .get(Calendar.DAY_OF_WEEK), (1000 * (60 * (60 * calendar
