/*
// $Id: //open/mondrian/src/main/mondrian/util/ServiceDiscovery.java#5 $
 ...
 * 
 * <p>
 * Java services are described in <a href=
 * "http://java.sun.com/j2se/1.5.0/docs/guide/jar/jar.html#Service%20Provider">
 * the JAR File Specification</a>.
 * 
 * <p>
 * Based on the suggested file format, this class reads the service entries in a
 * JAR file and discovers implementors of an interface.
 * 
 * @author Marc Batchelor
 * @version $Id: //open/mondrian/src/main/mondrian/util/ServiceDiscovery.java#5
 *          $
 */
public class ServiceDiscovery<T> {
    ...
    try {
      ...
      while (urlEnum.hasMoreElements()) {
        ...
        try {
          ...
          logger.warn("Error while finding service file " + resourceURL + " for "
            + theInterface, e);
        } finally {
          if (is != null) {
            is.close();
   ...
   * 
   * @param clazz
   *          Class name (or list of class names)
   * @param cLoader
   *          Class loader
   * @param uniqueClasses
   *          Set of classes (output)
   */
  protected void parseImplementor(String clazz, ClassLoader cLoader,
    ...
    try {
      ...
      if (theInterface.isAssignableFrom(interfaceImplementor)) {
        ...
        logger.error("Class " + interfaceImplementor
          + " cannot be assigned to interface " + theInterface);
      }
