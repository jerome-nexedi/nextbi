/*
// $Id: //open/mondrian/src/main/mondrian/util/AbstractMemoryMonitor.java#10 $
...
/**
 * Abstract implementation of {@link MemoryMonitor}. Base class for different
 * memory monitoring strategies.
 * 
 * @author Richard M. Emberson
 * @since Feb 03 2007
 * @version $Id:
 *          //open/mondrian/src/main/mondrian/util/AbstractMemoryMonitor.java#10
 *          $
 */
public abstract class AbstractMemoryMonitor implements MemoryMonitor,
	...
	static class Entry {
		...
		public boolean equals(final Object other) {
			return (other instanceof Entry) && (listener == ((Entry) other).listener);
		}
	 ...
	 * <code>LinkedList</code> was used for quick insertion and removal.
	 */
	private final LinkedList<Entry> listeners;

	/**
	 * The current low threshold level. This is the lowest level of any of the
	 * registered <code>Listener</code>s.
	 */
	...
	public boolean addListener(Listener listener, int percentage) {
		...
		try {
			/*
			 * // Should this listener being added be immediately // notified that
			 * memory is short. boolean notifyNow = (usagePercentage() >= percentage);
			 */
			...
			/*
			 * if (notifyNow) { listener.memoryUsageNotification( getUsedMemory(),
			 * getMaxMemory()); }
			 */
			return true;
		} finally {
	...
	/**
	 * Returns the lowest threshold from the list of <code>Listener</code>s. If
	 * there are no <code>Listener</code>s, then return the maximum memory usage.
	 * Returns <code>Long.MAX_VALUE</code> if there are no <code>Listener</code>s
	 * 
	...
	protected long generateLowThreshold() {
		...
		return listeners.isEmpty() ? Long.MAX_VALUE : listeners.get(0).threshold;
	}
	 ...
	 *          the maximum memory.
	 */
	protected void notifyListeners(final long usedMemory, final long maxMemory) {
		synchronized (listeners) {
	...
	/**
	 * Derived classes implement this method if they wish to be notified when
	 * there is a new lowest threshold.
	 * 
