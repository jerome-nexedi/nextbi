/*
// $Id: //open/mondrian/src/main/mondrian/util/PrimeFinder.java#8 $
...
final class PrimeFinder {
   ...
   * <tt>Integer.MAX_VALUE</tt>. yes, it is prime.
   */
   ...
   * numbers. A chunk starts with a prime P1. The next element is a prime P2. P2
   * is the smallest prime for which holds: P2 >= 2*P1. The next element is P3,
   * for which the same holds with respect to P2, and so on.
   * 
   ...
   * capacity >= 200 the list includes a prime number <= desired capacity * 1.16
   * (16%). For any desired capacity >= 16 the list includes a prime number <=
   * desired capacity * 1.21 (21%).
   * 
   ...
   * includes 1039,1117,1201,1277,1361,1439,1523,1597,1759,1907,2081. So if you
   * need a prime >= 1040, you will find a prime <= 1040*1.11=1154.
   * 
   * Chunks are chosen such that they are optimized for a hashtable growthfactor
   * of 2.0; If your hashtable has such a growthfactor then, after initially
   * "rounding to a prime" upon hashtable construction, it will later expand to
  ...
  private static final int[] primeCapacities = {
    ...
    // chunk #4
    1039, 2081, 4177, 8363, 16729, 33461, 66923, 133853,
    267713,
    535481,
    1070981,
    2141977,
    ...
    // chunk #5
    31, 67, 137, 277, 557, 1117, 2237, 4481, 8963, 17929, 35863, 71741, 143483,
    286973, 573953, 1147921,
    2295859,
    4591721,
    9183457,
    18366923,
    ...
    // chunk #7
    311, 631, 1277, 2557, 5119, 10243, 20507, 41017, 82037, 164089, 328213, 656429,
    1312867, 2625761, 5251529, 10503061, 21006137,
    42012281,
    84024581,
    168049163,
    336098327,
    ...
    // chunk #8
    3, 7, 17, 37, 79, 163, 331, 673, 1361, 2729, 5471, 10949, 21911, 43853, 87719,
    175447, 350899, 701819, 1403641, 2807303, 5614657, 11229331, 22458671, 44917381,
    89834777, 179669557,
    359339171,
    718678369,
    1437356741,
    ...
    // chunk #9
    43, 89, 179, 359, 719, 1439, 2879, 5779, 11579, 23159, 46327, 92657, 185323,
    370661, 741337, 1482707, 2965421, 5930887, 11861791, 23723597, 47447201,
    94894427, 189788857, 379577741, 759155483,
    1518310967,

    // chunk #10
    379, 761, 1523, 3049, 6101, 12203, 24407, 48817, 97649, 195311, 390647, 781301,
    1562611, 3125257, 6250537, 12501169, 25002389, 50004791, 100009607, 200019221,
    400038451, 800076929, 1600153859

  /*
   * // some more chunks for the low range [3..1000] //chunk #11 13, 29, 59,
   * 127, 257, 521, 1049, 2099, 4201, 8419, 16843, 33703, 67409, 134837, 269683,
   ...
   * 
   * //chunk #13 53, 107, 223, 449, 907, 1823, 3659, 7321, 14653, 29311, 58631,
   * 117269, 234539, 469099, 938207, 1876417, 3752839, 7505681, 15011389,
   * 30022781, 60045577, 120091177, 240182359, 480364727, 960729461, 1921458943
   */
  };
  ...
  /**
   * Returns a prime number which is <code>&gt;= desiredCapacity</code> and very
   * close to <code>desiredCapacity</code> (within 11% if
    ...
    for (int i = 0; i < primeCapacities.length - 1; i++) {
      if (primeCapacities[i] >= primeCapacities[i + 1]) {
        throw new RuntimeException(
          "primes are unsorted or contain duplicates; detected at " + i + "@"
            + primeCapacities[i]);
      }
    }
