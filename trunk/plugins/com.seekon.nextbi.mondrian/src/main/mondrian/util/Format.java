/*
// $Id: //open/mondrian/src/main/mondrian/util/Format.java#44 $
...
/**
 * <code>Format</code> formats numbers, strings and dates according to the same
 * specification as Visual Basic's <code>format()</code> function. This function
 * is described in more detail <a
 * href="http://www.apostate.com/programming/vb-format.html">here</a>. We have
 * made the following enhancements to this specification:
 * <ul>
 * 
 ...
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/util/Format.java#44 $
 */
public class Format {
	...
	/**
	 * Gets the dummy implementation of {@link FieldPosition} which the JDK uses
	 * when you don't care about the status of a call to {@link Format#format}.
	 */
	 ...
	 * 
	 * <p>
	 * If the number of entries in the cache exceeds 1000,
	 */
	...
	static final String[] daysOfWeekShort_en = { "", "Sun", "Mon", "Tue", "Wed",
			"Thu", "Fri", "Sat" };
	static final String[] daysOfWeekLong_en = { "", "Sunday", "Monday",
			"Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
	static final String[] monthsShort_en = { "Jan", "Feb", "Mar", "Apr", "May",
			"Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "", };
	static final String[] monthsLong_en = { "January", "February", "March",
	 ...
	 */
	private static final Map<String, MacroToken> macroTokenMap = new HashMap<String, MacroToken>();
	 ...
	 */
	static final FormatLocale locale_US = createLocale('\0', '\0', null, null,
			null, null, null, null, null, null, Locale.US);
	...
	static class Token {
		...
		Token(int code, int flags, String token) {
			...
			this.flags = flags;
			this.token = token;
			this.formatType = isNumeric() ? FormatType.NUMERIC
					: isDate() ? FormatType.DATE : isString() ? FormatType.STRING : null;
		}

		boolean compatibleWith(FormatType formatType) {
			return formatType == null || this.formatType == null
	 ...
	 * work. Whereas {@link Format} has only one method for formatting,
	 * {@link Format#format(Object)}, this class provides methods for several
	 * primitive types. To make it easy to combine formatting objects, all methods
	 * write to a {@link PrintWriter}.
	 * 
	 * <p>
	 * The base implementation of most of these methods throws; there is no
	 * requirement that a derived class implements all of these methods. It is up
	 * to {@link Format#parseFormatString} to ensure that, for example, the
	 * {@link #format(double,StringBuilder)} method is never called for
	 * {@link DateFormat}.
	 */
	static class BasicFormat {
		 ...
		 * 
		 * <p>
		 * Usually returns true; one notable exception is a format for negative
		 * numbers which causes the number to be underflow to zero and therefore be
	 ...
	 * 
	 * <p>
	 * If there are fewer than 4 formats, the first is used as a fall-back. See
	 * the <a href="http://www.apostate.com/programming/vb-format.html">the visual
	...
	static class AlternateFormat extends BasicFormat {
		...
		void format(double n, StringBuilder buf) {
			int i;
			if (n == 0 && formats.length >= 3 && formats[2] != null) {
				i = 2;
			} else if (n < 0) {
				if (formats.length >= 2 && formats[1] != null) {
					if (formats[1].isApplicableTo(n)) {
						...
						if (formats.length >= 3 && formats[2] != null) {
							i = 2;
						} else {
							i = 0;
						}
		...
		private static BigDecimal daysSince1900(Calendar calendar) {
			...
			final long leapDays = (yearForLeap - 1900) / 4 - (yearForLeap - 1900)
					/ 100 + (yearForLeap - 2000) / 400;
			final long days = (year - 1900) * 365 + leapDays + dayOfYear + 2; // kludge
																																				// factor
																																				// to
																																				// agree
																																				// with
																																				// Excel
			final long millis = calendar.get(Calendar.HOUR_OF_DAY) * 3600000
					+ calendar.get(Calendar.MINUTE) * 60000
					+ calendar.get(Calendar.SECOND) * 1000
					+ calendar.get(Calendar.MILLISECOND);
			return BigDecimal.valueOf(days).add(
					BigDecimal.valueOf(millis).divide(BigDecimal.valueOf(86400000), 8,
	...
	/**
	 * JavaFormat is an implementation of {@link Format.BasicFormat} which prints
	 * values using Java's default formatting for their type. <code>null</code>
	 * values appear as an empty string.
	 */
	 ...
	 * string. Better than giving an error. Abstract base class for NumericFormat
	 * and DateFormat.
	 */
	 ...
	 * 
	 * <p>
	 * It is implemented using {@link FloatingDecimal}, which is a barely-modified
	 * version of <code>java.lang.FloatingDecimal</code>.
	 */
	static class NumericFormat extends JavaFormat {
		...
		NumericFormat(String token, FormatLocale locale, int expFormat,
				int digitsLeftOfPoint, int zeroesLeftOfPoint, int digitsRightOfPoint,
				int zeroesRightOfPoint, int digitsRightOfExp, int zeroesRightOfExp,
			...
			if (macroTokenMap.containsKey(formatString)) {
				...
				if (macroToken.name.equals("Currency")) {
					formatString = locale.currencyFormat + ";(" + locale.currencyFormat
							+ ")";
				} else {
			...
			if (thousandSeparatorTokenMap.containsKey(formatString)) {
				cachedThousandSeparatorPositions = thousandSeparatorTokenMap
						.get(formatString);
			} else {
				...
				final int semiPos = formatStringBuffer
						.indexOf(getFormatToken(FORMAT_SEMI));
				if (semiPos > 0) {
				...
				if (nbThousandSeparators > 1) {
					...
					final int decimalPos = formatStringBuffer
							.indexOf(getFormatToken(FORMAT_DECIMAL));
					final int endIndex = decimalPos == -1 ? formatStringBuffer.length()
							: decimalPos;
					...
					while (st.hasMoreTokens()) {
						cachedThousandSeparatorPositions.push(st.nextToken().length());
					}
				} else if (nbThousandSeparators == 1) {
					// Use old style formatting.
					cachedThousandSeparatorPositions.add(3);
				}
				thousandSeparatorTokenMap.put(formatString,
		...
		void format(double n, StringBuilder buf) {
			...
			final int formatDigitsRightOfPoint = zeroesRightOfPoint
					+ digitsRightOfPoint;
			...
			String s = fd.toJavaFormatString(zeroesLeftOfPoint,
					locale.decimalPlaceholder, zeroesRightOfPoint,
					formatDigitsRightOfPoint, expChar, expSign, zeroesRightOfExp,
					useThouSep ? locale.thousandSeparator : '\0', useDecimal,
		...
		private static boolean shows(FloatingDecimal fd,
		...
		void format(long n, StringBuilder buf) {
			mondrian.util.Format.FloatingDecimal fd = new mondrian.util.Format.FloatingDecimal(
					n);
			fd.shift(decimalShift);
			String s = fd.toJavaFormatString(zeroesLeftOfPoint,
					locale.decimalPlaceholder, zeroesRightOfPoint, zeroesRightOfPoint
							+ digitsRightOfPoint, expChar, expSign, zeroesRightOfExp,
					useThouSep ? locale.thousandSeparator : '\0', useDecimal,
					getThousandSeparatorPositions());
			buf.append(s);
		}
	 ...
	 * value when applied to a {@link Calendar} object. (Values of type
	 * {@link Date} are automatically converted into {@link Calendar}s when you
	 * call {@link Format.BasicFormat#format(Date, StringBuilder)} calls to format
	 * other kinds of values give a runtime error.)
	 * 
	 * <p>
	 * In a typical use of this class, a format string such as "m/d/yy" is parsed
	 * into DateFormat objects for "m", "d", and "yy", and {@link LiteralFormat}
	 * objects for "/". A {@link Format.CompoundFormat} object is created to bind
	 * them together.
	 */
	static class DateFormat extends FallbackFormat {
		...
		private void format(int code, Calendar calendar, StringBuilder buf) {
			switch (code) {
			case FORMAT_C: {
				boolean dateSet = !(calendar.get(Calendar.DAY_OF_YEAR) == 0 && calendar
						.get(Calendar.YEAR) == 0);
				boolean timeSet = !(calendar.get(Calendar.SECOND) == 0
			...
			case FORMAT_H: {
				int h = calendar.get(twelveHourClock ? Calendar.HOUR
	 ...
	 * upon the locale of the end-user. Use {@link Format#createLocale} to make
	 * one.
	 */
	public static class FormatLocale {
		...
		private FormatLocale(char thousandSeparator, char decimalPlaceholder,
				String dateSeparator, String timeSeparator, String currencySymbol,
				String currencyFormat, String[] daysOfWeekShort,
				String[] daysOfWeekLong, String[] monthsShort, String[] monthsLong,
				Locale locale) {
			this.locale = locale;
			...
			if (daysOfWeekShort.length != 8 || daysOfWeekLong.length != 8
					|| monthsShort.length != 13 || monthsLong.length != 13) {
	...
	private static Token nfe(int code, int flags, String token, String purpose,
			String description) {
	 ...
	 * 
	 * @param code
	 *          The code of the token to obtain.
	...
	private static final Token[] tokens = {
			nfe(FORMAT_NULL, NUMERIC, null, "No formatting",
			...
			nfe(FORMAT_UPPER_AM_SOLIDUS_PM,
					DATE,
					"AM/PM",
					null,
			...
			nfe(FORMAT_LOWER_AM_SOLIDUS_PM,
					DATE,
					"am/pm",
					null,
			...
			nfe(FORMAT_E_PLUS_UPPER, NUMERIC | SPECIAL, "E+", "Scientific format",
					"See E-."),
			nfe(FORMAT_E_MINUS_LOWER, NUMERIC | SPECIAL, "e-", "Scientific format",
					"See E-."),
			nfe(FORMAT_E_PLUS_LOWER, NUMERIC | SPECIAL, "e+", "Scientific format",
					"See E-."),
			nfe(FORMAT_LITERAL,
					GENERAL,
					"-",
			...
			nfe(FORMAT_BACKSLASH,
					...
					"Many characters in the format expression have a special meaning "
							...
							+ "<, >, and !)."),
			nfe(FORMAT_QUOTE, GENERAL | SPECIAL, "\"",
			...
			nfe(FORMAT_CHARACTER_OR_NOTHING, STRING, "&", "Character placeholder",
					"Display a character or nothing. If the string has a character in "
							...
							+ "below."),
			nfe(FORMAT_LOWER, STRING | SPECIAL, "<", "Force lowercase",
					"Display all characters in lowercase format."),
			nfe(FORMAT_UPPER, STRING | SPECIAL, ">", "Force uppercase",
					"Display all characters in uppercase format."),
			nfe(FORMAT_FILL_FROM_LEFT, STRING | SPECIAL, "!",
			...
			nfe(FORMAT_INTL_CURRENCY, NUMERIC | SPECIAL, intlCurrencySymbol + "",
					null, "Display the locale's currency symbol."),
			nfe(FORMAT_USD, GENERAL, "USD", null, "Display USD (U.S. Dollars)."),
			nfe(FORMAT_GENERAL_NUMBER, NUMERIC | SPECIAL, "General Number", null,
	 ...
	 *          href="http://www.apostate.com/programming/vb-format.html">this
	...
	public Format(String formatString, FormatLocale locale) {
		...
		while (formatString.length() > 0) {
			formatString = parseFormatString(formatString, alternateFormatList,
					formatType);
		}
		...
		if (alternateFormatList.size() == 0 || alternateFormatList.get(0) == null) {
			...
			BasicFormat[] alternateFormats = alternateFormatList
					.toArray(new BasicFormat[alternateFormatList.size()]);
			format = new AlternateFormat(alternateFormats, locale);
		}
	}

	/**
	 * Constructs a <code>Format</code> in a specific locale, or retrieves one
	 * from the cache if one already exists.
	 * 
	 * <p>
	 * If the number of entries in the cache exceeds {@link #CacheLimit}, replaces
	 * the eldest entry in the cache.
	 * 
	 * @param formatString
	 *          the format string; see <a
	 *          href="http://www.apostate.com/programming/vb-format.html">this
	 ...
	 *          the character used to separate thousands in numbers, or ',' by
	 *          default. For example, 12345 is '12,345 in English, '12.345 in
	 *          French.
	 * @param decimalPlaceholder
	 *          the character placed between the integer and the fractional part
	 *          of decimal numbers, or '.' by default. For example, 12.34 is
	 *          '12.34' in English, '12,34' in French.
	 * @param dateSeparator
	 *          the character placed between the year, month and day of a date
	 *          such as '12/07/2001', or '/' by default.
	 * @param timeSeparator
	 *          the character placed between the hour, minute and second value of
	 *          a time such as '1:23:45 AM', or ':' by default.
	 * @param daysOfWeekShort
	 *          Short forms of the days of the week. The array is 1-based, because
	 *          position {@link Calendar#SUNDAY} (= 1) must hold Sunday, etc. The
	 *          array must have 8 elements. For example {"", "Sun", "Mon", ...,
	 *          "Sat"}.
	 * @param daysOfWeekLong
	 *          Long forms of the days of the week. The array is 1-based, because
	 *          position {@link Calendar#SUNDAY} must hold Sunday, etc. The array
	 *          must have 8 elements. For example {"", "Sunday", ..., "Saturday"}.
	 * @param monthsShort
	 *          Short forms of the months of the year. The array is 0-based,
	 *          because position {@link Calendar#JANUARY} (= 0) holds January,
	 *          etc. For example {"Jan", ..., "Dec", ""}.
	 * @param monthsLong
	 *          Long forms of the months of the year. The array is 0-based,
	 *          because position {@link Calendar#JANUARY} (= 0) holds January,
	 *          etc. For example {"January", ..., "December", ""}.
	 * @param locale
	 *          if this is not null, register that the constructed
	 *          <code>FormatLocale</code> is the default for <code>locale</code>
	 */
	public static FormatLocale createLocale(char thousandSeparator,
			char decimalPlaceholder, String dateSeparator, String timeSeparator,
			String currencySymbol, String currencyFormat, String[] daysOfWeekShort,
			String[] daysOfWeekLong, String[] monthsShort, String[] monthsLong,
			Locale locale) {
		FormatLocale formatLocale = new FormatLocale(thousandSeparator,
				decimalPlaceholder, dateSeparator, timeSeparator, currencySymbol,
				currencyFormat, daysOfWeekShort, daysOfWeekLong, monthsShort,
				monthsLong, locale);
		if (locale != null) {
	...
	public static FormatLocale createLocale(Locale locale) {
		...
		final java.text.DateFormat dateFormat = java.text.DateFormat
		...
		final java.text.DateFormat timeFormat = java.text.DateFormat
		...
		String currencyLeft = currencyValue
				.substring(0, currencyValue.indexOf("1"));
		String currencyRight = currencyValue
				.substring(currencyValue.indexOf("8") + 1);
		...
		if (currencyFormat.getMaximumFractionDigits() > 0) {
			...
			appendTimes(buf, '#', currencyFormat.getMaximumFractionDigits()
		...
		/*
		 * If the locale passed is only a language, Java cannot resolve the currency
		 * symbol and will instead return u00a4 (The international currency symbol).
		 * For those cases, we use the default system locale currency symbol.
		 */
		String currencySymbol = decimalSymbols.getCurrencySymbol();
		if (currencySymbol.equals(Format.intlCurrencySymbol + "")) {
			final DecimalFormatSymbols defaultDecimalSymbols = new DecimalFormatSymbols(
					Locale.getDefault());
			currencySymbol = defaultDecimalSymbols.getCurrencySymbol();
		}

		return createLocale(decimalSymbols.getGroupingSeparator(),
				decimalSymbols.getDecimalSeparator(), dateSeparator, timeSeparator,
				currencySymbol, currencyFormatString, dateSymbols.getShortWeekdays(),
				dateSymbols.getWeekdays(), dateSymbols.getShortMonths(),
				dateSymbols.getMonths(), locale);
	}
	 ...
	 * previous mapping.
	 */
	public static FormatLocale registerFormatLocale(FormatLocale formatLocale,
			Locale locale) {
	...
	/**
	 * Reads formatString up to the first semi-colon, or to the end if there are
	 * no semi-colons. Adds a format to alternateFormatList, and returns the
	 * remains of formatString.
	 */
	private String parseFormatString(String formatString,
			List<BasicFormat> alternateFormatList, FormatType[] formatTypeOut) {
		...
		if (macroTokenMap.containsKey(formatString)) {
			...
			if (macroToken.translation == null) {
				...
				if (macroToken.name.equals("Currency")) {
					...
					throw new Error("Format: internal: token " + macroToken.name
		...
		loop: while (formatString.length() > 0) {
			...
			for (int i = tokens.length - 1; i > 0; i--) {
				...
				if (token.isSpecial()) {
					...
					case FORMAT_MM: {
						...
						if (theyMeantMinute) {
							format = new DateFormat((token.code == FORMAT_M ? FORMAT_N
									: FORMAT_NN), matched, locale, false);
						} else {
			...
			if (format != null) {
				if (numberState != NOT_IN_A_NUMBER) {
					...
					NumericFormat numericFormat = new NumericFormat(prevIgnored, locale,
							expFormat, digitsLeftOfPoint, zeroesLeftOfPoint,
							digitsRightOfPoint, zeroesRightOfPoint, digitsRightOfExp,
		...
		if (numberState != NOT_IN_A_NUMBER) {
			...
			NumericFormat numericFormat = new NumericFormat(prevIgnored, locale,
					expFormat, digitsLeftOfPoint, zeroesLeftOfPoint, digitsRightOfPoint,
					zeroesRightOfPoint, digitsRightOfExp, zeroesRightOfExp, useDecimal,
					useThouSep, originalFormatString);
			formatList.add(numericFormat);
			numberState = NOT_IN_A_NUMBER;
			haveSeenNumber = true;
		}
		...
		// special powers.
		for (int i = 0; i < formatList.size(); ++i) {
			if (i > 0 && formatList.get(i) instanceof LiteralFormat
					&& formatList.get(i - 1) instanceof LiteralFormat) {
				formatList.set(i - 1,
						new LiteralFormat(((LiteralFormat) formatList.get(i - 1)).s
		...
		case 1:
			...
			break;
		default:
			alternateFormat = new CompoundFormat(
	...
	static class FloatingDecimal {
		 ...
		 * 
		 * <p>
		 * In particular: lvalue is a finite number (not Inf, nor NaN) lvalue > 0L
		...
		private void dtoa(int binExp, long fractBits, int nSignificantBits) {
			...
			if (binExp <= maxSmallBinExp && binExp >= minSmallBinExp) {
				...
				if ((nTinyBits < long5pow.length)
					...
					/*
					 * We can do this: take the fraction bits, which are normalized. (a)
					 * nTinyBits == 0: Shift left or right appropriately to align the
					 * binary point at the extreme right, i.e. where a long int point is
					 * expected to be. The integer result is easily converted to a string.
					 * (b) nTinyBits > 0: Shift right by expShift-nFractBits, which
					 * effectively converts to long and scales by 2^nTinyBits. Then
					 * multiply by 5^nTinyBits to complete the scaling. We know this won't
					 * overflow because we just counted the number of bits necessary in
					 * the result. The integer you get from this can then be converted to
					 * a string pretty easily.
					 */
					long halfULP;
					if (nTinyBits == 0) {
					 ...
					 * single-float case. Our manipulation of halfULP here is apparently
					 * not correct. If we better understand how this works, perhaps we can
					 * use this special case again. But for the time being, we do not.
					 * else { fractBits >>>= expShift + 1-nFractBits; fractBits *=
					 * long5pow[ nTinyBits ]; halfULP = long5pow[ nTinyBits ] >> (1 +
					 * nSignificantBits - nFractBits); developLongDigits(-nTinyBits,
					 * fractBits, halfULP); return; }
					 */
				}
			}
			/*
			 * This is the hard case. We are going to compute large positive integers
			 * B and S and integer decExp, s.t. d = (B / S) * 10^decExp 1 <= B / S <
			 * 10 Obvious choices are: decExp = floor(log10(d)) B = d * 2^nTinyBits *
			 * 10^max(0, -decExp) S = 10^max(0, decExp) * 2^nTinyBits (noting that
			 * nTinyBits has already been forced to non-negative) I am also going to
			 * compute a large positive integer M = (1/2^nSignificantBits) *
			 * 2^nTinyBits * 10^max(0, -decExp) i.e. M is (1/2) of the ULP of d,
			 * scaled like B. When we iterate through dividing B/S and picking off the
			 * quotient bits, we will know when to stop when the remainder is <= M.
			 * 
			 ...
			 * to estimate log10(d2) ~=~ (d2-1.5)/1.5 + log(1.5) and so we can
			 * estimate log10(d) ~=~ log10(d2) + binExp * log10(2) take the floor and
			 * call it decExp. FIXME -- use more precise constants here. It costs no
			 * more.
			 */
			 ...
			 * power-of-two bounds) for this reason, we hack M2. Hope this works.
			 */
			if (nFractBits == 1) {
				M2 -= 1;
			}
			 ...
			 * takes account of the assymetry of the spacing of floating-point numbers
			 * below perfect powers of 2 26 Sept 96 is not that day. So we use a
			 * symmetric test.
			 */
			...
			/*
			 * Detect the special cases where all the numbers we are about to compute
			 * will fit in int or long integers. In these cases, we will avoid doing
			 * FDBigInt arithmetic. We use the same algorithms, except that we
			 * "normalize" our FDBigInts before iterating. This is to make division
			 * easier, as it makes our fist guess (quotient of high-order words) more
			 * accurate!
			 * 
			 * Some day, we'll write a stopping test that takes account of the
			 * assymetry of the spacing of floating-point numbers below perfect powers
			 * of 2 26 Sept 96 is not that day. So we use a symmetric test.
			 */
			...
			if (Bbits < 64 && tenSbits < 64) {
				...
				/*
				 * We really must do FDBigInt arithmetic. Fist, construct our FDBigInt
				 * initial values.
				 */
				...
				/*
				 * Unroll the first iteration. If our decExp estimate was too high, our
				 * first quotient will be zero. In this case, we discard it and
				 * decrement decExp.
				 */
				ndigit = 0;
				...
				if (q >= 10) {
					// bummer, dude
					throw new RuntimeException("Assertion botch: excessivly large digit "
							+ q);
				} else if ((q == 0) && !high) {
					// oops. Usually ignore leading zero.
					decExp--;
				...
				/*
				 * HACK! Java spec sez that we always have at least one digit after the
				 * . in either F- or E-form output. Thus we will need more than one
				 * digit if we're using E-form
				 */
				if (decExp <= -3 || decExp >= 8) {
					high = low = false;
				}
		...
		public void shift(int i) {
			if (isExceptional || nDigits == 1 && digits[0] == '0') {
				; // don't multiply zero
			} else {
				decExponent += i;
			}
		...
		public String toJavaFormatString(int minDigitsLeftOfDecimal,
				char decimalChar, // '.' or ','
				int minDigitsRightOfDecimal, int maxDigitsRightOfDecimal, // todo: use
				char expChar, // 'E' or 'e'
				boolean expSign, // whether to print '+' if exp is positive
				int minExpDigits, // minimum digits in exponent
				char thousandChar, // ',' or '.', or 0
				boolean useDecimal, ArrayStack<Integer> thousandSeparatorPositions) {
			...
			int resultLen = 10 + Math.abs(decExponent) * 4 / 3
					+ maxDigitsRightOfDecimal;
			char result[] = new char[resultLen];
			int i = toJavaFormatString(result, 0, minDigitsLeftOfDecimal,
					decimalChar, minDigitsRightOfDecimal, maxDigitsRightOfDecimal,
					expChar, expSign, minExpDigits, thousandChar, useDecimal,
					thousandSeparatorPositions);
			return new String(result, 0, i);
		}

		// jhyde added
		private synchronized int toJavaFormatString(char result[], int i,
				int minDigitsLeftOfDecimal, char decimalChar, // '.' or ','
				int minDigitsRightOfDecimal, int maxDigitsRightOfDecimal, // todo: use
				char expChar, // 'E' or 'e'
				boolean expSign, // whether to print '+' if exp is positive
				int minExpDigits, // minimum digits in exponent
			...
			if (isExceptional) {
				...
				int wholeDigits = Math.max(decExponent, minDigitsLeftOfDecimal), fractionDigits = Math
						.max(nDigits - decExponent, minDigitsRightOfDecimal), totalDigits = wholeDigits
						+ fractionDigits;
				...
				int firstNonZero = wholeDigits, firstTrailingZero = 0;
				for (int j = 0; j < totalDigits; j++) {
					if (digits2[j] != '0') {
						if (j < firstNonZero) {
							firstNonZero = j;
				...
				if (thousandChar != '\0' && thousandSeparatorPositions.size() > 0) {
					// Now print the number. That will happen backwards, so we
					// store it temporarely and then invert.
					ArrayStack<Character> formatedWholeDigits = new ArrayStack<Character>();
				...
				if (wholeDigits < lastDigitToPrint
						|| (useDecimal && wholeDigits == lastDigitToPrint)) {
				...
				isNegative = false;
				i = toJavaFormatString(result, i, minDigitsLeftOfDecimal, decimalChar,
						minDigitsRightOfDecimal, maxDigitsRightOfDecimal, (char) 0, false,
						minExpDigits, '\0', useDecimal, thousandSeparatorPositions);
				decExponent = oldExp;
	...
	/**
	 * A really, really simple bigint package tailored to the needs of floating
	 * base conversion.
	 */
	static class FDBigInt {
		...
		public int normalizeMe() throws IllegalArgumentException {
			...
			/*
			 * In most cases, we assume that wordcount is zero. This only makes sense,
			 * as we try not to maintain any high-order words full of zeros. In fact,
			 * if there are zeros, we will simply SHORTEN our number at this point.
			 * Watch closely...
			 */
			nWords -= wordcount;
			/*
			 * Compute how far left we have to shift v s.t. its highest- order bit is
			 * in the right place. Then call lshiftMe to do the work.
			 */
			if ((v & 0xf0000000) != 0) {
		...
		/**
		 * Compute q = (int)(this / S) this = 10 * (this mod S) Return q. This is
		 * the iteration step of digit development for output. We assume that S has
		 * been normalized, as above, and that "this" has been lshift'ed
		...
		public int quoRemIteration(FDBigInt S) throws IllegalArgumentException {
			...
			if (diff != 0L) {
				...
				while (sum == 0L) {
					...
					/*
					 * Originally the following line read "if (sum !=0 && sum != -1)" but
					 * that would be wrong, because of the treatment of the two values as
					 * entirely unsigned, it would be impossible for a carry-out to be
					 * interpreted as -1 -- it would have to be a single-bit carry-out, or
					 * + 1.
					 */
					if (sum != 0 && sum != 1) {
