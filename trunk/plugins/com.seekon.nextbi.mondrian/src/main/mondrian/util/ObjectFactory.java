/*
// $Id: //open/mondrian/src/main/mondrian/util/ObjectFactory.java#17 $
...
/**
 * Concrete derived classes of the generic <code>ObjectFactory</code> class are
 * used to produce an implementation of an interface (a normal interface
 * implementation or a Proxy). In general, a factory should produce a default
 * implementation for general application use as well as particular
 * implementations used during testing. During testing of application code and
 * during normal execution, the application code uses one of the
 * <code>ObjectFactory</code>'s methods for producing implementation instances -
 * the same method is used both for test and non-test modes. There are two ways
 * of modifying the implementation returned to the application code. The first
 * is for the application to use Properties. The <code>ObjectFactory</code>
 * implementation looks for a given property (by default the name of the
 * property is the class name of the interfaceClass object) and if found uses it
 * as the classname to create. A second approach is to use a ThreadLocal; if the
 * ThreadLocal is non-empty then use it as the classname.
 * <p>
 * When to use a Factory?
 * <p>
 * Everyone has an opinion. For me, there are two criteria: enabling unit
 * testing and providing end-user/developer-customizer overriding.
 * <p>
 * If a method has side-effects, either its result depends upon a side-effect or
 * calling it causes a side-effect, then the Object hosting the method is a
 * candidate for having a factory. Why? Well, consider the case where a method
 * returns the value of a System property and the System property is determined
 * only once and set to a static final variable:
 * 
 * <pre>
 *      class OneValue {
 ...
 * </pre>
 * <p>
 * In this case, only one value is ever returned. If you have a module, a client
 * of the above code, that uses the value returned by a call to the
 * <code>hasInfo()</code> method, how do you write a unit test of your module
 * that tests both possible return values? You can not, its value is based upon
 * a side-effect, an external value that can not be controled by the unit test.
 ...
 * <code>OneValue</code> interface was returned and in one test arrange that
 * <code>true</code> was returned and in a second test, arrange that
 * <code>false</code> was returned.
 * <p>
 * The above is a trivial example of code that disallows clients of the code
 * from being properly tested.
 * <p>
 * Another example might be a module that directly initializes a JMS queue and
 * receives JMS message from the JMS queue. This code can not be tested without
 * having a live JMS queue. On the other hand, if one defines an interface
 * allowing one to wrap access to the JMS queue and accesses the implementation
 * via a factory, then unit tests can be create that use a mock JMS queue.
 * <p>
 * With regards to providing end-user/developer-customizer overriding, its
 * generally good to have a flexible application framework. Experimental or just
 * different implementations can be developed and tested without having to touch
 * a lot of the application code itself.
 * <p>
 * There is, of course, a trade-off between the use of a factory and the size or
 * simplicity of the object being created.
 * <p>
 * What are the requirements for a template ObjectFactory?
 * <p>
 * First, every implementation must support the writing of unit tests. What this
 * means it that test cases can override what the factory produces. The test
 * cases can all use the same produced Object or each can request an Object
 * targeted to its particular test. All this without changing the
 * <code>default</code> behavior of the factory.
 * <p>
 * Next, it should be possible to create a factory from the template that is
 * intended to deliver the same Object each time it is called, a different, new
 * Object each time it is called, or, based on the calling environment
 * (parameters, properties, <code>ThreadLocal</code>, etc.) one of a set of
 * Objects. These are possible <code>default</code> behaviors, but, again, they
 * can be overridden for test purposes.
 * <p>
 * While a factory has a <code>default</code> behavior in an application, it
 * must be possible for every factory's behavior in that application to be
 * globally overridden. What that means is if the application designer has
 * dictated a <code>default</code>, the application user should be able to
 * change the default. An example of this is overriding what Object is returned
 * based upon a <code>System</code> property value.
 * <p>
 * Lastly, every factory is a singleton - if an interface with an implementation
 * whose creation is mediated by a factory, then there is a single factory that
 * does that creating. This does not mean that such a factory always return the
 * same value, rather that there is only one instance of the factory itself.
 * <p>
 * The following is an example class that generates a factory singleton. In this
 * case, the factory extends the <code>ObjectFactory</code> rather than the
 * <code>ObjectFactory.Singleton</code>:
 * 
 * <pre>
 * 
 ...
 * </pre>
 * <p>
 * There are multiple ways of creating derived classes that have support for
 * unit testing. A very simple way is to use <code>ThreadLocal</code>s.
 * 
 * <pre>
 * 
 ...
 * </pre>
 * <p>
 * Here, the unit test will call the <code>setThreadLocalClassName</code> method
 * setting it with the class name of a specialized implemetation of the template
 * interface. In the <code>finally</code> clause of the unit test, it is very
 * important that there be a call to the <code>clearThreadLocalClassName</code>
 * method so that other tests, etc. do not get an instance of the test-specific
 ...
 * </pre>
 * <p>
 * While this is a very simple example, it shows how using such factories can
 * aid in creating testable code. The MyCode method is a client of the Boo
 * implementation. How to test the two different code branches the method can
 * take? Because the Boo object is generated by a factory, one can override what
 * object the factory returns.
 * 
 * @author Richard M. Emberson
 * @since Feb 01 2007
 * @version $Id: //open/mondrian/src/main/mondrian/util/ObjectFactory.java#17 $
 */
