/*
// $Id: //open/mondrian/src/main/mondrian/util/LockBox.java#1 $
 ...
 * and where tricks such as {@link ThreadLocal} do not work. The callee needs to
 * be on the same JVM, but other than that, the object does not need to have any
 * special properties. In particular, it does not need to be serializable.
 * 
 * <p>
 * First, register the object to obtain a lock box entry. Every lock box entry
 * has a string moniker that is very difficult to guess, is unique, and is not
 * recycled. Pass that moniker to the callee, and from that moniker the callee
 * can retrieve the entry and with it the object.
 * 
 * <p>
 * The entry cannot be forged and cannot be copied. If you lose the entry, you
 * can no longer retrieve the object, and the entry will eventually be
 * garbage-collected. If you call {@link #deregister(Entry)}, callees will no
 * longer be able to look up an entry from its moniker.
 * 
 * <p>
 * The same is not true of the moniker string. Having the moniker string does
 * not guarantee that the entry will not be removed. Therefore, the creator who
 * called {@link #register(Object)} and holds the entry controls the lifecycle.
 * 
 * <p>
 * The moniker consists of the characters A..Z, a..z, 0..9, $, #, and is thus a
 * valid (case-sensitive) identifier.
 * 
 * <p>
 * All methods are thread-safe.
 * 
 * @author jhyde
 * @version $Id: //open/mondrian/src/main/mondrian/util/LockBox.java#1 $
 * @since 2010/11/18
 */
