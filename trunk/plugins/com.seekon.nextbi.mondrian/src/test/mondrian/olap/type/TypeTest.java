 ...
 * @author jhyde
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/olap/type/TypeTest.java#13 $
 * @since Jan 17, 2008
 */
...
public class TypeTest extends TestCase {
  ...
  public void testConversions() {
    ...
    Cube salesCube = getCubeWithName("Sales", connection.getSchema().getCubes());
    ...
    Type memberType = new MemberType(customersDimension, hierarchy, level, member);
    final LevelType levelType = new LevelType(customersDimension, hierarchy, level);
    final HierarchyType hierarchyType = new HierarchyType(customersDimension,
      hierarchy);
    final DimensionType dimensionType = new DimensionType(customersDimension);
    ...
    final TupleType tupleType = new TupleType(new Type[] { memberType,
      unknownMemberType });
    ...
    Type[] types = { memberType, levelType, hierarchyType, dimensionType,
      numericType, dateTimeType, decimalType, integerType, scalarType, nullType,
      stringType, booleanType, tupleType, tupleSetType, setType,
      unknownDimensionType, unknownHierarchyType, unknownLevelType,
      unknownMemberType };
    ...
    for (Type fromType : types) {
      for (Type toType : types) {
        ...
        final String desc = "symmetric, from " + fromType + ", to " + toType;
        ...
        final List<Resolver.Conversion> conversions = new ArrayList<Resolver.Conversion>();
        final boolean canConvert = TypeUtil.canConvert(0, fromType, toCategory,
          conversions);
        if (canConvert && conversions.size() == 0 && type == null) {
          if (!(fromType == memberType && toType == tupleType
            || fromType == tupleSetType && toType == setType || fromType == setType
            && toType == tupleSetType)) {
  ...
  public void testCommonTypeWhenSetTypeHavingMemberTypeAndTupleType() {
    MemberType measureMemberType = getMemberTypeHavingMeasureInIt(getUnitSalesMeasure());

    MemberType genderMemberType = getMemberTypeHavingMaleChild(getMaleChild());

    MemberType storeMemberType = getStoreMemberType(getStoreChild());

    TupleType tupleType = new TupleType(new Type[] { storeMemberType,
      genderMemberType });
  ...
  public void testCommonTypeBetweenTuplesOfDifferentSizesIsATupleType() {
    ...
    TupleType tupleTypeLarger = new TupleType(new Type[] { storeMemberType,
      genderMemberType, measureMemberType });

    TupleType tupleTypeSmaller = new TupleType(new Type[] { storeMemberType,
      genderMemberType });
  ...
  private MemberType getStoreMemberType(Member storeChild) {
    return new MemberType(storeChild.getDimension(), storeChild.getDimension()
      .getHierarchy(), storeChild.getLevel(), storeChild);
  }
  ...
  private MemberType getMemberTypeHavingMaleChild(Member maleChild) {
    return new MemberType(maleChild.getDimension(), maleChild.getDimension()
      .getHierarchy(), maleChild.getLevel(), maleChild);
  }

  private MemberType getMemberTypeHavingMeasureInIt(Member unitSalesMeasure) {
    return new MemberType(unitSalesMeasure.getDimension(), unitSalesMeasure
      .getDimension().getHierarchy(), unitSalesMeasure.getDimension().getHierarchy()
      .getLevels()[0], unitSalesMeasure);
  }
  ...
  private SchemaReader getSalesCubeSchemaReader() {
    final Cube salesCube = getCubeWithName("Sales", getSchemaReader().getCubes());
    return salesCube.getSchemaReader(testContext.getConnection().getRole())
      .withLocus();
  }
