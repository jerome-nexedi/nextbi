/*
// $Id: //open/mondrian/testsrc/main/mondrian/util/ScheduleTest.java#12 $
...
public class ScheduleTest extends TestCase {
  ...
  public static final int weekdays = (1 << Calendar.MONDAY)
    | (1 << Calendar.TUESDAY) | (1 << Calendar.WEDNESDAY) | (1 << Calendar.THURSDAY)
    | (1 << Calendar.FRIDAY);

  private static final String[] daysOfWeek = { null, "Sun", "Mon", "Tue", "Wed",
    "Thu", "Fri", "Sat" };
  ...
  static void assertEquals(int year, int month, int day, String dow, int hour,
    int minute, Date actual) {
  ...
  static Date toDate(int year, int month, int day, String dow, int hour, int minute,
    TimeZone tz) {
    Calendar calendar = ScheduleUtil.createCalendar(year, month, day, hour, minute,
      0);
  ...
  public void testOnce() {
    Schedule schedule = Schedule.createOnce(toDate(2002, 04, 23, "Tue", 8, 27),
      gmtTz);
    Date d;
  ...
  public void testDaily() {
    int period = 1;
    Schedule schedule = Schedule.createDaily(toDate(2002, 04, 20, "Sat", 8, 27),
      toDate(2002, 06, 03, "Mon", 8, 27), gmtTz, time0827, period);
    Date d;
    d = schedule.nextOccurrence(null, false);
  ...
  public void testDailyNoUpperLimit() {
    int period = 1;
    Schedule schedule = Schedule.createDaily(toDate(2002, 4, 20, "Sat", 8, 27),
      null, gmtTz, time0827, period);
  ...
  public void testWeekly() {
    int thuesday = (1 << Calendar.TUESDAY) | (1 << Calendar.THURSDAY);
    int period = 1;
    Schedule schedule = Schedule.createWeekly(toDate(2002, 4, 20, "Sat", 8, 27),
      toDate(2002, 06, 05, "Wed", 12, 00), gmtTz, time0827, period, thuesday);
    Date d;
    d = schedule.nextOccurrence(null, false);
    assertEquals(2002, 04, 23, "Tue", 8, 27, d);
    d = schedule.nextOccurrence(toDate(2002, 04, 23, "Tue", 8, 27), false);
  ...
  public void testMonthlyByDay() {
    ...
    Schedule schedule = Schedule.createMonthlyByDay(
      toDate(2002, 4, 20, "Sat", 8, 27), toDate(2002, 07, 10, "Wed", 12, 00), gmtTz,
      time0827, period, daysOfMonth);
    Date d;
    d = schedule.nextOccurrence(null, false);
    assertEquals(2002, 04, 21, "Sun", 8, 27, d);
  ...
  public void testMonthlyByDayPeriodic() {
    int daysOfMonth = (1 << 12) | (1 << 21) | (1 << Schedule.LAST_DAY_OF_MONTH);
    int period = 2;
    Schedule schedule = Schedule.createMonthlyByDay(toDate(2002, 04, 30, "Tue", 8,
      27), toDate(2002, 7, 10, "Wed", 12, 00), gmtTz, time0827, period, daysOfMonth);
    Date d;
  ...
  public void testMonthlyByWeek() {
    ...
    Schedule schedule = Schedule.createMonthlyByWeek(toDate(2002, 4, 20, "Sat", 8,
      27), toDate(2004, 4, 19, "Mon", 12, 00), gmtTz, time0827, period, daysOfWeek,
      weeksOfMonth);
    Date d;
    d = schedule.nextOccurrence(null, false);
  ...
  public void testTimeZone() {
    ...
    Schedule schedule = Schedule.createMonthlyByWeek(toDate(2002, 3, 07, "Thu", 14,
      00), toDate(2004, 4, 19, "Mon", 12, 00), jstTz, time0827, period, daysOfWeek,
      weeksOfMonth);
    Date d;
    d = schedule.nextOccurrence(null, true);
    // 1st occurrence is
  ...
  public void testTimeZoneChange() {
    int period = 1;
    TimeZone tz = pstTz;
    Schedule schedule = Schedule.createDaily(toDate(2002, 04, 03, "Wed", 8, 27, tz),
      null, tz, time0233, period);
    Date d;
    d = schedule.nextOccurrence(null, false);
    // 1st occurrence is
