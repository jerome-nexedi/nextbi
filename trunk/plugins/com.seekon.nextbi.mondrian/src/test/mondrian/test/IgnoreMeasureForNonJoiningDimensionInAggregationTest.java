/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/IgnoreMeasureForNonJoiningDimensionInAggregationTest.java#12 $
...
/**
 * Test ignoring of measure when unrelated Dimension is in aggregation list when
 * IgnoreMeasureForNonJoiningDimension property is set to true.
 * 
 * @author ajoglekar
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/
 *          IgnoreMeasureForNonJoiningDimensionInAggregationTest.java#12 $
 * @since Dec 12, 2007
 */
public class IgnoreMeasureForNonJoiningDimensionInAggregationTest extends
  ...
  protected void setUp() throws Exception {
    ...
    propSaver.set(MondrianProperties.instance().IgnoreMeasureForNonJoiningDimension,
      true);
  ...
  public void testNoTotalsForCompdMeasureWithComponentsHavingNonJoiningDims() {
    assertQueryReturns("with member [Measures].[Total Sales] as "
      ...
      + "{Product.x} on 1 from [Warehouse and Sales]", "Axis #0:\n" + "{}\n"
  ...
  public void testNoTotalsForCompoundMeasureWithNonJoiningDimAtAllLevel() {
    assertQueryReturns("with member [Measures].[Total Sales] as "
      ...
      + "Gender.[All Gender]})' " + "select {[Measures].[Total Sales]} on 0, "
      + "{Product.x} on 1 from [Warehouse and Sales]", "Axis #0:\n" + "{}\n"
      + "Axis #1:\n" + "{[Measures].[Total Sales]}\n" + "Axis #2:\n"
      + "{[Product].[x]}\n" + "Row #0: 3,956,666.91\n");
  }
  ...
  public void testShouldTotalAMeasureWithAllJoiningDimensions() {
    assertQueryReturns("with member [Product].x as " + "'sum({Product.members})' "
      + "select " + "{[Measures].[Warehouse Sales]} on 0, " + "{Product.x} on 1 "
  ...
  public void testShouldNotTotalAMeasureWithANonJoiningDimension() {
    assertQueryReturns("with member [Gender].x as 'sum({Gender.members})'"
      + "select " + "{[Measures].[Warehouse Sales]} on 0, " + "{Gender.x} on 1 "
      + "from [Warehouse and Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "Axis #2:\n");
  }
  ...
  public void testGetMeasureCubeForCalcMeasureDoesNotThrowCastException() {
    getTestContext().assertQueryReturns(
      "WITH MEMBER [Measures].[My Profit] AS "
        + "'Measures.[Profit]', SOLVE_ORDER = 3000 " + "MEMBER Gender.G AS "
