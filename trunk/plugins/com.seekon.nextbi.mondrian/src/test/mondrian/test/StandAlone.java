/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/StandAlone.java#19 $
...
public class StandAlone {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Connected in " + (System.currentTimeMillis() - now)
      + " usec");
    processCommands();
  }
  ...
  private static void displayElapsedTime(long startTime, String message) {
    ...
    System.out.println(MessageFormat.format(
      "{2}: {0,number,0}.{1,number,000} ({3})", seconds, msecs, message, elapsed));
  }
  ...
  private static void processSlashCommand(String line) throws IOException {
    if (line.equals("\\schema")) {
      ...
      try {
        runTest(Integer.parseInt(threads), Integer.parseInt(seconds), Boolean
          .valueOf(useRandom));
      } catch (NumberFormatException nfe) {
        System.out.println("Please enter a valid integer for the number of threads "
  ...
  private static void runTest(int numThreads, int seconds, boolean randomQueries) {
    ...
    System.out.println("Running multi-threading test with " + numThreads
      + " threads for " + seconds + " seconds.");
    System.out.println("Queries will " + (randomQueries ? "" : "not ")
      + "be random.");
  ...
  private static void printSchema(Schema schema) {
    ...
    System.out.println("Schema: " + schema.getName() + " " + cubes.length
      + " cubes and " + hierarchies.length + " shared hierarchies");
  ...
  private static void printDimension(Dimension dim) {
    ...
    System.out.println("\tDimension " + dim.getName() + " type: "
      + dimensionType.name());
  ...
  private static void printHierarchy(int indent, Hierarchy hierarchy) {
    ...
    System.out.println(indentString + "    Description: "
      + hierarchy.getDescription());
    System.out.println(indentString + "    Default member: "
