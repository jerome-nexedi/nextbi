/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/clearview/ClearViewBase.java#14 $
 ...
 * queries against the FoodMart database. A concrete sub class and a ref.xml
 * file will be needed for each test suites to be added. MDX queries and their
 * expected results are maintained separately in *.ref.xml files. If you would
 * prefer to see them as inlined Java string literals, run ant target
 * "generateDiffRepositoryJUnit" and then use files *JUnit.java which will be
 ...
 * @since Jan 25, 2007
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/clearview/ClearViewBase
 *          .java#14 $
 */
public abstract class ClearViewBase extends BatchTestCase {
  ...
  protected void runTest() throws Exception {
    ...
    if (!(cubeName.equals("") || cubeName.equals("${modifiedCubeName}"))) {
      ...
      customDimensions = (!(customDimensions.equals("") || customDimensions
        .equals("${customDimensions}"))) ? customDimensions : null;
      String measures = diffRepos.expand(null, "${measures}");
      measures = (!(measures.equals("") || measures.equals("${measures}"))) ? measures
        : null;
      String calculatedMembers = diffRepos.expand(null, "${calculatedMembers}");
      calculatedMembers = (!(calculatedMembers.equals("") || calculatedMembers
        .equals("${calculatedMembers}"))) ? calculatedMembers : null;
      String namedSets = diffRepos.expand(null, "${namedSets}");
      namedSets = (!(namedSets.equals("") || namedSets.equals("${namedSets}"))) ? namedSets
        : null;
      testContext = TestContext.instance().createSubstitutingCube(cubeName,
        customDimensions, measures, calculatedMembers, namedSets);
    }
  ...
  protected void assertQuerySql(boolean flushCache) throws Exception {
    ...
    super.assertQuerySqlOrNot(getTestContext(), diffRepos.expand(null, "${mdx}"),
      buildSqlPatternArray(), false, false, flushCache);
  }
  ...
  protected void assertNoQuerySql(boolean flushCache) throws Exception {
    ...
    super.assertQuerySqlOrNot(getTestContext(), diffRepos.expand(null, "${mdx}"),
      buildSqlPatternArray(), true, false, flushCache);
  }
