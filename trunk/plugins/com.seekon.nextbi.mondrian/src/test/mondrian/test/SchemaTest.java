/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/SchemaTest.java#88 $
 ...
 * @author jhyde
 * @since August 7, 2006
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/SchemaTest.java#88 $
 */
public class SchemaTest extends FoodMartTestCase {
  ...
  private void assertContains(List<Exception> exceptionList, String expected) {
    ...
    fail("Exception list did not contain expected exception '" + expected
      + "'. Exception list is:" + buf.toString());
  }
    ...
    TestContext testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "  <Dimension name=\"Gender with default\" foreignKey=\"customer_id\">\n"
          ...
          + "    </Hierarchy>\n" + "  </Dimension>");
    testContext.assertQueryReturns(
      "select {[Gender with default]} on columns from [Sales]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Gender with default].[M]}\n"
        + "Row #0: 135,215\n");
  }

  /**
   * Test case for the issue described in <a
   * href="http://forums.pentaho.com/showthread.php?p=190737">Pentaho forum post
   * 'wrong unique name for default member when hasAll=false'</a>.
   */
  public void testDefaultMemberName() {
    ...
    testContext.assertQueryReturns(
      "select {[Product with no all]} on columns from [Sales]", "Axis #0:\n"
   ...
   * not issued, the generated SQL can be missing a join condition, as in <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-236"> Bug MONDRIAN-236,
   * "Mondrian generates invalid SQL"</a>.
   */
   ...
   * what is the expected result? Also, in this case, they share the same table
   * without an alias, and the system doesn't complain.
   */
      ...
      testContext
        .assertQueryReturns(
          "select NonEmptyCrossJoin({[Yearly Income2].[All Yearly Income2s]},{[Customers].[All Customers]}) on rows,"
            + "NON EMPTY {[Measures].[Unit Sales]} on columns" + " from [Sales]",
          "Axis #0:\n" + "{}\n" + "266,773");
    }
  }

  /**
   * Tests two dimensions using same table (via different join paths). Without
   * the table alias, generates SQL which is missing a join condition. See
   * {@link #testDuplicateTableAlias()}.
   */
  public void testDimensionsShareTable() {
    ...
    testContext.assertQueryReturns(
      "select {[Yearly Income].[$10K - $30K]} on columns,"
        + "{[Yearly Income2].[$150K +]} on rows from [Sales]", "Axis #0:\n" + "{}\n"
  ...
  /**
   * Tests two dimensions using same table (via different join paths). native
   * non empty cross join sql generation returns empty query. note that this
   * works when native cross join is disabled
   */
  ...
  /**
   * Tests two dimensions using same table with same foreign key one table uses
   * an alias.
   */
  ...
  /**
   * test hierarchy with completely different join path to fact table than first
   * hierarchy. tables are auto-aliased as necessary to guarantee unique joins
   * to the fact table.
   */
  public void testSnowflakeHierarchyValidationNotNeeded() {
    ...
    if ((MondrianProperties.instance().ReadAggregates.get() || MondrianProperties
      .instance().UseAggregates.get())
    ...
    testContext.assertQueryReturns("select  {[Store.MyHierarchy].[Mexico]} on rows,"
      + "{[Customers].[USA].[South West]} on columns" + " from "
      + "AliasedDimensionsTesting", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Customers].[USA].[South West]}\n" + "Axis #2:\n"
      + "{[Store].[MyHierarchy].[Mexico]}\n" + "Row #0: 51,298\n");
  }

  /**
   * test hierarchy with slightly different join path to fact table than first
   * hierarchy. tables from first and second hierarchy should contain the same
   * join aliases to the fact table.
   */
   ...
   * 
   * Tests two dimensions using same table (via different join paths). both
   * using a table alias.
   */
  public void testDimensionsShareJoinTable() {
    ...
    testContext.assertQueryReturns("select  {[Store].[USA].[South West]} on rows,"
      + "{[Customers].[USA].[South West]} on columns" + " from "
      + "AliasedDimensionsTesting", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Customers].[USA].[South West]}\n" + "Axis #2:\n"
  ...
  public void testTwoAliasesDimensionsShareTable() {
    ...
    testContext.assertQueryReturns("select {[StoreA].[USA]} on rows,"
      + "{[StoreB].[USA]} on columns" + " from " + "AliasedDimensionsTesting",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[StoreB].[USA]}\n" + "Axis #2:\n"
        + "{[StoreA].[USA]}\n" + "Row #0: 10,425\n");
  }

  /**
   * Tests two dimensions using same table with same foreign key. both using a
   * table alias.
   */
  ...
  /**
   * Test Multiple DimensionUsages on same Dimension. Alias the fact table to
   * avoid issues with aggregation rules and multiple column names
   */
  ...
  public void testMultipleDimensionHierarchyCaptionUsages() {
    ...
    // Time2.1997 Member
    Member member1 = result.getAxes()[0].getPositions().iterator().next().iterator()
      .next();
    ...
    Member member2 = result.getAxes()[1].getPositions().iterator().next().iterator()
      .next();
  ...
  /**
   * Test to verify naming of all member with dimension usage name is different
   * then source name
   */
  public void testAllMemberMultipleDimensionUsages() {
    ...
    final String store2AllMember = MondrianProperties.instance().SsasCompatibleNaming
      .get() ? "[Store2].[All Stores]" : "[Store2].[All Store2s]";
    testContext.assertQueryReturns("select\n"
    ...
    final Result result = testContext.executeQuery("select ([Store].[All Stores], "
      + store2AllMember + ") on 0\n" + "from [Sales Two Sales Dimensions]");
  ...
  /**
   * This test displays an informative error message if someone uses an
   * unaliased name instead of an aliased name
   */
  ...
  /**
   * Tests a cube whose fact table is a &lt;View&gt; element, and which has
   * dimensions based on the fact table.
   */
  public void testViewFactTable2() {
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        null);
    testContext.assertQueryReturns(
      "select {[Store Type].Children} on columns from [Store2]", "Axis #0:\n"
  ...
  /**
   * Tests that the deprecated "distinct count" value for the Measure@aggregator
   * attribute still works. The preferred value these days is "distinct-count".
   */
  public void testDeprecatedDistinctCountAggregator() {
    TestContext testContext = TestContext.instance().createSubstitutingCube(
      ...
      "  <Measure name=\"Customer Count2\" column=\"customer_id\"\n"
        ...
        + "  </CalculatedMember>");
    testContext.assertQueryReturns("select {[Measures].[Unit Sales],"
      + "    [Measures].[Customer Count], " + "    [Measures].[Customer Count2], "
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-291"> Bug
   * MONDRIAN-291, "'unknown usage' messages"</a>.
   */
  public void testUnknownUsages() {
    ...
    final Appender appender = new WriterAppender(new SimpleLayout(), sw);
      ...
      final TestContext testContext = TestContext
        ...
        .withSchema(
          "<?xml version=\"1.0\"?>\n"
            ...
            + "  <Measure name=\"Unit Sales\" column=\"unit_sales\" aggregator=\"sum\"\n"
            + "      formatString=\"#,###\"/>\n" + "</Cube>\n" + "</Schema>");
      testContext.assertQueryReturns("select from [Denormalized Sales]",
        "Axis #0:\n" + "{}\n" + "225,627.23");
    } finally {
      logger.removeAppender(appender);
    }
   ...
   * "ClassCastException in AggQuerySpec"</a> occurs when two cubes have the
   * same fact table, distinct aggregate tables, and measures with the same
   * name.
   * 
   * <p>
   * This test case attempts to reproduce this issue by creating that
   * environment, but it found a different issue: a measure came back with a
   * cell value which was from a different measure. The root cause is probably
   * the same: when measures are registered in a star, they should be qualified
   * by cube name.
   */
  public void testBugMondrian233() {
    ...
    final String expected = "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Row #0: 266,773\n";
    testContext.assertQueryReturns("select {[Measures]} on 0 from [Sales2]",
      expected);
    testContext
      .assertQueryReturns("select {[Measures]} on 0 from [Sales]", expected);
  }
  ...
  public void testCubeWithOneDimensionOneMeasure() {
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        null);
    testContext.assertQueryReturns(
      "select {[Promotion Media]} on columns from [OneDim]", "Axis #0:\n" + "{}\n"
  ...
  public void testCubeWithOneDimensionUsageOneMeasure() {
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        null);
    testContext.assertQueryReturns(
      "select {[Product].Children} on columns from [OneDimUsage]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Product].[Drink]}\n" + "{[Product].[Food]}\n"
  ...
  public void testCubeCaption() throws SQLException {
    final TestContext testContext = TestContext.instance().create(
      null,
      "<Cube name=\"Cube with caption\" caption=\"Cube with name\">"
        + "  <Table name='sales_fact_1997'/>" + "</Cube>\n",
    ...
    final NamedList<org.olap4j.metadata.Cube> cubes = testContext
      .getOlap4jConnection().getOlapSchema().getCubes();
    final org.olap4j.metadata.Cube cube = cubes.get("Cube with caption");
    assertEquals("Cube with name", cube.getCaption());
    final org.olap4j.metadata.Cube cube2 = cubes
      .get("Warehouse and Sales with caption");
  ...
  public void testCubeWithNoDimensions() {
    final TestContext testContext = TestContext
      ...
      .create(
        ...
        null);
    testContext.assertQueryReturns(
      "select {[Measures].[Unit Sales]} on columns from [NoDim]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Measures].[Unit Sales]}\n"
        + "Row #0: 266,773\n");
  }
  ...
  /**
   * Test case for feature <a
   * href="http://jira.pentaho.com/browse/MONDRIAN-960">MONDRIAN-960, "Ability
   * to define non-measure calculated members in a cube under a specifc
   * parent"</a>.
   */
  public void testCalcMemberInCube() {
    final TestContext testContext = TestContext.instance().createSubstitutingCube(
      "Sales",
      null,
      null,
      "<CalculatedMember\n" + "      name='SF and LA'\n"
  ...
  public void testLevelTableAttributeAsView() {
    ...
    Result result = testContext
      .executeQuery("select {[Gender2].members} on columns from [GenderCube]");

    TestContext.assertEqualsVerbose("[Gender2].[All Gender]\n" + "[Gender2].[F]\n"
      + "[Gender2].[M]", TestContext.toString(result.getAxes()[0].getPositions()));
  }
  ...
  public void testUnionRoleContainsGrants() {
    final TestContext testContext = TestContext.instance().create(
      ...
      .withRole("Role1Plus2");
    testContext.assertQueryThrows("select from [Sales]",
      "Union role must not contain grants");
  }
  ...
  public void testUnionRoleIllegalForwardRef() {
    ...
    testContext.assertQueryThrows("select from [Sales]", "Unknown role 'Role2'");
  }
    ...
    testContext
      .assertQueryReturns(
        "WITH "
          ...
          + "{[Measures].[Unit Sales]} ON 1 FROM [Sales]", "Axis #0:\n" + "{}\n"
          + "Axis #1:\n" + "{[Store].[USA]}\n" + "{[Store].[Total Non CA State]}\n"
          + "Axis #2:\n" + "{[Measures].[Unit Sales]}\n" + "Row #0: 266,773\n"
          + "Row #0: 192,025\n");

    testContext.assertQueryReturns("WITH " + "MEMBER "
      + "[Store].[Total Non CA State] AS \n" + "'SUM({[Non CA State Stores]})'\n"
      + "SELECT {[Store].[Store Country].[USA],[Store].[Total Non CA State]} ON 0,"
      + "{[Measures].[Unit Sales]} ON 1 FROM [Warehouse and Sales]", "Axis #0:\n"
      + "{}\n" + "Axis #1:\n" + "{[Store].[USA]}\n"
      + "{[Store].[Total Non CA State]}\n" + "Axis #2:\n"
      + "{[Measures].[Unit Sales]}\n" + "Row #0: 266,773\n" + "Row #0: 192,025\n");
  }
  ...
  public void testInvalidRoleError() {
    ...
    schema = schema.replaceFirst("<Schema name=\"FoodMart\"",
      "<Schema name=\"FoodMart\" defaultRole=\"Unknown\"");
    final TestContext testContext = TestContext.instance().withSchema(schema);
    final List<Exception> exceptionList = testContext.getSchemaWarnings();
    ...
    TestContext testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "  <Dimension name=\"Binary\" foreignKey=\"promotion_id\">\n"
          ...
          + "  </Dimension>\n");
    testContext.assertQueryReturns("select {[Binary].members} on 0 from [Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Binary].[Ben]}\n"
        + "{[Binary].[Ben].[2]}\n" + "{[Binary].[Bill]}\n"
        + "{[Binary].[Bill].[3]}\n" + "{[Binary].[Bill].[4]}\n" + "Row #0: \n"
        + "Row #0: \n" + "Row #0: \n" + "Row #0: \n" + "Row #0: \n");
    testContext.assertQueryReturns(
      "select hierarchize({[Binary].members}) on 0 from [Sales]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Binary].[Ben]}\n" + "{[Binary].[Ben].[2]}\n"
        + "{[Binary].[Bill]}\n" + "{[Binary].[Bill].[3]}\n"
        + "{[Binary].[Bill].[4]}\n" + "Row #0: \n" + "Row #0: \n" + "Row #0: \n"
        + "Row #0: \n" + "Row #0: \n");
   ...
   * 
   * <p>
   * See bug <a href="http://jira.pentaho.com/browse/MONDRIAN-896">
    ...
    TestContext testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "  <Dimension name=\"Big numbers\" foreignKey=\"promotion_id\">\n"
          ...
          + "  </Dimension>\n");
    testContext.assertQueryReturns(
      "select {[Big numbers].members} on 0 from [Sales]", "Axis #0:\n" + "{}\n"
        + "Axis #1:\n" + "{[Big numbers].[1234]}\n" + "{[Big numbers].[1234].[0]}\n"
  ...
  /**
   * Testcase for a problem which involved a slowly changing dimension. Not
   * actually a slowly-changing dimension - we don't have such a thing in the
   * foodmart schema - but the same structure. The dimension is a two table
   * snowflake, and the table nearer to the fact table is not used by any level.
   */
  ...
  public void testBugMondrian482() {
    ...
    if (!Bug.BugMondrian495Fixed
      && TestContext.instance().getDialect().getDatabaseProduct() != Dialect.DatabaseProduct.MYSQL) {
      return;
    }
    ...
    final TestContext testContext = TestContext.instance().create(null, salesCube1,
      null, null, null, null);
    ...
    testContext.assertQueryReturns(query1, "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
    ...
    testContext.assertQueryReturns(query2, "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Measures].[Unit Sales]}\n" + "Axis #2:\n"
  ...
  /**
   * Test case for <a href="http://jira.pentaho.com/browse/MONDRIAN-355">Bug
   * MONDRIAN-355,
  ...
  public void checkBugMondrian355(String timeHalfYear) {
    final String xml = "<Dimension name=\"Time2\" foreignKey=\"time_id\" type=\"TimeDimension\">\n"
      + "<Hierarchy hasAll=\"true\" primaryKey=\"time_id\">\n"
      + "  <Table name=\"time_by_day\"/>\n"
    ...
    try {
      TestContext.instance().createSubstitutingCube("Sales",
        Util.replace(xml, "TimeUndefined", "TimeUnspecified")).assertSimpleQuery();
      fail("expected error");
    } catch (Throwable e) {
      TestContext
        .checkThrowable(
  ...
  public void testCaptionDescriptionAndAnnotation() {
    ...
    final TestContext testContext = TestContext
      ...
      .withSchema(
        "<Schema name=\""
          ...
          + "  <VirtualCubeDimension cubeName=\""
          + warehouseCubeName
          + "\" name=\"Warehouse\"/>\n"
          + "  <VirtualCubeMeasure cubeName=\""
          + salesCubeName
    ...
    final List<Member> memberList = cube.getSchemaReader(null).withLocus()
    ...
    final boolean ssasCompatibleNaming = MondrianProperties.instance().SsasCompatibleNaming
      .get();
    ...
    assertEquals("Time usage caption.Time shared hierarchy description",
      timeHierarchy.getDescription());
    // The hierarchy caption is prefixed by the caption of the dimension
    // usage.
    assertEquals("Time usage caption.Time shared hierarchy caption", timeHierarchy
      .getCaption());
    // No annotations.
    ...
    assertEquals("Time2.Time shared hierarchy description", time2Hierarchy
      .getDescription());
    ...
    assertEquals(measure.getDescription(), measure
      .getPropertyValue(Property.DESCRIPTION.name));
    assertEquals(measure.getCaption(), measure
      .getPropertyValue(Property.CAPTION.name));
    assertEquals(measure.getCaption(), measure
      .getPropertyValue(Property.MEMBER_CAPTION.name));
    ...
    assertEquals(calcMeasure.getDescription(), calcMeasure
      .getPropertyValue(Property.DESCRIPTION.name));
    assertEquals(calcMeasure.getCaption(), calcMeasure
      .getPropertyValue(Property.CAPTION.name));
    assertEquals(calcMeasure.getCaption(), calcMeasure
      .getPropertyValue(Property.MEMBER_CAPTION.name));
    ...
    final Result result2 = testContext.executeQuery("select from ["
      + virtualCubeName + "]");
    ...
    final List<Member> measures2 = schemaReader2.getLevelMembers(measuresLevel2,
      true);
    ...
    assertEquals(measure2.getDescription(), measure2
      .getPropertyValue(Property.DESCRIPTION.name));
    assertEquals(measure2.getCaption(), measure2
      .getPropertyValue(Property.CAPTION.name));
    assertEquals(measure2.getCaption(), measure2
      .getPropertyValue(Property.MEMBER_CAPTION.name));
  ...
  private static void checkAnnotations(Map<String, Annotation> annotationMap,
    String... nameVal) {
    ...
    TestContext testContext = TestContext
      ...
      .createSubstitutingCube(
        ...
        "  <Dimension name=\"Gender2\" foreignKey=\"customer_id\">\n"
          ...
          + "    </Hierarchy>\n" + "  </Dimension>");
    Result result = testContext
      .executeQuery("select {[Gender2].Children} on columns from [Sales]");
    assertEquals("foobar", result.getAxes()[0].getPositions().get(0).get(0)
      .getCaption());
  }
  ...
  public static class DummyPropertyFormatter implements PropertyFormatter {
    ...
    public String formatProperty(Member member, String propertyName,
  ...
  /**
   * Unit test for bug <a href="http://jira.pentaho.com/browse/MONDRIAN-747">
   * MONDRIAN-747, "When joining a shared dimension into a cube at a level other
   * than its leaf level, Mondrian gives wrong results"</a>.
   */
  public void testBugMondrian747() {
    ...
    final String x = !Bug.BugMondrian747Fixed ? "1,379,620" : "266,773";
    testContext.assertQueryReturns(
    ...
    final String y = !Bug.BugMondrian747Fixed
      ...
      && MondrianProperties.instance().UseAggregates.get() ? "20,957" : "266,773";
    testContext.assertQueryReturns(
  ...
  /**
   * Unit test for bug <a href="http://jira.pentaho.com/browse/MONDRIAN-463">
  ...
  public void testBugMondrian463() {
    ...
    checkBugMondrian463(TestContext
      .instance()
      .createSubstitutingCube(
        "Sales",
  ...
  /**
   * Tests that a join nested left-deep, that is (Join (Join A B) C), fails. The
   * correct way to use a join is right-deep, that is (Join A (Join B C)). Same
   * schema as {@link #testBugMondrian463}, except left-deep.
   */
  ...
  public void testCaptionWithOrdinalColumn() {
    final TestContext tc = TestContext
      .instance()
      .createSubstitutingCube(
        "HR",
        "<Dimension name=\"Position\" foreignKey=\"employee_id\">\n"
          ...
          + "  </Hierarchy>\n" + "</Dimension>\n");
    String mdxQuery = "WITH SET [#DataSet#] as '{Descendants([Position].[All Position], 2)}' "
   ...
   * which was done at cube init time when resolving the calculated members of
   * the base cubes.
   */
    ...
    for (Cube cube : context.getConnection().getSchemaReader().getCubes()) {
      if (cube.getName().equals("Warehouse and Sales")) {
        for (Dimension dim : cube.getDimensions()) {
          if (dim.isMeasures()) {
            List<Member> members = context.getConnection().getSchemaReader()
              .getLevelMembers(dim.getHierarchy().getLevels()[0], true);
            assertTrue(members.toString().contains(
              "[Measures].[Profit Per Unit Shipped]"));
            assertTrue(members.toString().contains("[Measures].[Image Unit Sales]"));
            assertTrue(members.toString().contains("[Measures].[Arrow Unit Sales]"));
            assertTrue(members.toString().contains("[Measures].[Style Unit Sales]"));
            assertTrue(members.toString().contains(
  ...
  public void testCubesVisibility() throws Exception {
    for (Boolean testValue : new Boolean[] { true, false }) {
      ...
      final TestContext context = TestContext.instance().create(null, cubeDef, null,
        null, null, null);
  ...
  public void testVirtualCubesVisibility() throws Exception {
    for (Boolean testValue : new Boolean[] { true, false }) {
      String cubeDef = "<VirtualCube name=\"Foo\" defaultMeasure=\"Store Sales\" visible=\"@REPLACE_ME@\">\n"
        + "  <VirtualCubeDimension cubeName=\"Sales\" name=\"Customers\"/>\n"
        + "  <VirtualCubeMeasure cubeName=\"Sales\" name=\"[Measures].[Store Sales]\"/>\n"
        + "</VirtualCube>\n";
      cubeDef = cubeDef.replace("@REPLACE_ME@", String.valueOf(testValue));
      final TestContext context = TestContext.instance().create(null, null, cubeDef,
        null, null, null);
      final Cube cube = context.getConnection().getSchema().lookupCube("Foo", true);
      assertTrue(testValue.equals(cube.isVisible()));
    }
  ...
  public void testHierarchyVisibility() throws Exception {
    for (Boolean testValue : new Boolean[] { true, false }) {
      ...
      assertNotNull(hier);
      assertEquals(
        MondrianProperties.instance().SsasCompatibleNaming.get() ? "Bacon"
          : "Bar.Bacon", hier.getName());
