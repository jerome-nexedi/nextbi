 ...
 * 
 * <p>
 * It is not required that the Steel Wheels database be present, so each test
 * should check whether the database exists and trivially succeed if it does
 * not.
 * 
 * @author jhyde
 * @since 12 March 2009
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/SteelWheelsTestCase.java
 *          #8 $
 */
public class SteelWheelsTestCase extends TestCase {
   ...
   *          Base test context
   * @param schema
   *          A XML schema, or null Used for testing if the connection is valid.
   * @return TestContext which contains the given schema
   */
  public static TestContext createContext(TestContext context, final String schema) {
    ...
    properties.put(RolapConnectionProperties.Jdbc.name(), Util.replace(jdbc,
    ...
    if (schema != null) {
      ...
      final String catalog = properties
        .get(RolapConnectionProperties.Catalog.name());
      properties.put(RolapConnectionProperties.Catalog.name(), Util.replace(catalog,
        "FoodMart.xml", "SteelWheels.mondrian.xml"));
    }
