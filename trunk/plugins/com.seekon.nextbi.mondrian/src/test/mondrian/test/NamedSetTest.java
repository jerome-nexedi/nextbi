/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/NamedSetTest.java#35 $
 ...
 * @author jhyde
 * @since April 30, 2005
 * @version $Id: //open/mondrian/testsrc/main/mondrian/test/NamedSetTest.java#35
 *          $
 */
public class NamedSetTest extends FoodMartTestCase {
  ...
  public void testNamedSetAgainstCube() {
    final TestContext tc = getTestContext().withSchemaProcessor(
  ...
  public void testNamedSetAgainstSchema() {
    ...
    tc.assertQueryThrows("SELECT {[Measures].[Store Sales]} on columns,\n"
  ...
  public void testBadNamedSet() {
    final TestContext tc = TestContext.instance().create(null, null, null,
      "<NamedSet name=\"Bad\" formula=\"{[Store].[USA].[WA].Children}}\"/>", null,
      null);
    tc.assertQueryThrows("SELECT {[Measures].[Store Sales]} on columns,\n"
  ...
  /**
   * Test csae for bug 1971080, "hierarchize(named set) causes attempt to sort
   * immutable list".
   */
  ...
  public void testCurrentAndCurrentOrdinal() {
    assertQueryReturns("with set [Gender Marital Status] as\n"
  ...
  /**
   * Dynamic schema processor which adds two named sets to a the first cube in a
   * schema.
   */
  public static class NamedSetsInCubeProcessor extends FilterDynamicSchemaProcessor {
   ...
   * schema.
   */
  public static class NamedSetsInCubeAndSchemaProcessor extends
   ...
   */
  public static class MixedNamedSetSchemaProcessor extends
    FilterDynamicSchemaProcessor {
    protected String filter(String schemaUrl, Util.PropertyList connectInfo,
      InputStream stream) throws Exception {
      String s = super.filter(schemaUrl, connectInfo, stream);
