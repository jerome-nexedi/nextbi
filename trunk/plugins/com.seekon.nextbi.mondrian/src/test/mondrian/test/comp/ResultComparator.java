/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/comp/ResultComparator.java#20 $
...
/**
 * Compares the {@link Result} produced by a query with the expected result read
 * from an XML file.
 * 
...
class ResultComparator {
  ...
  private void compareSlicers() {
    ...
    final Element tuples = (Element) slicerTuple.getElementsByTagName("tuples")
      .item(0);
    final Element tuple = (Element) tuples.getElementsByTagName("tuple").item(0);
    ...
    for (int idx = 0; idx < numMembers; idx++) {
      String expectedMemberName = expectedTuple.item(idx).getFirstChild()
        .getNodeValue();
    ...
    _assertEquals("The query returned more slicer members than were expected",
  ...
  private void compareColumns() {
    ...
    if (axes.length >= 1) {
      ...
      Assert.assertTrue("Must be no columns", columnList.getLength() == 0);
    }
  }
  ...
  private void compareRows() {
    ...
    default:
      Assert.fail("Too many axes returned. " + "Expected 0, 1 or 2 but got "
        + axes.length);
      break;
    }
  }
  ...
  private void _failNotEquals(String message, Object expected, Object actual) {
    ...
    message += "; expected=" + expected + "; actual=" + actual + Util.nl + "Query: "
      + Util.unparse(result.getQuery()) + Util.nl;
    TestContext.assertEqualsVerbose(TestContext.fold(XmlUtility.toString(xmlRoot)),
      toString(result), false, message);
  }
  ...
  private Element toXml(Result result) {
    DocumentBuilder db = XmlUtility.createDomParser(false, true, false,
      new XmlUtility.UtilityErrorHandler());
    ...
    for (int i = 0; i < axes.length; i++) {
      Axis axis = axes[i];
      String axisName = AxisOrdinal.StandardAxisOrdinal.forLogicalOrdinal(i).name()
        .toLowerCase();
  ...
  private void cellsToXml(Document document, Result result, Element parentXml,
    int[] pos, int axisOrdinal) {
    ...
    for (int i = 0; i < axis.getPositions().size(); i++) {
      ...
      if (axisOrdinal == 0) {
        ...
        final Text textXml = document
          .createTextNode(String.valueOf(cell.getValue()));
  ...
  private void slicerAxisToXml(final Document document, final Element dataResultXml,
    final Result result) {
    ...
    for (Dimension dimension : dimensions) {
      ...
      final Text textXml = document.createTextNode(member.getDimension()
        .getUniqueName());
      ...
      final Text memberTextXml = document.createTextNode(member.getUniqueName());
      memberXml.appendChild(memberTextXml);
    }
  }

  /**
   * Returns which member of a given dimension appears in the slicer axis.
   * <p/>
   * <p/>
   * If the dimension occurs on one of the other axes, the answer is null. If
   * the dimension occurs in the slicer axis, the answer is that member.
  ...
  private void axisToXml(final Document document, final Element dataResultXml,
    final Axis axis, final String axisName) {
    final Element axisXml = document.createElement(axisName);
    dataResultXml.appendChild(axisXml);
    if (axis.getPositions().size() > 0) {
      final Element dimensionsXml = document.createElement("dimensions");
      ...
      for (Position position : axis.getPositions()) {
        ...
        for (final Member member : position) {
          final Element memberXml = document.createElement("member");
          tupleXml.appendChild(memberXml);
          final Text textXml = document.createTextNode(member.getUniqueName());
  ...
  private void _assertEquals(String message, double expected, double actual,
    double delta) {
  ...
  private void compareTuples(String message, NodeList axisValues,
    ...
    if (axisValues.getLength() != 0) {
      ...
      final Element dims = (Element) axisNode.getElementsByTagName("dimensions")
        .item(0);
      expectedDims = dims.getElementsByTagName("dim");
      final Element tuples = (Element) axisNode.getElementsByTagName("tuples").item(
        0);
      expectedTuples = tuples.getElementsByTagName("tuple");
    }

    int numExpectedTuples = expectedTuples == null ? 0 : expectedTuples.getLength();
    _assertEquals(message + " number of tuples", numExpectedTuples, resultTuples
      .size());

    if (numExpectedTuples != 0) {
      _assertEquals(
        "Invalid test case. Number of dimensions does not match tuple lengths",
        expectedDims.getLength(), ((Element) expectedTuples.item(0))
          .getElementsByTagName("member").getLength());
    }

    for (int idx = 0; idx < numExpectedTuples; idx++) {
      compareTuple(message + " tuple " + idx, (Element) expectedTuples.item(idx),
        resultTuples.get(idx));
    }
  }

  private void compareTuple(String message, Element expectedTuple,
    Position resultTuple) {
    ...
    _assertEquals(message + " number of members", numExpectedMembers, resultTuple
      .size());
  ...
  private void compareColumnsOnly(NodeList expectedRow, Axis[] axes) {
    ...
    _assertEquals("Unexpected number of columns", numColumns, axes[0].getPositions()
      .size());
  ...
  private void compareRowsAndColumns(NodeList expectedRows, Axis[] axes) {
    ...
    _assertEquals("Number of row tuples must match", numRows, axes[1].getPositions()
      .size());
    ...
    for (int rowIdx = 0; rowIdx < numRows; rowIdx++) {
      ...
      if (rowIdx == 0) {
        _assertEquals("Number of data columns: ", cellList.getLength(), axes[0]
          .getPositions().size());
      }
  ...
  private void compareCell(int[] coord, String expectedValue, Cell cell) {
    if (expectedValue.equalsIgnoreCase("#Missing")) {
      if (!cell.isNull()) {
        _failNotEquals(getErrorMessage("Expected missing value but got "
          + cell.getValue() + " at ", coord), null, null);
      }
      ...
      _assertEquals(getErrorMessage("Values don't match at ", coord), expectedValue,
        cell.getValue());
    }
  }
