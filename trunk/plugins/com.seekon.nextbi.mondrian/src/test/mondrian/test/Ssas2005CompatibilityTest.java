/*
// $Id: //open/mondrian/testsrc/main/mondrian/test/Ssas2005CompatibilityTest.java#18 $
 ...
 * 
 * <p>
 * This suite contains a MDX collection of queries that were run on SSAS. The
 ...
 * 
 * <p>
 * There are tests for features which are unimplemented or where mondrian's
 * behavior differs from SSAS2005. These tests will appear in this file disabled
 * or with (clearly marked) incorrect results.
 * 
 * @author jhyde
 * @since December 15, 2008
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/test/Ssas2005CompatibilityTest
 *          .java#18 $
 */
public class Ssas2005CompatibilityTest extends FoodMartTestCase {
  ...
  /**
   * Catch-all for tests that depend on something that hasn't been implemented.
   */
  ...
  public void testDimensionDotHierarchyAmbiguous() {
    ...
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      ...
      getTestContext().assertExprThrows("[Currency].[Currency].Ordinal",
        "No function matches signature '<Hierarchy>.Ordinal'");

      // SSAS succeeds with the '<Hierarchy>.Levels(<Numeric Expression>)'
      // function, returns 2
      getTestContext().assertExprReturns("[Currency].[Currency].Levels(0).Name",
        "(All)");

      // There are 4 hierarchy members (including 'Any currency')
      getTestContext()
        .assertExprReturns("[Currency].[Currency].Members.Count", "15");

      // There are 3 level members
      getTestContext().assertExprReturns(
        "[Currency].[Currency].[Currency].Members.Count", "14");
    } else {
      // Old mondrian behavior prefers level.
      getTestContext().assertExprReturns("[Currency].[Currency].Ordinal", "1");
  ...
  public void testHierarchyLevelsFunction() {
    ...
    // SSAS returns 7.
    getTestContext().assertExprReturns("[Product].[Products].Levels.Count", "7");
  }
  ...
  public void testDimensionDotHierarchyDotLevel() {
    ...
    // hierarchy
    // SSAS2005 succeeds
    assertQueryReturns("select [Store].[Stores].[Store State].MEMBERS on 0\n"
  ...
  public void testNamingDimensionDotLevel2() {
    ...
    runQ("select [Time].[Date2].MEMBERS on 0 from [Warehouse and Sales]");

    // SSAS returns [Time].[Time By Week].[Date2]
    runQ("with member [Measures].[Foo] as ' [Time].[Date2].UniqueName '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testNamingDimensionDotLevelNotUnique() {
    ...
    runQ("select [Time].[Year2].MEMBERS on 0 from [Warehouse and Sales]");

    // SSAS2005 returns [Time].[Time By Week].[Year2]
    // (Presumably because it comes first.)
    runQ("with member [Measures].[Foo] as ' [Time].[Year2].UniqueName '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testDimensionMembersOnSingleHierarchyDimension() {
    ...
    // (and no attributes)
    // SSAS2005 succeeds
    assertQueryReturns("select [Currency].Members on 0\n"
      + "from [Warehouse and Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testMultipleHierarchyRequiresQualification() {
    ...
    // specified.
    assertQueryThrows("select [Product].Members on 0\n"
      + "from [Warehouse and Sales]",
  ...
  /**
   * Tests that it is an error to define a calc member in a dimension with
   * multiple hierarchies without specifying hierarchy. Based on
   * {@link mondrian.test.BasicQueryTest#testHalfYears()}.
   */
  public void testCalcMemberAmbiguousHierarchy() {
    String mdx = "WITH MEMBER [Measures].[ProfitPercent] AS\n"
    ...
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      TestContext.instance().assertQueryThrows(mdx,
  ...
  public void testUnqualifiedHierarchy() {
    ...
    runQ("select [Products].Members on 0\n" + "from [Warehouse and Sales]");

    runQ("select {[Products]} on 0\n" + "from [Warehouse and Sales]");

    // TODO: run this in SSAS
    // [Measures] is both a dimension and a hierarchy;
    // [Products] is just a hierarchy.
    // SSAS returns 557863
    runQ("select [Measures].[Unit Sales] on 0,\n" + "  [Products].[Food] on 1\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testAxesOutOfOrder() {
    ...
    assertQueryReturns("select [Measures].[Unit Sales] on 1,\n"
  ...
  public void testDimensionMemberRequiresHierarchyQualification() {
    ...
    // one hierarchy, therefore the hierarchy must be explicitly
    // specified.
    final String[] exprs = { "[Product].CurrentMember",
      // TODO: Verify that this does indeed fail on SSAS
      "[Product].DefaultMember",
      // TODO: Verify that this does indeed fail on SSAS
      "[Product].AllMembers", "Dimensions(3).CurrentMember",
      "Dimensions(3).DefaultMember", "Dimensions(3).AllMembers", };
    final String expectedException = "The 'Product' dimension contains more than one hierarchy, "
      + "therefore the hierarchy must be explicitly specified.";
    assertQueryThrows("select [Product].CurrentMember on 0\n"
      + "from [Warehouse and Sales]", expectedException);
    assertQueryThrows("select [Product].DefaultMember on 0\n"
      + "from [Warehouse and Sales]", expectedException);
    assertQueryThrows("select [Product].AllMembers on 0\n"
      + "from [Warehouse and Sales]", expectedException);
    ...
    assertQueryReturns("select Dimensions(3).CurrentMember on 0\n"
      + "from [Warehouse and Sales]", expectedResult);
    assertQueryReturns("select Dimensions(3).DefaultMember on 0\n"
      + "from [Warehouse and Sales]", expectedResult);
    assertQueryReturns("select Head(Dimensions(7).AllMembers, 3) on 0\n"
      + "from [Warehouse and Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
      + "{[Currency].[All Currencys]}\n" + "{[Currency].[Bulk Mail]}\n"
      + "{[Currency].[Cash Register Handout]}\n" + "Row #0: 266,773\n"
      + "Row #0: 4,320\n" + "Row #0: 6,697\n");
  ...
  public void testImplicitCurrentMemberRequiresHierarchyQualification() {
    ...
    // TODO: check SSAS
    assertQueryReturns("select Ascendants([Store]) on 0\n"
      + "from [Warehouse and Sales]", "Axis #0:\n" + "{}\n" + "Axis #1:\n"
  ...
  public void testUnqualifiedHierarchyCurrentMember() {
    ...
    // SSAS2005 succeeds
    runQ("select [Products].CurrentMember on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testCannotDistinguishMdxFromSql() {
    ...
    // to ambiguity.
    assertQueryThrows("select [Time].Members\n" + "from [Warehouse and Sales]",
  ...
  public void testNamingDimensionAttr() {
    ...
    runQ("select [Store].[Store Manager].Members on 0 from [Warehouse and Sales]");
  }
  ...
  public void testAttrHierarchyMemberParent() {
    ...
    runQ("with member [Measures].[Foo] as ' [Store].[Store City].[San Francisco].Parent.UniqueName '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testAttrHierarchyMemberChildren() {
    ...
    runQ("select [Store].[Store City].[San Francisco].Children on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testAttrHierarchyAllMemberChildren() {
    ...
    // SSAS2005 succeeds
    runQ("select [Store].[Store City].Children on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testAttrHierarchyMemberLevel() {
    ...
    runQ("with member [Measures].[Foo] as [Store].[Store City].[San Francisco].Level.UniqueName\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testAttrHierarchyUniqueName() {
    ...
    runQ("with member [Measures].[Foo] as [Store].[Store City].UniqueName\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByLevelAndKey() {
    ...
    // (Returns 31, 5368)
    runQ("select {[Time].[Time By Week].[Week].[31]} on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByCompoundKey() {
    ...
    runQ("select [Time].[Time By Week].[Year2].[1998].&[30]&[1998] on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByPartialCompoundKey() {
    ...
    runQ("select [Time].[Time By Week].[Year2].[1998].&[30] on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByNonUniqueName() {
    ...
    runQ("select [Time].[Time2].[Month].[January] on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByLevelAndCompoundKey() {
    ...
    runQ("with member [Measures].[Foo] as ' [Time].[Time2].[Month].[January].UniqueName '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testMemberAddressedByLevelAndName() {
    ...
    runQ("with member [Measures].[Foo] as ' [Store].[Store City].[Month].[January].UniqueName '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testFoo31() {
    ...
    runQ("select Measures on 0,[Product].[Product Department].Members on 1\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testFoo32() {
    ...
    runQ("with member [Measures].[U] as ' [Product].UniqueName '\n"
  ...
  public void testNamingAttrVsLevel() {
    ...
    // [attribute] vs. [level]
    // SSAS2005 succeeds
    runQ("select [Store City].Members on 0\n" + "from [Warehouse and Sales]");

    // the attribute hierarchy wins over the level
    // SSAS2005 returns [Store].[Store City]
    assertQueryReturns("with member [Measures].[Foo] as [Store City].UniqueName\n"
  ...
  public void testDimensionAsScalarExpression() {
    ...
    runQ("with member [Measures].[Foo] as [Date2]\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]");
  }
  ...
  public void testDimensionWithMultipleHierarchiesDotParent() {
    ...
    // The 'Product' dimension contains more than one hierarchy,
    // therefore the hierarchy must be explicitly specified.
    assertExprThrows("[Time].Parent.UniqueName",
  ...
  /**
   * Test case for bug 2688790, "Hierarchy Naming Compatibility issue". Occurs
   * when dimension and hierarchy have the same name and are used with
   * [name.name].
   */
  ...
  public void testDimensionDotLevelDotHierarchyInBrackets() {
    ...
    // was parsed.
    assertQueryThrows("select [Time.Time2.Quarter].Members on 0\n"
      + "from [Warehouse and Sales]",
  ...
  public void testDimensionDotInvalidHierarchyInBrackets() {
    ...
    // was parsed.
    assertQueryThrows("select {[Time.Time By Week55].Members} on 0\n"
      + "from [Warehouse and Sales]",
  ...
  public void testDimensionDotDimensionInBrackets() {
    ...
    assertQueryThrows("select {[Time.Time].Members} on 0\n"
      + "from [Warehouse and Sales]",
  ...
  public void testDimensionDotHierarchyDotNonExistentLevel() {
    ...
    runQ("select [Time].[Time By Week].[Month].Members on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testDimensionDotHierarchyDotLevelMembers() {
    ...
    runQ("select [Time].[Time2].[Quarter].Members on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testDupHierarchyOnAxes() {
    ...
    // [Products] on 1
    // from [Warehouse and Sales]
    assertQueryThrows("select {[Products]} on 0,\n" + "  {[Products]} on 1\n"
      + "from [Warehouse and Sales]",
  ...
  public void testDifferentHierarchiesFromSameDimensionInCrossjoin() {
    ...
    // SSAS2005 succeeds
    runQ("select Crossjoin([Time].[Time By Week].Children, [Time].[Time2].Members) on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testFoo50() {
    ...
    runQ("select {[Store].[Store Country].[USA], [Store].[Stores].[Store Country].[USA]} on 0\n"
      + "from [Warehouse and Sales]");
  }
  ...
  public void testQuoteInStringInQuotedFormula() {
    ...
    // SSAS2005 returns 5
    assertQueryReturns("with member [Measures].[Foo] as ' len(\"can''t\") '\n"
      + "select [Measures].[Foo] on 0\n" + "from [Warehouse and Sales]",
      "Axis #0:\n" + "{}\n" + "Axis #1:\n" + "{[Measures].[Foo]}\n" + "Row #0: 5\n");
  }
  ...
  public void testAxisLabelsNotContiguousFails() {
    ...
    assertQueryThrows("select [Measures].[Unit Sales] on 1,\n"
  ...
  public void testLotsOfAxes() {
    ...
    runQ("select [Measures].[Unit Sales] on axis(0),\n"
  ...
  public void testOnAxesFails() {
    ...
    assertQueryThrows("select [Measures].[Unit Sales] on axes(0)\n"
      + "from [Warehouse and Sales]",
  ...
  public void testOnExpression() {
    ...
    assertQueryThrows("select [Measures].[Unit Sales] on 0 + 1\n"
      + "from [Warehouse and Sales]", "Syntax error at line 1, column 37, token '+'");
  }

  public void testOnFractionFails() {
    // SSAS gives syntax error
    assertQueryThrows("select [Measures].[Unit Sales] on 0.4\n"
      + "from [Warehouse and Sales]",
  ...
  public void testAxisFunction() {
    ...
    runQ("WITH MEMBER MEASURES.AXISDEMO AS\n"
  ...
  public void testAxisAppliedToExpr() {
    ...
    assertQueryReturns("WITH MEMBER MEASURES.AXISDEMO AS\n"
      + "  SUM(AXIS(1), [Measures].[Unit Sales])\n"
      + "SELECT {[Measures].[Unit Sales],MEASURES.AXISDEMO} ON 0,\n"
      + "{[Time].[Time by Week].Children} ON 1\n" + "FROM [Warehouse and Sales]",
      "xxx");
  ...
  public void testAxisFunctionReferencesSameAxisFails() {
    ...
    assertQueryThrows("WITH MEMBER MEASURES.AXISDEMO AS\n"
  ...
  /**
   * Tests the ambiguity between a level and a member of the same name, both in
   * SSAS compatible mode and in regular mode.
   * 
   * @throws SQLException
   *           If the test fails.
   */
  public void testCanHaveMemberWithSameNameAsLevel() throws SQLException {
    ...
    org.olap4j.metadata.Member member = testContext.getOlap4jConnection()
      .getOlapSchema().getCubes().get("Sales").getDimensions().get("SameName")
      .getHierarchies().get("SameName").getLevels().get("SameName").getMembers()
      .get(0);
    ...
    testContext
      .assertQueryThrows(
        "select {"
          + (MondrianProperties.instance().SsasCompatibleNaming.get() ? "[SameName].[SameName].[SameName]"
            : "[SameName].[SameName]") + "} on 0 from Sales",
    ...
    if (MondrianProperties.instance().SsasCompatibleNaming.get()) {
      ...
      testContext.assertQueryReturns(
        "select {[SameName].[SameName].[SameName]} on 0 from Sales", "Axis #0:\n"
          + "{}\n" + "Axis #1:\n" + "{[SameName].[SameName].[SameName]}\n"
          + "Row #0: \n");
    }
  }
  ...
  /**
   * SSAS can resolve root members of a hierarchy even if not qualified by
   * hierarchy, and even if the dimension has more than one hierarchy.
   */
  ...
  /**
   * Subclass of {@link mondrian.test.Ssas2005CompatibilityTest} that runs with
   * {@link mondrian.olap.MondrianProperties#SsasCompatibleNaming}=false.
   */
  ...
  /**
   * Subclass of {@link mondrian.test.Ssas2005CompatibilityTest} that runs with
   * {@link mondrian.olap.MondrianProperties#SsasCompatibleNaming}=true.
   */
