/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/RolapResultTest.java#16 $
 ...
 * @since Feb 21 2007
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/RolapResultTest.java#16
 *          $
 */
public class RolapResultTest extends AggTableTestCase {
  ...
  private static final String RESULTS_ALL = "Axis #0:\n" + "{}\n" + "Axis #1:\n"
    + "{[D1].[a]}\n" + "{[D1].[b]}\n" + "{[D1].[c]}\n" + "Axis #2:\n"
    + "{[D2].[x]}\n" + "{[D2].[y]}\n" + "{[D2].[z]}\n" + "Row #0: 5\n"
    + "Row #0: \n" + "Row #0: \n" + "Row #1: \n" + "Row #1: 10\n" + "Row #1: \n"
    + "Row #2: \n" + "Row #2: \n" + "Row #2: 15\n";
  ...
  public void testAll() throws Exception {
    ...
    String mdx = "select " + " filter({[D1].[a],[D1].[b],[D1].[c]}, "
      + "    [Measures].[Value] > 0) " + " ON COLUMNS, "
      + " {[D2].[x],[D2].[y],[D2].[z]} " + " ON ROWS " + "from FTAll";

    getCubeTestContext().assertQueryReturns(mdx, RESULTS_ALL);
    /*
     * Result result = getCubeTestContext().executeQuery(mdx); String
  ...
  public void testD1() throws Exception {
    ...
    String mdx = "select " + " filter({[D1].[a],[D1].[b],[D1].[c]}, "
      + "    [Measures].[Value] > 0) " + " ON COLUMNS, "
      + " {[D2].[x],[D2].[y],[D2].[z]} " + " ON ROWS " + "from FT1";
    ...
    // System.out.println(resultString);
    /*
     * This is what is produced Axis #0: {} Axis #1: Axis #2: {[D2].[x]}
     * {[D2].[y]} {[D2].[z]}
     */
  ...
  public void testD2() throws Exception {
    ...
    String mdx = "select " + " NON EMPTY filter({[D1].[a],[D1].[b],[D1].[c]}, "
      + "    [Measures].[Value] > 0) " + " ON COLUMNS, "
      + " {[D2].[x],[D2].[y],[D2].[z]} " + " ON ROWS " + "from FT2";
  ...
  /**
   * This ought to give the same result as the above testD2() method. In this
   * case, the FT2Extra cube has a default measure with no data (null) for all
   * members. This default measure is used in the evaluation even though there
   * is an implicit use of the measure [Measures].[Value].
   * 
   * @throws Exception
   */
  public void _testNullDefaultMeasure() throws Exception {
    ...
    String mdx = "select " + " NON EMPTY filter({[D1].[a],[D1].[b],[D1].[c]}, "
      + "    [Measures].[Value] > 0) " + " ON COLUMNS, "
      + " {[D2].[x],[D2].[y],[D2].[z]} " + " ON ROWS " + "from FT2Extra";
