/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/sql/SqlQueryTest.java#34 $
 ...
 * @author Thiyagu
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/sql/SqlQueryTest.java#34
 *          $
 * @since 06-Jun-2007
 */
public class SqlQueryTest extends BatchTestCase {
  ...
  public void testToStringForSingleGroupingSetSql() {
    ...
    for (boolean b : new boolean[] { false, true }) {
      ...
      if (!b) {
        ...
        expected = "select" + lineSep + "    c1 as \"c0\"," + lineSep
          + "    c2 as \"c1\"," + lineSep + "    grouping(gf0) as \"g0\"" + lineSep
          + "from" + lineSep + "    \"s\".\"t1\" =as= \"t1alias\"" + lineSep
          + "where" + lineSep + "    a=b" + lineSep + "group by grouping sets ("
          + lineSep + "    (gs1, gs2, gs3))";
      }
      assertEquals(dialectize(dialect.getDatabaseProduct(), expected), dialectize(
        sqlQuery.getDialect().getDatabaseProduct(), sqlQuery.toString()));
    }
  }
  ...
  public void testToStringForForcedIndexHint() {
    ...
    String unformattedMysql = "select c1 as `c0`, c2 as `c1` "
      + "from `s`.`t1` as `t1alias`" + " FORCE INDEX (myIndex)" + " where a=b";
    String formattedMysql = "select\n" + "    c1 as `c0`,\n" + "    c2 as `c1`\n"
      + "from\n" + "    `s`.`t1` as `t1alias` FORCE INDEX (myIndex)\n" + "where\n"
      + "    a=b";

    SqlPattern[] unformattedSqlPatterns = { new SqlPattern(
      Dialect.DatabaseProduct.MYSQL, unformattedMysql, null) };
    SqlPattern[] formattedSqlPatterns = { new SqlPattern(
      Dialect.DatabaseProduct.MYSQL, formattedMysql, null) };
  ...
  private void assertSqlQueryToStringMatches(SqlQuery query, SqlPattern[] patterns) {
    ...
    for (SqlPattern sqlPattern : patterns) {
      ...
      assertEquals(dialectize(dialect.getDatabaseProduct(), trigger), dialectize(
        query.getDialect().getDatabaseProduct(), query.toString()));
    }
  ...
  public void testTableNameIsIncludedWithParentChildQuery() {
    String sql = "select `employee`.`employee_id` as `c0`, "
      + "`employee`.`full_name` as `c1`, " + "`employee`.`marital_status` as `c2`, "
      + "`employee`.`position_title` as `c3`, " + "`employee`.`gender` as `c4`, "
      + "`employee`.`salary` as `c5`, " + "`employee`.`education_level` as `c6`, "
      + "`employee`.`management_role` as `c7` " + "from `employee` as `employee` "
    ...
    SqlPattern[] sqlPatterns = { new SqlPattern(Dialect.DatabaseProduct.ACCESS, sql,
      sql) };
  ...
  public void testPredicatesAreNotOptimizedWhenPropertyIsFalse() {
    ...
    String accessSql = "select `time_by_day`.`the_year` as `c0`, "
      ...
      + "= 1997 and `time_by_day`.`quarter` in " + "('Q1', 'Q2', 'Q3') group by "
      + "`time_by_day`.`the_year`, `time_by_day`.`quarter`";
  ...
  private void assertSqlEqualsOptimzePredicates(boolean optimizePredicatesValue,
    String inputMdx, SqlPattern[] sqlPatterns) {
    boolean intialValueOptimize = prop.OptimizePredicates.get();
  ...
  /**
   * Verifies that the correct SQL string is generated for literals of SQL type
   * "double".
   * 
   * <p>
   * Mondrian only generates SQL DOUBLE values in a special format for LucidDB;
   * therefore, this test is a no-op on other databases.
   */
  public void testDoubleInList() {
    ...
    // are both incorrect.
    String loadSqlLucidDB = "select cast(cast(\"salary\" as double)*cast(1000.0 as double)/cast(3.1234567890123456 as double) as double) as \"c0\", "
    ...
    SqlPattern[] patterns = { new SqlPattern(Dialect.DatabaseProduct.LUCIDDB,
      loadSqlLucidDB, loadSqlLucidDB) };
  ...
  /**
   * Testcase for <a href="http://jira.pentaho.com/browse/MONDRIAN-457">bug
   * MONDRIAN-457, "Strange SQL condition appears when executing query"</a>. The
   * fix implemented MatchType.EXACT_SCHEMA, which only queries known schema
   * objects. This prevents SQL such as
   * "UPPER(`store`.`store_country`) = UPPER('Time.Weekly')" from being
   * generated.
   */
  public void testInvalidSqlMemberLookup() {
    String sqlMySql = "select `store`.`store_type` as `c0` from `store` as `store` "
    ...
    String sqlOracle = "select \"store\".\"store_type\" as \"c0\" from \"store\" \"store\" "
      + "where UPPER(\"store\".\"store_type\") = UPPER('Time.Weekly') "
      + "group by \"store\".\"store_type\" "
    ...
    SqlPattern[] patterns = {
      ...
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, sqlOracle, sqlOracle), };

    assertNoQuerySql(
      "select {[Time.Weekly].[All Time.Weeklys]} ON COLUMNS from [Sales]", patterns);
  }

  /**
   * This test makes sure that a level which specifies an approxRowCount
   * property prevents Mondrian from executing a count() sql query. It was
   * discovered in bug MONDRIAN-711 that the aggregate tables predicates
   * optimization code was not considering the approxRowCount property. It is
   * fixed and this test will ensure it won't happen again.
   */
  public void testApproxRowCountOverridesCount() {
    final String cubeSchema = "<Cube name=\"ApproxTest\"> \n"
      + "  <Table name=\"sales_fact_1997\"/> \n"
    ...
    final String mdxQuery = "SELECT {[Gender].[Gender].Members} ON ROWS, {[Measures].[Unit Sales]} ON COLUMNS FROM [ApproxTest]";

    final String forbiddenSqlOracle = "select count(distinct \"customer\".\"gender\") as \"c0\" from \"customer\" \"customer\"";

    final String forbiddenSqlMysql = "select count(distinct `customer`.`gender`) as `c0` from `customer` `customer`;";

    SqlPattern[] patterns = {
      new SqlPattern(Dialect.DatabaseProduct.ORACLE, forbiddenSqlOracle, null),
      new SqlPattern(Dialect.DatabaseProduct.MYSQL, forbiddenSqlMysql, null) };

    final TestContext testContext = TestContext.instance().create(null, cubeSchema,
      null, null, null, null);

    assertQuerySqlOrNot(testContext, mdxQuery, patterns, true, true, true);
  }
}
