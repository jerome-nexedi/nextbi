/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/MemberCacheControlTest.java#16 $
 ...
 * 
 * <p>
 * The purpose of the cache control API is to clear the cache so that changes
 * made to the DBMS can be seen. However, it is difficult to write tests that
 * modify the database. So these tests just check that the relevant caches have
 * been cleared. It is assumed that the updated values will be loaded next time
 * mondrian goes to the database.
 * 
 * @author mberkowitz
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/MemberCacheControlTest
 *          .java#16 $
 * @since Jan 2008
 */
public class MemberCacheControlTest extends FoodMartTestCase {
   ...
   *          Cube name
   * @param names
   *          the full-qualified Member name
   * @return the Member
   * @throws MondrianException
   *           when not found.
   */
  static protected RolapMember findMember(TestContext tc, String cubeName,
    String... names) {
  ...
  private static PrintWriter printMemberProperties(PrintWriter pw, Member member) {
    ...
    for (Property p : member.getLevel().getProperties()) {
      ...
      if (value == null) {
        ...
        value = number.equals(number.intValue()) ? number.intValue() : Math
          .round(number.floatValue());
      }
      pw.print("  [");
      pw.print(name);
      pw.print("]=[");
  ...
  private static PrintWriter printRowMemberProperties(PrintWriter pw, Result result) {
    return printMemberProperties(pw,
      result.getAxes()[AxisOrdinal.StandardAxisOrdinal.ROWS.logicalOrdinal()]);
  }
  ...
  private CacheControl.MemberSet createInterestingMemberSet(TestContext tc,
    CacheControl cc) {
    return cc.createUnionSet(
    // all stores in OR
      cc.createMemberSet(findMember(tc, "Sales", "Retail", "OR"), true),
      // all stores in Hidalgo, Zacatecas
      cc.createMemberSet(findMember(tc, "Sales", "Retail", "Zacatecas", "Hidalgo"),
        true),
      // a single store
      cc.createMemberSet(findMember(tc, "Sales", "Retail", "CA", "Alameda", "HQ"),
        false),
      // a range of stores
      cc.createMemberSet(true, findMember(tc, "Sales", "Retail", "WA", "Bremerton"),
        true, findMember(tc, "Sales", "Retail", "Yucatan", "Merida"), false),
      // all stores in a range of states
      cc.createMemberSet(true, findMember(tc, "Sales", "Retail", "DF"), true,
        findMember(tc, "Sales", "Retail", "Jalisco"), true));
  }
  ...
  public void testFilter() {
    ...
    final CacheControl.MemberSet filteredMemberSet = cc.filter(orMember.getLevel(),
      memberSet);
  ...
  public void testMemberOpsFailIfCacheEnabled() {
    propSaver.set(MondrianProperties.instance().EnableRolapCubeMemberCache, true);
    final TestContext tc = getTestContext();
    final Connection conn = tc.getConnection();
    final CacheControl cc = conn.getCacheControl(null);
    final CacheControl.MemberEditCommand command = cc
      .createDeleteCommand(findMember(tc, "Sales", "Retail", "OR"));
    try {
      cc.execute(command);
      fail("expected exception");
    } catch (IllegalArgumentException e) {
      assertEquals("Member cache control operations are not allowed unless "
        + "property mondrian.rolap.EnableRolapCubeMemberCache is " + "false", e
        .getMessage());
    }
  }
  ...
  public void testSetPropertyCommandOnLeafMember() {
    ...
    dr.assertEquals("props before", "${props before}",
    ...
    // Change properties
    Member m = findMember(tc, "Sales", "Store", "USA", "CA", "San Francisco",
      "Store 14");
    cc.execute(cc.createSetPropertyCommand(m, "Store Manager", "Higgins"));
    cc.execute(cc.createCompoundCommand(Arrays
      .asList(cc.createSetPropertyCommand(m, "Street address", "770 Mission St"), cc
        .createSetPropertyCommand(m, "Store Sqft", 6000), cc
        .createSetPropertyCommand(m, "Has coffee bar", "false"))));

    // Repeat same query; verify properties are changed.
    // Changing properties does not affect measures, so results unchanged.
    r = conn.execute(q);
    dr.assertEquals("props after", "${props after}",
      getRowMemberPropertiesAsString(r));
    assertEquals(resultString, TestContext.toString(r));
  }
  ...
  public void testSetPropertyCommandOnNonLeafMember() {
    ...
    final Map<String, Object> propertyValues = createMap(Arrays.asList(
      "Has coffee bar", "Store Sqft"), Arrays.asList((Object) "true", 123));
    ...
    try {
      ...
      assertEquals("all members in set must belong to same level", e.getMessage());
    }

    // after we filter set to just members of store level we're ok
    final Member hqMember = findMember(tc, "Sales", "Retail", "CA", "Alameda", "HQ");
    final CacheControl.MemberSet filteredMemberSet = cc.filter(hqMember.getLevel(),
      memberSet);
  ...
  public void testAddCommand() {
    ...
    final Connection conn = tc.getConnection();
    final CacheControl cc = conn.getCacheControl(null);
    final RolapCubeMember caCubeMember = (RolapCubeMember) findMember(tc, "Sales",
      "Retail", "CA");
    ...
    final RolapMember berkeleyMember = (RolapMember) hierarchy.createMember(
      caMember, caMember.getLevel().getChildLevel(), "Berkeley", null);
    final RolapBaseCubeMeasure unitSalesCubeMember = (RolapBaseCubeMeasure) findMember(
      tc, "Sales", "Measures", "Unit Sales");
    final RolapCubeMember yearCubeMember = (RolapCubeMember) findMember(tc, "Sales",
      "Time", "Year", "1997");
    final Member[] cacheRegionMembers = new Member[] { unitSalesCubeMember,
      caCubeMember, yearCubeMember };

    tc.assertQueryReturns(
      "select {[Retail].[City].Members} on columns from [Sales]", "Axis #0:\n"
        + "{}\n" + "Axis #1:\n" + "{[Retail].[BC].[Vancouver]}\n"
        + "{[Retail].[BC].[Victoria]}\n" + "{[Retail].[CA].[Alameda]}\n"
    ...
    final MemberCache memberCache = ((SmartMemberReader) memberReader)
      .getMemberCache();
    ...
    assertEquals(Double.valueOf("74748"), AggregationManager.instance()
      .getCellFromCache(AggregationManager.makeRequest(cacheRegionMembers)));

    // Now tell the cache that [CA].[Berkeley] is new
    final CacheControl.MemberEditCommand command = cc
      .createAddCommand(berkeleyMember);
    cc.execute(command);

    // test that cells have been removed
    assertNull(AggregationManager.instance().getCellFromCache(
    ...
    List<RolapMember> rootChildren = memberCache.getChildrenFromCache(rootMember,
      null);
  ...
  public void testDeleteCommand() {
    ...
    final Connection conn = tc.getConnection();
    final CacheControl cc = conn.getCacheControl(null);
    final RolapCubeMember sfCubeMember = (RolapCubeMember) findMember(tc, "Sales",
      "Retail", "CA", "San Francisco");
    ...
    assertEquals(Double.valueOf("2117"), AggregationManager.instance()
      .getCellFromCache(AggregationManager.makeRequest(cacheRegionMembers)));

    // Now tell the cache that [CA].[San Francisco] has been removed.
    final CacheControl.MemberEditCommand command = cc
      .createDeleteCommand(sfCubeMember);
    cc.execute(command);
  ...
  public void testMoveCommand() {
    ...
    final MemberCache memberCache = ((SmartMemberReader) memberReader)
      .getMemberCache();
    final RolapMember alamedaMember = (RolapMember) hierarchy.createMember(caMember,
      caMember.getLevel().getChildLevel(), "Alameda", null);
    final RolapMember sfMember = (RolapMember) hierarchy.createMember(caMember,
      caMember.getLevel().getChildLevel(), "San Francisco", null);
    final RolapMember storeMember = (RolapMember) hierarchy.createMember(sfMember,
      sfMember.getLevel().getChildLevel(), "Store 14", null);

    // test axis contents
    tc.assertAxisReturns("[Retail].[CA].Children", "[Retail].[CA].[Alameda]\n"
      + "[Retail].[CA].[Beverly Hills]\n" + "[Retail].[CA].[Los Angeles]\n"
      + "[Retail].[CA].[San Diego]\n" + "[Retail].[CA].[San Francisco]");
    tc.assertAxisReturns("[Retail].[CA].[Alameda].Children",
    ...
    List<RolapMember> alamedaChildren = memberCache.getChildrenFromCache(
      alamedaMember, null);
    ...
    final MemberEditCommand command = cc.createMoveCommand(storeMember,
      alamedaMember);
    cc.execute(command);
  ...
  public void testMoveFailBadLevel() {
    ...
    try {
      ...
      assertEquals("new parent belongs to different level than old", e.getCause()
        .getMessage());
    }
  ...
  public void testAddCommandNegative() {
    ...
    final RolapCubeMember alamedaCubeMember = (RolapCubeMember) findMember(tc,
      "Sales", "Retail", "CA", "Alameda");
    final RolapMember alamedaMember = alamedaCubeMember.member;
    final RolapMember caMember = alamedaMember.getParentMember();

    final RolapCubeMember empCubeMember = (RolapCubeMember) findMember(tc, "HR",
    ...
    try {
      ...
      assertEquals("add member not supported for parent-child hierarchy", e
        .getMessage());
    }
    ...
    try {
      ...
      assertEquals("move member not supported for parent-child hierarchy", e
        .getMessage());
    }
    ...
    try {
      ...
      assertEquals("delete member not supported for parent-child hierarchy", e
        .getMessage());
    }
    ...
    try {
      ...
      assertEquals("set properties not supported for parent-child hierarchy", e
        .getMessage());
    }

    try {
      command = cc.createSetPropertyCommand(cc.createUnionSet(cc.createMemberSet(
        alamedaMember, false), cc.createMemberSet(caMember, false)), Collections
