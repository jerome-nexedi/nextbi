/*
// $Id: //open/mondrian/testsrc/main/mondrian/rolap/VirtualCubeTest.java#54 $
 ...
 * @since Feb 14, 2003
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/VirtualCubeTest.java#54
 *          $
 */
public class VirtualCubeTest extends BatchTestCase {
  ...
  public void testVirtualCubeMeasureInvalidCubeName() {
    TestContext testContext = TestContext.instance().create(
      ...
      "<VirtualCube name=\"Sales vs Warehouse\">\n"
        ...
        + "name=\"[Measures].[Unit Sales]\"/>\n" + "</VirtualCube>", null, null,
      null);
    testContext.assertQueryThrows("select from [Sales vs Warehouse]",
  ...
  /**
   * Query a virtual cube that contains a non-conforming dimension that does not
  ...
  public void testNonDefaultAllMember() {
    ...
    testContext.assertQueryReturns("select {[Warehouse].defaultMember} on columns, "
   ...
   * 
   * @return test context with a cube where the default member in the Warehouse
   *         dimension is USA
   */
  ...
  private void assertVisibility(Result result, int ordinal, String expectedName,
   ...
   * 
   * <p>
   * Since evaluation of the format_string used to happen after the aggregate
   * cache was cleared, this used to fail, this should be solved with the
   * caching of the format string.
   * 
   * <p>
  ...
  public void testLostData() {
    assertQueryReturns("select {[Time].[Time].Members} on columns,\n"
  ...
  /**
   * Test a virtual cube where one of the dimensions contains an ordinalColumn
   * property
   */
  ...
  public void testNativeSetCaching() {
    ...
    if (MondrianProperties.instance().EnableNativeCrossJoin.get()) {
      ...
      derbyNecjSql1 = "select " + "\"store\".\"store_country\" " + "from "
        + "\"store\" as \"store\", " + "\"sales_fact_1997\" as \"sales_fact_1997\" "
        + "where " + "\"sales_fact_1997\".\"store_id\" = \"store\".\"store_id\" "
        + "group by \"store\".\"store_country\" " + "order by 1 ASC";

      derbyNecjSql2 = "select " + "\"store\".\"store_country\" " + "from "
        + "\"store\" as \"store\", "
        + "\"inventory_fact_1997\" as \"inventory_fact_1997\" " + "where "
        + "\"inventory_fact_1997\".\"store_id\" = \"store\".\"store_id\" "
        + "group by \"store\".\"store_country\" " + "order by 1 ASC";
    }

    SqlPattern[] patterns1 = { new SqlPattern(Dialect.DatabaseProduct.DERBY,
      derbyNecjSql1, derbyNecjSql1) };

    SqlPattern[] patterns2 = { new SqlPattern(Dialect.DatabaseProduct.DERBY,
      derbyNecjSql2, derbyNecjSql2) };
   ...
   * MONDRIAN-322, "cube.getStar() throws NullPointerException"</a>. Happens
   * when you aggregate distinct-count measures in a virtual cube.
   */
  public void testBugMondrian322() {
     ...
     * unrelated base cube. The reason is that the constraint contains an
     * AllLevel member. Even though semantically constraining Cells using an
     * non-existent dimension perhaps does not make sense; however, in the case
     * where the constraint contains AllLevel member, the constraint can be
     * considered "always true".
     * 
     * See the next test case for a constraint that does not contain AllLevel
     * member and hence cannot be satisfied. The cell should be empty.
     */
    testContext
      .assertQueryReturns(
  ...
  public void testVirtualCubeMeasureCaption() {
    ...
    Result result = testContext
      .executeQuery("select {[Measures].[Store Sqft]} ON COLUMNS,"
        + "{[HCB]} ON ROWS " + "from [VirtualTestStore]");
  ...
  /**
   * Tests that the logic to apply non empty context constraint in virtual cube
   * is correct. The joins shouldn't be cartesian product.
   */
  public void testNonEmptyCJConstraintOnVirtualCube() {
    ...
    String query = "with "
      ...
      + "nonemptycrossjoin([foo],[bar]) on rows " + "From [Warehouse and Sales] "
  ...
  public void testNonEmptyConstraintOnVirtualCubeWithCalcMeasure() {
    ...
    String query = "with "
      ...
      + "Select " + "{[Measures].[CalcMeasure]} on columns, "
      + "non empty([Product].[Product Family].Members) on rows "
      + "From [Warehouse and Sales] " + "where [bar]";
    ...
    String mysqlSQL = "select `product_class`.`product_family` as `c0` "
      ...
      + "group by `product_class`.`product_family` " + "order by 1 ASC";

    String derbySQL = "select \"product_class\".\"product_family\" "
      + "from \"product\" as \"product\", \"product_class\" as \"product_class\", \"sales_fact_1997\" as \"sales_fact_1997\", \"store\" as \"store\" "
  ...
  public void testBugMondrian902() {
    ...
    assertEquals("[[Promotions].[Bag Stuffers], [Marital Status].[M]]", result
      .getAxes()[1].getPositions().get(0).toString());
