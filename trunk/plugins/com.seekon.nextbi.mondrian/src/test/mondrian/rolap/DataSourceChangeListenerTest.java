 ...
 * @since Jan 05, 2007
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/rolap/DataSourceChangeListenerTest
 *          .java#25 $
 */
public class DataSourceChangeListenerTest extends FoodMartTestCase {
  ...
  /**
   * Tests whether the data source plugin is able to tell mondrian to read the
   * hierarchy and aggregates again.
   */
  public void testDataSourceChangeListenerPlugin() {
    ...
    final mondrian.olap.CacheControl cacheControl = testContext.getConnection()
      .getCacheControl(null);

    // Flush the entire cache.
    final Connection connection = testContext.getConnection();
    final mondrian.olap.Cube salesCube = connection.getSchema().lookupCube("Sales",
      true);
    final mondrian.olap.CacheControl.CellRegion measuresRegion = cacheControl
    ...
    try {
      ...
      // Run query again, to make sure only cache is used
      Result r4 = executeQuery("select {[Store].[All Stores].[USA].[CA].[San Francisco]} "
      ...
      // Run query again, to make sure only cache is used
      Result r5 = executeQuery("select {[Store].[All Stores].[USA].[CA].[San Francisco]} "
        + "on columns from [Sales]");
      Util.discard(r5);
      ...
      // Run query again, to make sure only cache is used
      Result r6 = executeQuery("select {[Store].[All Stores].[USA].[CA].[San Francisco]} "
        + "on columns from [Sales]");
      Util.discard(r6);
   ...
   * @param cycleCount
   *          Number of cycles each thread should perform
   */
  private void checkCacheFlushing(final int workerCount, final int cycleCount) {
    ...
    SmartMemberReader smrStore = getSmartMemberReader(testContext.getConnection(),
      "Store");
    MemberCacheHelper smrStoreCacheHelper = (MemberCacheHelper) smrStore
      .getMemberCache();
    SmartMemberReader smrProduct = getSmartMemberReader(testContext.getConnection(),
      "Product");
    MemberCacheHelper smrProductCacheHelper = (MemberCacheHelper) smrProduct
      .getMemberCache();
