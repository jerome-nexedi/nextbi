/*
// $Id: //open/mondrian/testsrc/main/mondrian/xmla/XmlaErrorTest.java#23 $
...
/**
 * Test of the XMLA Fault generation - errors occur/are-detected in in Mondrian
 * XMLA and a SOAP Fault is returned.
 * 
 * <p>
 * There is a set of tests dealing with Authorization and HTTP Header Expect and
 * Continue dialog. These are normally done at the webserver level and can be
 * removed here if desired. (I wrote them before I realized that Mondrian XMLA
 * would not handle any Authorization issues if it were in a webserver.)
 * 
 * @author Richard M. Emberson
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/xmla/XmlaErrorTest.java#23 $
 */
...
public class XmlaErrorTest extends XmlaBaseTestCase implements XmlaConstants {
    ...
    public boolean processHttpHeader(HttpServletRequest request,
      ...
      if (doAuthorization) {
        ...
        if ((values == null) || (!values.hasMoreElements())) {
          throw XmlaRequestCallback.Helper.authorizationException(new Exception(
        ...
        if (!values.hasMoreElements()) {
          throw XmlaRequestCallback.Helper.authorizationException(new Exception(
            "Authorization: too few header value"));
        }
        ...
        if (index == -1) {
          throw XmlaRequestCallback.Helper.authorizationException(new Exception(
            "Authorization: badly formed userPass in encoding: " + encoded));
        }
        ...
        if (!Util.equals(userid, XmlaErrorTest.user)) {
          throw XmlaRequestCallback.Helper.authorizationException(new Exception(
            "Authorization: bad userid: " + userid + " should be: "
              + XmlaErrorTest.user));
        }
        if (!Util.equals(password, XmlaErrorTest.password)) {
          throw XmlaRequestCallback.Helper.authorizationException(new Exception(
            "Authorization: bad password: " + password + " should be: "
              + XmlaErrorTest.password));
        }
      }

      String expect = request.getHeader(EXPECT);
      if ((expect != null) && expect.equalsIgnoreCase(EXPECT_100_CONTINUE)) {
        XmlaRequestCallback.Helper.generatedExpectResponse(request, response,
          context);
        return false;
      } else {
        return true;
  ...
  private static class Fault {
    ...
    Fault(String faultCode, String faultString, String faultActor, String errorNS,
    ...
    Fault(Node[] faultNodes) throws Exception {
      ...
      if (faultNodes.length > 3) {
        ...
        if (faultNodes.length != 1) {
          throw new Exception("SOAP Fault detail node has " + faultNodes.length
            + " children");
        }
        // error element
        node = faultNodes[0];
    ...
    void checkSame(Fault expectedFault) throws Exception {
      ...
      if (!Util.equals(this.faultString, expectedFault.faultString)) {
        notSame("faultstring", expectedFault.faultString, this.faultString);
      }
      if (!Util.equals(this.faultActor, expectedFault.faultActor)) {
        notSame("faultactor", expectedFault.faultActor, this.faultActor);
      }
      if (!Util.equals(this.errorNS, expectedFault.errorNS)) {
        throw new Exception("For error element namespace " + " Expected "
          + expectedFault.errorNS + " but Got " + this.errorNS);
      }
    ...
    private void notSame(String elementName, String expected, String got)
      throws Exception {
      throw new Exception("For element " + elementName + " expected [" + expected
        + "] but got [" + got + "]");
    }
  }
  ...
  public void testJunk() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      USM_DOM_PARSE_CODE), USM_DOM_PARSE_FAULT_FS, FAULT_ACTOR, MONDRIAN_NAMESPACE,
  ...
  public void testBadAction01() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_SOAP_BODY_CODE), HSB_BAD_SOAP_BODY_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_SOAP_BODY_CODE, null);
  ...
  public void testAuth01() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      CHH_AUTHORIZATION_CODE), CHH_AUTHORIZATION_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, CHH_AUTHORIZATION_CODE, null);
  ...
  public void testBadHeader01() throws Exception {
    ...
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(
      MUST_UNDERSTAND_FAULT_FC, HSH_MUST_UNDERSTAND_CODE),
      HSH_MUST_UNDERSTAND_FAULT_FS, FAULT_ACTOR, null, null, null);
  ...
  public void testBadBody04() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_REQUEST_TYPE_CODE), HSB_BAD_REQUEST_TYPE_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_REQUEST_TYPE_CODE, null);
  ...
  public void testBadBody05() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_RESTRICTIONS_CODE), HSB_BAD_RESTRICTIONS_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_RESTRICTIONS_CODE, null);
  ...
  public void testBadBody06() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_PROPERTIES_CODE), HSB_BAD_PROPERTIES_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_PROPERTIES_CODE, null);
  ...
  public void testBadBody07() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_COMMAND_CODE), HSB_BAD_COMMAND_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_COMMAND_CODE, null);
  ...
  public void testBadBody09() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_RESTRICTION_LIST_CODE), HSB_BAD_RESTRICTION_LIST_FAULT_FS,
      FAULT_ACTOR, MONDRIAN_NAMESPACE, HSB_BAD_RESTRICTION_LIST_CODE, null);
  ...
  public void testBadBody10() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_PROPERTIES_LIST_CODE), HSB_BAD_PROPERTIES_LIST_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_PROPERTIES_LIST_CODE, null);
  ...
  public void testBadBody12() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_BAD_STATEMENT_CODE), HSB_BAD_STATEMENT_FAULT_FS, FAULT_ACTOR,
      MONDRIAN_NAMESPACE, HSB_BAD_STATEMENT_CODE, null);
  ...
  public void testBadBody13() throws Exception {
    Fault expectedFault = new Fault(XmlaException.formatFaultCode(CLIENT_FAULT_FC,
      HSB_DRILL_THROUGH_FORMAT_CODE), HSB_DRILL_THROUGH_FORMAT_FAULT_FS,
      FAULT_ACTOR, MONDRIAN_NAMESPACE, HSB_DRILL_THROUGH_FORMAT_CODE, null);
  ...
  protected void doTest(MockHttpServletRequest req, Fault expectedFault)
    throws Exception {
  ...
  protected void doTest(Fault expectedFault) throws Exception {
    String requestText = fileToString("request");
    Servlet servlet = getServlet(getTestContext());
    // do SOAP-XMLA
