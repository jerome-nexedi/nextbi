/*
// $Id: //open/mondrian/testsrc/main/mondrian/xmla/XmlaCognosTest.java#13 $
 ...
 * @version $Id:
 *          //open/mondrian/testsrc/main/mondrian/xmla/XmlaCognosTest.java#13 $
 */
...
public class XmlaCognosTest extends XmlaBaseTestCase {
  ...
  protected String filter(String testCaseName, String filename, String content) {
    if ("testWithFilter".equals(testCaseName) && filename.equals("response")) {
      Dialect dialect = TestContext.instance().getDialect();
      switch (dialect.getDatabaseProduct()) {
      case DERBY:
        content = Util.replace(content, "<Value xsi:type=\"xsd:double\">",
  ...
  public void testWithFilter() throws Exception {
    if (getTestContext().getDialect().getDatabaseProduct() == Dialect.DatabaseProduct.ACCESS) {
  ...
  private void executeMDX() throws Exception {
    ...
    doTest(requestType, getDefaultRequestProperties(requestType), TestContext
      .instance());
  }
