...
public class AvailableResourcesController extends AbstractController {
  ...
  protected ModelAndView handleRequestInternal(HttpServletRequest request,
    ...
    for (final String libraryPath : libraryPaths) {
      ...
      final String libraryName = libraryPath.substring(BASE_PATH.length(),
        libraryPath.length() - 1);
      if (!libraries.containsKey(libraryName)) {
        libraries.put(libraryName, new TreeMap<String, Map<String, Resource[]>>());
      }
      final Map<String, Map<String, Resource[]>> versions = libraries
        .get(libraryName);
      ...
      for (final String versionPath : versionPaths) {
        ...
        final String libraryVersion = versionPath.substring(libraryPath.length(),
          versionPath.length() - 1);
        ...
        final Resource[] jsResources = applicationContext.getResources(versionPath
          + "**/*.js");
        resources.put("js", jsResources);

        final Resource[] cssResources = applicationContext.getResources(versionPath
          + "**/*.css");
