...
/**
 * <code>FormatHandler1_0</code> Defines <code>{@link StartHandler}</code>s and
 * <code>{@link EndHandler}</code>s to read format definitions in cube views
 * which are stored using version 1.0
 * 
 * @author Philipp Bouillon
 * @version $Id: FormatHandler1_0.java,v 1.3 2009/12/17 16:14:29 PhilippBouillon
 *          Exp $
 **/
class FormatHandler1_0 extends FormatHandler {
  ...
  protected void registerStartHandlers() {
    registerStartHandler(new IPaloStartHandler() {
      ...
      public void startElement(String uri, String localName, String qName,
        ...
        if (id != null) {
          ...
          format.setBackgroundColor(colorDescriptorFromString(attributes
            .getValue("backgroundColor")));
          format.setFontData(fontDescriptorFromString(attributes
            .getValue("fontData")));
          format.setForegroundColor(colorDescriptorFromString(attributes
            .getValue("fontColor")));
    ...
    registerStartHandler(new IPaloStartHandler() {
      ...
      public void startElement(String uri, String localName, String qName,
        ...
        BorderData border = createBorderData(position, width, style, color);
  ...
  private ColorDescriptor colorDescriptorFromString(String color) {
    ...
    try {
      return new ColorDescriptor(Integer.parseInt(rgb[0]), Integer.parseInt(rgb[1]),
  ...
  private BorderData createBorderData(String position, String width, String style,
    String color) {
    try {
      int _width = Integer.parseInt(width);
      int _style = Integer.parseInt(style);
      int _position = Integer.parseInt(position);
      return new BorderData(_width, _style, colorDescriptorFromString(color),
        _position);
    } catch (Exception e) {
  ...
  private void addTrafficLightData(String min, String max, String bgColor,
    String fgColor, String font) {
    try {
      format.addTrafficLightData(toDouble(min, Double.MIN_VALUE), toDouble(max,
        Double.MAX_VALUE), colorDescriptorFromString(bgColor),
        colorDescriptorFromString(fgColor), fontDescriptorFromString(font));
    } catch (Exception e) {
      /* ignore */
    }
  }
