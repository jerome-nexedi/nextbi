...
public class XMLADatabaseLoader extends DatabaseLoader
{
  ...
  public XMLADatabaseLoader(XMLAConnection paramXMLAConnection, XMLAClient paramXMLAClient)
  {
  ...
  private final void loadAllDatabaseIds()
  {
    ...
    try
    {
      ...
      Document localDocument = this.xmlaClient.getCatalogList(localXMLARestrictions, localXMLAProperties);
      ...
      for (int i = 0; i < localNodeList1.getLength(); ++i)
      {
        ...
        {
          if ((localNodeList2.item(j).getNodeType() != 1) || (!localNodeList2.item(j).getNodeName().equals("CATALOG_NAME")))
            continue;
  ...
  {
    XMLADatabaseInfo localXMLADatabaseInfo = BuilderRegistry.getInstance().getDatabaseInfoBuilder().getDatabaseInfo((XMLAConnection)this.paloConnection, this.xmlaClient, paramString);
 ...
 * Qualified Name:     com.tensegrity.palo.xmla.loader.XMLADatabaseLoader
