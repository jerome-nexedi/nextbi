...
public class VariableInfoBuilder
{
  ...
  public XMLAVariableInfo[] requestVariables(XMLAClient paramXMLAClient, XMLADatabaseInfo paramXMLADatabaseInfo, String paramString)
  {
    try
    {
      ...
      Document localDocument = this.xmlaClient.getSAPVariableList(localXMLARestrictions, localXMLAProperties);
  ...
  private XMLAVariableInfo[] storeVariables(NodeList paramNodeList)
  {
    ...
    while (i < j)
    {
      ...
      localXMLAVariableInfo.setHideConsolidations(localXMLAVariableInfo.getReferenceDimension().equals(localXMLAVariableInfo.getReferenceHierarchy()));
  ...
  public void requestVarElements(XMLAVariableInfo paramXMLAVariableInfo, XMLAConnection paramXMLAConnection, XMLADatabaseInfo paramXMLADatabaseInfo, XMLAClient paramXMLAClient)
  {
    try
    {
      XMLARestrictions localXMLARestrictions = new XMLARestrictions();
      ...
      localXMLAProperties.setCatalog(this.xmlaDatabase.getId());
      localXMLARestrictions.setCatalog(this.xmlaDatabase.getId());
      localXMLARestrictions.setHierarchyUniqueName(paramXMLAVariableInfo.getReferenceHierarchy());
  ...
  private void buildStructure(NodeList paramNodeList, XMLAVariableInfo paramXMLAVariableInfo, XMLAConnection paramXMLAConnection)
  {
    if ((paramNodeList == null) || (paramNodeList.getLength() == 0))
    {
      paramXMLAVariableInfo.setElementDimension(new XMLADimensionInfo(this.xmlaClient, "VariableDimension", "varDimensionID", this.xmlaDatabase, null, paramXMLAConnection));
      return;
    }
    XMLADimensionInfo localXMLADimensionInfo = new XMLADimensionInfo(this.xmlaClient, "VariableDimension", "varDimensionID", this.xmlaDatabase, null, paramXMLAConnection);
    XMLAHierarchyInfo localXMLAHierarchyInfo = new XMLAHierarchyInfo(localXMLADimensionInfo, "VariableDimension", "varDimensionID");
    localXMLADimensionInfo.clearMembersInternal();
    ElementLoader localElementLoader = paramXMLAConnection.getElementLoader(localXMLADimensionInfo);
    Object localObject;
    int j;
      ...
      for (j = 0; j < ((NodeList)localObject).getLength(); ++j)
      {
        ...
        else if (str4.equals("MEMBER_UNIQUE_NAME"))
        {
          ...
          if (localXMLAElementInfo1 != null)
            continue;
          localXMLAElementInfo1 = new XMLAElementInfo(localXMLAHierarchyInfo, localXMLADimensionInfo, this.xmlaClient, paramXMLAConnection);
          if (str1.length() != 0)
          ...
          localXMLAElementInfo1.setPosition(localXMLADimensionInfo.getMemberCountInternal());
        ...
        else
        {
          ...
          {
            str5 = XMLAClient.getTextFromDOMElement(((NodeList)localObject).item(j));
            try
            {
          ...
          else if (str4.equals("PARENT_UNIQUE_NAME"))
          {
            ...
            XMLAElementInfo localXMLAElementInfo2 = localXMLADimensionInfo.getMemberInternal(str5);
            if (localXMLAElementInfo2 == null)
            {
              localXMLAElementInfo2 = new XMLAElementInfo(localXMLAHierarchyInfo, localXMLADimensionInfo, this.xmlaClient, paramXMLAConnection);
          ...
          {
            if (!((NodeList)localObject).item(j).getNodeName().equals("MEMBER_CAPTION"))
              continue;
            str5 = XMLAClient.getTextFromDOMElement(((NodeList)localObject).item(j));
 ...
 * Qualified Name:     com.tensegrity.palo.xmla.builders.VariableInfoBuilder
