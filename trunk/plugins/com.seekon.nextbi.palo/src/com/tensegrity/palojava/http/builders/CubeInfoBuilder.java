...
/*    */{
  ...
  /*    */{
    /* 49 */if (response.length < 8) {
      /* 50 */throw new PaloException(
        "CubeInfoBuilder: not enough information to create CubeInfo!!");
      /*    */}
    /*    */
    /* 54 */String id = response[0];
    /* 55 */String[] dims = BuilderUtils.getIDs(response[3]);
    /* 56 */int type = Integer.parseInt(response[7]);
    /* 57 */CubeInfoImpl info = new CubeInfoImpl((DatabaseInfo) parent, id, type,
      dims);
 ...
 * Qualified Name: com.tensegrity.palojava.http.builders.CubeInfoBuilder JD-Core
 * Version: 0.5.4
 */