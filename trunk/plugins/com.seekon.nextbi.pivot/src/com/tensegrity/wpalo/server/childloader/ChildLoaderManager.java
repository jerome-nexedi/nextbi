...
public class ChildLoaderManager {
    ...
    try {
      ...
      for (ChildLoader cl : childLoaders) {
        if (cl.accepts(parent)) {
          XObject[] result = cl.loadChildren(parent, userSession);
          return result;
        }
      }
      return new XObject[0];
  ...
  public XObject[] loadChildren(String parentType, String viewId, String axisId,
    String parentId, UserSession userSession) throws PaloGwtCoreException {
    try {
      if (parentId == null || parentType == null) {
        return new XObject[0];
      }
      if (elNodeLoader.accepts(parentType)) {
        return elNodeLoader.loadChildren(parentType, viewId, axisId, parentId,
          userSession);
      }
      return new XObject[0];
    } catch (Throwable t) {
