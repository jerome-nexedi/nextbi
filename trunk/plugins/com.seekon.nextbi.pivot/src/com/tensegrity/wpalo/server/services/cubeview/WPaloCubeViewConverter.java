...
public class WPaloCubeViewConverter {
  ...
  public final View convertLegacyView(XView xView, String sessionId)
    ...
    try {
      if (!doCreateDefaultView(xView)) {
        try {
          try {
            ...
            ConnectionPoolManager.getInstance().disconnect(
              getAccount(xView.getAccountId()), sessionId,
              "WPaloCubeViewConverter.convertLegacyView");
            View v = convert(cube.getCubeView(xView.getId()), xView.getName(),
              sessionId);
            return v;
          } finally {
            ConnectionPoolManager.getInstance().disconnect(
              getAccount(xView.getAccountId()), sessionId,
              "WPaloCubeViewConverter.convertLegacyView2");
          }

        } catch (PaloIOException e) {
          throw new OperationFailedException("Could not find legacy view for '"
            + xView.getName() + "'!");
        }
      }
      return null;
      ...
      ConnectionPoolManager.getInstance().disconnect(
        getAccount(xView.getAccountId()), sessionId,
        "WPaloCubeViewConverter.convertLegacyView3");
    }
  }
  ...
  private final synchronized Connection getConnection(String accountId,
    String sessionId) {
    /*
     * AdministrationService adminService =
     * ServiceProvider.getAdministrationService(user); Account account =
     * adminService.getAccountById(accountId); return ((PaloAccount)
     * account).login();
     */
  ...
  private final View convert(org.palo.api.CubeView legacyView, String newViewName,
    String sessionId) throws OperationFailedException {
  ...
  public final View createDefaultView(XView xView, String sessionId)
    ...
    try {
      ...
      ConnectionPoolManager.getInstance().disconnect(
        getAccount(xView.getAccountId()), sessionId,
        "WPaloCubeViewConverter.createDefaultView");
      return createDefaultViewFor(cube, xView.getName(), xView.getAccountId(),
        sessionId, xView.getExternalId());
    } finally {
      ConnectionPoolManager.getInstance().disconnect(
        getAccount(xView.getAccountId()), sessionId,
        "WPaloCubeViewConverter.createDefaultView2");
    }
  }

  private final View createDefaultViewFor(Cube cube, String viewName,
    String accountId, String sessionId, String externalId)
    throws OperationFailedException {
    return CubeViewConverter.createDefaultView(viewName, cube, accountId, user,
