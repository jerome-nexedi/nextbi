 ...
 * 
 * @version $Id: EditViewPropertiesDialog.java,v 1.7 2010/04/12 11:13:36
 *          PhilippBouillon Exp $
 **/
public class EditViewPropertiesDialog extends Window {
  ...
  public EditViewPropertiesDialog(XView view) {
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloCubeViewServiceProvider.getInstance().getRoles(sessionId, view.getId(),
  ...
  private final void initOlapCombos() {
    connectionCombo
      .addSelectionChangedListener(new SelectionChangedListener<XObjectWrapper>() {
  ...
  private final synchronized void fillConnections() {
    ...
    WPaloAdminServiceProvider.getInstance().getConnection(sessionId, xView,
      new Callback<XConnection>() {
        ...
        public synchronized void onSuccess(XConnection con) {
          ...
          WPaloAdminServiceProvider.getInstance().listAccounts(sessionId, user,
            new Callback<XAccount[]>() {
              public synchronized void onSuccess(XAccount[] accounts) {
                ...
                // fillDatabases((XConnection)
                // connectionCombo.getValue().xObj);
                // }
              }
            });
        }
  ...
  private final synchronized void fillDatabases(XConnection con) {
    ...
    databaseStore.removeAll();
    WPaloAdminServiceProvider.getInstance().getDatabases(sessionId, con,
      new Callback<XDatabase[]>() {
  ...
  private final synchronized void fillCubes(XDatabase db) {
    ...
    WPaloAdminServiceProvider.getInstance().getCubes(sessionId, con, db,
      new Callback<XCube[]>() {
        public void onFailure(Throwable t) {
          t.printStackTrace();
          super.onFailure(t);
  ...
  private final FieldSet createVisibilitySet() {
    ...
    String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
      .getSessionId();
    WPaloAdminServiceProvider.getInstance().getUsersForConnection(sessionId,
      xView.getId(), new AsyncCallback<XUser[]>() {
  ...
  public String getAccountId() {
    if (connectionCombo.isEnabled() && databaseCombo.isEnabled()
      && cubeCombo.isEnabled() && connectionCombo.getValue() != null
      && connection2Account.get(connectionCombo.getValue().xObj.getId()) != null) {
  ...
  public String getDatabaseId() {
    if (connectionCombo.isEnabled() && databaseCombo.isEnabled()
      && cubeCombo.isEnabled() && databaseCombo.getValue() != null) {
  ...
  public String getCubeId() {
    if (connectionCombo.isEnabled() && databaseCombo.isEnabled()
      && cubeCombo.isEnabled() && cubeCombo.getValue() != null) {
