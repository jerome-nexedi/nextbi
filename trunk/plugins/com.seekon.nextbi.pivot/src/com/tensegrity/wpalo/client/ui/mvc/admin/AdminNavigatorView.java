 ...
 * 
 * @version $Id: AdminNavigatorView.java,v 1.35 2010/03/02 08:59:12
 *          PhilippBouillon Exp $
 */
public class AdminNavigatorView extends View {
  ...
  protected void handleEvent(AppEvent<?> event) {
    ...
    case WPaloEvent.SAVED_ROLE_ITEM:
      ...
      if (event.type == WPaloEvent.SAVED_USER_ITEM) {
        ...
        WPaloAdminServiceProvider.getInstance().hasRoles(sessionId, lastCreatedUser,
          new String[] { "VIEWER", "EDITOR" }, new AsyncCallback<Boolean[]>() {
  ...
  private final void initUI(XUser forUser) {
    ...
    treeLoader = new BaseTreeLoader<TreeNode>(new TreeLoaderProxy()) {
      ...
      protected void onLoadSuccess(TreeNode parent, List<TreeNode> children) {
  ...
  private final Tree createTree(TreeLoader<TreeNode> loader) {
    ...
    tree.addListener(Events.OnDoubleClick, new Listener<BaseEvent>() {
      public void handleEvent(BaseEvent be) {
        TreeNode node = (TreeNode) tree.getSelectedItem().getModel(); // single
        // selection
        if (node != null && node.getXObject() != null) {
          String type = node.getXObject().getType();
          int eventType = -1;
  ...
  private final void fill(ToolBar toolbar) {
    addUser = createToolItem(constants.addUser(), "icon-user-add",
      WPaloEvent.ADD_USER_ITEM);
    toolbar.add(addUser);
    addGroup = createToolItem(constants.addGroup(), "icon-group-add",
      WPaloEvent.ADD_GROUP_ITEM);
    toolbar.add(addGroup);
    addRole = createToolItem(constants.addRole(), "icon-role2-add",
      WPaloEvent.ADD_ROLE_ITEM);
    toolbar.add(addRole);
    ...
    del.addSelectionListener(new SelectionListener<ComponentEvent>() {
      ...
      public final void handleEvent(ComponentEvent be) {
        ...
        if (type.equals(XUser.TYPE)) {
          ...
          if (role != null
            && role.getName() != null
            && (role.getName().equals("ADMIN") || role.getName().equals("EDITOR")
              || role.getName().equals("VIEWER") || role.getName().equals("OWNER"))) {
  ...
  private final TextToolItem createToolItem(String text, String icon,
    final int eventType) {
  ...
  private final void doDelete(final TreeNode node, final String confirmMsg) {
    ...
    if (admin != null) {
      ...
      WPaloAdminServiceProvider.getInstance().mayDelete(wb.getUser().getSessionId(),
        xObj, new Callback<String[]>(null) {
          private final void reallyDoDelete() {
            WPaloAdminServiceProvider.getInstance().delete(
              wb.getUser().getSessionId(), xObj, new Callback<Void>(null) {
          ...
          public void onSuccess(String[] result) {
            if (result == null || result.length == 0) {
              ...
              if (result[0].equals("_NO_ACCOUNT_ERROR_")) {
                ...
                MessageBox.confirm(constants.proceedWithDelete(), message,
                  new Listener<WindowEvent>() {
