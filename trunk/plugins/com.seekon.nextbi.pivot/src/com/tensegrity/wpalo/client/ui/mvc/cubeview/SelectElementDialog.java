 ...
 * 
 * @version $Id: SelectElementDialog.java,v 1.13 2010/02/12 13:49:50
 *          PhilippBouillon Exp $
 **/
public class SelectElementDialog extends Menu {
  ...
  public SelectElementDialog(XViewModel xViewModel, XAxisHierarchy hierarchy,
...
class SelectElementMenuItem extends LoadListener {
  ...
  public SelectElementMenuItem(SelectElementDialog dialog,
    final XViewModel xViewModel, final XAxisHierarchy hierarchy) {
    this.dialog = dialog;
      ...
      public void loaderLoad(LoadEvent le) {
        ...
        if (SelectElementMenuItem.this.dialog.isShowSelection()) {
          String sessionId = ((Workbench) Registry.get(Workbench.ID)).getUser()
            .getSessionId();
          String elementId = hierarchy.getSelectedElement() == null ? "" : hierarchy
            .getSelectedElement().getId();
          WPaloCubeViewServiceProvider.getInstance().getElementPath(sessionId,
            hierarchy.getId(), hierarchy.getViewId(), hierarchy.getAxisId(),
            elementId, new AsyncCallback<String>() {
